    ...
    With mshBill
        ...
        Select Case .Col
            ...
            Case marBillCol.最高价
                ...
                If strKey <> "" Then
                    ...
                    If Val(strKey) >= 10 ^ 14 - 1 Then
                        ...
                        Exit Sub
                    End If
                    If Val(strKey) < Val(.TextMatrix(.Row, marBillCol.最低价)) Then
                ...
                Else
                    If .TextMatrix(.Row, marBillCol.最高价) = "" Then
                        MsgBox "最高价不能为空", vbInformation + vbOKOnly, gstrSysName
                        Cancel = True
                        .TxtSetFocus
                        Exit Sub
                    End If
            ...
            Case marBillCol.分段最高限价
                ...
                If strKey = "" Then
                    .Text = " "
                    .TextMatrix(.Row, marBillCol.分段最高限价) = " "
                End If
        End Select
    End With
End Sub
    ...
    With mshBill
            For intLop = 1 To .Rows - 1
                If .TextMatrix(intLop, marBillCol.最低价) <> "" Or .TextMatrix(intLop, marBillCol.最高价) <> "" Then           '先判有否数据
                    ...
                    If Val(.TextMatrix(intLop, marBillCol.最低价)) > Val(.TextMatrix(intLop, marBillCol.最高价)) Then
...
Private Function LoadCardInfor() As Boolean
    ...
    If Not rsTmp.EOF Then
        For intRow = 0 To cbo计算方法.ListCount - 1
            If cbo计算方法.ItemData(intRow) = Val(NVL(rsTmp!计算方法)) Then
                cbo计算方法.ListIndex = intRow
                Exit For
            End If
        Next
        txt限价.Text = Format(Val(NVL(rsTmp!限价)), mFMT.FM_成本价)
    End If
    ...
    With mshBill
        ...
        If Not rsTmp.EOF Then
            ...
            Do While Not rsTmp.EOF
                .TextMatrix(intRow, marBillCol.序号) = NVL(rsTmp!序号, 0)
                .TextMatrix(intRow, marBillCol.最低价) = Format(Val(NVL(rsTmp!最低价)), mFMT.FM_成本价)
                .TextMatrix(intRow, marBillCol.最高价) = Format(Val(NVL(rsTmp!最高价)), mFMT.FM_成本价)
                .TextMatrix(intRow, marBillCol.加成率) = Format(Val(NVL(rsTmp!加成率)), GFM_VBJCL)
                .TextMatrix(intRow, marBillCol.分段最高限价) = Format(Val(NVL(rsTmp!限价)), mFMT.FM_成本价)
                .TextMatrix(intRow, marBillCol.说明) = NVL(rsTmp!说明)
                .Rows = .Rows + 1
                intRow = intRow + 1
                rsTmp.MoveNext
            Loop
...
Private Sub SetCtlEnable()
    ...
    blnSave = Trim(mshBill.TextMatrix(1, marBillCol.最低价)) <> "" Or Trim(mshBill.TextMatrix(1, marBillCol.最高价)) <> "" Or (Trim(mshBill.TextMatrix(1, marBillCol.最低价)) = "" And Trim(mshBill.TextMatrix(1, marBillCol.最高价)) = "" And Trim(mshBill.TextMatrix(1, marBillCol.加成率)) = "" And Trim(mshBill.TextMatrix(1, marBillCol.分段最高限价)) = "" And Trim(mshBill.TextMatrix(1, marBillCol.说明)) = "")
