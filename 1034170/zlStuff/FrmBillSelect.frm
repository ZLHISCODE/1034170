...
Private Sub Form_Load()
    ...
    Select Case mint计划类型
        Case 1
            mstrStart = Format(GetFirstDate(FirstDayOfMonth, zlDatabase.Currentdate), "yyyy-mm-dd")
        Case 2
            mstrStart = Format(GetFirstDate(FirstDayOfQuarter, zlDatabase.Currentdate), "yyyy-mm-dd")
        Case 3
            mstrStart = Format(GetFirstDate(FirstDayOfyear, zlDatabase.Currentdate), "yyyy-mm-dd")
        Case 4
            mstrStart = Format(GetFirstDate(FirstDayOfWeek, sys.Currentdate), "yyyy-mm-dd")
    End Select
    mblnFirst = True
    Me.dtp开始日期 = mstrStart
    Me.dtp开始日期.MaxDate = zlDatabase.Currentdate
    mstrEnd = Format(zlDatabase.Currentdate, "yyyy-MM-dd")
    Me.dtp结束日期 = zlDatabase.Currentdate
    Me.dtp结束日期.MaxDate = zlDatabase.Currentdate
    mblnBootUp = True
    Call SetDetal
    Call GetList
    ...
    With rsDetail
        ...
        If Not .EOF Then
            For lngLop = 1 To .RecordCount
                mshDetail.TextMatrix(lngLop, 0) = NVL(!材料信息)
                mshDetail.TextMatrix(lngLop, 1) = NVL(!规格)
                mshDetail.TextMatrix(lngLop, 2) = NVL(!产地)
                mshDetail.TextMatrix(lngLop, 3) = NVL(!单位)
                mshDetail.TextMatrix(lngLop, 4) = NVL(!请购数量)
                mshDetail.TextMatrix(lngLop, 5) = NVL(!审批数量)
                mshDetail.TextMatrix(lngLop, 6) = NVL(!单价)
                mshDetail.TextMatrix(lngLop, 7) = NVL(!金额)
