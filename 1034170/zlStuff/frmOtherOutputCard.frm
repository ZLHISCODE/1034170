...
Private Function SaveCheck() As Boolean
    ...
    '检查库存
    str材料 = CheckNumStock(mshBill, cboStock.ItemData(cboStock.ListIndex), 0, mconIntCol批次, mconIntCol数量, mconIntCol比例系数, 1, 1, g_小数位数.obj_最大小数.数量小数)
    If str材料 <> "" Then
        If mint库存检查 = 1 Then '不足提醒
            If MsgBox(str材料 & "卫材库存不足，是否继续？", vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                Exit Function
            End If
...
'                dbl成本金额 = Round(Val(.TextMatrix(intRow, mconIntCol采购金额)), g_小数位数.obj_最大小数.金额小数)
                dbl成本金额 = Val(Format(Val(Format(dbl成本价 * Val(.TextMatrix(intRow, mconIntCol比例系数)), mFMT.FM_成本价)) * Val(.TextMatrix(intRow, mconIntCol数量)), mFMT.FM_金额))
                
'                dbl零售价 = Round(Val(.TextMatrix(intRow, mconIntCol售价)) / Val(.TextMatrix(intRow, mconIntCol比例系数)), g_小数位数.obj_最大小数.零售价小数)
                dbl零售价 = Get售价(Split(.TextMatrix(intRow, mconIntCol指导差价率), "||")(1) = 1, lng材料ID, lng库房ID, lng批次)
                
                str灭菌效期 = IIf(.TextMatrix(intRow, mconIntCol灭菌失效期) = "", "", .TextMatrix(intRow, mconIntCol灭菌失效期))
                
'                dbl零售金额 = Round(Val(.TextMatrix(intRow, mconIntCol售价金额)), g_小数位数.obj_最大小数.金额小数)
                dbl零售金额 = Val(Format(Val(Format(dbl零售价 * Val(.TextMatrix(intRow, mconIntCol比例系数)), mFMT.FM_零售价)) * Val(.TextMatrix(intRow, mconIntCol数量)), mFMT.FM_金额))
