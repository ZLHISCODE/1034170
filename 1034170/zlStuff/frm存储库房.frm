...
Private Function SelectStuff(ByVal strSeach As String) As Boolean
    ...
    Else
        ...
        ElseIf zlCommFun.IsCharAlpha(strSeach) Then         '输入全是字母时只匹配简码
            If Mid(gSystem_Para.Para_输入方式, 2, 1) = "1" Then strWhere = " And (N.简码 Like Upper([1]) And N.码类=[2]) "
        ElseIf zlCommFun.IsCharChinese(strSeach) Then
...
Private Sub txtStuff_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode <> vbKeyReturn Then Exit Sub
    If txtStuff.Tag <> "" Then zlCommFun.PressKey vbKeyTab: Exit Sub
    If txtStuff.Text = "" Then zlCommFun.PressKey vbKeyTab: Exit Sub
...
'    If mblnFirst Then
    ...
    '将相应数据组织后装入单据控件
    gstrSQL = "" & _
        "   Select /*+ Rule*/ A.收费细目ID,A.开单科室ID,A.执行科室ID,B.名称 " & _
        "   From 收费执行科室 A,部门表 B,Table(Cast(f_Num2List([2]) As zlTools.t_NumList)) C " & _
        "   Where A.开单科室ID=B.ID(+) And A.收费细目ID=[1] And A.执行科室ID=C.Column_Value" & _
        "   Order by A.执行科室ID"
    ...
    If rsTemp.RecordCount = 0 And mlng材料ID <> 0 Then
        ...
        gstrSQL = "" & _
            ...
            " From 收费执行科室 A, 部门表 B, (Select 材料id From 材料特性 Where 诊疗id = [1] And Rownum < 2) C," & _
                " Table(Cast(f_Num2list([2]) As Zltools.t_Numlist)) D" & _
           " Where a.开单科室id = b.Id(+) And a.收费细目id = c.材料id And d.Column_Value = a.执行科室id" & _
            " Order By a.执行科室id"
        ...
        If rsTemp.RecordCount = 0 And mlng材料ID <> 0 Then
            gstrSQL = "" & _
                "   Select /*+ Rule*/ A.收费细目ID,A.开单科室ID,A.执行科室ID,B.名称 " & _
                "   From 收费执行科室 A,部门表 B,Table(Cast(f_Num2List([1]) As zlTools.t_NumList)) D, " & _
                "       ( Select A.ID From 收费项目目录 A,材料特性 B,收费执行科室 C " & _
                "         Where A.ID=B.材料ID And A.类别='4' And A.ID=C.收费细目ID And Rownum<2 ) C" & _
                "   Where A.开单科室ID=B.ID(+) And A.收费细目ID=C.ID And A.执行科室ID=D.Column_Value" & _
                "   Order by A.执行科室ID"
