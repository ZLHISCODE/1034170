    ...
    With mshList
        For intCol = 1 To .Cols - 1
            If .TextMatrix(0, intCol) = "库存差价" Or .TextMatrix(0, intCol) = "差价金额" Or .TextMatrix(0, intCol) = "成本金额" Or .TextMatrix(0, intCol) = "结算金额" Then
                .ColWidth(intCol) = IIf(mblnCostView = True, .ColWidth(intCol), 0)
            End If
        Next
    End With
    With mshDetail
        If mblnCostView = False Then '不查看成本价信息
            Select Case mlngMode
                Case 1712 '卫材外购
                    .ColWidth(.ColIndex("结算价")) = 0
                    .ColWidth(.ColIndex("结算金额")) = 0
                    .ColWidth(.ColIndex("差价")) = 0
                Case 1713 '自制入库
                    .ColWidth(.ColIndex("采购价")) = 0
                    .ColWidth(.ColIndex("采购金额")) = 0
                    .ColWidth(.ColIndex("差价")) = 0
                Case 1714, 1716, 1717, 1718 '其他入库 卫材移库 卫材领用 其他出库
...
Private Sub GetList(ByVal strFind As String)
    ...
    On Error GoTo ErrHandle
    If mblnBootUp Then SaveFlexState mshList, Me.Name '记录单据列表的宽度
    ...
    Select Case mlngMode
        ...
        Case 1716           '卫材移库管理
                ...
                If TabShow.Tab = 0 Then
                    ...
                    gstrSQL = "" & _
                        ...
                        "            a.记录状态, c.ID AS 移入库房ID, a.摘要 " & _
                        "   FROM 药品收发记录 a, 部门表 b ,部门表 c " & _
                        "   Where   a.库房id = b.ID AND a.对方部门id=c.id AND a.单据 = 19 AND  a.入出系数=-1 " & _
                                    strUserPart & strFind & _
                        "   GROUP BY a.no,c.名称,a.填制人,a.审核人,a.配药人,a.记录状态,a.摘要, c.ID " & _
                ...
                Else
                    ...
                    gstrSQL = "" & _
                        ...
                        "           a.记录状态, B.ID AS 移出库房ID, a.摘要 " & _
                        "   FROM 药品收发记录 a, 部门表 b ,部门表 c " & _
                        "   Where   a.库房id = b.ID AND a.对方部门id=c.id AND a.单据 = 19 AND  a.入出系数=-1 " & _
                                    strUserPart & strFind & _
                        "   GROUP BY a.no,b.名称,a.填制人,a.审核人,a.记录状态,a.摘要, B.ID " & _
        ...
        Case 1718          '卫材其他出库管理
            gstrSQL = "" & _
                ...
                "           a.填制人,TO_CHAR (min(a.填制日期), 'yyyy-mm-dd HH24:Mi:SS') AS 填制日期, a.审核人," & _
                "           TO_CHAR (min(a.审核日期), 'yyyy-mm-dd HH24:Mi:SS') AS 审核日期, a.记录状态, Max(费用id) As 费用id, a.摘要 " & _
...
Private Sub SetListColWidth()
    ...
    With mshList
        ...
        If TabShow.Tab = 0 Then
            intCol = GetCol(mshList, "移入库房ID")
        Else
            intCol = GetCol(mshList, "移出库房ID")
        End If
        If intCol >= 0 Then .ColWidth(intCol) = 0
        intCol = GetCol(mshList, "摘要")
        If intCol >= 0 Then .ColWidth(intCol) = 1000
        ...
        If mblnBootUp = False Then
            For intCol = 0 To .Cols - 1
                ...
                ElseIf intCol = GetCol(mshList, "财务标志") Then
                    .ColWidth(intCol) = 0
                ElseIf intCol = GetCol(mshList, "费用ID") Then
                    .ColWidth(intCol) = 0
                ...
                If mlngMode = 1716 Then
                    If TabShow.Tab = 0 Then
                        If intCol = GetCol(mshList, "移入库房ID") Then .ColWidth(intCol) = 0
                    Else
                        If intCol = GetCol(mshList, "移出库房ID") Then .ColWidth(intCol) = 0
                    End If
                End If
            ...
            Next
        Else
            RestoreFlexState mshList, Me.Name '还原单据列表的列宽度
        End If
    End With
End Sub
...
'            .ColKey(intCol) = .TextMatrix(0, intCol)
        ...
        If mblnCostView = False Then '不查看成本价信息
            Select Case mlngMode
                Case 1712 '卫材外购
                    .ColWidth(.ColIndex("结算价")) = 0
                    .ColData(.ColIndex("结算价")) = -1
                    .ColWidth(.ColIndex("结算金额")) = 0
                    .ColData(.ColIndex("结算金额")) = -1
                    .ColWidth(.ColIndex("差价")) = 0
                    .ColData(.ColIndex("差价")) = -1
                Case 1713 '自制入库
                    .ColWidth(.ColIndex("采购价")) = 0
                    .ColData(.ColIndex("采购价")) = -1
                    .ColWidth(.ColIndex("采购金额")) = 0
                    .ColData(.ColIndex("采购金额")) = -1
                    .ColWidth(.ColIndex("差价")) = 0
                    .ColData(.ColIndex("差价")) = -1
                Case 1714 '其他入库
                    .ColWidth(.ColIndex("成本价")) = 0
                    ...
                    .ColWidth(.ColIndex("成本金额")) = 0
                    .ColData(.ColIndex("成本金额")) = -1
                    .ColWidth(.ColIndex("差价")) = 0
                    .ColData(.ColIndex("差价")) = -1
                Case 1716, 1717, 1718 '卫材移库 卫材领用 其他出库
                    .ColWidth(.ColIndex("成本价")) = 0
                    .ColData(.ColIndex("成本价")) = -1
                    .ColWidth(.ColIndex("成本金额")) = 0
                    .ColData(.ColIndex("成本金额")) = -1
                    .ColWidth(.ColIndex("差价")) = 0
                    .ColData(.ColIndex("差价")) = -1
                    If blnCol成本 = True Then mshList.ColWidth(intCol) = 0
            End Select
    ...
    With mshList
        ...
        Select Case mlngMode
            ...
            Case 1716
                If TabShow.Tab = 0 Then
                    lng库房ID = cboStock.ItemData(cboStock.ListIndex)
                    lng对方部门id = Val(.TextMatrix(.Row, GetCol(mshList, "移入库房ID")))
                Else
                    lng库房ID = Val(.TextMatrix(.Row, GetCol(mshList, "移出库房ID")))
                    lng对方部门id = cboStock.ItemData(cboStock.ListIndex)
                End If
                
                If CheckPrint(.TextMatrix(.Row, 0), 19, lng库房ID, lng对方部门id, IIf(TabShow.Tab = 0, False, True)) = True Then
...
'                    End If
                ...
                If mlngMode = 1716 Then
                    If TabShow.Tab = 0 Then
                        mnuEditPrepare.Enabled = False
                        mnuEditSend.Enabled = (mshList.TextMatrix(mshList.Row, mshList.Cols - 4) = "")
