...
Public Function ShowMe(ByVal FrmParent As Object, ByVal 系统号 As Long, ByVal 票据号 As String, _
                       ByVal 记录状态 As Long, ByVal 单位系数 As Integer, ByVal 单据类型 As Integer, ByVal 单据名称 As String, ByVal 单据号 As String, Optional ByVal lng库房ID As Long, Optional ByVal lng对方部门ID As Long)
    lng系统号 = 系统号
    str票据号 = 票据号
    lng记录状态 = 记录状态
    int单位系数 = 单位系数
    ...
    If intPrintMode = 2 Then
        Select Case int单据类型
            ...
            Case 1714   '其他入库
                ...
                End If
            Case 1716   '移库
                If CheckPrint(str单据号, 19, mlng库房id, mlng对方部门ID) = False Then
                    Unload Me
                    Exit Sub
                End If
            Case 1717   '领用
