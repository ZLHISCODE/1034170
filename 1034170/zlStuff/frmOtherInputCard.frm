...
Private Sub CmdSave_Click()
    ...
    If mint编辑状态 = 3 Then        '审核
        ...
        For intLop = 1 To mshBill.Rows - 1
            If Trim(mshBill.TextMatrix(intLop, mshBill.ColIndex("卫材信息"))) <> "" Then
                '判断已失效卫材是否禁止入库
                If CheckLapse(Trim(mshBill.TextMatrix(intLop, mshBill.ColIndex("效期"))), True) = False Then
                    MsgBox "第" & intLop & "行卫材已经失效了，失效卫材不能保存！", vbInformation, gstrSysName
                    mshBill.SetFocus
                    mshBill.Row = intLop
                    mshBill.TopRow = intLop
                    mshBill.Col = mshBill.ColIndex("效期")
                    Exit Sub
                End If
            End If
        Next
...
'                    End If
                    ...
                    .TextMatrix(intRow, .ColIndex("采购价")) = Format(rsTemp!成本价, mFMT.FM_成本价)
...
'                    .TextMatrix(intRow, .ColIndex("售价")) = Format((Val(NVL(rsTemp!零售金额)) - Val(NVL(rsTemp!零售差价))) / Val(NVL(rsTemp!数量)), mFMT.FM_零售价)
                    .TextMatrix(intRow, .ColIndex("售价")) = Format(Val(NVL(rsTemp!零售价)) * Val(rsTemp!比例系数), mFMT.FM_零售价)
...
'                End If
                ...
                End If
        Case .ColIndex("效期")
            CheckLapse (.TextMatrix(.Row, .ColIndex("效期")))
        End Select
    End With
End Sub
        ...
        Select Case Col
          Case .ColIndex("灭菌日期")
           ...
           Case .ColIndex("生产日期"), .ColIndex("效期")
                ...
                If Trim(strKey) <> "" And Col = .ColIndex("生产日期") Then
        ...
        If .TextMatrix(1, .ColIndex("材料ID")) <> "" Then        '先判有否数据
            ...
            For intLop = 1 To .Rows - 1
                If Trim(.TextMatrix(intLop, .ColIndex("卫材信息"))) <> "" Then
                    '判断已失效卫材是否禁止入库
                    If CheckLapse(Trim(mshBill.TextMatrix(intLop, .ColIndex("效期"))), True) = False Then
                        MsgBox "第" & intLop & "行卫材已经失效了，失效卫材不能保存！", vbInformation, gstrSysName
                        mshBill.SetFocus
                        .Row = intLop
                        .TopRow = intLop
...
'
                    ...
                    For i = 1 To .Rows - 1
                        If i <> intLop Then
                            If .TextMatrix(intLop, .ColIndex("材料ID")) = .TextMatrix(i, .ColIndex("材料ID")) And Trim(.TextMatrix(intLop, .ColIndex("产地"))) = Trim(.TextMatrix(i, .ColIndex("产地"))) _
                                And Trim(.TextMatrix(intLop, .ColIndex("批号"))) = Trim(.TextMatrix(i, .ColIndex("批号"))) _
                                And Format(.TextMatrix(intLop, .ColIndex("效期")), "yyyy-MM-dd") = Format(.TextMatrix(i, .ColIndex("效期")), "yyyy-MM-dd") Then
                                MsgBox "第" & intLop & "行和第" & i & "行属于同一个卫材，且单据中的产地、批号、效期信息都相同，请检查！", vbInformation + vbOKOnly, gstrSysName
                                mshBill.SetFocus
                                .Row = intLop
                                .TopRow = intLop
                                .Col = .ColIndex("卫材信息")
...
'        For intRow = 1 To .Rows - 1
            If .TextMatrix(intRow, .ColIndex("材料ID")) <> "" Then
                ...
                If Val(Split(.TextMatrix(intRow, .ColIndex("原销期")), "||")(2)) = 0 Then
                    '如果是定价药品，则售价取原始价格保存
                    dbl零售价 = Get售价(False, lng材料ID, lng库房id, 0)
                End If
        ...
        For intRow = 1 To .Rows - 1
            If Val(.TextMatrix(intRow, .ColIndex("冲销数量"))) <> 0 Then
                ...
                If int库存检查 <> 0 Then
                    ...
                    If Check可用数量(lng库房id, Val(.TextMatrix(intRow, .ColIndex("材料ID"))), lng批次, dbl冲销数量, int库存检查, 0) = False Then .Row = intRow: .TopRow = intRow: Exit Function
                    If Check可用数量(lng库房id, Val(.TextMatrix(intRow, .ColIndex("材料ID"))), lng批次, dbl冲销数量, int库存检查, 1) = False Then .Row = intRow: .TopRow = intRow: Exit Function
                End If
            End If
        Next
...
Private Function SelDate(ByVal intCol As Integer) As Boolean
    ...
    With mshBill
        ...
        If intCol = .ColIndex("效期") Then CheckLapse (strDate): Call zl_VsGridGotFocus(mshBill)
    End With
