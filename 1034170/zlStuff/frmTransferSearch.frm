        ...
        Else
            ...
            Else
                ...
                ElseIf chkNoStrike.Value = 0 And chkYesStrike.Value = 1 Then
                    mstrFind = " and (A.记录状态=2 or mod(A.记录状态,3)=2) And ((A.填制日期 Between [2] And [3] and 审核日期 is null) " _
...
NO:
    ...
    Else
        ...
        If KeyCode = vbKeyReturn Then
            If Len(txt结束NO) < 8 And Len(txt结束NO) > 0 Then
                txt结束NO.Text = GetFullNO(txt结束NO.Text, IntBill, lng库房id)
            End If
            zlCommFun.PressKey (vbKeyTab)
        End If
    End If
...
NO:
    ...
    Else
        ...
        If KeyCode = vbKeyReturn Then
            If Len(txt开始No) < 8 And Len(txt开始No) > 0 Then
                txt开始No.Text = GetFullNO(txt开始No.Text, IntBill, lng库房id)
            End If
            zlCommFun.PressKey (vbKeyTab)
        End If
    End If
...
Private Function getDept(ByVal strKey As String) As Boolean
   ...
   '     blnSearch=是否显示行号,并可以输入行号定位
    ...
    Me.txtDept = NVL(rsTemp!编码) & "-" & NVL(rsTemp!名称)
    Me.txtDept.Tag = NVL(rsTemp!Id)
    getDept = True
