...
Private Sub ReFreshStuffData(ByVal lngDeptId As Long, strStartDate As String, strEndDate As String, lngUseId As Long)
   ...
   If lngDeptId = 0 Then
    ...
    Else
        Select Case mbytUint
        Case 0
            gstrSQL = ",Q.计算单位 as 单位,Nvl(m.成本价,0) As 上次采购价,decode(Q.是否变价,1" & _
            ...
            gstrSQL1 = " Group by M.材料ID,Q.编码,L.分类id,Q.名称,Q.规格,Q.是否变价,Q.产地,nvl(M.最大效期,0),M.库房分批,Q.计算单位,p.现价" & _
                       ",nvl(P.现价,0),nvl(M.换算系数,0),m.上次售价,m.指导零售价,Decode(To_Char(Q.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(Q.撤档时间,'yyyy-MM-dd')), g.名称, m.成本价 order by q.编码 "
        Case Else
            gstrSQL = ",M.包装单位 as 单位,Nvl(m.成本价 * Nvl(m.换算系数, 0),0) As 上次采购价,decode(Q.是否变价,1," & _
            ...
            gstrSQL1 = " Group by M.材料ID,Q.编码,L.分类id,Q.名称,Q.规格,Q.是否变价,Q.产地,nvl(M.最大效期,0),M.库房分批,M.包装单位,P.现价" & _
                       ",M.换算系数,m.上次售价,m.指导零售价,nvl(P.现价,0)*nvl(M.换算系数,0),Decode(To_Char(Q.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(Q.撤档时间,'yyyy-MM-dd')), g.名称, m.成本价 order by q.编码 "
        End Select
    End If
        ...
        If lng库房ID = 0 Or (int在用 = 1 And int分批 = 2) Or (int在用 = 0 And int分批 <> 0) Then
            ...
            If lng库房ID = 0 Then
                gstrSQL = "" & _
                    ...
                    " Group By D.名称"
            Else
               gstrSQL = "Select D.名称 as 库房,s.上次批号 as 批号,s.效期 失效期,s.上次产地 as 产地,Decode(sign(Add_Months(Sysdate," & mintMonths & ")-s.效期),-1,0,1) 报警," & _
...
Private Function ReFreshFilterData(ByVal lngDeptId As Long, strFind As String) As Boolean
    ...
    Else
       gstrSQL = "" & _
            ...
            "       ,Decode(M.库房分批,1,'是','否') as 库房分批 " & gstrSQL & _
            "   From 材料特性 M,收费项目目录 Q,诊疗项目目录 L ,收费价目 P,供应商 G, (Select Distinct 收费细目id, 执行科室id From 收费执行科室 Where 执行科室id=[1]) C,  " & _
            ...
            "   Where m.上次供应商id = g.Id(+) And P.收费细目Id=M.材料id and M.诊疗id=L.id and P.收费细目id=Q.id And m.材料id = c.收费细目id And (Q.站点=[6] or Q.站点 is null) " & _
