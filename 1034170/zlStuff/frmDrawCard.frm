        ...
        Case 2, 3, 4, 5, 6, 7
            ...
            With mshBill
                Do While Not rsTemp.EOF
                    ...
                    If mint编辑状态 = 7 Then '外购单生成领用单
                        .TextMatrix(intRow, mBillCol.C_采购价) = Format(Get成本价(rsTemp.Fields(0), cboStock.ItemData(cboStock.ListIndex), zlStr.NVL(rsTemp!批次, 0)) * rsTemp!比例系数, mFMT.FM_成本价)
                        .TextMatrix(intRow, mBillCol.C_采购金额) = Format(Val(.TextMatrix(intRow, mBillCol.C_采购价)) * rsTemp!实际数量, mFMT.FM_金额)
                        .TextMatrix(intRow, mBillCol.C_售价) = Format(Get售价(rsTemp!是否变价 = 1, rsTemp.Fields(0), cboStock.ItemData(cboStock.ListIndex), zlStr.NVL(rsTemp!批次, 0)) * rsTemp!比例系数, mFMT.FM_零售价)
                        .TextMatrix(intRow, mBillCol.C_售价金额) = Format(Val(.TextMatrix(intRow, mBillCol.C_售价)) * rsTemp!实际数量, mFMT.FM_金额)
                        .TextMatrix(intRow, mBillCol.C_差价) = Format(Val(.TextMatrix(intRow, mBillCol.C_售价金额) - Val(.TextMatrix(intRow, mBillCol.C_采购金额))), mFMT.FM_金额)
                    End If
...
'                dbl成本金额 = Round(Val(.TextMatrix(intRow, mBillCol.C_采购金额)), g_小数位数.obj_最大小数.金额小数)
                dbl成本金额 = Val(Format(Val(Format(dbl成本价 * Val(.TextMatrix(intRow, c_比例系数)), mFMT.FM_成本价)) * Val(.TextMatrix(intRow, C_实际数量)), mFMT.FM_金额))   '.TextMatrix(intRow, mconIntCol采购金额)
                
'                dbl零价 = Round(Val(.TextMatrix(intRow, mBillCol.C_售价)) / Val(.TextMatrix(intRow, mBillCol.C_比例系数)), g_小数位数.obj_最大小数.零售价小数)
                dbl零价 = Get售价(Split(.TextMatrix(intRow, C_指导差价率), "||")(1) = 1, lng材料ID, lng库房ID, lng批次)
                
'                dbl零售金额 = Round(Val(.TextMatrix(intRow, mBillCol.C_售价金额)), g_小数位数.obj_最大小数.金额小数)
                dbl零售金额 = Val(Format(Val(Format(dbl零价 * Val(.TextMatrix(intRow, c_比例系数)), mFMT.FM_零售价)) * Val(.TextMatrix(intRow, C_实际数量)), mFMT.FM_金额)) '.TextMatrix(intRow, mconIntCol售价金额)
