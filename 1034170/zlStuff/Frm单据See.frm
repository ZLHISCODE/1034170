...
Private Sub RefreshHeadData()
    ...
    Select Case byt单据
        Case 24, 25          '处方
            If byt单据 = 24 Then
                gstrSQL = " Select A.NO,B.名称 as 开单科室,G.姓名,G.性别,G.年龄,0 As 住院号,'' 床号,A.摘要,A.填制人,To_Char(A.填制日期,'yyyy-MM-dd') as 填制日期,A.审核人,To_Char(A.审核日期,'yyyy-MM-dd') as 审核日期,G.病人ID,0 主页ID,A.对方部门ID as 开单部门ID,G.操作员姓名 as 计费人,A.单据,A.ID,G.开单人 " & _
                          " From 药品收发记录 A,部门表 B,门诊费用记录 G " & _
                          " Where A.对方部门id=B.id(+) And A.单据=[2] And A.NO=G.NO(+) And A.费用ID=G.ID And A.NO=[1] And Rownum < 2 "
            Else
                gstrSQL = " Select A.NO,B.名称 as 开单科室,G.姓名,G.性别,G.年龄,C.住院号,G.床号,A.摘要,A.填制人,To_Char(A.填制日期,'yyyy-MM-dd') as 填制日期,A.审核人,To_Char(A.审核日期,'yyyy-MM-dd') as 审核日期,G.病人ID,G.主页ID,A.对方部门ID as 开单部门ID,G.操作员姓名 as 计费人,A.单据,A.ID,G.开单人 " & _
