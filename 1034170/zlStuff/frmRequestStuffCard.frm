...
'                dbl成本金额 = Round(Val(.TextMatrix(intRow, mBillCol.C_采购金额)), g_小数位数.obj_最大小数.金额小数)
                dbl成本金额 = Val(Format(Val(Format(dbl成本价 * Val(.TextMatrix(intRow, mBillCol.C_比例系数)), mFMT.FM_成本价)) * Val(.TextMatrix(intRow, mBillCol.C_实际数量)), mFMT.FM_金额))
...
Private Sub printbill()
    ...
    Dim strNo As String
    strNo = txtNO.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1722", mint记录状态, mintUnit, 1722, "卫材申领单", strNo, cboStock.ItemData(cboStock.ListIndex), mlngStockID
End Sub
        ...
        For intRow = 1 To .Rows - 1
            If Val(.TextMatrix(intRow, mBillCol.C_实际数量)) <> 0 Then
                ...
                If int库存检查 <> 0 Then
                    ...
                    '正常冲销和产生冲销单据都检查可用数量；正常冲销才检查实际数量
                    ...
                    If mint处理方式 = 0 Then If Check可用数量(lng库房ID, Val(.TextMatrix(intRow, 0)), lng批次, dbl冲销数量, int库存检查, 1) = False Then .Row = intRow: .MsfObj.TopRow = intRow: Exit Function
                End If
