...
Private Sub mnuEditItemMod_Click()
    ...
    Else
        lng分类id = Val(Mid(tvwClass.SelectedItem.Key, 2))
    End If
    If frmStuffSpec.ShowEditCard(Me, g修改, lng诊疗ID, lng分类id, CStr(lng材料ID), mstrPrivs, mbln启用) = False Then
        Exit Sub
    End If
...
Private Sub mnuEditStart_Click()
    ...
    Else
        With Me.vsStuff
            ...
            gstrSQL = "Select 1 From 材料特性 Where 库房分批 = 1 And 在用分批 = 0 and 材料id=[1]"
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "分批属性", lng材料ID)
            
            gstrSQL = "Select 1 From 药品库存 Where  药品id=[1] and 性质=1"
            Set rs是否有库存 = zlDatabase.OpenSQLRecord(gstrSQL, "是否有库存", lng材料ID)
            ...
            If rsTemp.RecordCount > 0 And rs是否有库存.RecordCount = 0 Then
                ...
                If rsTemp.RecordCount > 0 Then
                    MsgBox "当前有部门的工作性质同时设置了卫材库房和发料部门，启用该规格后请同时设置卫材库房和发料部门分批或不分批！", vbInformation, gstrSysName
                    mbln启用 = True
                    Call mnuEditItemMod_Click
                    mbln启用 = False
                End If
            End If
        End With
