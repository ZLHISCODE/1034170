...
Public Function CheckLapse(ByVal str效期 As String, Optional bln已失效卫材禁止入库 As Boolean = False) As Boolean
    Dim int已失效卫材禁止入库 As Integer
    int已失效卫材禁止入库 = Val(zldatabase.GetPara(309, glngSys, 0))
    '失效药品检查
    CheckLapse = True
    If Not IsDate(str效期) Then Exit Function
    If Format(str效期, "yyyy-MM-dd") <= Format(zldatabase.Currentdate, "yyyy-MM-dd") Then
        If bln已失效卫材禁止入库 = False Then MsgBox "该卫生材料已经失效了！", vbInformation, gstrSysName
        
        If int已失效卫材禁止入库 = 1 And bln已失效卫材禁止入库 = True Then
            CheckLapse = False
        End If
    End If
End Function

Public Function CheckpProduction(ByVal str生产日期 As String) As Boolean
    '检查生产日期是否比当前日期大
    CheckpProduction = True
    
    If Not IsDate(str生产日期) Then Exit Function
    
    If Format(str生产日期, "yyyy-MM-dd") > Format(zldatabase.Currentdate, "yyyy-MM-dd") Then
        CheckpProduction = False
    End If
        ...
        If bln发料部门 Then
            ...
            If lngModuleNO = 1716 Or lngModuleNO = 1722 Then    '移库管理、申领管理
                gstrSQL = "" & _
                    ...
                    "   where A.id=B.部门ID and a.id=d.id and b.工作性质  in ('卫材库','制剂室','发料部门') " & _
            ...
            Else
                gstrSQL = "" & _
                    ...
                    "          decode(To_Char(a.撤档时间, 'yyyy-mm-dd'),'3000-01-01','',To_Char(a.撤档时间, 'yyyy-mm-dd')) 撤档时间" & _
                    "   From 部门表 a,部门性质说明 B " & str材料流向 & _
                    "   where A.id=B.部门ID and a.id=d.id  and b.工作性质  in ('卫材库','制剂室','虚拟库房','发料部门') " & _
...
Public Function Get分段加成售价(ByVal dbl采购价 As Double, ByVal dbl比例系数 As Double, ByVal strFormCaption As String, ByRef sng售价 As Double) As Boolean
    ...
    gstrSQL = "Select 序号,最低价,最高价,1+nvl(加成率,0)/100 as 加成率,计算方法,限价 from 材料加成方案 order by 序号"
