...
Private Sub InitCardData(ByVal lng诊疗ID As Long)
    ...
    With rsTemp
        Do While Not .EOF
            If !性质 = 1 And !码类 = 1 Then Me.txt拼音.Text = NVL(!简码)
            If !性质 = 1 And !码类 = 2 Then Me.txt五笔.Text = NVL(!简码)
            If !性质 = 2 Then Me.txt英文.Text = NVL(!名称)
            .MoveNext
        Loop
    End With
    ...
    With rsTemp
        Do While Not .EOF
            ...
            Me.vsEditBill.TextMatrix(.AbsolutePosition, 1) = NVL(!名称)
            Me.vsEditBill.TextMatrix(.AbsolutePosition, 2) = NVL(!拼音)
            Me.vsEditBill.TextMatrix(.AbsolutePosition, 3) = NVL(!五笔)
            .MoveNext
        Loop
    End With
    Exit Sub
...
Private Function SaveData() As Boolean
    ...
    If mintEditType = g新增 Then
        lng诊疗ID = zldatabase.GetNextId("诊疗项目目录")
...
Private Function GetMaxCode() As String
    ...
    If intCodeType = 0 Or (intCodeType <> 0 And Len(strTemp) >= 7) Then
        '取最大编码
        gstrSQL = "Select Nvl(Max(编码), '000000000') As 编码" & vbNewLine & _
                        "From (Select 编码 From 诊疗项目目录 Where 类别 ='4' Order By Length(编码) Desc, 编码 Desc, 建档时间 Desc)" & vbNewLine & _
                        "Where Rownum = 1  "
        
        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption)
        With rsTemp
            str编码 = zlCommFun.IncStr(!编码)
            GetMaxCode = Right(String(mintCodeLength, "0") & str编码, Len(str编码))
        End With
    ...
    End If
        ...
        gstrSQL = "Select Nvl(Max(编码), '000000000') As 编码" & vbNewLine & _
                ...
                "       Where 类别 = '4' And 编码 Like [1] And Length(编码) >=[2] " & vbNewLine & _
                "       Order By Length(编码) Desc, 编码 Desc, 建档时间 Desc)" & vbNewLine & _
                "Where Rownum = 1"

        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, IIf(intCodeType = 1, "4", "") & strTemp & "%", Len("*" & strTemp & "**"))
                
    With rsTemp
        str编码 = rsTemp!编码
        str编码 = Mid(str编码, Len(strTemp) + IIf(intCodeType = 1, 2, 1))
        str编码 = zlCommFun.IncStr(str编码)
        GetMaxCode = IIf(intCodeType = 1, "4", "") & strTemp & str编码
    End With
...
Private Sub txt名称_Change()
    mblnChange = True
    '拼音和五笔
    Me.txt拼音.Text = zlGetSymbol(Me.txt名称.Text, 0, Me.txt拼音.MaxLength)
    Me.txt五笔.Text = zlGetSymbol(Me.txt名称.Text, 1, Me.txt五笔.MaxLength)
End Sub
...
Private Sub vsEditBill_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsEditBill
        ...
        Case .ColIndex("材料名称")
            ...
            .TextMatrix(Row, .ColIndex("拼音码")) = zlGetSymbol(strKey, 0, Me.txt拼音.MaxLength)
            .TextMatrix(Row, .ColIndex("五笔码")) = zlGetSymbol(strKey, 1, Me.txt五笔.MaxLength)
            If .Row = .Rows - 1 Then
                .Rows = .Rows + 1
                .Row = .Rows - 1
            End If
