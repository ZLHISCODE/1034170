...
Begin VB.Form frmStuffSpec 
   ...
   Begin TabDlg.SSTab stbSpec 
      ...
      Tab(1).Control(0)=   "cmd病案"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).Control(1)=   "fra分批核算"
      Tab(1).Control(2)=   "chk屏蔽费别"
      Tab(1).Control(3)=   "cbo服务对象"
      Tab(1).Control(4)=   "cbo费用类型"
      Tab(1).Control(5)=   "cbo收入项目"
      Tab(1).Control(6)=   "fra(2)"
      Tab(1).Control(7)=   "txt病案费目"
      Tab(1).Control(8)=   "lbl病案费目"
...
Public Function ShowEditCard(ByVal frmMain As Object, intEditType As gEditType, ByVal lng诊疗ID As Long, ByVal lng分类id As Long, _
    Optional str材料ID As String = "", Optional strPrivs As String = "", Optional bln启用 As Boolean) As Boolean
    '-----------------------------------------------------------------------------------------------------------
    '--功  能:编辑卫生材料
    '--入参数:frmMain-调用的主窗体
    '--       intEditType -编辑类型
...
'            Me.txtEdit(i).Text = zlGetSymbol(Me.txtEdit(GetTxtIdx("商品名")).Text, 1)
    ...
    '检查卫材库房分批发料部门不分批时，是否有部门性质同时设置了卫材库房和发料部门
    If chk库房.Value = 1 And chk在用.Value = 0 Then
        gstrSQL = "Select 1" & vbNewLine & _
                        "From 部门性质说明 T" & vbNewLine & _
                        "Where t.部门id In" & vbNewLine & _
                        "      (Select Distinct t.部门id From 部门性质说明 T Where (t.工作性质 like '%卫材库' or t.工作性质 like '%虚拟库房'))" & vbNewLine & _
                        "      And (t.工作性质 like '%发料部门' Or t.工作性质 Like '%制剂室')"
                        
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "是否有部门性质同时设置了卫材库房和发料部门")
        If rsTemp.RecordCount > 0 Then
            If Val(chk库房.Tag) = 1 And Val(chk在用.Tag) = 0 Then
                MsgBox "该卫材为卫材库房分批发料部门不分批，且当前有部门的工作性质同时设置了卫材库和发料部门，请注意查看！", vbInformation, gstrSysName
            Else
                MsgBox "当前有部门的工作性质同时设置了卫材库和发料部门，在设置分批管理时请同时设置卫材库房和发料部门分批或不分批！", vbInformation, gstrSysName
                Me.stbSpec.Tab = 1
                Exit Function
            End If
        End If
...
'            Me.cbo收入项目.Enabled = cbo价格属性.Enabled
    ...
    If mintEditType = g修改 Then
        chk库房.Tag = chk库房.Value
        chk在用.Tag = chk在用.Value
    End If
...
Private Sub Form_Activate()
    ...
    If mbln启用 Then Me.stbSpec.Tab = 1
...
Private Sub Form_Unload(Cancel As Integer)
    If mbln启用 = True Then
        MsgBox "当前有部门的工作性质同时设置了卫材库和发料部门，启用该规格后请同时设置卫材库房和发料部门分批或不分批，保存后才能退出！", vbInformation, gstrSysName
        Cancel = 1
        Me.stbSpec.Tab = 1
        Exit Sub
    End If
