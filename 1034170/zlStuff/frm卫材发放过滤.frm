...
'    Dim strTemp As String
'
'    If mintType = mFindType.IC卡号 Then
...
'    Else
'        If Not gobjSquareCard Is Nothing Then
'            Call gobjSquareCard.zlReadCard(Me, mlngModule, Val(Split(txtPati.Tag, "|")(gCardFormat.卡类别ID)), True, "", strTemp, strOutXML)
...
Private Sub InitData()
    ...
    Dtp结束Date.MaxDate = Format(zldatabase.Currentdate, "yyyy-mm-dd") & " 23:59:59"
    If mblnTrans Then
        Dtp开始Date.Value = CDate(mstrDrugStartDate)
        Dtp结束Date.Value = CDate(mstrDrugEndDate)
    Else
         Dtp结束Date.Value = Dtp结束Date.MaxDate
         Dtp开始Date.Value = Format(DateAdd("d", -7, zldatabase.Currentdate), "yyyy-mm-dd") & " 00:00:00"
    End If
...
Private Sub txtEDIT_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
    ...
    If Len(txtEdit(Index)) < 8 Then
        ...
        txtEdit(Index).Text = GetFullNO(txtEdit(Index).Text, intType, cbo发料部门.ItemData(cbo发料部门.ListIndex))
    End If
    zlCommFun.PressKey (vbKeyTab)
End Sub
...
Private Sub txtPati_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        ElseIf strCard = "IC卡号" Then
            'IC卡
            Call cmd刷新_Click
        ElseIf IDKNType.GetCurCard.消费卡 Then
            '银行卡
            Call cmd刷新_Click
        Else
            zlCommFun.PressKey vbKeyTab
...
Public Sub Set发料窗口条件(ByVal blnTran As Boolean, ByVal strNo As String, ByVal strStartDate As String, ByVal strEndDate As String, ByVal lng病人id As Long, ByVal lng库房ID As Long)
    ...
    Call InitData
    If mlng病人id <> 0 Then
        mintType = mFindType.病人ID + 1
        txtPati.Text = mlng病人id
        Me.IDKNType.IDKind = mintType
        
    End If
End Sub
Private Sub txt科室_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode <> vbKeyReturn Then Exit Sub
    If txt科室.Tag <> "" Then zlCommFun.PressKey vbKeyTab: Exit Sub
