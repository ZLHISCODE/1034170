...
Private Sub InitCommandBar()
    ...
    With objBar.Controls
        ...
        intIndex = 1
        If IsHavePrivs(mstrPrivs, "售价管理") Then
    ...
    With vsPrice
        ...
        .Col = .ColIndex("现价")
         Do While Not rsTemp.EOF
            .TextMatrix(i, .ColIndex("品名")) = "[" & NVL(rsTemp!编码) & "]" & NVL(rsTemp!名称)
            .Cell(flexcpData, i, .ColIndex("品名")) = NVL(rsTemp!Id)
            .TextMatrix(i, .ColIndex("规格")) = NVL(rsTemp!规格)
            .TextMatrix(i, .ColIndex("产地")) = NVL(rsTemp!产地)
            .TextMatrix(i, .ColIndex("单位")) = IIf(mintUnit = 0, NVL(rsTemp!计算单位), NVL(rsTemp!包装单位))
            .TextMatrix(i, .ColIndex("类型")) = NVL(rsTemp!类型)
            .Cell(flexcpData, i, .ColIndex("类型")) = NVL(rsTemp!跟踪在用)

            .TextMatrix(i, .ColIndex("系数")) = NVL(rsTemp!换算系数)
            .TextMatrix(i, .ColIndex("原成本价")) = Format(Val(NVL(rsTemp!成本价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_成本价)
            .Cell(flexcpData, i, .ColIndex("原成本价")) = Val(NVL(rsTemp!成本价))
            .TextMatrix(i, .ColIndex("现成本价")) = Format(Val(NVL(rsTemp!成本价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_成本价)
            .Cell(flexcpData, i, .ColIndex("现成本价")) = Val(NVL(rsTemp!成本价))
            
            .TextMatrix(i, .ColIndex("原采购限价")) = Format(Val(NVL(rsTemp!指导批发价)) * Val(rsTemp!换算系数), mFMT.FM_成本价)
            .TextMatrix(i, .ColIndex("现采购限价")) = .TextMatrix(i, .ColIndex("原采购限价"))
            .Cell(flexcpData, i, .ColIndex("原采购限价")) = Val(NVL(rsTemp!指导批发价))
            .Cell(flexcpData, i, .ColIndex("现采购限价")) = Val(NVL(rsTemp!指导批发价))
            
            .TextMatrix(i, .ColIndex("指导零售价")) = Format(Val(NVL(rsTemp!指导零售价)) * Val(rsTemp!换算系数), mFMT.FM_零售价)
            .TextMatrix(i, .ColIndex("原指导售价")) = .TextMatrix(i, .ColIndex("指导零售价"))
            .TextMatrix(i, .ColIndex("现指导售价")) = .TextMatrix(i, .ColIndex("原指导售价"))
            
            .Cell(flexcpData, i, .ColIndex("指导零售价")) = Val(NVL(rsTemp!指导零售价))
            .Cell(flexcpData, i, .ColIndex("原指导售价")) = Val(NVL(rsTemp!指导零售价))
            .Cell(flexcpData, i, .ColIndex("现指导售价")) = Val(NVL(rsTemp!指导零售价))
            
            Call zlGetPrice(Val(NVL(rsTemp!Id)), IIf(.TextMatrix(i, .ColIndex("类型")) = "时价", True, False), True, i)
            Call LoadStockData(Val(NVL(rsTemp!Id)), Val(.Cell(flexcpData, i, .ColIndex("原价"))), Val(.Cell(flexcpData, i, .ColIndex("现价"))))
            i = i + 1
            rsTemp.MoveNext
        Loop
...
Private Sub txtPriver_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If txtPriver.Tag <> "" Then zlCommFun.PressKey vbKeyTab: Exit Sub
    If txtPriver.Tag = "" And Trim(txtPriver.Text) = "" Then zlCommFun.PressKey vbKeyTab: Exit Sub
...
Private Sub vsPrice_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsPrice
        ...
        Case .ColIndex("现指导批价")
            If strKey <> "" Then
                If zlDblIsValid(strKey, 12, , False, , "现指导批价") = False Then Cancel = True: Exit Sub
                vsPrice.EditText = Format(Val(strKey), mFMT.FM_成本价)
            End If
        Case .ColIndex("现指导售价")
            If strKey <> "" Then
                If zlDblIsValid(strKey, 12, , False, , "现指导售价") = False Then Cancel = True: Exit Sub
                vsPrice.EditText = Format(Val(strKey), mFMT.FM_零售价)
...
Private Function Select诊疗分类(ByVal strSeach As String) As Boolean
    ...
    Set objCtl = txt分类
    vRect = GetControlRect(txt分类.hwnd)
    lngH = txt分类.Height
...
Private Function SelectStuff(ByVal strKey As String) As Boolean
    ...
    With Me.vsPrice
        ...
        For i = 1 To .Rows - 1
            If Val(.Cell(flexcpData, i, .ColIndex("品名"))) <> 0 Then
                If Val(.Cell(flexcpData, i, .ColIndex("品名"))) = Val(NVL(rsTemp!Id)) And i <> .Row Then
        ...
        '检查是否改变了原来已经存在的卫生材料
        If Val(.Cell(flexcpData, .Row, .ColIndex("品名"))) <> Val(NVL(rsTemp!Id)) And Val(.Cell(flexcpData, .Row, .ColIndex("品名"))) <> 0 Then
            '需要移除该卫生材料的库房变动情况，后才能更新
             Call MoveStockData(Val(.Cell(flexcpData, .Row, .ColIndex("品名"))))
        End If
        
        .Redraw = flexRDNone
        .TextMatrix(.Row, .ColIndex("品名")) = "[" & NVL(rsTemp!编码) & "]" & NVL(rsTemp!名称)
        .Cell(flexcpData, .Row, .ColIndex("品名")) = NVL(rsTemp!Id)
        .TextMatrix(.Row, .ColIndex("规格")) = NVL(rsTemp!规格)
        .TextMatrix(.Row, .ColIndex("产地")) = NVL(rsTemp!产地)
        .TextMatrix(.Row, .ColIndex("单位")) = IIf(mintUnit = 0, NVL(rsTemp!计算单位), NVL(rsTemp!包装单位))
        .TextMatrix(.Row, .ColIndex("类型")) = NVL(rsTemp!类型)
        .Cell(flexcpData, .Row, .ColIndex("类型")) = NVL(rsTemp!跟踪在用)
        
        int系数 = IIf(mintUnit = 0, 1, NVL(rsTemp!换算系数))
        .TextMatrix(.Row, .ColIndex("系数")) = int系数
        
        
        .TextMatrix(.Row, .ColIndex("现成本价")) = Format(Val(NVL(rsTemp!成本价ID)) * int系数, mFMT.FM_成本价)
        .Cell(flexcpData, .Row, .ColIndex("现成本价")) = Val(NVL(rsTemp!成本价ID))
        
        .TextMatrix(.Row, .ColIndex("原采购限价")) = Format(Val(NVL(rsTemp!指导批发价ID)) * int系数, mFMT.FM_成本价)
        .TextMatrix(.Row, .ColIndex("现采购限价")) = .TextMatrix(.Row, .ColIndex("原采购限价"))
        .Cell(flexcpData, .Row, .ColIndex("原采购限价")) = Val(NVL(rsTemp!指导批发价ID))
        .Cell(flexcpData, .Row, .ColIndex("现采购限价")) = Val(NVL(rsTemp!指导批发价ID))
        
        
        .TextMatrix(.Row, .ColIndex("指导零售价")) = Format(Val(NVL(rsTemp!指导零售价ID)) * int系数, mFMT.FM_零售价)
        .TextMatrix(.Row, .ColIndex("原指导售价")) = .TextMatrix(.Row, .ColIndex("指导零售价"))
        .TextMatrix(.Row, .ColIndex("现指导售价")) = .TextMatrix(.Row, .ColIndex("指导零售价"))
        
        .Cell(flexcpData, .Row, .ColIndex("指导零售价")) = Val(NVL(rsTemp!指导零售价ID))
        .Cell(flexcpData, .Row, .ColIndex("原指导售价")) = Val(NVL(rsTemp!指导零售价ID))
        .Cell(flexcpData, .Row, .ColIndex("现指导售价")) = Val(NVL(rsTemp!指导零售价ID))
        Call zlGetPrice(Val(NVL(rsTemp!Id)), IIf(.TextMatrix(.Row, .ColIndex("类型")) = "时价", True, False))
        Call LoadStockData(Val(NVL(rsTemp!Id)), Val(.Cell(flexcpData, .Row, .ColIndex("原价"))), Val(.Cell(flexcpData, .Row, .ColIndex("现价"))))
...
Private Sub zlGetPrice(lng材料id As Long, bln实价 As Boolean, Optional bln批量 As Boolean = False, Optional lngRow As Long = -1)
    ...
    With vsPrice
        ...
        If rsTemp.RecordCount > 0 Then
            ...
            .Cell(flexcpData, lngRow, .ColIndex("类型")) = NVL(rsTemp!跟踪在用)
            
            .TextMatrix(lngRow, .ColIndex("上次日期")) = Format(rsTemp!执行日期, "YYYY-MM-DD HH:MM:SS")
            .TextMatrix(lngRow, .ColIndex("原价")) = Format(Val(NVL(rsTemp!现价)) * Val(NVL(rsTemp!系数)), mFMT.FM_零售价)
            .Cell(flexcpData, lngRow, .ColIndex("原价")) = Val(NVL(rsTemp!现价))
            If bln批量 = False Then
                .TextMatrix(lngRow, .ColIndex("现价")) = Format(Val(NVL(rsTemp!现价)) * Val(NVL(rsTemp!系数)), mFMT.FM_零售价)
                .Cell(flexcpData, lngRow, .ColIndex("现价")) = Val(NVL(rsTemp!现价))
            Else
                If Val(txt调整额.Text) = 0 Then
                    .TextMatrix(lngRow, .ColIndex("现价")) = Format(Val(NVL(rsTemp!现价)) * Val(NVL(rsTemp!系数)), mFMT.FM_零售价)
                    .Cell(flexcpData, lngRow, .ColIndex("现价")) = Val(NVL(rsTemp!现价)) * Val(NVL(rsTemp!系数))
                Else
                    Select Case bytType
                    Case 1      '根据成本价加成
                        dbl比率 = 1 + Val(txt调整额.Text) / 100
                        .TextMatrix(lngRow, .ColIndex("现价")) = Format(Val(NVL(rsTemp!成本价)) * dbl比率 * Val(NVL(rsTemp!系数)), mFMT.FM_零售价)
                        .Cell(flexcpData, lngRow, .ColIndex("现价")) = Val(NVL(rsTemp!成本价)) * dbl比率
                    Case 2      '根据零售价按比例
                        dbl比率 = 1 + Val(txt调整额.Text) / 100
                        .TextMatrix(lngRow, .ColIndex("现价")) = Format(Val(NVL(rsTemp!现价)) * dbl比率 * Val(NVL(rsTemp!系数)), mFMT.FM_零售价)
                        .Cell(flexcpData, lngRow, .ColIndex("现价")) = Val(NVL(rsTemp!现价)) * dbl比率
                    Case 3      '根据零售价按固定金额加减
                        dbl比率 = Val(txt调整额.Text)
                        .TextMatrix(lngRow, .ColIndex("现价")) = Format((Val(NVL(rsTemp!现价)) * Val(NVL(rsTemp!系数))) + dbl比率, mFMT.FM_零售价)
                        .Cell(flexcpData, lngRow, .ColIndex("现价")) = Val(NVL(rsTemp!现价)) + dbl比率
                    End Select
                End If
                ...
                End If
            End If
            .TextMatrix(lngRow, .ColIndex("现成本价")) = Format(rsTemp!成本价 * Val(NVL(rsTemp!系数)), mFMT.FM_成本价)
            .Cell(flexcpData, lngRow, .ColIndex("现成本价")) = Val(NVL(rsTemp!成本价))
            
            .TextMatrix(lngRow, .ColIndex("原收入id")) = Val(NVL(rsTemp!收入项目id))
            .TextMatrix(lngRow, .ColIndex("收入名称")) = NVL(rsTemp!收入名称)
            .Cell(flexcpData, lngRow, .ColIndex("收入名称")) = Val(NVL(rsTemp!收入项目id))
        Else
            .RowData(lngRow) = -1
...
Private Function Select收入项目(ByVal strKey As String) As Boolean
    ...
    With Me.vsPrice
        .Redraw = flexRDNone
        .TextMatrix(.Row, .ColIndex("收入名称")) = NVL(rsTemp!名称)
        .Cell(flexcpData, .Row, .ColIndex("收入名称")) = NVL(rsTemp!Id)
        .Redraw = flexRDBuffered
    End With
    ...
    If chk立即执行.Value = 1 Then
        ...
        With vsPrice
            For i = 1 To .Rows - 1
                ...
                gstrSQL = "Select count(*) as 未执行 From 收费价目 where 变动原因=0 and  收费细目id=[1]"
                Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, lng材料id)
                If Not rsTemp.EOF Then
                    If Val(NVL(rsTemp!未执行)) <> 0 Then
            ...
            For i = 1 To .Rows - 1
                ...
                If lng材料id = Val(.Cell(flexcpData, i, .ColIndex("卫材信息"))) Then
                    If Val(.TextMatrix(i, .ColIndex("差价调整额"))) <> 0 Then
                        ...
                        Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, lng材料id)
                        If rsTemp.EOF Then Exit Function
                        MsgBox "卫生材料“" & NVL(rsTemp!名称) & "”存在未执行成本价，不能设置本次调价！", vbInformation, gstrSysName
                        Exit Function
                    End If
                    blnHaveData = True
                End If
...
Private Function SaveData() As Boolean
    ...
    If m调价方式 = T_成本价调价 Then
    Else
        lng收费价目id = zldatabase.GetNextId("收费价目")
...
End Function
 ...
 Private Sub printbill(ByVal intPrintMode As Byte)
    ...
    Set objRow = New zlTabAppRow
    objRow.Add "执行时间:" & Format(IIf(Me.chk立即执行.Value, zldatabase.Currentdate, Me.dtp执行日期.Value), "yyyy年MM月DD日 HH:mm:ss")
...
Private Sub InitBill()
    ...
    If mlngBillId = 0 Then
        ...
        With vsPrice
            If rsTemp.EOF Then
                Exit Sub
            End If
            txt分类.Text = NVL(rsTemp!分类)
            txt分类.Tag = NVL(rsTemp!分类id)
            .Redraw = flexRDNone
            .TextMatrix(.Row, .ColIndex("品名")) = "[" & NVL(rsTemp!编码) & "]" & NVL(rsTemp!名称)
            .Cell(flexcpData, .Row, .ColIndex("品名")) = NVL(rsTemp!Id)
            .TextMatrix(.Row, .ColIndex("规格")) = NVL(rsTemp!规格)
            ...
            .TextMatrix(.Row, .ColIndex("单位")) = IIf(mintUnit = 0, NVL(rsTemp!计算单位), NVL(rsTemp!包装单位))
            .TextMatrix(.Row, .ColIndex("类型")) = NVL(rsTemp!类型)
            .TextMatrix(.Row, .ColIndex("系数")) = NVL(rsTemp!换算系数)
            .TextMatrix(.Row, .ColIndex("原成本价")) = Format(Val(NVL(rsTemp!成本价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_成本价)
            .Cell(flexcpData, .Row, .ColIndex("原成本价")) = Val(NVL(rsTemp!成本价))
            .TextMatrix(.Row, .ColIndex("现成本价")) = Format(Val(NVL(rsTemp!成本价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_成本价)
            .Cell(flexcpData, .Row, .ColIndex("现成本价")) = Val(NVL(rsTemp!成本价))
            
            .TextMatrix(.Row, .ColIndex("原采购限价")) = Format(Val(NVL(rsTemp!指导批发价)) * Val(rsTemp!换算系数), mFMT.FM_成本价)
            .TextMatrix(.Row, .ColIndex("现采购限价")) = .TextMatrix(.Row, .ColIndex("原采购限价"))
            .Cell(flexcpData, .Row, .ColIndex("原采购限价")) = Val(NVL(rsTemp!指导批发价))
            .Cell(flexcpData, .Row, .ColIndex("现采购限价")) = Val(NVL(rsTemp!指导批发价))
            
            .TextMatrix(.Row, .ColIndex("指导零售价")) = Format(Val(NVL(rsTemp!指导零售价)) * Val(rsTemp!换算系数), mFMT.FM_零售价)
            .TextMatrix(.Row, .ColIndex("原指导售价")) = .TextMatrix(.Row, .ColIndex("指导零售价"))
            .TextMatrix(.Row, .ColIndex("现指导售价")) = .TextMatrix(.Row, .ColIndex("指导零售价"))
            
            .Cell(flexcpData, .Row, .ColIndex("指导零售价")) = Val(NVL(rsTemp!指导零售价))
            .Cell(flexcpData, .Row, .ColIndex("原指导售价")) = Val(NVL(rsTemp!指导零售价))
            .Cell(flexcpData, .Row, .ColIndex("现指导售价")) = Val(NVL(rsTemp!指导零售价))
            Call zlGetPrice(Val(NVL(rsTemp!Id)), IIf(.TextMatrix(.Row, .ColIndex("类型")) = "时价", True, False), False, .Row)
            Call LoadStockData(Val(NVL(rsTemp!Id)), Val(.Cell(flexcpData, .Row, .ColIndex("原价"))), Val(.Cell(flexcpData, .Row, .ColIndex("现价"))))
            .Col = .ColIndex("现价")
            .Redraw = flexRDBuffered
            mlngPreRow = 0:
    ...
    With vsPrice
        ...
        If rsTemp.EOF = False Then
            Me.txt说明 = NVL(rsTemp!调价说明)
            Me.txt调价人.Text = NVL(rsTemp!调价人)
            Me.dtp执行日期.Value = NVL(rsTemp!执行日期)
            .Rows = rsTemp.RecordCount + 1
        Else
            .Rows = 2
        End If
        Do While Not rsTemp.EOF
            ...
            .RowData(i) = Val(NVL(rsTemp!Id))
            .TextMatrix(i, .ColIndex("品名")) = NVL(rsTemp!品名)
            .Cell(flexcpData, i, .ColIndex("品名")) = NVL(rsTemp!材料ID)
            .TextMatrix(i, .ColIndex("规格")) = NVL(rsTemp!规格)
            .TextMatrix(i, .ColIndex("产地")) = NVL(rsTemp!产地)
            .TextMatrix(i, .ColIndex("单位")) = IIf(mintUnit = 0, NVL(rsTemp!单位), NVL(rsTemp!包装单位))
            .TextMatrix(i, .ColIndex("类型")) = NVL(rsTemp!类型)
            .TextMatrix(i, .ColIndex("系数")) = NVL(rsTemp!换算系数)
            .TextMatrix(i, .ColIndex("现成本价")) = Format(Val(NVL(rsTemp!成本价)) * Val(rsTemp!换算系数), mFMT.FM_成本价)
            .TextMatrix(i, .ColIndex("指导零售价")) = Format(Val(NVL(rsTemp!指导零售价)) * Val(rsTemp!换算系数), mFMT.FM_零售价)
            .TextMatrix(i, .ColIndex("上次日期")) = Format(rsTemp!执行日期, "YYYY-MM-DD HH:MM:SS")
            .TextMatrix(i, .ColIndex("原价")) = Format(Val(NVL(rsTemp!原价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_零售价)
            .TextMatrix(i, .ColIndex("现价")) = Format(Val(NVL(rsTemp!现价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_零售价)
            .TextMatrix(i, .ColIndex("原收入id")) = Val(NVL(rsTemp!收入项目id))
            .TextMatrix(i, .ColIndex("收入名称")) = NVL(rsTemp!收入名称)
            .Cell(flexcpData, i, .ColIndex("收入名称")) = Val(NVL(rsTemp!收入项目id))
            
            If NVL(rsTemp!执行日期) <= Format(dtDate, "yyyy-mm-dd HH:MM:SS") And rsTemp!变动原因 = 0 Then       '未进行调价计算,则执行计算
    ...
    Else
        ...
        "                   From 药品收发记录 N, (select ID,收费细目ID,执行日期,终止日期 from 收费价目 where ID=[1]) P" & _
        ...
        With vsStoce
            ...
            i = 1
            Do While Not rsTemp.EOF
                .TextMatrix(i, .ColIndex("库房")) = NVL(rsTemp!库房)
                .TextMatrix(i, .ColIndex("卫材信息")) = NVL(rsTemp!材料信息)
                ...
                .TextMatrix(i, .ColIndex("数量")) = Format(Val(NVL(rsTemp!数量)) / Val(NVL(rsTemp!换算系数)), mFMT.FM_数量)
                .TextMatrix(i, .ColIndex("原价")) = Format(Val(NVL(rsTemp!原价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_零售价)
                .TextMatrix(i, .ColIndex("现价")) = Format(Val(NVL(rsTemp!现价)) * Val(NVL(rsTemp!换算系数)), mFMT.FM_零售价)
                .TextMatrix(i, .ColIndex("调整额")) = Format(Val(NVL(rsTemp!调整金额)), mFMT.FM_金额)
                i = i + 1
                rsTemp.MoveNext
            Loop
        End With
   ...
   '-----------------------------------------------------------------------------------------------------------
    ...
    With vsStoce
        ...
        Do While Not rsTemp.EOF
            ...
            .Cell(flexcpData, lngRow, .ColIndex("库房")) = NVL(rsTemp!库房ID)
            .TextMatrix(lngRow, .ColIndex("供应商")) = NVL(rsTemp!供应商):
            .Cell(flexcpData, lngRow, .ColIndex("供应商")) = NVL(rsTemp!上次供应商ID)
            .TextMatrix(lngRow, .ColIndex("卫材信息")) = NVL(rsTemp!材料)
            .Cell(flexcpData, lngRow, .ColIndex("卫材信息")) = NVL(rsTemp!材料ID)
            .TextMatrix(lngRow, .ColIndex("规格|产地")) = NVL(rsTemp!规格) & IIf(IsNull(rsTemp!产地), "", "|" & rsTemp!产地)
            .TextMatrix(lngRow, .ColIndex("单位")) = IIf(mintUnit = 0, NVL(rsTemp!计算单位), NVL(rsTemp!包装单位))
            
            .Cell(flexcpData, lngRow, .ColIndex("单位")) = NVL(rsTemp!换算系数)
            .TextMatrix(lngRow, .ColIndex("批号")) = NVL(rsTemp!批号)
            .Cell(flexcpData, lngRow, .ColIndex("批号")) = NVL(rsTemp!批次)
            .TextMatrix(lngRow, .ColIndex("数量")) = Format(Val(NVL(rsTemp!数量)) / Val(NVL(rsTemp!换算系数)), mFMT.FM_数量)
            .Cell(flexcpData, lngRow, .ColIndex("数量")) = NVL(rsTemp!数量)
            
            '计算原价
            dblTemp = IIf(Val(NVL(rsTemp!变价)) = 1, Val(NVL(rsTemp!时价售价)), dbl原价) * Val(NVL(rsTemp!换算系数))
            .TextMatrix(lngRow, .ColIndex("原价")) = Format(dblTemp, mFMT.FM_零售价)
            .Cell(flexcpData, lngRow, .ColIndex("原价")) = IIf(Val(NVL(rsTemp!变价)) = 1, Val(NVL(rsTemp!时价售价)), dbl原价)
            
            .TextMatrix(lngRow, .ColIndex("现价")) = Format(dbl现价 * Val(NVL(rsTemp!换算系数)), mFMT.FM_零售价)
            .Cell(flexcpData, lngRow, .ColIndex("现价")) = dbl现价
            .TextMatrix(lngRow, .ColIndex("调整额")) = Format(Val(NVL(rsTemp!数量)) * (dbl现价 - Val(.Cell(flexcpData, lngRow, .ColIndex("原价")))), mFMT.FM_金额)
            .Cell(flexcpData, lngRow, .ColIndex("调整额")) = Val(NVL(rsTemp!数量)) * (dbl现价 - Val(.Cell(flexcpData, lngRow, .ColIndex("原价"))))
             
             dbl原成本价 = Val(NVL(rsTemp!成本价))
            ...
            .TextMatrix(lngRow, .ColIndex("原成本价")) = Format(dbl原成本价 * Val(NVL(rsTemp!换算系数)), mFMT.FM_成本价)
            ...
            .TextMatrix(lngRow, .ColIndex("现成本价")) = Format(dbl现成本价 * Val(NVL(rsTemp!换算系数)), mFMT.FM_成本价)
            .Cell(flexcpData, lngRow, .ColIndex("现成本价")) = dbl现成本价
            
            .TextMatrix(lngRow, .ColIndex("差价调整额")) = Format((dbl原成本价 - dbl现成本价) * Val(NVL(rsTemp!数量)), mFMT.FM_金额)
            .Cell(flexcpData, lngRow, .ColIndex("差价调整额")) = (dbl原成本价 - dbl现成本价) * Val(NVL(rsTemp!数量))
...
Private Sub vsStoce_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsStoce
        ...
        Case .ColIndex("加成率")
            If strKey <> "" Then
                If zlDblIsValid(strKey, 5, , False, , "加成率") = False Then Cancel = True: Exit Sub
...
Private Sub vsPay_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsPay
        ...
        Case .ColIndex("发票金额")
            If strKey <> "" Then
                If zlDblIsValid(strKey, 12, , False, , "发票金额") = False Then Cancel = True: Exit Sub
