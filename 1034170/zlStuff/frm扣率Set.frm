...
Public Function ShowCalc(ByVal frmMain As Form, _
    ...
    ByRef dbl零售价 As Double, ByRef dbl结算价 As Double, ByRef dbl加成率 As Double) As Boolean
    '-----------------------------------------------------------------------------------------------------------
    '功能:零售计算器
    '入参:
    '出参:
    ...
    '-----------------------------------------------------------------------------------------------------------
    mdbl零售价 = dbl零售价: mdbl结算价 = dbl结算价: mdbl加成率 = dbl加成率: mlng材料ID = lng材料ID
...
Private Function 校正零售价(ByVal dbl零售价 As Double) As Double
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "读取指导零售价", mlng材料ID)
    If rsTemp.EOF Then Exit Function
    sin指导零售价 = NVL(rsTemp!指导零售价, 0)
    sin指导零售价 = sin指导零售价 * Val(NVL(rsTemp!换算系数))
    If sin指导零售价 = 0 Then sin指导零售价 = dbl零售价
    校正零售价 = IIf(dbl零售价 > sin指导零售价, sin指导零售价, dbl零售价)
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
