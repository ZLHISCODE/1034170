...
Private Sub GetList(ByVal strFind As String)
    ...
    gstrSQL = "SELECT A.NO, C.名称 AS 发料库房,LTRIM(TO_CHAR (SUM (A.成本金额), " & mOraFMT.FM_金额 & ")) AS 成本金额, " & _
        ...
        " TO_CHAR (MIN(A.审核日期), 'YYYY-MM-DD HH24:MI:SS') AS 审核日期, A.记录状态, A.配药人 发送人, C.ID AS 发料库房ID ,A.摘要 " & _
        ...
        " GROUP BY A.NO,C.名称,A.填制人,A.审核人,A.记录状态,A.配药人,A.摘要, C.ID " & _
...
Private Sub SetStrikeColor()
    ...
    With mshList
        ...
        For intRow = 1 To .Rows - 1
            intStatus = Val(.TextMatrix(intRow, .Cols - 4))
            ...
            If intStatus Mod 3 = 2 Then
                ...
                For intCol = 0 To .Cols - 1
                    .Col = intCol
                    If .TextMatrix(intRow, .Cols - 5) = "" Then
    ...
    With mshList
        ...
        If mblnBootUp = False Then
            For intCol = 1 To .Cols - 1
                ...
                ElseIf intCol = .Cols - 4 Or intCol = .Cols - 2 Then '记录状态和发料库房id列隐藏
                    .ColWidth(intCol) = 0
                Else
...
Private Sub mnuEditDel_Click()
    ...
    With mshList
        ...
        If .TextMatrix(intRow, .Cols - 5) = "" And Val(.TextMatrix(.Row, .Cols - 4)) = 1 Then
            ...
            strTitle = "药品申领单"
        ElseIf Val(.TextMatrix(.Row, .Cols - 4)) Mod 3 = 2 And mint冲销申请 = 1 Then
            strTitle = "冲销申请单"
        End If
        ...
        If intReturn = vbYes Then
            gstrSQL = "zl_材料移库_Delete('" & StrBillNo & "'," & Val(.TextMatrix(.Row, .Cols - 4)) & " )"
...
Private Sub mnuEditDisplay_Click()
    ...
    With mshList
        ...
        frmRequestStuffCard.ShowCard Me, StrNo, 4, .TextMatrix(.Row, .Cols - 4), mstrPrivs, , cboStock.ItemData(cboStock.ListIndex)
    End With
End Sub
...
Private Sub mnuEditDisReceive_Click()
    ...
    With mshList
        StrNo = .TextMatrix(.Row, 0)
        frmRequestStuffCard.ShowCard Me, StrNo, 7, .TextMatrix(.Row, .Cols - 4), mstrPrivs, blnSuccess, cboStock.ItemData(cboStock.ListIndex), int处理方式
...
Private Sub mnuEditModify_Click()
    ...
    With mshList
        ...
        frmRequestStuffCard.ShowCard Me, StrNo, 2, mshList.TextMatrix(mshList.Row, mshList.Cols - 4), mstrPrivs, blnSuccess, cboStock.ItemData(cboStock.ListIndex)
        If blnSuccess = True Then
            mnuViewRefresh_Click
        End If
    End With
...
Private Sub mnuEditReceive_Click()
    ...
    With mshList
        StrNo = .TextMatrix(.Row, 0)
        frmRequestStuffCard.ShowCard Me, StrNo, 6, .TextMatrix(.Row, .Cols - 4), mstrPrivs, blnSuccess, cboStock.ItemData(cboStock.ListIndex)
    End With
    If blnSuccess = True Then
        mnuViewRefresh_Click
    End If
...
Private Sub mnuFileBillPreview_Click()
    With mshList
        If .TextMatrix(.Row, 0) = "" Then Exit Sub
        ReportOpen gcnOracle, glngSys, "zl1_bill_1722", Me, "单据编号=" & .TextMatrix(.Row, 0), "记录状态=" & .TextMatrix(.Row, .Cols - 4), "单位系数=" & mintUnit, 1
    End With
End Sub
...
Private Sub mnuFileBillPrint_Click()
    ...
    With mshList
        If .TextMatrix(.Row, 0) = "" Then Exit Sub
        If CheckPrint(.TextMatrix(.Row, 0), 19, Val(.TextMatrix(.Row, .Cols - 2)), cboStock.ItemData(cboStock.ListIndex), True) = True Then
            ReportOpen gcnOracle, glngSys, "zl1_bill_1722", Me, "单据编号=" & .TextMatrix(.Row, 0), "记录状态=" & .TextMatrix(.Row, .Cols - 4), "单位系数=" & mintUnit, 2
        End If
    End With
End Sub
...
Private Sub mshlist_EnterCell()
    ...
    If mshList.Row >= 1 And LTrim(mshList.TextMatrix(mshList.Row, 0)) <> "" Then
        ...
        Set rsDetail = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, mshList.TextMatrix(mshList.Row, 0), Val(mshList.TextMatrix(mshList.Row, mshList.Cols - 4)))
    ...
    With mshList
        ...
        If .TextMatrix(.Row, 0) = "" Or .Row = 0 Then         '没有单
         ...
         Else
            ...
            If .TextMatrix(.Row, .Cols - 5) = "" Then    '未审核单
                ...
                '如果冲销单还未审核，则允许删除
                If mint冲销申请 = 1 Then
                    If Val(.TextMatrix(.Row, .Cols - 4)) Mod 3 = 2 Then
                        mnuEditModify.Enabled = False
                        tlbTool.Buttons("Modify").Enabled = False
                        mnuEditReceive.Enabled = False
                        tlbTool.Buttons("Receive").Enabled = False
                ...
                Else
                    ...
                    End If
                End If
            ElseIf .TextMatrix(.Row, .Cols - 4) = 1 Then    '审核单
                ...
                End If
            Else   '2,3 冲销单
                If .TextMatrix(.Row, .Cols - 4) Mod 3 = 0 Then
                    .ToolTipText = "冲销单据的原单据"
                    If mnuEditDisReceive.Visible = True Then
                        mnuEditDisReceive.Enabled = True
                        tlbTool.Buttons("DisReceive").Enabled = True
                    End If
                ElseIf .TextMatrix(.Row, .Cols - 4) Mod 3 = 2 Then
                    .ToolTipText = "冲销单据"
                    If mnuEditDisReceive.Visible = True Then
                        mnuEditDisReceive.Enabled = False
                        tlbTool.Buttons("DisReceive").Enabled = False
