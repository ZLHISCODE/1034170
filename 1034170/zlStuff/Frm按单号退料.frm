...
Private Function Save退料(ByVal strDate As String) As Boolean
    ...
    With Bill
        For lngRow = 1 To .Rows - 1
                ...
                If Trim(.TextMatrix(lngRow, mCol.c_项目)) <> "" And Val(.TextMatrix(lngRow, mCol.c_退料数)) <> 0 Then
                   ...
                   strReturnInfo = IIf(strReturnInfo = "", "", strReturnInfo & "|") & Val(.RowData(lngRow)) & "," & dbl数量
                End If
        Next
    End With
    ...
    If Not mobjPlugIn Is Nothing And strReturnInfo <> "" Then
        mobjPlugIn.StuffReturnByID mlng发料部门, strReturnInfo, CDate(strDate), strReserve
    End If
...
Private Sub InitData()
    ...
    strReg = Trim(zlDatabase.GetPara("最后退料单据类型", glngSys, mlngModule, "25", Array(lbl(3), cbo收费单据), IsHavePrivs(mstrPrivs, "参数设置")))
