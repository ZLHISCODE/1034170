...
Private Sub Get本机参数()
    ...
    mintUnit = Val(zlDatabase.GetPara("卫材单位", glngSys, 1341, "0"))
...
Private Sub Get系统参数()
    ...
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, glngSys, 15)
    mstr门诊收费与发药分离 = rs!参数值
        
    '住院记帐与发药分离
    gstrSQL = "select 参数值 from Zlparameters Where 系统 = [1] And Nvl(私有, 0) = 0 And 模块 Is Null And 参数号=[2] "
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, glngSys, 16)
    mstr住院记帐与发药分离 = rs!参数值
        
        
    '费用金额保留位数
    gstrSQL = "select 参数值 from Zlparameters Where 系统 = [1] And Nvl(私有, 0) = 0 And 模块 Is Null And 参数号=[2] "
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, glngSys, 9)
    intTmp = CInt(rs!参数值)
...
Private Sub IniControls()
    ...
    If IsHavePrivs(mstrPrivs, mstr停止) = False Then
        optUnFlag.Enabled = False
        optFlag.Value = True
    End If
    
    If IsHavePrivs(mstrPrivs, mstr恢复) = False Then
...
'    strUnit = "住院单位"
    ...
    ''''from子句
    strSQL = strSQL & " From 药品收发记录 A,门诊费用记录 B,部门表 C,收费项目目录 D,收费项目别名 E,部门表 P "
    Select Case mintBillType
        Case 1
    ...
    ''''where 子句
    strSQL = strSQL & " where A.费用id=B.Id And A.药品id=D.Id And D.ID=E.收费细目ID(+) And B.收费细目id=D.Id " _
    ...
    With mshBill
        ...
        Do While Not rs.EOF
            If .Rows >= 2 And .TextMatrix(1, 1) <> "" Then
                .Rows = .Rows + 1
            End If
            .TextMatrix(.Rows - 1, mBillCol.AmountCol) = Format(rs.Fields(mBillCol.Amount).Value, mFMT.FM_数量)
...
Private Sub txtBillNo_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 And txtBillNo.Text <> "" Then
        ...
        txtBillNo.Text = GetFullNO(txtBillNo.Text, 13)
'        Call GetDetailBill
