...
'            End If
    ...
    If mint编辑状态 = 3 Then        '审核
        ...
        For lngLop = 1 To mshBill.Rows - 1
            If Trim(mshBill.TextMatrix(lngLop, mBillCol.C_材料)) <> "" Then
                '判断新批次已失效卫材是否禁止入库
                If Val(mshBill.TextMatrix(lngLop, mBillCol.c_批次)) = -1 Or Val(mshBill.TextMatrix(lngLop, mBillCol.c_新批次)) = 1 Then
                    If CheckLapse(Trim(mshBill.TextMatrix(lngLop, mBillCol.C_效期)), True) = False Then '检查失效期
                        MsgBox "第" & lngLop & "行卫材已经失效了，失效卫材不能审核！", vbInformation, gstrSysName
                        mshBill.SetFocus
                        mshBill.Row = lngLop
                        mshBill.MsfObj.TopRow = lngLop
                        mshBill.Col = mBillCol.C_效期
                        Exit Sub
                    End If
                End If
            End If
...
'    strPhysic = strPhysic & IIf(str库房货位 = "", "And Substr(E.盘点属性," & int盘点方式 & ",1)='1'", " and (Instr([6], ',' || e.库房货位 || ',') > 0 ) And Substr(E.盘点属性," & int盘点方式 & ",1)='1'")
    str材料储备限额条件 = IIf(str库房货位 = "", " And Substr(E.盘点属性," & int盘点方式 & ",1)='1'", " and (Instr([6], ',' || e.库房货位 || ',') > 0 ) And Substr(E.盘点属性," & int盘点方式 & ",1)='1'")
    
    Call zlCommFun.ShowFlash("正在计算卫生材料库存数据,请稍候 ...", Me)

    DoEvents    ': Me.Refresh
    Set rsData = GetDateStock(str盘点时间, lng库房ID, strPhysic, IIf(int盘无库存材料 = 0, False, True), , str分类ID, , bln盘点零数量且有金额, str库房货位, str材料储备限额条件)
...
'                    If i > 1 Then .Rows = .Rows + 1
            ...
            Else
                ...
                If rsData!是否变价 = 1 And Val(.TextMatrix(i, mBillCol.C_帐面数量)) = 0 Then
                    .TextMatrix(i, mBillCol.c_差价差) = Format(Val(.TextMatrix(i, mBillCol.C_数量差)) * (Val(.TextMatrix(i, mBillCol.C_售价)) - Val(.TextMatrix(i, mBillCol.C_成本价))) - Val(.TextMatrix(i, mBillCol.C_实际差价)), strMoneyDigit)
                Else
                    .TextMatrix(i, mBillCol.c_差价差) = Format(Val(.TextMatrix(i, mBillCol.C_实盘数量)) * (Val(.TextMatrix(i, mBillCol.C_售价)) - Val(.TextMatrix(i, mBillCol.C_成本价))) - Val(.TextMatrix(i, mBillCol.C_实际差价)), strMoneyDigit)
                End If
                dbl金额差 = .TextMatrix(i, mBillCol.c_金额差)
...
Private Function GetDateStock(str盘存时间 As String, lng库房ID As Long, str材料条件 As String, _
    ...
    Optional lng材料ID As Long = 0, Optional bln盘点零数量且有金额 As Boolean = False, Optional ByVal str库房货位 As String = "所有", _
    Optional ByVal str材料储备限额条件 As String = "") As ADODB.Recordset
    ...
    '取得当前库存
    gstrSQL = "" & _
        "   SELECT a.库房id, b.材料id, NVL (a.批次, 0) AS 批次, a.实际数量,0 盘点数量,a.实际金额, a.实际差价, a.可用数量, a.平均成本价 成本价,a.上次批号 AS 批号,a.上次产地 AS 产地,a.批准文号,a.效期 " & _
        "   FROM 药品库存 a, 材料特性 b,收费项目目录 c ,诊疗项目目录 D" & _
    ...
    gstrSQL = gstrSQL & _
        ...
        "           -SUM (DECODE (a.入出系数, 1, a.差价, -a.差价)) AS 实际差价,0 AS 可用数量,max(decode(a.单据,22,a.单量,23,0,a.成本价)) as 成本价,a.批号,a.批准文号,a.产地,a.效期 " & _
        "   FROM 药品收发记录 a,  材料特性 b,收费项目目录 c ,诊疗项目目录 D " & _
    ...
    If bln汇总 Then
        gstrSQL = gstrSQL & _
            ...
            "   FROM 药品收发记录 A, 材料特性 b,收费项目目录 c ,诊疗项目目录 D" & _
            "   Where A.药品ID+0 = B.材料ID And A.单据 = 23 and a.药品id+0=c.id and b.诊疗id=d.id AND A.库房ID + 0 =[1] " & _
    ...
    gstrSQL = "" & _
        ...
        "           SUM(可用数量) As 可用数量,max(成本价) as 成本价,max(批号) as 批号,max(产地) as 产地 ,max(批准文号) as 批准文号,max(效期) as 效期 " & _
        "   FROM ( " & gstrSQL & ") " & _
    ...
    Select Case mintUnit
        Case 0
            strUnitQuantity = "c.计算单位 AS 单位, nvl(a.帐面数量,0) AS 帐面数量, nvl(a.盘点数量,0) AS 盘点数量,nvl(a.可用数量,0) AS 可用数量, '1' as 比例系数," & _
             " f.现价 as 售价,nvl(a.成本价,B.成本价) as 成本价,b.成本价 as 最后进价,"
        Case Else
            strUnitQuantity = "b.包装单位 AS 单位, (nvl(a.帐面数量,0) / b.换算系数) AS 帐面数量, (nvl(a.盘点数量,0) / b.换算系数) AS 盘点数量,(nvl(a.可用数量,0) / b.换算系数) AS 可用数量,b.换算系数 as 比例系数," & _
             "f.现价*b.换算系数 as 售价,nvl(a.成本价,B.成本价)*b.换算系数 as 成本价,b.成本价*b.换算系数 as 最后进价,"
    End Select
    '成本价计算的方式:
    'a.如果不分批
    ...
    gstrSQL = "" & _
        ...
        "           zlSpellCode(c.名称) 名称,c.规格, Decode(a.产地, Null, decode(b.上次产地,null,c.产地,b.上次产地), a.产地) As 产地,A.批准文号,e.库房货位,nvl(a.批次,0) 批次, a.批号, a.效期," & strUnitQuantity & _
        "           nvl(a.实际金额,0) as 实际金额 ,nvl(a.实际差价,0) as 实际差价, b.指导差价率,c.是否变价,b.库房分批,b.在用分批,decode(a.批号,null,1,0) 批号编辑,decode(a.产地,null,1,0) 产地编辑,nvl(b.最大效期,0) 最大效期 " & _
        "   From (" & gstrSQL & ") A , 材料特性 b,收费项目目录 c ,收费执行科室 G,收费价目 F,材料储备限额 E "
    ...
    gstrSQL = gstrSQL & IIf(blnZero And str分类ID <> "", ", (select D.ID From 诊疗项目目录 D  where 1=1 " & IIf(str分类ID = "", "", " and D.分类id in (select /*+cardinality(X,10)*/ * from Table(Cast(f_Num2List([5]) As zlTools.t_NumList)) X) ") & ") D ", "") & _
        ...
        "           and b.材料id= e.材料id and e.库房ID = [1] " & str材料储备限额条件 & _
...
Private Function Get汇总记录单(ByVal lng库房ID As Long, ByVal str盘点时间 As String) As ADODB.Recordset
    ...
    str盘点单 = "" & _
            "   SELECT A.库房ID,B.材料ID,NVL(A.批次, 0) AS 批次,0 AS 实际数量,SUM(A.扣率) 盘点数量," & _
            "           0 AS 实际金额,0 AS 实际差价,0 AS 可用数量,null as 成本价,A.批号,A.产地,A.批准文号,A.效期" & _
            "   FROM 药品收发记录 A,材料特性 b " & _
    ...
    Select Case mintUnit
        Case 0
            strUnitQuantity = "c.计算单位 AS 单位, nvl(a.帐面数量,0) AS 帐面数量, nvl(a.盘点数量,0) AS 盘点数量,nvl(a.可用数量,0) AS 可用数量, '1' as 比例系数," & _
             " f.售价,nvl(a.成本价,B.成本价) as 成本价,b.成本价 as 最后进价,"
        Case Else
            strUnitQuantity = "b.包装单位 AS 单位, (nvl(a.帐面数量,0) / b.换算系数) AS 帐面数量, (nvl(a.盘点数量,0) / b.换算系数) AS 盘点数量,(nvl(a.可用数量,0) / b.换算系数) AS 可用数量,b.换算系数 as 比例系数," & _
             "f.售价*b.换算系数 as 售价,nvl(a.成本价,B.成本价)*b.换算系数 as 成本价,b.成本价*b.换算系数 as 最后进价, "
    End Select
...
'            End If
        ...
        Case 2, 3, 4
            ...
            gstrSQL = "" & _
                ...
                "                   A.零售金额 as 金额差,A.差价 as 差价差,nvl(b.最大效期,0) 最大效期, " & _
                ...
                "                   And A.药品ID=E.药品ID(+) And A.库房ID=E.库房ID(+) And nvl(A.批次,0) = nvl(E.批次(+),0) AND E.性质(+) = 1 AND A.单据 =[1] AND A.No =[2]" & _
                "       ) " & _
            ...
            With mshBill
                Do While Not rsTemp.EOF
                    ...
                    .RowData(lngRow) = IIf(IsNull(rsTemp!最大效期), 0, rsTemp!最大效期)
        ...
        Select Case .Col
            ...
            Case mBillCol.C_效期
                ...
                If .ColData(mBillCol.C_效期) = 2 Then
                    If .TextMatrix(.Row, mBillCol.c_批号) <> "" And Len(Trim(.TextMatrix(.Row, mBillCol.c_批号))) = 8 And Trim(.TextMatrix(.Row, mBillCol.C_效期)) = "" Then
                        Dim strxq As String
            ...
            Case mBillCol.C_效期
                ...
                If strKey <> "" Then
                    If Len(strKey) = 8 And InStr(1, strKey, "-") = 0 Then
                        ...
                        If CheckLapse(Trim(.Text), True) = False Then
                            MsgBox "该卫生材料已失效卫材！", vbInformation, gstrSysName
                            Cancel = True
                        End If
                ...
                ElseIf strKey = "" And strKey <> .TextMatrix(.Row, mBillCol.C_效期) Then
                    ...
                    If CheckLapse(Trim(.TextMatrix(.Row, mBillCol.C_效期)), True) = False Then
                        MsgBox "该卫生材料已失效卫材！", vbInformation, gstrSysName
                        Cancel = True
                    End If
            ...
            For lngLop = 1 To .Rows - 1
                If Trim(.TextMatrix(lngLop, mBillCol.C_材料)) <> "" Then
                    ...
                    If Val(.TextMatrix(lngLop, mBillCol.c_批次)) = -1 Or Val(.TextMatrix(lngLop, mBillCol.c_新批次)) = 1 Then
                        ...
                        If Val(.TextMatrix(lngLop, mBillCol.C_实盘数量)) = 0 Then
                            ShowMsgBox "第" & lngLop & "行新增分批材料实盘数量为0，请检查！"
                            mshBill.SetFocus
                            .Row = lngLop
                            .MsfObj.TopRow = lngLop
                            .Col = mBillCol.C_实盘数量
                        ...
                        If CheckLapse(Trim(mshBill.TextMatrix(lngLop, mBillCol.C_效期)), True) = False Then '检查失效期
                            MsgBox "第" & lngLop & "行卫材已经失效了，失效卫材不能保存！", vbInformation, gstrSysName
                            mshBill.SetFocus
                            .Row = lngLop
                            .MsfObj.TopRow = lngLop
                            .Col = mBillCol.C_效期
...
'        For lngRow = 1 To .Rows - 1
            If .TextMatrix(lngRow, 0) <> "" Then
                ...
                dat效期 = IIf(Trim(.TextMatrix(lngRow, mBillCol.C_效期)) = "", "", .TextMatrix(lngRow, mBillCol.C_效期))
                ...
                '判断有无库存记录；不分批查有无库存记录，分批判断是否新增批次
                bln有库存记录 = False
                If Get分批属性(lng库房ID, lng材料ID) = 0 Then '不分批
                    If CheckNoStock(lng库房ID, lng材料ID) = False Then bln有库存记录 = True
                Else '分批
                    If Val(.TextMatrix(lngRow, mBillCol.c_批次)) = -1 Or Val(.TextMatrix(lngRow, mBillCol.c_新批次)) = 1 Then
                    Else
                        bln有库存记录 = True
                ...
                If Val(Split(.TextMatrix(lngRow, mBillCol.C_指导差价率), "||")(1)) = 0 Or bln有库存记录 = True Then '定价或有库存记录的取原始售价
                    dbl售价 = Get售价(Val(Split(.TextMatrix(lngRow, mBillCol.C_指导差价率), "||")(1)) = 1, lng材料ID, lng库房ID, lng批次ID)
                End If
                
                If bln有库存记录 = True Then '有库存记录的取原始成本价
                    dbl成本价 = Get成本价(lng材料ID, lng库房ID, lng批次ID)
                End If
