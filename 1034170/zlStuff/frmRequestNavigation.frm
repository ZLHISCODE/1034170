...
Private Sub Form_Load()
   ...
   gstrSQL = "" & _
    ...
    '----设置缺省的时间范围（一个月）----
    Me.dtp开始时间.Value = Format(DateAdd("d", -7, zlDatabase.Currentdate()), "yyyy-MM-dd") & " 00:00:00"
    Me.dtp结束时间.Value = Format(zlDatabase.Currentdate(), "yyyy-MM-dd HH:mm:ss")
    Exit Sub
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub ResizeStuff()
    ...
    '判断是否允许用户输入其它条件
    blnEnable = (optMode(根据申领单未发数) Or optMode(根据消耗量))
...
Private Function CheckData(Optional str分类id_IN As String = "") As Boolean
    ...
    If optMode(根据消耗量) Then
     ...
     gstrSQL = "" & _
                 ...
                 "      (Select 收费细目ID,现价 From 收费价目 Where SysDate Between 执行日期 And Nvl(终止日期,Sysdate)) P, " & _
    ...
    Else    '根据申领单未发数（不加条件And Nvl(A.发药方式,0)=1 ，是因为审核时，是删除申领单，产生移库单再审核的，标志已经没有了）
    ...
    With rsCheck
        Do While Not .EOF
            If mbln明确批次 Then
                ...
                Set rsStock = zlDatabase.OpenSQLRecord(gstrSQL, "取该卫材的库存", lngTargetID, Val(NVL(rsCheck!材料ID)))
                    ...
                    Do While Not rsStock.EOF
                        ...
                        End If
                         ...
                         blnData = SetColValue(!材料ID, "[" & !编码 & "]" & !名称, IIf(IsNull(!规格), "", !规格), IIf(IsNull(rsStock!产地), "", rsStock!产地), _
                            ...
                            IIf(NVL(rsStock!灭菌效期) = "", "", Format(rsStock!灭菌效期, "yyyy-mm-dd")), _
                            !库房分批, NVL(!可用数量, 0), _
                ...
                Else
                    ...
                    If mintCheck <> 2 Then
                        ...
                        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取该卫材对于出库库房是否分批、时价的属性", Val(NVL(!材料ID)))
