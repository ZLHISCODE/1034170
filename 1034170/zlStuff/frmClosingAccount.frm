...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.Id
        ...
        Case mconMenu_CA_Refresh '刷新
            Call GetAccountRecord
            Call RefreshList
            If mblnStart = True Then Call CheckClosAccount
        ''''帮助
...
Private Sub GetDetailRecord(ByVal intType As Integer, ByVal lng结存id As Long, ByVal str期初日期 As String, ByVal str期末日期 As String)
    ...
    If str期初日期 <> "" Then
        ...
        "       From 药品收发记录 A, 药品入出类别 B,材料特性 C " & _
        "       Where A.入出类别id = B.ID And A.药品ID = C.材料ID And A.单据 In (13, 15, 16, 17, 18, 19, 20, 21, 22, 24, 25, 26) And " & _
...
Private Sub MediAccountProcess_AddIniAccount(ByVal int结存方式 As Integer)
    ...
    If int结存方式 = 1 Then
        If frmClosingAccountCondition.GetCondition(Me, Val(cbo库房.ItemData(cbo库房.ListIndex)), str结存时间) = False Then
            Call GetAccountRecord
            Call RefreshList
    ...
    '结存日期
    If Trim(str结存时间) <> "" Then gstrSQL = gstrSQL & ",to_date('" & Format(str结存时间, "yyyy-MM-dd hh:mm:ss") & "','yyyy-MM-dd hh:mm:ss')"
    gstrSQL = gstrSQL & ")"
