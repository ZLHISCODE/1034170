...
Begin VB.Form frm卫材销帐_已审核 
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsBatch 
      ...
      ColWidthMax     =   0
      ExtendLastCol   =   0   'False
      FormatString    =   $"frm卫材销帐_已审核.frx":01D2
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Private Function RefreshData() As Boolean
    ...
    gstrSQL = "" & _
        "   Select 单据, NO, 药品ID as 材料ID, 申请时间, 标识号, 姓名, 床号, 单位, 换算系数, Sum(数量) As 销帐数量 " & _
        "   From (  Select Distinct C.单据, C.NO, C.药品ID, A.申请时间, B.标识号, nvl(F.姓名,B.姓名) 姓名, B.床号, " & strFields & " " & _
        ...
        "           Group By 单据, NO, 药品ID, 申请时间, 标识号, 姓名, 床号, 单位, 换算系数 "
...
Private Sub Form_Load()
    ...
    vsBatch.ColHidden(vsBatch.ColIndex("准退数量")) = True
