...
Private Sub cboRoom_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If cboRoom.ListCount = 0 Then Call zlCtlSetFocus(msfLimit): Exit Sub
    
    If cboRoom.ListIndex >= 0 Then
        If Val(cboRoom.Tag) = cboRoom.ItemData(cboRoom.ListIndex) Then
            Call zlCtlSetFocus(msfLimit, True)
            Exit Sub
        End If
    End If
...
Private Sub zlLimitRef()
    ...
    gstrSQL = "Select  i.Id, i.编码, i.名称, i.规格, i.产地, i.包装单位 As 单位, i.换算系数 As 包装," & vbNewLine & _
                    ...
                    "      (p.终止日期 Is Null Or Sysdate Between p.执行日期 And Nvl(p.终止日期, To_Date('3000-01-01', 'yyyy-MM-dd'))) " & vbNewLine & _
                    "Order By i.编码"
    ...
    With mrsTemp
        ...
        Do While Not .EOF
            ...
            Me.msfLimit.TextMatrix(.AbsolutePosition, col包装) = NVL(!包装)
...
Private Sub txt查找_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    For lngStart = lngStart To lngRows
        ...
        str简码 = UCase(zlCommFun.SpellCode(str名称))
