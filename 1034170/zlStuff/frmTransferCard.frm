        ...
        Case 2, 3, 4, 6, 10, 11
            ...
            With mshBill
                Do While Not rsInitCard.EOF
                    ...
                    '外购生成移库单不分批的从库存表读取售价
                    If mint编辑状态 = 11 And Get分批属性(cboStock.ItemData(cboStock.ListIndex), rsInitCard.Fields(0)) = 0 Then
                        dbl零售价 = Get售价(rsInitCard!是否变价 = 1, rsInitCard.Fields(0), cboStock.ItemData(cboStock.ListIndex), IIf(IsNull(rsInitCard!批次), 0, rsInitCard!批次))
                        .TextMatrix(intRow, mBillCol.C_售价) = Format(dbl零售价 * rsInitCard!比例系数, mFMT.FM_零售价)
                        .TextMatrix(intRow, mBillCol.C_售价金额) = Format(Val(.TextMatrix(intRow, mBillCol.C_售价)) * rsInitCard!实际数量, mFMT.FM_金额)
...
Private Function SaveCheck(Optional ByVal bln强制保存 As Boolean = False) As Boolean
    ...
    '检查库存
    str材料 = CheckNumStock(mshBill, cboStock.ItemData(cboStock.ListIndex), 0, mBillCol.C_批次, mBillCol.C_实际数量, mBillCol.c_比例系数, 1, 1, g_小数位数.obj_最大小数.数量小数)
    If str材料 <> "" Then
        If mint库存检查 = 1 Then '不足提醒
            If MsgBox(str材料 & "卫材库存不足，是否继续？", vbYesNo + vbDefaultButton2 + vbQuestion, gstrSysName) = vbNo Then
                Exit Function
            End If
        ElseIf mint库存检查 = 2 Then '不足禁止
            MsgBox str材料 & "卫材库存不足，不能审核！", vbInformation, gstrSysName
            Exit Function
        End If
    End If
...
'                dbl成本价 = Round(Val(.TextMatrix(intRow, mBillCol.C_采购价)) / .TextMatrix(intRow, mBillCol.c_比例系数), g_小数位数.obj_最大小数.成本价小数)
                dbl成本价 = Get成本价(lng材料ID, lng库房ID, lng出批次)
...
'                dbl售价 = Round(Val(.TextMatrix(intRow, mBillCol.C_售价)) / .TextMatrix(intRow, mBillCol.c_比例系数), g_小数位数.obj_最大小数.零售价小数)
                dbl售价 = Get售价(Split(.TextMatrix(intRow, mBillCol.C_最大效期), "||")(1) = 1, lng材料ID, lng库房ID, lng出批次)
                ...
                If strKey <> "" Then
                    ...
                    If mint编辑状态 = 6 Then
                        If Val(strKey) >= 0 Then
                            If Val(strKey) > Val(.TextMatrix(.Row, mBillCol.C_填写数量)) Then
        ...
        If .TextMatrix(1, 0) <> "" Then         '先判有否数据
            ...
            For intLop = 1 To .Rows - 1
                If Trim(.TextMatrix(intLop, mBillCol.C_材料)) <> "" Then
                    ...
                    If mbln分批卫材批号产地控制 = True And Not ((mint编辑状态 = 1 Or mint编辑状态 = 2) And mbln移库明确批次 = False) Then
...
'                dbl采购价 = Round(Val(.TextMatrix(intRow, mBillCol.C_采购价)) / .TextMatrix(intRow, mBillCol.c_比例系数), g_小数位数.obj_最大小数.成本价小数)
                dbl采购价 = Get成本价(lng材料ID, lng库房ID, lng批次)
                
'                dbl成本金额 = Round(Val(.TextMatrix(intRow, mBillCol.C_采购金额)), g_小数位数.obj_最大小数.金额小数)
                dbl成本金额 = Val(Format(Val(Format(dbl采购价 * Val(.TextMatrix(intRow, mBillCol.c_比例系数)), mFMT.FM_成本价)) * Val(.TextMatrix(intRow, mBillCol.C_实际数量)), mFMT.FM_金额))
                
'                dbl零售价 = Round(Val(.TextMatrix(intRow, mBillCol.C_售价)) / Val(.TextMatrix(intRow, mBillCol.c_比例系数)), g_小数位数.obj_最大小数.零售价小数)
                dbl零售价 = Get售价(Split(.TextMatrix(intRow, mBillCol.C_最大效期), "||")(1) = 1, lng材料ID, lng库房ID, lng批次)
                
'                dbl零售金额 = Round(Val(.TextMatrix(intRow, mBillCol.C_售价金额)), g_小数位数.obj_最大小数.金额小数)
                dbl零售金额 = Val(Format(Val(Format(dbl零售价 * Val(.TextMatrix(intRow, mBillCol.c_比例系数)), mFMT.FM_零售价)) * Val(.TextMatrix(intRow, mBillCol.C_实际数量)), mFMT.FM_金额))
        ...
        For intRow = 1 To .Rows - 1
            If Val(.TextMatrix(intRow, mBillCol.C_实际数量)) <> 0 Then
                ...
                If int库存检查 <> 0 Or lng批次 > 0 Then '分批的检查,不足禁用
                    ...
                    End If
                    '正常冲销可用数量实际数量都检查；产生冲销单据检查可用数量；审核冲销单据检查实际数量
                    If mint冲销方式 <> 2 Then If Check可用数量(lng库房ID, Val(.TextMatrix(intRow, 0)), lng批次, dbl冲销数量, int库存检查, 0) = False Then .Row = intRow: .MsfObj.TopRow = intRow: Exit Function
                    If mint冲销方式 <> 1 Then If Check可用数量(lng库房ID, Val(.TextMatrix(intRow, 0)), lng批次, dbl冲销数量, int库存检查, 1) = False Then .Row = intRow: .MsfObj.TopRow = intRow: Exit Function
    ...
    With mshBill
        ...
        ElseIf mint库存检查 = 1 Then
            ...
            ElseIf mint编辑状态 = 2 Or mint编辑状态 = 3 Then
                numUsedCount = 0
                If gSystem_Para.para_卫材填单下可用库存 = True Then '勾选“卫材填单下可用库存”时才加上上次保存的数量
                    For Each vardrug In mcolUsedCount
...
Private Sub printbill()
    Dim strNo As String
    strNo = txtNO.Tag
    FrmBillPrint.ShowMe Me, glngSys, "zl1_bill_1716", mint记录状态, mintUnit, 1716, "卫材调拨单", strNo, cboStock.ItemData(cboStock.ListIndex), cboEnterStock.ItemData(cboEnterStock.ListIndex)
End Sub
