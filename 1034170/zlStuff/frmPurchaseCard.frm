...
Private Sub CmdSave_Click()
    ...
    If mint编辑状态 = 9 Then    '核查
        ...
        For intLop = 1 To mshBill.Rows - 1
            If Trim(mshBill.TextMatrix(intLop, mCol诊疗)) <> "" Then
                '判断已失效卫材是否禁止入库
                If CheckLapse(Trim(mshBill.TextMatrix(intLop, mCol效期)), True) = False Then
                    mbln日期提示 = True
                    MsgBox "第" & intLop & "行卫材已经失效了，失效卫材不能保存！", vbInformation, gstrSysName
                    mshBill.SetFocus
                    mshBill.Row = intLop
    ...
    With mshBill
        ...
        Select Case .Col
            ...
            Case mCol效期
                ...
                .MaxLength = 10
                .Value = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                .MaxDate = Format("3000-01-01", "yyyy-mm-dd")
...
'                                Call CheckLapse(.TextMatrix(.Row, mCol效期))
            ...
            Case mcol生产日期
                ...
                .MaxLength = 10
                .Value = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                .MaxDate = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                If Trim(.TextMatrix(.Row, mCol批号)) = "" Or IsNumeric(.TextMatrix(.Row, mCol批号)) = False Then
                    If Not IsDate(Trim(.TextMatrix(.Row, mcol生产日期))) Then
                        str批号 = ""
                    Else
                ...
                If str批号 <> "" And Len(Trim(str批号)) = 8 Then
                    
                    '存储格式值:最大效期||指导差价率||是否变价||在用分批||库房分批
                    If IsNumeric(str批号) Then
                        strxq = UCase(str批号)
                        If Trim(.TextMatrix(.Row, mcol生产日期)) = "" Then
                            If Not (InStr(1, strxq, "D") <> 0 Or InStr(1, strxq, "E") <> 0) Then
                                strxq = TranNumToDate(strxq)
                                If strxq = "" Then Exit Sub
                                If CheckpProduction(strxq) = False Then
'                                     MsgBox "跟据批号自动产生的生产日期必须小于等于当前日期！", vbInformation, gstrSysName
                                     Exit Sub
                                End If
                ...
                Else
                    ...
                    If rs产地.EOF Then
                        ...
                        Else
                            ...
                            Dim int编码 As Integer, strCode As String
                            ...
                            gstrSQL = "ZL_材料生产商_INSERT('" & strCode & "','" & strKey & "',zlSpellCode('" & strKey & "',10))"
                            Call zlDatabase.ExecuteProcedure(gstrSQL, mstrCaption)
                        End If
                    Else
            ...
            Case mcol生产日期
                ...
                If strKey <> "" Then
                    If Len(strKey) = 8 And InStr(1, strKey, "-") = 0 Then
                        ...
                        End If
                        .Text = strKey
                        If CheckpProduction(strKey) = False Then
                            MsgBox "该卫材生产日期必须小于等于当前日期，请检查！", vbInformation, gstrSysName
                            Cancel = True
                            .TxtSetFocus
                        End If
                    ...
                    End If
                    If CheckpProduction(strKey) = False Then
                        MsgBox "该卫材生产日期是比当前日期大，请检查！", vbInformation, gstrSysName
                        Cancel = True
                        .TxtSetFocus
                    End If
...
'                                        Call CheckLapse(.TextMatrix(.Row, mCol效期))
            ...
            Case mCol采购价
                ...
                If strKey <> "" Then
                    ...
                    '中标成本价判断
                    dblCostPrice = Get中标单位成本价(Val(.TextMatrix(.Row, lng材料ID)))
...
'                             .TextMatrix(.Row, mCol扣率) = Format(Val(.TextMatrix(.Row, mCol结算价)) / Val(.TextMatrix(.Row, mCol指导批发价)) * 100, mFMT.FM_成本价)
            ...
            Case mCol数量
                ...
                If strKey <> "" Then
                    ...
                    '检查是否有足够的库存可以退货
                    If mint编辑状态 = 8 Or mbln退货 Then
                        If Not CheckStock(Val(.TextMatrix(.Row, 0)), Val(.TextMatrix(.Row, mCol批次)), Val(.Text) * Val(.TextMatrix(.Row, mCol比例系数)), .Row) Then
                            Cancel = True
                            .TxtSetFocus
                            Exit Sub
                        End If
            ...
            Case mcol零售价
                ...
                dbl采购价 = IIf(mbln时价购前销售, Val(.TextMatrix(.Row, mCol采购价)), Val(.TextMatrix(.Row, mCol结算价))) '参与计算加成率
                
                '刘兴宏:零售价处理
                Call 计算零售价及零售差价(.Row, False)
                If strKey <> "" Then
                    .TextMatrix(.Row, mCol售价) = Format(Val(strKey) * Val(.TextMatrix(.Row, mCol比例系数)), mFMT.FM_零售价)
                    .TextMatrix(.Row, mcol加成率) = FormatEx(计算加成率(Val(.TextMatrix(.Row, 0)), Val(.TextMatrix(.Row, mCol售价)), dbl采购价), 2) & "%"
                    .TextMatrix(.Row, mCol售价金额) = Format(Val(.TextMatrix(.Row, mCol售价)) * Val(.TextMatrix(.Row, mCol数量)), mFMT.FM_金额)
...
Public Function SetColValue(ByVal intRow As Integer, ByVal lng材料ID As Long, ByVal str诊疗 As String, ByVal str规格 As String, _
    ...
    ByVal int是否变价 As Integer, ByVal int在用分批 As Integer, ByVal dbl指导差价率 As Double, ByVal str批准文号 As String, _
    Optional ByVal lng数量 As Double = 0) As Boolean
    'lng数量：退库点击按钮“提取数据”会比一般手动选择卫材多提取一个数量的数据
    ...
    Dim dbl结算价 As Double, dbl采购价 As Double, dbl售价 As Double
    ...
    With mshBill
        ...
        If mint编辑状态 = 8 Or mbln退货 Then
            gstrSQL = "" & _
                " Select 上次批号 批号,效期,上次生产日期,上次采购价,商品条码,内部条码 From 药品库存" & _
            ...
            If rsprice.RecordCount <> 0 Then
                ...
                .TextMatrix(intRow, mcol商品条码) = IIf(IsNull(rsprice!商品条码), "", rsprice!商品条码)
                .TextMatrix(intRow, mcol内部条码) = IIf(IsNull(rsprice!内部条码), "", rsprice!内部条码)
...
'               .TextMatrix(intRow, mcol加成率) = Format(Val(.TextMatrix(intRow, mCol售价)) / Val(), "###0.00") & "%"
    ...
    If lng数量 > 0 Then '退货“提取数据”用到
        With mshBill
            .TextMatrix(intRow, mCol数量) = Format(lng数量, mFMT.FM_数量)
            
            dbl结算价 = Val(.TextMatrix(intRow, mCol结算价))
            dbl采购价 = Val(.TextMatrix(intRow, mCol采购价))
            dbl成本价 = IIf(mbln时价购前销售, dbl采购价, dbl结算价)
            dbl售价 = Val(.TextMatrix(intRow, mCol售价))
            If int是否变价 = 1 Then .TextMatrix(intRow, mcol加成率) = Format(Val(计算加成率(lng材料ID, dbl售价, dbl成本价)), "###0.00") & "%" '时价加成率
        
            '计算金额差价
            .TextMatrix(intRow, mCol结算金额) = Format(dbl结算价 * Val(.TextMatrix(intRow, mCol数量)), mFMT.FM_金额)
            .TextMatrix(intRow, mCol售价金额) = Format(dbl售价 * Val(.TextMatrix(intRow, mCol数量)), mFMT.FM_金额)
            .TextMatrix(intRow, mCol差价) = Format(Val(.TextMatrix(intRow, mCol售价金额)) - Val(.TextMatrix(intRow, mCol结算金额)), mFMT.FM_金额)
        End With
    End If
    ...
    With mshBill
        If .Col = mcol生产日期 And .TextMatrix(.Row, mcol生产日期) <> "" Then
            If ProduceDateCheck(.TextMatrix(.Row, mcol生产日期)) = False Then
                ...
                Exit Sub
            End If
        ElseIf .Col = mCol效期 And .TextMatrix(.Row, mCol效期) <> "" Then
            If mbln日期提示 = False Then CheckLapse (.TextMatrix(.Row, mCol效期))
        End If
        ...
        If .TextMatrix(1, 0) <> "" Then         '先判有否数据
            ...
            For intLop = 1 To .Rows - 1
                If Trim(.TextMatrix(intLop, mCol诊疗)) <> "" Then
                    '判断已失效卫材是否禁止入库
                    If CheckLapse(Trim(.TextMatrix(intLop, mCol效期)), True) = False Then
                        mbln日期提示 = True
                        MsgBox "第" & intLop & "行卫材已经失效了，失效卫材不能保存！", vbInformation, gstrSysName
                        mshBill.SetFocus
                        .Row = intLop
                    ...
                    '检查生产日期是否比当前日期大
                    If CheckpProduction(.TextMatrix(intLop, mcol生产日期)) = False Then
                        ShowMsgBox "第" & intLop & "行卫生材料的生产日期是比当前日期大，请检查！"
                        mshBill.SetFocus
                        .Row = intLop
                        .MsfObj.TopRow = intLop
                        .Col = mcol生产日期
...
'                    End If
                    ...
                    '检查同一个卫材录入的产地，批号，效期是否都相同
                    For i = 1 To .Rows - 1
                        If i <> intLop Then
                            If .TextMatrix(intLop, 0) = .TextMatrix(i, 0) And Trim(.TextMatrix(intLop, mCol产地)) = Trim(.TextMatrix(i, mCol产地)) _
                                And Trim(.TextMatrix(intLop, mCol批号)) = Trim(.TextMatrix(i, mCol批号)) _
                                And Format(.TextMatrix(intLop, mCol效期), "yyyy-MM-dd") = Format(.TextMatrix(i, mCol效期), "yyyy-MM-dd") Then
                                ShowMsgBox "第" & intLop & "行和第" & i & "行属于同一个卫材，且单据中的产地、批号、效期信息都相同，请检查！"
                                mshBill.SetFocus
                                .Row = intLop
                                .MsfObj.TopRow = intLop
                                .Col = mCol诊疗
                    ...
                    '检查是否有足够的库存可以退货
                    If mint编辑状态 = 8 Or mbln退货 Then
                        If Not CheckStock(Val(.TextMatrix(intLop, 0)), Val(.TextMatrix(intLop, mCol批次)), Val(.TextMatrix(intLop, mCol数量)) * Val(.TextMatrix(intLop, mCol比例系数)), intLop) Then
                            mshBill.SetFocus
                            .Row = intLop
                            .MsfObj.TopRow = intLop
                            .Col = mCol数量
...
'        For intRow = 1 To .Rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                str产地 = .TextMatrix(intRow, mCol产地)
                str批号 = .TextMatrix(intRow, mCol批号)
                lng批次 = .TextMatrix(intRow, mCol批次)
                str批准文号 = .TextMatrix(intRow, mcol批准文号)
                ...
                dbl零售价 = GetFormat(Val(.TextMatrix(intRow, mcol零售价)), g_小数位数.obj_最大小数.零售价小数)
                If Val(Split(.TextMatrix(intRow, mCol原销期), "||")(2)) = 0 Then
                    '如果是定价药品，则售价取原始价格保存
                    dbl零售价 = Get售价(False, lng材料ID, lngStockID, 0)
                ...
                If mbln退货 = True Then '修改退货单
                    '成本价取上次采购价没有再调用过程取
                    gstrSQL = "" & _
                        " Select 上次批号 批号,效期,上次生产日期,上次采购价 From 药品库存" & _
                        " Where 库房ID=[1] And 药品ID=[2]" & _
                        "       And 性质=1 And nvl(批次,0)=[3]"
                    Set rsprice = zlDatabase.OpenSQLRecord(gstrSQL, mstrCaption, lngStockID, lng材料ID, lng批次)
                    If rsprice.RecordCount <> 0 Then
                        dbl成本价 = zlStr.NVL(rsprice!上次采购价, 0)
                    Else
                        dbl成本价 = Get成本价(lng材料ID, lngStockID, lng批次)
                    End If
                
                    dbl成本金额 = Val(Format(Val(Format(dbl成本价 * Val(.TextMatrix(intRow, mCol比例系数)), mFMT.FM_成本价)) * Val(.TextMatrix(intRow, mCol数量)), mFMT.FM_金额))
                    dbl零售价 = Get售价(Split(.TextMatrix(intRow, mCol原销期), "||")(2) = 1, lng材料ID, lngStockID, lng批次)
                    dbl零售金额 = Val(Format(Val(Format(dbl零售价 * Val(.TextMatrix(intRow, mCol比例系数)), mFMT.FM_零售价)) * Val(.TextMatrix(intRow, mCol数量)), mFMT.FM_金额))
                    dbl差价 = Val(Format(dbl零售金额 - dbl成本金额, mFMT.FM_金额))
                    
                    str零售差价 = GetFormat(0, g_小数位数.obj_最大小数.零售价小数) '退货新增保存（SaveRestore）没有处理该字段
                End If
        ...
        For intRow = 1 To .Rows - 1
            If Val(.TextMatrix(intRow, mCol冲销数量)) <> 0 Then
                ...
                If int库存检查 <> 0 And mint编辑状态 <> 7 And mbln退货 = False Then
                    ...
                    If Check可用数量(lng库房id, Val(.TextMatrix(intRow, 0)), lng批次, dbl冲销数量, int库存检查, 0) = False Then .Row = intRow: .MsfObj.TopRow = intRow: Exit Function
                    If Check可用数量(lng库房id, Val(.TextMatrix(intRow, 0)), lng批次, dbl冲销数量, int库存检查, 1) = False Then .Row = intRow: .MsfObj.TopRow = intRow: Exit Function
                End If
            End If
        Next
...
Private Function CheckStock(ByVal lng材料ID As Long, ByVal lng批次 As Long, ByVal dbl数量 As Double, ByVal intRow As Integer) As Boolean
    '---------------------------------------------------------------------------------------------------------
    '功能:检查库存的可用数量是否充足
    '---------------------------------------------------------------------------------------------------------
    Dim lng库房id As Long, intLop As Integer
    ...
    With mshBill
        ...
        If mint编辑状态 <> 8 Then
            For Each varStuff In mCllBillData
                If varStuff(0) = Val(.TextMatrix(intRow, 0)) & "_" & Val(.TextMatrix(intRow, mCol批次)) Then
                    dblSum = varStuff(1)
                    Exit For
                End If
            Next
    ...
    If dbltotal > dblSum Then
        Select Case mint库存检查
            Case 1  '仅提示
                If MsgBox("第" & intRow & "行，退货数量大于现有的可用库存（当前可用库存为：" & dblSum / Val(mshBill.TextMatrix(intRow, mCol比例系数)) & "），是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Case 2
                ShowMsgBox "第" & intRow & "行，退货数量大于现有的可用库存（当前可用库存为：" & dblSum / Val(mshBill.TextMatrix(intRow, mCol比例系数)) & "），请检查！"
                Exit Function
        End Select
    ...
    For lngRow = 1 To lngRows
        ...
        If lng材料ID <> 0 Then
            ...
            If rsTemp.EOF Then
                blnExit = True
            Else
                blnExit = (Val(Format(rsTemp!数量, mFMT.FM_数量)) < Abs(Val(mshBill.TextMatrix(lngRow, mCol数量))))
            End If
            
            If blnExit Then
                Select Case mint库存检查
                    Case 1  '仅提示
                        If MsgBox("第" & lngRow & "行的卫材库存数不够，是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                            Exit Function
                        End If
                    Case 2
