...
Private Sub ExtractStockData()
    ...
    gstrSQL = "Select b.Id, '[' || b.编码 || ']' || b.名称 名称, b.规格, a.上次产地 as 产地 ,a.批准文号, " & IIf(mintUnit = 0, "b.计算单位", "c.包装单位") & " 单位," & vbNewLine & _
              ...
              "       a.可用数量 / " & IIf(mintUnit = 0, "1", "c.换算系数") & " 数量," & vbNewLine & _
              ...
              "Where a.药品id = b.Id And a.药品id = c.材料id And a.药品id = d.收费细目id And a.性质 = 1 And a.库房id = [1] And a.可用数量 > 0 And" & vbNewLine & _
    ...
    With frmPurchaseCard
        ...
        For i = 1 To rsVal.RecordCount
            ...
            .SetColValue i, rsVal!Id, rsVal!名称, IIf(IsNull(rsVal!规格), "", rsVal!规格), IIf(IsNull(rsVal!产地), "", rsVal!产地) _
                ...
                , IIf(IsNull(rsVal!在用分批), 0, rsVal!在用分批), rsVal!指导差价率, IIf(IsNull(rsVal!批准文号), 0, rsVal!批准文号), rsVal!数量
