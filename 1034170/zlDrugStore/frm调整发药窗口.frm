...
Private Const MINTCOLS = 8
Private Const mIntCol类别 = 0
Private Const mIntCol类型 = 1
Private Const mIntColNO = 2
Private Const mIntCol姓名 = 3
Private Const mIntCol现窗口 = 4
Private Const mIntCol新窗口 = 5
Private Const mIntCol单据 = 6
Private Const mIntCol就诊卡号 = 7
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    Select Case Control.Id
        ...
        Case mconMenu_File_Exit      '执行退出操作
            Unload Me
        Case mconMenu_Edit_Recipe_Guide  '执行批量设置
            Edit_Recipe_Guide
            ...
            If UBound(Split(strConWin, ",")) <> -1 Or UBound(Split(str处方, ",")) <> -1 Then
                For k = 1 To Me.VSFWindows.rows - 1
                    ...
                    ElseIf UBound(Split(str处方, ",")) = -1 Then
                        ...
                        End If
                    Else
                        If Me.VSFWindows.TextMatrix(k, VSFWindows.ColIndex("现窗口")) = Split(strConWin, ",")(i + 1) And Me.VSFWindows.TextMatrix(k, VSFWindows.ColIndex("类型")) = Split(str处方, ",")(j + 1) Then
                             Me.VSFWindows.TextMatrix(k, VSFWindows.ColIndex("新窗口")) = str新窗口
                        End If
                    End If
                Next
...
Private Sub InitGridData()
    ...
    With Me.VSFWindows
        ...
        For i = 1 To mrsList.RecordCount
            ...
            .TextMatrix(i, .ColIndex("现窗口")) = NVL(mrsList!发药窗口)
            .TextMatrix(i, .ColIndex("单据")) = mrsList!单据
            .TextMatrix(i, .ColIndex("就诊卡号")) = NVL(mrsList!就诊卡号)
            .TextMatrix(i, .ColIndex("病人ID")) = NVL(mrsList!病人ID)
