...
'        .FindType = intFindType
    ...
    With vsfList
        Select Case strFindType
            Case "姓名"
                If Mid(mFindProcess.FindContent, 1, 1) = "-" And Val(Mid(mFindProcess.FindContent, 2)) <> 0 Then        '判断当前查找是否按“-病人ID”方式查找
                    '通过“-病人ID”方式查找
                    intCol = mIntCol病人ID                                                  '重新定义查找列
                    mFindProcess.FindContent = Val(Mid(mFindProcess.FindContent, 2))        '重写查找内容
                Else
                    '正常按姓名查找
                    intCol = mIntCol姓名
