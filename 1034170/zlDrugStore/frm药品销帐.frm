...
Begin VB.Form Frm药品销账 
   ...
   Begin TabDlg.SSTab sstabList 
      ...
      Tab(1).Control(0)=   "vsfList(1)"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).Control(1)=   "picBatHsc(1)"
      Tab(1).Control(1).Enabled=   0   'False
      Tab(1).Control(2)=   "picHsc(1)"
      Tab(1).Control(2).Enabled=   0   'False
      Tab(1).Control(3)=   "vsfMain(1)"
      Tab(1).Control(3).Enabled=   0   'False
      Tab(1).Control(4)=   "vsfDetail(1)"
      Tab(1).Control(4).Enabled=   0   'False
      Tab(1).Control(5)=   "vsfBatch(1)"
      Tab(1).Control(5).Enabled=   0   'False
      Tab(1).Control(6)=   "lbl提示(1)"
...
Private Sub GetRecord(ByVal int审核 As Integer)
    ...
    Dim lngLast收费细目ID As Long
    Dim dblLast销帐金额 As Double
    Dim dblLast销帐数量 As Double
...
'        MsgBox "没有找到满足条件的退药申请记录！", vbInformation, gstrSysName
    ...
    If Not rstemp.EOF Then
        For i = 1 To rstemp.RecordCount
            With vsfMain(int审核)
                If i >= 2 And rstemp!收费细目id = lngLast收费细目ID Then
                    If int审核 = 0 Then
                        mdblSum = mdblSum + FormatEx(rstemp!销帐数量 * rstemp!零售价, 2)
                        dblLast销帐金额 = dblLast销帐金额 + FormatEx(rstemp!销帐数量 * rstemp!零售价, 2)
                        .TextMatrix(.rows - 1, .ColIndex("销账金额")) = dblLast销帐金额
                    End If
                    dblLast销帐数量 = dblLast销帐数量 + rstemp!销帐数量
                    .TextMatrix(.rows - 1, .ColIndex("销帐数量")) = FormatEx(dblLast销帐数量 / rstemp!包装, 5)
                Else
                     .rows = .rows + 1
                     ...
                     If int审核 = 0 Then
                         ...
                         mdblSum = mdblSum + FormatEx(rstemp!销帐数量 * rstemp!零售价, 2)
                         dblLast销帐金额 = FormatEx(rstemp!销帐数量 * rstemp!零售价, 2)
                     End If
                     .TextMatrix(.rows - 1, .ColIndex("单位")) = rstemp!单位
                     .TextMatrix(.rows - 1, .ColIndex("来源")) = rstemp!药品来源
                     
                     lngLast收费细目ID = rstemp!收费细目id
                     dblLast销帐数量 = rstemp!销帐数量
                 End If
    ...
    If int审核 = 0 Then
        If mstrReceiveMsg <> "" Then
            gstrSQL = "Select /*+ Rule*/ 申请科室, 单据, NO, 序号, 药品编码, 通用名, 商品名,规格, 药品ID, 费用id, 申请时间, 标识号, 姓名, 病人id,床号, 单位, 包装, Sum(数量) As 销帐数量,零售价,当前床号 " & _
                " From (Select Distinct E.名称 As 申请科室, C.单据, C.NO,B.标准单价 零售价,b.序号, '[' || x.编码 || ']' As 药品编码, x.名称 As 通用名, w.名称 As 商品名,X.规格, C.药品ID, A.费用id, A.申请时间, B.标识号, nvl(F.姓名,b.姓名) 姓名, B.病人id,B.床号,G.当前床号, " & strSubUnit & " " & _
                ...
                " And a.申请时间 = To_Date(t.C1, 'yyyy-mm-dd hh24:mi:ss') And b.病人id = t.C2 "
        Else
            gstrSQL = "Select 申请科室, 单据, NO, 序号, 药品编码, 通用名, 商品名,规格, 药品ID, 费用id, 申请时间, 标识号, 姓名, 病人id,床号, 单位, 包装, Sum(数量) As 销帐数量,零售价,当前床号 " & _
                " From (Select Distinct E.名称 As 申请科室, C.单据, C.NO,B.标准单价 零售价,b.序号, '[' || x.编码 || ']' As 药品编码, x.名称 As 通用名, w.名称 As 商品名,X.规格, C.药品ID, A.费用id, A.申请时间, B.标识号, nvl(F.姓名,b.姓名) 姓名, B.病人id,B.床号,G.当前床号, " & strSubUnit & " " & _
        ...
        gstrSQL = gstrSQL & " And A.申请部门id = E.ID And B.执行部门id = [1]  " & _
            ...
            " Group By 申请科室, 单据, NO, 序号, 药品编码, 通用名, 商品名,规格, 药品ID, 费用id, 申请时间, 标识号, 姓名, 病人id, 床号,当前床号, 单位, 包装,零售价 " & _
...
Public Function ShowForm(FrmMain As Form, ByVal lng库房ID As Long, ByVal strUnit As String, ByVal int金额保留位数 As Integer, _
    ByVal strCards As String, ByVal int打印退药清单 As Integer, ByVal strWriteOffMsg As String, ByRef objSquareCard As Object, ByVal objPlugIn As Object) As String
