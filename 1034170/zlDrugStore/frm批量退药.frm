...
Begin VB.Form frm批量退药 
   Caption         =   "退其它药房的处方"
   ClientHeight    =   5160
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7725
   Icon            =   "frm批量退药.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   ScaleHeight     =   5160
...
Public Function ShowEditor(ByVal frmParent As Object, ByVal lng药房ID As Long, _
    Optional ByVal bln处方发药 As Boolean = True, _
    Optional ByVal int金额保留位数 As Integer = 2, Optional ByVal strPriv As String) As Boolean
    mblnRefresh = False
    mlng药房ID = lng药房ID
    mbln处方发药 = bln处方发药
...
Private Sub cmdOK_Click()
    ...
    Do While Not mrs退药.EOF
        ...
        If dbl退药数 <> 0 Then
            If CheckBill(2, Val(Bill.TextMatrix(mrs退药!行号, 列名.Id)), Val(Bill.TextMatrix(mrs退药!行号, 列名.记录性质)), Val(Bill.TextMatrix(mrs退药!行号, 列名.门诊标志))) <> 0 Then
                Exit Sub
            End If
            ...
            With RecRecord
                dbl退药数 = zlStr.NVL(!数量, 0)
            End With
            If Val(Bill.TextMatrix(mrs退药!行号, 列名.退药数)) = Val(Bill.TextMatrix(mrs退药!行号, 列名.准退数)) Then
                dbl退药数 = Val(Bill.TextMatrix(mrs退药!行号, 列名.实际数量))
            End If
...
Private Sub TxtNo_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    '按处方号读取对应的已发药记录
    gstrSQL = "" & _
            " SELECT DISTINCT S.ID,S.单据,S.药品ID,S.NO,S.序号,S.扣率,F.名称 AS 库房,P.名称 科室,C.记录性质,C.门诊标志,'' 床号,C.姓名,'['||X.编码||']' As 药品编码,X.名称 As 通用名,A.名称 As 商品名," & _
            ...
            "         FROM 药品收发记录 A" & _
            "         WHERE A.审核人 IS NOT NULL And A.库房ID+0<>[2] " & _
            "         AND NO =[1] And 单据 In (8,9)" & _
            ...
            "     门诊费用记录 C,部门表 P,药品规格 D,收费项目目录 X,收费项目别名 A,部门表 F"
    gstrSQL = gstrSQL & _
            " WHERE S.药品ID=D.药品ID AND D.药品ID=X.ID AND S.对方部门ID+0=P.ID AND S.费用ID=C.ID And S.库房ID+0=F.ID" & _
            " AND D.药品ID=A.收费细目ID(+) AND A.性质(+)=3 " & _
            " AND (S.记录状态=1 OR MOD(S.记录状态,3)=0) AND S.审核人 IS NOT NULL AND S.库房ID+0<>[2] " & _
            " AND S.实际数量*S.付数>S.已退数量 "
            
    strSQL = Replace(gstrSQL, "门诊费用记录", "住院费用记录")
    strSQL = Replace(strSQL, "'' 床号", "C.床号")
    gstrSQL = gstrSQL & " Union All " & strSQL
    gstrSQL = gstrSQL & " ORDER BY 单据,NO"
    
    Set rsBill = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption & "[读取该处方号对应的已发药记录]", strInput, mlng药房ID)
            
    If rsBill.RecordCount = 0 Then
        MsgBox "该处方号对应的处方还未发药或已全部退药或已转出到后备数据库！", vbInformation, gstrSysName
        Call zlControl.TxtSelAll(TxtNo)
        Exit Sub
    End If
    ...
    With rsBill
        Do While Not .EOF
            ...
            If InStr(1, strID, "," & !Id & ",") = 0 Then
                Bill.TextMatrix(lngRow, 列名.库房) = !库房
                ...
                If gint药品名称显示 = 0 Or gint药品名称显示 = 2 Then
                    Bill.TextMatrix(lngRow, 列名.药品名称) = !药品编码 & !通用名
                Else
                    Bill.TextMatrix(lngRow, 列名.药品名称) = !药品编码 & IIf(IsNull(!商品名), !通用名, !商品名)
                End If
                ...
                Bill.TextMatrix(lngRow, 列名.实际数量) = !准退数
                Bill.TextMatrix(lngRow, 列名.金额) = !金额
...
Private Function CheckBill(ByVal IntOper As Integer, ByVal LngID As Long, ByVal int记录性质 As Integer, ByVal int门诊标志 As Integer) As Integer
...
Dim intCol As Integer
    ...
    With Bill
        ...
        .ColWidth(列名.商品名) = IIf(gint药品名称显示 = 2, 2000, 0)
        .ColWidth(列名.规格) = 1500
        .ColWidth(列名.批号) = 1500
        .ColWidth(列名.效期) = 0
        .ColWidth(列名.产地) = 0
