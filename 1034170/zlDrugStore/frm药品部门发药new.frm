...
Private Function DrugStoreWork_StayProc(ByVal StrCurDate As String) As Boolean
    ...
    With rsData
        ...
        Do While Not .EOF
            ...
            '登记时间
            gstrSQL = gstrSQL & ",To_Date('" & !留存登记时间 & "','yyyy-MM-dd hh24:mi:ss')"
            gstrSQL = gstrSQL & ")"
...
Private Sub InitSendRec()
    ...
    With mrsSendData
        ...
        .Fields.Append "服药时间", adLongVarChar, 1000, adFldIsNullable
        ...
        .Fields.Append "留存登记时间", adLongVarChar, 20, adFldIsNullable
...
Private Function LoadSendRecord(ByVal rsData As ADODB.Recordset) As Boolean
    ...
    With rsData
        Do While Not .EOF
            ...
            '修正实际用药次数的误差
            If IsNull(!开始执行时间) Or IsNull(!首次时间) Or IsNull(!末次时间) Or IsNull(!频率间隔) Or IsNull(!间隔单位) Or IsNull(!执行时间方案) Or (Not mParams.bln加载服药时间和用药次数) Then
                mrsSendData!服药时间 = ""
                mrsSendData!用药次数 = ""
            Else
                mrsSendData!服药时间 = Calc段内分解时间(CDate(!首次时间), CDate(!末次时间), "", !执行时间方案, !频率次数, !频率间隔, !间隔单位, CDate(!开始执行时间))
                mrsSendData!用药次数 = UBound(Split(mrsSendData!服药时间, ",")) + 1
                
                '对服药时间重新格式化
                mrsSendData!服药时间 = Format服药时间(mrsSendData!服药时间, !间隔单位, !医嘱id, mrsSendData!填制日期, !执行时间方案)
            End If
...
'            mrsSendData!金额 = Format(!金额, "#####0.00;-#####0.00; ;")
            ...
            mrsSendData!留存登记时间 = IIf(IsNull(!留存登记时间), "", Format(!留存登记时间, "yyyy-MM-dd HH:mm:ss"))
...
Private Function Format服药时间(ByVal str服药时间 As String, ByVal str间隔单位 As String, ByVal lng医嘱id As Long, ByVal str填制日期 As String, ByVal str执行时间方案 As String) As String
    '【功能】:对服药时间按单位进行格式化
    Dim str单个时间 As String
    Dim str分解时间 As String
    Dim str首日时间 As String
    Dim str首日执行时间 As String
    Dim str平日执行时间 As String
    Dim strTemp As String   '用于冒泡排序时的缓存
    Dim arrTemp As Variant
    Dim i As Integer
    Dim j As Integer
    
    If str间隔单位 = "天" Then
        If Is首次执行(lng医嘱id, str填制日期) And InStr(str执行时间方案, ",") > 0 Then
            '首日执行
            '------------------------------------
            str首日时间 = Mid(Split(str服药时间, ",")(0), 1, 10)    '【年-月-日】
            
            For i = 0 To UBound(Split(str服药时间, ","))
                str单个时间 = Split(str服药时间, ",")(i)
                str分解时间 = Mid(str单个时间, 12, 5)   '【时:分】
                
                If InStr(str单个时间, str首日时间) > 0 Then
                    '首日执行时间点
                    If Not InStr(str首日执行时间, str分解时间) > 0 Then
                        str首日执行时间 = str首日执行时间 & IIf(str首日执行时间 = "", "", ",") & str分解时间
                    End If
                Else
                    '正常执行时间点
                    If Not InStr(str平日执行时间, str分解时间) > 0 Then
                        str平日执行时间 = str平日执行时间 & IIf(str平日执行时间 = "", "", ",") & str分解时间
                    End If
                End If
            Next
        Else
        ...
        For i = 0 To UBound(arrTemp)
            str平日执行时间 = str平日执行时间 & IIf(str平日执行时间 = "", "", ",") & arrTemp(i)
        Next
        '------------------------------------
        
        Format服药时间 = IIf(str首日执行时间 = "", "", "首日执行【" & str首日执行时间 & "】") & IIf(str首日执行时间 <> "" And str平日执行时间 <> "", "，", "") & IIf(str平日执行时间 = "", "", "执行时间【" & str平日执行时间 & "】")
    Else
        For i = 0 To UBound(Split(str服药时间, ","))
            str单个时间 = Split(str服药时间, ",")(i)
            str分解时间 = Mid(str单个时间, 6, 11)   '【月-日 时:分】
            
            Format服药时间 = Format服药时间 & IIf(Format服药时间 = "", "", ",") & str分解时间
        Next
    End If
End Function
...
Private Function Get服药时间(ByVal str开始执行时间 As String, ByVal str首次时间 As String, ByVal str末次时间 As String, _
                             ByVal int频率间隔 As Integer, ByVal str间隔单位 As String, ByVal str执行时间方案 As String, _
                             ByVal lng医嘱id As Long, ByVal int医嘱效期 As Integer) As String
    '【功能】:获取具体服药时间
    Dim rstemp As ADODB.Recordset
    Dim strSQL As String
    ...
    Set rstemp = zlDatabase.OpenSQLRecord(strSQL, "取实际服药次数", _
            lng医嘱id, _
            CDate(str首次时间), _
            CDate(str末次时间), _
            str执行时间方案, _
    ...
    If Not rstemp.EOF Then
        Get服药时间 = IIf(IsNull(rstemp.Fields(0).Value), "", rstemp.Fields(0).Value)
    End If
...
Private Sub RefreshSendDetail()
    ...
    '''select
    gstrSQL = "SELECT /*+rule*/ Distinct A.*, Nvl(C.留存数量,0) As 留存数量, c.留存登记时间 " & IIf(mcondition.bln显示领药退药人 = True, ", B.退药人", ",'' As 退药人") & " FROM ("
    ...
    strSqlTmp = "SELECT DISTINCT S.ID As 收发ID,to_char(s.效期,'yyyy-mm-dd') 效期,S.记录状态,S.药品ID,S.费用id,NVL(N.已收费,0) 已收费,P.名称 科室,S.配药人,C.开单人 开单医生,C.操作员姓名 审核人,S.单据,S.扣率, " & _
             ...
             "X.编码" & " As 药品编码," & IIf(mParams.int药品名称显示 = 1, "NVL(E.名称,X.名称)", "X.名称") & " As 药品名称,s.入出类别id,s.入出系数,s.填制人,s.填制日期,s.配药日期,Nvl(t.是否皮试,0) As 是否皮试,F.执行分类,F.类别,D.剂量系数, m.医生嘱托, m.间隔单位, m.执行时间方案, m.频率次数, m.开始执行时间, j.首次时间, j.末次时间, m.频率间隔, m.上次打印时间, m.医嘱期效 "
'    '测试分组（相关ID设为1）
...
'             "X.编码" & " As 药品编码," & IIf(mParams.int药品名称显示 = 1, "NVL(E.名称,X.名称)", "X.名称") & " As 药品名称,s.入出类别id,s.入出系数,s.填制人,s.填制日期,s.配药日期"
    ...
    '''from
    strSqlTmp = strSqlTmp & " FROM 药品收发记录 S,住院费用记录 C,病人医嘱记录 M,病人医嘱记录 G,未发药品记录 N,收费项目别名 E,收费项目目录 X,诊疗项目目录 I,诊疗项目目录 N,诊疗项目目录 F,病人医嘱发送 J," & _
    ...
    strSqlTmp = strSqlTmp & " WHERE S.NO=N.NO AND S.单据=N.单据 AND NVL(S.库房ID,[1])+0=NVL(N.库房ID,[1]) AND S.费用ID=C.ID And S.药品ID=D.药品ID And c.病人id = u.病人id(+) And c.婴儿费 = u.序号(+) And C.主页id=U.主页id(+) " & _
            " And C.病人id = R.病人id And C.病人id=Q.病人id And C.主页id=Q.主页id And Q.病人类型=W.名称(+) And j.医嘱id(+) = c.医嘱序号 And j.No(+) = c.No And j.记录性质(+) = c.记录性质 " & _
    ...
    gstrSQL = gstrSQL & ", (Select 药品id,库房id,部门id,Nvl(实际数量, 留存数量) As 留存数量, 登记时间 As 留存登记时间 From 药品留存计划  Where 状态 <> 1) C "
...
Private Sub GetParams()
    ...
    With mParams
        ...
        .bln加载服药时间和用药次数 = (Val(zlDatabase.GetPara("加载服药时间和用药次数", glngSys, 1342, 0)) = 1)
...
Private Function Check销账申请(ByRef rsData As ADODB.Recordset) As Boolean
    '功能:检查是否有销账申请的记录，有则剔除
    'return:False-不需要进行提示;True-需要在完成退药操作后进行提示
    Dim str收发ID As String
    Dim rstemp As ADODB.Recordset
    ...
    With rsData
        ...
        .Sort = "收发ID"
        Do While Not .EOF
            str收发ID = str收发ID & IIf(str收发ID = "", "", ",") & !收发Id
    ...
    gstrSQL = "Select " & vbNewLine & _
            " a.Id As 收发id" & vbNewLine & _
            "From 药品收发记录 A, 病人费用销帐 B, Table(Cast(f_Num2list([1]) As Zltools.t_Numlist)) C" & vbNewLine & _
            "Where a.费用id = b.费用id And a.Id = c.Column_Value And b.状态 + 0 = 0"
    
    Set rstemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, str收发ID)
    
    
    '剔除存在销帐申请记录的收发ID
    If Not rstemp.EOF Then
        Do While Not rstemp.EOF
            rsData.Filter = "收发ID=" & rstemp!收发Id
            rsData!执行状态 = mState.不处理
...
Private Sub DrugStoreWork_Return()
    ...
    '启用【有销帐申请时不能进行退药操作】参数,重新检查是否含有检查销账申请的记录，防止未及时刷新清单后，存在销帐申请的药品被退药
    If mParams.bln有销帐申请时不能进行退药操作 Then
        If Check销账申请(rsReturnData) Then
            bln销帐申请提示 = True
        End If
    End If
    ...
    Else
        If bln销帐申请提示 Then
            MsgBox "因存在销帐申请的记录，本次退药失败。" & vbCrLf & "请重新刷新清单后并核对！"
        Else
            MsgBox "本次没有退药。"
        End If
    ...
    '消息提示-销帐申请
    If bln销帐申请提示 Then
        MsgBox "因存在销帐申请的记录，部分药品未成功退药。" & vbCrLf & "请重新刷新清单后并核对！"
    End If
