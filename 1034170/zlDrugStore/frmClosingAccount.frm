...
Begin VB.Form frmClosingAccount 
   ...
   Begin XtremeCommandBars.ImageManager imgPublic 
      Bindings        =   "frmClosingAccount.frx":1E24
      Left            =   1200
      Top             =   120
      _Version        =   589884
      _ExtentX        =   635
      _ExtentY        =   635
      _StockProps     =   0
      Icons           =   "frmClosingAccount.frx":1E38
   End
...
Private Const mconMenu_Edit_CA_DeleteAccount = 3305             '删除结存误差
...
Private Sub GetAccountRecord()
    ...
    gstrSQL = "Select A.ID, Nvl(A.库房id, 0) As 库房id, B.名称 As 库房名称, A.期初日期, A.期末日期, A.填制人" & _
              "   , A.填制日期, 审核人, 审核日期, Nvl(A.上次结存id, 0) As 上次结存id,a.期间,a.性质 " & _
...
Private Sub GetSelect(ByVal strInput As String)
    ...
    vRect = GetControlRect(cbo库房.hWnd)
        ...
        Do While Not mrsDetail.EOF
            ...
            If lng药品id = 0 Then
                If mrsDetail!材质分类 = "西成药" Then
                    bln是否有西药 = True
                    If mrsDetail!数量 <> 0 Then .TextMatrix(intRow, .ColIndex("西药数量")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("西药数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                    If mrsDetail!金额 <> 0 Then .TextMatrix(intRow, .ColIndex("西药金额")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("西药金额"))) + mrsDetail!金额, intShowMoneyDigit)
                    If mrsDetail!差价 <> 0 Then .TextMatrix(intRow, .ColIndex("西药差价")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("西药差价"))) + mrsDetail!差价, intShowMoneyDigit)
                    .TextMatrix(intRow, .ColIndex("西药成本")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("西药金额"))) - Val(.TextMatrix(intRow, .ColIndex("西药差价"))), intShowMoneyDigit)
                ElseIf mrsDetail!材质分类 = "中成药" Then
                    bln是否有成药 = True
                    If mrsDetail!数量 <> 0 Then .TextMatrix(intRow, .ColIndex("成药数量")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("成药数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                    If mrsDetail!金额 <> 0 Then .TextMatrix(intRow, .ColIndex("成药金额")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("成药金额"))) + mrsDetail!金额, intShowMoneyDigit)
                    If mrsDetail!差价 <> 0 Then .TextMatrix(intRow, .ColIndex("成药差价")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("成药差价"))) + mrsDetail!差价, intShowMoneyDigit)
                    .TextMatrix(intRow, .ColIndex("成药成本")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("成药金额"))) - Val(.TextMatrix(intRow, .ColIndex("成药差价"))), intShowMoneyDigit)
                ElseIf mrsDetail!材质分类 = "中草药" Then
                    bln是否有草药 = True
                    If mrsDetail!数量 <> 0 Then .TextMatrix(intRow, .ColIndex("草药数量")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("草药数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                    If mrsDetail!金额 <> 0 Then .TextMatrix(intRow, .ColIndex("草药金额")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("草药金额"))) + mrsDetail!金额, intShowMoneyDigit)
                    If mrsDetail!差价 <> 0 Then .TextMatrix(intRow, .ColIndex("草药差价")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("草药差价"))) + mrsDetail!差价, intShowMoneyDigit)
                    .TextMatrix(intRow, .ColIndex("草药成本")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("草药金额"))) - Val(.TextMatrix(intRow, .ColIndex("草药差价"))), intShowMoneyDigit)
                End If
                
                .TextMatrix(intRow, .ColIndex("合计数量")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("合计数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                .TextMatrix(intRow, .ColIndex("合计金额")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("合计金额"))) + mrsDetail!金额, intShowMoneyDigit)
                .TextMatrix(intRow, .ColIndex("合计差价")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("合计差价"))) + mrsDetail!差价, intShowMoneyDigit)
                .TextMatrix(intRow, .ColIndex("合计成本")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("合计金额"))) - Val(.TextMatrix(intRow, .ColIndex("合计差价"))), intShowMoneyDigit)
            Else
                If mrsDetail!数量 <> 0 Then .TextMatrix(intRow, .ColIndex("数量")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                If mrsDetail!金额 <> 0 Then .TextMatrix(intRow, .ColIndex("售价金额")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("售价金额"))) + mrsDetail!金额, intShowMoneyDigit)
                If mrsDetail!差价 <> 0 Then .TextMatrix(intRow, .ColIndex("差价")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("差价"))) + mrsDetail!差价, intShowMoneyDigit)
                .TextMatrix(intRow, .ColIndex("成本金额")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("售价金额"))) - Val(.TextMatrix(intRow, .ColIndex("差价"))), intShowMoneyDigit)
            End If
        ...
        Do While Not mrsDetail.EOF
            ...
            Else
                ...
                If mrsDetail!业务大类 = "1-期初" Then
                    .TextMatrix(lngRow, .ColIndex("期初数量")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期初数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                    .TextMatrix(lngRow, .ColIndex("期初金额")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期初金额"))) + mrsDetail!金额, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("期初差价")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期初差价"))) + mrsDetail!差价, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("期初成本")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期初金额"))) - Val(.TextMatrix(lngRow, .ColIndex("期初差价"))), intShowMoneyDigit)
                ElseIf mrsDetail!业务大类 = "2-入库" Then
                    .TextMatrix(lngRow, .ColIndex("入库数量")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("入库数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                    .TextMatrix(lngRow, .ColIndex("入库金额")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("入库金额"))) + mrsDetail!金额, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("入库差价")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("入库差价"))) + mrsDetail!差价, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("入库成本")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("入库金额"))) - Val(.TextMatrix(lngRow, .ColIndex("入库差价"))), intShowMoneyDigit)
                ElseIf mrsDetail!业务大类 = "3-出库" Then
                    .TextMatrix(lngRow, .ColIndex("出库数量")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("出库数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                    .TextMatrix(lngRow, .ColIndex("出库金额")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("出库金额"))) + mrsDetail!金额, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("出库差价")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("出库差价"))) + mrsDetail!差价, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("出库成本")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("出库金额"))) - Val(.TextMatrix(lngRow, .ColIndex("出库差价"))), intShowMoneyDigit)
                ElseIf mrsDetail!业务大类 = "4-期末" Then
                    .TextMatrix(lngRow, .ColIndex("期末数量")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期末数量"))) + mrsDetail!数量 / dbl包装, intShowNumberDigit)
                    .TextMatrix(lngRow, .ColIndex("期末金额")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期末金额"))) + mrsDetail!金额, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("期末差价")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期末差价"))) + mrsDetail!差价, intShowMoneyDigit)
                    .TextMatrix(lngRow, .ColIndex("期末成本")) = GetFormat(Val(.TextMatrix(lngRow, .ColIndex("期末金额"))) - Val(.TextMatrix(lngRow, .ColIndex("期末差价"))), intShowMoneyDigit)
                End If
            End If
    ...
    If lng库房ID > 0 Then
        With vsfMistake(1)
            ...
            Do While Not mrsMistake.EOF
                ...
                .TextMatrix(intRow, .ColIndex("金额差")) = GetFormat(mrsMistake!金额差, intShowMoneyDigit)
                .TextMatrix(intRow, .ColIndex("差价差")) = GetFormat(mrsMistake!差价差, intShowMoneyDigit)
                
                Select Case intUnit
                Case 2  '"门诊单位"
                    .TextMatrix(intRow, .ColIndex("单位")) = mrsMistake!门诊单位
                    .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(mrsMistake!数量差 / mrsMistake!门诊包装, intShowNumberDigit)
                Case 3  '"住院单位"
                    .TextMatrix(intRow, .ColIndex("单位")) = mrsMistake!住院单位
                    .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(mrsMistake!数量差 / mrsMistake!住院包装, intShowNumberDigit)
                Case 4  '"药库单位"
                    .TextMatrix(intRow, .ColIndex("单位")) = mrsMistake!药库单位
                    .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(mrsMistake!数量差 / mrsMistake!药库包装, intShowNumberDigit)
                Case Else
                    .TextMatrix(intRow, .ColIndex("单位")) = mrsMistake!计算单位
                    .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(mrsMistake!数量差, intShowNumberDigit)
                End Select
    ...
    Else
        With vsfMistake(0)
            ...
            Do While Not mrsMistake.EOF
                ...
                Select Case intUnit
                Case 2  '"门诊单位"
                    .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("数量差"))) + mrsMistake!数量差 / mrsMistake!门诊包装, intShowNumberDigit)
                Case 3  '"住院单位"
                    .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("数量差"))) + mrsMistake!数量差 / mrsMistake!住院包装, intShowNumberDigit)
                Case 4  '"药库单位"
                    .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("数量差"))) + mrsMistake!数量差 / mrsMistake!药库包装, intShowNumberDigit)
                Case Else
                  .TextMatrix(intRow, .ColIndex("数量差")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("数量差"))) + mrsMistake!数量差, intShowNumberDigit)
                End Select
                
                .TextMatrix(intRow, .ColIndex("金额差")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("金额差"))) + mrsMistake!金额差, intShowMoneyDigit)
                .TextMatrix(intRow, .ColIndex("差价差")) = GetFormat(Val(.TextMatrix(intRow, .ColIndex("差价差"))) + mrsMistake!差价差, intShowMoneyDigit)
...
errHandle:
    ...
    Call SaveErrLog
End Sub
Private Sub MediAccountProcess_AddIniAccount()
    '新增初初始化
    Dim lng库房ID As Long
    Dim rsData As ADODB.Recordset
    Dim cbrControl As CommandBarControl
    ...
    If Not rsData.EOF Then
        MsgBox "[" & cbo库房.Text & "]" & "还有盘点单据未审核，请审核后再进行本次初始化！", vbInformation, gstrSysName
        Exit Sub
    End If
    ...
    If Not rsData.EOF Then
        MsgBox "[" & cbo库房.Text & "]" & "还有结存单据未审核，请审核后再进行本次初始化！", vbInformation, gstrSysName
        Exit Sub
    End If
    ...
    Call zlDatabase.ExecuteProcedure(gstrSQL, mstrCaption & "初始化")
...
Private Sub RefreshList()
    ...
    Str期间 = Format(zlDatabase.Currentdate, "yyyyMM")

    mrsAccount.Filter = "库房id=" & Val(cbo库房.ItemData(cbo库房.ListIndex))
    mrsAccount.Sort = "期间 Desc"
    ...
    With vsfList
        ...
        Do While Not mrsAccount.EOF
            ...
            .TextMatrix(.rows - 1, .ColIndex("期初日期")) = Format(mrsAccount!期初日期, "YYYY-MM-DD")
            .TextMatrix(.rows - 1, .ColIndex("期末日期")) = Format(mrsAccount!期末日期, "YYYY-MM-DD")
...
Private Function SelectInput(ByVal strkey As String, ByVal sngX As Single, ByVal sngY As Single, ByVal sngH As Single) As String
    ...
    If strkey <> "" Then
        ...
        ElseIf zlCommFun.IsCharAlpha(strkey) Then         '01,11.输入全是字母时只匹配简码
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.Id
        ...
        Case mconMenu_Edit_CA_VerifyAccount
            '审核结存
            MediAccountProcess_VerifyAccount
        Case mconMenu_Edit_CA_AddIniAccount
            '初始结存/初始化
            MediAccountProcess_AddIniAccount
...
Private Sub Form_Load()
    ...
    mstr当前日期 = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
