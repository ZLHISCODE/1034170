...
'        Else
    ...
    '----判断是否需要明确批次----
    mbln明确批次 = (gtype_UserSysParms.P73_明确申领药品批次 = 1)
...
Private Function CheckData() As Boolean
    ...
    If optMode(根据消耗量) Then
        ...
        gstrSQL = "" & _
                 ...
                 "        D.药品ID,F.编码,F.名称 As 通用名,E.名称 As 商品名,F.是否变价,D.药库分批,D.药房分批,nvl(P.现价,0) 售价,F.规格,F.产地,D.最大效期,D.指导差价率, " & _
    ...
    ElseIf optMode(根据上限) Then
       gstrSQL = "Select Distinct " & IIf(chk上限.Value = 1, "Nvl(A.上限,0)", "Nvl(A.上限,0)-Sum(Nvl(B.可用数量,0))") & " 申领数量,Sum(Nvl(K.可用数量,0)) 可用数量,Sum(Nvl(K.实际数量,0)) 实际数量,Sum(Nvl(K.实际金额,0)) 实际金额,Sum(Nvl(K.实际差价,0)) 实际差价,  " & _
                ...
                " Group By Nvl(A.上限,0),D.药品ID,F.编码,F.名称,E.名称,F.是否变价,D.药库分批,D.药房分批,P.现价,F.规格,F.产地,D.最大效期,D.指导差价率,  " & _
    ...
    ElseIf optMode(根据下限) Then
        gstrSQL = "Select Distinct " & IIf(chkLowerLimit(0).Value = 1, "Nvl(A.下限,0)", IIf(chkLowerLimit(1).Value = 1, "Nvl(A.上限,0)", "Nvl(A.下限,0)-Sum(Nvl(B.可用数量,0))")) & " 申领数量,Sum(Nvl(K.可用数量,0)) 可用数量,Sum(Nvl(K.实际数量,0)) 实际数量,Sum(Nvl(K.实际金额,0)) 实际金额,Sum(Nvl(K.实际差价,0)) 实际差价,  " & _
                ...
                "  Group By Nvl(A.上限,0),Nvl(A.下限,0),D.药品ID,F.编码,F.名称,E.名称,F.是否变价,D.药库分批,D.药房分批,P.现价,F.规格,F.产地,D.最大效期,D.指导差价率,  " & _
                "        D.门诊单位,D.门诊包装,D.住院单位,D.住院包装,D.药库单位,D.药库包装,F.计算单位, D.是否常备 "
        Set rsCheck = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption & "[检查是否存在符合条件的记录]", lngTargetID, mlngStockID, mstr分类, str用途ID)
        ...
        Do While Not .EOF
            ...
            If mbln明确批次 Then
                gstrSQL = " Select Nvl(可用数量,0) 可用数量,Nvl(实际数量,0) 实际数量,Nvl(实际金额,0) 实际金额,Nvl(实际差价,0) 实际差价," & _
                          "     Nvl(批次,0) 批次,效期,上次批号 批号,上次产地 产地,NVL(上次供应商ID,0) 上次供应商ID,批准文号 " & _
                ...
                If rsStock.RecordCount <> 0 Then
                    ...
                    Do While Not rsStock.EOF
                        ...
                        If dbl填写数量 <> 0 Then
                            ...
                            If SetColValue(!药品ID, "[" & !编码 & "]", !通用名, IIf(IsNull(!商品名), "", !商品名), IIf(IsNull(!规格), "", !规格), IIf(IsNull(rsStock!产地), "", rsStock!产地), _
                                ...
                                !售价, IIf(IsNull(rsStock!批号), "", rsStock!批号), IIf(IsNull(rsStock!效期), "", rsStock!效期), NVL(!最大效期, 0), !药库分批, IIf(IsNull(!可用数量), 0, !可用数量), _
                                ...
                                rsStock!批次, dbl填写数量, !药房分批, !是否变价, NVL(rsStock!上次供应商ID, 0), _
                                IIf(IsNull(rsStock!批准文号), "", rsStock!批准文号), blnStock) Then blnAdd = True
                        End If
                    ...
                    If dbl申领数量 > 0 And blnAdd Then
                        ...
                        If Me.chk申领数量.Value = 1 Then
                            frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol申领数量) = GetFormat(Val(frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol申领数量)) + dbl申领数量 / dbl比例系数, mintNumberDigit)
                        Else
                            frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol填写数量) = GetFormat(Val(frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol填写数量)) + dbl申领数量 / dbl比例系数, mintNumberDigit)
                        End If
                        ...
                        If chk整数.Value = 1 Then
                            If Val(frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol填写数量)) <> Int(Val(frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol填写数量))) Then
                                frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol填写数量) = GetFormat(Int(Val(frmRequestDrugCard.mshBill.TextMatrix(frmRequestDrugCard.mshBill.rows - 2, mIntCol填写数量))) + 1, mintNumberDigit)
                            End If
                        End If
                    End If
                Else
                            ...
                            If bln允许 Then
                                ...
                                If dbl申领数量 <> 0 Then
                                    Call SetColValue(!药品ID, "[" & !编码 & "]", !通用名, IIf(IsNull(!商品名), "", !商品名), IIf(IsNull(!规格), "", !规格), "", _
                                        IIf(strUnit = "住院单位", !住院单位, IIf(strUnit = "门诊单位", !门诊单位, IIf(strUnit = "药库单位", !药库单位, !售价单位))), _
                                        !售价, "", "", NVL(!最大效期, 0), !药库分批, IIf(IsNull(!可用数量), 0, !可用数量), _
                                        IIf(IsNull(!实际金额), 0, !实际金额), IIf(IsNull(!实际差价), 0, !实际差价), !指导差价率, _
                                        IIf(strUnit = "住院单位", !住院包装, IIf(strUnit = "门诊单位", !门诊包装, IIf(strUnit = "药库单位", !药库包装, 1))), _
                                        0, dbl申领数量, !药房分批, !是否变价, IIf(rstemp Is Nothing, 0, NVL(rstemp!上次供应商ID, 0)), "", blnStock)
                                End If
                            End If
                        End If
                    End If
            ...
            Else
                ...
                If dbl申领数量 <> 0 Then
                    ...
                    '产生数据
                    Call SetColValue(!药品ID, "[" & !编码 & "]", !通用名, IIf(IsNull(!商品名), "", !商品名), IIf(IsNull(!规格), "", !规格), _
                        IIf(rstemp Is Nothing, NVL(!产地), NVL(rstemp!上次产地)), _
                        IIf(strUnit = "住院单位", !住院单位, IIf(strUnit = "门诊单位", !门诊单位, IIf(strUnit = "药库单位", !药库单位, !售价单位))), _
                        !售价, "", "", NVL(!最大效期, 0), !药库分批, IIf(IsNull(!可用数量), 0, !可用数量), _
                        IIf(IsNull(!实际金额), 0, !实际金额), IIf(IsNull(!实际差价), 0, !实际差价), !指导差价率, _
                        IIf(strUnit = "住院单位", !住院包装, IIf(strUnit = "门诊单位", !门诊包装, IIf(strUnit = "药库单位", !药库包装, 1))), _
...
Private Function SetColValue(ByVal lng药品ID As Long, ByVal str药品编码 As String, ByVal str通用名 As String, ByVal str商品名 As String, ByVal str规格 As String, _
    ...
    ByVal lng上次供应商ID As Long, ByVal str批准文号 As String, ByVal bln是否常备 As Boolean) As Boolean
    ...
    Dim mconIntCol指导差价率 As Integer     '= 10
    ...
    Dim mconIntCol单位 As Integer  ' = 16
    Dim mconIntCol送货单位 As Integer '=17
    Dim mconIntCol批号 As Integer    '= 17
    Dim mconIntCol效期 As Integer     '= 18
    Dim mconIntCol批准文号 As Integer '= 19
    Dim mconintcol当前库存 As Integer
    Dim mconintcol对方库存 As Integer
    Dim mconIntCol填写数量 As Integer  ' = 20
    Dim mconIntCol申领数量 As Integer
    Dim mconIntCol实际数量 As Integer  ' = 21
    Dim mconIntCol采购价 As Integer     '= 22
    Dim mconIntCol采购金额 As Integer   '= 23
    Dim mconIntCol售价 As Integer   '= 24
    Dim mconIntCol售价金额 As Integer   ' = 25
    Dim mconintCol差价 As Integer   '= 26
    Dim mconIntCol上次供应商ID As Long '=27
    ...
    str药品来源 = NVL(rstemp!药品来源)
    str基本药物 = NVL(rstemp!基本药物)
    
    '如果明确批次时，时价药品根据批次提取售价;
    If mbln明确批次 And int是否变价 = 1 Then
    ...
    With frmRequestDrugCard.mshBill
        ...
        If mbln明确批次 = False And int是否变价 = 1 Then
            dblPrice = Get售价(True, lng药品ID, cbo库房.ItemData(cbo库房.ListIndex), lng批次) 'Get价格(lng药品id, Val(.TextMatrix(intRow, mconIntCol批次)), num比例系数)
            .TextMatrix(intRow, mconIntCol售价) = GetFormat(dblPrice * num比例系数, mintPriceDigit)
        End If

        .TextMatrix(intRow, mconIntCol分批核算) = int分批核算
        .TextMatrix(intRow, mconIntCol可用数量) = GetFormat(num可用数量 / num比例系数, mintNumberDigit)
        .TextMatrix(intRow, mconIntCol最大效期) = int最大效期 & "||" & int是否变价 & "||" & int药房分批
        .TextMatrix(intRow, mconIntCol实际差价) = GetFormat(num实际差价, mintMoneyDigit)
        .TextMatrix(intRow, mconIntCol实际金额) = GetFormat(num实际金额, mintMoneyDigit)
        ...
        If Me.chk申领数量.Value = 1 Then
            ...
            If (int是否变价 = 1 Or lng批次 <> 0) And mbln明确批次 Then
                .TextMatrix(intRow, mconIntCol申领数量) = GetFormat(IIf(Dbl数量 >= num可用数量, num可用数量, Dbl数量) / num比例系数, mintNumberDigit)
                .TextMatrix(intRow, mconIntCol填写数量) = GetFormat(0, mintNumberDigit)
                .TextMatrix(intRow, mconIntCol实际数量) = GetFormat(0, mintNumberDigit)
            Else
                .TextMatrix(intRow, mconIntCol申领数量) = GetFormat(Dbl数量 / num比例系数, mintNumberDigit)
                .TextMatrix(intRow, mconIntCol填写数量) = GetFormat(0, mintNumberDigit)
                .TextMatrix(intRow, mconIntCol实际数量) = GetFormat(0, mintNumberDigit)
            End If
        Else
            ...
            If (int是否变价 = 1 Or lng批次 <> 0) And mbln明确批次 Then
                .TextMatrix(intRow, mconIntCol填写数量) = GetFormat(IIf(Dbl数量 >= num可用数量, num可用数量, Dbl数量) / num比例系数, mintNumberDigit)
                .TextMatrix(intRow, mconIntCol实际数量) = GetFormat(IIf(Dbl数量 >= num可用数量, num可用数量, Dbl数量) / num比例系数, mintNumberDigit)
            Else
                .TextMatrix(intRow, mconIntCol填写数量) = GetFormat(Dbl数量 / num比例系数, mintNumberDigit)
                .TextMatrix(intRow, mconIntCol实际数量) = GetFormat(Dbl数量 / num比例系数, mintNumberDigit)
            End If
        End If
        
        If chk整数.Value = 1 Then
            If Val(.TextMatrix(intRow, mconIntCol填写数量)) <> Int(Val(.TextMatrix(intRow, mconIntCol填写数量))) Then
                .TextMatrix(intRow, mconIntCol填写数量) = GetFormat(Int(Val(.TextMatrix(intRow, mconIntCol填写数量))) + 1, mintNumberDigit)
                .TextMatrix(intRow, mconIntCol实际数量) = GetFormat(.TextMatrix(intRow, mconIntCol填写数量), mintNumberDigit)
            End If
        End If
        
        If .TextMatrix(intRow, mconIntCol售价) <> "" Then
            .TextMatrix(intRow, mconIntCol售价金额) = GetFormat(.TextMatrix(intRow, mconIntCol售价) * .TextMatrix(intRow, mconIntCol实际数量), mintMoneyDigit)
        End If
        
        .TextMatrix(intRow, mconIntCol采购价) = GetFormat(Get成本价(lng药品ID, Val(cbo库房.ItemData(cbo库房.ListIndex)), lng批次) * num比例系数, mintCostDigit)
        .TextMatrix(intRow, mconIntCol采购金额) = GetFormat(Val(.TextMatrix(intRow, mconIntCol采购价)) * Val(.TextMatrix(intRow, mconIntCol填写数量)), mintMoneyDigit)
        .TextMatrix(intRow, mconintCol差价) = GetFormat(Val(.TextMatrix(intRow, mconIntCol售价金额)) - Val(.TextMatrix(intRow, mconIntCol采购金额)), mintMoneyDigit)
