...
'------------------------------------------------------------------
'太元通审查问题详细信息查询接口
Public Declare Function queryCheckResult Lib "Midlayer.dll" (ByVal visitID As String, ByVal sysFlag As String)
'------------------------------------------------------------------
'ZLHIS中是否使用PASS系统
Public gintPass As Integer

'太元通合理用药接口对象
Public gobjPass As Object

Public Function PassUseable() As Integer
'功能：检查ZLHIS系统是否允许使用PASS系统
'说明：因为判断位置与InitSysPar不一致，所以单独处理
    Dim rsTmp As New ADODB.Recordset
        
    On Error GoTo errH
    
    gstrSQL = "Select 参数值 From Zlparameters Where 系统 = [1] And Nvl(私有, 0) = 0 And 模块 Is Null And 参数号=[2]"
    Set rsTmp = zldatabase.OpenSQLRecord(gstrSQL, "mdlPass", glngSys, 30)
    If Not rsTmp.EOF Then
        PassUseable = rsTmp!参数值
    Else
        PassUseable = 0
    End If
    Exit Function
...
'功能：对PASS接口进行注册和初始化，同时检查PASS接口DLL是否正确安装
    ...
    'PASS初始化
    If PassInit(gstrUserCode & "/" & gstrUserName, gstrDeptCode & "/" & gstrDeptName, 10) <> 1 Then
