...
Private Sub Form_Load()
    ...
    If IntOper <> 5 Then
        Me.Dtp开始Date.Enabled = IsHavePrivs(mstrPrivs, "修改过滤日期")
...
Private Sub Form_Resize()
    ...
    Me.Height = IIf((BlnState And IsHavePrivs(mstrPrivs, "过滤附加条件")), DblAdvanceHeight, DblNormalHeight)
    fra附加条件.Enabled = (BlnState And IsHavePrivs(mstrPrivs, "过滤附加条件"))
    
    With fra附加条件
        .Top = IIf((BlnState And IsHavePrivs(mstrPrivs, "过滤附加条件")), Fra基本条件.Top + Fra基本条件.Height + 80, CmdOK.Top + CmdOK.Height + 180)
    End With
    With CmdOK
        .Top = IIf((BlnState And IsHavePrivs(mstrPrivs, "过滤附加条件")), fra附加条件.Top + fra附加条件.Height + 80, Fra基本条件.Top + Fra基本条件.Height + 80)
    End With
    With CmdCancel
        .Top = CmdOK.Top
    End With
    ...
    If fra附加条件.Enabled = True Then
        Me.Cbo填制人.Enabled = IsHavePrivs(mstrPrivs, "医生查询")
    End If
End Sub
...
Private Function GetSQL()
    ...
    SQLCondition.str就诊卡 = txt就诊卡.Text
