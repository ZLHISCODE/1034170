...
Private Sub SetRecipeMedi(ByVal rsData As ADODB.Recordset)
    ...
    With vsfRecipe
        ...
        Do While Not rsData.EOF
            ...
            Else
                int门诊 = 2
            End If
            strDiag = RecipeSendWork_GetDiagnosis(int门诊, IIf(int门诊 = 1, Val(rsData!相关ID), Val(rsData!病人ID)), Val(rsData!主页id))
            If strDiag <> "" Then
                strDiag = strDiag & "|"
    ...
    '                    If i > 0 Then txt诊断内容.Height = txt诊断内容.Height + 200
            ...
            If mbln中药处方 Then
                ...
                If intCol = 0 Then
                    .TextMatrix(intRow, 中药列名.分组1) = rsData!药名 & " " & FormatEx(Abs(dblAmount), 1) & rsData!计算单位
                ElseIf intCol = 1 Then
                    .TextMatrix(intRow, 中药列名.分组2) = rsData!药名 & " " & FormatEx(Abs(dblAmount), 1) & rsData!计算单位
                ElseIf intCol = 2 Then
                    .TextMatrix(intRow, 中药列名.分组3) = rsData!药名 & " " & FormatEx(Abs(dblAmount), 1) & rsData!计算单位
                ElseIf intCol = 3 Then
                    .TextMatrix(intRow, 中药列名.分组4) = rsData!药名 & " " & FormatEx(Abs(dblAmount), 1) & rsData!计算单位
                End If
            ...
            Else
                ...
                .TextMatrix(intRow, 西药列名.总量) = FormatEx(dblAmount, 5) & rsData!单位
                
                .ColWidth(西药列名.单量) = 750
                .TextMatrix(intRow, 西药列名.单量) = IIf(IsNull(rsData!单量), "", FormatEx(rsData!单量, 5) & "(" & NVL(rsData!计算单位) & ")")
...
Private Sub IniRecipe()
    ...
    With vsfRecipe
        ...
        If Not mbln中药处方 Then
            ...
            .RowHeight(0) = 255
        Else
            .Cols = 中药列名.列数 + IIf(mcondition.intRowNum = 4, 2, 0)
            
            If mcondition.intRowNum = 4 Then
                .ColWidth(中药列名.分组1) = 2000
                .ColWidth(中药列名.分组2) = 2000
                .ColWidth(中药列名.分组3) = 2000
                .ColWidth(中药列名.分组4) = 2000
                .ColWidth(中药列名.间隔1) = 50
                .ColWidth(中药列名.间隔2) = 50
                .ColWidth(中药列名.间隔3) = 50
                
                .ColAlignment(中药列名.分组4) = flexAlignLeftCenter
                .TextMatrix(0, 中药列名.分组4) = ""
            Else
                .ColWidth(中药列名.分组1) = 3000
                .ColWidth(中药列名.分组2) = 3000
                .ColWidth(中药列名.分组3) = 3000
                .ColWidth(中药列名.间隔1) = 50
                .ColWidth(中药列名.间隔2) = 50
            End If
            .ColAlignment(中药列名.分组1) = flexAlignLeftCenter
            .ColAlignment(中药列名.分组2) = flexAlignLeftCenter
            .ColAlignment(中药列名.分组3) = flexAlignLeftCenter
    ...
    With mcondition
        ...
        If .intListType <> mListType.退药 Then
            For i = 0 To cbo配药人.ListCount - 1
                If InStr(cbo配药人.List(i), mstrDosUser) > 0 Then
                    cbo配药人.ListIndex = i
                    Exit For
                End If
            Next
            ...
            For i = 0 To cbo核查人.ListCount - 1
                If InStr(cbo核查人.List(i), mstr核查人) > 0 Then
                    cbo核查人.ListIndex = i
                    Exit For
                End If
            Next
...
Private Sub SetCmdSendPrivs(Optional ByVal int审查结果 As Integer)
    '权限控制
...
'        lblRP前记(RP前记.险类).Caption = IIf(IsNull(!险类), "", !险类)
        ...
        If mcondition.int金额显示 = 1 Then
            ...
            lblRP后记(RP后记.合计金额).Caption = GetFormat(dbl实收金额, 2) & "元"
        ElseIf mcondition.int金额显示 = 2 Then
            lblRP后记(RP后记.合计金额标签).Caption = "应收/实收合计："
            lblRP后记(RP后记.合计金额).Caption = GetFormat(dbl应收金额, 2) & "元/" & GetFormat(dbl实收金额, 2) & "元"
        Else
            lblRP后记(RP后记.合计金额标签).Caption = "应收合计："
            lblRP后记(RP后记.合计金额).Caption = GetFormat(dbl应收金额, 2) & "元"
        End If
