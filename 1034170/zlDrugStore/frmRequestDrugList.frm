...
Private Sub GetList(ByVal strFind As String)
    ...
    If (Not rsList.EOF) And (Not rsList.BOF) Then
        ...
        lbl1.Caption = "成本金额合计：" & GetFormat(dbl1, mintShowMoneyDigit)
        lbl2.Caption = "售价金额合计：" & GetFormat(dbl2, mintShowMoneyDigit)
        lbl3.Caption = "差价金额合计：" & GetFormat(dbl3, mintShowMoneyDigit)
    ...
    With vsfList
        ...
        For intRow = 1 To .rows - 1
            ...
            If intStatus Mod 3 = 2 Then
                ...
                Else
                    .Cell(flexcpForeColor, intRow, 0, intRow, .Cols - 1) = &HC0C0FF
                End If
            End If
        Next
    End With
...
Private Sub SetDetailColWidth()
    ...
    With vsfDetail
        ...
        .ColAlignment(.ColIndex("送货单位")) = flexAlignCenterCenter    '单位
    ...
    If vsfList.Row >= 1 And LTrim(vsfList.TextMatrix(vsfList.Row, 0)) <> "" Then
        ...
        Select Case mintUnit
            Case mconint售价单位
                strUnit = "D.计算单位"
                str包装系数 = "1"
                str送货单位 = "decode(b.送货单位,null,'',b.送货单位|| '(1'||b.送货单位||'='||b.送货包装/1|| d.计算单位 ||')') as 送货单位,"
            Case mconint门诊单位
                strUnit = "B.门诊单位"
                str包装系数 = "B.门诊包装"
        ...
        gstrSQL = " SELECT * FROM (SELECT DISTINCT 序号" & strSql药名 & ",B.药品来源,B.基本药物," & _
            " D.规格,A.产地, A.批号, " & strSql效期 & " ,A.批准文号,LTRIM(TO_CHAR(A.填写数量 /" & str包装系数 & "," & mstrNumberFormat & " )) AS 填写数量," & _
        ...
        If .TextMatrix(.Row, 0) = "" Or .Row = 0 Then         '没有单
         ...
         Else
            ...
            If .TextMatrix(.Row, .Cols - 4) = "" Then    '未审核单
                bln已发送 = (vsfList.TextMatrix(vsfList.Row, 10) <> "")
                If mnuEditModify.Visible = True Then
