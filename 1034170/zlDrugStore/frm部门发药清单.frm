...
Private Const mconIntCol科室汇总_列数 As Integer = 24
...
Private mIntCol科室汇总_留存登记时间 As Integer
'销账列表
...
Public Function GetStayRecord() As ADODB.Recordset
    ...
    With rsStay
        ...
        .Fields.Append "留存登记时间", adLongVarChar, 50, adFldIsNullable
        ...
        With vsfList(mListType.汇总)
            For i = 1 To .rows - 1
                If Val(.TextMatrix(i, mIntCol科室汇总_领药部门id)) > 0 And Val(.TextMatrix(i, mIntCol科室汇总_留存数量)) > 0 Then
                    ...
                    rsStay!留存登记时间 = .TextMatrix(i, mIntCol科室汇总_留存登记时间)
                    rsStay.Update
                End If
            Next
        End With
...
'        .Cell(flexcpPictureAlignment, 1, mIntCol发药_当前行, .Rows - 1, mIntCol发药_当前行) = flexPicAlignRightCenter
        ...
        VsfGridColFormat vsfList(mListType.发药), mIntCol发药_服药时间, "服药时间", IIf(mcondition.bln加载服药时间和用药次数, 2400, 0), flexAlignLeftCenter, "服药时间"
        VsfGridColFormat vsfList(mListType.发药), mIntCol发药_用法, "用法", 800, flexAlignLeftCenter, "用法"
        VsfGridColFormat vsfList(mListType.发药), mIntCol发药_用药次数, "用药次数", IIf(mcondition.bln加载服药时间和用药次数, 900, 0), flexAlignRightCenter, "用药次数"
        ...
        If mcondition.bln加载服药时间和用药次数 = False Then mstrUnallowShow(mListType.发药) = mstrUnallowShow(mListType.发药) & ";服药时间;用药次数"
...
Private Sub InitList_Sum()
    ...
    With vsfList(mListType.汇总)
        ...
        Else
            ...
            VsfGridColFormat vsfList(mListType.汇总), mIntCol科室汇总_留存登记时间, "留存登记时间", 0, flexAlignLeftCenter, "留存登记时间"
        End If
        ...
        mstrUnallowShow(mListType.汇总) = "当前行;批次;科室ID;药品ID;领药部门id;药品编码和名称;药品编码;药名;包装;留存登记时间"
...
'        mrsSendList.Sort = "领药部门,领药号,姓名,NO,序号"
        ...
        Else
            Do While Not mrsSendList.EOF
                ...
                .TextMatrix(lngRow, mIntCol发药_服药时间) = mrsSendList!服药时间
...
'                    .ColWidth(mIntCol科室汇总_批号) = 1200
                Do While Not mrsSendList.EOF
                    If str科室汇总 <> mrsSendList!领药部门 & mrsSendList!品名 & IIf(mcondition.bln按批次汇总, IIf(IsNull(mrsSendList!批次), 0, mrsSendList!批次), "") Then
                        ...
                        .TextMatrix(intRow, mIntCol科室汇总_留存登记时间) = mrsSendList!留存登记时间
