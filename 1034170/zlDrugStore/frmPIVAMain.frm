...
Private Sub InitVsfSum()
    ...
    With vsfSumDrug
        ...
        VsfGridColFormat vsfSumDrug, mintcolsum病区, "病区", 800, flexAlignRightCenter, "病区"
        VsfGridColFormat vsfSumDrug, mintcolsum打包, "打包", 2500, flexAlignLeftCenter, "打包"
        VsfGridColFormat vsfSumDrug, mintcolsum药品名称, "药品名称", 2000, flexAlignLeftCenter, "药品名称"
        ...
        With rsTrans
            ...
            If .RecordCount > 0 Then
                ...
                Do While Not .EOF
                    ...
                    mrsTrans!单量 = FormatEx(NVL(!单量, 0), 7)
                    mrsTrans!剂量单位 = !剂量单位
...
'            Else
                ...
                .TextMatrix(lngRow, .ColIndex("药品名称")) = mrsTrans!药品名称
                .TextMatrix(lngRow, .ColIndex("规格")) = NVL(mrsTrans!规格)
                .TextMatrix(lngRow, .ColIndex("单量")) = FormatEx(mrsTrans!单量, 7) & mrsTrans!剂量单位
                .TextMatrix(lngRow, .ColIndex("数量")) = FormatEx(Abs(mrsTrans!数量), mintNumberDigit) & mrsTrans!单位
...
Private Sub UpdateExeSign(ByVal LngID As Long, Optional ByVal intSign As Integer)
    ...
    If LngID = 0 Then
        ...
        '修正记录集游标EOF，导致不能有效更新【执行标志】的问题
        If mrsTrans.RecordCount > 0 Then mrsTrans.MoveFirst
        ...
        Else
            With Me.VSFLook
                For intRow = 1 To .rows - 1
                    If lng配药id <> Val(.TextMatrix(intRow, .ColIndex("配药id"))) And Val(.TextMatrix(intRow, .ColIndex("配药id"))) <> 0 Then
...
Private Sub Form_Load()
    ...
    '从参数表中取药品价格、数量、金额小数位数
    Call GetDrugDigit(mParams.lng配置中心, "静脉输液配置中心管理", 3, mintCostDigit, mintPriceDigit, mintNumberDigit, mintMoneyDigit)
