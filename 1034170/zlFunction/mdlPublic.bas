...
'        frmErrAsk.lblNumber = "序号：" & Err.Number
'        frmErrAsk.lblNote = Space(4) & strNote
'        frmErrAsk.txtHelp.Text = Err.Description
'        frmErrAsk.Show 1
'        ErrCenter = frmErrAsk.bytReturn
'        Unload frmErrAsk
'    Else
'        frmErrNote.lblNumber = "序号：" & Err.Number
'        frmErrNote.lblNote = Space(4) & strNote
'        frmErrNote.txtHelp.Text = Err.Description
'        frmErrNote.Show 1
'        ErrCenter = 0
'    End If
'
...
'功能：将刚才的错误信息写入数据库错误日志
    ...
    If lngErrNum <> 0 And bytErrType <> 0 Then
        strSQL = "Insert Into zlErrorLog(会话号,用户名,工作站,时间,类型,错误序号,错误信息)" & _
            " Select SID,USER,MACHINE,SYSDATE," & bytErrType & "," & lngErrNum & "," & _
            AdjustStr(strErrInfo) & " From v$Session Where USER#<>0 And STATUS='ACTIVE'"
        gcnOracle.Execute strSQL
        
        lngErrNum = 0: strErrInfo = "": bytErrType = 0
    End If
End Sub
...
Public Function zlRegInfo(strItem As String, Optional blnTemp As Boolean, Optional intBits As Integer) As String
    ...
    If blnTemp Or sblnTemp <> blnTemp Or (srsInfo.State <> adStateOpen) Then
        ...
        Set srsInfo = zlDatabase.OpenSQLRecord(strSQL, "zlRegInfo")
    End If
