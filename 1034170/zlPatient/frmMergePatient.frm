...
Public mlng病人ID As Long '入：初始要合并的病人ID
...
Private Sub Form_Load()
    ...
    gblnOK = False
    fra(1).Tag = ""
    If Not ShowPatiInfo(mlng病人ID, 0) Then Unload Me
End Sub
...
'        " And A.出院科室ID=B.ID And A.病人ID=[1]"
    'lpf:住院次数调整
    strSQL = "Select b.出院日期, b.住院号, b.出院病床, b.病人id, b.主页id,a.住院次数,c.名称 As 科室" & vbNewLine & _
            "From 病人信息 a, 病案主页 b, 部门表 c" & vbNewLine & _
            "Where a.病人id = b.病人id And a.主页id = b.主页id And b.出院科室id = c.Id And a.病人id = [1]"
...
Private Sub cmdMerge_Click()
    ...
    '45976:刘鹏飞，2012-09-21，合并是如果姓名或身份证号不同提示是否合并。
    strInfo = ""
    If txt姓名(0).Text <> txt姓名(1).Text And txt身份证号(0).Text <> txt身份证号(1).Text Then
        strInfo = "保留和被合并的病人姓名、身份证号都不同，是否继续进行合并?" & vbCrLf & _
                "要保留病人的姓名：" & Trim(txt姓名(1).Text) & Space(4) & "身份证号：" & Trim(txt身份证号(1).Text) & vbCrLf & _
                "被合并病人的姓名：" & Trim(txt姓名(0).Text) & Space(4) & "身份证号：" & Trim(txt身份证号(0).Text)
    ElseIf txt姓名(0).Text <> txt姓名(1).Text Then
        strInfo = "保留和被合并的病人身份证号相同但姓名不同，是否继续进行合并?" & vbCrLf & _
                "要保留病人的姓名：" & Trim(txt姓名(1).Text) & vbCrLf & _
                "被合并病人的姓名：" & Trim(txt姓名(0).Text)
    ElseIf txt身份证号(0).Text <> txt身份证号(1).Text Then
        strInfo = "保留和被合并的病人姓名相同但身份证号不同，是否继续进行合并?" & vbCrLf & _
                "要保留病人的身份证号：" & Trim(txt身份证号(1).Text) & vbCrLf & _
                "被合并病人的身份证号：" & Trim(txt身份证号(0).Text)
    End If
    If strInfo <> "" Then
