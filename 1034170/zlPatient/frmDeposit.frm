...
Private Sub FindPati(ByVal objCard As Card, ByVal blnCard As Boolean, ByVal strInput As String)
    ...
    '101326:李南春,2019/1/11,显示病人身份证号和联系电话
    lblIDCard.Caption = lblIDCard.Tag & IIf(IsNull(mrsInfo!身份证号), "", mrsInfo!身份证号)
...
Private Function GetPatient(ByVal objCard As Card, ByVal strInput As String, blnCancel As Boolean, Optional blnCard As Boolean = False) As Boolean
  ...
  '72828,冉俊明,2014-5-9,增加工作单位信息的显示，处理方式：增加A.工作单位字段
    ...
    "           B.险类,B.卡号,Nvl(B.医保号,A.医保号) 医保号,B.密码,Nvl(C.费别,A.费别) 费别,A.担保人,A.担保额,Nvl(A.担保性质,0) as 担保性质, A.工作单位,A.手机号,C.备注,Nvl(A.在院,0) As 在院标志,A.身份证号" & _
...
Private Function ReadBill(strNO As String) As Integer
   ...
   gstrSQL = "" & _
    ...
    "           B.病人类型,B.险类, NVL(A.预交类别,0) as 预交类别,B.身份证号, " & _
    ...
    '101326:李南春,2019/1/11,显示病人身份证号
    lblIDCard.Caption = lblIDCard.Tag & IIf(IsNull(rsTemp!身份证号), "", rsTemp!身份证号)
...
Private Sub SetMoneyInfo(blnClear As Boolean, Optional lng病人ID As Long, _
    ...
    If blnClear Then
        ...
        lblIDCard.Caption = lblIDCard.Tag
...
Private Function zlDepositDel(ByRef lng预交ID As Long, ByRef lng冲预交ID As Long, ByVal dblMoney As Double) As Boolean
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String, rsTemp As ADODB.Recordset
    Dim strXMLExpend As String, strSwapNO As String, strSwapMemo As String, strSwapExtendInfor As String
    ...
    Else
        str预交IDs = lng冲预交ID
    End If
    If Save三方交易(str预交IDs, mcurBill.lng卡类别ID, mcurBill.bln消费卡, mcurBill.str卡号, _
...
Private Sub Load支付方式()
    ...
    With cboStyle
        ...
        Do While Not rsTemp.EOF
            ...
            If Not blnFind And InStr(",3,7,8,", "," & rsTemp!性质 & ",") = 0 Then
