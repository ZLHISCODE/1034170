...
'    DoEvents
    ...
    If mlngFileType = cpr诊疗报告 Then
        ...
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, "提取诊疗项目ID", mlng记录ID)
        Do Until rs.EOF
            mstrItems = "," & rs!诊疗项目id
            rs.MoveNext
        Loop
...
Dim cbp帮助 As CommandBarPopup      '帮助菜单
    ...
    With BarMain.Controls
        ...
        Set objControl = .Add(xtpControlButton, ID_FILE_EXIT, "关闭"):objControl.IconId =191
        objControl.BeginGroup = True: objControl.IconId =191
        objControl.STYLE = xtpButtonIconAndCaption
    End With
    '图标绑定
...
Private Sub ShowReference()
    ...
    Call objReference.InitCommon(gcnOracle, glngSys)
    Call objReference.ShowClincHelp(IIf(mblnShowModeless, vbModeless, vbModal), Me, lngItem, False, mstrItems)
    Err.Clear
    Exit Sub
errHand:
    If ErrCenter() = 1 Then
