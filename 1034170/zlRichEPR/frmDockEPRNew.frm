...
Dim rptRow As ReportRow
    ...
    Case 1                  '门诊
        If mblnShowAll Then
            ...
            gstrSQL = "Select a.Id, a.种类, a.编号, a.名称,zlspellcode(a.名称) 简码, a.说明,b.事件 " & _
                    ...
                    " Where a.ID=b.文件id(+) And a.ID=c.文件id(+) And a.种类 In (1,5,6) And (a.通用=1 Or a.通用=2 And c.科室id=[2])" & _
                    " Order By a.种类, a.编号"
        Else
            ...
            gstrSQL = "Select a.Id, a.种类, a.编号, a.名称,zlspellcode(a.名称) 简码, a.说明,b.事件 " & _
                    " From 病历文件列表 a,病历时限要求 b " & _
                    " Where a.ID=b.文件id(+) And Instr(';' || [1] || ';', ';' || a.Id || ';') <> 0" & _
                    " Order By a.种类, a.编号"
        End If
    ...
    '------------------------------------------------------------------------------------------------------------------
        gstrSQL = "Select 文件ID ID,病历编号 || '-' || 病历名称 病历名称, 到期时间, Decode(必须,1,'是','否') 必写" & vbNewLine & _
                    ...
                    "Where 病人id = [1] And 主页id = [2] And 科室id =[3] and 病历种类 in (1,5,6) And 病人来源 = 1 And (Nvl(完成记录id, 0) = 0 And 完成时间 Is Null)" & vbNewLine & _
                    "Order By 到期时间"
    ...
    Case 2                  '住院
        If mblnShowAll Then
            ...
            gstrSQL = "Select a.Id, a.种类, a.编号, a.名称,zlspellcode(a.名称) 简码, a.说明,b.事件 " & _
                    " From 病历文件列表 a,病历时限要求 b,病历应用科室 c " & _
                    " Where a.ID=b.文件id(+) And a.ID=c.文件id(+) And a.种类 In (2,5,6) And Nvl(A.保留, 0) In (0,1,2,5) And (a.通用=1 Or a.通用=2 And c.科室id=[2])" & _
                    " Order By a.种类, a.编号"
        Else
    ...
    '------------------------------------------------------------------------------------------------------------------
        gstrSQL = "Select 文件ID ID,病历编号 || '-' || 病历名称 病历名称, 到期时间, Decode(必须,1,'是','否') 必写" & vbNewLine & _
                    "From 电子病历时机" & vbNewLine & _
                    "Where 病人id = [1] And 主页id = [2] And 科室id =[3] and 病历种类 in (2,5,6) And 病人来源 = 2 And (Nvl(完成记录id, 0) = 0 And 完成时间 Is Null)" & vbNewLine & _
                    "Order By 到期时间"
        Set rsLimit = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lngPatiID, lngVisit, lngDeptId, gstrUserName)
    Case 3                  '护理
        If mblnShowAll Then
