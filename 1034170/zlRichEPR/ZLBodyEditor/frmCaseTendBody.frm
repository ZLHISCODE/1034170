...
Begin VB.Form frmCaseTendBody 
   ...
   Begin VB.PictureBox picCustom 
      ...
      End
   End
   Begin zl9BodyEditor.usrBodyEditor BodyEdit 
      Height          =   4350
      Left            =   435
      TabIndex        =   0
      Top             =   615
...
Private mbytSize As Byte  'ShowEditor时使用
...
Private Sub ReSetFontSize()
    ...
    Me.Font = mbytFontSize
    If Not cbsThis Is Nothing Then
        txt.FontSize = mbytFontSize
        ...
        If mbytFontSize = 12 Then
            cbsThis.ActiveMenuBar.SetIconSize 24, 24
        Else
...
Public Function ShowEdit(ByVal frmMain As Object, strParam As String, Optional ByVal bytMode As Byte = 1, Optional strPrivs As String) As Boolean
    ...
    '------------------------------------------------------------------------------------------------------------------
    gstrSQL = "Select NVL(B.姓名,A.姓名) 姓名 from 病人信息 A,病案主页 B Where A.病人ID=B.病人ID And B.病人id=[1] And B.主页id=[2]"
    Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(mrsParam("病人id").Value), Val(mrsParam("主页id").Value))
    If rs.BOF = False Then
...
Private Function ShowTxtSelDialog(ByVal frmParent As Object, _
    ...
    If frmSelectDialog.ShowSelect(frmParent, WinStyle, rs, strLvw, strDescrible, lngX, lngY, lngCX, lngCY, objTXT.Height, strInitKey, strSavePath, , False, blnMuliSel, , blnSort, bytSize) Then
...
Private Sub cmd_Click()
    ...
    If mrsPatient.RecordCount > 0 Then
        mrsPatient.MoveFirst
        If ShowTxtSelDialog(Me, txt, "床号,1200,0,0;姓名,1200,0,1;性别,600,0,0;科室,1800,0,0;住院号,1080,0,0", Me.Name & "\病人清单选择", "请从下面选择一个病人。", mrsPatient, rs, BlowUp(5600), BlowUp(4500), , CStr(mlngRowNum), 2, True, mbytFontSize) Then
...
'        Call SaveWinState(Me, App.ProductName, "ChildForm")
    ...
    Set mrsParam = Nothing
    Set mcbr查看 = Nothing
    Set mcbrMenuBar曲线 = Nothing
    Set mcbrMenuBar部位 = Nothing
    Set mcbrMenuBar编辑 = Nothing
