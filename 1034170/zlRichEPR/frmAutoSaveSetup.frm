...
Begin VB.Form frmAutoSaveSetup 
   ...
   Begin VB.CheckBox chkSyncPage 
      Caption         =   "插入入/出院诊断时同步更新首页中的入/出院诊断"
      Height          =   285
      Left            =   270
      TabIndex        =   27
      ToolTipText     =   "在病历中插入初步、确诊、修正、补充诊断、出院诊断时同步更新首页中的入/出院诊断"
      Top             =   3000
      Width           =   4455
   End
   Begin VB.CheckBox ChkSignAlter 
      Caption         =   "签名移位(&S)"
      Height          =   285
      Left            =   3855
      TabIndex        =   26
      ToolTipText     =   "诊疗报告在签名时自动将签名内容移位到页面最底部"
      Top             =   1815
      Visible         =   0   'False
      Width           =   1305
   End
...
Private mblnSignAutoAlter As Boolean
Private mintSyncPage As Integer
...
Public Function ShowMe(ByRef frmParent As Object, ByVal strPrivs As String) As Boolean
    ...
    mintSign = zlDatabase.GetPara("SignShow", glngSys, 1070, 0, Array(optSign(0), optSign(1)), InStr(mstrPrivs, "参数设置"))
    mblnSignAutoAlter = zlDatabase.GetPara("签名自动位移", glngSys, 1070, 0, Array(ChkSignAlter), InStr(mstrPrivs, "参数设置") > 0) = 1
    mintSyncPage = zlDatabase.GetPara("SyncPage", glngSys, 1070, 0, Array(chkSyncPage), InStr(mstrPrivs, "参数设置") > 0)
    ...
    ChkSignAlter.Value = IIf(mblnSignAutoAlter, vbChecked, vbUnchecked)
    ChkSignAlter.Visible = frmParent.Document.EPRFileInfo.种类 = cpr诊疗报告
    chkSyncPage.Value = mintSyncPage
    
    If mintSign = 0 Then
        optSign(0).Value = True
    Else
        optSign(1).Value = True
    End If
    ...
    If mblnOK Then
        ...
        zlDatabase.SetPara "SignShow", mintSign, glngSys, 1070
        zlDatabase.SetPara "签名自动位移", IIf(mblnSignAutoAlter, 1, 0), glngSys, 1070
        zlDatabase.SetPara "SyncPage", IIf(mintSyncPage, 1, 0), glngSys, 1070
    End If
    ShowMe = mblnOK
End Function
...
Private Sub cmdOK_Click()
    ...
    mblnSignAutoAlter = (ChkSignAlter.Value = vbChecked)
    If optSign(0).Value Then mintSign = 0 Else mintSign = 1
    mintSyncPage = chkSyncPage.Value
    mblnOK = True
    Unload Me
End Sub
