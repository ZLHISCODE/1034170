...
Dim i As Integer
    ...
    If optBecause(0).Value And optSo(0).Value Then
        If CLng(Val(Split(cboElName.Tag, ";")(cboElName.ListIndex))) = CLng(Val(Split(cboSoElname.Tag, ";")(cboSoElname.ListIndex))) _
            And NeedName(cboElName.Text) = NeedName(cboSoElname.Text) Then
    ...
    If optBecause(0).Value And optSo(2).Value Then
        If CLng(Val(Split(cboElName.Tag, ";")(cboElName.ListIndex))) = CLng(Val(Split(cboDelElname.Tag, ";")(cboDelElname.ListIndex))) _
            And NeedName(cboElName.Text) = NeedName(cboDelElname.Text) Then
...
'不允许多个因素(病种/要素)控制同一要素
    ...
    Select Case intBecause
        Case 1
            ...
            strElname = NeedName(cboElName.Text)
        ...
        Case 4
            ...
            strElname = NeedName(cboSameElName.Text)
            strElContent = ""
    End Select
    ...
    Select Case intSo
        Case 1
            ...
            strSoElname = NeedName(cboSoElname.Text)
        ...
        Case 3
            ...
            strSoElname = NeedName(cboDelElname.Text)
            strSoElContent = ""
            strSoElOldContent = ""
        Case 4
            lngSoCompendID = 0
            lngSoID = Val(Split(cboSameElName.Tag, ";")(cboSameElName.ListIndex))
            strSoElname = NeedName(cboSameElName.Text)
            strSoElContent = ""
            strSoElOldContent = ""
        Case 5
            lngSoCompendID = 0
    ...
    With rptList
        ...
        With .FocusedRow.Record
            ...
            Select Case .Item(mCol.原因类型).Value
                Case 1
                    optBecause(0).Value = True
                    Call SeekIndex(cboElName, .Item(mCol.原因要素名称).Value, True)
                    Call SeekIndex(cboContent, .Item(mCol.原因要素内容).Value, True)
                Case 2
            ...
            Select Case .Item(mCol.结果类型).Value
                Case 1
                    optSo(0).Value = True
                    Call SeekIndex(cboSoElname, .Item(mCol.结果要素名称).Value, True)
                    txtSoElContent.Text = .Item(mCol.结果要素值域).Value
                Case 2
                    optSo(1).Value = True
                    Call SeekIndex(cboSoStCompend, .Item(mCol.结果提纲名称).Value, True)
                    Call SeekIndex(cboSoSentence, .Item(mCol.结果词句名称).Value, True)
                Case 3
                    optSo(2).Value = True
                    Call SeekIndex(cboDelElname, .Item(mCol.结果要素名称).Value, True)
                Case 4
                    optSo(3).Value = True
                    Call SeekIndex(cboDelElname, .Item(mCol.结果要素名称).Value, True)
                Case 5
                    optSo(4).Value = True
                    Call SeekIndex(cboAddSentence, .Item(mCol.结果词句名称).Value, True)
            End Select
        End With
    End With
    ...
    If KeyCode = vbKeyReturn Then
        ...
        If Trim(txtDiagnose.Text) <> "" Then
            gstrSQL = "Select A.ID,A.编码,A.名称 From 疾病诊断目录 A,疾病诊断别名 B Where A.ID=B.诊断ID AND (A.编码=[1]" & _
                                                            ...
                                                            " or " & zlCommFun.GetLike("B", "简码", txtDiagnose.Text) & ")"
        Else
            gstrSQL = "Select ID,编码, 名称 From 疾病诊断目录 A"
        End If
    ...
    If KeyCode = vbKeyReturn Then
        ...
        If Trim(txtDisease.Text) <> "" Then
            gstrSQL = "Select ID,编码, 名称 From 疾病编码目录 A Where 编码=[1]" & _
                                                            ...
                                                            " or " & zlCommFun.GetLike("A", "五笔码", txtDisease.Text)
        Else
            gstrSQL = "Select ID,编码, 名称 From 疾病编码目录 A"
        End If
