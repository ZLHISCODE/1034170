...
Begin VB.Form frmTendEdit 
   ...
   Begin VB.Frame fra 
      ...
      Begin VB.ComboBox Cbo 
         Height          =   300
         Index           =   0
         ItemData        =   "frmTendEdit.frx":000C
         Left            =   1380
         List            =   "frmTendEdit.frx":000E
         Style           =   2  'Dropdown List
         TabIndex        =   20
         Top             =   270
         Width           =   1815
...
Private Function CheckData() As Boolean
    ...
    If mlngKey <> 0 Then
        gstrSQL = "select 项目名称 From 护理记录项目 where 项目序号=[1]"
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, "护理波动项目", mlngKey)
        If rs.RecordCount > 0 Then
            If NVL(rs!项目名称) <> Trim(txt(0).Text) Then
                If mfrmMain.CheckItemExistData(2, NVL(rs!项目名称)) = True Then
...
Private Sub vsf_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    Case 1
        For lngLoop = 1 To vsf.Rows - 1
            If lngLoop <> Row Then
                If Trim(vsf.TextMatrix(lngLoop, 1)) = Trim(vsf.EditText) Then
