...
Dim lKSS As Long, lKSE As Long, lKES As Long, lKEE As Long, lKey As Long, bFinded As Boolean, sKeyType As String, bNeeded As Boolean
    ...
    If blnClearBColor Then
        For j = 1 To oEles.Count '删除空的要素，展开型要素重新刷新以去掉下波浪线
            If oEles(j).内容文本 = "" Or oEles(j).终止版 <> 0 Then
...
End Function
Public Function MergeRTF(strSourceRTF As String, strTargetRTF As String) As String
'合并的方法是源内容去掉首{，目标内容去掉尾部的 vbCrLf & "\par }" & vbCrLf
'rtf首尾由{}组成，其中尾部有VBCRLF，}前还有\par表示段落，如果不去掉，合并后会多出一行空行
Dim strSource As String, strTarget As String
    
    strSource = strSourceRTF
    strSource = Substr(strSource, 2, Len(strSource))
    strTarget = strTargetRTF
    strTarget = Substr(strTarget, 1, InStr(strTarget, vbCrLf & "\par }" & vbCrLf))
    MergeRTF = strTarget & strSource
End Function
...
' hWnd 一般可用Me.HWnd，也可为0
    ...
    EmptyClipboard
    If sText = "" Then CloseClipboard: Exit Function
    ' ANSI
