...
Public Function ShowMe(ByVal frmParent As Object, ByVal lngFileID As Long) As Boolean
    ...
    If ExecuteCommand("初始数据") = False Then GoTo EndPoint
...
Dim objNode As Node
    ...
    Case "读取诊断明细"
        ...
        gstrSQL = "Select i.Id, i.编码, i.名称, zlSpellcode(i.名称) As 简码" & _
                ...
                " Where i.id = j.诊断id And j.分类id = [1] And i.Id = s.诊断id(+) And s.诊断id Is Null"
        Set rs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, CLng(Mid(tvwClass.SelectedItem.Key, 2)), mlngFileID)
        With rs
            lvwItems.ListItems.Clear
            Do While Not .EOF
...
Dim objItem As ListItem
    ...
    Else
        ...
        gstrSQL = "Select i.Id, i.编码, i.名称, zlSpellCode(i.名称) As 简码" & _
                ...
                " Where (i.编码 like [1] or i.名称 like [2] or zlSpellCode(i.名称) like [2]) And i.Id = s.诊断id(+) And s.诊断id Is Null"
    End If
...
Private Function InitDiseaseList() As Boolean
    Dim rsTemp As ADODB.Recordset
    Dim strSQL As String
    On Error GoTo errHand
    
    strSQL = "select 编码,简码,名称 from 传染病目录"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
    If Not rsTemp Is Nothing Then
        If rsTemp.RecordCount > 0 Then
            Do While Not rsTemp.EOF
                cboReport.AddItem NVL(rsTemp!简码) & "-" & NVL(rsTemp!名称)
                rsTemp.MoveNext
            Loop
        End If
    End If
