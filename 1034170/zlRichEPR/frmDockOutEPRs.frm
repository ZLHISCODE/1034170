...
Dim rs传染 As ADODB.Recordset
    gstrSQL = "Select r.Id, r.病历种类, r.病历名称, r.创建人 As 创建人, To_Char(r.创建时间, 'yyyy-mm-dd hh24:mi') As 创建时间, r.保存人," & _
            ...
            "        d.名称 As 科室名, r.处理状态,r.打印人,To_Char(r.打印时间, 'yyyy-mm-dd hh24:mi') As 打印时间,Decode(r.编辑方式,2,0,r.编辑方式) as 编辑方式,null as 申报状态" & _
            " From 电子病历记录 r, 部门表 d" & _
            " Where r.科室id = d.Id And r.病人来源 = 1 And r.病历种类 In (1, 5, 6) And r.病人id = [1] And Nvl(r.主页id, 0) = [2]" & _
    ...
    gstrSQL = "Select a.处理状态,b.id,C.执行部门ID,C.执行状态 From 疾病申报记录 a,电子病历记录 b, 病人挂号记录 c  where a.文件id=b.id and b.病历种类=5" & vbNewLine & _
        "and b.病人id+0=c.病人id and b.主页id+0=c.id and c.id=[1] and a.处理状态 in (4,5)"
...
Dim lFileId As Long, strPrintName As String
    ...
    If vfgThis.TextMatrix(vfgThis.Row, mCol.编辑方式) = 0 Then
        ...
        mfrmPrintPreview.DoMultiDocPreview Me, cpr门诊病历, , , vfgThis.Cell(flexcpText, vfgThis.Row, mCol.病历种类) _
                        , , vfgThis.Cell(flexcpText, vfgThis.Row, mCol.ID), Not blnPreview, , , mblnMoved, mlngAdviceID, strPrintName, IIf(InStr(mstrPrivs, "取消打印") > 0, 0, 1)
