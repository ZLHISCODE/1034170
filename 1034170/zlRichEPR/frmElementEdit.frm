...
Public Sub ShowMe(ByRef Ele As cEPRElement, ByVal X As Long, ByVal Y As Long, _
    ...
    With Me.Element
        ...
        Case 0
            If Me.Element.替换域 = 1 And EditType = cprET_病历文件定义 Then
                ...
                ElseIf ofrmParent.Name = "frmTableEditor" Then
                    lbl示例 = GetReplaceEleValue(lbl名称, ofrmParent.frmParent.Document.EPRPatiRecInfo.病人ID, ofrmParent.frmParent.Document.EPRPatiRecInfo.主页ID, ofrmParent.frmParent.Document.EPRPatiRecInfo.病人来源, ofrmParent.Document.EPRPatiRecInfo.医嘱i)
                    If lbl示例 = "" Then
                        lbl示例.Visible = False
                        Label3.Visible = False
                        Me.Height = 1250
        ...
        Case 2
            ...
            Else
                ...
                End If
            End If
'            vfg单选.Cell(flexcpChecked, 1, 0, vfg单选.Rows - 1, 0) = flexUnchecked
            ...
            For i = 0 To vfg单选.Rows - 1
                If strTmp = vfg单选.Cell(flexcpText, i, 2) And blnFinded = False Then
                    'vfg单选.Cell(flexcpChecked, i, 1) = flexChecked
                    vfg单选.Cell(flexcpPicture, i, 1) = imgOpt2.Picture
                    vfg单选.Row = i
                    blnFinded = True
                Else
                    'vfg单选.Cell(flexcpChecked, i, 1) = flexUnchecked
                    vfg单选.Cell(flexcpPicture, i, 1) = imgOpt1.Picture
                End If
            Next
        Case 3
    ...
    Me.Left = X
    Me.Top = Y
    Me.Width = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "MainWidth", 2500)
    If Me.Element.要素表示 <> 1 And (Me.Element.替换域 = 0 Or EditType = cprET_单病历编辑 Or EditType = cprET_单病历审核) Then
        Me.Height = GetSetting("ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "MainHeight", 3690)
    End If
    Call Form_Resize
...
Private Sub mnu_Copy_Click()
    ...
    If glngTXTProc <> 0 Then
        Call SetWindowLong(txt文本.hWnd, GWL_WNDPROC, glngTXTProc)
        glngTXTProc = 0
    End If
    gstrCopyPID = CStr(frmParent.Document.EPRPatiRecInfo.病人ID)
    Clipboard.SetText txt文本.SelText
End Sub
...
Private Sub txt文本_KeyPress(KeyAscii As Integer)
    ...
    End If
End Sub
Private Sub txt文本_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Button = 2 Then
        glngTXTProc = GetWindowLong(txt文本.hWnd, GWL_WNDPROC)
        Call SetWindowLong(txt文本.hWnd, GWL_WNDPROC, AddressOf WndMessage)
        PopupMenu mnu_Edit
        Call SetWindowLong(txt文本.hWnd, GWL_WNDPROC, glngTXTProc)
        glngTXTProc = 0
    End If
...
Private Sub txt文本_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
...
Private Sub Form_Deactivate()
    ...
    If Me.Element.要素表示 <> 1 And (Me.Element.替换域 = 0 Or EditType = cprET_单病历编辑 Or EditType = cprET_单病历审核) Then SaveSetting "ZLSOFT", "私有模块\" & gstrDBUser & "\" & App.ProductName & "\" & Me.Name, "MainHeight", Me.Height
...
Private Sub Form_Resize()
    ...
    If Not Me.Element Is Nothing Then
        ...
        Else
            txtTip.Move 80, picTitle.Height + 100, ScaleWidth - 160, (UBound(Split(Element.提示, vbCrLf)) + 2) * TextHeight("提")
            txtTip.Text = "提示:  " & Element.提示
            txtTip.Visible = True
        End If
        ...
        Case 0
            If Me.Element.替换域 = 1 And EditType = cprET_病历文件定义 Then
                pic替换项目.Move 80, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 120, ScaleWidth - 160, ScaleHeight - 200 - picStatus.Height - picTitle.Height - IIf(Element.提示 = "", 0, txtTip.Height)
            ...
            Else
                txt文本.Move 80, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 120, ScaleWidth - 160, ScaleHeight - 200 - picStatus.Height - picTitle.Height - IIf(Element.提示 = "", 0, txtTip.Height)
        ...
        Case 1
            ...
            If Trim(Element.要素单位) <> "" Then
                ...
                lbl单位.Move Me.ScaleWidth - lbl单位.Width + lX * 3, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 170
                lbl单位.Visible = True
                lW5 = lbl单位.Width
            Else
                lbl单位.Visible = False
            ...
            End If
            lW4 = ud上下.Width + lX * 4
            ud上下.Move Me.ScaleWidth - lW4 - lW5 + lX * 3, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 120
            ud上下.Visible = True
            If Element.要素小数 > 0 Then
                txt上下2.Width = Me.TextWidth(Space(Element.要素小数)) + lX * 4
                lW3 = txt上下2.Width + lX
                txt上下2.Move Me.ScaleWidth - lW5 - lW4 - lW3 + lX, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 170
                ...
                lW2 = lblDot.Width
                lblDot.Move txt上下2.Left - lW2 + lX * 2, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 170
                lblDot.BackStyle = 0
                lblDot.Visible = True
            Else
                lW2 = 0
            ...
            Me.Height = txt上下1.Height + lY * 3 + picStatus.Height + picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 180
            
            txt上下1.Move 80, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 170, Me.ScaleWidth - lW5 - lW4 - lW3 - lW2 - lX * 4
        ...
        Case 2
            vfg单选.Move 80, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 120, ScaleWidth - 160, ScaleHeight - 200 - picStatus.Height - picTitle.Height - IIf(Element.提示 = "", 0, txtTip.Height)
        ...
        Case 3
            vfg复选.Move 80, picTitle.Height + IIf(Element.提示 = "", 0, txtTip.Height) + 120, ScaleWidth - 160, ScaleHeight - 200 - picStatus.Height - picTitle.Height - IIf(Element.提示 = "", 0, txtTip.Height)
...
Private Sub vfg复选_DblClick()
Dim lngCol As Long
    lngCol = vfg复选.MouseCol
    If lngCol = 1 Then
        Form_KeyPress vbKeyReturn
    End If
End Sub
