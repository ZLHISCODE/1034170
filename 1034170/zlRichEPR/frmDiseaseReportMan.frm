...
'-----------------------------------------------------
Private Enum mCol
    图标 = 0: ID: 状态: 报告: 科室: 就诊号: 姓名: 性别: 年龄: 填报时间: 填报人: 信息: 数据转出: 病人ID: 主页ID: 文件ID
End Enum
...
Dim rptRow As ReportRow
    ...
    If strFiles <> "" Then
        ...
        gstrSQL = "Select l.Id,l.文件id,l.病人ID,l.主页ID, l.病历名称 As 报告, Decode(l.病人来源, 1, '门诊: ', 2, '住院: ', '') || d.名称 As 科室," & _
                ...
                "        To_Char(l.完成时间, 'yyyy-mm-dd hh24:mi') As 填报时间, l.保存人 As 填报人," & _
                ...
                " From (Select l.Id,l.文件id,l.病人ID,l.主页ID, l.病历名称, l.病人来源, l.科室id, l.完成时间, l.保存人, l.保存时间," & _
        ...
        If strFiles <> "" Then
            ...
            Do Until rsTemp.EOF
                gstrSQL = "Select '" & rsTemp!ID & "' As ID, p.病人id, '" & rsTemp!AntetypeId & "' As 文件id, q.主页id," & vbNewLine & _
                            ...
                            "       '" & Format(rsTemp!完成时间, "yyyy-mm-dd HH:MM") & "' As 填报时间, p.姓名, p.性别, p.年龄,0 As 数据转出, '" & rsTemp!编辑人 & "' As 填报人," & vbNewLine & _
                            ...
                            "From 病人信息 P, 病案主页 Q, 部门表 A, 人员表 B, 部门人员 C" & vbNewLine & _
                            "Where p.病人id = [2] And p.病人id = q.病人id(+) And To_Date([3], 'yyyy-mm-dd hh24:mi:ss') Between q.入院日期(+) And" & vbNewLine & _
                            "      Nvl(q.出院日期(+), Sysdate) And b.姓名 = '" & rsTemp!完成人 & "' And c.人员id = b.Id And Nvl(c.缺省, 0) = 1 And c.部门id = a.Id" & vbNewLine & _
                            IIf(strPatient <> "", " And P.姓名=[5]", "") & IIf(lngOutNo = 0, "", " And p.门诊号=[6]") & IIf(lngInNo = 0, "", " And Q.住院号=[7]")
                Set rsData = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, CStr(rsTemp!ID), CLng(rsTemp!病人ID), Format(rsTemp!事件时间, "yyyy-MM-dd hh:mm:ss"), strSender, strPatient, lngOutNo, lngInNo)
...
Dim frmPrint As frmPrintPreview
Dim rsTemp As New ADODB.Recordset
    If mstrCurId = "" Then Exit Sub
    Err = 0: On Error GoTo errHand
    If IsNumeric(mstrCurId) Then
        gstrSQL = "Select l.病人来源, l.病人id, l.主页id, f.页面 From 电子病历记录 l, 病历文件列表 f Where l.文件id = f.Id And l.Id = [1]"
...
Dim cbrControl As CommandBarControl, strInfo As String
    ...
    Case conMenu_Edit_Audit '审核病历
        ...
        Else
            ...
            Call objAudit.zlRefresh(rptList.FocusedRow.Record.Item(mCol.病人ID).Value, rptList.FocusedRow.Record.Item(mCol.主页ID).Value, glngDeptId, 0, 2)
...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
    Case conMenu_File_Open, conMenu_File_Preview, conMenu_File_Print: Control.Enabled = (mstrCurId <> "")
...
Private Sub rptList_SelectionChanged()
    ...
    If IsNumeric(mstrCurId) Then
        ...
        Call mfrmPreview.zlRefresh(CLng(mstrCurId), "", , mblnCurMoved)
