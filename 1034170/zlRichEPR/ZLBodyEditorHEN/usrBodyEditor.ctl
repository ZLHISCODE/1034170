   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.PictureBox pic 
         ...
         Begin VB.PictureBox picLine 
            ...
            Width           =   15
         End
         Begin zl9BodyEditorHEN.VsfGrid vsf 
            Height          =   270
            Left            =   450
            TabIndex        =   5
            Top             =   4440
...
Private mArrTmp(0 To 41) As Variant
Private mblnAllowDraw As Boolean
...
Public Property Let Marr呼吸(vData() As Variant)
    Dim i As Integer
...
Public Property Get 是否病人本人() As Boolean
    If Not mrsParam Is Nothing Then
        If mrsParam.State = adStateOpen Then
            是否病人本人 = (Val(mrsParam("婴儿").Value) = 0)
        End If
    End If
End Property
...
Public Property Get Getvsf() As Object
    Set Getvsf = vsf
End Property
...
Public Property Get 呼吸表格() As Boolean
    呼吸表格 = Not mbln呼吸曲线
End Property
...
Public Function zlMenuClick(ByVal strMenuItem As String, Optional ByVal strParam As String) As Boolean
    ...
    Dim strFrom As String, strTime As String
    ...
    Case "初始数据"
        ...
        Call CreateParam(mrsParam, "入院时间", adVarChar, 20) '病人入院时间
...
'             If InitBody(Val(mrsParam("病人id").Value), Val(mrsParam("主页id").Value), Val(mrsParam("病区id").Value), Val(mrsParam("婴儿").Value)) = False Then Exit Function
             ...
             mrsParam("入院时间").Value = CStr(varParam(5)) '病人入院时间
...
'            Call UserControl_Resize
    ...
    Case "恢复数据"
        ...
        Call DrawGraph
    '------------------------------------------------------------------------------------------------------------------
    '39289:刘鹏飞,2013-08-07,允许设置体温单开始时间(入院还是入科)
    Case "设定开始日期"
        If Not frmTendBodyDateSet.ShowMe(Val(mrsParam("病人ID").Value), Val(mrsParam("主页id").Value)) Then Exit Function
        zlMenuClick = True
    '------------------------------------------------------------------------------------------------------------------
    Case "操作曲线"
    ...
    Case "添加项目"
        ...
        Dim int频次 As Integer, intColNum As Integer
        
        intRows = mshDownTab.Rows
        
        
        strNotItem = ""
        For intLoop = LBound(mItemStru) To UBound(mItemStru)
            ...
            If ShowTxtSelDialog(mfrmParent, Nothing, "名称,1500,0,1;单位,900,0,0;最小值,900,0,0;最大值,900,0,0", mfrmParent.Name & "\护理项目选择", "请从下面选择一个护理项目。", rsData, rsTmp, 6000, 3000, , , 2, False) Then
                If rsTmp.BOF = False Then
                    If AppendGridItem(rsTmp, True) Then
                        Call picPane_Resize
                        For intRow = intRows To mshDownTab.Rows - 1
                            int频次 = Val(mItemStru(intRow).记录频次)
                            If int频次 > 2 Or int频次 < 1 Then int频次 = 2
                            Select Case int频次
                                Case 1
                                    intColNum = 2
                                Case 2
            ...
            With mshScale
                ...
                If .TextMatrix(1, intNowCol) <> "" Then
                    If Split(.TextMatrix(1, intNowCol), ";")(mItemSerial.体温 + 1) <> "" Then
                        If Not ChekIsInput(intNowCol) Then Exit Function
    ...
    Case "填写记录线"
        ...
        If picScale.Tag <> "" Then
            intCOl = lblCur.Left \ HOUR_STEP_Twips
            If Not ChekIsInput(intCOl) Then Exit Function

            Call CalcMinMaxCol(picScale.Tag, intMinCol, intMaxCol)
            If frmCaseTendBodySetLine.ShowEdit(UserControl.Extender, lblCur.Left \ HOUR_STEP_Twips, intMinCol, intMaxCol, mrsParam("护理等级").Value, mint心率应用, mArrTmp) Then
                RaiseEvent DataChanged(True)
                '组织呼吸曲线输出
                Call ResizeBreath
            End If
    ...
    Case "清除记录线"
        ...
        If picScale.Tag <> "" Then
            ...
            If frmCaseTendBodyDelLine.ShowEdit(UserControl.Extender, lblCur.Left \ HOUR_STEP_Twips, Val(mrsParam("护理等级").Value), Val(mrsParam("婴儿").Value), mArrTmp) Then
                RaiseEvent DataChanged(True)
                '组织呼吸曲线输出
                Call ResizeBreath
            End If
...
Public Function ChekIsInput(ByVal intCOl As Integer) As Boolean
    Dim strFrom As String, strTmp As String, strTime As String
    If picScale.Tag <> "" Then
        strFrom = Split(picScale.Tag, ";")(0)
        strTmp = GetCurveDateTime(intCOl + mshScale.FixedCols, CDate(strFrom), mlngHourBegin)
         strTime = Split(strTmp, ",")(0)
        If Not IsAllowInput(Val(mrsParam("病人id")), Val(mrsParam("主页id")), strTime, Format(zlDatabase.Currentdate, "yyyy-MM-dd HH:mm")) Then
            MsgBox "超过数据补录的有效时限:" & mlngHours & "小时!", vbInformation, gstrSysName
            Exit Function
        End If
    End If
    ChekIsInput = True
...
'    Dim intPageNo As Integer
    ...
    '------------------------------------------------------------------------------------------------------------------
    '提取病案主页从表中的体温单开始日期
    strSQL = " Select 信息值 From 病案主页从表 Where 病人ID=[1] And 主页ID=[2] And 信息名='体温单开始日期'"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "提取病案主页从表中的体温单开始日期", Val(mrsParam("病人id")), Val(mrsParam("主页id")))
    If rsTemp.RecordCount <> 0 Then
        strEnterDate = Format(rsTemp!信息值, "yyyy-MM-dd HH:mm:ss")
    End If
    If Not IsDate(strEnterDate) Then strEnterDate = ""
    
    strSQL = "Select Decode(c.出生时间,Null," & IIf(strEnterDate = "", "b.入院时间", "to_date('" & strEnterDate & "','yyyy-MM-dd hh24:mi:ss')") & ",c.出生时间) As 入院时间, 出院时间," & _
                "   1 + Round((b.出院时间 - Decode(c.出生时间,Null," & IIf(strEnterDate = "", "b.入院时间", "to_date('" & strEnterDate & "','yyyy-MM-dd hh24:mi:ss')") & ",c.出生时间)) / 7) As 页数" & vbNewLine & _
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, mstrMsgTitle, Val(mrsParam("病人id")), Val(mrsParam("主页id")), intBaby)
    intCount = 0
...
Private Function InitBody(ByVal lng病人id As Long, ByVal lng主页id As Long, ByVal lng病区id As Long, ByVal int婴儿 As Integer) As Boolean
    ...
    '提取病案主页从表中的体温单开始日期
    strSQL = " Select 信息值 From 病案主页从表 Where 病人ID=[1] And 主页ID=[2] And 信息名='体温单开始日期'"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "提取病案主页从表中的体温单开始日期", lng病人id, lng主页id)
    If rsTemp.RecordCount <> 0 Then
        strEnterDate = Format(rsTemp!信息值, "yyyy-MM-dd HH:mm:ss")
    End If
    If Not IsDate(strEnterDate) Then strEnterDate = ""
    ...
    '------------------------------------------------------------------------------------------------------------------
     strSQL = "Select Decode(c.出生时间,Null," & IIf(strEnterDate = "", "b.入院时间", "to_date('" & strEnterDate & "','yyyy-MM-dd hh24:mi:ss')") & ",c.出生时间) As 入院时间, 出院时间," & _
            "   Decode(C.出生时间,NUll,B.入院时间,C.出生时间) 实际入院时间," & vbNewLine & _
            "   1 + Round((b.出院时间 - Decode(c.出生时间,Null," & IIf(strEnterDate = "", "b.入院时间", "to_date('" & strEnterDate & "','yyyy-MM-dd hh24:mi:ss')") & ",c.出生时间)) / 7) As 页数" & vbNewLine & _
            "   From (Select 病人ID,主页id,Min(开始时间) as 入院时间," & vbNewLine & _
            "               Max(Nvl(终止时间, Sysdate)) as 出院时间" & vbNewLine & _
            "          From 病人变动记录" & vbNewLine & _
    ...
    '病人实际入院时间
    strBeginDate = Format(rsTmp!实际入院时间, "yyyy-MM-dd HH:mm:ss")
    ...
    For lngLoop = 0 To rsTmp("页数").Value - 1
        ...
        If strDateFrom < Format(rsTmp("出院时间").Value, "yyyy-MM-dd HH:mm:ss") Then
            ...
            For intCOl = 1 To rs.RecordCount
                ...
                cbrItem.Parameter = strEnterDate & ";" & rs!病区ID & ";" & strDateFrom & ";" & strDateTo & ";" & lngLoop & ";" & strBeginDate
...
Private Function CheckTimeRange(ByVal intCOl As Integer, Optional ByVal blnCheckInput As Boolean = False) As Boolean
...
ErrNext:
    ...
    If CheckTimeRange = False And intCount < 1 Then
        intCOl1 = (intCOl - mshDownTab.FixedCols + 1) Mod 2
        If int频次 = 1 Then
             If intCOl1 = 1 Then
                intCOl = intCOl + 1
             Else
        ...
        End If
    End If
    If CheckTimeRange = True And blnCheckInput = True Then
        If intCount = 0 And int频次 = 1 Then
            strEnd = Format(strEnd, "YYYY-MM-DD") & " 23:59:59"
            If strEnd > strTo Then strEnd = strTo
        End If
        intCOl1 = GetCurveColumn(strEnd, CDate(strFrom), mlngHourBegin) - 1
        CheckTimeRange = ChekIsInput(intCOl1)
    End If
End Function
...
Private Function InitData() As Boolean
    ...
    vsf.FixedRows = 1
    vsf.Rows = 2
    vsf.Body.ROWHEIGHT(vsf.FixedRows) = 500
    vsf.Height = 500
    
    '复制呼吸位图
    Call DrawPicture(picBuffer, "BREATH", 0, 0, gintBmpW, gintBmpH, True)
...
'    Erase mvarDataType
    ...
    With rsTmp
        If rsTmp.RecordCount > 0 Then
            ...
            mshDownTab.Rows = 1
            mshDownTab.MergeCells = flexMergeRestrictRows
...
Private Function AppendGridItem(ByVal rsTmp As ADODB.Recordset, Optional ByVal blnAppend As Boolean) As Boolean
    ...
    Case "呼吸"
        ...
        vsf.ColData(0) = mItemOtherStru(0).最大值 & ";" & mItemOtherStru(0).最小值 & ";" & ";1"
    '------------------------------------------------------------------------------------------------------------------
    Case "舒张压"
    ...
    Case Else
        ...
        mshDownTab.TextMatrix(intRow, 1) = String(13, ";")
...
Private Function ShowOpsDays() As Boolean
    ...
    '找开始日期-14天前的手术次数
   ' mstrSQL = "SELECT Nvl(Count(a.发生时间),0) As 次数 "
    mstrSQL = "SELECT c.项目名称 " & _
    ...
    lng次数 = rsTmp.RecordCount
    rsTmp.Filter = "项目名称='分娩'"
    lngOpsNum = lng次数 - rsTmp.RecordCount
    ...
    For intCOl = 1 To mshUpTab.Cols - 1
        ...
        If DateDiff("d", CDate(strFrom), CDate(Split(picScale.Tag, ";")(1))) + 1 >= intCOl Then
            ...
            If mshUpTab.ColData(intCOl) = 1 Or mshUpTab.ColData(intCOl) = 2 And lng次数 < 12 Then
                ...
                '重新整理手术标记输出。
                intCount = GetCurveColumn(mstrOpsDays(intCOl), CDate(strFrom), mlngHourBegin) + mshScale.FixedCols - 1
                If intCount > 0 And intCount < mshScale.Cols Then
                    strOpsName = Trim(Mid(mshScale.TextMatrix(3, intCount), 1, 4))
                    If strOpsName <> "手术分娩" Then
                        strOpsName = Trim(Mid(mshScale.TextMatrix(3, intCount), 1, 2))
                    End If
                    Select Case strOpsName
                        Case "分娩"
                            If intCount > 0 And intCount < mshScale.Cols And mBodyFlag.分娩 > 0 Then
                                If mBodyFlag.分娩 = 2 Then
                                    mshScale.TextMatrix(3, intCount) = strOpsName & "于" & ConvertTimeToChinese(Format(mstrOpsDays(intCOl), "HH:mm"))
                                Else
                                    mshScale.TextMatrix(3, intCount) = strOpsName
                                End If
                                
                                 mshScale.Cell(flexcpData, 3, intCount, 3, intCount) = Format(mstrOpsDays(intCOl), "HH:mm:ss")
        
                            End If
                        Case Else
                            If intCount > 0 And intCount < mshScale.Cols And mBodyFlag.手术 > 0 Then
                                lngOpsNum = lngOpsNum + 1
                                strOpsName = strOpsName & IIf(lngOpsNum <= 1, "", lngOpsNum)
                                If mBodyFlag.手术 = 2 Then
                                    mshScale.TextMatrix(3, intCount) = strOpsName & "于" & ConvertTimeToChinese(Format(mstrOpsDays(intCOl), "HH:mm"))
                                Else
                                    mshScale.TextMatrix(3, intCount) = strOpsName
...
'                    End If
                Else
                    ...
                    If mshUpTab.TextMatrix(2, intCOl) <> "" Then
                        mshUpTab.TextMatrix(2, intCOl) = mshUpTab.TextMatrix(2, intCOl) ' strTmp & "/" & mshUpTab.TextMatrix(2, intCOl)
                    Else
                    ...
                    If Val(strTmp) <= mintOpDays Then
                        ...
                        Else
                            If mshUpTab.TextMatrix(2, intLoop) <> "" Then
                                mshUpTab.TextMatrix(2, intLoop) = strTmp & "/" & mshUpTab.TextMatrix(2, intLoop)
                            Else
                                mshUpTab.TextMatrix(2, intLoop) = strTmp
                            End If
                        End If
...
'    lblTime.Caption = "日期:" & Format(strFrom, "MM-DD") & "～" & Format(strTo, "MM-DD")
    ...
    '填写日期和住院天数等信息
    '------------------------------------------------------------------------------------------------------------------
    '如果病人23号入科22入院 体温单开始时间以23号开始 住院天数显示不对 此处进行日期特殊处理
    strDate = DateAdd("D", -Abs(DateDiff("d", CDate(mstrEnterDate), CDate(CStr(mrsParam("入院时间"))))), CDate(strFrom))
    ...
    With mshUpTab
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, mstrMsgTitle, Val(mrsParam("病人id").Value), Val(mrsParam("主页id").Value), Val(mrsParam("婴儿").Value), (Int(CDate(strDate))))
        If rsTmp.BOF = False Then
        ...
        For intCOl = 1 To .Cols - 1
            ...
            ElseIf strTmp = Format(mstrEnterDate, "yyyy-MM-dd") Then
                '入院第一天，写上年份
                .TextMatrix(0, intCOl) = strTmp
            ElseIf intCOl = 1 Then '河南要求每页第一天也显示年月日
                .TextMatrix(0, intCOl) = strTmp ' Right(strTmp, 5)
        ...
        Do While Not rsTmp.EOF
            ...
            For intCOl = 1 To mshUpTab.Cols - 1
                ...
                If DateDiff("d", CDate(strFrom), CDate(Split(picScale.Tag, ";")(1))) + 1 >= intCOl Then
                    ...
                    Case 1 To mintOpDays
                        ...
                        If intDays >= intCOl Then
                            ...
                            If mshUpTab.TextMatrix(2, intCOl) <> "" And Not mblnStopFlag Then
                                mshUpTab.TextMatrix(2, intCOl) = intDays & "/" & mshUpTab.TextMatrix(2, intCOl)
                            Else
                    ...
                    If intCount > 0 And intCount < mshScale.Cols Then
                        mshScale.TextMatrix(3, intCount) = ""
                        Select Case mshScale.TextMatrix(3, intCount)
                            Case "分娩"
                                If mBodyFlag.分娩 > 0 Then mshScale.TextMatrix(3, intCount) = rsTmp("项目名称").Value
                            Case Else
                                If mBodyFlag.手术 > 0 Then mshScale.TextMatrix(3, intCount) = rsTmp("项目名称").Value
                        End Select
                    End If
                    '转到ShowOpsDays中进行处理
...
'                        If intCount > 0 And intCount < mshScale.Cols And mBodyFlag.分娩 > 0 Then
'                            If mBodyFlag.分娩 = 2 Then
'                                mshScale.TextMatrix(3, intCount) = rsTmp("项目名称").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
'                            Else
...
'                    End Select
    ...
    bln入科显入院 = CDate(CStr(mrsParam("入院时间"))) < CDate(mstrEnterDate)
        ...
        If rsTmp.BOF = False Then
            Do While Not rsTmp.EOF
                ...
                If zlCommFun.NVL(rsTmp("内容")) <> "" Then
                    ...
                    '如果体温单开始时间以入科开始 则入科显示未入院,如果入院时间和入科时间相等 如果入院为不显示同样 入科显示入院
                    If bln入科显入院 = True Then
                        If Val(rsTmp("行号").Value) = 5 Then
                            bytShow = 0
                        ElseIf Val(rsTmp("行号").Value) = 9 Then
                            str内容 = "入院" '入科改成入院
                        End If
                    Else
                        If CDate(CStr(mrsParam("入院时间"))) = CDate(mstrEnterDate) And mBodyFlag.入院 = 0 Then
                            If Val(rsTmp("行号").Value) = 9 Then
                                 str内容 = "入院" '入科改成入院
                            End If
                        End If
                    End If
                    ...
                    If intCOl >= mshScale.FixedCols And intCOl < mshScale.Cols And bytShow > 0 Then
                        ...
                        If blnShow Then
                            ...
                            Select Case bytShow
                            Case 1
                                mshScale.TextMatrix(Val(rsTmp("行号").Value), intCOl) = str内容
                            Case 2
                                mshScale.TextMatrix(Val(rsTmp("行号").Value), intCOl) = str内容 & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                            Case 3
                                mshScale.TextMatrix(Val(rsTmp("行号").Value), intCOl) = str内容 & rsTmp("科室").Value
                            Case 4
                                mshScale.TextMatrix(Val(rsTmp("行号").Value), intCOl) = str内容 & rsTmp("科室").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                            End Select
                        End If
                    End If
                End If
                ...
                Do While Not rsTmp.EOF
                    ...
                    If zlCommFun.NVL(rsTmp("内容")) <> "" Then
                        ...
                        If intCOl >= mshScale.FixedCols And intCOl < mshScale.Cols And mBodyFlag.出生 > 0 Then
                            ...
                            Case 2
                                mshScale.TextMatrix(Val(rsTmp("行号").Value), intCOl) = rsTmp("内容").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                            Case 3
                                mshScale.TextMatrix(Val(rsTmp("行号").Value), intCOl) = rsTmp("内容").Value & rsTmp("科室").Value
                            Case 4
                                mshScale.TextMatrix(Val(rsTmp("行号").Value), intCOl) = rsTmp("内容").Value & rsTmp("科室").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                            End Select
    ...
    mstrSQL = "SELECT a.发生时间 As 时间,c.记录内容 As 数值,c.体温部位,c.复试合格,D.记录名,E.保留项目,D.项目序号,C.记录标记,C.未记说明 " & _
...
Private Function ReadGridData(ByVal byt护理等级 As Byte, _
    ...
    Dim arrTime(0 To 13) As String
    Dim blnChanged As Boolean
    Dim intCount As Integer
    Dim strPart As String  '呼吸部位
    Dim strTmp As String
    Dim int频次 As Integer
...
'    End With
  ...
  '读取数据
    ...
    If strItemName <> "" Then
        ...
        mstrSQL = "SELECT a.发生时间 As 时间,C.记录内容 As 结果,nvl(C.体温部位,'') 部位,C.未记说明,E.保留项目,D.记录名,D.项目序号,nvl(D.记录频次,2) 记录频次 " & _
                    "FROM 病人护理记录 A,病人护理内容 C,体温记录项目 D,护理记录项目 E " & _
                    "Where A.ID = c.记录ID " & _
                        "AND A.病人来源=2 " & _
                        "AND Nvl(a.婴儿,0)=[6] " & _
                ...
                For i = 0 To rsTmp.RecordCount - 1
                    ...
                    Case mItemNo.呼吸
                        ...
                        If intCOl < vsf.Cols Then
                            strTmp = Trim(zlCommFun.NVL(!结果, ""))
                            strPart = Trim(zlCommFun.NVL(!部位, ""))
                            If strPart <> "呼吸机" Then strPart = ""
                            mArrTmp(intCOl - vsf.FixedCols) = strTmp & "-" & strPart & "-" & Trim(zlCommFun.NVL(!未记说明, ""))
                    ...
                    Case Else
                        ...
                        intCOl = Int((!时间 - Int(dt开始时间)) * 24) \ 12 + mshDownTab.FixedCols
                        strTime = Format(Int(dt开始时间) + (intCOl - mshDownTab.FixedCols) \ 2, "YYYY-MM-DD")
                        arrTime(intCOl - mshDownTab.FixedCols) = Format(!时间, "YYYY-MM-DD HH:mm:ss")
                        mshDownTab.TextMatrix(intRow, 1) = Join(arrTime, ";")
                        ...
                        If intCOl < mshDownTab.Cols Then
                            ...
                            Case 9          '出液量
                                ...
                                If Right(Trim(mshDownTab.TextMatrix(intRow, intColTmp)), 1) = "C" Then
                                    mshDownTab.Cell(flexcpData, intRow, intColTmp) = 4
                                End If
                                
                                If Right(Trim(mshDownTab.TextMatrix(intRow, intColTmp)), 2) = "/C" Then
...
Private Sub ResizeBreath()
    Dim int呼吸表格输出格式 As Integer
    Dim int呼吸位置 As Integer, i As Integer
    ...
    If mbln呼吸曲线 = False Then
        mblnAllowDraw = True
        For i = 0 To UBound(mArrTmp)
            If InStr(1, mArrTmp(i), "-") = 0 Then
                strTmp = mArrTmp(i)
                strPart = ""
            Else
                strTmp = Split(mArrTmp(i), "-")(0)
                strPart = Split(mArrTmp(i), "-")(1)
            End If
            
            intCOl = i + vsf.FixedCols
             '打印呼吸值（间隔错开打印） 第一行始终在上面
            If strPart = "" Then
                vsf.TextMatrix(1, intCOl) = strTmp
            ...
            Else
                ...
                vsf.TextMatrix(1, intCOl) = strTmp
                vsf.Cell(flexcpAlignment, 1, intCOl, 1, intCOl) = flexAlignCenterTop
'                If Not (vsf.Cell(flexcpPicture, 1, intCol) Is Nothing) Then Exit Function
'                Set vsf.Cell(flexcpPicture, 1, intCol, 1, intCol) = Nothing
'                Set vsf.Cell(flexcpPicture, 1, intCol) = ils16.ListImages(1).Picture
'                vsf.Cell(flexcpPictureAlignment, 1, intCol, 1, intCol) = flexAlignCenterTop
            End If
        Next i
    End If
    Exit Sub
...
Private Sub ResizeDownTab(ByVal dt开始时间 As Date, Optional ByVal intStartRow As Integer = 1)
'--------------------------------------------------
'功能:重新整理表下表格数据信息(主要是进行合并处理)
'--------------------------------------------------
    Dim strTime As String, strCenterTime As String, strTime1 As String, strValue As String
    Dim intRow As Integer, intCOl As Integer, i As Integer, int频次 As Integer
    ...
    For intRow = intStartRow To mshDownTab.Rows - 1
        lngItemNO = Val(mshDownTab.RowData(intRow))
        int频次 = Val(mItemStru(intRow).记录频次)
        If int频次 > 2 Or int频次 < 1 Then int频次 = 2
        arrTime = Split(mshDownTab.TextMatrix(intRow, 1), ";")
        '对频次为1 的项目进行和并
        If int频次 = 1 Then
            For i = mshDownTab.FixedCols To mshDownTab.Cols - 1
                intCOl = i - mshDownTab.FixedCols + 1
                strTime = Trim(arrTime(i - mshDownTab.FixedCols))
                If strTime = "" Then strTime = Format(Int(dt开始时间) + (intCOl - mshDownTab.FixedCols) \ 2, "YYYY-MM-DD")
                strTime = Format(strTime, "YYYY-MM-DD HH:mm:ss")
                strCenterTime = Format(Format(strTime, "YYYY-MM-DD") & " 12:00:00", "YYYY-MM-DD HH:mm:ss")
                Select Case lngItemNO
                    Case 7, 9 '入液量,出液量 汇总
                        If intCOl Mod 2 = 0 Then
                            intCOl = intCOl + mshDownTab.FixedCols - 1
                            mshDownTab.TextMatrix(intRow, intCOl - 1) = Space(intRow + intCOl - 1) & Trim(mshDownTab.TextMatrix(intRow, intCOl)) & Space(intRow + intCOl - 1)
                            mshDownTab.TextMatrix(intRow, intCOl) = Space(intRow + intCOl - 1) & Trim(mshDownTab.TextMatrix(intRow, intCOl)) & Space(intRow + intCOl - 1)
                        End If
                    Case Else '按照接近中点时间去 12 点
                        If intCOl Mod 2 = 0 Then
                            If strValue <> "" And Trim(mshDownTab.TextMatrix(intRow, i)) <> "" Then
                                If Abs(DateDiff("s", CDate(strTime), CDate(strCenterTime))) < Abs(DateDiff("s", CDate(strTime1), CDate(strCenterTime))) Then
                                    strValue = Trim(mshDownTab.TextMatrix(intRow, i))
                                End If
                            ElseIf strValue = "" And Trim(mshDownTab.TextMatrix(intRow, i)) = "" Then
                                strValue = ""
                            ElseIf strValue = "" Then
                                strValue = Trim(mshDownTab.TextMatrix(intRow, i))
                            End If
                            intCOl = intCOl + mshDownTab.FixedCols - 1
                            mshDownTab.TextMatrix(intRow, intCOl - 1) = Space(intRow + intCOl - 1) & strValue & Space(intRow + intCOl - 1)
                            mshDownTab.TextMatrix(intRow, intCOl) = Space(intRow + intCOl - 1) & strValue & Space(intRow + intCOl - 1)
                        Else
                            strTime1 = strTime
                            strValue = Trim(mshDownTab.TextMatrix(intRow, i))
                        End If
                ...
                If (i - mshDownTab.FixedCols + 1) Mod 2 = 0 Then
                    If Trim(mshDownTab.TextMatrix(intRow, intCOl - 1)) <> "" Then
                        If InStr(mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl - 1), ";") = 0 Then
                            mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl - 1) = "1"
                        Else
                            aryValue() = Split(mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl - 1), ";")
                            aryValue(intRow - 1) = 1
                            mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl - 1) = Join(aryValue, ";")
                        End If
                    End If
                End If
        ...
        Else
            For i = mshDownTab.FixedCols To mshDownTab.Cols - 1
                intCOl = i
                mshDownTab.TextMatrix(intRow, i) = Space(intRow + i) & Trim(mshDownTab.TextMatrix(intRow, i)) & Space(intRow + i)
                mshDownTab.Cell(flexcpAlignment, intRow, i) = flexAlignCenterCenter
                If Trim(mshDownTab.TextMatrix(intRow, intCOl)) <> "" Then
                    If InStr(mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl), ";") = 0 Then
                        mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl) = "1"
                    Else
                        aryValue() = Split(mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl), ";")
                        aryValue(intRow - 1) = 1
                        mshDownTab.TextMatrix(GridDataRow.修改标志, intCOl) = Join(aryValue, ";")
                    End If
                End If
            Next i
        End If
...
Private Function DrawScale() As Boolean
    ...
    X0 = 0
    'Y0 = mshScale.ROWHEIGHT(0) / 2 '原来  mshScale.ROWHEIGHT(0)=600 现在为400
...
Public Function DrawGraph() As Boolean
    ...
    Dim mpt脉搏() As POINTAPI
    Dim mpt心率() As POINTAPI
    ReDim byt脉搏(0 To mshScale.Cols - mshScale.FixedCols - 1)
    ReDim mpt脉搏(0 To mshScale.Cols - mshScale.FixedCols - 1)
    ReDim mpt心率(0 To mshScale.Cols - mshScale.FixedCols - 1)
    Dim rsPoint As ADODB.Recordset
    Dim rs As New ADODB.Recordset
...
'                End If
                ...
                If intCOl = mItemSerial.脉搏 Then
                    byt脉搏(intCount) = IIf(blnStop = True, 1, 0)
                End If
        ...
        '--------------------------------------------------------------------------------------------------------------
        'Call DrawPoly(picGraph, mpt脉搏, mpt心率)
        Call DrawPoly(picGraph, mpt脉搏, mpt心率, byt脉搏)
...
'            Y1 = ConvertToY(mItemSerial.体温, 42)
            ...
            If .TextMatrix(手术标志, intCOl + .FixedCols) <> "" Then
                ...
                rsTmp.Fields("结果").Value = Replace(.TextMatrix(手术标志, intCOl + .FixedCols), "手术分娩", "手术")
            End If
...
Private Function GetFontSize(ByVal objDraw As Object, ByVal dblHeight As Double, ByVal strText As String, ByRef Y1 As Single, Optional ByVal intType As GraphDataRow = 入院标志) As Single
    ...
    For sinFontSize = objDraw.FontSize To 5 Step -1
        ...
        For intCount = 1 To Len(strText)
            ...
            blnOps = (intType = 手术标志) And (Asc(strChar) > 0) And intLen >= intCount
            
            If Asc(strChar) < 0 Then
                If intCharNumber Mod 2 = 1 Then Y1 = Y1 + ROWHEIGHT * 2.5
            End If
            
            If Asc(strChar) < 0 Or blnOps = True Then
                intCharNumber = 0
                Y1 = Y1 + ROWHEIGHT * 5
            Else
                Y1 = Y1 + ROWHEIGHT * 2.5
...
Private Sub OutputNote(ByVal objDraw As Object, ByVal dblHeight As Double, ByRef rsNote As ADODB.Recordset)
    ...
    '手术标记变量(手术以第2此手术表示为手术2，其中的数字不进行反转)
    Dim intLen As Integer
    Dim blnOps As Boolean
        ...
        Do While Not .EOF
            If Trim(NVL(!结果)) <> "" Then
                If Not (!类型 = 未记说明 Or !类型 = 上标说明) Then
                    ...
                    '计算得到合适的字体大小及高度
                    !字体大小 = GetFontSize(objDraw, dblHeight, NVL(!结果), sinY1, !类型)
                    !高度 = sinY1
        ...
        Do While Not .EOF
            If Trim(NVL(!结果)) <> "" Then
                ...
                For intCount = 1 To Len(!结果)
                    ...
                    blnOps = (Val(NVL(!类型)) = 手术标志) And (Asc(strChar) > 0) And intLen >= intCount
                    If Asc(strChar) < 0 Then
                        If intCharNumber Mod 2 = 1 Then sinY1 = sinY1 + ROWHEIGHT * 2.5
                    End If
                    If blnOps = True Then strChar = Mid(!结果, intCount, intLen - intCount + 1)
                    Call DrawRotateText(objDraw, sinX1 - objDraw.TextWidth(strChar) / 2, sinY1 + 15, strChar, IIf(!类型 = 未记说明 Or !类型 = 上标说明, -2147483635, 255), , blnOps)
                    If Asc(strChar) < 0 Or blnOps = True Then
                        intCharNumber = 0
                    ...
                    Else
                        ...
                        intCharNumber = intCharNumber + 1
                    End If
                    If blnOps = True Then intCount = intLen
                Next
            End If
...
Private Function SaveData() As Boolean
    ...
    With mshScale
        ...
        For intCOl = .FixedCols To .Cols - 1
            ...
            End If
            '如果发生时间超出结束时间,则以开始时间为准
            If strMakeTime > strEnd Then strMakeTime = strTime
    ...
    With vsf
        For intCOl = 2 To .Cols - 1
            ...
            strEnd = Split(strTmp, ",")(1)
            strPart = ""
            If UBound(mArrTmp) > intCOl - .FixedCols Then
                If InStr(1, mArrTmp(intCOl - .FixedCols), "-") > 0 Then
                    strPart = Split(mArrTmp(intCOl - .FixedCols), "-")(1)
                End If
            End If
...
Private Sub mshDownTab_DblClick()
    ...
    If CheckTimeRange(mshDownTab.Col, True) = False Then Exit Sub
...
Private Function ShowInput() As Boolean
    ...
    With mshDownTab
        ...
        If mItemNo.血压 = Val(.RowData(.Row)) Then
            ...
            strTmp = Trim(.TextMatrix(.Row, .Col))
        ...
        Else
            ...
            txtInput(0).Text = Trim(.TextMatrix(.Row, .Col))
...
Private Sub mshDownTab_RowColChange()
    ...
    If (Val(mshDownTab.TextMatrix(mshDownTab.Row, 2)) <> 0 Or Val(mshDownTab.TextMatrix(mshDownTab.Row, 3)) <> 0) And mItemStru(mshDownTab.Row).数据类型 = 0 Then
        strInfo = "“" & mshDownTab.TextMatrix(mshDownTab.Row, 0) & "”项目范围：" & Val(mshDownTab.TextMatrix(mshDownTab.Row, 3)) & "～" & Val(mshDownTab.TextMatrix(mshDownTab.Row, 2)) & " " & strInfo
    End If
    ...
    Case vbKeyReturn ' 13     '设置手术日
        With mshUpTab
            ...
            If frmInputDate.ShowMe(strTime, Split(picScale.Tag, ";")(0), Split(picScale.Tag, ";")(1), strCaption) Then
                ...
                If Not ChekIsInput(intCOl) Then Exit Sub
                ...
                For intLoop = intStart To intEnd
                    ...
                    If Left(mshScale.TextMatrix(3, intLoop), 2) = "分娩" Then
                        mshScale.TextMatrix(3, intLoop) = ""
                    End If
                    If Left(mshScale.TextMatrix(3, intLoop), 4) = "手术分娩" Then
                        mshScale.TextMatrix(3, intLoop) = ""
                    End If
                Next
                ...
                Case "分娩"
                    If mBodyFlag.分娩 = 2 Then
                        mshScale.TextMatrix(3, intCOl) = strCaption & "于" & ConvertTimeToChinese(Format(strTime, "HH:mm"))
                    Else
...
Private Sub picGraph_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    '超过补录时限,就是改也不允许
    Dim strTmp As String, strTime As String, strFrom As String
    strFrom = Split(picScale.Tag, ";")(0)
    strTmp = GetCurveDateTime(intCOl + mshScale.FixedCols, CDate(strFrom), mlngHourBegin)
    strTime = Split(strTmp, ",")(0)
    If Not IsAllowInput(Val(mrsParam("病人id")), Val(mrsParam("主页id")), strTime, Format(zlDatabase.Currentdate, "yyyy-MM-dd HH:mm")) Then
    ...
    If Button = 1 Then
        ...
        ElseIf 体温项目 Then
            dblY = Format(ConvertToValue(mItemSerial.体温, Y), "#0.0")
            Y = ConvertToY(mItemSerial.体温, dblY)
        Else
            dblY = Format(ConvertToValue(mItemSerial.脉搏, Y), "#0")
            Y = ConvertToY(mItemSerial.脉搏, dblY)
        End If
...
Private Sub picPane_Resize()
    ...
    With mshScale
        ...
        .Height = .Rows * .ROWHEIGHT(.Rows - 1) + 400
        .Refresh
    End With
...
Private Function WriteDownTab(ByVal intRow As Integer, ByVal intCOl As Integer, ByVal strInput As String) As Boolean
    ...
    With mshDownTab
        ...
        strTmp = Trim(.TextMatrix(intRow, intCOl))
        
        int频次 = mItemStru(intRow).记录频次
        If int频次 > 2 And int频次 < 1 Then int频次 = 2
        If int频次 = 1 Then
            intColNum = intCOl - .FixedCols + 1
            If intColNum Mod 2 = 0 Then
                strText = Space(intRow + intCOl - 1) & strInput & Space(intRow + intCOl - 1)
                .TextMatrix(intRow, intCOl - 1) = strText
                .TextMatrix(intRow, intCOl) = strText
            Else
                strText = Space(intRow + intCOl) & strInput & Space(intRow + intCOl)
                .TextMatrix(intRow, intCOl) = strText
                .TextMatrix(intRow, intCOl + 1) = strText
            End If
        Else
            .TextMatrix(intRow, intCOl) = Space(intRow + intCOl) & Trim(strInput) & Space(intRow + intCOl)
            .Cell(flexcpAlignment, intRow, intCOl) = flexAlignCenterCenter
        End If
        
        If Trim(strTmp) = Trim(strInput) Then Exit Function
    ...
    With mshDownTab
        ...
        If mItemStru(intRow).数据类型 = 0 Then
            If Val(mshDownTab.RowData(mshDownTab.Row)) = mItemNo.血压 Then
                ...
                If strInput <> "" Then
                    If Trim(txtInput(0).Text) = "" Or Trim(txtInput(1).Text) = "" Then
                        strInput = ""
                        txtInput(0).Text = ""
                        txtInput(1).Text = ""
                        mshDownTab.SetFocus
...
Private Sub vsf_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    Else
        vsf.ColData(Col) = 4
    End If
    intIndex = Col - vsf.FixedCols
    If UBound(mArrTmp) >= intIndex Then
        strInput = mArrTmp(intIndex)
        strInput = strInput & String(2 - UBound(Split(strInput, "-")), "-")
        arrCode = Split(strInput, "-")
        arrCode(0) = vsf.TextMatrix(Row, Col)
        mArrTmp(intIndex) = Join(arrCode, "-")
    End If
...
'-----------------------------------
    ...
    If Not mblnAllowDraw Then Exit Sub
    If UBound(mArrTmp) = -1 Then Exit Sub
    ...
    If strPart <> "" Then
        ...
        '开始进行图形
        Call BitBlt(hDC, t_ClientRect.Left, t_ClientRect.Top, gintBmpW, gintBmpH, picBuffer.hDC, 0, 0, SRCCOPY)
    End If
End Sub
...
Private Sub vsf_KeyUp(KeyCode As Integer, Shift As Integer)
    If KeyCode = 46 Then        'Delete清除单元
        If Not ChekIsInput(vsf.Col - vsf.FixedCols) Then Exit Sub
...
Private Sub vsf_StartEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    If Not ChekIsInput(vsf.Col - vsf.FixedCols) Then Cancel = True
...
Private Function IsAllowInput(ByVal lng病人id As Long, ByVal lng主页id As Long, ByVal strTime As String, ByVal strCurTime As String) As Boolean
    '取出指定病人在指定时间之后关键点的时间
