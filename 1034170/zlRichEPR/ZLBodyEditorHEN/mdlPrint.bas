...
Public Function DrawPatiInfo(objDraw As Object, X As Long, Y As Long, strInfo As String, ByVal lngPaperWidth As Long) As Boolean
    ...
    If zlDatabase.GetPara("体温单显示诊断", glngSys, 1255, 1) = 0 Then
        strTmp = ""
    Else
        strTmp = "诊断:" & Split(strInfo, "'")(7)
    End If
    
    If X + objDraw.TextWidth("住院号:" & Split(strInfo, "'")(1) + "科室:" & Split(strInfo, "'")(2) + "床号:" & Split(strInfo, "'")(4) + strTmp) + W_9pt / 3 > lngPaperWidth Then
        
        X = lngPaperWidth - objDraw.TextWidth("住院号:" & Split(strInfo, "'")(1))
        Call DrawText(objDraw, X, Y, "住院号:", 0)
        Call DrawText(objDraw, X + objDraw.TextWidth("住院号:"), Y, Split(strInfo, "'")(1), 16711680)
            
        '如果超出来页宽，则将科室和床号打印到第二行上
        Y = lngTmpY + H_9pt + H_9pt \ 2
        X = lngTmpX
        
        Call DrawText(objDraw, X, Y, "科室:", 0)
        Call DrawText(objDraw, X + objDraw.TextWidth("科室:"), Y, Split(strInfo, "'")(2), 16711680)
        X = X + objDraw.TextWidth("科室:" & Split(strInfo, "'")(2)) + W_9pt / 3

        Call DrawText(objDraw, X, Y, "床号:", 0)
        Call DrawText(objDraw, X + objDraw.TextWidth("床号:"), Y, Split(strInfo, "'")(4), 16711680)
        X = X + objDraw.TextWidth("床号:" & Split(strInfo, "'")(4)) + W_9pt / 3
        
        If strTmp <> "" Then
            Dim lngUsefull As Long                          '可用的宽度
            Dim int汉字数 As Integer
            
            '把第二行诊断打完,如果已超出则换新行
            If lngPaperWidth < X Then
                Y = Y + H_9pt + H_9pt \ 2
                X = lngTmpX
            End If
            
            Call DrawText(objDraw, X, Y, "诊断:", 0)
            X = X + objDraw.TextWidth("诊断:")
            '计算还剩多少宽度，可以打多少字
            lngUsefull = lngPaperWidth - X
            int汉字数 = lngUsefull \ W_9pt                '计算可以打多少个汉字（诊断嘛，应该都是全角的啥，有点误差也无所谓）
            If int汉字数 < 0 Then int汉字数 = 0
            
            If int汉字数 > 0 Then
                Call DrawText(objDraw, X, Y, Mid(Split(strInfo, "'")(7), 1, int汉字数), 16711680)
            End If
            '如果超长,说明诊断比较多,计算剩下的内容并打在第三行上
            If lngPaperWidth - lngTmpX - objDraw.TextWidth("诊断:" & Split(strInfo, "'")(7)) < X Then
                Y = Y + H_9pt + H_9pt \ 2
                X = lngTmpX
                Call DrawText(objDraw, X, Y, Mid(Split(strInfo, "'")(7), int汉字数 + 1), 16711680)
            End If
        End If
        lngTmpY = Y
    ...
    Else
        ...
        If strTmp <> "" Then
            X = lngPaperWidth - objDraw.TextWidth("诊断:" & Split(strInfo, "'")(7))
            Call DrawText(objDraw, X, Y, "诊断:", 0)
            Call DrawText(objDraw, X + objDraw.TextWidth("诊断:"), Y, Split(strInfo, "'")(7), 16711680)
        End If
...
Private Sub DrawBodyRecordItem(objDraw As Object, X As Long, Y As Long, ByVal colFirstWidth As Long, strValue() As String, ByVal rsArrRecordItemInfo As ADODB.Recordset, Optional sngScale As Single = 1, Optional ByVal intGridRows As Integer = 0)
    '******************************************************************************************************************
    '功能:画底部的录入项目
    '参数:intRows=要画的行数
    '     colFirstWidth=首列的宽度
            ...
            If intGridRows = 0 Or intGridRows > intFactRow + IIf(bln呼吸, -1, 0) Then
                ...
                If Split(strValue(intRow), ";")(0) = "呼吸" Then
                        ...
                        Else
                            ...
                            Else
                                ...
                                If UBound(Split(strTmp, "-")) > 0 Then
                                    strTmp1 = IIf(Split(strTmp, "-")(1) = "呼吸机", "BREATH", "")
                            ...
                            If strTmp1 = "" Then
                                ...
                                End If
                            Else
                                Set objMap = VB.LoadResPicture(strTmp1, vbResBitmap)
                                Call DrawCell(objDraw, objMap, lngTmpX + colFirstWidth + HOUR_STEP_Twips * (intCOl - 1), Y + intFactRow * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips, (H_9pt + H_9pt \ 2), , , , COLOR.红色, , objDraw.Font, "0000")
                            End If
                        End If
                    Next
                    ...
                    For intCOl = 0 To 42
                        If (intCOl) Mod 6 = 0 Then
                            Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCOl, Y + intFactRow * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCOl, Y + intFactRow * (H_9pt + H_9pt \ 2) + 2 * (H_9pt + H_9pt \ 2), , , 2)
                        Else
                ...
                Else
                    ...
                    If rsArrRecordItemInfo.RecordCount > 0 Then
                        int频次 = Val(rsArrRecordItemInfo("记录频次").Value)
                    ...
                    If blnExistData Then
                        ...
                        For intCOl = 0 To 14
                            If intCOl = 0 Then
                                ...
                                Call DrawCell(objDraw, strTmp, lngTmpX, Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), colFirstWidth, (H_9pt + H_9pt \ 2), , , , , , objDraw.Font, , 1, 1)
                            ...
                            Else
                                ...
                                If Split(strValue(intRow), ";")(0) Like "*血压*" Then
                                    objDraw.FontSize = IIf(int频次 < 2, 9, 7.8) * sngScale
                                End If
                                ...
                                If intCOl Mod 2 = 1 Then
                                    ...
                                    If (Split(strValue(intRow), ";")(intCOl + 2) <> "" And Split(strValue(intRow), ";")(intCOl + 3) <> "") Or Val(Split(strValue(intRow), ";")(1)) = 2 Then
                                        ...
                                        Call DrawCell(objDraw, IIf(blnGrade, "", strTmp), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips * 3, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1)
                                        If blnGrade Then
                                    ...
                                    Else
                                        ...
                                        lngColor = GridTextColor(Split(strValue(intRow), ";")(0), strTmp)
                                        Call DrawCell(objDraw, IIf(blnGrade, "", strTmp), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 6 * Fix(intCOl / 2), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), HOUR_STEP_Twips * 6, (H_9pt + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1)
                                        If blnGrade Then
                                            objDraw.FontSize = 7.5 * sngScale
                                        ...
                                        Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 6 * Fix(intCOl / 2), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), , , 2)
                            ...
                            If intCOl = 14 Then
                                Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1) + HOUR_STEP_Twips * 3, Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1) + HOUR_STEP_Twips * 3, Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), , , 2)
                            End If
                        Next
                    End If
                End If
        ...
        If intGridRows > 0 And intGridRows > intFactRow + IIf(bln呼吸, -1, 0) Then
            ...
            For intRow = 1 To intCount
                ...
                For intCOl = 1 To 14
                    ...
                    If intCOl = 14 Then
                        Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1) + HOUR_STEP_Twips * 3, Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1) + HOUR_STEP_Twips * 3, Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), , , 2)
                        Call DrawLine(objDraw, lngTmpX, Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * 14, Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), , , 1)
...
Private Function DrawBodyInfo(objDraw As Object, X As Long, Y As Long, ByVal colFirstWidth As Long, _
    ...
    For intDay = 0 To 6
        ...
        Call DrawLine(objDraw, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y + H_9pt + H_9pt \ 2, , , 2)
        Call DrawLine(objDraw, lngLeft + (HOUR_STEP_Twips * 6) * intDay + (HOUR_STEP_Twips * 6), Y, lngLeft + (HOUR_STEP_Twips * 6) * intDay + (HOUR_STEP_Twips * 6), Y + H_9pt + H_9pt \ 2, , , 2)
    ...
    Next
    X = lngTmpX
    Y = lngTmpY + (H_9pt + H_9pt \ 2) * 3  '+ H_9pt \ 2
End Function
...
'                objDraw.FontSize = 9
        ...
        For j = lngHourBegin To 12 - (4 - lngHourBegin) Step 4
            ...
            If j = 12 - lngHourBegin Then
'                objDraw.FontSize = 8
                Call DrawCell(objDraw, CStr(12 + j), X, Y, HOUR_STEP_Twips, (H_9pt * 2 + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
                X = X + HOUR_STEP_Twips
            Else
'                objDraw.FontSize = 9
                Call DrawCell(objDraw, CStr(12 + j), X, Y, HOUR_STEP_Twips, (H_9pt * 2 + H_9pt \ 2), , , , lngColor, , objDraw.Font, , 1, 1, True)
    ...
    For i = 1 To 7
        ...
        Call DrawLine(objDraw, X, Y, X, Y + (H_9pt * 2 + H_9pt \ 2), , 1, 2)
    Next
    
    objDraw.FontSize = 9
    X = lngTmpX
    Y = lngTmpY + H_9pt * 2 + H_9pt \ 2
    'Y = lngTmpY + H_9pt * 4 + H_9pt \ 2
End Sub
'
...
Private Function DrawBodyGraph(objDraw As Object, _
                                ...
                                strArrValueComment() As String, _
                                strArrValueInOut() As String, _
                                strArrValueInOutType() As String, _
                                Optional strIndex As String = "", _
                                Optional sngScale As Long = 1) As Boolean
...
'    H_9pt = objDraw.TextHeight("字")
    ...
    For lngCol = 0 To 41
        ...
        If strArrValueInOut(lngCol) <> "" Then
            ...
            strComment = strArrValueInOut(lngCol)
            aryTmp = Split(strComment, ";")
            strInOutType = strArrValueInOutType(lngCol)
            aryType = Split(strInOutType, ";")
            For i = 0 To UBound(aryTmp)
                If Trim(aryTmp(i)) <> "" Then
                    rsTmp.AddNew
                    rsTmp.Fields("类型").Value = Val(CStr(aryType(i)))                              '打印中未区分入出转手术等的类型(不含未记说明,上标说明)
...
Public Sub DrawRotateText(objDraw As Object, ByVal X As Single, ByVal Y As Single, ByVal Text As String, Optional ByVal ForeColor As Long = 0, Optional ByVal sglScale As Single = 1.5, Optional ByVal blnRotate As Boolean = False)
    '在(X,Y)处输出Text文本
    Dim lngSaveForeColor As Long
    Dim lngLoop As Long
    ...
    With objDraw
        ...
        If Asc(Text) < 0 Or blnRotate = True Then
            '全角
            .CurrentX = X
            .CurrentY = Y
...
'        If (intRow + 1) Mod 5 = 0 Then
    ...
    For intCOl = 0 To 6
        ...
        Call DrawLine(objDraw, X0 + HOUR_STEP_Twips * 6, lngTmpY, X0 + HOUR_STEP_Twips * 6, Y, COLOR.黑色, , 2)
    Next
...
Private Function DrawBodyScale(objDraw As Object, X As Long, Y As Long, intGuageRow As Integer, arrstrItem() As String, Optional sngScale As Long = 1)
    ...
    lngPercW = mlngFirstWidth / (intCountItem + 1)
    ...
    For i = 0 To intCountItem '纵
        ...
        For j = 0 To intGuageRow '横
            ...
            If j = 0 Then
                ...
                'Y = Y + H_9pt * 3 + H_9pt \ 2 + H_9pt + H_9pt \ 2
                Y = Y + H_9pt + H_9pt + H_9pt \ 2
                
            ElseIf j >= intItemTop And lngTmpVal >= CLng(GetSplitStr(aryItem(i), 4)) Then
                ...
                If (lngTmpVal - Fix(lngTmpVal)) = 0 Then
                    ...
                    Case "脉搏", "心率", "呼吸"
                        ...
                        End If
                    Case Else
                        Call DrawText(objDraw, X + (lngPercW - objDraw.TextWidth(CStr(CStr(lngTmpVal)))) / 2, Y + objDraw.TextHeight(CStr(CStr(lngTmpVal))) / 2 - 30, CStr(lngTmpVal), lngColor)
                    End Select
    ...
    '为了将值传给下一个画图的起始点
    X = lngTmpX + lngPercW * (intCountItem + 1) '+ W_9pt \ 2  '
    Y = lngTmpY
End Function
...
'                    lngArryOPSDay(i) = "0"
'                Else
'                    lngArryOPSDay(i) = strTmp & "- 0"
'                End If
            Else
                lngArryOPSDay(i) = IIf(lngArryOPSDay(i) <> "" And lngArryOPSDay(i) <> "-1", lngArryOPSDay(i), "")
                'lngArryOPSDay(i) = IIf(lngArryOPSDay(i) <> "" And lngArryOPSDay(i) <> "-1", strTmp & "/" & lngArryOPSDay(i), strTmp)
            End If
...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    Dim strEnterDate As String, strCurBeginDate As String
    ...
    Dim lngOPSDayCount As Long '所有手术的次数
    Dim lngOpsNum As Long  '记录手术次数，不包含分娩
    ...
    Dim strDate As String, str内容 As String
    ...
    Dim strArrItemDataInOut(0 To 41) As String '记录标记说明信息(入院、入科、上标、手术等)
    Dim strArrItemDataIn0utType(0 To 41) As String '上下标 2；手术3，其他1
...
'            objOut.SetPages
    ...
    strSQL = "Select Decode(c.出生时间,Null," & IIf(strEnterDate = "", "b.入院时间", "to_date('" & strEnterDate & "','yyyy-MM-dd hh24:mi:ss')") & ",c.出生时间) As 开始, B.出院时间 终止," & _
    ...
    If rsTemp.RecordCount > 0 Then
        ...
        strCurBeginDate = Format(rsTemp!实际入院时间, "YYYY-MM-DD HH:MM:SS")
    Else
        CloseRs rsTemp
        GoTo ErrPrint '无数病人变动信息退出
    End If
    
    If CDate(strBeginDate) > CDate(strCurBeginDate) Then bln入科显示入院 = True
    If CDate(strBeginDate) = CDate(strCurBeginDate) And mBodyFlag.入院 = 0 Then bln入科显示入院 = True
    ...
    '------------------------------------------------------------------------------------------------------------------
    '求出所有录入项目
    mstrSQL = " Select RowNum-1 As 序号,A.* From (Select Decode(A.项目序号,4,'血压',A.记录名) as 项目名,C.项目单位 As 单位,A.项目序号,DECODE(nvl(A.记录频次,0),1,1,2) 记录频次,C.项目性质 " & _
                " From 体温记录项目 A,护理记录项目 C " & _
                " Where A.项目序号=C.项目序号 And A.记录法=[1] AND C.护理等级>=[2] And A.项目序号<>5 And Nvl(c.应用方式,0)=1 And Nvl(c.适用病人,0) In (0,[4]) " & _
                " And (c.适用科室=1 Or (c.适用科室=2 And Exists (Select 1 From 护理适用科室 D Where D.项目序号=c.项目序号 And D.科室id=[3]))) " & _
                " Order by DECODE(A.项目序号,3,0,1),C.项目性质, A.排列序号) A"
    ...
    mlngFirstWidth = (UBound(strArrItemInfo) + 1) * W_9pt * 1.75
    If mlngFirstWidth < W_9pt * 6 Then mlngFirstWidth = W_9pt * 6
...
'                    objOut.txtPage.Text = "当前页" & Space(17) & "共 " & objOut.picPage.UBound + 1 & " 页"
        ...
        strDate = DateAdd("D", -Abs(DateDiff("d", CDate(strBeginDate), CDate(strCurBeginDate))), CDate(strTmpDay))
        
        lngValue = 0
        mstrSQL = "Select zl_CalcInDays([1],[2],[3],[4]) As 开始天数 From Dual"
        Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, "体温单", lng病人id, lng主页id, intBaby, (Int(CDate(strDate))))
        If rsTmp.BOF = False Then
...
'                strTmpString1 = strTmpString1 & "'" & Format(Int(CDate(strTmpDay)) + i - Int(CDate(strBeginDate)) + 1)
        ...
        'lngNewTmpY = Y + (intDrawLineRows) * (H_9pt + H_9pt \ 2) + H_9pt * 7 + H_9pt / 2 - 15
        lngNewTmpY = Y + (intDrawLineRows) * (H_9pt + H_9pt \ 2) + H_9pt * 4 + H_9pt / 2 - 15
                ...
                Do While Not rsTmp.EOF
                    ...
                    If zlCommFun.NVL(rsTmp("内容")) <> "" Then
                        ...
                        If bytShow > 0 Then
                            ...
                            If blnShow Then
                                ...
                                Case 1
                                    strTmp = str内容
                                Case 2
                                    strTmp = str内容 & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                                Case 3
                                    strTmp = str内容 & rsTmp("科室").Value
                                Case 4
                                    strTmp = str内容 & rsTmp("科室").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                                End Select
                        ...
                        If intCount >= 0 And intCount <= 41 Then
                            ...
                            strArrItemDataIn0utType(intCount) = IIf(Trim(strArrItemDataIn0utType(intCount)) = "", "", Trim(strArrItemDataIn0utType(intCount)) & ";") & "1"
                        End If
                    ...
                    Do While Not rsTmp.EOF
                        ...
                        If zlCommFun.NVL(rsTmp("内容")) <> "" Then
                            ...
                            If mBodyFlag.出生 > 0 Then
                                ...
                                Case 2
                                    strTmp = rsTmp("内容").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                                Case 3
                                    strTmp = rsTmp("内容").Value & rsTmp("科室").Value
                                Case 4
                                    strTmp = rsTmp("内容").Value & rsTmp("科室").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                                End Select
                                strTmp = strTmp & "'" & Format(rsTmp("时间").Value, "HH:mm:ss")
                            Else
                                strTmp = ""
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, "mdlPrint", _
                                            lng病人id, _
                                            lng主页id, _
                                            CDate(strTmpDay), intBaby)
        lngOpsNum = rsTmp.RecordCount
        rsTmp.Filter = "项目名称='分娩'"
        lngOpsNum = lngOpsNum - rsTmp.RecordCount
                ...
                If IsNull(rsTmp!时间) = False Then
                    ...
                    Case "分娩"
                        If lngCol >= 0 And lngCol <= 41 And mBodyFlag.分娩 > 0 Then
                            If mBodyFlag.分娩 = 2 Then
                                strTmp = rsTmp("项目名称").Value & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                            Else
                            ...
                            strArrItemDataIn0utType(lngCol) = IIf(Trim(strArrItemDataIn0utType(lngCol)) = "", "", Trim(strArrItemDataIn0utType(lngCol)) & ";") & "1"
                        End If
                    ...
                    Case Else
                        If lngCol >= 0 And lngCol <= 41 And mBodyFlag.手术 > 0 Then
                            ...
                            strTmp = strTmp & IIf(lngOpsNum <= 1, "", lngOpsNum)
                            If mBodyFlag.手术 = 2 Then
                                strTmp = strTmp & "于" & ConvertTimeToChinese(Format(rsTmp("时间").Value, "HH:mm"))
                            Else
                                strTmp = strTmp 'rsTmp("项目名称").Value
                            End If
                            strTmp = strTmp & "'" & Format(rsTmp("时间").Value, "HH:mm:ss")
                            
                            strArrItemDataInOut(lngCol) = IIf(Trim(strArrItemDataInOut(lngCol)) = "", "", Trim(strArrItemDataInOut(lngCol)) & ";") & strTmp
                            strArrItemDataIn0utType(lngCol) = IIf(Trim(strArrItemDataIn0utType(lngCol)) = "", "", Trim(strArrItemDataIn0utType(lngCol)) & ";") & "3"
                        End If
                    End Select
                End If
                rsTmp.MoveNext
        ...
        Call DrawBodyGraph(objDraw, X, Y, strArrItemInfo, strArrItemDataInfo, strArrItemDataComment, strArrItemDataInOut, strArrItemDataIn0utType, "")
        
        '8、求出记录项目记录列表数组，重新对 X ，Y 等变量赋值
        X = lngNewTmpX
        Y = lngNewTmpY - H_9pt / 2 - H_9pt * 2 - 30   '上午/下午标签行的高度=H_9pt * 2
        ...
        If intDrawGridRows > 0 Then
            ...
            For i = LBound(strArrRecordItemInfo) To UBound(strArrRecordItemInfo)
                ...
                mstrSQL = "SELECT A.发生时间 As 当时产生的时间,C.记录内容 As 说明,nvl(C.体温部位,'') as 部位,E.保留项目,D.记录名,D.项目序号,DECODE(nvl(D.记录频次,0),1,1,2) 记录频次 " & _
                            "FROM 病人护理记录 A,病人护理内容 C,体温记录项目 D,护理记录项目 E " & _
                            "Where A.ID=C.记录id " & _
                                "AND A.病人id=[1] " & _
                                "AND A.主页id=[2]  And Nvl(a.婴儿,0)=[7] " & _
                ...
                If rsTemp.BOF = False Then
                    ...
                    varNewTime = Split(strNewTmpString, ";")
                    ...
                    Do While Not rsTemp.EOF
                        ...
                        If CStr(Split(strArrRecordItemInfo(i), "'")(0)) = "呼吸" Then
                            ...
                            If intCOl >= LBound(varNewTmpString) And intCOl <= UBound(varNewTmpString) Then
                                varNewTmpString(intCOl) = zlCommFun.NVL(rsTemp!说明) & "-" & zlCommFun.NVL(rsTemp!部位)
                            End If
                        Else
                            ...
                            If intCOl >= 1 And intCOl <= 14 Then
                                varNewTime(intCOl) = Format(rsTemp!当时产生的时间, "YYYY-MM-DD HH:MM:ss")
...
'                                        varNewTmpString(intCol) = rsTemp!说明
                    ...
                    Loop
                    '重新组织数据信息
                    Call ResizeIntputDownTab(CDate(strTmpDay), Val(CStr(Split(strArrRecordItemInfo(i), "'")(2))), varNewTmpString, varNewTime)
...
'        y = y - 90
        ...
        Call DrawBodyRecordItem(objDraw, X, Y, mlngFirstWidth, strArrNewTmp, rsArrRecordItemInfo, sngScale, intRepairRows)
...
Private Sub ResizeIntputDownTab(ByVal dt开始时间 As Date, ByVal int频次 As Integer, varNewTmpString As Variant, varNewTime As Variant)
'--------------------------------------------------
...
'--------------------------------------------------
    ...
    '对频次为1 的项目进行和并
    If int频次 = 1 Then
        For i = 1 To UBound(varNewTmpString)
            intCOl = i
            strTime = CStr(varNewTime(i))
            If strTime = "" Then strTime = Format(Int(dt开始时间) + (intCOl - 1), "YYYY-MM-DD")
            ...
            Select Case lngItemNO
                Case 7, 9 '入液量,出液量 汇总
                    If intCOl Mod 2 = 0 Then
                        varNewTmpString(intCOl - 1) = varNewTmpString(intCOl)
                        varNewTmpString(intCOl) = ""
                    End If
                Case Else '按照接近中点时间去 12 点
                    If intCOl Mod 2 = 0 Then
                        If strValue <> "" And Trim(varNewTmpString(intCOl)) <> "" Then
                            If Abs(DateDiff("s", CDate(strTime), CDate(strCenterTime))) < Abs(DateDiff("s", CDate(strTime1), CDate(strCenterTime))) Then
                                strValue = Trim(varNewTmpString(intCOl))
                            End If
                        ElseIf strValue = "" And Trim(varNewTmpString(intCOl)) = "" Then
                            strValue = ""
                        ElseIf strValue = "" Then
                            strValue = Trim(varNewTmpString(intCOl))
                        End If
                        varNewTmpString(intCOl - 1) = strValue
                        varNewTmpString(intCOl) = ""
                    Else
                        strTime1 = strTime
                        strValue = Trim(varNewTmpString(intCOl))
...
Private Function GetFontSize(ByVal objDraw As Object, ByVal dblHeight As Double, ByVal strText As String, ByRef Y1 As Single, Optional ByVal intType As Integer = 1) As Single
    ...
    For sinFontSize = objDraw.FontSize To 5 Step -1
        ...
        For intCount = 1 To Len(strText)
            ...
            blnOps = (intType = 3) And (Asc(strChar) > 0) And intLen >= intCount
        ...
        Do While Not .EOF
            If Trim(NVL(!结果)) <> "" Then
                ...
                For intCount = 1 To Len(!结果)
                    ...
                    blnOps = (Val(NVL(!类型)) = 3) And (Asc(strChar) > 0) And intLen >= intCount
                    If Asc(strChar) < 0 Then
                        If intCharNumber Mod 2 = 1 Then sinY1 = sinY1 + ROWHEIGHT * 2.5
                    End If
                    If blnOps = True Then strChar = Mid(!结果, intCount, intLen - intCount + 1)
                    Call DrawRotateText(objDraw, sinX1 - objDraw.TextWidth(strChar) / 2, sinY1 + 15, strChar, IIf(!类型 = 2, -2147483635, 255), , blnOps)
                    If Asc(strChar) < 0 Or blnOps = True Then
                        intCharNumber = 0
