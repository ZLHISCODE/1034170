...
Dim strTemp As String, lngCount As Long
    ...
    Case 5      '疾病证明与报告
        ...
        gstrSQL = "Select '疾病' As 分类,编码, 名称, 简码 From 疾病编码目录 i, 疾病报告前提 p Where i.Id = p.疾病id And p.文件id = [1]"
        gstrSQL = gstrSQL & " Union All Select  '诊断' As 分类,编码, 名称, zlSpellCode(名称) As 简码 From 疾病诊断目录 i, 疾病报告前提 p Where i.Id = p.诊断id And p.文件id = [1]"
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, mlngFileId)
        ...
        With vgdRequest
            ...
            .MergeCol(0) = True
            .ColWidth(0) = 510
            .ColWidth(1) = 1000: .ColWidth(3) = 700
        End With
