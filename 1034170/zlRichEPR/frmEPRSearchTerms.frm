...
Dim lngCount As Long
    ...
    If Trim(txtPati(IIf(optPati(0).Value, 0, IIf(optPati(1).Value, 1, IIf(optPati(2).Value, 2, IIf(optPati(3).Value, 3, 4))))).Text) = "" Then
    ...
    If Trim(Me.txtContent.Text) = "" Then
        ...
        If Me.optPati(0).Value Then
            If Val(Me.txtPati(0).Text) <> 0 Then gstrSQL = gstrSQL & " And i.就诊卡号='" & Me.txtPati(0).Text & "'"
        ElseIf Me.optPati(1).Value Then
            If Val(Me.txtPati(1).Text) <> 0 Then gstrSQL = gstrSQL & " And i.门诊号=" & Me.txtPati(1).Text
        ElseIf Me.optPati(2).Value Then
            If Val(Me.txtPati(2).Text) <> 0 Then gstrSQL = gstrSQL & " And A.住院号=" & Me.txtPati(2).Text
        ElseIf Me.optPati(3).Value Then
            If Trim(Me.txtPati(3).Text) <> "" Then gstrSQL = gstrSQL & IIf(blnSpecify, " And i.姓名='" & Trim(Me.txtPati(3).Text) & "'", " And i.姓名 like '%" & Trim(Me.txtPati(3).Text) & "%'")
    ...
    Else
        ...
        If strDepts <> "" Then gstrSQL = gstrSQL & " And l.科室id + 0 In (" & strDepts & ")"
        If Me.optPati(0).Value Then
            If Trim(Me.txtPati(0).Text) <> "" Then gstrSQL = gstrSQL & " And i.就诊卡号='" & Me.txtPati(0).Text & "'"
        ElseIf Me.optPati(1).Value Then
            If Val(Me.txtPati(1).Text) <> 0 Then gstrSQL = gstrSQL & " And i.门诊号=" & Me.txtPati(1).Text
        ElseIf Me.optPati(2).Value Then
            If Val(Me.txtPati(2).Text) <> 0 Then gstrSQL = gstrSQL & " And A.住院号=" & Me.txtPati(2).Text
...
Dim lngCount As Long
    ...
    Next
    If Me.optPati(0).Value Then
        Me.txtPati(0).Enabled = True: Me.txtPati(0).SetFocus
    ElseIf Me.optPati(1).Value Then
        Me.txtPati(1).Enabled = True: Me.txtPati(1).SetFocus
    ElseIf Me.optPati(2).Value Then
        Me.txtPati(2).Enabled = True: Me.txtPati(2).SetFocus
    Else
        Me.txtPati(3).Enabled = True: Me.txtPati(3).SetFocus
        For lngCount = 0 To Me.chkSex.Count - 1
