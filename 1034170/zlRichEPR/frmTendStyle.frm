...
Begin VB.Form frmTendFileStyle 
   ...
   Begin VB.PictureBox picCloumn 
      ...
      Begin TabDlg.SSTab Tabcol 
         ...
         TabCaption(1)   =   "设置医嘱对应列  "
         TabPicture(1)   =   "frmTendStyle.frx":05A6
         Tab(1).ControlEnabled=   -1  'True
         Tab(1).Control(0)=   "vsfColDoctor"
         Tab(1).Control(0).Enabled=   0   'False
         Tab(1).Control(1)=   "picImg(1)"
         ...
         Begin VSFlex8Ctl.VSFlexGrid vsfColDoctor 
            ...
            ColWidthMax     =   0
            ExtendLastCol   =   0   'False
            FormatString    =   $"frmTendStyle.frx":D722
            ScrollTrack     =   0   'False
            ScrollBars      =   3
            ScrollTips      =   0   'False
            MergeCells      =   0
   ...
   Begin VB.PictureBox picFoot 
      ...
      Begin VB.CheckBox chkU 
         ...
         Height          =   350
         Left            =   4920
         Picture         =   "frmTendStyle.frx":154DE
         Style           =   1  'Graphical
         TabIndex        =   107
      ...
      Begin VB.CheckBox chkB 
         ...
         Height          =   350
         Left            =   4590
         Picture         =   "frmTendStyle.frx":1BD30
         Style           =   1  'Graphical
         TabIndex        =   106
      ...
      Begin RichTextLib.RichTextBox rtbHead 
         ...
         OLEDragMode     =   0
         OLEDropMode     =   0
         TextRTF         =   $"frmTendStyle.frx":22582
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "宋体"
            Size            =   10.5
            Charset         =   134
      ...
      Begin RichTextLib.RichTextBox rtbFoot 
         ...
         OLEDragMode     =   0
         OLEDropMode     =   0
         TextRTF         =   $"frmTendStyle.frx":2261F
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "宋体"
            Size            =   10.5
            Charset         =   134
   ...
   Begin MSComctlLib.StatusBar stbThis 
      ...
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Panel1 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            ...
            Object.Width           =   2355
            MinWidth        =   882
            Picture         =   "frmTendStyle.frx":226BC
            Text            =   "中联软件"
            TextSave        =   "中联软件"
            Key             =   "ZLFLAG"
   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vfgThis 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmTendStyle.frx":22F50
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   1
   ...
   Begin XtremeDockingPane.DockingPane dkpMan 
      Bindings        =   "frmTendStyle.frx":23038
      Left            =   75
      Top             =   60
      _Version        =   589884
      _ExtentX        =   450
...
Private Sub cbo页码_Click()
    If Val(cbo页码.Tag) = Val(cbo页码.ItemData(cbo页码.ListIndex)) Then Exit Sub
    cbo页码.Tag = Val(cbo页码.ItemData(cbo页码.ListIndex))
    mblnChanged = True
End Sub

Private Sub cbo页码_KeyPress(KeyAscii As Integer)
    Call zlControl.CboMatchIndex(cbo页码.hWnd, KeyAscii)
End Sub

Private Sub chkHideTime_Click()
    mblnChanged = True
    If chk首列合并.Value > 0 And chkHideTime.Value > 0 Then
        chk首列合并.Value = 0
    End If
End Sub
...
Private Function SaveData() As Boolean
    ...
    If chk重算数据行.Value = 1 Then
        If lngRows <> mlngPageRow And mlngPageRow > 0 Then
            ...
            '87990,陈刘,2015-09-02
        Else
            If MsgBox("    本次修改由于您勾选了重算数据行，默认将会对使用该格式的文件重新计算数据行数，是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Function
            blnReCalc = True
        End If
    Else
        lngRows = mlngPageRow
    ...
    gstrSQL = gstrSQL & "," & Me.udRecordFrom.Value & "," & Me.udRecordTo.Value & ",'" & Me.lblRecordFont.Caption & "'," & Me.lblRecordColor.ForeColor & "," & lngRows & "," & chk首列合并.Value & "," & chkHideTime.Value
...
Private Sub picImg_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    ElseIf Index = 1 Then
        ...
        strInfo = strInfo & vbCrLf & "2.对照项目绑定之后通过入量导入,自动添加所有绑定医嘱的项目,绑定的医嘱项目要与设定的项目类型相同(用量和总给予量需绑定数值类型)"
        strInfo = strInfo & vbCrLf & "3.点击对照项目的名称列可选择项目。"
    End If
    ...
    With vfgColRelation
        If Row >= .FixedRows And Col = .ColIndex("对照名称") And .ColComboList(.ColIndex("对照名称")) <> "" Then
            ...
            If blnCancle = True Then
                ...
                Cancel = True
            Else
                .TextMatrix(Row, .ColIndex("对照列号")) = .ComboData(.ComboIndex)
...
Private Sub vsfColDoctor_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsfColDoctor
        If .ColComboList(.ColIndex("对照名称")) <> "" Then
            If Trim(vsfColDoctor.EditText) = Trim(.TextMatrix(Row, .ColIndex("对照名称"))) Then Exit Sub
        End If
        ...
        For lngRow = .FixedRows To .Rows - 1
            If lngRow <> Row Then
                If .TextMatrix(lngRow, .ColIndex("对照名称")) = vsfColDoctor.EditText Then
                    blnCancle = True
                    Exit For
                End If
            End If
        ...
        Select Case vsfColDoctor.EditText
            Case "用量", "总给予量"
                ...
                If Not rsItems.EOF Then
                    If rsItems!项目类型 <> 0 Then
                        Cancel = True
                        MsgBox "绑定医嘱列项目时，项目的类型必须一致！此项目是文本项目！", vbInformation, gstrSysName
                        Cancel = True
                    Else
                        DataChanged = True
                    End If
            ...
            Case Else
                rsItems.Filter = "项目名称='" & strName & "'"
                If Not rsItems.EOF Then
                    If rsItems!项目类型 <> 1 Then
                        Cancel = True
                        MsgBox "绑定医嘱列项目时，项目的类型必须一致！此项目是数值项目！", vbInformation, gstrSysName
                        Cancel = True
                    Else
                        DataChanged = True
                    End If
