...
Private Sub Cmd确定_Click()
    ...
    If mstrSQL = "" Then
        ...
        Set mrsSystems = zlDatabase.OpenSQLRecord(mstrSQL, "读取可用报表", gobjRelogin.DBUser)
        mstrSQL = ""
...
Private Sub Form_Load()
    ...
    Set mrsSystems = zlDatabase.OpenSQLRecord(mstrSQL, "读取可用系统", gobjRelogin.DBUser)
    gobjRelogin.IsSysOwner = mrsSystems.RecordCount > 0
    mstrSQL = "Select 1 From Zlsystems Where Mod(编号, 100) <> 0"
    Set mrsSystems = zlDatabase.OpenSQLRecord(mstrSQL, "是否存在非标准帐套", gobjRelogin.DBUser)
    blnMutilAccout = mrsSystems.RecordCount > 0
    
    If gobjRelogin.IsSysOwner Then
        '所有者只允许进入自己的系统(因为有对象重名的情况)
        '现在取消上面的逻辑，合并两个分支，主要是由于一个系统的所有者是另一独立系统的授权用户。
        mstrSQL = " And ((p.系统,p.序号,p.功能) In (Select Distinct 系统,序号,功能 From zlRoleGrant G, zlUserRoles S Where g.角色 = s.角色 and s.用户 = [1]) OR p.系统 In (Select 编号 From zlSystems Where 所有者 = [1]))"
    Else
        '普通用户只允许进入所属角色的权限所允许的系统
        mstrSQL = " And (p.系统,p.序号,p.功能) In (Select Distinct 系统,序号,功能 From zlRoleGrant G, zlUserRoles S Where g.角色 = s.角色 and s.用户 = [1])"
    End If
    mstrSQL = "Select Substr(LPad(编号, 5, '0'), 4) 编号, 编号 系统, 名称" & vbNewLine & _
            ...
            "            (Select Distinct p.系统, p.序号" & vbNewLine & _
            "              From zlProgFuncs P, zlRegFunc R" & vbNewLine & _
            "              Where Trunc(p.系统 / 100)  = r.系统 And p.序号 = r.序号 And p.功能 = r.功能" & mstrSQL & ") f" & vbNewLine & _
            "       WHERE p.系统 = f.系统 AND p.序号 = f.序号 AND " & vbNewLine & _
            "            Upper(p.部件) IN (" & mstrComponent & ")) " & vbNewLine & _
            " ORDER BY 名称, 编号"
