...
Private mblnPrintSend As Boolean     '没发送之前是否打印了
...
On Error GoTo errH
    ...
    vPoint = GetCoordPos(txtInfo(2).hwnd, txtInfo(2).Left, txtInfo(2).Top)

    Set rsTemp = gobjComlib.zlDatabase.ShowSQLMultiSelect(Me, strSQL, 0, "诊断", False, "", "疑似疾病的诊断选择", False, False, False, vPoint.X, vPoint.Y, 1000, blnCancle, False, True, mstr疑似传染病)
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    Select Case Control.ID
            ...
            If mintType = 0 Or mintType = 3 Then
                If CheckData Then
                    If SendDiseaseRecord Then
						If mintType = 0 Then mblnPrintSend = True
        ...
        Case conMenu_Tool_Send
            If CheckData Then
                If SendDiseaseRecord Then
                    If mintType = 0  Or mblnPrintSend Then
                        Call SendMsg
                    End If
                    mintResult = 1
                    Unload Me
...
'参数：0 文本框按回车，1 点按钮
    ...
    vRect = GetControlRect(txtInfo(txt疑似疾病).hwnd)
...
'功能：提取病人基本信息
    ...
    If mintType <> 0 Then
        strSQL = "Select a.Id, a.病人id, a.主页id, a.挂号单, a.医嘱ID,a.送检时间, a.送检科室id, a.送检医生, a.标本名称, a.反馈结果, a.传染病名称 As 疑似疾病, " & vbNewLine & _
                " a.检查时间, a.登记时间, a.登记人, a.登记科室id,a.记录状态, a.处理人, a.处理时间, a.处理情况说明" & vbNewLine & _
                "From 疾病阳性记录 A  Where a.Id = [1] "
...
'功能：获取标本名称
    ...
    vRect = GetControlRect(txtInfo(txt标本名称).hwnd)
...
'功能：获取送检科室
    ...
    vRect = GetControlRect(txtInfo(txt送检科室).hwnd)
...
'功能：获取送检医生,人员性质为 医生 ，部门性质为 临床，服务于住院或者门诊 下面的人员
    ...
    vRect = GetControlRect(txtInfo(txt送检医生).hwnd)
...
Private Sub SetFormState(ByVal intType As Integer)
    ...
    If intType = 0 Or intType = 3 Then
        ...
        txtInfo(txt登记人).Locked = True
        txtInfo(txt登记科室).Locked = True
        txtInfo(txt登记时间).Locked = True
...
'参数：intType   1-最开始进来时改变状态，时间，医生；2-确定；3-非传染病；4-转科；5-关闭；6-打印前保存
    ...
    If (mIntState = 1 Or mIntState = 4) And (intType = 1 Or intType = 2) Then
...
Private Sub mclsDiagEdit_Closed(ByVal blnEditCancel As Boolean, ByVal str疾病ID As String, ByVal str诊断ID As String, ByVal strTag As String)
    ...
    On Error GoTo errH
     If Not blnEditCancel Then
        ...
        mblnDiagnose = True
        If Not InStr(";" & GetPrivFunc(glngSys, IIf(mint场合 = 0, 1251, 1250)) & ";", ";病历书写;") > 0 Then Exit Sub
        ...
        strSQL = "Select /*+ Rule*/" & vbNewLine & _
                ...
                "Order By a.编号"
        Set rsTemp = gobjComlib.zlDatabase.OpenSQLRecord(strSQL, "CheckDiseaseFile", mlng病人ID, mlng主页ID, str疾病ID, str诊断ID, mlng科室ID)
        ...
        '没有查找到一年之内的重复的报告卡
        If Not frmDisStation.ShowDiseaseStation(Me, strName, mlng病人ID, IIf(mint场合 = 0, mlng主页ID, mlng挂号ID), IIf(mint场合 = 0, 2, 1), _
                                    mlng科室ID, str疾病ID, str诊断ID) Then
            Call clsDisease.EditDiseaseDoc(Me, mlng病人ID, IIf(mint场合 = 0, mlng主页ID, mlng挂号ID), IIf(mint场合 = 0, 2, 1), mlng科室ID, str疾病ID, str诊断ID, strReason)
...
'功能：发送 传染病阳性结果 消息
    ...
    strTmp = ""
    strTmp = "" & RowValue("部门表", IIf(mlng登记科室ID = 0, "NULL", mlng登记科室ID), "名称")
    If strTmp <> "" Then
