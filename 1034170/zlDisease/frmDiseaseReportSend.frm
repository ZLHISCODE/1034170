...
'strInfo=报告|科室|姓名|性别|年龄|就诊号|填报人|填报时间|病人ID|主页ID|文件ID
    ...
    Me.dtpTime.MaxDate = Format(gobjComlib.zlDatabase.Currentdate, "yyyy-MM-dd HH:mm")
    Me.dtpTime.Value = Me.dtpTime.MaxDate
    Me.txtPerson.Text = UserInfo.姓名
    strSQL = "Select 名称 From 疾病报送单位 Order By 编码"
    ...
    If IsNumeric(strFileID) Then
        strSQL = "Select l.报送人, l.报送时间, l.诊断描述1, l.诊断描述2  " & vbNewLine & _
        ...
        strSQL = strSQL & strDiag1 & strDiag2
        Set rsTemp = gobjComlib.zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
        With rsTemp
...
Private Sub cmdOK_Click()
    ...
    strSQL = "Zl_疾病申报记录_Send('" & mstrId & "'," & _
...
Private Sub dtpTime_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then Call gobjComlib.zlCommFun.PressKey(vbKeyTab)
End Sub
