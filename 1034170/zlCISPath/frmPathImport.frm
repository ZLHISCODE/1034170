...
Public Function ShowMe(frmParent As Object, t_pati As TYPE_Pati, ByVal lngFun As Long, _
    ...
    Optional ByVal blnChoose As Boolean) As Boolean
'参数：
            ...
            If mblnPathSend = False Or mlngFun = 1 Then
                ...
                If mblnPathSend = False And bln中医 Then
                    ...
                    If mrsPath Is Nothing Then
                        MsgBox "当前科室没有适合于该病人主要诊断[" & str诊断描述 & "]的临床路径。", vbInformation, gstrSysName
                        Exit Function
                    End If
                Else
                     Set mrsPath = GetPathTable(mlng疾病ID, mlng诊断ID, mPati.科室ID, lngCurPathID)
        ...
        If mlngFun = 0 Or mlngFun = 1 Then
            If Not CheckPathSend(mPati.病人ID, mPati.主页ID) Then
                If lng住院天数 > lng标准住院日 Then
                    ...
                    str未导入名称 = "超过标准住院日"
                    GoTo UnImport
                ElseIf lng住院天数 > mrsPath!确诊天数 And mrsPath!确诊天数 <> 0 Then
