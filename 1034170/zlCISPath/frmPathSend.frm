    ...
    Else
        ...
        If mlngFun = 0 Then
            If mint场合 = 1 Then
                ...
                Else
                    '加载护士生成可选的合并路径阶段
                    Set GetPhase = GetPhaseMerge(mPP.病人路径ID, lng当前阶段ID, lng天数, lng路径ID, lng版本号)
                End If
                Exit Function
            ...
            End If
        Else
            If mlngFun = 1 And mint场合 = 1 And lng合并路径记录ID <> 0 Then
                Set GetPhase = GetPhaseMerge(mPP.病人路径ID, lng当前阶段ID, lng天数, lng路径ID, lng版本号)
                Exit Function
            End If
            strIF = " And a.id = [4]"
...
Private Sub Form_Load()
    ...
    End If
    If mlngFun <> 0 Or mint场合 = 1 Then cmdMergeStep.Visible = False
...
Private Sub vsPhase_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    If OldCol <> NewCol And Me.Visible = True And NewCol >= 0 And NewRow >= 0 And vsPhase.Redraw = flexRDDirect Then
        ...
        If mint场合 = 1 And mlngFun = 0 Then
            ...
            lblDate.Caption = "生成路径项目日期：" & Format(mdat时间, "yyyy-MM-dd") & ",第" & mlng天数 & "天"
            mlng提前天数 = mlng天数
            If mint场合 = 1 Then Call LoadMerge
        Else
...
'功能：加载合并路径项目
    ...
    If rsTmp.RecordCount > 0 Then
        Do While Not rsTmp.EOF
            ...
            Else
                Set rsMerge = GetPhase(Val(rsTmp!路径ID & ""), Val(rsTmp!版本号 & ""), Val(vsPhase.ColData(vsPhase.Col)), 0, mlng天数, Val(rsTmp!ID & ""))
            End If            
	    If rsMerge.RecordCount > 0 Then
                ...
                Else
                    rsMerge.MoveFirst
                    Call LoadItem(Val(rsMerge!ID & ""), vsItem(1), Val(rsTmp!路径ID & ""), Val(rsTmp!版本号 & ""), mlng天数, Val(rsTmp!ID & ""))
                End If
            End If
            rsTmp.MoveNext
        Loop
...
'参数：strAdviceOfItem=路径项目与医嘱ID的对应,例：38:1983,69:1978
    ...
    If mint场合 = 0 Then strMergeStep = Mid(mstrMergeStep, 2)
    lng天数 = mlng天数
...
Private Function GetPhaseMerge(ByVal lng病人路径ID As Long, ByVal lng当前阶段ID As Long, ByVal lng天数 As Long, ByVal lng路径ID As Long, ByVal lng版本号 As Long) As ADODB.Recordset
'功能:加载护士生成可选的合并路径阶段
    Dim strSQL As String
    
    strSQL = "Select *" & vbNewLine & _
            "From (Select Distinct a.Id, Nvl(a.父id, 0) As 父id, a.序号, a.名称, a.说明, a.开始天数, a.结束天数, a.分类, Nvl(a.分支id, 0) As 分支id" & vbNewLine & _
            "       From 临床路径阶段 A, 临床路径项目 B, 病人路径执行 C" & vbNewLine & _
            "       Where c.项目id = b.Id And b.阶段id = a.Id And c.路径记录id = [1] And c.阶段id = [2] And c.天数 = [3] And b.路径id = [4] And" & vbNewLine & _
            "             b.版本号 = [5])" & vbNewLine & _
            "Order By 序号"
    On Error GoTo errH
    Set GetPhaseMerge = zlDatabase.OpenSQLRecord(strSQL, "获取适用阶段", lng病人路径ID, lng当前阶段ID, lng天数, lng路径ID, lng版本号)
    Exit Function
errH:
    If ErrCenter() = 1 Then
        Resume
