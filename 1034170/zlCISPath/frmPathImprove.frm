...
'根据输入   字符检索下拉列表
    ...
    Call zlControl.CboSetIndex(cboCategory.Hwnd, zlControl.CboMatchIndex(cboCategory.Hwnd, KeyAscii))
...
Private Sub Form_Load()
    ...
    '加载分类
    Call zlControl.CboSetHeight(cboCategory, 2000)
...
'功能:加载路径分类
    ...
    Next
    '缺省分类
    Call zlControl.CboLocate(cboCategory, mstr分类, False)  '会触发cboCategory_Click事件
    Exit Sub
errH:
...
'功能:加载分支路径信息
    ...
    For i = 1 To rsTmp.RecordCount
        ...
        rsTmp.MoveNext
    Next
    Call zlControl.CboSetIndex(cboBranch.Hwnd, 0)
    Exit Sub
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
