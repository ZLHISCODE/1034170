...
Private Sub picLeft_Resize()
    On Error Resume Next
    lblFind.Move 120, lbl单量.Top
    txtFind.Move lblFind.Left + lblFind.Width, txt单量.Top
...
'---------------------------------------
    ...
    strSql = "Select a.诊疗项目id, a.收费细目id, f.类别, Nvl(g.编码, f.编码) As 编码," & vbNewLine & _
            "       Nvl(g.名称, f.名称) || Decode(Nvl(g.规格, '0'), '0', '', ' ' || g.规格) As 名称, f.操作类型, f.执行分类,f.计算方式, f.计算单位, g.产地, k.名称 As 商品名," & vbNewLine & _
            "       h.药品剂型, -null As 标本部位, -null As 检查方法, -null As ID, -null As 相关id" & vbNewLine & _
            "From 诊疗项目目录 F, 收费项目目录 G, 药品特性 H, 收费项目别名 K," & vbNewLine & _
            "     (Select Distinct a.诊疗项目id, a.收费细目id" & vbNewLine & _
            "       From 路径医嘱内容 A, 临床路径医嘱 B, 临床路径项目 C, 临床路径版本 D" & vbNewLine & _
            "       Where a.Id = b.医嘱内容id And b.路径项目id = c.Id And c.路径id = d.路径id And c.版本号 = d.版本号 And d.停用人 Is Null And" & vbNewLine & _
            "             (a.诊疗项目id In (Select ID" & vbNewLine & _
            "                           From 诊疗项目目录" & vbNewLine & _
            "                           Where Nvl(撤档时间, Sysdate) <> To_Date('3000-01-01', 'yyyy-mm-dd') And 撤档时间 Is Not Null And" & vbNewLine & _
            "                                 类别 In ('5', '6', '7')) Or" & vbNewLine & _
            "             a.收费细目id In (Select ID" & vbNewLine & _
            "                           From 收费项目目录" & vbNewLine & _
            "                           Where Nvl(撤档时间, Sysdate) <> To_Date('3000-01-01', 'yyyy-mm-dd') And 撤档时间 Is Not Null And" & vbNewLine & _
            "                                 类别 In ('5', '6', '7')))) A" & vbNewLine & _
...
'
    ...
    End With
    If lng收费细目ID = 0 Then
        strSql = "Select a.类别 As 类别id, a.计算方式, a.执行频率, a.计算单位 From 诊疗项目目录 A Where a.Id = [1]"
    Else
...
'      strPathTag 形如: 路径Id_最新版本1,路径Id_最新版本2:...
    ...
    With rptList.SelectedRows(0)
        ...
        If InStr(",5,6,", str类别) > 0 Then
            strSql = "Select a.C1 as 路径ID,a.C2 as 版本号,c.路径项目ID,e.期效,e.Id As 内容id, E.相关id, E.诊疗项目id, E.收费细目id " & vbNewLine & _
            ...
            If chkPra(chk_频率).Value = vbChecked Then
                ...
                End If
            End If
            strSql = strSql & "　Order By b.Id, e.序号"
        ...
        strItemAdvices = Mid(strItemAdvices, 2)
        
    End With
    arrSQL = Array()
    With mrsAdvice
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = "Zl_路径医嘱变动_Insert('" & strItemIDs & "'," & "To_Date('" & Format(zlDatabase.Currentdate, "yyyy-MM-dd HH:mm:ss") & "','YYYY-MM-DD HH24:MI:SS')" & ",'" & UserInfo.姓名 & "')"
        ...
        If InStr(",5,6,", str类别) > 0 Then
            For i = 1 To rsTmp.RecordCount
                ...
                Else
                    .Filter = "相关Id<>NULL"
                End If
                If (InStr(",5,6,", str类别) > 0 And lng诊疗项目ID = Val(rsTmp!诊疗项目ID & "") And Val(rsTmp!收费细目ID & "") = lng收费细目ID) Then '待替换项目为按规格下达
        ...
        ElseIf InStr(",D,C,", str类别) > 0 Then
            ...
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
            arrSQL(UBound(arrSQL)) = "Zl_路径医嘱内容_Update(2,Null,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,Null,Null,Null,null,null,'" & strTmp & "','" & strItemAdvices & "','" & str类别 & "')"
        Else
            '其他单条替换
