...
'      str期间:缺省的当前期间
    ...
    If cboDate.ListCount > 0 Then
        If str期间 = "" Then
            cboDate.ListIndex = 0
        Else
            zlControl.CboLocate cboDate, Mid(str期间, 1, 4) & "年" & Mid(str期间, 5, 2) & "月"
        End If
    Else
        ...
        For i = lngS To lngE
            ...
            If strSql <> "" Then
                ...
                Case 26 '单病种日均费用(如果定义了，缺省未定义)
                    If lng总住院天数 = 0 Then lng总住院天数 = Get总数(strPatiPage, lng总人次, 2)
                ...
                Case 27 '抗菌药物费用比
                    If blnGeted总费用 = False Then lng总费用 = Get总数(strPatiPage, lng总人次, 25): blnGeted总费用 = True
...
Private Function Get总数(ByVal strPatiPage As String, ByVal lng总人次 As Long, ByVal lngRow As Long) As Long
