...
'      strSelectedIDs:  bytUseType=3时传入，界面上已经选择的医嘱内容IDs。
    ...
    If bytUseType <> 0 And bytUseType <> 4 And bytUseType <> 3 Then
        ...
        Else
            strSQL = "Select /*+cardinality(B,10)*/ a.*" & vbNewLine & _
                    ...
                    "Union All" & vbNewLine & _
                    "Select /*+cardinality(B,10)*/ Distinct c.*" & vbNewLine & _
    ...
    strSql = "Select A.ID,A.相关ID,A.序号," & IIf(InStr(",0,3,4,", "," & bytUseType & ",") > 0, "A.期效", "A.医嘱期效") & " as 期效,A.诊疗项目ID,A.医嘱内容," & _
