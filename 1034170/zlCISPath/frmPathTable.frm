            ...
            Do While Not rsTmp.EOF
                ...
                ElseIf rsTmp!执行方式 = 3 Then                          '可选项，深蓝色
                    ...
                    If Not IsNull(rsTmp!变异原因) Then '93648 中药配方 变异原因
                        .Cell(flexcpBackColor, lngRow, lngCol) = CON_PathOutItemColor    '浅黄色
                        .TextMatrix(lngRow, lngCol) = .TextMatrix(lngRow, lngCol) & vbCrLf & "变异原因：" & rsTmp!变异原因
                    End If
                End If
...
'参数：blnPrompt=是否弹出询问提示
    ...
    If Not mbln启用不评估 Then
        ...
        End If
    Else
        strSql = "Select A.阶段ID,A.日期,A.天数,A.原路径id From (Select t.阶段id, t.日期, t.天数,t.原路径id From 病人路径评估 T Where t.路径记录id = [1] Order By t.登记时间 Desc, t.天数 Desc) A where rownum<2"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mPP.病人路径ID)
        If rsTmp.RecordCount > 0 Then
            If Val(rsTmp!原路径ID & "") <> 0 Then
                '当前评估后存在跳转路径项目,不允许取消。
                If Val(rsTmp!原路径ID & "") <> mPP.路径ID Then
                    strSql = "Select Count(1) As 行" & vbNewLine & _
                            "From 病人路径执行 A" & vbNewLine & _
                            "Where 路径记录id = [1] And Exists (Select 1 From 临床路径阶段 B Where b.Id = a.阶段id And b.路径id = [2] And b.版本号 = [3])"
                    Set rsTurn = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mPP.病人路径ID, mPP.路径ID, mPP.版本号)
                    If Not rsTurn.EOF Then
                        If rsTurn!行 > 0 Then
                            MsgBox "当前阶段已经跳转路径且生成了跳转后的路径项目,必须取消跳转后的路径项目后,才允许取消评估！", vbOKOnly + vbInformation, gstrSysName
                            Exit Function
                        End If
                    End If
                End If
