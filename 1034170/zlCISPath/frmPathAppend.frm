...
Public Function ShowMe(frmParent As Object, int场合 As Integer, t_pati As TYPE_Pati, t_pp As TYPE_PATH_Pati, _
    ByVal strItemType As String, ByVal bytUseType As Byte, ByVal str医嘱IDs As String, ByVal lng执行ID As Long, _
    Optional ByRef objMip As Object, Optional ByVal datDate As Date = CDate(0)) As Boolean
...
Private Sub Form_Load()
    ...
    Call zlControl.CboSetHeight(cboItems, 500)
    Call zlControl.CboSetWidth(cboItems.Hwnd, 3500)
    Call SetFormFace
End Sub
...
'功能：读取路径外项目的相关信息
    ...
    For i = 1 To rsTmp.RecordCount
        If i = 1 Then
            Call zlControl.CboLocate(cboItemType, rsTmp!分类)
...
'功能:加载路径项目
    ...
    Call zlControl.CboAddData(cboItems, rsData)
    ...
    If mlngFun = 0 Or mlngFun = 1 Then  '0-直接添加,1-医嘱新开时添加,2-修改路径外项目
        ...
        End If
        If cboItems.ListCount > 0 Then Call zlControl.CboLocate(cboItems, "路径外项目", False)
    ElseIf mlngFun = 2 Then
        If Not zlControl.CboLocate(cboItems, mlng执行ID, True) Then
