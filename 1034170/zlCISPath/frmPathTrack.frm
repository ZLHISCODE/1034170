...
'功能：加载路径的分支路径
    ...
    If mlng路径ID <> lngPathID Then
        ...
        If lngPathID <> 0 Then
            ...
            Call zlControl.CboSetIndex(cbo路径范围.Hwnd, cbo路径范围.NewIndex)
            cbo路径范围.AddItem "主路径"
            On Error GoTo errH
    ...
    With chtThis
        ...
        If Not mblnIsEdition And (mlngOldPathID <> lngPathID Or mdateOldStart <> CDate(Format(dtpStart.Value, "yyyy-MM-dd 00:00:00")) Or _
            ...
            Do Until rsTmp.EOF
                ...
                rsTmp.MoveNext
            Loop
            If cboPathEdition.ListCount > 0 Then Call zlControl.CboSetIndex(cboPathEdition.Hwnd, 0)
...
'参数：lng合并路径ID<>0则不清除合并路径列表
    ...
    If lng合并路径ID = 0 Then
        ...
        Call zlControl.CboSetIndex(cbo性质(0).Hwnd, 0)
        Call zlControl.CboSetIndex(cbo性质(1).Hwnd, 0)
...
'参数:bytMode=1 打印;2 预览;3 输出到EXCEL
    ...
    If blnIsRPT Then
        ...
        If Not zlReportToVSFlexGrid(vsTemp, objReport) Then Exit Sub
    Else
        Set objVSF = objTable
        If Not zlCopyVSFlexGrid(vsTemp, objVSF) Then Exit Sub
    End If
...
Private Sub FindPerson()
    ...
    vRect = GetControlRect(txtPerson.Hwnd)
