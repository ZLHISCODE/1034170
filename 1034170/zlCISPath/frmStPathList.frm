...
Public Function ShowMe(frmParent As Object, ByVal str疾病编码 As String) As Boolean
'功能：根据疾病编码获取相关标准路径
'参数：frmParent  父窗体
...
'       2、参看所有标准路径 str疾病编码=""
    ...
    If Trim(str疾病编码) = "" Then
        Call frmStandardPathRef.ShowMe(mfrmParent, 0)
        Exit Function
    Else
    ...
    If mrsStPath.RecordCount = 0 Then '没有符合首要诊断的标准路径,就截取疾病编码大类，不要子分类进行匹配查找
        ...
        ElseIf mrsStPath.RecordCount = 1 Then '仅有一条标准路径符合首要诊断
            Call frmStandardPathRef.ShowMe(mfrmParent, mrsStPath!ID)
            Exit Function
        Else '仅有多条标准路径符合首要诊断
            Me.Show 1, frmParent
            Call frmStandardPathRef.ShowMe(mfrmParent, mlStPathID)
            ShowMe = mblnOK
            Exit Function
        End If
    ElseIf mrsStPath.RecordCount = 1 Then '仅有一条标准路径符合首要诊断
