...
'剔除自由录入诊断
    ...
    ElseIf lngType = 2 Then
        strSql = "Select 疾病id, 诊断id, 诊断描述,诊断类型,记录来源" & vbNewLine & _
            "From 病人诊断记录" & vbNewLine & _
            "Where 记录来源 In (1, 2, 3) And 诊断类型 In ( 2,3,12,13) And 取消时间 Is Null And 病人id = [1] And 主页id = [2] And Nvl(是否疑诊,0) = 0 And NVL(编码序号,1) = 1 And Not (NVl(疾病ID,0)=0 and NVl(诊断ID,0)=0) " & vbNewLine & _
