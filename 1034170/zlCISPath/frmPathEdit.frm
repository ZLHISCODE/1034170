...
Private Sub cmdOK_Click()
    ...
    If mlng路径ID = 0 Then
        strSql = "Zl_临床路径目录_Insert('" & cbo分类.Text & "','" & txt编码.Text & "','" & txt名称.Text & "'," & _
                 "'" & txt说明.Text & "','" & NeedName(cbo病例分型.Text) & "','" & IIf(cbo适用病情.ListIndex = 0, "", NeedName(cbo适用病情.Text)) & "'," & _
...
Private Sub Form_Load()
    ...
    If mlng路径ID = 0 Then
        ...
        cbo分类.ListIndex = SeekCboIndex(cbo分类, mstr分类)    '隐含调用Call AddDept
    Else
        vsDept.Enabled = opt应用范围(1).Value
        chk连续.Visible = False
        ...
        If Not IsNull(rsTmp!病例分型) Then
            cbo病例分型.ListIndex = SeekCboIndex(cbo病例分型, CStr(rsTmp!病例分型))
        End If

        If Not IsNull(rsTmp!适用病情) Then
            cbo适用病情.ListIndex = SeekCboIndex(cbo适用病情, CStr(rsTmp!适用病情))
        End If
        ...
        If Not IsNull(rsTmp!适用年龄) Then
            ...
            cbo年龄单位.ListIndex = SeekCboIndex(cbo年龄单位, CStr(Right(Split(rsTmp!适用年龄, "-")(1), 1)))
        End If
