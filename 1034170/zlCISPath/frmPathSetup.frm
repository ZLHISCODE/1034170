...
Begin VB.Form frmPathSetup 
   ...
   Begin VB.PictureBox picPath 
      ...
      Begin VB.CheckBox chkOverPath 
         ...
         Width           =   3015
      End
      Begin VB.CheckBox chkMatchPath 
...
Private Sub SaveData()
    ...
    If mbytFun = 0 Then
        ...
        Call zlDatabase.SetPara("匹配时期效不同算路径外项目", chkMatchPath.Value, glngSys, p临床路径应用, blnSetup)
        Call zlDatabase.SetPara("出院后不允许取消完成路径", chkOverPath.Value, glngSys, p临床路径应用, blnSetup)
        Call zlDatabase.SetPara("双审核模式", chk双审核.Value, glngSys, p临床路径管理, blnSetup)
...
Private Sub Form_Load()
    ...
    If mbytFun = 0 Then
        ...
        chkMatchPath.Value = Val(zlDatabase.GetPara("匹配时期效不同算路径外项目", glngSys, p临床路径应用, "0", Array(chkMatchPath), blnParSet))
        chkOverPath.Value = Val(zlDatabase.GetPara("出院后不允许取消完成路径", glngSys, p临床路径应用, "0", Array(chkOverPath), blnParSet))
        ...
        chk双审核.Value = Val(zlDatabase.GetPara("双审核模式", glngSys, p临床路径管理, "0", Array(chk双审核), blnParSet))
        If chkIsEvaluate.Value = Unchecked Then
            chkIsForwardSend.Value = Unchecked
            chkIsForwardSend.Enabled = False
        End If
