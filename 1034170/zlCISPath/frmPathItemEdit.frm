...
Begin VB.Form frmPathItemEdit 
   ...
   Begin VB.PictureBox picInfo 
      ...
      Begin VB.Label lblNote 
         BackStyle       =   0  'Transparent
         Caption         =   "    设置临床路径表中一个阶段中的项目信息，包括对应的医嘱、病历等，并可以指定项目的可选执行结果和执行人。"
         Height          =   360
         Left            =   1095
         TabIndex        =   7
         Top             =   480
...
Private mblnInherit As Boolean '出：是否设置为继承
...
Public Function ShowEdit(frmParent As Object, rsAdvice As ADODB.Recordset, vItem As TYPE_PATH_ITEM, vPreItem As TYPE_PATH_ITEM, _
    ByVal blnAdjust As Boolean, blnInherit As Boolean, Optional ByVal lng路径ID As Long, Optional ByVal strPrivs As String) As Boolean
'功能：设置当前选择项目的详细内容
...
Private Sub chkInherit_Click()
    ...
    If chkInherit.Value = 1 Then
        If mvPreItem.ID = 0 Then
            MsgBox "前一阶段中同一分类下没有相同名称的路径项目。", vbInformation, gstrSysName
            mblnUnCheck = True: chkInherit.Value = 0: mblnUnCheck = False: Exit Sub
        End If
        If mvPreItem.医嘱IDs = "" Then
            MsgBox "前一阶段中相同名称的路径项目没有设置对应的医嘱内容。", vbInformation, gstrSysName
            mblnUnCheck = True: chkInherit.Value = 0: mblnUnCheck = False: Exit Sub
        End If
        
        strFilter = ""
        For i = 0 To UBound(Split(mvPreItem.医嘱IDs, ","))
            strFilter = strFilter & " Or ID=" & Split(mvPreItem.医嘱IDs, ",")(i)
        Next
        mrsAdvice.Filter = Mid(strFilter, 5)
        Do While Not mrsAdvice.EOF
            If Nvl(mrsAdvice!期效, 0) = 1 Then
                MsgBox "前一阶段中相同名称的路径项目所对应的不全是长期医嘱。", vbInformation, gstrSysName
        ...
        mvItem.医嘱IDs = mvPreItem.医嘱IDs
        Call ShowAdvice(mvItem.医嘱IDs)
        
        '其他相关属性也缺省继承
        mvItem.项目内容 = mvPreItem.项目内容
        txtItem.Text = mvItem.项目内容
        txtItem.Enabled = False
        
        mvItem.内容要求 = mvPreItem.内容要求
        optSend(0).Value = mvItem.内容要求 = 0
        optSend(1).Value = Not optSend(0).Value
        optSend(0).Enabled = False
        optSend(1).Enabled = False
        
        If mvPreItem.图标ID <> 0 Then
            mvItem.图标ID = mvPreItem.图标ID
            Call DrawPicture(GetPathIcon(mvItem.图标ID))
        End If
        
        mvItem.执行方式 = mvPreItem.执行方式
        optExecute(mvItem.执行方式).Value = True
        
        mvItem.生成者 = mvPreItem.生成者
        If mvItem.生成者 = 1 Then
            optSendor(Ix医生).Value = True
        Else
            optSendor(Ix护士).Value = True
        End If
        
        mvItem.执行者 = mvPreItem.执行者
        If mvItem.执行者 = 1 Then
            optExecutor(0).Value = True
        ElseIf mvItem.执行者 = 2 Then
            optExecutor(1).Value = True
        End If
        
        If mvPreItem.项目结果 <> "" Then
            mvItem.项目结果 = mvPreItem.项目结果
            With vsResult
                .Rows = .FixedRows
                ...
                For i = 0 To UBound(arrResult)
                    ...
                    End If
                Next
                .Rows = .Rows + 1 '保持一空行用于输入
            End With
        End If
    Else
        If MsgBox("确实要取消与前面相邻阶段相同项目的医嘱关联吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
            mblnUnCheck = True: chkInherit.Value = 1: mblnUnCheck = False: Exit Sub
        End If
        If mvBakItem.医嘱IDs <> mvItem.医嘱IDs Then
            mvItem.医嘱IDs = mvBakItem.医嘱IDs '恢复成继承之前本身的医嘱内容
        Else
            mvItem.医嘱IDs = "" '不继承就清除要输入
        End If
...
'功能：编辑项目所对应的医嘱
    ...
    On Error GoTo errH
    strSql = "Select 科室ID From 临床路径科室 Where 路径ID=[1]"
    ...
    Set rsScheme = gobjKernel.ShowSchemeEdit(Me, 2, rsScheme, False, optSend(1).Value, Mid(str使用科室, 2), 1)
    If Not rsScheme Is Nothing Then
        'blnUpdate=True 将审核且未停用的路径医嘱变动记录保存到【路径医嘱变动】中，待审核后再更新表单路径医嘱。
        blnUpdate = InStr(mstrPrivs, "路径医嘱调整") = 0 And mvItem.原医嘱IDs <> ""
...
Private Sub cmdOK_Click()
    ...
    If optType(0).Value Then
        ...
        Do While Not mrsAdvice.EOF
            If strFilter = "" Then
                strFilter = mrsAdvice!期效
            ElseIf strFilter <> mrsAdvice!期效 And blnPrompted = False Then
                blnPrompted = True
                If MsgBox("一个路径项目的所有医嘱要求期效相同，建议全部为临嘱或者长嘱。" & vbCrLf & _
                    "因为连续两天生成同一长嘱的路径项目时，缺省会自动勾选该项目。" & vbCrLf & vbCrLf & "你确定要这样做吗？", vbYesNo + vbDefaultButton1 + vbInformation, gstrSysName) = vbNo Then
                    If cmdAdvice.Enabled Then cmdAdvice.SetFocus
                    Exit Sub
                End If
            End If
    ...
    Else
        mvItem.执行者 = 0
    End If
    mvItem.生成者 = IIf(optSendor(Ix医生).Value, 1, 2)
...
Private Sub Form_Load()
    ...
    If mblnReadOnly And mlngItemID <> 0 Then
        strSql = "Select 项目内容,执行方式,执行者,生成者,项目结果,图标ID,内容要求 From 临床路径项目 Where ID=[1]"
        ...
        mvItem.执行者 = Nvl(rsTmp!执行者, 0)
        mvItem.生成者 = Nvl(rsTmp!生成者, 0)
        mvItem.图标ID = Nvl(rsTmp!图标ID, 0)
        mvItem.内容要求 = Val("" & rsTmp!内容要求)
        
        '关联医嘱信息
        strSql = "Select 医嘱内容ID From 临床路径医嘱 Where 路径项目ID=[1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mlngItemID)
        strSql = ""
        ...
        '关联病历信息
        strSql = "Select 文件ID,原型ID From 临床路径病历 Where 项目ID=[1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, mlngItemID)
        strSql = ""
        Do While Not rsTmp.EOF
            If rsTmp!文件ID & "" <> "" Then
    ...
    If mvItem.ID <> 0 Then
        ...
        If mvItem.生成者 = 1 Then
            ...
            If mvItem.医嘱IDs <> "" Then
                ...
                If mvItem.医嘱IDs = mvPreItem.医嘱IDs Then
                    chkInherit.Value = 1
                    txtItem.Enabled = False
                End If
                ...
                If mvItem.Edit = 0 Then
                    ...
                    ElseIf mvItem.病历IDs <> "" Then '新版+老版
                        strSql = "Select A.文件ID,A.原型ID,Nvl(a.名称, b.名称) as 名称,decode(文件ID,NULL,2,1) as 版本 From 临床路径病历 A, 病历文件列表 B Where a.项目id = [3] And a.文件id = b.Id(+)" & vbNewLine & _
                            "order by a.序号"
                    Else '新版
                        strSql = "Select T.原型ID,T.名称,2 as 版本 From 临床路径病历 T Where t.项目id = [3] And t.文件id Is Null And t.原型ID IN(Select Column_Value From Table(Cast(f_STR2list([2]) As zlTools.t_Strlist)))" & _
                            " Order by 序号"
                    End If
        ...
        Else
            '护理项目只有文本类
            optSendor(Ix护士).Value = True
            optType(2).Value = True  '文本对象
        End If
        optType(0).Enabled = optSendor(Ix医生).Value
        optType(1).Enabled = optSendor(Ix医生).Value
        '----
        optExecute(mvItem.执行方式).Value = True
        If mvItem.执行者 = 1 Then
            optExecutor(0).Value = True
...
Private Sub optSendor_Click(Index As Integer)
    ...
    If Visible Then
        optType(0).Enabled = (Index = Ix医生)
        optType(1).Enabled = (Index = Ix医生)
        If Index = Ix护士 Then
            For i = optType.LBound To optType.UBound
                If optType(i).Value Then Exit For
            Next
            optSendor(Ix医生).Tag = i
            optType(2).Value = True
            optExecutor(1).Value = True
        Else
            optType(Val(optSendor(Ix医生).Tag)).Value = True
            optExecutor(0).Value = True
        End If
    End If
End Sub
...
Private Sub txtItem_Validate(Cancel As Boolean)
    ...
    '如果与前一阶段项目相同,则医嘱缺省继续
    If mvItem.ID = 0 And mvItem.医嘱IDs = "" And mvPreItem.ID <> 0 And mvPreItem.医嘱IDs <> "" Then
        If txtItem.Text = mvPreItem.项目内容 And optType(2).Value And chkInherit.Value = 0 Then
            strFilter = ""
            For i = 0 To UBound(Split(mvPreItem.医嘱IDs, ","))
                strFilter = strFilter & " Or ID=" & Split(mvPreItem.医嘱IDs, ",")(i)
            Next
            mrsAdvice.Filter = Mid(strFilter, 5)
            Do While Not mrsAdvice.EOF
                If Nvl(mrsAdvice!期效, 0) = 1 Then Exit Sub
                mrsAdvice.MoveNext
            Loop
            mrsAdvice.Filter = ""
    ...
    With vsEPR
        ...
        Else
            ...
            Else
                ...
                strSql = FuncRecordSetToSQL(rsTmp)
            End If
        End If
