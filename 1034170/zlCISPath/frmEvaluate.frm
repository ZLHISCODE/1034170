...
'功能：选择需要完成的合并路径
    ...
    If mrsMerge.RecordCount > 0 Then
        ...
        Loop
        mrsMerge.MoveFirst
        If Not objPathImport.ShowMe(Me, mPati, 4, , , , , , mrsMerge, True) Then
            '还原信息
            Set mrsMerge = rsTmp
            Exit Sub
        End If
...
Private Sub cmdTurn_Click()
    ...
    If objPathImport.ShowMe(Me, mPati, 1, mPP.路径ID, lngPathID, lngPathVersion) Then
...
Private Sub cmdOK_Click()
    ...
    If mlngFun = 0 Then
        lngLen = GetColumnLength("病人临床路径", "导入说明")
    Else
        lngLen = GetColumnLength("病人路径评估", "评估说明")
    End If
