...
Public Function ShowMe(frmMain As Object, ByVal lng路径ID As Long, ByVal strCaption As String) As Boolean
    mlng路径ID = lng路径ID
...
'功能：保存数据
    ...
    Dim blnGrant As Boolean, blnSetup As Boolean
    ...
    Call zlDatabase.SetPara("必须填写出径登记表", chk必须填写.Value, glngSys, p临床路径应用, blnSetup)
...
Private Sub LoadData()
    ...
    Call FuncNoASC
    vsItem.Row = 1
    chk必须填写.Value = zlDatabase.GetPara("必须填写出径登记表", glngSys, p临床路径应用, 0)
    Exit Sub
errH:
    If ErrCenter() = 1 Then
        Resume
            ...
            Else
                ...
                If MsgBox("你确定要删除当前行吗？", vbQuestion + vbYesNo + vbDefaultButton1, gstrSysName) = vbYes Then
                    ...
                    If CheckItemNO(lngRowNO) Then
                        If MsgBox("在“病人出径记录”中已存在当前行的相关数据，删除后将破坏数据之间的关联，你确定要删除吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                            Exit Sub
                        End If
                    End If
