    ...
    If InStr(",5,6,7,", "," & str诊疗类别 & ",") = 0 Then
        If str分解时间 <> "" And Not rsExec Is Nothing Then
            ...
            For i = 0 To UBound(arrTmp)
                ...
                If blnMakeMoney Then
                    '卫材也可以输入单量总量
                    If str频率 <> "" And (lng计算方式 = 1 Or lng计算方式 = 2 Or str诊疗类别 = "4") Then
                        '计量和计时的需要乘以数次
                        If int期效 = 0 Then
                            '1、长嘱可选频率、持续性、必要时和不定时以单量作为数次。
                            dbl数量 = dbl计价数量 * dbl单量
...
Public Function ActualMoney(str费别 As String, ByVal lng收入项目ID As Long, ByVal cur应收金额 As Currency, _
    Optional ByVal lng收费细目ID As Long, Optional ByVal lng库房ID As Long, Optional ByVal dbl数量 As Double, Optional ByVal dbl加班加价率 As Double, Optional ByVal dbl医嘱ID As Double) As Currency
...
'      对于时价分批，"药品售价=Nvl(零售价,实际金额/实际数量)"；分批或时价药品库存不足时，不予打折计算。
    ...
    On Error GoTo errH
    '[7]这个参数 34版本上未用直接去掉--即入参dbl医嘱ID
    StrSQL = "Select Zl_Actualmoney([1],[2],[3],[4],[5],[6]) as Actualmoney From Dual"
    Set rsTmp = gobjComlib.zlDatabase.OpenSQLRecord(StrSQL, App.ProductName, str费别, lng收费细目ID, lng收入项目ID, cur应收金额 / (1 + dbl加班加价率), dbl数量, lng库房ID, dbl医嘱ID)
