...
2         blnFtp = False
3         If strFtpPara = "" Then
4             strFtpPara = gobjDatabase.GetPara("FTP设置", glngSys, 1208, "")
5         End If
6         If UBound(Split(strFtpPara, ";")) >= 3 Then
7             strFTPuser = Split(strFtpPara, ";")(0)
8             If Split(strFtpPara, ";")(1) Like "ZLSV*:*" Then
9                 strFTPpass = gobjComLib.zlStr.Sm4DecryptEcb(Split(strFtpPara, ";")(1))
10            Else
11                strFTPpass = Split(strFtpPara, ";")(1)
12            End If
13            strFTPIP = Split(strFtpPara, ";")(2)
14            strFPTPath = Split(strFtpPara, ";")(3)
15            If TestFTP(strFTPuser, strFTPpass, strFTPIP, strFPTPath) = "" Then
16                blnFtp = True
17            End If
18        End If

19        aGraphItem = Split(strImg, "^")


20        For intLoop = 0 To UBound(aGraphItem)
21            strImageVal = Replace(aGraphItem(intLoop), vbCrLf, "")
22            strImageType = Mid(strImageVal, 1, InStr(strImageVal, ";") - 1)
23            strImageData = Mid(strImageVal, InStr(strImageVal, ";") + 1)

24            If Mid(strImageData, 1, InStr(strImageData, ";") - 1) >= 100 And Mid(strImageData, 1, InStr(strImageData, ";") - 1) <= 227 Then
                  '组织图片数据

25                intLayOut = Mid(strImageData, 1, InStr(strImageData, ";") - 1)
26                strPicPath = Mid(strImageData, InStr(strImageData, ";") + 1)

27                If InStr(strPicPath, ";") > 0 Then
28                    If Left(strPicPath, 2) = "1;" Then
29                        blnDeleImg = True
30                    End If
31                    strPicPath = Mid(strPicPath, InStr(strPicPath, ";") + 1)
32                End If

33                If Dir(strPicPath) <> "" Then
34                    If UCase(Right(strPicPath, 4)) = ".BMP" And intLayOut >= 100 And intLayOut <= 107 Then
35                        strBMPFile = strPicPath
36                    ElseIf (UCase(Right(strPicPath, 4)) = ".JPG" Or UCase(Right(strPicPath, 4)) = ".GIF") And intLayOut >= 110 And intLayOut <= 127 Then
37                        strBMPFile = strPicPath
38                    ElseIf intLayOut >= 200 And intLayOut <= 227 Then
39                        strPicPath = UCase$(strPicPath)
40                        strBMPFile = zlFileZip(strPicPath)
41                    Else
42                        frmLISSrv.picTmp.Picture = LoadPicture(strPicPath)
43                        If Dir(App.Path & "\zlLisIn.bmp") <> "" Then Kill App.Path & "\zlLisIn.bmp"
44                        SavePicture frmLISSrv.picTmp.Picture, App.Path & "\zlLisIn.bmp"
45                        strBMPFile = App.Path & "\zlLisIn.bmp"
46                    End If

47                    If Not blnFtp Then
                          '保存到数据库
48                        If zlLisBlobSql(lngID, strImageType, strBMPFile, intLayOut, strSQL) Then
49                            WriteLog "执行 SaveImg", LOG_通讯日志, 0, "开始时间"
50                            For IntCount = LBound(strSQL) To UBound(strSQL)
51                                If strSQL(IntCount) <> "" Then
52                                    gstrSQL = strSQL(IntCount)
53                                    gobjDatabase.ExecuteProcedure Replace(strSQL(IntCount), "Call", ""), "保存图像数据"
54                                End If
55                            Next
56                            WriteLog "执行 SaveImg", LOG_通讯日志, 0, "结束时间"
57                        End If
58                    Else
                          '保存到FTP
                          '图像位置保存的数据格式为：图像格式;FTP文件路径
                          '图像格式为100-227 。
59                        strFTPDir = strFPTPath & IIf(Right(strFPTPath, 1) = "/", "", "/") & "Dev_" & lngDevID & "/" & Format(gobjDatabase.Currentdate, "yyyyMM")
60                        strNewName = lngID & "_" & strImageType & Right(strPicPath, 4)
61                        strUploadOk = UploadFile(strFTPuser, strFTPpass, strFTPIP, strFTPDir, strBMPFile, strNewName)
62                        If strUploadOk = "" Then
63                            gstrSQL = "Zl_检验图像结果_Update(" & lngID & ",'" & strImageType & "',Null,0,1,'" & _
                                        intLayOut & ";" & strFTPDir & "/" & strNewName & "')"
64                            gobjDatabase.ExecuteProcedure gstrSQL, "保存检验图形数据"
65                        Else
66                            WriteLog "上传图片文件到FTP", LOG_通讯日志, 0, strUploadOk
67                        End If
68                    End If

69                    If blnDeleImg Then
70                        IntCount = 0
71                        Do While Dir(strPicPath) <> "" And IntCount < 100
72                            IntCount = IntCount + 1
73                            gobjFSO.DeleteFile strPicPath, True
74                        Loop
75                        If Dir(strPicPath) <> "" Then
76                            Call WriteLog("SaveImg", LOG_错误日志, 0, "文件" & strPicPath & "估计还有其他程序在用，未能删除，需事后手工删除！")
77                        End If
78                    End If
79                End If
80            Else
                  '图形数据
81                If Not blnFtp Then
82                    If Len(strImageData) > 2000 Then
                          '保存大于2000以上数据
83                        For IntCount = 1 To CInt(Len(strImageData) / 1000) + 1
84                            If Len(strImageData) > 0 Then

85                                gstrSQL = "Zl_检验图像结果_Update(" & lngID & ",'" & strImageType & "','" & _
                                            Mid(strImageData, IntCount * 1000 - 999, 1000) & "'," & _
                                            "1," & IntCount & ")"
86                                gobjDatabase.ExecuteProcedure gstrSQL, "检验图像保存"
87                            End If
88                        Next
89                    Else
90                        gstrSQL = "Zl_检验图像结果_Update(" & lngID & ",'" & strImageType & "','" & strImageData & "',0,1)"
91                        gobjDatabase.ExecuteProcedure gstrSQL, "检验图像保存"
92                    End If
93                Else
                      '存为TXT文件然后上传
94                    intLayOut = Mid(strImageData, 1, InStr(strImageData, ";") - 1)
95                    strPicPath = Mid(strImageData, InStr(strImageData, ";") + 1)

96                    strBMPFile = App.Path & "\" & lngID & "_" & strImageType & ".txt"
97                    If gobjFSO.FileExists(strBMPFile) Then gobjFSO.DeleteFile strBMPFile
98                    Set objStream = gobjFSO.CreateTextFile(strBMPFile)
99                    objStream.Write strPicPath
100                   objStream.Close
...
102                   strFTPDir = strFPTPath & IIf(Right(strFPTPath, 1) = "/", "", "/") & "Dev_" & lngDevID & "/" & Format(gobjDatabase.Currentdate, "yyyyMM")
103                   strNewName = lngID & "_" & strImageType & ".txt"
104                   strUploadOk = UploadFile(strFTPuser, strFTPpass, strFTPIP, strFTPDir, strBMPFile, strNewName)
105                   If strUploadOk = "" Then
106                       gstrSQL = "Zl_检验图像结果_Update(" & lngID & ",'" & strImageType & "',Null,0,1,'" & _
                                    intLayOut & ";" & strFTPDir & "/" & strNewName & "')"
107                       gobjDatabase.ExecuteProcedure gstrSQL, "保存检验图形数据"
108                   Else
109                       WriteLog "上传图像数据文件到FTP", LOG_通讯日志, 0, strUploadOk
110                   End If
111                   If gobjFSO.FileExists(strBMPFile) Then gobjFSO.DeleteFile strBMPFile
112               End If
113           End If
114       Next
...
115       Exit Sub
ErrHandle:
116       Call WriteLog("SaveImg-" & CStr(Erl()), LOG_错误日志, Err.Number, Err.Description)
