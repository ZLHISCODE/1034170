...
Private blnDrop As Boolean, blnHead As Boolean
...
Private Sub cmdAtt_Click()
    ...
    Select Case mshAtt.TextMatrix(mshAtt.Row, 0)
        ...
        Case "字体" '可以改变字体,字号,粗体,斜体
            ...
            cdg.ShowFont
            If Err.Number = 0 Then
                mshAtt.TextMatrix(mshAtt.Row, mshAtt.Col) = cdg.FontName
                '如果被选中的所有元素都是相同的元素
                If LblSize.count > 9 Then
                    For Each tmpObj In LblSize
                        If tmpObj.Index Mod 8 = 1 Then
        ...
        Case "关联报表"
            ...
            If X > k And X > Y And X <> 0 And k <> 0 Then
                lngReportID = FindReport("", txtAtt.hwnd, strInfo, objReport.Items("_" & intCurID).Relations.Item(1).关联报表ID)
                If lngReportID <> 0 Then
                    '如果切换了报表，没有设置参数，则清空参数对照
                    If lngReportID <> objReport.Items("_" & intCurID).Relations.Item(1).关联报表ID Then
                        For i = objReport.Items("_" & intCurID).Relations.count To 1 Step -1
                            objReport.Items("_" & intCurID).Relations.Remove i
                        Next
                    End If
                    If frmRelationSetup.ShowMe(Me, lngReportID, Mid(objReport.Items("_" & intCurID).内容, k + 1, Y - k - 1), objReport, strInfo, objReport.Items("_" & intCurID).Relations, 2) Then
                        '如果对照了关联报表，却一个参数都没有对照，则增加一条参数名为NULL的表示
                        If lngReportID <> 0 Then
                            If objReport.Items("_" & intCurID).Relations.count = 0 Then
                                objReport.Items("_" & intCurID).Relations.Add lngReportID, "NULL", "", strInfo
                            End If
                        End If
                        mshAtt.TextMatrix(mshAtt.Row, 1) = strInfo
                        mshAtt.RowData(mshAtt.Row) = lngReportID
...
Private Sub Form_Resize()
    ...
    If blnMax Or WindowState = 2 Then
        picSQL.Width = 3500
        picAtt.Width = 2400
        lvwPar.Height = 1700
        lblNote.Height = 900
        blnMax = False
    End If
    If WindowState = 2 Then blnMax = True
...
Private Sub mshAtt_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    If mshAtt.TextMatrix(mshAtt.Row, 0) = "内容" Then
        If objReport.Items("_" & intCurID).类型 = 11 Then
            ...
            cmdAtt.Visible = True ': cmdAtt.SetFocus
            Exit Sub
    ...
    '编辑处理
    Select Case mshAtt.TextMatrix(mshAtt.Row, 0)
        Case "名称", "内容", "X坐标", "Y坐标", "宽度", "高度", "行高", "分栏", "字号", "格式", "行距", "左右间距", "上下间距", "数据源行号", "横向分栏", "纵向分栏"
            ...
            If objReport.Items("_" & intCurID).类型 <> 2 Or mshAtt.TextMatrix(mshAtt.Row, 0) <> "内容" Then
                ...
                txtAtt.Width = mshAtt.CellWidth - 60
            ...
            Else
                ...
                cboText.Width = mshAtt.CellWidth
...
'功能：对汇总表格,对插入各种子项目在后
    ...
    If selCell.Col1 <= msh(intCurID).FixedCols - 1 And selCell.Row1 >= msh(intCurID).FixedRows - 1 Then
        ...
        If gblnOK Then
            ...
            Next
            intMaxID = intMaxID + 1
            Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, "元素" & intMaxID, intCurID, 7, selCell.Col1 + 1, "", 0, frmData.txtData.Text, "", 0, 0, 1000, 0, 0, 0, False, "", 0, _
                frmData.IO_FontBold = 1, False, False, 0, frmData.IO_FontColor, 0, False, 0, IIF(frmData.optDesc, ",", "") & frmData.txtOrder.Text, "", "", False, , False, , , , "_" & intMaxID)
    ...
    ElseIf selCell.Row1 <= msh(intCurID).FixedRows - 2 Then
        ...
        If gblnOK Then
            ...
            Next
            intMaxID = intMaxID + 1
            Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, "元素" & intMaxID, intCurID, 8, selCell.Row1 + 1, "", 0, frmData.txtData.Text, "", 0, 0, 0, 0, 0, 0, False, "", 0, _
                frmData.IO_FontBold = 1, False, False, 0, frmData.IO_FontColor, 0, False, 0, IIF(frmData.optDesc, ",", "") & frmData.txtOrder.Text, "", "", False, , False, , , , "_" & intMaxID)
    ...
    ElseIf selCell.Col1 >= msh(intCurID).FixedCols And selCell.Col1 <= msh(intCurID).FixedCols + intState - 1 And selCell.Row1 >= msh(intCurID).FixedRows - 1 Then
        ...
        If gblnOK Then
            ...
            Next
            intMaxID = intMaxID + 1
            Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, "元素" & intMaxID, intCurID, 9, selCell.Col1 - msh(intCurID).FixedCols + 1, "", 0, frmData.txtData.Text, "", _
                0, 0, 1000, 0, 0, 0, False, "", 0, frmData.IO_FontBold = 1, False, False, 0, frmData.IO_FontColor, 0, False, 0, "", frmData.txtFormat.Text, "", False, , False, , , , "_" & intMaxID)
...
'功能：对汇总表格,对插入各种子项目在前
    ...
    If selCell.Col1 <= msh(intCurID).FixedCols - 1 And selCell.Row1 >= msh(intCurID).FixedRows - 1 Then
        ...
        If gblnOK Then
            ...
            Next
            intMaxID = intMaxID + 1
            Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, "元素" & intMaxID, intCurID, 7, selCell.Col1, "", 0, frmData.txtData.Text, "", 0, 0, 1000, 0, 0, 0, False, "", 0, _
            frmData.IO_FontBold = 1, False, False, 0, frmData.IO_FontColor, 0, False, 0, IIF(frmData.optDesc, ",", "") & frmData.txtOrder.Text, "", "", False, , False, , , , "_" & intMaxID)
    ...
    ElseIf selCell.Row1 <= msh(intCurID).FixedRows - 2 Then
        ...
        If gblnOK Then
            ...
            Next
            intMaxID = intMaxID + 1
            Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, "元素" & intMaxID, intCurID, 8, selCell.Row1, "", 0, frmData.txtData.Text, "", 0, 0, 0, 0, 0, 0, False, "", 0, frmData.IO_FontBold = 1, _
            False, False, 0, frmData.IO_FontColor, 0, False, 0, IIF(frmData.optDesc, ",", "") & frmData.txtOrder.Text, "", "", False, , False, , , , "_" & intMaxID)
    ...
    ElseIf selCell.Col1 >= msh(intCurID).FixedCols And selCell.Col1 <= msh(intCurID).FixedCols + intState - 1 And selCell.Row1 >= msh(intCurID).FixedRows - 1 Then
        ...
        If gblnOK Then
            ...
            Next
            intMaxID = intMaxID + 1
            Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, "元素" & intMaxID, intCurID, 9, selCell.Col1 - msh(intCurID).FixedCols, "", 0, frmData.txtData.Text, "", 0, 0, _
            1000, 0, 0, 0, False, "", 0, frmData.IO_FontBold = 1, False, False, 0, frmData.IO_FontColor, 0, False, 0, "", frmData.txtFormat.Text, "", False, , False, , , , "_" & intMaxID)
...
'功能：对任意表格,设置当前列的数据计算方式
    ...
    For Each tmpID In objReport.Items("_" & intCurID).SubIDs
        ...
        End If
    Next
    frmFormula.strInit = tmpItem.内容
    frmFormula.strFormat = tmpItem.格式
    
    frmFormula.mbln换页 = tmpItem.边框
    frmFormula.mblnCan换页 = True 'objReport.Items("_" & intCurID).分栏 <= 1
    
    frmFormula.mblnMerge = tmpItem.自调
    frmFormula.mblnPreMerge = blnPreMerge
    
    frmFormula.mblnAutoFont = tmpItem.行高 = 1
    frmFormula.mblnVisible = tmpItem.分栏 = 1
    
    frmFormula.intCol = intCurCol
    frmFormula.intCur = intCurID
    Set frmFormula.frmParent = Me
    Set frmFormula.objReport = objReport
    Call SetCopyRelations(tmpItem.Relations, objRelations)
    Call SetCopyColProtertys(tmpItem.ColProtertys, objColProtertys)
    Set frmFormula.mobjRelations = objRelations
    Set frmFormula.mobjColProtertys = objColProtertys
    frmFormula.Show 1, Me
    If gblnOK Then
            ...
            If objReport.Items("_" & tmpItem.上级ID).父ID <> 0 Then
                If objReport.Items("_" & objReport.Items("_" & tmpItem.上级ID).父ID).数据源 <> "" Then
                    ...
                    If X > k And X > Y And X <> 0 And k <> 0 And Y <> 0 Then
                        If Mid(frmFormula.txtFormula.Text, k + 1, Y - k - 1) <> objReport.Items("_" & objReport.Items("_" & tmpItem.上级ID).父ID).数据源 Then
        ...
        tmpItem.行高 = frmFormula.chkAutoFont.Value
...
'功能：对任意表格,在当前列左边插入一空列
    ...
    objReport.Items.Add intMaxID, mbytCurrFmt, "元素" & intMaxID, i, 6, intCurCol, "", 0, "", strHead, 0, 0, 1000, 0, 0, 0, False, "", 0, False, False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMaxID
    objReport.Items("_" & intCurID).SubIDs.Add intMaxID, "_" & intMaxID
...
'功能：对任意表格,在当前列右边插入一空列
    ...
    objReport.Items.Add intMaxID, mbytCurrFmt, "元素" & intMaxID, i, 6, intCurCol + 1, "", 0, "", strHead, 0, 0, 1000, 0, 0, 0, False, "", 0, False, False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMaxID
    objReport.Items("_" & intCurID).SubIDs.Add intMaxID, "_" & intMaxID
    ...
    For Each tmpObj In LblSize
        If tmpObj.Index Mod 8 = 1 Then
            With objReport.Items("_" & tmpObj.Tag)
                ...
                .图片, .系统, .父ID, .SubIDs, .CopyIDs, "_" & .ID, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏)
                '处理子项
            ...
            For Each tmpItem In frmGuide.objGuide.Items
                With tmpItem
                    objReport.Items.Add .ID, mbytCurrFmt, .名称, .上级ID, .类型, .序号, .参照, .性质, .内容, .表头 _
                        ...
                        , .边框, .分栏, .排序, .格式, .汇总, .表格线加粗, .图片, .系统, .父ID, .SubIDs, .CopyIDs, "_" & .ID, .数据源 _
                        ...
                        If .类型 = 2 Or .类型 = 12 Then '@@@
                            ...
                            If .参照 <> "" Then
                                ...
                                Case 1  '表上项
                                    ...
                                    Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, .名称, 0, .类型, .序号, .参照, .性质, .内容, .表头, RectTest.Left, RectTest.Top, RectTest.Right, RectTest.Bottom, _
                                        .行高, .对齐, .自调, .字体, .字号, .粗体, .下线, .斜体, .网格, .前景, .背景, .边框, .分栏, .排序, .格式, .汇总, .表格线加粗, .图片, .系统, lng父ID, , , "_" & intMaxID, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏)
                                Case Else
                                    If (.Y + .H + 200) * sgnMode < picPaper.Height - 100 Then RectTest.Top = RectTest.Top + RectTest.Bottom + 200
                                    If .系统 Then Call GetCoordinate(RectTest)
                                    Set tmpItem = objReport.Items.Add(intMaxID, mbytCurrFmt, .名称, 0, .类型, .序号, .参照, .性质, .内容, .表头, RectTest.Left, RectTest.Top, RectTest.Right, RectTest.Bottom, _
                                        .行高, .对齐, .自调, .字体, .字号, .粗体, .下线, .斜体, .网格, .前景, .背景, .边框, .分栏, .排序, .格式, .汇总, .表格线加粗, .图片, .系统, lng父ID, , , "_" & intMaxID, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏)
                                End Select
                    ...
                    If (.类型 = 4 Or .类型 = 5) And .SubIDs.count > 0 Then
                        For Each tmpID In .SubIDs
                            ...
                            With objClip("_" & tmpID.ID)
                                objReport.Items.Add intMaxID, mbytCurrFmt, strName, tmpItem.ID, .类型, .序号, .参照, .性质, .内容, .表头, .X + 300, .Y + 300, .W, .H, _
                                    .行高, .对齐, .自调, .字体, .字号, .粗体, .下线, .斜体, .网格, .前景, .背景, .边框, .分栏, .排序, .格式, .汇总, .表格线加粗, .图片, .系统, lng父ID, , , "_" & intMaxID, .数据源, .上下间距, .左右间距, .源行号, .横向分栏, .纵向分栏
...
Private Sub msh_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    Else
        If objReport.Items("_" & Index).类型 = 5 Then
            If Button = 1 And Mid(msh(Index).Tag, 1, 2) <> "" Then
                If blnLock Then Exit Sub
                Call MoveSelect(X - lngPreX, Y - lngPreY)
                If GetSelNum() = 1 Then ShowAttrib Index
            End If
...
Private Sub mshAtt_DblClick()
    ...
    Dim blnFlag As Boolean, ObjSel As Object
...
Private Sub ResizeAttrib()
    Dim lngW As Long
    
    txtAtt.Visible = False
    cboAtt.Visible = False
    
    lngW = IIF(ScrollExist(mshAtt), mshAtt.Width - 250, mshAtt.Width)
    
    mshAtt.ColWidth(0) = 800
    mshAtt.ColWidth(1) = lngW - mshAtt.ColWidth(0)
End Sub
    ...
    If Not blnDrop Then
        Select Case bytCurTool
            Case 1 '线条(lblLine)
                ...
                '加入数据对象
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(1), 0, 1, 0, "", 0, "", "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                0, 0, True, "", 0, False, False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intCurID
            Case 3  '标签(lbl)
                Load lbl(intCurID)
                ...
                '加入数据对象
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(IIF(bytCurTool = 2, 10, 2)), 0, IIF(bytCurTool = 2, 10, 2), 0, "", 0, GetNextName(IIF(bytCurTool = 2, 10, 2)), "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                0, 0, IIF(bytCurTool = 3, True, False), "宋体", 9, False, False, False, 0, 0, &HFFFFFF, False, 0, "", "", "", False, , False, , , , "_" & intCurID
            Case 2 '框线(shp)
                ...
                '加入数据对象
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(10), 0, 10, 0, "", 0, GetNextName(10), "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                0, 0, IIF(bytCurTool = 3, True, False), "宋体", 9, False, False, False, 0, 0, &HFFFFFF, False, 0, "", "", "", False, , False, , , , "_" & intCurID
            Case 4 '图片
                Load Img(intCurID)
                ...
                '照片字段
                '加入数据对象
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(11), 0, 11, 0, "", 0, "", "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                0, 0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, True, 0, "", "", "", False, , False, , , , "_" & intCurID
            Case 8 '卡片
                Load pic(intCurID)
                ...
                '照片字段
                '加入数据对象
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(14), 0, 14, 0, "", 0, "", "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                0, 0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, True, 0, "", "", "", False, , False, , , , "_" & intCurID
            Case 5 '任意表格(特殊)
                Load msh(intCurID)
                ...
                Set tmpItem = objReport.Items.Add(intCurID, mbytCurrFmt, GetNextName(4), 0, 4, 0, "", 0, "", "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                255, 0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, True, 1, "", "", "", False, , False, , , , "_" & intCurID)
                ...
                For i = 0 To msh(intCurID).Cols - 1
                    ...
                    objReport.Items.Add intMaxID, mbytCurrFmt, "表列" & intMaxID, intCurID, 6, i, "", 0, "", _
                        ...
                        0, 0, False, "", 0, False, False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMaxID
                        tmpItem.SubIDs.Add intMaxID, "_" & intMaxID
                Next
                '可以合并
                For i = 0 To msh(intCurID).FixedRows - 1
            ...
            Case 6 '图表@@@
                ...
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(12), 0, 12, 1, "", 0, "", "", _
                    ...
                    Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                    0, 1, True, "宋体", 9, False, True, False, 0, 0, &HFFFFFF, False, 0, "", "", "", False, , False, , , , "_" & intCurID
            Case 7 '条码
                ...
                Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), _
                Format(Me.ScaleX(sngWidth, vbMillimeters, vbTwips), "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                2, 0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, False, 0, "", "", "", False, , False, , , , "_" & intCurID
        End Select
        ...
        If tvwSQL.SelectedItem.Children = 0 Then
            If Not objParent Is Nothing Then
                If objReport.Items("_" & objParent.Index).数据源 = "" Then
                    If MsgBox("当前卡片未绑定数据源，绑定后将分组打印多张卡片，数据源中存在""分组标识""字段则""分组标识""相同的为一组,否则一行数据为一组；" & vbCrLf & _
                         "不绑定则只打印一张卡片，是否绑定数据源""" & tvwSQL.SelectedItem.Parent.Text & """?", vbQuestion + vbYesNo + vbDefaultButton1, App.Title) = vbYes Then
            ...
            If IsType(Val(tvwSQL.SelectedItem.Tag), adLongVarBinary) Then
                ...
                '加入数据对象
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(2), 0, 2, 0, "", 0, newObj.Caption, "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                0, 0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, True, 0, "", "", "", False, , False, , , , "_" & intCurID
            Else
                ...
                '加入数据对象
                objReport.Items.Add intCurID, mbytCurrFmt, GetNextName(2), 0, 2, 0, "", 0, newObj.Caption, "", Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                0, bytAlign, True, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, False, 0, "", "", "", False, , False, , , , "_" & intCurID
            End If
        Else
            If objReport.Datas(tvwSQL.SelectedItem.Key).类型 = 0 Then
                '任意表只能放入和卡片数据源相同的数据源
                If Not objParent Is Nothing Then
                    If mdlPublic.GetStdNodeText(tvwSQL.SelectedItem.Text) <> objReport.Items("_" & objParent.Index).数据源 And objReport.Items("_" & objParent.Index).数据源 <> "" Then
                    ...
                    If objReport.Items("_" & objParent.Index).数据源 = "" Then
                        If MsgBox("当前卡片未绑定数据源，绑定后将分组打印多张卡片，数据源中存在""分组标识""字段则""分组标识""相同的为一组,否则一行数据为一组；" & vbCrLf & _
                             "不绑定则只打印一张卡片，是否绑定数据源""" & tvwSQL.SelectedItem.Text & """?", vbQuestion + vbYesNo + vbDefaultButton1, App.Title) = vbYes Then
                ...
                '加入数据对象(1:内容为数据源名)
                Set tmpItem = objReport.Items.Add(intCurID, mbytCurrFmt, GetNextName(4), 0, 4, 0, "", 0, mdlPublic.GetStdNodeText(tvwSQL.SelectedItem.Text), "", _
                    Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                    255, 0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, True, 1, "", "", "", False, , False, , , , "_" & intCurID)
                ...
                For i = 0 To msh(intCurID).Cols - 1
                    ...
                    For j = 0 To msh(intCurID).FixedRows - 1
                        msh(intCurID).Row = j: msh(intCurID).Col = i
                        Str表头 = Str表头 & "|" & msh(intCurID).CellAlignment & "^" & msh(intCurID).RowHeight(j) & "^" & msh(intCurID).TextMatrix(j, i)
                    Next
                    Str表头 = Mid(Str表头, 2)
                    objReport.Items.Add intMaxID, mbytCurrFmt, "表列" & intMaxID, intCurID, 6, i, _
                    "", 0, msh(intCurID).TextMatrix(1, i), Str表头, _
                    0, 0, msh(intCurID).ColWidth(i) / sgnMode, 0, 0, arrAlign(i), False, "", 0, False, _
                    False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMaxID
            ...
            Else
                ...
                With msh(intCurID)
                    ...
                    '加入数据对象(1:内容为数据源名)
                    Set tmpItem = objReport.Items.Add(intCurID, mbytCurrFmt, GetNextName(5), 0, 5, 0, "", 0, mdlPublic.GetStdNodeText(tvwSQL.SelectedItem.Text), "", _
                        Format(selArea.Left / sgnMode, "0.00"), Format(selArea.Top / sgnMode, "0.00"), Format((selArea.Right - selArea.Left) / sgnMode, "0.00"), Format((selArea.Bottom - selArea.Top) / sgnMode, "0.00"), _
                        255, 0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, True, 0, "", "", "", False, , False, , , , "_" & intCurID)
                            ...
                            Select Case objNode.Tag
                                Case adChar, adVarChar, adLongVarChar, adWChar, adVarWChar, adLongVarWChar, adBSTR, _
                                    ...
                                    If i <= X Then
                                        ...
                                        "", 0, mdlPublic.GetStdNodeText(objNode.Text), "", 0, 0, 1000, 0, 255, 0, False, "", 0, False, _
                                        False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMaxID
                                    Else
                                        ...
                                        "", 0, mdlPublic.GetStdNodeText(objNode.Text), "", 0, 0, 1000, 0, 255, 0, False, "", 0, False, False, False, _
                                        0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMaxID
                                    End If
                                Case adNumeric, adVarNumeric, adSingle, adDouble, adCurrency, adDecimal, adBigInt, adInteger, adSmallInt, adTinyInt, adUnsignedBigInt, adUnsignedInt, adUnsignedSmallInt, adUnsignedTinyInt
                                    ...
                                    "", 0, mdlPublic.GetStdNodeText(objNode.Text), "", 0, 0, 1000, 0, 255, 2, False, "", 0, False, _
                                    False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMaxID
                                    j = j + 1 'j为序号
                            End Select
                            tmpItem.SubIDs.Add intMaxID, "_" & intMaxID
        ...
        If idx <> 0 Then
            ...
            Select Case intType
                ...
                Case 2 '标签
                    ...
                    .TextMatrix(20, 0) = "关联报表": .TextMatrix(20, 1) = objReport.Items("_" & idx).Relations.Item(1).关联报表名称
                Case 4 '任意表格
                    .Rows = 18
    ...
    If KeyAscii = 13 Then
        ...
        Select Case mshAtt.TextMatrix(mshAtt.Row, 0)
            ...
            Case "关联报表"
                ...
                k = InStr(1, objReport.Items("_" & intCurID).内容, "[")
                If X > k And X > Y And X <> 0 And k <> 0 Then
                    lngReportID = FindReport(txtAtt.Text, txtAtt.hwnd, strInfo, objReport.Items("_" & intCurID).Relations.Item(1).关联报表ID)
                    If lngReportID <> 0 Then
                        '如果切换了报表，没有设置参数，则清空参数对照
                        If lngReportID <> objReport.Items("_" & intCurID).Relations.Item(1).关联报表ID Then
                            For i = objReport.Items("_" & intCurID).Relations.count To 1 Step -1
    ...
    If Not rsTmp.EOF Then
        ...
        Do While Not rsTmp.EOF
            ...
            If IsNull(rsTmp!功能) Then
                gcnOracle.Execute "Delete From zlProgPrivs Where 序号=" & rsTmp!程序ID & " And 功能 is Null And Nvl(系统,0)=" & Nvl(rsTmp!系统, 0)
            Else
                gcnOracle.Execute "Delete From zlProgPrivs Where 序号=" & rsTmp!程序ID & " And 功能='" & rsTmp!功能 & "' And Nvl(系统,0)=" & Nvl(rsTmp!系统, 0)
            End If
            If strObject <> "" Then '该表格有可能不访问数据库
                For i = 0 To UBound(Split(strObject, ","))
                    ...
                    If strOwner <> "SYS" And strOwner <> "ZLTOOLS" And strOwner <> "SYSTEM" Then
                        ...
                        IIF(IsNull(rsTmp!系统), "NULL", rsTmp!系统) & "," & rsTmp!程序ID & ",'" & rsTmp!功能 & "','" & strName & "','" & strOwner & "','SELECT')"
                    End If
                Next
            End If
            rsTmp.MoveNext
