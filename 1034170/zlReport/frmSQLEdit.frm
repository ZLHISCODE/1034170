...
Begin VB.Form frmSQLEdit 
   ...
   Begin VB.PictureBox picTop 
      ...
      Begin VB.ComboBox cboData 
         Height          =   300
         ItemData        =   "frmSQLEdit.frx":0222
         Left            =   2280
         List            =   "frmSQLEdit.frx":0224
         Style           =   2  'Dropdown List
         TabIndex        =   5
         Top             =   1080
         Visible         =   0   'False
      ...
      Begin VB.Image Image1 
         Height          =   480
         Left            =   330
         Picture         =   "frmSQLEdit.frx":0226
         Top             =   195
         Width           =   480
      End
      Begin VB.Label lblCaption 
         BackStyle       =   0  'Transparent
         Caption         =   $"frmSQLEdit.frx":0530
         Height          =   735
         Left            =   1110
         TabIndex        =   22
         Top             =   75
...
Private Sub cmdOK_Click()
    ...
    For i = 1 To intCount
        With mobjPars("_" & i - 1)
            '格式属性
            mobjData.Pars.Add .组名, .序号, .名称, .类型, .缺省值, .格式, .值列表, .分类SQL, .明细SQL, .分类字段, .明细字段, .对象, "_" & .Key, , .是否锁定
        End With
    Next
...
Private Sub Form_Load()
    ...
    '获取数据连接
    Call SetControlDBConnect(cboConn)
    ...
    ElseIf mlngType = 1 Then
        Caption = "查看历史数据源"
        lblCaption = "请根据条件查看数据源历史修改记录、点击对比查看历史SQL差异情况；按F3：拷贝可运行的SQL语句。"
...
'功能：读取历史数据源记录
    ...
    On Error GoTo errH
    strSQL = "select  RPAD(修改人,25) as 修改人,修改时间,行号,内容 from zlRPTSQLsHistory Where 报表ID=[1] and 数据源名称=[2] Order By 修改时间 Desc,行号"
