...
'        Next
    ...
    For i = 0 To lblNO.UBound
        ...
        End If
        Set curPar = Nothing
        Set curPar = mobjPars.Add(arrCustom(i).组名, CByte(i), txtName(i).Text, cboType(i).ListIndex, cboValue(i).Text, arrCustom(i).格式, arrCustom(i).值列表, arrCustom(i).分类SQL, arrCustom(i).明细SQL, arrCustom(i).分类字段, arrCustom(i).明细字段, arrCustom(i).对象, "_" & i, , chkLock(i).Value)
