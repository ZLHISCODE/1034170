...
Private Sub cmdOK_Click()
    ...
    If Val(txtRelation.Tag) <> mobjRelations.Item(1).关联报表ID Then
...
Private Sub cmdRelation_Click()
    ...
    If X > k And X > Y And X <> 0 And k <> 0 Then
        lngReportID = FindReport("", txtRelation.hwnd, strInfo, Val(txtRelation.Tag))
...
Private Sub cmdRelationSetup_Click()
    ...
    k = InStr(1, txtFormula.Text, "[")
    If X > k And X > Y And X <> 0 And k <> 0 Then
        Call frmRelationSetup.ShowMe(Me, Val(txtRelation.Tag), Mid(txtFormula.Text, k + 1, Y - k - 1), objReport, txtRelation.Text, mobjRelations, 1)
    Else
        MsgBox "当前列必须先绑定一个数据源，例如：[数据源.字段],绑定后再设置关联报表。", vbInformation, Me.Caption
    End If
End Sub
...
Private Sub txtRelation_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        k = InStr(1, txtFormula.Text, "[")
        If X > k And X > Y And X <> 0 And k <> 0 Then
            lngReportID = FindReport(txtRelation.Text, txtRelation.hwnd, strInfo, Val(txtRelation.Tag))
            If lngReportID <> 0 Then
                txtRelation.Text = strInfo
                txtRelation.Tag = lngReportID
            Else
