...
Private Sub cmdOK_Click()
    ...
    Dim lngProgID As Long, lngSys As Long
    ...
    '系统号
    lngSys = Val(frmMain.cboSys.ItemData(frmMain.cboSys.ListIndex))
    
    '报表组是否已发布
    lngProgID = ReportGroupIssue(mlngGroupID)
    
    '保存设置
    gcnOracle.BeginTrans
    
    '先清除当前组的所有子报表记录
    gcnOracle.Execute "Delete zlRPTSubs Where 组ID=" & mlngGroupID
    
    If lngProgID <> 0 Then
        '报表已发布，清除当前组的所有子报表的权限记录
        gcnOracle.Execute _
            "Delete zlProgFuncs Where 序号=" & lngProgID & " And nvl(系统,0)=" & lngSys
    End If
    ...
    For IntItems = 1 To lvwSet.ListItems.count
        ...
        gcnOracle.Execute "Insert Into zlRPTSubs(组ID,报表ID,序号,功能) Values(" & _
            mlngGroupID & _
            "," & Mid(lvwSet.ListItems(IntItems).Key, 2) & _
            "," & lvwSet.ListItems(IntItems).Text & _
            ",'" & lvwSet.ListItems(IntItems).ListSubItems(1).Tag & "')"
        ...
        If lngProgID <> 0 Then
            ...
            gcnOracle.Execute _
                ...
                "," & lngProgID & _
                ",'" & lvwSet.ListItems(IntItems).ListSubItems(1).Tag & "' " & vbCr & _
                "From Dual " & vbCr & _
                " "
        End If
    Next
...
Private Function LoadAnother()
    ...
    With frmMain.cboSys
        ...
        Set mrsLoad = OpenSQLRecord(strSql, Me.Caption, .ItemData(.ListIndex), mlngGroupID)
    End With
    lvwFrom.ListItems.Clear
...
Private Function LoadHold()
    ...
    Set mrsLoad = OpenSQLRecord(strSql, Me.Caption, mlngGroupID)
    lvwSet.ListItems.Clear
    Do While Not mrsLoad.EOF
        intIcon = IIF(mrsLoad!程序ID = 0, 1, 2)
