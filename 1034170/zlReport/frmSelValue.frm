...
Private Sub cmdPlan_Click()
    ...
    frmSQLPlanEx.ShowMe Me, mobjData.数据连接编号, IIF(tbs.SelectedItem.Index = 1, mstrSQLList, mstrSQLTree), , "参数：" & mstrName
End Sub
...
'功能：检查选择器定义的合法性
    ...
    If mstrSQLList <> "" Then
        If CheckSQLPlan(mstrSQLList) = True Then
    ...
    End If
    If mstrSQLTree <> "" Then
        If CheckSQLPlan(mstrSQLTree) = True Then
