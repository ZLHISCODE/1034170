...
Private Sub cboPrinter_Click()
    ...
    If cboPrinter.ListIndex = -1 Then Exit Sub
...
'End Sub
'
'Private Sub mnuEditFormat_Click(Index As Integer)
'    Dim i As Integer
'    Dim blnFind As Boolean
'
'    If Index > 0 Then
...
'        With vsfReportFormat
'            For i = 0 To .Rows - 1
'                If Trim(mnuEditFormat(Index).Caption) = Trim(.TextMatrix(i, .ColIndex("格式"))) Then
'                    blnFind = True
'                    .Col = .ColIndex("格式")
'                    .Row = i
'                    Exit For
...
'                .Row = .Rows - 1
'                i = .Row
'                .TextMatrix(i, .ColIndex("格式")) = Trim(mnuEditFormat(Index).Caption)
'                Call PrinterInfo2CellData(i, rsInfo!编号, Trim(mnuEditFormat(Index).Caption))
'
'                '控件刷新
'                Call SetPrintInfo(.Cell(flexcpData, i, .ColIndex("格式")))
'            End If
...
Private Function GetPrintInfo(ByVal strInfo As String, Optional ByVal blnFromInterface As Boolean = False) As String
    ...
    If blnFromInterface Then
        strResult = strResult & "|" & arrItems(0) & "=" & Trim(cboBin.Text)
    ...
    For i = LBound(arrItems) To UBound(arrItems)
        ...
        Case "paperbin"
            For j = 0 To cboBin.ListCount - 1
                If cboBin.ItemData(cboBin.ListIndex) = Val(arrKey(1)) Then
                    cboBin.ListIndex = j
                    Exit For
                End If
            Next
