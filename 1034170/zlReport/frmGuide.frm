...
Private Sub cmdOK_Click()
                ...
                For Each tmpPar In tmpData.Pars
                    For i = 1 To lvwIf.ListItems.count
                        ...
                        Else
                            ...
                            End Select
                        End If
                        If tmpPar.名称 = lvwIf.ListItems(i).Text And _
                            (tmpPar.缺省值 <> lvwIf.ListItems(i).SubItems(3) Or _
                            tmpPar.类型 <> msgR) Then
                            MsgBox "在报表现有的数据源中发现有与条件[" & tmpPar.名称 & "]同名,但缺省值或类型不相同.请重新设置该条件！", vbInformation, App.Title: Exit Sub
                        End If
                    Next
                Next
            Next
    ...
    If txtTitle.Text <> "" Then
        ...
        '###格式
        '###名称、图片、参照、性质、自调属性的处理
        objGuide.Items.Add intMax, 1, "元素" & intMax, 0, 2, 0, "", 0, txtTitle.Text, "", (mobjFmt.W - picGuide(0).TextWidth(txtTitle.Text)) / 2, _
            ...
            picGuide(0).Font.Size, True, False, False, 0, 0, &HFFFFFF, False, 0, 0, 0, 0, False, , False, , , , "_" & intMax
    ...
    If optType(0).Value Then
        ...
        For i = 1 To lvwIf.ListItems.count
            ...
            End If
            '###格式属性
            objPars.Add "", i - 1, lvwIf.ListItems(i).Text, CByte(msgR), lvwIf.ListItems(i).SubItems(3), 1, "", "", "", "", "", "", "_" & i - 1
        Next
        ...
        intMax = intMax + 1
        '宽度
        i = IIF((UBound(Split(strFields, "|")) + 1) * 1000 + 300 > mobjFmt.W - 600, mobjFmt.W - 600, (UBound(Split(strFields, "|")) + 1) * 1000 + 300)
        '###格式
        '###名称、图片、参照、性质、自调属性的处理
        Set tmpItem = objGuide.Items.Add(intMax, 1, "元素" & intMax, 0, 4, 0, "", 0, strWhere, "", (mobjFmt.W - i) / 2, _
        picGuide(0).TextHeight(txtTitle.Text) * 3, i, mobjFmt.H - picGuide(0).TextHeight(txtTitle.Text) * 4, 255, 0, False, "宋体", 9, _
        False, False, False, 0, 0, &HFFFFFF, True, 1, "", "", "", False, , False, , , , "_" & intMax)
        ...
        For i = 1 To lvwItem.ListItems.count
            If lvwItem.ListItems(i).Checked Then
                ...
                '###格式
                '###名称、图片、参照、性质、自调属性的处理
                objGuide.Items.Add intMax, 1, "元素" & intMax, tmpItem.ID, 6, msgR, "", 0, "[" & strWhere & "." & lvwItem.ListItems(i).Text & "]", _
                "4^255^" & lvwItem.ListItems(i).Text, 0, 0, 1000, 0, 0, CByte(j), False, "", 0, False, False, False, 0, 0, 0, False, 0, "", "", "", False, , False, , , , "_" & intMax
                msgR = msgR + 1
    ...
    Else
        ...
        picGuide(0).TextHeight(txtTitle.Text) * 3, mobjFmt.W - 600, mobjFmt.H - picGuide(0).TextHeight(txtTitle.Text) * 4, 255, _
        0, False, "宋体", 9, False, False, False, 0, 0, &HFFFFFF, True, 1, "", "", "", False, , False, , , , "_" & intMax)
        ...
        For i = 1 To lvwVsc.ListItems.count
            With lvwVsc.ListItems(i)
                ...
                "", 0, 0, 1000, 0, 0, 0, False, "", 0, False, False, _
                False, 0, 0, 0, False, 0, .SubItems(1), "", strFields, False, , False, , , , "_" & intMax
        ...
        For i = 1 To lvwState.ListItems.count
            With lvwState.ListItems(i)
                ...
                objGuide.Items.Add intMax, 1, "元素" & intMax, tmpItem.ID, 9, i - 1, "", 0, .SubItems(1), _
                    "", 0, 0, 1000, 0, 0, CByte(msgR), False, "宋体", 9, False, False, _
                    False, 0, 0, 0, False, 0, "", IIF(InStr(lvwState.ListItems(i).SubItems(2), "合计平均") > 0, "0.00", ""), _
                    "", False, , False, , , , "_" & intMax
