...
Public Function ShowMe(frmParent As Object, ByVal lngReportID As Long, ByVal strData As String, ByRef objReport As Report, ByVal strCaption As String, _
...
Private Sub LoadPars()
    ...
    strSQL = "Select distinct a.名称" & vbNewLine & _
    ...
    With vsPars
        ...
        Do While Not rsTmp.EOF
            ...
            '已绑定的参数对照
            .TextMatrix(.Rows - 1, col参数值来源) = mobjRelations.Item("_" & rsTmp!名称).参数值来源
            rsTmp.MoveNext
        Loop
    End With
    Exit Sub
    ...
    With vsPars
        For i = 1 To .Rows - 1
            If .TextMatrix(i, col参数值来源) <> "" Then
                mobjRelations.Add mlngReportID, .TextMatrix(i, col参数名), .TextMatrix(i, col参数值来源), mstrCaption
            End If
        Next
    End With
    mblnOK = True
