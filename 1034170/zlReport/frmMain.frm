...
Private Sub lvwReport_DblClick()
    mcsActive = CS_报表
    If mblnItem Then mnuEdit_Modi_Click
End Sub
...
Private Sub mnuEdit_Group_Setup_Click()
    ...
    With frmSetGroup
        .mlngGroupID = Mid(lvwGroup.SelectedItem.Key, 2)
        .Caption = "设置报表组[" & lvwGroup.SelectedItem & "]的从属报表"
        .Show 1, Me
    End With
...
Private Function ReadReports(ByVal lngKey As Long) As Boolean
    ...
    If lngKey = -1 Then     '所有报表
        If mnuViewOnly.Checked Then '仅显示独立项
            strSql = _
                "Select Distinct a.ID,a.编号,a.名称,a.说明,a.程序ID,a.修改时间,a.发布时间,a.系统,Nvl(a.票据,0) 票据 " & vbCr & _
                "     ,zlSpellCode(名称) 简码,b.其他数据连接 " & vbCr & _
                ...
                IIF(cboSys.ItemData(cboSys.ListIndex) = 0, " and 系统 Is Null ", " and 系统=[1] ") & vbCr & _
                "    And Not Exists(Select 1 From zlRPTSubs Where 报表id = a.Id) " & vbCr & _
                "Order by 编号"
        Else
            strSql = _
                "Select Distinct a.ID,a.编号,a.名称,a.说明,a.程序ID,a.修改时间,a.发布时间,a.系统,Nvl(票据,0) 票据 " & vbCr & _
                "     ,zlSpellCode(名称) 简码,b.其他数据连接 " & vbCr & _
                "From zlReports A, " & vbCr & _
                "     (Select B1.报表id, f_list2str(Cast(Collect(B2.名称) As t_Strlist)) 其他数据连接 " & vbCr & _
                "      From zlRPTDatas B1, zlConnections B2 " & vbCr & _
...
'功能：检查当前列表中的报表执行计划是否存在性能问题
    ...
    If MsgBox("当前目录一共" & lvwReport.ListItems.count & "张报表，即将对这些报表及参数的数据源解析执行计划后进行性能检查。" & _
         "性能检查是指，报表数据源及参数数据源的SQL执行计划中存在以下情况：" & vbCrLf & _
            ...
            With objData
                ...
                For Each objPar In .Pars
                    ...
                    If objPar.明细SQL <> "" And InStr(strMsg, "(" & objPar.名称 & ")[明细]") = 0 Then
                        ...
                        strSql = Replace(strSql, "[系统]", cboSys.ItemData(cboSys.ListIndex))
                        Call CheckParsRela(strSql, objReport.Datas, objPar.名称, True)
                        strFields = CheckSQL(strSql, strErr, , strSQLCheck, , , objData.数据连接编号)
                        If strFields <> "" Then
                            If strSQLCheck <> "" Then
                                If CheckSQLPlan(strSQLCheck, , objData.数据连接编号) = True Then
...
makContinue:
        ...
        ShowFlash "正在检查报表数据源是否存在性能问题,请稍候 ...", i / lvwReport.ListItems.count
    Next
    ShowFlash
    If lngCount > 0 Then
        MsgBox "一共对" & lvwReport.ListItems.count & "张报表进行了性能检查，其中" & lngCount & "张报表的数据源及参数数据源可能存在性能问题，详见""性能问题数据源""列的信息。" & vbCrLf & vbCrLf & _
            "请在报表设计界面查看详细的执行计划，然后进行SQL性能优化。", vbInformation, "性能检查结果"
    End If
End Sub
