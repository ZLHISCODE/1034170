...
Begin VB.Form frmColSetup 
   ...
   Begin VB.ComboBox cboRelation 
      Height          =   300
      ItemData        =   "frmColSetup.frx":0D02
      Left            =   2760
      List            =   "frmColSetup.frx":0D0F
...
Public Function ShowMe(objParent As Object, objColProtertys As RPTColProtertys, ByVal lngType As Long, ByVal strData As String, _
    ...
    With lvwIf.ListItems
        If .count = 1 Then
            ...
            If Val(.Item(i).SubItems(col_字体颜色)) <> vbBlack Or Val(.Item(i).SubItems(col_背景颜色)) <> vbWhite _
                Or Val(.Item(i).SubItems(col_是否加粗)) = 1 Then
                mobjColProtertys.Add .Item(i).Text, Nvl(.Item(i).SubItems(col_条件字段), ""), Nvl(.Item(i).SubItems(col_条件关系), "") _
                    , Nvl(.Item(i).SubItems(col_条件值), ""), Val(.Item(i).SubItems(col_字体颜色)), Val(.Item(i).SubItems(col_背景颜色)) _
                    , Val(.Item(i).SubItems(col_是否加粗)) = 1, Val(.Item(i).SubItems(col_是否整行应用)) = 1, "_" & .Item(i).Text
            End If
        Else
            For i = 1 To .count
                mobjColProtertys.Add .Item(i).Text, Nvl(.Item(i).SubItems(col_条件字段), ""), Nvl(.Item(i).SubItems(col_条件关系), "") _
...
Private Sub Form_Load()
    ...
    cboRelation.Clear
    cboRelation.AddItem ""
    cboRelation.AddItem "等于"
    cboRelation.AddItem "大于"
    cboRelation.AddItem "小于"
    cboRelation.AddItem "大于等于"
    cboRelation.AddItem "小于等于"
    cboRelation.AddItem "左匹配"
    cboRelation.AddItem "双向匹配"
