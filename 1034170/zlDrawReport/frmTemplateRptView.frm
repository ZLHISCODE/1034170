    ...
    If grsData.RecordCount > 0 Then
        ...
        If grsPage.RecordCount > 0 Then
            ...
            Do While Not grsData.EOF
                strTmp = zlCommFun.NVL(grsData("内容").value)
                varTmp = Split(strTmp, "^")
        ...
        If grsData.RecordCount > 0 Then
            ...
            Do While Not grsData.EOF
                ...
                If grsData("类别") = "附件" Then
                    ...
                    varTmp = Split(zlCommFun.NVL(grsData("内容").value), "^")
                    If UBound(varTmp) > 0 Then
...
Private Function OutputObject(ByRef objDraw As Object, ByVal rs As ADODB.Recordset, Optional ByVal intPage As Integer, Optional ByVal intPageTotal As Integer) As Boolean
    ...
    strTmp = zlCommFun.NVL(rs("内容").value)
    
    '调试
    If zlCommFun.NVL(rs("调试").value, 0) = 1 Then
        strTmp = zlCommFun.NVL(rs("内容").value)
    End If
...
'        End If
    ...
    Case "图形"
        ...
        Call DrawPicture(objDraw, rs("内容").value, Val(rs("X0").value), Val(rs("Y0").value), Val(rs("X1").value), Val(rs("Y1").value), Val(rs("旋转角度").value))
...
Public Function DrawPicture(objDraw As Object, ByVal strFile As String, ByVal X1 As Long, ByVal Y1 As Long, ByVal X2 As Long, ByVal Y2 As Long, Optional ByVal bytAngle As Byte) As Boolean
    '******************************************************************************************************************
    ...
    If bytAngle = 4 Then
        strFile = RotateImage(strFile, X1, Y1, X2, Y2)
        Call DrawPictureEx(objDraw, strFile, X1, Y1, X2, Y2)
    Else
        Call DrawPictureEx(objDraw, strFile, X1, Y1, X2, Y2)
    End If
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_File_Print
        ...
        If PrintReport Then RaiseEvent AfterPrinted
