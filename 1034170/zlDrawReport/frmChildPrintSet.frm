...
'        Call zlControl.CboLocate(cboPrinter, mstrPrinter, False)
        On Error GoTo errHand
        
        If cboPrinter.ListCount > 0 And cboPrinter.ListIndex = -1 Then
            cboPrinter.Text = Printer.DeviceName
'             Call zlControl.CboLocate(cboPrinter, Printer.DeviceName)
        End If
...
Private Sub ReadPaperBinData(ByVal intDefaultPaperBin As Integer)
    Dim strPaperBin As String * 100
    Dim intTemp As Integer
    Dim strPaperBins As String
    ...
    For lngLoop = 1 To lngCount
        intTemp = Asc(Mid(strPaperBin, lngLoop * 2, 1)) * 256# + Asc(Mid(strPaperBin, lngLoop * 2 - 1, 1))
        If intTemp >= 1 And intTemp <= 11 Then '只列出标准支持的进纸大小
            If intTemp = intDefaultPaperBin Then
                strPaperBins = strPaperBins & "," & intTemp & "*" '原有的
            Else
                strPaperBins = strPaperBins & "," & intTemp
            End If
        End If
    Next
...
Private Function GetPaperBin(ByVal intBin As Integer) As String
    '------------------------------------------------
    ...
    If intBin >= 1 And intBin <= 11 Then
        GetPaperBin = Switch(intBin = 1, "上层纸盒进纸", _
                            intBin = 2, "下层纸盒进纸", _
                            intBin = 3, "中间纸盒进纸", _
                            intBin = 4, "等待手动插入每页纸", _
                            intBin = 5, "信封进纸器进纸", _
                            intBin = 6, "信封进纸器进纸；但要等待手动插入", _
                            intBin = 7, "当前缺省纸盒进纸", _
                            intBin = 8, "拖拉进纸器进纸", _
                            intBin = 9, "小型进纸器进纸", _
                            intBin = 10, "大型纸盒进纸", _
