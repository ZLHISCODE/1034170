...
Private Sub CMD确认_Click()
    ...
    If Not OraDataOpen(strServerName, strUserName, _
        IIf(UCase(strUserName) = "SYS" Or UCase(strUserName) = "SYSTEM", strPassword, IIf(mbln转换, TranPasswd(strPassword), strPassword)), optDriver(1).Value) Then
        TXT密码.Text = ""
        If TXT密码.Enabled Then TXT密码.SetFocus
        SetConState
        Exit Sub
...
Private Sub Form_Activate()
    If mblnFirst = False Then
        Call SetWindowLong(Me.hwnd, GWL_EXSTYLE, GetWindowLong(Me.hwnd, GWL_EXSTYLE) Or WinStyle)
