...
Private Function LoadPatiBaseInfo() As Boolean
    ...
    If mlng就诊ID <> 0 Then
        If mint场合 = 1 Then '门诊病人
            strSQL = "Select  Nvl(a.姓名, b.姓名) 姓名, Nvl(a.性别, b.性别) 性别,nvl(a.年龄,b.年龄) 年龄,b.出生日期,B.病人类型,B.险类,A.NO " & vbNewLine & _
    ...
    If Not rsTmp.EOF Then
        ...
        If cboSex.ListIndex = -1 And Not IsNull(rsTmp!性别) Then
            ...
            cboSex.ListIndex = cboSex.NewIndex
        End If
        If mlng就诊ID <> 0 And mint场合 = 1 Then
            mstr挂号NO = rsTmp!NO & ""
        End If
...
Private Function isCheckExists(ByVal strNo As String, Optional blnLocateItem As Boolean = False) As Boolean
    ...
    For i = 0 To cmbNum.ListCount - 1
        If IIf(optType(0).Value = True, NeedName(cmbNum.List(i)), Val(cmbNum.List(i))) = strNo Then
...
'功能：完成数据校验和保存
    ...
    If Not CheckTextLength("姓名", txtPatient) Then Exit Sub
    ...
    If Trim(txtPatient.Text) <> Trim(mstrName) Then
        If MsgBox("你是否将病人名字【" & mstrName & "】调整为【" & Trim(txtPatient.Text) & "】,是否真的调整吗？", vbYesNo, gstrSysName) = vbYes Then
            If SaveBaseInfo(mlng病人ID, mlng就诊ID, Trim(txtPatient.Text), str性别, str年龄, str出生日期, mstr模块, mint场合, mstrInfo, True, blnEMPI) = False Then
                If mstrInfo <> "" Then
                    MsgBox mstrInfo, vbInformation, gstrSysName
                End If
                Exit Sub
            End If
        Else
...
Private Sub Form_Load()
    ...
    If mblnBatch = True Then
        ...
        Call CreateSquareCardObject(Me, 1101)
         '初始化
        Call IDKind.zlInit(Me, 100, 1101, gcnOracle, gstrDBUser, gobjSquare.objSquareCard, "")
