...
Public Function ShowMe(ByRef objfrmMain As Object, ByVal rsRet As ADODB.Recordset, Optional ByRef strAdvice As String) As Boolean
'功能:
    Set mrsAdvice = rsRet
...
Private Sub lblBtn_Click()
    ...
    With vsReason
        For i = 1 To .Rows - 1
            If Trim(.TextMatrix(i, COL_用药理由)) <> "" Then
                ...
                If Not mrsAdvice.EOF Then
                    mstrAdvice = mstrAdvice & "," & .Cell(flexcpData, i, COL_医嘱内容)
                    strRefuse = strRefuse & "," & "{""dr_refuse_comment"":""" & Trim(.TextMatrix(i, COL_用药理由)) & """}"
                End If
            End If
        Next
        ...
        End If
        If mstrAdvice <> "" Then mstrAdvice = Mid(mstrAdvice, 2)
    End With
    Unload Me
End Sub
