...
Public Function YWS_ReturnRS(ByVal strXML As String) As ADODB.Recordset
...
'</ui_results_xml>
    ...
    If xmlDoc.documentElement Is Nothing Then
        Set xmlDoc = Nothing
        Set YWS_ReturnRS = rsRet
        Exit Function
    End If
    
    '读取XML内容
    Set xmlRoot = xmlDoc.selectSingleNode("ui_results_xml/result_data")
    Set xmlNodes = xmlRoot.selectNodes("result")

    If Not xmlNodes Is Nothing Then
        For Each xmlNode In xmlNodes
            str警示值 = xmlNode.selectSingleNode("result_type").Text
            If Val(str警示值) > 0 Then
                str医嘱串 = xmlNode.selectSingleNode("oeridid").Text   '30064、9911
                arrTmp = Split(str医嘱串, "、")
                For i = LBound(arrTmp) To UBound(arrTmp)
                    str医嘱ID = Val(arrTmp(i))
                    If Val(arrTmp(i)) <> 0 Then
                        rsRet.Filter = "医嘱ID ='" & str医嘱ID & "'"
                        If Not rsRet.EOF Then
                            If Val(rsRet!警示值 & "") < Val(str警示值) Then
                                rsRet!警示值 = Val(str警示值)
                            End If
                        Else
                            rsRet.AddNew
                            rsRet!警示值 = Val(str警示值)
                            rsRet!医嘱ID = Val(str医嘱ID)
                            rsRet.Update
                        End If
                    End If
                Next
    ...
    Exit Function
errH:
    MsgBox "YWS_ReturnRS 错误号:" & Err.Number & "错误描述:" & Err.Description, vbOKOnly, gstrSysName
End Function
