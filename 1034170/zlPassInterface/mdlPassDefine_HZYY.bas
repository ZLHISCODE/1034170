...
Option Explicit
'1-application/xml;charset=utf-8;0-application/x-www-form-urlencoded;charset=utf-8
Public gbytContentType  As Byte
Public gstrUrlCheck     As String      '审查地址 http://192.168.0.1:8080/v4/engineAsync"(由合理用药厂商提供)
Public gstrUrlUpLoad    As String     '上传有效处方 http://192.168.0.1:8080/v4/valid"(由合理用药厂商提供)
Public gstrUrlDel       As String        '删除医嘱 http://192.168.0.1:8080/v4/invalid"(由合理用药厂商提供)
Public gstrUrlDrug      As String       '药品说明书 http://192.168.0.1:8080/v4/invalid"(由合理用药厂商提供)
Public gstrEduURL       As String           '杭州患者教育URL
Private mstrTag         As String
Private mlngTime        As String

Private Function Get药品信息_HZYY(ByVal strDrugIDs As String) As ADODB.Recordset
    Dim strSQL As String
    Dim rsTemp As New ADODB.Recordset
    On Error GoTo errH
    strSQL = "Select a.药品id, a.门诊单位, a.门诊包装, a.住院单位, a.住院包装, d.药品剂型, e.规格,e.计算单位 , f.现价,b.名称 As 厂家名称,b.编码 as 厂家编码 " & vbNewLine & _
            "From 药品规格 A, 药品特性 D, 收费项目目录 E, 收费价目 F, 药品生产商 B" & vbNewLine & _
            "Where a.药名id = d.药名id And a.药品id = e.Id And a.药品id = f.收费细目id(+) And a.上次产地 = b.名称(+) And" & vbNewLine & _
...
'bytFunc  =0-处方审查;1-删除处方
    ...
    strXML = strXML & _
            ...
            strTab & "<herb_unit_price><![CDATA[" & IIf(.strHerbUnitPrice <> "", .strHerbUnitPrice, 0) & "]]></herb_unit_price>" & _
...
Private Function HZYY_GetOPTPresItem(ByRef udtItem As OptPRESCRIPTIONSITEM, ByVal strTab As String) As String
    ...
    With udtItem
        ...
        strXML = strXML & _
                ...
                strTab & "<unit_price><![CDATA[" & IIf(.strUnitPrice <> "", .strUnitPrice, 0) & "]]></unit_price>" & _
                ...
                strTab & "<drug_return_flag><![CDATA[" & IIf(.strdrugReturnFlag <> "", .strdrugReturnFlag, 0) & "]]></drug_return_flag>" & _
                ...
                strTab & "<stop_flag><![CDATA[0]]></stop_flag>" & _
...
'参数:lngDrugID-药品ID
    ...
    If gbytType = 0 Then
        strUrl = gstrUrlDrug & "?webHisId=" & lngDrugID
    Else
        '产品非共用
        strUrl = gstrUrlDrug & "?webHisId=" & lngDrugID & "&hospitalCode=" & gstrHOSCODE
    End If
...
20
        '审查服务,删除服务,医院编码,说明书,应用模式,患者教育,CONTENTTYPE,上传服务
30      gstrUrlCheck = GetThirdPara(G_STR_PARA_HZYY, "审查服务")
40      gstrUrlDel = GetThirdPara(G_STR_PARA_HZYY, "删除服务")
50      gstrHOSCODE = GetThirdPara(G_STR_PARA_HZYY, "医院编码")    '医院编码
60      gstrUrlDrug = GetThirdPara(G_STR_PARA_HZYY, "说明书")
70      gbytType = Val(GetThirdPara(G_STR_PARA_HZYY, "应用模式") & "")
80      gstrEduURL = GetThirdPara(G_STR_PARA_HZYY, "患者教育")
90      gbytContentType = Val(GetThirdPara(G_STR_PARA_HZYY, "CONTENTTYPE") & "")
100     gstrUrlUpLoad = GetThirdPara(G_STR_PARA_HZYY, "上传服务")
        
110     gstrUrlCheck = IIf(gstrUrlCheck = "", "http://118.31.246.211:8080/v4/engineAsync", gstrUrlCheck)
120     gstrUrlDel = IIf(gstrUrlDel = "", "http://118.31.246.211:8080/v4/invalid", gstrUrlDel)
130     gstrHOSCODE = IIf(gstrHOSCODE = "", "cqzl123", gstrHOSCODE)
140     gstrUrlDrug = IIf(gstrUrlDrug = "", "http://118.31.246.211:6000/zlcx/data_detail.action", gstrUrlDrug)
150     gstrEduURL = IIf(gstrEduURL = "", "http://118.31.246.211:8080/v4/engineIncrementAsync", gstrEduURL)
160     gstrUrlUpLoad = IIf(gstrUrlUpLoad = "", "http://118.31.246.211:8080/v4/valid", gstrUrlUpLoad)
        
170     Exit Function
errH:
180     MsgBox "读取参数失败！" & vbNewLine & "HZYY_GetPara:第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function

Public Sub HZYY_SetPara()
    Call UpdateThirdPara(G_STR_PARA_HZYY, 1, "审查服务", gstrUrlCheck, "审查服务地址")
    Call UpdateThirdPara(G_STR_PARA_HZYY, 2, "删除服务", gstrUrlDel, "删除服务地址")
    Call UpdateThirdPara(G_STR_PARA_HZYY, 3, "医院编码", gstrHOSCODE, "医院唯一标识")
    Call UpdateThirdPara(G_STR_PARA_HZYY, 4, "说明书", gstrUrlDrug, "药品说明书服务地址")
    Call UpdateThirdPara(G_STR_PARA_HZYY, 5, "应用模式", gbytType & "", "0-共用;1-非共用")
    Call UpdateThirdPara(G_STR_PARA_HZYY, 6, "患者教育", gstrEduURL, "患者教育服务地址")
    Call UpdateThirdPara(G_STR_PARA_HZYY, 7, "CONTENTTYPE", gbytContentType, "1-application/xml;charset=utf-8;0-application/x-www-form-urlencoded;charset=utf-8")
    Call UpdateThirdPara(G_STR_PARA_HZYY, 8, "上传服务", gstrUrlUpLoad, "上传有效处方\医嘱服务地址")
End Sub
...
'   True -允许保存
    ...
    On Error GoTo errH
    '合川二院 同一病人同一处方频繁传入导致系统崩溃
    If Timer - mlngTime < 3 Then '3秒内,同一病人,同一操作,弹出提示禁止
        If mstrTag = lngPatiID & "_" & bytFunc Then
            Call MsgBox("当前操作过于频繁，请稍等片刻再处理。", vbExclamation, gstrSysName)
            Exit Function
        End If
    End If
    mlngTime = Timer  '记录本次响应时间
    mstrTag = lngPatiID & "_" & bytFunc
    ...
    Case 0, 1, 4
                ...
                For i = 1 To rsAdvice.RecordCount
                    If Val(strRecipeNo) <> Val(rsAdvice!处方ID & "") Then
                        ...
                        With udtPresInfo
                            ...
                            If rsAdvice!诊疗类别 = "7" Then
                                .strHerbPacketCount = FormatEx(rsAdvice!总量 & "", 5)           '饮片帖数 '如果是中药饮片处方，这部分信息需要填写，西药方可为空
                                .strHerbUnitPrice = GetHerbUnitPrice(rsAdvice, rs规格)          '饮片单帖价格(每一付单价)
                            Else
                    ...
                    With udtPresItem
                        ...
                        If Not rs规格.EOF Then
                            ...
                            If rsAdvice!诊疗类别 = "7" Then
                                .strFeeTotal = FormatEx(rs规格!现价 & "", 5)         '药品的单价*发药数量，计量单位为元
                            Else
                                .strFeeTotal = FormatEx(Val(rsAdvice!总量 & "") * Val(rs规格!现价 & ""), 5)      '药品的单价*发药数量，计量单位为元
                            End If
                            .strManufacturerID = rs规格!厂家编码 & ""                    '生产厂家id
                            .strManufacturerName = rs规格!厂家名称 & ""                 '生产厂家名称
                        Else
                ...
                For i = 1 To rsAdvice.RecordCount
                    ...
                    With udtOrderItem
                        ...
                        If Not rs规格.EOF Then
                            ...
                            .strFeeTotal = FormatEx(Val(rsAdvice!总量 & "") * Val(rs规格!现价 & ""), 5)                 '总价
                            .strManufacturerID = rs规格!厂家编码 & ""                    '生产厂家id
                            .strManufacturerName = rs规格!厂家名称 & ""                 '生产厂家名称
                        Else
                            .strPreparation = ""                   '药品剂型名称
                        ...
                        .strdrugReturnFlag = "0"                 '是否退药标志
                        .strStopFlag = "0"                        '是否停药标志
                ...
                For i = 1 To rsAdvice.RecordCount
                    ...
                    If rsAdvice!相关ID <> strRecipeNo Then
                        ...
                        With udtHerbInfo
                            ...
                            .strOrderDocTitle = rsAdvice!专业技术职务 & ""                       '医嘱医生职称
                            .strOrderType = rsAdvice!医嘱期效 & ""                          '医嘱类型
                            .strHerbUnitPrice = GetHerbUnitPrice(rsAdvice, rs规格)   '饮片单帖价格
                            .strHerbPacketCount = FormatEx(rsAdvice!总量 & "", 5)                   '饮片帖数
                            .strIsCream = ""                            '膏方
                            .strCheckTime = ""                          '复核时间
                            .strCheckNurseID = ""                       '复核护士id
                            .strCheckNurseName = ""                     '复核护士姓名
                    ...
                    With udtHerbItem
                        ...
                        If Not rs规格.EOF Then
                            ...
                            .strManufacturerID = rs规格!厂家编码 & ""                    '生产厂家id
                            .strManufacturerName = rs规格!厂家名称 & ""                 '生产厂家名称
                            .strFeeTotal = FormatEx(rs规格!现价 & "", 5)
                        Else
                            .strPreparation = ""                    '药品剂型名称
                            .strSpecifications = ""                 '药品规格描述
                            .strUnitPrice = ""                      '单价
                            ...
                            .strFeeTotal = ""
                        End If
                        .strDespensingNum = "1"                       '发药数量 草药默认为1
                                                                                      '总价
                        .strSpecialPrompt = ""                                                           '特殊要求
                    End With
        ...
        Else
            ...
            If str挂号单 = "" Then
                Set rsTmp = zlDatabase.CopyNewRec(rsTmp, , "ID,编码,名称,记录日期")
            ...
            For i = 1 To rsTmp.RecordCount
                With udtDiag
                    ...
                    .strDiagDeptID = lngDeptID
                    .strDiagDeptName = strDeptName
                    .strDiagDate = Format(Nvl(rsTmp!记录日期), "YYYY-MM-DD HH:MM:SS")
...
'            strURL = "http://" & gstrIP & ":" & gstrPortPlus & "/v4/valid"
        ...
        '142088 传入检验信息标签
        strTmp = GetExams(lngPatiID, lng主页ID, lng挂号ID)
        If strTmp <> "" Then
            strXML = Replace(strXML, "</root>", strTmp & "</root>")
            strTmp = ""
        End If
        If gbytContentType = 1 Then
            strUrl = strUrl & "?charset=utf-8&post_type=1"
        Else
            '142015XML中含有特殊字符如"%"等合理用药服务器接收不到XML参数,返回解析XML失败 因此先转码URLEncode再发送
            strXML = "charset=utf-8&post_type=1&xml=" & URLEncode(strXML)
        End If
        '医生开药
        WriteLog "传入值:" & vbCrLf & "URL:" & strUrl & vbCrLf & "XML:" & strXML
        strRet = HttpPost(strUrl, strXML, responseText, IIf(gbytContentType = 1, "application/xml;charset=utf-8", "application/x-www-form-urlencoded;charset=utf-8"))
        strRet = Replace(strRet, "<![CDATA[]]>", "")
    ...
    Case 2, 3   '删除处方及医嘱
        ...
        WriteLog "" & glngModel & "HttpPost" & "URL:" & strUrl & vbCrLf _
                                             & "传入值:" & strXML
        strRet = HttpPost(strUrl, strXML, responseText, IIf(gbytContentType = 1, "application/xml;charset=utf-8", "application/x-www-form-urlencoded;charset=utf-8"))
        strRet = Replace(strRet, "<![CDATA[]]>", "")
        WriteLog "" & glngModel & "HttpPost" & "返回值:" & strRet
...
End Function
Private Function GetExams(ByVal lngPatiID As Long, ByVal lngVisitID As Long, ByVal lngRegID As Long) As String
'功能:功能获取检验标签
'lngPatiID-病人ID
'lngVisitID-主页ID
'lngRegID-挂号ID
    ...
    If CreatePlugInOK Then
        On Error Resume Next
        Call gobjPlugIn.SetPassExams(lngPatiID, lngVisitID, lngRegID, strInfo)
        Call zlPlugInErrH(Err, "SetPassExams")
        Err.Clear: On Error GoTo 0
    End If
    GetExams = strInfo
End Function
...
Private Function GetHerbUnitPrice(ByRef rsAdvice As ADODB.Recordset, ByRef rs规格 As ADODB.Recordset) As Double
'功能:计算中药饮片单付金额
    ...
    lngPosition = rsAdvice.AbsolutePosition     '记录记录集位置
    lngGroupId = Nvl(rsAdvice!相关ID, 0)
    Do While Not rsAdvice.EOF
        If lngGroupId <> Nvl(rsAdvice!相关ID, rsAdvice!医嘱id) Then Exit Do
        rs规格.Filter = "药品ID=" & rsAdvice!药品ID
        If Not rs规格.EOF Then
            dblTotalCost = dblTotalCost + (Val(rs规格!现价 & "") * Val(rsAdvice!单次用量 & ""))
        End If
        rsAdvice.MoveNext
    Loop
    rsAdvice.AbsolutePosition = lngPosition  '定位到记录位置
    GetHerbUnitPrice = FormatEx(dblTotalCost, 5)
End Function
