...
Begin VB.UserControl HRuler 
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   378
   ToolboxBitmap   =   "HRuler.ctx":0000
End
Attribute VB_Name = "HRuler"
...
'##修 改 人：吴庆伟
'##日    期：2005年3月2日
'##描    述：一个类 Word 2000 样式的水平标尺控件。RulerScale 表示不同的刻度。
'##版    本：
'######################################################################################
...
Public Property Set Font(sFont As StdFont)
   ...
   m_lFontHeight = CLng(UserControl.TextHeight("6"))
...
Static lngMoveStart    As Long
   ...
   If m_lLeftMargin > 0 Then
      lBrush = CreateSolidBrush(&H99A8AC) 'Dark Grey Brush
      lBrushOld = SelectObject(mHdc, lBrush)
      lPen = CreatePen(0, 1, &H99A8AC)
      lpenOld = SelectObject(mHdc, lPen)
   ...
   For sglPos = m_lLeftMargin + m_lRulerStep To m_lRulerLength Step IIf(m_lRulerStep < 1, 1, m_lRulerStep)
...
Dim idRes As Long
   ...
   If (m_bInDev) Then
      ...
      m_cIL.AddFromHandle stdPic.Handle, ImageTypes.IMAGE_BITMAP, , &HFF00FF
      Set stdPic = Nothing
   Else
      m_cIL.AddFromResourceID idRes, App.hInstance, ImageTypes.IMAGE_BITMAP, , False, &HFF00FF
   End If
...
'For Example every 0.125 Inch or 6 Pixels
   If PtInRect(m_rHangingIndent, CLng(X), CLng(Y)) Then
      UserControl.Extender.ToolTipText = "悬挂缩进"
   ElseIf PtInRect(m_rLeftIndent, CLng(X), CLng(Y)) Then
      UserControl.Extender.ToolTipText = "左缩进"
   ElseIf PtInRect(m_rRightIndent, CLng(X), CLng(Y)) Then
      UserControl.Extender.ToolTipText = "右缩进"
   ElseIf PtInRect(m_rFirstLineIndent, CLng(X), CLng(Y)) Then
      UserControl.Extender.ToolTipText = "首行缩进"
   ElseIf X >= m_lLeftMargin - 4 And X <= m_lLeftMargin + 4 Then
      UserControl.Extender.ToolTipText = "左边距"
   ElseIf X >= mRect.Right - m_lRightMargin - 4 And X <= mRect.Right - m_lRightMargin + 4 Then
      UserControl.Extender.ToolTipText = "右边距"
   Else
      ...
      Case 0
         ...
         If X >= m_lLeftMargin - 4 And X <= m_lLeftMargin + 4 Then
            ...
            ElseIf PtInRect(m_rLeftIndent, CLng(X), CLng(Y)) Then
               Screen.MousePointer = 0
            Else
               Screen.MousePointer = 0      'vbSizeWE
            End If
         ElseIf X >= mRect.Right - m_lRightMargin - 4 And X <= mRect.Right - m_lRightMargin + 4 Then
