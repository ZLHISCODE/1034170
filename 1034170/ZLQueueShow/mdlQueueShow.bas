...
Public glngNameSec As Long              '用户隐私保护
Public gstrSysName As String
Public gstrSystems As String
Public gstrStation As String
Public gblnUseSound As Boolean          '是否启用本地语音呼叫
Public glngLoopInterval As Long
...
Public Sub Main()
    ...
    Set gcnOracle = objLogin.OpenLoginWindow(G_STR_REGPATH)
    
    If gcnOracle.ConnectionString = "" Then Exit Sub
    
    gstrSysName = objLogin.getSysName
    gstrUserName = objLogin.getUserName

    '初始化zlcomlib对象
    gobjComLib.InitCommon gcnOracle
    
    gstrDbUser = UCase(gstrUserName)
...
'不论已那种方式显示后，都需要显示托盘图标
    ...
    Dim blnAutoLogin As Boolean     '自动登录
On Error GoTo ErrorHand
    ...
    gblnUseSound = Val(GetSetting("ZLSOFT", G_STR_REGPATH, "启用语音呼叫", 1)) = 1
    glngLoopInterval = Val(GetSetting("ZLSOFT", G_STR_REGPATH, "轮询间隔时间", 30))
...
On Error GoTo errHandle
    ...
    strSql = "ZL_排队叫号队列_清除业务(" & glngBusinessType & ",65535)"
    Call gobjComLib.zlDatabase.ExecuteProcedure(strSql, "清除排队数据")
Exit Sub
errHandle:
    MsgBox Err.Description, vbExclamation, gstrSysName
End Sub
...
Public Function GetSecName(ByVal strName As String) As String
    If glngNameSec = 1 Then
        GetSecName = Mid(strName, 1, 1) & Replace(strName, Mid(strName, 2, 1), "*", 2, 1)
    Else
        GetSecName = strName
    End If
End Function
...
'blnOpenOldLcd,是否用老版的LCD模式显示
    ...
    glngNameSec = Val(GetSetting("ZLSOFT", G_STR_REGPATH, "姓名隐私保护", 1))
