...
'开始语音播放
    mdtLastVoiceDate = Timer - glngLoopInterval
    
    tmrBroadCast.Interval = glngLoopInterval * 1000
    tmrBroadCast.Enabled = True
    tmrBroadCast.Tag = 0
End Sub
...
Private Sub LoopPlayVoice()
    ...
    If blnAllowQuery Then
        If Timer < mdtLastVoiceDate + glngLoopInterval Then Exit Sub
    ...
    '刷新界面数据信息
    Call SyncRefreshWindow(lngQueueId)
...
Private Sub SyncRefreshWindow(Optional ByVal lngQueueId As Long = 0)
On Error GoTo errHandle
    Dim i As Long
