...
Public Sub FormMouseMove(ByVal strKEY As String, ByVal strFormName As String)

    If Not gcolclsMain Is Nothing Then
        On Error Resume Next
        If Not gcolclsMain(strKEY) Is Nothing Then
            Set gclsMain = gcolclsMain(strKEY)
        End If
        If Err.Number <> 0 Then On Error GoTo 0
    End If
    
    If Not gcolclsPros Is Nothing Then
        On Error Resume Next
        If Not gcolclsPros(strFormName) Is Nothing Then
            Set gclsPros = gcolclsPros(strFormName)
        End If
        If Err.Number <> 0 Then On Error GoTo 0
    End If
End Sub
    ...
    Else
        ...
        If intIndex = SLC_年龄 And gclsPros.FuncType = f病案首页 Or intIndex = SLC_婴幼儿年龄 Then
          If cboTmp.ListCount + vbKey1 >= intKeyAscii And intKeyAscii >= vbKey1 Then
                If intKeyAscii - vbKey1 <= cboTmp.ListCount - 1 Then
...
'                    .txtInfo(GC_其他关系).Visible = False
            ...
            Case BCC_证件类别
                strTmp = zlStr.NeedName(.cboBaseInfo(intIndex).Text)
                If strTmp = "" Then
                    Call SetCtrlLocked(.txtInfo(GC_其他证件), True, True)
                Else
                    Call SetCtrlLocked(.txtInfo(GC_其他证件), False, True)
                End If
        End Select
        If gclsPros.MedPageSandard <> ST_门诊首页 Then
...
'vsFees_KeyDown事件
    ...
    With vsFree
        ...
        lngCol = .Col
        If intKeyCode = vbKeyDelete And .Editable <> flexEDNone Then
            If Not FreeHaveLowLevel(lngRow, IIf(lngCol Mod 2 = 0, lngCol, lngCol - 1)) Then
                If MsgBox("是否删除该费用？", vbQuestion + vbYesNo, gstrSysName) = vbYes Then
                    Call AddOrDelFreeCols(vsFree, .TextMatrix(lngRow, IIf(lngCol Mod 2 = 0, lngCol, lngCol - 1)), .TextMatrix(lngRow, IIf(lngCol Mod 2 = 0, lngCol + 1, lngCol)), False)
                End If
            End If

        ElseIf intKeyCode = vbKeyReturn Then
            intKeyCode = 0
            If .TextMatrix(lngRow, IIf(lngCol Mod 2 = 0, lngCol, lngCol - 1)) = "" Or .Editable = flexEDNone Then
                ...
                End If
            Else
                If IIf(lngCol Mod 2 = 0, lngCol, lngCol - 1) = 4 Then
                    .Col = 0
                    ...
                    End If
                Else
                    .Col = IIf(lngCol Mod 2 = 0, lngCol, lngCol - 1) + 2
                End If
            End If
        End If
    End With
                ...
                Case DI_诊断编码, DI_诊断描述, DI_中医证候, DI_ICD附码 '西医中医证候隐藏,中医无ICD附码隐藏
                    ...
                    Else
                        ...
                        If vsDiag.Name = "vsDiagXY" Then
                            If .TextMatrix(intRow, DI_诊断分类) = DT_出院诊断XY And intCol = DI_诊断编码 Then
                                mstrMsgXY = .TextMatrix(intRow, DI_诊断编码) & ";" & .TextMatrix(intRow, DI_诊断描述) & ";" & .TextMatrix(intRow, DI_备注)
                                Clipboard.SetText mstrMsgXY
                            End If
                        ElseIf vsDiag.Name = "vsDiagZY" Then
                            If .TextMatrix(intRow, DI_诊断分类) = DT_出院诊断ZY And intCol = DI_诊断编码 Then
                                mstrMsgZY = .TextMatrix(intRow, DI_诊断编码) & ";" & .TextMatrix(intRow, DI_诊断描述) & ";" & .TextMatrix(intRow, DI_中医证候) & ";" & .TextMatrix(intRow, DI_备注)
            ...
            Case PI_手术编码, PI_手术名称
                ...
                If int手术输入方式 = 0 And gclsPros.FuncType <> f病案首页 Then
                    ...
                    strSql = "Select 0 as 末级,ID,上级ID,编码,名称,NULL as 规模" & _
                        ...
                        "Union ALL " & _
                        "Select 1 As 末级, a.Id, a.分类id As 上级id, a.编码, a.名称, a.操作类型 As 规模" & vbNewLine & _
                        "From 诊疗项目目录 A, 疾病诊断对照 B" & vbNewLine & _
                        "Where a.Id = b.手术id(+) And a.类别 = 'E' And a.服务对象 In (2, 3) And b.手术id Is Not Null And (a.站点 = '" & gstrNodeNo & "' Or a.站点 Is Null)" & _
                        IIf(int性别 <> 0, " And Nvl(a.适用性别,0) IN(0,[2])", "") & _
...
'                            Call SetDiagMatchInfo(IIf(bln西医, BCC_入院与出院XY, BCC_入院与出院ZY))
                            If bln西医 And strDiagTypeName = "出院诊断" Then
                                If Val(.TextMatrix(i, DI_诊断分类)) = DT_出院诊断XY Then
                                    ...
                                    Else
                                        bln分化程度 = Trim(.TextMatrix(i, DI_诊断描述)) <> ""
                                    End If
...
Public Sub ChangeOutInfoSub(Optional ByVal str出院情况 As String)
    ...
    Call SetCtrlLocked(gclsPros.CurrentForm.cboBaseInfo(BCC_出院方式), str出院情况 = "死亡", False)
    If str出院情况 = "死亡" Then
