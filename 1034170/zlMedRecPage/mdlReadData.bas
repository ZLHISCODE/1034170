...
'返回：过滤后的基础字典记录集
    ...
    If gclsPros.BaseCode Is Nothing Then
        ...
        ElseIf gclsPros.PatiType = PF_门诊 Then
            strTables = "医疗付款方式;性别;婚姻状况;职业;民族;国籍;血型;学历;身份证未录原因;证件类型"
        Else
            strTables = "医疗付款方式;性别;婚姻状况;职业;民族;国籍;血型;社会关系;病情;入院方式;分化程度;最高诊断依据;出院方式;感染部位;治疗结果;诊疗麻醉类型;住院死亡原因;学历;出院转入;身份证未录原因;路径上报变异原因;证件类型"
        End If
...
Public Function GetPatiTransfer(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As ADODB.Recordset
    ...
    strSql = _
                ...
                " And A.科室ID=B.ID And A.开始原因=3 And A.开始时间 is Not NULL And 附加床位<>1" & _
                " Order by A.开始时间"
    ...
    With rsReturn
        ...
        If blnReadAdvice Then
            strSql = "Select a.Id, NVL(Trunc(E.安排时间),NVL(Trunc(a.手术时间),Trunc(a.开始执行时间))) 手术日期, Nvl(D.编码,B.编码) 手术编码 , NVL(E.安排时间,NVL(A.手术时间,A.开始执行时间)) 手术开始时间, NVL(E.完成时间,a.停嘱时间) 手术结束时间, a.诊疗项目id, d.Id 手术操作id, b.名称 手术名称," & vbNewLine & _
                ...
                "Where a.诊疗项目id = b.Id And A.id = e.医嘱id And b.Id = c.手术id(+) And c.疾病id = d.Id(+) And a.病人id = [1] And 主页id = [2] And" & vbNewLine & _
                "医嘱状态 = 8 And ((a.诊疗类别 = 'E' And Nvl(d.Id, 0) <> 0) Or a.诊疗类别 = 'F')"
            Set rsTmp = zlDatabase.OpenSQLRecord(strSql, "读取手术操作信息", lng病人ID, lng主页ID)
            While Not rsTmp.EOF
                blnDefault = True
                .AddNew
