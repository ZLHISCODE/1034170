...
Private Sub CMD确认_Click()
    ...
    Call gclsMsgSystem.InitBusiness(gclsMsgOracle, "", gstrDbUser)
    
    '------------------------------------------------------------------------------------------------------------------
    '检查是否为产品所有者登录，如果不是，则禁止并退出
    If gclsMsgSystem.IsOwner = False Then
    ...
    '启动SQL Trace
    '-----------------------------------------------
    strNote = gclsMsgOracle.SetSQLTrace(gstrDeptName, gstrUserName, strServerName)
    If strNote <> "" Then
