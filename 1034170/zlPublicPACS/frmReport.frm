...
'---------------------工具栏定义------------------------------------------
    ...
    Set cbrControl = CreateModuleMenu(cbrToolBar.Controls, xtpControlButton, conMenu_File_Refer, "诊疗参考", "诊疗项目参考", 801, True)
    ...
    For Each objXmlNode In objXmlNodes
        ...
        If Not objXmlNodeAttribute Is Nothing Then
            ...
            If objSvg.IsSvgContext(strImgSVG) Then
                ...
                '将图像名称放入图像集合中
                Call objImgFileName.Add(objXmlNodeAttribute.Text & ".jpg")
            End If
        End If
    Next
...
Private Sub cbrMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case conMenu_File_ViewImage '观片
            If vsfReportList.RowSel <= 0 Then Exit Sub
            Call mdlPublic.ViewImage(vsfReportList.Cell(flexcpText, vsfReportList.RowSel, _
                GetColNum(vsfReportList, "医嘱ID")), Me, , mblnRIS)
        
        Case conMenu_File_Refresh   '查询，刷新数据
            Call LoadReport(GetFilter)
            
        Case conMenu_File_Refer
            Call ShowItemReferWindow
...
Private Sub ShowItemReferWindow()
    Dim objCisKernel As Object
    Dim strItemId As String
    
    Set objCisKernel = CreateObject("zlCisKernel.clsCISKernel")
    If objCisKernel Is Nothing Then Exit Sub
    
    strItemId = vsfReportList.Cell(flexcpText, vsfReportList.RowSel, GetColNum(vsfReportList, "诊疗项目ID"))
    Call objCisKernel.ShowClincHelp(1, Me, Val(strItemId), False, "")
...
On Error GoTo Errorhand
    ...
    If tabReport.Tab = 0 Then
        ...
        If mblnRIS = True Then
            strSql = "Select * from (Select 2 as 类型,TO_CHAR(RAWTOHEX(b.检查报告id)) 报告ID,a.执行科室id, a.影像类别,a.接收日期 As 检查时间, c.医嘱内容, c.开嘱时间, " & _
                "  f.创建人 as 报告人,f.最后审核人 as 审核人, c.Id As 医嘱id,c.诊疗项目ID,b.病历id " & _
                ...
                " a.报告人,a.复核人 审核人,c.Id As 医嘱id, c.诊疗项目ID,b.病历id " & _
        ...
        Else
            strSql = "Select* from (Select distinct m.类型, m.报告ID, m.执行科室id, m.查阅, m.打印, m.姓名,m.影像类别, m.医嘱内容,m.申请时间 , m.性别, m.年龄,  " & _
                ...
                " m.申请人, m.报告人, m.终审人, n.门诊号, n.就诊卡号, n.身份证号,m.病人id,m.医嘱id,m.诊疗项目ID,m.开嘱科室ID " & _
                ...
                "c.开嘱时间 as 申请时间,c.开嘱医生 申请人,b.创建人 报告人,b.最后审核人 终审人,c.病人id, c.Id As 医嘱id,c.诊疗项目ID,c.开嘱科室ID " & _
                ...
                "c.开嘱时间 as 申请时间,c.开嘱医生 申请人,a.报告人,a.复核人 终审人,c.病人id, c.Id As 医嘱id,c.诊疗项目ID,c.开嘱科室ID " & _
    ...
    Else
        strSql = "Select * from (select distinct 2 as 类型,TO_CHAR(RAWTOHEX(f.id)) 报告ID, b.执行科室id,decode(nvl(g.查阅状态,0),0,'','√') 查阅," & _
                ...
                "d.出院病床 as 床号, b.开嘱医生 申请人,f.创建人 as 报告人,f.最后审核人 终审人,c.门诊号, c.就诊卡号, c.身份证号,c.病人id,b.id as 医嘱ID,b.诊疗项目ID,b.开嘱科室id " & _
                ...
                "d.出院病床 as 床号, b.开嘱医生 申请人,e.报告人,e.复核人 终审人,c.门诊号, c.就诊卡号, c.身份证号,c.病人id,b.id as 医嘱ID,b.诊疗项目ID,b.开嘱科室id " & _
...
'说明：此过程目前只有体检调用，一个医嘱对应一份报告
    ...
    strSql = "Select Length(a.报告内容.GetClobVal()) as ContentLength, '' || A.ID As Id From 影像报告记录 a Where " & strFilter
    Set rsTemp = GetRecordset(strSql)
