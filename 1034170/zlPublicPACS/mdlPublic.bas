...
'新网ADView观片，打开指定图像： long OEMViewOpen ( long lPlanID, LPCTSTR cpFilter, long lFunc, LPCTSTR cpReserved );
...
Public Sub ViewImage(ByVal lng医嘱ID As Long, frmParent As Object, Optional ByVal blnMoved As Boolean = False, _
...
'功能：调用观片站
    ...
    If intImageLocation = 1 Then
        If blnXWADViewer = True Then
            Call XWADViewerOpen(CStr(lng医嘱ID))
        Else
...
'--------------------------------------------
    ...
    lngPlanId = Val(gobjComLib.zlDatabase.GetPara("XW检查方案号", 100, 1288, "1"))
