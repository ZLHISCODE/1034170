...
Private Sub CDM确认_Click()
    ...
    If Not OraDataOpen(gcnOracle, strServerName, strUserName, IIf(UCase(strUserName) = "SYS" Or UCase(strUserName) = "SYSTEM", strPassword, TranPasswd(strPassword))) Then
        TXT密码.Text = ""
        If TXT密码.Enabled Then TXT密码.SetFocus
        SetConState
        Exit Sub
    ...
    '创建快捷方式用
    SaveSetting "ZLSOFT", "注册信息\登陆信息", "程序路径", App.Path & "\" & App.EXEName & ".exe"
...
Private Sub Form_Load()
    ...
    txt数据库.Text = GetSetting(appName:="ZLSOFT", Section:="注册信息\登陆信息", Key:="SERVER", Default:="")
End Sub
