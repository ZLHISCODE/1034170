...
End Function
Public Function zlGetKindCards(Optional strIDKindStr As String = "", Optional blnOnlyAccouct As Boolean = False, Optional NotAutoAppendKind As Boolean = False) As Cards
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取有效的卡对象
    '返回: 成功,卡对象
    '编制:刘兴洪
    ...
    For i = 0 To UBound(varData)
        ...
        If Trim(varTemp(1)) <> "" Then
           ...
           If blnFind = False Then
                ...
                '短名1|全名1|读卡标志1|卡类别ID1|卡号长度1|缺省标志1(1-当前缺省;0-非缺省)|是否存在帐户1(1-存在帐户;0-不存在帐户)|
                '卡号密文1(第几位至第几位加密,空为不加密)
                With objCard
                    .接口编码 = "-"
                    .名称 = varTemp(1)
                    .短名 = varTemp(0)
    ...
    Next
    '未加入的，放入最后
    If NotAutoAppendKind = False Then
        If Not gobjCards Is Nothing Then
            For Each objCard In gobjCards
...
Public Function zlGetPati(ByVal cnOracle As ADODB.Connection, _
    ...
    "        a.病人类型 as 病人类型,sysdate as 挂失时间, 0  as 挂失有效天数,sysdate as 当前时间,a.手机号" & _
    "   From 病人信息 A,医疗付款方式 P" & _
    "   Where a.医疗付款方式=P.名称(+)  " & strWhere
    If Not objDatabase Is Nothing Then
...
Public Function FromXMLPati(ByVal strPatiXml As String, ByRef strErrMsg As String) As PatiInfor
   ...
   '加载病人信息
    ...
    '    标识    数据类型    长度    精度    说明
    Call objXML.zlXML_GetNodeValue("PatiXML", , strValue)
    objPati.PatiXML = strPatiXml
    '    卡号    Varchar2    20
