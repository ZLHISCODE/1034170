...
Private Sub SaveParaSet()
    ...
    If mblnConn Then Call objDatabase.SetPara("向前滚动-功能键", strValue, glngSys, mlngModul, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向前滚动-功能键", strValue)
    strValue = Trim(cboFastkey(0).Text)
    If mblnConn Then Call objDatabase.SetPara("向前滚动-快键", strValue, glngSys, mlngModul, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向前滚动-快键", strValue)
    
    strValue = Trim(cboFun(1).Text)
    If mblnConn Then Call objDatabase.SetPara("向后滚动-功能键", strValue, glngSys, mlngModul, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向后滚动-功能键", strValue)
     strValue = Trim(cboFastkey(1).Text)
     If mblnConn Then Call objDatabase.SetPara("向后滚动-快键", strValue, glngSys, mlngModul, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "向后滚动-快键", strValue)
    
    strValue = Trim(cboFun(2).Text)
    If mblnConn Then Call objDatabase.SetPara("读卡-功能键", strValue, glngSys, mlngModul, True)
    Call SaveRegInFor(mRegType, "医疗卡类别", "读卡-功能键", strValue)
    strValue = Trim(cboFastkey(2).Text)
    If mblnConn Then Call objDatabase.SetPara("读卡-快键", strValue, glngSys, mlngModul, True)
    ...
    With vsGrid
        For i = 2 To .Rows - 1
            If Left(.RowData(i), 1) = "K" Then
                ...
                '103310:李南春,2016/12/6,启用回车后增加卡号长度
    ...
    With cboDefaultCard
        ...
        If InStr(1, mstrPrivs, ";参数设置;") > 0 Then
            Call objDatabase.SetPara("缺省读卡类别", .ItemData(.ListIndex), glngSys, mlngModul, True)
...
Private Function InitData(Optional blnRestoreDefault As Boolean = False) As Boolean
    Dim i As Long
    Dim strValue As String, strA1 As String
    Dim objDatabase As Object, objTemp As Object
    ...
    If Not blnRestoreDefault Then
        If mblnConn Then
            strA1 = objDatabase.getPara("向前滚动-功能键", glngSys, mlngModul)
            strValue = objDatabase.getPara("向前滚动-快键", glngSys, mlngModul)
        Else
    ...
    If Not blnRestoreDefault Then
        If mblnConn Then
            strValue = objDatabase.getPara("向后滚动-功能键", glngSys, mlngModul)
        Else
    ...
    If Not blnRestoreDefault Then
        If mblnConn Then
            strValue = objDatabase.getPara("向后滚动-快键", glngSys, mlngModul)
        Else
    ...
    If Not blnRestoreDefault Then
        If mblnConn Then
            strValue = objDatabase.getPara("读卡-功能键", glngSys, mlngModul)
        Else
    ...
    If Not blnRestoreDefault Then
        If mblnConn Then
            strValue = objDatabase.getPara("读卡-快键", glngSys, mlngModul)
        Else
    ...
    With vsGrid
        ...
        For i = 0 To .Cols - 1
            .MergeCol(i) = True
        Next
        .Rows = IIf(mobjCards.Count = 0, 1, mobjCards.Count) + 2
        For i = 1 To mobjCards.Count
            ...
            If strValue = "" Then   '缺省启用
                .TextMatrix(i + 1, .ColIndex("启用")) = 1
            Else
                .TextMatrix(i + 1, .ColIndex("启用")) = Val(strValue)
            End If
            ...
            End If
            If strValue = "" Then   '缺省启用
                .TextMatrix(i + 1, .ColIndex("回车符")) = "缺省"
            Else
                .TextMatrix(i + 1, .ColIndex("回车符")) = strValue
            End If
            ...
            End If
            .TextMatrix(i + 1, .ColIndex("功能键")) = IIf(strValue = "", " ", strValue)
            
            strValue = ""
            If Not blnRestoreDefault Then
                    Call GetRegInFor(mRegType, "医疗卡类别\" & mobjCards(i).名称, "读卡-快键", strValue)
            End If
            .TextMatrix(i + 1, .ColIndex("快键")) = IIf(strValue = "", " ", strValue)
            .TextMatrix(i + 1, .ColIndex("名称")) = mobjCards(i).名称
            If InStr(1, "," & mstrMustSelectItems & ",", "," & .TextMatrix(i + 1, .ColIndex("名称")) & ",") > 0 Then
                .Cell(flexcpForeColor, i + 1, 0, i + 1, .Cols - 1) = &H80000011
                .Cell(flexcpForeColor, i + 1, .ColIndex("回车符")) = &H80000008
            End If
            '78768:李南春,2016/3/3,缺省读卡类别
            .TextMatrix(i + 1, .ColIndex("刷卡")) = IIf(mobjCards(i).是否刷卡, 1, 0)
            .Cell(flexcpData, i + 1, .ColIndex("名称")) = mobjCards(i).接口序号
            .RowData(i + 1) = "K" & i
...
Private Sub Form_Load()
    ...
    mlngModul = 1153
    Call InitFace
    If mblnConn Then mstrPrivs = ";" & gobjComLib.GetPrivFunc(glngSys, mlngModul) & ";"
End Sub
...
Private Sub vsGrid_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    With vsGrid
        ...
        Case .ColIndex("回车符")
            If Left(.RowData(Row), 1) <> "K" Then Cancel = True: Exit Sub
            If mobjCards(Row - 1).接口序号 <= 0 Or Not mobjCards(Row - 1).是否刷卡 Then Cancel = True: Exit Sub
        Case Else
            Cancel = True
        End Select
    End With
...
'将缺省读卡类型保存在参数表和注册表
    ...
    If objDatabase Is Nothing Then Set objDatabase = gobjDatabase
    If mblnConn Then
        strValue = objDatabase.getPara("缺省读卡类别", glngSys, mlngModul, 0, Array(lblDefaultCard, cboDefaultCard), InStr(1, mstrPrivs, ";参数设置;") > 0)
    Else
