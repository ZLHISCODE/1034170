...
Public Function ShowMe(frmParent As Object, ByVal str类别 As String, ByVal lng病人科室ID As Long, Optional ByVal str性别 As String, _
    Optional ByVal blnMultiSel As Boolean, Optional ByVal blnICD10 As Boolean = True, Optional ByVal strSel编码 As String, Optional ByVal lngSys As Long = 100, Optional ByVal intPatiType As Integer = 0) As ADODB.Recordset
    mstr类别 = str类别
        ...
        If cbo类别.ItemData(cbo类别.ListIndex) <> 0 Then '为0表示该种疾病没有分类
            If cbo科室.ItemData(cbo科室.ListIndex) = 0 Then
                strSQL = "Select ID,上级ID,序号,名称 From 疾病编码分类 Where 类别=[1]" & _
                    " And (撤档时间 is Null Or 撤档时间=To_Date('3000-01-01','YYYY-MM-DD')) " & vbNewLine & _
            ...
            Else
                ...
                strSQL = _
                    ...
                    "From 疾病编码分类 Where 类别=[1]  And (撤档时间 is Null Or 撤档时间=To_Date('3000-01-01','YYYY-MM-DD')) " & vbNewLine & _    
...
Private Sub FillListData()
    ...
    If mblnICD10 Then
        If cbo类别.ItemData(cbo类别.ListIndex) <> 0 Then '为0表示该种疾病没有分类
            ...
            strSQLTmp = " And (A.分类id = [3] Or" & vbNewLine & _
    ...
    If cbo科室.ItemData(cbo科室.ListIndex) <> 0 Then
        If mblnICD10 Then '按疾病编码输入
            If mbln病案系统 Then
                strSQL = "Select A.Id As 项目id, A.编码, A.序号, A.附码, D.ID 附码ID, D.名称 附码名称, A.名称, A.说明, Null 编者, A.分类id, A.简码, A.疗效限制, A.分娩, C.是否病人,A.编码 疾病编码, A.Id 疾病id,A.类别 疾病类别, Null 诊断id" & vbNewLine & _
                        "From 疾病编码目录 A, 疾病编码科室 B, 疾病编码分类 C, 疾病编码目录 D " & vbNewLine & _
                        "Where A.Id = B.疾病id And A.附码=D.编码(+) And A.类别 = [2] And A.分类id = C.Id(+)" & IIf(cbo科室.List(cbo科室.ListIndex) = "个人常用", " And b.人员id = [5]", " ") & vbNewLine & _
    ...
    Else
        If mblnICD10 Then '按疾病编码输入
            ...
            Else
                strSQL = "Select A.Id As 项目id, A.编码, A.序号, A.附码,D.ID 附码ID, D.名称 附码名称, A.名称, A.说明, Null 编者, A.分类id, A.简码,  A.疗效限制, A.分娩, C.是否病人,A.编码 疾病编码, A.Id 疾病id,A.类别 疾病类别," & vbNewLine & _
                        "       Max(B.诊断id) 诊断id" & vbNewLine & _
                        "From 疾病编码目录 A, 疾病诊断对照 B, 疾病编码分类 C , 疾病编码目录 D" & vbNewLine & _
                        "Where A.类别 = [2] And A.Id = B.疾病id(+) And A.分类id = C.Id(+) And A.附码=D.编码(+)  And" & vbNewLine & _
                        "      (A.撤档时间 Is Null Or A.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD'))" & IIf(str性别 <> "", " And (A.性别限制=[4] Or A.性别限制 is Null) ", " ") & strSQLTmp & vbNewLine & _
                        IIf(mInt适用范围 = 0, "", " And (Nvl(A.适用范围,0) = 0 Or A.适用范围 = " & CStr(mInt适用范围) & ") ") & _
                        "Group By A.Id, A.编码, A.序号, A.附码, D.ID, D.名称, A.名称, A.说明, A.分类id, A.简码, A.疗效限制, A.分娩,A.类别, C.是否病人"
            End If
        Else '按诊断输入
