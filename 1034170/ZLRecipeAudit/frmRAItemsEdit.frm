...
'返回：True成功；False失败
    ...
    If Len(txtCode.Text) > txtCode.MaxLength Then
        MsgBox mdlDefine.FormatString("“编码”超长，最多能输入[1]个汉字或[2]个字符！", txtCode.MaxLength \ 2, txtCode.MaxLength), vbInformation, gstrSysName
        txtCode.SetFocus
        Exit Function
    End If
    
    If Len(txtSimName.Text) > txtSimName.MaxLength Then
        MsgBox mdlDefine.FormatString("“简称”超长，最多能输入[1]个汉字或[2]个字符！", txtSimName.MaxLength \ 2, txtSimName.MaxLength), vbInformation, gstrSysName
        txtSimName.SetFocus
        Exit Function
    End If
    
    If Len(txtContent.Text) > txtContent.MaxLength Then
        MsgBox mdlDefine.FormatString("“内容描述”超长，最多能输入[1]个汉字或[2]个字符！", txtContent.MaxLength \ 2, txtContent.MaxLength), vbInformation, gstrSysName
        txtContent.SetFocus
        Exit Function
    End If
