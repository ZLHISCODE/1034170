    ...
    With lvwVar
        ...
        For i = LBound(arrCols) To UBound(arrCols)
            ...
            If UBound(arrElements) < 2 Then
                MsgBox mdlDefine.FormatString("设置“[1]”控件列头的参数不正确！", lvwVar.Name), vbInformation, gstrSysName
                Exit Sub
            End If
            '列名
            strText = Trim(arrElements(0))
            If strText = "" Then
                MsgBox mdlDefine.FormatString("设置“[1]”控件列头名称的参数不正确！", lvwVar.Name), vbInformation, gstrSysName
                Exit Sub
            End If
            'Key
...
Public Sub SetTextMaxLen(ByRef txtVal As TextBox, ByVal strTableField As String)
    ...
    On Error GoTo errHandle
    gstrSQL = mdlDefine.FormatString("Select [2] as 字段 From [1] Where Rownum < 1 ", _
