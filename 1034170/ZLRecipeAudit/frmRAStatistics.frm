        ...
        Case enuMenus.打印预览, enuMenus.打印, enuMenus.输出Excel
            ...
            If TypeName(objTmp) = "VSFlexGrid" Then
                ...
                If UCase(objTmp.Name) = "VSFSTATISTICS" Then
                    strTitle = mdlDefine.FormatString("不合格审查项目统计（[1]）", IIf(optClass1(0).Value, "门诊", "住院"))
                ElseIf UCase(objTmp.Name) = "VSFSTATISTICSDETAIL" Then
                    strTitle = mdlDefine.FormatString("不合格审查项目明细（[1]）", IIf(optClass1(0).Value, "门诊", "住院"))
                ElseIf UCase(objTmp.Name) = "VSFDETAIL" Then
                    strTitle = mdlDefine.FormatString("审查不合格药嘱明细（[1]）", IIf(optClass2(0).Value, "门诊", "住院"))
                End If
                If strTitle <> "" Then
