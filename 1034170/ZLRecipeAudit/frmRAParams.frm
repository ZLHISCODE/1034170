...
Private Sub cmdOK_Click()
        ...
        For l = 1 To .Rows - 1
            If Val(.TextMatrix(l, .ColIndex("选择"))) = -1 Then
                If Val(.TextMatrix(l, .ColIndex("部门ID"))) > 0 Then
                    strTmp = strTmp & mdlDefine.FormatString("[1],", Val(.TextMatrix(l, .ColIndex("部门ID"))))
                End If
            End If
        Next
    ...
    strSQL = mdlDefine.FormatString("Zl_处方审查参数_Save(1, [1], [2], Null, [3])", _
                    "'" & mstrPCName & "'", _
                    mbytMode, _
...
Private Sub Form_Load()
    ...
    mblnEnter = False
    lblFromDept.Caption = mdlDefine.FormatString("设置本机（[1]）的来源科室：", mstrPCName)
...
Private Sub SetPCName()
    ...
    With vsfFromDept
        For l = 1 To .Rows - 1
            '取科室id相同的机器名
            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取机器名", mbytMode, mdlDefine.FormatString("%,[1],%", .TextMatrix(l, .ColIndex("部门ID"))))
            If rsTemp.EOF = False Then
                .TextMatrix(l, .ColIndex("已选机器")) = zlCommFun.NVL(rsTemp!机器名)
