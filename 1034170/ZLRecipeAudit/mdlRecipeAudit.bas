...
'返回：0-异常/未知；1-合格；2-不合格
    ...
    Select Case UCase(fldItem!编码)
        ...
        Case "C01"          'PASS结果
            strMedicalID = RAI_PASS(lngPatientID, bytMedicalClass, lngBillID, zlCommFun.NVL(fldItem!PASS结果), strID)
            AuditDrug = IIf(strMedicalID = "", 1, 2)
...
'返回：不合格的医嘱ID。 格式：医嘱ID[;医嘱ID]
    ...
    With rsTemp
        Do While .EOF = False
            strReturn = strReturn & mdlDefine.FormatString("[1];", !ID)
            .MoveNext
        Loop
...
'返回：热量需要量公式和值（如：66.5 + 13.8 * 61KG + 5.0 * 172CM - 6.8 * 30岁 = 1564.30）
    ...
    If rsTemp.EOF = False Then
        GetCalorie = zlCommFun.NVL(rsTemp!热量)
    End If
...
'返回：True成功；False失败
    ...
    If rsTemp.EOF = False Then
        ...
        objXML.AppendData "patient_id", zlCommFun.NVL(rsTemp!病人ID)
        objXML.AppendData "patient_name", zlCommFun.NVL(rsTemp!姓名)
        objXML.AppendData "in_number", zlCommFun.NVL(rsTemp!住院号)
        objXML.AppendData "out_number", zlCommFun.NVL(rsTemp!门诊号)
        objXML.AppendNode "patient_info", True
        
        '医嘱信息
        objXML.AppendNode "patient_clinic"
        objXML.AppendData "patient_source", zlCommFun.NVL(rsTemp!病人来源)
        objXML.AppendData "clinic_id", zlCommFun.NVL(rsTemp!就诊id)
        objXML.AppendData "clinic_area_id", zlCommFun.NVL(rsTemp!当前病区id)
        objXML.AppendData "clinic_area_title", zlCommFun.NVL(rsTemp!当前病区)
        objXML.AppendData "clinic_dept_id", zlCommFun.NVL(rsTemp!当前科室id)
        objXML.AppendData "clinic_dept_title", zlCommFun.NVL(rsTemp!当前科室)
        objXML.AppendData "clinic_room", ""
        objXML.AppendData "clinic_bed", zlCommFun.NVL(rsTemp!当前床号)
        objXML.AppendNode "patient_clinic", True
        
        '审方信息
        objXML.AppendNode "recipe_audit_info"
        objXML.AppendData "create_doctor_name", zlCommFun.NVL(rsTemp!开嘱医生)
        objXML.AppendData "ra_time", zlCommFun.NVL(rsTemp!审查时间)
        objXML.AppendData "ra_chemist_id", UserInfo.ID
        objXML.AppendData "ra_chemist_name", zlCommFun.NVL(rsTemp!审查人)
        objXML.AppendData "ra_result", zlCommFun.NVL(rsTemp!审查结果)
        objXML.AppendData "order_ids", zlCommFun.NVL(rsTemp!ids)
...
'  lngMedicalID：药嘱ID
    ...
    If rsTemp.EOF = False Then
        lngPatientID = zlCommFun.NVL(rsTemp!病人ID, 0)
        If blnOutPatient Then
            strNO = zlCommFun.NVL(rsTemp!挂号单)
        Else
            lngPageID = zlCommFun.NVL(rsTemp!主页ID, 0)
        End If
    End If
    rsTemp.Close
...
'  lblDisp：要显示的控件
    ...
    End With
    If intCount > 0 Then
        lblDisp = STR_ITEMS & mdlDefine.FormatString("（共有[1]项不合格）", intCount)
    Else
        lblDisp = STR_ITEMS
    End If
