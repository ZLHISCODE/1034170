...
'    End If
    ...
    Select Case mintParaPass
        Case 1
            chkPass.Caption = mdlDefine.FormatString("合理用药结果审查（[1]）", "美康")
            strPASS = "4-橙(较高度关注)|3-黑(严重关注)|2-红(高度关注)|1-黄(适度关注)|0-蓝(提醒)"
        Case 2
            chkPass.Caption = mdlDefine.FormatString("合理用药结果审查（[1]）", "大通")
            chkPass.Enabled = False
        Case 3
            chkPass.Caption = mdlDefine.FormatString("合理用药结果审查（[1]）", "太元通")
            chkPass.Width = 2650
            strPASS = "1-红(禁忌)|2-黄(慎用)|3-蓝(提醒)"
        Case 4
            chkPass.Caption = mdlDefine.FormatString("合理用药结果审查（[1]）", "保进")
    ...
    '获取PASS项目
    mrsItems.Filter = mdlDefine.FormatString("类别='[1]'", "3-固定")
    ...
    '加载数据
    mrsItems.Filter = mdlDefine.FormatString("类别='[1]' or 类别='4-自定义'", IIf(optAccord(0).Value, "1-7项", "2-28项"))     '1-7项、2-28项、3-固定1项；4-自定义
...
Private Sub cmdDel_Click()
    With vsfItems
        If MsgBox(mdlDefine.FormatString("是否确认删除“[1]”审查项目？", .TextMatrix(.Row, .ColIndex("简称"))), vbQuestion + vbYesNo + vbDefaultButton2) = vbYes Then
...
'返回：True成功；False失败
    ...
    With vsfItems
        ...
        For lngRow = 1 To .Rows - 1
            ...
            '3.编码
            strSQL = strSQL & mdlDefine.FormatString("'[1]',", .TextMatrix(lngRow, .ColIndex("编码")))
            '4.简称
            strSQL = strSQL & mdlDefine.FormatString("'[1]',", .TextMatrix(lngRow, .ColIndex("简称")))
            '5.内容描述
            strSQL = strSQL & mdlDefine.FormatString("'[1]',", .TextMatrix(lngRow, .ColIndex("内容描述")))
            '6.是否门诊启用
            ...
            strSQL = strSQL & "Null,"
            '10.操作人
            strSQL = strSQL & mdlDefine.FormatString("'[1]',", UserInfo.姓名)
            '11.是否作废
    ...
    If Val(chkPass.Tag) = 1 Then
        ...
        strSQL = strSQL & "2,"
        '9.PASS结果
        strSQL = strSQL & IIf(strPASS = "", "Null", mdlDefine.FormatString("'[1]'", strPASS)) & ","
        '10.操作人
        strSQL = strSQL & mdlDefine.FormatString("'[1]',", UserInfo.姓名)
        '11.是否作废
        strSQL = strSQL & "Null)"
