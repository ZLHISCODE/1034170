...
Private Sub InitCommandbars()
    ...
    With cbrTmp
        ...
        Set cbcTmp = .Controls.Add(xtpControlLabel, enuToolsID.姓名, mdlDefine.FormatString("病人：[1]  ", strName))
        Set cbcTmp = .Controls.Add(xtpControlLabel, enuToolsID.性别, mdlDefine.FormatString("性别：[1]  ", strSex))
        cbcTmp.BeginGroup = True
        Set cbcTmp = .Controls.Add(xtpControlLabel, enuToolsID.年龄, mdlDefine.FormatString("年龄：[1]  ", strAge))
        cbcTmp.BeginGroup = True
    End With
...
Private Sub SetAuditInfo()
    ...
    If rsTemp.EOF = False Then
        '更新控件
        mblnLocking = zlCommFun.NVL(rsTemp!锁定用户) <> ""
        mintStatus = Val(zlCommFun.NVL(rsTemp!状态))
        mintResult = Val(zlCommFun.NVL(rsTemp!审查结果))
        
        lblAuditInfo(1).Caption = mdlDefine.FormatString("审查人：[1]", zlCommFun.NVL(rsTemp!审查人))
        lblAuditInfo(2).Caption = mdlDefine.FormatString("审查时间：[1]", zlCommFun.NVL(rsTemp!审查时间))
        ...
        picAuditInfo.Tag = zlCommFun.NVL(rsTemp!ID)           '审方ID
    Else
...
Private Sub SetPatientInfo()
    ...
    If rsTemp.EOF = False Then
         strName = mdlDefine.FormatString("病人：[1]  ", rsTemp!姓名)
         strSex = mdlDefine.FormatString("性别：[1]  ", rsTemp!性别)
         strAge = mdlDefine.FormatString("年龄：[1]  ", rsTemp!年龄)
    End If
    rsTemp.Close
