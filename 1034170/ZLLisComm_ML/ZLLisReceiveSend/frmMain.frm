...
164             strCmd = Mid(strCmd, 3)
166             If Val(aTmp(0)) = 1 Then '仪器请求获取标本信息，并非检验结果
                
168                 If Len(strCmd) > 0 Then
                        '发送消息
170                     Call SendCmd(strCmd)
                    End If
                
172                 If Len(strResult) > 0 Then '保存双向通讯请求
174                     strTmp = Format(Now, "YYYY-MM-DD") & "|^^0"
176                     If strResult = strTmp Then
                            'UF系统尿机上架时的双向通讯处理:
...
208                                 Put lngFileNo, , strResult
210                                 Close lngFileNo
                                    addDebug "", vbNewLine & Format(Now, "yyyy-MM-dd HH:mm:ss") & "向LIS发出查询请求" & vbNewLine & strResult
                                    Exit Do
                                End If
                            Loop
                        End If
...
214                 If Len(strCmd) > 0 Then
                        '发送消息
216                     Call SendCmd(strCmd)
                    End If
                End If
            Else
...
220     If Len(strResult) > 0 Then
            '保存解析结果
222         addDebug "", vbNewLine & Format(Now, "yyyy-MM-dd HH:mm:ss") & " 解码结果" & vbNewLine & strResult
...
240                 Put lngFileNo, , Replace(strResult, Chr(&HD) & Chr(&HA), "CHR(10) CHR(13)")
242                 Close lngFileNo
                    Exit Do
                End If
            Loop
