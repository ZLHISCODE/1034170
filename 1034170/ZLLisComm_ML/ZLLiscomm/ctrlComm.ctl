...
Private Sub timInData_Timer()
    ...
    On Error GoTo errH
    '优先处理 样本请求文件
    strData = ReadResultDirFile("IQ" & Format(Now, "yyyyMMdd"), True)
    If strData <> "" Then
        WriteLog "timInData-发送请求", LOG_通讯日志, 0, strData
        Call WriteSampleInfo(strData)   '= True Then RaiseEvent DataReceived
        Exit Sub
    End If
    
    '返回结果文件
    strData = ReadResultDirFile("RE" & Format(Now, "yyyyMMdd"), True)
    If strData <> "" Then
        ii = UBound(mItem, 2)
        strData = Replace(strData, "CHR(10) CHR(13)", vbCrLf)
        WriteLog "TimInData-保存数据", LOG_通讯日志, 0, strData
        Call InDataBase(strData)  '= True ' Then RaiseEvent DataReceived
    End If
...
Private Function ReadResultDirFile(ByVal strFileType As String, ByVal blnDelete As Boolean) As String
    '读返回文件
    ...
    strFileName = Dir(mstrReceiveDir & "\Result\" & strFileType & "_*.txt")
    If strFileName <> "" Then
        Do While lngCount < 1000
            lngCount = lngCount + 1
            strFileName = mstrReceiveDir & "\Result\" & strFileType & "_" & lngCount & ".txt"
            If mfsoTmp.FileExists(strFileName) Then Exit Do
        Loop
