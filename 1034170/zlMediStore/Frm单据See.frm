...
Public byt单据 As Byte               '输入参数:单据标志:8-收费处方；9-记帐单处方；10-记帐表处方
...
Private Sub RefreshHeadData()
    ...
    Select Case byt单据
        Case 8       '处方
                gstrSQL = " Select A.NO,B.名称 as 开单科室,G.姓名,G.性别,G.年龄,0 As 住院号,'' As 床号,A.摘要,A.填制人,To_Char(A.填制日期,'yyyy-MM-dd') as 填制日期,A.审核人,To_Char(A.审核日期,'yyyy-MM-dd') as 审核日期,G.病人ID,0 As 主页ID,A.对方部门ID as 开单部门ID,G.操作员姓名 as 计费人,A.单据,A.ID,G.开单人 " & _
                          " From 药品收发记录 A,部门表 B,门诊费用记录 G " & _
                          " Where A.对方部门id=B.id(+) And A.单据=[1] And A.NO=G.NO(+) And A.费用ID=G.ID And A.NO=[2] "
        Case 9
                gstrSQL = " Select A.NO,B.名称 as 开单科室,G.姓名,G.性别,G.年龄,C.住院号,'' As 床号,A.摘要,A.填制人,To_Char(A.填制日期,'yyyy-MM-dd') as 填制日期,A.审核人,To_Char(A.审核日期,'yyyy-MM-dd') as 审核日期,G.病人ID,0 As 主页ID,A.对方部门ID as 开单部门ID,G.操作员姓名 as 计费人,A.单据,A.ID,G.开单人 " & _
                          " From 药品收发记录 A,部门表 B,门诊费用记录 G,病人信息 C " & _
                          " Where A.对方部门id=B.id(+) And A.单据=[1] And C.病人id=G.病人id And A.费用ID=G.ID And A.NO=G.NO(+) And A.NO=[2] " & _
                          " Union All " & _
                          " Select A.NO,B.名称 as 开单科室,G.姓名,G.性别,G.年龄,C.住院号,G.床号,A.摘要,A.填制人,To_Char(A.填制日期,'yyyy-MM-dd') as 填制日期,A.审核人,To_Char(A.审核日期,'yyyy-MM-dd') as 审核日期,G.病人ID,G.主页ID,A.对方部门ID as 开单部门ID,G.操作员姓名 as 计费人,A.单据,A.ID,G.开单人 " & _
                          " From 药品收发记录 A,部门表 B,住院费用记录 G,病人信息 C " & _
                          " Where A.对方部门id=B.id(+) And A.单据=[1] And C.病人id=G.病人id And A.费用ID=G.ID And A.NO=G.NO(+) And A.NO=[2] "
        Case 27
                gstrSQL = " Select A.NO,B.名称 as 开单科室,A.摘要,A.填制人,To_Char(A.填制日期,'yyyy-MM-dd') as 填制日期,A.审核人,To_Char(A.审核日期,'yyyy-MM-dd') as 审核日期,A.对方部门ID as 开单部门ID,A.单据,A.ID, C.名称 库房 " & _
                          " From 药品收发记录 A,部门表 B, 部门表 C " & _
                          " Where A.对方部门id=B.id(+) And A.库房id = C.ID(+) And A.单据=[1] And A.NO=[2] "
        Case 10                     '摆药单
                gstrSQL = " Select A.NO,A.摘要,A.填制人,To_Char(A.填制日期,'yyyy-MM-dd') as 填制日期,A.审核人,To_Char(A.审核日期,'yyyy-MM-dd') as 审核日期,G.执行部门ID,A.对方部门ID as 开单部门ID,G.操作员姓名 as 计费人,A.单据,A.ID,G.开单人 " & _
                          " From 药品收发记录 A,部门表 B,住院费用记录 G " & _
                          " Where A.对方部门id=B.id(+) And A.单据=[1] And A.NO=G.NO(+) And A.费用ID=G.ID And A.NO=[2] "
           
    End Select
    
    Set RsHead = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, CInt(byt单据), strNo)
    ...
    With RsHead
        ...
        Select Case byt单据
            ...
            Case 27
                Me.Cbo库房.AddItem IIf(IsNull(!库房), "", !库房)
...
Private Sub RefreshBodyData()
    ...
    Select Case byt单据
        Case 8        '收费处方
'            gstrSQL = "SELECT DISTINCT '【'||F.编码||'】'||NVL(E.名称,F.名称) AS 药品信息,F.规格,DECODE(NVL(A.产地,''),'',DECODE(NVL(F.产地,''),'','',F.产地),A.产地) 产地,B.门诊单位 AS 单位," & _
'                " B.门诊包装,A.实际数量/DECODE(B.门诊包装,0,1,B.门诊包装) AS 数量,A.零售价*B.门诊包装 AS 单价," & _
...
'                " AND A.记录状态=" & int记录状态 & " AND A.单据=8 AND A.NO ='" & strNo & "'" & _
'                " ORDER BY A.序号"
            gstrSQL = "SELECT DISTINCT '【'||F.编码||'】'||NVL(E.名称,F.名称) AS 药品信息,F.规格,DECODE(NVL(A.产地,''),'',DECODE(NVL(F.产地,''),'','',F.产地),A.产地) 产地,B.门诊单位 AS 单位," & _
                " B.门诊包装,A.实际数量/DECODE(B.门诊包装,0,1,B.门诊包装) AS 数量,A.零售价*B.门诊包装 AS 单价," & _
                " A.零售金额,A.付数,A.成本金额,A.差价,A.药品ID,A.库房ID,A.入出系数,A.序号,A.ID,B.最大效期,B.指导零售价,B.指导差价率,a.审核人" & _
                " FROM 药品收发记录 A,药品规格 B,收费项目别名 E,收费项目目录 F " & _
                " WHERE A.药品ID=B.药品ID AND B.药品ID=F.ID " & _
                " AND B.药品ID=E.收费细目ID(+) AND E.性质(+)=3 AND E.码类(+)=1" & _
                " AND A.记录状态=[3] AND A.单据=[1] AND A.NO =[2] " & _
                " ORDER BY A.序号"
                
        Case 9, 27      '记帐单处方
'            gstrSQL = "SELECT DISTINCT '【'||F.编码||'】'||NVL(E.名称,F.名称) AS 药品信息,F.规格,DECODE(NVL(A.产地,''),'',DECODE(NVL(F.产地,''),'','',F.产地),A.产地) 产地,B.住院单位 AS 单位," & _
'                " B.住院包装,A.实际数量/DECODE(B.住院包装,0,1,B.住院包装) AS 数量,A.零售价*B.住院包装 AS 单价," & _
'                " A.零售金额,A.付数,A.成本金额,A.差价,A.药品ID,A.库房ID,A.入出系数,A.序号,A.ID,B.最大效期,B.指导零售价,B.指导差价率" & _
'                " FROM 药品收发记录 A,药品规格 B,收费项目别名 E,收费项目目录 F " & _
'                " WHERE A.药品ID=B.药品ID AND B.药品ID=F.ID " & _
'                " AND B.药品ID=E.收费细目ID(+) AND E.性质(+)=3 AND E.码类(+)=1" & _
'                " AND A.单据=9 AND A.记录状态=" & int记录状态 & " AND A.NO ='" & strNo & "'" & _
'                " ORDER BY A.序号"
            gstrSQL = "SELECT DISTINCT '【'||F.编码||'】'||NVL(E.名称,F.名称) AS 药品信息,F.规格,DECODE(NVL(A.产地,''),'',DECODE(NVL(F.产地,''),'','',F.产地),A.产地) 产地,B.住院单位 AS 单位," & _
                " B.住院包装,A.实际数量/DECODE(B.住院包装,0,1,B.住院包装) AS 数量,A.零售价*B.住院包装 AS 单价," & _
                " A.零售金额,A.付数,A.成本金额,A.差价,A.药品ID,A.库房ID,A.入出系数,A.序号,A.ID,B.最大效期,B.指导零售价,B.指导差价率,a.审核人" & _
                ...
                " WHERE A.药品ID=B.药品ID AND B.药品ID=F.ID " & _
                " AND B.药品ID=E.收费细目ID(+) AND E.性质(+)=3 AND E.码类(+)=1" & _
                " AND A.单据=[1] AND A.记录状态=[3] AND A.NO =[2] " & _
                " ORDER BY A.序号"
        Case 10        '记帐表处方
            gstrSQL = "SELECT DISTINCT P.名称 科室,G.姓名,G.性别,G.年龄,G.病人ID,G.主页ID,D.住院号,G.床号," & _
                "'【'||F.编码||'】'||NVL(C.名称,F.名称) AS 药品信息,F.规格,DECODE(NVL(A.产地,''),'',DECODE(NVL(F.产地,''),'','',F.产地),A.产地) 产地," & _
                " B.住院包装,A.实际数量/DECODE(B.住院包装,0,1,B.住院包装) AS 数量,B.住院单位 AS 单位,A.零售价*B.住院包装  AS 单价," & _
                " A.零售金额,A.付数,A.成本金额,A.差价,A.药品ID,A.库房ID,A.入出系数,A.序号,A.ID,B.最大效期,B.指导零售价,B.指导差价率" & _
                " FROM 药品收发记录 A,药品规格 B,住院费用记录 G,病人信息 D,收费项目别名 C,收费项目目录 F,部门表 P " & _
                " WHERE A.NO=G.NO(+) AND A.费用ID=G.ID AND A.对方部门ID=P.ID AND A.药品ID=B.药品ID AND B.药品ID=F.ID AND G.病人ID=D.病人ID " & _
                " AND B.药品ID=C.收费细目ID(+) AND C.性质(+)=3 AND C.码类(+)=1 " & _
                " AND A.单据=[1] AND A.记录状态=[3] AND A.NO =[2] " & _
                " ORDER BY A.序号"
    End Select
    
    Set RsBody = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, CInt(byt单据), strNo, int记录状态)
        ...
        Select Case byt单据
            Case 8, 9, 27         '处方
                    For intRow = 1 To .RecordCount
                        msf.TextMatrix(intRow, 0) = !药品信息
                        msf.TextMatrix(intRow, 1) = IIf(IsNull(!规格), "", !规格)
                        msf.TextMatrix(intRow, 2) = IIf(IsNull(!产地), "", !产地)
                        msf.TextMatrix(intRow, 3) = !单位
                        msf.TextMatrix(intRow, 4) = Format(!数量, "#####0.00000;-#####0.00000; ;")
                        msf.TextMatrix(intRow, 5) = Format(!付数, "#####0.00000;-#####0.00000; ;")
                        msf.TextMatrix(intRow, 6) = Format(!单价, "#####0.00000;-#####0.00000; ;")
                        msf.TextMatrix(intRow, 7) = Format(!零售金额, "#####0.00;-#####0.00; ;")
                        msf.TextMatrix(intRow, 8) = Format(!成本金额, "#####0.00;-#####0.00; ;")
                        msf.TextMatrix(intRow, 9) = Format(!差价, "#####0.00;-#####0.00; ;")
                        msf.RowData(intRow) = Nvl(!序号, 1)
                        Txt发药人.Text = IIf(IsNull(!审核人), "", !审核人)
                        .MoveNext
                    Next
            Case 10         '摆药
                    For intRow = 1 To .RecordCount
                        ...
                        msf.TextMatrix(intRow, 2) = !药品信息
                        msf.TextMatrix(intRow, 3) = IIf(IsNull(!规格), "", !规格)
                        msf.TextMatrix(intRow, 4) = IIf(IsNull(!产地), "", !产地)
                        msf.TextMatrix(intRow, 5) = !单位
                        msf.TextMatrix(intRow, 6) = Format(!数量, "#####0.00000;-#####0.00000; ;")
                        msf.TextMatrix(intRow, 7) = Format(!单价, "#####0.00000;-#####0.00000; ;")
                        msf.TextMatrix(intRow, 8) = Format(!零售金额, "#####0.00;-#####0.00; ;")
                        msf.TextMatrix(intRow, 9) = Format(!成本金额, "#####0.00;-#####0.00; ;")
                        msf.TextMatrix(intRow, 10) = Format(!差价, "#####0.00;-#####0.00; ;")
...
Private Sub SumDataMSf()
    ...
    Case 8, 9, 27                     ' 直接输入处方
        ...
        Next
        TxtMoney = "金额合计：" & Format(curTotal, "#####0.00;-#####0.00; ;") & Space(10) & "记帐金额合计：" & Format(Cur记帐金额, "#####0.00;-#####0.00; ;") & Space(10) & "记帐差价合计：" & Format(Cur记帐差价, "#####0.00;-#####0.00; ;")
    Case 10                          ' 直接输入摆药单
        For intLop = 1 To msf.rows - 1
            curTotal = curTotal + Val(msf.TextMatrix(intLop, 7))
            Cur记帐金额 = Cur记帐金额 + Val(msf.TextMatrix(intLop, 8))
...
Private Function SetEV()
    ...
    End Select
    If Not IsHavePrivs(mstrPrivs, "医生查询") Then
        Lbl科室.Visible = False
        Me.Cbo科室.Visible = False
        Lbl开单人.Visible = False
        Me.Txt开单人.Visible = False
    End If
End Function
