...
Public Function ValidData() As Boolean
    ...
    '保存时数据检查
    If vsfBill.TextMatrix(1, vsfBill.ColIndex("药品id")) = "" Then
        MsgBox "没有录入药品，不能保存！", vbInformation, gstrSysName
        vsfBill.Row = 1
        vsfBill.Col = vsfBill.ColIndex("药品名称")
        vsfBill.SetFocus
        Exit Function
    ...
    With vsfBill
        For lngRow = 1 To .rows - 1
            ...
            If LenB(StrConv(.TextMatrix(lngRow, .ColIndex("药品批号")), vbFromUnicode)) > 20 Then
                MsgBox "第" & lngRow & "行数据药品批号不能超过10个汉字或20个字符！", vbInformation, gstrSysName
                .Row = lngRow
                .Col = .ColIndex("药品批号")
                .SetFocus
                Exit Function
            ...
            If LenB(StrConv(.TextMatrix(lngRow, .ColIndex("批准文号")), vbFromUnicode)) > 40 Then
                MsgBox "第" & lngRow & "行数据批准文号不能超过20个汉字或40个字符！", vbInformation, gstrSysName
                .Row = lngRow
                .Col = .ColIndex("批准文号")
                .SetFocus
                Exit Function
...
Private Sub txtProvider_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    With txtProvider
        ...
        Set rsProvider = zlDatabase.ShowSQLSelect(Me, gstrSQL, 0, "供药单位", False, "", "", False, False, _
                        True, dblLeft, dblTop, vsfBill.Height, blnCancel, False, True, IIf(gstrMatchMethod = "0", "%", "") & strProviderText & "%", gstrNodeNo)
        If rsProvider Is Nothing Then
            Exit Sub
        Else
            txtProvider.Text = rsProvider!名称
    ...
    With vsfBill
        ...
        Else
            If mint编辑状态 = 3 Or mint编辑状态 = 4 Then
                .Editable = flexEDNone
            Else
                If .Tag <> "T" Then .Editable = flexEDKbdMouse
            End If
        ...
        With vsfBill
            ...
            If .Col <> .Cols - 1 Then
                If .Col = .ColIndex("药品名称") Then
                    .Col = .Col + 4
                ElseIf .Col = .ColIndex("进药日期") Or .Col = .ColIndex("产地") Then
                    .Col = .Col + 2
                Else
                    .Col = .Col + 1
                End If
        ...
        Select Case .ColKey(Col)
            ...
            Case "成本价"
                ...
                If KeyAscii = vbKeyReturn Then
                    .EditText = GetFormat(strkey, mintCostDigit)
            ...
            Case "零售价"
                If Not (KeyAscii = vbKeyReturn Or KeyAscii = vbKeyBack) Then
                    If InStr(1, strkey, ".") > 0 Then
                        ...
                        End If
                        intPosition = InStr(1, strkey, ".") + 1
                        If Len(Mid(strkey, intPosition)) >= mintPriceDigit Then
                            If strkey = .TextMatrix(.Row, .Col) Then
                                strkey = Chr(KeyAscii)
                            Else
                                KeyAscii = 0
                ...
                End If
                If KeyAscii = vbKeyReturn Then
                    .EditText = GetFormat(strkey, mintPriceDigit)
                    .TextMatrix(.Row, .Col) = .EditText
                End If
            Case "进药数量"
                If Not (KeyAscii = vbKeyReturn Or KeyAscii = vbKeyBack) Then
                    If InStr(1, strkey, ".") > 0 Then
                        ...
                        End If
                        intPosition = InStr(1, strkey, ".") + 1
                        If Len(Mid(strkey, intPosition)) >= mintNumberDigit Then
                            If strkey = .TextMatrix(.Row, .Col) Then
                                strkey = Chr(KeyAscii)
                            Else
                                KeyAscii = 0
                ...
                End If
                If KeyAscii = vbKeyReturn Then
                    .EditText = GetFormat(strkey, mintNumberDigit)
                    .TextMatrix(.Row, .Col) = .EditText
                End If
            Case "药品批号"
                If Not (KeyAscii = vbKeyReturn Or KeyAscii = vbKeyBack) Then
                    If LenB(StrConv(.EditText, vbFromUnicode)) >= 20 Or InStr(" ^&`'""", Chr(KeyAscii)) > 0 Then
                        KeyAscii = 0
                    End If
                End If
            Case "批准文号"
                If Not (KeyAscii = vbKeyReturn Or KeyAscii = vbKeyBack) Then
                    If LenB(StrConv(.EditText, vbFromUnicode)) >= 40 Or InStr(" ^&`'""", Chr(KeyAscii)) > 0 Then
                        KeyAscii = 0
                    End If
                End If
        End Select
...
Private Sub initCard()
    ...
    With vsfBill
        Do While Not rsTemp.EOF
            ...
            .TextMatrix(.rows - 1, .ColIndex("成本价")) = IIf(IsNull(rsTemp!成本价), "", GetFormat(rsTemp!成本价, mintCostDigit))
            .TextMatrix(.rows - 1, .ColIndex("零售价")) = IIf(IsNull(rsTemp!零售价), "", GetFormat(rsTemp!零售价, mintPriceDigit))
            .TextMatrix(.rows - 1, .ColIndex("进药数量")) = IIf(IsNull(rsTemp!进药数量), "", GetFormat(rsTemp!进药数量, mintNumberDigit))
