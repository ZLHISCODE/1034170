        ...
        Case 2, 3, 4
            ...
            If mint业务模式 = 1 Then
                gstrSQL = "SELECT * " & _
                    ...
                    "     A.摘要,填制人,填制日期,审核人,审核日期,A.库房ID,A.填写数量 实际数量,a.扣率 as 原成本价,A.单量 As 新成本价 " & _
            ...
            With mshBill
                ...
                Do While Not rsInitCard.EOF
                    ...
                    .TextMatrix(intRow, mconIntCol库存金额) = GetFormat(rsInitCard!库存金额, intMoneyDigit)
                    .TextMatrix(intRow, mconIntCol库存差价) = GetFormat(IIf(IsNull(rsInitCard!库存差价), 0, rsInitCard!库存差价), intMoneyDigit)
                    .TextMatrix(intRow, mconintCol调整额) = GetFormat(rsInitCard!调整额, intMoneyDigit)
                    ...
                    .TextMatrix(intRow, mconIntCol实际数量) = GetFormat(IIf(IsNull(rsInitCard!实际数量), "0", rsInitCard!实际数量), intNumberDigit)
                    If mint业务模式 = 1 Then
                        ...
                        .TextMatrix(intRow, mconintCol成本价) = GetFormat(rsInitCard!原成本价 * rsInitCard!比例系数, intCostDigit)
                        ...
                        If Not IsNull(rsInitCard!新成本价) Then
                            .TextMatrix(intRow, mconintCol新成本价) = GetFormat(rsInitCard!新成本价 * rsInitCard!比例系数, intCostDigit)
                        End If
                    Else
                        .TextMatrix(intRow, mconintCol成本价) = GetFormat((rsInitCard!库存金额 - rsInitCard!库存差价) / rsInitCard!可用数量, intCostDigit)
                        If Not IsNull(rsInitCard!新成本价) Then
                            .TextMatrix(intRow, mconintCol新成本价) = GetFormat(rsInitCard!新成本价 * rsInitCard!比例系数, intCostDigit)
                        End If
                    End If
...
'                        End If
            ...
            Case mconintCol新成本价
                ...
                If strkey <> "" Then
                    .TextMatrix(.Row, mconintCol调整额) = GetFormat(IIf(.TextMatrix(.Row, mconIntCol库存金额) = "", 0, .TextMatrix(.Row, mconIntCol库存金额)) - Val(IIf(.TextMatrix(.Row, mconIntCol可用数量) = "", 0, .TextMatrix(.Row, mconIntCol可用数量))) * Val(IIf(.TextMatrix(.Row, mconintCol新成本价) = "", 0, .TextMatrix(.Row, mconintCol新成本价))) _
                        - Val(IIf(.TextMatrix(.Row, mconIntCol库存差价) = "", 0, .TextMatrix(.Row, mconIntCol库存差价))), mintMoneyDigit)
                End If
            ...
            Case mconintCol调整额
                ...
                If strkey <> "" Then
                    ...
                    If strkey <> "" And Val(.TextMatrix(.Row, mconIntCol可用数量)) <> 0 Then
                        .TextMatrix(.Row, mconintCol新成本价) = GetFormat((IIf(.TextMatrix(.Row, mconIntCol库存金额) = "", 0, .TextMatrix(.Row, mconIntCol库存金额)) - Val(IIf(.TextMatrix(.Row, mconIntCol库存差价) = "", 0, .TextMatrix(.Row, mconIntCol库存差价))) - Val(strkey)) / Val(IIf(.TextMatrix(.Row, mconIntCol可用数量) = "", 0, .TextMatrix(.Row, mconIntCol可用数量))), mintCostDigit)
                    End If
                End If
                Call 显示合计金额
        End Select
...
Private Function SetColValue(ByVal intRow As Integer, ByVal int药品id As Long, _
    ...
    With mshBill
        ...
        .TextMatrix(intRow, mconIntCol库存金额) = GetFormat(num库存金额, mintMoneyDigit)
        .TextMatrix(intRow, mconIntCol库存差价) = GetFormat(num库存差价, mintMoneyDigit)
        ...
        If mint业务模式 = 1 Then
            ...
            End If
            .TextMatrix(intRow, mconintCol成本价) = GetFormat(Get成本价(int药品id, txtStock.Tag, Val(lng批次)) * num比例系数, mintCostDigit)
        Else
            .TextMatrix(intRow, mconIntCol可用数量) = GetFormat(num可用数量 / num比例系数, mintNumberDigit)
            .TextMatrix(intRow, mconintCol成本价) = GetFormat((((num库存金额 - num库存差价)) / num可用数量) / num比例系数, mintCostDigit)
        End If
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        If chrNo = "" Then chrNo = zlDatabase.GetNextNo(25, lng库房id)
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                dbl可用数量 = GetFormat(Val(.TextMatrix(intRow, mconIntCol实际数量)) * Val(.TextMatrix(intRow, mconIntCol比例系数)), gtype_UserDrugDigits.Digit_数量)
                dbl库存金额 = .TextMatrix(intRow, mconIntCol库存金额)
                dbl库存差价 = .TextMatrix(intRow, mconIntCol库存差价)
                dbl调整额 = .TextMatrix(intRow, mconintCol调整额)
                dbl新成本价 = GetFormat(Val(.TextMatrix(intRow, mconintCol新成本价)) / Val(.TextMatrix(intRow, mconIntCol比例系数)), gtype_UserDrugDigits.Digit_成本价)
                dbl原成本价 = GetFormat(Val(.TextMatrix(intRow, mconintCol成本价)) / Val(.TextMatrix(intRow, mconIntCol比例系数)), gtype_UserDrugDigits.Digit_成本价)
                lng序号 = intRow
                ...
                gstrSQL = "zl_药品库存差价调整_INSERT(" & lng入出类别id & ",'" & chrNo & "'," & lng序号 & "," _
                    ...
                    & str摘要 & "'," & mlng供药单位ID & "," & dbl原成本价 & "," & dbl新成本价 & "," & IIf(mint业务模式 = 1, 0, 1) & ")"
                ReDim Preserve arrSql(UBound(arrSql) + 1)
                arrSql(UBound(arrSql)) = gstrSQL
            End If
            recSort.MoveNext
...
Private Sub 显示合计金额()
    ...
    lblPurchasePrice.Caption = "库存金额合计：" & GetFormat(dbl库存金额, mintMoneyDigit)
    lblSalePrice.Caption = "库存差价合计：" & GetFormat(dbl库存差价, mintMoneyDigit)
    lblDifference.Caption = "调整额合计：" & GetFormat(dbl调整额, mintMoneyDigit)
...
Private Sub SearchData(ByVal str用途ID, ByVal str剂型编码 As String, _
    ...
    With mshBill
        Do While Not rsData.EOF
            ...
            .TextMatrix(i, mconIntCol库存金额) = GetFormat(rsData!实际金额, mintMoneyDigit)
            .TextMatrix(i, mconIntCol库存差价) = GetFormat(rsData!实际差价, mintMoneyDigit)
            .TextMatrix(i, mconintCol调整额) = GetFormat(rsData!差价调整额, mintMoneyDigit)
            .TextMatrix(i, mconIntCol比例系数) = rsData!比例系数
            .TextMatrix(i, mconintCol成本价) = GetFormat(rsData!成本价, mintCostDigit)
