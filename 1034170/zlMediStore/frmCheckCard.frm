                        ...
                        If Not rs时价分批.EOF Then
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    dbl零售价 = 时价药品零售价(rsDetail!药品id, dbl加成率, dbl成本价)
                                End If
                            End If
                        Else
                            If Nvl(rsDetail!帐面数量, 0) <> 0 Then
...
Private Sub CmdSave_Click()
    ...
    Dim str停用药品 As String
    Dim lngSum As Long '记录库存不足药品数量
    ...
    If mbln盘停用药品 = False Then '未开启“盘停用药品”参数，不对停用药品操作（保存、审核）
        For intLop = 1 To vsfBill.rows - 1
            If vsfBill.TextMatrix(intLop, 0) <> "" Then '有药品
                If CheckStopDrug(Val(vsfBill.TextMatrix(intLop, 0))) = True Then '药品停用
                   lng停用药品 = lng停用药品 + 1 '计数
                   If lng停用药品 = 1 Then lngFirstRow = intLop '记录第一个停用药品所在行
                   If lng停用药品 <= 3 Then str停用药品 = str停用药品 & IIf(lng停用药品 = 1, "", ",") & "【" & vsfBill.TextMatrix(intLop, mconIntCol药名) & "】"
                End If
            End If
        Next
        ...
        If str停用药品 <> "" Then
            vsfBill.SetFocus
            vsfBill.Row = lngFirstRow
            vsfBill.TopRow = lngFirstRow
            MsgBox str停用药品 & IIf(lng停用药品 <= 3, "", "等") & lng停用药品 & "个药品已停用！" & Chr(10) & "请删除对应的药品或开启“允许盘点停用药品”参数后再继续！", vbInformation, gstrSysName
            Exit Sub
        End If
    End If
    ...
    If mint编辑状态 = 3 Then        '审核
        ...
        For lngLoop = 1 To vsfBill.rows - 1
            If vsfBill.TextMatrix(lngLoop, mconintCol标志) = "亏" Then '盘亏时出库，检查库存是否足够
                'mintUnit-单位系数：1-售价;2-门诊;3-住院;4-药库（说明，等于0时有大小包装区分，大于0时为默认包装）
                dbl换算系数 = IIf(mintUnit > 0, Val(vsfBill.TextMatrix(lngLoop, mconIntCol比例系数)), Val(vsfBill.TextMatrix(lngLoop, mconIntCol比例系数小)))
                
                If Not 库存实际数量检查(Val(vsfBill.TextMatrix(lngLoop, 0)), Val(txtStock.Tag), Val(vsfBill.TextMatrix(lngLoop, mconIntCol批次)), _
                Val(vsfBill.TextMatrix(lngLoop, mconintCol数量差)), dbl换算系数, IIf(mintUnit > 0, mintNumberDigit, mintNumberDigit0)) Then
                    lngSum = lngSum + 1
                    If lngSum <= 3 Then '拼提示信息串
                        If lngSum = 3 Then mstrMsg = mstrMsg & "【" & vsfBill.TextMatrix(lngLoop, mconIntCol药名) & "(" & vsfBill.TextMatrix(lngLoop, mconIntCol批号) & "）" & "】，"
                        If lngSum <> 3 Then mstrMsg = mstrMsg & "【" & vsfBill.TextMatrix(lngLoop, mconIntCol药名) & "(" & vsfBill.TextMatrix(lngLoop, mconIntCol批号) & "）" & "】" & Chr(10)
                    End If
                    
                    If lngSum = 1 Then vsfBill.Row = lngLoop: vsfBill.TopRow = lngLoop
                End If
        ...
        If lngSum > 0 Then
            If mint库存检查 = 1 Then '不足提醒
                If MsgBox(mstrMsg & IIf(lngSum <= 3, lngSum & "个药品库存不足，是否继续？", "等" & lngSum & "个药品库存不足，是否继续？"), vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    mstrMsg = ""
                    Exit Sub
                End If
            ElseIf mint库存检查 = 2 Then '不足禁止
                MsgBox mstrMsg & IIf(lngSum <= 3, lngSum & "个药品库存不足，不能审核！", "等" & lngSum & "个药品库存不足，不能审核！"), vbInformation, gstrSysName
                mstrMsg = ""
                Exit Sub
            End If
        End If
...
Private Function SaveCheck() As Boolean
    ...
    Call CallPlugInDrugStuffWork(mobjPlugIn, 3, Val(txtStock.Tag), strNo, 单据号.盘点表)
    ...
    With vsfBill
        ...
        For n = 1 To recSort.RecordCount
            ...
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                '判断药品在库存中是否有记录
                bln有库存记录 = False
                If Get分批属性(lng库房ID, lng药品ID) = 0 Then '不分批
                    If CheckNoStock(lng库房ID, lng药品ID) = False Then bln有库存记录 = True
                Else '分批
                    If int新批次 = 1 Then '新批次
                    Else
                        bln有库存记录 = True
                    End If
                End If
                
                If Val(Split(.TextMatrix(intRow, mconIntCol指导差价率), "||")(1)) = 0 Or bln有库存记录 = True Then
                    '定价药品或不是新增批次取原始售价
...
Private Function GetPhysic(ByVal lng库房ID As Long, ByVal str盘点属性 As String, _
    ...
    gstrSQL = "SELECT Distinct A.药品ID,B.编码,B.名称,E.库房货位" & _
             ...
             IIf(bln盘无库存药品 = True, "", " And (Nvl(实际数量,0)<>0 Or Nvl(实际金额,0)<>0 Or Nvl(实际差价,0)<>0 )") & _
             IIf(bln盘无库存有金额药品 = True, " And 实际数量=0 And (实际金额<>0 Or 实际差价<>0)", "") & _
             ") D, "
    If bln汇总盘点单 Then
...
Private Function GetPhysicDetail(ByVal lng库房ID As Long, ByVal lng药品ID As Long, _
    ...
    gstrSQL = "" & _
        ...
        IIf(bln盘无库存药品 = True, "", " And (Nvl(A.实际数量,0)<>0 Or Nvl(A.实际金额,0)<>0 Or Nvl(A.实际差价,0)<>0 )") & _
        IIf(bln盘无库存有金额药品 = True, " And A.实际数量=0 And (A.实际金额<>0 Or A.实际差价<>0)", "") & _
...
Private Function CheckStopDrug(ByVal lng药品ID As Long) As Boolean
    '检查药品是否停用:停用-true，未停用-false
    '使用在用药品缓存数据集（grsUsingDrugs）来判断
    
    On Error GoTo errHandle
    
    With grsUsingDrugs
        .Filter = ""
        If .RecordCount = 0 Then CheckStopDrug = True: Exit Function '无在用药品
        
        .Filter = "药品ID = " & lng药品ID '过滤
        
        If .RecordCount = 0 Then '未在“在用药品”缓存中过滤出数据
            CheckStopDrug = True
        Else
            CheckStopDrug = False
        End If
