    ...
    With cboDate
        ...
        Select Case .Text
            Case "一个月内"
                ...
                mdatEndDate = zldatabase.Currentdate
            Case "三个月内"
...
Private Sub Form_Load()
    ...
    Call SetCBOValue
    dtpBeginDate.Value = DateAdd("d", -7, zldatabase.Currentdate)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        If KeyCode = vbKeyReturn Then
            If Len(txtNO) < 8 And Len(txtNO) > 0 Then
                txtNO.Text = GetFullNO(txtNO.Text, intNO, mlng库房id)
            End If
            Call cmdFind_Click
        End If
    End If
        ...
        If chkALLVisible1.Value = 1 Then '显示完整单据
            ...
            If strNo <> "" Then
                ...
                For i = 0 To UBound(arrNo)
                    ...
                    .TextMatrix(.rows - 1, .ColIndex("采购金额")) = GetFormat(dbl采购金额合计, mintMoneyDigit)
                    .TextMatrix(.rows - 1, .ColIndex("售价金额")) = GetFormat(dbl售价金额合计, mintMoneyDigit)
                    .TextMatrix(.rows - 1, .ColIndex("差价")) = GetFormat(dbl差价金额合计, mintMoneyDigit)
                    .TextMatrix(.rows - 1, .ColIndex("发票金额")) = GetFormat(dbl发票金额合计, mintMoneyDigit)
                Next
            End If
...
Private Sub SetDetailsData()
    ...
    With vsfList
        ...
        Do While Not mrsData.EOF
            ...
            vsfList.TextMatrix(vsfList.rows - 1, .ColIndex("数量")) = GetFormat(mrsData!实际数量 / str换算系数, mintNumberDigit) & "(" & str单位 & ")"
            vsfList.TextMatrix(vsfList.rows - 1, .ColIndex("采购价")) = GetFormat(mrsData!成本价 * str换算系数, mintCostDigit)
            vsfList.TextMatrix(vsfList.rows - 1, .ColIndex("采购金额")) = GetFormat(mrsData!成本金额, mintMoneyDigit)
            vsfList.TextMatrix(vsfList.rows - 1, .ColIndex("售价")) = GetFormat(mrsData!零售价 * str换算系数, mintPriceDigit)
            vsfList.TextMatrix(vsfList.rows - 1, .ColIndex("售价金额")) = GetFormat(mrsData!零售金额, mintMoneyDigit)
            vsfList.TextMatrix(vsfList.rows - 1, .ColIndex("差价")) = GetFormat(mrsData!差价, mintMoneyDigit)
            ...
            vsfList.TextMatrix(vsfList.rows - 1, .ColIndex("发票金额")) = IIf(dbl发票金额 = 0, "", GetFormat(dbl发票金额, mintMoneyDigit))
