...
Private Function GetAllDrug() As Boolean
    ...
    With mshBill
        ...
        Do While Not rsTmp.EOF
            ...
            .TextMatrix(.rows - 1, mconIntCol结存金额) = FormatEx(rsTmp!结存金额, gtype_UserSysParms.P9_费用金额保留位数)
            .TextMatrix(.rows - 1, mconIntCol结存差价) = FormatEx(rsTmp!结存差价, gtype_UserSysParms.P9_费用金额保留位数)
            Call ShowPercent(rsTmp.AbsolutePosition / rsTmp.RecordCount)
            rsTmp.MoveNext
        ...
        Select Case .Col
            Case mconIntCol实际差价
                ...
                If strkey <> "" Then
                    ...
                    strkey = GetFormat(strkey, gtype_UserSysParms.P9_费用金额保留位数)
                    .Text = strkey
