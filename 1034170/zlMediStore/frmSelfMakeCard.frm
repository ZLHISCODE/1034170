...
Private Const mconIntCol售价 As Integer = 13
Private Const mconIntCol售价金额 As Integer = 14
Private Const mconintCol差价 As Integer = 15
Private Const mconIntCol药品编码和名称 = 16
Private Const mconIntCol药品编码 = 17
Private Const mconIntCol药品名称 = 18
Private Const mconIntCol剂量系数 = 19
...
Private Const mconIntCol构药品编码和名称 = 17
Private Const mconIntCol构药品编码 = 18
Private Const mconIntCol构药品名称 = 19
Private Const mconIntCol构剂量系数 = 20
Private Const mconInt构ColS As Integer = 21             '总列数
'=========================================================================================
...
Private Sub Form_Load()
    ...
    mintSelectStock = Val(zldatabase.GetPara("是否选择库房", glngSys, 模块号.自制入库))
    ...
    Select Case mint编辑状态
        ...
        Case 2, 3, 4
            ...
            gstrSQL = " SELECT * FROM (SELECT DISTINCT 序号,A.药品ID,'[' || D.编码 || ']' As 药品编码, D.名称 As 通用名, E.名称 As 商品名,D.规格,A.产地, A.批号, A.效期," & _
                ...
                " B.基本药物,B.最大效期,A.对方部门ID,D.是否变价,B.指导差价率/100 AS 指导差价率,B.药房分批 As 药房分批核算,B.剂量系数 " & _
                ...
                If .TextMatrix(1, 0) <> "" Then
                    If SetStructure(.TextMatrix(1, 0)) <> False Then
                        If .TextMatrix(1, mconIntCol数量) <> "" Then
                            GetStructureNum .TextMatrix(1, mconIntCol数量) * .TextMatrix(1, mconIntCol比例系数), Val(.TextMatrix(1, mconIntCol剂量系数)), dblCostPrice, False
                        End If
                    End If
                End If
...
Private Sub mshbill_CommandClick()
    ...
    Select Case mintUnit
        Case mconint售价单位
            strUnitQuantity = "D.计算单位 AS 单位, trim(to_char(s.库存数量,'99999999990.00000')) AS 数量,'1' as 比例系数," _
                & "trim(to_char(p.现价,'99999999990.00000')),"
        Case mconint门诊单位
    ...
    gstrSQL = "" & _
        " SELECT DECODE(D.类别,5,'西成药',6,'中成药','中草药') AS 材质分类,D.剂型,D.编码,D.通用名称,D.商品名,D.规格,D.基本药物,D.产地,D.药品ID," & _
             strUnitQuantity & _
        "    S.库存金额, D.最大效期,D.是否变价,D.指导差价率/100 AS 指导差价率,D.药房分批核算,E.库房货位, D.剂量系数 " & _
        " FROM  " & _
        ...
        "        D.药库单位,TO_CHAR(D.药库包装, '999999999990.99999') 药库包装,M.是否变价,D.指导差价率,D.药房分批 AS 药房分批核算, D.剂量系数 " & _
            ...
            Select Case .Col
                    ...
                    If strkey <> "" Then
                        ...
                        ElseIf RecReturn.RecordCount = 1 Then
                            If SetColValue(.Row, RecReturn!药品id, "[" & RecReturn!编码 & "]", RecReturn!通用名称, IIf(IsNull(RecReturn!商品名), "", RecReturn!商品名), IIf(IsNull(RecReturn!规格), "", RecReturn!规格), _
                               ...
                               RecReturn!指导差价率, RecReturn!药房分批核算, RecReturn!剂量系数, "" & RecReturn!基本药物) = False Then
                               RecReturn.Close
                               Cancel = True
                               Exit Sub
                            End If
                ...
                Case mconIntCol采购价
                    ...
                    If strkey <> "" And strkey <> .TextMatrix(.Row, mconIntCol采购价) And .TextMatrix(.Row, mconIntCol数量) <> "" Then
                        .TextMatrix(.Row, mconIntCol采购金额) = GetFormat(.TextMatrix(.Row, mconIntCol数量) * strkey, mintMoneyDigit)
                ...
                Case mconIntCol采购金额
                    ...
                    If strkey <> "" And strkey <> .TextMatrix(.Row, mconIntCol采购金额) Then
                        If .TextMatrix(.Row, mconIntCol数量) <> "" Then
                            .TextMatrix(.Row, mconIntCol采购价) = GetFormat(strkey / .TextMatrix(.Row, mconIntCol数量), mintPriceDigit)
                        End If
                        ...
                        .TextMatrix(.Row, mconIntCol采购金额) = GetFormat(strkey, mintMoneyDigit)
                    End If
                    显示合计金额
                Case mconIntCol数量
                    ...
                    If strkey <> "" Then
                        ...
                        If GetStructureNum(strkey * Val(.TextMatrix(.Row, mconIntCol比例系数)), Val(.TextMatrix(.Row, mconIntCol剂量系数)), dblCostPrice) = False Then
                            Cancel = True
                            Exit Sub
                        Else
                            If mstrWay成本价 = "0" Then '根据原料药计算
                                .TextMatrix(.Row, mconIntCol采购价) = GetFormat(dblCostPrice * .TextMatrix(.Row, mconIntCol比例系数), mintPriceDigit)
                            Else    '根据最后一次入库成本价来
                                ...
                                If rsTemp.RecordCount = 0 Then
                                    Exit Sub
                                Else
                                    .TextMatrix(.Row, mconIntCol采购价) = GetFormat(IIf(IsNull(rsTemp!成本价), 0, rsTemp!成本价) * .TextMatrix(.Row, mconIntCol比例系数), mintPriceDigit)
                                End If
                            End If
                        End If
                        ...
                        If Split(.TextMatrix(.Row, mconIntCol原销期), "||")(2) = 1 Then
                            .TextMatrix(.Row, mconIntCol售价) = GetFormat(.TextMatrix(.Row, mconIntCol采购价) / (1 - Split(.TextMatrix(.Row, mconIntCol原销期), "||")(1)), mintPriceDigit)
                        End If
...
Private Function GetStructureNum(ByVal dblNum As Double, ByVal dbl剂量系数 As Double, ByRef dblCostPrice As Double, _
    ...
    With mshStructure
        For intRow = 1 To .rows - 1
            If .TextMatrix(intRow, 0) <> "" Then
                ...
                If mint库存检查 = 0 Then
                    If Split(.TextMatrix(intRow, mconintcol构指导差价率), "||")(1) = 1 Then
                        If mint编辑状态 = 1 Then
                            If dblConstruct > GetFormat(IIf(.TextMatrix(intRow, mconIntCol构可用数量) = "", 0, .TextMatrix(intRow, mconIntCol构可用数量)), intStruNumberDigit) Then
                                MsgBox "该自制药品的原料药“" & .TextMatrix(intRow, mconIntCol构药名) & "”可用库存数不够，请检查该原料药的库存！", vbInformation + vbOKOnly, gstrSysName
                                Exit Function
                            End If
                        ElseIf mint编辑状态 = 1 Or mint编辑状态 = 2 Then
                            ...
                            If dblConstruct > GetFormat(IIf(.TextMatrix(intRow, mconIntCol构可用数量) = "", 0, .TextMatrix(intRow, mconIntCol构可用数量)) + dbl原填写数量, intStruNumberDigit) Then
                                MsgBox "该自制药品的原料药“" & .TextMatrix(intRow, mconIntCol构药名) & "”可用库存数不够，请检查该原料药的库存！", vbInformation + vbOKOnly, gstrSysName
                                Exit Function
                            End If
                        End If
                ...
                ElseIf mint库存检查 = 1 Then
                        ...
                        If mint编辑状态 = 1 Then
                            If dblConstruct > GetFormat(IIf(.TextMatrix(intRow, mconIntCol构可用数量) = "", 0, .TextMatrix(intRow, mconIntCol构可用数量)), intStruNumberDigit) Then
                                If Split(.TextMatrix(intRow, mconintcol构指导差价率), "||")(1) = 1 Then
                                    ...
                                    Exit Function
                                Else
                                    intReturn = MsgBox("该自制药品的原料药“" & .TextMatrix(intRow, mconIntCol构药名) & "”可用库存数不够，是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName)
                    ...
                    End If
                ElseIf mint库存检查 = 2 And bln判断库存 = True Then
                    If dblConstruct > GetFormat(IIf(.TextMatrix(intRow, mconIntCol构可用数量) = "", 0, .TextMatrix(intRow, mconIntCol构可用数量)), intStruNumberDigit) Then
                        MsgBox "对不起，该自制药品的原料药“" & .TextMatrix(intRow, mconIntCol构药名) & "”可用库存数不够，请检查该原料药的库存！", vbInformation + vbOKOnly, gstrSysName
                        Exit Function
                    End If
                End If
                .TextMatrix(intRow, mconIntCol构数量) = GetFormat(dblConstruct, intStruNumberDigit)
                .TextMatrix(intRow, mconIntCol构售价金额) = GetFormat(dblConstruct * .TextMatrix(intRow, mconIntCol构售价), intStruMoneyDigit)
                If .TextMatrix(intRow, mconintcol构实际金额) <= "0" Then
'                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(.TextMatrix(intRow, mconIntCol构售价金额) * Split(.TextMatrix(intRow, mconintcol构指导差价率), "||")(0) / 100, intStruMoneyDigit)
'                    .TextMatrix(intRow, mconIntCol构采购价) = FormatEx((.TextMatrix(intRow, mconIntCol构售价金额) - .TextMatrix(intRow, mconintCol构差价)) / dblConstruct, intStruCostDigit)
                    .TextMatrix(intRow, mconIntCol构采购金额) = GetFormat(.TextMatrix(intRow, mconIntCol构采购价) * dblConstruct, intStruMoneyDigit)
                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(.TextMatrix(intRow, mconIntCol构售价金额) - .TextMatrix(intRow, mconIntCol构采购金额), intStruMoneyDigit)
                Else
'                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(.TextMatrix(intRow, mconIntCol构售价金额) * (.TextMatrix(intRow, mconintcol构实际差价) / .TextMatrix(intRow, mconintcol构实际金额)), intStruMoneyDigit)
'                    .TextMatrix(intRow, mconIntCol构采购价) = FormatEx((.TextMatrix(intRow, mconIntCol构售价金额) - .TextMatrix(intRow, mconintCol构差价)) / dblConstruct, intStruCostDigit)
                    .TextMatrix(intRow, mconIntCol构采购金额) = GetFormat(.TextMatrix(intRow, mconIntCol构采购价) * dblConstruct, intStruMoneyDigit)
                    .TextMatrix(intRow, mconintCol构差价) = GetFormat(.TextMatrix(intRow, mconIntCol构售价金额) - .TextMatrix(intRow, mconIntCol构采购金额), intStruMoneyDigit)
                End If
                dblPurchase = GetFormat(dblPurchase + .TextMatrix(intRow, mconIntCol构采购金额) / dblNum, intStruCostDigit)
...
Private Function SetColValue(ByVal intRow As Integer, ByVal int药品id As Long, ByVal str药品编码 As String, _
    ...
    ByVal int是否变价 As Integer, ByVal dbl指导差价率 As Double, ByVal int药房分批核算 As Integer, _
    ...
    With mshBill
        ...
        .TextMatrix(intRow, mconIntCol原销期) = IIf(IsNull(int原效期), "0", int原效期) & "||" & dbl指导差价率 & "||" & int是否变价 & "||" & int药房分批核算
...
Private Function SetStructure(ByVal int药品id As Long) As Boolean
    ...
    If mint编辑状态 <> 4 Then
        gstrSQL = "SELECT DISTINCT B.药品ID,'[' || F.编码 || ']' As 编码,F.名称 As 通用名称,E.名称 AS 商品名称," & _
            ...
            " (A.分子/A.分母) AS 组成,C.可用数量,B.指导差价率,F.是否变价,B.药房分批 药房分批核算, B.剂量系数,c.平均成本价 " & _
        ...
        gstrSQL = gstrSQL & " UNION " & _
            ...
            " (A.分子 / A.分母) AS 组成,C.可用数量,B.指导差价率,F.是否变价,B.药房分批 As 药房分批核算, B.剂量系数,c.平均成本价 " & _
        ...
        With mshStructure
            ...
            Do While Not rsStructure.EOF
                ...
                .TextMatrix(.Row, mconIntCol构售价) = GetFormat(rsStructure!售价, mintStruPriceDigit)
                .TextMatrix(.Row, mconIntCol构可用数量) = GetFormat(IIf(IsNull(rsStructure!可用数量), "0", rsStructure!可用数量), mintStruNumberDigit)
                ...
                .TextMatrix(.Row, mconIntCol构剂量系数) = rsStructure!剂量系数
                If IsNull(rsStructure!平均成本价) Then
                    ...
                    If rs成本价.RecordCount > 0 Then
                        .TextMatrix(.Row, mconIntCol构采购价) = GetFormat(rs成本价!成本价, mintStruCostDigit)
                    End If
                Else
                    .TextMatrix(.Row, mconIntCol构采购价) = GetFormat(rsStructure!平均成本价, mintStruCostDigit)
                End If
    ...
    Else            '查看
        ...
        With mshStructure
            ...
            Do While Not rsStructure.EOF
                ...
                .TextMatrix(.Row, mconIntCol构单位) = rsStructure!单位
                .TextMatrix(.Row, mconIntCol构数量) = GetFormat(rsStructure!实际数量, mintStruNumberDigit)
                .TextMatrix(.Row, mconIntCol构采购价) = GetFormat(rsStructure!成本价, mintStruCostDigit)
                .TextMatrix(.Row, mconIntCol构采购金额) = GetFormat(IIf(IsNull(rsStructure!成本金额), 0, rsStructure!成本金额), mintStruMoneyDigit)
                .TextMatrix(.Row, mconIntCol构售价) = GetFormat(rsStructure!零售价, mintStruPriceDigit)
                .TextMatrix(.Row, mconIntCol构售价金额) = GetFormat(IIf(IsNull(rsStructure!零售金额), 0, rsStructure!零售金额), mintStruMoneyDigit)
                .TextMatrix(.Row, mconintCol构差价) = GetFormat(IIf(IsNull(rsStructure!差价), 0, rsStructure!差价), mintStruMoneyDigit)
                .TextMatrix(.Row, mconintcol构药品id) = rsStructure!药品id
...
Private Function SaveCard() As Boolean
    ...
    With mshBill
        ...
        gstrSQL = "zl_药品自制原料出库_insert('" & chrNo & "'," & lng制剂室 & ")"
        ReDim Preserve arrSql(UBound(arrSql) + 1)
        arrSql(UBound(arrSql)) = gstrSQL
