...
Private Sub Cmd供应商_Click()
    ...
    vRect = GetControlRect(txt供应商.hWnd) '获取位置
    
    gstrSQL = "select id,编码,名称,简码 from 供应商 Where (站点 = '-' Or 站点 Is Null) And" & _
...
Private Sub Form_Load()
    ...
    Me.Caption = "核对发票"
    dtp开始时间.Value = DateAdd("d", -7, zldatabase.Currentdate)
...
Private Sub SetColumn(ByVal rsRecord As ADODB.Recordset)
    ...
    With vsf未标记
        ...
        For i = 1 To rsRecord.RecordCount
            ...
            Select Case mint单位系数
                Case 4  '售价单位
                    .TextMatrix(i, mColumnMark.数量) = GetFormat(IIf(IsNull(rsRecord!数量), 0, rsRecord!数量), mintShowNumberDigit)
                    .TextMatrix(i, mColumnMark.采购价) = GetFormat(rsRecord!采购价, mintShowCostDigit)
                Case 1  '药库单位
                    .TextMatrix(i, mColumnMark.数量) = GetFormat(IIf(IsNull(rsRecord!数量), 0, rsRecord!数量 / IIf(IsNull(rsRecord!药库包装), 1, rsRecord!药库包装)), mintShowNumberDigit)
                    .TextMatrix(i, mColumnMark.采购价) = GetFormat(rsRecord!采购价 * IIf(IsNull(rsRecord!药库包装), 1, rsRecord!药库包装), mintShowCostDigit)
                Case 2  '门诊单位
                    .TextMatrix(i, mColumnMark.数量) = GetFormat(IIf(IsNull(rsRecord!数量), 0, rsRecord!数量 / IIf(IsNull(rsRecord!门诊包装), 1, rsRecord!门诊包装)), mintShowNumberDigit)
                    .TextMatrix(i, mColumnMark.采购价) = GetFormat(rsRecord!采购价 * IIf(IsNull(rsRecord!门诊包装), 1, rsRecord!门诊包装), mintShowCostDigit)
                Case 3  '住院单位
                    .TextMatrix(i, mColumnMark.数量) = GetFormat(IIf(IsNull(rsRecord!数量), 0, rsRecord!数量 / IIf(IsNull(rsRecord!住院包装), 1, rsRecord!住院包装)), mintShowNumberDigit)
                    .TextMatrix(i, mColumnMark.采购价) = GetFormat(rsRecord!采购价 * IIf(IsNull(rsRecord!住院包装), 1, rsRecord!住院包装), mintShowCostDigit)
            End Select
            
            .TextMatrix(i, mColumnMark.采购金额) = GetFormat(rsRecord!采购金额, mintShowMoneyDigit)
...
'            .TextMatrix(i, mColumnMark.全院库存) = Get全院库存(rsRecord!项目Id)
            ...
            .TextMatrix(i, mColumnMark.发票金额) = GetFormat(IIf(IsNull(rsRecord!发票金额), "", rsRecord!发票金额), mintShowMoneyDigit)
...
Private Sub txt供应商_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        ...
        gstrSQL = "select id,编码,名称,简码 from 供应商 where 编码 like [1] OR 名称 like [1] OR 简码 like [1]"
        Set rsRecord = zldatabase.ShowSQLSelect(Me, gstrSQL, 0, "供应商", False, "", "", False, False, _
...
Private Sub txt药品名称_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    vRect = GetControlRect(txt药品名称.hWnd) '获取位置
    dblLeft = vRect.Left
           ...
           If Not rsTemp Is Nothing Then
                With vsf库存
                    ...
                    For i = 1 To rsTemp.RecordCount
                        ...
                        Select Case mint单位系数
                            Case 4  '售价单位
                                .TextMatrix(1, i) = GetFormat(IIf(IsNull(rsTemp!数量), 0, rsTemp!数量), mintShowNumberDigit)
                            Case 1  '药库单位
                                .TextMatrix(1, i) = GetFormat(IIf(IsNull(rsTemp!数量), 0, rsTemp!数量 / int药库系数), mintShowNumberDigit)
                            Case 2  '门诊单位
                                .TextMatrix(1, i) = GetFormat(IIf(IsNull(rsTemp!数量), 0, rsTemp!数量 / int门诊系数), mintShowNumberDigit)
                            Case 3  '住院单位
                                .TextMatrix(1, i) = GetFormat(IIf(IsNull(rsTemp!数量), 0, rsTemp!数量 / int住院系数), mintShowNumberDigit)
                        End Select
                        rsTemp.MoveNext
                    Next
