...
Private Function ReFreshDrugData(ByVal lngDeptId As Long, _
    ...
    Else
        Select Case intChoose级数
        Case 1
            gstrSQL = ",A.计算单位 as 单位,Nvl(m.成本价,0) as 上次采购价,nvl(P.现价,0) as 当前售价,nvl(M.剂量系数,0) as 系数1,Sum(S.可用数量) as 可用数量, Sum(S.实际数量) as 实际数量,sum(i.下限) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,1 as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
        ...
        Case 2
            gstrSQL = ",M.门诊单位 as 单位,Nvl(m.成本价*nvl(M.门诊包装,0),0) as 上次采购价,nvl(P.现价,0)*nvl(M.门诊包装,0) as 当前售价,nvl(M.门诊包装,0) as 系数1,Sum(S.可用数量 /Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 可用数量, Sum(S.实际数量 /Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 实际数量,sum(i.下限/Decode(M.门诊包装,0,1,null,1,M.门诊包装)) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价)*Decode(M.门诊包装,0,1,null,1,M.门诊包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.门诊包装,0,1,null,1,M.门诊包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
        ...
        Case 3
            gstrSQL = ",M.药库单位 as 单位,Nvl(m.成本价*nvl(M.药库包装,0),0) as 上次采购价,nvl(P.现价,0)*nvl(M.药库包装,0) as 当前售价,nvl(M.药库包装,0) as 系数1,Sum(S.可用数量 /Decode(M.药库包装,0,1,null,1,M.药库包装)) as 可用数量,Sum(S.实际数量 /Decode(M.药库包装,0,1,null,1,M.药库包装)) as 实际数量,sum(i.下限/Decode(M.药库包装,0,1,null,1,M.药库包装)) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价)*Decode(M.药库包装,0,1,null,1,M.药库包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.药库包装,0,1,null,1,M.药库包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
        ...
        Case 4
            gstrSQL = ",M.住院单位 as 单位,Nvl(m.成本价*nvl(M.住院包装,0),0) as 上次采购价,nvl(P.现价,0)*nvl(M.住院包装,0) as 当前售价,nvl(M.住院包装,0) as 系数1,Sum(S.可用数量 /Decode(M.住院包装,0,1,null,1,M.住院包装)) as 可用数量, Sum(S.实际数量 /Decode(M.住院包装,0,1,null,1,M.住院包装)) as 实际数量,sum(i.下限/Decode(M.住院包装,0,1,null,1,M.住院包装)) as 下限,Sum(S.实际金额) as 实际金额,Sum(S.实际差价) as 实际差价,Nvl(s.平均成本价,m.成本价)*Decode(M.住院包装,0,1,null,1,M.住院包装) 平均成本价,Decode(To_Char(A.撤档时间,'yyyy-MM-dd'),'3000-01-01',' ',To_Char(A.撤档时间,'yyyy-MM-dd')) 撤档时间,Decode(M.住院包装,0,1,null,1,M.住院包装) as 除数, Nvl(A.是否变价, 0) 变价, G.名称 As 上次供应商, C.库房货位 "
