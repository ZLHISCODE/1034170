...
Begin VB.Form frmCheckClass 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Tab(1).Control(0)=   "chk货位"
      Tab(1).Control(0).Enabled=   0   'False
      Tab(1).Control(1)=   "vsfStock"
   ...
   Begin MSComctlLib.ImageList img16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmCheckClass.frx":6949
            Key             =   ""
         EndProperty
      EndProperty
   End
...
Private Sub Load库房货位()
    ...
    If chk货位.Value = 1 Then
        gstrSQL = "Select Distinct Level As 级数, 编码, 名称, 上级, 末级 " & _
            " From (Select Distinct B.编码, B.名称, B.上级, B.末级 " & _
            " From 药品储备限额 A, 药品库房货位 B " & _
            " Where A.库房货位 = B.名称 And A.库房id = [1] And B.末级 = 1 " & _
            " Union All " & _
            " Select 编码, 名称, 上级, 末级 " & _
            " From 药品库房货位 " & _
            " Where 编码 In (Select Distinct B.上级 " & _
            " From 药品储备限额 A, 药品库房货位 B " & _
            " Where A.库房货位 = B.名称 And A.库房id = [1] And B.末级 = 1)) " & _
            " Start With 上级 Is Null " & _
            " Connect By Prior 编码 = 上级 " & _
            " Order By Level, 编码 "
    Else
    gstrSQL = "Select Distinct Level As 级数, 编码, 名称, 上级, 末级" & _
        " From 药品库房货位 " & _
    ...
    With vsfStock
        ...
        '添加“所有库房”
        .rows = .rows + 1
        .TextMatrix(.rows - 1, .ColIndex("级数")) = 1
        .TextMatrix(.rows - 1, .ColIndex("上级")) = ""
        .TextMatrix(.rows - 1, .ColIndex("名称")) = "所有库房"
        .TextMatrix(.rows - 1, .ColIndex("选择")) = 0
        .TextMatrix(.rows - 1, .ColIndex("编码")) = "所有"
        ...
        Do While Not rsClone.EOF
            ...
            .TextMatrix(.rows - 1, .ColIndex("上级")) = Nvl(rsClone!上级, "所有")
        ...
        For lngRow = 1 To .rows - 1
            .RowOutlineLevel(lngRow) = Val(.TextMatrix(lngRow, .ColIndex("级数")))
            .IsSubtotal(lngRow) = True

            If Val(.TextMatrix(lngRow, .ColIndex("级数"))) > intLevel Then
                intLevel = Val(.TextMatrix(lngRow, .ColIndex("级数")))
            End If
        Next
        
        .OutlineCol = .ColIndex("Line")
        .OutlineBar = flexOutlineBarSimpleLeaf
        
        .ColWidth(.ColIndex("Line")) = 300 + (intLevel - 1) * 125
...
Public Sub ShowME(ByVal frmCard As frmCheckCourseCard, ByVal lng库房ID As Long, ByRef str货位 As String, ByRef str药品ID As String, ByRef int退出 As Integer)
    mBlnClick = False
        ...
        If InStr(1, mstrSelRows, ",") > 0 Then
            ...
            For lngRows = 1 To .rows - 1
                If InStr(1, "," & mstrSelRows & ",", "," & lngRows & ",") > 0 Then
                    ...
                    '子节点处理
                    Call CheckItem(.TextMatrix(lngRows, .ColIndex("编码")), IntCheck)
                End If
            Next
