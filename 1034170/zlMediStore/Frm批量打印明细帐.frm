...
Private Sub Form_Load()
    ...
    gstrSQL = "Select distinct a.ID,a.编码,a.名称 From 部门表 a,部门性质说明 b,部门性质分类 C " & _
              ...
              IIf(IsHavePrivs(gstrStockSearchPrivs, "所有库房"), "", " And A.id In (Select 部门ID From 部门人员 Where 人员ID=[1]) ") & _
    ...
    With rs用途分类
        ...
        Cbo库房.Clear
        If IsHavePrivs(gstrStockSearchPrivs, "所有库房") Then Cbo库房.AddItem "所有库房"
        Do While Not .EOF
            Cbo库房.AddItem !名称
    ...
    Me.Dtp开始日期.Value = Format(DateAdd("m", -1, zldatabase.Currentdate), "yyyy年MM月dd日")
    Me.Dtp结束日期.Value = Format(zldatabase.Currentdate, "yyyy年MM月dd日")
    Dtp开始日期.MaxDate = Format(zldatabase.Currentdate, "yyyy年MM月dd日")
    Dtp结束日期.MaxDate = Format(zldatabase.Currentdate, "yyyy年MM月dd日")
    Me.Txt用途分类 = ""
    Me.Txt用途分类.Tag = ""
    
    '打开药品用途分类
    gstrSQL = "": Str材质 = ""
    If IsHavePrivs(gstrStockSearchPrivs, "西成药") Then
        Str材质 = Str材质 & ",'西成药'"
        gstrSQL = gstrSQL & _
                  " Select -1 id,'1' 编码,'西成药' 名称,to_number(NULL,0) 上级ID,-1 末级 from dual" & _
                  " Union all"
    End If
    If IsHavePrivs(gstrStockSearchPrivs, "中成药") Then
        Str材质 = Str材质 & ",'中成药'"
        gstrSQL = gstrSQL & _
                " Select -2 id,'3' 编码,'中成药' 名称,to_number(NULL,0) 上级ID,-1 末级 from dual" & _
                " Union all"
    End If
    If IsHavePrivs(gstrStockSearchPrivs, "中草药") Then
        Str材质 = Str材质 & ",'中草药'"
        gstrSQL = gstrSQL & _
