...
Private Sub bfgPrice_EnterCell(Row As Long, Col As Long)
    Select Case Col
    Case conCol收入名称
        Me.bfgPrice.TextMatrix(Row, conCol现价) = FormatEx(Me.bfgPrice.TextMatrix(Row, conCol现价), 7)
    End Select
            ...
            If lngMediId <> 0 And dblOldPrice <> dblNewPrice Then
                ...
                With rsTemp
                    ...
                    Do While Not .EOF
                        ...
                        Me.hgdStore.TextMatrix(intRows + .AbsolutePosition - 1, 6) = FormatEx(dblOldPrice, mconintPriceBit)
                        Me.hgdStore.TextMatrix(intRows + .AbsolutePosition - 1, 7) = FormatEx(dblNewPrice, mconintPriceBit)
...
Private Sub cmdPrint_Click()
    ...
    Set objRow = New zlTabAppRow
    objRow.Add "执行时间:" & Format(IIf(Me.chkImmediately.Value, zldatabase.Currentdate, Me.dtpRunDate.Value), "yyyy年MM月DD日 HH:mm:ss")
...
Private Sub Form_Load()
    ...
    If lngBillId = 0 Then
        ...
        Me.dtpRunDate.MinDate = DateAdd("s", 1, Format(zldatabase.Currentdate, "yyyy-MM-dd"))
        Me.dtpRunDate.Value = DateAdd("d", 1, Format(zldatabase.Currentdate, "YYYY-MM-DD"))
    ...
    Else
        ...
        Do While Not rsTemp.EOF
            ...
            Me.bfgPrice.TextMatrix(rsTemp.AbsolutePosition, conCol原价) = FormatEx(rsTemp!原价, mconintPriceBit)
            Me.bfgPrice.TextMatrix(rsTemp.AbsolutePosition, conCol现价) = FormatEx(rsTemp!现价, mconintPriceBit)
        ...
        Else
            ...
            Do While Not rsTemp.EOF
                ...
                Me.hgdStore.TextMatrix(rsTemp.AbsolutePosition, 6) = FormatEx(rsTemp!原价, mconintPriceBit)
                Me.hgdStore.TextMatrix(rsTemp.AbsolutePosition, 7) = FormatEx(rsTemp!现价, mconintPriceBit)
...
'
    ...
    With rsTemp
        If .RecordCount > 0 Then
            ...
            Me.bfgPrice.TextMatrix(Me.bfgPrice.Row, conCol原价) = FormatEx(!现价, mconintPriceBit)
            Me.bfgPrice.TextMatrix(Me.bfgPrice.Row, conCol现价) = FormatEx(!现价, mconintPriceBit)
