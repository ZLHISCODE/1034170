    ...
    With mshBill
        For n = 1 To .rows - 1
            If Val(.TextMatrix(n, 0)) <> 0 Then
                ...
                str名称 = .TextMatrix(n, mconIntCol药名)
...
'                num成本价 = FormatEx(.TextMatrix(intRow, mconIntCol采购价) / .TextMatrix(intRow, mconIntCol比例系数), gtype_UserDrugDigits.Digit_成本价)
    ...
    Call CallPlugInDrugStuffWork(mobjPlugIn, 3, lng库房ID, strNo, 单据号.其他出库)
        ...
        If .TextMatrix(1, 0) <> "" Then         '先判有否数据
            ...
            For intLop = 1 To .rows - 1
                If Trim(.TextMatrix(intLop, mconIntCol药名)) <> "" Then
                    ...
                    If Not CompareUsableQuantity(intLop, Val(.TextMatrix(intLop, mconIntCol数量))) Then
                        mshBill.SetFocus
                        .Row = intLop
                        .MsfObj.TopRow = intLop
                        .Col = mconIntCol数量
...
Private Function CompareUsableQuantity(ByVal intRow As Integer, ByVal dbl填写数量 As Double) As Boolean
    ...
    With mshBill
        ...
        '获取库存可用数量
        gstrSQL = "select 可用数量/" & Val(.TextMatrix(intRow, mconIntCol比例系数)) & " as  可用数量   from 药品库存 where 库房id=[1] " _
            & " and 药品id=[2] " _
            & " and 性质=1 and " _
            & " nvl(批次,0)=[3]"
...
Private Function CheckDrugStock(ByVal str药品名称 As String, ByVal lng药品ID As Long, ByVal lng批次 As Long, ByVal Dbl药品数量 As Double, Optional ByVal intRow As Integer) As Boolean
    ...
    If Dbl药品数量 > Dbl数量 Then
        blnMsg = True
        '时价或分批药品，按不足禁止检查
        If (Get分批属性(lng库房ID, lng药品ID) = 1) Or Split(mshBill.TextMatrix(intRow, mconIntCol指导差价率), "||")(1) = 1 Then int库存检查 = 2
    End If
    
    If blnMsg Then
        If int库存检查 = 1 Then        '不足提醒
            If MsgBox("药品【" & str药品名称 & "】领用数量大于现有的库存数量（当前库存数量为：" & Dbl数量 / Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)) & "），是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                Exit Function
            End If
        ElseIf int库存检查 = 2 Then                            '不足禁止
            MsgBox "药品【" & str药品名称 & "】出库数量大于现有的库存数量（当前库存数量为：" & Dbl数量 / Val(mshBill.TextMatrix(intRow, mconIntCol比例系数)) & "），不能出库！", vbInformation, gstrSysName
            Exit Function
        End If
    End If
    CheckDrugStock = True
