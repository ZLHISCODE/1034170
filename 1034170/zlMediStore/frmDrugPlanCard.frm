...
Private mint计划时间 As Integer             '0-默认下期计划；1-本期计划
...
Private mint计算库存数量时方式 As Integer         '0-采用实际数量，1-采用可用数量
...
Private Sub GegReg()
    ...
    mintPlanPoint = Val(zlDatabase.GetPara("全院计划不管站点", glngSys, 模块号.药品计划, 0))
    mint计算库存数量时方式 = Val(zlDatabase.GetPara("计算库存数量时方式", glngSys, 模块号.药品计划))
End Sub
...
Public Sub ShowCard(FrmMain As Form, ByVal str单据号 As String, _
        ByVal int编辑状态 As Integer, Optional blnSuccess As Boolean = False, Optional lng库房ID As Long, Optional int计划时间 As Integer = 0)
    mblnSave = False
    mblnSuccess = False
    mstr单据号 = str单据号
    mint编辑状态 = int编辑状态
...
Private Sub Form_Activate()
    ...
    If mint编辑状态 = 1 Then
        ...
        If frmDrugPlanCondition.GetCondition(mfrmMain, str用途ID, str剂型编码, lng库房ID, int计划类型, _
                ...
                strToxicologyClass, mbln按销量产生计划, mstr来源药房, mstr来源库房, mstrAll来源药房, mstrAll来源库房, mint计划时间) = True Then
            mlng库房ID = lng库房ID
            mint计划类型 = int计划类型
            mint编制方法 = int编制方法
            mbln数量方式 = bln数量方式
            ...
            Select Case mint计划类型
                Case 1       '月计划
                    ...
                    Else
                        mstrNow = Format(DateAdd("m", -1, zlDatabase.Currentdate), "yyyy-mm-dd")
                    End If
                    
                    Str期间 = Format(DateAdd("m", 1, mstrNow), "yyyyMM")
                ...
                Case 2       '季计划
                    If mint计划时间 = 0 Then
                        mstrNow = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                    Else
                        mstrNow = Format(DateAdd("Q", -1, zlDatabase.Currentdate), "yyyy-mm-dd")
                    End If
                    
                    intMonth = Month(DateAdd("Q", 1, mstrNow))
                    Str期间 = Format(DateAdd("Q", 1, mstrNow), "yyyy") & IIf(intMonth <= 3, 1, IIf(intMonth >= 10, 4, IIf(intMonth <= 9 And intMonth >= 7, 3, 2)))
                ...
                Case 3      '年计划
                    If mint计划时间 = 0 Then
                        mstrNow = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                    Else
                        mstrNow = Format(DateAdd("YYYY", -1, zlDatabase.Currentdate), "yyyy-mm-dd")
                    End If
                    
                    Str期间 = Format(DateAdd("yyyy", 1, mstrNow), "yyyy")
                ...
                Case 4      '周计划
                    If mint计划时间 = 0 Then
                        mstrNow = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                    Else
                        mstrNow = Format(DateAdd("d", -7, zlDatabase.Currentdate), "yyyy-mm-dd")
                    End If
                    
                    mdatThisMondyDate = zlCommFun.GetWeekBase(mstrNow)
...
Private Sub ReFreshALLDrug(ByVal str用途ID, ByVal str剂型 As String, _
    ...
    If lng库房ID = 0 Then
        ...
        gstrSQL = "( " & gstrSQL & ") D," & _
                  ...
                  " From (Select 药品id, Sum(" & IIf(mint计算库存数量时方式 = 0, "实际数量", "可用数量") & ") As 库存数量, " & _
    ...
    Else
        ...
        gstrSQL = gstrSQL & " Group By 药品id) A, " & _
                  " (Select 药品id, max(上次供应商id) as 上次供应商id, max(上次产地) as 上次产地 From 药品库存 " & _
...
Private Function GetDate(ByVal int模式 As Integer, ByVal datCurrent As Date, _
    ...
    Case 1
        ...
        strEnd = Format(rsdate.Fields(0), "yyyy-mm-dd") & " 23:59:59"
        rsdate.Close
    Case 2
        Select Case DatePart("Q", datCurrent)
            Case 1
                strBegin = Year(datCurrent) & "-01-01"
                strEnd = Year(datCurrent) & "-03-31 23:59:59"
            Case 2
                strBegin = Year(datCurrent) & "-04-01"
                strEnd = Year(datCurrent) & "-06-30 23:59:59"
            Case 3
                strBegin = Year(datCurrent) & "-07-01"
                strEnd = Year(datCurrent) & "-09-30 23:59:59"
            Case 4
                strBegin = Year(datCurrent) & "-10-01"
                strEnd = Year(datCurrent) & "-12-31 23:59:59"
        End Select
    Case 3
        strBegin = Year(datCurrent) & "-01-01"
        strEnd = Year(datCurrent) & "-12-31 23:59:59"
    Case 4
        ...
        strEnd = Format(DateAdd("d", 6, datCurrent), "yyyy-mm-dd") & " 23:59:59"
    End Select
    GetDate = True
    Exit Function
errHandle:
    ...
    With mshBill
        ...
        If mlng库房ID = 0 Then
            ...
            gstrSQL = "Select B.名称 供应商, C.上次产地, Nvl(A.库存数量,0) 库存数量" & _
                      " From (Select 药品id, Sum(" & IIf(mint计算库存数量时方式 = 0, "实际数量", "可用数量") & ") As 库存数量 From 药品库存 " & _
        ...
        Else
            ...
            gstrSQL = " Select C.名称 As 供应商, B.上次产地, A.库存数量 " & _
                      " From (Select 库房id, 药品id, Sum(" & IIf(mint计算库存数量时方式 = 0, "实际数量", "可用数量") & ") As 库存数量 " & _
                      " From 药品库存 " & _
