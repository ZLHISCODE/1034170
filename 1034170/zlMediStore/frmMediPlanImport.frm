...
Private Sub cmdImport_Click()
    ...
    With rsTmp
        ...
        Do While Not .EOF
            ...
            '成本价
            strInsert = strInsert & "," & Round(rsTmp!成本价 / rsTmp!药库包装, gtype_UserDrugDigits.Digit_成本价)
            '成本金额
            strInsert = strInsert & "," & Round(rsTmp!成本金额, mintMoneyDigit)
            '扣率
            strInsert = strInsert & ",100"
            '零售价
            strInsert = strInsert & "," & Round(rsTmp!零售价 / rsTmp!药库包装, gtype_UserDrugDigits.Digit_零售价)
                ...
                If IIf(IsNull(rsVal!是否变价), 0, rsVal!是否变价) = 1 Then
                    ...
                    Else
                        dblTmp = dblTmp _
                               ...
                               * (1 - (IIf(IsNull(rsVal!差价让利比), 100, rsVal!差价让利比) / 100))
                    End If
                Else
                    '定价
