...
Private mblnActive As Boolean '是否已经进入过Form_Activate过程
...
Private Function Check退库(ByVal lng药品ID As Long, ByVal lng批次 As Long, ByVal Dbl数量 As Double, Optional ByVal intRow As Integer) As Boolean
    ...
    If blnMsg Then
        If rsCheck.RecordCount <> 0 Then
            If mint库存检查 = 1 Then
                If MsgBox("【" & mshBill.TextMatrix(intCurRow, mconIntCol药名) & "】退货数量大于现有的库存数量（当前库存数量为：" & Abs(dbl可用数量) / Val(mshBill.TextMatrix(intCurRow, mconIntCol比例系数)) & "）！是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else
                MsgBox "【" & mshBill.TextMatrix(intCurRow, mconIntCol药名) & "】退货数量不能大于现有的库存数量（当前库存数量为：" & Abs(dbl可用数量) / Val(mshBill.TextMatrix(intCurRow, mconIntCol比例系数)) & "）！", vbInformation, gstrSysName
                Exit Function
            End If
        Else
            If mint库存检查 = 1 Then
                If MsgBox("【" & mshBill.TextMatrix(intCurRow, mconIntCol药名) & "】退货数量大于现有的库存数量（当前库存数量为：0）！是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                    Exit Function
                End If
            Else
                MsgBox "【" & mshBill.TextMatrix(intCurRow, mconIntCol药名) & "】退货数量不能大于现有的库存数量（当前库存数量为：0）！", vbInformation, gstrSysName
                Exit Function
            End If
        End If
    End If
...
Private Sub Form_Activate()
    ...
    End If
    If mblnActive = False And (mint编辑状态 = 2 Or mint编辑状态 = 3) Then '修改或审核将焦点从供药单位移到单据上
        mshBill.SetFocus
        mblnActive = True
    End If
End Sub
...
Private Function SaveCheck(Optional ByVal strNo As String = "", Optional ByVal blnTrans As Boolean = False) As Boolean
    ...
    Call CallPlugInDrugStuffWork(mobjPlugIn, 3, Val(cboStock.ItemData(cboStock.ListIndex)), txtNO.Tag, 单据号.外购入库)
...
Private Function SaveStrike() As Boolean
    ...
    With mshBill
        ...
        '检查库存 防止可以数量大于库存数量，冲销后库存数量为负（财务审核时的冲销，退库时冲销也不检查）
        If mint库存检查 <> 0 And mint编辑状态 <> 7 And mbln退货 = False Then
