...
Private Sub GetList(ByVal StrFind As String)
    ...
    If TabShow.Tab = 1 Then
        ...
        gstrSQL = "Select NO, 盘点时间, 填制人, 填制日期, 审核人, 审核日期, " & _
                ...
                & "ltrim(to_char(to_char((A.扣率-A.填写数量) /" & str包装系数 & "," & mstrNumberFormat & ") * TO_CHAR (a.零售价* Decode(记录状态, 1, 1, Decode(Mod(记录状态, 3), 0, 1, -1))*" & str包装系数 & ", " _
                & mstrPriceFormat & "),decode(a.扣率,0,'999999999990.00000'," & mstrMoneyFormat & "))) AS 账面金额差," _
...
Private Sub Form_Activate()
    ...
    If mbln盘停用药品 = False Then SetUsingDrugsRS '加载“在用药品”数据集用于检查是否存在停用药品
...
Private Function CheckStockNum(ByVal intType As Integer) As Boolean
    ...
    If int库存检查 <> 0 Then
        gstrSQL = "Select A.药品信息 " & _
            " From (Select Distinct '(' || I.编码 || ')' || Nvl(N.名称, I.名称) As 药品信息, A.实际数量, Nvl(K.可用数量, 0) As 可用数量, Nvl(K.实际数量, 0) As 库存数量 " & _
            " From 药品收发记录 A, (Select 药品id, 库房id, 可用数量, 实际数量, Nvl(批次, 0) 批次 From 药品库存 Where 性质 = 1) K, 药品规格 B, 收费项目目录 I, 收费项目别名 N " & _
            ...
            " Where " & IIf(intType = 0, " A.实际数量 > A.可用数量 ", "A.实际数量 > A.库存数量")
        ...
        With rstemp
            If .RecordCount > 0 Then
                ...
                If int库存检查 = 1 Then
                    If MsgBox("注意，以下药品" & IIf(intType = 0, "可用", "实际") & "库存不足：" & vbCrLf & strMsg & vbCrLf & Space(4) & "是否继续？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Function
                ElseIf int库存检查 = 2 Then
                    MsgBox "对不起，以下药品" & IIf(intType = 0, "可用", "实际") & "库存不足，不能冲销！" & vbCrLf & strMsg, vbInformation, gstrSysName
                    Exit Function
                End If
            End If
        End With
    ...
    If vsfList.Row >= 1 And LTrim(vsfList.TextMatrix(vsfList.Row, 0)) <> "" Then
        ...
        If TabShow.Tab = 1 Then
            gstrSQL = "Select DISTINCT a.序号" & strSql药名 & "," _
                    ...
                    & "     LTRIM(TO_CHAR (to_char((A.扣率-A.填写数量) /" & str包装系数 & "," & mstrNumberFormat & ") * TO_CHAR (a.零售价* Decode(记录状态, 1, 1, Decode(Mod(记录状态, 3), 0, 1, -1))*" _
                    & str包装系数 & ", " & mstrPriceFormat & "),decode(a.扣率,0,'999999999990.00000'," & mstrMoneyFormat & "))) AS 账面金额差," _
