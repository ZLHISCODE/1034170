...
Begin VB.Form frmMediPriceSearch 
   ...
   ClientTop       =   375
   ClientWidth     =   6660
   Icon            =   "frmMediPriceSearch.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   3960
   ...
   End
End
Attribute VB_Name = "frmMediPriceSearch"
...
Private Sub Cmd确定_Click()
    ...
    If Val(Txt药品.Tag) <> 0 Then
        If mstrResult = "" Then
            mstrResult = " a.调价号 In (Select 调价汇总号 From 收费价目 Where 收费细目id = " & Txt药品.Tag & _
                            " union all " & _
                         " Select  调价汇总号 From 成本价调价信息 Where 药品id = " & Txt药品.Tag & ")"
        Else
            mstrResult = mstrResult & " and a.调价号 In (Select 调价汇总号 From 收费价目 Where 收费细目id =" & Txt药品.Tag & _
                        " union all " & _
                         " Select  调价汇总号 From 成本价调价信息 Where 药品id = " & Txt药品.Tag & ")"
        End If
    End If
...
Private Sub Cmd药品_Click()
    ...
    End If
    Set RecReturn = frmSelector.showMe(Me, 0, 1, , , , , , , , , , , , False)
...
Private Sub Txt药品_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    sngLeft = Me.Left + fra范围.Left + Txt药品.Left
    sngTop = Me.Top + fra范围.Top + Txt药品.Top + Txt药品.Height + Me.Height - Me.ScaleHeight '  50
...
'    Set RecReturn = Frm药品多选选择器.ShowME(Me, 1, , mfrmMain.cboStock.ItemData(mfrmMain.cboStock.ListIndex), mfrmMain.cboStock.ItemData(mfrmMain.cboStock.ListIndex), strkey, sngLeft, sngTop)
    Set RecReturn = frmSelector.showMe(Me, 1, 1, strkey, sngLeft, sngTop, , , , , , , , , False)
