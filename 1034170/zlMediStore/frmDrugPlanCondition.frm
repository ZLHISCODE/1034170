...
Public Function GetCondition(FrmMain As Form, ByRef str用途ID, ByRef str剂型 As String, _
    ...
    ByRef bln按销量产生计划 As Boolean, ByRef str来源药房 As String, ByRef str来源库房 As String, Optional ByRef strAll来源药房 As String, Optional ByRef strAll来源库房 As String, Optional int计划时间 As Integer = 0) As Boolean
...
Private Sub opt计划_Click(Index As Integer)
    ...
    Case 0 '月计划
        If mint计划时间 = 0 Then
            ...
            lbl注意事项.Caption = "计划周期：" & Str期间
        Else
            Str期间 = Format(zlDatabase.Currentdate, "yyyyMM")
            lbl注意事项.Caption = "计划周期(增补)：" & Str期间
        End If
    Case 1 '季计划
        If opt方法(3).Value Then
            opt方法(3).Value = False
        ...
        Else
            ...
            Str期间 = Format(zlDatabase.Currentdate, "yyyy") & IIf(intMonth <= 3, 1, IIf(intMonth >= 10, 4, IIf(intMonth <= 9 And intMonth >= 7, 3, 2)))
            lbl注意事项.Caption = "计划周期(增补)：" & Str期间
        End If
    ...
    Case 2 '年计划
        ...
        If mint计划时间 = 0 Then
            ...
            lbl注意事项.Caption = "计划周期：" & Str期间
        Else
            Str期间 = Format(zlDatabase.Currentdate, "yyyy")
            lbl注意事项.Caption = "计划周期(增补)：" & Str期间
        End If
    Case 3 '周计划
        ...
        datCurrDate = zlCommFun.GetWeekBase(datCurrDate)
        If mint计划时间 = 0 Then
            Str期间 = Format(DateAdd("d", 7, datCurrDate), "yyyyMMDD")
            lbl注意事项.Caption = "计划周期：" & Str期间
        Else
            Str期间 = Format(datCurrDate, "yyyyMMDD")
            lbl注意事项.Caption = "计划周期(增补)：" & Str期间
        End If
