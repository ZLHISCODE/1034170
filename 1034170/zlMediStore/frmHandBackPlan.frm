...
Private Sub GetMainDate(ByVal intType As Integer)
    ...
    With rsTmp
        Do While Not .EOF
            ...
            vsfMain(intType).TextMatrix(vsfMain(intType).rows - 1, 汇总列表.成本金额) = GetFormat(!成本金额, 2)
...
Private Sub GetDetailDate(ByVal intType As Integer, ByVal strNo As String)
    ...
    With rsTmp
        Do While Not .EOF
            ...
            vsfDetail(intType).TextMatrix(vsfDetail(intType).rows - 1, 明细列表.数量) = GetFormat(!实际数量 / !包装, 2)
            vsfDetail(intType).TextMatrix(vsfDetail(intType).rows - 1, 明细列表.成本价) = GetFormat(!成本价 * !包装, 5)
            vsfDetail(intType).TextMatrix(vsfDetail(intType).rows - 1, 明细列表.成本金额) = GetFormat(!成本金额, 2)
