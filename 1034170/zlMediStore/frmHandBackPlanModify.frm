...
Private Sub GetDate(ByVal strNo As String)
    ...
    With rsTmp
        ...
        Do While Not .EOF
            ...
            vsfBill.TextMatrix(vsfBill.rows - 1, 明细列表.数量) = GetFormat(!实际数量 / !包装, 2)
            vsfBill.TextMatrix(vsfBill.rows - 1, 明细列表.成本价) = GetFormat(!成本价 * !包装, 5)
            vsfBill.TextMatrix(vsfBill.rows - 1, 明细列表.成本金额) = GetFormat(!成本金额, 2)
...
Private Function GetSeleterReturn(ByVal intType As Integer, ByVal objInputObj As Object, ByVal strInput As String) As Boolean
    ...
    If TypeName(objInputObj) = "TextBox" Then
        vRect = GetControlRect(objInputObj.hWnd)
        sngX = vRect.Left
        sngY = vRect.Top
...
'        gcnOracle.BeginTrans
    ...
    With vsfBill
        '产生新NO号
        If strNo_In = "" Then
            strNo_In = zlDatabase.GetNextNo(100)
        End If
        ...
        For i = 1 To .rows - 1
            If Val(.TextMatrix(i, 明细列表.数量)) > 0 Then
                ...
                dbl数量_In = GetFormat(Val(.TextMatrix(i, 明细列表.数量)) * Val(.TextMatrix(i, 明细列表.包装)), 5)
                dbl成本价_In = GetFormat(Val(.TextMatrix(i, 明细列表.成本价)) / Val(.TextMatrix(i, 明细列表.包装)), 5)
...
'        Set RecReturn = Frm药品选择器.ShowME(Me, 1, 0, cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex))
        ...
        Set RecReturn = frmSelector.showMe(Me, 0, 1, , , , cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), 0, True, True, True, False, False)
...
'        Set RecReturn = Frm药品多选选择器.ShowME(Me, 1, , cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), strkey, sngLeft, sngTop)
        Set RecReturn = frmSelector.showMe(Me, 1, 1, strkey, sngLeft, sngTop, cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), cboStock.ItemData(cboStock.ListIndex), 0, True, True, True, False, False)
...
Private Sub vsfBill_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    With vsfBill
        ...
        Case 明细列表.数量
            ...
            .TextMatrix(Row, 明细列表.成本金额) = GetFormat(Val(.TextMatrix(Row, 明细列表.数量)) * Val(.TextMatrix(Row, 明细列表.成本价)), 2)
'        Case 明细列表.成本价
'            .EditText = GetFormat(Val(.EditText), 5)
