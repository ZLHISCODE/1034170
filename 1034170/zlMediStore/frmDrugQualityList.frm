...
Private Sub GetList(ByVal StrFind As String)
    ...
    On Error GoTo errHandle
    Call zlCommFun.ShowFlash("正在搜索药品质量管理记录,请稍候 ...", Me)
    DoEvents
    ...
    gstrSQL = "SELECT DISTINCT A.ID" & strSql药名 & _
        ",A.药品ID,A.批次,A.供药单位ID,F.规格,A.产地,A.批号," & _
        " ltrim(to_char(a.成本单价*" & str比例系数 & "," & mstrCostFormat & ")) as 成本单价," & _
        " ltrim(to_char(a.成本金额," & mstrMoneyFormat & ")) as 成本金额," & _
        " ltrim(to_char(a.销售单价*" & str比例系数 & "," & mstrPriceFormat & ")) as 销售单价," & _
...
Private Sub SetListColWidth()
    ...
    With vsfList
        ...
        .ColAlignment(8 + IIf(gint药品名称显示 = 2, 1, 0)) = flexAlignRightCenter
        .ColAlignment(9 + IIf(gint药品名称显示 = 2, 1, 0)) = flexAlignRightCenter
        .ColAlignment(10 + IIf(gint药品名称显示 = 2, 1, 0)) = flexAlignRightCenter
        .ColAlignment(11 + IIf(gint药品名称显示 = 2, 1, 0)) = flexAlignRightCenter
        .ColAlignment(13 + IIf(gint药品名称显示 = 2, 1, 0)) = flexAlignRightCenter
...
'            Next
        ...
        .ColWidth(14 + IIf(gint药品名称显示 = 2, 1, 0)) = 0
...
Private Sub SetVisable()
    ...
    If Not IsHavePrivs(mstrPrivs, "质量登记") Then
    ...
    If Not IsHavePrivs(mstrPrivs, "解决处理") Then
        mnuEditVerify.Visible = False
        tlbTool.Buttons("Verify").Visible = False
    End If
    
    If Not IsHavePrivs(mstrPrivs, "清除记录") Then
