...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case mconMenu_File_Parameter '参数设置
        frm参数设置.设置参数 Me, gstrprivs, 1333, MStrCaption
        Call initJinDu
        Call getListInfo
...
'        .GridColor = &H0&
        ...
        .TextMatrix(0, menuPriceCol.药品信息) = "药品信息"
...
'        .GridColor = &H0&
        ...
        .TextMatrix(0, mEnuCostCol.药品信息) = "药品信息"
...
Private Sub getListInfo()
    ...
    mdaStart = Format(DateAdd("d", -7, dateCurrentDate), "yyyy-MM-dd")
    mdaEnd = Format(dateCurrentDate, "yyyy-MM-dd")
    ...
    Else '默认只查询填制日期在一个周的调价信息
        gstrSQL = gstrSQL + " where " + " a.填制日期 between sysdate -7 and sysdate" + " and a.分类=0 order by a.调价号 desc"
    End If
...
Private Sub getCostInfo()
    ...
    On Error GoTo errHandle
    gstrSQL = "Select b.库房id,b.No, i.Id As 药品id, '[' || i.编码 || ']' || i.名称 || ' ' || i.产地 As 药品, p.名称 As 库房, a.批号, a.效期, a.产地, i.规格," & vbNewLine & _
                    "       i.计算单位 As 单位, s.药库单位, Nvl(s.药库包装, 1) 药库包装, s.住院单位, s.住院包装, s.门诊单位, s.门诊包装, a.原成本价, a.新成本价 As 成本价, a.执行日期, b.摘要" & vbNewLine & _
                    "From 药品收发记录 B, 收费项目目录 I, 药品规格 S, 部门表 P, 成本价调价信息 A" & vbNewLine & _
                    "Where a.收发id = b.Id(+) And a.药品id = i.Id And i.Id = s.药品id And a.库房id = p.Id(+) And a.调价汇总号 =[1] " & vbNewLine & _
                    "Order By 药品, 库房id, 批号, 执行日期 Desc, NO Desc"
    ...
    With vsfCost
        For i = 0 To rsTemp.RecordCount - 1
            ...
            .TextMatrix(.rows - 1, mEnuCostCol.原价) = GetFormat(IIf(IsNull(rsTemp!原成本价), 0, rsTemp!原成本价) * db包装系数, mintPriceDigit)
            .TextMatrix(.rows - 1, mEnuCostCol.现价) = GetFormat(IIf(IsNull(rsTemp!成本价), 0, rsTemp!成本价) * db包装系数, mintPriceDigit)
...
Private Sub getPriceInfo()
    ...
    With vsfPrice
        For i = 0 To rsTemp.RecordCount - 1
            ...
            .TextMatrix(.rows - 1, menuPriceCol.原价) = GetFormat(IIf(IsNull(rsTemp!原价), 0, rsTemp!原价) * db包装系数, mintCostDigit)
            .TextMatrix(.rows - 1, menuPriceCol.现价) = GetFormat(IIf(IsNull(rsTemp!现价), 0, rsTemp!现价) * db包装系数, mintCostDigit)
...
Private Sub vsfList_EnterCell()
    ...
    With vsfList
        ...
        If .TextMatrix(.Row, mEnuListCol.调价号) <> "" And .Row > 0 Then
            ...
            Set cbrMenuPop = Me.cbsMain.ActiveMenuBar.Controls.Find(xtpControlButton, mconMenu_Edit_Update, , True)
            Set cbrMenuControl = Me.cbsMain(2).Controls.Find(xtpControlButton, mconMenu_Edit_Update, , True)
            If CDate(.TextMatrix(.Row, mEnuListCol.执行日期)) < zldatabase.Currentdate() Then
                If Not cbrMenuPop Is Nothing Then cbrMenuPop.Enabled = False
                If Not cbrMenuControl Is Nothing Then cbrMenuControl.Enabled = False
            Else
...
Private Sub deleteNotExecutePirce()
    ...
    With vsfList
        If .TextMatrix(.Row, mEnuListCol.调价号) <> "" Then
            ...
            gstrSQL = "select 收费细目id as id from 收费价目 where 调价汇总号=[1]" & _
                        " union " & _
