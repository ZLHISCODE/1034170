...
Private mintcol指导差价率 As Integer
...
'----------------------------------------------
    ...
    Select Case mlng模块号
        ...
        Case 模块号.药品领用
            ...
            strSQL = "Select w.*, z.可用数量 / w.比例系数 可用数量, z.实际金额, z.实际差价" & _
                ...
                "a.批号, Nvl(a.批次, 0) 批次,b.指导差价率, a.效期," & strUnitQuantity & _
        ...
        Case 模块号.其他入库
            ...
            strSQL = " Select w.*,z.可用数量 / w.比例系数 可用数量" & _
                ...
                strUnitQuantity & "a.成本金额, 0 零售金额, 0 差价, b.指导差价率 / 100 As 指导差价率," & _
        ...
        Case 模块号.其他出库
            ...
            strSQL = "Select w.*, z.可用数量, z.实际金额, z.实际差价" & _
                    ...
                    "b.指导差价率, a.效期, g.名称 As 外调单位, h.名称 As 外销单位, a.增值税率," & strUnitQuantity & _
    ...
    If rsData.RecordCount > 0 Then
       '加载药品的基本信息
        ...
        str药品信息 = str药品信息 & IIf(gint药品名称显示 <> 0 And Nvl(rsData!商品名) <> "", "(" & Nvl(rsData!商品名) & ")", "")
        str药品信息 = str药品信息 & "   " & Nvl(rsData!规格)
        str药品信息 = str药品信息 & "   (" & Nvl(rsData!单位) & ")"
        ...
        For i = 1 To rsData.RecordCount
            With Me.vsfList
               ...
               '药品名称显示方式
                ...
                .TextMatrix(i, mintcol药名) = Nvl(rsData!通用名)
                .TextMatrix(i, mintcol商品名) = Nvl(rsData!商品名)
                .TextMatrix(i, mintcol药品来源) = Nvl(rsData!药品来源)
                .TextMatrix(i, mintcol基本药物) = Nvl(rsData!基本药物)
                .TextMatrix(i, mintcol规格) = Nvl(rsData!规格)
                .TextMatrix(i, mintcol产地) = Nvl(rsData!产地)
                .TextMatrix(i, mintcol单位) = Nvl(rsData!单位)
                .TextMatrix(i, mintcol批号) = Nvl(rsData!批号)
                .TextMatrix(i, mintcol有效期至) = Format(Nvl(rsData!效期), "yyyy-mm-dd")
                .TextMatrix(i, mintcol填制人) = Nvl(rsData!填制人)
                .TextMatrix(i, mintcol填制日期) = Format(Nvl(rsData!填制日期), "yyyy-MM-dd hh:mm:ss")
                .TextMatrix(i, mintcol审核人) = Nvl(rsData!审核人)
                .TextMatrix(i, mintcol审核日期) = Format(Nvl(rsData!审核日期), "yyyy-MM-dd hh:mm:ss")
                .TextMatrix(i, mintcol数量) = GetFormat(Val(rsData!填写数量), mintNumberDigit)
                .TextMatrix(i, mintcol冲销数量) = 0
                .TextMatrix(i, mintcol成本价) = GetFormat(Val(rsData!成本价), mintCostDigit)
                .TextMatrix(i, mintcol成本金额) = 0
                .TextMatrix(i, mintcol售价) = GetFormat(Val(rsData!零售价), mintPriceDigit)
                ...
                .TextMatrix(i, mintcol可用数量) = GetFormat(Nvl(rsData!可用数量, 0), mintNumberDigit)
                .TextMatrix(i, mintcol是否变价) = Nvl(rsData!是否变价, 0)
                .TextMatrix(i, mintcol指导差价率) = Val(rsData!指导差价率)
                ...
                If mlng模块号 = 模块号.外购入库 Or mlng模块号 = 模块号.其他入库 Then .TextMatrix(i, mintcol药价级别) = Nvl(rsData!药价级别)
                If mlng模块号 = 模块号.外购入库 Or mlng模块号 = 模块号.其他入库 Then .TextMatrix(i, mintcol生产日期) = Format(Nvl(rsData!生产日期), "yyyy-mm-dd")

                If mlng模块号 = 模块号.药品领用 Or mlng模块号 = 模块号.药品移库 Then .TextMatrix(i, mintcol部门名称) = Nvl(rsData!对方库房)
                If mlng模块号 <> 模块号.外购入库 Then .TextMatrix(i, mintcol批准文号) = Nvl(rsData!批准文号)
                If mlng模块号 = 模块号.其他入库 Then .TextMatrix(i, mintcol其他外观) = Nvl(rsData!外观)

                If mlng模块号 = 模块号.外购入库 Then
                    .TextMatrix(i, mintcol采购价) = GetFormat(Val(rsData!成本价) / (Val(rsData!扣率 / 100)), mintCostDigit)
                    .TextMatrix(i, mintcol采购限价) = GetFormat(Val(rsData!指导批发价), mintCostDigit)
                    .TextMatrix(i, mintcol扣率) = Val(rsData!扣率)
                    .TextMatrix(i, mintcol加成率) = Val(rsData!加成率) * 100 & "%"
                End If
                
'                If mlng模块号 <> 模块号.其他入库 Then
                .TextMatrix(i, mintcol批次) = Val(rsData!批次)
                If mlng模块号 = 模块号.药品移库 Or mlng模块号 = 模块号.其他入库 Then .TextMatrix(i, mintcol真实数量) = GetFormat(Val(rsData!真实数量), mintNumberDigit)
                
                If mlng模块号 = 模块号.外购入库 Or mlng模块号 = 模块号.其他入库 Then
                    .TextMatrix(i, mintcol实际差价) = GetFormat(.TextMatrix(i, mintcol售价金额) - .TextMatrix(i, mintcol成本金额), mintMoneyDigit)
                Else
                    .TextMatrix(i, mintcol实际差价) = GetFormat(Nvl(rsData!实际差价, 0), mintMoneyDigit)
                End If
                If mlng模块号 <> 模块号.外购入库 And mlng模块号 <> 模块号.其他入库 Then .TextMatrix(i, mintcol实际金额) = GetFormat(Nvl(rsData!实际金额, 0), mintMoneyDigit)
                
                If mlng模块号 = 模块号.药品移库 Then .TextMatrix(i, mintcol对方部门) = Val(rsData!对方部门id)
                
                If mlng模块号 = 模块号.外购入库 Then
                    If Val(.TextMatrix(i, mintcol是否变价)) = 1 And Nvl(.TextMatrix(i, mintcol批次), 0) <> 0 Then
                        .TextMatrix(i, mintcol零售价) = GetFormat(rsData!零售价 / Val(rsData!比例系数), gtype_UserDrugDigits.Digit_零售价)
                        .TextMatrix(i, mintcol零售单位) = rsData!售价单位
                        .TextMatrix(i, mintcol零售金额) = 0
                        .TextMatrix(i, mintcol零售差价) = 0
                     End If
                    .TextMatrix(i, mintcol外购批准文号) = Nvl(rsData!批准文号)
                    .TextMatrix(i, mintcol随货单号) = Nvl(rsData!随货单号)
                    .TextMatrix(i, mintcol发票号) = Nvl(rsData!发票号)
                    .TextMatrix(i, mintcol发票代码) = Nvl(rsData!发票代码)
                    .TextMatrix(i, mintcol发票信息) = Nvl(rsData!发票日期)
                    .TextMatrix(i, mintcol发票金额) = GetFormat(Nvl(rsData!发票金额, 0), mintMoneyDigit)
                End If
...
Private Sub Form_Load()
    ...
    Me.DTP开始时间.Value = DateSerial(Year(zldatabase.Currentdate), Month(zldatabase.Currentdate), Day(zldatabase.Currentdate))
    Me.DTP结束时间.Value = DateSerial(Year(zldatabase.Currentdate), Month(zldatabase.Currentdate), Day(zldatabase.Currentdate))
    Me.DTP开始时间.Enabled = False
    Me.DTP结束时间.Enabled = False
    
    Me.DTP开始填制.Value = DateSerial(Year(zldatabase.Currentdate), Month(zldatabase.Currentdate), Day(zldatabase.Currentdate)) + 1
    Me.DTP结束填制.Value = DateSerial(Year(zldatabase.Currentdate), Month(zldatabase.Currentdate), Day(zldatabase.Currentdate)) + 1
    Me.DTP开始填制.Enabled = False
    Me.DTP结束填制.Enabled = False
...
'----------------------------------
    ...
    With Me.vsfList
        ...
        .TextMatrix(0, mintcol指导差价率) = "指导差价率"
    ...
    With Me.vsfList
        If Val(.TextMatrix(1, mintcol批次)) = 0 Then
            For intRow = 1 To .rows - 1
                冲销数量_IN = 冲销数量_IN + GetFormat(.TextMatrix(intRow, mintcol冲销数量) * .TextMatrix(intRow, mintcol比例系数), gtype_UserSaleDigits.Digit_数量)
            Next
        End If
        For intRow = 1 To .rows - 1
            '检查冲销数量，不能小于零
            ...
            If mlng模块号 <> 模块号.其他出库 And mlng模块号 <> 模块号.药品领用 Then
                ...
                If mint库存检查入库库房 <> 0 And .TextMatrix(intRow, 1) <> "" Then
                    If Val(.TextMatrix(intRow, mintcol冲销数量)) = Val(.TextMatrix(intRow, mintcol数量)) Then
                        int全部冲销 = 1
                        If Val(.TextMatrix(intRow, mintcol批次)) <> 0 Then '分批的才统计各行数量检查
                            冲销数量_IN = GetFormat(同批次冲销数量(Val(.TextMatrix(intRow, mintcol批次))), gtype_UserSaleDigits.Digit_数量)  'Val(.TextMatrix(intRow, mintcol数量)) * Val(.TextMatrix(intRow, mintcol比例系数))
                        End If
                    Else
                        int全部冲销 = 0
                        If Val(.TextMatrix(intRow, mintcol批次)) <> 0 Then '分批的才统计各行数量检查
                            冲销数量_IN = GetFormat(同批次冲销数量(Val(.TextMatrix(intRow, mintcol批次))), gtype_UserSaleDigits.Digit_数量) 'GetFormat(.TextMatrix(intRow, mintcol冲销数量) * .TextMatrix(intRow, mintcol比例系数), gtype_UserSaleDigits.Digit_数量)
                        End If
                    End If
                    '移库、其他入库、外购调CheckStrickUsable，外购传入序号无用
                    ...
                    '移库、其他入库、外购调批量冲销库存数量检查，外购传入序号无用
                    If 批量冲销库存数量检查(mInt单据号, Me.cbo库房.ItemData(Me.cbo库房.ListIndex), Val(.TextMatrix(intRow, 1)), .TextMatrix(intRow, mintcol药名), _
                        IIf(mlng模块号 = 模块号.其他入库, 0, (.TextMatrix(intRow, mintcol批次))), Val(冲销数量_IN), mint库存检查入库库房, Trim(.TextMatrix(intRow, mintcolNO)), Val(.TextMatrix(intRow, mintcol序号))) = False Then
                        Exit Function
                    End If
        ...
        For i = 1 To mrecSort.RecordCount
            ...
            If .TextMatrix(intRow, 1) <> "" And Val(.TextMatrix(intRow, mintcol冲销数量)) <> 0 Then
                ...
                If Val(.TextMatrix(intRow, mintcol冲销数量)) = Val(.TextMatrix(intRow, mintcol数量)) Then
                    ...
                    If mlng模块号 = 模块号.外购入库 Then int全部冲销 = 1 '外购对应单据药品全冲
                Else
                    冲销数量_IN = GetFormat(.TextMatrix(intRow, mintcol冲销数量) * .TextMatrix(intRow, mintcol比例系数), gtype_UserSaleDigits.Digit_数量)
                    If mlng模块号 = 模块号.外购入库 Then int全部冲销 = 0 '外购对应单据药品未全冲
                End If
...
Private Sub vsfList_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    For i = 1 To Me.vsfList.rows - 1
        If Nvl(Me.vsfList.TextMatrix(i, mintcol冲销数量)) <> 0 Then
            count = 1
    ...
    If Val(strkey) <> 0 Then
        If InStr(1, strkey, ".") <> 0 Then vsfList.TextMatrix(Row, mintcol冲销数量) = GetFormat(vsfList.TextMatrix(Row, mintcol冲销数量), mintNumberDigit)
    Else
        vsfList.TextMatrix(Row, mintcol冲销数量) = 0
    End If
    If Me.vsfList.TextMatrix(Row, Col) <> 0 Then
        With Me.vsfList
            If .TextMatrix(Row, mintcol售价) <> "" Then
                .TextMatrix(Row, mintcol售价金额) = GetFormat(.TextMatrix(Row, mintcol售价) * strkey, mintMoneyDigit)
            End If
            
            .TextMatrix(Row, mintcol成本金额) = GetFormat(Val(.TextMatrix(Row, mintcol成本价)) * strkey, mintMoneyDigit)
            .TextMatrix(Row, mintcol差价) = GetFormat(Val(.TextMatrix(Row, mintcol售价金额)) - Val(.TextMatrix(Row, mintcol成本金额)), mintMoneyDigit)
            
            If mlng模块号 = 模块号.外购入库 And Val(.TextMatrix(Row, mintcol是否变价)) = 1 And Nvl(.TextMatrix(Row, mintcol批次), 0) <> 0 Then
                 .TextMatrix(Row, mintcol零售金额) = GetFormat(Val(.TextMatrix(Row, mintcol零售价)) * Val(strkey), mintMoneyDigit)
                 .TextMatrix(Row, mintcol零售差价) = GetFormat(Val(.TextMatrix(Row, mintcol零售金额)) - Val(.TextMatrix(Row, mintcol成本金额)), mintMoneyDigit)
            End If
    ...
    If mlng模块号 = 模块号.外购入库 Or mlng模块号 = 模块号.其他入库 Or mlng模块号 = 模块号.药品移库 Then
        If IIf(vsfList.TextMatrix(Row, mintcol批次) = "", 0, vsfList.TextMatrix(Row, mintcol批次)) = 0 Then
            ...
            For i = 1 To Me.vsfList.rows - 1
                vsfList.TextMatrix(i, mintcol可用数量) = GetFormat(dblNum, mintNumberDigit)
            Next
        End If
    End If
...
'--------------------------------------------
    ...
    If mlng模块号 = 模块号.外购入库 Or mlng模块号 = 模块号.其他入库 Or mlng模块号 = 模块号.药品移库 Then
        If Val(Me.vsfList.TextMatrix(vsfList.Row, mintcol批次)) = 0 Then
            Me.vsfList.TextMatrix(vsfList.Row, mintcol可用数量) = GetFormat(Val(Me.vsfList.TextMatrix(vsfList.Row, mintcol可用数量)) + Val(Me.vsfList.TextMatrix(vsfList.Row, mintcol冲销数量)), mintNumberDigit)
        End If
    End If
    
    If Nvl(Me.vsfList.TextMatrix(Me.vsfList.Row, mintcol冲销数量), 0) = 0 Then
        Me.vsfList.TextMatrix(Me.vsfList.Row, mintcol冲销数量) = GetFormat(Me.vsfList.TextMatrix(Me.vsfList.Row, mintcol数量), mintNumberDigit)
    Else
...
Private Sub AllWriteOff()
    ...
    With Me.vsfList
        For i = 1 To .rows - 1
            ...
            .TextMatrix(i, mintcol冲销数量) = GetFormat(.TextMatrix(i, mintcol数量), mintNumberDigit)
            ...
            If mlng模块号 = 模块号.外购入库 Or mlng模块号 = 模块号.其他入库 Or mlng模块号 = 模块号.药品移库 Then
                If Val(.TextMatrix(i, mintcol批次)) = 0 Then
                    ...
                    .Cell(flexcpText, 1, mintcol可用数量, .rows - 1, mintcol可用数量) = GetFormat(.TextMatrix(1, mintcol可用数量) + dblOldSum - dblSum, mintNumberDigit)
                End If
            End If
            ...
            If .TextMatrix(i, mintcol冲销数量) <> 0 Then
                ...
                If .TextMatrix(i, mintcol售价) <> "" Then
                    .TextMatrix(i, mintcol售价金额) = GetFormat(.TextMatrix(i, mintcol售价) * strkey, mintMoneyDigit)
                End If
                 
                .TextMatrix(i, mintcol成本金额) = GetFormat(Val(.TextMatrix(i, mintcol成本价)) * strkey, mintMoneyDigit)
                .TextMatrix(i, mintcol差价) = GetFormat(Val(.TextMatrix(i, mintcol售价金额)) - Val(.TextMatrix(i, mintcol成本金额)), mintMoneyDigit)
                
                If mlng模块号 = 模块号.外购入库 And Val(.TextMatrix(i, mintcol是否变价)) = 1 And Nvl(.TextMatrix(i, mintcol批次), 0) <> 0 Then
                     .TextMatrix(i, mintcol零售金额) = GetFormat(Val(.TextMatrix(i, mintcol零售价)) * Val(.TextMatrix(i, mintcol冲销数量)), mintMoneyDigit)
                     .TextMatrix(i, mintcol零售差价) = GetFormat(Val(.TextMatrix(i, mintcol零售金额)) - Val(.TextMatrix(i, mintcol成本金额)), mintMoneyDigit)
                End If
...
Private Sub AllEliminate()
    ...
    With Me.vsfList
        ...
        If .rows > 1 And Val(.TextMatrix(1, mintcol批次)) = 0 Then .Cell(flexcpText, 1, mintcol可用数量, .rows - 1, mintcol可用数量) = GetFormat(.TextMatrix(1, mintcol可用数量) + dblSum, mintNumberDigit)
    End With
End Sub
Private Sub Filter()
'-----------------------
...
Private Function 批量冲销库存数量检查(ByVal int单据 As Integer, ByVal lng库房ID As Long, _
    ...
    '冲销单据时检查：原单据入库库房是否实际数量足够，实际冲销数量不能大于实际数量
    '对于移库单据、他入库单，需要取原单据入库那笔的批次，再根据批次来取实际数量；
    '其他单据可直接根据批次取库存实际数量
    'int库存检查：表示药品出库时是否进行库存检查：0-不检查;1-检查，不足提醒；2-检查，不足禁止
    '只有冲销时是出库类型（原单据是入库类型）的要做此检查：外购入库、、其他入库、移库（原单据入的那笔）
    ...
    Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, "取实际数量", lng库房ID, lng药品ID, lng入库批次)
