...
Private Sub CmdSave_Click()
    ...
    If Not (Format(Me.Dtp结束日期.Value, "yyyy-MM-dd HH:mm:ss") > Format(Me.Dtp开始日期.Value, "yyyy-MM-dd HH:mm:ss")) Then
        ...
        Exit Sub
    End If
    If Format(Me.Dtp结束日期.Value, "yyyy-MM-dd HH:mm:ss") > Format(zldatabase.Currentdate, "yyyy-MM-dd HH:mm:ss") Then
...
Private Sub GetPeriod(ByVal str月份 As String, str开始日期 As String, str结束日期 As String)
    ...
    Else
        ...
        If mstr缺省结束日期 <> "" And mstr缺省结束日期 > str开始日期 Then
            str结束日期 = mstr缺省结束日期
        Else
            str结束日期 = Format(zldatabase.Currentdate(), "yyyy-MM-dd HH:mm:ss")
        End If
    End If
    Exit Sub
errHandle:
