...
Begin VB.Form frmCooperateUnitsRegPlan 
   Caption         =   "挂号单位计划控制"
   ClientHeight    =   10125
   ClientLeft      =   120
   ClientTop       =   450
   ...
   Begin MSComctlLib.StatusBar stbThis 
      ...
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Panel1 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            ...
            Object.Width           =   2355
            MinWidth        =   882
            Picture         =   "frmCooperateUnitsRegPlan.frx":0000
            Text            =   "中联软件"
            TextSave        =   "中联软件"
            Key             =   "ZLFLAG"
   ...
   End
End
Attribute VB_Name = "frmCooperateUnitsRegPlan"
...
Private mbln时段                As Boolean '如果安排计划设置了时段则严格按照时段来分配
Private mrs时间段               As ADODB.Recordset
Private mstrKey       As String
Private mrsSource     As ADODB.Recordset
Private mrsUnitsReg   As ADODB.Recordset
Private WithEvents mfrmReg       As frmCooperateRegPlan
Attribute mfrmReg.VB_VarHelpID = -1
Private WithEvents mfrmRegNoTime As frmCooperateRegArrangePlan
...
Public Function zlShowMe(ByVal lng计划ID As Long, ByVal lngModule As Long, ByVal strPrivs As String) As Boolean
    mlng计划Id = lng计划ID
...
Private Sub InitPage()
    ...
    tbPage.RemoveAll
    If mbln序号控制 Then
    Set mfrmReg = New frmCooperateRegPlan
        mfrmReg.frmInit mlng计划Id
        Set ObjItem = tbPage.InsertItem(1, "", mfrmReg.hWnd, 0)
        ObjItem.Tag = 1
    Else
        Set mfrmRegNoTime = New frmCooperateRegArrangePlan
        
        mfrmRegNoTime.frmInit mlng计划Id, mlngModule, mstrPrivs
...
Public Function InitData() As Boolean
    ...
    Set mrs计划 = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng计划ID)
...
'    strSQL = "Select 限制项目,限号数,  限约数,限制项目 as 星期 From  挂号安排计划限制 where 计划ID=[1]  Order BY 限制项目      "
'    Set mrs限号 = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng计划ID)
    cbo号类.Text = Nvl(mrs计划!号类)
    txt号别.Tag = Nvl(mrs计划!计划Id)
      
    txt号别.Text = Nvl(mrs计划!号码)
    cbo科室.Text = Nvl(mrs计划!科室)
    cboItem.Text = Nvl(mrs计划!项目)
    cboDoctor.Text = Nvl(mrs计划!医生姓名)
    chk病案.Value = IIf(Val(Nvl(mrs计划!病案必须)) = 1, 1, 0)
    chk序号控制.Value = IIf(Val(Nvl(mrs计划!序号控制)) = 1, 1, 0):  chk序号控制.Tag = chk序号控制.Value
