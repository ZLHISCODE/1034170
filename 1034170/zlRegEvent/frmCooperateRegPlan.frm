...
Begin VB.Form frmCooperateRegPlan 
   BorderStyle     =   0  'None
   ClientHeight    =   10155
   ClientLeft      =   0
   ClientTop       =   0
   ...
   Begin VB.PictureBox PicUnit 
      ...
      Begin VB.ListBox lstUnits 
         ...
         EndProperty
         Height          =   2910
         ItemData        =   "frmCooperateRegPlan.frx":0000
         Left            =   960
         List            =   "frmCooperateRegPlan.frx":0002
         Sorted          =   -1  'True
         TabIndex        =   1
         Top             =   1440
         Width           =   2295
   ...
   End
End
Attribute VB_Name = "frmCooperateRegPlan"
...
Private Sub cmdMoveAllSource_Click()
    ...
    For i = 1 To lvwSource.ListItems.Count
        ...
        End If
        mrsSource.Filter = 0
        InsertUnitReg lstUnits.Text, Val(lvwitem1.Text), mlng计划Id, Mid(Me.tbWeekTime.SelectedItem.Key, 2), 1, lvwitem1.SubItems(1)
    Next
...
Public Function SaveData() As Boolean
    ...
    Do While Not mrsUnits.EOF
        strSQL = "Zl_合作单位计划控制_Delete(" & mlng计划Id & ",'" & mrsUnits!名称 & "')"
        zlAddArray colExec, strSQL
         With mrsUnitsReg
                ...
                End If
                If strDisable <> "" Then
                    strSQL = "Zl_合作单位计划控制_Insert(" & mlng计划Id & ",'" & mrsUnits!名称 & "',Null,Null,'" & strDisable & "')"
                    zlAddArray colExec, strSQL
                Else
                    If strTmp <> "" Or strLimit <> "" Then
                        strSQL = "Zl_合作单位计划控制_Insert(" & mlng计划Id & ",'" & mrsUnits!名称 & "'," & IIf(strTmp = "", "Null,", "'" & strTmp & "',") & IIf(strLimit = "", "Null)", "'" & strLimit & "')")
                        zlAddArray colExec, strSQL
                    End If
                End If
        End With
...
Private Sub cmdOK_Click()
    If SaveData() = True Then RaiseEvent frmUnload(False)
End Sub
...
Public Function frmInit(ByVal lng计划ID As Long) As Boolean
    mlng计划Id = lng计划ID
...
Public Function InitData() As Boolean
    ...
    strSQL = "" & "   Select decode(星期,'周日',1,'周一',2,'周二',3,'周三',4,'周四',5,'周五',6,7) as 排序,星期,to_char(开始时间,'HH24')||':00' as 时点,序号,to_char(开始时间,'hh24:mi')||'-' ||to_char(结束时间,'hh24:mi') as 时间范围, " & "               限制数量,是否预约" & "   From  挂号计划时段 " & "   Where 计划ID=[1]" & "   Order by 排序,时点,序号"
...
Private Sub InsertUnitReg(ByVal str合作单位 As String, ByVal lng序号 As Long, ByVal lng计划ID As Long, ByVal str限制项目 As String, ByVal lng数量 As Long, Optional ByVal str时间段 As String = "")
    ...
    mrsUnitsReg.Filter = "合作单位='" & str合作单位 & "' and 序号=" & lng序号 & " and  计划ID=" & lng计划ID & " And 限制项目='" & str限制项目 & "' And 数量=" & lng数量
...
Private Sub UnitRegToSource(ByVal str限制项目 As String, ByVal lng序号 As Long, ByVal lng计划ID As Long, ByVal str时间段 As String, ByVal lng数量 As Long)
...
Private Function InitRs() As Boolean
    ...
    '已经分配序号
    strSQL = "Select 合作单位, 计划ID, 限制项目, 序号, 数量 From 合作单位计划控制  Where 计划ID=[1] And 序号 <> 0"
