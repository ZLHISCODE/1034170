...
Private Sub InitCon()
    ...
    '检查门诊临床科室
    If IsHavePrivs(mstrPrivs, "所有科室") Then
        cbo科室.AddItem "所有科室"
        cbo科室.ListIndex = 0
    Else
...
Private Sub cmdDoct_Click()
    ...
    If cbo科室.ItemData(cbo科室.ListIndex) <= 0 Then
        ...
        " Where A.ID=B.部门ID And B.工作性质='临床' And B.服务对象 IN(1,3) " & _
                    IIf(IsHavePrivs(mstrPrivs, "所有科室") = False, " And Exists(Select 1 From 部门人员 where 人员id=[1] and C.部门id=部门id) ", "")
    ...
    strSQL = "Select Distinct A.ID,A.编号,A.姓名,A.简码" & _
        ...
        " Order by A.编号"
    vRect = GetControlRect(txtDoct.Hwnd)
    ...
    If txtDoct.Text <> "" Then
        ...
        If cbo科室.ItemData(cbo科室.ListIndex) <= 0 Then
            ...
            strSQL = "Select Distinct A.ID From 部门表 A,部门性质说明 B" & _
                " Where A.ID=B.部门ID And B.工作性质='临床' And B.服务对象 IN(1,3) " & _
            IIf(IsHavePrivs(mstrPrivs, "所有科室") = False, " And Exists(Select 1 From 部门人员 where 人员id=[1] and A.id=部门id) ", "")
            strSQL = "And C.部门ID IN(" & strSQL & ")"
        Else
            '当前选择的科室的医生
            lngDeptID = cbo科室.ItemData(cbo科室.ListIndex)
