...
Public Function GetDateRange(ByVal intYear As Integer, ByVal intMonth As Integer, _
    Optional ByVal intWeek As Integer, Optional ByVal bln允许跨月 As Boolean = True) As Variant
    '功能：根据年月周信息获取日期范围
    '返回：Array(开始日期,结束日期)
    Dim dtFirstDay As Date
    Dim dtStart As Date, dtEnd As Date
    
    dtFirstDay = intYear & "-" & intMonth & "-01"
    If intWeek = 0 Then
        '整月日期范围
        dtStart = dtFirstDay
        '本月第一天加一月再减一天就是本月的结束日期
        dtEnd = DateAdd("d", -1, DateAdd("m", 1, dtStart))
    Else
        dtEnd = DateAdd("d", 7 * intWeek - Weekday(dtFirstDay, vbMonday), dtFirstDay)
        dtStart = DateAdd("d", -6, dtEnd) '结束日期减6天就是开始日期
        
        If bln允许跨月 = False Then
            '开始日期跳月了,直接取本月第一天
            If DateDiff("m", dtStart, dtFirstDay) > 0 Then dtStart = dtFirstDay
            '结束日期跳月了,直接取本月最后一天
            If DateDiff("m", dtFirstDay, dtEnd) > 0 Then dtEnd = DateAdd("d", -1, DateAdd("m", 1, dtFirstDay))
        End If
    End If
    GetDateRange = Array(dtStart, dtEnd)
End Function
