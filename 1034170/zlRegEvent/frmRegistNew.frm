...
Begin VB.Form frmRegistNew 
   ...
   Begin MSComctlLib.ImageList imgColor 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":DC07
            Key             =   "Print"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":DE21
            Key             =   "Add"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":E51B
            Key             =   "Del"
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":EC15
            Key             =   "预约"
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":F30F
            Key             =   "接收"
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":FA09
            Key             =   "取消"
         EndProperty
         BeginProperty ListImage8 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":10103
            Key             =   "View"
         EndProperty
         BeginProperty ListImage9 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":107FD
            Key             =   "Go"
         EndProperty
         BeginProperty ListImage10 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":10A17
            Key             =   "Filter"
         EndProperty
         BeginProperty ListImage11 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":10C31
            Key             =   "Help"
         EndProperty
         BeginProperty ListImage12 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":10E4B
            Key             =   "Quit"
         EndProperty
         BeginProperty ListImage13 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmRegistNew.frx":11065
            Key             =   "RollingCurtain"
            Object.Tag             =   "RollingCurtain"
         EndProperty
      EndProperty
...
Private Sub mnuEdit_Bespeak_Click()
    ...
    If Format(datNow, "yyyy-mm-dd hh:mm:ss") < Format(gdatRegistTime, "yyyy-mm-dd hh:mm:ss") Then
        ...
        If strMsgResult = "出诊表排班预约" Then
            ...
            frmRegistEditNew.Show GetModuleType, Me
        End If
    Else
        frmRegistEditNew.mlngModul = mlngModul
        frmRegistEditNew.mstrPrivs = mstrPrivs
...
Private Sub mnuEdit_DelBook_Click()
    ...
    If frmRegistEditNew.CancelBill(Me, strNO, mlngModul, mstrPrivs, 1) = False Then Exit Sub
...
Private Sub mnuEdit_DelExtra_Click()
    ...
    If frmRegistEditNew.CancelBill(Me, strNO, mlngModul, mstrPrivs, 2) = False Then Exit Sub
...
Private Sub mnuEdit_Defer_Click()
    ...
    If Format(datNow, "yyyy-mm-dd hh:mm:ss") < Format(gdatRegistTime, "yyyy-mm-dd hh:mm:ss") Then
        ...
        End If
        If strMsgResult = "出诊表排班预约" Then
            frmBookingDeferNew.Show GetModuleType, Me
        End If
    Else
        frmBookingDeferNew.Show GetModuleType, Me
    End If
...
Private Sub DeleteRegist()
    ...
    If gbln消费验证 And bln记帐 = False Then
        dblAdvanceMoney = zlGetRegAdvanceMoney(lng结帐ID, lngCard结帐ID)
        If dblAdvanceMoney <> 0 Then
            If Not zlDatabase.PatiIdentify(Me, glngSys, lngPatientID, dblAdvanceMoney, mlngModul, 1, True) Then Exit Sub
        End If
    End If
...
'参数：strNo = 挂号单单据号
    ...
    strSql = "Select 1 From 病人挂号记录 Where NO = [1] And 出诊记录Id Is Not Null"
...
'功能:按条件读取单据列表(过滤功能)
    ...
    If Not blnSort Then
        ...
        If tbsType.SelectedItem.Key = "挂号" Then
            ...
            '问题号:51672
              strSql = "  " & _
                "       Select a.No As 单据号, f.实际票号 As 首张票据, Decode(a.号别, Null, Null, '[' || a.号别 || ']') || Max(Decode(f.序号,1,c3.名称,Null)) As 号别, a.号序, e.名称 As 科室," & vbNewLine & _
                ...
                "       Group By a.No, f.实际票号, a.号别, a.号序, e.名称, a.执行人, a.门诊号, a.姓名, d.就诊卡号, f.费别, a.发生时间,a.登记时间,Decode(A.记录状态,2,A.登记时间,Null), a.操作员姓名, a.摘要," & _
              ...
              strSql = strSql & " Union All " & _
                ...
                "              a.执行人 As 医生, Decode(Max(Nvl(f.附加标志, 0)), 1, '√', Null) As 病历, a.门诊号, a.姓名, d.就诊卡号, f.费别, " & vbNewLine & _
                "              To_Char(Decode(a.记录状态,2,-1*Sum(Nvl(f.实收金额, 0)),Sum(Nvl(f.实收金额, 0))), '99999999999999990.00') As 金额,To_Char(Decode(a.记录状态,2,-1*Sum(Decode(Sign(Nvl(f.附加标志, 0)), 1, 0, 1) * Nvl(f.实收金额, 0)),Sum(Decode(Sign(Nvl(f.附加标志, 0)), 1, 0, 1) * Nvl(f.实收金额, 0))), '99999999999999990.00') As 挂号费," & _
                "               To_Char(Decode(a.记录状态,2,-1*Sum(Decode(Sign(Nvl(f.附加标志, 0)), 1, 1, 0) * Nvl(f.实收金额, 0)),Sum(Decode(Sign(Nvl(f.附加标志, 0)), 1, 1, 0) * Nvl(f.实收金额, 0))), '99999999999999990.00') As 病例费, a.发生时间 as 挂号时间,a.登记时间,Decode(A.记录状态,2,A.登记时间,Null) As 退号时间 ," & IIf(mbytCancel = 2, "a.操作员姓名 as 退号员", "a.操作员姓名 as 挂号员") & ", Max(Decode(f.记录状态,0,Null,f.操作员姓名)) As 收费员, a.收费单,a.摘要, " & vbNewLine & _
                "              Decode(a.预约, 1, a.发生时间, Null) As 预约时间, Decode(a.社区, 1, '√', Null) As 社区, a.病人id, a.记录状态, a.退号审核人,A.退号审核时间,a.预约操作员 ," & _
                "              Max(A.险类) as 险类,Max(F.记帐费用) as 记帐费用" & vbNewLine & _
                "       From 病人挂号记录 A, 病人信息 D, 临床出诊记录 B, 临床出诊号源 B1, 挂号安排 B2, 部门表 E, 门诊费用记录 F, 收费项目目录 C, 收费项目目录 C1, 收费项目目录 C2, 收费项目目录 C3 " & vbNewLine & _
        ...
        ElseIf tbsType.SelectedItem.Key = "接收" Then
            ...
            "                 And (c.站点 = '" & gstrNodeNo & "' Or c.站点 Is Null)" & vbNewLine & _
            "           Group By a.No, a.发生时间, F.费别,a.号别, Nvl(c.名称,Nvl(c1.名称,c2.名称)), a.号序, a.执行人, a.门诊号, a.姓名, d.身份证号, d.家庭电话, a.摘要, a.登记时间, a.操作员姓名,a.退号审核人,a.退号审核时间,a.记录状态,a.预约操作员 ,A.病人ID" & vbNewLine & _
