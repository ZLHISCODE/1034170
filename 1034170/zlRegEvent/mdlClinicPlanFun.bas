...
Public Function GetNewPlanInfo(ByVal frmParent As Form, ByVal strPrivs As String, ByVal blnMonth As Boolean, _
    ByRef varPlanInfo As Variant) As Boolean
    '获取和检查下一个月/周出诊表的信息
    '入参：
    '   blnMonth - 是否月出诊表
    '出参：
    '   varPlanInfo 新出诊表信息，Array(出诊ID,出诊表名,年份,月份,周数,开始时间,终止时间,出诊表Key值,是否删除安排)
    '   说明：
    '       是否删除安排：是否删除未用于挂号预约的固定出诊安排
    '       出诊表Key值：新出诊表Key值，用于定位
    '返回：成功返回True，否则返回False
    Dim lng出诊ID As Long, str出诊表名 As String, strPlanKey As String, blnDeletePlan As Boolean
    Dim intYear As Integer, intMonth As Integer, intWeek As Integer, dtStart As Date, dtEnd As Date
    Dim varDateRange As Variant, blnCancel As Boolean
    ...
    Err = 0: On Error GoTo errHandler
    blnDeletePlan = False
    lngUserID = IIf(zlStr.IsHavePrivs(strPrivs, "所有科室"), 0, UserInfo.ID)
    If GetNextPlanDate(frmParent, IIf(blnMonth, 1, 2), intYear, intMonth, intWeek, lngUserID, , blnCancel) = False Then
        If blnCancel = False Then MsgBox "确定下一个出诊表的日期时出错，请重试！", vbInformation, gstrSysName
        Exit Function
    End If
    varDateRange = GetDateRange(intYear, intMonth, intWeek)
    dtStart = varDateRange(0): dtEnd = varDateRange(1)
    
    If CheckExistsFixedToOthers(IIf(blnMonth, 1, 2), lngUserID, dtStart, strDelInfo, strNotDelInfo) Then
        strInfo = "提示：" & vbCrLf & _
    ...
    '检查是否有按月/周排班的有效号源
    If CheckIsHavePlan(IIf(blnMonth, 1, 2), lngUserID, dtStart, dtEnd, blnDeletePlan) = False Then
    ...
    strSQL = "Select ID" & vbNewLine & _
            ...
            " Where 排班方式 = [1] And 年份 = [2] And 月份 = [3]" & _
                    IIf(blnMonth, "", " And 周数 = [4]") & " And Nvl(站点,'-') = Nvl([5],'-')"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "mdlClinicPlanFun", _
        IIf(blnMonth, 1, 2), intYear, intMonth, intWeek, gstrNodeNo)
    If rsTemp.EOF Then
    ...
    If blnMonth Then
        str出诊表名 = intYear & "年" & intMonth & "月出诊表"
        'XX月出诊表节点：K2_年份_月份
        strPlanKey = "K2_" & intYear & "_" & intMonth
    Else
        str出诊表名 = intYear & "年" & intMonth & "月第" & intWeek & "周出诊表"
        'XX周出诊表节点：K3_年份_月份_周数
        strPlanKey = "K3_" & intYear & "_" & intMonth & "_" & intWeek
    End If
    
    'Array(出诊ID,出诊表名,年份,月份,周数,开始时间,终止时间,出诊表Key值,是否删除安排)
    varPlanInfo = Array(lng出诊ID, str出诊表名, intYear, intMonth, intWeek, dtStart, dtEnd, strPlanKey, blnDeletePlan)
    GetNewPlanInfo = True
    Exit Function
errHandler:
    If ErrCenter() = 1 Then
