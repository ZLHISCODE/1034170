...
Private mfrmMain As Object, mstrPreNote As String
...
Public Sub LoadData(ByVal lngID As Long)
    ...
    Case 1
        ...
        strSQL = "Select a.处理时间, c.姓名, c.门诊号, c.性别, c.年龄, b.家庭电话, c.No As 单据号, c.预约方式, Nvl(d.原预约时间, Nvl(c.预约时间,c.发生时间)) As 原预约时间," & vbNewLine & _
        ...
        With vsfList
            ...
            Do While Not rsDetail.EOF
                ...
                .TextMatrix(.Rows - 1, 7) = FormatEx(Get金额(Val(Nvl(rsDetail!原项目ID))), 2)
                ...
                Else
                    .TextMatrix(.Rows - 1, 14) = FormatEx(Get金额(Val(Nvl(rsDetail!现项目ID))), 2)
                End If
...
Private Sub imgColApp_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(picImgApp.Hwnd)
    lngLeft = vRect.Left
...
Private Sub SelectDoc()
    ...
    With frmServiceChangeNum
        lngRow = vsfList.Row
        .InitValue vsfList.TextMatrix(vsfList.Row, 1), vsfList.TextMatrix(vsfList.Row, 3), vsfList.TextMatrix(vsfList.Row, 4), _
                        vsfList.TextMatrix(vsfList.Row, 5), vsfList.TextMatrix(vsfList.Row, 6), vsfList.TextMatrix(vsfList.Row, 9), lblInfo.Caption
        .mlng消息ID = Val(vsfList.RowData(vsfList.Row))
        .Show 1, Me
    End With
