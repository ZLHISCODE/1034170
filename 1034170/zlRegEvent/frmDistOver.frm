...
Public Function zlShowEdit(ByVal frmMain As Form, ByVal strPrivs As String, ByVal strQueuePrivs As String, _
    ...
    '说明：
    '------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String, rsTemp As ADODB.Recordset, rsTmp As ADODB.Recordset
    ...
    Else
        If Format(zlDatabase.Currentdate, "yyyy-mm-dd hh:mm:ss") < Format(gdatRegistTime, "yyyy-mm-dd hh:mm:ss") Then
            strSQL = _
                ...
                " Where a.门诊诊室=b.名称 And a.号表ID=c.id And c.号码=d.号别 And d.NO=[1] AND d.记录性质=1 and d.记录状态=1 "
        Else
            strSQL = "Select 分诊方式 From 临床出诊记录 A,病人挂号记录 B Where B.NO = [1] And B.记录性质 = 1 And A.ID = B.出诊记录ID"
