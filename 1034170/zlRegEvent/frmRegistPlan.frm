...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_NewItem '增加
        If Val(Me.tbPage.Selected.Tag) = mPgIndex.Pg_当前有效号别 Then
            Control.Visible = IsHavePrivs(mstrPrivs, "安排")
        Else
    ...
    Case conMenu_Edit_AllStopNO      '全部取消序号控制
        Control.Visible = IsHavePrivs(mstrPrivs, "安排")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_Stop, conMenu_Edit_StopPlanTimes
        If Val(Me.tbPage.Selected.Tag) = mPgIndex.Pg_当前有效号别 Then
            Control.Visible = IsHavePrivs(mstrPrivs, "安排")
    ...
    Case conMenu_Edit_PlanAdd   '计划安排
        If Val(Me.tbPage.Selected.Tag) = mPgIndex.Pg_当前有效号别 Then
            Control.Visible = IsHavePrivs(mstrPrivs, "增加计划")
            Control.Enabled = mfrm有效号别.zlGet安排ID <> 0
        Else
            Control.Visible = False
        End If
   ...
   ' Case conMenu_Edit_PlanModify    '修改计划
    ...
    Case conMenu_Edit_PlanDelete, conMenu_Edit_PlanModify, conMenu_Edit_SetPlanDateSeqment, ComMenu_Edit_UnitRegArrangeModify  '删除计划
        If Control.ID = conMenu_Edit_PlanModify Or Control.ID = conMenu_Edit_SetPlanDateSeqment Then
            Control.Visible = IsHavePrivs(mstrPrivs, "修改计划")
        Else
            Control.Visible = IsHavePrivs(mstrPrivs, "删除计划")
        End If
        If Control.ID = ComMenu_Edit_UnitRegArrangeModify Then
            '处理规则
             Control.Visible = IsHavePrivs(mstrPrivs, "挂号合作单位控制")
        End If
        ...
        End If
    Case conMenu_Edit_ClearStopPlan '清除所有停用计划
        Control.Visible = IsHavePrivs(mstrPrivs, "清除所有停用计划")
        
    Case conMenu_Edit_PlanVerify      '审核
        Control.Visible = IsHavePrivs(mstrPrivs, "计划审核")
        If Val(Me.tbPage.Selected.Tag) = mPgIndex.Pg_当前有效号别 Then
            'zlPlanStatus() '0-不存在计划安排,1-未审核,2-已经审核,3-已经生效\
            lngID = mfrm有效号别.zlPlanStatus
            Control.Enabled = lngID = 1
        ...
        Control.Enabled = Control.Visible And Control.Enabled
    Case conMenu_Edit_PlanCancel   '取消审核
        Control.Visible = IsHavePrivs(mstrPrivs, "取消审核")
         If Val(Me.tbPage.Selected.Tag) = mPgIndex.Pg_当前有效号别 Then
            'zlPlanStatus() '0-不存在计划安排,1-未审核,2-已经审核,3-已经生效\
            lngID = mfrm有效号别.zlPlanStatus
    ...
    Case conMenu_Edit_Modify, conMenu_Edit_Delete, comMenu_Edit_UnitRegModify ', comMenu_Edit_SetDateSegment
        If Val(Me.tbPage.Selected.Tag) = mPgIndex.Pg_当前有效号别 Then
            Control.Visible = IsHavePrivs(mstrPrivs, "安排")
            'zlPlanStatus() '0-不存在计划安排,1-未审核,2-已经审核,3-已经生效\
            lngID = mfrm有效号别.zlPlanStatus
...
'        End If
    ...
    Case conMenu_Manage_Bespeak '时间段设置
        Control.Visible = IsHavePrivs(mstrPrivs, "时间段设置")
        Control.Enabled = Control.Visible
...
Private Sub Form_Load()
    ...
    '46639
    chk禁止删除.Value = IIf(Val(zlDatabase.GetPara("预约单存在禁止删除", glngSys, mlngModule, "", Array(chk禁止删除), InStr(mstrPrivs, ";参数设置;") > 0)) = 1, 1, 0)
    Call InitData
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    '46639
    Call zlDatabase.SetPara("预约单存在禁止删除", chk禁止删除.Value, glngSys, mlngModule, InStr(mstrPrivs, ";参数设置;") > 0)
...
Private Sub zlDeleteItem()
    ...
    mfrmFilter.ShowDel = mblnDisDel
    Set mArrFilter = mfrmFilter.GetCondition
    If mfrm有效号别.zlExecuteDeleteList(chk禁止删除.Value = 1) = False Then Exit Sub
    mfrm有效号别.Tag = ""
End Sub
...
Private Sub picList_Resize()
    ...
    With picList
        ...
        chk禁止删除.Left = .ScaleWidth - chk禁止删除.Width - 50
        chk禁止删除.Top = tbPage.Top + 100
    End With
End Sub
