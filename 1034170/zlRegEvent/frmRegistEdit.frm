...
Private Function zlExcuteDelRegistered() As Boolean
    ...
    '123874:李南春,2018/4/28，获取部分退结算信息,如果销账ID不为空，则已经部分退费过
...
'    End If
    ...
    If mbytMode <> 1 And mblnStation And Not mblnStationPrice And cbo结算方式.Visible Then
        If cbo结算方式.Text = "" And Not blnPrice Then
            If FormatEx(curMoney - Val(txt个帐支付.Text) - Val(txt预交支付.Text), 5) > 0 Then
                MsgBox "不能确定挂号费用的结算方式，不能挂号！", vbInformation, gstrSysName
    ...
    If Val(txt缴款.Text) <> 0 And txt缴款.Enabled And txt缴款.Visible Then
        curMoney = mcur应缴 + GetRegistMoney - Val(txt个帐支付.Text) - Val(txt预交支付.Text)
        If FormatEx(txt缴款.Text, 5) < FormatEx(curMoney, 5) Then
...
'---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Dim lng就诊ID As Long, lngNoSort As Long, strErrMsg As String
    Dim strPatiInforXML As String, lng身份证类别ID As Long, bln是否签约 As Boolean
...
'        End If
        ...
        Else
            ...
            '保存附加费
            If Not mrsExpenses Is Nothing Then
                If GetSqlExpenses(cllPro, mrsExpenses, strNO, k, bln存为划价单 And GetRegistMoney <> 0, str划价NO, lng病人ID, str门诊号, lngSN, str发生时间, str登记时间, blnNoPrint, str结算方式, strRoom, lng结帐ID) = False Then
                    Exit Sub
                End If
            End If
        End If
...
Private Function GetSqlExpenses(ByRef cllPro As Collection, ByVal rsExpenses As ADODB.Recordset, _
                    ...
                    Optional ByVal str结算方式 As String, Optional ByVal strRoom As String, Optional ByVal lng结帐ID As Long) As Boolean
    '获取附加费记录sql
    ...
    For i = 1 To rsExpenses.RecordCount
        ...
        Else
            ...
            str发生时间 & "," & str登记时间 & "," & _
            "'" & mstr医生姓名 & "'," & ZVal(mlng医生ID) & ",0," & IIf(lbl急.Visible, 1, 0) & "," & _
            "'" & IIf(txt号别.Text = "+", "", txt号别.Text) & "','" & strRoom & "'," & ZVal(lng结帐ID) & "," & IIf(blnNoPrint, "NULL", ZVal(mlng领用ID)) & "," & _
    ...
    For i = 1 To mrsBillAdvance.RecordCount
        If dblTotal <> 0 Then
            ...
            If mrsBillAdvance!记录性质 = 1 Or mrsBillAdvance!记录性质 = 11 Then
                ...
                txt预交支付.Text = Format(Val(txt预交支付.Text) + Val(dblBalance), "0.00")
...
Reg:
    ...
    Else
        ...
        If Not mrsExpenses Is Nothing Then
            ...
            Do While Not mrsExpenses.EOF
                 For j = 1 To mshMoney.Rows - 1
                    If Trim(mshMoney.TextMatrix(j, 0)) = mrsExpenses!项目名称 Then
                        cur合计 = cur合计 + Val(mshMoney.TextMatrix(j, 2))
                        Exit For
                    End If
                 Next
...
Private Sub ReCalc预约接收发卡()
    ...
    '显示卡费数据
     ...
     If Not mrsExpenses Is Nothing Then
        ...
        If mrsExpenses.RecordCount > 0 Then mrsExpenses.MoveFirst
        Do While Not mrsExpenses.EOF
            mshMoney.Rows = mshMoney.Rows + 1
            mshMoney.TextMatrix(mshMoney.Rows - 1, 0) = mrsExpenses!项目名称
            mshMoney.TextMatrix(mshMoney.Rows - 1, 1) = Format(mrsExpenses!应收, "0.00")
            mshMoney.TextMatrix(mshMoney.Rows - 1, 2) = Format(mrsExpenses!实收, "0.00")
...
'功能：根据当前界面输入的号别,读取挂号费用集,显示在表格中
    Dim lng项目id As Long, bln病历 As Boolean, str费别 As String
    Dim cur应收 As Currency, cur实收 As Currency, cur合计 As Currency, cur病历 As Currency
    Dim lngRow As Long, i As Long, j As Long, lng病人ID As Long
