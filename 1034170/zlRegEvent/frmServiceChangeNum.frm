...
Public Sub InitValue(strName As String, strGender As String, strAge As String, strPhone As String, strNO As String, strAppTime As String, strInfo As String)
    mstrName = strName
...
Public Sub InitWithValue(strName As String, strGender As String, strAge As String, strPhone As String, strNO As String, strAppTime As String, strRegNo As String, strDoc As String)
    mstrName = strName
    mstrGender = strGender
    mstrAge = strAge
    mstrPhone = strPhone
...
Private Function Get金额(ByVal lng项目id As Long) As Double
    Dim strSQL As String, rsTemp As ADODB.Recordset
    strSQL = "Select Sum(Nvl(现价, 0)) As 金额" & vbNewLine & _
            "From (Select b.现价" & vbNewLine & _
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng项目id)
    If rsTemp.EOF Then
        Get金额 = 0
    Else
        Get金额 = Val(Nvl(rsTemp!金额))
    End If
End Function
    ...
    Do While Not rsPlan.EOF
        ...
        If mbln是否收费 Then
            strSQL = "Select Sum(Nvl(现价, 0)) As 金额" & vbNewLine & _
                    ...
                    "       Where b.收费细目id = a.Id And b.收入项目id = c.Id And a.Id = d.从项id And d.主项id = [1] And Sysdate Between b.执行日期 And" & vbNewLine & _
                    "             Nvl(b.终止日期, To_Date('3000-1-1', 'YYYY-MM-DD')))"
            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, Val(Nvl(rsPlan!项目ID)))
            
            '收费的挂号记录,必须金额相同的才能换诊
            If mdbl收费金额 <> Val(Nvl(rsTemp!金额)) Then
                blnAdd = False
            End If
        End If
        ...
        If blnAdd Then
            With vsfPlan
                ...
                .TextMatrix(.Rows - 1, .ColIndex("金额")) = Format(Get金额(Nvl(rsPlan!项目ID)), "0.00")
...
Private Sub LoadTimePlan()
    ...
    If Val(vsfPlan.TextMatrix(vsfPlan.Row, vsfPlan.ColIndex("序号控制"))) = 1 Then
        strSQL = "Select 序号, To_Char(开始时间,'hh24:mi:ss') As 开始时间, 开始时间 As 序号时间, To_Char(终止时间,'hh24:mi:ss') As 终止时间, 是否预约, Decode(是否停诊,1,6,挂号状态) As 挂号状态, 名称, 类型 From 临床出诊序号控制 Where 记录id = [1] And 开始时间 <> 终止时间 Order By 序号,开始时间"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, Val(vsfPlan.RowData(vsfPlan.Row)))
