...
Private Function IsValied(Optional ByVal blnSave As Boolean = False) As Boolean
    ...
    If mEditType = ed_计划安排 Then
        '新增加计划时,检查
        If CheckExistsBooking = False Then Exit Function
    End If
...
Private Function IsValidation() As Boolean
    '检查 生效时间是否合法
    On Error GoTo errH
    If mEditType <> Ed_安排审核 Then IsValidation = True: Exit Function
    If CheckExistsBooking = False Then Exit Function
    IsValidation = True
    Exit Function
errH:
 ...
 Private Function CheckExistsBooking() As Boolean
    ...
    strSQL = " Select 1 From 病人挂号记录 Where 号别=[1] and 记录状态 In (0, 1) and " & _
                  " 发生时间 between [2] and [3] and 发生时间>登记时间 "
                  
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, txt号别.Text, dtpBegin, dtpEndDate)
    If rsTmp.EOF Then CheckExistsBooking = True: Exit Function
    If MsgBox("该计划在生效日期后已存在预约号,是否继续?", vbYesNo + vbDefaultButton1 + vbInformation, Me.Caption) = vbNo Then
        CheckExistsBooking = False: Exit Function
    End If
