...
Begin VB.Form frmCooperateRegArrange 
   BorderStyle     =   0  'None
   ClientHeight    =   7965
   ClientLeft      =   0
   ClientTop       =   0
   ...
   Begin VB.PictureBox picUnitReg 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsUnits 
         ...
         FormatString    =   $"frmCooperateRegArrange.frx":0000
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   110
   ...
   Begin VB.PictureBox PicUnit 
      ...
      Begin VB.ListBox lstUnits 
         ...
         EndProperty
         Height          =   2910
         ItemData        =   "frmCooperateRegArrange.frx":00D5
         Left            =   0
         List            =   "frmCooperateRegArrange.frx":00D7
         Sorted          =   -1  'True
         TabIndex        =   1
         Top             =   960
         Width           =   2295
   ...
   End
End
Attribute VB_Name = "frmCooperateRegArrange"
...
Public Function SaveData() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '日期:2013-10-29 14:00:33
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim i As Long, j As Long, strSQL As String, strTmp As String
    Dim cllPro As New Collection
    Dim str合作单位 As String, strPre合作单位 As String
    Dim varCol As Variant
    Dim strDisable As String
    
    If mblnChange Then Call MoveUnitReg
    
    If mbln时段 Then
        For i = 0 To lstUnits.ListCount - 1
            mrsUnitsReg.Filter = "合作单位='" & lstUnits.List(i) & "'"
             strSQL = "Zl_合作单位安排控制_Delete(" & mlng安排ID & ",'" & lstUnits.List(i) & "')"
            ...
            If mrsUnitsReg.RecordCount > 0 Then
                With mrsUnitsReg
                    ...
                    If strTmp <> "" And strDisable = "" Then
                        strSQL = "Zl_合作单位安排控制_Insert(" & mlng安排ID & ",'" & lstUnits.List(i) & "','" & strTmp & "')"
                        zlAddArray cllPro, strSQL
                    End If
                End With
            ...
            End If
            If strDisable <> "" Then
                strSQL = "Zl_合作单位安排控制_Insert(" & mlng安排ID & ",'" & lstUnits.List(i) & "',Null,Null,'" & strDisable & "')"
                zlAddArray cllPro, strSQL
            End If
        Next
    End If
    If Not mbln时段 Then
        With vsUnits
            For i = 2 To .Rows - 1
               ...
               If str合作单位 <> "" Then
                    strSQL = "Zl_合作单位安排控制_Delete(" & mlng安排ID & ",'" & str合作单位 & "')"
                    zlAddArray cllPro, strSQL
                    'Zl_合作单位安排控制_Insert
                    ...
                    If .Cell(flexcpChecked, i, .Cols - 1, i, .Cols - 1) = 1 Then
                        ...
                        strSQL = "Zl_合作单位安排控制_Insert(" & mlng安排ID & ",'" & str合作单位 & "',Null,Null,'" & strTmp & "')"
                        zlAddArray cllPro, strSQL
                    Else
                        strTmp = ""
                        For j = 1 To .Cols - 2
                        ...
                        If strTmp <> "" Then
                            strTmp = Mid(strTmp, 2)
                            strSQL = "Zl_合作单位安排控制_Insert(" & mlng安排ID & ",'" & str合作单位 & "','" & strTmp & "')"
                            zlAddArray cllPro, strSQL
                        End If
                    End If
               End If
...
Public Function frmInit(ByVal lng安排ID As Long, _
        ...
        End If
          With vsUnits
            ...
            For i = 0 To UBound(strList)
                mrsSource.Filter = "限制项目='" & Split(strList(i), "|")(0) & "'"
                 If mrsSource.RecordCount > 0 Then
                    ...
                    Do While Not mrsSource.EOF
                        ...
                        mrsUnitsReg.Filter = "限制项目='" & Split(strList(i), "|")(0) & "' and 合作单位='" & lstUnits.Text & "' and 序号=" & Val(Nvl(mrsSource!序号))
                        If mrsUnitsReg.RecordCount > 0 Then
...
'-=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=-
'Name        : MoveUnitReg
'Description : 对挂号合作单位的号,进行重新计算
'Author      : 李光福
'Date        : 07-11-2012 16:14:06
'Input       :
'Output      :
'-=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=--=-
Private Sub MoveUnitRegTime(ByVal str挂号合作单位 As String)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:时间段更改到本地数据集中
    '编制:刘兴洪
    '---------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub MoveUnitReg(Optional ByVal str合作单位 As String)
    ...
    End If
    If mbln时段 Then
        Call MoveUnitRegTime(str挂号合作单位)
        Exit Sub
    End If
End Sub
...
Public Function InitData() As Boolean
    ...
    "   From 挂号安排 A,收费项目目录 B,部门表 D " & "   Where A.项目id=b.Id(+) And A.科室id =d.Id(+) " & _
    "         And A.Id=[1]"
    ...
    "               限制数量,是否预约" & _
    "   From  挂号安排时段 " & "   Where 安排ID=[1]" & "   Order by 排序,时点,序号"
...
Private Function InitPage() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '返回:成功返回true,否则返回False
    '编制:李光福(刘兴洪更改)
    '日期:2013-10-29 13:51:58
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim i As Long, j As Long
    Dim varCol As Variant, str限制项目 As String
    Dim lng限制数量     As Long, varData As Variant
    On Error GoTo errHandle
...
Private Function InitRs()
    ...
    '已经分配序号
    strSQL = "Select 合作单位, 安排id, 限制项目, 序号, 数量 From 合作单位安排控制  Where 安排ID=[1]"
...
Private Function chkExists时段(ByVal lng安排ID As Long) As Boolean
    ...
    On Error GoTo Hd
    strSQL = "Select 安排id From 挂号安排时段 A Where 安排ID=[1] And RowNum=1"
