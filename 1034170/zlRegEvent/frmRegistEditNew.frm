...
Begin VB.Form frmRegistEditNew 
   ...
   Begin MSComctlLib.StatusBar stbThis 
      ...
      _ExtentX        =   26194
      _ExtentY        =   635
      SimpleText      =   $"frmRegistEditNew.frx":7DE8
      _Version        =   393216
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Panel1 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            ...
            Object.Width           =   2619
            MinWidth        =   882
            Picture         =   "frmRegistEditNew.frx":7E2F
            Text            =   "中联软件"
            TextSave        =   "中联软件"
            Key             =   "ZLFLAG"
   ...
   Begin VB.PictureBox picPlan 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfList 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmRegistEditNew.frx":86C3
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin XtremeDockingPane.DockingPane dkpMain 
      Bindings        =   "frmRegistEditNew.frx":87CF
      Left            =   1095
      Top             =   570
      _Version        =   589884
      _ExtentX        =   450
   ...
   Begin VB.Image imgDel 
      Height          =   240
      Left            =   2670
      Picture         =   "frmRegistEditNew.frx":87E3
      Top             =   705
      Width           =   240
   End
End
...
Private mblnOnlyDelBill As Boolean '只退单据'如果存在挂号费,只退单据
...
Private mlngEMPI病人ID As Long '接口中的病人
...
Private Sub cbo年龄单位_Validate(Cancel As Boolean)
    If txt年龄.Text <> "" Then Call Led欢迎信息(2)
End Sub
...
Private Sub cbo性别_Click()
    ...
    If cbo性别.Tag <> cbo性别.Text And txt号别.Text <> "" Then
        Call ShowRegistFromInput
    End If
...
Private Sub cbo性别_Validate(Cancel As Boolean)
    Call Led欢迎信息(1)
End Sub
...
Private Sub chkShowAll_Click()
    If mblnUnChkClick Or mblnReadBooking Then Exit Sub
    Call ShowPlans
End Sub
...
Private Function IsCheckBackExtra(Optional ByVal bln病历费 As Boolean) As Boolean
    ...
    If Not mrsBillAdvance Is Nothing Then
        ...
        Do While Not mrsBillAdvance.EOF
            If InStr(",7,8,", "," & Nvl(mrsBillAdvance!性质) & ",") > 0 And (Val(Nvl(mrsBillAdvance!记录性质)) <> 1 And Val(Nvl(mrsBillAdvance!记录性质)) <> 11) Then
                ...
                Exit Function
            End If
            If InStr(",3,", "," & Nvl(mrsBillAdvance!性质) & ",") > 0 And (MCPAR.不收病历费 = False Or Not bln病历费) Then
...
'功能:清除病人相关信息
    ...
    With vsfPay
        For i = 1 To .Rows - 1
            If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                .TextMatrix(i, .ColIndex("余额")) = 0
            End If
        Next i
    End With
    mdbl个帐余额 = 0
...
Private Sub vsfPay_AfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsfPay
        If Col = 1 Then
            str应缴 = txt本次应缴.Text
            txt本次应缴.Text = Format(Val(txt本次应缴.Text) + mdbl原金额 - Val(.TextMatrix(Row, .ColIndex("金额"))), "0.00")
            If txt本次应缴.Text < mcur应缴 Or Val(.TextMatrix(Row, .ColIndex("金额"))) > Val(.TextMatrix(Row, .ColIndex("余额"))) Then
                txt本次应缴.Text = str应缴
                .TextMatrix(Row, .ColIndex("金额")) = Format(mdbl原金额, "0.00")
            Else
                .TextMatrix(Row, .ColIndex("金额")) = Format(Val(.TextMatrix(Row, .ColIndex("金额"))), "0.00")
            End If
        End If
        If Col = 2 Then
            If zlCommFun.ActualLen(.TextMatrix(Row, .ColIndex("结算号码"))) > 30 Then
...
Private Sub vsfPay_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    With vsfPay
        ...
        If Col = 1 Then
            If Val(.TextMatrix(Row, .ColIndex("金额修改"))) <> 1 Then
                Cancel = True
            Else
                .ComboList = ""
                mdbl原金额 = Val(.TextMatrix(Row, .ColIndex("金额")))
            End If
        End If
        ...
        If Col = 0 Then
            If mbln连续挂号 Then
                Cancel = True
            Else
                If Val(.TextMatrix(Row, .ColIndex("修改"))) = 1 Then
                    .ComboList = "..."
                    .CellButtonPicture = imgDel
                Else
                    Cancel = True
        ...
        If Col = 2 Then
            If Val(.RowData(Row)) = 2 Then
                .ComboList = ""
            Else
                If Val(.RowData(Row)) <> 8 And Val(.RowData(Row)) <> 7 And .TextMatrix(Row, .ColIndex("支付方式")) Like "*卡*" Then
                    .ComboList = ""
                Else
                    Cancel = True
                End If
...
Private Sub vsfPay_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsfPay
        ...
        If mbytMode = 4 Or chkCancel.Value = 1 Then
            If Val(.RowData(Row)) = 7 Or Val(.RowData(Row)) = 8 And Val(.TextMatrix(Row, .ColIndex("修改"))) = 0 Then
                If InStr(mstrCardPrivs, ";三方退款强制退现;") = 0 Then
                    ...
                    If str退款操作员 = "" Then
                        ...
                        Exit Sub
                    End If
                    mstrForceNote = str退款操作员 & "强制退现:" & .TextMatrix(Row, .ColIndex("结算方式")) & "," & .TextMatrix(Row, .ColIndex("金额")) & "元"
                Else
                    If MsgBox(.TextMatrix(Row, .ColIndex("支付方式")) & _
                              "不支持退现，是否强制退现？", vbQuestion + vbYesNo, gstrSysName) <> vbYes Then Exit Sub
                    mstrForceNote = UserInfo.姓名 & "强制退现:" & _
                                    .TextMatrix(Row, .ColIndex("结算方式")) & "," & .TextMatrix(Row, .ColIndex("金额")) & "元"
                End If
            End If
        
            dblMoney = Val(.TextMatrix(Row, .ColIndex("金额")))
            .RemoveItem Row
            blnFind = False
            For i = 1 To .Rows - 1
                If .RowData(i) = 1 Then
                    .TextMatrix(i, .ColIndex("金额")) = Format(Val(.TextMatrix(i, .ColIndex("金额"))) + dblMoney, "0.00")
                    blnFind = True
                End If
            ...
            If blnFind = False Then
                ...
                For i = 1 To .Rows - 1
                    If .TextMatrix(i, .ColIndex("支付方式")) = "" Then
                        blnFind = True
                        .TextMatrix(i, .ColIndex("支付方式")) = Nvl(rsTemp!名称)
                        .TextMatrix(i, .ColIndex("金额")) = Format(dblMoney, "0.00")
                        .TextMatrix(i, .ColIndex("修改")) = "0"
                        .RowData(i) = 1
                    End If
                    If blnFind Then Exit For
                Next i
                ...
                If blnFind = False Then
                    ...
                    .TextMatrix(.Rows - 1, .ColIndex("金额")) = Format(dblMoney, "0.00")
                    .TextMatrix(.Rows - 1, .ColIndex("修改")) = "0"
                    .RowData(.Rows - 1) = 1
                End If
            End If
        Else
            dblMoney = Val(.TextMatrix(Row, 1))
...
Private Sub vsfPlan_EnterCell()
    ...
    '125595:李南春，2018/10/22，预约接收出诊记录定位问题
...
Private Sub txtIDCard_Validate(Cancel As Boolean)
    ...
    If txtIDCard.Text = "" Or txtIDCard.Tag = txtIDCard.Text Then
        ...
        End If
        If txtIDCard.Tag <> txtIDCard.Text Then
             If txt号别.Text <> "" And txtPatient.Text <> "" Then
                mblnBuyHisBook = True
                Call ShowRegistFromInput
                mblnBuyHisBook = False
            End If
    ...
    '127920:李南春，2018/7/5，通过身份证识别病人也要初始化病人可用费别
...
Private Sub zlInusreIdentify()
    ...
    If cur余额 > 0 Then
        ...
        With vsfPay
            For i = 1 To .Rows - 1
                curPayed = curPayed + Val(.TextMatrix(i, .ColIndex("金额")))
            Next i
            For i = 1 To .Rows - 1
                If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                    .TextMatrix(i, .ColIndex("余额")) = mdbl预交余额
                    If gblnPrePayPriority Then
                        If mdbl预交余额 > curTotal - curPayed Then
                            .TextMatrix(i, .ColIndex("金额")) = Format(curTotal - curPayed, "0.00")
                        Else
                            .TextMatrix(i, .ColIndex("金额")) = Format(mdbl预交余额, "0.00")
                        End If
                        Call Set连续挂号
                    End If
                End If
...
Private Sub Led欢迎信息(Optional bytMode As Byte)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:
    '入参:bytMode-0-姓名;1-性别;2-年龄;3-身份证号识别或按身份证查找
    '出参:
    '返回:成功返回true,否则返回False
    '编制:刘兴洪
    '日期:2018-05-30 17:10:56
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If mbytMode = 0 And mbytInState = 0 And gblnLED Then
        ...
        If bytMode = 0 Then zl9LedVoice.Speak "#1"

        If Not mrsInfo Is Nothing Then
            If mrsInfo.State = 1 Then strInfo = strInfo & " " & mrsInfo!性别 & " " & mrsInfo!年龄: lngPatient = Val("" & mrsInfo!病人ID)
        ElseIf bytMode = 1 Then
            strInfo = strInfo & " " & NeedName(cbo性别.Text)
        ElseIf bytMode = 2 Or bytMode = 3 Then
            strInfo = strInfo & " " & NeedName(cbo性别.Text) & " " & txt年龄.Text & cbo年龄单位.Text
        End If
...
Private Sub GetPatient(objCard As zlIDKind.Card, ByVal strInput As String, ByVal blnCard As Boolean, Optional blnInputIDCard As Boolean = False, Optional ByRef Cancel As Boolean)
    ...
    '问题号:54399
    '74354:刘尔旋,2014-06-25,SQL语句字段书写错误
    If objCard.名称 Like "IC卡号" Or objCard.名称 Like "IC卡" Then '这里IC卡的情况应该取IC的密码
        strSQL = "Select  A.病人ID,A.门诊号,A.住院号,A.就诊卡号,A.费别,A.医疗付款方式,A.姓名,A.性别,A.年龄,A.出生日期,A.出生地点,A.身份证号,A.其他证件,A.身份,A.职业,A.民族,A.病人类型, " & _
                 ...
                 "B.名称 险类名称,C.密码 As 卡验证码,A.结算模式,A.手机号 From 病人信息 A,保险类别 B,病人医疗卡信息 C Where A.险类 = B.序号(+) And A.停用时间 is NULL And A.病人ID= C.病人ID(+) And C.卡号= '" & UCase(strInput) & "'"
    Else
    ...
    '刘兴洪:28439 And mstrYBPati = ""
   '‘ If (blnCard Or IDKind.IDKind = IDKindDefaultKind) And mstrYBPati = "" And Not (Left(strInput, 1) = "-" And IsNumeric(Mid(strInput, 2))) Then
    'If (blnCard Or IDKind.IDKind = IDKindDefaultKind) And mstrYBPati = "" And Not (Left(strInput, 1) = "-" And IsNumeric(Mid(strInput, 2))) Then
        
'        If blnCard And IDKind.IDKind <> IDKind.GetKindIndex("姓名") _
'            And IDKind.IDKind <> IDKindDefaultKind Then
'            lng卡类别ID = IDKind.GetCurCard.接口序号
'            blnOtherType = True
'        Else
...
'        End If
    ...
    ElseIf blnInputIDCard Then  '单独的身份证识别
        ...
        lng病人ID = GetPatiID(mlngModul, Me, strInput, txtPatient, Trim(txtPatient.Text), IIf(Trim(txtPatient.Text) <> "", NeedName(cbo性别.Text), ""), blnCancel)
                ...
                Else
                    ...
                    If mbytMode <> 1 And mstrYBPati = "" Then
                        ...
                        If cur余额 > 0 Then
                            ...
                            With vsfPay
                                For i = 1 To .Rows - 1
                                    If .RowData(i) = 0 And vsfPay.TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                                        .TextMatrix(i, .ColIndex("余额")) = mdbl预交余额
                                    End If
                                Next i
                            End With
...
'                                txt预交支付.Text = "0.00"
    ...
    If blnInputIDCard And Not mrsInfo Is Nothing Then
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strInput, Val(Mid(strInput, 2)), strTemp)
        If rsTmp.EOF Then
    ...
    Else
ReadPati:
        Set mrsInfo = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strInput, Val(Mid(strInput, 2)), strTemp)
    End If
...
'功能：显示/隐藏预交支付信息
    ...
    With vsfPay
        If Not blnShow Then
            ...
            For i = 1 To .Rows - 1
                If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then .RowHidden(i) = True
            Next i
        Else
            For i = 1 To .Rows - 1
                If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then .RowHidden(i) = False
            Next i
        End If
    End With
End Sub
...
'功能：显示/隐藏医保个人帐户支付信息
    ...
    With vsfPay
        If Not blnShow Then
            ...
            For i = 1 To .Rows - 1
                If .RowData(i) = 3 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then .RowHidden(i) = True
            Next i
        Else
            If MCPAR.使用个人帐户 Then
                For i = 1 To .Rows - 1
                    If .RowData(i) = 3 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then .RowHidden(i) = False
                Next i
            End If
        End If
    End With
...
Private Function zlExcuteDelRegistered() As Boolean
    ...
    Dim dblCheckThreeMoney As Double, dbl预交 As Double
    Dim bln打印 As Boolean, str卡类别ID As String
    ...
    If Not bln记帐 Then
        ...
        If Not objCard Is Nothing Then
            ...
            With vsfPay
                For i = 1 To .Rows - 1
                    If .RowData(i) = 1 Then
                        strTemp = .TextMatrix(i, .ColIndex("支付方式"))
                    End If
                    str卡类别ID = Nvl(.TextMatrix(i, .ColIndex("卡类别ID")))
                    If str卡类别ID <> "" Then
                        str卡类别ID = Mid(str卡类别ID, 1, InStr(str卡类别ID, "|") - 1)
                        If Val(str卡类别ID) = objCard.接口序号 Then
                            bln原样退 = True
                        End If
                    End If
                Next i
    ...
    With vsfPay
        For i = 1 To .Rows - 1
            If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                dbl预交支付 = dbl预交支付 + Val(.TextMatrix(i, .ColIndex("金额")))
            End If
        Next i
    End With
    ...
    If mintInsure <> 0 Then
        ...
        If bln记帐 = False Then
            ...
            With vsfPay
                For i = 1 To .Rows - 1
                    If (.TextMatrix(i, 0) = mstr个人帐户) And .TextMatrix(i, .ColIndex("支付方式")) <> "" And .RowHidden(i) = False Then
                        bln医保原样退 = True
                    End If
                Next i
            End With
    ...
    With vsfPay
        For i = 1 To .Rows - 1
            If .RowData(i) = 1 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                str现金 = .TextMatrix(i, .ColIndex("支付方式"))
                dbl现金 = Val(.TextMatrix(i, .ColIndex("金额")))
                Exit For
            End If
        Next i
        dbl预交 = 0
        For i = 1 To .Rows - 1
            If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" And Val(.TextMatrix(i, .ColIndex("金额"))) <> 0 Then
                dbl预交 = dbl预交 + Val(.TextMatrix(i, .ColIndex("金额")))
            End If
        Next i
        If str现金 = "" Then
            strSQL = "Select 名称 From 结算方式 Where 性质=1 Order By 缺省标志 Desc"
        ...
        For i = 1 To .Rows - 1
            If .TextMatrix(i, 0) <> "" Then
                If .TextMatrix(i, .ColIndex("支付方式")) = mstr个人帐户 Then
                    If Val(.TextMatrix(i, .ColIndex("金额"))) <> 0 Then
                        If InStr(strAdvance, mstr个人帐户) <> 0 Then
                            dbl现金 = dbl现金 + Val(.TextMatrix(i, .ColIndex("金额")))
                        Else
                            strBalance = strBalance & "|" & .TextMatrix(i, .ColIndex("支付方式")) & "," & Val(.TextMatrix(i, .ColIndex("金额"))) & ",0"
                        End If
                    End If
                Else
                    If .RowData(i) = 7 Or .RowData(i) = 8 Then
                        strBalance = strBalance & "|" & .TextMatrix(i, .ColIndex("结算方式")) & "," & Val(.TextMatrix(i, .ColIndex("金额"))) & ",1"
                    Else
                        If .RowData(i) <> 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" And .TextMatrix(i, .ColIndex("支付方式")) <> str现金 Then
                            strBalance = strBalance & "|" & .TextMatrix(i, .ColIndex("支付方式")) & "," & Val(.TextMatrix(i, .ColIndex("金额"))) & ",0"
                        End If
                    End If
                End If
    ...
    strSQL = strSQL & IIf(dbl预交 = 0, "Null", dbl预交) & ","
    '  回收票据号_In Varchar2 := Null
    strSQL = strSQL & "'" & strBackInvoice & "',"
    '  交易说明_In   病人预交记录.交易说明%Type := Null
    strSQL = strSQL & "'" & IIf(mstrForceNote = "", "", mstrForceNote) & "')"
    zlAddArray cllPro, strSQL
...
Private Function CheckInputValied() As Boolean
    ...
    '128436:李南春，2018/7/7，医生站没有结算方式这时候要检查是否使用了预交或个人账户
    '74550,冉俊明,2014-7-2,在病人来院就诊,医生在门诊医生站挂号时能够选择结算方式(包含性质为7的一卡通结算)
    If mbytMode <> 1 And (mblnStation And Not mblnStationPrice) Then
        If cbo结算方式.ListIndex = -1 And Not blnPrice And Not mRegistFeeMode = EM_RG_记帐 And mdbl个帐余额 = 0 And mdbl预交余额 = 0 Then
...
'        Exit Function
    ...
    If Not mrsExpenses Is Nothing Then
        ...
        Do While Not mrsExpenses.EOF
            If Val(Nvl(mrsExpenses!项目ID)) <> 0 Then
                If CheckServeRange(0, Val(Nvl(mrsExpenses!项目ID))) = False Then Exit Function
            End If
    ...
    With vsfPay
        For i = 1 To .Rows - 1
            If .TextMatrix(i, .ColIndex("支付方式")) <> "" And .RowData(i) = 0 Then
                dbl预交支付 = Val(.TextMatrix(i, .ColIndex("金额")))
                Exit For
            End If
        Next i
    End With
...
Private Function CheckPayStyleValied(ByRef lngRow As Long) As Boolean
    ...
    With vsfPay
        ...
        If mbln连续挂号 Then
            For i = 1 To .Rows - 1
                If .TextMatrix(i, .ColIndex("支付方式")) = NeedName(cbo结算方式.Text) Then
                    If Val(txt缴款.Text) <> 0 Then
        ...
        For i = 1 To .Rows - 1
            If .TextMatrix(i, .ColIndex("支付方式")) = NeedName(cbo结算方式.Text) And cbo结算方式.Enabled And ((Val(txt缴款.Text) <> 0 And Val(txt本次应缴.Text) = 0) Or (Val(txt缴款.Text) = 0 And Val(txt本次应缴.Text) <> 0) Or (Val(txt缴款.Text) <> 0 And Val(txt本次应缴.Text) <> 0)) Then
                If Val(.TextMatrix(i, .ColIndex("金额"))) <> 0 Then
                ...
                End If
            End If
            If .TextMatrix(i, .ColIndex("支付方式")) = "" Then lngRow = i
        Next i
        If lngRow = 0 Then lngRow = .Rows: .Rows = .Rows + 1
        ...
        If Val(rsTemp!性质) = 3 Or Val(rsTemp!性质) = 8 Or Val(rsTemp!性质) = 7 Then
            For i = 1 To .Rows - 1
                If (Val(.RowData(i)) = 3 Or Val(.RowData(i)) = 7 Or Val(.RowData(i)) = 8) And Val(.TextMatrix(i, .ColIndex("金额"))) <> 0 Then
...
Private Sub RestorePay()
    ...
    With vsfPay
        For i = 1 To .Rows - 1
            If (.RowData(i) = 1 Or .RowData(i) = 2) And .TextMatrix(i, .ColIndex("支付方式")) = NeedName(cbo结算方式.Text) Then
                If Val(.TextMatrix(i, .ColIndex("已支付"))) = 0 Then
                    txt本次应缴.Text = Format(Val(.TextMatrix(i, .ColIndex("金额"))), "0.00")
                    For j = 0 To .Cols - 1
                        .TextMatrix(i, j) = ""
                    Next j
                    .RowData(i) = ""
                Else
                    txt本次应缴.Text = Format(Val(.TextMatrix(i, .ColIndex("金额"))), "0.00")
                    .TextMatrix(i, 1) = Format(.TextMatrix(i, .ColIndex("已支付")), "0.00")
                End If
            End If
        Next i
    End With
...
'---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Dim lng记录ID As Long, lngNoSort As Long, lng身份证类别ID As Long, bln是否签约 As Boolean
    ...
    Else
        ...
        If cbo结算方式.Visible Then
            ...
            mblnPre连续 = mbln连续挂号
            mbln连续挂号 = False
            '137598：李南春，2019/1/31，尚未完成支付，所有支付方式都可以改
            If lngRow <> 0 Then
                If Val(txt缴款.Text) = 0 Then
                    int性质 = Get性质(NeedName(cbo结算方式.Text), strStyle)
                    If gbln缴款结束 And int性质 <> 7 And int性质 <> 8 Then
                        With vsfPay
                            .TextMatrix(lngRow, .ColIndex("支付方式")) = NeedName(cbo结算方式.Text)
                            .TextMatrix(lngRow, .ColIndex("修改")) = 1
                            If Val(txt本次应缴.Text) <> 0 Then
                                .TextMatrix(lngRow, .ColIndex("金额")) = Format(Val(.TextMatrix(lngRow, .ColIndex("金额"))) + Val(txt本次应缴.Text) - mcur应缴, "0.00")
                            End If
                    ...
                    Else
                        With vsfPay
                            .TextMatrix(lngRow, .ColIndex("支付方式")) = NeedName(cbo结算方式.Text)
                            .TextMatrix(lngRow, .ColIndex("修改")) = 1
                            .TextMatrix(lngRow, .ColIndex("金额")) = Format(Val(txt本次应缴.Text), "0.00")
                            .RowData(lngRow) = int性质
                            .TextMatrix(lngRow, .ColIndex("结算方式")) = strStyle
                        End With
                        txt本次应缴.Text = "0.00"
                ...
                Else
                    ...
                    Else
                        With vsfPay
                            .TextMatrix(lngRow, .ColIndex("支付方式")) = NeedName(cbo结算方式.Text)
                            .TextMatrix(lngRow, .ColIndex("修改")) = 1
                            .TextMatrix(lngRow, .ColIndex("金额")) = Format(Val(txt缴款.Text), "0.00")
                            .RowData(lngRow) = Get性质(NeedName(cbo结算方式.Text), strStyle)
                            .TextMatrix(lngRow, .ColIndex("结算方式")) = strStyle
                        End With
...
InvoiceHandle:
        ...
        '挂号费用信息
        '各类金额
        If Not blnPrice And Not bln存为划价单 And mbytMode <> 1 Then
            curCard = 0
            mstrCard结算方式 = ""
            If Not mrsItems Is Nothing Then
                mrsItems.Filter = "性质=4"
            ...
            '137473:李南春，2019/1/25，三方卡(消费卡)支付时区分支付费用
            Call ClearCardMoney
            With vsfPay
                For i = 1 To .Rows - 1
                    If .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                        If .RowData(i) = 0 Then
                            cur预交 = Val(.TextMatrix(i, .ColIndex("金额")))
                        Else
                            If .RowData(i) = 3 Then
                                cur个帐 = Val(.TextMatrix(i, .ColIndex("金额")))
                            Else
                                If strFilter = "" Then
                                    strFilter = "结算方式='" & .TextMatrix(i, .ColIndex("结算方式")) & "'"
                                Else
                                    strFilter = strFilter & " Or 结算方式='" & .TextMatrix(i, .ColIndex("结算方式")) & "'"
                                End If
                                ...
                                If curCard <> 0 Then
                                    If Val(.TextMatrix(i, .ColIndex("金额"))) = curCard Then
                                        ...
                                        End If
                                    ElseIf Val(.TextMatrix(i, .ColIndex("金额"))) > curCard Then
                                        str结算方式 = str结算方式 & "|" & .TextMatrix(i, .ColIndex("结算方式")) & "," & _
                                                      Val(.TextMatrix(i, .ColIndex("金额"))) - curCard - Val(.TextMatrix(i, .ColIndex("已支付"))) & _
                                                      "," & .TextMatrix(i, .ColIndex("结算号码")) & "," & IIf(.RowData(i) = 7 Or .RowData(i) = 8, 1, 0)
                                        mstrCard结算方式 = .TextMatrix(i, .ColIndex("结算方式"))
                                        curCard = 0
                                        If .RowData(i) = 7 Or .RowData(i) = 8 Then
                                            mCurCardPay.Have卡费 = True
                                ...
                                Else
                                    str结算方式 = str结算方式 & "|" & .TextMatrix(i, .ColIndex("结算方式")) & "," & _
                                                  .TextMatrix(i, .ColIndex("金额")) - Val(.TextMatrix(i, .ColIndex("已支付"))) & "," & _
                                                  .TextMatrix(i, .ColIndex("结算号码")) & "," & IIf(.RowData(i) = 7 Or .RowData(i) = 8, 1, 0)
                                    If .RowData(i) = 7 Or .RowData(i) = 8 Then
                                        mCurCardPay.Have挂号费 = True
                                    End If
                                End If
                                cur现金 = cur现金 + Val(.TextMatrix(i, .ColIndex("金额"))) - Val(.TextMatrix(i, .ColIndex("已支付")))
                            End If
                        End If
                    End If
                Next i
                
                '128436:李南春，2018/7/7，需要检查付款金额是否满足支付本次费用
                If FormatEx(cur现金 + cur个帐 + cur预交, 6) < FormatEx(GetRegistMoney, 6) Then
                    MsgBox "支付金额少于本次应付金额，请输入正确的支付金额!", vbInformation, gstrSysName
                    RestorePay
                    Exit Sub
                End If
                If curCard > 0 Then
            ...
            If mblnOneCard And cur现金 <> 0 And mRegistFeeMode <> EM_RG_记帐 Then
                ...
                If mrsOneCard.RecordCount > 0 Then
                    ...
                    With vsfPay
                        For i = 1 To .Rows - 1
                            If .TextMatrix(i, .ColIndex("支付方式")) = Nvl(mrsOneCard!结算方式) Then
                                curOneCard = mobjICCard.GetSpare
                                If curOneCard < Val(.TextMatrix(i, .ColIndex("金额"))) Then
                                    MsgBox "卡上余额" & Format(curOneCard, "0.00") & ",本次要求支付金额" & Format(Val(.TextMatrix(i, .ColIndex("金额"))), "0.00"), vbInformation, gstrSysName
                                    RestorePay
                                    Exit Sub
                                Else
                                    curOneCard = Val(.TextMatrix(i, .ColIndex("金额")))
                                End If
                                Exit For
                            End If
                        Next i
            ...
            If mRegistFeeMode <> EM_RG_记帐 Then
                With vsfPay
                    For i = 1 To .Rows - 1
                        ...
                        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, .TextMatrix(i, .ColIndex("支付方式")))
                        If Not rsTemp.EOF Then
                            mCurCardPay.lng医疗卡类别ID = Nvl(rsTemp!ID)
                            mCurCardPay.bln消费卡 = False
                            mCurCardPay.str结算方式 = Nvl(rsTemp!结算方式)
                            mCurCardPay.str名称 = Nvl(rsTemp!名称)
                            If CheckBrushCard(CDbl(.TextMatrix(i, .ColIndex("金额")))) = False Then RestorePay: Exit Sub
                            dblThreeSwap = CDbl(.TextMatrix(i, .ColIndex("金额")))
                            Exit For
                        Else
                            strSQL = "Select 编号,名称,结算方式 From 卡消费接口目录 Where 名称= [1] "
                            Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, .TextMatrix(i, .ColIndex("支付方式")))
                            If Not rsTemp.EOF Then
                                mCurCardPay.lng医疗卡类别ID = Nvl(rsTemp!编号)
        ...
        If mcustomTime = t_时段 Then    '该时段代表只要所有安排中有一个安排设置了时段该mcustomTime的值都被设置为了t_时段（也就是说该条件成立）
            ...
            If Not mrs时间段 Is Nothing And dtpAppointmentTime.Visible = False And (mViewMode = v_专家号分时段 Or mViewMode = V_普通号分时段) And mbytMode <> 2 And mstrNoIn = "" Then
                mrs时间段.MoveLast
                With vsfPlan
        ...
        If blnPati Then
            With mobjfrmPatiInfo
                ...
                If mobjfrmPatiInfo.mbln允许签约 And Trim(txtIDCard.Text) = Trim(mobjfrmPatiInfo.txt身份证号.Text) And Trim(txtIDCard.Text) <> "" Then
                    bln是否签约 = 是否已经签约(Trim(txtIDCard.Text), lng身份证类别ID)
                    If Not bln是否签约 Then
                        strSQL = "Zl_医疗卡变动_Insert(" & "11," & lng病人ID & "," & lng身份证类别ID & ",NULL" & ",'" & Trim(txtIDCard.Text) & "'," & "'扫描身份证号签约'," & _
                                    "NULL," & "'" & UserInfo.姓名 & "'," & "to_date('" & Format(Datsys, "yyyy-mm-dd HH:MM:SS") & "','yyyy-mm-dd hh24:mi:ss'))"
                        zlAddArray cllProBefor, strSQL
                    End If
                End If
        ...
        If (mbytMode = 2 Or mbytMode = 0 And mbytInState = 0 And mstrNoIn <> "") And mTy_Para.bln预约接收确定挂号费 = False Then
            ...
            If Not mrsBill Is Nothing And (mbytMode = 2 Or (mbytMode = 0 And mbytInState = 0 And mstrNoIn <> "")) And InStr(1, mstrPrivs, ";允许修改费别;") > 0 Then
                ...
                Do While Not mrsBill.EOF
                    ...
                    strSQL = strSQL & "" & Format(GetActualMoney(str费别, mrsBill!收入项目ID, mrsBill!应收, mrsBill!收费细目ID), "0.00") & ","
                    '  病历费_In Number, --该条记录是否病历工本费  =3目前只发现一个用途，不更新病历费费用信息
                ...
                If Not mrsExpenses Is Nothing Then
                    lngNoSort = lngNoSort + 1
                    If GetSqlExpenses(cllPro, mrsExpenses, strNO, lngNoSort, False, str划价NO, lng病人ID, str门诊号, , str发生时间, str登记时间) = False Then
                        Exit Sub
                    End If
                End If
            End If
        Else
            ...
            '保存附加费
            If Not mrsExpenses Is Nothing Then
                If GetSqlExpenses(cllPro, mrsExpenses, strNO, k, bln存为划价单, str划价NO, lng病人ID, str门诊号, lngSN, str发生时间, str登记时间, blnNoPrint, str结算方式, strRoom, lng结帐ID, str医生姓名, lng医生ID) = False Then
                    Exit Sub
                End If
            End If
        End If
...
Private Function GetSqlExpenses(ByRef cllPro As Collection, ByVal rsExpenses As ADODB.Recordset, _
                    ...
                    Optional ByVal str医生姓名 As String, Optional ByVal lng医生ID As Long) As Boolean
    '获取附加费记录sql
    Dim str费别 As String, str年龄 As String, strSQL As String
    Dim byt复诊 As Byte
    ...
    For i = 1 To rsExpenses.RecordCount
        ...
        If (mbytMode = 2 Or mbytMode = 0 And mbytInState = 0 And mstrNoIn <> "") Then
            strSQL = "Zl_病人预约挂号记录_Update("
            '  单据号_In     门诊费用记录.NO%Type,
            strSQL = strSQL & "'" & strRegNo & "',"
            '  序号_In       门诊费用记录.序号%Type,
            strSQL = strSQL & "" & lngNoSort & ","
            '  价格父号_In   门诊费用记录.价格父号%Type,
            strSQL = strSQL & IIf(lngPre项目ID = rsExpenses!项目ID, int价格父号, "NULL") & ","
            '  从属父号_In   门诊费用记录.从属父号%Type,
            strSQL = strSQL & "NULL,"
            '  收费类别_In   门诊费用记录.收费类别%Type,
            strSQL = strSQL & "'" & rsExpenses!类别 & "',"
            '  收费细目id_In 门诊费用记录.收费细目id%Type,
            strSQL = strSQL & "'" & rsExpenses!项目ID & "',"
            '  数次_In       门诊费用记录.数次%Type,
            strSQL = strSQL & "" & Val(Nvl(rsExpenses!数次)) & ","
            '  标准单价_In   门诊费用记录.标准单价%Type,
            strSQL = strSQL & "" & Val(Nvl(rsExpenses!单价)) & ","
            '  收入项目id_In 门诊费用记录.收入项目id%Type,
            strSQL = strSQL & "" & Val(Nvl(rsExpenses!收入项目ID)) & ","
            '  收据费目_In   门诊费用记录.收据费目%Type,
            strSQL = strSQL & "'" & Trim(Nvl(rsExpenses!收据费目)) & "',"
            '  应收金额_In   门诊费用记录.应收金额%Type,
            strSQL = strSQL & "" & IIf(bln存为划价单, 0, rsExpenses!应收) & ","
            '  实收金额_In   门诊费用记录.实收金额%Type,
            strSQL = strSQL & "" & IIf(bln存为划价单, 0, rsExpenses!实收) & ","
            '  病历费_In Number, --该条记录是否病历工本费
            strSQL = strSQL & "0,"
            '  保险大类id_In 门诊费用记录.保险大类id%Type,
            strSQL = strSQL & "" & ZVal(Nvl(rsExpenses!保险大类id, 0)) & ","
            '  保险项目否_In 门诊费用记录.保险项目否%Type,
            strSQL = strSQL & "" & ZVal(Nvl(rsExpenses!保险项目否, 0)) & ","
            '  统筹金额_In   门诊费用记录.统筹金额%Type,
            strSQL = strSQL & "" & ZVal(Nvl(rsExpenses!统筹金额, 0)) & ","
            '  保险编码_In   门诊费用记录.保险编码%Type,
            strSQL = strSQL & "'" & Trim(Nvl(rsExpenses!保险编码)) & "',"
            '  病人科室id_In 门诊费用记录.病人科室id%Type,
            strSQL = strSQL & "" & mlng挂号科室ID & ","
            '  执行部门id_In 门诊费用记录.执行部门id%Type
            strSQL = strSQL & "" & IIf(rsExpenses!执行科室ID = 0, mlng挂号科室ID, rsExpenses!执行科室ID) & ")"
            Call zlAddArray(cllPro, strSQL)
        Else
            byt复诊 = Val(mobjfrmPatiInfo.chk复诊.Value)
            
            strSQL = _
            "zl_病人挂号记录_出诊_INSERT(" & ZVal(vsfPlan.TextMatrix(vsfPlan.Row, vsfPlan.ColIndex("记录ID"))) & "," & ZVal(lng病人ID) & "," & IIf(str门诊号 = "", "NULL", str门诊号) & ",'" & txtPatient.Text & "','" & NeedName(cbo性别.Text) & "'," & _
            "'" & str年龄 & "','" & NeedCode(cbo付款方式.Text) & "','" & str费别 & "','" & strRegNo & "'," & _
            "'" & IIf(blnNoPrint, "", txtFact.Text) & "'," & lngNoSort & "," & IIf(lngPre项目ID = rsExpenses!项目ID, int价格父号, "NULL") & ",NULL," & _
            ...
            mlng挂号科室ID & "," & IIf(mblnStation, mlng挂号科室ID, UserInfo.部门ID) & "," & IIf(rsExpenses!执行科室ID = 0, mlng挂号科室ID, rsExpenses!执行科室ID) & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & _
            str发生时间 & "," & str登记时间 & "," & _
            "'" & str医生姓名 & "'," & ZVal(lng医生ID) & ",0," & IIf(lbl急.Visible, 1, 0) & "," & _
            "'" & IIf(txt号别.Text = "+", "", txt号别.Text) & "','" & strRoom & "'," & ZVal(lng结帐ID) & "," & IIf(blnNoPrint, "NULL", ZVal(mlng领用ID)) & "," & _
            "0, 0, 0, " & ZVal(Nvl(rsExpenses!保险大类id, 0)) & "," & _
            ZVal(Nvl(rsExpenses!保险项目否, 0)) & "," & ZVal(Nvl(rsExpenses!统筹金额, 0)) & "," & _
            "'" & IIf(str划价NO <> "", "划价:" & str划价NO, Me.cbo备注.Text) & "'," & IIf(mbytMode = 1, 1, 0) & "," & IIf(gblnSharedInvoice, 1, 0) & ",'" & rsExpenses!保险编码 & "'," & byt复诊 & "," & ZVal(lngSN) & "," & ZVal(mint社区) & "," & _
            IIf(mbytMode = 2 Or chkBooking.Value = 1 Or mbytMode = 1, 1, 0) & "," & IIf(mbytMode = 1 Or chkBooking.Value = 1, "'" & Mid(cbo预约方式.Text, InStr(cbo预约方式.Text, ".") + 1) & "'", "NULL") & "," & _
            IIf(mTy_Para.bln挂号生成队列, 1, 0) & ","
            '卡类别id_In   病人预交记录.卡类别id%Type := Null,
            strSQL = strSQL & "" & IIf(mCurCardPay.Have挂号费 And mCurCardPay.lng医疗卡类别ID <> 0 And mCurCardPay.bln消费卡 = False, mCurCardPay.lng医疗卡类别ID, "NULL") & ","
        ...
        If bln存为划价单 Then
            ...
            "zl_门诊划价记录_Insert('" & str划价NO & "'," & lngNoSort & "," & lng病人ID & ",NULL," & _
                     ...
                     "NULL,1," & Val(Nvl(rsExpenses!数次)) & ",NULL," & IIf(rsExpenses!执行科室ID = 0, mlng挂号科室ID, rsExpenses!执行科室ID) & "," & _
...
Private Function ReadBooking(ByVal strNO As String) As Boolean
    ...
    Else
        mlng记录ID = 0
    End If
    If mblnUnload Then mstrNoIn = "": mblnReadBooking = False: Exit Function
...
Private Function ReadBill(strNO As String, Optional blnGetBooking As Boolean = False) As Boolean
    ...
    If mbytInState <= 1 Then
        ...
        strSQL = "" & _
                " Select A.NO,A.实际票号,A.序号,A.从属父号,A.标识号,D.病人类型," & _
                ...
                " Group by A.NO,A.实际票号,A.序号,A.从属父号,A.标识号,D.病人类型,A.病人ID,A.付款方式,D.医疗付款方式,F.医疗类别,A.姓名,A.性别,D.身份证号,D.家庭电话," & _
...
chkBooking:
    ...
    If mrsBill.RecordCount = 1 And Nvl(mrsBill!附加标志, 0) = 1 Then
        ...
        chk病历费.Enabled = False
        mbln包含病历费 = True
        '133819:李南春，2019/1/7,值勾选后会加载病历费收费项目，导致后面加载时多出来一条
    ...
    '128441:李南春,2018/7/6,退费时需要获取已退费用的结帐ID，查看单据只需要原始单据
...
'    txt预交支付.Tag = ""
    ...
    strSQL = "Select Mod(A.记录性质,10) as 记录性质,B.性质,A.结算方式, " & _
        ...
        " A.卡类别ID,A.结算卡序号,Nvl(C.是否退现, D.是否退现) as 是否退现 " & _
        ...
        " Group by Mod(A.记录性质,10),B.性质,A.结算方式,A.结算号码,C.名称,D.名称,a.卡类别id,c.是否退现, a.结算卡序号, d.是否退现" & _
...
'    vsfPay.RowHidden(1) = False
    ...
    For i = 1 To mrsBillAdvance.RecordCount
        If Val(Nvl(mrsBillAdvance!记录性质)) = 1 Or Val(Nvl(mrsBillAdvance!记录性质)) = 11 Then
        Else
            ...
            Case 7, 8    '一卡通相关
                If mobjPayCard Is Nothing Then
                    If bln退号处理 Then
                        ...
                        cbo结算方式.ListIndex = -1
                    Else
                        cbo结算方式.ListIndex = GetCboIndex(cbo结算方式, Nvl(mrsBillAdvance!结算方式))
                    End If
                    If cbo结算方式.ListIndex = -1 Then
                        cbo结算方式.AddItem Nvl(mrsBillAdvance!结算方式), 0
                        cbo结算方式.ListIndex = cbo结算方式.NewIndex
                    End If
                ...
                Else
                  cbo结算方式.Clear
                   ...
                   End If
                   cbo结算方式.AddItem IIf(Nvl(mobjPayCard.名称) = "", Nvl(mrsBillAdvance!结算方式), Nvl(mobjPayCard.名称))
            ...
            Case Else '1,2或其他
                ...
                Else
                  cbo结算方式.Clear
                   ...
                   End If
                   mobjDelCards.Add mobjPayCard
                    cbo结算方式.AddItem IIf(Nvl(mobjPayCard.结算方式) = "", Nvl(mrsBillAdvance!结算方式), Nvl(mobjPayCard.结算方式))
                    If (mobjPayCard.启用 Or cbo结算方式.ListIndex < 0 Or mobjPayCard.是否退现 = False) Then
                        cbo结算方式.ListIndex = cbo结算方式.NewIndex
                    End If
                End If
...
Private Function Load结算信息(ByVal dblTotal As Double, Optional ByVal dblDiff As Double) As Boolean
    ...
    For i = 1 To mrsBillAdvance.RecordCount
        If dblTotal <> 0 Then
            If dblTotal < Val(Nvl(mrsBillAdvance!金额)) Then
                dblBalance = dblTotal
                dblTotal = 0
            Else
            ...
            If Val(Nvl(mrsBillAdvance!记录性质)) = 1 Or Val(Nvl(mrsBillAdvance!记录性质)) = 11 Then
                With vsfPay
                    .TextMatrix(.Rows - 1, .ColIndex("支付方式")) = "冲预交"
                    .TextMatrix(.Rows - 1, .ColIndex("金额")) = Format(dblBalance, "0.00")
                    .TextMatrix(.Rows - 1, .ColIndex("修改")) = "0"
                    .TextMatrix(.Rows - 1, .ColIndex("结算号码")) = Nvl(mrsBillAdvance!结算号码)
                    .RowData(.Rows - 1) = 0
                End With
            Else
                With vsfPay
                    '问题号:116146,焦博,2017/11/24,退号时,结算方式显示的是医疗卡的结算方式，统一调整为医疗卡名称
                    .TextMatrix(.Rows - 1, .ColIndex("支付方式")) = Nvl(mrsBillAdvance!名称)
                    .TextMatrix(.Rows - 1, .ColIndex("金额")) = Format(dblBalance, "0.00")
                    .TextMatrix(.Rows - 1, .ColIndex("结算号码")) = Nvl(mrsBillAdvance!结算号码)
                    .TextMatrix(.Rows - 1, .ColIndex("结算方式")) = Nvl(mrsBillAdvance!结算方式)
                    ...
                    If Val(Nvl(mrsBillAdvance!性质, -1)) = 7 Or Val(Nvl(mrsBillAdvance!性质, -1)) = 8 Then
                        ...
                        bln消费卡 = Val(Nvl(mrsBillAdvance!结算卡序号)) <> 0
                        lng卡类别ID = Val(Nvl(mrsBillAdvance!卡类别ID))
                        If lng卡类别ID = 0 Then lng卡类别ID = Val(Nvl(mrsBillAdvance!结算卡序号))
                        .TextMatrix(.Rows - 1, .ColIndex("卡类别ID")) = lng卡类别ID & "|" & IIf(bln消费卡, 1, 0)  '卡类别ID|是否消费卡
                        .TextMatrix(.Rows - 1, .ColIndex("修改")) = IIf(Val(Nvl(mrsBillAdvance!是否退现)) = 1, "1", "0")
                    End If
...
Reg:
    ...
    Else
        ...
        If Not mrsExpenses Is Nothing Then
            ...
            Do While Not mrsExpenses.EOF
                 For j = 1 To vsfMoney.Rows - 1
                    If Trim(vsfMoney.TextMatrix(j, 0)) = mrsExpenses!项目名称 Then
                        cur合计 = cur合计 + Val(vsfMoney.TextMatrix(j, 2))
                        Exit For
                    End If
                 Next
...
Private Sub InitVsfPay(Optional ByVal bln连续挂号 As Boolean)
        ...
        With vsfPay
            For i = 1 To .Rows - 1
                If .RowData(i) = 0 And .TextMatrix(i, 0) = "预交金" Then
                    .TextMatrix(i, .ColIndex("支付方式")) = "预交金"
                    .TextMatrix(i, .ColIndex("金额")) = "0.00"
                    .TextMatrix(i, .ColIndex("结算号码")) = ""
                    .TextMatrix(i, .ColIndex("修改")) = 0
                    .TextMatrix(i, .ColIndex("结算方式")) = "预交金"
                    .TextMatrix(i, .ColIndex("金额修改")) = 1
                    .TextMatrix(i, .ColIndex("余额")) = 0
                    If mdbl预交余额 = 0 Then .RowHidden(i) = True
                End If
                If .RowData(i) = 3 And .TextMatrix(i, 0) <> "" And mstr个人帐户 <> "" Then
                    .TextMatrix(i, .ColIndex("支付方式")) = mstr个人帐户
                    .TextMatrix(i, .ColIndex("金额")) = "0.00"
                    .TextMatrix(i, .ColIndex("结算号码")) = ""
                    .TextMatrix(i, .ColIndex("修改")) = 0
                    .TextMatrix(i, .ColIndex("结算方式")) = mstr个人帐户
                    .TextMatrix(i, .ColIndex("金额修改")) = 1
                    .TextMatrix(i, .ColIndex("余额")) = 0
                    If mdbl个帐余额 = 0 Then .RowHidden(i) = True
                End If
                If (.RowData(i) = 1 Or .RowData(i) = 2) And .TextMatrix(i, .ColIndex("支付方式")) = NeedName(cbo结算方式.Text) Then
                    .TextMatrix(i, .ColIndex("已支付")) = Val(.TextMatrix(i, .ColIndex("金额")))
                Else
                    If .RowData(i) <> 0 And .RowData(i) <> 3 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                        .TextMatrix(i, .ColIndex("支付方式")) = "删除"
                        blnFinish = False
            ...
            Do While blnFinish = False
                For i = 1 To .Rows - 1
                    If .TextMatrix(i, .ColIndex("支付方式")) = "删除" Then
                        .RemoveItem i
                        Exit For
                    End If
    ...
    Else
        With vsfPay
            ...
            .Col = 0: .ColSel = .Cols - 1
            '加载预交金
            .TextMatrix(.Rows - 1, .ColIndex("支付方式")) = "预交金"
            .TextMatrix(.Rows - 1, .ColIndex("金额")) = "0.00"
            .TextMatrix(.Rows - 1, .ColIndex("结算号码")) = ""
            .TextMatrix(.Rows - 1, .ColIndex("修改")) = 0
            .TextMatrix(.Rows - 1, .ColIndex("结算方式")) = "预交金"
            .TextMatrix(.Rows - 1, .ColIndex("金额修改")) = 1
            .TextMatrix(.Rows - 1, .ColIndex("余额")) = 0
            .RowData(.Rows - 1) = 0
            If mdbl预交余额 = 0 Then .RowHidden(.Rows - 1) = True
            .Rows = .Rows + 1
            '加载个人帐户
            If mstr个人帐户 <> "" Then
                .TextMatrix(.Rows - 1, .ColIndex("支付方式")) = mstr个人帐户
                .TextMatrix(.Rows - 1, .ColIndex("金额")) = "0.00"
                .TextMatrix(.Rows - 1, .ColIndex("结算号码")) = ""
                .TextMatrix(.Rows - 1, .ColIndex("修改")) = 0
                .TextMatrix(.Rows - 1, .ColIndex("结算方式")) = mstr个人帐户
                .TextMatrix(.Rows - 1, .ColIndex("金额修改")) = 1
                .TextMatrix(.Rows - 1, .ColIndex("余额")) = 0
                .RowData(.Rows - 1) = 3
...
'功能：计算并显示当前医保病人个人帐户可以支持的金额
    ...
    With vsfPay
        For i = 1 To .Rows - 1
            If .RowData(i) = 3 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then lngYBRow = i
            If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then lngYJRow = i
        Next i
        ...
        If mintInsure <> 0 And mstr个人帐户 <> "" Then
            If gclsInsure.GetCapability(support挂号使用个人帐户, lng病人ID, mintInsure) Then
                If mdbl个帐余额 - cur合计 >= -1 * mcur个帐透支 Then
                    .TextMatrix(lngYBRow, .ColIndex("金额")) = Format(cur合计, "0.00")
                    .TextMatrix(lngYBRow, .ColIndex("余额")) = cur合计
                Else
                    If mblnStation Then
                        .TextMatrix(lngYBRow, .ColIndex("金额")) = "0.00"
                    ElseIf mcur个帐透支 = 0 And mdbl个帐余额 > 0 Then
                        .TextMatrix(lngYBRow, .ColIndex("金额")) = Format(mdbl个帐余额, "0.00")
                        .TextMatrix(lngYBRow, .ColIndex("余额")) = mdbl个帐余额
                    Else
                        .TextMatrix(lngYBRow, .ColIndex("金额")) = "0.00"
                    End If
                End If
            ...
            End If
        Else
            If lngYBRow <> 0 Then .TextMatrix(lngYBRow, .ColIndex("金额")) = "0.00"
        End If
        
        If gblnPrePayPriority And mdbl预交余额 >= Val(curTotal - Val(.TextMatrix(lngYBRow, .ColIndex("金额")))) Then
            .TextMatrix(lngYJRow, .ColIndex("金额")) = Format(curTotal - Val(.TextMatrix(lngYBRow, .ColIndex("金额"))), "0.00")
        End If
        If lngYJRow <> 0 Then .TextMatrix(lngYJRow, .ColIndex("余额")) = mdbl预交余额
    End With
    '获取医保统筹相关内容
    If mintInsure <> 0 And mstrYBPati <> "" Then
        If Not mrsItems Is Nothing Then
...
Private Sub ReCalc预约接收发卡()
    ...
    Dim str费别 As String, str年龄 As String
    ...
    mrsBill.MoveFirst
    Call ReadRegistPrice(mrsBill!收费细目ID, mbln包含病历费, mblnAddCardItem, str费别, rsItems, rsIncomes, mlng挂号科室ID)
    ...
    End If
    str年龄 = txt年龄.Text & IIf(cbo年龄单位.Visible, cbo年龄单位.Text, "")
    
    Call ReadRegistPrice(0, False, mblnAddCardItem, str费别, mrsItems, mrsInComes, mlng挂号科室ID)
    
    Call ReadExRegistPrice(mrsExpenses, mblnAppointPrice, lng病人ID, mintInsure, txt号别.Text, txtPatient.Text, NeedName(cbo性别), _
            str年龄, txtIDCard.Text, str费别, NeedName(cbo付款方式.Text))
    ...
    '显示卡费数据
     ...
     If Not mrsExpenses Is Nothing Then
        ...
        If mrsExpenses.RecordCount > 0 Then mrsExpenses.MoveFirst
        Do While Not mrsExpenses.EOF
            vsfMoney.Rows = vsfMoney.Rows + 1
            vsfMoney.TextMatrix(vsfMoney.Rows - 1, 0) = mrsExpenses!项目名称
            vsfMoney.TextMatrix(vsfMoney.Rows - 1, 1) = Format(mrsExpenses!应收, "0.00")
            vsfMoney.TextMatrix(vsfMoney.Rows - 1, 2) = Format(mrsExpenses!实收, "0.00")
            cur合计 = cur合计 + CCur(mrsExpenses!实收)
            mrsExpenses.MoveNext
        Loop
        vsfMoney.Redraw = True
    End If
...
Private Sub ShowAcceptFromInput(ByRef cur合计 As Currency)
    ...
    If mbytMode = 2 And Not mrsBill Is Nothing Then
        ...
        If Not mrsItems Is Nothing Or Not mrsExpenses Is Nothing Then
...
'功能：根据当前界面输入的号别,读取挂号费用集,显示在表格中
    ...
    Dim str医生姓名 As String, lng病人ID As Long, str年龄 As String
    
    If mblnReadBooking Then Exit Sub
    If mblnBuyHisBook = False Then
        If (mbytMode = 2 Or mbytMode = 0 And mbytInState = 0 And mstrNoIn <> "") And mTy_Para.bln预约接收确定挂号费 = False Then
            '56240 lgf 20120312
            Call ShowAcceptFromInput(cur合计)
            GoTo CalcOther:
        End If
    End If
    If chkCancel.Value = 1 Then Exit Sub
    ...
    Set mrsInComes = Nothing
    Set mrsExpenses = Nothing
    Call ReadRegistPrice(lng项目id, bln病历, mblnAddCardItem, str费别, mrsItems, mrsInComes, mlng挂号科室ID)
    ...
    '显示挂号费用
    If Not mrsItems Is Nothing Then
        '126802:李南春，2018/6/5，通过附加费实现固定折扣,仅预约时不处理
        If mbytMode <> 1 Then
            str年龄 = txt年龄.Text & IIf(cbo年龄单位.Visible, cbo年龄单位.Text, "")
            Call ReadExRegistPrice(mrsExpenses, mblnAppointPrice, lng病人ID, mintInsure, txt号别.Text, txtPatient.Text, NeedName(cbo性别.Text), _
                str年龄, txtIDCard.Text, str费别, NeedName(cbo付款方式.Text))
        
            If Not mblnAppointPrice And lng项目id = 0 And bln病历 Then
                Set mrsExpenses = Nothing '兼容以前单收病历费时不加收附加费，但不影响本次固定折扣
            End If
        End If
...
CalcOther:
    ...
    With vsfPay
        If (gblnPrePayPriority Or (mblnStation And Not mblnStationPrice And Not cbo结算方式.Visible)) And mdbl预交余额 >= cur合计 And mblnAddCardItem = False Then
            For i = 1 To .Rows - 1
                If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                    .TextMatrix(i, .ColIndex("金额")) = Format(cur合计, "0.00")
                    .TextMatrix(i, .ColIndex("余额")) = mdbl预交余额
                End If
            Next i
        Else
            For i = 1 To .Rows - 1
                If .RowData(i) = 0 And .TextMatrix(i, .ColIndex("支付方式")) <> "" Then
                    .TextMatrix(i, .ColIndex("金额")) = Format(0, "0.00")
                    .TextMatrix(i, .ColIndex("余额")) = mdbl预交余额
                End If
            Next i
        End If
...
Private Function GetPayedMoney() As Double
    ...
    With vsfPay
        For i = 1 To .Rows - 1
            dblReturn = dblReturn + (Val(.TextMatrix(i, .ColIndex("金额"))) - Val(.TextMatrix(i, .ColIndex("已支付"))))
        Next i
    End With
    ...
    With rsMoney
        ...
        If (mbytMode = 2 Or mbytMode = 0 And mbytInState = 0 And mstrNoIn <> "") And Not mTy_Para.bln预约接收确定挂号费 Then      '预约接收
            ...
            If Not mrsExpenses Is Nothing Then
                If mrsExpenses.RecordCount > 0 Then mrsExpenses.MoveFirst
                Do While Not mrsExpenses.EOF
                    rsMoney.Filter = "收费类别='" & Nvl(mrsExpenses!类别, "无") & "'"
                    If rsMoney.EOF Then
                        .AddNew
                    End If
                    !收费类别 = Nvl(mrsExpenses!类别, "无")
                    !金额 = Val(Nvl(!金额)) + Val(Nvl(mrsExpenses!实收))
                    .Update
                    mrsExpenses.MoveNext
                Loop
            End If
...
Private Sub AddCardDataSQL(ByVal lng病人ID As Long, ByVal dtCurdate As Date, _
    ...
    Else
        ...
        dtCurdate, mlng磁卡领用ID, mCurSendCard.rs卡费, strICCard, lngPay卡类别id, blnPay消费卡, strPayCardNo, lng结帐ID)
        zlAddArray cllPro, strSQL
    End If
    Exit Sub
errHandle:
