...
Begin VB.Form frmChangeNum 
   ...
   End
End
Attribute VB_Name = "frmChangeNum"
...
'显示本窗体并返回选择的是否正确
    ...
    strSQL = _
        ...
        " D.号类,C.名称 as 收费项目名称,B.名称 as 执行部门名称" & _
        " From 门诊费用记录 A,部门表 B,收费项目目录 C,挂号安排 D,病人挂号记录 X" & _
        " Where A.记录性质=4 And A.记录状态=1 And A.序号=1 And A.NO=X.NO" & _
        " AND X.记录状态=1 and x.记录性质=1 And A.收费细目ID=C.ID And X.号别=D.号码 And X.执行部门ID=B.ID" & _
        " And X.ID=[1]"
    ...
    If rsTmp.RecordCount > 0 Then
        ...
        strSQL = "SELECT b.* FROM 部门人员 a,人员表 b,人员性质说明 c  " & _
...
'功能：根据号别的分诊方式获取号别的诊室
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "病人换号", str号别)
    ...
    If rsTmp!分诊 = 1 Then
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "病人换号", CLng(rsTmp!ID))
...
'选择号别
    ...
    If frmNumSortSel.ShowMe(mlng挂号ID, strReturn, Me) Then
        '号类及号别
        ...
        '找到诊室
        Me.cmbDiagRoom2.Text = GetRoom(mstr号别)
...
On Error GoTo errHandle
    ...
    If ExcPlugInFun(1, mlng挂号ID, Trim(Split(cmbDocTor2.Text, STR_COMP)(0)), Me.cmbDiagRoom2.Text, lblItem2.Tag) = False Then Exit Sub
    
    strSQL = "ZL_病人挂号记录_换号('" & mstrNO & "','" & lblItem2.Tag & "','" & _
            Me.cmbDiagRoom2.Text & "'," & Me.cmbSect2.Tag & "," & strDoctor & "," & strSQL & ")"
