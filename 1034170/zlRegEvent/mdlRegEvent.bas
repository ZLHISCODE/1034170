...
Public Function ReadExRegistPrice(ByRef rsExpenses As ADODB.Recordset, ByRef blnAppointPrice As Boolean, _
    ...
    Set rsFee = zlDatabase.OpenSQLRecord(strFee, "zl_Fun_CustomRegExpenses", _
...
'            strWherePriceGrade
    ...
    Set rsFee = zlDatabase.OpenSQLRecord(strSql, "mdlRegEvent", str附加项)
    ...
    With rsExpenses
        ...
        Do While Not rsFee.EOF
            ...
            If blnAppointPrice Then
                For i = 0 To UBound(varFees)
                    If Val(varFees(i)(0)) = !项目ID Then
                        ...
                        !单价 = Format(varFees(i)(2), gstrFeePrecisionFmt)
            ...
            Else
                ...
                !单价 = Format(Nvl(rsFee!单价, 0), gstrFeePrecisionFmt)
...
'功能：初始化本机参数
    ...
    If lngModul = 1111 Then
        ...
        End If
        If glng挂号ID > 0 Then
            If Not ExistShareBill(glng挂号ID, IIf(gblnSharedInvoice, 1, 4)) Then
                If gblnSharedInvoice Then
        ...
        With ty_Card
            ...
            For i = 0 To UBound(varData)
                 ...
                 If Val(varTemp(0)) <> 0 Then
                    If ExistShareBill(Val(varTemp(1)), 5) Then
...
Public Function RePrintBill(ByVal frmParent As Object, ByVal bytFunc As Byte, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '执行数据处理
    '134155:李南春，2018/12/4，需要传入险类和结帐ID，医保接口打印时需要
    Call frmPrint.ReportPrint(bytFunc, strNO, strBackInvoice, lng领用ID, glng挂号ID, strInvoice, _
        zlDatabase.Currentdate, , , , blnVirtualPrint, bytFunc = 2, strUseType, , intInsure, lng结帐ID)
...
Public Function 是否已经签约(strCardNo As String, Optional ByRef lng身份证类别ID As Long) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查需要绑定的卡号是否已经签约
    '入参:绑定卡号
    '编制:王吉
...
Public Function GetPatiID(ByVal lngModel As Long, ByVal frmParent As Object, _
                                        ...
                                        Optional ByRef blnCancel As Boolean) As Long
