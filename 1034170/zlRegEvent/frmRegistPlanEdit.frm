...
Begin VB.Form frmRegistPlanEdit 
   ...
   Begin VB.PictureBox picTimeSet 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsTime 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmRegistPlanEdit.frx":00F2
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
...
Private Function LoadCard() As Boolean
    ...
    If mEditType = edt_新增 Then
        ...
        opt天.Value = True
        cbo天.Enabled = True
        cbo天.ListIndex = GetCboIndex(cbo天, "全日")
    ...
    cbo号类.ListIndex = GetCboIndex(cbo号类, Nvl(rsTemp!号类))
    txt号别.Text = Nvl(rsTemp!号码)

    cbo科室.ListIndex = GetCboIndex(cbo科室, Nvl(rsTemp!科室))
    cboItem.ListIndex = GetCboIndex(cboItem, Nvl(rsTemp!项目))

    cboDoctor.ListIndex = GetCboIndex(cboDoctor, Nvl(rsTemp!医生姓名))
   ...
   If bln每周 Or mrsRegHistory.RecordCount > 0 Then
    ...
    Else
        '每天
        opt天.Value = True:  cbo天.ListIndex = GetCboIndex(cbo天, Nvl(rsTemp!周日))
        If cbo天.ListIndex = -1 Then cbo天.ListIndex = 0:
...
Private Function InitData() As Boolean
   ...
   '取出门诊临床科室
    Set mrs科室 = GetDepartments("'临床'", "1,3", Not IsHavePrivs(mstrPrivs, "所有科室"))
...
Private Sub Form_Load()
    ...
    Call LoadvsDept
    lbl医生.Tag = IIf(InStr(1, mstrPrivs, ";参数设置;") > 0 And InStr(1, ",1,3,15,", "," & intType & ",") > 0, "1", "0")
...
Private Sub Form_Unload(Cancel As Integer)
    Err = 0: On Error Resume Next
    mstr限制修改 = ""
    zlDatabase.SetPara "只允许选院内医生", IIf(mblnOnly院内医生, 1, 0), glngSys, mlngModule, InStr(1, mstrPrivs, ";参数设置;") > 0
