...
Private Sub mobjIDCard_ShowIDCardInfo(ByVal strID As String, ByVal strName As String, ByVal strSex As String, _
    ...
    If Not txtPatient.Locked And txtPatient.Text = "" And Me.ActiveControl Is txtPatient Then
        Call LoadPatinInforFromID(strID, strName, strSex, strNation, datBirthDay, strAddress): Exit Sub
    End If
    
    If Not ActiveControl Is txt身份证号 Then Exit Sub
    
    If txtPatient.Text = "" Or mrsPatiInfo Is Nothing Then   '如果没有病人信息
        Call LoadPatinInforFromID(strID, strName, strSex, strNation, datBirthDay, strAddress): Exit Sub
    End If
...
Private Sub cmdOK_Click()
    ...
    Dim lng身份证类别ID As Long, str身份证 As String
    ...
    If mbytFun = 1 And (mstrCard <> "" Or txtPatient.Text <> "") And gbln卡费仅划价 Or (mbytFun = 0 And mlng病人ID <> 0) Then
        ...
        If byt类型 <> 3 Or blnCard Then
            ...
            '98364:2018-06-08,不管卡费是否为0，发卡都应该正常产生费用记录。
            ...
            If Me.txt卡号.Text <> "" And str就诊卡 = "" Then
                str就诊卡 = GetCardDataSql(11, lng病人ID, gCurSendCard.lng卡类别ID, mstrCard, Me.txt卡号.Text, Me.txt密码.Text, _
                                    Datsys, "", "挂号绑定卡")
            End If
            If mbln允许签约 And Trim(txt身份证号.Text) <> "" Then
                lng身份证类别ID = Get医疗卡类别ID("二代身份证")
                str身份证 = GetCardDataSql(11, lng病人ID, lng身份证类别ID, "", Trim(txt身份证号.Text), "", Datsys, "", "扫描身份证号签约")
            End If
        End If
        ...
        If str身份证 <> "" Then
            mlngListSql = mlngListSql + 1
            ReDim Preserve mstrArrListSql(mlngListSql - 1)
            mstrArrListSql(mlngListSql - 1) = str身份证
        End If
    End If
...
Private Sub Form_Load()
    ...
    HookDefend txt密码.Hwnd: HookDefend txt验证.Hwnd
    HookDefend txt支付密码.Hwnd: HookDefend txt验证密码.Hwnd
End Sub
...
Private Sub txt年龄_Validate(Cancel As Boolean)
    ...
    If txt年龄.Text <> txt年龄.Tag Then
        ...
        If mblnGetBirth Then
            strBirth = ReCalcBirth(Trim(txt年龄.Text), IIf(cbo年龄单位.Visible, cbo年龄单位.Text, ""))
            txt出生日期.Text = Format(strBirth, "yyyy-mm-dd")
            txt出生时间.Text = Format(strBirth, "hh:mm")
        End If
        mblnChange = True
...
Private Sub txt身份证号_Change()
    ...
    If Not mobjIDCard Is Nothing Then mobjIDCard.SetEnabled txt身份证号.Text = ""
    mbln允许签约 = False
End Sub
...
Private Sub txt身份证号_GotFocus()
    ...
    If txt身份证号.Text = "" Then OpenIDCard
End Sub
...
Private Sub txt身份证号_Validate(Cancel As Boolean)
    If isCheckInputIDCard(Trim(txt身份证号.Text)) = False Then Exit Sub
    Call LoadPatinInforFromID(Trim(txt身份证号.Text))
End Sub
...
Private Function LoadPatinInforFromID(ByVal strID As String, Optional ByVal strName As String, Optional ByVal strSex As String, _
                          Optional ByVal strNation As String, Optional ByVal datBirthDay As Date, Optional ByVal strAddress As String) As Boolean
    ...
    '     strName-姓名(刷身份证时传入)
    '     strSex-性别(刷身份证时传入)
    '     strNation-民族(刷身份证时传入)
    '     datBirthDay-出生日期(刷身份证时传入)
    '     strAddress-住址(刷身份证时传入)
    '返回:成功返回true,否则返回Fale
    '编制:刘兴洪
    '日期:2018-03-02 10:45:07
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim bln是否签约 As Boolean
    Dim lng病人ID As Long, str出生时间 As String, str性别 As String
    Dim strPassWord As String, strErrMsg As String, lng病人IDCus As Long
    Dim objControl As Object, blnCancel As Boolean
    On Error GoTo errHandle
    Set objControl = IIf(Me.ActiveControl Is txtPatient, txtPatient, txt身份证号)
    lng病人ID = GetPatiID(mlngModul, Me, strID, objControl, , , blnCancel)
    If lng病人ID <= 0 And Not blnCancel Then
         If gobjSquare.objSquareCard.zlGetPatiID("身份证", strID, False, lng病人ID, strPassWord, strErrMsg, , , , InStr(mstrPrivs, ";合并病人信息;") > 0) = False Then
             If IsDate(zlCommFun.GetIDCardDate(strID)) = False Then Exit Function
             If Format(zlCommFun.GetIDCardDate(strID), "yyyy-mm-dd") <> Format(txt出生日期.Text, "yyyy-mm-dd") Then
                 If IsDate(txt出生日期.Text) Then MsgBox "输入的身份证号与输入的出生日期不一致，将使用身份证号获取的日期替换！", vbInformation, gstrSysName
                 txt出生日期.Text = zlCommFun.GetIDCardDate(strID)
                 txt出生时间.Text = "00:00"
             End If
             ...
             txt年龄.Tag = txt年龄.Text
             cbo年龄单位.Tag = cbo年龄单位.Text
             If Trim(strName) = "" Then  '不是自动读取的，则根据身份证号来获取性别
                 If Len(txt身份证号.Text) = 15 Then
                     str性别 = IIf(Mid(strID, 15, 1) Mod 2 = 1, "男", "女")
                 Else
                     str性别 = IIf(Mid(strID, 17, 1) Mod 2 = 1, "男", "女")
                 End If
                 Call zlControl.CboLocate(cbo性别, str性别)
                 LoadPatinInforFromID = True
                 Exit Function
             End If
             mblnNotChange = False
    ...
    If mrsPatiInfo.EOF Then Exit Function
    If strName = "" Then    '手功输入的身份证号
        Call zl加载病人信息(mrsPatiInfo)
        LoadPatinInforFromID = True
        Exit Function
    End If
    
    If (Nvl(mrsPatiInfo!姓名) <> Trim(strName) _
        Or Nvl(mrsPatiInfo!性别) <> strSex _
        Or Format(Nvl(mrsPatiInfo!出生日期, "00:00:00"), "yyyy-MM-dd") <> Format(datBirthDay, "yyyy-MM-dd")) Then
        
        mbln允许签约 = False: mbln扫描身份证 = False
        txt支付密码.Text = "": txt支付密码.Tag = ""
        txt验证密码.Text = "": txt验证密码.Tag = ""
        ...
        Else
            '非二代身份证绑定
            Call zl加载病人信息(mrsPatiInfo)
            If txtRegLocation.Text = "" Then txtRegLocation.Text = strAddress
            If padd户口地址.Value = "" Then padd户口地址.Value = strAddress
        End If
        Set mrsPatiInfo = Nothing
        If Trim(txtPatient.Text) <> strName Then
            If MsgBox("身份证上的姓名" & strName & "与原有病人的姓名" & Trim(txtPatient.Text) & "不一致,是否将病人的姓名更新为身份证上的姓名?", vbQuestion + vbYesNo, gstrSysName) = vbYes Then
                txtPatient.Text = strName
            End If
        End If
        If NeedName(cbo性别.Text) <> strSex Then
            If MsgBox("身份证上的性别" & strSex & "与原有病人的姓名" & NeedName(cbo性别.Text) & "不一致,是否将病人的性别更新为身份证上的性别?", vbQuestion + vbYesNo, gstrSysName) = vbYes Then
                Call zlControl.CboLocate(cbo性别, strSex)
            End If
        End If
    Else
        Call zl加载病人信息(mrsPatiInfo)
        mbln允许签约 = True: mbln扫描身份证 = True
        If imgPatient.Picture = 0 Then Call LoadIDImage   '74430,冉俊明,2014-7-7,挂号中的病人信息编辑功能中提供采集照片功能
        If txtRegLocation.Text = "" Then txtRegLocation.Text = strAddress
        If padd户口地址.Value = "" Then padd户口地址.Value = strAddress
    End If
...
Private Sub zl是否允许扫描身份证签约(ByVal strID As String)
    Dim bln是否签约 As Boolean
     If mbln扫描身份证签约 And mbln允许签约 Then
