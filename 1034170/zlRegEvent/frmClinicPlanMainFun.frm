...
'    End If
    strSQL = "Select a.ID, a.关联ID, a.出诊表名, a.排班方式, a.年份, a.月份, a.周数," & vbNewLine & _
            "       Decode(a.发布时间, Null, 0, 1) As 是否发布,Nvl(模板类型,0) As 模板类型," & vbNewLine & _
            "       Nvl((Select 1 From 临床出诊安排 Where 出诊id = Nvl(a.关联ID,a.Id) And 终止时间 >= Trunc(Sysdate) And Rownum < 2), 0) As 是否有效" & vbNewLine & _
        ...
        Do While Not .EOF
            ...
            Else '月排班和周排班
                ...
                If Nvl(!排班方式) = 2 Then  '周排班
                    ...
                    objNode.Tag = Val(Nvl(!关联ID, Nvl(!ID)))
...
Private Sub tvwPlan_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    '       XX月出诊表节点：K2_年份_月份
    '       XX周出诊表节点：K3_年份_月份_周数
    Select Case varKey(0)
    Case "K1" '固定安排
        Call mfrmMain.SelectedChange(Pane_FixedPlan, Val(Node.Tag))
    Case "K2" '月排班
        Call mfrmMain.SelectedChange(Pane_MonthPlan, Val(Node.Tag), Val(varKey(1)), Val(varKey(2)))
    Case "K3" '周排班
        Call mfrmMain.SelectedChange(Pane_WeekPlan, Val(Node.Tag), Val(varKey(1)), Val(varKey(2)), Val(varKey(3)))
    Case Else
        If Node.Key = "K_StopPlan" Then '停诊管理
            Call mfrmMain.SelectedChange(Pane_StopPlan)
        ElseIf Node.Key = "K_FixedRoot" Then
            Call mfrmMain.SelectedChange(Pane_FixedPlan, Val(Node.Tag))
        Else
            Call mfrmMain.SelectedChange(Pane_WeekPlan, Val(Node.Tag), Val(varKey(1)))
        End If
    End Select
    Exit Sub
errHandler:
...
Private Sub tvwPlanTemplet_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    Else
        Call mfrmMain.SelectedChange(Pane_PlanTemplet, Val(Node.Tag), , , , bytTempletType)
    End If
    Exit Sub
errHandler:
    If ErrCenter() = 1 Then
