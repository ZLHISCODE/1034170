...
Begin VB.Form frmUnitReg 
   ...
   Begin VB.PictureBox picTime 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsThis 
         ...
         FormatString    =   $"frmUnitReg.frx":0000
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
...
Public Sub ShowUnitReg(ByVal lng安排ID As Long)
   mlng安排ID = lng安排ID
   If Not mrsUnitRegPlan Is Nothing Then Set mrsUnitRegPlan = Nothing
   If InitData() = False Then Exit Sub
   InitPage
   If mrsUnits Is Nothing Then LoadUnitRegPlan mlng安排ID, True, "": Exit Sub '问题号:51156
   If mrsUnits.RecordCount = 0 Then LoadUnitRegPlan mlng安排ID, True, "": Exit Sub  '问题号:51156
End Sub

'Public Sub LoadUnitReg(ByVal lng安排Id As Long, ByVal str合作单位 As String)
'    mlng安排ID = lng安排Id: mstr合作单位 = str合作单位
'    If InitData() = False Then Exit Sub
'
'   ' Call LoadUnitRegPlan(lng安排Id, IIf(mbytMode = 1, True, False), str合作单位)
'
'End Sub
...
Private Function InitPage() As Boolean
    ...
    If blnExit Then
        ...
        '67824:刘尔旋,2013-11-21,挂号安排表丢失焦点的问题
        'tbUunits.Item(0).Selected = True
        tbUunits.PaintManager.Position = xtpTabPositionBottom '问题号:51156
        vsThis.Clear
       ' vsThis.Rows = 1
        Exit Function
    End If
...
Private Function InitData() As Boolean
    ...
    Dim BytMode As Byte
    strSQL = "Select 1 As 属性, Nvl(序号控制, 0) As 数据 ,'' as 单位 From 挂号安排 Where ID = [1] "
    ...
    strSQL = "    Select   合作单位  From 合作单位安排控制 Where 安排id = [1]  Group By 合作单位  "
    Set mrsUnits = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng安排ID)
    Set rsTmp = Nothing
    InitData = True
    Exit Function
Hd:
...
Private Sub tbUunits_SelectedChanged(ByVal Item As XtremeSuiteControls.ITabControlItem)
    ...
    Call LoadUnitRegPlan(mlng安排ID, IIf(mbytMode = 1, True, False), mstr合作单位)
End Sub
...
Private Sub LoadUnitRegPlan(ByVal lng安排ID As Long, ByVal bln序号控制 As Boolean, ByVal str合作单位 As String, _
   Optional blnReload As Boolean = False)
    ...
    ElseIf mrsUnitRegPlan.State <> 1 Then
         lngPre安排ID = -1
    End If
    If lngPre安排ID <> lng安排ID Or strPre合作单位 <> str合作单位 Or blnReload Then
        lngPre安排ID = lng安排ID
        strPre合作单位 = str合作单位
        strSQL = "" & _
        "   Select decode(限制项目,'周日',1,'周一',2,'周二',3,'周三',4,'周四',5,'周五',6,7) as 排序,限制项目 As 星期,序号, " & _
        "       数量 as 限制数量 , 安排ID, 合作单位 " & _
        "   From 合作单位安排控制" & _
        "   Where  安排ID=[1] And 合作单位=[2] And   数量>0 " & _
        "   Order by 排序,序号"
        Set mrsUnitRegPlan = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng安排ID, str合作单位)
        tbWeekTime.Tabs.Clear
    End If
