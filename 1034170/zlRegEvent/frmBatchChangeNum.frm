...
Begin VB.Form frmBatchChangeNum 
   ...
   ClientTop       =   375
   ClientWidth     =   10740
   Icon            =   "frmBatchChangeNum.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5220
   ...
   End
End
Attribute VB_Name = "frmBatchChangeNum"
...
Private Sub Show挂号安排信息(enum查询类型 As mEnum_查询类型)
    ...
    strSQLBody = "" & _
    " Select A.ID As 安排ID,A.号类,A.号码,B.名称 As 科室,A.科室ID,C.名称 As 项目,A.医生姓名,A.医生ID,Nvl(E.限号数,0) As 限号数,Nvl(E.限约数,0) As 限约数,Nvl(D.已挂数,0) As 已挂数,Nvl(D.已约数,0) As 已约数,Nvl(D.其中已接收,0) As 已接收数,Decode(to_char([1],'D'),'1', 周日, '2', 周一, '3', 周二, '4', 周三, '5', 周四, '6', 周五, " & _
    "            '7', 周六, Null) As 时间段,Decode(A.病案必须,0,'','√') As 病案,Decode(A.分诊方式,1,'指定',2,'动态',3,'平均','') As 分诊,Decode(A.序号控制,0,'','√') As 序号控制 " & _
    " From 挂号安排 A,部门表 B,收费项目目录 C,(Select 科室ID,项目ID,医生姓名,医生ID,日期,号码,已挂数,已约数,其中已接收 From 病人挂号汇总 A Where A.日期=Trunc(" & str时间 & ")) D,挂号安排限制 E " & _
    " Where A.科室ID=B.ID(+)  " & IIf(enum查询类型 = 可换号排班, " And A.停用日期 Is Null ", "") & _
    " And A.ID=E.安排ID(+) And E.限制项目=Decode(to_char([1],'D'),'1', '周日', '2', '周一', '3', '周二', '4', '周三', '5', '周四', '6', '周五'," & _
    "            '7', '周六', Null)  " & _
    " And A.项目ID=C.ID(+) " & _
    " And A.科室ID=D.科室ID(+) And A.项目ID=D.项目ID(+) And Nvl(A.医生姓名,'医生')=Nvl(D.医生姓名(+),'医生') And Nvl(A.医生ID,'0')=Nvl(D.医生ID(+),'0') And A.号码=D.号码(+)" & _
    " And Not Exists" & _
    "            (Select 1 From 挂号安排计划 E" & _
    "                      Where E.安排id=A.Id And E.审核时间 Is Not Null And" & _
    "                      [1] Between Nvl(E.生效时间, To_Date('1900-01-01', 'yyyy-mm-dd')) And" & _
    "                      Nvl(E.失效时间, To_Date('3000-01-01', 'yyyy-mm-dd')) Group By E.安排ID Having Count(1) > 1 " & _
    "              ) "
    strSQLBody = strSQLBody & " Union All " & _
    " Select A.安排ID As 安排ID,A.号类,A.号码,A.科室,A.科室ID,A.项目,A.医生姓名,A.医生ID,A.限号数,A.限约数,Nvl(E.已挂数,0) As 已挂数,Nvl(E.已约数,0) As 已约数,Nvl(E.其中已接收,0) As 已接收数,Decode(to_char([1],'D'),'1', A.周日, '2', A.周一, '3', A.周二, '4', A.周三, '5', A.周四, '6', A.周五," & _
    "            '7',A.周六, Null) As 时间段,Decode(A.病案必须,0,'','√') As 病案,Decode(A.分诊方式,1,'指定',2,'动态',3,'平均','') As 分诊,Decode(A.序号控制,0,'','√') As 序号控制" & _
    "    From" & _
    "       (Select A.安排ID,B.号类,A.号码,C.名称 As 科室,D.名称 As 项目,A.医生姓名,B.科室ID,B.项目ID,A.医生ID,A.周日,A.周一,A.周二,A.周三,A.周四,A.周五,A.周六,B.病案必须,A.分诊方式,A.序号控制,Nvl(E.限号数,0) As 限号数,Nvl(E.限约数,0) As 限约数" & _
    "               From 挂号安排计划 A,挂号安排 B,部门表 C,收费项目目录 D,挂号计划限制 E" & _
    "               Where A.号码=B.号码(+) And A.安排ID=B.Id(+) " & _
    "               And B.科室ID =C.Id(+) " & IIf(enum查询类型 = 可换号排班, " And B.停用日期 Is Null ", "") & _
    "               And A.项目ID =D.Id(+) " & _
    "               And A.Id =E.计划ID(+) And E.限制项目=Decode(to_char([1],'D'),'1', '周日', '2', '周一', '3', '周二', '4', '周三', '5', '周四', '6', '周五'," & _
    "               '7', '周六', Null) " & _
    "               And " & str时间 & " Between A.生效时间 And A.失效时间" & _
    "               And  Nvl(A.生效时间, To_Date('1900-01-01', 'yyyy-mm-dd'))=" & _
    "                          (Select Max(E.生效时间) From 挂号安排计划 E" & _
    "                                Where E.安排id=A.安排Id And E.审核时间 Is Not Null And" & _
    "                                [1] Between Nvl(E.生效时间, To_Date('1900-01-01', 'yyyy-mm-dd')) And" & _
    "                                Nvl(E.失效时间, To_Date('3000-01-01', 'yyyy-mm-dd')) Group By E.安排ID Having Count(1) > 1" & _
    "                           )" & _
    "         ) A,(Select 科室ID,项目ID,医生姓名,医生ID,日期,号码,已挂数,已约数,其中已接收 From 病人挂号汇总 A Where A.日期=Trunc(" & str时间 & ")) E" & _
    "    Where A.科室ID=E.科室ID(+) And A.项目ID=E.项目ID(+) And Nvl(A.医生姓名,'医生')=Nvl(E.医生姓名(+),'医生') And Nvl(A.医生ID,'0')=Nvl(E.医生ID(+),'0') And A.号码=E.号码(+)  "
    ...
    strSQL = "" & _
    "   Select 安排ID As ID,号类,号码,科室,科室ID,项目,医生姓名,医生ID,限号数,限约数,已挂数,已约数,已接收数,时间段,病案,分诊,序号控制 From(" & strSQLBody & ") " & strSQLWhere & " Order By 号码"
    
    If enum查询类型 = 需换号排班 Then
        Set mrs挂号安排 = zlDatabase.ShowSQLSelect(frmBatchChangeNum, strSQL, 0, "原挂号安排信息", False, "", "", False, False, False, txtFilter.Left, txtFilter.Top + txtFilter.Height, 1000, True, False, False, dat时间)
    ...
    If enum查询类型 = 可换号排班 Then
        ...
        '获取相同时段的排班信息SQL
        strSQL当前时段 = "" & _
        "   Select A.安排ID,A.序号,A.星期,A.开始时间,A.结束时间,A.限制数量,A.是否预约,B.序号控制" & _
        "   From 挂号安排时段 A,挂号安排 B" & _
        "   Where A.安排ID = [2] And A.星期=Decode(to_char([1],'D'),'1', '周日', '2', '周一', '3', '周二', '4', '周三', '5', '周四', '6', '周五','7', '周六', Null)" & _
        "   And A.安排ID=B.ID And B.停用日期 is Null" & _
        "   And [1] Between Nvl(B.开始时间,to_date('1900-01-01','yyyy-MM-dd')) And Nvl(B.终止时间,to_date('3000-01-01','yyyy-MM-dd'))" & _
        "   And A.安排ID not in" & _
        "           (Select E.安排ID From 挂号安排计划 E" & _
        "                     Where E.安排id = A.安排ID And E.审核时间 Is Not Null And" & _
        "                     [1] Between Nvl(E.生效时间, To_Date('1900-01-01', 'yyyy-mm-dd')) And" & _
        "                     Nvl(E.失效时间, To_Date('3000-01-01', 'yyyy-mm-dd')) Group By E.安排ID Having Count(1) > 1)" & _
        "   Union All" & _
        "   Select B.安排ID,A.序号,A.星期,A.开始时间,A.结束时间,A.限制数量,A.是否预约,C.序号控制" & _
        "   From 挂号计划时段 A,挂号安排计划 B,挂号安排 C" & _
        "   Where B.安排id = [2]" & _
        "   And C.ID=B.安排ID And C.停用日期 is Null" & _
        "   And A.计划ID=B.ID And A.星期=Decode(to_char([1],'D'),'1', '周日', '2', '周一', '3', '周二', '4', '周三', '5', '周四', '6', '周五','7', '周六', Null)" & _
        "   And [1] Between Nvl(B.生效时间,to_date('1900-01-01','yyyy-MM-dd')) And Nvl(B.失效时间,to_date('3000-01-01','yyyy-MM-dd')) And B.审核时间 is Not Null" & _
        "   And  Nvl(B.生效时间, To_Date('1900-01-01', 'yyyy-mm-dd'))=" & _
        "                                         (Select Max(E.生效时间) From 挂号安排计划 E" & _
        "                                               Where E.安排id=C.Id And E.审核时间 Is Not Null And" & _
        "                                               [1] Between Nvl(E.生效时间, To_Date('1900-01-01', 'yyyy-mm-dd')) And" & _
        "                                               Nvl(E.失效时间, To_Date('3000-01-01', 'yyyy-mm-dd')) Group By E.安排ID Having Count(1) > 1" & _
        "                                          )"
        ...
        strSQL可换号时段 = "" & _
        "   Select A.安排ID As ID From (" & _
        "   Select Avg(A.安排ID) As 安排ID,Count(1) As 时段设置数量" & _
        "   From (" & strSQL其他可用时段 & ") A,(" & strSQL当前时段 & ") B" & _
        "   Where A.序号 =  B.序号 And to_char(A.开始时间,'HH24:mi:ss') = to_char(B.开始时间,'HH24:mi:ss') And to_char(A.结束时间,'HH24:mi:ss') = to_char(B.结束时间,'HH24:mi:ss') And Nvl(A.限制数量,0) = Nvl(B.限制数量,0) And Nvl(A.序号控制,0)=Nvl(B.序号控制,0)" & _
        "   Group By A.安排ID " & _
        "   ) A," & " (Select Count(1) As 时段设置数量 From (" & strSQL当前时段 & ") A Group By A.安排ID) B" & _
        "   Where A.时段设置数量=B.时段设置数量 "
        ...
        Set mrs可换号排班 = zlDatabase.ShowSQLSelect(frmBatchChangeNum, strSQL可换号排班, 0, "新挂号安排信息", False, "", "", False, False, False, txtFilter.Left, txtFilter.Top + txtFilter.Height, 1000, True, False, False, dat时间, Val(txt安排ID.Text))
...
Private Sub Show挂号信息()
    ...
    "      And A.记录状态=1 " & _
    "      And A.病人ID=B.病人ID(+) " & _
    "      And Nvl(号别,'号别')= '" & IsNothing(Trim(txt号别.Text), "号别") & "'" & _
    ""
'    "      Union ALL" & _
'    "      Select A.No,A.号序 As 序号,A.发生时间 As 预约时间,A.姓名,A.性别,A.年龄,B.身份证号,B.联系人电话,Decode(A.记录性质,1,'已接收',2,'已预约','已预留') As 状态" & _
'    "      From 病人挂号记录 A,病人信息 B " & _
