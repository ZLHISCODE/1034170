...
Public Sub RefreshData(ByVal bytFun As Byte, ByVal lng出诊ID As Long, Optional ByVal blnClear As Boolean)
    '功能：刷新安排详情数据
    '入参：
    '   bytFun - 1-月安排，2-周安排
    '   lng出诊ID - 出诊ID
...
Private Function GetPlanRecords(ByVal blnMonth As Boolean, Optional ByVal lng出诊ID As Long) As ADODB.Recordset
    ...
    strSQL = "Select 1 As 是否有效," & vbNewLine & _
            ...
            "        Decode(b.ID,Null,i.标识符,j.标识符)as 标识符,c.Id As 记录id, To_Date(Decode(c.限制项目, Null, '', '1900-01-' || Replace(c.限制项目, '日', '')), 'yyyy-mm-dd') As 出诊日期, " & vbNewLine & _
