...
Public Sub zlUpdateCommandBars(ByVal Control As CommandBarControl)
    ...
    blnVisible = IsHavePrivs(mstrPrivs, "门诊诊室设置")
    ...
    If Not mrsDoctorOffice Is Nothing Then
        With mrsDoctorOffice
            ...
            Else
                ...
                Case 1   '诊室(简码)
                    .Filter = "诊室 Like '" & strKey & "' Or 诊室简码 Like '" & strKey & "'"
                Case 2   '站点
                    If Trim(txtFind(1).Text) = "全院" Then
                        .Filter = "站点=null"
                    Else
                        .Filter = "站点 Like '" & strKey & "'"
                    End If
                Case Else
                    .Filter = ""
                End Select
...
Private Sub GetRecords()
    ...
    Err = 0: On Error GoTo ErrHandler
    strSql = "Select a.ID, d.名称 As 站点, c.名称 As 科室, c.简码 As 科室简码," & vbNewLine & _
            "       a.编码, a.名称 As 诊室, a.简码 As 诊室简码, a.位置, b.缺省标志" & vbNewLine & _
...
Public Function LoadData(Optional ByVal blnReRead As Boolean = True, _
    ...
    Do While Not mrsDoctorOffice.EOF
        Call InsertRowData(Val(Nvl(mrsDoctorOffice!ID)), Nvl(mrsDoctorOffice!站点, "全院"), _
            Nvl(mrsDoctorOffice!科室, "所有"), Nvl(mrsDoctorOffice!编码), Nvl(mrsDoctorOffice!诊室), _
            Nvl(mrsDoctorOffice!诊室简码), Nvl(mrsDoctorOffice!位置), Val(Nvl(mrsDoctorOffice!缺省标志)))
...
Private Sub InsertRowData(ByVal lngID As Long, ByVal str站点 As String, ByVal str科室 As String, ByVal str编码 As String, _
    ByVal str门诊诊室 As String, ByVal str简码 As String, ByVal str位置 As String, ByVal byt闲忙标志 As Byte)
    Dim objRecord As ReportRecord, ObjItem As ReportRecordItem
    
    Err = 0: On Error GoTo ErrHandler
    With rptData
        Set objRecord = .Records.Add()
        Set ObjItem = objRecord.AddItem(lngID)
        Set ObjItem = objRecord.AddItem(str站点)
        Set ObjItem = objRecord.AddItem(str科室)
        Set ObjItem = objRecord.AddItem(str编码)
        Set ObjItem = objRecord.AddItem(str门诊诊室)
        Set ObjItem = objRecord.AddItem(str简码)
        Set ObjItem = objRecord.AddItem(str位置)
        Set ObjItem = objRecord.AddItem(IIf(byt闲忙标志 = 0, "闲", "忙"))
    End With
    Exit Sub
ErrHandler:
    If ErrCenter() = 1 Then
...
Private Sub rptData_RowDblClick(ByVal Row As XtremeReportControl.IReportRow, ByVal Item As XtremeReportControl.IReportRecordItem)
    ...
    If rptData.SelectedRows.Count > 0 Then
        If Not rptData.SelectedRows(0).GroupRow Then
            ...
            If IsHavePrivs(mstrPrivs, "门诊诊室设置") Then
