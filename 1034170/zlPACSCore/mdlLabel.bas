...
Private Sub subPeriodMovee5X(la, lb, ll, xx, Yy, E5, basex, baseY, im As DicomImage)
'------------------------------------------------
'功能：矢冠状中心点的移动
...
Private Sub subPeriodMove(ByVal la As DicomLabel, ByVal x As Long, ByVal y As Long, ByVal lb As DicomLabel, _
...
Private Sub subMoveMPRLabel(f As frmViewer, la As DicomLabel, xxx As Long, yyy As Long, basex As Long, baseY As Long)
'------------------------------------------------
...
'------------------------------------------------
    ...
    If intIndex = G_INT_SYS_LABEL_MPR_POINT_V1 Or intIndex = G_INT_SYS_LABEL_MPR_POINT_V2 _
        Or intIndex = G_INT_SYS_LABEL_MPR_POINT_O Then
        Call funGetMPRImageAndShow(f.SelectedImage.Labels(G_INT_SYS_LABEL_MPRV), f, _
                                    f.Viewer(ZLMPRCube(1).intViewerIndex), f.SelectedImageIndex, _
                                    ZLMPRCube(2).intViewerIndex, ToltalHeight, 1, False)
    End If
    ...
    If intIndex = G_INT_SYS_LABEL_MPR_POINT_H1 Or intIndex = G_INT_SYS_LABEL_MPR_POINT_H2 _
        Or intIndex = G_INT_SYS_LABEL_MPR_POINT_O Then
        Call funGetMPRImageAndShow(f.SelectedImage.Labels(G_INT_SYS_LABEL_MPRH), f, _
                                    f.Viewer(ZLMPRCube(1).intViewerIndex), f.SelectedImageIndex, _
                                    ZLMPRCube(3).intViewerIndex, ToltalHeight, 2, False)
    End If
    '''''''''矢冠状点线同步'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
'参数： thisForm --- MPR所在的窗体
    ...
    '重新显示MPR控制线-竖线对应的结果图
    Call funGetMPRImageAndShow(thisForm.SelectedImage.Labels(G_INT_SYS_LABEL_MPRV), thisForm, _
                                thisForm.Viewer(ZLMPRCube(1).intViewerIndex), thisForm.SelectedImageIndex, _
                                ZLMPRCube(2).intViewerIndex, ToltalHeight, 1, False)
    
    '重新显示MPR控制线-横线对应的结果图
    Call funGetMPRImageAndShow(thisForm.SelectedImage.Labels(G_INT_SYS_LABEL_MPRH), thisForm, _
                                thisForm.Viewer(ZLMPRCube(1).intViewerIndex), thisForm.SelectedImageIndex, _
                                ZLMPRCube(3).intViewerIndex, ToltalHeight, 2, False)
...
'------------------------------------------------
    ...
    If f.SelectedImage.Labels.IndexOf(la) >= G_INT_SYS_LABEL_MPRV And f.SelectedImage.Labels.IndexOf(la) <= G_INT_SYS_LABEL_MPR_POINT_O Then ''[矢冠状线的移动]
        ...
        Call subMoveMPRLabel(f, la, xxx, yyy, basex, baseY)
    Else                                '用户标注的移动
        la.left = la.left + x
        la.top = la.top + y
