...
'------------------------------------------------
    ...
    If blLocalRun = True Then
        strSQL = "SELECT 影像类型,自动序列布局,自动图像布局,序列行数,序列列数,图像行数,图像列数" & _
                ",自动反白,显示病人信息,选择定位线,选择序列同步,插值模式 FROM 影像屏幕布局 ORDER BY 影像类型"
        Set rsTmp = cnAccess.Execute(strSQL, , adCmdText)
    Else
        strSQL = "SELECT 影像类型,自动序列布局,自动图像布局,序列行数,序列列数,图像行数,图像列数" & _
                ",自动反白,显示病人信息,选择定位线,选择序列同步,插值模式 FROM 影像屏幕布局 Where 人员ID = [1] ORDER BY 影像类型"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, App.ProductName, lngUserID)
    End If
    i = 1
    With rsTmp
...
'        If aModifiedPresetLayout(i).bModified Then
            ...
            If blLocalRun = True Then
                ...
                strSQL = "UPDATE 影像屏幕布局 SET 自动序列布局=" & _
                         ...
                         aModifiedPresetLayout(i).lngInterpolationMode & " WHERE 影像类型='" & _
            ...
            Else
                ...
                "," & aModifiedPresetLayout(i).lngInterpolationMode & ")"
                zlDatabase.ExecuteProcedure strSQL, App.ProductName
...
'------------------------------------------------
    ...
    blfrmRefresh = True
End Sub
Public Sub WriteLogFile(StrOutPut As String)
    '写入Log文件已方便错误的判断
    Dim strTmpFile As String
    strTmpFile = IIf(Len(App.Path) > 3, App.Path & "\PacsCoreLog.txt", App.Path & "PacsCoreLog.txt")
    Open strTmpFile For Append As #1
        Print #1, StrOutPut & vbCrLf
    Close #1
End Sub
