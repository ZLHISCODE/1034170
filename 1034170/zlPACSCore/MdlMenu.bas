...
'------------------------------------------------
    ...
    '清除所有的排序方式
    If lngToolID = ID_View_PhotoSerial_PhotoNumber Or lngToolID = ID_View_PhotoSerial_BedASC Or lngToolID = ID_View_PhotoSerial_BedDESC _
         Or lngToolID = ID_View_PhotoSerial_CollectionTime Or lngToolID = ID_View_PhotoSerial_PhotoTime Then
        '处理菜单和工具条的check状态
        ...
        '处理系统临时变量的值
        Button_miImageNumber = False                 ''图像号
        Button_miSliceLocation = False                   ''床位正序
        Button_miReverseSliceLocation = False             ''床位逆序
        Button_miAcquisitionTime = False                 ''采集时间
        Select Case lngToolID
            Case ID_View_PhotoSerial_PhotoNumber
                Button_miImageNumber = True                 ''图像号
            Case ID_View_PhotoSerial_BedASC
                Button_miSliceLocation = True                   ''床位正序
            Case ID_View_PhotoSerial_BedDESC
                Button_miReverseSliceLocation = True             ''床位逆序
            Case ID_View_PhotoSerial_CollectionTime
                Button_miAcquisitionTime = True                 ''采集时间
        End Select
        ...
        If f.intSelectedSerial > 0 And f.intSelectedSerial < f.MSFViewer.Rows And f.MSFViewer.TextMatrix(f.intSelectedSerial, 1) = "True" Then
            '设置图像的排序方式
            If Button_miImageNumber = True Then         '按照图像号排序
                iSortType = 0
            ElseIf Button_miSliceLocation = True Then   '按照切片位置正序排序
                iSortType = 1
            ElseIf Button_miReverseSliceLocation = True Then    '按照切片位置逆序排序
                iSortType = 2
            ElseIf Button_miAcquisitionTime = True Then '按照采集时间排序
                iSortType = 3
            Else                                        '按照图像时间排序,不是上述情况，那就是按照图像时间排序
                iSortType = 4
            End If
...
'------------------------------------------------
    ...
    If Not Button_miFullScreen Then
        ...
        '''''''''''''''''''''''''''''''''''''''[关闭窗体的标题栏]'''''''''''''''''''''''''''''''''''''''''''''
        ToggleTitleBar Frm, False ''''设置窗体是否显示标题栏
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        '隐藏工具栏和菜单
        For i = 1 To 8
            If i <> 7 Then
    ...
    Else
        ...
        '''''''''''''''''''''''''''''''''''''''[关闭窗体的标题栏]'''''''''''''''''''''''''''''''''''''''''''''
        ToggleTitleBar Frm, True ''''设置窗体是否显示标题栏
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'        ''''''''''''''''''''''''''''''''''''''[开启窗体的标题栏]''''''''''''''''''''''''''''''''''''''''''''''
        '显示工具栏和菜单
        For i = 1 To 8
