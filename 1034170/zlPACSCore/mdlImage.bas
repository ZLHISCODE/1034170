...
'2009用
'------------------------------------------------
     Dim i As Integer, EnterpointX As Integer, Enterpointy As Integer
...
'------------------------------------------------
    ...
    If Not LeagelToACRebuild(Viewer.Images) Then
    ...
    On Error GoTo err
    ''''获取图像的层厚'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ''在前面调用LeagelToACRebuild函数的时候，已经对以下的 层厚，位置，图像数量，像素距离作了检查，本函数中不用再检查了。
    ...
    '''''''''''''''''''重定义和填充像素值数组''''''''''''''''''''''''''''''''''''''
    zl9ComLib.zlCommFun.ShowFlash "正在初始化矢冠状重建，请等待！", thisForm
...
Public Function LeagelToACRebuild(imgs As DicomImages) As Boolean
'------------------------------------------------
'功能：判断图像是否满足矢冠状重建的条件
'参数：imgs进行矢冠状重建的图像集
'返回：True--可以进行重建；False--不能进行重建
'2009用
'------------------------------------------------
    Dim i As Long, j As Long
    LeagelToACRebuild = False
    ''''''图像数量是否够3幅'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    If (imgs.Count < 3) Then
       Exit Function
    End If
    
    ''''''图像数量大于500幅，限制最多可以做500个图的重建'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    If (imgs.Count > 500) Then
       Exit Function
    End If
    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
'------------------------------------------------
   ...
   For k = 1 To Int(IIf(dblThick / 2 > 5, 5, dblThick / 2))
        For i = 3 To TolHeight - 2
            For j = 2 To lngWidth - 1
                '这个模板很好，对细节处理很锐利
                lngTemp = CLng(b(j - 1, i - 1)) + CLng(b(j - 1, i + 1)) + _
                          CLng(2 * b(j, i - 2)) + CLng(2 * b(j, i - 1)) + CLng(b(j, i))
                lngTemp = (CLng(lngTemp) + CLng(2 * b(j, i + 1)) + CLng(2 * b(j, i + 2)) _
                         + CLng(b(j + 1, i - 1)) + CLng(b(j + 1, i + 1)) _
                         ) / 13
                b(j, i) = lngTemp
            Next
        Next
        '''''对最上两根线做处理''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        For j = 2 To lngWidth - 1
            lngTemp = CLng(b(j - 1, 1)) + CLng(b(j - 1, 2)) + CLng(2 * b(j - 1, 3)) + CLng(b(j, 1)) + CLng(b(j, 2))
            lngTemp = (CLng(lngTemp) + CLng(2 * b(j, 3)) + CLng(b(j + 1, 1)) + CLng(b(j + 1, 2)) + CLng(2 * b(j + 1, 3))) / 12
            b(j, 2) = lngTemp
        Next
        '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        For j = 1 To lngWidth - 1
            lngTemp = CLng(b(j - 1, 1)) + CLng(b(j - 1, 2)) + CLng(b(j - 1, 3)) + CLng(b(j, 1))
            lngTemp = (CLng(lngTemp) + CLng(b(j, 2)) + CLng(b(j, 3)) + CLng(b(j + 1, 1)) + CLng(b(j + 1, 2)) + CLng(b(j + 1, 3))) / 9
            b(j, 1) = lngTemp
        Next
        ''''''对最下两根线做处理''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        For j = 2 To lngWidth - 1
            lngTemp = CLng(2 * b(j - 1, TolHeight - 2)) + CLng(b(j - 1, TolHeight - 1)) + CLng(b(j - 1, TolHeight)) + CLng(2 * b(j, TolHeight - 2))
            lngTemp = (CLng(lngTemp) + CLng(b(j, TolHeight - 1)) + CLng(b(j, TolHeight)) + CLng(2 * b(j + 1, TolHeight - 2)) + CLng(b(j + 1, TolHeight - 1)) + CLng(b(j + 1, TolHeight))) / 12
            b(j, TolHeight - 1) = lngTemp
        Next
        '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        For j = 1 To lngWidth - 1
            lngTemp = CLng(b(j - 1, TolHeight)) + CLng(b(j - 1, TolHeight - 1)) + CLng(b(j - 1, TolHeight - 2)) + CLng(b(j, TolHeight)) + CLng(b(j, TolHeight - 1))
            lngTemp = (CLng(lngTemp) + CLng(b(j, TolHeight - 2)) + CLng(b(j + 1, TolHeight)) + CLng(b(j + 1, TolHeight - 1)) + CLng(b(j + 1, TolHeight - 2))) / 9
            b(j, TolHeight) = lngTemp
        Next
   Next
End Sub
