...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Public gintClickImageIndex As Integer                            ''''当前点击的图像序列号,0标识没有点中,在MOUSE中填写,供双击使用
...
'        If i = 0 Then   ''选中一个标注
    ...
    With Viewer(intSelectedSerial)
        ...
        ElseIf gintClickImageIndex > 0 And intDblClickButton = 1 Then
            ...
            If .MultiColumns > 1 Or .MultiRows > 1 Then
                ...
                .CurrentIndex = gintClickImageIndex
...
Private Sub Viewer_MouseDown(Index As Integer, Button As Integer, Shift As Integer, x As Long, y As Long)
    ...
    lngBaseYY = y
    gintClickImageIndex = Viewer(Index).ImageIndex(x, y)         ''''当前点击的图像INDEX
    intDblClickButton = Button                                  ''''供DblClick使用的鼠标按
    '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    '选择一个Viewer
    Call subSelectAViewer(Index, gintClickImageIndex)
    ...
    End If
    ''''''''''''''''处理鼠标左键,操作图像选择句柄''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    If Button = 1 And gintClickImageIndex <> 0 Then
        Set ls = Viewer(Index).LabelHits(x, y, True, False, False)
        If ls.Count <> 0 Then
            For Each l In ls ''''处理选择句柄
                If Mid(l.Tag, 1, 1) = "B" Then
                    l.Visible = Not l.Visible
                    ZLShowSeriesInfos(Index).ImageInfos(Viewer(Index).Images(gintClickImageIndex).Tag).blnSelected = IIf(l.Visible, True, False)
                    Viewer(Index).Refresh
                    Exit Sub
                End If
            Next
    ...
    End If
    ''''''''''''[标注的选择、移动、调整大小]'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    If Button = cMouseUsage("201").lngMouseKey And Shift = cMouseUsage("201").lngShift And gintClickImageIndex > 0 And Not LabelDrawing Then
...
'                viewer(Index).Refresh
    ...
    If ((Button = cMouseUsage("1").lngMouseKey And Shift = cMouseUsage("1").lngShift) _
       ...
       And gintClickImageIndex <> 0 And Not (blnTextInput Or blnTextInputM) Then
...
Private Sub Viewer_MouseMove(Index As Integer, Button As Integer, Shift As Integer, x As Long, y As Long)
    ...
    Dim i As Long
    Dim intMoveImageIndex As Integer
    
    If SelectedImage Is Nothing Then Exit Sub
    
    On Error GoTo err
    '''''''''''''''''''''''''''''''''''[在状态栏显示X/Y坐标和图像点值]'''''''''''''''''''''''''''''''''''''
    intMoveImageIndex = Viewer(Index).ImageIndex(x, y)
    
    '增加条件判断是否按下鼠标,按下不执行计算灰度值提高效率
    If (intMoveImageIndex <> 0 And Not blDicomDown) Then
        If Viewer(Index).Images(intMoveImageIndex).FrameCount > 1 And Button_miMouseShowValue = False Then
            '多帧图像，而且不随鼠标显示像素值，则不进行计算
            Me.Viewer(Index).ToolTipText = ""
        Else
            '存储像素值的调整解决和斜率
            If strInstanceUID <> Viewer(Index).Images(intMoveImageIndex).InstanceUID Then
                strInstanceUID = Viewer(Index).Images(intMoveImageIndex).InstanceUID
                intIntercept = 0
                intSlope = 1
                If Not IsNull(Viewer(Index).Images(intMoveImageIndex).Attributes(&H28, &H1052).Value) Then
                    intIntercept = Viewer(Index).Images(intMoveImageIndex).Attributes(&H28, &H1052)
                End If
                If Not IsNull(Viewer(Index).Images(intMoveImageIndex).Attributes(&H28, &H1053)) Then
                    intSlope = Viewer(Index).Images(intMoveImageIndex).Attributes(&H28, &H1053)
                End If
            End If
            a = Viewer(Index).Images(intMoveImageIndex).Pixels
            If Viewer(Index).ImageXPosition(x, y) > 0 And Viewer(Index).ImageYPosition(x, y) > 0 And Viewer(Index).ImageXPosition(x, y) < Viewer(Index).Images(intMoveImageIndex).sizex And Viewer(Index).ImageYPosition(x, y) < Viewer(Index).Images(intMoveImageIndex).sizey Then
    ...
    If Button = 0 And (Not SelectedLabel Is Nothing And Not blnReSizeLabel Or Button_miCutOut) Then
        If intMoveImageIndex = Viewer(Index).Images.IndexOf(SelectedImage) And (isSelectedLabel Or Button_miCutOut) Then
            i = funMouseOverPeriod(Viewer(Index), SelectedImage, x, y)  '返回鼠标所越过的句柄编号
    ...
    End If
    ''''''''''''''''''''''''''''''''''''''[调窗]''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    If (Button = cMouseUsage("102").lngMouseKey And Shift = cMouseUsage("102").lngShift And Button_miWidthLevel And (intMoveImageIndex <> 0 Or blnMouseStart)) _
...
'            viewer(intSelectedSerial).Refresh
    ...
    End If
    ''''''''''''''''''''''''''''''''''''''[缩放]''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    If (Button = cMouseUsage("104").lngMouseKey And Shift = cMouseUsage("104").lngShift And Button_miZoom And (intMoveImageIndex <> 0 Or blnMouseStart)) _
    ...
    End If
    ''''''''''''''''''''''''''''''''''''''[漫游]''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    If (Button = cMouseUsage("103").lngMouseKey And Shift = cMouseUsage("103").lngShift And Button_miCruise And (intMoveImageIndex <> 0 Or blnMouseStart)) _
...
'            viewer(intSelectedSerial).Refresh
    ...
    If Button = 0 And intMoveImageIndex <> 0 And blnAngle And Not (blnTextInput Or blnTextInputM) _
...
'------------------------------------------------
    Dim im As DicomImage
    Dim imgs As New DicomImages
    Dim imgImages As New DicomImages
    ...
    '如果没有选择图像，则将当前图像保存成报告图
    If imgImages.Count = 0 Then
        If gintClickImageIndex <> 0 And intSelectedSerial <> 0 Then
            imgImages.Add Viewer(intSelectedSerial).Images(gintClickImageIndex)
        End If
    End If
    
    '采集报告图像，根据参数，去掉标注
    For Each im In imgImages
        If strStudyUID = "" Then
             '从数据库中读取检查UID
