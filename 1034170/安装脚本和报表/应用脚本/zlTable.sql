...
CREATE TABLE 消费卡充值记录 (
	ID		Number(18),
	消费卡ID	number(18),
	序号            number(18),
	记录状态	number(18),--1-正常,2-充正
	结算方式  varchar2(20),
	充值金额	Number(16,5),
	充值折扣	Number(16,5),
	缴款金额	Number(16,5),
...
Create Global Temporary Table 医嘱打印记录(
		医嘱ID NUMBER(18),
		顺序 NUMBER(18),
		打印标记 NUMBER(1),
		特殊医嘱 number(1))
...
Create Table 排队记录(
       ...
       状态           Number(2), -- 0-正常 1-完成 2-弃号 3-退号 4-结束 5-待穿刺 6-待执行 7-执行中
       开始操作员  Varchar2(20),
       开始时间    Date,
       结束操作员  Varchar2(20),
       结束时间    Date,
...
Create Table 门诊输液操作日志(
	...
	科室ID  Number(18),
	挂号单	Varchar2(8),
	类别	Number(2),    --1-排队操作日志 2－医嘱操作日志 3-呼叫操作日志 4-坐位操作日志
	时间	Date,
	内容 Varchar2(4000),
	操作员	Varchar2(20))
	TABLESPACE zl9CisRec PCTFREE 5;
...
CREATE TABLE 病人护理活动项目(
	...
	页号 NUMBER (5),
	列号 NUMBER (5),
	列头名称 VARCHAR2 (20),		--此列的表头内容，缺省居中对齐
...
Create Table 检验酶标记录(
	...
	存放位置	varchar(50),		--用于保存存放位置
	单板单项	Number(1),		--是否是进行的单版单项测试 1=单版单项
	测试项目	VarChar(300),		--如是单板单项就只有一个项目 如单版多项格式：A_ID;B_ID:C_ID... 共8个项目
	阳性公式	VarChar(1000),		--如是单板单项就只有一个公式 如单版多项格式: A_公式;B_公式;C_公式...共8个公式
	弱阳性公式	VarChar(1000),		--如是单板单项就只有一个公式 如单版多项格式: A_公式;B_公式;C_公式...共8个公式
	CutOff公式	VarChar(1000),		--如是单板单项就只有一个公式 如单版多项格式: A_公式;B_公式;C_公式...共8个公式
	测试结果	varchar(3000),		--编号1^结果;编号2^结果...编号12^结果|编号1^结果;编号2结果...编号12^结果 共8行每行12个结果为空填为"^"
	试剂记录	varchar(1000)		--记录试剂:试剂批号;试剂效期;试剂厂商;测试方法|试剂批号;试剂效期;试剂厂商;测试方法|.....
	)
	TABLESPACE zl9CisRec;
