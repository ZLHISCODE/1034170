   ...
   Begin MSComctlLib.StatusBar sta 
      ...
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Panel6 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            Object.Width           =   617
            MinWidth        =   617
            Picture         =   "frmStuffCharge.frx":1548
            Key             =   "PY"
         ...
         BeginProperty Panel7 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            ...
            Object.Width           =   617
            MinWidth        =   617
            Picture         =   "frmStuffCharge.frx":1B82
            Key             =   "WB"
      ...
      Begin MSComctlLib.ImageList imgList 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmStuffCharge.frx":21BC
               Key             =   ""
            EndProperty
         EndProperty
      End
...
Public Function zlBillEdit(ByVal frmMain As Form, _
    ...
    Optional strFeeTab As Long, Optional blnDelete As Long, _
    Optional strSuccesSaveNos As String, Optional ByVal byt应用场合 As Byte = 0, Optional ByRef objSaveData As Object, _
    ...
    strSuccesSaveNos = mstrSuccesSaveNos
        ...
        Select Case Bill.TextMatrix(0, Bill.Col)
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    Else 'If Not IsNull(mrsInfo!险类) Then '90304
...
'功能：检查单据中输入的负数数量及退回科室是否正确
    ...
    On Error GoTo errH
     Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, mobjBill.病人ID, mobjBill.主页ID, mlng执行库房ID, strValues(0), strValues(1), strValues(2), strValues(3), strValues(4), strValues(5), strValues(6), strValues(7), strValues(8), strValues(9), strValues(10))
    ...
    If mbytInState = 3 Then
        ...
        gcnOracle.BeginTrans: blnTrans = True
            ...
            If Not mobjSaveData Is Nothing Then
                If mobjSaveData(mstrInNO) = False Then
    ...
    With rsSel
        ...
        Do While Not .EOF
            ...
            If bln重复 = False Then
                ...
                '114498
                str摘要 = mobjBill.Details(mobjBill.Details.Count).摘要
                str摘要 = gclsInsure.GetItemInfo(Val(Nvl(mrsInfo!险类)), mrsInfo!病人ID, _
                    mobjBill.Details(mobjBill.Details.Count).收费细目ID, str摘要, 2, , "|1")
                mobjBill.Details(mobjBill.Details.Count).摘要 = str摘要
...
'功能：获取病人信息
    ...
    Else
        strSQL = "Select" & _
            ...
            " A.担保人," & IIf(mint病人来源 = 2 And mint记录性质 = 2, "Decode(A.担保额,null,A.担保额,Zl_Patientsurety(A.病人ID,B.主页ID)) 担保额,", "A.担保额,") & _
            " Nvl(B.医疗付款方式,A.医疗付款方式) as 医疗付款方式,Y.编码 as 付款码,zl_PatiWarnScheme(A.病人ID,B.主页ID) as 适用病人," & _
            " zl_PatiDayCharge(A.病人ID) as 当日额,Nvl(C.险类,A.险类) as 险类,Nvl(B.病人性质,0) as 病人性质,b.审核标志,B.备注 as 病人备注," & _
            " NVL(B.病人类型,A.病人类型) as 病人类型" & _
        ...
        With mobjBill.Details(lngRow)
            ...
            If .Detail.变价 Then
                ...
                If dblAllTime <> 0 Then
                    dblPrice = Get时价材料应收金额(mlng虚拟库房ID, .收费细目ID, .Detail.批次, dblAllTime, gSysPara.Money_Decimal.strFormt_VB, dblPriceSingle)
                    If dblAllTime <> 0 Then
                        '数量未分解完毕
            ...
            For Each mobjBillIncome In mobjBillDetail.InComes
                ...
                If mint病人来源 = 2 Then
                    gstrSQL = gstrSQL & int划价 & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & _
                        ...
                        IIf(lng医疗小组ID = 0, -1, lng医疗小组ID) & ",1," & IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & ")"
                        '    医技补临床费用_In Number := 0,
                ...
                Else
                    ...
                    Else
                        gstrSQL = gstrSQL & "'" & UserInfo.姓名 & "'," & _
                            ...
                            "'" & mobjBillDetail.保险编码 & "','" & mobjBillDetail.Detail.类型 & "'," & IIf(mobjBillDetail.保险项目否, 1, 0) & "," & ZVal(mobjBillDetail.保险大类ID) & "," & _
                            "NULL,1," & IIf(mobjBillDetail.Detail.批次 = -1 Or mobjBillDetail.Detail.批次 = 0, "Null", mobjBillDetail.Detail.批次) & "  )"
                            '频次_In       药品收发记录.频次%Type := Null,
                            '单量_In       药品收发记录.单量%Type := Null,
                            '用法_In       药品收发记录.用法%Type := Null, --用法[|煎法]
                            '期效_In       药品收发记录.扣率%Type := Null,
...
Private Function SelectItem(ByVal blnInput As Boolean) As Boolean
    ...
    If frmStuffSelect.ShowSelect(Me, mstrPrivs, mint病人来源, int险类, strInput, lngTXTHwnd, str特准项目, mlng虚拟库房ID, False, rsItem, mbln条码刷卡) = False Then GoTo GoNotSel
...
Private Function Set虚拟库房ID(ByVal lng执行科室 As Long) As Long
    ...
    Set rsTemp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng执行科室)
...
Private Function AddStuffItemFromBarCode(ByVal strBarCode As String) As Boolean
    ...
    Dim blnAdd As Boolean, rsItem As ADODB.Recordset
    On Error GoTo errHandle
    ...
    If frmStuffSelect.ShowSelect(Me, mstrPrivs, mint病人来源, intInsure, strBarCode, txtBarCode.hWnd, str特准项目, mlng虚拟库房ID, False, rsItem, True) = False Then Exit Function
    If rsItem Is Nothing Then Exit Function
    If rsItem.State <> 1 Then Exit Function
    If rsItem.RecordCount = 0 Then Exit Function
    
    lng项目ID = Val(Nvl(rsItem!收费项目ID))
    Set mobjDetail = GetInputDetail(lng项目ID)
    If intInsure <> 0 Then sta.Panels(4).Text = Get医保大类(lng项目ID, intInsure)
    mobjDetail.批次 = Val(Nvl(rsItem!批次))
    mobjDetail.商品条码 = Trim(Nvl(rsItem!商品条码))
    mobjDetail.内部条码 = Trim(Nvl(rsItem!内部条码))
    mobjDetail.库存 = Val(Nvl(rsItem!可用库存))
