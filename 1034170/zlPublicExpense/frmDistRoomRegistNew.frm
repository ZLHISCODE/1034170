...
Private Sub cmdOK_Click()
    ...
    Dim strNotValiedNos As String, lng出诊记录ID As Long
    ...
    If blnBalance Then
        For i = 1 To mcolCardPayMode.Count
            ...
            End If
        Next i
        If CheckBrushCard(Val(cur现金), lng医疗卡类别ID, bln消费卡, mrsItems, mrsInComes, mrsInComes) = False Then Exit Sub
    ...
    If cboAppointStyle.Visible And mblnAppointment Then
        ...
        Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊记录ID, CDate(strTimeCheck), lngSN, NeedName(cboAppointStyle.Text))
        If rsTmp.EOF Then
            ...
            Exit Sub
        Else
            If Val(Mid(Nvl(rsTmp!预约检查), 1, 1)) <> 0 Then
                MsgBox "当前选择的预约方式无法预约,请选择其他预约方式!" & vbCrLf & "原因:" & Mid(Nvl(rsTmp!预约检查), InStr(Nvl(rsTmp!预约检查), "|") + 1), vbInformation, gstrSysName
                If cboAppointStyle.Enabled And cboAppointStyle.Visible Then cboAppointStyle.SetFocus
                Exit Sub
            End If
        End If
    ...
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng出诊记录ID, txtGender.Text, txtAge.Text)
    If rsTmp.EOF Then
        MsgBox "当前选择的病人不适用该号别!", vbInformation, gstrSysName
        Exit Sub
    Else
        If Val(Mid(Nvl(rsTmp!适用性检查), 1, 1)) <> 0 Then
            MsgBox "当前选择的病人不适用该号别!" & vbCrLf & "原因:" & Mid(Nvl(rsTmp!适用性检查), InStr(Nvl(rsTmp!适用性检查), "|") + 1), vbInformation, gstrSysName
            Exit Sub
        End If
    End If
    ...
    If GetSqlExpenses(cllPro, mrsExpenses, strNO, k, mRegistFeeMode = EM_RG_划价, str划价NO, ZVal(Nvl(mrsInfo!病人ID)), _
                ...
                cboRoom.Text, lng结帐ID, blnBalance, lng医生ID, str医生姓名, byt复诊) = False Then
        Exit Sub
    End If
...
Private Function GetSqlExpenses(ByRef cllPro As Collection, ByVal rsExpenses As ADODB.Recordset, _
                    ...
                    Optional ByVal blnBalance As Boolean, Optional ByVal lng医生ID As Long, Optional ByVal str医生姓名 As String, _
                    Optional ByVal byt复诊 As Byte) As Boolean
    '获取附加费记录sql
    Dim str费别 As String, str年龄 As String, strSQL As String
    Dim i As Long, lngPre项目ID As Long, lng挂号科室ID As Long, lng出诊记录ID As Long
    ...
    For i = 1 To rsExpenses.RecordCount
        ...
        strSQL = _
            "zl_病人挂号记录_出诊_INSERT(" & lng出诊记录ID & "," & ZVal(lng病人ID) & "," & IIf(str门诊号 = "", "NULL", str门诊号) & ",'" & txtPatient.Text & "','" & NeedName(txtGender.Text) & "'," & _
                     ...
                     "'" & txtArrangeNO.Text & "','" & cboRoom.Text & "'," & ZVal(lng结帐ID) & "," & IIf(blnNoPrint, "NULL", ZVal(mlng领用ID)) & "," & _
