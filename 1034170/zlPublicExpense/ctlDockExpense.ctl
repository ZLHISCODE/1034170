...
Private Function LoadDataFromAdvices(ByVal blnDataMoved As Boolean) As Boolean
    ...
    '只有一张单据,可能含其它医嘱费用;
    '显示原始单据信息和剩余部份金额
    '128369:李南春，2018/7/5，用户环境原因用Decode参与值比较有性能问题
    If InStr(mTYAdviceProperty.str计费状态, ",1,") > 0 Then
        ...
        "   And A.NO=B.NO " & _
        "   And ((A.记录性质 = 1 And B.记录性质 = 11) Or (A.记录性质 = B.记录性质 And B.记录性质 <> 11))" & _
  ...
  '以原始记录为准求(审核的登记时间和执行状态)
    strSQL = _
        ...
        " Where ((C.记录性质 = 1 And A.记录性质 = 11) Or (A.记录性质 = C.记录性质 And A.记录性质 <> 11)) And A.NO=C.NO "
