...
Public Function ShowSelect(frmParent As Object, ByVal strPrivs As String, _
    ...
    Optional rsReturnSel As ADODB.Recordset, _
    Optional ByVal bln条码扫描 As Boolean) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:显示卫生材料项目选择器
    ...
    mblnMyStyle = Val(gobjDatabase.GetPara("使用个性化风格")) = 1
    mint简码 = Val(gobjDatabase.GetPara("简码方式")) '简码匹配方式：0-拼音,1-五笔
    mbln输入内部条码 = Val(gobjDatabase.GetPara("备货卫材只能输内部条码", glngSys, 1257, 0)) = 1
    mbln条码扫描 = bln条码扫描
...
Private Sub Form_Load()
    ...
    mlng项目ID = 0: mstrLike = IIf(Val(gobjDatabase.GetPara("输入匹配")) = 0, "%", "")
...
Private Function FillTree() As Boolean
    ...
    On Error GoTo errH
    Set rsTmp = gobjDatabase.OpenSQLRecord(strSQL, Me.Name)
...
Private Function FillList(Optional ByVal blnClass As Boolean, Optional strIDs As String) As Boolean
    ...
    If mstr输入 <> "" Then
        If mbln输入内部条码 Then
            '备货卫材只能输内部条码
            strInput = " And M.内部条码=[4] And B.码类=[8]"
        Else
            If mbln条码扫描 Then
                strInput = " And (M.商品条码=[4] Or M.内部条码=[4] ) And B.码类=[8]"
                If Len(mstr输入) > 10 Then
                    strInput = " And (A.编码 Like [6] Or M.商品条码=[4] Or M.内部条码=[4] Or B.简码 Like [7]) And B.码类=[8]"
                ElseIf IsNumeric(mstr输入) Then                         '10,11.输入全是数字时只匹配编码,内部码,商品码
                    If Mid(gSysPara.strMatchMode, 1, 1) = "1" Then strInput = " And (A.编码 Like [6] Or M.商品条码=[4] Or M.内部条码=[4]) And B.码类=[8]"
        ...
        If mint险类 = 0 Then
            ...
            "       M.批次,M.商品条码,M.内部条码,nvl(M.可用数量,0) as 可用库存, M.上次供应商ID " & _
    ...
    "           Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价),'999999" & gSysPara.Price_Decimal.strFormt_VB & "'))) as 单价,A.可用库存," & _
                IIf(InStr(1, mstrPrivsOpt, "显示库存") > 0, " To_Char(A.可用库存,'9999990.00000')", "Decode(Sign(A.可用库存),1,'有','无')") & " as 库存,F.名称 As 供应商" & _
    " From (" & strMain & ") A,收费项目类别 B,收费价目 C,材料特性 D,诊疗项目目录 E,供应商 F" & _
    " Where A.类别=B.编码 And A.ID=C.收费细目ID   " & IIf(mbln包含核算材料, "", " And nvl(D.核算材料 ,0)=0 ") & _
    "           And A.ID=D.材料ID and A.上次供应商ID=F.ID(+) and nvl(D.跟踪在用,0)=1 And D.诊疗ID=E.ID" & str分类ID & _
    ...
    "               A.规格,A.产地,A.费用类型,A.医保大类,A.说明,A.批次,A.商品条码,A.内部条码,A.是否变价,A.计算单位,A.可用库存, To_Char(A.可用库存,'9999990.00000'),F.名称"
    strSQL = strSQL & " Order by 顺序ID,编码,批次,内部条码,商品条码" '使用类别序号排序
    
    On Error GoTo errH
    Screen.MousePointer = 11
    Set mrsItem = gobjDatabase.OpenSQLRecord(strSQL, Me.Name, int类型, lng诊疗分类ID, lng收费分类ID, UCase(mstr输入), "4", _
