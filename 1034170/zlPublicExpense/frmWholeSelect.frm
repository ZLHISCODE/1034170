...
Private Function InitSelFinelds() As Boolean
    ...
    mrsOutSel.Fields.Append "收费类别", adBigInt, , adFldIsNullable
    ...
    With vsWholeSet
        For i = 1 To .Rows - 1
            If Val(.Cell(flexcpData, i, .ColIndex("收费项目"))) <> 0 And GetVsGridBoolColVal(vsWholeSet, i, .ColIndex("选择")) Then
                ...
                mrsOutSel!收费类别 = Val(.TextMatrix(i, .ColIndex("类别")))
                mrsOutSel.Update
            End If
        Next
    End With
...
Private Function FillWholeItem(ByVal lng分类id As Long, Optional blnSearch As Boolean = False) As Boolean
    ...
    If blnSearch = False Or mrs成套项目 Is Nothing Then
            ...
            Set mrs成套项目 = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, lng分类id, UserInfo.ID)
    End If
     If Trim(txtFind.Text) <> "" Then
            strInput = Trim(txtFind.Text)
            If IsNumeric(strInput) Then
                    strInput = "'" & gSysPara.strLike & Trim(txtFind.Text) & "%'"
                    mrs成套项目.Filter = "编码 like " & UCase(strInput)
            ElseIf gobjCommFun.IsNumOrChar(strInput) Then
                    'gSysPara.bytCode As Byte '简码生成方式，0-拼音,1-五笔,2-两者
                    strInput = "'" & gSysPara.strLike & Trim(txtFind.Text) & "%'"
                    If gSysPara.bytCode = 0 Then
                        mrs成套项目.Filter = "拼音 like " & UCase(strInput)
                    ElseIf gSysPara.bytCode = 1 Then
...
Private Function FillWholeSetItemChildData(ByVal lng成套ID As Long) As Boolean
    ...
    Set rsOthers = gobjDatabase.OpenSQLRecord(gstrSQL, Me.Caption, str成套)
    ...
    Set rsTemp = gobjDatabase.OpenSQLRecord(strSQL, Me.Caption, str成套, lngSelect成套ID)
...
Private Sub Form_Load()
    ...
    gobjControl.LvwSelectColumns lvwWholeSetItem, mstrLvwWholeSet, True
