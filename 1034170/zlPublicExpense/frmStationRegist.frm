...
Private Function GetSqlExpenses(ByRef cllPro As Collection, ByVal rsExpenses As ADODB.Recordset, _
    ...
    For i = 1 To rsExpenses.RecordCount
        ...
        strSQL = _
        "zl_病人挂号记录_INSERT(" & ZVal(lng病人ID) & "," & IIf(str门诊号 = "", "NULL", str门诊号) & ",'" & txtPatient.Text & "','" & mstrGender & "'," & _
        ...
        "'" & Get号别 & "','" & strRoom & "'," & ZVal(lng结帐ID) & "," & IIf(blnNoPrint, "NULL", ZVal(mlng领用ID)) & "," & _
    ...
    If Not mrsInfo Is Nothing And (mblnAppointment = False Or mty_Para.bln预约时收款) Then
        If mrsInfo.State = 1 Then
            If Not mrsInfo.EOF Then
                ...
                Call ReadExRegistPrice(mrsExpenses, mblnAppointPrice, Val(Nvl(mrsInfo!病人ID)), mintInsure, Get号别, _
