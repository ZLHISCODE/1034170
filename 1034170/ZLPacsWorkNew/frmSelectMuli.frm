...
Private mblnHaveSel As Boolean '是否已经全选过了
...
'intSearchType:0-按检查uid搜索,1-按序列UID搜索,2-按图像UID搜索
    ...
    If mlngReleationType = 2 Then
        ...
        strSql = "Select rownum as 顺序号,  A.图像号,d.FTP用户名 As User1, d.FTP密码 As Pwd1, d.Ip地址 As Host1," & _
                ...
                " Where A.序列UID = B.序列UID And B.检查UID = C.检查UID And  C.位置一 = D.设备号 " & IIf(mlngModule = G_LNG_PACSSTATION_MODULE, " and A.图像号<=100 ", " ")
    Else
        '取消关联
        ...
        strSql = "Select rownum as 顺序号, A.图像号,D.FTP用户名 As User1,D.FTP密码 As Pwd1," & _
            ...
            "Where A.序列UID=B.序列UID And B.检查UID=C.检查UID And C.位置一=D.设备号(+) And C.位置二=E.设备号(+) " & IIf(mlngModule = G_LNG_PACSSTATION_MODULE, " and A.图像号<=100 ", " ")
...
'        Call funGetStorageDevice(Me, strDeviceNO, strFtpUrl, strFTPIP, strFTPUser, strFTPPwd)
        ...
        On Error GoTo errHandle
            ...
            '创建FTP目录
            Call CreateFTPDir(objDestFtp, strFtpUrl, Format(curDate, "YYYYMMDD") & "/" & strNewStudyUID)
...
Private Sub CreateFTPDir(objFtp As clsFtp, strFtpUrl As String, strPath As String)
    
    
    '创建FTP目录
    objFtp.FuncFtpMkDir strFtpUrl, strPath
End Sub
        ...
        Else
            ...
            'FTP中存在
                ...
                If lngResult <> 0 Then
                    ...
                    If lngResult <> 0 Then
                        Call CreateFTPDir(objDestFtp, "", strFtpVirtualPath)
                        lngResult = objDestFtp.FuncReNameFile(strVirtualPath & "/" & strImageUID, strFtpVirtualPath & "/" & strImageUID)
                ...
                End If
            ElseIf Len(Dir(IIf(Len(App.Path) > 3, App.Path & "\TmpImage\", App.Path & "TmpImage\") & Nvl(rsImageDatas!Url) & "/" & strImageUID)) > 0 Then
            'FTP上不存在而本地存在
                lngResult = objDestFtp.FuncUploadFile(strFtpVirtualPath, IIf(Len(App.Path) > 3, App.Path & "\TmpImage\", App.Path & "TmpImage\") & Nvl(rsImageDatas!Url) & "/" & strImageUID, strImageUID)
                
                If lngResult <> 0 Then
                    Call CreateFTPDir(objDestFtp, "", strFtpVirtualPath)
                    lngResult = objDestFtp.FuncUploadFile(strFtpVirtualPath, IIf(Len(App.Path) > 3, App.Path & "\TmpImage\", App.Path & "TmpImage\") & Nvl(rsImageDatas!Url) & "/" & strImageUID, strImageUID)
                    If lngResult <> 0 Then
                        MsgBoxD Me, "上传关联图像失败，请检查FTP设置。", vbInformation, gstrSysName
                        Exit Function
                    End If
                End If
                blnIsMove = True
...
'选择全部影像
    ...
    cmdSelAll.tag = IIf(cmdSelAll.tag = "1", "2", "1") '1---全清,2---全选
    cmdSelAll.Caption = IIf(cmdSelAll.tag = "1", "全 选(&A)", "全 清(&A)")
    
    If (cmdSelAll.tag = "2") And mblnHaveSel = False Then
        For i = vsfTree.Rows - 1 To 0 Step -1
            If vsfTree.RowOutlineLevel(i) = 1 Then
                Call LoadSeriesNode(i)
            End If
        Next i
        mblnHaveSel = True
    End If
    
    If vsfTree.Rows > 0 Then
        vsfTree.Cell(flexcpChecked, 0, 0, vsfTree.Rows - 1, 0) = IIf(cmdSelAll.tag = "2", True, False)
    End If
End Sub
...
On Error GoTo errHandle
    ...
    cmdDel.Top = 60
    If mlngReleationType = 2 Then
        cmdSelAll.Left = cmdDel.Left + cmdDel.Width + 120
        cmdSelAll.tag = "1"
    Else
        cmdSelAll.Left = 120
        cmdSelAll.Caption = "全 清(&A)"
        cmdSelAll.tag = "2"
    End If
    cmdSelAll.Top = 60
errHandle:
