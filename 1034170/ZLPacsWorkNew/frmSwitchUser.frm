...
Public Sub SetModule(ByVal lngValue As Long)
    mlngModule = lngValue
End Sub
...
Private Sub cmdSwitch_Click()
    ...
    If UCase(Trim(txtUserID.Text)) = gstrDBUser Then
        MsgBoxD Me, "将要切换的用户是当前用户，请检查", vbOKOnly, "提示"
        Exit Sub
    End If
    ...
    '查找用户权限
    '注意GetPrivFuncByUser 待查用户不能是当前gcnOracle 用户，否则不能准确查到权限。
    strPrivs = GetPrivFuncByUser(100, mlngModule, strUserIDNew)
    If strPrivs = "" And UCase(strUserIDNew) <> "ZLHIS" Then
        MsgBoxD Me, "新用户不具备使用“影像工作站”" & mlngModule & "模块的权限！切换用户操作失败"
        Exit Sub
    End If
