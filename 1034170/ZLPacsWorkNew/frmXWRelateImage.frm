...
'关联图像
    ...
    lngSelectState = GetSelectData(lngStudyId, strSeriesIds, strUnCheckIds)
    ...
    strStudyDate = lvwUnMatched.SelectedItem.SubItems(5) & " " & lvwUnMatched.SelectedItem.SubItems(6)
    
    If lngSelectState = 2 Then
        '关联整个检查
        ReleationImages = ReleationStudy(lngOrderID, lngStudyId, strStudyDate, IIf(lngSourceStudyId = 0, True, False))
    Else
        '关联部分序列
...
Private Function ReleationStudy(ByVal lngOrderID As Long, ByVal lngStudyId As Long, _
    ByVal strStudyDate As String, Optional ByVal blnUpdateRis As Boolean = True) As Boolean
'关联检查
    Dim strSql As String
    Dim rsOrderInfo As ADODB.Recordset
    ...
    If rsOrderInfo.RecordCount > 0 Then
        ...
        If blnUpdateRis = True Then
            ...
            strSql = IIf(Trim(gstrOracleOwner) <> "", gstrOracleOwner & ".", "") & "b_XINWANGInterface.PacsStatusChange(1," & lngOrderID & ",'" & Nvl(rsOrderInfo!影像类别) & "'," & rsOrderInfo!检查号 & ",to_date('" _
                        & Trim(strStudyDate) & "','yyyy-mm-dd hh24:mi:ss'),null,null)"
...
Private Function GetSelectData(ByRef lngStudyId As Long, ByRef strSeriesIds As String, _
    ByRef strUnCheckIds As String) As Long
'获取界面的数据选择情况
'返回值定义：0-未选择,1-选择部分序列,2-选择整个检查
    Dim i As Long
    Dim lngSelectState As Long
