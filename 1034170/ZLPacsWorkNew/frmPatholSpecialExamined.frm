...
On Error GoTo ErrorHand
    ...
    Select Case control.ID
        Case conMenu_PatholSpeExam_LAB, conMenu_PatholSpeExam_PreviewLAB
            '预览标签
        ...
        Case conMenu_PatholSpeExam_List, conMenu_PatholSpeExam_PreviewList
            '预览清单
...
On Error GoTo ErrorHand
    ...
    Select Case control.ID
        Case conMenu_PatholSpeExam_LAB, conMenu_PatholSpeExam_List, conMenu_PatholSpeExam_RequestView
            control.Enabled = blnIsAllowSpeExam And blHavePatholNumber
        Case conMenu_PatholSpeExam_Accept, conMenu_PatholSpeExam_Finish
            control.Enabled = blnIsAllowSpeExam And Not mblnReadOnly And blHavePatholNumber
    End Select
    Exit Sub
ErrorHand:
    If ErrCenter() = 1 Then Resume
...
'创建影像记录对应的菜单
    ...
    If Not HasMenu(objMenuBar, conMenu_PatholSpeExam) Then
        ...
        With cbrMenuBar.CommandBar
            ...
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlPopup, conMenu_PatholSpeExam_List, "清单打印(&L)", "", 1, True)
            ...
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_PatholSpeExam_Accept, "特检接受(&R)", "", 1, True)
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_PatholSpeExam_Finish, "特检完成(&F)", "", 1, True)
        End With
    End If
End Sub
...
'更新菜单
    ...
    Select Case control.ID
        ...
        Case conMenu_PatholSpeExam_Accept, conMenu_PatholSpeExam_Finish
            control.Enabled = blnIsAllowSpeExam And Not mblnReadOnly And mrecStudyInf.strPatholNumber <> ""
    End Select
...
Private Sub AdjustFace()
    ...
    framSpeExam.Height = Me.Height - 1000
    
    
    ufgData.Left = 120
    ufgData.Top = 280 + IIf(mbytFontSize = C_INT_FONTSISE_SMALL, 0, 120)
...
'载入特检数据（包括免疫组化，分子病理，特殊染色）
    ...
    strSql = "select to_Number(a.ID) as ID,a.材块ID,to_Number(b.序号) as 序号,b.标本名称,to_Number(a.申请ID) as 申请ID,to_Number(a.抗体ID) as 抗体ID,c.抗体名称,to_Number(a.制作类型) as 制作类型,to_Number(a.当前状态) as 当前状态,a.项目结果, d.申请时间, a.完成时间,a.特检医师 as 特检技师,to_Number(a.清单状态) as 清单状态,to_Number(a.特检类型) as 特检类型,to_Number(a.特检细目) as 特检细目" & _
...
On Error GoTo errHandle
    ...
    ''''''''''''''''''''单独调整位置  107522
    optMianYi.Left = TextWidth(framSpeExam.Caption) * 1.4
    optTeShu.Left = optMianYi.Left + optMianYi.Width + TextWidth("12")
    optFenZi.Left = optTeShu.Left + optTeShu.Width + TextWidth("12")
    
    FramCheck.Width = chkYSQ.Width + chkYJS.Width + chkYWC.Width
    chkYSQ.Left = FramCheck.Width - chkYSQ.Width
    chkYJS.Left = FramCheck.Width - chkYSQ.Width - chkYJS.Width
    chkYWC.Left = FramCheck.Width - chkYSQ.Width - chkYJS.Width - chkYWC.Width
...
On Error GoTo errH
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlSplitButtonPopup, conMenu_PatholSpeExam_LAB, "标签打印"): cbrControl.IconId = 5001: cbrControl.ToolTipText = "标签打印"
            With cbrControl
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholSpeExam_PreviewLAB, "预览", "", 0, False)
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholSpeExam_PrintLab, "打印", "", 0, False)
            End With
        Set cbrControl = .Add(xtpControlSplitButtonPopup, conMenu_PatholSpeExam_List, "清单打印"): cbrControl.IconId = 5010: cbrControl.ToolTipText = "清单打印"
            With cbrControl
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholSpeExam_PreviewList, "预览", "", 0, False)
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholSpeExam_PrintList, "打印", "", 0, False)
            End With
        cbrControl.BeginGroup = True
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholSpeExam_RequestView, "申请查看"): cbrControl.IconId = 5008: cbrControl.ToolTipText = "申请查看"
        cbrControl.BeginGroup = True
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholSpeExam_Accept, "特检接受"): cbrControl.IconId = 5006: cbrControl.ToolTipText = "特检接受"
        cbrControl.BeginGroup = True
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholSpeExam_Finish, "特检完成"): cbrControl.IconId = 5011: cbrControl.ToolTipText = "特检完成"
        cbrControl.BeginGroup = True
