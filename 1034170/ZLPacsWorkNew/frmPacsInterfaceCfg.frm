...
On Error GoTo ErrorHand
    ...
    With vsfAppFuns
        ...
        .ColWidth(mAppFuncCol.功能名称) = 900
        .ColWidth(mAppFuncCol.启用功能) = 900
        .ColWidth(mAppFuncCol.加入右键菜单) = 900
        .ColWidth(mAppFuncCol.加入工具栏) = 700
        ...
        .ColHidden(mAppFuncCol.对应方法) = True
...
Private Sub InitAppFuncs()
    ...
    With vsfFuncs
        ...
        .TextMatrix(0, mAppFuncsCol.方法序号) = "≡"
        .TextMatrix(0, mAppFuncsCol.功能方法) = "方法执行序列"

        For i = 1 To .Rows - 1
            .TextMatrix(i, mAppFuncsCol.方法序号) = i
        Next
...
'    Call vsfAppFuns.Move(120, 300, fraAppFuns.Width - 240)
    Call vsfAppFuns.Move(120, 300, fraAppFuns.Width - 3000)
    
    Call vsfFuncs.Move(vsfAppFuns.Left + vsfAppFuns.Width, 300, 2760, vsfAppFuns.Height)
...
Private Sub vsfAppFuns_BeforeRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long, Cancel As Boolean)
    ...
    If Trim(vsfFuncs.TextMatrix(1, mAppFuncsCol.功能方法)) = "" And vsfAppFuns.Rows > 2 Then
        MsgBox "功能对应方法不能为空，请输入！", vbExclamation, gstrSysName
        Exit Sub
    End If
...
On Error GoTo ErrorHand
    ...
    If vsfAppFuns.TextMatrix(vsfAppFuns.RowSel, mAppFuncCol.对应方法) <> "" Then
        ...
        For i = 0 To UBound(Split(strFuncInfo, "★"))
            vsfFuncs.TextMatrix(i + 1, mAppFuncsCol.功能方法) = Split(strFuncInfo, "★")(i)
        Next
        
        For i = 0 To UBound(Split(strParaInfo, "★"))
            vsfFuncs.TextMatrix(i + 1, mAppFuncsCol.方法参数) = Split(strParaInfo, "★")(i)
        Next
    End If
    
    If vsfFuncs.RowSel <> 1 Then
        vsfFuncs.Row = 1
        vsfFuncs.RowSel = 1
    Else
        Call LoadFuncParaCfg
