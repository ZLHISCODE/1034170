...
Public Sub OpenClearType()
    Call SystemParametersInfo(SPI_SETFONTSMOOTHING, 1, ByVal 0&, SPIF_UPDATEINIFILE Or SPIF_SENDCHANGE)
    Call SystemParametersInfo(SPI_SETFONTSMOOTHINGTYPE, 0, ByVal FE_FONTSMOOTHINGCLEARTYPE, SPIF_UPDATEINIFILE Or SPIF_SENDCHANGE)
End Sub
...
'Public Function funRelateSeries(frmParent As Form, ByVal lng医嘱ID As Long, ByVal lng发送号 As Long, _
'    Optional ByVal blnSaveReportImg = False, Optional ByVal blnMoved = False, Optional ByRef RptViewer As DicomViewer = Null)
''-----------------------------------------------------------------------------
...
'
'    frmSelectMuli.ShowImageReleation lng
'
'    If frmSelectMuli.mblnOK = True And frmSelectMuli.strFilter <> "ID=-1" Then
'
'        If MsgBoxD(frmParent, "是否确认选择的影像是当前检查的？", vbDefaultButton2 + vbQuestion + vbYesNo, gstrSysName) = vbNo Then Exit Function
...
'        While Not rsTmp.EOF
'
'            strRelateStudyUids = strRelateStudyUids & ";" & rsTmp("ID")
'
'            If strStudyUID = "" Then
...
'                    strStudyUID = Nvl(rsStudyUID!检查uid)
'                End If
'            End If
'
'            If Len(Trim(strStudyUID)) > 0 Then
'                If MergeImageFiles(rsTmp("ID"), strStudyUID, "", "", True) = False Then
'                    MsgBoxD frmParent, "文件转移失败，不能关联影像。" & vbCrLf & vbCrLf & "可能是网络连接有问题，请检查。"
'                    Exit Function
'                End If
'            End If
'
'            gstrSQL = "ZL_影像检查_SET(" & lng医嘱ID & "," & lng发送号 & ",'" & rsTmp("ID") & "')"
...
'
'        '保存报告图
'        If blnSaveReportImg Then
'
'            '重新获取检查UID
'            gstrSQL = "Select 检查UID,报告图象 From 影像检查记录 Where 医嘱ID=[1] And 发送号=[2]"
'            Set rsStudyUID = zlDatabase.OpenSQLRecord(gstrSQL, "关联影像", lng医嘱ID, lng发送号)
'
'            strStudyUID = ""
'            strReportImages = ""
...
'            If Not rsStudyUID.EOF Then
'                strStudyUID = Nvl(rsStudyUID!检查uid)
'                strReportImages = Nvl(rsStudyUID!报告图象)
'            End If
'
'
'            gstrSQL = "select 图像UID from 影像检查图象 A, 影像检查序列 B where a.序列UID=b.序列UID and b.检查UID=[1]"
'            Set rsImageUID = zlDatabase.OpenSQLRecord(gstrSQL, "关联影像", strStudyUID)
'
'            While Not rsImageUID.EOF
'                If InStr(1, strReportImages, rsImageUID!图像UID) <= 0 Then
'                    strReportImages = strReportImages & ";" & rsImageUID!图像UID & ".jpg"
'
'                    If Len(strReportImages) < 4000 Then
'                        gstrSQL = "ZL_影像检查报告_ADD('" & strStudyUID & "','" & rsImageUID!图像UID & ".jpg')"
'                        zlDatabase.ExecuteProcedure gstrSQL, "保存报告图像"
'                    End If
'                End If
'
'                '移动到rsImageUID数据末尾
...
'
'            '下载报告图
'            Call GetRptImages(RptViewer, lng医嘱ID, blnMoved)
'        End If
'
'    End If
'
...
'参数： lng医嘱ID --医嘱ID
'       lng来源 -- 病人来源
'返回值： 0--欠费；1--已收费；2--无收费，3-记账记录，4-补缴，5-退费，6-销账，7-有调动
...
Public Sub PromptResult(lngOrderID As Long, lngModul As Long, frmParent As Form, lngCur科室ID As Long, strResultInput As String)
    ...
    If lngModul <> 1294 Then    '除病理外，需记录诊断符合情况
        If Split(strResult, "-")(4) = 1 Then    '影像质量
...
'------------------------------------------------
    ...
    If gethostname(hostname, 256) = SOCKET_ERROR Then
        MsgBoxD frmParent, "Windows Sockets error " & str(WSAGetLastError())
        Exit Function
    Else
    ...
    If hostent_addr = 0 Then
        MsgBoxD frmParent, "Winsock.dll is not responding."
        Exit Function
    End If
...
Dim lReturn As Long
    ...
    If lReturn <> 0 Then
        MsgBoxD frmParent, "Socket error " & Trim$(str$(lReturn)) & " occurred in Cleanup "
        Exit Sub
    End If
End Sub
...
'------------------------------------------------
    ...
    strFileName = App.Path & "\" & Format(date, "yyyymmdd") & "-" & intHour & ".log"
