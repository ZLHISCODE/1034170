...
'***********************************************************************************
Private mblnBeginchange As Boolean   '开始变更，含新增和修改
Private mblnChanged As Boolean '是否修改了内容 （右侧输入框录入）
...
Private Sub cbrMain_Update(ByVal control As XtremeCommandBars.ICommandBarControl)
    Select Case control.ID
        Case conMenu_Edit_NewItem '新增 没权限和增删改、配置任一功能开始后其它按键都不可用
            If Not CheckPopedom(mstrPrivs, "增删改") Or mblnBeginchange Then control.Enabled = False: Exit Sub
...
Private Sub cmdCancel_Click()
    If mblnChanged Then
        If MsgBoxD(Me, "当前变更尚未保存,确定要取消吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then TxtName.SetFocus: Exit Sub
    End If
    mblnBeginchange = False
    mblnChanged = False
    Call ClearControl
    Call ControlLocked
...
Private Sub Form_Load()
    ...
    Call HookDefend(txtPassWord.hwnd)
    Call HookDefend(txtSDPassword.hwnd)
End Sub
...
Dim cbrControl As CommandBarControl
    If mblnBeginchange Then TxtName.SetFocus: Exit Sub
...
Private Sub lvwItem_ItemClick(ByVal Item As MSComctlLib.ListItem)
    ...
    Dim i As Integer
'显示数据
    If mblnBeginchange And mblnChanged Then '开始修改或新增后
        If MsgBoxD(Me, "当前变更尚未保存，确定要更换查看吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
            TxtName.SetFocus
            Exit Sub
        End If
    End If
...
'处理控件锁定状态，只有点击新增或者修改后才能改动输入控件的内容
    Cbosort.Locked = Not mblnBeginchange
    cboType.Locked = Not mblnBeginchange
    TxtName.Locked = Not mblnBeginchange
    txtDevAdress.Locked = Not mblnBeginchange
    txtFtpPath.Locked = Not mblnBeginchange
    txtUser.Locked = Not mblnBeginchange
    txtPassWord.Locked = Not mblnBeginchange
    txtShareDir.Locked = Not mblnBeginchange
    txtSDUser.Locked = Not mblnBeginchange
    txtSDPassword.Locked = Not mblnBeginchange
    txtDevLocalAE.Locked = Not mblnBeginchange
    txtDevAE.Locked = Not mblnBeginchange
    txtDevPort.Locked = Not mblnBeginchange
    txtDirPath.Locked = Not mblnBeginchange
End Sub

Private Sub txtDevAdress_Change()
    mblnChanged = True
End Sub

Private Sub txtDevAE_Change()
    mblnChanged = True
End Sub

Private Sub txtDevLocalAE_Change()
    mblnChanged = True
End Sub

Private Sub txtDevPort_Change()
    mblnChanged = True
End Sub

Private Sub txtDirPath_Change()
    mblnChanged = True
End Sub
...
Private Sub txtSDPassword_Change()
    mblnChanged = True
End Sub

Private Sub txtSDUser_Change()
    mblnChanged = True
End Sub

Private Sub txtShareDir_Change()
    mblnChanged = True
End Sub
