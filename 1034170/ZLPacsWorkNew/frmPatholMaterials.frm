...
Begin VB.Form frmPatholMaterials 
   ...
   Begin VB.PictureBox picPane1 
      ...
      Begin TabDlg.SSTab tsFilter 
         ...
         WordWrap        =   0   'False
         TabCaption(0)   =   "巨检描述(&D)"
         TabPicture(0)   =   "frmPatholMaterials.frx":005E
         Tab(0).ControlEnabled=   0   'False
         Tab(0).ControlCount=   0
         TabCaption(1)   =   "脱钙管理(&C)"
         TabPicture(1)   =   "frmPatholMaterials.frx":007A
...
On Error GoTo ErrorHand
    ...
    Select Case control.ID
        Case conMenu_PatholMaterial_LAB, conMenu_PatholMaterial_PreviewAll
            '预览标签
...
On Error GoTo ErrorHand
    ...
    Select Case control.ID
        Case conMenu_PatholMaterial_LAB, conMenu_PatholMaterial_PreviewAll, conMenu_PatholMaterial_PrintAll
            control.Enabled = blnIsAllowMaterial And blHavePatholNumber
        Case conMenu_PatholMaterial_Get, conMenu_PatholMaterial_Del, conMenu_PatholMaterial_Save, _
        conMenu_PatholMaterial_Sure, conMenu_PatholMaterial_RequestView
            control.Enabled = blnIsAllowMaterial And Not mblnReadOnly And blHavePatholNumber
    End Select
    Exit Sub
ErrorHand:
    If ErrCenter() = 1 Then Resume
...
'创建影像记录对应的菜单
    ...
    If Not HasMenu(objMenuBar, conMenu_PatholMaterial) Then
        ...
        With cbrMenuBar.CommandBar
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlPopup, conMenu_PatholMaterial_LAB, "标签预览(&V)", "", 1, False)
            With cbrControl
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholMaterial_PreviewAll, "预览", "", 1, False)
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholMaterial_PrintAll, "打印", "", 1, False)
            End With
            
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_PatholMaterial_RequestView, "申请查看(&R)", "", 1, True)
            ...
            Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_PatholMaterial_Save, "材块保存(&S)", "", 0, True)
...
'更新菜单
    ...
    Select Case control.ID
        ...
        Case conMenu_PatholMaterial_Get, conMenu_PatholMaterial_Del, conMenu_PatholMaterial_Save, _
        conMenu_PatholMaterial_Sure, conMenu_PatholMaterial_RequestView
            control.Enabled = blnIsAllowMaterial And Not mblnReadOnly And mrecStudy.strPatholNumber <> ""
            
        Case conMenu_PatholMaterial_Decalcification, conMenu_PatholMaterial_ChangeVat, conMenu_PatholMaterial_CancelVat, conMenu_PahtolMaterial_Finish
 ...
 On Error GoTo errHandle
    ...
    For Each objCtrl In Me.Controls
        ...
        Case UCase("CommandButton")
            objCtrl.FontName = strFontType
            objCtrl.FontSize = bytFontSize
            objCtrl.Width = TextWidth("罗" & objCtrl.Caption)
            objCtrl.Height = TextHeight("罗") * 1.7
        End Select
    Next
    
    wtDescription.FontSize = bytFontSize
...
Private Sub RefreshMaterialCount()
    '刷新材块数量
    Dim lngTotal As Long '材块总数
    Dim lngTotalBlock As Long '蜡块总数
    Dim i As Long
            ...
            If Not ufgData.IsNullRow(i) Then
                ...
                Select Case mrecStudy.lngStudyType
                    Case StudyType.stNormal, StudyType.stMeet, StudyType.stAutopsy, StudyType.stIce, StudyType.stSpeed
                        If Val(ufgData.Text(i, gstrMaterial_是否蜡块)) = 1 Then
                            lngTotal = lngTotal + Val(ufgData.Text(i, gstrMaterial_材块数))
                            lngTotalBlock = lngTotalBlock + 1
                        End If
    ...
    Select Case mrecStudy.lngStudyType
        Case StudyType.stNormal, StudyType.stMeet, StudyType.stAutopsy, StudyType.stIce, StudyType.stSpeed
            labRecordInf.Caption = "蜡块总数：" & lngTotalBlock & "  材块总数：" & lngTotal
        Case StudyType.stCell
            labRecordInf.Caption = "细胞块数：" & lngTotal
    End Select
End Sub
...
'    If mblnMoved Then strSql = GetMovedDataSql(strSql)
    ...
    Do While Not rsSpeciman.EOF
        ...
        If mrecStudy.lngStudyType <> StudyType.stCell And mrecStudy.lngStudyType <> StudyType.stIce Then
            ufgData.Text(lngRow, gstrMaterial_材块数) = "1"
        ...
        Else
            If mrecStudy.lngStudyType = StudyType.stIce Then
                ...
                ufgData.Text(lngRow, gstrMaterial_材块数) = "0"
            Else
...
'保存当前取材信息
    ...
    Dim blnValid As Boolean
    Dim blnIsRestoreFocus As Boolean
    
    blnIsRestoreFocus = IIf(TypeName(Me.ActiveControl) = "WordInputText", True, False)
    
    '材块保存
    If ufgData.ShowingDataRowCount <= 0 Then
        Call MsgBoxD(Me, "没有找到需要保存的材块信息，请录入材块数据。", vbOKOnly, Me.Caption)
        If blnIsRestoreFocus Then Call RestoreDescriptFocus
...
Private Sub RestoreDescriptFocus()
'恢复描述焦点
On Error Resume Next
    tsFilter.SetFocus
    wtDescription.SetFocus
err.Clear
End Sub
        ...
        If ufgData.RowState(i) = TDataRowState.Add And Not blnIsSaveOnlyDel Then
            ...
            Select Case mrecStudy.lngStudyType
                Case StudyType.stNormal, StudyType.stMeet, StudyType.stAutopsy, StudyType.stSpeed
                    ...
                    lngCount = 1
                    If Not ufgData.DataGrid.ColHidden(ufgData.GetColIndex(gstrMaterial_材块数)) Then
                        lngCount = Val(ufgData.Text(i, gstrMaterial_材块数))
                    End If
            ...
            ufgData.Text(i, gstrMaterial_蜡块号) = strNewSeq
            ...
            Select Case mrecStudy.lngStudyType
                Case StudyType.stNormal, StudyType.stMeet, StudyType.stAutopsy, StudyType.stSpeed
                    strSql = "Zl_病理取材_常规更新('" & ufgData.KeyValue(i) & "','" & _
                                                        ufgData.Text(i, gstrMaterial_取材位置) & "','" & _
                                                        ufgData.Text(i, gstrMaterial_形状) & "'," & _
                                                        Val(ufgData.Text(i, gstrMaterial_材块数)) & "," & _
                                                        Val(ufgData.Text(i, gstrMaterial_制片数)) & ",'" & _
                                                        ufgData.Text(i, gstrMaterial_主取医师) & "','" & _
                                                        ufgData.Text(i, gstrMaterial_副取医师) & "'," & _
                                                        Val(ufgData.Text(i, gstrMaterial_是否蜡块)) & "," & _
...
'确认当前取材信息
    ...
    For i = 1 To ufgDecalin.DataGrid.Rows - 1
        If (Not ufgDecalin.IsNullRow(i)) And ufgDecalin.RowState(i) <> Del And (Not ufgDecalin.IsEmptyKey(i)) Then
...
On Error Resume Next
    ...
    txtPos.Height = IIf(mbytFontSize = C_INT_FONTSISE_SMALL, 315, 330)
    txtPos.Top = framMaterial.Height - txtPos.Height - 120 + IIf(mbytFontSize = C_INT_FONTSISE_SMALL, 0, 30)
    ...
    cbxSpecimenProcess.Left = labSpecimenProcess.Left + labSpecimenProcess.Width
...
Private Sub ufgData_OnAfterEdit(ByVal Row As Long, ByVal Col As Long)
    ...
    Select Case mrecStudy.lngStudyType
        Case StudyType.stNormal, StudyType.stMeet, StudyType.stAutopsy, StudyType.stSpeed  '常规,会诊,尸体检查
            If Val(ufgData.Text(Row, gstrMaterial_材块数)) < 1 And ufgData.Text(Row, gstrMaterial_是否蜡块) = "1-是" Then
                ufgData.Text(Row, gstrMaterial_材块数) = "1"
            End If
            ...
            If Not ufgData.DataGrid.ColHidden(ufgData.GetColIndex(gstrMaterial_材块数)) Then
                ...
                '如果未录入蜡块数量，则显示淡红色
                iCol = ufgData.GetColIndex(gstrMaterial_材块数)
                
                ufgData.CellColor(Row, iCol) = IIf(Val(ufgData.Text(Row, gstrMaterial_材块数)) <= 0, ufgData.ErrCellColor, ufgData.BackColor)
            End If
            
        Case StudyType.stIce  '冰冻检查
            '如果蜡块数为空则默认等于0  不为空则跳过
            If ufgData.Text(Row, gstrMaterial_材块数) = "" Then
                ufgData.Text(Row, gstrMaterial_材块数) = "0"
            End If
...
Private Sub ufgData_OnStartEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    If Row > 0 Then
        ...
        If mrecStudy.lngStudyType = StudyType.stIce Then
            ...
            '如果不是冰余，蜡块数则不允许进行编辑
            'If Col = ufgData.vfgHelper.GetColumnIndex(gstrMaterial_材块数) Then
            If Not ufgData.DataGrid.ColHidden(ufgData.GetColIndex(gstrMaterial_材块数)) Then
                If InStr(ufgData.Text(Row, gstrMaterial_是否冰余), "是") > 0 Then
                    If Val(ufgData.Text(Row, gstrMaterial_材块数)) <= 0 Then ufgData.Text(Row, gstrMaterial_材块数) = "1"
                Else
                    If Col = ufgData.GetColIndex(gstrMaterial_材块数) Then Cancel = True
                End If
            End If
            'End If
...
On Error GoTo errH
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlSplitButtonPopup, conMenu_PatholMaterial_LAB, "标签打印"): cbrControl.IconId = 5001: cbrControl.ToolTipText = "标签打印"
            With cbrControl
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholMaterial_PreviewAll, "预览", "", 0, False)
                Set cbrPopControl = CreateModuleMenu(.CommandBar.Controls, xtpControlButton, conMenu_PatholMaterial_PrintAll, "打印", "", 0, False)
            End With
        cbrControl.BeginGroup = True
        
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholMaterial_RequestView, "查看申请"): cbrControl.IconId = 5008: cbrControl.ToolTipText = "查看申请"
        cbrControl.BeginGroup = True
        
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholMaterial_Get, "提取材块"): cbrControl.IconId = 5009: cbrControl.ToolTipText = "提取材块"
        cbrControl.BeginGroup = True
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholMaterial_Del, "删除材块"): cbrControl.IconId = 5004: cbrControl.ToolTipText = "删除材块"
        
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholMaterial_Save, "保存材块"): cbrControl.IconId = 5005: cbrControl.ToolTipText = "保存材块"
        cbrControl.BeginGroup = True
        
        Set cbrControl = .Add(xtpControlButton, conMenu_PatholMaterial_Sure, "确认取材"): cbrControl.IconId = 5006: cbrControl.ToolTipText = "确认取材"
        cbrControl.BeginGroup = True
    End With
    Exit Sub
errH:
