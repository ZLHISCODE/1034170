...
Private mblnCanCut As Boolean '右键菜单是否允许剪切、粘贴、删除
...
Private mblnUfgColsList As Boolean            '是否在进行数据列表配置
...
Public Event OnAfterScroll(ByVal OldTopRow As Long, ByVal OldLeftCol As Long, ByVal NewTopRow As Long, ByVal NewLeftCol As Long)
Public Event OnBeforeScroll(ByVal OldTopRow As Long, ByVal OldLeftCol As Long, ByVal NewTopRow As Long, ByVal NewLeftCol As Long, Cancel As Boolean)
...
Public Event OnFilterData(rsData As ADODB.Recordset, rsClone As ADODB.Recordset)

Private mblnIsRefreshList As Boolean
...
'数据载入状态
Property Get IsLoading() As Boolean
    IsLoading = mblnIsRefreshList
End Property
...
'重置排序
    If vfgData.Rows <= 1 Then Exit Sub
    vfgData.Col = lngCol
    vfgData.Sort = lngWay
...
On Error Resume Next
    ...
    If mblnIsRefreshList = True Then Exit Sub
    mblnIsRefreshList = True
...
'滚动条滚动后，调用重新计算位置方法
    ...
    RaiseEvent OnAfterScroll(OldTopRow, OldLeftCol, NewTopRow, NewLeftCol)
    
    mblnIsRefreshList = False
Exit Sub
errHandle:
    mblnIsRefreshList = False
    Debug.Print "vfgData_AfterScroll Exception:" + err.Description
    err.Clear
End Sub
...
Private Sub vfgData_BeforeScroll(ByVal OldTopRow As Long, ByVal OldLeftCol As Long, ByVal NewTopRow As Long, ByVal NewLeftCol As Long, Cancel As Boolean)
On Error GoTo errHandle
    mblnIsRefreshList = True
    
    RaiseEvent OnBeforeScroll(OldTopRow, OldLeftCol, NewTopRow, NewLeftCol, Cancel)
    
    mblnIsRefreshList = False
Exit Sub
errHandle:
    mblnIsRefreshList = False
    Debug.Print "vfgData_BeforeScroll Exception:" + err.Description
    err.Clear
End Sub
...
Public Sub BindData(Optional ByVal blnUseScrollRefresh As Boolean = False)
'直接绑定数据
    Dim rsBind As ADODB.Recordset
    ...
    If strBindFilter <> "" Then
        strOldDsFilter = Replace(strOldDsFilter, " and " & strBindFilter, "")
        strOldDsFilter = Replace(strOldDsFilter, strBindFilter, "")
    ...
    If strCloneFilter <> "" Then
        strOldDsFilter = Replace(strOldDsFilter, " and " & strCloneFilter, "")
        strOldDsFilter = Replace(strOldDsFilter, strCloneFilter, "")
...
'    mrsData.Filter = IIf(strOldDsFilter = "0", "", strOldDsFilter)
    ...
    Call mrsData.Save(adoSourceStream, adPersistADTG)
    
    Set rsFilter = New ADODB.Recordset
    rsFilter.Open adoSourceStream
    
    RaiseEvent OnFilterData(rsFilter, rsClone)
    
    If rsFilter.RecordCount > 0 Then rsFilter.MoveFirst
    While Not rsFilter.EOF
        blnFilterOut = False
        RaiseEvent OnFilterRowData(rsFilter, rsClone, blnFilterOut)
    ...
    rsFilter.UpdateBatch
    
    If adoSourceStream.State = adStateOpen Then Call adoSourceStream.Close
    
    adoSourceStream.type = adTypeText
    Call rsFilter.Save(adoSourceStream, adPersistXML)
...
'    DoEvents
    ...
    '启动行数据刷新
    If blnUseScrollRefresh = False Then TimerRefreshData.Enabled = True
End Sub
...
'更源数据值
    ...
    If IsDate(strNewValue) Then strNewValue = Format(strNewValue, "yyyy-mm-dd hh:mm:ss")
...
On Error Resume Next
    ...
    ElseIf Button = 2 Then
        If mblnIsShowPopupMenu Then
            mnuCut.Enabled = Not IsReadCol(mDataGrid.Col) And mblnCanCut
...
On Error Resume Next
    ...
    If Not mblnUfgColsList Then RaiseEvent OnSelChange
...
Private Function GetImg(ByVal lngImgIndex As Long) As StdPicture
    ...
    If lngImgIndex < 0 Or lngImgIndex >= mobjImageList.ListImages.Count Then Exit Function
    ...
    If mStrDefaultColNames <> "" And mStrDefaultColNames <> strCols Then
        ...
        For i = 0 To UBound(Cols)
            If Cols(i) <> "" Then
                ...
                If InStr("|" & mStrDefaultColNames & "|", "|" & strColName & ",") <= 0 _
                    And InStr("|" & mStrDefaultColNames & "|", "|" & strColName & "|") <= 0 Then
                    '删除不需加载的数据列
        ...
        For i = 0 To UBound(aryDefaultCol)
            If aryDefaultCol(i) <> "" Then
                ...
                If InStr("|" & strCurCols & "|", "|" & strColName & ",") <= 0 And InStr("|" & strCurCols & "|", "|" & strColName & "|") <= 0 Then
...
Public Sub SetMenuState(ByVal blCanCut As Boolean)
    mblnCanCut = blCanCut
End Sub
