...
Begin VB.Form frmPatholSpecimen 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin RichTextLib.RichTextBox txtHistoryRecord 
         ...
         ScrollBars      =   3
         Appearance      =   0
         TextRTF         =   $"frmPatholSpecimen.frx":000C
      End
   End
   Begin VB.Frame Frame1 
      ...
      Begin VB.ListBox lstPartment 
         ...
         EndProperty
         Height          =   2790
         ItemData        =   "frmPatholSpecimen.frx":00A9
         Left            =   7320
         List            =   "frmPatholSpecimen.frx":00AB
         TabIndex        =   7
         Top             =   1080
         Width           =   2415
      End
...
Public Sub LoadSpecimenAcceptOrRejectHistoryData()
    ...
    strSql = "select 送检单位,送检科室,送检人,送检日期,联系方式,登记人,核收状态,拒收原因,通知人,备注 from 病理送检信息 where 医嘱ID=[1] order by 送检日期"
...
'删除选择的标本
    ...
    If Not Val(ufgData.Text(ufgData.SelectionRow, gSpecimen_送检ID)) <= 0 Then
        Call MsgBoxD(Me, "标本已被核收，不能进行删除。", vbOKOnly, Me.Caption)
        Exit Sub
    End If
...
'    End If
    ...
    blnIsSucceed = frmPatholSpecimen_AcceptOrReject.ShowAcceptOrRejectSpecimenWindow(mlngAdviceID, _
                                                                                    mlngCurDeptId, txtHistoryRecord, False, Me, mstrPrivs)
    
    If blnIsSucceed Then
'        '如果核收成功，则自动保存标本数据
'        Call SaveSpecimenData
...
'拒收标本
    Dim blnIsSucceed As Boolean
    blnIsSucceed = frmPatholSpecimen_AcceptOrReject.ShowAcceptOrRejectSpecimenWindow(mlngAdviceID, "", txtHistoryRecord, True, Me, mstrPrivs)
...
Public Sub SaveSpecimenData(Optional ByVal blnIsSaveOnlyDel As Boolean = False)
'保存标本数据
    Dim i As Long
    Dim strSql As String
