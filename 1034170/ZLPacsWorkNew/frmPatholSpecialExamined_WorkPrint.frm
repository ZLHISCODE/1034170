...
'根据过滤条件查询特检数据
    ...
    strSQL = "select * from (select /*+ Rule*/ a.Id,c.检查类型,c.病理号,c.病理医嘱ID, e.姓名, a.材块ID,b.序号, b.标本名称, a.特检类型,a.特检细目, a.抗体id,  d.抗体名称, a.制作类型,a.当前状态,a.清单状态,f.补费状态, " & _
                " (select count(*) from 病人医嘱附费 X,门诊费用记录 Y where X.记录性质=decode(Y.记录性质,1,1,11,1,Y.记录性质) and X.no = Y.no and Y.记录状态=0 and X.医嘱Id=c.医嘱ID) as 附费, f.申请时间, a.完成时间" & _
...
'Begin------------------------编辑菜单--------------------------------------默认可见
    ...
    With cbrMenuBar.CommandBar.Controls
        Set cbrControl = .Add(xtpControlButton, TMenuType.mtAccept, "特检接受(&R)"): cbrControl.IconId = 747
