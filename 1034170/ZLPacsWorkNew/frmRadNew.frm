...
Public Event OnRadNew(ByVal lngProId As Long)
...
On Error GoTo ErrHandle
    ...
    For Each objItem In Me.lvwItem.ListItems
        If objItem.Checked Then
            ...
            Call zlDatabase.ExecuteProcedure(gstrSQL, "保存影像检查项目")
            RaiseEvent OnRadNew(Val(Mid(objItem.Key, 2)))
        End If
    Next
    
    Call gcnOracle.CommitTrans
    
    MsgBoxD Me, "本次设置的影像检查项目保存完毕！", vbExclamation, gstrSysName
...
Private Sub LoadClass()
    ...
    gstrSQL = "Select Distinct ID, 编码, 名称, 上级id" & vbNewLine & _
                "From 诊疗分类目录" & vbNewLine & _
                "Where 类型 = '5'" & vbNewLine & _
                "Start With ID In (Select Distinct 分类id From 诊疗项目目录)" & vbNewLine & _
                "Connect By Prior 上级id = ID" & vbNewLine & _
                "Order By 编码"
    
    err = 0: On Error GoTo errHand
    
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "查询诊疗分类项目")
    With rsTemp
...
Private Sub tvwClass_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    err = 0: On Error GoTo errHand
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "查询诊疗项目目录", CLng(Mid(Node.Key, 2)))
