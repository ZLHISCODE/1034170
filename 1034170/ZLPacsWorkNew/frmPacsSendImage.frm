...
Private Sub CmdSend_Click()
    ...
    If Me.CboPath.Text = "" Then
        MsgBoxD Me, "请选择要发送的目录!", vbInformation, Me.Caption
        Exit Sub
    End If
    '检查目录是否可写
    ...
    If Me.CboPath.list(Me.CboPath.ListIndex) <> "" Then
        ...
        strSQL = "select 设备号,本机目录,FTP用户名,FTP密码 from 影像设备目录 where 类型 = 5 and 设备号 = [1]"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strTmp)
        If rsTmp.EOF = True Then
            MsgBoxD Me, "没有找到要发送目录的信息！", vbInformation, Me.Caption
            Exit Sub
        End If
        zl9comlib.zlCommFun.ShowFlash "正在连接测试请等待.....", Me
        zl9comlib.zlCommFun.ShowFlash
        strRemotePath = rsTmp("本机目录")
        strPas = Nvl(rsTmp("FTP密码"))
        strUse = Nvl(rsTmp("FTP用户名"))
    ...
    If WriteTest(False, strRemotePath) = False Then
        MsgBoxD Me, "写入测试失败请检查共享目录!", vbQuestion, App.EXEName
        Exit Sub
    End If
    
    With Me.tvwImageDate
        If .Nodes.Count = 0 Then
            MsgBoxD Me, "没有可以发送的文件!请选择查询条件后点击刷新更新列表!", vbInformation, App.EXEName
            Exit Sub
        End If
        '清除以前的错误日志
        ...
        For i = 1 To .Nodes.Count
            If .Nodes(i).Checked = True And .Nodes(i).Child Is Nothing Then
                ...
                strCheckDate = Format(Mid(strDate, 1, InStr(1, strDate, "]") - 1), "yyyy-mm-dd")
                ...
                blnMoved = MovedByDate(strCheckDate)
                '发送
                If SendFilesToDir(strSq, DicomPath, strRemotePath & "\DICOM\IMAGE\", strPath, blnMoved) Then
                    '失败
        ...
        If n > 0 Then
            If MsgBoxD(Me, "发送完成!发送成功" & m & "个图像,失败" & n & "个图像." & _
            ...
            End If
        Else
            MsgBoxD Me, "发送完成!发送成功" & m & "个图像,失败" & n & "个图像.", vbInformation, App.EXEName
        End If
        Me.MousePointer = 0
    End With
...
'功能：根据病人来源读取病人科室
    ...
    If cboDept.ListCount > 0 Then
        If cboDept.ListIndex < 0 Then cboDept.ListIndex = 0
    End If
    
    strSQL = "select 设备号, 设备名,本机目录,FTP用户名,FTP密码  from 影像设备目录 where 类型 = 5 and NVL(状态,0)=1"
...
Private Sub Form_Unload(Cancel As Integer)
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "间隔日期", CInt(Me.dtpEnd - Me.dtpBegin)
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "结束检查时间", Format(Me.dtpEnd.value, "yyyy-mm-dd")
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "科室", Me.cboDept.ListIndex
'    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "发送路径", Me.TxtPath
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "门诊病人", Me.chk来源(0).value
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "住院病人", Me.chk来源(1).value
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "发送目录", Me.CboPath.ListIndex
    
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "预览", Me.chkShowImage.value
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "格式0", Me.ChkImageFormat(0).value
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "格式1", Me.ChkImageFormat(1).value
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "格式2", Me.ChkImageFormat(2).value
    SaveSetting "ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "观片站", Me.ChkSendTool.value
...
Private Sub Form_Load()
    ...
    intDiffDay = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "间隔日期", 3)
    Me.dtpBegin = Format(curDate - intDiffDay, "yyyy-mm-dd")
    Me.dtpEnd = Format(curDate, "yyyy-mm-dd")
    intDept = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "科室", 0)
'    Me.TxtPath = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "发送路径", "")
    Me.chk来源(0).value = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "门诊病人", 1)
    Me.chk来源(1).value = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "住院病人", 1)
    Me.chkShowImage.value = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "预览", 1)
    Me.ChkImageFormat(0).value = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "格式0", True)
    Me.ChkImageFormat(1).value = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "格式1", False)
    Me.ChkImageFormat(2).value = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "格式2", False)
    Me.ChkSendTool.value = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "观片站", 1)
    intPath = GetSetting("ZLSOFT", "公共模块\" & App.ProductName & "\frmPacsSendImage", "发送目录", 0)
    LoadData
...
Private Sub RefreshImageDate()
    ...
    strDeptNo = Me.cboDept.ItemData(Me.cboDept.ListIndex)
    
    If strDeptNo = "" Then
        MsgBoxD Me, "请选择病人科室", vbOKOnly, "提示信息"
        Exit Sub
    End If
    
    strSQL = "select /*+ RULE*/ a.医嘱ID,a.影像类别,a.检查号,nvl(g.姓名,a.姓名) 姓名,a.检查UID,a.接收日期,b.病人来源,c.编码||'-'||c.名称 as 部门名称, " & _
             ...
             " a.检查UID = e.检查UID and e.序列UID = f.序列UID and b.病人ID = g.病人ID and b.诊疗类别 = 'D' and " & _
             " d.首次时间 >= [1] and d.首次时间 <= [2] "
             
    strSQL = strSQL & " and c.id = [3] "
    
    If Trim(txtChkNoBegin) <> "" And Trim(txtChkNoEnd) <> "" Then
    ...
    If Len(Trim(Me.txtPatientID)) > 0 Then
        strSQL = strSQL & " and Decode(B.病人来源,1,G.门诊号,2,G.住院号,NULL)=[9] "
    End If
    If blnMoved Then
        strSQLBak = strSQL
    ...
    End If
    Set rsMain = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, CDate(Format(Me.dtpBegin.value, "yyyy-mm-dd")), _
    CDate(Format(Me.dtpEnd.value, "yyyy-mm-dd 23:59:59")), strDeptNo, _
    Val(IIf(Trim(txtChkNoBegin) = "", 1, txtChkNoBegin)), Val(IIf(Trim(txtChkNoEnd) = "", 1, txtChkNoEnd)), _
    CStr(IIf(Trim(txt姓名) = "", 1, txt姓名)), intMZ, intZY, Val(IIf(Trim(txtPatientID) = "", 1, txtPatientID)))
    Me.tvwImageDate.Nodes.Clear
        
    Do Until rsMain.EOF Or blnExit = True
...
CopyError:
    ...
    If err.Number = 75 Then
        MsgBoxD Me, "写入测试失败!请查看[" & strPath & "]是否有写入权限!", vbInformation, App.EXEName
    Else
        MsgBoxD Me, "发生其他错误！", vbQuestion, App.EXEName
    End If
End Function

Private Function SendFilesToDir(lngSeqUID As String, DicomDirPath As DicomDataSet, DestinationBoot As String, DestinationDir As String, Optional blnMoved As Boolean = False) As Boolean
    '功能:从FTP下载文件
    '传入:序列UID
    '返回下载成功后的文件路径
    Dim strSQL As String
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, SerialUID, ImageUID)
...
Private Sub tvwImageDate_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    If Node.Child Is Nothing And Me.chkShowImage.value = 1 Then
        ...
        strDate = Mid(Node.Parent.Parent.Text, InStr(1, Node.Parent.Parent.Text, "[") + 1)
        strDate = Format(Mid(strDate, 1, InStr(1, strDate, "]") - 1), "yyyy-mm-dd")
        blnMoved = MovedByDate(strDate)
                
        ShowImage strImageUID, strSerialUID, blnMoved
    End If
End Sub
...
Private Function IsStrValib() As Boolean
    '检查字串的合法性
    If zlCommFun.StrIsValid(Me.txtChkNoBegin) = False Then
        MsgBoxD Me, "开始检查号中包括了非法字符串！", vbInformation, App.EXEName
        With Me.txtChkNoBegin
            .SetFocus
            .SelStart = 0
            .SelLength = Len(.Text)
    ...
    End If
    If zlCommFun.StrIsValid(Me.txtChkNoEnd) = False Then
        MsgBoxD Me, "结束检查号中包括了非法字符串！", vbInformation, App.EXEName
        With Me.txtChkNoEnd
            .SetFocus
            .SelStart = 0
            .SelLength = Len(.Text)
    ...
    If zlCommFun.StrIsValid(Me.txt姓名, 12) = False Then
        MsgBoxD Me, "姓名中包括了非法字符串！", vbInformation, App.EXEName
        With Me.txt姓名
            .SetFocus
            .SelStart = 0
            .SelLength = Len(.Text)
...
Private Sub txt姓名_GotFocus()
    ...
    End With
End Sub
Private Function ShowImage(lngImageUID As String, lngSerialUID As String, Optional blnMoved As Boolean = False) As Boolean
    '功能:从FTP下载文件
    '传入:序列UID
    '返回下载成功后的文件路径
    Dim strSQL As String
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lngImageUID, lngSerialUID)
    strCachePath = App.Path & "\TmpImage\"
    ClearCacheFolder strCachePath
    If rsTmp.EOF <> True Then
        MkLocalDir strCachePath & objFile.GetParentFolderName(Nvl(rsTmp("URL1")))
