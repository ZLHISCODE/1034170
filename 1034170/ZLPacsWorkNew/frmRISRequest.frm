...
Private mblnIDCardDetect    As Boolean '身份证识别
...
Private Function IDCardValid(objTxtCard As Object, ByVal strCardNo As String, _
  Optional ByVal strName As String, Optional ByVal strSex As String) As Long
    ...
    '没有启用身份证唯一识别则退出处理
    If Trim(Txt身份证号.Text) = "" Then Exit Function
    
    strSQL = "Select 病人ID, 姓名,性别,年龄,出生日期,籍贯 From 病人信息 Where 身份证号=[1]"
    
    If mlngPatiId = 0 Then
        '新病人
        Set rsData = zlDatabase.OpenSQLRecord(strSQL, "身份证查询", strCardNo)
        ...
        If rsData.RecordCount = 1 Then
            '存在一条相同的身份证号
            If MsgBoxD(Me, "已存在身份证号为 " & Txt身份证号.Text & " 的病人，患者信息如下：" & vbCrLf & _
                Nvl(rsData!姓名) & "  " & Nvl(rsData!性别) & "  " & Nvl(rsData!年龄) & vbCrLf & _
                "是否以患者 [" & Nvl(rsData!姓名) & "] 身份继续申请?", vbYesNo, "提示信息") = vbYes Then
 
                strSQL = "Select distinct A.病人id,A.姓名,A.性别,A.年龄,A.就诊时间,A.入院时间,A.登记时间,to_char(A.出生日期,'yyyy-mm-dd') 出生日期,Decode(A.当前科室id,Null,1,2) As 来源ID,A.主页ID," & _
                        ...
                        "nvl(A.家庭地址邮编,A.单位邮编) 邮编,nvl(A.家庭地址,A.工作单位) 地址,A.合同单位ID " & _
                " From 病人信息 A where 病人ID=[1]"
                
                Set rsData = zlDatabase.OpenSQLRecord(strSQL, "查询病人信息", Nvl(rsData!病人ID))
        ...
        Else
            ...
            Set rsData = zlDatabase.OpenSQLRecord(strSQL, "身份证查询", mlngModul, strCardNo, strName, strSex)
            ...
            If rsData.RecordCount > 0 Then
                If Nvl(rsData!IDs) = "" Then
                    ...
                    strSQL = "Select distinct A.病人id,A.姓名,A.性别,A.年龄,A.就诊时间,A.入院时间,A.登记时间,to_char(A.出生日期,'yyyy-mm-dd') 出生日期,Decode(A.当前科室id,Null,1,2) As 来源ID,A.主页ID," & _
                            ...
                            "A.费别,A.医疗付款方式,A.身份证号,A.民族,A.职业,A.婚姻状况,nvl(A.家庭电话,A.联系人电话) 电话," & _
                            "nvl(A.家庭地址邮编,A.单位邮编) 邮编,nvl(A.家庭地址,A.工作单位) 地址,A.合同单位ID " & _
                    " From 病人信息 A where 身份证号=[1] Order By Nvl(就诊时间,nvl(入院时间,登记时间)) Desc"
                ...
                Else
                    ...
                    strSQL = "Select distinct A.病人id,A.姓名,A.性别,A.年龄,A.就诊时间,A.入院时间,A.登记时间,to_char(A.出生日期,'yyyy-mm-dd') 出生日期,Decode(A.当前科室id,Null,1,2) As 来源ID,A.主页ID," & _
                            ...
                            "A.费别,A.医疗付款方式,A.身份证号,A.民族,A.职业,A.婚姻状况,nvl(A.家庭电话,A.联系人电话) 电话," & _
                            "nvl(A.家庭地址邮编,A.单位邮编) 邮编,nvl(A.家庭地址,A.工作单位) 地址,A.合同单位ID " & _
                    " From 病人信息 A,Table(f_num2list([1])) B  where A.病人ID=B.Column_Value Order By Nvl(就诊时间,nvl(入院时间,登记时间)) Desc"
                                    
                End If
                
                strSQL = "select RowNum as ID,病人id,姓名,性别,年龄,就诊时间,入院时间,登记时间,出生日期,来源ID,主页ID,病人科室ID,当前科室ID,医生,门诊号," & _
                            "住院号,就诊卡号, 就诊状态,卡验证码,当前床号,费别,医疗付款方式,身份证号,民族,职业,婚姻状况,电话,邮编,地址,合同单位ID " & _
                            " From (" & strSQL & ")"
                    
                '弹出选择器
                Set rsData = zlDatabase.ShowSQLSelect(Me, strSQL, 0, "病人选择", False, "病人ID", _
                                                "已存在与身份证号 " & strCardNo & " 相同的病人信息，请选择对应病人信息进行申请。", False, False, False, _
        ...
        End If
    Else
        '如果不是新病人，则根据患者的身份证查找患者ID,
        If objTxtCard.Text <> objTxtCard.tag Then
            '修改身份证
            '判断身份证在数据库中是否存在，如果已经存在，则不允许修改
            Set rsData = zlDatabase.OpenSQLRecord(strSQL, "身份证查询", strCardNo)
            If rsData.RecordCount > 0 Then
                MsgBoxD Me, "已存在身份证号为 " & strCardNo & " 的病人，病人信息如下：" & vbCrLf & _
                    Nvl(rsData!姓名) & "  " & Nvl(rsData!性别) & "  " & Nvl(rsData!年龄) & vbCrLf & _
                    "操作将不能继续。", vbOKOnly, "提示信息"
                IDCardValid = 1 '取消登记
            End If
        End If
    End If
...
Private Sub CmdOK_Click()
    ...
    If mblnIDCardDetect And Trim(Txt身份证号.Text) <> "" Then
        '身份证唯一验证
        If IDCardValid(Txt身份证号, Txt身份证号.Text, PatiIdentify.Text, zlStr.NeedName(cbo性别.Text)) = 1 Then Exit Sub
    End If
    ...
    If mintEditMode = 2 Or (mblnRegToCheck And mintEditMode = 0) Then
            ...
            If Not ItemHaveCash(mintSourceType, False, mlngAdviceID, 0, lng发送号, str诊疗类别, str单据号, int记录性质, _
                ...
                If gbln执行前先结算 Then
                    ...
                    If mobjSquareCard.zlSquareAffirm(Me, mlngModul, mstrPrivs, mlngPatiId, 0, False, , , str医嘱IDs) = False Then
                        '如果是急诊病人 并且启用了  mblEmergencyRequestNotExecuteMoney 急诊病人报到时不执行费用，不终止本过程
                        If Not (chk紧急.value = 1 And mblnEmergencyRequestNotExecuteMoney) Then
        ...
        If ((mlngModul = G_LNG_VIDEOSTATION_MODULE And mlngVideoStationMoneyExeModle = 0) Or _
           (mlngModul = G_LNG_PACSSTATION_MODULE And mlngPacsStationMoneyExeModle = 0)) And Not (mblnEmergencyRequestNotExecuteMoney And chk紧急 = 1) Then
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
    ...
    If (mintEditMode = 2 Or txt检查号.tag <> txt检查号.Text) And Not blnOk Then
        If CheckNoValidate = False Then
            Exit Sub
        End If
    End If
...
'lngRegType  '1-新登记、2-提取登记、3-复制登记
    ...
    If mblnIDCardDetect Then
        '启用身份设备后，只有身份证为空的外来患者允许被修改
        blnIsModifyInfo = IIf(mintSourceType = 3 And Trim(Txt身份证号.Text) = "", True, False)
    End If
    ...
    cbo性别.Enabled = blnIsModifyInfo: cboAge.Enabled = blnIsModifyInfo
    Call sutSetTxtEnable(txt年龄, blnIsModifyInfo)
    dtp出生日期.Enabled = blnIsModifyInfo
    Call sutSetTxtEnable(Txt身份证号, blnIsModifyInfo)
...
Private Sub InitParameter()
    ...
    End If
    mblnEmergencyRequestNotExecuteMoney = GetDeptPara(mlngCurDeptId, "急诊病人报到时不执行费用", 0) = "1" '       '急诊病人报到时不执行费用
...
'返回： True--复制成功；False--复制信息不完整
    ...
    Txt身份证号.Text = Nvl(rsTemp!身份证号)
    Txt身份证号.tag = Nvl(rsTemp!身份证号)
...
Private Sub PatiIdentify_FindPatiArfter(ByVal objCard As zlIDKind.Card, ByVal blnCard As Boolean, ShowName As String, objHisPati As zlIDKind.PatiInfor, objCardData As zlIDKind.PatiInfor, strErrMsg As String, blnCancel As Boolean)
    ...
    If Not objHisPati Is Nothing Then
        If objHisPati.病人ID > 0 Then
            Call FindPatient(blnCard, Nothing, objHisPati.病人ID)
        End If
    End If
...
Private Sub PatiIdentify_FindPatiBefore(ByVal objCard As zlIDKind.Card, blnCard As Boolean, strShowText As String, objCardData As zlIDKind.PatiInfor, blnFindPatied As Boolean, blnCancel As Boolean)
    Dim blnFinded As Boolean 'pacs弹出的查找窗体找到了病人
...
Private Sub Txt身份证号_Change()
    ...
    If mintEditMode = 0 And mlngPatiId <> 0 And Txt身份证号.Text <> "" Then
        MsgBoxD Me, "病人修改身份证后，就作为新病人处理了。", vbOKOnly, "提示信息"
        Call InitEdit(False)
...
'lngPatiID 如果大于0 且 rsSelPatient=nothing  说明强制使用病人ID查询病人并且显示相关信息到界面控件中
On Error GoTo err
    Dim rsTmp As ADODB.Recordset
    Dim lngAge As Long
    Dim curDate As Date
    ...
    ElseIf rsSelPatient Is Nothing And lngPatiID > 0 Then
        '单独根据病人ID查询.
        strSQL = "select 病人id,姓名,性别,年龄,出生日期,来源ID,主页ID,病人科室ID,当前科室ID,医生,门诊号,住院号,就诊卡号,就诊状态,卡验证码,当前床号,费别" & _
                        ",医疗付款方式,身份证号,民族,职业,婚姻状况,电话,邮编,地址,合同单位ID, 新病人" & _
                    " From(Select distinct A.病人id,A.姓名,A.性别,A.年龄,to_char(A.出生日期,'yyyy-mm-dd') 出生日期,Decode(A.当前科室id,Null,1,2) As 来源ID,A.主页ID," & _
                        ...
                        "A.费别,A.医疗付款方式,A.身份证号,A.民族,A.职业,A.婚姻状况,nvl(A.家庭电话,A.联系人电话) 电话," & _
                        "nvl(A.家庭地址邮编,A.单位邮编) 邮编,nvl(A.家庭地址,A.工作单位) 地址,A.合同单位ID, 0 as 新病人,B.登记时间" & _
                  " From 病人信息 A,病人挂号记录 B Where A.病人ID=[1] And A.病人ID=B.病人ID(+) And A.门诊号=B.门诊号(+) " & _
                  " order by B.登记时间 desc) where rownum=1"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "查找病人", lngPatiID)
    Else
...
Private Sub PatiIdentify_Change()
    ...
    If mintEditMode = 0 And mlngPatiId <> 0 And PatiIdentify.Text <> "" Then
        ...
        If mblnIDCardDetect Then Txt身份证号.Text = ""
