...
On Error GoTo errHandle
    Dim lngID As Long
    Dim intDelete As Integer '是否删除报告  1删除   0不删除

    Menu_Manage_无报告完成 = False

        intDelete = 1
    If (mListAdviceInf.strReportDoctor <> "" Or mListAdviceInf.strReportOperation <> "") And Not mSysPar.blnNoSignFinish Then
        If MsgBoxD(Me, "是否无报告直接完成,直接完成将删除已填写的报告!", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Function
    End If
    
        If mSysPar.blnNoSignFinish Then intDelete = 0
    If mSysPar.blnFinishCommit And CheckPopedom(mstrPrivs, "检查完成") Then '无报告完成后无需再次确认完成,但需要有检查完成的权限
        ...
        If mrtReportType = 报告文档编辑器 Then
            ...
            & "6," & intDelete & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & lngID & "," & To_Date(zlDatabase.Currentdate) & ")"
        Else
    ...
    Else
        If mrtReportType = 报告文档编辑器 Then
            gstrSQL = "ZL_影像检查_状态更新(" & mListAdviceInf.lngAdviceID & "," & mListAdviceInf.lngSendNO & ",'',5," & intDelete & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & mlngCur科室ID & ")"
        Else
            gstrSQL = "ZL_影像检查_STATE(" & mListAdviceInf.lngAdviceID & "," & mListAdviceInf.lngSendNO & ",5," & intDelete & ",'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & mlngCur科室ID & ")"
        End If
    End If
...
On Error GoTo errHandle
    ...
    strSQL = "select 医嘱ID from 病人医嘱发送 where 医嘱ID=[1] and 执行过程<4"
    Set rsData = zlDatabase.OpenSQLRecord(strSQL, "查询医嘱报告", curAdviceInf.lngAdviceID)
    If rsData.RecordCount > 0 Then
        If Menu_Manage_无报告完成 = False Then Exit Sub
    End If
        ...
        Case conMenu_Manage_PacsCriticalReg     '危急患者登记
            ...
            Else                                            '外来、体检
                Call mobjPublicAdvice.ShowAppCritical(Me, True, 0, 3, _
                            mcurAdviceInf.lngPatId, 0, "", mcurAdviceInf.lngBaby, lngCriticalId, _
                            mcurAdviceInf.lngAdviceID, , , , mlngCur科室ID, UserInfo.姓名, mobjMsgCenter.Msg)
            End If
...
On Error GoTo errHand
    ...
    If rsTemp.EOF Then Exit Sub
    If Not mSysPar.blnPatTrack Then
    '原来的方式
    ...
    Else
    '新的方式
        strBillNo = "ZLCISBILL" & Format(rsTemp!编号, "00000") & "-" & rsTemp!通用
        '弹出格式选择窗体
        ...
        Else
            ...
            For i = 0 To UBound(strArry)
                ReportOpen gcnOracle, glngSys, strBillNo, Me, "NO=" & strExseNo, "性质=" & intExseKind, "医嘱ID=" & lngAdviceID, "ReportFormat=" & Val(strArry(i)), 2
            Next
        End If
    End If
        ...
        Else
            ...
            If mSysPar.blnUseQueue = True Then
                If mobjQueue Is Nothing Then
                    ...
                    Call mobjQueue.zlInitPacsQueueCfg(mlngModule, mlngCur科室ID, NeedName(mstrCur科室), mstrPrivs, mblnAllDepts)
...
'        Set cbrControl = CreateModuleMenu(.Controls, xtpControlButton, conMenu_Manage_ModifBaseInfo, "基本信息调整", "", 4113, False)
        ...
        '绿色通道
        Set cbrControl = CreateModuleMenu(.Controls, xtpControlPopup, conMenu_Manage_GChannel, "绿色通道", "", 0, False, , False)
            If Not cbrControl Is Nothing Then
                Set cbrPopControl = CreateModuleMenu(cbrControl.CommandBar.Controls, xtpControlButton, conMenu_Manage_GChannelOk, "标记", "", 0, False, , False)
                Set cbrPopControl = CreateModuleMenu(cbrControl.CommandBar.Controls, xtpControlButton, conMenu_Manage_GChannelCancel, "取消", "", 0, False, , False)
            End If
...
Private Function CreateModuleMenu(objMenuControl As CommandBarControls, _
    ...
    Optional strToolTip As String = "", Optional lngIconId As Long = 0, Optional blnStartGroup As Boolean = False, Optional ByVal lngIndex As Long = -1, _
    Optional blnIsControlCreate As Boolean = True) As CommandBarControl
'创建该模块内的菜单
On Error GoTo err
    ...
    Select Case lngID
        ...
        Case conMenu_Manage_Logout '取消报到
            If Not (CheckPopedom(mstrPrivs, "取消报到") And CheckPopedom(mstrPrivs, "检查登记")) Then blHavePrives = False
        ...
        Case conMenu_Manage_ConfigQuery
...
On Error GoTo errHandle
    ...
    If frmSwitchUser.blnOk Then
        ...
        If gstrDBUser <> mstrUserIDNew Then
            ...
            If UCase(mstrUserIDNew) = "ZLHIS" Then
            '切换到ZLHIS用户 需要另外的获取权限的方式
                ...
                mstrPrivs = ";" & GetPrivFunc(100, mlngModule) & ";"
