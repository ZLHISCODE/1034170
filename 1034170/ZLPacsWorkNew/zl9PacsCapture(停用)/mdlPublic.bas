...
Public gtFileLoadType As FileLoadType
...
'-----------------------------------------------
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "判断存储设备是否存在", objFtp.strDeviceId)
...
Public Function GetSingleImage(lngImageUID As String, lngSerialUID As String, ObjFrm As Object, Optional blnMoved As Boolean = False) As String
    '功能:从FTP下载文件
    '传入:序列UID
    '返回下载成功后的文件路径
    Dim strSQL As String
...
'初始化科室级参数
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "获取存储设备信息", strSaveDeviceID)
