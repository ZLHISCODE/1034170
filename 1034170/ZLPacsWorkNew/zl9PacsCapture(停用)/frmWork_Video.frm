...
'取得后台采集标记
    ...
    strSQL = "Select nvl(max(检查号),0)+1 as 检查号 from 影像临时记录 where 姓名 = '" & mstrAfterStationName & "-后台' and 接收日期>=trunc(sysdate)"
    Set rsData = zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
    
    If rsData.RecordCount <= 0 Then Exit Function
    
    GetCaptureTag = Lpad(rsData!检查号, 3, "0")
End Function
...
BUGEX "gtFileLoadType = " & gtFileLoadType
    If gtFileLoadType = Service Then
