...
Public Function ShowMe(ByRef fParent As Object, ByVal lngPassType As Long, ByVal lngReportID As Long, ByVal lngPatientID As Long, ByVal lngPageID As Long, _
    ...
    '根据权限，确定是否显示“审核签名”按钮
    cmdOK(2).Visible = CheckPopedom(mstrPrivs, "PACS报告修订")
    ...
    If chkEsign.value = vbChecked Then
        ...
        If gobjESign Is Nothing Then
            ...
            If Not gobjESign Is Nothing Then
                If gobjESign.Initialize(gcnOracle, glngSys) = False Then
        ...
        '先检查数字证书跟登陆用户是否一致
        If gobjESign.CheckCertificate(strDBUser) = False Then
            Exit Function
        End If
        ...
        If gobjESign.CertificateStoped(UserInfo.姓名) Then
            'Validation = True
            If MsgBoxD(Me, "当前证书已停用，不能继续数字签名，是否改用密码签名方式？", vbYesNo, "书写签名") = vbNo Then
                Exit Function
            End If
        Else
            '获取签名的源文
            ...
            strSign = gobjESign.Signature(strSource, strDBUser, lngCertID, str时间戳, objPic, str时间戳Base64)
            If strSign = "" Then
                MsgBoxD Me, "验证失败！请重新输入验证信息！", vbInformation + vbOKOnly, "书写签名"
                GoTo LL
            End If
