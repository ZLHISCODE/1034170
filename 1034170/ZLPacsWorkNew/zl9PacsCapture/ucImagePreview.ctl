    ...
    Select Case slQueryLevel
        Case slStudy
            ...
            If IsNumeric(strSearchValue) Then
                strSQL = "select count(1)  as 返回值 from 影像检查图象 a, 影像检查序列 b, 影像检查记录 c where a.序列UID=b.序列UID and b.检查UID=c.检查UID and (c.检查UID=[1] or c.医嘱ID=[1])"
            Else
...
'intSearchType:0-按检查uid搜索,1-按序列UID搜索,2-按图像UID搜索
    ...
    Select Case slQueryLevel
        Case slStudy
            If IsNumeric(strSearchValue) Then
                strSQL = "select * from ( " & strSQL & "and c.检查UID=[1] " & vbCrLf & _
                                          " union all " & vbCrLf & _
                                          strSQL & " and c.医嘱ID=[1] ) "
            Else
