...
Begin VB.Form frmWork_Video 
   ...
   Begin ScanLibCtl.ImgScan ImageScanner 
      ...
      ScanTo          =   4
   End
   Begin zl9PacsCapture.ucSplitter ucSplitter1 
      Height          =   135
      Left            =   0
      TabIndex        =   1
      Top             =   4620
   ...
   Begin VB.Timer timerRePaint 
      ...
      Top             =   6780
   End
   Begin zl9PacsCapture.ucImagePreview ucPreview 
      Height          =   4125
      Left            =   0
      TabIndex        =   12
      Top             =   4755
...
'初始化参数设置
    ...
    '鼠标移动时，显示大图
    If gobjCapturePar.IsShowBigImage Then
        ucPreview.BigImageWay = 1
        ucPreview.MouseMoveZoom = gobjCapturePar.ImageZoom
    Else
        ucPreview.BigImageWay = 0
...
'配置ftp存储设备
    ...
    Set rsTmp = zlCL_GetDBObj.OpenSQLRecord(strSQL, "查询存储设备", strCurStorageNo)
    ...
    If Val(strBakStorageNo) > 0 Then
        strSQL = "Select 设备号,设备名 From 影像设备目录 Where 类型=1 and 设备号=[1] and NVL(状态,0)=1"
        Set rsTmp = zlCL_GetDBObj.OpenSQLRecord(strSQL, "查询备份设备", strBakStorageNo)
...
'初始化模块参数
    gblnUseActivexLoad = CBool(GetSetting("ZLSOFT", "公共模块", "UseActiveLoad", "True"))
    SaveSetting "ZLSOFT", "公共模块", "UseActiveLoad", gblnUseActivexLoad
    
    If gblnUseActivexLoad Then ReDim gobjGetImage(0) As Object
...
'更新医嘱信息
    ...
    Set rsTemp = zlCL_GetDBObj.OpenSQLRecord(strSQL, "提取病人基本信息", lngAdviceId)
...
'根据当前状态确定菜单的可视和可操作
    ...
    Select Case control.ID
        ...
        Case conMenu_Cap_MarkMap       '影像采集
            control.Enabled = Not mblnReadOnly And (mVideoCapture.IsStartup Or IsTwainCaptureWay)
        ...
        Case conMenu_Cap_Record        '录像
            control.Enabled = Not mblnReadOnly And gobjCapturePar.VideoDirverType = vdtWDM And mVideoCapture.IsStartup
            control.Visible = Not IsTwainCaptureWay
            
        Case conMenu_Cap_After_Record   '后台录像
            control.Enabled = gobjCapturePar.VideoDirverType = vdtWDM And mVideoCapture.IsStartup
            control.Visible = Not IsTwainCaptureWay And gobjCapturePar.IsUseAfterCapture And False
...
'判断是否为正常的采集方式
    ...
    Set rsData = zlCL_GetDBObj.OpenSQLRecord(strSQL, "查询图像数量", mlngAdviceId)
...
Public Sub subCaptureImg(ByVal RealTimeCap As Boolean, _
                        ...
                        Optional ByVal blnIsAfterCapture As Boolean = False)
'------------------------------------------------
'功能：采集并存储图像
'参数：无
'返回：无，直接保存新采集的图像
        ...
        If blnResult = True Then    '删除成功，则修改缩略图状态，并触发StateChanged事件
            ...
            If ucPreview.CurImageCount = 0 Then
                ...
                If mlngStudyState = 3 Then
                    ...
                    zlCL_ExecuteProcedure strSQL, "删除最后一个图像"
                End If
...
On Error GoTo errHandle
    ...
    If dcmView.Images(1).Tag.Tag = VIDEOTAG Then
        ...
        '下载需要播放的视频
        Call GetSingleImage(dcmView.Images(1).InstanceUID, dcmView.Images(1).SeriesUID, Me, mblnMoved)
...
On Error GoTo errHandle:
    ...
    Set rsTemp = zlCL_GetDBObj.OpenSQLRecord(strSQL, "已启用数量")
...
continue2:
        ...
        dcmTag.CaptureTime = zlCL_Currentdate
...
'保存后台采集图像
    ...
    For i = 1 To dcmAfter.Images.Count
        ...
        If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
            ...
            '存储为报告图像
            ImgTmp.FileExport mstrBufferDir & strReceivedTime & "/" & mAfterCaptureInf.strAfterStudyUid & "/" & ImgTmp.InstanceUID & ".jpg", "JPG", 80
        End If
        ...
        If gblnUseActivexLoad Then
            ...
            If gobjGetImage(UBound(gobjGetImage)) Is Nothing Then Exit Function
            
            Set objImgInfo = gobjGetImage(UBound(gobjGetImage)).ImgInfo
            With objImgInfo
                .AfterModality = mAfterCaptureInf.strAfterModality
                .StudyUID = mAfterCaptureInf.strAfterStudyUid
                .SeriesUID = mAfterCaptureInf.strAfterSeriesUid
                .InstanceUID = ImgTmp.InstanceUID
                .Tag = mAfterCaptureInf.strAfterTag
                .DeviceId = mobjFtp.strDeviceId
                .nowTime = zlCL_Currentdate
                ...
                .SubDir = strReceivedTime & "/" & mAfterCaptureInf.strAfterStudyUid & "/" & ImgTmp.InstanceUID
                .DestMainDir = mstrBufferDir
            End With
                
            If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
                Call gobjGetImage(UBound(gobjGetImage)).RegEventObj(Me)
                Call gobjGetImage(UBound(gobjGetImage)).zlInitModule(True, UBound(gobjGetImage))
        ...
        Else
            ...
            Call zlCL_ExecuteProcedure(strSQL, Me.Caption)
...
'------------------------------------------------
    ...
    If gblnUseActivexLoad Then
        If Not mdcmTmpImg Is Nothing Then
            Set ImgTmp = mdcmTmpImg
            Set mdcmTmpImg = Nothing
        Else
            Exit Sub
        End If
    Else
...
BUGEX "subSaveImage 1"
    '先保存FTP图像
    '读取接收日期
    strSQL = "select 检查UID ,接收日期,报告图象,发送号 from 影像检查记录 where 医嘱ID = [1] "
    Set rsTmp = zlCL_GetDBObj.OpenSQLRecord(strSQL, App.ProductName, lngAdviceId)
    nowTime = zlCL_Currentdate
    
BUGEX "subSaveImage 2"
    ...
    Else
        ...
        blnFirstImage = False
    End If
BUGEX "subSaveImage 3"
    '创建缓冲目录
    MkLocalDir mstrBufferDir & dtReceived & "/" & strStudyUid & "/"
    lngSendNo = rsTmp!发送号
    
BUGEX "subSaveImage 4"

    Set imgTag = ImgTmp.Tag
    If imgTag Is Nothing Then
BUGEX "imgTag Is Nothing"
    Else
BUGEX "imgTag Is Not Nothing"
    End If
    
    If imgTag.Tag <> VIDEOTAG And imgTag.Tag <> AUDIOTAG Then
BUGEX "subSaveImage 4.1"
        strReportImages = Nvl(rsTmp("报告图象"))
    
BUGEX "subSaveImage 5"
...
BUGEX "subSaveImage 6"
        ...
        End Select
BUGEX "subSaveImage 7"
        ImgTmp.FileExport mstrBufferDir & dtReceived & "/" & strStudyUid & "/" & ImgTmp.InstanceUID & ".jpg", "JPG", 80
    End If
BUGEX "subSaveImage 8"
BUGEX "gblnUseActivexLoad=" & gblnUseActivexLoad
    If gblnUseActivexLoad Then
    ...
    BUGEX "subSaveImage 9"
        ...
        ReDim Preserve gobjGetImage(UBound(gobjGetImage) + 1) As Object
        Set gobjGetImage(UBound(gobjGetImage)) = DynamicCreate("zlGetImageEx.clsImageTransfer", "zlGetImageEx.exe")
        If gobjGetImage(UBound(gobjGetImage)) Is Nothing Then Exit Sub
        
        Set objImgInfo = gobjGetImage(UBound(gobjGetImage)).ImgInfo
        With objImgInfo
            .AdviceId = lngAdviceId
            .StudyUID = strStudyUid
            .nowTime = zlCL_Currentdate
            .SeriesUID = ImgTmp.SeriesUID
            .InstanceUID = ImgTmp.InstanceUID
            .MediaType = ImgTmp.Tag.Tag
            .SeriesDescription = ImgTmp.SeriesDescription
            .EncoderName = mstrEncoderName
            .RecordTimeLen = ImgTmp.Tag.RecordTimeLen
            .IsFirstImage = blnFirstImage
            .SendNo = lngSendNo
            .dtReceived = dtReceived
            .IsAfterCapture = False
            .IsUpLoad = True
            Set .dcmTmpImg = ImgTmp
        End With
        ...
        With objImgInfo
            ...
            .SubDir = dtReceived & "/" & strStudyUid & "/" & ImgTmp.InstanceUID
            .DestMainDir = mstrBufferDir
        End With
        
        '备份
        If Val(gobjCapturePar.BakStorageDeviceNo) > 0 Then
            With objImgInfo
    ...
    BUGEX "subSaveImage 9"
        If Val(mobjBakFtp.strDeviceId) > 0 Then
            ...
            End If
        End If
    BUGEX "subSaveImage 10"
    
        If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
    BUGEX "subSaveImage 11"
            '保存dicom图像
    ...
    BUGEX "subSaveImage 15"
        mobjFtpConnection.FuncFtpDisConnect
        
        If mobjBakFtpConnection.hConnection <> 0 Then mobjBakFtpConnection.FuncFtpDisConnect
        
    BUGEX "subSaveImage 16"
        '图像存储成功后，存储数据库信息
    ...
    #If DebugVer = True Then
        BUGEX "subSaveImage 17"
    #End If
        
        strSQL = "Select 序列UID From 影像检查序列  Where 序列UID=[1]"
        Set rsTmp = zlCL_GetDBObj.OpenSQLRecord(strSQL, "PACS图像保存", CStr(ImgTmp.SeriesUID))
    BUGEX "subSaveImage 18"
    ...
    BUGEX "subSaveImage 21"
            ...
            End If
        End If
    BUGEX "subSaveImage 22"
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = strSQL
        
        If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
            '如果不是检查图像，则不保存报告图
            strSQL = "ZL_影像检查报告_ADD('" & strStudyUid & "','" & ImgTmp.InstanceUID & ".jpg')"
            ReDim Preserve arrSQL(UBound(arrSQL) + 1)
            arrSQL(UBound(arrSQL)) = strSQL
        End If
    BUGEX "subSaveImage 23"
        gcnVideoOracle.BeginTrans        '----------保存图像
        blnInTrans = True
        For i = 0 To UBound(arrSQL)
            Call zlCL_ExecuteProcedure(CStr(arrSQL(i)), "保存图像")
        Next i
        
    BUGEX "subSaveImage 24"
        gcnVideoOracle.CommitTrans
        blnInTrans = False
    BUGEX "subSaveImage 25"
        If ImgTmp.Tag.Tag <> VIDEOTAG And ImgTmp.Tag.Tag <> AUDIOTAG Then
            Call frmCaptureHint.ShowCaptureHint( _
                IIf(gobjCapturePar.IsWindowHint, mstrBufferDir & dtReceived & "/" & strStudyUid & "/" & ImgTmp.InstanceUID, ""), _
                gobjCapturePar.IsSountHint, hpRB, Me)
        End If
    BUGEX "subSaveImage 26"
...
Public Sub OnComplete(ByVal objMsg As Object)
'图像采集后的数据处理
'blnAfterCapture，true：后台采集；
    Dim rsTmp As ADODB.Recordset
    Dim arrSQL() As Variant
    Dim blnInTrans As Boolean       '在事物处理过程中
    Dim i As Integer
...
On Error GoTo errorHandle
    If Not objMsg.IsUpLoad Then     '下载图像
        Call ucPreview.OnComplete(objMsg)
        
        Exit Sub
    End If
    
    If objMsg.IsAfterCapture Then
        '图像存储成功后，存储数据库信息
        strSQL = "ZL_影像检查_后台采集('" & objMsg.AfterModality & "','" & objMsg.StudyUID & "','" & objMsg.SeriesUID & "','" & _
                                        objMsg.InstanceUID & "','" & objMsg.Tag & "','" & objMsg.DeviceId & "'," & _
                                        "to_Date('" & Format(objMsg.nowTime, "yyyy-mm-dd hh:mm") & "','YYYY-MM-DD HH24:MI:SS'),'" & objMsg.AfterStationName & "')"
        ...
        Call frmCaptureHint.ShowCaptureHint( _
            IIf(gobjCapturePar.IsWindowHint, mstrBufferDir & Format(objMsg.nowTime, "yyyyMMdd") & "/" & objMsg.StudyUID & "/" & objMsg.InstanceUID, ""), _
            gobjCapturePar.IsSountHint, hpRB, Me)
        Exit Sub
    End If
...
BUGEX "subSaveImage 16"
    ...
    If objMsg.IsFirstImage Then
        strSQL = "ZL_影像检查记录_SET(" & objMsg.AdviceId & "," & objMsg.SendNo & ",'" & _
            objMsg.StudyUID & "',null," & _
            "to_Date('" & Format(objMsg.nowTime, "yyyy-mm-dd hh:mm") & "','YYYY-MM-DD HH24:MI:SS'),'" & mobjFtp.strDeviceId & "')"
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = strSQL
    End If
...
#End If
    ...
    strSQL = "Select 序列UID From 影像检查序列  Where 序列UID=[1]"
    Set rsTmp = zlCL_GetDBObj.OpenSQLRecord(strSQL, "PACS图像保存", CStr(objMsg.SeriesUID))
BUGEX "subSaveImage 18"
    '插入新的检查序列,如果为录像，则插入新的序列
    If rsTmp.EOF Or objMsg.MediaType = VIDEOTAG Or objMsg.MediaType = AUDIOTAG Then
        If objMsg.MediaType = VIDEOTAG Then
            strSQL = "ZL_影像序列_INSERT('" & objMsg.StudyUID & "','" & objMsg.SeriesUID & "','视频录像',0)"
        ElseIf objMsg.MediaType = AUDIOTAG Then
            strSQL = "ZL_影像序列_INSERT('" & objMsg.StudyUID & "','" & objMsg.SeriesUID & "','音频数据',0)"
        Else
            strSQL = "ZL_影像序列_INSERT('" & objMsg.StudyUID & "','" & objMsg.SeriesUID & "','" & objMsg.SeriesDescription & "',0)"
        End If
...
BUGEX "subSaveImage 19"
    ...
    If objMsg.MediaType <> VIDEOTAG And objMsg.MediaType <> AUDIOTAG Then
BUGEX "subSaveImage 20"
        '插入新的图像记录
        strSQL = "ZL_影像图象_INSERT('" & objMsg.InstanceUID & "','" & objMsg.SeriesUID & "',NULL,0, null, sysdate)"
    Else
BUGEX "subSaveImage 21"
        If objMsg.MediaType = VIDEOTAG Then
            '插入新的视频记录
            strSQL = "ZL_影像图象_INSERT('" & objMsg.InstanceUID & "','" & objMsg.SeriesUID & "',Null,0" & _
            ",null,sysdate,null,null,null,null,null,null,null,null,null," & VIDEOTAG & ",'" & objMsg.EncoderName & "'," & objMsg.RecordTimeLen & ")"
        Else
            '插入新的音频记录
            strSQL = "ZL_影像图象_INSERT('" & objMsg.InstanceUID & "','" & objMsg.SeriesUID & "',Null,0" & _
            ",null,sysdate,null,null,null,null,null,null,null,null,null," & AUDIOTAG & ",''," & objMsg.RecordTimeLen & ")"
        End If
    End If
BUGEX "subSaveImage 22"
    ReDim Preserve arrSQL(UBound(arrSQL) + 1)
    ...
    If objMsg.MediaType <> VIDEOTAG And objMsg.MediaType <> AUDIOTAG Then
        '如果不是检查图像，则不保存报告图
        strSQL = "ZL_影像检查报告_ADD('" & objMsg.StudyUID & "','" & objMsg.InstanceUID & ".jpg')"
        ReDim Preserve arrSQL(UBound(arrSQL) + 1)
        arrSQL(UBound(arrSQL)) = strSQL
    End If
BUGEX "subSaveImage 23"
...
BUGEX "subSaveImage 25"
    If objMsg.MediaType <> VIDEOTAG And objMsg.MediaType <> AUDIOTAG Then
        Call frmCaptureHint.ShowCaptureHint( _
            IIf(gobjCapturePar.IsWindowHint, mstrBufferDir & objMsg.dtReceived & "/" & objMsg.StudyUID & "/" & objMsg.InstanceUID, ""), _
            gobjCapturePar.IsSountHint, hpRB, Me)
    End If
BUGEX "subSaveImage 26"
    '图像保存成功，将其加到略缩图中
    If objMsg.AdviceId = mlngAdviceId Then Call ucPreview.AddImage(objMsg.dcmTmpImg, objMsg.dcmTmpImg.Tag)
BUGEX "objMsg.AdviceId = " & objMsg.AdviceId & "    mlngAdviceId = " & mlngAdviceId
BUGEX "mintCaptureFlag=" & mintCaptureFlag
    If mintCaptureFlag = 1 Or mintCaptureFlag = 4 Or mintCaptureFlag = 5 Then
        If ucPreview.CurImageCount = 1 Then
            Call DoStateChange(vetCaptureFirstImg, objMsg.AdviceId, objMsg.SendNo, objMsg.StudyUID)
        End If
    ElseIf mintCaptureFlag = 2 Then
        '设置影像检查状态，如果采集第一张图，且原来的状态是已报到，则修改成已检查
BUGEX "ucPreview.ImgViewer.Images.Count = " & ucPreview.ImgViewer.Images.Count
        If ucPreview.ImgViewer.Images.Count <= 1 Then
            If mlngStudyState < 3 Then
BUGEX "mlngStudyState = " & mlngStudyState
                strSQL = "Zl_影像检查_State(" & objMsg.AdviceId & "," & objMsg.SendNo & ",3,NULL,'" & UserInfo.编号 & "','" & UserInfo.姓名 & "'," & glngDepartId & ")"
                zlCL_ExecuteProcedure strSQL, "采集第一个图像"
            End If
        End If
        ...
        Else
            '更新图像
            Call DoStateChange(vetUpdateImg, objMsg.AdviceId, objMsg.SendNo, objMsg.StudyUID)
        End If
    ElseIf mintCaptureFlag = 3 Then
        '设置影像检查状态，如果采集第一张图，且原来的状态是已报到，则修改成已检查
        If ucPreview.CurImageCount <= 1 Then
...
'lngThreadId，当前下载图像的进程标记
    Dim i As Long
    Dim blnAllLoadFinish As Boolean '是否所以图像都上传完成
    
On Error GoTo err
    '图像上传成功后，释放其对象
    If blnLoadFinish Then Set gobjGetImage(lngThreadId) = Nothing
    
    '判断是否还有进程在进行上传
    blnAllLoadFinish = True

    For i = 0 To UBound(gobjGetImage)
        If Not gobjGetImage(i) Is Nothing Then
BUGEX "OnState lngThreadId=" & lngThreadId
            blnAllLoadFinish = False
            Exit For
        End If
    Next
BUGEX "OnState blnAllLoadFinish = " & blnAllLoadFinish
    '通知工作站，上传或下载情况
    RaiseEvent OnImgLoadState(blnAllLoadFinish, blnUpLoad)

    Exit Sub
err:
BUGEX "OnState err " & err.Description
End Sub
...
'功能创建工具条
    ...
    Set Me.cbrMain.Icons = zlCL_GetPubIcons 'imgPublic.Icons '
