...
Private mblnAllDept As Boolean
Private mblnIsSame As Boolean      '当前科室与保存的本机执行科室是否一致
...
'载入检查科室
    ...
    cbxDept.Enabled = mblnAllDept
    
    strCfgDept = zlDatabase.GetPara("本机执行间科室", glngSys, mlngModule)
    
    mblnIsSame = mlngDept = Val(strCfgDept)
    ...
    Do Until rsData.EOF
        ...
        If Not mblnAllDept Then
            If mlngDept = Val(Nvl(rsData!ID)) Then
