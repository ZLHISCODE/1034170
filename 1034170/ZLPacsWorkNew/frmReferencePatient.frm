...
Begin VB.Form frmReferencePatient 
   ...
   Begin VB.Frame Frame3 
      ...
      Top             =   6360
      Width           =   10335
      Begin VB.CheckBox chkQueryType 
         Caption         =   "只查询影像类别或者执行科室相同的病人"
         BeginProperty Font 
            Name            =   "宋体"
            Size            =   12
            Charset         =   134
         ...
         Left            =   5520
         TabIndex        =   28
         ToolTipText     =   "检查号生成方式为0时，只过滤类别相同的病人，为1时只过滤执行科室相同的病人"
         Top             =   1800
         Value           =   1  'Checked
         Width           =   4790
      End
...
Private mblLoad As Boolean    '是否赋值的加载
...
'       mlngDetpID --- 执行科室ID，用来提取可是流程参数
    ...
    strSql = "Select Distinct a.病人id, b.检查号, b.关联id, a.姓名, a.性别, a.年龄, a.出生日期, a.门诊号, a.住院号, a.就诊卡号, a.费别," & _
             ...
             " Where c.病人id = a.病人id And c.ID = b.医嘱ID Order By a.病人id "
    
    Set rsToBeRelate = zlDatabase.OpenSQLRecord(strSql, "提取关联病人", mstr姓名, mlngPatietID)
    
    If chkQueryType.value = 1 Then
...
Private Sub ChangeRelationPar(ByVal blchkVal As Boolean)
'参数变化将会重新加载blchkVal：变化后的参数值
...
'    End If
    ...
    '查询是否有同名而且没有关联的病人
'    If mlngPatietID = 0 Then Exit Sub
    ...
    '如果没有关联的病人，且不显示窗体，则退出
    If rsToBeRelate.EOF = True Or rsToBeRelate.RecordCount = 0 Then
...
Private Sub chkQueryType_Click()
    If mblLoad = False Then Call ChangeRelationPar(chkQueryType.value = False)
End Sub
...
Private Sub Form_Load()
    ...
    mblLoad = True
    chkQueryType.value = Val(GetSetting("ZLSOFT", strRegPath, "只查询影像类别或者执行科室相同的病人", 0))
    mblLoad = False
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    Call SaveSetting("ZLSOFT", strRegPath, "只查询影像类别或者执行科室相同的病人", chkQueryType.value)
End Sub
