...
'获取检查核收信息
    ...
    strSQL = "select 病理号,号码规则ID from 病理检查信息 where 医嘱ID=[1]"
    ...
    mlngStudyType = Val(Nvl(rsData!号码规则ID))
End Sub
...
Private Sub LoadStudyType()
    ...
    If mlngStudyType >= 0 Then
        '已经报到了
        strSQL = "select B.ID as 执行分类 from  病理检查信息 A,病理号码规则 B where A.号码规则ID=B.ID and 医嘱id=[1]"
