...
Private mobjReportImageInfo As Scripting.Dictionary
...
On Error GoTo errHandle
    ...
    '如果安装了xw viewer，则注册表中的安装信息，而且授权XW观片模块，才启动
    blnResult = IIf(RegOpenKeyEx(HKEY_CURRENT_USER, "Software\Silver\Silver Pacs\General", 0, 1, lngPhkResult) = 0&, True, False)
    
    blnResult = blnResult And CheckPopedom(";" & GetPrivFunc(glngSys, G_LNG_XWPACSVIEW_MODULE) & ";", "基本")
...
'--------------------------------------------
    ...
    If Not CheckPopedom(";" & GetPrivFunc(glngSys, G_LNG_XWPACSVIEW_MODULE) & ";", "基本") Then
        MsgBox "用户没有授权，无法打开观片"
        Exit Function
    End If
...
'--------------------------------------------
    ...
    If Not CheckPopedom(";" & GetPrivFunc(glngSys, G_LNG_XWPACSVIEW_MODULE) & ";", "基本") Then Exit Function
    ...
    strSqlPWD = GetDecryptionPassW(zlDatabase.GetPara("XW数据库服务器密码", glngSys, G_LNG_XWPACSVIEW_MODULE, ""))
...
'-101    其他错误
    ...
    If lngAdviceID <= 0 Then
        '记录接口日志
        If gblnXWLog = True Then
            Call WriteCommLog("XWWeb3DViewerOpen", "XW接口", "调XW的3D观片,医嘱ID为空")
        End If
    ...
    Else
        '记录接口日志
        If gblnXWLog = True Then
            Call WriteCommLog("XWWeb3DViewerOpen", "XW接口", "调XW的3D观片,医嘱ID为:" & lngAdviceID)
        End If
...
Public Function XWShowImage(ByVal lngViewerType As Long, ByVal strFilter As String, ByVal lngOrderID As Long, Optional ByVal lngPlanId As Long = 1) As Long
''--------------------------------------------
''功能： 打开新网的ADViewer或者WEB Viewer
''参数：    lngViewerType -- 打开Viewer的方式；1-放射科ADViewer；2-临床WEB Viewer
'           strFilter -- 打开ADView的过滤语句
'           lngOrderID -- 医嘱ID
'           lngPlanId -- 打开ADView的方案ID
''返回：0-成功;1-出错
''--------------------------------------------
    On Error GoTo err
    ...
    ElseIf lngViewerType = 2 Then
        Call XWWebViewerOpen(lngOrderID)
    End If
...
'------------------------------------------------
    ...
    Dim iMax As Integer
    Dim frmReportImg As frmReportImage
    Dim i As Integer
    Dim rtReportType As ReportType
    ...
    '将报告图保存成文件，控制结果图象的分辨率在500*500之内
    iPlane = 1
    If Not IsNull(dcmImage.Attributes(&H28, &H4).value) And dcmImage.Attributes(&H28, &H4).Exists Then
        If dcmImage.Attributes(&H28, &H4).value = "RGB" Then
            iPlane = 3
        End If
    End If
    ...
    '控制结果图象的分辨率在1024*1024之内
    If dcmImage.SizeX > iMax Or dcmImage.SizeY > iMax Then
        dblZoom = iMax / dcmImage.SizeX
        If dblZoom > iMax / dcmImage.SizeY Then dblZoom = dcmImage / dcmImage.SizeY
    Else
        dblZoom = 1
    End If
    
    Call dcmImage.PrinterImage(8, iPlane, True, dblZoom, 0, dcmImage.SizeX, 0, dcmImage.SizeY).FileExport(strLocalPath & "\" & strFileName, "JPG")
    ...
    If lngResult <> 0 Then
        ...
        If lngResult = 0 Then
            ...
            '将报告图直接添加到缩略图和报告中，89766
            If Not dcmImage Is Nothing Then
                rtReportType = GetDeptPara(glngXWDeptID, "报告编辑器", 0)
                
                If rtReportType = 报告文档编辑器 Then   'PACS报告文档编辑器
                    Call AddXWReportImgToRichRPT(lngOrderID, strLocalPath & "\" & strFileName)
                Else
                    If Not mobjReportImageInfo Is Nothing Then
                        For i = 0 To mobjReportImageInfo.Count - 1
                            Set frmReportImg = mobjReportImageInfo(mobjReportImageInfo.Keys(i))
                            
                            If Not frmReportImg Is Nothing Then
                                Call frmReportImg.LoadMiniImages
                                Call frmReportImg.DcmAddXWImage(dcmImage.PrinterImage(8, iPlane, True, dblZoom, 0, dcmImage.SizeX, 0, dcmImage.SizeY))
                            End If
                        Next
                    End If
                End If
...
Private Sub AddXWReportImgToRichRPT(ByVal lngOrderID As Long, ByVal strDcmImgPath As String)
    Dim objRichReport As New clsRichReport
On Error GoTo err:
                        
    Call objRichReport.CreatePacsInterface
    Call objRichReport.AddXWReportImgToRichRPT(lngOrderID, strDcmImgPath)
...
err:
    Set objRichReport = Nothing
    BUGEX "AddXWReportImgToRichRPT err = " & err.Description
End Sub

Public Function RegXWAddReportImgWindow(ByVal hWnd As Long, ByVal objRptImg As frmReportImage) As Long
    If mobjReportImageInfo Is Nothing Then Set mobjReportImageInfo = New Scripting.Dictionary
    
    If Not mobjReportImageInfo.Exists("_" & hWnd) Then
        Call mobjReportImageInfo.Add("_" & hWnd, objRptImg)
    End If
End Function

Public Function DisRegXWAddReportImgWindow(ByVal hWnd As Long)
    If mobjReportImageInfo.Exists("_" & hWnd) Then
        Call mobjReportImageInfo.Remove("_" & hWnd)
    End If
    
    If mobjReportImageInfo.Count <= 0 Then Set mobjReportImageInfo = Nothing
End Function
...
'------------------------------------------------
    ...
    If intType = 1 Then     '删除图像
        ...
        strPswd = GetDecryptionPassW(zlDatabase.GetPara("XW删除图像密码", glngSys, G_LNG_XWPACSVIEW_MODULE, ""))
    ElseIf intType = 2 Then     '发送图像
        strUser = zlDatabase.GetPara("XW发送图像用户名", glngSys, G_LNG_XWPACSVIEW_MODULE, "")
        strPswd = GetDecryptionPassW(zlDatabase.GetPara("XW发送图像密码", glngSys, G_LNG_XWPACSVIEW_MODULE, ""))
    Else    '光盘刻录
        strUser = zlDatabase.GetPara("XW光盘刻录用户名", glngSys, G_LNG_XWPACSVIEW_MODULE, "")
        strPswd = GetDecryptionPassW(zlDatabase.GetPara("XW光盘刻录密码", glngSys, G_LNG_XWPACSVIEW_MODULE, ""))
    End If
...
Public Function GetEncryptionPassW(ByVal strPswd As String) As String
'获取加密密码
    ...
    strEncryptionPassW = objFtp.GetEncryptionPassW(Trim(strPswd))
...
Public Function GetDecryptionPassW(ByVal strPswd As String) As String
'获取解密密码
    Dim objFtp As New clsFtp
