   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picSchemeCfg 
         ...
         Begin VB.Frame framBase 
            ...
            Top             =   360
            Width           =   9090
            Begin VB.CheckBox chkEnabledRules 
               Caption         =   "是否启用规则查询"
               Height          =   255
               Left            =   6480
               TabIndex        =   22
...
'查询方案列定义
Private Const M_STR_SCHEME_COLS As String = "|Id,hide,key|方案序号,hide|查询语句,hide|所属科室,hide|方案名称,read,w2100|是否默认,read,w1000|是否系统查询,read,w1500|是否启用规则,hide|使用状态,read,w1000|方案说明,read,w2400|"
Private Const M_STR_SCHEME_CONVERT As String = "|是否默认:0-,1-默认|是否系统查询:0-,1-是|使用状态:0-禁用,1-启用|"
...
'Begin------------------------编辑菜单--------------------------------------默认可见
    ...
    Set cbrControl = cbrMenuBar.CommandBar.Controls.Add(xtpControlButton, TMenuType.mtSetSysQuery, "是否系统查询(&S)"): cbrControl.IconId = 9004
    ...
    Set cbrControl = cbrToolBar.Controls.Add(xtpControlButton, TMenuType.mtSetSysQuery, "是否系统查询"): cbrControl.IconId = 9004
...
On Error GoTo errHandle
    Select Case control.ID
        ...
        Case TMenuType.mtSetSysQuery
            Call SetSysQuery    '设置是否系统查询
...
'导出方案
    ...
    strSQL = "select a.id, 方案名称,方案说明,查询语句,方案序号,是否默认,是否系统查询,是否启用规则,使用状态,b.id as 配置id,录入项目,录入类型,录入顺序,默认值,数据来源 " & _
...
On Error Resume Next
    ...
    Select Case control.ID
        ...
        Case TMenuType.mtSetSysQuery
            ...
            If blnHasRecord Then
                If control.Parent.type = xtpControlPopup Then
                    control.Caption = IIf(ufgScheme.CurText("是否系统查询") = "是", "取消系统查询(&F)", "设置系统查询(&F)")
                    control.IconId = IIf(ufgScheme.CurText("是否系统查询") = "是", 9003, 9004)
                Else
                    control.Caption = IIf(ufgScheme.CurText("是否系统查询") = "是", "取消系统查询", "设置系统查询")
                    control.IconId = IIf(ufgScheme.CurText("是否系统查询") = "是", 9003, 9004)
                End If
...
'设置是否系统查询
    ...
    strCurDefaultState = ufgScheme.CurText("是否系统查询")
    
    If ufgInputCfg.GridRows > 1 And strCurDefaultState = "" Then
        MsgBoxD Me, "此查询方案包含了录入项，不能设置为系统查询。", vbOKOnly + vbExclamation, Me.Caption
        Exit Sub
    End If
    
    strSQL = "zl_影像查询方案_Set(" & ufgScheme.CurKeyValue & "," & IIf(strCurDefaultState = "是", 0, 1) & ")"
    Call zlDatabase.ExecuteProcedure(strSQL, "配置系统查询")
    
    For i = 1 To ufgScheme.GridRows - 1
        ufgScheme.Text(i, "是否系统查询") = ""
    Next i
    
    ufgScheme.CurText("是否系统查询") = IIf(strCurDefaultState = "是", "", "是")
...
'需要判断是修改原来的方案还是新增的方案
    ...
    If lngSchemeId <= 0 Then
        ...
        strExeSql(0) = "zl_影像查询方案_新增方案(" & lngSchemeId & ",'" & _
                                                    ...
                                                    cbxDepart.ItemData(cbxDepart.ListIndex) & "," & _
                                                    chkEnabledRules.value & ")"
    ...
    With ufgScheme
        ...
        .CurText("是否启用规则") = chkEnabledRules.value
...
'载入方案
    ...
    strSQL = "select id, 方案名称,方案说明,查询语句,方案序号,是否默认,是否系统查询,是否启用规则,使用状态,所属科室 from 影像查询方案 where 所属科室=[1] order by 方案序号"
...
'载入方案配置数据
    ...
    chkEnabledRules.value = Val(ufgScheme.Text(lngSchemeRowIndex, "是否启用规则"))
...
'功能：初始化住院临床科室
    ...
    str来源 = "1,2,3"
    If CheckPopedom(gstrPrivs, "所有科室") Then
        strSQL = _
...
Private Sub ConfigFaceEditState(ByVal blnIsEdit As Boolean)
    ...
    chkEnabledRules.Enabled = blnIsEdit
    
    ufgInputCfg.ReadOnly = Not blnIsEdit
    
    cmdNewInput.Enabled = blnIsEdit And ufgScheme.CurText("是否系统查询") = ""
    cmdDelInput.Enabled = blnIsEdit And ufgScheme.CurText("是否系统查询") = ""
    cmdMoveLast.Enabled = blnIsEdit And ufgScheme.CurText("是否系统查询") = ""
    cmdMoveNext.Enabled = blnIsEdit And ufgScheme.CurText("是否系统查询") = ""
...
On Error Resume Next
    ...
    framBase.Width = picSchemeCfg.ScaleWidth
        txtSchemeMemo.Width = framBase.Width - txtSchemeMemo.Left - chkEnabledRules.Width - 240
        chkEnabledRules.Left = txtSchemeMemo.Left + txtSchemeMemo.Width + 120
    framInput.Left = 0
