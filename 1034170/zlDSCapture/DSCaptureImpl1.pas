...
begin
  ...
  try
    ...
    if Failed(hr) then begin
      errMsg := '创建SmartTeeFilter分流接口,并加入到FilterGraphic中时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;
    ...
    if Failed(hr) then begin
      errMsg := '创建SmartTeeFilter1分流接口,并加入到FilterGraphic中时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;

    hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _CapSourceFilter, _SmartTee, False, 0);
    if Failed(hr) then begin
      errMsg := '创建CapSourceFilter与SmartTeeFilter之间的连接时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;
    ...
    if isCaptureVideo then begin
      ...
      if Failed(hr) then begin
        errMsg := '创建SmartTeeFilter与EncoderFilter之间的连时失败。[错误代码:' + IntToStr(hr) + ']';
        Exit;
      end;
    ...
    hr := TDS9Ex.ConnectFilters(_FilterGraphic as IGraphBuilder, _SmartTee1, _ImgCaptureFilter  as IBaseFilter, false, 0);
    if Failed(hr) then begin
      errMsg := '创建SmartTee与ImgCaptureFilter之间的连接时失败。[错误代码:' + IntToStr(hr) + ']';
      Exit;
    end;
...
begin
  ...
  //亮度
  if captureParameter.Brightness < 0 then captureParameter.Brightness := GetDefaultQualityInf(amVideoProcAmp, VideoProcAmp_Brightness);
  amVideoProcAmp.Set_(
  ...
  //对比度
  if captureParameter.Contrast < 0 then captureParameter.Contrast := GetDefaultQualityInf(amVideoProcAmp, VideoProcAmp_Contrast);
  amVideoProcAmp.Set_(
  ...
  //色调
  if captureParameter.Hue < 0 then captureParameter.Hue := GetDefaultQualityInf(amVideoProcAmp, VideoProcAmp_Hue);
  amVideoProcAmp.Set_(
  ...
  //饱和度
  if captureParameter.Saturation < 0 then captureParameter.Saturation := GetDefaultQualityInf(amVideoProcAmp, VideoProcAmp_Saturation);
  amVideoProcAmp.Set_(
  ...
  //伽马
  if captureParameter.Gamma < 0 then captureParameter.Gamma := GetDefaultQualityInf(amVideoProcAmp, VideoProcAmp_Gamma);
  amVideoProcAmp.Set_(
  ...
  //白平衡
  if captureParameter.WhiteBlance < 0 then captureParameter.WhiteBlance := GetDefaultQualityInf(amVideoProcAmp, VideoProcAmp_WhiteBalance);  
  amVideoProcAmp.Set_(
