...
// *****************************************************************************
  ...
  private
    ...
    FMonitorIndex  : Integer;        //modify by tjh at 2010-03-11
    FFullHandle: THandle;            //modify by tjh at 2010-03-11
    FIsStreach: Boolean;             //modify by tjh at 2011-01-05
  ...
  published
    ...
    property MonitorIndex: Integer read FMonitorIndex write FMonitorIndex;
    property FullHandle: THandle read FFullHandle write FFullHandle;
    property IsStreach: Boolean read FIsStreach write FIsStreach;
...
//******************************************************************************
  ...
  var
    ...
    function GetZoomRate(const sourceWidth, sourceHeight, destWidth, destHeight: Integer): Double;
    begin
      try
        //È¡µÃËõ·Å±ÈÀý
        if sourceHeight > sourceWidth then begin
          if destWidth > destHeight then
            Result := destHeight / sourceWidth
          else
            Result := destHeight / sourceWidth;
        end else begin
          if destWidth > destHeight then
      ...
      case Value of
        true:
          begin
            ...
            hr := (FVideoWindow as IBaseFilter).QueryInterface(IID_IBasicVideo, ibv);
            if not Failed(hr) then begin
              ibv.get_SourceWidth(vw);
              ibv.get_SourceHeight(vh);

              ibv := nil;
            end;

            //CheckDSError(FVideoWindow.put_Owner(0));
            CheckDSError(FVideoWindow.put_Owner(FullHandle));
            CheckDSError(FVideoWindow.put_Caption('È«ÆÁÄ£Ê½'));
            ...
            if FIsStreach then begin
              //ÐÞ¸Ä¶Ô¶àÆÁÊ±µÄÖ§³Ö modify by tjh at 2010-03-11
              CheckDSError(FVideoWindow.SetWindowPosition(0, 0,
                                                          Screen.Monitors[FMonitorIndex].Width,
                                                          Screen.Monitors[FMonitorIndex].Height));
            end else begin
              rate := GetZoomRate(vw, vh, Screen.Monitors[FMonitorIndex].Width, Screen.Monitors[FMonitorIndex].Height);
              //rate := 1;

              vw := Round(vw * rate);
              vh := Round(vh * rate);

              OutputDebugString(Pchar('Rate:' + FloatToStr(rate)));

              //modify by tjh at 2011-01-5
              CheckDSError(FVideoWindow.SetWindowPosition((Screen.Monitors[FMonitorIndex].Width - vw) div 2,
                                                          (Screen.Monitors[FMonitorIndex].Height - vh) div 2,
                                                          vw, vh));
