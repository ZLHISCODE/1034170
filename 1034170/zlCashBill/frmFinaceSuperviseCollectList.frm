...
Private Sub InitGrid()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    With vsRollingCurtain
        ...
        .Editable = IIf(zlCheckPrivs(mstrPrivs, "轧帐收款"), flexEDKbdMouse, flexEDNone)
        .Tag = ""
    End With
...
Public Function LoadCollectData() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If mCollectType = EM_TY_收费员 Then
        strSQL = "" & _
        "   Select " & IIf(zlCheckPrivs(mstrPrivs, "轧帐收款"), "-1", "0") & " as 选择,a.Id,a.No As 轧帐单号, " & _
        "         decode(nvl(A.类别,0),0,'所有类别',1,'收费',2,'预交',21, '门诊预交', 22, '住院预交',3,'结帐',4,'挂号',5,'就诊卡',6,'消费卡','-') as 轧帐类别, " & _
    ...
    ElseIf mCollectType = EM_TY_小组 Then
        strSQL = "" & _
        "   Select  " & IIf(zlCheckPrivs(mstrPrivs, "轧帐收款"), "-1", "0") & " as 选择,a.Id,a.No As 轧帐单号,'' as 轧帐类别, a.开始时间, a.终止时间, a.登记人 As 轧帐人, a.登记时间 As 轧帐时间,  " & _
        "         a.收款员 ,b.名称 As 收款部门, " & _
        "         ltrim(to_char(a.冲预交款,'9999999999990.00')) as 冲预交款, " & _
        "         ltrim(to_char(a.借入合计,'9999999999990.00')) as 借入合计, " & _
        "         ltrim(to_char(a.借出合计,'9999999999990.00')) as 借出合计," & _
...
Private Sub txtNO_GotFocus()
    SelAll txtNO: zlCommFun.OpenIme False
End Sub
...
Private Sub vsRollingCurtain_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModule, vsRollingCurtain, Me.Name, "轧帐信息列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Sub vsGroupPerson_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModule, vsGroupPerson, Me.Name, "小组收费员轧帐信息列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Sub vsGroup_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModule, vsGroup, Me.Name, "组收款信息列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Sub imgColPlanRC_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(picImgPlanRC.hWnd)
    lngLeft = vRect.Left
