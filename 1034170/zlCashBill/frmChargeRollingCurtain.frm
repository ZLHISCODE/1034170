...
Private mintRollingType As Integer '类别
Private mintPreRollingType As Integer '上次轧帐类别
...
Public Sub zlInitVar(frmMain As Object, ByVal lngModule As Long, ByVal strPrivs As String, _
    ByVal strPreDate As String, ByVal strOperatorName As String, ByVal intRollingType As Integer, Optional ByVal strDefaultTime As String = "")
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:初始化相关变量
    '入参:lngModule-模块号
    '     strPrivs-权限串
    ...
    mstrPreDate = strPreDate: mintRollingType = intRollingType
...
Private Sub InitFace()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Else
        ...
        Call mobjChargeBill.LoadChargeAndBillTotalData(Me, mlngModule, mstrPrivs, 1, 0, dtpStartDate, dtpEndDate, False, , , mintRollingType)
...
Private Sub SetPopedom()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '说明:
    '---------------------------------------------------------------------------------------------------------------------------------------------
    cmdOK.Visible = zlCheckPrivs(mstrPrivs, "轧帐")
End Sub
...
Private Sub BillPrint(ByVal strNO As String)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    blnPrint = False
    If Not zlCheckPrivs(mstrPrivs, "缴款书打印") Then Exit Sub
...
Public Function SaveData(ByRef lngID As Long, ByRef strNO As String) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    blnOK = objChargeBillTotal.SaveData(strStartDate, strEndDate, strMemo, lngDeptID, strNO, lngID, Val(txtRemain.Text), mintRollingType)
...
Private Sub cmdRefresh_Click()
    ...
    'If mdatEnd = dtpEndDate And mdatBegin = dtpStartDate And mintPreRollingType <> mintRollingType Then Exit Sub
    Call mobjChargeBill.LoadChargeAndBillTotalData(Me, mlngModule, mstrPrivs, 1, 0, dtpStartDate, dtpEndDate, False, , , mintRollingType)
...
Public Sub zlRefresh()
    ...
    Call cmdRefresh_Click
End Sub
Public Sub ShowChargeList(ByVal frmMain As Object, Optional intRollingType As Integer)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:显示明细收款数据
    '入参:frmMain-调用的主窗体
    ...
    Load frmNew
    Call frmNew.ShowMe(frmMain, mlngModule, mstrPrivs, 1, "", dtpStartDate.Value, dtpEndDate.Value, False, , intRollingType)
...
'        If cboDept.Enabled And cboDept.Visible Then
'            cboDept.SetFocus
'        End If
'        Exit Sub
'    End If
