...
'功能：重新提取当前缴款人员的缴款明细
    ...
    If mEditType = PM_全额缴款 Or mEditType = PM_按日缴款 And (bytFlag = 0 Or bytFlag = 2) Then
        ...
        strSQL = strSQL & IIf(strSQL = "", "", " Union ALL ") & _
            ...
            " Where A.ID=B.结帐ID And A.操作员姓名||''=[1] And A.收费时间>[2] And A.收费时间<=[3] And Not Exists(" & strSub & ")" & _
            " Group by Decode(Mod(B.记录性质,10),1,'[冲预交款]',B.结算方式)"
    End If
    ...
    If mEditType = PM_全额缴款 Or mEditType = PM_按日缴款 And (bytFlag = 0 Or bytFlag = 6) Then
        ...
        strSQL = strSQL & IIf(strSQL = "", "", " Union ALL ") & _
            " Select A.结算方式,Sum(A.销售金额) as 金额" & _
            " From 消费卡目录 A" & _
            " Where  发卡人||''=[1] And 发卡时间>[2] And 发卡时间<=[3] And Not Exists(" & strSub & ") " & _
            " Group by 结算方式"
        
        strSub = "Select Y.记录ID From 人员缴款记录 X,人员缴款对照 Y Where Y.记录ID=A.ID And X.收款员=[1] And X.ID=Y.单据ID And Y.性质=5"
        strSQL = strSQL & IIf(strSQL = "", "", " Union ALL ") & _
            " Select A.结算方式, Sum(A.缴款金额) as 金额" & _
            " From 消费卡充值记录 A" & _
            " Where  A.操作员姓名||''=[1] And 充值时间>[2] And 充值时间<=[3]  And Not Exists(" & strSub & ") " & _
            " Group by 结算方式"
    End If
    ...
    If mEditType = PM_全额缴款 Or mEditType = PM_按日缴款 And (bytFlag = 0 Or bytFlag = 2) Then
        ...
        " Where B.记帐费用=1 And A.ID=B.结帐ID And A.操作员姓名||''=[1] And A.收费时间>[2] And A.收费时间<=[3]" & _
        "       And Not Exists(" & strSub & ") " & _
        " Group by B.收入项目ID " & _
        " Union ALL  " & _
        " Select B.收入项目ID,Sum(B.结帐金额) as 结帐金额" & _
   ...
   '消费卡:
    If mEditType = PM_全额缴款 Or mEditType = PM_按日缴款 And (bytFlag = 0 Or bytFlag = 6) Then
        ...
        strSub = "Select Y.记录ID From 人员缴款记录 X,人员缴款对照 Y Where Y.记录ID=A.ID And X.收款员=[1] And X.ID=Y.单据ID And Y.性质=6"
        strSQL = "" & _
            " Select  Sum(A.销售金额) as 金额" & _
            " From 消费卡目录 A" & _
            " Where  发卡人||''=[1] And 发卡时间>[2] And 发卡时间<=[3] And Not Exists(" & strSub & ") "
        ...
        strSub = "Select Y.记录ID From 人员缴款记录 X,人员缴款对照 Y Where Y.记录ID=A.ID And X.收款员=[1] And X.ID=Y.单据ID And Y.性质=5"
        strSQL = "" & _
            " Select  Sum(A.缴款金额) as 金额" & _
            " From 消费卡充值记录 A" & _
            " Where  A.操作员姓名||''=[1] And 充值时间>[2] And 充值时间<=[3]  And Not Exists(" & strSub & ") "
    ...
    If mEditType = PM_全额缴款 Or mEditType = PM_按日缴款 And (bytFlag = 0 Or bytFlag = 2) Then
        ...
        strSQL = strSQL & IIf(strSQL = "", "", " Union ALL ") & _
            ...
            " From 病人结帐记录 A" & _
            " Where 操作员姓名||''=[1] And 收费时间>[2] And 收费时间<=[3] And Not Exists(" & strSub & ")"
    End If
