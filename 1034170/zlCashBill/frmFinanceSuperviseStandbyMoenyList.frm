...
Private Function LoadData() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    On Error GoTo errHandle
     strPerson = NeedName(cboPerson.Text)
    If txtNO.Text <> "" Then
...
Private Sub cboPerson_Validate(Cancel As Boolean)
    If cboPerson.Text <> "" Then
        If GetCboIndex(cboPerson, NeedName(cboPerson.Text)) = -1 Then cboPerson.ListIndex = -1: cboPerson.Text = ""
    End If
...
Private Function isCheckPersonExists(ByVal str姓名 As String, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    For i = 0 To cboPerson.ListCount - 1
        If NeedName(cboPerson.List(i)) = str姓名 Then
...
Public Sub RePrintBill()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strNO As String
    If Not (zlCheckPrivs(mstrPrivs, "备用金领用单打印") And zlCheckPrivs(mstrPrivs, "重打备用金领用单")) Then Exit Sub
    With vsList
...
Public Function zlPayOnWorkMoney(ByVal frmMain As Object) As Boolean
    ...
    If cboPerson.ListIndex > 0 Then
        If cboPerson.ItemData(cboPerson.ListIndex) <> 0 Then
            str领用人 = NeedName(cboPerson.Text)
        End If
    End If
...
Private Sub vsList_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModule, vsList, Me.Name, "备用金信息列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
