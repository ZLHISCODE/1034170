...
Private mintRollingType As Integer '轧帐类别(0-所有类别(按全额轧帐),1-收费,2-预交,3-结帐,4-挂号,5-就诊卡,6-消费卡)
...
Public Sub ShowMe(ByVal frmMain As Object, _
      ...
      Optional intRollingType As Integer)
    '-------------------------------------------------------------------------------------------------
    ...
    '    intRollingType-轧帐类别,bytType=1时有效分别为:
    ...
    mintRollingType = intRollingType: mblnFirst = True
...
Private Function LoadPersonList() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If mintRollingType <> 0 Then
        If Get轧帐结算性质(mintRollingType, strRollingType) = False Then
    ...
    If InStr(",0,5,", "," & mintRollingType & ",") > 0 Then '就诊卡
        strSQL = "" & _
    ...
    If InStr(",0,1,4,", "," & mintRollingType & ",") > 0 Then '就收费,挂号诊卡,补结算
    ...
    If InStr(",0,3,", "," & mintRollingType & ",") > 0 Then '结帐
        strSQL = strSQL & IIf(strSQL <> "", vbCrLf & " Union ALL ", "") & vbCrLf & _
        ...
        "   Where A.ID=B.结帐ID  And B.结算性质=2 And A.病人ID=C.病人ID(+) " & vbCrLf & _
    ...
    If InStr(",0,2,", "," & mintRollingType & ",") > 0 Then '预交款(充值)
        strSQL = strSQL & IIf(strSQL <> "", vbCrLf & " Union ALL ", "") & vbCrLf & _
    ...
    If InStr(",21,", "," & mintRollingType & ",") > 0 Then '门诊预交款(充值)
        strSQL = strSQL & IIf(strSQL <> "", vbCrLf & " Union ALL ", "") & vbCrLf & _
        "   Select 7 As 记录性质,A.ID as 记录ID, a.No, a.记录状态, Max(nvl(M.姓名,c.姓名)) As 病人姓名, Max(c.性别) As 性别, Max(c.年龄) As 年龄, Max(c.门诊号) As 门诊号, " & vbCrLf & _
        "      Max(Decode(m.住院号, Null, c.住院号, m.住院号)) As 住院号, Max(a.操作员姓名) As 操作员姓名,Max(结算方式) AS 结算方式, Sum(a.金额) As 金额合计, Max(a.收款时间) As 收费时间 " & vbCrLf & _
        "   From 病人预交记录 A, 病人信息 C, 病案主页 M " & vbCrLf & _
    ...
    If InStr(",22,", "," & mintRollingType & ",") > 0 Then '住院预交款(充值)
        strSQL = strSQL & IIf(strSQL <> "", vbCrLf & " Union ALL ", "") & vbCrLf & _
        "   Select 7 As 记录性质,A.ID as 记录ID, a.No, a.记录状态, Max(nvl(M.姓名,c.姓名)) As 病人姓名, Max(c.性别) As 性别, Max(c.年龄) As 年龄, Max(c.门诊号) As 门诊号, " & vbCrLf & _
        "      Max(Decode(m.住院号, Null, c.住院号, m.住院号)) As 住院号, Max(a.操作员姓名) As 操作员姓名,Max(结算方式) AS 结算方式, Sum(a.金额) As 金额合计, Max(a.收款时间) As 收费时间 " & vbCrLf & _
        "   From 病人预交记录 A, 病人信息 C, 病案主页 M " & vbCrLf & _
    ...
    If InStr(",0,3,", "," & mintRollingType & ",") > 0 Then '结帐(充值)
        strSQL = strSQL & IIf(strSQL <> "", vbCrLf & " Union ALL ", "") & vbCrLf & _
        "   Select 7 As 记录性质,A.ID as 记录ID, a.No, a.记录状态, Max(nvl(M.姓名,c.姓名)) As 病人姓名, Max(c.性别) As 性别, Max(c.年龄) As 年龄, Max(c.门诊号) As 门诊号, " & vbCrLf & _
        "      Max(Decode(m.住院号, Null, c.住院号, m.住院号)) As 住院号, Max(a.操作员姓名) As 操作员姓名,Max(结算方式) AS 结算方式, Sum(a.金额) As 金额合计, Max(a.收款时间) As 收费时间 " & vbCrLf & _
        "   From 病人预交记录 A, 病人信息 C, 病案主页 M " & vbCrLf & _
    ...
    If InStr(",0,6,", "," & mintRollingType & ",") > 0 Then '消费卡(充值,面值)
        strSQL = strSQL & IIf(strSQL <> "", vbCrLf & " Union ALL ", "") & vbCrLf & _
        "   Select 8 as 记录性质,A.ID as 记录ID,ltrim (to_Char(a.ID)) AS no,a.记录状态,'' as 病人姓名,'' as 性别,'' as 年龄,null 门诊号,NULL as 住院号, " & vbCrLf & _
        "        A.操作员姓名  AS 操作员姓名,a.结算方式, a.缴款金额  As 金额合计,   A.充值时间 AS 收费时间  " & _
        "   From 消费卡充值记录 A " & vbCrLf & _
        "   Where a.操作员姓名 || '' = [1]  " & vbCrLf & _
        "        And 充值时间 Between [2] And [3]  " & vbCrLf & _
        "        And Not Exists (Select y.记录id From 人员收缴记录 X, 人员收缴对照 Y Where y.记录id = a.Id And x.收款员 = [1] And x.Id = y.收缴ID And y.性质 = 5 And x.作废时间 Is Null)" & vbCrLf & _
        "   Union All " & vbCrLf & _
        "   Select 9 As 记录性质,A.ID as 记录ID, a.卡号 As NO, Decode(a.回收时间, Null, 1, 2) As 记录状态, '' As 病人姓名, '' As 性别, '' As 年龄, Null 门诊号, " & vbCrLf & _
        "       Null As 住院号, a.发卡人 As 操作员姓名, a.结算方式, a.销售金额 As 金额合计, a.发卡时间 " & vbCrLf & _
        "   From 消费卡目录 A " & vbCrLf & _
        "   Where 发卡人 || '' = [1] And 发卡时间 Between [2] And [3] " & vbCrLf & _
        "       And Not Exists (Select y.记录id From 人员收缴记录 X, 人员收缴对照 Y Where y.记录id = a.Id And x.收款员 = [1] And x.Id = y.收缴id And y.性质 = 6 And x.作废时间 Is Null) " & vbCrLf
    End If
...
Private Function LoadFeeData(Optional ByVal blnRollingData As Boolean) As Boolean
    ...
    If blnRollingData Then
        ...
        'mintRollingType:轧帐类别(0-所有类别(按全额轧帐),1-收费,2-预交,3-结帐,4-挂号,5-就诊卡,6-消费卡)
        '收缴对照:1-收费(含挂号),2-结帐,3-预交,4-借款;5-消费卡充值;6--消费卡面值;7-暂存金(本次增加)；８－收款或轧帐作废对照（本次增加）；9-二次结算记录
        If InStr(",2,6,", "," & mintRollingType & ",") > 0 Then
            '预交款和消费卡无收据费目
...
'        If mintRollingType = 1 Or mintRollingType = 0 Then  '收费和所有类别,包含二次补结算数据
...
'        "           Where a.Id = b.收缴id And b.性质=9 And B.记录ID=b1.结算ID " & strWhere & ") "
    ...
    strSQL = "" & _
    "   SELECT /*+ rule */ A.收据费目,ltrim(to_char(sum(A.结帐金额),'99999990.00')) AS 结帐金额 " & _
    "   FROM ( " & strSQL & " ) a " & _
    "   GROUP BY A.收据费目 " & _
    "   ORDER BY 收据费目"
...
Private Function LoadList() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    strSQL = "" & _
         ...
         "   Union ALL  " & _
         "  Select 记录性质, Min(记录id) As 记录id, NO, Max(记录状态) As 记录状态, 病人姓名, 性别, 年龄, 门诊号, 住院号, 操作员姓名, 结算方式, Sum(金额合计) As 金额合计, 收费时间 From (" & _
         "   Select  mod(a.记录性质,10) as 记录性质,A.结帐ID as 记录ID,A.NO,decode( a.记录状态,1,1,3,1,a.记录状态) as 记录状态,max(A.姓名) AS 病人姓名,max(A.性别) AS 性别,max(A.年龄) AS 年龄,max(decode(A.门诊标志,2,-1*NULL,A.标识号)) AS 门诊号,max(decode(A.门诊标志,2,标识号,null)) AS 住院号, " & _
         "     Max(A.操作员姓名) AS 操作员姓名, NULL AS 结算方式, " & _
         "     sum(结帐金额) AS 金额合计,Max(登记时间) AS 收费时间 " & _
         "   From 门诊费用记录 A,c_对照信息 Q1 " & _
         "   Where (MOD(A.记录性质,10)=1 OR A.记录性质=4) AND Nvl(A.记帐费用, 0) = 0 And A.记录状态 <> 0 And Nvl(A.费用状态, 0) <> 1  " & _
         "        And A.结帐ID=Q1.记录ID and Q1.性质=1 " & _
         "   GROUP BY mod(a.记录性质,10),A.结帐ID,A.NO,decode( a.记录状态,1,1,3,1,a.记录状态)) " & _
        ...
        "   Where A.ID=B.结帐ID and A.病人ID=C.病人ID(+)  " & _
        ...
        strSQL = strSQL & _
         ...
         "   Union All " & _
         "   Select 8 as 记录性质,A.ID as 记录ID,ltrim (to_Char(a.ID)) AS no,a.记录状态,'' as 病人姓名,'' as 性别,'' as 年龄,null 门诊号,NULL as 住院号, " & _
         "        A.操作员姓名  AS 操作员姓名,a.结算方式, a.缴款金额  As 金额,   A.充值时间 AS 收费时间  " & _
         "   From 消费卡充值记录 A,c_对照信息 Q1 " & _
         "   Where    A.ID=Q1.记录ID and Q1.性质=5   " & _
         "   Union All " & _
         "   Select 9 As 记录性质,A.ID as 记录ID, a.卡号 As NO, Decode(a.回收时间, Null, 1, 2) As 记录状态, '' As 病人姓名, '' As 性别, '' As 年龄, Null 门诊号, " & _
         "       Null As 住院号, a.发卡人 As 操作员姓名, a.结算方式, a.销售金额 As 金额, a.发卡时间 " & _
         "   From 消费卡目录 A,c_对照信息 Q1 " & _
         "   Where A.ID=Q1.记录ID and Q1.性质=6    "
    ...
    Select Case mbytType
     Case EM_收费员轧帐, EM_财务收款_非收费员
        ...
        End If
         ...
         strSQL = "" & _
        "   Select A.NO,to_char(A.开始时间,'yyyy-mm-dd hh24:mi:ss')||'至'||to_char(A.终止时间,'yyyy-mm-dd hh24:mi:ss') as 时间范围 " & _
        "   From 人员收缴记录 A ,Table( f_Num2list([1])) J" & _
        "   Where A.ID= J.Column_Value" & _
        "   Order by A.NO "
     Case EM_小组收款
        strSQL = "" & _
        "   Select A.NO,to_char(A.登记时间,'yyyy-mm-dd hh24:mi:ss') as 时间范围 " & _
        "   From 人员收缴记录 A ,Table( f_Num2list([1])) J" & _
        "   Where A.ID= J.Column_Value" & _
        "   Order by A.NO "
     Case EM_小组轧帐
...
Private Sub vsList_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngMode, vsList, Me.Name, "明细信息列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Sub vsBalance_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngMode, vsBalance, Me.Name, "结算信息列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Sub vsBillList_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngMode, vsBillList, Me.Name, "票据明细列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
