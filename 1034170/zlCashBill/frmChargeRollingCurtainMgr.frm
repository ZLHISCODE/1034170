...
Private Function GetPreRollingCurtainTime(ByVal intType As Integer) As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取上次轧帐时间
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, UserInfo.姓名, intType)
...
Public Function zlDefCommandBars() As Boolean
    ...
    With mcbrToolBar.Controls
        ...
        Set mobjComBar = .Add(xtpControlComboBox, conMenu_COMBOX_INTERFACE, "轧帐类别")
        mobjComBar.Flags = xtpFlagRightAlign
        mobjComBar.HideFlags = xtpNoHide
        mobjComBar.Width = (TextWidth("刘") * 16) / Screen.TwipsPerPixelX
        mobjComBar.Style = xtpComboLabel
        With mobjComBar
            i = 1
            .AddItem "所有类别": .ListIndex = 1: .ItemData(i) = 0: i = i + 1
            ...
            If InStr(1, mstr轧帐人员性质, ",门诊挂号员,") > 0 _
               ...
               Or InStr(1, mstr轧帐人员性质, ",发卡登记人,") > 0 Then
                ...
                .ItemData(i) = 5: i = i + 1
            End If
            If InStr(1, mstr轧帐人员性质, ",入院登记员,") > 0 _
               Or InStr(1, mstr轧帐人员性质, ",发卡登记人,") > 0 Then
                .AddItem "6-消费卡"
                .ItemData(i) = 6: i = i + 1
            End If
...
Private Sub InitPage()
    ...
    End If
    intRollingType = GetRollingType
    '初始化变量
    Call mfrmChargeRollingCurtain.zlInitVar(Me, mlngModule, mstrPrivs, mstrPreDate, UserInfo.姓名, intRollingType, GetDefaultTime)
...
Private Sub RollingCurtainCancel()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If mfrmHistory.CancelData() Then
        '作废轧帐后,重新读取上次轧帐时间
        Call InitData
        Call mfrmChargeRollingCurtain.zlInitVar(Me, mlngModule, mstrPrivs, mstrPreDate, UserInfo.姓名, intRollingType)
...
Private Sub ReloadData()
    ...
    On Error GoTo errHandle
    intRollingType = GetRollingType
    mstrPreDate = GetPreRollingCurtainTime(intRollingType)
    Call mfrmChargeRollingCurtain.zlInitVar(Me, mlngModule, mstrPrivs, mstrPreDate, UserInfo.姓名, intRollingType)
    Call mfrmChargeRollingCurtain.RefreshPage
    Exit Sub
errHandle:
...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_RollingCurtain ' "轧账(&Z)")
        blnHavePrivs = zlCheckPrivs(mstrPrivs, "轧帐")
    ...
    Case conMenu_Edit_RollingCurtain_Cancel ' "作废轧账(&D)")
        blnHavePrivs = zlCheckPrivs(mstrPrivs, "轧帐作废")
        Control.Visible = blnHavePrivs
    ...
    Case conMenu_Edit_ChargeBook_Reprint ' "重打缴款书(&R)")
        blnHavePrivs = zlCheckPrivs(mstrPrivs, "重打缴款书") And zlCheckPrivs(mstrPrivs, "缴款书打印")
        Control.Visible = blnHavePrivs
        Control.Enabled = blnHavePrivs And Val(tbPage.Selected.Tag) = EM_PG_历史列表
        If Control.Enabled Then
            Control.Enabled = mfrmHistory.GetChargeRollingCurtainID <> 0 _
...
Private Sub vsBalance_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModule, vsBalance, Me.Name, "暂存金列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Public Sub RefreshBasic()
    Call InitData
End Sub
Private Function GetRollingType() As Integer
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取轧帐类别
    '返回:返回轧帐类别
    '编制:刘兴洪
    ...
    On Error GoTo errHandle
    intRollingType = 0
    If Not mobjComBar Is Nothing Then
      If mobjComBar.ListIndex >= 0 Then intRollingType = mobjComBar.ItemData(mobjComBar.ListIndex)
    End If
    GetRollingType = intRollingType
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub ShowChargeList()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    intRollingType = GetRollingType
    If Val(tbPage.Selected.Tag) = EM_PG_轧帐列表 Then
         Call mfrmChargeRollingCurtain.ShowChargeList(Me, intRollingType)
         Exit Sub
    End If
    '历史数据显示
