...
Private Const mstrLvw As String = "开始号码,1000,0,1;终止号码,1000,0,2;使用类别,1000,0,1;领用人,800,0,2;当前号码,1000,0,0;剩余数量,600,0,0;" & _
    "批次,1000,0,0;使用方式,600,0,0;登记时间,1200,0,0;登记人,800,0,2;核对人,800,0,2;前缀文本,0,0,2;签字人,800,0,2;签字时间,1200,0,2"
...
Private Function InitFace() As Boolean
    ...
    If mbln药店 = False Then
        ...
        End If
        If InStr(1, mstrPrivs, ";预交收据;") > 0 And _
    ...
    Else
        ...
        lvwMain.ListItems.Add , "C5", "会员卡", "C6"
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    mstr票据 = ""
    mblnItem = False
    zlDatabase.SetPara "显示所有领用记录", IIf(mnuViewAll.Checked, 1, 0), glngSys, mlngModul, InStr(1, mstrPrivs, ";参数设置;") > 0
    zlDatabase.SetPara "查看核对信息", IIf(mnuViewCheck.Checked, 1, 0), glngSys, mlngModul, InStr(1, mstrPrivs, ";参数设置;") > 0
...
Private Sub mnuBillCheck_Click(Index As Integer)
    Dim lng领用ID As Long, str前缀 As String, blnChecked As Boolean, lng票种 As Long
    ...
    If Index = 0 Then
        ...
        If blnChecked Then
            ...
            gstrSQL = "zl_票据领用记录_check(" & lng领用ID & ",null,null,null,null)"
            Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
        Else
            Call frmBillEdit.ShowMe(Me, 1, mlngModul, mstrPrivs, lng领用ID)
        End If
        Call ShowItem(lvw领用_S.SelectedItem)
        Call SetMenu
    Else
        str前缀 = lvw领用_S.SelectedItem.SubItems(GetItemCOL("前缀文本"))
        Call frmBillUses.ShowMe(Me, 1, True, mblnNOMoved, lng票种, lng领用ID, str前缀)
...
Private Sub mnuBillDelete_Click()
    ...
    If MsgBox("你确认要删除开始号码为“" & lvw领用_S.SelectedItem.Text & "”的" & _
        lvwMain.SelectedItem.Text & "领用记录吗？", vbQuestion Or vbYesNo Or vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
...
Private Sub mnuBillGet_Click()
    Dim int票种 As Integer, str类别 As String
...
'    lngLen = Val(Mid(mstr票据长度, Mid(lvwMain.SelectedItem.Key, 2), 1))
'    If lngLen = 0 Then lngLen = 10
 
    '102181:李南春,2016/11/11,医疗卡票据长度
    If Mid(lvwMain.SelectedItem.Key, 2) <> 5 Then
        lngLen = Val(Split(mstr票据长度, "|")(Mid(lvwMain.SelectedItem.Key, 2) - 1))
    End If
    If msh汇总.Rows > 3 And Len(lvw领用_S.SelectedItem.Text) <> lngLen And mstr票据 <> "C5" Then
    ...
    Call frmBillEdit.ShowMe(Me, 0, mlngModul, mstrPrivs, Mid(lvw领用_S.SelectedItem.Key, 2))
...
Private Sub mnuBillCancel_Click()
    ...
    End If
    If zlIsModify(Val(Mid(lvw领用_S.SelectedItem.Key, 2))) = False Then Exit Sub
    Call frmBillDiscard.编辑票据报损(mstrPrivs, Mid(lvw领用_S.SelectedItem.Key, 2))
    
End Sub

Private Sub mnuFileLocalSet_Click()
    Call frmBillSuperviseParaSet.zlSetPara(Me, mlngModul, mstrPrivs)
End Sub
...
Private Sub mnuReportItem_Click(Index As Integer)
    ...
    Call ReportOpen(gcnOracle, Split(mnuReportItem(Index).Tag, ",")(0), Split(mnuReportItem(Index).Tag, ",")(1), Me, _
        "票种=" & InStr(1, mstr票据, 2), "领用人=" & str领用人, "领用ID=" & str领用ID)
End Sub
...
Private Sub mnuViewDetail_Click()
    ...
    Dim blnOne As Boolean, lng票种 As Long
    ...
    Call frmBillUses.ShowMe(Me, 0, mnuViewCheck.Checked, mblnNOMoved, lng票种, lng领用ID, str前缀, strCondition, lng原因, lng性质, str使用人, str提示)
End Sub
...
'功能:根据名称返回listsubitems列表的列号
    ...
    For lngCol = 2 To lvw领用_S.ColumnHeaders.Count 'ColumnHeaders的第一列是listitem的开始号码,listsubitems的第一列是从ColumnHeaders的第2列开始的
...
Private Sub mnuViewFilter_Click()
    ...
    If frmTimeSet.ShowMe(Me, 1, lngKind, mlngModul, mstrPrivs, mdatBegin, mdatEnd, mstrOperator, mblnDateMoved) = True Then
        Call Fill记录
    End If
End Sub
...
Public Sub ShowItem(lst As ListItem)
'修改或新增后,重新显示某一行,用于刷新
    Dim rsTmp As ADODB.Recordset
    Dim lngCol  As Long, varValue As Variant
    Dim int票种 As Integer, str类别 As String, strWhere As String
    Dim intIndex As Integer, str使用类别 As String
    
    On Error GoTo errHandle
    
    '新增的不会在后备表
    int票种 = Mid(lvwMain.SelectedItem.Key, 2)
    strWhere = ""
    Select Case int票种
    Case 1, 3
        str类别 = cbo类别.Text
        If str类别 = " " Then
              strWhere = strWhere & " And nvl(使用类别,'LXH')=[2]"
              str类别 = "LXH"
        ElseIf str类别 <> "所有类别" Then
              strWhere = strWhere & " And nvl(使用类别,'LXH')=[2]"
        End If
    Case 2    '1-收费收据,2-预交收据,3-结帐收据,4-挂号收据,5-就诊卡
        If cbo类别.ListIndex < 0 Then Exit Sub
        str类别 = cbo类别.ItemData(cbo类别.ListIndex)
        '58071
        If Val(str类别) <> -1 Then
            str类别 = Val(str类别)
            strWhere = strWhere & " And nvl(A.使用类别,'0')=[2]"
        End If
    Case 5
        If cbo类别.ListIndex < 0 Then Exit Sub
        str类别 = cbo类别.ItemData(cbo类别.ListIndex)
    ...
    "           A.领用人,A.前缀文本,A.开始号码,A.终止号码," & _
    "           Decode(A.使用方式,1,'自用','共用') as 使用方式,to_Char(A.登记时间,'YYYY-MM-DD') as 登记时间," & _
    "           A.登记人,A.当前号码,A.剩余数量,A.批次,A.核对人,A.签字人,to_Char(A.签字时间,'YYYY-MM-DD HH24:mi:ss') as 签字时间 " & _
    "   From  票据领用记录 A,人员表 B" & _
            IIf(int票种 = 5, ",医疗卡类别 M", "") & _
    "   Where A.ID=[1] " & IIf(mnuViewHave.Checked, " And A.剩余数量<>0", "") & strWhere & vbNewLine & _
    "           And A.领用人=B.姓名  " & _
            IIf(int票种 = 5, " And to_number(nvl(A.使用类别,'0'))=M.ID(+)", "")
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Mid(lst.Key, 2), str类别)
    If rsTmp.EOF Then
        With lvw领用_S
            intIndex = .SelectedItem.Index
            .ListItems.Remove .SelectedItem.Key
    ...
    For lngCol = 2 To lvw领用_S.ColumnHeaders.Count
        varValue = rsTmp(lvw领用_S.ColumnHeaders(lngCol).Text).Value
...
'功能:装入所有收费员到lvw领用_S_S
    Dim rsTmp As ADODB.Recordset
    Dim lst As ListItem, int票种 As Integer, strWhere As String
    ...
    "           Decode(A.使用方式,1,'自用','共用') as 使用方式,to_Char(A.登记时间,'YYYY-MM-DD') as 登记时间," & _
    "           A.登记人,A.当前号码,A.剩余数量,A.批次,A.核对人,A.签字人,to_Char(A.签字时间,'YYYY-MM-DD HH24:mi:ss') as 签字时间" & _
    "   From " & IIf(mblnDateMoved, zlGetFullFieldsTable("票据领用记录"), "票据领用记录 A") & " ,人员表 B" & _
            IIf(int票种 = 5, ",医疗卡类别 M", "") & _
    "   Where A.票种=[1] " & IIf(mnuViewHave.Checked, " And A.剩余数量<>0", "") & _
    "           And (B.站点='" & gstrNodeNo & "' Or B.站点 is Null)" & strWhere & vbNewLine & _
    "           And A.领用人=B.姓名 And A.登记时间 Between [2] And [3]" & IIf(mstrOperator = "", "", " And (A.领用人=[4] Or nvl(A.使用方式,0)=2)") & _
            IIf(int票种 = 5, " And to_number(nvl(A.使用类别,'0'))=M.ID(+)", "")


    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, int票种, mdatBegin, DateAdd("s", -1, DateAdd("d", 1, mdatEnd)), mstrOperator, UserInfo.姓名, str类别)
...
'功能:对票据使用情况汇总
    ...
    If mnuViewCheck.Checked Then
        gstrSQL = "" & _
        "Select 使用人, Sum(Decode(原因, 1, 1, 0)) As 正常, Sum(Decode(原因, 2, 1, 0)) As 作废," & vbNewLine & _
        "       Sum(Decode(原因, 3, 1, 0)) As 重打, Sum(Decode(原因, 4, 1, 0)) As 重打收回, Sum(Decode(原因, 5, 1, 0)) As 报损," & vbNewLine & _
        "       Sum(Decode(核对结果, 1, 1, 0)) As C正常, Sum(Decode(核对结果, 2, 1, 0)) As C作废," & vbNewLine & _
        "       Sum(Decode(核对结果, 3, 1, 0)) As C重打, Sum(Decode(核对结果, 4, 1, 0)) As C重打收回," & vbNewLine & _
    ...
    Else
        gstrSQL = "" & _
        " Select 使用人,sum(decode(原因,1,1,0)) as 正常,sum(decode(原因,2,1,0)) as 作废, " & _
        "       sum(decode(原因,3,1,0)) as 重打,sum(decode(原因,4,1,0)) as 重打收回,sum(decode(原因,5,1,0)) as 报损 " & _
        " From " & IIf(mblnDateMoved, zlGetFullFieldsTable("票据使用明细"), "票据使用明细") & _
        " Where 领用ID = [1] " & _
        " group by 使用人"
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Mid(lvw领用_S.SelectedItem.Key, 2))
...
'参数:bytMode=1 打印;2 预览;3 输出到EXCEL
    ...
    If ActiveControl Is msh汇总 Then
        ...
        objRow.Add "号码：" & lvw领用_S.SelectedItem.Text & "——" & lvw领用_S.SelectedItem.SubItems(GetItemCOL("终止号码"))
    ...
    Else
        ...
        objPrint.UnderAppItems.Add "领用时间：" & Format(mdatBegin, "yyyy年MM月dd日") & "——" & Format(mdatEnd, "yyyy年MM月dd日")
...
'功能:由于有的用户权限不够,故使一些菜单项或按钮不可见
    If InStr(mstrPrivs, "增删改") = 0 And InStr(mstrPrivs, "票据报损") = 0 And InStr(mstrPrivs, "票据核对") = 0 Then
    ...
    ElseIf InStr(mstrPrivs, "增删改") = 0 Then
        ...
        Toolbar1.Buttons("Delete").Visible = False
        Toolbar1.Buttons("Split1").Visible = False
    ElseIf InStr(mstrPrivs, "票据报损") = 0 And InStr(mstrPrivs, "票据核对") = 0 Then
        mnuBillSplit.Visible = False
...
Private Sub SetMenu()
    ...
    blnHavePrivs = IsHavePrivs(mstrPrivs, "允许操作他人登记票据")
    ...
    End If
    mnuBillCheck(1).Enabled = blnModify And blnDetail
