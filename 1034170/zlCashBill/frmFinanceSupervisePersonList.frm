...
Private Function LoadPerson(Optional blnFilter As Boolean = False) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If zlCheckPrivs(mstrPrivs, "收费员收款") = False Then LoadPerson = True: Exit Function
...
Private Function LoadOtherPerson(Optional blnFilter As Boolean = False) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    On Error GoTo errHandle
    If zlCheckPrivs(mstrPrivs, "其他人员收款") = False Then LoadOtherPerson = True: Exit Function
    '读取收费员信息
...
Private Function LoadGroup() As Boolean
   ...
   Dim str负责人 As String
    ...
    If zlCheckPrivs(mstrPrivs, "财务组收款") = False Then LoadGroup = True: Exit Function
    '读取财务组
    strSQL = " " & _
    "Select a.Id As 编码, a.组名称, a.简码, b.姓名 As 组负责人, a.负责人id, a.说明" & vbNewLine & _
    "From 财务缴款分组 A, 人员表 B" & vbNewLine & _
    "Where a.负责人id = b.Id And Nvl(a.删除日期, To_Date('3000-01-01', 'yyyy-mm-dd')) >= To_Date('3000-01-01', 'yyyy-mm-dd') And" & vbNewLine & _
    "      (b.撤档时间 Is Null Or b.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD')) And (b.站点 = 'A' Or b.站点 Is Null)" & vbNewLine & _
    "Union" & vbNewLine & _
    "Select a.Id As 编码, a.组名称, a.简码, b.姓名 As 组负责人, b.Id, a.说明" & vbNewLine & _
    "From 财务缴款分组 A, 人员表 B, 财务组组长构成 C" & vbNewLine & _
    "Where a.Id = c.组id And c.组长id = b.Id And" & vbNewLine & _
    "      Nvl(a.删除日期, To_Date('3000-01-01', 'yyyy-mm-dd')) >= To_Date('3000-01-01', 'yyyy-mm-dd') And" & vbNewLine & _
    "      (b.撤档时间 Is Null Or b.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD')) And (b.站点 = 'A' Or b.站点 Is Null)" & vbNewLine & _
    "Order By 组名称"
    ...
    With lvwGroup_S.ListItems
        ...
        Do While Not mrsGroup.EOF
            Set objItem = .Add(, "K" & Nvl(mrsGroup!编码) & "-" & Nvl(mrsGroup!负责人id), Nvl(mrsGroup!组名称) & "(" & Nvl(mrsGroup!组负责人) & ")", "Group", "Group")
...
Private Sub InitPage()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    mblnNotBrush = True
    picPersonList.Visible = zlCheckPrivs(mstrPrivs, "收费员收款")
     If zlCheckPrivs(mstrPrivs, "收费员收款") Then
        Set objItem = tbPage.InsertItem(EM_PG_收费员, "收费员", picPersonList.hWnd, 0)
        objItem.Tag = EM_PG_收费员
    End If
    
    picGroupList.Visible = zlCheckPrivs(mstrPrivs, "财务组收款")
     If zlCheckPrivs(mstrPrivs, "财务组收款") Then
        Set objItem = tbPage.InsertItem(EM_PG_财务组, "财务组", picGroupList.hWnd, 0)
        objItem.Tag = EM_PG_财务组
     End If
    picOtherList.Visible = zlCheckPrivs(mstrPrivs, "其他人员收款")
    If zlCheckPrivs(mstrPrivs, "其他人员收款") Then
