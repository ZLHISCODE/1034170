...
Begin VB.Form frmBillIn 
   ...
   Begin VB.PictureBox picBillIn 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsBillIn 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   -1  'True
         FormatString    =   $"frmBillIn.frx":1191
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin MSComctlLib.ImageList ils32 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBillIn.frx":215C
            Key             =   "Item"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBillIn.frx":25AE
            Key             =   "C2"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBillIn.frx":28C8
            Key             =   "C3"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBillIn.frx":2BE2
            Key             =   "C5"
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBillIn.frx":2EFC
            Key             =   "C1"
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBillIn.frx":3216
            Key             =   "C4"
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBillIn.frx":3530
            Key             =   "C6"
         EndProperty
      EndProperty
   End
...
Public Sub zlUpdateCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_NewItem     '增加入库
        Control.Visible = zlCheckPrivs(mstrPrivs, "增加票据")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_Modify     '修改入库
        Control.Visible = zlCheckPrivs(mstrPrivs, "修改票据")
        With vsBillIn
            If .Row > 0 Then
        ...
        End With
    Case conMenu_Edit_Delete      '删除入库
        Control.Visible = zlCheckPrivs(mstrPrivs, "删除票据")
        With vsBillIn
            If .Row > 0 Then
                Control.Enabled = Control.Visible And Val(.RowData(.Row)) > 0
            Else
        ...
        End With
    Case conMenu_Edit_DamnifyAdd      '增加报损
        Control.Visible = zlCheckPrivs(mstrPrivs, "增加报损")
        With vsBillIn
            If .Row > 0 Then
                Control.Enabled = Control.Visible And Val(.RowData(.Row)) > 0
            Else
        ...
        End With
    Case conMenu_Edit_DamnifyDelete      '删除报损
        Control.Visible = zlCheckPrivs(mstrPrivs, "删除报损")
...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    Dim blnHaveData As Boolean, int票种 As Integer
    ...
    Case conMenu_Edit_UserType
        If lvwMain.SelectedItem Is Nothing Then
             int票种 = 0
         Else
             int票种 = Val(Mid(lvwMain.SelectedItem.Key, 2))  '1-收费收据,2-预交收据,3-结帐收据,4-挂号收据,5-就诊卡
         End If
        Select Case int票种
        Case 1, 3   '收费,结帐
...
Private Sub Form_Unload(Cancel As Integer)
   ...
   zl_vsGrid_Para_Save mlngModul, vsBillIn, Me.Name, "入库列表", True, zlCheckPrivs(mstrPrivs, "参数设置")
   zl_vsGrid_Para_Save mlngModul, vsDrawBill, Me.Name, "领用列表", True, zlCheckPrivs(mstrPrivs, "参数设置")
   zl_vsGrid_Para_Save mlngModul, vsDamnifyBill, Me.Name, "报损列表", True, zlCheckPrivs(mstrPrivs, "参数设置")
...
Private Sub imgColDamnifyBill_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(picColDamnifyBill.hWnd)
    lngLeft = vRect.Left
...
Private Sub imgColDrawBill_Click()
    Dim lngLeft As Long, lngTop As Long
    Dim vRect  As RECT
    vRect = GetControlRect(picColDrawBill.hWnd)
    lngLeft = vRect.Left
...
Private Sub imgColIn_Click()
    Dim lngLeft As Long, lngTop As Long
    Dim vRect  As RECT
    vRect = GetControlRect(picImgIn.hWnd)
    lngLeft = vRect.Left
...
Private Function LoadInDataToRpt() As Boolean
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strWhere As String, int票种 As Integer, lngPreID As Long
    ...
    If int票种 = 5 Then
        gstrSQL = "" & _
        "  Select A.Id,nvl(B.名称,'就诊卡') as 使用类别, A.票种, A.前缀文本, A.开始号码,A.终止号码, A.入库数量," & _
        "       A.剩余数量, A.备注, A.登记人, A.登记时间  " & vbCrLf & _
    ...
    Else
        gstrSQL = "" & _
        "  Select A.Id,A.使用类别, A.票种, A.前缀文本, A.开始号码,A.终止号码, A.入库数量," & _
        "       A.剩余数量, A.备注, A.登记人, A.登记时间  " & vbCrLf & _
        "  From 票据入库记录 A" & vbCrLf & _
        "  where   " & Mid(strWhere, 5)
    End If
    ...
    With vsBillIn
        ...
        Do While Not rsTemp.EOF
            ...
            .TextMatrix(lngRow, .ColIndex("入库批次")) = Nvl(rsTemp!ID)
...
Public Sub LoadDamnifyData(ByVal lng入库批次 As Long)
    ...
    gstrSQL = "Select Id, 入库id, 开始号码, 终止号码, 数量, 报损原因, 报损人, 报损时间 From 票据报损记录 where 入库ID=[1]"
...
Private Sub ExcuteDamnifyFun(ByVal bytFun As Byte)
    ...
    End With
    If bytFun = 1 Then '新增
        If frmBillInDamnifyEdit.zlBillEdit(Me, EdS_报损, mstrPrivs, mlngModul, lngID, 0) = False Then Exit Sub
...
Private Sub DeleteBillIn()
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim lngRow As Long, lngID As Long
    Dim strExpended As String, int票种 As Integer, blnTrans As Boolean
    With vsBillIn
        If .Row <= 0 Then Exit Sub
        lngID = Val(.RowData(.Row))
        If lngID < 0 Then Exit Sub
        If MsgBox("你确认要删除" & lvwMain.SelectedItem.Text & "的批次为『" & .TextMatrix(.Row, .ColIndex("入库批次")) & "』;开始号码为“" & .TextMatrix(.Row, .ColIndex("开始票号")) & "”的" & _
...
Private Sub BillInModify()
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim lngID  As Long, strNO As String, rsTemp As ADODB.Recordset, lngLen As Long, int票种 As Integer
    With vsBillIn
        If .Row <= 0 Then Exit Sub
        lngID = Val(.RowData(.Row))
        If lngID < 0 Then Exit Sub
    ...
    End If
    int票种 = Val(Mid(lvwMain.SelectedItem.Key, 2))
    If zlCheckPrivs(mstrPrivs, "修改票据") = False Then
...
Private Sub BillInAdd()
    ...
    If zlCheckPrivs(mstrPrivs, "增加票据") = False Then Exit Sub
    
    int票种 = Val(Mid(lvwMain.SelectedItem.Key, 2))
    If int票种 = 1 Or int票种 = 3 Then
        If Not mcbrCmb Is Nothing Then str类别 = Trim(mcbrCmb.Text)
    ElseIf int票种 = 2 Or int票种 = 5 Then
