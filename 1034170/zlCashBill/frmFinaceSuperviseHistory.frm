...
Private Sub vsCollect_AfterMoveColumn(ByVal Col As Long, Position As Long)
    zl_vsGrid_Para_Save mlngModule, vsCollect, Me.Name, "收款信息列表", False, zlCheckPrivs(mstrPrivs, "参数设置")
End Sub
...
Private Function LoadDetail() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '加载轧帐信息列表
    strSQL = "" & _
    "   Select /*+ rule */a.Id,a.No As 轧帐单号,Decode(nvl(类别,0),0,'所有类别',1,'收费',2,'预交',21, '门诊预交', 22, '住院预交',3,'结帐',4,'挂号',5,'就诊卡',6,'消费卡','-') as 轧帐类别," & _
...
Public Sub RePrintBill()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If Not (zlCheckPrivs(mstrPrivs, "收款收据打印") And zlCheckPrivs(mstrPrivs, "重打收款收据")) Then Exit Sub
    With vsCollect
        If .Row < 1 Then Exit Sub
        strNO = Trim(.TextMatrix(.Row, .ColIndex("收款单号")))
        int记录性质 = Val(.TextMatrix(.Row, .ColIndex("记录性质")))
...
Private Sub imgColPlanCollect_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(picImgPlanCollect.hWnd)
    lngLeft = vRect.Left
