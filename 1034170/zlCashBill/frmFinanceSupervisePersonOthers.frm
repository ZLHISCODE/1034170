...
Private Sub SetPopedom()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '说明:
    '---------------------------------------------------------------------------------------------------------------------------------------------
    cmdOK.Visible = zlCheckPrivs(mstrPrivs, "其他人员收款") And zlCheckPrivs(mstrPrivs, "轧帐收款")
End Sub
...
Private Sub BillPrint(ByVal strNO As String)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    blnPrint = False
    If Not zlCheckPrivs(mstrPrivs, "收款收据打印") Then Exit Sub
...
Private Sub cmdRefresh_Click()
    If dtpEndDate - dtpStartDate > 14 Then
        If MsgBox("当前选择的操作员所设置的轧帐时间过长,提取轧帐数据可能需要一定的时间，你是否还要继续轧帐收款?", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) <> vbYes Then Exit Sub
    End If
    Call mobjChargeBill.LoadChargeAndBillTotalData(Me, mlngModule, mstrPrivs, 5, 0, dtpStartDate, dtpEndDate, False, , IIf(mstrPersonName = "", "-", mstrPersonName))
...
Public Sub ShowChargeList(ByVal frmMain As Object)
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Load frmNew
   Call frmNew.ShowMe(frmMain, mlngModule, mstrPrivs, 5, "", dtpStartDate.Value, dtpEndDate.Value, False, mstrPersonName)
