...
Public Function zlBillEdit(ByVal frmMain As Form, ByVal EditType As EditDamnifyType, ByVal strPrivs As String, _
    ByVal lngModule As Long, ByVal lng入库ID As Long, Optional lng报损ID As Long = 0) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    mEditType = EditType: mstrPrivs = strPrivs: mlngModule = lngModule: mlng入库ID = lng入库ID: mlng报损ID = lng报损ID
    mintSucceed = False
    Me.Show 1, frmMain
...
Private Function LoadCardData() As Boolean
    ...
    If mEditType <> EdS_报损 Then
        gstrSQL = "Select Id, 入库id, 开始号码, 终止号码, 数量, 报损原因, 报损人, 报损时间 From 票据报损记录 where id=[1]"
    ...
    gstrSQL = "" & _
    "   Select Id, 票种, 前缀文本, 开始号码, 终止号码, 入库数量, 剩余数量, 备注, 登记人, 登记时间  " & _
    "   From 票据入库记录 " & _
    "   Where Id=[1]"
...
Private Sub cmdAdd_Click()
    ...
    zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损开始)
    Exit Sub
errHandle:
...
Private Function CheckInputValied() As Boolean
    ...
    If Trim(txtEdit(mTxtIdx.idx_报损开始).Text) = "" And Trim(txtEdit(mTxtIdx.idx_报损结束).Text) = "" Then
        ShowMsgbox "注意" & vbCrLf & "    报损范围中的开始号码或结束号码必须输入,请检查!"
        zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损开始): Exit Function
    End If
    lngLen = Len(txtEdit(mTxtIdx.idx_开始前缀))
    strTemp = Mid(txtEdit(mTxtIdx.idx_报损开始).Text, lngLen + 1)
    If strTemp <> "" Then
        If zlIsOnlyNum(strTemp) = False Then
            MsgBox "报损范围中的开始号码中含有非数字字符，字母只能作为前缀。", vbExclamation, gstrSysName
            zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损开始): Exit Function
        End If
    End If
    ...
    If strTemp <> "" Then
        If zlIsOnlyNum(strTemp) = False Then
                MsgBox "报损范围中的终止号码中含有非数字字符，字母只能作为前缀。", vbExclamation, gstrSysName
                zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损结束): Exit Function
        End If
    End If
    ...
    If txtEdit(mTxtIdx.idx_报损开始).Text <> "" Then
        If zlcommfun.actuallen(txtEdit(mTxtIdx.idx_报损开始).Text) <> mlng长度 Then
            ShowMsgbox "注意" & vbCrLf & "    报损范围中的开始号码长度不对(应为" & mlng长度 & "位()),请检查!"
            zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损开始): Exit Function
        End If
    End If
    If txtEdit(mTxtIdx.idx_报损结束).Text <> "" Then
        If zlcommfun.actuallen(txtEdit(mTxtIdx.idx_报损结束).Text) <> mlng长度 Then
            ShowMsgbox "注意" & vbCrLf & "    报损范围中的结束号码长度不对(应为" & mlng长度 & "位),请检查!"
            zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损结束): Exit Function
        End If
        If txtEdit(mTxtIdx.idx_报损结束).Text < txtEdit(mTxtIdx.idx_报损开始) _
            ...
            ShowMsgbox "注意" & vbCrLf & "    报损范围中的结束号码小于了开始号码,请检查!"
            zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损结束): Exit Function
        End If
    End If
    '检查是否包含在网格中了
             ...
             If .TextMatrix(i, .ColIndex("报损票号")) <> "" Then
                ...
                If varTemp(0) <> "" And varTemp(1) <> "" Then
                    If strStartNo >= varTemp(0) And strStartNo <= varTemp(1) Then
                        ShowMsgbox "注意" & vbCrLf & "    报损范围中的开始号码已经包含在了第" & i & "行中了,请检查!"
                        .Row = i
                    ...
                    If strEndNo >= varTemp(0) And strEndNo <= varTemp(1) And strEndNo <> "" Then
                        ShowMsgbox "注意" & vbCrLf & "    报损范围中的结束号码已经包含在了第" & i & "行中了,请检查!"
                        .Row = i
                        If .RowIsVisible(.Row) = False Then Call .ShowCell(.Row, .Col)
                        zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损结束): Exit Function
                    End If
    ...
    "   From 票据领用记录 " & _
    "   Where (([1]  between 开始号码  and 终止号码  ) or ([2] between 开始号码  and 终止号码  )) and 批次=[3] and 票种=[4] " & _
    "   "
        ...
        If Not rsTemp.EOF Then
            ...
            Loop
            If str报损 <> "" Then
                ShowMsgbox "注意:" & vbCrLf & "    当前报损范围中的票号已经被报损,已报损的号码如下:" & vbCrLf & str报损
                zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损开始): Exit Function
                Exit Function
            End If
            If str领用 <> "" Then
                ShowMsgbox "注意:" & vbCrLf & "    当前报损范围中的票号已经被领用,已领用的号码如下:" & vbCrLf & str领用
                zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损开始): Exit Function
                Exit Function
            End If
        End If
...
Private Function IsValied() As Boolean
    ...
    If zlcommfun.actuallen(Trim(txtEdit(mTxtIdx.idx_报损原因))) > 200 Then
        ...
        zl_CtlSetFocus txtEdit(mTxtIdx.idx_报损原因): Exit Function
    End If
...
'                        Loop
'                        If str报损 <> "" Then
'                            ShowMsgbox "注意:" & vbCrLf & "    在第2行记录中包含了已经包含报损的票号,已报损的号码如下:" & vbCrLf & str报损
'                            Exit Function
'                        End If
'                        If str领用 <> "" Then
'                            ShowMsgbox "注意:" & vbCrLf & "    在第2行记录中包含了已经领用的票号,已领用的号码如下:" & vbCrLf & str领用
'                            Exit Function
'                        End If
'                    End If
            End If
    ...
    If Not blnHaveData Then
        ShowMsgbox "注意:" & vbCrLf & "    你没有选择要报损的票据,不能继续!"
        Exit Function
    End If
    IsValied = True
    Exit Function
    ...
    If Index = mTxtIdx.idx_报损开始 Or Index = mTxtIdx.idx_报损结束 Then
        ...
        If Len(txtEdit(Index)) < lngLen Then
            If zlIsOnlyNum(strTemp) Then
                strTemp = str前缀 & Lpad(strTemp, lngLen, "0")
            ElseIf UCase(Mid(strTemp, 1, lng前缀Len)) = str前缀 Then
                  strTemp = str前缀 & Lpad(Mid(strTemp, lng前缀Len + 1), lngLen, "0")
            Else
                strNum = ""
                For i = 1 To Len(strTemp)
                    ...
                    End If
                Next
                strTemp = str前缀 & Lpad(strNum, lngLen, "0")
            End If
        ElseIf UCase(Mid(strTemp, 1, lng前缀Len)) = str前缀 Then
