...
Private Sub cboOperator_KeyPress(KeyAscii As Integer)
   Dim lngIdx As Long, lng医生ID As Long
    ...
    If zlPersonSelect(Me, mlngModule, cboOperator, mrsPerson, cboOperator.Text, True, strAllCaption) = False Then
        KeyAscii = 0: Exit Sub
    End If
    Exit Sub
...
'
'    If KeyAscii >= 32 Then
'        lngIdx = zlControl.CboMatchIndex(cboOperator.hwnd, KeyAscii, 0.8)
'        If lngIdx = -1 And cboOperator.ListCount > 0 Then lngIdx = 0
'        cboOperator.ListIndex = lngIdx
'    End If
End Sub
...
Public Function ShowMe(ByVal frmOwner As Form, ByVal bytInFun As Byte, ByVal bytInvoiceKind As Byte, ByVal lngModule As Long, ByVal strPrivs As String, _
    datBegin As Date, datEnd As Date, strOperator As String, blnDateMoved As Boolean, Optional strPersonelKind As String, Optional blnOnlyHave As Boolean) As Boolean
'参数：
'    bytInFun:0-收费缴款过滤，1-票据使用过滤
'    bytInvoiceKind:当bytInFun=1时，1-收费收据,2-预交收据,3-结帐收据,4-挂号收据,5-就诊卡
...
Private Sub Fill缴款人(strOperator As String, strPersonelKind As String, blnOnlyHave As Boolean)
    ...
    If blnOnlyHave Then
        '在指点定期间内有暂存金的操作员
        gstrSQL = "Select Distinct B.ID,B.编号, B.姓名,B.简码 From 人员缴款余额 A,人员表 B,人员性质说明 C " & vbNewLine & _
            "Where A.收款员=B.姓名 And 余额<>0 And (B.站点='" & gstrNodeNo & "' Or B.站点 is Null) And B.id=C.人员ID" & gstrSQL & vbNewLine & _
            "Order by 姓名"
    Else
        '所有期间内操作员
        gstrSQL = "Select Distinct A.ID,A.编号, A.姓名,A.简码  From 人员表 A,人员性质说明 C " & vbNewLine & _
    ...
    Set mrsPerson = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, strPersonelKind)
    cboOperator.Clear
...
Private Sub FillOperator(ByVal bytInvoiceKind As Byte)
    Dim strSQL As String
    Dim strValue As String, i As Long, strID As Long
    
    If InStr(mstrPrivs, "所有操作员") = 0 Then
        cboOperator.Clear
        strSQL = "Select distinct A.ID, A.编号, A.姓名,A.简码   From 人员表 A Where A.编号 =[1] " & _
                " And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & vbNewLine & _
                " And (A.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or A.撤档时间 Is Null)"
        On Error GoTo errH
        Set mrsPerson = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, UserInfo.编号)
        cboOperator.AddItem UserInfo.编号 & "-" & UserInfo.姓名
        cboOperator.ListIndex = 0
        If mrsPerson.EOF = False Then
            cboOperator.ItemData(cboOperator.NewIndex) = Val(Nvl(mrsPerson!ID))
        End If
    
    Else
        If bytInvoiceKind > 0 And bytInvoiceKind <= 5 Then
            '如果是入院登记员，则需要同时设置对应的发卡或预交人员属性这里才显示，病人信息管理同样也有这两项功能了
            strValue = Choose(bytInvoiceKind, "门诊收费员", "预交收款员", "住院结帐员", "门诊挂号员", "发卡登记人")
        End If
        strSQL = "Select distinct A.ID, A.编号, A.姓名,A.简码   From 人员表 A, 人员性质说明 B Where A.ID = B.人员id And B.人员性质 = [1] " & _
                " And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & vbNewLine & _
                " And (A.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or A.撤档时间 Is Null)"
