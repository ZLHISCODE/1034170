...
Begin VB.Form frmChargeRollingCurtainSet 
   ...
   Begin VB.CheckBox chkDepositType 
      Caption         =   "预交轧帐按门诊和住院分别轧帐"
      Height          =   345
      Left            =   645
      TabIndex        =   7
      Top             =   3285
...
Private Sub LoadPara()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    chkDepositType.Value = Val(zlDatabase.GetPara("预交轧帐按门诊或住院分别轧帐", glngSys, mlngModule, 0, chkDepositType, InStr(1, mstrPrivs, ";参数设置;") > 0))
End Sub
...
Private Sub SavePara()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    End If
    Call zlDatabase.SetPara("预交轧帐按门诊或住院分别轧帐", chkDepositType.Value, glngSys, mlngModule, InStr(1, mstrPrivs, ";参数设置;") > 0)
End Sub
