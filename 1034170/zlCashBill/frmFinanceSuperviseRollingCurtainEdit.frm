    ...
    With vsBalance
        ...
        Do While Not mrsBalance.EOF
            ...
            If Val(Nvl(mrsBalance!金额)) <> 0 _
                ...
                .TextMatrix(lngRow, .ColIndex("金额")) = Format(Val(.TextMatrix(lngRow, .ColIndex("金额"))) + Val(Nvl(mrsBalance!金额)), "0.00")
...
Private Function isValied(ByVal strSel轧帐IDs As String) As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If zlCommFun.ActualLen(txtMemo.Text) > 500 Then
        MsgBox "摘要超长,最多只能输入500个字符或250个汉字", vbInformation, gstrSysName
