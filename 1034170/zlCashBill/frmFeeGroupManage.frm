...
Private mcbrPopupMain As CommandBar, mcbrMenuView As CommandBarPopup
...
Private mfrmFeeGroupCollectFee As Form, mfrmFeeGroupRollingCurtain As Form  '收款和轧帐子页面
...
Private Sub ChangeListViewType(intType As Integer)
    ...
    Set cbrListView = mfrmFeeGroupCollectFee.GetListViewMenu
    Select Case intType
        Case 1
...
Private Sub ButtonCollectFees()
    ...
    End With
    With mfrmFeeGroupCollectFee.lvwSubWorker
...
Public Sub AutoPrint(ByVal lngID As Long, ByVal strNO As String, ByVal intType As Integer)
    ...
    '打印收款收据
    If intType = 1 Then
        If zlCheckPrivs(mstrPrivs, "人员缴款收据") = False Then Exit Sub
    ...
    '打印缴款书
    If intType = 2 Then
        If zlCheckPrivs(mstrPrivs, "缴款书打印") = False Then Exit Sub
...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        Case conMenu_Edit_CollectFees '收款按钮
            If zlCheckPrivs(mstrPrivs, "收款") = False Then Exit Sub
        ...
        Case conMenu_Edit_RollingCurtain '轧帐按钮
            If zlCheckPrivs(mstrPrivs, "轧帐") = False Then Exit Sub
        ...
        Case conMenu_Edit_CollectFees_Cancel '收款作废按钮
            If zlCheckPrivs(mstrPrivs, "收款作废") = False Then Exit Sub
            If tabMain.Selected.Index = 1 Then
                Control.Visible = True
                With mfrmFeeGroupRollingCurtain.vsCollectHistory
        ...
        Case conMenu_Edit_RollingCurtain_Cancel '轧帐作废按钮
            If zlCheckPrivs(mstrPrivs, "轧帐作废") = False Then Exit Sub
        ...
        Case conMenu_Edit_ReprintReceipt    '重打收据按钮
            If zlCheckPrivs(mstrPrivs, "人员缴款收据") = False Then Exit Sub
            If zlCheckPrivs(mstrPrivs, "重打收据") = False Then Exit Sub
            Select Case tabMain.Selected.Index
                Case 0
                    Control.Visible = False
                    Control.Enabled = False
        ...
        Case conMenu_Edit_ChargeBook_Reprint '重打缴款书按钮
            If zlCheckPrivs(mstrPrivs, "缴款书打印") = False Then Exit Sub
            If zlCheckPrivs(mstrPrivs, "重打缴款书") = False Then Exit Sub
            If tabMain.Selected.Index = 2 Then
                Control.Visible = True
                With vsRollingCurtainHistory
...
Public Function zlDefCommandBars() As Boolean
    ...
    With mcbrMenuBar.CommandBar.Controls
        ...
        mcbrControl.Visible = zlCheckPrivs(mstrPrivs, "重打收据") And zlCheckPrivs(mstrPrivs, "人员缴款收据")
        Set mcbrControl = .Add(xtpControlButton, conMenu_Edit_ChargeBook_Reprint, "重打缴款书(&R)")
        mcbrControl.Visible = zlCheckPrivs(mstrPrivs, "重打缴款书") And zlCheckPrivs(mstrPrivs, "缴款书打印")
    End With
...
Private Sub vsRollingCurtainHistory_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    With vsCollectorDetail
        .Rows = 1
        strSQL = "" & _
        "Select No, decode(nvl(类别,0),0,'所有类别',1,'收费',2,'预交',21, '门诊预交', 22, '住院预交',3,'结帐',4,'挂号',5,'就诊卡',6,'消费卡','-') as 轧帐类别," & _
...
Private Sub imgColPlanHistory_Click()
    ...
    Dim vRect  As RECT
    vRect = GetControlRect(picImgPlanHistory.hWnd)
    lngLeft = vRect.Left
...
Private Sub imgColPlanGroup_Click()
    Dim lngLeft As Long, lngTop As Long
    Dim vRect  As RECT
    vRect = GetControlRect(picImgPlanGroup.hWnd)
    lngLeft = vRect.Left
