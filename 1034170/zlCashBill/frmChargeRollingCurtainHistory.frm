...
Private Function LoadHistoryData() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    strSQL = "" & _
    "   Select /*+ rule */a.Id,a.No As 轧帐单号,A.类别, a.开始时间, a.终止时间, a.登记人 As 轧帐人, a.登记时间 As 轧帐时间,  " & _
    ...
    With vsRollingCurtain
        ...
        Do While Not rsTemp.EOF
            ...
            .TextMatrix(.Rows - 1, .ColIndex("轧帐类别")) = _
                Decode(Val(Nvl(rsTemp!类别)), 0, "所有类别", 1, "收费", 2, "预交", 21, "门诊预交", 22, "住院预交", 3, "结帐", 4, "挂号", 5, "就诊卡", 6, "消费卡", "")
            .Cell(flexcpData, .Rows - 1, .ColIndex("轧帐类别")) = Val(Nvl(rsTemp!类别))
...
Private Function CheckCancelValied() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    With vsRollingCurtain
        ...
        strNO = Val(.TextMatrix(.Row, .ColIndex("轧帐单号")))
        blnDel = Trim(.TextMatrix(.Row, .ColIndex("作废时间"))) <> ""
        lng轧帐类别 = Val(.Cell(flexcpData, .Row, .ColIndex("轧帐类别")))
    End With
    ...
    "               And ID+0<>[1] And instr([3],','|| nvl(类别,0)||',')>0 and 记录性质=1  and 收款员=[2]  And 作废时间 Is Null"
    'lng轧帐类别:0-所有类别(按全额轧帐),1-收费,2-预交,3-结帐,4-挂号,5-就诊卡,6-消费卡
    strRollingType = ",0," & lng轧帐类别 & ","
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng轧帐ID, UserInfo.姓名, strRollingType)
...
Public Sub RePrintBill()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strNO As String
    If Not (zlCheckPrivs(mstrPrivs, "缴款书打印") And zlCheckPrivs(mstrPrivs, "重打缴款书")) Then Exit Sub
    With vsRollingCurtain
        If .Row < 1 Then Exit Sub
