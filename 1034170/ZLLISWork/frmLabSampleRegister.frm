...
Private mblnPosition As Boolean                                              '是否需要定位病人，从查询框查询医嘱时需要定位病人
...
'    strSQL = strSQL & " union all " & strSQLbak
    ...
    strSQL = strSQL & IIf(intBabyNo <> 0, " and nvl(a.婴儿,0)= " & intBabyNo, IIf(blOnlyWhere, "", "and nvl(a.婴儿,0) = 0 ")) & " Order By 样本条码, 试管编码, Nvl(b.相关id, b.id), 医嘱id, 标本, 开嘱时间 "
    ...
    If rsTmp.RecordCount > 0 Then
        ...
        '通过文本框查询定位病人
        If mblnPosition Then
            For Each rptRow In rptPlist.Rows
                If Val(rptRow.Record(mPcol.病人ID).Value) = Val(rsTmp("病人ID") & "") And Trim(rsTmp("病人来源") & "") = Trim(rptRow.Record(mPcol.来源).Value) Then
                    Set rptPlist.FocusedRow = rptRow
                    mblnPosition = False
                    Exit For
                End If
            Next
...
Private Function SaveRegister(intState As Integer) As Boolean
    ...
    With Me.rptAlist(TabCtr.Selected.Index)
        For intLoop = 0 To .Rows.Count - 1
            If .Rows(intLoop).Record(mAcol.选择).Checked = True And .Rows(intLoop).Record(mAcol.执行状态).Value = 0 Then
                '相同条码不同检验项目，其中一个核收后，不能再对其他未核收项目登记
                blnExit = False
                If .Rows(intLoop).Record(mAcol.条码).Value <> "" And intState = 0 Then
                    gstrSql = "Select b.执行状态, b.采样人, b.采样时间, b.接收人, b.接收时间, b.标本送出时间, a.医嘱内容" & vbNewLine & _
                            "From 病人医嘱记录 A, 病人医嘱发送 B" & vbNewLine & _
                            "Where a.Id = b.医嘱id And a.相关id Is Not Null And b.样本条码 = [1]" & vbNewLine & _
                            "Order By a.开嘱时间 Desc"
                    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, CStr(.Rows(intLoop).Record(mAcol.条码).Value))
                    If rsTmp.RecordCount > 1 Then
                        Do While Not rsTmp.EOF
                            If rsTmp("执行状态") = 1 Or rsTmp("执行状态") = 3 Then
                                strMsg = strMsg & "具有相同条码的其他医嘱已被核收，《" & .Rows(intLoop).Record(mAcol.医嘱内容).Value & "》不允许登记！" & vbNewLine
                                blnExit = True
                                Exit Do
                            End If
                            rsTmp.MoveNext
            ...
            End If
        Next
        If strMsg <> "" Then MsgBox strMsg, vbInformation, "标本登记"
    End With
    ...
    If strAdvice <> "" Then
        ...
        For intLoop = 0 To UBound(strArr)
            strSQL = "Zl_病人医嘱发送_SampleInput('" & strArr(intLoop)
            ...
            End If
            arrSql(intLoop) = strSQL
        Next
        
        gcnOracle.BeginTrans
        blnTran = True
        For intLoop = 0 To UBound(arrSql)
            If arrSql(intLoop) <> "" Then
                zlDatabase.ExecuteProcedure arrSql(intLoop), gstrSysName
            End If
        Next
        ...
        '登记信息写入LIS
        If intState = 0 Or intState = 3 Or intState = 4 Then
            Call WriterCheckSampleToLIS(strAdvice, UserInfo.姓名, mlngBatch, Trim(Me.txt送检人.Text), strErr)
        ElseIf intState = 1 Then
            Call WriterCheckSampleToLIS(strAdvice, "", 0, "", strErr)
        End If
        
        '写入LIS报错，取消之前的登记或取消登记
        If strErr <> "" Then
            For intLoop = 0 To UBound(strArr)
                strSQL = "Zl_病人医嘱发送_SampleInput('" & strArr(intLoop)
                If intState = 0 Or intState = 3 Or intState = 4 Then
...
Private Sub GetBatch()
    ...
    strSQL = "Select /*+ RULE */ distinct b.接收批次 " & vbCrLf & _
             " From 病人医嘱记录 A, 病人医嘱发送 B, 病人信息 C, 部门表 E, 诊疗项目目录 F,病人挂号记录 G,病人医嘱记录 H, " & vbCrLf & _
             "      诊疗项目目录 K ,病人医嘱发送 M" & vbCrLf & _
             " Where A.ID = H.相关ID  And A.病人id = C.病人id And A.病人科室id = E.ID And A.诊疗项目id = f.ID " & vbCrLf & _
             "      And h.诊疗项目ID = k.id " & vbCrLf & _
             " And A.挂号单 = G.No(+) and (g.病人ID is null or (g.记录状态 =1 and g.记录性质 =1) ) and a.诊疗类别 = 'E' and F.操作类型 = '6'  and b.执行部门ID + 0 = [1] " & vbCrLf & _
             " And A.ID = M.医嘱ID And k.试管编码 is not null  and b.接收人 is not null "
...
Private Sub WriterCheckSampleToLIS(strAdvices As String, strName As String, strBatchNO As Long, Optional ByVal strSentName As String, Optional strError As String)
    '功能   把签收信息写入LIS
    Dim strErr As String
    If Not mobjLisInsideComm Is Nothing Then
