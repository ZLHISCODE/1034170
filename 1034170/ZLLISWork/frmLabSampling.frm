...
Private Sub RefreshPatientData()
    ...
    strSQL = strSQL & "Select    distinct a.病人id,decode(a.病人来源,1,'门诊',2,'住院',3,'院外',4,'体检') as 病人来源, " & vbCrLf & _
             ...
             " decode(b.执行状态,0,'',2,'拒收') as 拒收,b.执行状态,nvl(b.重采标本,0) as 重采标本,b.发送时间,nvl(l.路径状态,0) as 临床路径病人,a.医嘱内容 " & vbCrLf & _
             " From 病人医嘱记录 A, 病人医嘱发送 B, 病人信息 C, 部门表 E, 诊疗项目目录 F,病人挂号记录 G,病人医嘱记录 H, " & vbCrLf & _
             "      诊疗项目目录 K ,病案主页 L,病人医嘱发送 M,检验标本记录 J " & vbCrLf & _
             ...
             " And A.挂号单 = G.No(+) and a.病人id = g.病人id(+)  and a.病人id = g.病人id(+)  and (g.病人ID is null or (g.记录状态 =1 and g.记录性质 =1) ) And  a.诊疗类别 = 'E' And f.操作类型 = '6' and   a.病人id = l.病人ID(+) and a.主页ID = l.主页ID(+) and  m.执行部门id = [1] " & vbCrLf & _
             " And A.ID = M.医嘱ID And k.试管编码 is not null " & IIf(Me.rptPlist.Tag = "", "and a.开始执行时间 < to_date('" & Format(NowDate, "yyyy-mm-dd 23:59:59") & "','yyyy-mm-dd hh24:mi:ss')", "")
...
Private Function GetBooldPatientDataSql() As String
    ...
    strBloodSQL = strBloodSQL & "Select    distinct a.病人id,decode(a.病人来源,1,'门诊',2,'住院',3,'院外',4,'体检') as 病人来源, " & vbCrLf & _
                ...
                " Where A.ID = H.相关ID And H.id = B.医嘱id And A.病人id = C.病人id And A.病人科室id = E.ID And A.诊疗项目id+0 = f.ID  " & vbCrLf & _
                "      And h.诊疗项目ID = k.id and a.id = j.医嘱id(+)  " & vbCrLf & _
                " And A.挂号单 = G.No(+) and a.病人id = g.病人id(+)  and a.病人id = g.病人id(+)  and (g.病人ID is null or (g.记录状态 =1 and g.记录性质 =1) ) And a.诊疗类别 = 'K'  and  k.操作类型='9' and a.病人id = l.病人ID(+) and a.主页ID = l.主页ID(+) and b.执行部门id = [1] " & vbCrLf & _
                " And A.ID = M.医嘱ID And k.试管编码 is not null " & IIf(Me.rptPlist.Tag = "", "and a.开始执行时间 < to_date('" & Format(NowDate, "yyyy-mm-dd 23:59:59") & "','yyyy-mm-dd hh24:mi:ss')", "")
...
Private Function GetBloodAdviceSql(intState As Integer, intPatientType As Integer, Optional blOnlyWhere As Boolean = False, Optional intPatientPage As Integer) As String
    ...
    Else
        strSQL = strSQL & " and Nvl(a.主页id, 0) = 0 "
    End If
    ...
    If blOnlyWhere = True Then
        ...
        '条码
        If BlnIsNumber(txtGoto) And (IDKind.IDKind = IDKind.GetKindIndex("姓名") Or IDKind.IDKind = IDKind.GetKindIndex("就诊卡")) Then
...
'        End If
    ...
    Do Until rsTmp.EOF
        ...
        If InStr(mstrPrivs, "显示划价记录") <= 0 And Nvl(rsTmp("记录状态"), "NULL") <> "NULL" Then
            ...
            End If
        ElseIf InStr(mstrPrivs, "显示划价记录") <= 0 And Nvl(rsTmp("记录状态"), "NULL") = "NULL" Then
            If Nvl(rsTmp("门诊标志"), 1) = 1 Then
                '只处理门诊病人
                If Nvl(rsTmp("计费状态"), 0) = 0 Then blnShowExec = False
            End If
        Else
            '退费的项目也不显示
            If Nvl(rsTmp("记录状态"), 0) > 1 Then blnShowExec = False
...
Private Function FindPatient(strFind As String) As Boolean
    ...
    For intLoop = 0 To 2
        For Each Row In Me.rptPlist.Rows '门诊号、住院号、病人id、姓名、就诊卡号、条码
            If (Trim(strFind) = "*" & Trim(Row.Record(mPcol.标识号).Value) And "门诊" = Trim(Row.Record(mPcol.来源).Value)) Or _
                (Trim(IDKind.IDKind) = Trim(IDKind.GetKindIndex("门诊号")) And Trim(strFind) = Trim(Row.Record(mPcol.标识号).Value) And "门诊" = Trim(Row.Record(mPcol.来源).Value)) Or _
...
Private Sub InitRecordSet(rsNumber As ADODB.Recordset)
    ...
    rsNumber.Fields.Append "婴儿姓名", adVarChar, 200
...
Private Sub FilterPatient()
    ...
    If rptPlist.FocusedRow Is Nothing And rptPlist.Rows.Count > 0 Then
        Set rptPlist.FocusedRow = rptPlist.Rows(0)
    End If
End Sub
