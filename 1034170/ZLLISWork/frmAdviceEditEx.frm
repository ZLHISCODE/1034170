...
Private mlngDeptID As Long              '诊疗项目使用科室id
...
Public Function ShowMe(ByVal frmParent As Object, ByVal lngHwnd As Long, ByRef t_Pati As TYPE_PatiInfoEx, ByVal int场合 As Integer, _
            ...
            Optional ByRef strAppend As String, Optional ByVal strAdvItem As String, Optional ByVal strDiagnosis As String, Optional ByRef str手术部位 As String, _
            Optional ByVal lngDeptID As Long) As Boolean
'参数:
'     frmParent         父窗体
...
'功能：打开项目选择器
    ...
    If mintType = 1 Then
        ...
        strSQLItem = _
            " From 诊疗项目目录 A Where A.类别='F' And A.ID<>-1*" & mlng项目ID & _
                ...
                IIf(mlngDeptID <> 0, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[4]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))", "") & _
        ...
        Set rsTmp = zlDatabase.ShowSQLSelect(Me, strsql, 2, "手术", False, "", "", False, True, False, 0, 0, 0, blnCancel, False, False, _
            mint服务对象, IIf(mint期效 = 0, 2, 1), int性别, mlngDeptID)
    ...
    ElseIf mintType = 4 Then
        ...
        strsql = "Select 0 as 末级,名称 as ID,-Null as 上级ID,编码,名称,' ' as 检验类型,' ' As 标本部位,NULL as 试管编码 From 诊疗检验类型" & _
            ...
            " And (A.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or A.撤档时间 IS NULL)" & _
            " And (A.站点='" & gstrNodeNo & "' Or A.站点 is Null)" & _
            IIf(mlngDeptID <> 0, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[3]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))", "") & _
...
Private Function CheckDocEmpowerEx(ByVal lng诊疗项目ID As Long, ByVal strAppend As String) As Boolean
    ...
    ElseIf mintType = 1 Or mintType = 4 Then '附加手术及麻醉项目；检验项目及标本
        ...
        If mintType = 1 And mbln手术分级管理 Then
            ...
            If Decode(str手术类型, "丁", 1, "丙", 2, "乙", 3, "甲", 4, "一级", 1, "二级", 2, "三级", 3, "四级", 4, 0) > _
                Decode(mstr手术等级, "丁", 1, "丙", 2, "乙", 3, "甲", 4, "一级", 1, "二级", 2, "三级", 3, "四级", 4, 0) Then
                blnMsg = True
            End If
        End If
...
'参数：mstrExtData=包含附加手术及麻醉项目的信息,其中可能没有附加手术；为空时表示新输入手术项目
    ...
    If str手术IDs <> "" Then
        strsql = "Select /*+ Rule*/ A.ID,A.编码,A.名称,A.操作类型" & _
            ...
            IIf(mlngDeptID <> 0, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[2]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))", "") & _
            " Order by A.编码"
        Set rsTmp = zlDatabase.OpenSQLRecord(strsql, Me.Caption, str手术IDs, mlngDeptID)
        i = rsTmp.RecordCount
    End If
    ...
    If lng麻醉ID <> 0 Then
        strsql = "Select A.ID,A.编码,A.名称,操作类型 From 诊疗项目目录 A Where A.类别='G' And A.ID=[1]" & _
                IIf(mlngDeptID <> 0, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[2]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))", "")
        Set rsTmp = zlDatabase.OpenSQLRecord(strsql, Me.Caption, lng麻醉ID, mlngDeptID)
...
'返回：对应的单据定义了早请附项时返回True
    ...
    strsql = "Select C.项目,C.内容,C.要素ID,C.必填,d.中文名" & _
...
'功能：获取指定的申请附项值
    ...
    '1.如果有对应要素，从要素提取函数读取
    If lng要素ID <> 0 Then
        strText = GetOrderInspectInfo(mlng病人ID, str中文名)
        If strText = "" Then ' 要素没有提取到再从老版中提取
...
'功能：初始化检验项目
    ...
    If strItems <> "" Then
        ...
        strsql = "Select * From (With 检验组合项目 As (Select /*+ Rule*/ A.ID,A.编码,A.名称,A.操作类型,A.试管编码, a.编码 As 序号,null as 父ID  From 诊疗项目目录 A " & _
            ...
            strLIS & _
            ") Order by 序号,编码"
        Set rsTmp = zlDatabase.OpenSQLRecord(strsql, Me.Caption, strItems, mlngDeptID)
    End If
...
Private Function InitCombox(Optional ByVal strNewItemID As String = "", Optional ByVal DefaultValue As String = "") As Boolean
    ...
    Else
        strsql = "Select /*+ Rule*/ 标本类型,Sum(1) From (" & _
            ...
            "   Where A.ID=D.诊疗项目ID(+) And D.报告项目ID=C.项目ID(+)" & _
            "       And (C.标本类型 Is Null Or C.标本类型=B.名称)" & _
            "       And A.ID In (Select Column_Value From Table(Cast(f_Num2list([1]) As zlTools.t_Numlist)))" & _
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strsql, Me.Name, strTmp, lngItemCount, mstr性别, mlngDeptID)
    End If
    If rsTmp.EOF Then
        ...
        If mintType = 1 Then
            ...
            strsql = _
                ...
                " Where A.ID=B.诊疗项目ID And A.类别='G' And A.服务对象 IN([3],3)" & _
                    ...
                    IIf(mlngDeptID <> 0, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[5]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))", "") & _
                " Order by A.编码"
            ...
            Set rsTmp = zlDatabase.ShowSQLSelect(Me, strsql, 0, "麻醉项目", False, "", "", False, False, True, vRect.Left, vRect.Top, txtData.Height, blnCancel, False, True, _
                UCase(txtData.Text) & "%", strLike & UCase(txtData.Text) & "%", mint服务对象, mint简码 + 1, mlngDeptID)
            If rsTmp Is Nothing Then
                If Not blnCancel Then
                    MsgBox "未找到匹配项目！", vbInformation, gstrSysName
                End If
...
Private Sub ShowWordInput(Optional ByVal bytType As Byte)
'功能：显示词句输入
'参数：bytType 0-空为词句，1-输入人员，2-输入科室
    ...
    If rtfAppend.Visible And rtfAppend.Enabled And Not rtfAppend.Locked Then
...
'功能：根据部位的方法定义，如果只有一个方法可选，则返回该方法
    Dim arrMethod As Variant, i As Long
    
    If strMethod = "" Then Exit Function
    
    If InStr(strMethod, vbTab) = 0 Then
        strMethod = strMethod & vbTab
    End If
    arrMethod = Split(strMethod, vbTab)
    If arrMethod(0) = "" Or arrMethod(1) = "" Then
        If arrMethod(0) <> "" Then
            strMethod = arrMethod(0)
        Else
            strMethod = arrMethod(1)
        End If
        If UBound(Split(strMethod, ";")) = 0 Then
            strMethod = Split(strMethod, ";")(0)
            If UBound(Split(strMethod, ",")) = 0 Then
                strMethod = Split(strMethod, ",")(0)
                GetOnlyOneMethod = Mid(strMethod, 2)
            End If
        End If
    End If
End Function
...
Private Sub vsExt_CellButtonClick(ByVal Row As Long, ByVal Col As Long)
    ...
    With vsMethod
        ...
        '根据原始的方法关系展现方法选择器
        '互斥部份
        arrMethod = Split(Split(strMethod, vbTab)(0), ";")
    ...
    If KeyAscii = 13 Then
        ...
        If mintType = 1 Then
            ...
            strsql = _
                ...
                " Where A.ID=B.诊疗项目ID And A.类别='F' And A.ID<>-1*[3]" & IIf(strLike = "", "", " And Rownum<=100") & _
                    ...
                    IIf(mlngDeptID <> 0, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[8]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))", "") & _
                " Order by A.编码"
            ...
            Set rsTmp = zlDatabase.ShowSQLSelect(Me, strsql, 0, "手术", False, "", "", False, False, True, vPoint.X, vPoint.Y, vsExt.CellHeight, blnCancel, False, True, _
                UCase(vsExt.EditText) & "%", strLike & UCase(vsExt.EditText) & "%", mlng项目ID, mint简码 + 1, mint服务对象, IIf(mint期效 = 0, 2, 1), int性别, mlngDeptID)
            If rsTmp Is Nothing Then
                If Not blnCancel Then
                    MsgBox "未找到匹配项目！", vbInformation, gstrSysName
                End If
        ...
        ElseIf mintType = 4 Then
            ...
            strsql = "Select A.ID,A.编码,A.名称,A.操作类型,A.标本部位,A.试管编码" & _
                ...
                IIf(mlngDeptID <> 0, " And (Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID And 科室ID=[6]) Or Not Exists(Select 1 From 诊疗适用科室 Where 项目ID=A.ID))", "") & _
            ...
            Set rsTmp = zlDatabase.ShowSQLSelect(Me, strsql, 0, "检验项目", False, "", "", False, False, True, vPoint.X, vPoint.Y, vsExt.CellHeight, blnCancel, False, True, _
                UCase(vsExt.EditText) & "%", strLike & UCase(vsExt.EditText) & "%", mint简码 + 1, mint服务对象, int性别, mlngDeptID)
            If rsTmp Is Nothing Then
                If Not blnCancel Then
                    MsgBox "未找到匹配项目！", vbInformation, gstrSysName
                End If
