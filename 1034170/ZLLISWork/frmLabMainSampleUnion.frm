...
Private Sub GetNewResult(ByVal lngKey As Long)
    '合并后获取新的结果参考和结果标志
    ...
    strSQL = "Select b.检验标本id, b.检验项目id, b.检验结果, c.结果类型," & vbNewLine & _
            "       Trim(Replace(Replace(' ' || Zlgetreference(b.检验项目id, a.标本类型, Decode(a.性别, '男', 1, '女', 2, 0), a.出生日期, a.仪器id," & vbNewLine & _
            "                                                   a.年龄, a.申请科室id), ' .', '0.'), '～.', '～0.')) As 结果参考," & vbNewLine & _
            "       Trim(Replace(Replace(' ' || Zl_Get_Reference(4, b.检验项目id, a.标本类型, Decode(a.性别, '男', 1, '女', 2, 0), a.出生日期, a.仪器id," & vbNewLine & _
            "                                                     a.年龄, a.申请科室id), ' .', '0.'), '～.', '～0.')) As 警戒上限," & vbNewLine & _
            "       Trim(Replace(Replace(' ' || Zl_Get_Reference(3, b.检验项目id, a.标本类型, Decode(a.性别, '男', 1, '女', 2, 0), a.出生日期, a.仪器id," & vbNewLine & _
            "                                                     a.年龄, a.申请科室id), ' .', '0.'), '～.', '～0.')) As 警戒下限" & vbNewLine & _
            "From 检验标本记录 A, 检验普通结果 B, 检验项目 C" & vbNewLine & _
            "Where a.Id = b.检验标本id And b.检验项目id = c.诊治项目id And a.Id = [1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "获取结果参考", lngKey)
    
    Do While Not rsTmp.EOF
        str结果标志 = frmLisStationWrite.CalcDefaultFlag(rsTmp!检验结果 & "", rsTmp!结果参考 & "", Val(rsTmp!结果类型 & ""), rsTmp!警戒下限 & "", rsTmp!警戒上限 & "", rsTmp!检验项目id)
        str结果标志 = Decode(str结果标志, "↑", 3, "↓", 2, "异常", 4, "↓↓", 5, "↑↑", 6, 1)
        strResult = strResult & ";" & rsTmp!检验项目id & "," & IIf(IsNull(rsTmp!结果参考), " ", rsTmp!结果参考) & "," & str结果标志
        rsTmp.MoveNext
    Loop
    
    If strResult <> "" Then
        strSQL = "Zl_检验标本记录_Union(" & lngKey & ",0,'" & Mid(strResult, 2) & "')"
...
Private Sub RefreshUnion()
    ...
    If lngUnionKey > 0 Then
        gstrSql = "Select C.中文名 || Decode(C.英文名, Null, '', '(' || C.英文名 || ')') As 检验项目, B.检验结果, C.单位," & vbNewLine & _
                ...
                "Where A.ID = B.检验标本id And B.检验项目id = C.ID And B.诊疗项目id = D.ID(+) And B.检验项目id = E.诊治项目id And A.ID = [1]" & vbNewLine & _
                "Order By Decode(E.排列序号, Null, Nvl(D.编码, 9999999999), E.排列序号), B.排列序号 "
        Set rsTmp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, lngUnionKey)
