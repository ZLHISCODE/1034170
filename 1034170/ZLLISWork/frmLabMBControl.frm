...
Private Function MB_SendCommand(ByVal str_Command As String, ByVal str_Info As String, Optional ByVal intOutTime As Integer = 10, Optional ByVal intType As Integer = 0, Optional ByRef str_Data As String) As Boolean
    ...
    '--- 日志
    MbLog "SendCommand", strCmd, int超时
    ...
    If Trim(strCmd) <> "" Then
        ...
        Else                '要返回解析数据
            ...
            '--- 日志
            MbLog "接收酶标数据", strgetval & "|" & strGetCmd, str_Data
...
Private Sub MbLog(ByVal strFunc As String, ByVal strInput As String, ByVal strOutput As String)
    '------------------------------------------------------
    '--  功能:根据调试标志,写日志到当前目录
    '------------------------------------------------------
    ...
    End If
    strFileName = App.path & "\mb_" & Format(Date, "yyyyMMdd") & ".LOG"
