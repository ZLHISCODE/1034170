...
Private Function OpenSelect(ByVal strText As String, Optional ByVal blnWhere As Boolean = False) As Byte
    ...
    If mlngDefaultDevice = -1 Then
        If blnCheck Then '显示收费
            ...
            mstrSql = mstrSql & ",A.相关ID AS ID,  " & _
                        ...
                        "WHERE A.诊疗类别 = 'C' And Nvl(z.记录状态, 0) <> 0 " & _
                              "AND A.病人ID=C.病人ID " & _
                              "AND A.开嘱科室ID=D.ID " & _
            ...
            Else
                mstrSql = mstrSql & "SELECT Decode(Nvl(z.医嘱序号,0), 0, 1, 0) As 选择"
            End If
            mstrSql = mstrSql & ",A.相关ID AS ID,  " & _
                              "C.姓名||Decode(A.婴儿,0,'',Null,'','(婴儿)') As 姓名," & _
                              "C.门诊号," & _
                              ...
                              "A.开嘱医生 AS 申请人," & _
                              "f.接收人,f.接收时间, " & _
                              "'Item' AS 图标,Decode(Nvl(z.医嘱序号,0), 0, '√', '×') As 收费,NVL(A.紧急标志,0) AS 紧急,Y.操作类型,MAX(Decode(H.项目类别,2,2,1)) As 项目类别,MAX(F.采样人) AS 采样人,MAX(F.采样时间) AS 采样时间 " & _
                         "FROM 病人医嘱记录 A," & _
                         "病人信息 C,部门表 D,病人医嘱发送 F,检验报告项目 G,检验项目 H,诊疗项目目录 Y,住院费用记录 Z " & _
                        "WHERE A.诊疗类别 = 'C' And Nvl(z.记录状态, 0) = 0 " & _
                              "AND A.病人ID=C.病人ID " & _
	...
	Else
            ...
            mstrSql = mstrSql & ",A.相关ID AS ID, " & _
                              ...
                              "A.开嘱医生 AS 申请人," & _
                              "f.接收人,f.接收时间, " & _
                              "'Item' AS 图标,Decode(Nvl(z.医嘱序号,0), 0, '√', '×') As 收费,NVL(A.紧急标志,0) AS 紧急,Y.操作类型,Decode(H.项目类别,2,2,1) As 项目类别,F.采样人,F.采样时间 As 采样时间 " & _
                         "FROM 病人医嘱记录 A," & _
                         "病人信息 C,部门表 D,病人医嘱发送 F,检验报告项目 G,检验项目 H,诊疗项目目录 Y,住院费用记录 Z " & _
                        "WHERE A.诊疗类别 = 'C' And Nvl(z.记录状态, 0) = 0 " & _
                              "AND A.病人ID=C.病人ID " & _
    ...
    Else
        If blnCheck Then '显示收费
            ...
            mstrSql = mstrSql & ",A.相关ID AS ID, " & _
                              ...
                              "A.开嘱医生 AS 申请人," & _
                              "f.接收人,f.接收时间, " & _
                              "'Item' AS 图标,Decode(Nvl(z.医嘱序号,0), 0, '√', '×') As 收费,NVL(A.紧急标志,0) AS 紧急,H.操作类型,MAX(Decode(I.项目类别,2,2,1)) As 项目类别,MAX(F.采样人) AS 采样人,MAX(F.采样时间) AS 采样时间 " & _
                         "FROM 病人医嘱记录 A," & _
                         "病人信息 C,部门表 D,病人医嘱发送 F,检验报告项目 G,诊疗项目目录 H,检验项目 I,检验仪器项目 Y,住院费用记录 Z " & _
                        "WHERE A.诊疗类别 = 'C' And Nvl(z.记录状态, 0) = 0 " & _
                              "AND A.病人ID=C.病人ID " & _
	...
	Else
            ...
            mstrSql = mstrSql & ",A.相关ID AS ID, " & _
                              ...
                              "A.开嘱医生 AS 申请人," & _
                              "f.接收人,f.接收时间, " & _
                              "'Item' AS 图标,Decode(Nvl(z.医嘱序号,0), 0, '√', '×') As 收费,NVL(A.紧急标志,0) AS 紧急,H.操作类型,Decode(I.项目类别,2,2,1) As 项目类别,F.采样人,F.采样时间 As 采样时间 " & _
                         "FROM 病人医嘱记录 A," & _
                         "病人信息 C,部门表 D,病人医嘱发送 F,检验报告项目 G,诊疗项目目录 H,检验项目 I,检验仪器项目 Y,住院费用记录 Z " & _
                        "WHERE A.诊疗类别 = 'C' And Nvl(z.记录状态, 0) = 0 " & _
                              "AND A.病人ID=C.病人ID " & _
