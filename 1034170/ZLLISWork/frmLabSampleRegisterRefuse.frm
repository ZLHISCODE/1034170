...
Private Sub rptAlist_ItemCheck(ByVal Row As XtremeReportControl.IReportRow, ByVal Item As XtremeReportControl.IReportRecordItem)
    ...
    If Item.Record(mAcol.条码).Value <> "" Then
        For i = 0 To rptAlist.Rows.Count - 1
            If rptAlist.Records(i).Item(i).Record(mAcol.条码).Value = Item.Record(mAcol.条码).Value Then
                rptAlist.Records(i).Item(i).Record(mAcol.选择).Checked = Item.Record(mAcol.选择).Checked
            End If
        Next
    End If
    Call chkHide_Click
