...
'返回：交易成功返回true；否则，返回false
    ...
    '获取住院医师代码
    gstrSQL = "Select * from 人员表 Where 姓名=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, gstrSysName, CStr(rsTemp!门诊医师))
...
'返回：交易成功返回true；否则，返回false
    ...
    '获取住院医师代码
    gstrSQL = "Select * from 人员表 Where 姓名=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, gstrSysName, CStr(rsTemp!住院医师))
