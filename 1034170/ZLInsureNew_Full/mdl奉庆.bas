...
Public Function 医保初始化_奉庆() As Boolean
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "读取医院编码", TYPE_临沧奉庆)
...
Private Function Open中间库() As Boolean
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "渝北医保", TYPE_临沧奉庆)
    Do Until rsTemp.EOF
...
'返回: 返回个人帐户余额
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "读取个人帐户余额", lng病人ID, TYPE_临沧奉庆)
...
Private Function 检查中心维护单价(ByVal lng细目ID As Long, ByVal dbl单价 As Double) As Boolean
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "检查中心维护单价", TYPE_临沧奉庆, lng细目ID)
    If rsTemp.EOF Then
        检查中心维护单价 = True
        'MsgBox "有项目未设置医保编码，不能结算。", vbInformation, gstrSysName
        Exit Function
...
Public Function 门诊结算_奉庆(lng结帐ID As Long, cur个人帐户 As Currency, str医保号 As String, cur全自付 As Currency) As Boolean
    ...
    gstrSQL = "Select a.*, a.付数*a.数次 as 数量,a.实收金额/(nvl(a.付数,1)*nvl(a.数次,1)) as 单价 From 门诊费用记录 a Where 结帐ID=[1] And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0"
...
Public Function 门诊结算冲销_奉庆(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    gstrSQL = "Select * From 门诊费用记录 " & _
        " Where 结帐ID=[1] And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "获取冲销记录", lng冲销ID)
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "获取原来的结算记录", TYPE_临沧奉庆, lng结帐ID)
    ...
    If Abs(dbl个人帐户) <> Abs(Nvl(rsTemp!个人帐户支付, 0)) Then
        Err.Raise 9000, gstrSysName, "本次中心冲帐的个人帐户支付不等于上次结算的个人帐户支付!"
        Exit Function
    End If
...
Private Function 病人入院登记处理(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select C.住院号,C.当前床号,to_char(A.确诊日期,'yyyy-MM-dd') as 确诊日期,A.登记人 经办人,B.名称 入院科室,A.住院医师,to_char(A.登记时间,'yyyy-mm-dd hh24:mi:ss') 入院经办时间," & _
        " to_char(A.入院日期,'yyyy-mm-dd') 入院日期  ,to_char(A.入院日期,'yyyy-mm-dd hh24:mi:ss') 入院时间,D.入院诊断编码,D.入院诊断名称,G.确诊诊断编码,g.确诊诊断名称 " & _
...
Private Function 处方上传(ByVal lng记录性质 As Long, lng记录状态 As Long, ByVal str单据号 As String) As Boolean
   ...
   '读出该张单据的费用明细
    gstrSQL = "Select A.ID,A.NO,A.序号,A.病人ID,A.主页ID,to_char(A.发生时间,'yyyy-mm-dd hh24:mi:ss') as 登记时间,Round(A.实收金额,4) 实收金额 " & _
              ...
              "  From 住院费用记录 A,收费类别 J,收费细目 B,保险帐户 M,(Select  项目编码, 附注,是否医保,收费细目ID From 保险支付项目 where 险类=[3]) C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G,部门表 Y " & _
              "  where a.病人id=M.病人id  and M.险类=[3] and A.NO=[1] and A.记录性质=[2] and A.记录状态=1 And Nvl(A.是否上传,0)=0 " & _
              "        and A.收费类别=J.编码(+)  and A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=[3]" & _
    ...
    Set rs明细 = zlDatabase.OpenSQLRecord(gstrSQL, "处方明细上传", str单据号, lng记录性质, TYPE_临沧奉庆)
    Dim lng冲销ID As Long
...
'            End If
    ...
    With rs明细
        Do Until .EOF
            ...
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "处方上传", CLng(Val(Nvl(!附注, 0))), CStr(Nvl(!项目编码)))
'            If rsTemp.EOF Then
...
Private Function 补传住院明细记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select A.ID,A.NO,A.记录性质,J.类别 as 收费类别,A.记录状态,A.序号,A.病人ID,A.主页ID,A.发生时间 as 登记时间,Round(A.实收金额,4) 实收金额" & _
              ...
              "  From 住院费用记录 A,收费类别 J,收费细目 B, (Select 项目编码, 附注,是否医保,收费细目ID From 保险支付项目 where 险类=" & TYPE_临沧奉庆 & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G ,部门表 Y" & _
              "  where A.病人ID=[1] and A.主页ID=[2] and A.记帐费用=1 and nvl(A.实收金额,0)<>0 and nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0 " & _
              "         and a.开单部门id=y.id(+) and A.收费类别=J.编码(+) and A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=[1]" & _
              "        and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID(+) " & _
              "        AND B.ID=E.药品ID(+) AND E.药名ID=F.药名ID(+) AND F.剂型=G.编码(+) " & _
              "  Order by A.发生时间,A.记录性质,Decode(A.记录状态,2,2,1)"
    Set rs明细 = zlDatabase.OpenSQLRecord(gstrSQL, "虚拟结算", TYPE_临沧奉庆, lng病人ID, lng主页ID)
...
Private Function Get原单据摘要(ByVal strNO As String, ByVal int序号 As Integer, ByVal int性质 As Integer) As Variant
    ...
    gstrSQL = " Select 摘要 From 住院费用记录" & _
              " Where NO=[1] And 序号=[2] And 记录性质=[3] And 记录状态=3"
...
Public Function GetItemInfo_奉庆(ByVal lngPatiID As Long, ByVal lngItemID As Long, Optional ByVal str摘要 As String, Optional intType As Integer = 0) As String
    ...
    gstrSQL = "Select a.项目编码,a.附注,b.编码 from 保险支付项目 a,收费细目 b where a.收费细目id=b.id and  nvl(a.是否医保,0)=0 and 险类=[1] and a.收费细目iD=[2]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "获取是否特殊治疗，特殊检查和贵重药品!", TYPE_临沧奉庆, lngItemID)
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "获取医保证号", lngPatiID)
    If rsTemp.EOF Then
