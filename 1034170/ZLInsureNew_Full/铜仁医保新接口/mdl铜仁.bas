...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    If frmIdentify铜仁.GetPatient(bytType, False, lng病种ID) = True Then
        ...
        With gIC铜仁
            ...
            '得到中心序号
            gstrSQL = "select 序号 from 保险中心目录 where 险类=" & TYPE_铜仁 & " and 编码='" & .CenterCode & "'"
            Call OpenRecordset(rsTemp, "铜仁医保")
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    gstrSQL = "Select B.编码 From 保险帐户 A,保险病种 B where A.险类=" & TYPE_铜仁 & " And A.病人ID=" & lng病人ID & " And A.病种ID=B.ID(+)"
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              ...
              "        and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & TYPE_铜仁 & _
    ...
    gstrSQL = "SELECT A.病人ID,A.NO,A.实际票号,substr(B.姓名,1,8) as 姓名,substr(B.性别,1,2) as 性别,floor(MONTHS_BETWEEN(A.登记时间,B.出生日期)/12) AS 年龄" & _
              ...
              "  Where A.结帐ID =" & lng结帐ID & " And A.病人ID = B.病人ID And B.病人ID = C.病人ID And C.险类 =" & TYPE_铜仁 & " And A.开单部门ID = D.ID(+) and rownum<2"
    Call OpenRecordset(rs信息, "铜仁医保")
    lng病人ID = rs信息("病人ID")
    ...
    With ic门诊
        gstrSQL = "INSERT INTO 保险结算记录 " & _
         ...
         gstrSQL = gstrSQL & " (1," & lng结帐ID & "," & TYPE_铜仁 & "," & .MediYear & ",'" & .CenterCode & "','" & rs信息("NO") & "1'," & lng病人ID & ",0,'" & rs信息("姓名") & _
                  "','" & rs信息("性别") & "'," & rs信息("年龄") & ",'" & rs信息("医保号") & "','" & rs信息("卡号") & "','" & rs信息("身份证号") & "','" & .ClassCode & "','" & Trim(.UnitCode) & "' " & _
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    With m铜仁
        ...
        '避免在出院结帐后再次进行结帐
        gstrSQL = "SELECT 病人ID FROM 保险结算记录 WHERE 中途结帐=0 AND 病人ID=" & .病人ID & " AND 主页ID=" & .主页ID & " AND 险类=" & TYPE_铜仁
        Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              "         ,A.数次*nvl(A.付数,1) as 数量,Decode(A.数次*nvl(A.付数,1),0,0,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4)) as 单价 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C " & _
              "  where A.病人ID=" & m铜仁.病人ID & " and A.主页ID=" & m铜仁.主页ID & " and A.记帐费用=1 And A.操作员姓名 is not null AND A.实收金额 IS NOT NULL " & _
              "        and nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0 and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & TYPE_铜仁 & _
              "  Order by A.病人ID,A.发生时间"
    Call OpenRecordset(rsTemp, "虚拟结算")
    If rsTemp.EOF = False Then
        '还存在没有分割费用的明细
    ...
    gstrSQL = "select A.医保年,B.编码,B.序号 " & _
            ...
            " where A.病人ID=" & m铜仁.病人ID & " and A.险类=" & TYPE_铜仁 & _
    ...
    gstrSQL = "Select B.编码 From 保险帐户 A,保险病种 B where A.险类=" & TYPE_铜仁 & " And A.病人ID=" & m铜仁.病人ID & " And A.病种ID=B.ID(+)"
    ...
    gstrSQL = "select 入院日期,nvl(出院日期,to_date('3000-01-01','yyyy-MM-dd')) as 出院日期,sysdate 当前日期 " & _
              "from 病案主页 where 病人ID=" & m铜仁.病人ID & " and 主页ID=" & m铜仁.主页ID
    Call OpenRecordset(rsTemp, "虚拟结算")
...
'返回：交易成功返回true；否则，返回false
    ...
    gstrSQL = "SELECT A.病人ID,A.NO,A.实际票号,substr(B.姓名,1,8) as 姓名,substr(B.性别,1,2) as 性别,floor(MONTHS_BETWEEN(A.收费时间,B.出生日期)/12) AS 年龄" & _
              ...
              "  Where A.ID =" & lng结帐ID & " And A.病人ID = B.病人ID And B.病人ID = C.病人ID And C.险类 =" & TYPE_铜仁 & _
    ...
    With ic铜仁
        gstrSQL = "INSERT INTO 保险结算记录 " & _
         ...
         gstrSQL = gstrSQL & " (2," & lng结帐ID & "," & TYPE_铜仁 & "," & .MediYear & ",'" & .CenterCode & "','" & rs信息("NO") & "1'," & m铜仁.病人ID & "," & m铜仁.主页ID & ",'" & rs信息("姓名") & _
                  "','" & rs信息("性别") & "'," & rs信息("年龄") & ",'" & rs信息("医保号") & "','" & rs信息("卡号") & "','" & rs信息("身份证号") & "','" & Trim(.ClassCode) & "','" & Trim(.UnitCode) & "' " & _
                  "," & .IsOffical & "," & .IsAttend & "," & m铜仁.参加补充保险 & "," & .InPerAcc & "," & .OutPerAcc & "," & .PlanPaidAmt & "," & .PlanPaidFee & _
...
'----------------------------------------------------------------
    ...
    Call OpenRecordset(rs结算计算, "铜仁医保")
...
'返回:
    ...
    gstrSQL = " Select A.ID,A.NO,A.病人ID,A.收费类别,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              "         ,A.数次*nvl(A.付数,1) as 数量,Decode(A.数次*nvl(A.付数,1),0,0,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4)) as 单价 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C,保险帐户 D " & _
              "  where A.NO='" & str单据号 & "' and A.记录性质=" & int性质 & " and A.记录状态=1 And Nvl(A.是否上传,0)=0" & _
              "        and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID And A.病人ID=D.病人ID And D.险类=" & TYPE_铜仁 & " And C.险类= " & TYPE_铜仁 & _
              "  Order by A.病人ID,A.发生时间"
...
'返回指定用户的医保灰度级
    ...
    gstrSQL = "select 灰度 from 黑名单 where 中心代码='" & str中心 & "' and 卡号='" & Trim(str卡号) & "' And 医保年='" & Get当前医保年 & "'"
    rsTemp.Open gstrSQL, gcn铜仁, adOpenStatic, adLockReadOnly
...
'功能：判断该病人的卡是否插入正确
    ...
    gstrSQL = "select A.卡号,A.医保号,B.编码 from 保险帐户 A,保险中心目录 B " & _
              " where A.险类=" & TYPE_铜仁 & " and A.病人ID=" & lng病人ID & " and a.险类=B.险类 and A.中心=B.序号"
    Call OpenRecordset(rsTemp, "铜仁医保")
...
Public Function 检查医保服务器_铜仁() As Boolean
    ...
    '读出连接医保服务器的配置
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_铜仁
    Call OpenRecordset(rsTemp, "铜仁医保")
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select A.在职 from 保险帐户 A where A.险类=" & TYPE_铜仁 & " and A.病人ID=" & 病人ID
    Call OpenRecordset(rsTemp, "铜仁医保")
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select A.医保号,A.密码,B.身份证号 from 保险帐户 A,病人信息 B where A.险类=" & _
        TYPE_铜仁 & " and A.病人ID=" & 病人ID & " And A.病人ID=B.病人ID"
    Call OpenRecordset(rsTemp, "铜仁医保")
...
'调用位置：门诊预算、门诊结算、住院记帐、住院预算、住院结算、费用明细上传
    ...
    Do Until rs费用明细.EOF
        If lng病人ID <> rs费用明细("病人ID") Then
            ...
            bln医保病人 = False
            If Not bln门诊 Then
                gstrSQL = "Select Count(*) Records From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & rs费用明细!病人ID & " And A.主页ID=B.住院次数 And A.险类=" & TYPE_铜仁
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    With m铜仁
        gstrSQL = "select A.人员身份,A.在职,A.年龄段," & _
                  ...
                  "     and B.年度(+)=" & .年度 & " and A.病人ID=" & .病人ID & " and A.险类=" & TYPE_铜仁
        Call OpenRecordset(rsTemp, "虚拟结算")
...
'返回：成功计算，则返回True
    ...
    With m铜仁
        ...
        .补充基金支付 = (dbl甲类 + (dbl乙类 * 0.8)) * dbl比例
        .补充基金自付 = .超基本封顶线 - (dbl甲类 + (dbl乙类 * 0.8)) * dbl比例
...
'功能：获得保险参数
    ...
    gstrSQL = "select A.参数名,A.参数值 from 保险参数 A " & _
              " where A.险类=" & TYPE_铜仁 & " and A.中心 is null "
    Call OpenRecordset(rsTemp, "铜仁医保")
...
Private Function Get当前医保年() As String
    ...
    gstrSQL = " Select A.医保年 From 保险主机 A,保险中心目录 B" & _
              " Where A.险类=" & TYPE_铜仁 & " And A.编码=B.主机编码 "
    With rsTemp
...
Private Function Get主机IP() As String
    ...
    gstrSQL = "Select 装钱IP地址1 IP From 保险主机参数 A,保险中心目录 B " & _
             " Where A.险类=" & TYPE_铜仁 & " And A.主机=B.主机编码 "
    With rsTemp
        If .State = 1 Then .Close
        .Open gstrSQL, gcn铜仁
    End With
...
Private Function Get护理限额_离休() As Single
    ...
    gstrSQL = " Select B.名称 From 病案主页 A,护理等级 B" & _
              " Where A.护理等级ID=B.序号 And 病人ID=" & m铜仁.病人ID & " And 主页ID=" & m铜仁.主页ID
    Call OpenRecordset(rsTemp, "获取该病人当前的护理等级")
...
Private Function Calc基本统筹_离休(Optional ByVal rsOutExse As ADODB.Recordset, Optional ByVal bln住院 As Boolean = True) As Boolean
    Dim str日期 As String
    Dim lng病人ID As Long
    Dim bln全额报销 As Boolean      '医保编码=999999的项目全额报销
    ...
    With rsExse
        ...
        Do While Not .EOF
            '取该项目对应的医保编码
            gstrSQL = "Select 项目编码 From 保险支付项目 Where 险类=" & TYPE_铜仁 & " And 收费细目ID=" & !收费细目ID
            Call OpenRecordset(rsTemp, "取该项目对应的医保编码")
            If rsTemp.RecordCount <> 0 Then
                cur统筹 = cur统筹 + !实收金额
            End If
...
Private Function IS遗孀(ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = "Select 医保号 From 保险帐户 Where 病人ID=" & lng病人ID & " ANd 险类=" & TYPE_铜仁
    Call OpenRecordset(rsTemp, "提取医保号")
    str医保号 = rsTemp!医保号
    
    gstrSQL = "Select 1 From 离休人员 Where 医保号='" & str医保号 & "' And 身份代码='52'"
    If rsTemp.State = 1 Then rsTemp.Close
    rsTemp.Open gstrSQL, gcn铜仁
