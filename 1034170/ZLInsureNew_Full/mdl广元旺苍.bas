...
Public Function 医保初始化_广元旺苍() As Boolean
   ...
   End If
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "读取医院编码", TYPE_广元旺苍)
    InitInfor_广元旺苍.医院编码 = Nvl(rsTemp!医院编码)
    
    
    '中间库连接
    gstrSQL = "select 参数名,参数值 from 保险参数 where  险类=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "保险参数", TYPE_广元旺苍)
    Do Until rsTemp.EOF
...
'返回: 返回个人帐户余额
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "读取个人帐户余额", lng病人ID, TYPE_广元旺苍)
...
Public Function 门诊虚拟结算_广元旺苍(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    With rs明细
        Do While Not .EOF
            ...
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "确定医保支付项目", TYPE_广元旺苍, g病人身份_广元旺苍.社保中心, CLng(Nvl(!收费细目ID, 0)))
...
'
    ...
    gstrSQL = "" & _
        ...
        "   From 门诊费用记录 a " & _
        "   Where nvl(实收金额,0)<>0 and  结帐ID=[1] And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0"
...
Public Function 住院结算冲销_广元旺苍(lng结帐ID As Long) As Boolean
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "结算冲销", TYPE_广元旺苍, lng结帐ID)
...
Public Function 处方登记_广元旺苍(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
  ...
  gstrSQL = "Select A.ID,a.标识号 住院号,a.序号,a.NO,A.记录性质,A.记录状态,A.病人ID,A.主页ID,to_char(A.发生时间,'yyyy-mm-dd') as 登记时间,Round(A.实收金额,4) 实收金额 " & _
    ...
    Set rs明细 = zlDatabase.OpenSQLRecord(gstrSQL, "处方明细上传", str单据号, lng记录性质, lng记录状态, TYPE_广元旺苍)
    ...
    With rs明细
        ...
        Do While Not .EOF
            gstrSQL = "Select * From 医保支付项目 where 险类=[1] and 中心=[2] and 收费细目id=[3]"
            Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "确定医保支付项目", TYPE_广元旺苍, CLng(Nvl(!中心, 0)), CLng(Nvl(!收费细目ID, 0)))
            If rsTemp.EOF Then
...
Private Function 补传住院明细记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    Set rs明细 = zlDatabase.OpenSQLRecord(strSQL, "虚拟结算", lng病人ID, lng主页ID, TYPE_广元旺苍)
