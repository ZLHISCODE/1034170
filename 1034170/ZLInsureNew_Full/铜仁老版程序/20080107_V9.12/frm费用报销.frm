...
Private Sub cbo就医_Click(Index As Integer)
    ...
    gstrSQL = "select 金额 from 保险报销政策 " & _
             ...
             " And 险类=" & gintInsure & " And 年度=" & Format(zlDataBase.Currentdate, "yyyy")
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Function ReadData() As Boolean
    ...
    '将计算结果写回界面，供操作员校验
    '读取结算记录信息
    gstrSQL = " Select A.*,B.名称 病种名称 From 保险结算记录 A,(Select ID 病种ID,名称 From 保险病种 Where 险类=" & gintInsure & ") B" & _
...
Private Function 住院结算(lng结帐ID As Long, Optional ByVal bln本院治疗 As Boolean = True) As Boolean
    ...
    '将结算信息保存到保险结算记录中
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & gintInsure & "," & gComInfo_眉山.病人ID & "," & _
