...
Public Function 个人余额_眉山(ByVal strSelfNo As String) As Currency
    ...
    gstrSQL = " Select Nvl(帐户余额,0) 帐户余额 From 保险帐户 " & _
              " Where 险类=" & TYPE_四川眉山 & " And 医保号='" & strSelfNo & "'"
...
Optional ByVal bln本院治疗 As Boolean = True) As Boolean
    ...
    '将结算信息保存到保险结算记录中
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & gintInsure & "," & gComInfo_眉山.病人ID & "," & _
    ...
    With rs大类_米易
        ...
        Do While Not .EOF
            If NVL(!费用总额, 0) <> 0 Then
                ...
                "'" & !大类编码 & "','" & !大类名称 & "'," & !统筹比额 & "," & _
                "" & !特准定额 & "," & !特准天数 & "," & !费用总额 & "," & !报销总额 & ")"
                Call ExecuteProcedure("保险门诊收费数据")
            End If
            .MoveNext
        Loop
    End With
    ...
    If cur个人帐户 <> 0 Then
        ...
        gstrSQL = " Select Nvl(帐户增加累计,0) 帐户余额,Nvl(进入统筹累计,0) 统筹累计,Nvl(住院次数累计,0) 本院,Nvl(外院住院次数,0) 外院  From 帐户年度信息" & _
                  " Where 病人ID=" & gComInfo_眉山.病人ID & " ANd 年度=" & Format(zlDatabase.Currentdate, "yyyy")
...
Public Function 门诊结算冲销_眉山(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    gstrSQL = "Select * From 保险结算记录 Where 险类=25 And 记录ID=" & lng结帐ID
    ...
    With rsTemp
        ...
        gstrSQL = "Select * From 保险报销记录 Where 记录ID=" & lng结帐ID
        Call OpenRecordset(rsTemp, "读取大类报销记录")
        '保险各大类的报销明细
        Do While Not .EOF
            If NVL(!费用总额, 0) <> 0 Then
    ...
    If cur个人帐户 <> 0 Then
        ...
        cur帐户余额 = 0: cur统筹累计 = 0: int住院次数 = 0
        gstrSQL = " Select Nvl(帐户增加累计,0) 帐户余额,Nvl(进入统筹累计,0) 统筹累计,Nvl(住院次数累计,0) 本院,Nvl(外院住院次数,0) 外院  From 帐户年度信息" & _
                  " Where 病人ID=" & lng病人ID & " ANd 年度=" & lng年度
        Call OpenRecordset(rsTemp, "读取帐户余额")
        '由于下个人帐户时，已经冲减了帐户，所以本次不作加减运算
        If Not rsTemp.EOF Then
            cur帐户余额 = NVL(rsTemp!帐户余额, 0)
...
Private Function CalcPrepare(Optional ByVal bln本院治疗 As Boolean = True) As Boolean
    ...
    gstrSQL = " Select A.*,B.名称 病种名称,C.名称 人群 " & _
              ...
              " (Select * From 保险人群 Where 险类=" & gintInsure & ") C" & _
              " Where A.病种ID=B.ID(+) And A.险类=" & gintInsure & " And A.病人ID=" & gComInfo_眉山.病人ID & _
              " And A.在职=C.序号 "
    Call OpenRecordset(rsTemp, "获取医保病人帐户信息")
    ...
    '检查是否设置保险参数
    gstrSQL = "Select count(*) Records From 保险参数 Where 险类=" & gintInsure
    Call OpenRecordset(rsTemp, "获取系统参数")
    If rsTemp!Records = 0 Then
    ...
    gstrSQL = " Select Count(*) Records From 保险报销政策 A,保险人群 B" & _
            " Where A.险类=" & gintInsure & " And A.中心=" & gComInfo_眉山.中心 & _
            " And A.性质=1 And A.本院=" & IIf(bln本院治疗, 1, 2) & " And A.年度=" & str年度 & _
            " And A.人群=B.序号 And A.险类=B.险类 And B.名称='" & gComInfo_眉山.人群 & "'"
    Call OpenRecordset(rsTemp, "支付比例")
    If rsTemp!Records = 0 Then
    ...
    gstrSQL = " Select Count(*) Records From 保险报销政策 A,保险人群 B" & _
            " Where A.险类=" & gintInsure & " And A.中心=" & gComInfo_眉山.中心 & _
            " And A.性质=2 And A.本院=" & IIf(bln本院治疗, 1, 2) & " And A.年度=" & str年度 & _
            " And A.人群=B.序号 And A.险类=B.险类 And B.名称='" & gComInfo_眉山.人群 & "'"
    Call OpenRecordset(rsTemp, "支付比例")
    If rsTemp!Records = 0 Then
        MsgBox "还未设置本年度的保险报销政策！[年度结算规则]", vbInformation, gstrSysName
    ...
    '取本年度住院次数
    gstrSQL = "Select Nvl(住院次数累计,0) 住院次数 From 帐户年度信息 Where 险类=" & gintInsure & " And 病人ID=" & gComInfo_眉山.病人ID
    Call OpenRecordset(rsTemp, "本年度住院次数统计")
    If Not rsTemp.EOF Then
...
Public Sub Init_大类_米易()
    ...
    gstrSQL = "Select * From 保险支付大类 Where 险类=" & TYPE_四川眉山
    Call OpenRecordset(rs支付大类_米易, "保险支付大类")
    With rs支付大类_米易
        Do While Not .EOF
            rs大类_米易.AddNew
            rs大类_米易!大类编码 = !编码
...
Public Sub Calc_实际报销_米易()
    ...
    gstrSQL = " Select A.名称,B.参数值 比例 From 保险支付大类 A,(Select * From 保险参数 Where 险类=25 And 中心=1 And 序号>10) B " & _
              " Where A.险类=25 And A.名称=B.参数名(+) Order by 编码"
...
Public Sub Calc_门诊报销计算_米易(Optional ByVal bln本院 As Boolean = True, Optional ByVal bln医院调用 As Boolean = False)
    ...
    '先取保险参数
    gstrSQL = "Select 中心,序号,参数值 From 保险参数 Where 险类=" & gintInsure & " Order by 中心,序号"
    Call OpenRecordset(rsPara, "读取保险参数")
    
    '计算实际进入统筹金额
    gstrSQL = "Select 比例 From 保险报销政策 A,保险人群 B" & _
             " Where A.性质=1 And A.中心=" & gComInfo_眉山.中心 & " And A.本院=" & IIf(bln本院, 1, 2) & _
             " And B.险类=A.险类 And A.险类=" & gintInsure & _
             " And A.人群=B.序号 And B.名称='" & gComInfo_眉山.人群 & "' And A.档次=0 And A.年度=" & Format(zlDatabase.Currentdate, "yyyy")
    Call OpenRecordset(rsScale, "读取门诊报销比例")
    If rsScale.EOF Then
        sin实际允许进入统筹比例 = 0
    Else
        ...
        If gComInfo_眉山.病种名称 = "慢特病" Then
            ...
            If gComInfo_眉山.最高限额 <> 0 Then
                gstrSQL = " Select Sum(进入统筹金额) 已报销金额 From 保险结算记录 " & _
                          " Where 险类=" & gintInsure & " And 性质=1 And 病人ID=" & gComInfo_眉山.病人ID & _
                          " And 年度='" & Format(zlDatabase.Currentdate, "yyyy") & "' And 病种ID=" & gComInfo_眉山.病种ID
                Call OpenRecordset(rsSum, "取本年度已报销金额")
...
Public Sub Calc_住院报销计算_米易(Optional ByVal bln本院 As Boolean = True)
    ...
    gstrSQL = "Select A.比例,C.名称,C.档次,C.下限,C.上限 From 保险报销政策 A,保险人群 B,保险费用档 C " & _
             " Where A.性质=1 And A.中心=" & gComInfo_眉山.中心 & " And A.本院=" & IIf(bln本院, 1, 2) & _
             " And B.险类=A.险类 And A.险类= " & gintInsure & "  And A.档次<>0 " & _
             " And A.人群=B.序号 And B.名称='" & gComInfo_眉山.人群 & "' And A.年度=" & Format(zlDatabase.Currentdate(), "yyyy") & _
             " And A.险类=C.险类 And C.档次=A.档次 And C.中心=A.中心" & _
             " Order by 档次"
    Call OpenRecordset(rs档次, "读取保险费用档")
...
'            gComInfo_眉山.进入统筹 = gComInfo_眉山.进入实际报销
    ...
    Call OpenRecordset(rsTemp, "读取本年度进入统筹累计")
...
Public Function 检查帐户信息_米易(ByVal strCardNO As String, Optional ByVal blnUpdate As Boolean = False, Optional ByVal bln卡号 As Boolean = True) As Boolean
    ...
    gstrSQL = "Select Nvl(帐户余额,0)*100 金额,卡号,加密串,病人ID From 保险帐户 Where 险类=" & gintInsure
    If bln卡号 Then
        gstrSQL = gstrSQL & " And 医保号='" & strCardNO & "'"
    Else
        gstrSQL = gstrSQL & " And 病人ID=" & Val(strCardNO)
    End If
    Call OpenRecordset(rsTemp, "检查帐户信息")
    If Not rsTemp.EOF Then
        strCardNO = rsTemp!卡号
        lng病人ID = rsTemp!病人ID
