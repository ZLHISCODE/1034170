...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    '报销规定
    m政策.个人帐户支付全自费 = cls医保.GetCapability(support收费帐户全自费, 0)
    m政策.个人帐户支付首先自付 = cls医保.GetCapability(support收费帐户首先自付, 0)
...
redo:
    ...
    If rsTemp.EOF = False Then
        If IsNull(rsTemp!编码) Then
            ...
            lng病人ID = 0
            strIdentify = 身份标识_铜仁(2, lng病人ID)
            If strIdentify = "" Then Exit Function
            If lng病人ID <> m铜仁.病人ID Then
                MsgBox "待结帐的病人与当前身份验证的病人不是同一个病人,请重新验证!", vbInformation, gstrSysName
            End If
            GoTo redo
        End If
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    m政策.个人帐户支付全自费 = cls医保.GetCapability(support结算帐户全自费, 0)
    m政策.个人帐户支付首先自付 = cls医保.GetCapability(support结算帐户首先自付, 0)
    m政策.个人帐户支付超限 = cls医保.GetCapability(support结算帐户超限, 0)
