...
Private Sub cmd医疗机构_Click()
    ...
    If frmListSel.ShowSelect(rsHospital, "ID", "选择定点医疗机构", "请选择一家定点医疗机构：") = True Then
...
Private Sub txt医院名称_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If chk定点医院.Value = 0 Then SendKeys "{Tab}", 1: Exit Sub
    ...
    Else
        If rsHospital.RecordCount > 1 Then
            '对于字段大于3的，即使只有一条记录把该对话框显示出来，以便让用户得到更多的信息
            blnReturn = frmListSel.ShowSelect(rsHospital, "ID", "选择定点医疗机构", "请选择一家定点医疗机构：")
        Else
            blnReturn = True
        End If
    End If
