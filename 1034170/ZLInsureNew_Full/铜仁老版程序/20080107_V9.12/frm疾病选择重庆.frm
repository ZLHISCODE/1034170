...
Public Function GetCode(ByVal varList As Variant, strCode As String, str病种 As String, str并发症 As String) As Boolean
'功能：获得一个收费项目的医保编码
...
'返回：成功返回True
    ...
    If strCode = "急诊" Or strCode = "出院" Then
        ...
        If strCode = "急诊" Then
            tab疾病.Visible = False
            rsTemp.Open "select BZBM 编码,BZMC 名称,ZJM 简码  from BZML where bzfl=3 Order by BZBM", mcnYB, adOpenStatic, adLockReadOnly
        Else
            '可以看见Tab
            tab疾病.Visible = True
            tab疾病.Tag = "出院"
            '20031231:周韬:精神病的单独成5了
            rsTemp.Open "select BZBM 编码,BZMC 名称,ZJM 简码  from BZML where bzfl in(1,5) Order by BZBM", mcnYB, adOpenStatic, adLockReadOnly
        End If
        ...
        Do Until rsTemp.EOF
            ...
            If lst.Text = str病种 Then lst.Selected = True
            rsTemp.MoveNext
        Loop
    Else
        '特殊病
