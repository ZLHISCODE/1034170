...
Private gcn重庆 As New ADODB.Connection
...
Private Function MakeFile_Recipe(ByVal rsDetail As ADODB.Recordset, Optional ByVal bln门诊 As Boolean = True, _
    Optional ByVal bln预结算 As Boolean = True, Optional ByVal str原处方流水号 As String = "") As Boolean
    Dim intDo As Integer
    Dim lng病人ID As Long
    Dim bln急诊 As Boolean, bln药品 As Boolean, bln上传 As Boolean
    ...
    Dim str项目流水号 As String, str项目类别 As String
...
'    26.     string  3       统筹区号
                ...
                If bln上传 Then
                    ...
                    If !收费类别 = 5 Or !收费类别 = 6 Or !收费类别 = 7 Then
                        ...
                        gstrSQL = " Select 流水号,项目类型,剂型,包装数量,包装单位,含量,含量单位,容量,容量单位" & _
                                  " From zlcq.中间库_药品目录 Where 流水号=" & _
                                  "     (Select 项目编码 From 保险支付项目 " & _
                                  "     Where 收费细目ID=" & !收费细目ID & " And 险类=" & TYPE_重庆银海版 & ")"
                    Else
                        bln药品 = False
                        gstrSQL = " Select 流水号,项目类别 项目类型" & _
                                  " From zlcq.中间库_诊疗项目 Where 流水号=" & _
                                  "     (Select 项目编码 From 保险支付项目 " & _
                                  "     Where 收费细目ID=" & !收费细目ID & " And 险类=" & TYPE_重庆银海版 & ")"
                    End If
                    Call OpenRecordset(rsItem, "获取收费细目医保相关信息")
                    ...
                    Else
                        ...
                        str开单时间 = Format(!登记时间, "yyyyMMdd HH:mm:ss")
                        str开单医生 = NVL(!医生)
                        str处方号 = !NO
                    End If
                    ...
                    If str退单处方流水号 <> str处方流水号 Then
                        gstrSQL = "Select to_char(开方日期,'yyyyMMdd hh24:mi:ss') 开方日期 From zlcq.中间库_处方明细 Where 处方流水号='" & str退单处方流水号 & "'"
                        Call OpenRecordset(rsTemp, "取原始的开单时间")
                    ...
                    gstrSQL = "Select 编码,名称 From 收费细目 Where ID=" & !收费细目ID
                    Call OpenRecordset(rsTemp, "获取医院项目的编码与名称")
                    str医院项目编码 = rsTemp!编码
                    str医院项目名称 = rsTemp!名称
    ...
    If Not MakeFile_Recipe2(rsRecipe) Then Exit Function
...
Private Sub Get处方流水号(ByVal strNO As String, ByVal str性质 As String, ByVal str状态 As String, _
    ...
    If strNO = "" Then      '说明是门诊预结算
        str处方流水号 = ToVarchar(lng病人ID & Format(zlDatabase.Currentdate(), "yyMMddHHmmss") & str序号, 18)
        str退单处方流水号 = str处方流水号
        Exit Sub
    End If
    
    str处方流水号 = strNO & String(3 - Len(str性质), "0") & str性质 & _
                    String(3 - Len(str状态), "0") & str状态 & _
...
Private Function MakeFile_Recipe2(ByVal rsRecipe As ADODB.Recordset) As Boolean
...
Private Function MakeFile_RecipeCalculated(ByVal str流水号 As String) As Boolean
...
'        公务员门诊费用累计,特病门诊医保费累计,历史未补助先自付,转院前已进入统筹,转院前公务员补助起付线,开始时间,结束时间,统筹区号)
    ...
    Do While Not mobjStream.AtEndOfStream
        ...
        For lngCol = 0 To lngCols
            ...
            End Select
        Next
        strDeal = strData & Mid(strDeal, 2) & ")"
        If blnSave Then gcn重庆.Execute strDeal, , adCmdStoredProc
    Loop
    mobjStream.Close
...
Private Function AnalyFile_Recipe(Optional ByVal blnSave As Boolean = False) As Boolean
    ...
    Do While Not mobjStream.AtEndOfStream
        ...
        If blnSave Then gcn重庆.Execute strRecipe, , adCmdStoredProc
    Loop
    mobjStream.Close
...
'        转院起付线纳入基本,转院起付线纳入公务员,转院起付线纳入大病,转院起付线纳入自付,发票号,经办人,经办时间,统筹区号)
    ...
    Do While Not mobjStream.AtEndOfStream
        ...
        If blnSave Then gcn重庆.Execute strBalance, , adCmdStoredProc
...
ErrHand:
    If ErrCenter = 1 Then
        Resume
    End If
    If blnTrans Then gcn重庆.RollbackTrans
End Function
...
'返回：初始化成功，返回true；否则，返回false
    ...
    If mblnInit = False Then
        If Not blnTest Then '如果是测试，则说明是保险参数设置处调用
            ...
            If OraDataOpen(gcn重庆, strServer, strUser, strPass, False) = False Then
        ...
        End If
        '取医院编码
        gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_重庆银海版
...
Public Function 调用接口_重庆银海版() As Boolean
    ...
    If lngResult < 0 Then               '错误信息
        MsgBox "交易类型[" & mstrBusiness & "]错误代码[" & lngResult & "]" & mstrAppMsg, vbInformation, gstrSysName
        Exit Function
    ElseIf lngResult > 0 Then           '仅仅是应用提示信息
        MsgBox mstrAppMsg, vbInformation, gstrSysName
    End If
...
Public Function 门诊结算_重庆银海版(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    gstrSQL = "Select Rownum 标识号,A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,A.登记时间,A.开单人," & _
            "   A.收费细目ID,A.收入项目ID,A.计算单位,A.数次*A.付数 as 数量,Round(A.结帐金额/(A.数次*A.付数),4) as 单价,A.实收金额," & _
            "   A.收费类别,B.标识主码,B.编码 as 编码,B.名称 as 项目名称,B.规格,D.项目编码 医保编码," & _
            "   C.名称 开单部门,E.名称 受单部门" & _
            " From (Select * From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E " & _
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+) And A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类=" & TYPE_重庆银海版 & _
            " Order by A.ID"
    Call OpenRecordset(rs明细, "读取费用明细记录")
...
'    序号    数据类型    长度    精度    说明
'    1.      string          传入的处方明细信息文件保存的路径及文件名
    Call 调用接口_准备_重庆银海版("12", GetFileName(处方明细))
    If Not 调用接口_重庆银海版 Then
        gcn重庆.RollbackTrans
        MsgBox "处方明细上传失败！", vbInformation, gstrSysName
        Exit Function
    End If
    With rs明细
        .Filter = 0
    ...
    '得到开始发票号
    gstrSQL = "Select 病人ID,实际票号 From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And 序号=1"
    Call OpenRecordset(rsTemp, "获取发票号")
...
'    1.      string  18      结算交易流水号
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & TYPE_重庆银海版 & "," & lng病人ID & "," & _
        ...
        0 & "," & cur个人帐户 & ",'" & gComInfo_重庆银海版.处方流水号 & "',null,null,'" & gComInfo_重庆银海版.业务类型 & "|" & gComInfo_重庆银海版.就诊流水号 & "|" & gComInfo_重庆银海版.结算流水号 & "|" & gComInfo_重庆银海版.就诊时间 & "|" & gComInfo_重庆银海版.疾病编码 & "|" & gComInfo_重庆银海版.并发症 & "')"
...
'    12.     string  50      并发症
    ...
    gstrSQL = " Select A.流水号,A.业务类型,B.编码 疾病编码,A.并发症 From 保险帐户 A,保险病种 B " & _
              " Where A.病人ID=" & lng病人ID & " And A.险类=" & TYPE_重庆银海版 & " And A.病种ID=B.ID"
    Call OpenRecordset(rsTemp, "获取其它相关信息")
...
Public Function 门诊结算冲销_重庆银海版(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    gstrSQL = " Select B.医保号 个人编号,A.支付顺序号,A.备注,B.结算流水号 From 保险结算记录 A,保险帐户 B " & _
              " Where A.性质=1 And A.记录ID=" & lng结帐ID & " And A.病人ID=B.病人ID And B.险类=" & TYPE_重庆银海版
    Call OpenRecordset(rsTemp, "获取上次结算时的就诊流水号")
    ...
    gComInfo_重庆银海版.处方流水号 = NVL(rsTemp!支付顺序号)
    ...
    '取冲销记录的结帐ID，单据号
    gstrSQL = "select distinct A.结帐ID,A.NO from 病人费用记录 A,病人费用记录 B where A.NO=B.NO and A.记录性质=B.记录性质 and A.记录状态=2 and B.结帐ID=" & lng结帐ID
    ...
    gstrSQL = " Select A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 登记时间,A.收费类别," & _
              ...
              " From 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & TYPE_重庆银海版 & ") C " & _
              " Where A.记录性质=1 And A.记录状态=2 And A.NO='" & strBillNO & "'" & _
    ...
    '产生处方明细计算用的交换文件（因为NO号在预结算时还没有，因此无法使用预结算接口返回的处方明细文件）
    '----处方明细计算----
    strBillNO = "Z9999999"      '退费时无发票号
    If Not MakeFile_Recipe(rs明细, True, False, gComInfo_重庆银海版.处方流水号) Then
        gcn重庆.RollbackTrans
        Exit Function
    End If
    strFileName = GetFileName(处方明细)
...
Public Function 入院登记_重庆银海版(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
    ...
    gstrSQL = " Select to_char(A.入院日期,'yyyy-MM-dd') 入院日期,B.名称 科室,A.住院医师 医生 From 病案主页 A,部门表 B " & _
              " Where A.病人ID=" & lng病人ID & " And A.主页ID=" & lng主页ID & " And A.入院科室ID=B.ID"
    Call OpenRecordset(rsTemp, "获取入院日期")
    ...
    strInput = gComInfo_重庆银海版.个人编号 & gstrSplit_Col_重庆银海版 & str流水号 & gstrSplit_Col_重庆银海版 & _
             ...
             rsTemp!医生 & gstrSplit_Col_重庆银海版 & gComInfo_重庆银海版.就诊时间 & gstrSplit_Col_重庆银海版 & _
             gComInfo_重庆银海版.疾病编码 & gstrSplit_Col_重庆银海版 & ToVarchar(gstrUserName, 20) & gstrSplit_Col_重庆银海版 & _
             ToVarchar(gComInfo_重庆银海版.并发症, 50) & gstrSplit_Col_重庆银海版 & GetFileName(待遇信息)
    Call 调用接口_准备_重庆银海版("08", strInput)
    If Not 调用接口_重庆银海版() Then
...
Public Function 入院登记撤销_重庆银海版(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select 1 From 病人费用记录 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsTemp, "判断是否存在费用记录")
    ...
    '获取原就诊流水号
    gstrSQL = "Select 流水号 From 保险帐户 Where 险类=" & TYPE_重庆银海版 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "获取就诊流水号")
...
'    akc195  9   出院原因    其他
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) Then
        ...
        Call OpenRecordset(rsTemp, "判断是否该调用就诊登记撤销")
        If Not rsTemp.EOF Then
            bln结帐 = True
        End If
        ...
        If bln结帐 Then
            ...
            str科室 = rsTemp!科室
            str医生 = IIf(IsNull(rsTemp!医生), "", rsTemp!医生)
...
Public Function 住院虚拟结算_重庆银海版(rsExse As Recordset, ByVal lng病人ID As Long) As String
    ...
    gstrSQL = " Select A.医保号,A.流水号,A.业务类型,A.统筹区号,A.结算流水号,A.并发症,B.编码 疾病编码 From 保险帐户 A,保险病种 B" & _
              " Where A.病人ID=" & lng病人ID & " And A.险类=" & TYPE_重庆银海版 & " And A.病种ID=B.ID"
    Call OpenRecordset(rsTemp, "取该病人的相关业务信息")
    With gComInfo_重庆银海版
        .个人编号 = rsTemp!医保号
        .并发症 = NVL(rsTemp!并发症)
        .就诊流水号 = rsTemp!流水号
    ...
    With rsExse
        ...
        If bln上传 Then
            ...
            '对未上传的明细进行计算
            If Not MakeFile_Recipe(rsExse, False, False) Then
                gcn重庆.RollbackTrans
                gcn上传.RollbackTrans
                Exit Function
            End If
    ...
    If blnTrans = False Then gcn重庆.BeginTrans: blnTrans = True
...
Public Function 住院结算_重庆银海版(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    '得到开始发票号
    gstrSQL = "Select 病人ID,实际票号 From 病人结帐记录 Where ID=" & lng结帐ID & " And Rownum<2"
    Call OpenRecordset(rsTemp, "获取发票号")
    strBillNO = NVL(rsTemp!实际票号)
    
    '得到主页ID
    gstrSQL = "Select Nvl(住院次数,0) 主页ID From 病人信息 Where 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取该病人的主页ID")
    lng主页ID = rsTemp!主页ID
    
    '结算类型  0-正常结算;1-中途结算;2-转家庭病床结算;3-卡挂失结算
    intState = 1
    gstrSQL = "Select Nvl(当前状态,0) 状态 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_重庆银海版
    Call OpenRecordset(rsTemp, "取病人当前状态")
...
'    1.      string  18      结算交易流水号
    ...
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & TYPE_重庆银海版 & "," & lng病人ID & "," & _
        ...
        0 & "," & lng主页ID & "," & 0 & "," & 0 & "," & 0 & "," & _
        gComInfo_重庆银海版.总费用 & "," & cur现金 & "," & 0 & "," & cur公务员补助基金 & "," & cur医保基金 & "," & cur大病基金 & "," & _
        0 & "," & cur个人帐户 & ",null,null,null,'" & gComInfo_重庆银海版.业务类型 & "|" & gComInfo_重庆银海版.就诊流水号 & "|" & gComInfo_重庆银海版.结算流水号 & "|" & gComInfo_重庆银海版.就诊时间 & "|" & gComInfo_重庆银海版.疾病编码 & "|" & gComInfo_重庆银海版.并发症 & "')"
    ...
    住院结算_重庆银海版 = True
    '同时办理出院登记
    If intState <> 1 Then Call 出院登记_重庆银海版(lng病人ID, lng主页ID)
    Exit Function
ErrHand:
    If ErrCenter = 1 Then
        Resume
...
'功能：更新病人的出院疾病。如果是肿瘤，则结算时起付线会减半
    ...
    gstrSQL = " Select B.编码 病种编码,A.并发症,A.业务类型,A.流水号 From 保险帐户 A,保险病种 B " & _
              " Where A.病种ID=B.ID And A.险类=" & TYPE_重庆银海版 & " ANd A.险类=B.险类 And A.病人ID=" & lng病人ID
    ...
    '根据病种编码取该病种的ID
    gstrSQL = "Select ID From 保险病种 Where 险类=" & TYPE_重庆银海版 & " And 编码='" & str疾病编码 & "'"
    Call OpenRecordset(rsTemp, "获取病种ID")
    lng病种ID = rsTemp!ID
...
Private Sub TestVerifyItem()
    ...
    gstrSQL = " Select 流水号,处方流水号 From 中间库_处方明细" & _
              " Where Nvl(trim(审批标志),0) = 0 And 流水号='" & gComInfo_重庆银海版.就诊流水号 & "'"
    Call OpenRecordset(rsTemp, "再次获取未通过审批的处方明细的审批信息")
    ...
    Do While Not rsTemp.EOF
        ...
        If 调用接口_重庆银海版 Then
            ...
            gcn重庆.Execute "zl_中间库_处方明细_UPDATE(" & _
...
Public Sub 核对费用结算_重庆银海版()
    ...
    Dim cur基本统筹_医保 As String, cur大病统筹_医保 As Currency, cur公务员补助_医保 As Currency
    Dim cur基本统筹_医院 As String, cur大病统筹_医院 As Currency, cur公务员补助_医院 As Currency
    Dim arrReturn
    Dim rsTemp As New ADODB.Recordset
    
    If frm日期范围_米易.Show_ME(strStart, strEnd) = False Then Exit Sub
    gstrSQL = " Select min(流水号) 开始流水号,max(流水号) 结束流水号 From zlcq.中间库_结算信息" & _
            " Where 经办时间 Between To_Date('" & strStart & "','yyyy-MM-dd hh24:mi:ss') And " & _
            " To_Date('" & strEnd & "','yyyy-MM-dd hh24:mi:ss')"
    Call OpenRecordset(rsTemp, "获取指定时间范围内的流水号")
...
'    5.      number  14  2   在核对范围内的所有记录的大额理赔总额累加值
    ...
    gstrSQL = " Select Sum(Nvl(本次基本统筹金额,0)) 基本统筹,SUM(Nvl(大病支付金额,0)) 大病统筹," & _
              ...
              " From zlcq.中间库_结算信息" & _
              " Where 流水号>='" & str就诊流水号_开始 & "' And 流水号<='" & str就诊流水号_结束 & "'"
    Call OpenRecordset(rsTemp, "提取中间库中的结算信息")
    cur基本统筹_医保 = Val(NVL(rsTemp!基本统筹, 0))
    cur公务员补助_医保 = Val(NVL(rsTemp!大病统筹, 0))
    cur大病统筹_医保 = Val(NVL(rsTemp!公务员补助, 0))
