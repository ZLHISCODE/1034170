...
Private Function GetServerInfo() As Boolean
    ...
    gstrSQL = " Select 参数名,参数值 From 保险参数" & _
              " Where 险类=" & TYPE_沈阳市 & " And 参数名 Like '服务器%'"
    Call OpenRecordset(rsTemp, "获取服务器地址、端口及入口名称")
...
Public Function 挂号结算_沈阳市(ByVal lng结帐ID As Long) As Boolean
    ...
    gstrSQL = "Select 参数值 Value From 保险参数 Where 险类=" & TYPE_沈阳市 & " And 参数名='个人帐户支出(挂号)'"
    Call OpenRecordset(rsTemp, "读取由个人帐户支付的收入项目ID")
    ...
    gstrSQL = "Select Rownum 标识号,A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,A.登记时间,A.开单人," & _
            ...
            " From (Select * From 病人费用记录 Where 结帐ID=" & lng结帐ID & ") A,收费细目 B,部门表 C,保险支付项目 D,部门表 E " & _
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+) And A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类=" & gintInsure & _
            " Order by A.ID"
    Call OpenRecordset(rsTemp, "读取本次结帐费用明细")
    With rsTemp
        Do While Not .EOF
...
Public Function 挂号冲销_沈阳市(ByVal lng结帐ID As Long) As Boolean
    ...
    '取被冲销记录的结帐ID，病人ID
    gstrSQL = "select distinct A.结帐ID,A.病人ID from 病人费用记录 A,病人费用记录 B where A.NO=B.NO and A.记录性质=B.记录性质 and A.记录状态=2 and B.结帐ID=" & lng结帐ID
    ...
    gstrSQL = "Select Nvl(A.冲预交,0) 个人帐户 " & _
        ...
        " And A.结算方式 in ('个人帐户') And A.结帐ID=" & lng记录ID
    Call OpenRecordset(rsTemp, "获取原始单据的个人帐户支付额")
    cur个人帐户 = 0
    If Not rsTemp.EOF Then
        cur个人帐户 = rsTemp!个人帐户
    End If
...
Optional ByVal bln挂号 As Boolean = False, Optional ByVal lng收入项目ID As Long = 0) As Boolean
    ...
    '校验个人编号是否正确
    gstrSQL = "Select 卡号 From 保险帐户 Where 险类=" & gintInsure & " And 病人ID=" & rs明细!病人ID
    Call OpenRecordset(rsTemp, "校验该病人是否使用自己的卡进行交易")
    ...
    If bln挂号 Then
        ...
        gstrSQL = "select 医生姓名 from 挂号安排 where 号码='" & rs明细!计算单位 & "'"
        Call OpenRecordset(rsTemp, "获取医生编号")
    ...
    If Trim(str医生姓名) <> "" Then
        gstrSQL = "Select 编号 From 人员表 Where 姓名='" & str医生姓名 & "'"
        Call OpenRecordset(rsTemp, "获取医生编号")
        str医生编号 = rsTemp!编号
    End If
...
'            12  opp_serial_fee 对应费用序列号      12  是
            ...
            If !收费类别 = 5 Or !收费类别 = 6 Or !收费类别 = 7 Then
                gstrSQL = " Select C.标识码,C.编码,B.名称 剂型 From 药品信息 A,药品剂型 B,药品目录 C " & _
                          " Where A.剂型=B.编码 And A.药名ID=C.药名ID And C.药品ID =" & !收费细目ID
                Call OpenRecordset(rsPhysic, "取剂型")
            ...
            '取规格和产地
            gstrSQL = "Select 编码,名称,规格,标识主码 From 收费细目 Where ID=" & !收费细目ID
            Call OpenRecordset(rsTemp, "读取收费细目相关信息")
            str产地 = ""
            str规格 = NVL(rsTemp!规格)
...
Optional ByVal bln挂号 As Boolean = False, Optional ByVal lng收入项目ID As Long = 0) As Boolean
    ...
    If bln挂号 Then
        gstrSQL = "select 医生姓名 开单人 from 挂号安排 where 号码=(Select 计算单位 From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Rownum<2)"
        Call OpenRecordset(rsTemp, "获取医生编号")
    Else
        gstrSQL = "Select 开单人 From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Rownum<2"
        Call OpenRecordset(rsTemp, "读取开单医生")
    End If
    str医生姓名 = NVL(rsTemp!开单人)

    If Trim(str医生姓名) <> "" Then
        gstrSQL = "Select 编号,姓名 From 人员表 Where 姓名='" & str医生姓名 & "'"
        Call OpenRecordset(rsTemp, "获取医生编号")
        str医生编号 = rsTemp!编号
    End If
...
Public Function 门诊结算冲销_沈阳市(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long, Optional ByVal bln退费 As Boolean = True) As Boolean
    ...
    '校验个人编号是否正确
    gstrSQL = "Select 卡号 From 保险帐户 Where 险类=" & gintInsure & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "校验该病人是否使用自己的卡进行交易")
    If rsTemp!卡号 <> str个人编号 Then
        MsgBox "该病人不是使用自己的医保卡，结算操作中止！", vbInformation, gstrSysName
        Exit Function
    ...
    '取原结算记录的明细
    gstrSQL = "Select * From 保险结算记录 Where 险类=" & gintInsure & " And 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "读取结算记录")
    str业务序列号 = rsTemp!支付顺序号
    str业务类型 = rsTemp!备注
    
    '产生冲销结算记录
    With rsTemp
        gstrSQL = "zl_保险结算记录_insert(" & !性质 & "," & lng记录ID & "," & gintInsure & "," & !病人ID & "," & _
        ...
        Call ExecuteProcedure("产生冲销结算记录")
    End With
    ...
    Else
        ...
        gstrSQL = "Select 记录性质,NO From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Rownum<2"
        Call OpenRecordset(rsTemp, "获取原单据相关信息")
        int性质 = rsTemp!记录性质
        int状态 = 2
        str单据号 = rsTemp!NO
...
'    1   serialno    业务序列号  12
    ...
    '获取入院病种编码
    gstrSQL = "Select 编码,名称 From 保险病种 Where ID=(Select 病种ID From 保险帐户 Where 病人ID=" & lng病人ID & ")"
    Call OpenRecordset(rsTemp, "获取疾病ID")
    ...
    gstrField_沈阳市 = "hospital_id||indi_id||busi_type||reg_staff||reg_man||reg_flag||" & _
                       ...
                       "foregift||foregift_remain||in_disease||ic_flag"
    gstrValue_沈阳市 = gCominfo_沈阳市.医院编码 & "||" & gCominfo_沈阳市.个人编号 & "||" & _
                       gCominfo_沈阳市.业务类型 & "||" & gCominfo_沈阳市.操作员工号 & "||" & _
                       gstrUserName & "||0||||||" & Format(zlDatabase.Currentdate, "yyyy-MM-dd") & "||" & _
    ...
    '更新个人帐户中的信息
    gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & gintInsure & ",'顺序号','''" & gCominfo_沈阳市.业务序列号 & "''')"
...
Public Function 出院登记_沈阳市(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    Dim str就诊日期 As String, str病种编码 As String, str病种名称 As String
...
'    12  ic_flag        用卡标志    1   否  "0"：不使用IC卡；"1"：使用IC卡
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) Then
        ...
        Call OpenRecordset(rsTemp, "出院方式")
        int出院情况 = rsTemp!出院方式
    ...
    '    16  note           备注        100 是
        gstrField_沈阳市 = "hospital_id||serial_no||busi_type||staff_no||staff_name||begin_date||" & _
                        "in_dept||in_dept_name||in_area||in_area_name||in_bed||bed_type||patient_id||old_patient_id||note"
        gstrValue_沈阳市 = gCominfo_沈阳市.医院编码 & "||" & gCominfo_沈阳市.业务序列号 & "||" & _
                        gCominfo_沈阳市.业务类型 & "||" & gCominfo_沈阳市.操作员工号 & "||" & _
                        ...
                        arrPatient(入院病区名称) & "||" & arrPatient(入院病床编号) & "||" & _
                        arrPatient(床位类型) & "||" & arrPatient(住院号) & "||" & _
                        arrPatient(住院号) & "||" & str病种编码 & "||"
        If Not 调用接口_准备_沈阳市(Function_沈阳市.住院信息_修改) Then Exit Function
        If Not 调用接口_写入口参数_沈阳市(1) Then Exit Function
        If Not 调用接口_执行_沈阳市 Then Exit Function
        
        '需要操作员选择出院病种，如果不选择，出院病种编码传入院病种编码
        Dim rs病种 As ADODB.Recordset
        gstrSQL = " Select A.ID,A.编码,A.名称,A.简码" & _
                " From 保险病种 A where A.险类=" & TYPE_沈阳市 & " Order by A.编码"
        Set rs病种 = frmPubSel.ShowSelect(Nothing, gstrSQL, 0, "请选择该参保病人的出院病种")
        ...
        gstrValue_沈阳市 = gCominfo_沈阳市.医院编码 & "||" & gCominfo_沈阳市.业务序列号 & "||" & _
                        ...
                        str病种编码 & "||" & Format(zlDatabase.Currentdate, "yyyy-MM-dd") & "||||" & _
...
'    6   patient_id     住院号          20  否
    ...
    Call OpenRecordset(rsTemp, "取住院相关信息")
    If Not rsTemp.EOF Then
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) Then
        ...
        gstrSQL = "Select decode(A.出院方式,'转院',3,0) 出院方式,B.住院号 " & _
                ...
                " Where A.病人ID=B.病人ID And A.病人ID = " & lng病人ID & " And A.主页ID = " & lng主页ID
        Call OpenRecordset(rsTemp, "出院方式")
        int出院情况 = rsTemp!出院方式
...
Public Function 住院虚拟结算_沈阳市(rsExse As Recordset, ByVal lng病人ID As Long) As String
    ...
    If Not IsNull(rsExse!医生) Then
        gstrSQL = "Select 编号 From 人员表 Where 姓名='" & rsExse!医生 & "'"
        Call OpenRecordset(rsTemp, "获取医生编号")
        str医生编号 = rsTemp!编号
        str医生姓名 = rsExse!医生
    End If
        ...
        With rsExse
            ...
            Do While Not .EOF
                If NVL(!金额, 0) < 0 And NVL(!是否上传, 0) = 0 Then
                    ...
                    If !记录状态 = 1 Then
                        ...
                        If !收费类别 = 5 Or !收费类别 = 6 Or !收费类别 = 7 Then
                            gstrSQL = "Select Decode(trim(标识码),NULL,编码,'',编码,标识码) 编码 From 药品目录 Where 药品ID=" & !收费细目ID
                        Else
                            gstrSQL = "Select Decode(trim(标识主码),NULL,编码,'',编码,标识主码) 编码 From 收费细目 Where ID=" & !收费细目ID
                        End If
                        Call OpenRecordset(rsTemp, "读取项目的编码或标识码")
                        str医保序号 = str医保序号 & IIf(str医保序号 = "", "", "|") & GetInsureSerial2(rsTemp!编码, NVL(!金额, 0), IIf(str医保序号 = "", True, False))
                    Else
                        gstrSQL = " Select ID From 病人费用记录" & _
                                  " Where 记录性质=" & !记录性质 & " And 记录状态=3 And NO='" & !NO & "' And 序号=" & !序号
                        Call OpenRecordset(rsTemp, "读取原始记录的费用ID")
                        lng原始费用ID = rsTemp!ID
            ...
            Do While Not .EOF
                ...
                If NVL(!是否上传, 0) = 0 Then
                    ...
                    gstrSQL = "Select 标识主码,编码,名称,规格 From 收费细目 Where ID=" & !收费细目ID
                    Call OpenRecordset(rsTemp, "读取收费细目相关信息")
                    str规格 = NVL(rsTemp!规格)
                    str产地 = ""
                    If InStr(1, str规格, "|") <> 0 Then
                    ...
                    gstrSQL = "Select ID From 病人费用记录 Where NO='" & !NO & "' And 记录性质=" & !记录性质 & " And 记录状态=" & !记录状态 & " And 序号=" & !序号
                    Call OpenRecordset(rsDetail, "读取费用ID")
...
Public Function 住院结算_沈阳(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = "Select Nvl(A.冲预交,0) 个人帐户 " & _
        " From 病人预交记录 A,保险帐户 B " & _
        " Where A.病人ID=B.病人ID And B.险类=" & gintInsure & _
        " And A.结算方式='个人帐户' And A.结帐ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "读取帐户支付额")
    cur帐户支付 = 0
    If Not rsTemp.EOF Then
...
Public Function 住院结算冲销_沈阳市(lng结帐ID As Long) As Boolean
    ...
    '取冲销记录的结帐ID
    gstrSQL = "select distinct A.ID,A.病人ID from 病人结帐记录 A,病人结帐记录 B where A.NO=B.NO and  A.记录状态=2 and B.ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "读新产生的结帐ID")
    lng记录ID = rsTemp!ID
    lng病人ID = rsTemp!病人ID
...
'    3   ic_flag 用卡标志    1   是  "0"：不使用IC卡；"1"：使用IC卡
    ...
    gstrValue_沈阳市 = gCominfo_沈阳市.医院编码 & "||" & gCominfo_沈阳市.业务序列号 & "||" & mintICCard
...
Public Function 个人余额_沈阳市(ByVal strSelfNo As String, Optional ByVal bln卡号 As Boolean = True) As Currency
    ...
    gstrSQL = " Select A.卡号,B.工作单位,Nvl(A.帐户余额,0) 帐户余额 From 保险帐户 A,病人信息 B " & _
              " Where A.病人ID=B.病人ID And A.险类=" & gintInsure
    If bln卡号 Then
        gstrSQL = gstrSQL & " And A.医保号='" & strSelfNo & "'"
    Else
        gstrSQL = gstrSQL & " And A.病人ID=" & Val(strSelfNo)
    End If
    Call OpenRecordset(rsAccount, "获取卡号及单位编码")
...
'    3   corp_ID    单位编号    3
    ...
    '更新个人帐户并返回
    gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & gintInsure & ",'帐户余额','" & cur帐户余额 & "')"
    Call ExecuteProcedure("更新帐户余额")
    个人余额_沈阳市 = cur帐户余额
...
Private Function 获取病人基本信息(ByVal lng病人ID As Long, Optional bln门诊 As Boolean = True) As Boolean
    ...
    Call OpenRecordset(rsTemp, "读取病人基本信息")
...
'    "3"：高干
    ...
    gstrSQL = " Select nvl(住院次数累计,1) 住院次数 From 帐户年度信息 " & _
              " Where 病人ID=" & lng病人ID & " And 年度=" & Format(zlDatabase.Currentdate, "yyyy")
    Call OpenRecordset(rsTemp, "读取病人相关信息")
    ...
    gstrSQL = "select C.编码 入院科室编号,C.名称 入院科室名称,B.编码 入院病区编号,B.名称 入院病区名称, " & _
             ...
             " Where A.入院病区ID=B.ID(+) And A.入院科室ID=C.ID(+) And A.病人ID=E.病人ID ANd A.病人ID=" & lng病人ID & " And A.主页ID=" & lng主页ID & _
...
Public Function 处方登记_沈阳市(ByVal int性质 As Integer, ByVal int状态 As Integer, ByVal str单据号 As String) As Boolean
    ...
    Dim blnUp As Boolean, blnMoveNext As Boolean, blnInsure As Boolean, blnFirstAID As Boolean
    Dim rsExse As New ADODB.Recordset
    Dim rsTemp As New ADODB.Recordset
    ...
    gstrSQL = " Select A.ID,A.病人ID,A.主页ID,A.NO,A.序号,A.记录性质,A.记录状态,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 登记时间,A.收费类别," & _
              ...
              " From 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & TYPE_沈阳市 & ") C " & _
        ...
        Do While Not .EOF
            If NVL(!金额, 0) < 0 Then
                If lng病人ID <> !病人ID Then
                    '检查本次是否以医保身份入院
                    gstrSQL = "Select Count(*) Records From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & !病人ID & " And A.主页ID=B.住院次数 And A.险类=" & TYPE_沈阳市
        ...
        Do While Not .EOF
            ...
            If lng病人ID <> !病人ID Or strNO <> !NO Then
                ...
                If blnInsure Then
                    ...
                    If Not 调用接口_准备_沈阳市(Function_沈阳市.住院费用_上传明细) Then Exit Function
                    If Not 调用接口_写入口参数_沈阳市(1) Then Exit Function
                    blnUp = True
            ...
            If blnInsure Then
                ...
                If Not 调用接口_写入口参数_沈阳市(lngRecords) Then Exit Function
            ...
            .MoveNext
        Loop
        If blnUp And blnInsure Then If Not 调用接口_执行_沈阳市() Then Exit Function
    End With
...
Public Function 处方作废_沈阳市(ByVal int性质 As Integer, ByVal int状态 As Integer, ByVal str单据号 As String) As Boolean
    ...
    With rsExse
        ...
        Do While Not .EOF
            ...
            If blnInsure Then
                gstrSQL = " Select ID From 病人费用记录" & _
                          " Where 记录性质=" & int性质 & " And 记录状态=" & int原始记录状态 & " And NO='" & str单据号 & "' And 序号=" & !序号
                Call OpenRecordset(rsTemp, "读取原始记录的费用ID")
                lng原始费用ID = rsTemp!ID
...
Public Function 提取结算单_沈阳市(ByVal lng病人ID As Long, ByVal lng结帐ID As Long, int业务类型 As Integer, str业务序列号 As String) As Boolean
    ...
    gstrSQL = " Select 性质,支付顺序号 业务序列号,备注 业务类型 From 保险结算记录 " & _
              " Where 险类=" & TYPE_沈阳市 & " And 病人ID=" & lng病人ID & " And 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "提取该病人本次业务的业务类型及业务序列号")
    If rsTemp.EOF Then
...
'    '取各项业务基金支付信息（多条）
'    Call DebugTool("取基金支付信息")
'    If Not 调用接口_指定记录集_沈阳市("Fund") Then GoTo ExitSub
'    gstrSQL = " Insert Into 结算单_结算明细" & _
'              "(业务序列号,基金编码,基金名称,支付金额)" & _
'              " Values ('" & str业务序列号 & "'"
'    If 调用接口_记录数_沈阳市 Then
'        Do While True
'            strTemp = gstrSQL
'            Call 调用接口_读取数据_沈阳市("fund_id", strData)
'            Call CombinateString(strTemp, strData)
'            Call 调用接口_读取数据_沈阳市("fund_name", strData)
'            Call CombinateString(strTemp, strData)
'            Call 调用接口_读取数据_沈阳市("real_pay", strData)
'            Call CombinateString(strTemp, strData)
'            strTemp = strTemp & ")"
'            gcnOracle.Execute strTemp
'
...
Public Sub 更新病种_沈阳市(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal str病种编码 As String)
    Dim arrPatient
    Dim str就诊日期 As String
    Dim rsTemp As New ADODB.Recordset
    ...
    '    16  note           备注        100 是
    gstrField_沈阳市 = "hospital_id||serial_no||busi_type||staff_no||staff_name||begin_date||" & _
                    "in_dept||in_dept_name||in_area||in_area_name||in_bed||bed_type||patient_id||old_patient_id||in_disease||note"
    gstrValue_沈阳市 = gCominfo_沈阳市.医院编码 & "||" & gCominfo_沈阳市.业务序列号 & "||" & _
                    gCominfo_沈阳市.业务类型 & "||" & gCominfo_沈阳市.操作员工号 & "||" & _
                    gstrUserName & "||" & str就诊日期 & "||" & arrPatient(入院科室编号) & "||" & _
                    arrPatient(入院科室名称) & "||" & arrPatient(入院病区编号) & "||" & _
...
Private Function FirstAid() As Boolean
    Dim rsTemp As New ADODB.Recordset
    '检查是否存在使用急救用药，如果是，返回真
    
    gstrSQL = " Select Nvl(参数值,0) As 参数值 From 保险参数 " & _
              " Where 险类=" & TYPE_沈阳市 & " And 参数名='允许急救用药'"
    Call OpenRecordset(rsTemp, "判断是否存在急救用药")
    If Not rsTemp.EOF Then
        FirstAid = (rsTemp!参数值 = 1)
    End If
End Function
