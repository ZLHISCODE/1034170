...
'       医疗照顾人员的自费部分、医疗照顾人员的统筹部分、公务员统筹支付部分、人员状态、初始化机构名称、特殊挂钩支付部分、错误代码；
...
Public Function 医保初始化_云南() As Boolean
...
Public Function 身份标识_云南(Optional bytType As Byte, Optional lng病人ID As Long) As String
...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    Dim str身份证号 As String, str出生日期 As String, lng年龄 As Long, str单位编码 As String, str单位名称 As String
    ...
    If bytType = 0 And lng病种ID > 0 Then
        ...
        Call yh_admit(str卡类型, LeftDB(UserInfo.姓名, 8), gstr医院编码, LeftDB(UserInfo.姓名, 8), "门诊", _
            LeftDB(lng病人ID, 12), LeftDB(lng病人ID, 12), IIf(Val(str疾病编码) >= 90, "0", "1"), LeftDB(str疾病编码, 8), _
...
Public Function 个人余额_云南(strSelfNo As String, ByVal bytPlace As Byte) As Currency
'功能: 提取参保病人个人帐户余额
'参数: strSelfNO-病人个人编号
...
Public Function 门诊虚拟结算_云南(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
'参数：rsDetail     费用明细(传入)
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    On Error GoTo errHandle
    '判断该病人是否属于特殊门诊
    gstrSQL = "select nvl(病种ID,0) 病种ID from 保险帐户 where 病人ID=" & mlng病人ID & " and 险类=" & gintInsure
...
'    End If
    ...
    Do Until rs明细.EOF
        ...
        If rsTemp.EOF = True Then
            ...
            Exit Function
        End If
        If str疾病编码 >= 6 And str疾病编码 < 90 Then
            If ToVarchar(rsTemp("项目编码"), 2) <> "01" And ToVarchar(rsTemp("项目编码"), 2) <> "02" Then
...
Public Function 门诊结算_云南(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
...
'        Loop
    ...
    str初始化机构 = Space(4)
    mstrErr = "0000"
    If CDbl(cur个人帐户) <> 0 Then Call yh_cardpay(str卡类型, mstr顺序号, str医生, "门诊收费", CDbl(cur个人帐户), str初始化机构, mstrErr)
    ...
    str初始化机构 = Space(4)
    mstrErr = "0000"
    Call yh_feebalance(mstr顺序号, str医生, str科室, str结算事务号, _
        cur全自付, cur挂钩自付, cur统筹支付, cur统筹自付, cur基数自付, cur超限自付, cur大病统筹, _
        cur大病自付, cur特殊人员自付, cur特殊人员统筹, cur公务员统筹, str人员状态, str初始化机构, cur特殊挂钩支付部分, mstrErr)
    If mstrErr <> "0000" Then
        MsgBox GetErrInfo(mstrErr), vbInformation, gstrSysName
        '医保数据库回滚
...
Public Function 门诊结算冲销_云南(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
'功能：将门诊收费的明细和结算数据转发送医保前置服务器确认；
'参数：lng结帐ID     收费记录的结帐ID；，从预交记录中可以检索医保号和密码
'      cur个人帐户   从个人帐户中支出的金额
...
Public Function 个人帐户转预交_云南(lng预交ID As Long, cur个人帐户 As Currency, strSelfNo As String, str顺序号 As String, ByVal lng病人ID As Long) As Boolean
...
Public Function 入院登记_云南(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
...
'返回：交易成功返回true；否则，返回false
    ...
    mstrErr = "0000"
    If str疾病编码 = "0093" Then    '只有市医保才支持年终结转住院
...
Public Function 出院登记_云南(lng病人ID As Long, lng主页ID As Long, str顺序号 As String, Optional ByVal bln结帐出院 As Boolean = False) As Boolean
'功能：将出院信息发送医保前置服务器确认；
'参数：lng病人ID-病人ID；lng主页ID-主页ID
'返回：交易成功返回true；否则，返回false
            '取入院登记验证所返回的顺序号
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) Then
        ...
        mstrErr = "0000"
        Call yh_feebalance(mstr顺序号, LeftDB(UserInfo.姓名, 8), LeftDB(UserInfo.部门, 24), str事务号, cur全自付, _
            cur挂钩自付, cur统筹支付, cur统筹自付, cur基数自付, cur超限自付, _
            cur大病统筹, cur大病自付, cur特殊人员自付, cur特殊人员统筹, cur公务员统筹, _
            str人员状态, str初始化机构, cur特殊挂钩支付部分, mstrErr)
        If mstrErr <> "0000" Then
            MsgBox GetErrInfo(mstrErr), vbInformation, gstrSysName
            '医保数据库回滚
...
Public Function 出院登记撤消_云南(lng病人ID As Long, lng主页ID As Long) As Boolean
'功能：将出院信息发送医保前置服务器确认；
'参数：lng病人ID-病人ID；lng主页ID-主页ID
'返回：交易成功返回true；否则，返回false
            '取入院登记验证所返回的顺序号
...
Public Function 住院虚拟结算_云南(rsExse As Recordset, ByVal lng病人ID As Long) As String
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    Dim str发生时间 As String, str本次入院时间 As String
    Dim rsTemp As New ADODB.Recordset
    ...
    Do Until rsExse.EOF
        ...
        str发生时间 = Format(rsExse("登记时间"), "yyyy-MM-dd HH:mm:ss")
        ...
        '如果登记时间小于本次住院时间则不上传
        If Format(rsExse("登记时间"), "yyyy-MM-dd") >= str本次入院时间 Then
...
Public Function 住院结算_云南(lng结帐ID As Long, str顺序号 As String, ByVal lng病人ID As Long) As Boolean
...
'      3)由于结帐之后，可能使用结帐作废交易，这时需要结帐时执行结算交易的交易号，因此我们需要同时结帐交易号。(由于门诊收费作废时，已经不再和医保有关系，所以不需要保存结帐的交易号)
    ...
    '先读取医保号
    gstrSQL = "Select 医保号 From 保险帐户 Where 险类=" & gintInsure & " And 病人ID=" & lng病人ID
    ...
    str医生 = LeftDB(UserInfo.姓名, 8)
    mstrErr = "0000"
    If CDbl(cur个人帐户) <> 0 Then Call yh_cardpay(str卡类型, mstr顺序号, str医生, "住院结算", CDbl(cur个人帐户), str初始化机构, mstrErr)
    If mstrErr <> "0000" Then
        MsgBox GetErrInfo(mstrErr) & "（如果下卡失败,请补缴现金￥" & Format(cur个人帐户, "#####0.00") & "）, vbInformation, gstrSysName"
        cur个人帐户 = 0
    End If
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) And 医保病人已经出院(lng病人ID) Then
        ...
        Call 出院登记_云南(lng病人ID, lng主页ID, rsTemp!顺序号, True)
    End If
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
...
Public Function 住院结算冲销_云南(lng结帐ID As Long) As Boolean
    '----------------------------------------------------------------
    ...
    If CDbl(cur个人帐户) <> 0 Then
        ...
        If mstrErr <> "0000" Then
            MsgBox GetErrInfo(mstrErr) & "（如果恢复个人帐户余额失败,请退给医保病人￥" & Format(cur个人帐户, "#####0.00") & "现金）", vbInformation, gstrSysName
            cur个人帐户 = 0
        End If
    End If
...
Public Function 撤消急诊登记_云南(ByVal str顺序号 As String) As Boolean
'功能：撤消急诊登记
    Dim rsTemp As New ADODB.Recordset
    Dim str事务号 As String   '事务控制号
    ...
    mstrErr = "0000"
    Call yh_feebalance(str顺序号, LeftDB(UserInfo.姓名, 8), LeftDB(UserInfo.部门, 24), str事务号, cur全自付, _
        cur挂钩自付, cur统筹支付, cur统筹自付, cur基数自付, cur超限自付, _
        cur大病统筹, cur大病自付, cur特殊人员自付, cur特殊人员统筹, cur公务员统筹, _
        str人员状态, str初始化机构, cur特殊挂钩支付部分, mstrErr)
    If mstrErr <> "0000" Then
