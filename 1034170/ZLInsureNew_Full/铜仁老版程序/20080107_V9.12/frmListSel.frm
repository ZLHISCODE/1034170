...
'     blnMutilSelect=多选标志（如果允许多选，则固定为第一列是选择列）
    ...
    If gintInsure = TYPE_福建巨龙 Or gintInsure = TYPE_福建省 Or gintInsure = TYPE_福州市 Or gintInsure = TYPE_南平市 Then
        mshSelect.Cols = 7
        mshSelect.TextMatrix(0, 1) = "名称"
        mshSelect.TextMatrix(0, 2) = "剂型"
        mshSelect.TextMatrix(0, 3) = "单位"
        mshSelect.TextMatrix(0, 4) = "大类"
        mshSelect.TextMatrix(0, 5) = "发票项目"
        mshSelect.TextMatrix(0, 6) = "简码"
    End If
    ...
    If gintInsure = TYPE_福建巨龙 Or gintInsure = TYPE_福建省 Or gintInsure = TYPE_福州市 Or gintInsure = TYPE_南平市 Then
        ...
        Do Until rsSelect.EOF
            ...
            mshSelect.TextMatrix(lngRow, 6) = IIf(IsNull(rsSelect!简码), "", rsSelect!简码)
    ...
    If Not (gintInsure = TYPE_福建巨龙 Or gintInsure = TYPE_福建省 Or gintInsure = TYPE_福州市 Or gintInsure = TYPE_南平市) Then
