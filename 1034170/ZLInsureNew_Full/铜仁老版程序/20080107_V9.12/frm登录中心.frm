...
Private Sub AutoLogin()
    ...
    Case TYPE_沈阳市, TYPE_乐山
        If mint险类 = TYPE_沈阳市 Then
            gstrSQL = "Select 序号,参数值 From 保险参数 Where 险类=" & TYPE_沈阳市 & " And 序号 In (4,5) Order by 序号"
        Else
            gstrSQL = "Select 序号,参数值 From 保险参数 Where 险类=" & TYPE_乐山 & " And 序号 In (2,3) Order by 序号"
        End If
        Call OpenRecordset(rsTemp, "读取登录参数")
