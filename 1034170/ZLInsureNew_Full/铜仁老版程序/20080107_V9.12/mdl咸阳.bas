...
'        strComputer = "work1"
    ...
    ElseIf rsTemp.RecordCount > 1 Then
        '多于一个时要进行选择
        If frmListSel.ShowSelect(rsTemp, "医保卡号", "医保病人选择", "请选择已验证的病人，然后点击确定。") = False Then
            Exit Function
        End If
    End If
...
'返回: 返回个人帐户余额的金额
    ...
    gstrSQL = "Select 帐户余额 From 保险帐户 where 险类=" & gintInsure & " and 病人ID=" & lng病人ID & ""
    Call OpenRecordset(rsTemp, "咸阳医保")
...
'    strmachine = "WORK1"
    ...
    gstrSQL = "select B.医保号,C.姓名 " & _
              ...
              "  where B.病人id=" & rs明细("病人ID") & " and B.险类=" & TYPE_咸阳市 & " and B.病人ID=C.病人ID"
   ...
   dat发生时间 = zlDatabase.Currentdate
    ...
    Do Until rs明细.EOF
        gstrSQL = "select A.编码,A.名称,A.计算单位,A.类别,B.名称 as 大类,B.统筹比额  " & _
                   " from 收费细目 A , " & _
                   " (select B. 收费细目ID,C.名称,C.统筹比额 from 保险支付大类 C,保险支付项目 B where B.险类=" & gintInsure & " and B.大类ID=C.ID) B " & _
                   " where A.ID=B.收费细目ID(+) and A.ID=" & rs明细("收费细目ID")
        Call OpenRecordset(rsTemp, "门诊预算")
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "select 帐户余额 from 保险帐户 where 险类=" & gintInsure & " and 病人id=" & lng病人ID
    Call OpenRecordset(rs帐户余额, "咸阳医保")
...
'返回：交易成功返回true；否则，返回false
    ...
    '获得病人医保号
    gstrSQL = "select 医保号 from 保险帐户 where 病人ID=" & lng病人ID & " and 险类=" & TYPE_咸阳市
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    gstrSQL = "select B.医保号,C.姓名 " & _
              "  from 保险帐户 B,病人信息 C " & _
              "  where B.病人id=" & lng病人ID & " and B.险类=" & TYPE_咸阳市 & " and B.病人ID=C.病人ID"
    Call OpenRecordset(rsTemp, "门诊预算")
    str姓名 = rsTemp("姓名")
    dat发生时间 = zlDatabase.Currentdate
    
    '首先完成费用明细的传输
    gstrSQL = "select ID,编码,名称,统筹比额,特准定额 FROM 保险支付大类  where 险类=" & gintInsure
    Call OpenRecordset(rs大类, "咸阳医保")
    ...
    Do Until rsExse.EOF
        ...
        If rsExse("是否上传") = 0 Then
            ...
            gstrSQL = "select distinct 编码 from 收费细目 where 名称='" & rsExse("收费名称") & "'"
            Call OpenRecordset(rsTemp, "咸阳医保")
            ...
            gstrSQL = "select b.编码 " _
                    & " From " _
                    & " (select 收入项目id from 病人费用记录 " _
                       & "where no like '" & rsExse("NO") _
                       & "' and 记录性质=" & rsExse("记录性质") _
                       & " and 序号=" & rsExse("序号") _
                       & " and 记录状态=" & rsExse("记录状态") _
                       & " and 病人id=" & rsExse("病人id") & ")  a,收入项目 b " _
                        & " Where a.收入项目id = b.ID "
            Call OpenRecordset(rsTemp, "咸阳医保")
            ...
            gstrSQL = "INSERT INTO InpatiExses(ID, CardNo, PatiName, RecordTime, ItemCode, ItemName, ItemUnit, Price, Amount, Money, ExseKind, InsureKind, PayTax) " & _
                      ...
                      Format(rsExse("登记时间"), "yyyy-MM-dd HH:mm:ss") & "','" & str项目编码 & "','" & ToVarchar(rsExse("收费名称"), 90) & "','个'," & _
            ...
            '由于操作员可能多次进入结算管理，但不结算，所以，这里把本次上传的记录做上标志，以免下次再传
            gstrSQL = "update 病人费用记录 set 是否上传=1 where no like '" & rsExse("NO") & "' and 记录性质=" & rsExse("记录性质") & " and 序号=" & rsExse("序号") & " and 记录状态=" & rsExse("记录状态") & " and 是否上传 is null and 病人id=" & rsExse("病人id")
...
'功能：测试是否可以连接到前置服务器上
    ...
    '首先读出参数，打开连接
    gstrSQL = "Select 参数名,参数值 From 保险参数 Where 险类=" & TYPE_咸阳市
