...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    '获得病人医保号、分中心编号等信息
    If frmIdentify贵阳.GetIdentify(TYPE_成都郊县, str卡号, str医保号, str中心编号, str密码) = False Then Exit Function
    
    '检查该病人是否以医保身份正在住院
    Dim rsTemp As New ADODB.Recordset
    '检查该病人是否在院
    gstrSQL = "select nvl(当前状态,0) as 当前状态 from 保险帐户 where 医保号='" & str医保号 & "' and 险类=" & TYPE_成都郊县
    ...
    str附加 = ";"                                       '8.中心代码
    str附加 = str附加 & ";" & arrOutput(12)             '9.顺序号
    ...
    str附加 = str附加 & ";"                             '17灰度级
    str附加 = str附加 & ";" & arrOutput(11)             '18帐户增加累计
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    str结算方式 = "个人帐户;" & Val(arrOutput(1)) & ";1"  '允许修改个人帐户
    门诊虚拟结算_成都郊县 = True
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    '调用结算
    With g结算数据
    Call WriteLog("reckoning(" & str卡号 & "," & str医保号 & "," & str中心编号 & "," & mstr密码 & "," & str就诊顺序号 & "," & "11" & "," & str医院编码 & "," & "000" & "," & CStr(cur个人帐户) & "," & Format(datCurr, "yyyy-MM-dd HH:mm:ss") & "," & _
...
'参数：lng病人ID=是否只上传单据中指定病人的费用
    ...
    Do Until rsBill.EOF
        ...
        strInput = strInput & "|" & arrOutput(4)                       '自付比例
        strInput = strInput & "|" & Val(arrOutput(1)) * rsBill("数量") '全自费部分
        strInput = strInput & "|" & Val(arrOutput(2)) * rsBill("数量") '挂钩自费部分
        strInput = strInput & "|" & Val(arrOutput(3)) * rsBill("数量") '允许报销部分
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    If frmIdentify贵阳.GetIdentify(TYPE_成都郊县, str卡号_New, str医保号_New, str中心编号_New, str密码_New) = False Then
        '身份验证未通过
        Exit Function
    End If
