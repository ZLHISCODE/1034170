...
'功能：判断IC卡是否合法
    ...
    If TruncZero(mIC铜仁.Password) <> "9999" Then
    ...
    If txt新密码(0).Visible = False Then
        ...
        gstrSQL = "SELECT B.有效期,B.是否可用,A.运行模式,A.开展慢病报销,A.开展大病报销 " & _
                   ...
                   " WHERE A.险类=" & gintInsure & " AND A.编码='" & mIC铜仁.CenterCode & "' AND A.主机编码=B.编码 AND A.险类=B.险类 "
        If rsTemp.State = adStateOpen Then rsTemp.Close
        rsTemp.Open gstrSQL, gcn铜仁, adOpenStatic, adLockReadOnly
...
'        If mbln判断在院 = True Then
            ...
            gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & gintInsure & " and 卡号='" & TrimStr(mIC铜仁.Cardno) & "'"
            Call OpenRecordset(rsTemp, "判断是否已经在院")
...
Private Sub cmd病种_Click()
    ...
    gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
            " From 保险病种 A where A.险类=" & TYPE_铜仁 & " And A.类别 IN (" & IIf(mint场合 = 0, "1,2)", "0)")
...
Private Sub txt病种_KeyPress(KeyAscii As Integer)
    ...
    gstrSQL = "Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特殊病','普通病') 类别 " & _
             "   FROM 保险病种 A WHERE A.险类=" & TYPE_铜仁 & " And A.类别 IN (" & IIf(mint场合 = 0, "1,2)", "0)") & " And (" & _
