...
Option Explicit
'Modified by 朱玉宝 20031218 地区：福州 省市医保要分开，因此将所有TYPE_福建巨龙改为gintInsure，注意提交程序
...
Public Function 身份标识_福建巨龙(ByVal 操作方式_IN As Integer, Optional ByVal lng病人ID As Long = 0) As String
    Dim str顺序号 As String, str科室 As String, arrReturn
    身份标识_福建巨龙 = ""
    
    '如果是单纯的在帐户管理中进行验证，验证完即退出（提供给用户修改保险病种的一种途径）
    If 操作方式_IN = 操作方式.验证 Then
        If Not frm等待响应.ShowME(操作方式_IN, 请求目的.刷卡) Then Exit Function
        身份标识_福建巨龙 = "小宝"  '用来强迫刷新
        Exit Function
    End If
...
Public Function 个人余额_福建巨龙(ByVal lng病人ID As Long) As Currency
'功能: 提取参保病人个人帐户余额
'返回: 返回个人帐户余额
    Dim rsTemp As New ADODB.Recordset
...
Public Function 入院登记_福建巨龙(ByVal lng病人ID As Long) As Boolean
...
Public Function 出院登记_福建巨龙(ByVal lng病人ID As Long, ByVal lng主页ID As Long, Optional ByVal bln撤销入院 As Boolean = False) As Boolean
    '先结算后出院:
    '   如果发生费用--医保出院、HIS出院
    '   未发生费用  --医保撤销入院、HIS出院
    '先出院后结算:
        ...
        If 发生费用(lng病人ID, lng主页ID) Then
            If 操作模式 = 0 Then
                ...
                If 存在未结费用(lng病人ID, lng主页ID) Then
                    ...
                    Exit Function
                End If
                If Not frm等待响应.ShowME(操作方式.出院, 请求目的.刷卡) Then Exit Function
                If lng病人ID <> 获取病人ID Then
                    MsgBox "病人信息不符！", vbInformation, gstrSysName
                    Exit Function
                End If
                If Not frm等待响应.ShowME(操作方式.出院, 请求目的.申请, lng病人ID) Then Exit Function
            ...
            End If
        Else
            If Not frm等待响应.ShowME(操作方式.入院, 请求目的.冲销, lng病人ID) Then Exit Function
        End If
    Else
        If Not frm等待响应.ShowME(操作方式.入院, 请求目的.冲销, lng病人ID) Then Exit Function
    End If
...
Public Function 出院登记撤销_福建巨龙(ByVal lng病人ID As Long) As Boolean
    '先结算后出院:
    '   如果发生费用--医保撤销出院
    '   未发生费用  --调医保入院
    '先出院后结算:
    ...
    Call OpenRecordset(rs住院次数, "取主页ID")
    lng主页ID = rs住院次数!主页ID
    
    If 存在费用记录(lng病人ID, lng主页ID) Then
        If 发生费用(lng病人ID, lng主页ID) Then
            If 操作模式 = 0 Then    '先结算后出院
                If Not frm等待响应.ShowME(操作方式.出院, 请求目的.冲销, lng病人ID) Then Exit Function
            Else
                If Not 存在未结费用(lng病人ID, lng主页ID) Then
                    If Not frm等待响应.ShowME(操作方式.出院, 请求目的.冲销, lng病人ID) Then
            ...
            Call ExecuteProcedure("入院登记")
        Else
            If Not 入院登记_福建巨龙(lng病人ID) Then Exit Function
        End If
    Else
        If Not 入院登记_福建巨龙(lng病人ID) Then Exit Function
    End If
...
Public Function 挂号结算_福建巨龙(ByVal lng结帐ID As Long) As Boolean
    Dim lng病人ID As Long
    ...
    lng病人ID = 获取病人ID
    If Not frm等待响应.ShowME(操作方式.挂号, 请求目的.申请, lng病人ID, lng结帐ID) Then Exit Function
    If gintInsure = TYPE_南平市 Then
        gstrSQL = "Select A.结算方式,Nvl(A.冲预交,0) 金额 " & _
                    " From 病人预交记录 A,保险帐户 B " & _
                    " Where A.病人ID=B.病人ID And B.险类=" & gintInsure & " And A.结帐ID=" & lng结帐ID & " And 结算方式='现金' And not (记录性质=1 or 记录性质=11)"
        '肯定只有现金支付
        Call OpenRecordset(rsTemp, "读取现金支付额")
        cur现金 = NVL(rsTemp!金额, 0)
        
        cur个人帐户 = 个人余额_福建巨龙(lng病人ID)
        If cur个人帐户 <> 0 Then
        ...
        End If
    End If
    If Not 保存挂号结算记录(lng病人ID, lng结帐ID) Then Exit Function
    If gintInsure = TYPE_南平市 Then frm结算信息.ShowME (lng结帐ID)
...
Public Function 挂号结算冲销_福建巨龙(ByVal lng结帐ID As Long) As Boolean
    Dim lng病人ID As Long
    ...
    If Not frm等待响应.ShowME(操作方式.挂号, 请求目的.冲销, lng病人ID, lng结帐ID) Then Exit Function
    If Not 保存挂号结算记录(lng病人ID, lng结帐ID, True) Then Exit Function
...
Private Function 保存挂号结算记录(ByVal lng病人ID As Long, ByVal lng结帐ID As Long, Optional ByVal bln冲销 As Boolean = False) As Boolean
    ...
    If bln冲销 Then
        ...
        Call OpenRecordset(rsTemp, "取冲销ID")
        lng结帐ID = rsTemp("结帐ID")
    End If
...
Public Function 门诊虚拟结算_福建巨龙(ByVal rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    If Not frm等待响应.ShowME(操作方式.收费, 请求目的.预结算, 获取病人ID) Then Exit Function
...
Public Function 门诊结算_福建巨龙(ByVal lng病人ID As Long, ByVal lng结帐ID As Long) As Boolean
    Dim curMoney As Currency
    ...
    If Not frm等待响应.ShowME(操作方式.收费, 请求目的.申请, 获取病人ID, lng结帐ID) Then Exit Function
    
    'Modified by 朱玉宝 20031218 地区：福州
    '如果是省市医保，由于无虚拟结算，需要将结算信息保存
    If gintInsure <> TYPE_福建巨龙 Then
        Call Record_Locate(mrsIniItems, "名称,Zhzfe0")
        curMoney = Val(NVL(mrsIniItems!值, 0))
        If curMoney <> 0 Then
   ...
   End If
    ...
    If Not 保存门诊收费结算记录(lng病人ID, lng结帐ID) Then Exit Function
    
    'Modified by 朱玉宝 20031218 地区：福州
    '如果是省市医保，由于无虚拟结算，需要将结算信息显示出来
    If gintInsure <> TYPE_福建巨龙 Then frm结算信息.ShowME (lng结帐ID)
...
Public Function 门诊结算冲销_福建巨龙(ByVal lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    On Error GoTo ErrHand
    门诊结算冲销_福建巨龙 = False
    
    If Not frm等待响应.ShowME(操作方式.收费, 请求目的.冲销, lng病人ID, lng结帐ID) Then Exit Function
    If Not 保存门诊收费结算记录(lng病人ID, lng结帐ID, True) Then Exit Function
...
Private Function 保存门诊收费结算记录(ByVal lng病人ID As Long, ByVal lng结帐ID As Long, Optional ByVal bln冲销 As Boolean = False) As Boolean
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & gintInsure & "," & lng病人ID & "," & _
        ...
        0 & "," & curMzzhzfe & ",'" & strMzlsh & "')"
'        .年度 & "," & .帐户累计增加 & "," & .帐户累计支出 & "," & .累计进入统筹 & "," & _
'        .累计统筹报销 & "," & IIf(.主页ID = 0, "NULL", .主页ID) & "," & .起付线 & "," & .封顶线 & "," & .实际起付线 & "," & _
'        .存在费用记录金额 & "," & .全自费金额 & "," & .首先自付金额 & "," & .进入统筹金额 & "," & .统筹报销金额 & ",0," & _
'        .超限自付金额 & "," & cur个人帐户 & ",'')"
    Call ExecuteProcedure("保存门诊收费数据")
...
Public Function 住院虚拟结算_福建巨龙(ByVal lng病人ID As Long) As String
    ...
    If Not frm等待响应.ShowME(操作方式.结帐, 请求目的.刷卡) Then Exit Function
    lngPatient = 获取病人ID
    ...
    If Not frm等待响应.ShowME(操作方式.结帐, 请求目的.预结算, lng病人ID) Then Exit Function
...
Public Function 住院结算_福建巨龙(ByVal lng病人ID As Long, ByVal lng结帐ID As Long) As Boolean
    Dim curMoney As Currency
    Dim lng预交ID As Long
    ...
    If Not frm等待响应.ShowME(操作方式.结帐, 请求目的.申请, lng病人ID, lng结帐ID) Then Exit Function
    ...
    If Not 保存住院结算记录(lng病人ID, lng结帐ID) Then Exit Function
    住院结算_福建巨龙 = True
    ...
    If 操作模式 = 1 Then
        If 医保病人已经出院(lng病人ID) Then
            ...
            bln出院 = True '调用出院接口
            bln出院成功 = frm等待响应.ShowME(操作方式.出院, 请求目的.申请, lng病人ID)
        End If
        
        '显示调用接口给操作员，以便了解当前操作的医保病人是否正常办理出院手续
        If bln出院 Then
            If Not bln出院成功 Then
                MsgBox "出院结算调用失败，请到保险帐户中补办出院手续", vbInformation, gstrSysName
            Else
                MsgBox "该病人在医保中心成功办理出院手续！", vbInformation, gstrSysName
            End If
        End If
...
Public Function 住院结算冲销_福建巨龙(ByVal lng结帐ID As Long) As Boolean
    Dim lng病人ID As Long
    Dim rsTmp As New ADODB.Recordset
    On Error GoTo ErrHand
    住院结算冲销_福建巨龙 = False
    ...
    If Not frm等待响应.ShowME(操作方式.结帐, 请求目的.冲销, lng病人ID, lng结帐ID) Then Exit Function
    If Not 保存住院结算记录(lng病人ID, lng结帐ID, True) Then Exit Function
...
Private Function 保存住院结算记录(ByVal lng病人ID As Long, ByVal lng结帐ID As Long, Optional ByVal bln冲销 As Boolean = False) As Boolean
    Dim lng主页ID As Long
...
Private Function 存在费用记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    '检查该次住院是否没有费用发生
    gstrSQL = "Select nvl(count(病人ID),0) as 金额  from 病人费用记录 where 病人ID=" & lng病人ID & " and 主页ID=" & lng主页ID
    Call OpenRecordset(rs费用, "是否住院期间记过帐")
...
Private Function 发生费用(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    Dim rs费用 As New ADODB.Recordset
    '检查该次住院是否还有费用未结算
    gstrSQL = "Select Sum(nvl(应收金额,0)) as 金额  from 病人费用记录 where 病人ID=" & lng病人ID & " and 主页ID=" & lng主页ID
    Call OpenRecordset(rs费用, "是否存在未结费用")
    If rs费用.EOF = True Then
        发生费用 = False
    Else
...
Private Sub 更新相关信息(ByVal lng病人ID As Long, ByVal 操作方式_IN As Integer, ByVal 请求目的_IN As Integer)
    ...
    Call OpenRecordset(rsInfo, "获取医保病人相关信息")
    ...
    strSql = "zl_保险帐户_更新信息(" & lng病人ID & "," & gintInsure
...
'------------------------------------其它过程与函数------------------------------------
Public Function SendRequest(ByVal 操作方式_IN As Integer, ByVal 请求目的_IN As Integer, ByVal lng病人ID As Long, ByVal lng结帐ID As Long) As Boolean
    ...
    If objFileSys.FileExists(mstrPath_福建巨龙 & gintInsure & "\" & mstrRequest_福建巨龙) Then Call objFileSys.DeleteFile(mstrPath_福建巨龙 & gintInsure & "\" & mstrRequest_福建巨龙, True)
    If objFileSys.FileExists(mstrPath_福建巨龙 & gintInsure & "\" & mstrTemp_福建巨龙) Then Call objFileSys.DeleteFile(mstrPath_福建巨龙 & gintInsure & "\" & mstrTemp_福建巨龙, True)
    If objFileSys.FileExists(mstrPath_福建巨龙 & gintInsure & "\" & mstrReply_福建巨龙) Then Call objFileSys.DeleteFile(mstrPath_福建巨龙 & gintInsure & "\" & mstrReply_福建巨龙, True)
    Set objStream = objFileSys.CreateTextFile(mstrPath_福建巨龙 & gintInsure & "\" & mstrTemp_福建巨龙)
    '先写节头
    With mrsIniSection
        .MoveFirst
    ...
    Case 请求目的.申请, 请求目的.预结算
        ...
        Case 操作方式.收费
            Call 获取病种信息(lng病人ID)
            Call OutputData(objStream, "Mzlsh0=" & Get流水号(操作方式_IN, 请求目的_IN, lng病人ID, lng结帐ID))
        ...
        Case 操作方式.结帐
            '取得主页ID
            gstrSQL = "Select Nvl(住院次数,0) 主页ID From 病人信息 Where 病人ID=" & lng病人ID
            Call OpenRecordset(rsTmp, "取主页ID")
            g结算数据.主页ID = rsTmp!主页ID
            Call 获取病种信息(lng病人ID)
            Call OutputData(objStream, "Zylsh0=" & Get流水号(操作方式_IN, 请求目的_IN, lng病人ID, lng结帐ID))
            Call Record_Locate(mrsIniItems, "名称,Bqbm00")
            Call OutputData(objStream, "Bqbm00=" & NVL(mrsIniItems!值, ""))
            bln明细 = True
        Case 操作方式.出院
            ...
            If gintInsure = TYPE_福建省 Or gintInsure = TYPE_福州市 Then
                gstrSQL = "Select A.出院方式 From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.主页ID=B.住院次数 And A.病人ID=" & lng病人ID
                Call OpenRecordset(rsTemp, "读取出院状态")
                Call OutputData(objStream, "cyztlx=" & IIf(rsTemp!出院方式 = "正常", "治愈", rsTemp!出院方式))
            ElseIf gintInsure = TYPE_南平市 Then
    ...
    Case 请求目的.冲销
        ...
        Case 操作方式.入院
            '要冲销的住院流水号
            Call OutputData(objStream, "Cxlsh0=" & Get流水号(操作方式_IN, 请求目的_IN, lng病人ID, lng结帐ID))
        Case 操作方式.挂号
            '要冲销的挂号流水号
            Call OutputData(objStream, "Ghlsh0=" & Get流水号(操作方式_IN, 请求目的_IN, lng病人ID, lng结帐ID))
        Case 操作方式.收费, 操作方式.结帐
            'Modified by 朱玉宝 20031218 地区：福州 只有福州市医保存在更正收费
'            If 操作方式_IN = 操作方式.收费 And gintInsure = TYPE_福州市 Then
'                Call 获取病种信息(lng病人ID)
'                Call Record_Locate(mrsIniItems, "名称,Bqbm00")
        ...
        If 操作方式_IN = 操作方式.结帐 Then
            ...
            Else
                gstrSQL = "Select 'HIS' 医生,A.收费细目ID,C.项目编码 医保项目编号,decode(C.是否医保,1,'Y','N') 是否医保,A.收据费目 发票项目,E.名称 医保项目名称,  " & _
                    ...
                    " From 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & gintInsure & ") C,收费类别 D,收费细目 E  " & _
                    " Where A.执行部门ID+0=B.ID And A.收费细目ID+0=C.收费细目ID(+) And A.病人ID=" & lng病人ID & " ANd A.结帐ID=" & lng结帐ID & _
                    " And A.收费类别=D.编码 And A.收费细目ID=E.ID And Nvl(A.是否上传,0)=0 And Nvl(A.附加标志,0)<>9 " & _
                    " Group By A.收费细目ID,C.项目编码,C.是否医保,A.收据费目,E.名称,substrb(E.规格,1,20),A.计算单位  " & _
                    " Having Sum(A.数次*Nvl(A.付数,1))<>0"
            End If
            Call OpenRecordset(mrsDetail, "上传结帐费用")
        Else
            '取收费细目ID
            str收费细目 = ",,"
            bln收费 = True
            ...
            If 请求目的_IN = 请求目的.申请 Then
                ...
                gstrSQL = "Select A.开单人,A.收费细目ID,C.项目编码 医保项目编号,decode(C.是否医保,1,'Y','N') 是否医保,A.收据费目,E.名称 医保项目名称,  " & _
                    ...
                    " From 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & gintInsure & ") C,收费类别 D,收费细目 E  " & _
                    " Where A.执行部门ID+0=B.ID And A.收费细目ID+0=C.收费细目ID(+) And A.病人ID=" & lng病人ID & " ANd A.结帐ID=" & lng结帐ID & _
                    " And A.收费类别=D.编码 And A.收费细目ID=E.ID And Nvl(A.是否上传,0)=0 And Nvl(A.附加标志,0)<>9"
                Call OpenRecordset(mrsDetail, "获取费用明细")
            End If
            ...
            gstrSQL = "Select D.ID 收费细目ID,D.名称 医保项目名称,substrb(D.规格,1,20) 规格,C.项目编码 医保项目编号" & _
                    " From 收费细目 D,(Select * From 保险支付项目 Where 险类=" & gintInsure & ") C" & _
                    " Where D.ID=C.收费细目ID(+) And D.ID IN (" & str收费细目 & ")"
            Call OpenRecordset(rsSecond, "获取费用明细")
        End If
        
        With mrsDetail
            'Modified by 朱玉宝 20031218 地区：福州
            '填写处方明细记录数
            If 请求目的_IN = 请求目的.预结算 Or (gintInsure <> TYPE_福建巨龙) Then curTotalMoney = 0
...
'                If gintInsure = TYPE_福建巨龙 Then If 检查项目("", !收费细目ID) = False Then GoTo ClearFiles
                ...
                If Not bln收费 Then
                    ...
                    str收据费目 = NVL(!发票项目, "")
                    If gintInsure <> TYPE_福建巨龙 Then     '省市医保都是诊察费
                    ...
                    'Modified by 朱玉宝 20031218 地区：福州
                    If 请求目的_IN = 请求目的.预结算 Or (gintInsure <> TYPE_福建巨龙) Then curTotalMoney = curTotalMoney + NVL(!金额, 0)
                Else
                    rsSecond.MoveFirst
                    ...
                    'Modified by 朱玉宝 20031218 地区：福州
                    If 请求目的_IN = 请求目的.预结算 Or (gintInsure <> TYPE_福建巨龙) Then curTotalMoney = curTotalMoney + NVL(!实收金额, 0)
                End If
                If curMoney < 0 Then
            ...
            If lng结帐ID <> 0 Then
                gstrSQL = "Select Sum(结帐金额) 结帐金额 From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9"
                Call OpenRecordset(rs结帐, "本次结帐金额")
...
ReturnCall:
    objStream.Close
    objFileSys.GetFile(mstrPath_福建巨龙 & gintInsure & "\" & mstrTemp_福建巨龙).Name = mstrRequest_福建巨龙
    SendRequest = True
    Exit Function
ErrHand:
    If ErrCenter = 1 Then
...
Private Function Get流水号(ByVal 操作方式_IN As Integer, ByVal 请求目的_IN As Integer, ByVal lng病人ID As Long, ByVal lng结帐ID As Long) As String
    ...
    Case 操作方式.入院
        ...
        Case 请求目的.冲销
            gstrSQL = " Select 顺序号 as 流水号 From 保险帐户" & _
                     " Where 病人ID=" & lng病人ID & " And 险类=" & gintInsure
        End Select
    Case 操作方式.挂号
        Select Case 请求目的_IN
        'Case 请求目的.申请
    ...
    Case 操作方式.出院
        gstrSQL = " Select 顺序号 as 流水号 From 保险帐户" & _
                 ...
                 " And 险类=" & gintInsure
    End Select
    Call OpenRecordset(rsPatient, "获取医保病人的流水号")
    Get流水号 = rsPatient!流水号
End Function

Public Function AnalyseReply(ByVal 操作方式_IN As Integer, ByVal 请求目的_IN As Integer) As Integer
    Dim objFileSys As New FileSystemObject, objStream As TextStream
...
'            自付比例=自付比例
    ...
    If Not objFileSys.FileExists(mstrPath_福建巨龙 & gintInsure & "\" & mstrReply_福建巨龙) Then Exit Function
    Err = 0
    Set objStream = objFileSys.OpenTextFile(mstrPath_福建巨龙 & gintInsure & "\" & mstrReply_福建巨龙, ForReading, False, TristateMixed)
    ...
    If 请求目的_IN = 请求目的.刷卡 And (操作方式_IN = 操作方式.挂号 Or 操作方式_IN = 操作方式.收费 Or 操作方式_IN = 操作方式.入院) Then
        If lng病人ID <> 0 And lng病人ID <> 获取病人ID Then
        ...
        int住院次数 = 0: lng病种ID = 0
        If lng病人ID = 0 Then lng病人ID = 获取病人ID
        If lng病人ID <> 0 Then
            gstrSQL = "Select Nvl(人员身份,0) 住院次数 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & gintInsure
            Call OpenRecordset(rsTmp, "获取住院次数")
            int住院次数 = rsTmp!住院次数
            
            gstrSQL = "Select Nvl(病种ID,0) 病种ID From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & gintInsure
            Call OpenRecordset(rsTmp, "获取病种ID")
            lng病种ID = rsTmp!病种ID
        End If
        Call Record_Locate(mrsIniItems, "名称,Bckbcs")
        ...
        If bln补充入院 Then
            gstrSQL = "Select 病人ID From 保险帐户 Where 卡号='" & str卡号 & "' And 医保号='" & str医保号 & "'"
            Call OpenRecordset(rsTmp, "读取该病人的保险帐户中的病人ID")
        ...
        Else
            ...
            gstrSQL = "Select 病人ID From 保险帐户 Where 卡号='" & str卡号 & "' And 医保号='" & str医保号 & "'"
            Call OpenRecordset(rsTmp, "是否存在保险帐户")
            If Not rsTmp.EOF Then
                lng病人ID = rsTmp!病人ID
            End If
        End If
        lng病人ID = BuildPatiInfo(1, strIdentify & strAddition, lng病人ID)
        '返回格式:中间插入病人ID
        If lng病人ID > 0 Then
    ...
    Call 更新相关信息(获取病人ID, 操作方式_IN, 请求目的_IN)
    
    '如果是住院收费刷卡，则更新帐户中的入院顺序号（避免由于其它原因引起的顺序号为空的问题）
    If 请求目的_IN = 请求目的.刷卡 And 操作方式_IN = 操作方式.结帐 Then
        Call Record_Locate(mrsIniItems, "名称,Zylsh0")
        str顺序号 = NVL(mrsIniItems!值, "")
        gstrSQL = "zl_保险帐户_更新信息(" & 获取病人ID & "," & gintInsure & ",'顺序号','''" & str顺序号 & "''')"
        Call ExecuteProcedure("更新入院流水号")
    End If
    
    '读取完后，删除应答文件
'    If objFileSys.FileExists(mstrPath_福建巨龙 & gintinsure & "\" & mstrRequest_福建巨龙) Then
'        Call objFileSys.DeleteFile(mstrPath_福建巨龙 & gintinsure & "\" & mstrRequest_福建巨龙, True)
'    End If
    Call objFileSys.DeleteFile(mstrPath_福建巨龙 & gintInsure & "\" & mstrReply_福建巨龙, True)
    AnalyseReply = 1
    Exit Function
ErrHand:
    If ErrCenter = 1 Then
...
Private Sub 获取登录信息()
    ...
    gstrSQL = "Select * From 保险参数 Where 险类=" & gintInsure & " Order by 序号"
    Call OpenRecordset(rsInfo, "获取医保参数")
...
Private Sub 获取入院信息(ByVal lng病人ID As Long)
    ...
    gstrSQL = "Select to_char(A.入院时间,'yyyy-MM-dd hh24:mi:ss') 入院时间,B.名称 科室 " & _
            " From 病人信息 A,部门表 B " & _
            " Where A.当前科室ID=B.ID And A.病人ID=" & lng病人ID
    Call OpenRecordset(rs入院, "获取入院信息")
...
Private Sub 获取挂号信息(ByVal lng病人ID As Long, ByVal lng结帐ID As Long)
    ...
    gstrSQL = "Select Sum(A.实收金额) 费用,B.名称 科室 " & _
            ...
            " Where A.执行部门ID=B.ID ANd A.病人ID=" & lng病人ID & " And 记录性质=4 And 结帐ID=" & lng结帐ID & _
            " Group by B.名称"
    Call OpenRecordset(rs挂号, "获取挂号信息")
...
Private Function 获取病种信息(ByVal lng病人ID As Long) As ADODB.Recordset
    Dim strValue As String
    Dim rs收费 As New ADODB.Recordset
    
    gstrSQL = "Select substr(B.名称,1,instr(B.名称,'@@')-1) 病种 From 保险帐户 A,保险病种 B " & _
                    " Where A.险类=B.险类(+) And A.病种ID=B.ID(+) And A.病人ID=" & lng病人ID & " And A.险类=" & gintInsure
    Call OpenRecordset(rs收费, "获取医保病人病种信息")
...
Private Sub 获取出院信息(ByVal lng病人ID As Long)
    ...
    gstrSQL = "Select to_char(出院时间,'yyyy-MM-dd hh24:mi:ss') 出院时间 " & _
            " From 病人信息 " & _
            " Where 病人ID=" & lng病人ID
    Call OpenRecordset(rs出院, "获取出院信息")
...
Private Sub 获取卡号信息(ByVal lng病人ID As Long)
    ...
    gstrSQL = "Select 卡号 " & _
            " From 保险帐户 " & _
            " Where 病人ID=" & lng病人ID
    Call OpenRecordset(rs卡号, "获取医保病人的卡号")
...
Public Function 获取病人ID() As Long
    Dim rsTmp As New ADODB.Recordset
    
    获取病人ID = 0
    Call Record_Locate(mrsIniItems, "名称,ID0000")
    gstrSQL = " Select 病人ID From 保险帐户 Where 医保号='" & NVL(mrsIniItems!值, "") & "' And 险类=" & gintInsure
    Call OpenRecordset(rsTmp, "获取该医保病人的基本信息")
    If Not rsTmp.EOF Then
        获取病人ID = NVL(rsTmp!病人ID, 0)
    End If
End Function

Private Function 操作模式() As Long
    Dim intValue As Integer
    Dim rsTmp As New ADODB.Recordset
    
    '获取参数值(0-先结算,后出院;1-先出院,后结算)
    intValue = 0
    gstrSQL = "Select Nvl(参数值,0) Value From 保险参数 Where 险类=" & gintInsure & " And 参数名='操作模式'"
    Call OpenRecordset(rsTmp, "获取参数值")
...
Private Function 检查项目(ByVal strNO As String, ByVal lng收费细目ID As Long) As Boolean
    ...
    Call OpenRecordset(rsCheck, "取服务网点编号")
    With rsCheck
        If Not .EOF Then
    ...
    '取审核标志
    gstrSQL = "Select nvl(Sfsh00,0) As Value From yydy.Yy_Yydyb0 Where FWWDBH='" & strCode & "' And YYXMBH=" & lng收费细目ID
    Call OpenRecordset(rsCheck, "取审核标志")
    With rsCheck
        If Not .EOF Then
            If Not IsNull(!Value) Then
                intVerify = !Value
    ...
    End With
    If intVerify = 0 Then
        gstrSQL = "Select '['||编码||']'||名称 项目 From 收费细目 Where ID=" & lng收费细目ID
        Call OpenRecordset(rsCheck, "取项目名称")
        If strNO <> "" Then
...
Private Sub 调试用主函数(ByVal 操作方式_IN As Integer, ByVal 请求目的_IN As Integer)
    Call InitStruc
    Call SendRequest(操作方式_IN, 请求目的_IN, 0, 0)
End Sub
