...
Begin VB.Form frmIdentify贵阳 
   ...
   Begin VB.Image Image1 
      Height          =   480
      Left            =   255
      Picture         =   "frmIdentify贵阳.frx":030A
      Top             =   345
      Width           =   480
   End
End
...
Private Declare Function IC_ExitComm_2 Lib "ftic_32.dll" Alias "ic_exit%" (ByVal icdev As Long)
Private Declare Function HexToAsc Lib "ftic_32.dll" Alias "hex_asc%" (ByVal hex As String, ByVal asc$, ByVal le&)
'------------------------------------------------------------
...
Public Function GetIdentify(ByVal intInsure As Integer, str卡号 As String, str医保号 As String, str分中心编号 As String, str密码 As String, _
                            Optional ByVal blnPass As Boolean = True, Optional ByVal blnChangePassword As Boolean = False) As Boolean
    ...
    frmIdentify贵阳.Height = cmdOK.Top + cmdOK.Height + 500
...
Private Sub cmdTest_Click()
    ...
    If Trim(txtEdit(0).Text) = "" Then
        MsgBox "请刷卡或手工输入卡内数据！", vbInformation, gstrSysName
        txtEdit(0).SetFocus
        Exit Sub
    End If
    If chk手工.Value = 1 Then
        If Trim(txtEdit(2).Text) = "" Then
            MsgBox "手工输入磁卡数据时,必需输入分中心编号！", vbInformation, gstrSysName
            txtEdit(2).SetFocus
            Exit Sub
        End If
    End If
    ...
    str类别 = GetElemnetValue("PERSONTYPE")
    If InStr(1, "32|34", str类别) <> 0 Then
        cbo保险类别.ListIndex = 1
...
'                                      Exit Sub
                    ...
                    Else
                        ...
                        Call HexToAsc(str卡号_IC, strData, 64)
                        
                        str医保号_IC = Mid(strData, 13, 17)
                        str卡号_IC = Mid(strData, 55, 6)
                        str卡号 = Replace(str卡号_IC, " ", "")
                        str医保号 = Replace(str医保号_IC, " ", "")
                    End If
...
Private Sub CloseCommon(ByVal lngDev As Long)
    ...
    Else
        ...
        intReturn = IC_ExitComm_2(lngDev)
    End If
    If intReturn < 0 Then
