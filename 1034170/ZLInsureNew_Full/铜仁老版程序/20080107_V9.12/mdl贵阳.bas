...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    Call InsertChild(mdomInput.documentElement, "CARDID", str卡号)            ' 磁卡编号
    Call InsertChild(mdomInput.documentElement, "CENTERCODE", str分中心编号)            ' 密码
    Call InsertChild(mdomInput.documentElement, "INSURETYPE", str保险类别)            ' 密码
    Call InsertChild(mdomInput.documentElement, "PASSWORD", str密码)            ' 密码
    
    '调用接口
    If CommServer("READCARD_M") = False Then Exit Function
...
'    str人员类别 = Switch(str人员类别 = "11", "在职", str人员类别 = "12", "在职驻外", str人员类别 = "21", "退休" _
'                      , str人员类别 = "22", "异地驻外", str人员类别 = "31", "离休", str人员类别 = "32", "老红军" _
'                      , str人员类别 = "33", "军残", str人员类别 = "34", "特殊全免", str人员类别 = "35", "异地离休" _
'                      , str人员类别 = "41", "下岗职工", str人员类别 = "91", "其它", True, "")
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    If Get验证(str卡号, str医保号, str分中心编号, str密码, lng病人id) = False Then Exit Function
    ...
    Else
        ...
        If str疾病编码 <> "" Then
            str结算方式 = str结算方式 & "|医保基金;" & Val(GetElemnetValue("FUND1PAY")) + _
                        Val(GetElemnetValue("FUND2PAY")) & ";0"
        End If
    End If
    门诊虚拟结算_贵阳 = True
    Exit Function
...
'    Dim str卡号Re As String, str医保号Re As String, str分中心编号Re As String, str密码Re As String
    ...
    gstrSQL = "SELECT Nvl(从属父号,Nvl(价格父号,序号)) AS 主序号 FROM 病人费用记录  " & _
             " WHERE 结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9" & _
    ...
    gstrSQL = "Select A.ID,A.序号,A.病人ID,A.NO,A.登记时间,A.开单人 as 医生,A.登记时间," & _
            ...
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID And A.收费细目ID=D.收费细目ID  AND B.ID=F.药品ID(+) And D.险类=" & gintInsure & " And Nvl(A.附加标志,0)<>9" & _
            " Order by A.ID"
    Call OpenRecordset(rs明细, "贵阳医保")
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    If Get验证(str卡号, str医保号, str分中心编号, str密码, lng病人id, True) = False Then Exit Function
...
Private Function Get验证(str卡号 As String, str医保号 As String, str分中心编号 As String, str密码 As String, _
