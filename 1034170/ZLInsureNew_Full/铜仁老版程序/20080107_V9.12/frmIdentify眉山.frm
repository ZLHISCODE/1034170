...
'返回：
    ...
    '1-在职;2-退休;3-离休
    gstrSQL = "Select 序号,名称 From 保险人群 Where 险类=" & gintInsure
    Call OpenRecordset(rsTemp, Me.Caption)
    Cbo当前状态.Clear
...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select nvl(当前状态,0) as 状态,灰度级,备注 from 保险帐户 where 险类=" & gintInsure & " and 中心=" & lng中心 & " and 医保号='" & Trim(txt卡号.Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
    ...
    gstrSQL = " Select Nvl(进入统筹累计,0) 统筹累计 From 帐户年度信息 " & _
              ...
              "     (Select 病人ID From 保险帐户 where 险类=" & gintInsure & " and 中心=" & lng中心 & " and 医保号='" & Trim(txt卡号.Text) & "')"
    Call OpenRecordset(rsTemp, "获取年度统筹累计")
    ...
    If txt帐户余额.Enabled Then
        ...
        gstrSQL = "ZL_帐户变动记录_INSERT(" & _
                 lng变动ID & "," & gintInsure & ",1," & lng病人ID & "," & _
                 Val(txt帐户余额.Text) & ",'" & gstrUserName & "','创建医保病人档案时录入的初始值',1)"
        Call ExecuteProcedure(Me.Caption)
    End If
    '更新个人帐户中的信息
    gstrSQL = "zl_保险帐户_更新信息(" & mlng病人ID & "," & gintInsure & ",'密码','''" & strNewPass & "''')"
    ...
    Select Case Index
        Case Select卡号
            gstrSQL = " Select A.病人ID as ID,A.卡号,A.医保号,'******' 密码,B.姓名,B.性别,B.出生日期,B.身份证号,C.序号 as 中心ID " & _
                    ...
                    "  where A.病人ID=B.病人ID And Nvl(A.灰度级,0)<>9 And A.险类=" & gintInsure & _
...
'从已经存在的记录中读出帐户信息
    ...
    If Not rs帐户 Is Nothing Then
        ...
        If mbytType = 2 Or mbytType = 4 Then
            gstrSQL = "Select 密码 From 保险帐户 Where 险类=" & gintInsure & " And 医保号='" & txt卡号 & "'"
            Call OpenRecordset(rsTemp, Me.Caption)
...
'功能：检查数据的正确性
    ...
    '校验密码正确性
    gstrSQL = "Select 密码 From 保险帐户 Where 险类=" & gintInsure & " And 医保号='" & Trim(txt卡号.Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
    If Not rsTemp.EOF Then
...
'返回：病人ID
    ...
    If Len(Trim(strInfo)) <> 0 Then
        ...
        If lng病人ID > 0 Then
            '该病人已经存在
            gstrSQL = "Select nvl(病人ID,0) 病人ID from 保险帐户 where 医保号='" & CStr(array信息(1)) & "' and 险类=" & gintInsure
