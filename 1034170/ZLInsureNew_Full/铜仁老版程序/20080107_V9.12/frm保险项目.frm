...
'                    收费细目ID,险类,大类ID,项目编码,项目名称,附注
                    ...
                    If mint险类 = TYPE_泸州市 And .TextMatrix(lngRow, col医保编码) <> .TextMatrix(lngRow, col原编码) Then
                        '保存修改记录
                ...
                Case "删除"
                    '删除的项目
                    If mint险类 = TYPE_泸州市 And .TextMatrix(lngRow, col原编码) <> "" Then
                        gstrSQL = "Insert Into 项目对应日志(中心药典序号,中心药典名称,医院药典名称,修改人,发生日期) " & _
...
Private Sub Form_Load()
    ...
    '得到查询的时间范围
    If mint险类 = TYPE_沈阳市 Then mnuEdit.Visible = True
End Sub
...
'返回：医保项目编码
    ...
    With mshSum_S
        ...
        If strSelected <> "" Then
            ...
            If strName = "" Or mint险类 = TYPE_重庆银海版 Then
                Call Get保险名称
            Else
                '已经传入名称，就不用再调用
                        ...
                        Case TYPE_重庆市
                            If Left(tvwMain_S.SelectedItem.Key, 1) = "D" Or Left(tvwMain_S.SelectedItem.Key, 1) = "E" Or Left(tvwMain_S.SelectedItem.Key, 1) = "F" Then
                                ...
                                gstrSQL = "select YPLSH  医保编码,YPBM 药品编码,TYM 通用名称,SPM 商品名,SPMZJM 商品名简码,YCMC 药厂名称,decode(FYDJ,1,'甲类',2,'乙类','自费') 费用等级 " & _
                                          ...
                                          "      ,DECODE(CFYBZ,1,'是') 处方药标志,decode(GMP,1,'是') GMP标志,decode(YPXJFS,1,'限价') 限价 " & _
                            ...
                            Else
                                ...
                                gstrSQL = "Select XMLSH 医保编码,XMBM 诊疗编码,XMMC 项目名称,ZJM 简码,decode(FYDJ,1,'甲类',2,'乙类','自费') 费用等级,DW 单位 " & _
                                         ...
                                         "       ,decode(TPXMBZ,1,'是') 特批项目标志,BZ 备注 " & _
