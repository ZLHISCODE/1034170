...
Private Sub Form_Load()
    ...
    rsTemp.Filter = 0
    gstrSQL = " Select A.名称,B.参数值 From 保险支付大类 A,(Select * From 保险参数 Where 险类=" & mlng险类 & " And 中心=1 And 序号>10) B " & _
              " Where A.险类=" & mlng险类 & " And A.名称=B.参数名(+) Order by 编码"
    Call OpenRecordset(rsTemp, Me.Caption)
    With rsTemp
    ...
    For Each TextBox In Me.Controls
        If TypeName(TextBox) = "TextBox" Then
            If InStr(1, str限额, TextBox.Name) <> 0 Then
                '不能大于99999999999.99且不能小于0
                sinMax = 99999999999.99
                sinMin = 0
            Else
                '不能大于100且小于0
                sinMax = 100
