...
'返回：交易成功返回true；否则，返回false
    ...
    Dim cur累计进入统筹 As Currency
    Dim rs帐户 As New ADODB.Recordset
    
    cur累计进入统筹 = 0
    gstrSQL = "select B.进入统筹累计 " & _
              " from 保险帐户 A,帐户年度信息 B " & _
              " where A.病人ID=B.病人ID(+) and A.险类=B.险类(+) and B.年度(+)=" & Year(zlDatabase.Currentdate) & " and A.病人ID=" & lng病人ID & " and A.险类=" & gintInsure
    ...
    With g结算数据
        ...
        gstrSQL = "zl_帐户年度信息_insert(" & lng病人ID & "," & gintInsure & "," & .年度 & "," & _
            .帐户累计增加 & "," & .帐户累计支出 + cur个人帐户 & "," & cur累计进入统筹 & "," & _
            0 & "," & IIf(.主页ID = 0, "NULL", .主页ID) & ")"
        Call ExecuteProcedure("松藻医保")
        
        '保险结算记录(因为"性质,记录ID"唯一,所以本次新结帐ID肯定为插入)
        gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & gintInsure & "," & .病人ID & "," & _
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    '保险结算记录(因为"性质,记录ID"唯一,所以本次新结帐ID肯定为插入)
    gstrSQL = "zl_保险结算记录_insert(1," & lng冲销ID & "," & gintInsure & "," & rsTemp("病人ID") & "," & _
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    Dim cur全自费 As Currency, cur首先自付 As Currency, cur个人自付段 As Currency
    ...
    gstrSQL = "select nvl(sum(A.累计进入统筹),0) as 起付线 " & _
              "  from 保险结算记录 A " & _
              "  Where A.病人ID = " & g结算数据.病人ID & " And A.险类 = " & gintInsure & " And A.年度=" & g结算数据.年度
    ...
    With g结算数据
        ...
        cur个人自付段 = IIf(IsNull(rsTemp("进入统筹累计")), 0, rsTemp("进入统筹累计"))
    ...
    '－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
    '2、获取实际发生费用
    If Not cls医保.GetCapability(support允许不设置医保项目) Then
    ...
    With g结算数据
        ...
        Else
            ...
            If .起付线 = 0 Then
                ...
                Exit Function
            End If
            If cur个人自付段 < .起付线 Then .起付线 = cur个人自付段
        End If
...
'      3)由于结帐之后，可能使用结帐作废交易，这时需要结帐时执行结算交易的交易号，因此我们需要同时结帐交易号。(由于门诊收费作废时，已经不再和医保有关系，所以不需要保存结帐的交易号)
    ...
    With g结算数据
        ...
        gstrSQL = "select B.进入统筹累计 " & _
                  " from 保险帐户 A,帐户年度信息 B " & _
                  " where A.病人ID=B.病人ID(+) and A.险类=B.险类(+) and B.年度(+)=" & Year(zlDatabase.Currentdate) & " and A.病人ID=" & .病人ID & " and A.险类=" & gintInsure
        Call OpenRecordset(rs帐户, "松藻医保")
