...
Private Sub cmd疾病信息_Click()
    ...
    If frm病种选择_重庆银海版.ShowSelect(Me, Me.cbo医疗类别.ItemData(Me.cbo医疗类别.ListIndex), str病种编码, str并发症) = True Then
        gstrSQL = "Select 名称 From 保险病种 Where 编码='" & str病种编码 & "' And 险类=" & TYPE_重庆银海版
...
Private Sub txt疾病信息_KeyPress(KeyAscii As Integer)
    ...
    gstrSQL = "Select A.ID,A.编码,A.名称,A.简码" & _
             "   FROM 保险病种 A WHERE A.险类=" & TYPE_重庆银海版 & " And (" & _
                zlCommFun.GetLike("A", "编码", strText) & " or " & zlCommFun.GetLike("A", "名称", strText) & " or " & zlCommFun.GetLike("A", "简码", strText) & ")" & Get病种类别
...
Private Sub cmd确定_Click()
    ...
    If mbytType <> 2 Then
        '检查病人状态
        gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & TYPE_重庆银海版 & " and 医保号='" & txt医疗证号 & "'"
        Call OpenRecordset(rsTemp, Me.Caption)
        If rsTemp.RecordCount > 0 Then
            If rsTemp("状态") > 0 Then
