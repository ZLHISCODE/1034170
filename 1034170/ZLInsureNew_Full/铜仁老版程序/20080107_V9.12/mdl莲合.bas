...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    With frmlhIDentified
        ...
        If 身份标识_莲合 <> "" Then
            ...
            lng病人ID = BuildPatiInfo(bytType, 身份标识_莲合 & ";;;;;;;;;;;;;;;;", lng病人ID)
            '返回格式:中间插入病人ID
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    With rsList
        ...
        gstrSQL = _
            ...
            " From 病人费用记录" & _
            " Where Nvl(附加标志,0)<>9 And 结帐ID=" & lng结帐ID & _
            " Group by NO,登记时间,开单人,姓名,病人ID"
        Call OpenRecordset(rsList, "莲合医保")
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    gstrSQL = "select 住院号,姓名 from 病人信息 where 病人id=" & rsExse!病人ID
    Call OpenRecordset(rsTmp, "莲合医保")
    ...
    gstrSQL = "select a.id, A.NO,A.序号,B.名称 as 开单部门,C.项目编码 as 医保项目编码,d.名称 as 项目," & _
        " A.发生时间,A.开单人 as 医生,decode(d.是否变价,1,a.实收金额,Nvl(A.付数,1)*A.数次) as 数量,decode(d.是否变价,1,1,a.标准单价) 单价" & _
        " from 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & gintInsure & ") C,收费细目 d " & _
        " where A.开单部门ID=B.ID(+) And A.收费细目ID=C.收费细目ID(+) and a.收费细目id=d.id " & _
        " And 病人id=" & rsExse!病人ID & " And 记帐费用=1 And Nvl(是否上传,0)=0"
    Call OpenRecordset(rsExpen, "莲合医保")
    ...
    With rsExpen
        ...
        Do While Not .EOF
            ...
            'gstrSQL = "delete from rcpt_tab where LPAD(RTrim(hosp_id),8,'0')='" & Format(str住院号, "00000000") & "' and rcpt_no='" & !no & "' and sno=" & !序号 & " and class='02' and to_char(r_date,'yyyy-mm-dd HH24:MI:SS')='" & Format(!发生时间, "yyyy-mm-dd hh:mm:ss") & "'"
            'gcnSybase.Execute gstrSQL
            ...
            '上传后就不再上传
            gstrSQL = "Update 病人费用记录 set 是否上传=1 where id=" & !ID
        ...
        Do While True
            dbl自付金额 = 0
            dbl统筹资金 = 0
            gstrSQL = "select acct_pay,self_pay from diag_rec where LPAD(RTrim(hosp_id),8,'0')='" & Format(str住院号, "00000000") & "' and rcpt_no is null and class='02'" _
...
'      3)由于结帐之后，可能使用结帐作废交易，这时需要结帐时执行结算交易的交易号，因此我们需要同时结帐交易号。(由于门诊收费作废时，已经不再和医保有关系，所以不需要保存结帐的交易号)
    ...
    gstrSQL = "select 住院号,姓名 from 病人信息 where 病人id=" & rs帐户!病人ID
    Call OpenRecordset(rsTmp, "莲合医保")
    ...
    gstrSQL = "Select Sum(结帐金额) as 结帐金额 From 病人费用记录 Where Nvl(附加标志,0)<>9 And 结帐ID=" & lng结帐ID
    Call OpenRecordset(rsTmp, "莲合医保")
...
Public Function 住院结算冲销_莲合(lng结帐ID As Long, rs帐户 As ADODB.Recordset) As Boolean
    ...
    With rsTmp
        ...
        gstrSQL = "Select * From 保险结算计算 Where Nvl(档次,0)=0 And 结帐ID=" & lng结帐ID
        Call OpenRecordset(rsTmp, "莲合医保")
...
Public Function 传输明细_莲合(ByVal str单据号 As String, ByVal intClinic As Integer) As Boolean
'功能: 提取参保病人个人帐户实际报销余额
'参数: rsExse包括字段（病人id、病人姓名(10)、单据号(8)、序号(4,0)、
'        医保项目编码(6,0)、数量(8,2)、单价、金额、开单医生(6)、
'        开单部门(10)、操作员、操作员部门(4)、发生时间）
'返回: 个人帐户实际报销余额;为0表示不能报个人帐户的钱。
'------------------------------------------------------------------------------------------------------------------
'调用模块：1121-门诊收费
    ...
    '门诊：病人id、病人姓名、单据号、序号、医保项目编码、数量、单价、金额、
    '      开单医生、开单部门、操作员、发生时间"
