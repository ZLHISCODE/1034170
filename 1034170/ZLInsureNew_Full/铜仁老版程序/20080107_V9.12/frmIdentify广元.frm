...
Private Sub cmdChangPass_Click()
    initType
    mblnReturn = ChangePassword(gstr医保机构编码, gstr医院编码, gstrOutPara)
    TrimType
...
'    mblnReturn = dall(gstr医保机构编码, gstr医院编码, "2003121000031", gstrOutPara)
        mblnReturn = reg(gstr医保机构编码, gstr医院编码, 0, UserInfo.姓名, Format(zlDatabase.Currentdate, "yyyy-MM-dd"), "0", gstrOutPara)
        TrimType
        If mblnReturn = False Then
...
Private Sub cmdRead_Click()
    ...
    initType
    mblnReturn = readicxx(gstr医保机构编码, gstr医院编码, "0", gstrOutPara)
    If mblnReturn = False Then
        MsgBox gstrOutPara.errtext, vbInformation, Me.Caption
        Exit Sub
    End If
