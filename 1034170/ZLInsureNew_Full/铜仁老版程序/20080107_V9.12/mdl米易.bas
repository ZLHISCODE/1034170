...
Public Sub 补充上传门诊明细()
    ...
    gstrSQL = "Select A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,A.登记时间,A.开单人 as 医生," & _
            ...
            " From (Select * From 病人费用记录 Where 记录性质=1 And Nvl(是否上传,0)=0 And Nvl(附加标志,0)<>9) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E,保险结算记录 F,保险帐户 G,(Select * From 保险病种 Where 险类=" & gintInsure & ") H " & _
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+) And A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类=" & gintInsure & _
