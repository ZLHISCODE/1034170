...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    If frmIdentify泸州.GetPatient(bytType, False, lng病种ID) = True Then
        '身份识别完成，返回病人信息
        With gIC泸州
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              ...
              "  where A.结帐ID=" & lng结帐ID & " And Nvl(A.附加标志,0)<>9" & _
    ...
    With ic门诊
        gstrSQL = "INSERT INTO 保险结算记录 " & _
                  ...
                  " Values "
         gstrSQL = gstrSQL & " (1," & lng结帐ID & "," & gintInsure & "," & .MediYear & ",'" & .CenterCode & "','" & rs信息("NO") & "1'," & lng病人ID & ",0,'" & rs信息("姓名") & _
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    With ic门诊
        gstrSQL = "INSERT INTO 保险结算记录 " & _
                  ...
                  ",卡灰度级,发票号,票据日期,冲票标志,被冲票据号,支付顺序号,是否上传) " & _
                  " Values "
         gstrSQL = gstrSQL & " (1," & lng序号 & "," & gintInsure & "," & .MediYear & ",'" & .CenterCode & "','" & Mid(rs结算("序号"), 1, Len(rs结算("序号")) - 1) & "2'," & lng病人ID & ",0,'" & rs结算("姓名") & _
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              ...
              "        and nvl(A.是否上传,0)=0 and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & gintInsure & _
              "  Order by A.病人ID,A.发生时间"
    Call OpenRecordset(rsTemp, "虚拟结算")
    If rsTemp.EOF = False Then
        '还存在没有分割费用的明细
    ...
    If m泸州.灰度 >= deg医保支付 Then
        With m泸州
            ...
            '进入统筹费用不能大于超基本封顶线
            If .进入统筹费用 > .封顶线 Then .进入统筹费用 = .超基本封顶线
...
'
    ...
    With m泸州
        If .灰度 >= deg个人支付 Then
            ...
            Dim dbl剩余费用 As Double, dbl剩余统筹 As Double, dbl本次进入统筹 As Double
            If m泸州.封顶线 > 0 Then
                '首先求出还可以使用的费用
            ...
            dbl本次进入统筹 = dbl剩余统筹
            'Modified end
            .个人帐户支付 = dbl本次进入统筹 - .统筹基金支付 - .补充基金支付
...
'返回：交易成功返回true；否则，返回false
    ...
    Call OpenRecordset(rs信息, "泸州医保")
    m泸州.住院床日 = Fix(rs信息("出院日期") - rs信息("入院日期"))
...
'----------------------------------------------------------------
    ...
    With ic住院
        gstrSQL = "INSERT INTO 保险结算记录 " & _
                ...
                ",卡灰度级,发票号,票据日期,冲票标志,被冲票据号,支付顺序号,中途结帐,是否上传) " & _
                  " Values "
         gstrSQL = gstrSQL & " (2," & lng冲销ID & "," & gintInsure & "," & .MediYear & ",'" & .CenterCode & "','" & Mid(rs结算("序号"), 1, Len(rs结算("序号")) - 1) & "2'," & rs结算("病人ID") & "," & rs结算("主页ID") & ",'" & rs结算("姓名") & _
                  "','" & rs结算("性别") & "'," & rs结算("年龄") & ",'" & rs结算("医保号") & "','" & rs结算("卡号") & "','" & rs结算("身份证号") & "','" & .ClassCode & "','" & .UnitCode & "' " & _
                  "," & .IsOffical & "," & .IsAttend & "," & rs结算("参加补充保险") & "," & .InPerAcc & "," & .OutPerAcc & "," & .PlanPaidAmt & "," & .PlanPaidFee & _
                  "," & .ChronicPaidAmt & "," & .ChronicPaidFee & "," & .ChronicSillPaidAmt & ",null" & _
                  "," & .ClinicPaidAmt & "," & .InHosPaidAmt & "," & .QuotaPaidAmt & ",'" & rs结算("部门名称") & "','" & rs结算("医生名称") & "','" & rs结算("病种代码") & "','" & rs结算("病种名称") & "','" & rs结算("病种类型") & "' " & _
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    gstrSQL = _
        ...
        "   From 病人费用记录 A" & _
        "   Where A.记帐费用=1 And A.病人ID=" & m泸州.病人ID & " and A.主页ID=" & m泸州.主页ID & " And A.操作员姓名 is not null" & _
    ...
    With m泸州
        gstrSQL = "select max(decode(A.性质,'A',A.金额,0)) as 封项线 ,max(decode(A.性质,'1',A.金额,0)) as 起付线 " & _
                  ...
                  "  from 保险支付限额 A " & _
                  "  where A.险类=" & gintInsure & " and A.中心=" & .中心序号 & " and A.年度=" & .年度 & " And A.在职=" & lng在职
        If rsTemp.State = adStateOpen Then rsTemp.Close
        rsTemp.Open gstrSQL, gcn泸州, adOpenStatic, adLockReadOnly
    ...
    With m泸州
        ...
        If .封顶线 > 0 Then
            ...
            Else
                '支付封顶，只有统筹部分
                .超基本封顶线 = .进入统筹 - .实际起付线 - (m泸州.统筹基金支付 + m泸州.统筹基金自付)
            End If
...
Public Function 挂号结算_泸州(ByVal lng结帐ID As Long) As Boolean
    ...
    '取病人ID
    gstrSQL = "Select 病人ID From 病人费用记录 Where 结帐ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "读取病人ID")
    lng病人ID = rsTemp!病人ID
    
    '取费用总额
    gstrSQL = "Select Sum(实收金额 ) as 金额 From 病人费用记录 Where 结帐ID= " & lng结帐ID
