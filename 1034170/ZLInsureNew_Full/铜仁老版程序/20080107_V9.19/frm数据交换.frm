...
Private Sub cmdOK_Click()
    ...
    If mbytType = 1 Then
        strIdentify = mdl南京市.analyseStr(mstrStream, 1, 20) & ";"                                      '0卡号
        strIdentify = strIdentify & mdl南京市.analyseStr(mstrStream, 1, 20) & ";"                    '1医保号（个人编号）
        strIdentify = strIdentify & ";"                                 '2密码
        strIdentify = strIdentify & mdl南京市.analyseStr(mstrStream, 55, 12) & ";"                   '3姓名
        strIdentify = strIdentify & mdl南京市.analyseStr(mstrStream, 67, 1) & ";"                               '4性别
        strIdentify = strIdentify & ";"                            '5出生日期
        strIdentify = strIdentify & ";"                                 '6身份证
        strIdentify = strIdentify & ";"                              '7.单位名称(编码)
        strAddition = "0;"                                          '8.中心代码
        strAddition = strAddition & mdl南京市.analyseStr(mstrStream, 21, 20) & ";"                             '9.顺序号
        strAddition = strAddition & ";"                            '10人员身份
        strAddition = strAddition & mdl南京市.analyseStr(mstrStream, 74, 10) & ";"                             '11帐户余额
        ...
        mlng病人ID = BuildPatiInfo(1, strIdentify & strAddition, mlng病人ID)
        '返回格式:中间插入病人ID
        If mlng病人ID > 0 Then
...
Private Sub Timsearch_Timer()
    ...
    Select Case mbytType
        Case 1
            strFile = "C:\NJYB\ZYDJXX.TXT"
        Case 9
            strFile = "C:\NJYB\CYJSD.TXT"
        Case Else
            strFile = "C:\NJYB\MZJSHZ.TXT"
    End Select
...
'        Timsearch.Enabled = False
    ...
    Else
        ...
        lbl医保卡号.Caption = mdl南京市.analyseStr(mstrStream, 1, 20)
        lbl提示尾.Caption = IIf(mbytType = 1, "姓名:" & mdl南京市.analyseStr(mstrStream, 55, 12), "是否正确！")
    End If
End Sub
...
End Sub
Public Function getFeeBalance(Optional bytType As Byte) As String
    mbytType = bytType
    Me.Show 1
