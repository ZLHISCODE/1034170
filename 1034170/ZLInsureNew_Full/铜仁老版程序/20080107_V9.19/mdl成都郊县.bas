...
'返回：初始化成功，返回true；否则，返回false
    ...
    '提取当前接口适用地区
    mint适用地区_成都郊县 = 0
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & TYPE_成都郊县 & " ANd 序号=2"
    Call OpenRecordset(rsTemp, "提取当前接口适用地区")
    If Not rsTemp.EOF Then mint适用地区_成都郊县 = Nvl(rsTemp!参数值, 0)
