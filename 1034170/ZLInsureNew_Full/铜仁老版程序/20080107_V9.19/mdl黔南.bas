...
Public Function 医保初始化_黔南() As Boolean
    ...
    '取医院编码
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_黔南
...
Private Function Open中间库() As Boolean
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_黔南
    ...
    '中间库连接
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '中心%' and 险类=" & TYPE_黔南
    Call OpenRecordset(rsTemp, "渝北医保")
    Do Until rsTemp.EOF
        Select Case rsTemp("参数名")
            Case "中心用户名"
...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select nvl(帐户余额,0) as 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_黔南
    ...
    With rs明细
        ...
        Do Until rs明细.EOF
            gstrSQL = "select A.名称,A.编码,A.类别,A.规格,A.计算单位,B.项目编码,B.附注,B.是否医保,A.计算单位,E.规格,G.名称 剂型 " & _
                      "from 收费细目 A,(Select * From 保险支付项目 where 险类=" & gintInsure & ") B,药品目录 E ,药品信息 F,药品剂型 G " & _
...
Private Function 检查中心维护单价(ByVal lng细目ID As Long, ByVal dbl单价 As Double) As Boolean
    ...
    gstrSQL = "" & _
        "   Select * From 医保收费目录 " & _
        "   Where (类别,编码) in (select 附注,项目编码 From 保险支付项目 where 险类=" & gintInsure & " and 收费细目id=" & lng细目ID & " )"
    ...
    If Nvl(rsTemp!维护标志) = STR_中心维护代码 Then
        ...
        If Val(strOutput) <> dbl单价 Then
            ...
            ShowMsgbox "存在项目“" & rsTemp!名称 & "”的单价不一样：" & vbCrLf & " 　医院:" & Format(dbl单价, "0.0000") & vbCrLf & "　中心:" & Format(Val(Split(strOutput, "1")(1)), "0.0000")
            Exit Function
        End If
    End If
    检查中心维护单价 = True
...
Public Function 门诊结算_黔南(lng结帐ID As Long, cur个人帐户 As Currency, str医保号 As String, cur全自付 As Currency) As Boolean
    ...
    If lng病人ID = oldlng病人ID And str结算时间 = Format(rs明细!登记时间, "yyyy-mm-dd HH:MM:SS") Then
            '需要重新分配一个新号给门诊
            gstrSQL = "Select nvl(就诊次数,0)+1 as 就诊次数 From 保险帐户 where 险类=" & gintInsure & " and 病人id=" & lng病人ID
            ...
            '更新保险帐户
            gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & gintInsure & ",'就诊次数','" & lng就诊次数 & "')"
            Call ExecuteProcedure("保存就诊次数")
    ...
    Call ExecuteProcedure("保存帐户年度信息")
   ...
   '插入保险结算记录
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & gintInsure & "," & lng病人ID & "," & Year(datCurr) & "," & _
...
errHandle:
    ...
    End If
End Function
Public Function 更新就诊信息_黔南(ByVal bytType As Byte, strOutPutstring As String, Optional ByVal str入院日期 As String = "", Optional ByVal str出院日期 As String = "", Optional bln反结算 As Boolean = False) As Boolean
...
Public Function 门诊结算冲销_黔南(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    '帐户年度信息
    Call Get帐户信息(lng病人ID, Year(curDate), int住院次数累计, dbl帐户增加累计, dbl帐户支出累计, dbl进入统筹累计, dbl统筹报销累计)
    ...
    Call ExecuteProcedure("插入帐户年度信息")
    '插入保险结算记录
    '原过程参数:
    '   性质_IN  ,记录ID_IN,险类_IN,病人ID_IN,年度_IN," & _
    "   帐户累计增加_IN,帐户累计支出_IN,累计进入统筹_IN,累计统筹报销_IN,住院次数_IN,起付线_IN,封顶线_IN,实际起付线_IN,
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng冲销ID & "," & gintInsure & "," & lng病人ID & "," & Year(curDate) & "," & _
...
Private Function 病人入院登记处理(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    '保存将交易流水号
    gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & gintInsure & ",'顺序号','''" & str交易流水号 & "''')"
...
Public Function 住院结算_黔南(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
  '功能：将需要本次结帐的费用明细和结帐数据发送医保前置服务器确认；
   ...
   '插入保险结算记录
    ...
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & gintInsure & "," & lng病人ID & "," & Year(datCurr) & "," & _
            cur帐户增加累计 & "," & cur帐户支出累计 & "," & cur进入统筹累计 & "," & cur统筹报销累计 & "," & int住院次数累计 & ",0,0," & IIf(g病人身份_黔南.下个人帐户, "1", "0") & "," & _
            g病人身份_黔南.费用总额 & ",0,0," & _
...
Public Function 住院结算冲销_黔南(lng结帐ID As Long) As Boolean
   ...
   '插入保险结算记录
    ...
    '   进入统筹金额_IN(统筹支付),统筹报销金额_IN(统筹支付),大病自付金额_IN(无),超限自付金额_IN(无),个人帐户支付_IN(个人帐户支付),"
    '   支付顺序号_IN(结算时产生流水号),主页ID_IN,中途结帐_IN,备注_IN
    gstrSQL = "zl_保险结算记录_insert(2," & lng冲销ID & "," & gintInsure & "," & rsTemp("病人ID") & "," & Year(curDate) & "," & _
...
Private Function 处方上传(ByVal lng记录性质 As Long, lng记录状态 As Long, ByVal str单据号 As String) As Boolean
   ...
   '读出该张单据的费用明细
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.主页ID,to_char(A.发生时间,'yyyy-mm-dd hh24:mi:ss') as 登记时间,Round(A.实收金额,4) 实收金额 " & _
              ...
              "  From 病人费用记录 A,收费类别 J,收费细目 B,保险帐户 M,(Select * From 保险支付项目 where 险类=" & gintInsure & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
              "  where a.病人id=M.病人id  and M.险类=" & gintInsure & " and A.NO='" & str单据号 & "' and A.记录性质=" & lng记录性质 & " and A.记录状态=1 And Nvl(A.是否上传,0)=0 " & _
...
Public Function 住院虚拟结算_黔南(rsExse As Recordset, ByVal lng病人ID As Long, Optional bln结帐处 As Boolean = True) As String
    ...
    If 更新就诊信息_黔南(2, strOutput, "", str出院日期) = False Then Exit Function
...
Private Function Get费用类别(ByVal str收费类别 As String) As String
    ...
    If zlCommFun.ActualLen(str收费类别) = 1 Then
            ...
            End If
    End If
    gstrSQL = "Select * From 保险参数 where 参数名='" & str收费 & "'"
...
Private Function 补传住院明细记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select A.ID,A.NO,A.记录性质,J.类别 as 收费类别,A.记录状态,A.序号,A.病人ID,A.主页ID,A.发生时间 as 登记时间,Round(A.实收金额,4) 实收金额" & _
              ...
              "  From 病人费用记录 A,收费类别 J,收费细目 B, (Select * From 保险支付项目 where 险类=" & gintInsure & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
...
Public Function 下载服务项目目录_黔南(ByVal bytType As Byte, ByVal objProgss As Object) As Boolean
    ...
    With rsData
        Do While Not .EOF
            ...
            Select Case bytType
            Case 1
                strSql = strSql & "'" & Nvl(!AKA060) & "',"    'AKA060  VARCHAR2(20)                   药品编码
                strSql = strSql & "'" & Nvl(!AKA061) & "',"   'AKA061 VARCHAR2(50)  Y                中文名称
                strSql = strSql & "'" & Nvl(!AKA062) & "',"  'AKA062 VARCHAR2(50)  Y                英文名称
                strSql = strSql & "'" & Nvl(!AKA063) & "'," 'AKA063 VARCHAR2(3)   Y                收费类别
                strSql = strSql & "'" & Nvl(!AKA065) & "'," 'AKA065 VARCHAR2(3)   Y                收费项目等级
                strSql = strSql & "'" & Nvl(!AKA066) & "',"  'AKA066 VARCHAR2(14)  Y                助记码
                strSql = strSql & "'" & Nvl(!AKA067) & "',"  'AKA067 VARCHAR2(20)  Y                单位
                strSql = strSql & "" & Val(Nvl(!AKA068)) & ","  'AKA068 NUMBER(8,2)   Y                标准价格
                strSql = strSql & "'" & Nvl(!AKA070) & "',"   'AKA070 VARCHAR2(50)  Y                剂型
                strSql = strSql & "'" & Nvl(!AKA074) & "',"    'AKA074 VARCHAR2(50)  Y                规格
                strSql = strSql & "'" & Nvl(!AAE013) & "',"      'AAE013 VARCHAR2(100) Y                备注
                'AAE035 DATE          Y                变更日期
                strSql = strSql & "to_date('" & Format(!AAE035, "yyyy-mm-dd HH:MM:SS") & "','yyyy-mm-dd hh24:mi:ss'),"
                strSql = strSql & "'" & Nvl(!AAA104) & "',"     'AAA104 VARCHAR2(3)   Y                代码可维护标志
                strSql = strSql & "'" & "" & "',"  'AAA104 VARCHAR2(3)   Y                支付标准
                
                strSql = strSql & "'" & Nvl(!AKA305) & "',"      'AKA305 VARCHAR2(3)   Y                药品种类
                strSql = strSql & "'" & Nvl(!AKA064) & "',"      'AKA064 VARCHAR2(3)   Y                处方药标志
                strSql = strSql & "" & Val(Nvl(!AKA069)) & ","       'AKA069 NUMBER(5,4)   Y                自付比例
                strSql = strSql & "" & Val(Nvl(!AKA071)) & ","      'AKA071 NUMBER(5,2)   Y                每次用量
                strSql = strSql & "'" & Nvl(!AKA072) & "',"      'AKA072 VARCHAR2(20)  Y                使用频次
                strSql = strSql & "'" & Nvl(!AKA073) & "',"      'AKA073 VARCHAR2(50)  Y                用法
                strSql = strSql & "'" & Nvl(!AKA030) & "',"      'AKA030 VARCHAR2(3)   Y                当前使用标志
                strSql = strSql & "'" & Nvl(!AAB034) & "',"      'AAB034 VARCHAR2(14)  Y                社保经办机构
