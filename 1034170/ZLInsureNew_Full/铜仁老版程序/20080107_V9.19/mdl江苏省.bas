...
Public Function 挂号结算_江苏(ByVal lng结帐ID As Long, cur金额 As Currency) As Boolean
    ...
    '获取病人挂号信息
    gstrSQL = "select a.no,a.id,a.病人id,a.姓名,a.病人科室id,c.名称 as 科室名称,a.实收金额,a.操作员姓名,a.费别,to_char(a.发生时间,'yyyy-mm-dd HH24:MI:SS') as 时间,a.收据费目,b.名称,b.编码 from 病人费用记录 a,收费细目 b,部门表 c where 结帐id=" & lng结帐ID & " and b.id=a.收费细目id and a.病人科室id=c.id order by 收据费目"
    Call OpenRecordset(rsTemp, gstrSysName)
    lng病人id = rsTemp!病人ID
    strCode1 = rsTemp!编码
    strName1 = rsTemp!名称
...
'        sng个帐支付 = cur个帐余额
'    End If
    Call Get帐户信息(lng病人id, Year(Date), int住院次数累计, cur帐户增加累计, cur帐户支出累计, cur进入统筹累计, cur统筹报销累计)
    gstrSQL = "zl_帐户年度信息_insert(" & lng病人id & _
            "," & gintInsure & "," & Year(Date) & "," & cur帐户增加累计 & _
...
'Public Declare Function FCancleOutpatReg Lib "HInterface.dll" (ByVal strRecCode As String, ByVal strOpCode As String) As Integer
    gstrSQL = "Select 退休证号 From 保险帐户 Where 病人id In (Select 病人id From 病人费用记录 Where 结帐id=" & lng结帐ID & ")"
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select nvl(帐户余额,0) as 帐户余额 from 保险帐户 where 病人ID='" & lng病人id & "' and 险类=" & TYPE_江苏
    Call OpenRecordset(rsTemp, gstrSysName)
...
Public Function 门诊虚拟结算_江苏(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    '调用预结算函数进行门诊预结算
    gstrSQL = "select a.姓名,a.编号,a.id,c.编码 as 部门id,c.名称 from 人员表 a,部门人员 b,部门表 c where a.id=b.人员id and a.姓名='" & rs明细!开单人 & "' and c.id=b.部门id"
    Call OpenRecordset(rsTemp, gstrSysName)
...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "select 开单部门id,开单人,b.编码,c.编号 from 病人费用记录 a,部门表 b,人员表 c where b.id=a.开单部门id and c.姓名=a.开单人 and a.结帐id=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
    ...
    intReturn = FOutpatBalance(gstrRecCode, UserInfo.编号, "是", rsTemp!编码, rsTemp!编号, CStr(gint医疗方式), _
        ...
        sngArrInfo(10), sngArrInfo(11), sngArrInfo(12), sngArrInfo(13), sngArrInfo(14))
    If intReturn <> 0 Then
        MsgBox "在进行医保门诊结算时发生错误，未取得错误信息。", vbInformation, gstrSysName
        门诊结算_江苏 = False
        Exit Function
    End If
...
'        MsgBox "没有需要上传的收费记录", vbExclamation, gstrSysName
    ...
    lng病人id = rs明细("病人ID")
    If int门诊标志 = 2 Then
        gstrSQL = "Select nvl(顺序号,0) as 顺序号 From 保险帐户 Where 病人ID=" & lng病人id & " And 险类=" & TYPE_江苏
    Else
        gstrSQL = "Select nvl(退休证号,0) as 顺序号 From 保险帐户 Where 病人ID=" & lng病人id & " And 险类=" & TYPE_江苏
    End If
    Call OpenRecordset(rsTemp, gstrSysName)
...
'13是否急诊
            ...
            Err.Clear
            On Error GoTo errHandle
            gstrSQL = "select b.编号,b.姓名,c.编码,c.名称 from 部门人员 a,(select id,编号,姓名 from 人员表 Where 姓名='" & strTemp & "') b,(select id,编码,名称 from 部门表) c where a.部门id=c.id and a.人员id=b.id"
            Call OpenRecordset(rsTemp, gstrSysName)
...
'            gstrSQL = "Select * From 收费细目 Where ID=" & rs明细!收费细目ID
            gstrSQL = "select a.费用类型,A.名称,C.项目编码 as 编码,A.计算单位,B.产地,decode(B.药品来源,'国产',1,'合资',2,'进口',3,null) 产地特征,B.规格" & _
                  " from 收费细目 A,药品目录 B,保险支付项目 C where A.id = C.收费细目id and A.id=B.药品id(+) and A.id =" & rs明细!收费细目ID & " And C.险类=" & gintInsure
            Call OpenRecordset(rsTemp, gstrSysName)
...
'入口参数：收费流水号,结账流水号,操作员工号
    intReturn = FCancelOutpatBalance(strRecCode, strBillCode, UserInfo.编号, sngArrInfo(0), sngArrInfo(1), sngArrInfo(2), _
        ...
        sngArrInfo(10), sngArrInfo(11), sngArrInfo(12), sngArrInfo(13), sngArrInfo(14))
    If intReturn <> 0 Then
        MsgBox "进行门诊结算冲销时发生错误，未获得错误信息。", vbInformation, gstrSysName
        门诊结算冲销_江苏 = False
        Exit Function
    End If
...
'返回：交易成功返回true；否则，返回false
    ...
    '获取住院医师代码
    gstrSQL = "Select * from 人员表 Where 姓名='" & rsTemp!住院医师 & "'"
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "Select 病人ID,结帐金额,主页id From 病人费用记录 Where nvl(附加标志,0)<>9 and 结帐ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
    lng主页ID = rsTemp!主页ID
    Do Until rsTemp.EOF
        If lng病人id = 0 Then lng病人id = rsTemp("病人ID")
    ...
    gstrSQL = "Select * from 保险帐户 where 病人ID=" & lng病人id & " And 险类=" & TYPE_江苏
    Call OpenRecordset(rsTemp, gstrSysName)
...
'          13个人储蓄帐户支付,14现金支付
    ...
    If intReturn <> 0 Then
        MsgBox "住院结算冲销时发生错误。", vbInformation, gstrSysName
        住院结算冲销_江苏 = False
        Exit Function
    End If
...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select nvl(顺序号,0) as 顺序号,帐户余额 From 保险帐户 Where 病人ID=" & lng病人id & " And 险类=" & TYPE_江苏
    Call OpenRecordset(rsTemp, gstrSysName)
    str就诊编号 = rsTemp!顺序号
    cur余额 = rsTemp!帐户余额
...
'          13个人医疗帐户支付,14个人储蓄帐户支付,15现金支付
    ...
    If intReturn <> 0 Then
        MsgBox "住院病人预结算时发生错误，未获得错误信息。", vbInformation, gstrSysName
        住院结算_江苏 = False
        Exit Function
    End If
    ...
    Call ExecuteProcedure(gstrSysName)
    '保险结算记录
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & gintInsure & "," & _
            lng病人id & "," & Year(datCurr) & "," & _
...
'    End If
    ...
    住院虚拟结算_江苏 = "个人帐户;" & cur个人帐户支付 & ";0" '不允许修改个人帐户
    If cur统筹支付 <> 0 Then
        住院虚拟结算_江苏 = 住院虚拟结算_江苏 & "|统筹基金;" & cur统筹支付 & ";0" '不允许修改统筹支付
    End If
    If cur补充医保 <> 0 Then
        住院虚拟结算_江苏 = 住院虚拟结算_江苏 & "|大病统筹;" & cur补充医保 & ";0"
    End If
    If cur医保支付 <> 0 Then
        住院虚拟结算_江苏 = 住院虚拟结算_江苏 & "|公务员/企业补充支付;" & cur医保支付 & ";0"
    End If
    WriteInfo "完成预结"
    Exit Function
errHandle:
...
'返回：交易成功返回true；否则，返回false
    ...
    gstrSQL = "Select Sum(实收金额) From 病人费用记录 Where 病人id=" & lng病人id & " And 主页id=" & lng主页ID
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
...
'            MsgBox "撤消入院登记时发生错误，未获取错误信息。", vbInformation, gstrSysName
    ...
    Else
        gstrSQL = "select A.出院日期,D.名称 as 出院科室,D.编码 as 科室编码,A.出院病床,A.住院医师," & _
                ...
                "And A.出院科室ID = D.ID And A.主页ID = " & lng主页ID & " And A.病人ID = " & lng病人id
        Call OpenRecordset(rsTemp, gstrSysName)
        If rsTemp.EOF Then
            MsgBox "未能获取入院病人的相关信息。", vbInformation, gstrSysName
            出院登记_江苏 = False
...
'功能：通过医保中心号码和医保号求出病人ID
    ...
    On Error GoTo errHandle
    gstrSQL = "select 病人ID from 保险帐户 where 险类 = '" & TYPE_江苏 & _
            "' and 医保号 = '" & str医保号 & "'"
    Call OpenRecordset(rsTmp, gstrSysName)
...
Public Function 记帐传输_江苏(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人id As Long = 0) As Boolean
    ...
    If str消息 = "" Then
        gstrSQL = "Select * From 病人费用记录 Where 门诊标志=2 And 实收金额 Is Not Null And 记录状态<>0 And Nvl(是否上传,0)=0 And nvl(附加标志,0)<>9 and 病人id=" & lng病人id & " And 主页id=" & lng主页ID & " order by NO,序号"
    Else
        gstrSQL = "Select * From 病人费用记录 Where 门诊标志=2 And 实收金额 Is Not Null And 记录状态<>0 And nvl(附加标志,0)<>9 and 病人id=" & lng病人id & " And 主页id=" & lng主页ID & " order by NO,序号"
    End If
