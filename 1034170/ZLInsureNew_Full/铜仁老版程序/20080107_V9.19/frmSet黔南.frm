...
End Function
 Private Function iniData() As Boolean
    ...
    Call initGrid
    strSql = "" & _
        "   Select A.类别,b.参数值 From 收费类别 a,(Select * From 保险参数 where 险类=" & gintInsure & ") b " & _
        "   Where A.类别=b.参数名(+) " & _
        "   order by A.编码 "
...
Private Function IsValid() As Boolean
    ...
    If mcnZxTest.State = adStateClosed Then
        If OraDataOpen(mcnZxTest, txtEdit(Text中心服务器).Text, txtEdit(text中心用户).Text, txtEdit(Text中心密码).Tag, False) = False Then
            If MsgBox("医保服务器不能正常连接，是否继续？", vbQuestion Or vbYesNo Or vbDefaultButton2, gstrSysName) = vbNo Then
                Exit Function
            End If
        End If
    ...
    With mshBill
        For lngRow = 1 To .Rows - 1
            If .TextMatrix(lngRow, mColHead.收费类别) <> "" Then
                '新增参数数据
                gstrSQL = "zl_保险参数_Insert(" & gintInsure & ",null,'" & .TextMatrix(lngRow, mColHead.收费类别) & "' ,'" & .TextMatrix(lngRow, mColHead.保费项目) & "'," & lngRow + 2 & ")"
                Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
            End If
        Next
    End With
