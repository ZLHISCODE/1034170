...
Private Sub cmd疾病信息_Click(Index As Integer)
    ...
    gstrSQL = " Select A.ID,A.编码,A.名称,A.简码 " & _
            " From 保险病种 A where A.险类=" & TYPE_沈阳市 & " Order by A.简码"
    Set rs病种 = New ADODB.Recordset
    Call OpenRecordset(rs病种, "身份验证")
    If rs病种.RecordCount > 0 Then
        If frmListSel.ShowSelect(rs病种, "ID", "医保病种选择", "请选择" & IIf(Index = 0, "入院", "出院") & "病种：") = True Then
...
Private Sub Form_Load()
    ...
    gstrSQL = " Select B.姓名,A.卡号,A.医保号,C.ID 入院病种ID,C.编码 入院病种编码,C.名称 入院病种名称," & _
              ...
              " Where A.病人ID=B.病人ID And A.病人ID=" & mlng病人ID & " And A.险类=" & mint险类 & _
              " And C.ID(+)=A.病种ID And D.ID(+)=A.出院病种ID"
    Call OpenRecordset(rsTemp, "读取该病人的基本信息")
...
Private Sub txt疾病信息_KeyPress(Index As Integer, KeyAscii As Integer)
    ...
    gstrSQL = "Select A.ID,A.编码,A.名称,A.简码" & _
             "   FROM 保险病种 A WHERE A.险类=" & mint险类 & " And (" & _
                zlCommFun.GetLike("A", "编码", strText) & " or " & zlCommFun.GetLike("A", "名称", strText) & " or " & zlCommFun.GetLike("A", "简码", strText) & ")" & _
             " Order by A.简码"
