...
Private Sub cmdCancel_Click()
    ...
    End If
    '先检查应答文件是否存在，如果存在则提示
    If objFileSys.FileExists(mstrPath_福建巨龙 & gintInsure & "\" & mstrReply_福建巨龙) Then
...
Private Sub Form_Activate()
    ...
    If mint操作方式 <> 操作方式.验证 Then
        If Not objFileSys.FolderExists(mstrPath_福建巨龙 & gintInsure) Then objFileSys.CreateFolder (mstrPath_福建巨龙 & gintInsure)
        LblNote.Caption = "  正在向医保服务器发送请求..."
        If SendRequest(mint操作方式, mint请求目的, mlng病人ID, mlng结帐ID) = False Then
            mblnReturn = False
            Unload Me
            Exit Sub
        End If
...
Public Function ShowME(ByVal 操作 As Integer, ByVal 目的 As Integer, Optional ByVal 病人ID As Long = 0, _
...
Private Sub TimeSearch_Timer()
    ...
    If mblnReturn Then
        ...
        Or mint操作方式 = 操作方式.收费 Or mint操作方式 = 操作方式.入院 Or mint操作方式 = 操作方式.验证) Then
            mblnReturn = frmIdentify福建巨龙.ShowCard(获取病人ID)
        End If
    End If
...
Private Function SearchFile() As Integer
    ...
    If mint操作方式 <> 操作方式.验证 Then
        ...
        SearchFile = AnalyseReply(mint操作方式, mint请求目的)
    Else
        SearchFile = 1
    End If
End Function
