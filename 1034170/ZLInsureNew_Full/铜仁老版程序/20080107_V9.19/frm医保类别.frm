...
Private Sub lvwKind_S_ItemClick(ByVal Item As MSComctlLib.ListItem)
    ...
    If mbln年龄段 = True Then
        ...
        '然后再按新的人群分类打开
        gstrSQL = "select * from 保险人群 where 险类=" & Mid(Item.Key, 2) & " order by 序号"
...
'功能：显示所有医保类别列表
    ...
    gstrSQL = "select 序号,名称,说明,医院编码,是否固定,具有中心,是否禁止 from 保险类别"
    Call OpenRecordset(rsTemp, Me.Caption)
...
'功能：根据医保中心的序号显示参数
    ...
    With msh参数
        Select Case Val(Mid(lvwKind_S.SelectedItem.Key, 2))
            ...
            Case TYPE_福建巨龙, TYPE_福建省, TYPE_福州市, TYPE_南平市
                gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & Mid(lvwKind_S.SelectedItem.Key, 2) & " and (中心=" & cmb中心.ItemData(cmb中心.ListIndex) & _
                        " or 中心 is null) order by 序号"
            ...
            Case Else
                ...
                '固定参数不能修改与查看，只能由
                gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & Mid(lvwKind_S.SelectedItem.Key, 2) & " and (中心=" & cmb中心.ItemData(cmb中心.ListIndex) & _
                        " or 中心 is null) and 参数名 not like '%密码%' and 参数名 not in('卡验证码') and (是否固定<>1 Or 是否固定 Is null ) order by 序号"
                rsTemp.CursorLocation = adUseClient
                Call OpenRecordset(rsTemp, Me.Caption)
...
'功能：根据医保中心的序号显示年龄分段与费用档次
    ...
    With msh分段
        ...
        gstrSQL = "select A.在职,A.名称,A.下限,A.上限,B.名称 as 人群名称,B.序号 " & _
                ...
                " where A.险类(+)=B.险类 and A.在职(+)=B.序号 and A.中心(+)=" & lng中心 & " and B.险类=" & lng险类 & _
                " Order by A.在职,A.年龄段"
        Call OpenRecordset(rsTemp, Me.Caption)
