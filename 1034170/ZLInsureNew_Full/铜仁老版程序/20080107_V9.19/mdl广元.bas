...
'         单位,单价,数量,时间,录入人,标志
        If IsNull(rs明细!是否上传) Or rs明细!是否上传 = 0 Then
            mblnReturn = wdetails(gstr医保机构编码, gstr医院编码, str就诊编号, str处方号, iLoop, _
                rsTemp!类别 & "_" & rsTemp!ID, rsTemp!名称, " ", ToVarchar(strTemp, 10), Nvl(rsTemp!费用类型, " "), Nvl(rsTemp!计算单位, " "), rs明细!标准单价, _
...
Public Function 记帐传输_广元(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人id As Long = 0) As Boolean
    ...
    Else
        gstrSQL = "Select * From 病人费用记录 Where 记录状态<>0 And Nvl(是否上传,0)=0 And nvl(附加标志,0)<>9 and 病人id=" & lng病人id & " order by 主页ID,序号"
    End If
