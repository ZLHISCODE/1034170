...
Public Function 门诊虚拟结算_大连(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    '门诊的起付线为零
    If 大连医保结算(0, lng病人ID, 0, 0, 0, False, True, 0, dbl诊察费, dbl草药费, dbl成药费, dbl西药费, dbl检查费, dbl治疗费, dbl血费, dbl血费自费, dbl大检费, dbl大检自费, dbl特殊治疗费, dbl特殊治疗自费, dbl保险内自费费用, dbl非保险费用, dbl其它费, curTotal, str医生, str结算方式) = False Then
        Exit Function
    End If
    Call WriteDebugInfor_大连("门诊虚拟结算_大连", lng病人ID)
    门诊虚拟结算_大连 = True
End Function
Private Function Get结算方式(ByVal strOutput As String) As String
    '功能:获取结算方式
    '参数:strOutPut-输出串
    '返回:结算方式
    '开发区:
   ...
   '大连市:
    ...
    '现金=本次非保险自付
End Function
Public Function 门诊记帐虚拟结算_大连(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    '参数：rsExse     费用明细(传入)
    '      cur结算方式  "报销方式;金额;是否允许修改|...."
    '明细字段
    ...
    Dim strInfor As String  '定义中心返回串
    Dim dbl诊察费 As Double, dbl草药费 As Double, dbl成药费 As Double, dbl西药费 As Double, dbl检查费 As Double
    Dim dbl治疗费 As Double, dbl大检费 As Double, dbl大检自费 As Double, dbl特殊治疗费 As Double, dbl特殊治疗自费 As Double
    ...
    If 身份标识_大连(0, lng病人ID) = "" Then
        ...
        门诊记帐虚拟结算_大连 = False
        Call WriteDebugInfor_大连("门诊记帐虚拟结算_大连(身份标识_大连)", lng病人ID)
    ...
    With rs明细
        ...
        Do While Not .EOF
            '---周顺利,对金额是否为负数进行判断,如果为负数不准执行医保收费
            '门诊记帐记录中可能会包含有负数冲抵的项目,因此取消负数控制的检查
            'If !实收金额 < 0 Then
            '    ShowMsgbox "该单据中包含有金额为负数的项目,不能执行医保收费!请检查后重新收费"
            '    门诊记帐虚拟结算_大连 = False
            '    Exit Function
...
'                End If
    ...
    If 大连医保结算(0, lng病人ID, 0, 0, 0, False, True, dbl起付标准, dbl诊察费, dbl草药费, dbl成药费, dbl西药费, dbl检查费, dbl治疗费, dbl血费, dbl血费自费, dbl大检费, dbl大检自费, dbl特殊治疗费, dbl特殊治疗自费, dbl保险内自费费用, dbl非保险费用, dbl其它费, curTotal, str医生, str结算方式) = False Then
        Exit Function
    End If
    
    门诊记帐虚拟结算_大连 = True
End Function
Private Function 门诊记帐结算及冲帐_大连(ByVal bln冲销 As Boolean, ByVal lng病人ID As Long, ByVal lng结帐ID As Long, ByVal 原结帐id As Long, ByVal lng主页ID As Long) As Boolean
  ...
  '功能：将门诊收费的明细和结算数据转发送医保前置服务器确认；
    ...
    Dim strInfor As String  '定义中心返回串
    Dim dbl诊察费 As Double, dbl草药费 As Double, dbl成药费 As Double, dbl西药费 As Double, dbl检查费 As Double, dbl治疗费 As Double
    ...
    If bln冲销 Then
        ...
        '确定退费记录
        '退费
        gstrSQL = "select id from 病人费用记录 where 结帐id=" & lng冲销ID
    ...
    gstrSQL = " " & _
        ...
        "  From (Select * From 病人费用记录 Where 记录状态<>0 and 结帐ID=" & IIf(bln冲销, lng冲销ID, lng结帐ID) & " and  Nvl(附加标志,0)<>9 ) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E,  " & _
...
'                End If
            ...
            If gbln门诊明细时实上传 Then
                    ...
                    If Nvl(!险类, 0) = type_大连开发区 Then
                        str明细 = str明细 & Lpad(g病人身份_大连.治疗序号 + 1, 4) '治疗序号    NUM 23  4   住院明细：必须等于入院登记时治疗序号门诊明细:                         必须等于本次结算治疗序号 院端
                    Else
                        '刘兴宏:2004/09/11,治疗序号不加1
                        'str明细 = str明细 & Lpad(g病人身份_大连.治疗序号 + 1, 4)    '治疗序号
                ...
                '上传明细
                '1003    7   230 实时医疗明细数据提交
                门诊记帐结算及冲帐_大连 = 业务请求_大连(IIf(Nvl(!险类, 0) = type_大连开发区, 2, 1), 1003, str明细)
                If 门诊记帐结算及冲帐_大连 = False Then
                    ShowMsgbox "门诊记帐结算时医疗明细数据提交失败,不能继续!"
                    Exit Function
                End If
                '上传医嘱明细
                If Nvl(!医嘱序号, 0) <> 0 Then
                    If 医嘱明细数据提交(!医嘱序号, "", str项目统计分类) = False Then
    ...
    If 大连医保结算(2, lng病人ID, 0, IIf(bln冲销, lng冲销ID, lng结帐ID), 原结帐id, bln冲销, False, 0, dbl诊察费, dbl草药费, dbl成药费, dbl西药费, dbl检查费, dbl治疗费, dbl血费, dbl血费自费, dbl大检费, dbl大检自费, dbl特殊治疗费, dbl特殊治疗自费, dbl保险内自费费用, dbl非保险费用, dbl其它费, curTotal, str医生, strInfor) = False Then
        Exit Function
    End If
    门诊记帐结算及冲帐_大连 = True
    Exit Function
ErrHand:
    ...
    End If
End Function
Private Function 大连医保结算(ByVal bytType As Byte, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng结帐ID As Long, ByVal lng原结帐ID As Long, ByVal bln冲销 As Boolean, ByVal bln虚拟结算 As Boolean, dbl起付标准 As Double, _
        ...
        dbl其它费 As Double, curTotal As Double, str医生 As String, str结算方式 As String, Optional str住院号 As String = "", Optional str出院日期 As String = "") As Boolean
    ...
    With g病人身份_大连
        ...
        End If
        strInputString = strInputString & String(11, " ")      '错误代码
    End With
    ...
    '业务请求
    If bln虚拟结算 Then
        blnReturn = 业务请求_大连(IIf(gintInsure = type_大连开发区, 2, 1), 1006, strInputString)
        
    Else
        blnReturn = 业务请求_大连(IIf(gintInsure = type_大连开发区, 2, 1), 1002, strInputString)
    End If
    
    If blnReturn = False Then Exit Function
    
    If bln虚拟结算 = True Then
        str结算方式 = Get结算方式(strInputString)
        大连医保结算 = True
        Exit Function
    End If
    Dim i As Long
    ...
    '周海全调试 2004-10-20
    '冲销时，如果值应该为负数:
    '   1)诊察费/草药费/成药费/西药费/检查费/治疗费/血费/血费自费/大检费/大检自费/特殊治疗费/特殊治疗自费/保险内自费费用/非医疗保险费用
    '   2)发生费用总额也应该同时为负

    If bln冲销 Then
        gstrSQL = "zl_保险结算记录_insert(" & IIf(bytType = 0, 1, 2) & "," & lng结帐ID & "," & gintInsure & "," & lng病人ID & "," & Format(zlDatabase.Currentdate, "YYYY") & "," & _
...
Public Function 门诊结算_大连(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    Call WriteDebugInfor_大连("门诊结算_大连", lng病人ID)
    Exit Function
ErrHand:
...
'-----------------------------------------------------调整开始-------------------------------------------------------
            ...
            If gbln门诊明细时实上传 Then
                    ...
                    '产生本地明细SQL语句:
                    '过程参数:
                          '费用ID_IN ,病人ID_IN,险类_IN,医院代码_IN,个人编号_IN,病志号_IN,治疗序号_IN,处方号_IN,处方项目序号_IN,
                          '    医嘱号,就诊分类_IN,处方生成时间_IN,项目代码_IN,项目名称_IN,自费比例_IN,项目统计分类_IN,数量_IN,
                          '    单价_IN,单位_IN,剂型_IN,医师姓名_IN,诊断编码_IN,诊断名称_IN,单量,频次,用法
                    
                    strInsertSQL = "ZL_上传明细记录_INSERT("
                    strInsertSQL = strInsertSQL & "" & Nvl(!ID, 0) & ","
                    strInsertSQL = strInsertSQL & "" & Nvl(!病人ID, 0) & ","
                    strInsertSQL = strInsertSQL & "" & Nvl(!险类, 0) & ","
                    
                    strInsertSQL = strInsertSQL & "'" & gstr医院编码_大连 & "',"
                    strInsertSQL = strInsertSQL & "'" & Nvl(!医保号) & "',"
                    strInsertSQL = strInsertSQL & "'" & "" & "',"
                    
                    '刘兴宏:2004/09/11取消加1
                    strInsertSQL = strInsertSQL & "" & g病人身份_大连.治疗序号 & ","
                    
                    
                    'Modified By 朱玉宝 2004-07-29 原因：处理NO号
                    strInsertSQL = strInsertSQL & "" & Mid(Nvl(!NO, "00000000"), 2, 7) & ","
                    strInsertSQL = strInsertSQL & "" & Lpad(CStr(.AbsolutePosition), 10) & ","
                    strInsertSQL = strInsertSQL & "" & "NULL" & ","
                    
                    strInsertSQL = strInsertSQL & "'" & Get就诊分类(int业务, Nvl(!灰度级, 0)) & "',"
                    If Nvl(!险类, 0) = type_大连开发区 Then '开发区
                        strInsertSQL = strInsertSQL & "to_date('" & Format(!就诊时间, "yyyy-MM-dd HH:mm:ss") & "','yyyy-mm-dd HH24:mi:ss'),"
                    Else
                        strInsertSQL = strInsertSQL & "to_date('" & Format(!登记时间, "yyyy-MM-dd HH:mm:ss") & "','yyyy-mm-dd HH24:mi:ss'),"
                    End If
                    strInsertSQL = strInsertSQL & "'" & Nvl(!国家编码) & "',"
                    strInsertSQL = strInsertSQL & "'" & Nvl(!项目名称) & "',"
                    
                    If Nvl(!险类, 0) = type_大连开发区 Then '开发区
                        strInsertSQL = strInsertSQL & "" & "NULL" & "," '自费比例
                    Else
                        If !是否医保 = 1 Then
                            strInsertSQL = strInsertSQL & "" & 1 - dbl比例 & "," '自费比例
                        Else
                            strInsertSQL = strInsertSQL & "" & 1 & ","  '自费比例
                        End If
                    End If
                    
                    strInsertSQL = strInsertSQL & "'" & str项目统计分类 & "',"
                    strInsertSQL = strInsertSQL & "" & Nvl(!数量, 0) & ","
                    strInsertSQL = strInsertSQL & "" & Nvl(!实际价格, 0) & ","
                    strInsertSQL = strInsertSQL & "'" & Nvl(!计算单位) & "',"
                    strInsertSQL = strInsertSQL & "'" & Nvl(!剂型) & "',"
                    
                    If Nvl(!险类, 0) = type_大连开发区 Then '开发区
                        strInsertSQL = strInsertSQL & "'" & Nvl(!医生编号) & "',"
                    Else
                        strInsertSQL = strInsertSQL & "'" & Nvl(!医生) & "',"
                    End If
                    
                    strInsertSQL = strInsertSQL & "'" & g病人身份_大连.诊断编码 & "',"
                    strInsertSQL = strInsertSQL & "'" & g病人身份_大连.诊断名称 & "',"
                    ...
                    If Nvl(!险类, 0) = type_大连开发区 Then '开发区
                        If rsTemp.EOF Then
                            ...
                            strInsertSQL = strInsertSQL & "" & "NULL" & ","
                        Else
                            strInsertSQL = strInsertSQL & "" & Nvl(rsTemp!单量, 0) & ","
                            strInsertSQL = strInsertSQL & "'" & Nvl(rsTemp!频次, " ") & "',"
                            strInsertSQL = strInsertSQL & "'" & Nvl(rsTemp!用法, " ") & "',"
                        End If
                    Else
                            strInsertSQL = strInsertSQL & "" & "NULL" & ","
                            strInsertSQL = strInsertSQL & "" & "NULL" & ","
                            strInsertSQL = strInsertSQL & "" & "NULL" & ","
                    End If
                    strInsertSQL = strInsertSQL & "'" & Nvl(!开单部门, " ") & "')"
                    
                    '保存明细数据
                    zlDatabase.ExecuteProcedure strInsertSQL, "保存明细数据"
    ...
    '门诊的起付线为零
    If 大连医保结算(0, lng病人ID, 0, IIf(bln冲销, lng冲销ID, lng结帐ID), lng结帐ID, bln冲销, False, 0, dbl诊察费, dbl草药费, dbl成药费, dbl西药费, dbl检查费, dbl治疗费, dbl血费, dbl血费自费, dbl大检费, dbl大检自费, dbl特殊治疗费, dbl特殊治疗自费, dbl保险内自费费用, dbl非保险费用, dbl其它费, curTotal, str医生, strInfor) = False Then
        Exit Function
    End If
...
Public Function 门诊结算冲销_大连(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    Call WriteDebugInfor_大连("门诊结算冲销_大连", lng病人ID)
    Exit Function
ErrHand:
    门诊结算冲销_大连 = False
End Function
...
Public Function 入院登记_大连(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
    ...
    Call WriteDebugInfor_大连("入院登记_大连", lng病人ID)
    入院登记_大连 = True
    Exit Function
ErrHand:
    If ErrCenter = 1 Then Resume
...
'--大连市医保的入院、撤消都与HIS入出基本无关,因此取消是否发生费用的判断
...
'
    ...
    入院登记撤销_大连 = True
    Call WriteDebugInfor_大连("入院登记撤销_大连", lng病人ID)
    Exit Function
ErrHand:
    If ErrCenter = 1 Then
        Resume
...
'    Call ExecuteProcedure("出院登记")
    出院登记_大连 = True
    Call WriteDebugInfor_大连("出院登记_大连", lng病人ID)
End Function
...
'    Call ExecuteProcedure("办理撤销出院登记")
    ...
    Else
        出院登记撤销_大连 = True
    End If
    Call WriteDebugInfor_大连("出院登记撤销_大连", lng病人ID)
    Exit Function
ErrHand:
    If ErrCenter = 1 Then
        Resume
...
Private Sub 获取病人信息_大连(ByVal lng病人ID As Long)
    ...
    If Not rsTemp.EOF Then
        ...
        End With
    End If
   Call WriteDebugInfor_大连("获取病人信息_大连", lng病人ID)
End Sub
Private Function Get治渝情况_大连(lng病人ID As Long, lng主页ID As Long) As String
    ...
    End If
    strTmp = Decode(strTmp, "治愈", "1", "好转", "2", "未愈", "3", "死亡", "4", "其他", "5")
   Call WriteDebugInfor_大连("Get治渝情况_大连", lng病人ID)
End Function
...
End Function
Private Function 计算各项费用额(ByVal lng病人ID As Long, ByVal str参数 As String, ByVal int算法 As Byte, ByVal str收费类别 As String, _
    ...
    ByVal dbl特准定额 As Double, dblmoney As Variant) As Boolean
        ...
        Dim rsTemp As New ADODB.Recordset
        Dim i As Long
        Const strTemp = "诊察费;草药费;成药费;西药费;检查费;治疗费;大检费;大检自费;血费;血费自费;特殊治疗费;特殊治疗自费;保险内自费费用;非保险费用;其他费"
        Dim strArr
        ...
        If dbl比例 <> 0 Then
            For i = 0 To UBound(strArr)
                If strTmp = strArr(i) Then
                    '"诊察费;草药费;成药费;西药费;检查费;治疗费;大检费;大检自费;血费;血费自费;特殊治疗费;特殊治疗自费;保险内自费费用;非保险费用;其他费"
                    Select Case strTmp
                        Case "诊察费", "草药费", "成药费", "西药费", "检查费"
...
ErrHand:
    ...
    Exit Function
End Function
Private Function Get获取历次病人结帐信息(ByVal lng病人ID As Long, lng主页ID As Long) As Boolean
    '获取历次病人结算时的信息
    Dim rsTemp As New ADODB.Recordset
    Dim strArr
    Get获取历次病人结帐信息 = False
    ...
    gstrSQL = "" & _
            ...
            "   Where A.险类=" & gintInsure & " And A.病人ID=" & lng病人ID & _
            "         and A.病人id=B.病人id and A.病人id=C.病人id and C.性质=2 and C.主页ID=" & lng主页ID & _
            ""
    Call OpenRecordset(rsTemp, "读取医保病人的相关信息")
    If rsTemp.EOF Then
...
ErrHand:
        If ErrCenter = 1 Then Resume
End Function
Private Function 历次费用结算预处理(rsExse As Recordset, ByVal lng病人ID As Long, ByVal lng主页ID As Long) As String
    ...
    '获取历次病人信息
    If Get获取历次病人结帐信息(lng病人ID, lng主页ID) = False Then
        Screen.MousePointer = intMouse
        Call WriteDebugInfor_大连("历次费用结算预处理", lng病人ID)
        历次费用结算预处理 = ""
        Exit Function
    End If
    ...
    gstrSQL = " Select B.住院次数 主页ID,to_char(A.入院日期,'yyyy') 入院年份,A.出院日期,B.住院号" & _
              ...
              " Where B.病人ID=" & lng病人ID & " And A.主页ID=" & lng主页ID & "　 And A.病人ID=B.病人ID"
    ...
    '重新获取记录
    Set rs费用 = Get住院虚拟记录(lng病人ID, lng主页ID)
    ...
    With rs费用
        ...
        Do While Not .EOF
            ...
            If 计算各项费用额(lng病人ID, Nvl(!参数值), Nvl(!算法, 0), Nvl(!收费类别), Nvl(!床号, 0), Nvl(!医保项目编码), Format(!发生时间, "yyyy-mm-dd HH:MM:SS"), _
                Nvl(!保险项目否, 0), Nvl(!数量, 0), Nvl(!价格, 0), Nvl(!金额, 0), Nvl(!婴儿费, 0), Nvl(!住院比额, 0), Val(Nvl(!医保项目名称, 0)), Nvl(!特准定额, 0), dblmoney) = False Then
                Exit Function
            End If
        ...
        '诊察费;草药费;成药费;西药费;检查费;治疗费;大检费;大检自费;血费;血费自费;特殊治疗费;特殊治疗自费;保险内自费费用;非保险费用;其他费
        If 大连医保结算(1, lng病人ID, lng主页ID, 0, 0, False, True, g病人身份_大连.起付线, _
            dblmoney(0), dblmoney(1), dblmoney(2), dblmoney(3), dblmoney(4), dblmoney(5), dblmoney(8), dblmoney(9), dblmoney(6), dblmoney(7), dblmoney(10), dblmoney(11), dblmoney(12), dblmoney(13), dblmoney(14), curTotal, str医生, str结算方式, str住院号, str出院日期) = False Then
            Exit Function
        End If
...
'        str结算方式 = 历次费用结算预处理(rsExse, lng病人ID, lng主页ID)
'        住院虚拟结算_大连 = str结算方式
'        Exit Function
'    End If
    ...
    '--判断是否能够作为门诊记帐进行结算
    gstrSQL = "select 当前状态 from 保险帐户 where 病人id=" & lng病人ID
    Call OpenRecordset(rs帐户状态, "读取保险帐户状态")
    ...
    If Nvl(rs帐户状态!当前状态, 0) = 0 And g病人身份_大连.历次结算 = False Then
        ...
        If MsgBox("该病人待结费用中只有门诊记帐费用,是否按照门诊进行结算?", vbQuestion + vbOKCancel + vbDefaultButton2) = vbOK Then
            ...
            If 门诊记帐虚拟结算_大连(rsExse, str门诊记帐结算方式) Then
...
'-------------------------------------------------------------------------------------------------------------------
        ...
        If 大连医保结算(1, lng病人ID, lng主页ID, 0, 0, False, True, dbl起付标准, dbl诊察费, dbl草药费, dbl成药费, dbl西药费, dbl检查费, dbl治疗费, dbl血费, dbl血费自费, dbl大检费, dbl大检自费, dbl特殊治疗费, dbl特殊治疗自费, dbl保险内自费费用, dbl非保险费用, dbl其它费, curTotal, str医生, str结算方式, str住院号, str出院日期) = False Then
            Exit Function
        End If
        g病人身份_大连.支付金额 = curTotal
    End With
...
Public Function Get慢病帐户余额_大连() As Boolean
    ...
    End With
    'Call WriteDebugInfor_大连("Get慢病帐户余额_大连", lng病人id)
...
Private Function 住院结算及冲帐_大连(ByVal bln冲销 As Boolean, ByVal lng病人ID As Long, ByVal lng结帐ID As Long, ByVal 原结帐id As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = " " & _
        ...
        "               F.参数值,D.编码 as 项目编码,D.名称 as 项目名称,decode(J.标识码,null,D.标识主码||D.标识子码,nvl(J.标识码,' ')) as 国家编码, " & _
    ...
    With rs明细
        ...
        Do While Not .EOF
            ...
            If gbln住院明细时实上传 And bln冲销 = False And Nvl(!是否上传, 0) = 0 And Nvl(!结帐金额, 0) <> 0 And Nvl(!实收金额, 0) <> 0 Then
                 ...
                 strInsertSQL = strInsertSQL & "'" & gstr医院编码_大连 & "',"
                 strInsertSQL = strInsertSQL & "'" & Nvl(!医保号) & "',"
                 strInsertSQL = strInsertSQL & "'" & Nvl(!住院号) & "',"
                 strInsertSQL = strInsertSQL & "" & Nvl(!顺序号, 0) & ","
                ...
                'Modified By 朱玉宝 2004-07-29 原因：处理NO号
                 ...
                 Else
                     If !是否医保 = 1 Then
                         strInsertSQL = strInsertSQL & "" & 1 - Nvl(!统筹比额) & "," '自费比例
                     Else
                         strInsertSQL = strInsertSQL & "" & 1 & ","  '自费比例
                     End If
                 End If
    ...
    If 大连医保结算(1, lng病人ID, lng主页ID, lng结帐ID, 原结帐id, bln冲销, False, dbl起付标准, dbl诊察费, dbl草药费, dbl成药费, dbl西药费, dbl检查费, dbl治疗费, dbl血费, dbl血费自费, dbl大检费, dbl大检自费, dbl特殊治疗费, dbl特殊治疗自费, dbl保险内自费费用, dbl非保险费用, dbl其它费, curTotal, str医生, strInfor, str住院号, str出院日期) = False Then
        Exit Function
    End If
    '如果是历次结算,就不管入出了
    If g病人身份_大连.历次结算 Then
    Else
        If bln冲销 Then
            ...
            Call WriteDebugInfor_大连("住院结算及冲销（冲销部分）_大连", lng病人ID)
        Else
            gstrSQL = "zl_保险帐户_出院(" & lng病人ID & "," & gintInsure & ")"
            zlDatabase.ExecuteProcedure gstrSQL, "医保帐户出院"
            Call WriteDebugInfor_大连("住院结算及冲销（结算部分）_大连", lng病人ID)
        End If
    End If
    住院结算及冲帐_大连 = True
Exit Function
...
Public Function 住院结算_大连(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    Call OpenRecordset(rsTemp, "检查病人是否门诊记帐结算")
    ...
    If int主页id = 0 Then
        If 门诊记帐结算及冲帐_大连(False, lng病人ID, lng结帐ID, lng结帐ID, 0) = True Then
            '门诊结算成功则向HIS返回成功标志
            Call WriteDebugInfor_大连("门诊记帐结算及冲帐_大连", lng病人ID)
            住院结算_大连 = True
            Exit Function
        Else
            住院结算_大连 = False
...
Public Function 住院结算冲销_大连(lng结帐ID As Long) As Boolean
    ...
    If lng主页ID = 0 Then
        If 门诊记帐结算及冲帐_大连(True, lng病人ID, lng冲销ID, lng结帐ID, 0) Then
            住院结算冲销_大连 = True
            Call WriteDebugInfor_大连("住院结算冲销_大连(门诊记帐结算及冲帐_大连)", lng病人ID)
            Exit Function
        Else
            住院结算冲销_大连 = False
            Call WriteDebugInfor_大连("住院结算冲销_大连(门诊记帐结算及冲帐_大连)", lng病人ID)
    ...
    '重新读卡
    If 读取病人身份_大连(IIf(gintInsure = type_大连开发区, 2, 1)) = False Then
        Call WriteDebugInfor_大连("住院结算冲销_大连(读取病人身份_大连)", lng病人ID)
        Exit Function
    End If
    ...
    If lng中心 = 2 Then
        '开发区为:住院 2、家床 4取消住院登记 C
        '---周顺利,开发区撤消出院登记跟大连市一样,是重新办理一次入院登记,就诊分类=2
        strInfor = strInfor & "2"                  '就诊分类    CHAR    51  1   2住院、4家床、O生育、   Y   院端
    Else
        strInfor = strInfor & Lpad(str就诊分类, 1)                  '就诊分类    CHAR    51  1   2住院、4家床、O生育、   Y   院端
    End If
    gstrSQL = "Select * From 床位状况记录 D where 病区ID=" & Nvl(rsTemp!当前病区ID, 0) & " And 床号=" & Nvl(rsTemp!当前床号, 0)
    ...
    '调用医保接口操作
    '1004    9   206 实时住院登记数据提交
    住院结算冲销_大连 = 业务请求_大连(lng中心, 1004, strInfor)
    If 住院结算冲销_大连 = False Then
        ShowMsgbox "实时住院登记撤消数据提交失败!"
        Exit Function
    End If
    Call WriteDebugInfor_大连("住院结算冲销_大连", lng病人ID)
...
Private Function 上传处方_大连(ByVal rsExse As ADODB.Recordset) As Boolean
    ...
    With rsExse
        Do While Not .EOF
            ...
            'Modified By 朱玉宝 2004-07-29 原因：处理NO号
             strInsertSQL = strInsertSQL & "" & Mid(Nvl(!NO, "00000000"), 2, 7) & ","
'             If IsNumeric(Mid(Nvl(!NO, 0), 1, 1)) Then
'                 strInsertSQL = strInsertSQL & "" & Nvl(!NO, 0) & ","
'             Else
'                 strInsertSQL = strInsertSQL & "" & asc(Mid(!NO, 1, 1)) - asc("A") + 1 & Mid(!NO, 2) & ","   '处方号  NUM 27  10      院端
'             End If
             
             strInsertSQL = strInsertSQL & "" & Lpad(CStr(.AbsolutePosition), 10) & ","
             strInsertSQL = strInsertSQL & "" & "NULL" & ","
             
             strInsertSQL = strInsertSQL & "'" & Get就诊分类(0, Nvl(!灰度级, 0)) & "',"
             If Nvl(!险类, 0) = type_大连开发区 Then '开发区
                 strInsertSQL = strInsertSQL & "to_date('" & Format(!就诊时间, "yyyy-MM-dd HH:mm:ss") & "','yyyy-mm-dd HH24:mi:ss'),"
             Else
                 strInsertSQL = strInsertSQL & "to_date('" & Format(!登记时间, "yyyy-MM-dd HH:mm:ss") & "','yyyy-mm-dd HH24:mi:ss'),"
             ...
             strInsertSQL = strInsertSQL & "'" & str诊断编码 & "',"
             strInsertSQL = strInsertSQL & "'" & str诊断名称 & "',"
...
ErrHand:
    ...
    Exit Function
End Function
Private Function Get住院虚拟记录(ByVal lng病人ID As Long, Optional lng主页ID As Long = 0) As ADODB.Recordset
    '-----------------------------------------------------------------------------------------------------------
    '--功  能:获取本次虚拟未结记录
    '--入参数:
    '--出参数:
    ...
    If lng主页ID <> 0 Then
        ...
        strSQL = _
            ...
            "           From 病人费用记录 A,收入项目 B,保险帐户 C" & _
            "           Where a.记录状态<>0 and A.主页id=" & lng主页ID & " and  A.记帐费用=1 and A.病人id=C.病人id And A.收入项目ID=B.ID And nvl(A.婴儿费,0)=0 And A.病人ID=" & lng病人ID & " and C.险类=" & gintInsure & _
    ...
    Else
        strSQL = _
            ...
            "                   Sum(A.标准单价) as 标准单价,Sum(Nvl(A.实收金额,0))-Sum(Nvl(A.结帐金额,0)) as 金额,A.发生时间,A.登记时间,Nvl(A.是否急诊,0) as 是否急诊,Nvl(A.保险项目否,0) as 保险项目否,A.摘要" & _
            "           From 病人费用记录 A,收入项目 B,保险帐户 C" & _
            "           Where a.记录状态<>0 and  A.记帐费用=1 and A.病人id=C.病人id And A.收入项目ID=B.ID And nvl(A.婴儿费,0)=0 And A.病人ID=" & lng病人ID & " and C.险类=" & gintInsure & _
            "           Group by    C.险类,Mod(A.记录性质,10),A.NO,Nvl(A.价格父号,序号),A.病人ID,A.主页ID,A.床号,Nvl(A.婴儿费,0),A.开单人," & _
            "                       A.开单部门ID,A.收费类别,A.收费细目ID,Nvl(A.保险大类ID,0),A.发生时间,A.登记时间,Nvl(A.是否急诊,0),Nvl(A.保险项目否,0),A.摘要" & _
            "           Having Sum(Nvl(A.实收金额,0))-Sum(Nvl(A.结帐金额,0))<>0) A,收费细目 B,部门表 X," & _
            "           (Select * From 保险支付项目 Where 险类=" & gintInsure & ") C," & _
...
Public Function 在院病人信息_大连(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    If 在院病人信息_大连 = False Then
        ...
        Exit Function
    End If
    Call WriteDebugInfor_大连("在院病人信息_大连", lng病人ID)
    在院病人信息_大连 = True
    Exit Function
ErrHand:
    If ErrCenter = 1 Then Resume
