...
'如果门诊费用明细传递失败，就直接结束函数，返回函数失败
    ...
    If Val(Get保险参数_壁山("适用地区")) = 1 Then
        ...
        If Not frm等待响应_黔江.ShowME(strInipath) Then
            WriteInfo "读取文件操作中断"
            MsgBox "等待文件返回操作被中断,请检查中心是否已结算,并对数据进行核对", vbInformation, gstrSysName
            Exit Function
        End If
        
        If GetIniS("Sign", "Sign", "0", strInipath) = "0" Then
            WriteInfo "返回错误:" & GetIniS("Sign", "Error_txt", "", strInipath)
            MsgBox GetIniS("Sign", "Error_txt", "", strInipath), vbInformation, "医保返回"
            Exit Function
        End If
...
'lng病人ID  默认为0，表示传输整张单据，否则为单据中指定病人的。（主要是因为医嘱在保存记帐单时，是分病人在提交数据而不是一起提交）
    ...
    If lng类别 = 1 Then
        ...
        gstrSQL = "select A.ID,A.发生时间,A.序号,A.NO,A.开单人,A.登记时间," & _
                ...
                " where A.记录状态<>0 And Nvl(A.是否上传,0)=0 And A.收费细目ID = B.ID and A.收入项目ID = C.ID and A.结帐ID =" & _
    ...
    Else
        gstrSQL = "select A.ID,A.发生时间,A.序号,A.NO,A.开单人,A.登记时间," & _
                ...
                "保险支付项目 D where A.记录状态<>0 And Nvl(A.是否上传,0)=0 And A.收费细目ID = B.ID and A.收入项目ID = C.ID " & _
    ...
    Do While Not rsTmp.EOF
        ...
        If Val(Get保险参数_壁山("适用地区")) = 3 And lng类别 = 2 Then
            strSql = "Select Flag From Check_item_list_interface Where Flag='1' And HIS_PK=" & rsTmp!ID
            If rs壁山.State = adStateOpen Then rs壁山.Close
            rs壁山.Open strSql, gcn壁山, adOpenStatic, adLockReadOnly
            If rs壁山.EOF Then
                '表中无此记录，可继续上传
            Else         '此ID的记录已经成功上传,跳转
                GoTo nextRec
            End If
        End If
...
'返回：交易成功返回true；否则，返回false
    ...
    If Val(Get保险参数_壁山("适用地区")) = 1 Then
        strSql = "Insert into Check_doex_interface(Bill_no,App_code,Doct_flag," & _
                ...
                "','" & Mid(gstr医院编码, 1, 4) & "','1','" & Nvl(rsTmp("住院号")) & "_" & lng主页ID & "','1','" & _
...
'    If cur个人帐户支付 <> 0 Then
        住院虚拟结算_壁山 = "个人帐户;" & cur个人帐户支付 & ";" & IIf(Val(Get保险参数_壁山("适用地区")) = 3, 1, 0) '不允许修改个人帐户
'    End If
'    If 住院虚拟结算_壁山 = "" Then 住院虚拟结算_壁山 = "个人帐户;" & 0 & ";1"
    ...
    End If
    If cur大额支付 <> 0 Then
        住院虚拟结算_壁山 = 住院虚拟结算_壁山 & IIf(住院虚拟结算_壁山 = "", "", "|") & "大额统筹;" & cur大额支付 & ";0" '不允许修改大额支付
    End If
    '清除本次数据上传请求
    strSql = "Delete check_item_request where Bill_no = '" & strBillNO & _
             "' and App_code = '" & Mid(gstr医院编码, 1, 4) & "'"
...
'将病人的费用进行结算，由于壁山医保不需要进行出院登记，因此不进行出院登记
    ...
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & gintInsure & "," & lng病人id & "," & _
        ...
        cur统筹支付 + cur统筹自付 & "," & cur统筹支付 & "," & cur大病自付 & "," & cur超限自付 & "," & cur个人帐户支付 & ",NULL,NULL,NULL,'" & strBillNO & "')"
    Call ExecuteProcedure("壁山医保")
...
Public Function 住院结算冲销_壁山(lng结帐ID As Long) As Boolean
    ...
    If rsTemp.EOF Then
        MsgBox "原单据的费用记录不存在，不能作废。", vbInformation, gstrSysName
        Exit Function
    End If
