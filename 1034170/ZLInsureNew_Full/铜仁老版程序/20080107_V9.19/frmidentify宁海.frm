...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select 病人ID,nvl(当前状态,0) as 状态,顺序号,灰度级 from 保险帐户 where 险类=" & TYPE_宁海 & " and 医保号='" & txt医疗证号.Text & "'"
...
'    IC_Data_宁海.mstr业务类型 = Me.cbo业务类型.ListIndex
'    gstrSQL = "zl_保险帐户_更新信息(" & mlng病人ID & "," & TYPE_宁海 & ",'业务类型','''" & cbo业务类型.ItemData(cbo业务类型.ListIndex) & "''')"
'    Call ExecuteProcedure("保存业务类型")
    
    '保存用户IC卡内所有数据
    gstrSQL = "zl_保险帐户_更新信息(" & mlng病人ID & "," & TYPE_宁海 & ",'IC','''" & IC_Data_宁海.IC卡数据 & "''')"
    Call ExecuteProcedure("保存用户IC卡内所有数据")
...
Private Sub cmdRead_Click()
    '读卡正确完成后，自动跳到确定处
    If Not ReadIC_宁海 Then Exit Sub
...
Private Sub Form_Load()
    Me.cbo业务类型.Clear
    Me.cbo业务类型.AddItem "普通"
    If mbytType = 0 Then
        Me.cbo业务类型.AddItem "急诊"
    ElseIf mbytType = 1 Then
        Me.cbo业务类型.AddItem "转入院"
    End If
