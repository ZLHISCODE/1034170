   ...
   Begin MSComctlLib.ImageList img16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm保险项目选择毕节.frx":0E42
            Key             =   "Detail"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm保险项目选择毕节.frx":1C94
            Key             =   "Class"
         EndProperty
      EndProperty
   End
...
Private Sub cmdOK_Click()
    ...
    '返回选择项目编码
    Call GetValueByCol("项目编码", mstrCode)
...
'返回：成功返回True
    ...
    '显示药品类别
    gstrSQL = "Select 编码,名称,DECODE(名称,'药品','Y','Z') AS 类别 From 保险支付大类 Where 险类=" & TYPE_毕节
    ...
    frm保险项目选择毕节.Show vbModal, frm保险项目
    '返回值
    If mblnOK = True Then
        strCode = mstrCode
        strName = mstrName
...
'功能：显示当前类别下的医保明细
    ...
    If Left(tvwClass.SelectedItem.Key, 1) = "Y" Then
        '当前选择是的一个药品类别
        gstrSQL = "select ID,药品代码 AS 项目编码,中文名称 AS 项目名称,英文名称,商品名称,药品大类,所属类型,个人自付比例||'%' AS 个人自付比例,个人起付金额" & _
                  "  FROM 药品目录表 "
    Else
        '当前选择是的一个诊疗类别
        gstrSQL = "Select ID,诊疗项目代码 AS 项目编码,诊疗项目名称 AS 项目名称,费用类别,一级医院定额,二级医院定额,三级医院定额,个人自付比例||'%' AS 个人自付比例,个人起付金额" & _
                 "   FROM 诊疗项目表 "
    End If
    ...
    Do Until rsTemp.EOF
        Set lst = lvwDetail.ListItems.Add(, "K" & rsTemp("ID"), rsTemp("ID"), "Detail", "Detail")
