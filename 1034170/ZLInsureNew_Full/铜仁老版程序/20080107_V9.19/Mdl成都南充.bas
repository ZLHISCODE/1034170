...
Public Function 处方登记_成都南充_指定费用(ByVal lngPatient As Long) As Boolean
    ...
    With rsTemp
        ...
        gstrSQL = " Select A.ID 处方明细流水号,A.标识号 as 住院号,A.病人ID,A.记录性质,A.记录状态,A.NO,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 处方时间," & _
                  ...
                  " From 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & TYPE_成都南充 & ") C,收费类别 D,收费细目 E " & _
                  " Where A.执行部门ID+0=B.ID And A.收费细目ID+0=C.收费细目ID(+) And A.收费类别=D.编码 And A.收费细目ID=E.ID And Nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0" & _
                  " And A.病人ID=" & lngPatient & _
                  " Order by A.标识号"
...
Public Function 处方登记_成都南充(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    ...
    With rsTemp
        ...
        gstrSQL = " Select A.ID 处方明细流水号,A.标识号 as 住院号,A.病人ID,A.记录性质,A.记录状态,A.NO,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 处方时间," & _
                  ...
                  " And A.执行部门ID+0=B.ID And A.收费细目ID+0=C.收费细目ID(+) And A.收费类别=D.编码 And A.收费细目ID=E.ID And Nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0" & _
                  " Order by A.标识号"
        Call OpenRecordset(rsTemp, "处方登记")
...
Public Function 处方删除_成都南充(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    ...
    With rsTemp
        ...
        gstrSQL = " Select A.ID 处方明细流水号,A.标识号 as 住院号,A.病人ID,A.记录性质,A.记录状态,A.NO,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 处方时间," & _
                  ...
                  " And A.执行部门ID+0=B.ID And A.收费细目ID+0=C.收费细目ID(+) And A.收费类别=D.编码 And A.收费细目ID=E.ID And Nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0" & _
                  " Order by A.标识号"
        Call OpenRecordset(rsTemp, "处方删除")
        If .RecordCount = 0 Then
...
Public Function 入院登记_成都南充(ByVal lngPatient As Long) As Boolean
    ...
    '入院登记
    gstrSQL = "zl_保险帐户_入院(" & lngPatient & "," & gintInsure & ")"
    Call ExecuteProcedure("南充医保")
...
Public Function 出院登记_成都南充(ByVal lngPatient As Long) As Boolean
    ...
    gstrSQL = "zl_保险帐户_出院(" & lngPatient & "," & gintInsure & ")"
    Call ExecuteProcedure("南充医保")
...
Public Function Get住院号(ByVal lngPatient As Long) As String
    ...
    With rsTemp
        ...
        gstrSQL = "Select 住院号||'_'||住院次数 住院号 From 病人信息 Where 病人ID=" & lngPatient
        Call OpenRecordset(rsTemp, "南充医保")
...
Public Function IsYBPatient(ByVal lngPatient As Long) As Boolean
    ...
    With rsYbPatient
        If .State = 1 Then .Close
        .CursorLocation = adUseClient
        gstrSQL = "Select Count(*) Records From 保险帐户 Where 险类=" & gintInsure & " And 病人ID=" & lngPatient
        Call OpenRecordset(rsYbPatient, "南充医保")
...
Public Function 住院结算_成都南充(ByVal lng结帐ID As Long, ByVal rstmp As ADODB.Recordset) As Boolean
    ...
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & gintInsure & "," & rstmp!病人ID & "," & _
