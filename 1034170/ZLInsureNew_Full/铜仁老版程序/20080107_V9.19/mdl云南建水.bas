...
Private Declare Sub yh_feedetailtrans Lib "Hisint" Alias "int_feedetailtrans" _
    ...
    ByVal Doctor_Name As String, ByRef Pay_Proportion As Double, ByRef Pay_amount As Double, ByRef Wipe_Amount As Double, ByRef error_code As String)
...
Private Declare Sub yh_admit Lib "Hisint" Alias "int_admit" _
    ...
    ByRef Personal_No As String, ByRef Name As String, ByRef Sex As String, ByRef birthdate As String, _
    ByRef Identify As String, ByRef initinstitution As String, ByRef error_code As String)
...
Private Declare Sub yh_outpatientidentify Lib "Hisint" Alias "int_outpatientidentify" _
    ...
    ByRef Identify As String, ByRef initinstitution As String, ByRef accountremain As Double, ByRef error_code As String)
...
ByVal Section_Name As String, ByVal Ifspecialsick As String, ByVal admit_time As String, ByVal admit_diagnose As String, _
...
Private Declare Sub yh2_recedefeedetail Lib "Hisint" Alias "int_recedefeedetail" _
    ...
    ByRef Pay_Proportion As Double, ByRef Pay_amount As Double, ByRef Wipe_Amount As Double, ByRef error_code As String)
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    Call OpenRecordset(rsTemp, "建水医保")
    mstrErr = Space(4)
    Call yh_Recedeadmit(mstr顺序号, LeftDB(rsTemp!入院科室, 24), IIf(Nvl(rsTemp!病种编码) = "", "0", "1"), _
        Format(rsTemp!入院日期, "yyyy-MM-dd"), LeftDB(获取入出院诊断(lng病人ID, lng主页ID, True, False), 50), mstrErr)
    If mstrErr <> "0000" Then
        MsgBox GetErrInfo(mstrErr), vbInformation, gstrSysName
        Exit Function
    End If
