...
Public Function GetCode(strCode As String, ByVal int中心 As Integer) As Boolean
'功能：获得一个收费项目的医保编码
...
'返回：成功返回True
    ...
    gstrSQL = "Select 编码 AS CODE,名称 AS NAME From 保险支付大类 where 险类=" & gintInsure & " order by 编码"
    Call OpenRecordset(rsTmp, "保险项目选择")
    
    gstrSQL = "Select 大类编码 as CLASSCODE,编码 AS CODE,名称 AS NAME ,简码,附注 " & _
                   " from 医保收费项目 where 险类=" & gintInsure & " and 中心=" & int中心 & " order by 大类编码,编码"
...
Private Sub cmdRequery_Click()
    ...
    If bln全量 Then
        gstrSQL = "zl_医保收费项目_Clear(" & gintInsure & "," & mint中心 & ")"
    ...
    Do Until rsTemp.EOF
        ...
        '插入保险项目
        gstrSQL = "zl_医保收费项目_Insert(" & gintInsure & "," & mint中心 & ",'" & rsTemp("CODE") & "','" & ToVarchar(rsTemp("NAME"), 40) & _
