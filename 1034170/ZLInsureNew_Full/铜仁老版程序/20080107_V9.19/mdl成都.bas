...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    For i = 1 To rsList.RecordCount
        ...
        If Left(Nvl(rsList!保险编码, rsList!医保项目编码), 3) = "002" Then
            '床位费部分
            If Mid(Nvl(rsList!保险编码, rsList!医保项目编码), 8, 1) = "1" Then '全公费床位费
                'cur床位超限自付 = cur床位超限自付 + 0
                cur床位限额报销 = cur床位限额报销 + Format(rsList!金额, "0.00")
            ElseIf Mid(Nvl(rsList!保险编码, rsList!医保项目编码), 8, 1) = "3" Then '全自费床位费
                'cur床位限额报销 = cur床位限额报销 + 0
                cur床位超限自付 = cur床位超限自付 + Format(rsList!金额, "0.00")
            ElseIf Mid(Nvl(rsList!保险编码, rsList!医保项目编码), 8, 1) = "2" Then '比例限额床位费
                Select Case Mid(Nvl(rsList!保险编码, rsList!医保项目编码), 11, 2)
                    Case "01": cur限额 = 5
                    Case "02": cur限额 = 6.5
                    Case "03": cur限额 = 7
                    Case "04": cur限额 = 7.5
                ...
                End If
            End If
        ElseIf Left(Nvl(rsList!保险编码, rsList!医保项目编码), 3) = "012" _
            And Mid(Nvl(rsList!保险编码, rsList!医保项目编码), 8, 1) = "0" Then
            '特殊医用材料费
            If cur价格 <= 100 Then '报销比例为%100
                sng比例 = 1
            ElseIf cur价格 > 100 And cur价格 <= 1000 Then '报销比例为%80
                sng比例 = 0.8
            ElseIf cur价格 > 1000 And cur价格 <= 5000 Then '报销比例为%70
                sng比例 = 0.7
            ElseIf cur价格 > 5000 And cur价格 <= 10000 Then '报销比例为%60
                sng比例 = 0.6
            ElseIf cur价格 > 10000 And cur价格 <= 50000 Then '报销比例为%50
                sng比例 = 0.5
            ElseIf cur价格 > 50000 Then '报销比例为%40
                sng比例 = 0.4
            End If
            cur比例自付 = cur比例自付 + Format(Format(rsList!金额, "0.00") * (1 - sng比例), "0.00")
            cur比例报销 = cur比例报销 + Format(Format(rsList!金额, "0.00") * sng比例, "0.00")
        Else
            '普通费用
            Select Case Mid(Nvl(rsList!保险编码, rsList!医保项目编码), 8, 1)
                Case "1" '公费
                    cur公费 = cur公费 + Format(rsList!金额, "0.00")
                Case "2" '比例自付20%
                    cur比例自付 = cur比例自付 + Format(Format(rsList!金额, "0.00") * 0.2, "0.00")
                    cur比例报销 = cur比例报销 + Format(Format(rsList!金额, "0.00") * 0.8, "0.00")
                Case "3" '全自费
    ...
    strSql = "insert into zyxnjs(" & _
        ...
        Format(cur总额, "0.00") & "," & Format(cur全自费 + cur床位超限自付, "0.00") & "," & Format(cur比例自付, "0.00") & "," & _
        Format(cur比例报销 + cur床位限额报销 + cur公费, "0.00") & ",0,0,0,0,0,'')"
    gcnSybase.Execute strSql
