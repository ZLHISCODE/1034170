...
'            strAddition = strAddition & ";" & IIf(Left(TrimStr(.ClassCode), 1) = "0", 1, 0)    '14在职
            ...
            If bytType = 1 Then
                gstrSQL = "zl_病种信息_INSERT(" & gintInsure & "," & lng病人id & ",'" & str病种 & "')"
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    With ic门诊读卡
        ...
        gstrSQL = "zl_保险结算记录_insert(1," & lng序号 & "," & gintInsure & "," & lng病人id & "," & _
            Format(dat当前日期, "yyyy") & "," & .InPerAcc & "," & .OutPerAcc - cur个人帐户 & "," & .OutAnnOverLine & "," & _
            .OutAnnPlan & "," & .AnnInpatientTimes & ",0,0,0," & _
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    With gIC中软
        gstrSQL = "zl_帐户年度信息_insert(" & rsExse("病人ID") & "," & gintInsure & "," & .MediYear & "," & _
...
'            '更新费用明细
'            gstrSQL = ""
'            Call ExecuteProcedure("更新医保大类ID")
    ...
    '2.2、计算进入统筹金额
    gstrSQL = "select ID,编码,算法,统筹比额,特准定额,特准天数,是否医保 FROM 保险支付大类  where 险类=" & gintInsure
...
Private Function 检查医保服务器_中软() As Boolean
    ...
    '读出连接医保服务器的配置
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & gintInsure
    Call OpenRecordset(rsTemp, "中软医保")
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select 在职,医保号 from 保险帐户 where 险类=" & gintInsure & " and 病人ID=" & lng病人id
    Call OpenRecordset(rsTemp, "中软医保")
