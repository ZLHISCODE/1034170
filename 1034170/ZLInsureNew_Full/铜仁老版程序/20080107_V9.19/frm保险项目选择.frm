...
'返回：成功返回True
    ...
    int险类 = TYPE_福建省 Or int险类 = TYPE_福州市 Or int险类 = TYPE_南平市 Or int险类 = TYPE_成都内江 Then
        '明细可以更新
...
Private Function 医保项目_成都内江(ByVal rsTemp As ADODB.Recordset) As Boolean
    ...
    Const COL_简码  As Long = 3
    Const COL_大类  As Long = 4
...
'   10   code_py        拼音码      20
    ...
    If 调用接口_记录数_沈阳市 Then
        Do While True
            ...
            Call AddRecord(rsTemp, str编码, ToVarchar(str名称, 40), ToVarchar(str简码, 10), int类型 & "||" & str规格 & "||" & str厂家 & "||" & str剂型 & "||" & str单价 & "^^" & strTmp, ToVarchar(str大类, 6))
...
Private Function 下载模式(ByVal lng险类 As Long) As Boolean
    ...
    gstrSQL = "Select 1 From 保险项目 Where 险类=" & lng险类 & " And Rownum<2"
    Call OpenRecordset(rsTemp, "检查是否已存在医保项目")
