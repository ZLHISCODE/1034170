...
Public Function 医保初始化_沈阳市() As Boolean
    ...
    If glngInterface_沈阳市 = 0 Then
        ...
        '取适用地区
        mint适用地区_沈阳 = 0
        gstrSQL = "Select 参数值 From 保险参数 Where 参数名='适用地区' And 险类=" & TYPE_沈阳市
...
Optional ByVal bln挂号 As Boolean = False, Optional ByVal lng收入项目ID As Long = 0) As Boolean
    ...
    If str医生姓名 <> "" And mint适用地区_沈阳 = 2 Then
        ...
        gstrSQL = " Select 编码,名称 From 部门表 " & _
                  ...
                  "     Where 人员ID=" & _
                  "         (Select ID From 人员表 Where 姓名='" & str医生姓名 & "'))"
        Call OpenRecordset(rsTemp, "取科室编号与名称")
...
Optional ByVal bln挂号 As Boolean = False, Optional ByVal lng收入项目ID As Long = 0) As Boolean
    ...
    gstrSQL = "Select Rownum 标识号,A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,A.登记时间,A.开单人 as 医生," & _
            ...
            "   C.名称 开单部门,E.名称 受单部门" & _
            " From (Select * From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E " & _
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+) And A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类=" & gintInsure & _
            " Order by A.ID"
    Call OpenRecordset(rsTemp, "读取本次结帐费用明细")
    lng病人id = rsTemp!病人ID
...
'    1   serialno    业务序列号  12
    ...
    '获取病人入院日期
    gstrSQL = "Select 入院日期 From 病案主页 Where 病人ID=" & lng病人id & " And 主页ID=" & lng主页ID
    ...
    On Error Resume Next
    '更新并发症信息
    gstrSQL = "zl_保险帐户_更新信息(" & lng病人id & "," & gintInsure & ",'并发症','''" & arrPatient(入院诊断) & "''')"
    Call ExecuteProcedure("更新并发症信息")
