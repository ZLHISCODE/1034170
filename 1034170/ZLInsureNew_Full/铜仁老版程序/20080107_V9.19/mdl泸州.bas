...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    If frmIdentify泸州.GetPatient(bytType, False, lng病种ID, str病种ID) = True Then
        ...
        With gIC泸州
            ...
            If bytType = 1 Then
                '对有限制的病人进行提醒
                If m泸州.灰度 = deg个人支付 Or m泸州.灰度 = deg上传明细 Then
            ...
            '得到中心序号
            gstrSQL = "select 序号 from 保险中心目录 where 险类=" & gintInsure & " and 编码='" & .CenterCode & "'"
            ...
            '得到原住院病种
            If bytType <> 1 Then
                gstrSQL = "Select Nvl(病种ID,0) 病种ID From 保险帐户 Where 险类=" & gintInsure & " And 医保号='" & TrimStr(.MediAccountNo) & "'"
                Call OpenRecordset(rsTemp, "得到原住院病种")
...
'            strAddition = strAddition & ";" & IIf(Left(TrimStr(.ClassCode), 1) = "0", 1, 0)    '14在职
            ...
            lng病人ID = BuildPatiInfo(bytType, strIdentify & strAddition, lng病人ID)
            '返回格式:中间插入病人ID
            身份标识_泸州 = strIdentify & ";" & lng病人ID & strAddition
            
            '更新病种信息
            If bytType = 1 Then
                gstrSQL = "zlyb.zl_病种信息_INSERT(" & TYPE_泸州市 & "," & lng病人ID & ",'" & str病种ID & "')"
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    gstrSQL = "Select B.编码 From 保险帐户 A,保险病种 B where A.险类=" & TYPE_泸州市 & " And A.病人ID=" & lng病人ID & " And A.病种ID=B.ID(+)"
    Call OpenRecordset(rsTemp, "门诊虚拟结算")
    ...
    '报销规定
    m政策.个人帐户支付全自费 = cls医保.GetCapability(support收费帐户全自费)
    m政策.个人帐户支付首先自付 = cls医保.GetCapability(support收费帐户首先自付)
    
    gstrSQL = "SELECT B.医保年,A.起付线在段中,A.段值类型,A.封顶类型,A.使用累计报销,A.个人账户可支付首先自付 " & _
               " FROM 保险中心目录 A,保险主机 B " & _
               " WHERE A.险类=" & gintInsure & " AND A.编码='" & gIC泸州.CenterCode & "' AND A.主机编码=B.编码 AND A.险类=B.险类 "
    If rsTemp.State = adStateOpen Then rsTemp.Close
    rsTemp.Open gstrSQL, gcn泸州, adOpenStatic, adLockReadOnly
    If rsTemp.EOF = False Then
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.记录性质,A.记录状态,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              ...
              "  where A.结帐ID=" & lng结帐ID & " And Nvl(A.附加标志,0)<>9 And Nvl(A.记录状态,0)<>0" & _
              "        and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & gintInsure & _
              "  Order by A.病人ID,A.发生时间"
    Call OpenRecordset(rsTemp, "门诊结算")
    'Modified By 朱玉宝 2003-12-10 地区：泸州
    ...
    gstrSQL = "SELECT A.病人ID,A.NO,A.实际票号,A.记录性质,substr(B.姓名,1,8) as 姓名,substr(B.性别,1,2) as 性别,floor(MONTHS_BETWEEN(A.登记时间,B.出生日期)/12) AS 年龄" & _
              ...
              "  FROM 病人费用记录 A,病人信息 B,保险帐户 C,部门表 D" & _
              "  Where A.结帐ID =" & lng结帐ID & " And A.病人ID = B.病人ID And B.病人ID = C.病人ID And C.险类 =" & TYPE_泸州市 & " And A.开单部门ID = D.ID(+) and rownum<2"
    Call OpenRecordset(rs信息, "泸州医保")
    lng病人ID = rs信息("病人ID")
    ...
    With ic门诊
        ...
        gstrSQL = "zl_帐户年度信息_insert(" & lng病人ID & "," & gintInsure & "," & Format(dat当前日期, "yyyy") & "," & _
            .InPerAcc & "," & .OutPerAcc + cur个人帐户 & "," & .PlanPaidFee & "," & _
            .PlanPaidAmt & "," & .InHosTimes & ")"
        Call ExecuteProcedure("泸州医保")
    ...
    With ic门诊
        gstrSQL = "INSERT INTO 保险结算记录 " & _
                  ...
                  " Values "
         gstrSQL = gstrSQL & " (1," & lng结帐ID & "," & gintInsure & "," & .MediYear & ",'" & .CenterCode & "','" & rs信息("NO") & "1" & rs信息!记录性质 & "'," & lng病人ID & ",0,'" & rs信息("姓名") & _
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    If Val(ic门诊.MediYear) <> rs结算("年度") Then
        MsgBox "跨年不能作废。", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    With ic门诊
        ...
        gstrSQL = "zl_保险结算记录_insert(1," & lng序号 & "," & gintInsure & "," & lng病人ID & "," & _
            Format(dat当前日期, "yyyy") & "," & .InPerAcc & "," & .OutPerAcc & "," & .PlanPaidFee & "," & _
            .PlanPaidAmt & "," & .InHosTimes & ",0,0,0," & _
    ...
    With ic门诊
        gstrSQL = "INSERT INTO 保险结算记录 " & _
                  ...
                  ",卡灰度级,发票号,票据日期,冲票标志,被冲票据号,支付顺序号,是否上传) " & _
                  " Values "
         gstrSQL = gstrSQL & " (1," & lng序号 & "," & gintInsure & "," & .MediYear & ",'" & .CenterCode & "','" & Mid(rs结算("序号"), 1, Len(rs结算("序号")) - 2) & "2" & Right(rs结算!序号, 1) & "'," & lng病人ID & ",0,'" & rs结算("姓名") & _
                  ...
                  "," & m泸州.灰度 & ",'" & Nvl(rs结算("发票号")) & "',sysdate,-1,'" & Mid(rs结算("序号"), 1, Len(rs结算("序号")) - 1) & "1','" & .OutSerialNo + 1 & "',0)"
        '准备写卡
        .OutPerAcc = .OutPerAcc - cur个人帐户                  '个人帐户累计支出金额
        .ClinicPaidAmt = .ClinicPaidAmt - cur个人帐户           '门诊个人帐户支出金额
...
'返回：交易成功返回true；否则，返回false
    ...
    gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & gintInsure & ",'医保年','''" & Get当前医保年 & "''')"
    Call ExecuteProcedure("泸州医保")
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    With m泸州
        ...
        '避免在出院结帐后再次进行结帐
        gstrSQL = "SELECT 病人ID FROM 保险结算记录 WHERE 中途结帐=0 AND 病人ID=" & .病人ID & " AND 主页ID=" & .主页ID & " AND 险类=" & TYPE_泸州市
        Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.记录状态,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              "         ,A.数次*nvl(A.付数,1) as 数量,Decode(A.数次*nvl(A.付数,1),0,0,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4)) as 单价 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C " & _
              "  where A.病人ID=" & m泸州.病人ID & " and A.主页ID=" & m泸州.主页ID & " and A.记帐费用=1 And A.操作员姓名 is not null AND A.实收金额 IS NOT NULL " & _
              "        and nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0 and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & gintInsure & _
              "  Order by A.病人ID,A.发生时间"
    Call OpenRecordset(rsTemp, "虚拟结算")
    If rsTemp.EOF = False Then
        '还存在没有分割费用的明细
    ...
    gstrSQL = "select A.病种ID,A.医保年,B.编码,B.序号 " & _
            ...
            " where A.病人ID=" & m泸州.病人ID & " and A.险类=" & gintInsure & _
    ...
    gstrSQL = "Select B.编码 From 保险帐户 A,保险病种 B where A.险类=" & TYPE_泸州市 & " And A.病人ID=" & m泸州.病人ID & " And A.病种ID=B.ID(+)"
    Call OpenRecordset(rsTemp, "住院虚拟结算")
    If rsTemp.EOF = False Then
        gstrSQL = "Select B.编码,名称,类别 From 保险病种 B where B.险类=" & TYPE_泸州市 & " And B.编码='" & rsTemp("编码") & "'"
        If rsTemp.State = adStateOpen Then rsTemp.Close
        rsTemp.Open gstrSQL, gcn泸州, adOpenStatic, adLockReadOnly
    ...
    gstrSQL = "select 入院日期,nvl(出院日期,to_date('3000-01-01','yyyy-MM-dd')) as 出院日期,sysdate 当前日期 " & _
              "from 病案主页 where 病人ID=" & m泸州.病人ID & " and 主页ID=" & m泸州.主页ID
    Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    With gIC泸州
        gstrSQL = "zl_帐户年度信息_insert(" & m泸州.病人ID & "," & gintInsure & "," & .MediYear & "," & _
...
'返回：交易成功返回true；否则，返回false
    ...
    gstrSQL = "SELECT A.病人ID,A.NO,A.实际票号,substr(B.姓名,1,8) as 姓名,substr(B.性别,1,2) as 性别,floor(MONTHS_BETWEEN(A.收费时间,B.出生日期)/12) AS 年龄" & _
              ...
              "  Where A.ID =" & lng结帐ID & " And A.病人ID = B.病人ID And B.病人ID = C.病人ID And C.险类 =" & TYPE_泸州市 & _
             "         And B.病人ID=D.病人ID And D.主页ID=" & m泸州.主页ID
    Call OpenRecordset(rs信息, "泸州医保")
    '只要是中途结算，就要多加1天
...
'    End If
    ...
    With m泸州
        ...
        For Each var结算计算 In gcol结算计算
            ...
            lng段数 = lng段数 + 1
            If lng段数 <= 5 Then
                str分段 = str分段 & "," & var结算计算(2) & "," & IIf(m泸州.灰度 < deg医保支付, 0, (var结算计算(1) - var结算计算(2)))
            End If
        Next
        '补足五段
    ...
    With ic泸州
        gstrSQL = "INSERT INTO 保险结算记录 " & _
                ...
                ",卡灰度级,发票号,票据日期,冲票标志,被冲票据号,支付顺序号,中途结帐,是否上传) " & _
                  " Values "
         gstrSQL = gstrSQL & " (2," & lng结帐ID & "," & gintInsure & "," & .MediYear & ",'" & .CenterCode & "','" & rs信息("NO") & "1'," & m泸州.病人ID & "," & m泸州.主页ID & ",'" & rs信息("姓名") & _
                  "','" & rs信息("性别") & "'," & rs信息("年龄") & ",'" & rs信息("医保号") & "','" & rs信息("卡号") & "','" & rs信息("身份证号") & "','" & .ClassCode & "','" & .UnitCode & "' " & _
                  "," & .IsOffical & "," & .IsAttend & "," & m泸州.参加补充保险 & "," & .InPerAcc & "," & .OutPerAcc & "," & .PlanPaidAmt & "," & .PlanPaidFee & _
...
'----------------------------------------------------------------
    ...
    If m泸州.灰度 = deg停止支付 Then
        '不用再处理后续过程
        住院结算冲销_泸州 = False
        MsgBox "该病人已经停止医保支付，不能进行冲销操作。", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    Call OpenRecordset(rs结算计算, "泸州医保")
    ...
    With ic住院
        gstrSQL = "INSERT INTO 保险结算记录 " & _
                ...
                ",卡灰度级,发票号,票据日期,冲票标志,被冲票据号,支付顺序号,中途结帐,是否上传) " & _
                  " Values "
         gstrSQL = gstrSQL & " (2," & lng冲销ID & "," & gintInsure & "," & .MediYear & ",'" & .CenterCode & "','" & Mid(rs结算("序号"), 1, Len(rs结算("序号")) - 2) & "2'," & rs结算("病人ID") & "," & rs结算("主页ID") & ",'" & rs结算("姓名") & _
                  "','" & rs结算("性别") & "'," & rs结算("年龄") & ",'" & rs结算("医保号") & "','" & rs结算("卡号") & "','" & rs结算("身份证号") & "','" & .ClassCode & "','" & .UnitCode & "' " & _
                  "," & .IsOffical & "," & .IsAttend & "," & rs结算("参加补充保险") & "," & .InPerAcc & "," & .OutPerAcc & "," & .PlanPaidAmt & "," & .PlanPaidFee & _
                  "," & .ChronicPaidAmt & "," & .ChronicPaidFee & "," & .ChronicSillPaidAmt & ",null" & _
                  "," & .ClinicPaidAmt & "," & .InHosPaidAmt & "," & .QuotaPaidAmt & ",'" & rs结算("部门名称") & "','" & rs结算("医生名称") & "','" & rs结算("病种代码") & "','" & rs结算("病种名称") & "','" & rs结算("病种类型") & "' " & _
                  ...
                  "," & rs结算("超基本封顶线") & "," & rs结算("超补充封顶线") & "," & rs结算("进入额度支付") & "," & rs结算("进入门诊个人帐户支付") & "," & rs结算("进入住院个人帐户支付") & "," & rs结算("进入慢性病起付金") & " " & _
                  "," & m泸州.灰度 & ",'" & Nvl(rs结算("发票号"), " ") & "',sysdate,-1,'" & Mid(rs结算("序号"), 1, Len(rs结算("序号")) - 1) & "1','" & .OutSerialNo + 1 & "'," & rs结算("中途结帐") & ",0)"
        '准备写卡
        .OutPerAcc = .OutPerAcc - cur个人帐户                  '个人帐户累计支出金额
...
'功能：首先断断是否要装钱，然后完成相应操作
    ...
    gstrSQL = "SELECT B.医保年,B.装钱序号,B.装钱模式 " & _
               " FROM 保险中心目录 A,保险主机 B " & _
               " WHERE A.险类=" & gintInsure & " AND A.编码='" & ic卡.CenterCode & "' AND A.主机编码=B.编码 AND A.险类=B.险类 "
    rsTemp.Open gstrSQL, gcn泸州, adOpenStatic, adLockReadOnly
    If rsTemp.EOF = False Then
...
'功能：判断该病人的卡是否插入正确
    ...
    gstrSQL = "select A.卡号,A.医保号,B.编码 from 保险帐户 A,保险中心目录 B " & _
              " where A.险类=" & gintInsure & " and A.病人ID=" & lng病人ID & " and a.险类=B.险类 and A.中心=B.序号"
    Call OpenRecordset(rsTemp, "泸州医保")
...
Public Function 检查医保服务器_泸州() As Boolean
    ...
    '读出连接医保服务器的配置
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_泸州市
    Call OpenRecordset(rsTemp, "泸州医保")
...
'      IC泸州        根据读出的信息填写IC卡结构
    ...
    With ic泸州
        ...
        .Password = "9999"       'As String * 4      ' 个人密码
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select A.在职 from 保险帐户 A where A.险类=" & gintInsure & " and A.病人ID=" & 病人ID
    Call OpenRecordset(rsTemp, "泸州医保")
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select A.医保号,A.密码,B.身份证号 from 保险帐户 A,病人信息 B where A.险类=" & _
        gintInsure & " and A.病人ID=" & 病人ID & " And A.病人ID=B.病人ID"
    Call OpenRecordset(rsTemp, "泸州医保")
...
'调用位置：门诊预算、门诊结算、住院记帐、住院预算、住院结算、费用明细上传
    ...
    Dim bln医保病人 As Boolean, bln医保项目 As Boolean, bln更新 As Boolean
    ...
    Do Until rs费用明细.EOF
        If lng病人ID <> rs费用明细("病人ID") Then
            ...
            bln医保病人 = False
            If Not bln门诊 Then
                gstrSQL = "Select Count(*) Records From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & rs费用明细!病人ID & " And A.主页ID=B.住院次数 And A.险类=" & TYPE_泸州市
                Call OpenRecordset(rsTemp, "判断是否医保病人")
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    m政策.个人帐户支付全自费 = cls医保.GetCapability(support结算帐户全自费)
    m政策.个人帐户支付首先自付 = cls医保.GetCapability(support结算帐户首先自付)
    m政策.个人帐户支付超限 = cls医保.GetCapability(support结算帐户超限)
    ...
    gstrSQL = _
        ...
        "   From 病人费用记录 A" & _
        "   Where A.记帐费用=1 And Nvl(A.记录状态,0)<>0 And A.病人ID=" & m泸州.病人ID & " and A.主页ID=" & m泸州.主页ID & " And A.操作员姓名 is not null" & _
    ...
    With m泸州
        Do Until rsTemp.EOF
            ...
            If rsTemp("保险项目否") = 0 Or Not bln医保项目 Then
                .全自费 = .全自费 + rsTemp("实收金额")
            Else
...
'                        End If
    ...
    With m泸州
        ...
        gstrSQL = "select 年龄段,nvl(全额统筹,0) as 全额统筹 ,nvl(无起付线,0) as 无起付线 ,nvl(无封顶线,0) as 无封顶线 " & _
                " from 保险年龄段" & _
                " where 险类=" & gintInsure & " and nvl(中心,0)=" & .中心序号 & _
    ...
    With m泸州
        gstrSQL = "select max(decode(A.性质,'A',A.金额,0)) as 封项线 ,max(decode(A.性质,'1',A.金额,0)) as 起付线 " & _
                  ...
                  "  from 保险支付限额 A " & _
                  "  where A.险类=" & gintInsure & " and A.中心=" & .中心序号 & " and A.年度=" & .年度 & " And A.在职=" & lng在职 & " And A.人员类别=" & IIf(gIC泸州.DomainCode = 0, 1, 2)
        If rsTemp.State = adStateOpen Then rsTemp.Close
        rsTemp.Open gstrSQL, gcn泸州, adOpenStatic, adLockReadOnly
    ...
    With m泸州
        ...
        gstrSQL = "select B.档次,B.下限,B.上限,A.比例 " & _
                  ...
                  "  Where A.险类 =" & gintInsure & " And A.中心 =" & m泸州.中心序号 & " And A.年度 =" & m泸州.年度 & " And A.在职 =" & lng在职 & " And A.年龄段 =" & lng年龄段 & _
...
'返回：成功计算，则返回True
    ...
    gstrSQL = "SELECT A.开展补充保险报销,A.补充报销比例,A.补充报销限额,A.补充报销限额类型 " & _
               " FROM 保险中心目录 A " & _
               " WHERE A.险类=" & gintInsure & " AND A.编码='" & gIC泸州.CenterCode & "'"
    rsTemp.Open gstrSQL, gcn泸州, adOpenStatic, adLockReadOnly
...
'功能：获得保险参数
    ...
    gstrSQL = "select A.参数名,A.参数值 from 保险参数 A " & _
              " where A.险类=" & TYPE_泸州市 & " and A.中心 is null "
    Call OpenRecordset(rsTemp, "泸州医保")
...
Public Function 挂号结算_泸州(ByVal lng结帐ID As Long) As Boolean
    ...
    '取病人ID
    gstrSQL = "Select 病人ID From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Rownum<2"
    Call OpenRecordset(rsTemp, "读取病人ID")
    lng病人ID = rsTemp!病人ID
    
    '取费用总额
    gstrSQL = "Select Sum(实收金额 ) as 金额 From 病人费用记录 Where 结帐ID= " & lng结帐ID & " And Nvl(记录状态,0)<>0"
    Call OpenRecordset(rsTemp, "取费用总额")
    cur总额 = rsTemp!金额
...
Public Function 挂号结算冲销_泸州(ByVal lng结帐ID As Long) As Boolean
    ...
    '读取原结算记录信息
    gstrSQL = "Select 病人ID,发生费用金额,全自付金额 from 保险结算记录 Where 记录ID=" & lng原结帐ID
    Call OpenRecordset(rsTemp, "读取原结算记录信息")
    
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & gintInsure & "," & rsTemp!病人ID & "," & _
        Format(zlDatabase.Currentdate, "YYYY") & "," & 0 & "," & 0 & "," & 0 & "," & _
        0 & "," & "NULL" & "," & 0 & "," & 0 & "," & 0 & "," & _
...
Public Function 身份标识_泸州2(ByVal strCard As String, ByVal strPass As String, Optional lng病人ID As Long) As String
    ...
    'H） 密码校验是否正确：验证IC卡中Password。（如果Password为9999不对密码进行验证）。
    If TruncZero(mIC泸州.Password) <> "9999" Then
        If bln远程验证 = False Then
    ...
    '读取医保病人的ID
    gstrSQL = "Select 病人ID From 保险帐户 Where 险类=" & gintInsure & " And 卡号='" & mIC泸州.Cardno & "'"
    Call OpenRecordset(rsTemp, "读取医保病人的ID")
    If rsTemp.EOF Then
