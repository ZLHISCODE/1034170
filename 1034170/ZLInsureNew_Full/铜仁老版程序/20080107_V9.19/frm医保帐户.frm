...
Private Sub mnuEditDisease_Click()
    ...
    With msh帐户_S
        ...
        ElseIf mint险类 = TYPE_自贡市 Or mint险类 = TYPE_泸州市 Then
            '提取已选择的病种
            Dim rsSelected As New ADODB.Recordset
            gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
                    " From 保险病种 A,zlyb.病种信息 B where A.险类=" & mint险类 & " And B.病人ID=" & lng病人id & " And A.ID=B.病种ID And A.险类=B.险类"
...
Private Sub mnuEditPassword_Click()
    ...
    Dim lng病种ID As Long, str病种ID As String
...
Private Sub mnuEditQuery_Click()
    ...
    With msh帐户_S
        ...
        '判断该病人是否有住院记录
        gstrSQL = "select A.单位编码,A.卡号,A.医保号,A.退休证号" & _
                  "  from 保险帐户 A " & _
...
Private Sub mnuEditReckoning_Click()
    ...
    With msh帐户_S
        ...
        If 设置清算方式_贵阳(lng病人id, Me) Then
            MsgBox "清算方式修改成功！", vbInformation, gstrSysName
        End If
    End With
End Sub
'------------------------------------------------
...
Private Sub mnuEditRollIncome_Click()
    ...
    With msh帐户_S
        ...
        ElseIf mint险类 = TYPE_涪陵 Then
            ...
            str就诊编号 = rsTemp!顺序号
            Call initType
            blnReturn = dall(gstr医保机构编码, gstr医院编码, str就诊编号, gstrOutPara)
        ...
        ElseIf mint险类 = TYPE_福建巨龙 Or mint险类 = TYPE_福建省 Or mint险类 = TYPE_福州市 Or mint险类 = TYPE_南平市 Then
            ...
            Else
                If mint险类 = type_大连市 Or mint险类 = type_大连开发区 Then
                    If 入院登记撤销_大连(lng病人id, lng主页ID) = True Then
                        gstrSQL = "ZL_病案主页_撤消医保入院(" & lng病人id & "," & lng主页ID & ")"
                        Call ExecuteProcedure("撤消医保入院")
                        MsgBox "撤消成功，该病人已经由医保病人转为普通病人。", vbInformation, gstrSysName
                    End If
...
Public Sub ShowForm(frmParent As Form)
    ...
    mnuEditQuery.Visible = (gintInsure = TYPE_贵阳市) Or (gintInsure = TYPE_成都内江)
    
    'Modified By 朱玉宝 2004-05-25 原因：医保接口变动
    '------------------------------------------------
    mnuEditReckoning.Visible = (gintInsure = TYPE_贵阳市)
    '------------------------------------------------
    mnuEditDisease.Visible = (gintInsure = TYPE_重庆市 Or gintInsure = TYPE_重庆银海版 Or gintInsure = TYPE_自贡市 Or gintInsure = TYPE_泸州市 Or gintInsure = TYPE_沈阳市 Or gintInsure = TYPE_毕节) '重庆医保和自贡医保才可见
    'Modified by 朱玉宝 20031218 地区：福州
    mnuEditRollIncome.Visible = (gintInsure = TYPE_重庆市 Or gintInsure = TYPE_福建巨龙 Or gintInsure = TYPE_南平市 Or gintInsure = TYPE_福建省 Or gintInsure = TYPE_福州市 Or gintInsure = type_大连市 Or gintInsure = type_大连开发区 Or gintInsure = TYPE_宁海 Or gintInsure = TYPE_涪陵) '重庆医保才可见
    'Modified by 朱玉宝 20031218 地区：福州
...
'        mnuEditVerify_Detail.Visible = True
    ...
    ElseIf gintInsure = TYPE_重庆银海版 Then
        mnuEditVerify_Price.Visible = True
    ElseIf gintInsure = TYPE_重庆渝北 Then
        mnuEditSplit3.Visible = True
        mnuEditVerify_Account.Visible = True
        mnuEditVerify_Hospital.Visible = True
