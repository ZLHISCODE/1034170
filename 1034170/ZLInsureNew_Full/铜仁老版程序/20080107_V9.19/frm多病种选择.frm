...
Begin VB.Form frm多病种选择 
   Caption         =   "多病种选择"
   ClientHeight    =   6315
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   9090
   Icon            =   "frm多病种选择.frx":0000
   LinkTopic       =   "Form2"
   ScaleHeight     =   6315
   ScaleWidth      =   9090
...
Public Function ShowSelect(rsSelect As ADODB.Recordset, ByVal strKey As String, _
    ...
    Optional ByVal rsSelected As ADODB.Recordset, Optional ByVal blnShow As Boolean = True) As Boolean
'功能：多功能选择器
'参数：
'     frmParent=显示的父窗体
'     rsSelect=选择的记录集
...
'注意，为了提高用户输入的速度，特增加了按简码匹配，请传入记录集时，一定要有简码字段
    ...
    If InStr(1, strSQL, " And (编码") <> 0 Then strSQL = Mid(strSQL, 1, InStr(1, strSQL, " And (编码"))
    mrsSel.Open strSQL & " And (编码 Like '%" & strFind & "' Or 名称 Like '%" & strFind & "' Or 简码 Like '%" & strFind & "')", gcnOracle
