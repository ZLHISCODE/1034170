...
'验证医保病人身份
    Dim clsInsure As New clsInsure, lng病人ID As Long
...
Private Sub cob住院次数_Click()
 If gintInsure <> type_大连开发区 And gintInsure <> type_大连市 Then Exit Sub
...
'返回:是否读取成功,成功时rsInfo中包含病人信息,失败时rsInfo=Close
    ...
    bln大连 = (gintInsure = type_大连开发区 Or gintInsure = type_大连市)
...
Private Function Load住院次数() As Boolean
    ...
    ElseIf (Left(strCode, 1) = "B" Or Left(strCode, 1) = "+") And IsNumeric(Mid(strCode, 2)) Then '住院号
        gstrSQL = _
            "Select C.病人ID,C.主页ID" & _
            " From 病人信息 A,病案主页 C" & _
            " Where A.病人ID=C.病人ID  And A.住院号=" & Val(Mid(strCode, 2)) & _
