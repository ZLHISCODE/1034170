...
Public Function 医保初始化_重庆渝北() As Boolean
    ...
    '取医院编码
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_重庆渝北
    Call OpenRecordset(rsTemp, "读取医院编码")
    InitInfor_重庆渝北.医院编码 = Nvl(rsTemp!医院编码)

    '中间库连接
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_重庆渝北
    ...
    '限价确定
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名='处方单价限制' and 险类=" & TYPE_重庆渝北
    Call OpenRecordset(rsTemp, "渝北医保")
    ...
    End If
    '限价确定
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名='个人帐户' and 险类=" & TYPE_重庆渝北
    Call OpenRecordset(rsTemp, "渝北医保")
    If Not rsTemp.EOF Then
...
Public Function 身份鉴别_重庆渝北() As Boolean
    ...
    With g病人身份_重庆渝北
        ...
        .帐户余额 = Val(GetXMLOutput("ThisBaseICUsable")) + Val(GetXMLOutput("ThisOfficialICUsable"))
    End With
...
Private Function Save审批信息(ByVal lng病人id As Long, Optional bln虚拟结算 As Boolean = False) As Boolean
    ...
    g病人身份_重庆渝北.帐户余额 = Val(GetXMLOutput("ThisBaseICUsable")) + Val(GetXMLOutput("ThisOfficialICUsable"))
    ...
    For lngRow = 0 To lngCount - 1
        ...
        strText = strText & g病人身份_重庆渝北.病种编码 & vbTab
...
Private Function IsertInto医保明细(ByVal lng费用ID As Long, ByVal strNO As String, ByVal lng序号 As Long, ByVal lng记录性质 As Long, ByVal strCode As String) As Boolean
    ...
    If g病人身份_重庆渝北.冲销 Then
        gstrSQL = "Select ID From 病人费用记录 where no='" & strNO & "' and 记录性质=" & lng记录性质 & " and 记录状态=3 and 序号=" & lng序号
        OpenRecordset rsTemp, "退单流水号"
        If rsTemp.EOF Then
            IsertInto医保明细 = False
            Exit Function
        End If
        gstrSQL = "Select 记帐流水号,审批人,审批人职称,审批标志,项目编码 From 医保明细费用 where 费用id= " & Nvl(rsTemp!ID)
        OpenRecordset_ZLYB rsTemp, "退单数据插入"
        '--ZL_医保明细费用_INSERT(
...
ECal2:
  ...
  '   性质_IN  ,记录ID_IN,险类_IN,病人ID_IN,年度_IN," & _
    ...
    Else
        ...
        gstrSQL = "zl_保险结算记录_insert(" & IIf(g病人身份_重庆渝北.结算标志 = 1, 2, 1) & "," & g病人身份_重庆渝北.结帐ID & "," & gintInsure & "," & lng病人id & "," & Format(zlDatabase.Currentdate, "YYYY") & "," & _
          dblSumMony(9) & "," & dblSumMony(8) & "," & dblSumMony(5) & ",NULL,NULL," & dblSumMony(4) & "," & "1" & g病人身份_重庆渝北.支付类别 & ",NULL," & _
...
Private Function Get结算方式(ByVal strDblCur As Variant) As Boolean
    ...
    If g病人身份_重庆渝北.帐户余额 <= dbl个人帐户 Then
        ...
        dbl个人帐户 = g病人身份_重庆渝北.帐户余额
    End If
    str结算方式 = str结算方式 & "||医保基金|" & strDblCur(6)
    str结算方式 = str结算方式 & "||大额补助|" & strDblCur(8)
    str结算方式 = str结算方式 & "||公务员补助|" & strDblCur(9)
    str结算方式 = str结算方式 & "||个人帐户|" & dbl个人帐户
    
    If Format(g病人身份_重庆渝北.费用总额, "###0.00;-###0.00;0;0") <> Format(strDblCur(3), "###0.00;-###0.00;0;0") Then
        Dim blnYes As Boolean
        '费用总额与医保中心返回总额不致,不能进行结算
        ShowMsgbox "本次结算总额(" & g病人身份_重庆渝北.费用总额 & ") 与" & vbCrLf & _
                    "   中心返回的总额(" & strDblCur(3) & ")不致产能结算?"
        Exit Function
    End If
...
Private Function Get结算编号() As String
    ...
    gstrSQL = "Select 支付顺序号,备注 摘要 From 保险结算记录 where 记录ID= " & g病人身份_重庆渝北.结帐ID & " And 险类=" & TYPE_重庆渝北
    
    OpenRecordset rsTemp, "获取结算编号"
...
Private Function Get明细记录(ByRef lng结帐ID As Long, Optional strNO As String, Optional lng记录性质 As Long, Optional lng记录状态 As Long) As String
    ...
    If lng结帐ID = 0 And g病人身份_重庆渝北.结算标志 <> 1 Then
            ...
            strSql = " " & _
                ...
                "  Where A.开单部门id=C.id(+) and  A.病人id=L.病人id  and a.收费细目id=J.id and L.险类=" & gintInsure & "  And A.收费细目ID=D.收费细目ID And D.险类= " & gintInsure & _
    ...
    Else
        If g病人身份_重庆渝北.结算标志 = 1 And lng结帐ID = 0 Then
            ...
            strSql = "Select Rownum 标识号, " & _
                     ...
                     "   Where a.记录状态<>0 and nvl(a.附加标志,0)<>9 and A.开单部门id = C.id(+) and a.id=b.费用id and b.就诊编号='" & g病人身份_重庆渝北.就诊编号 & "' " & IIf(g病人身份_重庆渝北.lng病人id = 0, "", " And A.病人id =" & g病人身份_重庆渝北.lng病人id) & "   and A.病人id = L.病人id and L.险类 = " & gintInsure & "  And " & _
                     "          A.收费细目ID = D.收费细目ID and a.收费细目id=J.id And D.险类 =  " & gintInsure & _
                     "    Order by a.NO,A.记录性质,A.记录状态,a.序号"
        Else
            '--需确定审批标志
            strSql = " " & _
...
Public Function 门诊结算_重庆渝北(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    gstrSQL = "Select 病人id, 登记时间 From 病人费用记录 where rownum<=1 and 结帐id=" & lng结帐ID
...
Private Function Get冲销ID() As Long
    ...
    '取冲销记录的结帐ID
    gstrSQL = "select distinct A.结帐ID from 病人费用记录 A,病人费用记录 B where A.NO=B.NO and A.记录性质=B.记录性质 and A.记录状态=2 and B.结帐ID=" & g病人身份_重庆渝北.结帐ID
...
'    Call ExecuteProcedure("保存就诊编号")
    ...
    gstrSQL = "Select C.住院号,C.当前病区id,C.当前床号,c.就诊卡号 as 病历号,c.住院号,to_char(A.确诊日期,'yyyy-MM-dd hh24:mi:ss') as 确诊日期,A.登记人 经办人,B.名称 入院科室,A.住院医师,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 入院经办时间," & _
        ...
        "       (Select 病人id,主页id,max(DECODE(a.诊断次序,1,b.编码||'-'||b.名称,'')) AS 入院诊断, max(DECODE(a.诊断次序,2,b.编码||'-'||b.名称,'')) AS 入院诊断1,max(DECODE(a.诊断次序,3,b.编码||'-'||b.名称,'')) AS 入院诊断2, max(DECODE(a.诊断次序,4,b.编码||'-'||b.名称,'')) AS 入院诊断3 From 诊断情况 A ,疾病编码目录 B Where a.疾病ID = b.ID And a.诊断类型 =1 and a.主页id=" & lng主页ID & " and a.病人id=" & lng病人id & " Group by  病人id,主页id)   D" & _
        "        " & _
        "        " & _
    ...
    AppendXMLNode gobjXMLInPut.documentElement, "akc193", Nvl(rsTemp!入院诊断)
    ...
    AppendXMLNode gobjXMLInPut.documentElement, "ykc149", Nvl(rsTemp!入院诊断1)
    'ykc150  string  100     入院诊断2
    AppendXMLNode gobjXMLInPut.documentElement, "ykc150", Nvl(rsTemp!入院诊断2)
    'ykc151  string  100     入院诊断3
    AppendXMLNode gobjXMLInPut.documentElement, "ykc151", Nvl(rsTemp!入院诊断3)
...
Public Function 出院登记_重庆渝北(lng病人id As Long, lng主页ID As Long) As Boolean
   ...
   '获取基本信息
    ...
    gstrSQL = "Select 就诊编号,结算编号 From 保险帐户 Where 险类=" & TYPE_重庆渝北 & " And 病人ID=" & lng病人id
    Call OpenRecordset(rsTemp, "获取就诊编号和结算编号")
...
'    End If
    ...
    'ykc152  string  100     出院诊断1
    AppendXMLNode gobjXMLInPut.documentElement, "ykc152", Nvl(rsTemp!出院诊断1)
    'ykc153  string  100     出院诊断2
    AppendXMLNode gobjXMLInPut.documentElement, "ykc153", Nvl(rsTemp!出院诊断2)
    'ykc154  string  100     出院诊断3
    AppendXMLNode gobjXMLInPut.documentElement, "ykc154", Nvl(rsTemp!出院诊断3)
    'ykc016  string  12      出院床位
...
Private Function Get入出SQL(ByVal lng病人id As Long, ByVal lng主页ID As Long) As String
    ...
    strSql = "Select C.住院号,C.当前病区id,A.入院病床 ,c.就诊卡号 as 病历号,c.住院号,to_char(A.确诊日期,'yyyy-MM-dd hh24:mi:ss') as 确诊日期,A.登记人 经办人,B.名称 入院科室,A.住院医师,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 入院经办时间," & _
        ...
        "       (Select 病人id,主页id,max(DECODE(a.诊断次序,1,b.编码||'-'||b.名称,'')) AS 出院诊断, max(DECODE(a.诊断次序,2,b.编码||'-'||b.名称,'')) AS 出院诊断1,max(DECODE(a.诊断次序,3,b.编码||'-'||b.名称,'')) AS 出院诊断2, max(DECODE(a.诊断次序,4,b.编码||'-'||b.名称,'')) AS 出院诊断3 From 诊断情况 A ,疾病编码目录 B Where a.疾病ID = b.ID And a.诊断类型 = 3 and a.主页id=" & lng主页ID & " and a.病人id=" & lng病人id & " Group by 病人id,主页id)   G" & _
        " Where A.病人id=C.病人id and C.病人id=" & lng病人id & _
...
Public Function 住院虚拟结算_重庆渝北(rsExse As Recordset, ByVal lng病人id As Long, Optional bln结帐处 As Boolean = True) As String
    ...
    gstrSQL = "" & _
        "   Select ID,NO,记录状态,记录性质,序号 From 病人费用记录 " & _
        "   Where 病人id=" & lng病人id & " and id not in(Select a.id From 病人费用记录 a,医保明细费用 b Where a.id=b.费用id And a.病人id=" & lng病人id & ") " & _
...
Public Function 住院结算_重庆渝北(lng结帐ID As Long, ByVal lng病人id As Long) As Boolean
    ...
    '求本次结算的费用总额
    gstrSQL = "Select Sum(nvl(结帐金额,0)) as 总费用 From 病人费用记录 where 结帐id=" & lng结帐ID
...
Public Function 处方登记_重庆渝北(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    ...
    '更新分配结算号
    gstrSQL = " Select distinct a.病人id,b.就诊编号,b.结算编号  From 病人费用记录 a,保险帐户 b  where a.no='" & str单据号 & "'   and a.记录状态=" & lng记录状态 & " and a.记录性质=" & lng记录性质 & " and a.病人id=b.病人id  and b.险类=" & gintInsure
            
    OpenRecordset rsTemp, "获取病人相关信息"
    '主要考虑记帐表的这种表况
...
Public Function 核对病人就诊信息_重庆渝北(ByVal lng病人id As Long) As String
  '-----------------------------------------------------------------------------------------------------------
    ...
    OpenRecordset rsTemp, "获取医院医保病人的入出信息"
...
Public Function 核对病人帐户支付信息_重庆渝北(ByVal lng病人id As Long) As String
  '-----------------------------------------------------------------------------------------------------------
    ...
    '查找入出记录数
    gstrSQL = "Select count(记录ID) as 记录数,sum(个人帐户支付) as 总额  From 保险结算记录 where nvl(个人帐户支付,0)<>0 and 备注='" & g病人身份_重庆渝北.就诊编号 & "'"
    OpenRecordset rsTemp, "获取医院核对帐户支付信息"
...
Public Function 核对处方明细信息_重庆渝北(ByVal lng病人id As Long) As String
  '-----------------------------------------------------------------------------------------------------------
    ...
    If GetXML串(strOutPut) = False Then
        ...
        Exit Function
    End If
    gstrSQL = "Select count(ID) as 记录数,Sum(付数*数次) as 数量,sum(Round(A.实收金额/(A.数次*A.付数),2)) as 价格,sum(实收金额) as 费用总额   From 病人费用记录 a  where ID in (Select 费用ID From 医保明细费用 where  就诊编号='" & g病人身份_重庆渝北.就诊编号 & "')"
    OpenRecordset rsTemp, "获取处方明细信息"
...
Private Function Get就诊时间(ByVal lng病人id As Long) As String
    ...
    gstrSQL = "Select 就诊时间 From 保险帐户 where 险类=" & gintInsure & " and 病人id=" & lng病人id
    zlDatabase.OpenRecordset rsTemp, gstrSQL, "获取就诊时间"
...
Private Function 病人结算冲销(ByVal lng结帐ID As Long) As Boolean
    ...
    gstrSQL = "select  支付顺序号,备注 from 保险结算记录 where 险类=" & gintInsure & " and 记录id=" & lng结帐ID
...
Private Function 费用结算结果冲销上传(ByVal str结算编号 As String) As Boolean
  ...
  '   性质_IN  ,记录ID_IN,险类_IN,病人ID_IN,年度_IN," & _
    ...
    gstrSQL = "zl_保险结算记录_insert(" & IIf(g病人身份_重庆渝北.结算标志 = 1, 2, 1) & "," & g病人身份_重庆渝北.冲销ID & "," & gintInsure & "," & g病人身份_重庆渝北.lng病人id & "," & Format(zlDatabase.Currentdate, "YYYY") & "," & _
