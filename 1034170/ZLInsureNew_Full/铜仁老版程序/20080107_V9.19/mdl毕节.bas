...
Public Function 医保初始化_毕节(Optional ByVal blnTest As Boolean = False) As Boolean
    ...
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_毕节
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & TYPE_毕节
...
Public Function 门诊结算_毕节(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    gstrSQL = " Select  A.病人ID,A.收费类别,A.收费细目ID,round(A.实收金额,2) 实收金额,B.项目编码,Nvl(B.项目名称,C.名称) AS 项目名称,B.附注" & _
              " From 病人费用记录 A,保险支付项目 B,收费细目 C" & _
              " Where A.结帐ID=" & lng结帐ID & " And A.收费细目ID=B.收费细目ID(+) And B.险类(+)=" & TYPE_毕节 & _
              " And Nvl(A.附加标志,0)<>9 And Nvl(A.记录状态,0)<>0" & _
              " And A.收费细目ID=C.ID"
    Call OpenRecordset(rsDetail, "提取本次结算费用明细")
    lng病人ID = rsDetail!病人ID
    
    '判断卡是不是当前病人的
    gstrSQL = "Select 卡号,医保号 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_毕节
    Call OpenRecordset(rsCheck, "判断卡是不是当前病人的")
    ...
    '如果使用个人帐户支付，显示消费提示框
    If cur个人帐户 <> 0 Then Call Frm交易提示框.ShowME(IC_Data_毕节.姓名, IC_Data_毕节.个人帐户余额 + cur个人帐户, _
...
Public Function 门诊结算冲销_毕节(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    If IC_Data_毕节.最后就诊医院代码 <> gCominfo_毕节.strHospitalCode Then
        Call IC_End(True)
        MsgBox "你已在其他医疗机构发生就诊行为，不能退单！", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    Call OpenRecordset(rsTemp, "提取原始的保险结算记录")
    ...
    With rsCheck
        ...
        If IsNull(!就诊登记号) Then
            Call IC_End(True)
            MsgBox "没有找到最后一次的就诊登记号，无法退单！", vbInformation, gstrSysName
            Exit Function
        End If
        If Nvl(!就诊登记号) <> str就诊登记号 Then
            Call IC_End(True)
            MsgBox "只能退该病人在本院就诊的最后一笔门诊单据！", vbInformation, gstrSysName
            Exit Function
        End If
        If str就诊登记号 <> str退单流水号 Then
            Call IC_End(True)
            MsgBox "输入的就诊登记号与原单据的就诊登记号不符，无法退单！", vbInformation, gstrSysName
            Exit Function
        End If
    End With
...
Public Function 入院登记_毕节(lng病人ID As Long, lng主页ID As Long) As Boolean
    '因入院登记前必须要进行身份验证,因此,病人相关信息可直接从卡数据中获取
    Dim blnTrans As Boolean
    Dim str就诊登记号 As String
    Dim rsTemp As New ADODB.Recordset
    On Error GoTo ErrHand
    ...
    gstrSQL = " Select B.入院日期,D.名称 As 入院科室,B.入院病床,B.登记人 As 操作员,E.病种名称,Sum(Nvl(F.金额,0)) As 预交款" & _
              " From 保险帐户 A,病案主页 B,病人信息 C,部门表 D,zlbj.病种目录表 E,病人预交记录 F" & _
              " Where A.险类=" & TYPE_毕节 & " And A.病人ID=" & lng病人ID & " And B.主页ID=" & lng主页ID & " ANd A.病人ID=B.病人ID And B.病人ID=C.病人ID And B.主页ID=C.住院次数" & _
    ...
    Call OpenRecordset(rsTemp, "获取本次入院相关信息")
    ...
    gstrSQL = " Insert Into 入院登记表" & _
              ...
              "'" & Nvl(rsTemp!入院病床) & "'," & Val(Nvl(rsTemp!预交款, 0)) & ",'" & Nvl(rsTemp!操作员, "ZLHIS") & "','否'," & IIf(gCominfo_毕节.blnOnLine, "'10'", "'00'") & ")"
    gcnGYBJYB.Execute gstrSQL
    If gCominfo_毕节.blnOnLine Then
        gstrSQL = " Insert Into 住院登记表" & _
                  ...
                  " Values" & _
                  " ('" & IC_Data_毕节.社会保障号 & "','" & IC_Data_毕节.姓名 & "','" & str就诊登记号 & "'," & _
                  "'" & Format(zlDataBase.Currentdate, "yyyy.MM.dd") & "','','" & rsTemp!病种名称 & "','" & Nvl(rsTemp!入院科室) & "'," & _
                  "'" & Nvl(rsTemp!入院病床) & "'," & Val(Nvl(rsTemp!预交款, 0)) & ",'" & Nvl(rsTemp!操作员, "ZLHIS") & "','否'," & _
...
Public Function 入院登记撤销_毕节(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select 住院号 From zlbj.入院登记表 Where 社会保障号=" & _
            "   (Select 医保号 From 保险帐户 Where 病人ID=" & lng病人ID & ")"
    Call OpenRecordset(rsTemp, "取出本次住院号")
    str住院号 = rsTemp!住院号
...
Public Function 出院登记_毕节(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    If 存在未结费用(lng病人ID, lng主页ID) Then
        MsgBox "只有费用结清后才允许出院！", vbInformation, gstrSysName
        Exit Function
    End If
    
    '提取参保人的社会保障号
    gstrSQL = "Select 医保号 From 保险帐户 Where 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "提取参保人的社会保障号")
...
Public Function 住院虚拟结算_毕节(rsExse As Recordset, ByVal lng病人ID As Long) As String
    ...
    '取该病人的社会保障号
    gstrSQL = " Select B.姓名,A.卡号,A.医保号 From 保险帐户 A,病人信息 B" & _
              " Where A.险类=" & TYPE_毕节 & " And A.病人ID=B.病人ID And A.病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取该病人的社会保障号")
        ...
        Do While Not .EOF
            ...
            If Nvl(!是否上传, 0) = 0 Then
                ...
                gstrSQL = " Select A.项目编码,A.项目名称,B.名称,A.附注 As 大类 From 保险支付项目 A,收费细目 B " & _
                          " Where B.ID=" & !收费细目ID & " And B.ID=A.收费细目ID(+) And A.险类(+)=" & TYPE_毕节
                Call OpenRecordset(rsTemp, "取该医保项目相关信息")
                If rsTemp.RecordCount <> 0 Then
...
Public Function 住院结算_毕节(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    '取病人的主页ID
    gstrSQL = "Select nvl(住院次数,0) AS 主页ID From 病人信息 Where 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取病人的主页ID")
    lng主页ID = rsTemp!主页ID
    ...
    gstrSQL = "Select Nvl(A.冲预交,0) 个人帐户 " & _
        ...
        " Where A.病人ID=B.病人ID And B.险类=" & TYPE_毕节 & _
...
Public Function 住院结算冲销_毕节(lng结帐ID As Long) As Boolean
    '只有门诊允许结算作废，住院不允许
    MsgBox "医保接口不支持作废住院结算单！", vbInformation, gstrSysName
    住院结算冲销_毕节 = False
End Function
...
Public Function 处方上传_毕节(ByVal int性质 As Integer, ByVal int状态 As Integer, ByVal str单据号 As String) As Boolean
    ...
    gstrSQL = " Select A.病人ID,A.收费类别,A.收费细目ID,A.序号,A.发生时间," & _
              ...
              " From 病人费用记录 A,保险支付项目 B,收费细目 C" & _
              " Where A.收费细目ID=C.ID And A.收费细目ID=B.收费细目ID(+) And B.险类(+)=" & TYPE_毕节 & _
              " And A.记录性质=" & int性质 & " And A.记录状态=" & int状态 & " And A.NO='" & str单据号 & "'" & _
              " And Nvl(A.附加标志,0)<>9 And Nvl(A.记录状态,0)<>0 And Nvl(A.是否上传,0)=0"
    Call OpenRecordset(rsDetail, "提取本张处方明细")
    
    With rsDetail
        Do While Not .EOF
            If lng病人ID <> !病人ID Then
                '检查本次是否以医保身份入院
                gstrSQL = "Select Count(*) Records From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & !病人ID & " And A.主页ID=B.住院次数 And A.险类=" & TYPE_毕节
                Call OpenRecordset(rsInsure, "判断是否医保病人")
                blnInsure = (rsInsure!Records = 1)
                If blnInsure Then
                    lng病人ID = !病人ID
                    '提取病人的社会保障号
                    gstrSQL = "Select 医保号 As 社会保障号 From 保险帐户 Where 病人ID=" & lng病人ID
                    Call OpenRecordset(rsTemp, "提取病人的社会保障号")
                    str社会保障号 = Nvl(rsTemp!社会保障号)
                    '取本次入院的就诊登记号(从中间库的入院登记表中取)
                    gstrSQL = " Select B.医保号 As 社会保障号,C.姓名,A.住院号" & _
                              " From zlbj.入院登记表 A,保险帐户 B,病人信息 C" & _
                    ...
                    If Not CheckCard(str社会保障号) Then
                        If Not BalanceLack(lng病人ID) Then
                            MsgBox "该病人[社会保障号：" & str社会保障号 & "]的卡已被封锁，必须全部缴现金，而预交款不足，请缴款！", vbInformation, gstrSysName
                            Call 事务_回滚
                            Exit Function
                        End If
                    End If
...
Private Function Calc统筹金额_病种(ByVal dbl进入统筹 As Double, ByVal lng病人ID As Long) As Double
    ...
    '先取该病人当前的病种ID
    gstrSQL = "Select Nvl(病种ID,0) AS 病种ID From 保险帐户 Where 险类=" & TYPE_毕节 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsCalc, "先取该病人当前的病种ID")
    lng病种ID = rsCalc!病种ID
...
Private Function Calc统筹金额_分档(ByVal dbl统筹金额 As Double, ByVal str社会保障号 As String) As Double
    ...
    '准备进行分档计算
    intLoops = IIf(Right(str参加险种, 1) = "1", 4, 3)
...
Private Function 检查连接密码() As Boolean
    ...
    gstrSQL = " Select 连接密码,单位名称,是否使用IC卡密码,住院启用病种自付,住院启用药品诊疗自付,住院按年度起付" & _
              " From 医疗机构基本情况表 Where 单位代码='" & gCominfo_毕节.strHospitalCode & "'"
    With rsTemp
        If .State = 1 Then .Close
        .Open gstrSQL, gcnGYBJYB
        If .RecordCount = 0 Then
            MsgBox "没有找到本医疗机构的基本信息，请与中心联系！[检查连接密码]", vbInformation, gstrSysName
            Exit Function
        Else
            If gCominfo_毕节.strConnectPass <> Nvl(!连接密码) Then
                MsgBox "连接密码错误，可能中心已禁止本医疗机构使用，请与中心联系！[检查连接密码]", vbInformation, gstrSysName
                Exit Function
            End If
        End If
...
Private Function 检查是否上传明细() As Boolean
    ...
    '检查是否存在未上传的明细
    gstrSQL = " Select 1 " & _
        " From 保险帐户 A,病人费用记录 B,病人信息 C" & _
        " Where A.险类=" & TYPE_毕节 & " And A.病人ID=B.病人ID And C.病人ID=B.病人ID And B.主页ID=C.住院次数" & _
        " And B.记录性质=3 And Nvl(B.是否上传,0)=0 And B.发生时间 Between Sysdate-3 and Sysdate-1 And Rownum<2"
    Call OpenRecordset(rsTemp, "检查是否存在未上传的费用明细")
...
Private Function 事务_提交() As Boolean
    ...
    If gCominfo_毕节.blnOnLine Then Call gobjCenter.CommitTrans
    
    事务_提交 = True
    If gCominfo_毕节.blnOnLine Then Call 关闭中心连接
    Exit Function
ErrHand:
    If ErrCenter = 1 Then Resume
End Function
...
Private Function CheckCard(ByVal str社会保障号 As String) As Boolean
...
Private Function BalanceLack(ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = "Select 预交余额,费用余额 From 病人余额 Where 性质=1 And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "提取预交余额及费用余额")
