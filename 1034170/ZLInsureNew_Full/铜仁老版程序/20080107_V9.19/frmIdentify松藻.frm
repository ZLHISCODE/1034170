...
Begin VB.Form frmIdentify松藻 
   ...
   End
End
Attribute VB_Name = "frmIdentify松藻"
...
'返回：
    ...
    cmb中心.Clear
    gstrSQL = "select A.具有中心,B.序号,B.编码,B.名称 from 保险类别 A,保险中心目录 B where A.序号=" & gintInsure & " and A.序号=b.险类"
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmb中心_Click()
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & gintInsure & " and 中心=" & cmb中心.ItemData(cmb中心.ListIndex)
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & gintInsure & " and 中心=" & lng中心 & " and 医保号='" & Trim(txtEdit(Text医保号).Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
    ...
    strAddition = strAddition & ";" & Val(txtEdit(Text个人自付段).Text)        '20上年工资总额
    strAddition = strAddition & ";" & Int(Val(txtEdit(Text住院次数).Text))      '21住院次数累计
    
    lng病人ID = BuildPatiInfo_松藻(mbytType, strIdentify & strAddition, mlng病人ID)
    '返回格式:中间插入病人ID
    If lng病人ID > 0 Then
...
'功能：检查数据的正确性
    ...
    For lngIndex = txtEdit.LBound To txtEdit.UBound
        ...
        If lngIndex >= Text住院次数 And lngIndex <= Text个人自付段 Then
    ...
    Select Case Index
        Case Select卡号
            gstrSQL = " Select A.病人ID as ID,A.卡号,A.医保号,B.姓名,B.性别,B.出生日期,B.身份证号,C.序号 as 中心ID " & _
                    ...
                    "  where A.病人ID=B.病人ID and A.险类=" & gintInsure & _
...
Private Sub txtEdit_LostFocus(Index As Integer)
    If Index >= Text住院次数 And Index <= Text个人自付段 Then
...
'从已经存在的记录中读出帐户信息
    ...
    If Not rs帐户 Is Nothing Then
        ...
        '再读出帐户年度信息
        gstrSQL = "select * from 帐户年度信息 where 险类=" & gintInsure & _
            " and 病人ID=" & rs帐户("ID") & " and 年度=" & Year(dtp生日.MaxDate)
        Call OpenRecordset(rsTemp, Me.Caption)
        
        If rsTemp.EOF = False Then
            '设置帐户情况
            txtEdit(Text住院次数).Text = Format(IIf(IsNull(rsTemp("住院次数累计")), 0, rsTemp("住院次数累计")), "#0;0;0;0")
            txtEdit(Text帐户增加累计).Text = Format(IIf(IsNull(rsTemp("帐户增加累计")), 0, rsTemp("帐户增加累计")), "######0.00;0.00;0.00;0.00")
            txtEdit(Text帐户支出累计).Text = Format(IIf(IsNull(rsTemp("帐户支出累计")), 0, rsTemp("帐户支出累计")), "######0.00;0.00;0.00;0.00")
            txtEdit(Text个人自付段).Text = Format(IIf(IsNull(rsTemp("进入统筹累计")), 0, rsTemp("进入统筹累计")), "######0.00;0.00;0.00;0.00")
        Else
            txtEdit(Text住院次数).Text = "0"
            txtEdit(Text帐户增加累计).Text = "0.00"
            txtEdit(Text帐户支出累计).Text = "0.00"
            txtEdit(Text个人自付段).Text = "0.00"
        End If
    End If
End Sub
