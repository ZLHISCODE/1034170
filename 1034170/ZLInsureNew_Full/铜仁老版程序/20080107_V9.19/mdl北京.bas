...
'    13  分解状态    C   1   0-正常，1-不符合特殊标识，2-医保目录内不存在，3-对照错误
    ...
    gstrSQL = "" & _
        " SELECT 实际票号,开单人" & _
        " From 病人费用记录" & _
        " WHERE 结帐ID=" & lng结帐ID & " And Rownum<2"
    Call OpenRecordset(rsTemp, "提取收费单据号")
    ...
    If bln住院 Then
        ...
        gstrSQL = "" & _
            ...
            " From 病人结帐记录" & _
            " WHERE ID=" & lng结帐ID
        Call OpenRecordset(rsTemp, "从结帐单中取发票号")
        strNO = Nvl(rsTemp!实际票号)
    End If
    ...
    For lngRow = 1 To lngRows
        ...
        gstrSQL = "" & _
            ...
            " From 收费细目 A,保险支付项目 B," & GetUser & ".药品目录 C,病人费用记录 F" & _
            ...
            " From 收费细目 A,保险支付项目 B," & GetUser & ".诊疗目录 C,病人费用记录 F" & _
            " Where A.ID=F.收费细目ID ANd A.ID=B.收费细目ID And B.项目编码=C.编码 " & _
            " And F.结帐ID=" & lng结帐ID & " AND F.序号=" & Split(arrRow(lngRow), mstrSplit)(col项目序号) & "" & _
            " And A.类别 Not In ('5','6','7')"
        Call OpenRecordset(rsDetail, "获取项目信息")
        
        '如果为空，说明是非医保项目，传固定的值
        If rsDetail.RecordCount = 0 Then
            gstrSQL = " Select A.规格,A.标识码 As 收费类别,'' AS 剂型 " & _
                      " From 药品目录 A,病人费用记录 F" & _
                      " Where A.药品ID=F.收费细目ID And F.结帐ID=" & lng结帐ID & " AND F.序号=" & Split(arrRow(lngRow), mstrSplit)(col项目序号)
            gstrSQL = gstrSQL & " UNION " & _
                      " Select A.规格,A.标识主码 As 收费类别,'' AS 剂型 " & _
                      " From 收费细目 A,病人费用记录 F" & _
                      " Where A.ID=F.收费细目ID And F.结帐ID=" & lng结帐ID & " AND F.序号=" & Split(arrRow(lngRow), mstrSplit)(col项目序号) & _
                      " And A.类别 Not In ('5','6','7')"
            Call OpenRecordset(rsDetail, "获取非医保项目的信息")
        End If
...
'    13  分解状态    C   1   0-正常，1-不符合特殊标识，2-医保目录内不存在，3-对照错误
    ...
    gstrSQL = "Select min(登记时间) 开始日期 From 病人费用记录 Where 结帐ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "读取费用开始日期")
    ...
    gstrSQL = "Select 出院日期,出院方式 From 病案主页 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsTemp, "提取出院日期及出院方式")
    If rsTemp.EOF Then
    ...
    gstrSQL = "" & _
        ...
        " WHERE A.病人ID=B.病人ID And A.病人ID=C.病人ID And B.主页ID=C.住院次数 And B.病人ID=D.病人ID(+) And B.主页ID=D.主页ID(+) And A.卡号='" & gComInfo_北京.卡号 & "'"
    Call OpenRecordset(rsTemp, "提取参保人基本信息")
    ...
    For lngRow = 1 + (Val(Split(arrRow(0), "|")(col汇总_费用分段数))) To lngRows
        ...
        gstrSQL = "" & _
            " Select A.规格,C.收费类别,C.剂型" & _
            " From 收费细目 A,保险支付项目 B," & strUser & ".药品目录 C,病人费用记录 F" & _
            " Where A.ID=F.收费细目ID ANd A.ID=B.收费细目ID And B.项目编码=C.编码 " & _
            " And F.NO='" & str明细_NO & "' And F.记录性质=" & str明细_性质 & " ANd F.记录状态=" & str明细_状态 & " ANd F.序号=" & str明细_序号 & _
            " Union" & _
            " Select A.规格,C.收费类别,'' AS 剂型" & _
            " From 收费细目 A,保险支付项目 B," & strUser & ".诊疗目录 C,病人费用记录 F" & _
            " Where A.ID=F.收费细目ID ANd A.ID=B.收费细目ID And B.项目编码=C.编码 " & _
            " And F.NO='" & str明细_NO & "' And F.记录性质=" & str明细_性质 & " ANd F.记录状态=" & str明细_状态 & " ANd F.序号=" & str明细_序号 & _
            " And A.类别 Not In ('5','6','7')"
        Call OpenRecordset(rsDetail, "获取项目信息")
        ...
        If rsDetail.RecordCount = 0 Then
            gstrSQL = " Select A.规格,A.标识码 As 收费类别,'' AS 剂型 " & _
                      " From 药品目录 A,病人费用记录 F" & _
                      " Where A.药品ID=F.收费细目ID And F.NO='" & str明细_NO & "' And F.记录性质=" & str明细_性质 & " ANd F.记录状态=" & str明细_状态 & " ANd F.序号=" & str明细_序号
            gstrSQL = gstrSQL & " UNION " & _
                      " Select A.规格,A.标识主码 As 收费类别,'' AS 剂型 " & _
                      " From 收费细目 A,病人费用记录 F" & _
                      " Where A.ID=F.收费细目ID And F.NO='" & str明细_NO & "' And F.记录性质=" & str明细_性质 & " ANd F.记录状态=" & str明细_状态 & " ANd F.序号=" & str明细_序号 & _
                      " And A.类别 Not In ('5','6','7')"
            Call OpenRecordset(rsDetail, "获取非医保项目的信息")
        End If
        
        gstrSQL = "Select to_Char(发生时间,'yyyy-MM-dd hh24:mi:ss') AS 发生时间 From 病人费用记录 F" & _
            " WHERE F.NO='" & str明细_NO & "' And F.记录性质=" & str明细_性质 & " ANd F.记录状态=" & str明细_状态 & " ANd F.序号=" & str明细_序号
        Call OpenRecordset(rs费用发生日期, "取该记录的费用发生时间")
...
Private Function GetUser() As String
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & TYPE_北京 & " And 参数名='医保用户名'"
    Call OpenRecordset(rsTemp, "获取中间库用户名")
    GetUser = rsTemp!参数值
End Function
...
Private Function TransationHosp(ByVal str待遇 As String) As String
    ...
    gstrSQL = "" & _
        ...
        " And A.病人ID=D.病人ID And B.主页ID=D.住院次数 And A.卡号='" & gComInfo_北京.卡号 & "'"
    Call OpenRecordset(rsTemp, "提取参保人本次住院基本信息")
...
Public Function CheckTradeName(ByVal lng收费细目ID As Long, ByVal str医保编码 As String) As Boolean
    ...
    On Error GoTo ErrHand
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & TYPE_北京 & " And 参数名='医保用户名'"
    Call OpenRecordset(rsTemp, "获取中间库的用户名")
    strUser = rsTemp!参数值
    ...
    gstrSQL = " Select 1 From " & strUser & ".药品别名 " & _
              ...
              "         (Select 药名ID From 药品目录 Where 药品ID=" & lng收费细目ID & "))"
    If rsTemp.State = 1 Then rsTemp.Close
...
Private Sub GetSequence_北京(ByVal lng病人ID As Long)
    ...
    gstrSQL = "Select to_char(就诊时间,'yyyy-MM-dd hh24:mi:ss') As 就诊时间 From 保险帐户" & _
        " Where 险类=" & TYPE_北京 & " And 病人ID=" & lng病人ID
...
Public Function 医保初始化_北京(Optional ByVal blnTest As Boolean = False) As Boolean
    ...
    If mblnInit = False Then
        '读出连接医保服务器的配置
        gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & TYPE_北京
        ...
        '取医院编码
        gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_北京
...
Public Sub WriteBusinessLOG(ByVal strFunc As String, ByVal strInput As String, ByVal strOutput As String)
    ...
    If (GetSetting("ZLSOFT", "医保", "调试", 0) = 0) Then Exit Sub
...
'    9   费用发生日期    D   8   YYYYMMDD
    ...
    Do Until rs明细.EOF
        ...
        gstrSQL = "Select A.项目编码 As 医保编码,C.通用名称 AS 项目名称 " & _
            ...
            " Where A.险类(+)=" & TYPE_北京 & " And A.收费细目ID(+)=B.药品ID And B.药名ID=C.药名ID " & _
            " AND B.药品ID=" & rs明细!收费细目ID & _
            " UNION " & _
            " Select A.项目编码 As 医保编码,B.名称 AS 项目名称" & _
            " From 保险支付项目 A,收费细目 B" & _
            " Where A.险类(+)=" & TYPE_北京 & " AND B.ID=" & rs明细!收费细目ID & _
...
Public Function 门诊结算冲销_北京(ByVal lng结帐ID As Long, ByVal cur个人帐户 As Currency, ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = "Select 卡号,交易流水号,结算时间 From " & GetUser & ".交易待遇信息 " & _
            " Where 交易流水号=(Select 支付顺序号 From 保险结算记录 Where 性质=1 AND 记录ID=" & lng结帐ID & ")"
    Call OpenRecordset(rsTemp, "取出原始结算记录的结算时间")
    str卡号 = rsTemp!卡号
...
Public Function 入院登记_北京(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    Call OpenRecordset(rsTemp, "提取入院日期")
...
Public Function 出院登记_北京(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select Count(*) AS Records From 病人费用记录" & _
        " Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsTemp, "'如果没有发生任何费用，不允许办理出院")
    If rsTemp!Records = 0 Then
    ...
    If Not 门诊特殊病(lng病人ID) Then
        ...
        gstrSQL = " Select B.工作性质,A.出院方式,A.出院日期 " & _
                  ...
                  " Where A.出院科室ID=B.部门ID(+) And A.病人ID=" & lng病人ID & " And A.主页ID=" & lng主页ID
        Call OpenRecordset(rsTemp, "提取临床部门性质及出院情况")
...
Public Function 住院结算_北京(ByVal lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    '读取本次费用明细
    gstrSQL = " Select 主页ID From 病人费用记录 Where 结帐ID=" & lng结帐ID & " ANd Rownum<2"
    Call OpenRecordset(rsTemp, "读取本次费用明细")
    lng主页ID = rsTemp!主页ID
    ...
    Do Until rs明细.EOF
        ...
        If rs明细!记录状态 = 1 Or rs明细!记录状态 = 3 Then
            gstrSQL = "Select A.项目编码 As 医保编码,C.通用名称 AS 项目名称,D.名称 AS 限制等级,F.备注 " & _
                ...
                " Where A.险类=" & TYPE_北京 & " And A.收费细目ID=B.药品ID And B.药名ID=C.药名ID And F.编码=A.项目编码 " & _
                " AND B.药品ID=" & rs明细!收费细目ID & " ANd F.使用限制等级=D.编码"
...
Public Function 住院结算冲销_北京(ByVal lng结帐ID As Long) As Boolean
    ...
    gstrSQL = "Select 病人ID From 病人费用记录 Where 结帐ID=" & lng结帐ID & " ANd Rownum<2"
    Call OpenRecordset(rsTemp, "获取病人ID")
    lng病人ID = rsTemp!病人ID
    ...
    gstrSQL = "Select 卡号,交易流水号,结算时间 From " & GetUser & ".交易待遇信息 " & _
            " Where 交易流水号=(Select 支付顺序号 From 保险结算记录 Where 性质=2 AND 记录ID=" & lng结帐ID & ")"
    Call OpenRecordset(rsTemp, "取出原始结算记录的结算时间")
    str卡号 = rsTemp!卡号
    str原交易流水号 = rsTemp!交易流水号
    str结算时间 = Format(rsTemp!结算时间, "yyyy-MM-dd HH:mm:ss")
