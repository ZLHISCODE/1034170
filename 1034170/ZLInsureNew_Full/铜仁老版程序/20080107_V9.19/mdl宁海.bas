...
Public Function ReadIC_宁海() As Boolean
    Dim arrReturn
    Dim intReturn As Integer
    Dim strTest As String, strBit As String
    
    '完成IC卡的读操作，返回数据填写到IC_Data_宁海
    Call Interface_Prepare_宁海(ReadIC, "1~~~~10", "")
    ...
    If Not ExchangeICData(arrReturn(4)) Then Exit Function
    
    ReadIC_宁海 = True
End Function

Private Function ExchangeICData(ByVal strBuffer As String) As Boolean
    On Error GoTo ErrHand
    With IC_Data_宁海
        .医疗证号 = Getsubstr(strBuffer, 1, 1, 14)
        .帐号 = Getsubstr(strBuffer, 1, 15, 10)
...
'返回：初始化成功，返回true；否则，返回false
    ...
    If mblnInit = False Then
        If Not blnTest Then '如果是测试，则说明是保险参数设置处调用
            '读出连接医保服务器的配置
            gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_宁海
        ...
        '取医院编码
        gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_宁海
...
Private Function Get住院号(ByVal lng病人ID As Long, Optional ByVal blnNew As Boolean = False) As String
    ...
    '获取该病人的就诊时间
    gstrSQL = "Select to_char(就诊时间,'yyyy-MM-dd hh24:mi:ss') As 就诊时间,退休证号 From 保险帐户" & _
        " Where 险类=" & TYPE_宁海 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "获取该病人的就诊时间")
    '肯定不会为空
    If Not blnNew Then
...
'    Prescription = 单据号码(N10)+药品诊疗类型(N1:1药品，2诊疗)+项目编号(N10)+项目医院端名称(VC80)+ 医院端规格(VC20) + 单复方标志(N1:0非草药(不存在单复方)，1草药单方，2草药复方) + 单价N(14,4) + 数量N(14,4)+自付比率N(5,4)。
    With rs明细
        ...
        Do While Not .EOF
            ...
            Call OpenRecordset(rsTemp, "提取医保项目编号、项目名称及规格")
    ...
    If strDoctor <> "" Then
        gstrSQL = "SELECT C.名称 AS 开单科室 " & _
                 ...
                 " WHERE A.人员ID= " & _
                 "     (SELECT ID FROM 人员表 WHERE 姓名='" & rs明细!开单人 & "') " & _
                 " AND A.部门ID=B.部门ID AND A.部门ID=C.ID AND B.工作性质='临床' AND 服务对象 IN (1,3) " & _
                 " AND ROWNUM<2"
        Call OpenRecordset(rsTemp, "提取开单科室")
        strDepart = "0"
    ...
    strInput = "1" & strField & IC_Data_宁海.IC卡数据 & strField & strField & strField & "1" & strField & strBill & strSplit & strDetail
...
Public Function 门诊结算_宁海(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    Dim dbl当年帐户 As Double, dbl往年帐户 As Double
    ...
    '门诊结算
    '缺省现金支付
    strInput = "1~~1~~" & IC_Data_宁海.mstr结算入口参数串 & strField & UserInfo.姓名
...
'        End If
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & gintInsure & "," & lng病人ID & "," & _
        Year(zlDatabase.Currentdate()) & ",0,0,0,0,0,0,0,0," & dbl费用总额 & "," & dbl现金支付 & ",0," & _
        dbl统筹基金 & "," & dbl统筹基金 & "," & dbl大病补助 & "," & dbl公务员补助 & "," & dbl当年帐户 + dbl往年帐户 & ",'" & IC_Data_宁海.mstr交易流水号 & "')"
...
Public Function 门诊结算冲销_宁海(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    '取原交易流水号
    gstrSQL = "Select 支付顺序号 From 保险结算记录 Where 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "取原交易流水号")
    str原交易流水号 = Nvl(rsTemp!支付顺序号)
    ...
    Call OpenRecordset(rsTemp, "提取原结算记录，做为产生本次结算记录的依据")
...
Public Function 入院登记_宁海(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
    ...
    Call OpenRecordset(rsTemp, "获取入院基本信息")
    ...
    strInput = "1" & strField & strField & strField & strField & str住院号 & strField & _
        str入院日期 & strField & str医生 & strField & str入院诊断 & strField & str疾病编号 & strField & _
    ...
    Call ExecuteProcedure("更新顺序号")
    gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & TYPE_宁海 & ",'退休证号','''" & str住院号 & "''')"
    Call ExecuteProcedure("更新住院号")
...
Public Function 入院登记撤销_宁海(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    strInput = "1" & strField & strField & strField & strField & str住院号
...
Public Function 住院虚拟结算_宁海(rsExse As Recordset, ByVal lng病人ID As Long) As String
    ...
    '先提取该病人的住院流水号
    gstrSQL = "Select A.医保号,A.顺序号,A.IC,A.病种ID,B.住院次数 主页ID From 保险帐户 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "先提取该病人的住院流水号")
    str医保证号 = rsTemp!医保号
    str住院流水号 = rsTemp!顺序号
    lng主页ID = rsTemp!主页ID
    ...
    Call OpenRecordset(rs明细, "提取本次费用明细")
    ...
    With rs明细
        Do While Not .EOF
            ...
            If str医保编码 <> "" Then
                ...
                '再提取医院项目的规格及单位
                gstrSQL = "Select 名称,规格,计算单位 From 收费细目 Where ID=" & !收费细目ID
                Call OpenRecordset(rsItem, "再提取医院项目的规格及单位")
...
'                    -3至少有一条费用明细不能保存，此时只要读取无法保存的费用明细列表，无法保存的住院号列表域为空，即出口参数为：空~空~空+空+无法保存的费用明细列表。
    ...
    strInput = "1" & strField & IC_Data_宁海.IC卡数据 & strField & strField & strField & str住院号 & strField & int记录数 & strField & dbl非医保金额
...
'    交易结果~错误信息+写医保卡结果　+ 扣银行账户结果 + 写卡后IC卡数据 + 结算结果(参考住院预结算)
    strInput = "1" & strField & IC_Data_宁海.IC卡数据 & strField & "1" & strField & strField & _
    str住院号 & strField & IC_Data_宁海.mstr结算入口参数串 & strField & UserInfo.姓名
    ...
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & gintInsure & "," & lng病人ID & "," & _
        Year(zlDatabase.Currentdate()) & ",0,0,0,0,0,0,0,0," & dbl费用总额 & "," & dbl现金支付 & ",0," & _
        dbl统筹基金 & "," & dbl统筹基金 & "," & dbl大病补助 & "," & dbl公务员补助 & "," & dbl帐户支付 & ",'" & str交易流水号 & "'," & lng主页ID & ")"
    gcnOracle.Execute gstrSQL, , adCmdStoredProc
    ...
    MsgBox "出院结算办理成功，请为病人办理出院手续！", vbInformation, gstrSysName
    住院结算_宁海 = True
    Exit Function
ErrHand:
    If ErrCenter = 1 Then
...
Public Function 住院信息变动_宁海(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select A.当前科室ID,B.名称,A.当前床号 From 病人信息 A,部门表 B" & _
        " Where A.病人ID=" & lng病人ID & " And A.当前科室ID=B.ID(+)"
    Call OpenRecordset(rsTemp, "提取科室名称及床号")
    ...
    Call OpenRecordset(rsTemp, "获取医生姓名及出院日期")
    If Nvl(rsTemp!出院日期) <> "" Then
        str出院日期 = Format(rsTemp!出院日期, "yyyy.MM.dd")
    End If
    str医生 = Nvl(rsTemp!住院医师)
    gstrSQL = "Select Nvl(病种ID,0) 病种ID From 保险帐户 Where 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取病种ID")
    lng病种ID = rsTemp!病种ID
    If lng病种ID <> 0 Then
        '提取前置机中的疾病信息
        gstrSQL = "Select JBBZDM From SIM_JBDA Where JBBM=" & lng病种ID
    ...
    strInput = "~~~~" & str住院号 & strField & Format(zlDatabase.Currentdate(), "yyyy.MM.dd") & strField & _
        str床号 & strField & str医生 & strField & str出院诊断 & strField & str疾病编号 & strField & str科室名称 & strField & lng科室ID
...
Public Function 出院登记_宁海(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    If 存在未结费用(lng病人ID, lng主页ID) Then
        MsgBox "必须先住院结算，才能办理出院！", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    strInput = "~~~~" & str住院号 & strField & str出院诊断 & strField & str疾病编号 & strField & str出院日期
...
Public Function 出院登记撤销_宁海(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    str住院号 = Get住院号(lng病人ID)
    '提取结算流水号
    gstrSQL = "Select 支付顺序号 From 保险结算记录 Where 记录ID=(Select Max(记录ID) From 保险结算记录 Where 性质=2 And 主页ID=" & lng主页ID & " And 病人ID=" & lng病人ID & ")"
    Call OpenRecordset(rsTemp, "提取结算流水号")
    str结算流水号 = rsTemp!支付顺序号
    
'    入口参数 (Data)
'    是否有医保卡 IC信息(可以传空) + 空 + 空 + 要作废的住院结算交易号 + 要作废的住院号
    strInput = "1" & strField & strField & strField & strField & str结算流水号 & strField & str住院号
...
Public Function 个人余额_宁海(ByVal lng病人ID As Long) As Currency
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select Nvl(帐户余额,0) From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_宁海
    Call OpenRecordset(rsTemp, "提取帐户余额")
