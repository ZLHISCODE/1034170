...
Public Function 医保初始化_涪陵() As Boolean
    ...
    gstrSQL = "Select 参数名,参数值 From 保险参数 Where 险类=" & TYPE_涪陵
...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_涪陵
...
Public Function 门诊结算_涪陵(lng结帐ID As Long, cur个人帐户 As Currency, str医保号 As String, cur全自付 As Currency) As Boolean
...
checkCard:
    ...
    If 费用明细传递_涪陵(lng结帐ID) = False Then
        MsgBox "记录上传时发生错误", vbInformation, gstrSysName
        Exit Function
    End If
...
'    strBillNo = CStr(lng病人ID) & "_" & CStr(rsTemp("主页ID"))
    gstrSQL = "Select nvl(顺序号,0) as 顺序号,病种ID,中心,退休证号 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_涪陵
    Call OpenRecordset(rsTemp, gstrSysName)
    str处方号 = rsTemp!退休证号
    str就诊编号 = rsTemp!顺序号
...
checkCard:
    ...
    initType
    mblnReturn = reg(gstr医保机构编码, gstr医院编码, 1, UserInfo.姓名, Format(datCurr, "yyyy-MM-dd"), "0", gstrOutPara)
    TrimType
    If mblnReturn = False Then
        MsgBox gstrOutPara.errtext, vbInformation, gstrSysName
        入院登记_涪陵 = False
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "Select * from 保险帐户 where 病人ID=" & lng病人ID & " And 险类=" & TYPE_涪陵
    Call OpenRecordset(rsTemp, gstrSysName)
    str就诊编号 = Nvl(rsTemp!顺序号, "0")
...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    Dim cur大额统筹限额 As Currency, cur基数自付 As Currency, cur余额 As Currency
    Dim cur发生费用 As Currency, cur统筹支付 As Currency, cur补充医保 As Currency, cur全自付 As Currency, cur先自付 As Currency
    Dim lng主页ID As Long
...
checkCard:
    ...
    gstrSQL = "Select * From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Rownum<2"
    Call OpenRecordset(rs明细, gstrSysName)
    ...
    gstrSQL = " Select A.出院日期 From 病案主页 A,病人信息 B" & _
              " Where A.病人ID=B.病人ID And A.主页ID=B.住院次数 And A.病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, gstrSysName)
    ...
    datCurr = zlDatabase.Currentdate
    initType
    mblnReturn = calc(gstr医保机构编码, gstr医院编码, str就诊编号, str出院日期, str操作员, "0", gstrOutPara)
    TrimType
    If mblnReturn = False Then
        MsgBox gstrOutPara.errtext, vbInformation, gstrSysName
        住院结算_涪陵 = False
...
checkCard:
    ...
    gstrSQL = "Select nvl(顺序号,0) as 顺序号,中心 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_涪陵
    Call OpenRecordset(rsTemp, gstrSysName)
    str就诊编号 = rsTemp!顺序号
'    gstr医保机构编码 = rsTemp!中心
    '医保机构编码, 医院编号, 医保就诊编号， 出院日期，操作员，显示标志
...
'功能：通过医保中心号码和医保号求出病人ID
    ...
    On Error GoTo errHandle
    gstrSQL = "select 病人ID from 保险帐户 where 险类 = '" & TYPE_涪陵 & _
            "' and 医保号 = '" & str医保号 & "'"
    Call OpenRecordset(rsTmp, gstrSysName)
...
Public Function 记帐传输_涪陵(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人ID As Long = 0) As Boolean
    ...
    If str单据号 <> "" Then
        '可能存在一张单据内有多个病人的情况
        gstrSQL = "Select Distinct 病人ID From 病人费用记录 Where 记录状态<>0 And Nvl(是否上传,0)=0 And nvl(附加标志,0)<>9 and 记录性质=" & int性质 & " and NO='" & str单据号 & "'"
        Call OpenRecordset(rsPatient, "提取病人信息")
        With rsPatient
            Do While Not .EOF
                '判断是否是医保病人
                gstrSQL = "Select Count(*) Records From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & !病人ID & " And A.主页ID=B.住院次数 And A.险类=" & TYPE_涪陵
                Call OpenRecordset(rsTemp, "判断是否医保病人")
                If (rsTemp!Records = 1) Then
                    '提取指定病人，指定单据明细
                    gstrSQL = "Select * From 病人费用记录 Where 记录状态<>0 And Nvl(是否上传,0)=0 And nvl(附加标志,0)<>9 and 记录性质=" & int性质 & " and NO='" & str单据号 & "' And 病人ID=" & !病人ID & " order by 主页ID,序号"
    ...
    Else
        ...
        gstrSQL = "Select * From 病人费用记录 Where 记录状态<>0 And Nvl(是否上传,0)=0 And nvl(附加标志,0)<>9 and 病人id=" & lng病人ID & " And 主页id=" & lng主页ID & " order by 主页ID,序号"
        Call OpenRecordset(rsTemp, gstrSQL)
        记帐传输_涪陵 = 费用明细传递_涪陵(0, rsTemp)
    End If
End Function
