...
Private Sub cmd社保机构_Click()
    ...
    For i = 0 To UBound(strArr)
        ...
        gstrSQL = "Select 序号 From 保险中心目录 where 险类=" & gintInsure & "  and 编码 ='" & strArr1(0) & "'"
        zlDatabase.OpenRecordset rsTemp, gstrSQL, Me.Caption
        If rsTemp.EOF Then
            '新增:
            gstrSQL = "Select nvl(Max(序号),0)+1 as 序号 from 保险中心目录 where 险类=" & gintInsure
            zlDatabase.OpenRecordset rsTemp, gstrSQL, Me.Caption
            If rsTemp.EOF Then
                lng序号 = 1
            Else
            ...
            End If
              gstrSQL = "zl_保险中心目录_Insert(" & gintInsure & "," & lng序号 & ",'" & strArr1(0) & "','" & strArr1(1) & "')"
        Else
              gstrSQL = "zl_保险中心目录_Update(" & gintInsure & "," & Nvl(rsTemp!序号, 0) & ",'" & strArr1(0) & "','" & strArr1(1) & "')"
        End If
...
Private Function SaveData() As Boolean
    ...
    gstrSQL = "zl_保险参数_Insert(" & TYPE_广元旺苍 & ",null,'检查拨号连接','" & IIf(chk拨号.Value = 1, 1, 0) & "',4)"
    Call ExecuteProcedure(Me.Caption)
...
Private Sub LoadCbo()
        ...
        Dim rsTemp As New ADODB.Recordset
        Dim i As Long
        gstrSQL = "Select * From 保险中心目录 where 险类=" & gintInsure & " and 序号<>0 order by 编码"
