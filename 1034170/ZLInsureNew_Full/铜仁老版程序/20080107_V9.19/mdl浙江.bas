...
'功能：获得保险参数
    ...
    gstrSQL = "select A.参数名,A.参数值 from 保险参数 A " & _
              " where A.参数名='" & str参数名 & "' and A.险类=" & TYPE_浙江 & " and A.中心 is null "
...
'功能：通过医保中心号码和医保号求出病人ID
    ...
    gstrSQL = "select 病人ID from 保险帐户 where 险类 = '" & gintInsure & "' and 医保号 = '" & str医保号 & "'"
...
Public Function openConn浙江() As Boolean
    ...
    '首先读出参数，打开连接
    gstrSQL = "Select 参数名,参数值 From 保险参数 Where 险类=" & TYPE_浙江
    Call OpenRecordset(rsTemp, gstrSysName)
    Do Until rsTemp.EOF
...
'功能：测试是否可以连接到前置服务器上
    ...
    '如果连接已经打开，那就不用再测试
    If gcn浙江.State = adStateOpen Then
        gstrSQL = "Select * From 保险中心目录 Where 险类=" & gintInsure
        Call OpenRecordset(rsTemp, "医保初始化")
        gstr医保机构编码 = rsTemp!编码
        gstrSQL = "Select * From 保险类别 Where 序号=" & gintInsure
        Call OpenRecordset(rsTemp, "医保初始化")
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    End If
    str就诊编号 = rsTemp!备注
    strTemp = "99|" & str就诊编号 & "|" & Trim(gstr医院编码) & "|"
    WriteInfo "调用：" & strTemp
    '调用接口数冲销
    glngReturn = BUSINESS_HANDLE(strTemp, gstrInfo)
    WriteInfo "返回：" & gstrInfo
...
Public Function 费用明细传递_浙江(lng结帐ID As Long, Optional rs明细IN As ADODB.Recordset = Nothing, Optional str住院号 As String = "") As Boolean
    Dim lng病人ID  As Long, rs明细 As New ADODB.Recordset, rsTemp As New ADODB.Recordset, _
        ...
        str收费类别 As String, str药品等级 As String, str自理金额 As String, cur自付比例 As Currency
'    病人ID         adBigInt, 19, adFldIsNullable
'    收费类别       adVarChar, 2, adFldIsNullable
'    收据费目       adVarChar, 20, adFldIsNullable
'    计算单位       adVarChar, 6, adFldIsNullable
    ...
    Do Until rs明细.EOF
        ...
        Select Case rsTemp!类别
            ...
            Case Else
                str收费类别 = "91"
        End Select
        gstrSQL = "Select * From 保险支付项目 Where 险类=" & gintInsure & " And 是否医保=1 And 收费细目ID=" & rs明细!收费细目ID
'        gstrSQL = "Select A.*,B.名称 As 大类名称,B.编码 As 大类编码 from 保险支付项目 A,保险支付大类 B Where A.险类=B.险类 And " & _
            "A.大类ID=B.ID And A.险类=" & gintInsure & " And A.是否医保=1 And A.收费细目ID=" & rs明细!收费细目ID
        Call OpenRecordset(rsTemp, gstrSysName)
        If rsTemp.EOF Then
            str明细内码 = str明细编码
        ...
        Else
            ...
            Else
                ...
                If Nvl(rsTemp!大类id, "") <> "" Then
                    gstrSQL = "Select * From 保险支付大类 Where ID=" & rsTemp!大类id & " And 险类=" & gintInsure
                    Call OpenRecordset(rs大类, gstrSysName)
...
'            str明细名称 = Nvl(rsTemp!项目名称, str明细名称)
        ...
        If str药品等级 = "2" Then
            If str收费类别 = "11" Or str收费类别 = "12" Or str收费类别 = "13" Then
                strTemp = "Select AKA069 From KA02 Where AKA060='" & str明细内码 & "'"
            Else
                strTemp = "Select AKA069 From KA03 Where AKA090='" & str明细内码 & "'"
            End If
        ...
        If rs明细.Fields.Count < 26 Then
            strTemp = "Insert Into KC22 (AKB020,AKC190,CKC250,AAE072,CKC130,CKC131,AKC515,AKC221,AKA063,AKC220," & _
                "AKC222,AKC223,AKC224,AKA065,AKC225,AKC226,AKC227,AKC228,AKC253) Values ('" & Trim(gstr医院编码) & "','" & _
                ...
                str药品等级 & "'," & rs明细!单价 & "," & rs明细!数量 & "," & rs明细!实收金额 & "," & IIf(str药品等级 = "3", "0," & str自理金额, str自理金额 & ",0") & ")"
        Else
            gstrSQL = "Select * From 病人费用记录 Where NO='" & rs明细!NO & "' And 序号=" & rs明细!序号 & " And 门诊标志=2 And 记录性质=" & rs明细!记录性质 & " And 记录状态=" & rs明细!记录状态
            Call OpenRecordset(rsTemp, gstrSysName)
            strTemp = "Insert Into KC22 (AKB020,AKC190,CKC250,AAE072,CKC130,CKC131,AKC515,AKC221,AKA063,AKC220," & _
                "AKC222,AKC223,AKC224,AKA065,AKC225,AKC226,AKC227,AKC228,AKC253) Values ('" & Trim(gstr医院编码) & "','" & _
                str住院号 & "','" & iLoop & "','" & str门诊号 & "',1,'" & str处方号 & "','" & str明细编码 & "',to_date('" & _
                ...
                str门诊号 & "','" & str明细内码 & "','" & str明细名称 & "','" & IIf(InStr("11 12", str收费类别) > 0, "0", IIf(str收费类别 = "13", "1", "2")) & "','" & _
                str药品等级 & "'," & rs明细!价格 & "," & rs明细!数量 & "," & rs明细!金额 & "," & IIf(str药品等级 = "3", "0," & str自理金额, str自理金额 & ",0") & ")"
        End If
        WriteInfo strTemp
        If rs明细.Fields.Count >= 26 Then
