...
Private Function MakeFile_Recipe(ByVal rsDetail As ADODB.Recordset, Optional ByVal bln门诊 As Boolean = True, _
    Optional ByVal bln预结算 As Boolean = True) As Boolean
        ...
        For intDo = 1 To 2
            ...
            Do While Not .EOF
                ...
                If bln上传 Then
                    ...
                    If rsItem.EOF Then
                        MsgBox "第" & !序号 & "行的明细记录未找到对应的保险项目，请检查！", vbInformation, gstrSysName
                        Exit Function
                    End If
...
Private Sub Get处方流水号(ByVal strNO As String, ByVal str性质 As String, ByVal str状态 As String, _
    ...
    If str状态 = 1 Then
        ...
        gstrSQL = " Select 病人ID,主页ID,收费细目ID,Nvl(标准单价,0) 单价,NVl(实收金额,0) 金额" & _
                  " From 病人费用记录" & _
                  " Where NO='" & strNO & "' And 记录性质=" & str性质 & " And 记录状态=" & str状态 & " And 序号=" & str序号
        Call OpenRecordset(rsHandback, "提取该明细，看是否是负数记帐")
    ...
    Else
        If lng病人ID <> 0 Then
            ...
            gstrSQL = " Select 摘要 From 病人费用记录 " & _
                      " Where 结帐ID=" & gComInfo_重庆银海版.冲销ID & " And 序号=" & Val(str序号)
            Call OpenRecordset(rsHandback, "从保险结算记录中取出摘要")
...
Public Function 门诊结算_重庆银海版(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    blnTrans = True
    '取出本次所有费用明细
    gstrSQL = "Select ID From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Nvl(记录状态,0)<>0 Order by 序号"
    Call OpenRecordset(rs明细, "取出本次所有费用明细")
    '计算循环次数，因为每个文件上传的记录数只有20条
...
Public Function 入院登记撤销_重庆银海版(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select 1 From 病人费用记录 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID & " And Rownum<2"
        ...
        If bln上传 Then
            ...
            For intDo = 1 To 2
                ...
                Do While Not rsExse.EOF
                    ...
                    If Not blnTrans Then gcn重庆.BeginTrans: blnTrans = True
                    ...
                    If rsRecipe.RecordCount <> 0 Then
                        ...
                        rsRecipe.Filter = 0
                        rsRecipe.MoveFirst
                        If Not MakeFile_Recipe(rsRecipe, False, False) Then
                            .Filter = 0
                            rsRecipe.Filter = 0
                            gcn重庆.RollbackTrans
                            gcn上传.RollbackTrans
...
Private Function GetSequence(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng收费细目ID As Long) As String
    ...
    gstrSQL = " Select NO,记录性质,记录状态,序号 From 病人费用记录" & _
