...
Private Sub Bill_CommandClick(Index As Integer)
    ...
    With Bill(Index)
        If Bill(Index).Col = col_医疗机构 Then
            ...
            Call SQLTest(App.Title, "ZL9INSURE\保险参数设置", gstrSQL): rsTemp.Open gstrSQL, gcnBJYB: Call SQLTest
            ...
            Else
                ...
                If rsTemp.RecordCount > 1 Then
                    ...
                    blnReturn = frmListSel.ShowSelect(rsTemp, "医院编码", "医院等级选择", "请选择医院等级：")
                Else
                    blnReturn = True
                End If
            End If
...
Private Sub cmdOK_Click()
    ...
    '先取出该病人的当前状态
    intCurState = 0
    gstrSQL = "Select Nvl(当前状态,0) 当前状态 From 保险帐户 Where 卡号='" & txt确认卡号.Text & "' And 险类=" & TYPE_北京
    Call OpenRecordset(rsTemp, "先取出该病人的当前状态")
    If Not rsTemp.EOF Then intCurState = rsTemp!当前状态
