...
Public Function GetIdentifyMode(ByVal bytType As Byte, str卡类型 As String, lng疾病ID As Long, str疾病编码 As String) As Boolean
'功能：获得身份验证的模式
...
'    If bytType = 0 Or bytType = 1 Then
        '门诊与住院都可能要选择疾病
'        gstrSQL = "select 参数值 from 保险参数 where 险类=" & gintInsure & " and 中心=0 and 参数名='支持慢性病、特种病'"
    ...
    Else
        ...
        If bytType = 0 Then
            ...
            gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
                    " From 保险病种 A where A.类别 in (0,1,2) and A.险类=" & gintInsure
        Else
            '住院，使用普通病
            'Modified by ZYB 2004-10-12 昆明
            '-------------------------------
            gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
                    " From 保险病种 A where A.类别 in (0,1,2) and A.险类=" & gintInsure & IIf(gintInsure = TYPE_云南建水, "", " And A.编码='0094'")
        End If
