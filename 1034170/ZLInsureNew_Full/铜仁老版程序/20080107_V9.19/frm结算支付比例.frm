    ...
    With msh比例
        For lngRow = .FixedRows To .Rows - 1
            For lngCol = .FixedCols To .Cols - 1
                ...
                strSects = strSects & .RowData(lngRow) & ";"  '在职
                
                If .RowData(lngPreRow) <> .RowData(lngRow) Then
                    lngPreRow = lngRow
                    lng年龄段 = 1
                Else
                    '同一人群
                    lng年龄段 = lngRow - lngPreRow + 1
                End If
                
                strSects = strSects & lng年龄段 & ";"  '年龄段
                strSects = strSects & lngCol & ";"            '费用档
                strSects = strSects & Val(.TextMatrix(lngRow, lngCol)) & ";"
            Next
        Next
    End With
    
    On Error GoTo ErrHand
    gstrSQL = "zl_保险支付比例_Update(" & mlng险类 & "," & mlng中心 & "," & mlng年度 & ",'" & strSects & "')"
...
Private Sub txtInput_Validate(Cancel As Boolean)
    ...
    With msh比例
        ...
        If Val(txtInput.Text) <= 0 Or Val(txtInput.Text) > 100 Then
            MsgBox "比例必须大于0且小于等于100。", vbInformation, gstrSysName
            DoEvents
            Cancel = True
            txtInput.Visible = True
            zlControl.TxtSelAll txtInput
...
'返回值:编辑成功返回True,否则为False
    ...
    mblnChange = False
    frm结算支付比例.Show vbModal, frm结算规则
    编辑支付比例 = mblnOK
End Function
