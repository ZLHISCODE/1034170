...
'返回：
    ...
    cbo类别.Clear
    gstrSQL = "select A.序号,A.名称 from 保险人群 A where A.险类=" & gintInsure
    Call OpenRecordset(rsTemp, Me.Caption)
    Do Until rsTemp.EOF
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & gintInsure & " and 中心=0"
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & gintInsure & " and 中心=" & lng中心 & " and 医保号='" & Trim(txtEdit(Text卡号).Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
