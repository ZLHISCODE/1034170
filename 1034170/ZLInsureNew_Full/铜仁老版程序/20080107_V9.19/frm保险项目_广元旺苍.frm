        ...
        For lngRow = 1 To .Rows - 1
            Select Case .TextMatrix(lngRow, col改变方式)
                Case "新增", "修改"
                    ...
                    gstrSQL = "ZL_医保支付项目_Modify(" & .RowData(lngRow) & "," & gintInsure & "," & mint中心 & "," & _
                ...
                Case "删除"
                    ...
                    gstrSQL = "ZL_医保支付项目_Delete(" & .RowData(lngRow) & "," & gintInsure & "," & mint中心 & ")"
                    Call ExecuteProcedure_广元旺苍(Me.Caption)
                    .TextMatrix(lngRow, col原编码) = .TextMatrix(lngRow, COL医保编码)
            End Select
        Next
...
Private Sub Form_Load()
    ...
    gstrSQL = "select 序号,名称 from 保险中心目录 where 序号<>0 and 险类=" & gintInsure & " order by 序号"
...
'返回：医保项目编码
    ...
    With mshSum_S
        ...
        If frm保险项目选择广元旺苍.GetCode(strCode, mint中心) = True Then
            strSelected = strCode
        End If
            ...
            If .TxtVisible = True Then
                ...
                Else
                    ...
                    gstrSQL = "Select 编码  医保编码,名称,简码,附注 " & _
                             "   FROM 医保收费项目 WHERE 险类=" & gintInsure & " and 中心=" & mint中心 & " and (编码 like '" & strText & "%' or Upper(名称) like '" & str前 & UCase(strText) & "%' Or Upper(简码) like '" & str前 & UCase(strText) & "%')"
                    Call OpenRecordset(rsTemp, Me.Caption)
...
'功能：根据当前行的保险项目编码，得到其它信息
    ...
    With mshSum_S
        gstrSQL = "select 名称,大类编码,附注 from 医保收费项目 where 编码='" & .TextMatrix(.Row, COL医保编码) & _
                  "' and 险类=" & gintInsure & " and 中心=" & cmb险类.ItemData(cmb险类.ListIndex)
        Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub Fill大类()
    ...
    gstrSQL = "select ID,编码,名称 from 保险支付大类 " & _
              "where 险类=" & gintInsure & " order by 编码"
    Call OpenRecordset(rsTemp, Me.Caption)
