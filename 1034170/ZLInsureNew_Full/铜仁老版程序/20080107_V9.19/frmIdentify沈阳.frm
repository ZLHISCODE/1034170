...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select nvl(当前状态,0) as 状态,顺序号 from 保险帐户 where 险类=" & TYPE_沈阳市 & " and 医保号='" & txt医保号 & "'"
    ...
    If txt疾病信息.Tag = "" Then
        ...
        Exit Sub
    End If
    gstrSQL = "Select ID From 保险病种 Where 编码='" & txt疾病信息.Tag & "'"
    Call OpenRecordset(rsTemp, "获取保险病种")
    ...
    If mbytType = 0 And bln在院 = False Then
        gstrSQL = "Select nvl(参数值,缺省值) 挂号有效天数 From 系统参数表 Where 参数名='挂号有效天数'"
        Call OpenRecordset(rsTemp, "获取挂号有效天数")
        intDays = Nvl(rsTemp!挂号有效天数, 0) - 1       '算当天，因此要减1
        ...
        If intDays > -1 Then
            ...
            gstrSQL = " Select 1 From 病人费用记录" & _
                      " Where 记录性质=4 And 记录状态=1 And 病人ID=" & mlng病人ID & " And 发生时间>To_date('" & strBeginDate & "','yyyy-MM-dd')"
            Call OpenRecordset(rsTemp, "取规定的时间内，该病人有无挂号记录")
            If rsTemp.EOF Then
    ...
    If mbytType = 1 Then
        ...
        Call ExecuteProcedure("保存住院业务类型")
    End If
...
'        4   newpassword新密码      6   否
        If 调用接口_准备_沈阳市(Function_沈阳市.其他_修改密码) Then
            ...
            If 调用接口_执行_沈阳市() Then
                '更新个人帐户中的信息
                gstrSQL = "zl_保险帐户_更新信息(" & mlng病人ID & "," & gintInsure & ",'密码','''" & mstr新密码 & "''')"
                Call ExecuteProcedure("更新密码")
            Else
...
Private Sub cmd疾病信息_Click()
    ...
    Else
        If mrs病种.RecordCount > 0 Then
            If frmListSel.ShowSelect(mrs病种, "ID", "特殊病种选择", "请选择特定的医保病种：") = True Then
...
Private Sub Form_Load()
    ...
    '取住院病人是否允许门诊业务
    gstrSQL = "Select Nvl(参数值,0) 参数值 From 保险参数 Where 序号=7 And 险类=" & TYPE_沈阳市
    Call OpenRecordset(rsTemp, "取住院病人是否允许门诊业务")
    If Not rsTemp.EOF Then
    ...
    If mbytType = 3 Then
        ...
        Call OpenRecordset(rsTemp, "读取门诊挂号缺省病种")
        If rsTemp.EOF Then
    ...
    If Not bln特殊病 Then
        ...
        Else
            ...
            If rsTemp.RecordCount > 1 Then
                ...
                blnReturn = frmListSel.ShowSelect(rsTemp, "ID", "医保病种选择", "请选择医保病种：")
            Else
                blnReturn = True
            End If
        End If
    Else
        ...
        Else
            If mrs病种.RecordCount > 1 Then
                blnReturn = frmListSel.ShowSelect(mrs病种, "ID", "特殊病种选择", "请选择特定的医保病种：")
            Else
                blnReturn = True
            End If
        End If
...
'        5   password       密码            6   否
    ...
    '填写入口参数
    'Modified By 朱玉宝 地区：长沙 原因：将传入参数由卡号改为身份证号
    Call CZ_DataPut(glngInterface_沈阳市, 1, "indi_id", str个人编号)
...
'（规定病）    24  disease        疾病名称            60  申请的门诊规定病的疾病名称
    ...
    Case 业务分类_沈阳市.门诊规定病
        ...
        If mbln多病种 Then
            ...
            Do While True
                ...
                gstrSQL = " Select ID From 保险病种" & _
                          " Where 险类=" & TYPE_沈阳市 & " And 编码='" & str编码 & "'"
                Call OpenRecordset(rsTemp, "是否存在此病种")
                If rsTemp.RecordCount = 0 Then
        ...
        ElseIf mrs病种.RecordCount > 1 Then
            If frmListSel.ShowSelect(mrs病种, "ID", "疾病选择", "请选择医保病种：") = True Then
                txt疾病信息.Tag = mrs病种!编码
                txt疾病信息.Text = "(" & mrs病种!编码 & ")" & mrs病种!名称
                lbl疾病信息.Tag = txt疾病信息.Text '用于恢复显示
