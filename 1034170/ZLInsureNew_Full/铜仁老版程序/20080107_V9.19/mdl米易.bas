...
'需要在身份验证时: 显示待遇审批信息结算
'结算时，经办人（aae011）固定为"999999999999999"
'住院不支持中途结算 , 通过模拟结算实现
'必须以门诊的方式下个人帐户
...
Public Function 医保初始化_米易() As Boolean
    ...
    Call OpenRecordset(rs服务机构编号, "获取医院编码")
    
    '由于圈存机编号与服务机构编号一致，所以同时对它赋值
    gComInfo_米易.服务机构编号 = Nvl(rs服务机构编号!医院编码, "")
    gComInfo_米易.圈存机编号 = gComInfo_米易.服务机构编号
    
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & gintInsure
    Call OpenRecordset(rs保险参数, "获取参数")
    gintComPort = 1
...
Public Function 身份标识_米易(Optional bytType As Byte, Optional lng病人ID As Long) As String
    ...
    '强制把报销比例等数据填入
    gstrSQL = "Select 病人ID From 保险帐户 Where 险类=" & gintInsure & " And 医保号='" & gComInfo_米易.个人编号 & "'"
    Call OpenRecordset(rsPatient, "读取医保病人的基本信息")
...
Public Function 个人余额_米易(Optional ByVal bln读卡 As Boolean = False, Optional ByVal strSelfNo As String = "", Optional ByVal bln住院 As Boolean = False) As Currency
    ...
    '读卡失败则退出
    gstrSQL = "Select Nvl(帐户余额,0) 帐户余额 From 保险帐户 Where 险类=" & type_米易
    If bln读卡 Then
    ...
    Else
        gstrSQL = gstrSQL & " And 医保号='" & strSelfNo & "'"
    End If
...
Private Function ReadICCard(Optional ByVal bln住院 As Boolean = False) As Long
    ...
    gstrSQL = "Select 病人ID From 保险帐户 Where 险类=" & gintInsure & " And 医保号='" & gComInfo_米易.个人编号 & "'"
    Call OpenRecordset(rsTemp, "取该医保病人的病人ID")
...
Public Function 门诊虚拟结算_米易(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    With rs明细
        ...
        Do While Not .EOF
            ...
            Call OpenRecordset(rsTemp, "判断是否设置了对应的医保编码")
    ...
    gstrSQL = "Select count(*) Records From 保险结算记录 Where 性质=1 And 险类=" & gintInsure & _
            " And 结算时间 Between to_date('" & strDate & " 00:00:00" & "','yyyy-MM-dd hh24:mi:ss')" & _
            " And to_date('" & strDate & " 23:59:59" & "','yyyy-MM-dd hh24:mi:ss') And 支付顺序号='" & gComInfo_米易.就诊编号 & "'"
    Call OpenRecordset(rsTemp, "检查是否存在本次就诊编号")
...
Public Function 门诊结算_米易(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    gstrSQL = " Select A.病人ID,Nvl(B.类别,0) 特病种 " & _
              " From 保险帐户 A,(Select * From 保险病种 Where 险类=" & gintInsure & ") B " & _
              " Where A.险类=" & gintInsure & " And A.医保号='" & strSelfNo & "' And A.病种ID=B.ID(+)"
    Call OpenRecordset(rsTemp, "判断是否是特种病")
    lng病人ID = rsTemp!病人ID
    bln特种病 = (rsTemp!特病种 <> 0)
    '门诊特种病不由医院端支付
        ...
        If Not 调用接口_米易("dataupload") Then
            ...
            Do While 1
                If WriteICCard(lng病人ID, cur个人帐户) Then
                    Exit Do
                Else
                    MsgBox "写卡不成功，请插卡！", vbInformation, gstrSysName
                End If
            Loop
            Exit Function
        End If
...
Public Function 出院登记_米易(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) Then
        ...
        gstrSQL = "select b.名称 出院科室,床号,终止时间,操作员姓名  " & _
                 ...
                 " and A.科室ID=B.ID"
        Call OpenRecordset(rsTemp, "出院情况")
    ...
    With rsExse
        ...
        Do While Not .EOF
            ...
            If blnUpload Then
                ...
                '取该收费细目的编码与名称
                gstrSQL = "Select 编码 项目编码 ,名称 项目名称 From 收费细目 Where ID = " & !收费细目ID
                Call OpenRecordset(rsTemp, "取该收费细目的编码与名称")
        ...
        gstrPara_米易 = GetParaCode(个人编号, gComInfo_米易.个人编号) & GetParaCode(卡号, gComInfo_米易.卡号) & _
            ...
            GetParaCode(病种编码, gComInfo_米易.病种编码) & GetParaCode(经办人, "999999999999999") & _
...
Public Function 住院结算_米易(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    If cur个人帐户 <> 0 Then
        ...
        If blnError Then
            '不能退出，提示以现金支付
            gcnOracle.Execute "Delete 保险结算记录 Where 性质=2 And 险类=" & gintInsure
            ...
            Call ExecuteProcedure("保存住院结算数据")
            MsgBox "下个人帐户失败，注意收取现金（" & Format(cur个人帐户, "#####0.00;-#####0.00; ;") & "）", vbInformation, gstrSysName
        End If
    End If
    Call 医保终止_米易
...
Public Function 住院结算冲销_米易(lng结帐ID As Long) As Boolean
    ...
    Call OpenRecordset(rsTemp, "米易医保")
...
Private Sub 费用分隔(ByVal lng病人ID As Long, lng主页ID As Long)
    ...
    gstrSQL = "SELECT SUM(发生费用金额) 发生费用金额,SUM(进入统筹金额) 进入统筹金额,SUM(统筹报销金额) 统筹报销金额, " & _
             ...
             " FROM  " & _
             "      (SELECT Distinct 病人ID,结帐ID FROM 病人费用记录 " & _
             "      WHERE 病人ID=" & lng病人ID & " AND 主页ID= " & lng主页ID & _
             "      ) A,保险结算记录 B " & _
             " WHERE A.病人ID=B.病人ID AND B.记录ID=A.结帐ID AND B.险类=" & gintInsure & " AND B.性质=2 " & _
             " GROUP BY A.病人ID"
    Call OpenRecordset(rsTemp, "获取本次住院期间以往结算的费用总额及统筹报销总额")
    If Not rsTemp.EOF Then
...
Public Sub 获取病种编码(ByVal lng病种ID As Long)
    ...
    gstrSQL = "Select nvl(类别,0) 类别 From 保险病种 Where 险类=" & gintInsure & " And ID=" & lng病种ID
    Call OpenRecordset(rsTemp, "获取病种信息")
    If Not rsTemp.EOF Then
        If rsTemp!类别 <> 0 Then
...
Public Sub 核对帐户支付_米易()
    ...
    gstrSQL = "Select SUM(冲预交) 个人帐户 " & _
        ...
        " And 收款时间 Between to_date('" & str开始日期 & "','yyyy-MM-dd hh24:mi:ss')" & _
        " And to_date('" & str结束日期 & "','yyyy-MM-dd hh24:mi:ss')"
    Call OpenRecordset(rsAccount, "统计帐户支付总额")
...
Public Sub 核对门诊结算_米易()
    ...
    gstrSQL = "Select SUM(发生费用金额) 发生费用金额,SUM(首先自付金额) 首先自付金额 ,SUM(个人帐户支付) 个人帐户支付" & _
        " From 保险结算记录 " & _
        " Where 性质=1 " & _
        " And 结算时间 Between to_date('" & str开始日期 & "','yyyy-MM-dd hh24:mi:ss')" & _
        " And to_date('" & str结束日期 & "','yyyy-MM-dd hh24:mi:ss')"
    Call OpenRecordset(rsAccount, "统计门诊结算")
    If Not rsAccount.EOF Then
...
Public Sub 核对住院结算_米易()
    ...
    gstrSQL = "Select SUM(统筹报销金额) 统筹支付金额,SUM(进入统筹金额-统筹报销金额) 统筹自付金额 " & _
        ...
        " And 结算时间 Between to_date('" & str开始日期 & "','yyyy-MM-dd hh24:mi:ss')" & _
        " And to_date('" & str结束日期 & "','yyyy-MM-dd hh24:mi:ss')"
    Call OpenRecordset(rsAccount, "统计统筹支付总额")
    If Not rsAccount.EOF Then
...
Private Function FUNC_就诊编号(ByVal str开始日期 As String, ByVal str结束日期 As String, _
    ...
    gstrSQL = gstrSQL & _
             " From 保险结算记录 A" & _
             " Where A.险类=" & type_米易 & _
             " And 结算时间 between to_date('" & str开始日期 & "','yyyy-MM-dd hh24:mi:ss')" & _
             " And to_date('" & str结束日期 & "','yyyy-MM-dd hh24:mi:ss')"
    Call OpenRecordset(rs顺序号, "获取就诊编号")
...
Public Sub 补充上传门诊明细()
    ...
    gstrSQL = "Select A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,A.登记时间,A.开单人 as 医生," & _
            ...
            " From (Select * From 病人费用记录 Where 记录性质=1 And Nvl(是否上传,0)=0 And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E,保险结算记录 F,保险帐户 G,(Select * From 保险病种 Where 险类=" & gintInsure & ") H " & _
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+) And A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类=" & gintInsure & _
...
Public Function 处方登记_米易(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    ...
    With rsTemp
        ...
        gstrSQL = " Select A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,to_char(A.登记时间,'yyyy-MM-dd hh24:mi:ss') 登记时间," & _
                  ...
                  " From 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & type_米易 & ") C " & _
                  " Where A.记录性质=" & lng记录性质 & " And A.记录状态=" & lng记录状态 & " And A.NO='" & str单据号 & "'" & _
    ...
    With rsExse
        ...
        Do While Not .EOF
            If lng病人ID <> !病人ID Then
                '检查本次是否以医保身份入院
                gstrSQL = "Select Count(*) Records From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & !病人ID & " And A.主页ID=B.住院次数 And A.险类=" & type_米易
