...
Private Sub cmdComp_Click()
    ...
    ElseIf mbytType = 0 Then                    '无刷卡器读卡时可以这样操作，如果改用读卡方式就需要进行修改
        gstrSQL = "Select * From 保险帐户 where 医保号='" & strVoucherID & "'"
...
'        psOthers1       :pChar;     13     // O其它(保留)[C200]
    ...
    intReturn = FGetCardInfo(intRecType, gstrRecCode, strVoucherID, intInsID, strArrInfo(0), strArrInfo(1), _
        strArrInfo(2), strArrInfo(3), strArrInfo(4), strArrInfo(5), strArrInfo(6), strArrInfo(7), strArrInfo(8), _
        strArrInfo(9), strArrInfo(10), strArrInfo(11), strArrInfo(12), strArrInfo(13), sngArrInfo(19), sngArrInfo(0), _
    ...
    If intReturn <> 0 Then
        ...
        Exit Sub
    End If
'    If Trim(gstrrecode) = "" Then
'        gstrSQL = "Select * From 保险帐户 where 医保号='" & strVoucherID & "'"
'        OpenRecordset rsTemp, gstrSysName
'        gstrRecCode = rsTemp!退休证号
'    End If
