...
'     gstrSQL = "zl_保险参数_Insert(" & mlng险类 & ",null,'住院明细时实上传' ,'" & IIf(chk实时(1).Value = 1, "1", "0") & "'," & 2 & ")"
    ...
    Call zlDataBase.ExecuteProcedure(gstrSQL, Me.Caption)
'    gstrSQL = "zl_保险参数_Update(" & mlng险类 & ",NULL,'医嘱明细时实上传' ,'" & IIf(chk实时(2).Value = 1, "1", "0") & "')"
'    Call zlDataBase.ExecuteProcedure(gstrSQL, Me.Caption)
...
Private Sub pic_Resize()
    ...
    Err = 0
    On Error Resume Next
    sngWidth = pic.ScaleWidth
    chk实时(1).Left = sngWidth - chk实时(1).Width - 50
    chk实时(0).Left = chk实时(1).Left - chk实时(0).Width - 100
End Sub
...
Private Function iniData() As Boolean
    ...
    With mshBill
        ...
        strSQL = "Select * From 保险参数 where 参数名 in('门诊明细时实上传','住院明细时实上传','医嘱明细时实上传') and 险类=" & mlng险类
        zlDataBase.OpenRecordset rsTmp, strSQL, Me.Caption
        chk实时(0).Value = 1
        chk实时(1).Value = 1
        Do While Not rsTmp.EOF
...
'            Case "医嘱明细时实上传"
'                chk实时(2).Value = IIf(Val(Nvl(rsTmp!参数值)) = 1, 1, 0)
            End Select
            rsTmp.MoveNext
        Loop
