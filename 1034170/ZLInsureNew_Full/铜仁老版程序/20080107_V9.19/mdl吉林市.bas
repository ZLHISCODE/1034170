...
Public Declare Function InhosRegister Lib "YhYbClient.dll" (ByVal int个人句柄 As Integer, _
    ...
    ByVal str入院诊断代码 As String) As Long
...
Public Function 医保初始化_吉林() As Boolean
    ...
    '取医院编码
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_吉林
...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select nvl(帐户余额,0) as 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_吉林
...
Public Function 门诊结算_吉林(lng结帐ID As Long, cur个人帐户 As Currency, str医保号 As String, cur全自付 As Currency) As Boolean
...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = " " & _
        ...
        "  From (Select * From 病人费用记录 Where 记录状态<>0 and 结帐ID=" & lng结帐ID & " and  Nvl(附加标志,0)<>9 ) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E,  " & _
        "       (Select distinct Q.药品id,Q.标识码,T.名称 From 药品目录 Q,药品信息 R,药品剂型 T  Where  Q.药名id=R.药名id and R.剂型=T.编码 ) J, " & _
        "       人员表 H,保险帐户 L" & _
        "  Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+)  and A.病人id=L.病人id  and L.险类=" & gintInsure & " and a.收费细目id=J.药品id(+) " & _
    ...
    rs明细.MoveFirst
    If dbl金额 = 0 Then
        MsgBox "病人没有发生费用,不能进行医保处理", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    gstrSQL = "Select * From 保险病种 Where ID=" & Nvl(rs明细!病种ID, 0) & " And 险类=" & gintInsure
    ...
    strInput = strInput & vbTab & Substr(str医保病种代码, 1, 10)
   ...
   '插入保险结算记录
    ...
    With g病人身份_吉林
        ...
        Call ExecuteProcedure("保存保险结算记录附加信息")
    End With
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    Call OpenRecordset(rs明细, gstrSysName)
    If rs明细.EOF Then
        MsgBox "没有找到原单据的明细数据，不能进行冲销", vbInformation, gstrSysName
        Exit Function
    End If
   ...
   '插入保险结算记录
    ...
    gstrSQL = "Select * From 保险结算记录 Where 记录ID=" & lng结帐ID & " And 险类=" & gintInsure
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
        MsgBox "没有找到原有的保险结算记录", vbInformation, gstrSysName
        Exit Function
    End If
...
Public Function 入院登记_吉林(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
    ...
    Call OpenRecordset(rsData, "读取床位信息")
    If rsData.EOF Then
        'aRoom：房间(10位)
        strInput = strInput & vbTab & ""
        'aBedNo：床号(3位)
    ...
    Else
        ...
        'aBedNo：床号(3位)
        strInput = strInput & vbTab & Nvl(rsData!床号)
    End If
...
Public Function 业务请求_吉林(ByVal intType As 业务类型_吉林, strInputString As String, strOutPutstring As String) As Boolean
    ...
    Select Case intType
        ...
        Case 初始化服务调用
           ...
           End If
        Case 入院登记
           lngReturn = InhosRegister(Val(strArr(0)), strArr(1), strArr(2), strArr(3), strArr(4), strArr(5), strArr(6), strArr(7), strArr(8))
           If lngReturn < 0 Then
...
Public Function 出院登记_吉林(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    '获取住院医师
    gstrSQL = "select A.入院日期,(sysdate-a.入院日期)/365 as 住院天数,b.当前床号,B.住院号,to_char(A.入院日期,'ss') as 序号,A.当前病区id,D.名称 as 住院科室,A.入院病床,A.住院医师,C.卡号," & _
            "C.密码,D.编码 As 科室编码,J.名称 as 病区,A.出院方式 from 病案主页 A,病人信息 B,保险帐户 C,部门表 D,部门表 J " & _
    ...
    Else
        ...
        strInput = strInput & Substr(Nvl(rsData!床号), 1, 3) & vbTab
    End If
...
Public Function 住院结算_吉林(lng结帐ID As Long) As Boolean
    '功能：将门诊收费的明细和结算数据转发送医保前置服务器确认；
    '参数：lng结帐ID     收费记录的结帐ID；，从预交记录中可以检索医保号和密码
    '      cur支付金额   从个人帐户中支出的金额
    '返回：交易成功返回true；否则，返回false
    ...
    Dim str病种代码 As String, str结算方式 As String
    ...
    gstrSQL = " " & _
        ...
        "               L.险类,l.中心 , l.卡号, l.医保号, l.人员身份, l.单位编码, l.顺序号, l.退休证号, l.帐户余额, l.当前状态, l.病种ID, l.在职, l.年龄段, l.灰度级, l.就诊时间 " & _
        "        from 病人费用记录 a,收费类别 b,病案主页 F1,人员表 c,收费细目 D,保险支付项目 E,保险帐户 L,部门表 H, " & _
        "             (Select distinct Q.药品id,Q.标识码,T.名称 From 药品目录 Q,药品信息 R,药品剂型 T  Where  Q.药名id=R.药名id and R.剂型=T.编码 ) J " & _
...
End Function
   ...
   strInput = strInput & vbTab & lng结帐ID
    ...
    With g病人身份_吉林
        gstrSQL = "zl_保险结算记录_insert( 2," & lng冲销ID & "," & gintInsure & "," & lng病人ID & "," & Format(zlDatabase.Currentdate, "YYYY") & "," & _
          -1 * Nvl(rsTemp!帐户累计增加, 0) & "," & -1 * Nvl(rsTemp!帐户累计支出, 0) & "," & -1 * Nvl(rsTemp!累计进入统筹, 0) & "," & -1 * Nvl(rsTemp!累计统筹报销, 0) & ",NULL," & -1 * Nvl(rsTemp!起付线, 0) & "," & -1 * Nvl(rsTemp!封顶线, 0) & "," & -1 * Nvl(rsTemp!实际起付线, 0) & "," & _
         -1 * Nvl(rsTemp!发生费用金额, 0) & "," & -1 * Nvl(rsTemp!全自付金额, 0) & "," & -1 * Nvl(rsTemp!首先自付金额, 0) & "," & _
          -1 * Nvl(rsTemp!进入统筹金额, 0) & "," & -1 * Nvl(rsTemp!统筹报销金额, 0) & "," & -1 * Nvl(rsTemp!大病自付金额, 0) & "," & -1 * Nvl(rsTemp!超限自付金额, 0) & "," & -1 * Nvl(rsTemp!个人帐户支付, 0) & "," & _
          "NULL,Null,Null,NULl)"
...
Public Function 处方登记_吉林(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    ...
    If lng记录状态 = 1 Then
        ...
        strInput = strInput & vbTab & Substr(dbl付数, 1, 10)
        'aAmount：金额(8位，2位小数)
        strInput = strInput & vbTab & Substr(dbl金额, 1, 10)
...
Private Function 补传明细记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = "" & _
            ...
            "   From 病人费用记录 A,部门表 B,病案主页 F,收费细目 G," & _
            "       (Select M.项目编码,M.收费细目id From 保险支付项目 M Where M.险类=" & TYPE_吉林 & ") C," & _
            "       (Select J.名称,O.药品id From 药品目录 O, 药品信息 H,药品剂型 J WHERE O.药名id=H.药名id and H.剂型=J.编码) K " & _
            "   where nvl(A.是否上传,0)=0 and A.记录状态<>0 and A.记帐费用=1 and A.结帐ID is null and A.实收金额<>0  and  " & _
            ...
            "   ORDER BY a.记录性质,A.NO,a.记录状态,A.序号"
...
Private Function GetSumJe(ByVal lng记录性质 As Long, ByVal strNO As String, ByVal lng记录状态 As Long, dbl中药付数 As Double, dbl总额 As Double) As Boolean
    ...
    gstrSQL = "" & _
            ...
            "           Sum(Nvl(A.实收金额,0))-Sum(Nvl(A.结帐金额,0)) as 金额 From 病人费用记录 a " & _
