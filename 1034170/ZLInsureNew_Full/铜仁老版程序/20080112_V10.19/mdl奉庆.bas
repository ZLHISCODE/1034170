...
Private Type InitbaseInfor
    ...
    解析卡内数据 As Boolean
End Type
Public InitInfor_奉庆 As InitbaseInfor
...
Public g病人身份_奉庆 As 病人身份
Public gcnOracle_奉庆 As ADODB.Connection     '中间库连接
Private gcnOracle_中心 As ADODB.Connection     '奉庆医保中心联接串
...
'4.读写卡组件对象声明
Public gobj奉庆 As Object
Public gobj奉庆Err As Object
Private Const STR_中心维护代码 = "1"

Public Function 医保初始化_奉庆() As Boolean
    ...
    If Val(strReg) = 1 Then
        InitInfor_奉庆.模拟数据 = True
    Else
        InitInfor_奉庆.模拟数据 = False
    End If
    
    Call GetRegInFor(g公共模块, "操作", "解析卡内数据", strReg)
    If Val(strReg) = 1 Then
        InitInfor_奉庆.解析卡内数据 = True
    Else
        InitInfor_奉庆.解析卡内数据 = False
    End If
    
    InitInfor_奉庆.解析卡内数据 = InitInfor_奉庆.解析卡内数据 Or InitInfor_奉庆.模拟数据
    ...
    Set gobj奉庆 = CreateObject("SiCard.SiCardOperator")
    If Err <> 0 Then
        If InitInfor_奉庆.模拟数据 Then
        Else
            ShowMsgbox "创建东软读写卡组失败!"
            Exit Function
        End If
    End If
    Set gobj奉庆Err = CreateObject("SiCommTool.SiErrorCtl")
    If Err <> 0 Then
        If InitInfor_奉庆.模拟数据 Then
        Else
            ShowMsgbox "创建东软读写卡组失败!"
    ...
    '取医院编码
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_临沧奉庆
    Call OpenRecordset(rsTemp, "读取医院编码")
    InitInfor_奉庆.医院编码 = Nvl(rsTemp!医院编码)
    ...
    If gInitCard Then
        '初始化医保接口
        If 业务请求_奉庆(奉庆_医保初始化, "", strOutput) = False Then
            Exit Function
        End If
    End If
    mblnInit = True
...
Private Function Open中间库() As Boolean
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_临沧奉庆
    ...
    If OraDataOpen(gcnOracle_奉庆, strServer, strUser, strPass, False) = False Then
    ...
    '中间库连接
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '中心%' and 险类=" & TYPE_临沧奉庆
    Call OpenRecordset(rsTemp, "渝北医保")
    Do Until rsTemp.EOF
        Select Case rsTemp("参数名")
            Case "中心用户名"
...
End Function
Public Function 解析卡_奉庆() As Boolean
    '-----------------------------------------------------------------------------------------------------------
    '--功  能:解析卡内数据
...
Private Function sCard_CloseCardWithoutSave() As Boolean
    ...
    Err = 0
    On Error GoTo ErrHand:
    If gobj奉庆 Is Nothing Then Exit Function
    lngReturn = gobj奉庆.CloseCardWithoutSave()
    If lngReturn <> 0 Then
        '
        strErrInfor = sCard_ErrInfor(lngReturn)
        If strErrInfor <> "" Then
...
Private Function sCard_SaveCard() As Boolean
    ...
    If InitInfor_奉庆.模拟数据 Then
        ...
        Exit Function
    End If
    lngReturn = gobj奉庆.SaveCard("Apollo")
    If lngReturn <> 0 Then
        strErrInfor = sCard_ErrInfor(lngReturn)
        If strErrInfor <> "" Then
            ShowMsgbox strErrInfor
...
Public Function sCard_SetupCardOption_奉庆() As Boolean
    Dim lngReturn As Long
    Dim strErrInfor As String
    ...
    On Error GoTo ErrHand:
    '先初始化卡
    If gobj奉庆 Is Nothing Then
         Set gobj奉庆 = CreateObject("SiCard.SiCardOperator")
    End If
    lngReturn = gobj奉庆.SetupCardOption()
    If lngReturn <> 0 Then
        '
        strErrInfor = sCard_ErrInfor(lngReturn)
        If strErrInfor <> "" Then
...
Private Function sCard_ErrInfor(lngReturn As Long) As String
    ...
    Err = 0
    On Error GoTo ErrHand:
    If gobj奉庆Err Is Nothing Then
        Set gobj奉庆Err = CreateObject("SiCommTool.SiErrorCtl")
    End If
    strReturn = gobj奉庆Err.Describe(lngReturn)
...
Public Function 获取住院状态_奉庆(ByRef lng状态 As Long) As Boolean
    '返回lng状态,(0-门诊,1在院)
    Dim strOutput As String
    Dim strArr As Variant
...
Public Function 医保终止_奉庆() As Boolean
    '结束读写卡组件
    Dim bln读卡器 As Boolean
    ...
    Set gobj奉庆 = Nothing
    Set gobj奉庆Err = Nothing
    If gcnOracle_奉庆.State = 1 Then
        gcnOracle_奉庆.Close
    End If
...
Public Function 身份标识_奉庆(Optional bytType As Byte, Optional lng病人ID As Long) As String
    ...
    Err = 0
    On Error GoTo ErrHand:
    身份标识_奉庆 = frmIdentify奉庆.GetPatient(bytType, lng病人ID)
    Exit Function
ErrHand:
    If ErrCenter() = 1 Then
        Resume
...
Public Function 个人余额_奉庆(ByVal lng病人ID As Long) As Currency
'功能: 提取参保病人个人帐户余额
'返回: 返回个人帐户余额
    Dim rsTemp As New ADODB.Recordset
    
    gstrSQL = "select nvl(帐户余额,0) as 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_临沧奉庆
    ...
    End If
End Function
Public Function 参保资格审查_奉庆() As Boolean
        '功能：验证当前医保人员的资格审查
        '返回：返回true,否则返回False
        ...
        Err = 0
        On Error GoTo ErrHand:
        If 业务请求_奉庆(奉庆_参保资格审查, StrInput, strOutput) = False Then
            Exit Function
        End If
        参保资格审查_奉庆 = True
        Exit Function
    ...
    With rs明细
        ...
        Do Until rs明细.EOF
            gstrSQL = "select A.名称,A.编码,A.类别,A.规格,A.计算单位,B.项目编码,B.附注,B.是否医保,A.计算单位,E.规格,G.名称 剂型 " & _
                      "from 收费细目 A,(Select 项目编码, 附注,是否医保,收费细目ID From 保险支付项目 where 险类=" & TYPE_临沧奉庆 & ") B,药品目录 E ,药品信息 F,药品剂型 G " & _
            ...
            gstrSQL = "" & _
                  "   Select 收费等级,收费类别 From 医保收费目录 " & _
                  "   Where 类别='" & Nvl(rsTemp!附注) & "' and 编码='" & Nvl(rsTemp!项目编码) & "'"
            Call OpenRecordset(rsTmp, "取医保相关信息")
            
              
            If Val(Nvl(rs明细("实收金额"), 0)) <> 0 Then
                
                '获取经过审批的项目编号
                If Nvl(rsTemp!是否医保, 1) = 0 Then
                    '个人编号|项目编号|处方日期
                    StrInput = g病人身份_奉庆.医保证号 & "|"
                    
                    If Val(Nvl(rsTemp!附注)) = 1 Then
                        '吕说:药品只能传医院编码,而诊疗只能等只能传医保编码
                        StrInput = StrInput & Nvl(rsTemp!编码, "9999999999") & "|"
                    Else
                        StrInput = StrInput & Nvl(rsTemp!项目编码, "9999999999") & "|"
                    End If
                    
                    StrInput = StrInput & strDate
                    
                    If 业务请求_奉庆(奉庆_项目审批结果查询, StrInput, strOutput) = False Then
                        strOutput = "|"
                    End If
                    strArr = Split(strOutput, "|")
                    str审批编号 = strArr(1)
                Else
                    str审批编号 = ""
                End If
                '住院(门诊)号|处方号|处方内序号|审批编号|医院内码|医保编码|项目名称|费用等级|
                '费用类别|单价|数量|金额|单位|规格|剂型|开方日期|开方科室|开方医生|录入标志
                StrInput = g病人身份_奉庆.门诊号 & "|"
                StrInput = StrInput & g病人身份_奉庆.门诊号 & "|"
                If Not bln虚拟 Then
                    StrInput = StrInput & Nvl(rs明细!ID, 0) & "|"
                Else
                    StrInput = StrInput & Nvl(rs明细.AbsolutePosition, 0) & "|"
                End If
                ...
                DebugTool "门诊虚拟结算"
                If bln虚拟 Then
                    StrInput = StrInput & UserInfo.部门 & "|"
                Else
                    gstrSQL = "Select 名称 From 部门表 where id=" & Nvl(!开单部门ID, 0)
                    zlDatabase.OpenRecordset rsTemp, gstrSQL, "获取部门数据"
                    StrInput = StrInput & Nvl(rsTemp!名称) & "|"
                End If
                StrInput = StrInput & Nvl(rs明细!开单人) & "|"
...
'                End If
                ...
                If 业务请求_奉庆(奉庆_录入处方明细, StrInput, strOutput) = False Then
                    Exit Function
                End If
                ...
                If Not bln虚拟 Then
                    ...
                    strOutput = strArr(3) & "|" & str审批编号 & "|" & g病人身份_奉庆.门诊号 & "|" & g病人身份_奉庆.门诊号 & "|" & Val(strArr(1)) & "|" & strArr(2)
                    gstrSQL = "ZL_病人费用记录_更新医保(" & Nvl(!ID, 0) & ",NULL,NULL,NULL,NULL,1,'" & strOutput & "')"
                    zlDatabase.ExecuteProcedure gstrSQL, "打上上传标志"
                End If
            End If
            g病人身份_奉庆.费用总额 = g病人身份_奉庆.费用总额 + Nvl(rs明细!实收金额, 0)
            rs明细.MoveNext
        Loop
    End With
    门诊明细写入 = True
...
End Function
Private Function 检查中心维护单价(ByVal lng细目ID As Long, ByVal dbl单价 As Double) As Boolean
    ...
    On Error GoTo ErrHand:
    gstrSQL = "" & _
        "   Select 维护标志,编码,收费类别,类别 From 医保收费目录 " & _
        "   Where (类别,编码) in (select 附注,项目编码 From 保险支付项目 where 险类=" & TYPE_临沧奉庆 & " and 收费细目id=" & lng细目ID & " )"
    Call OpenRecordset(rsTemp, "检查中心维护单价")
    If rsTemp.EOF Then
        检查中心维护单价 = True
        'MsgBox "有项目未设置医保编码，不能结算。", vbInformation, gstrSysName
        Exit Function
    End If
    '经过咨询:由医保维护的价格为1或NULL(在下载时，已经更新该标志为1),而中心维护的价格为:0
    If Nvl(rsTemp!维护标志) <> STR_中心维护代码 Then
        ...
        If 业务请求_奉庆(奉庆_确定明细项目单价, StrInput, strOutput) = False Then
            Exit Function
        End If
...
ErrHand:
    If ErrCenter = 1 Then Resume
End Function
Public Function 门诊虚拟结算_奉庆(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    '参数：rsDetail     费用明细(传入)
    ...
    If str上次门诊号 = g病人身份_奉庆.门诊号 Then
        '已经赋值，说明该病人进行过预算
        StrInput = str上次门诊号 & "|" & str上次门诊号
        If 业务请求_奉庆(奉庆_处方退方, StrInput, strOutput) = False Then
            'Exit Function
        End If
    End If
    ...
    With rs明细
        Do While Not .EOF
            If 检查中心维护单价(Nvl(!收费细目ID, 0), Nvl(!单价, 0)) = False Then Exit Function
            .MoveNext
        Loop
    End With
    
'    str上次门诊号 = g病人身份_奉庆.门诊号
    '然后插入处方明细
    ...
    '                        2.41|0|0|2.41|0
    StrInput = g病人身份_奉庆.门诊号
    'strInput = strInput & "|" & IIf(g病人身份_奉庆.下个人帐户, "1", "0")
    
    If 更新就诊信息_奉庆(0, strOutput) = False Then Exit Function
    If 业务请求_奉庆(奉庆_预结算, StrInput, strOutput) = False Then
        '出错,需对原处方进行退方
        '已经赋值，说明该病人进行过预算
        StrInput = g病人身份_奉庆.门诊号 & "|" & g病人身份_奉庆.门诊号
        If 业务请求_奉庆(奉庆_处方退方, StrInput, strOutput) = False Then
            Exit Function
        End If
    End If
...
Private Function Get扩展信息(ByVal lng住院状态 As String, Optional str出院日期 As String) As String
    ...
    Dim strArr
    Dim i As Integer
    If g病人身份_奉庆.扩展信息 = "" Then Exit Function
    
    strArr = Split(g病人身份_奉庆.扩展信息, "|")
    '定点医院1|定点医院2|定点医院3|定点医院4|定点医院5|出院日期|住院状态（1—住院；2—出院）|就诊医院|医疗类别
    If str出院日期 = "" Then
    Else
        strArr(5) = str出院日期
    End If
    strArr(6) = lng住院状态
    'strArr(7) = InitInfor_奉庆.医院编码
    'strArr(8) = g病人身份_奉庆.医疗类别
...
Public Function 门诊结算_奉庆(lng结帐ID As Long, cur个人帐户 As Currency, str医保号 As String, cur全自付 As Currency) As Boolean
    ...
    gstrSQL = "Select a.*, a.付数*a.数次 as 数量,a.实收金额/(nvl(a.付数,1)*nvl(a.数次,1)) as 单价 From 病人费用记录 a Where 结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0"
    ...
    If lng病人ID = oldlng病人ID And str结算时间 = Format(rs明细!登记时间, "yyyy-mm-dd HH:MM:SS") Then
            '需要重新分配一个新号给门诊
            gstrSQL = "Select nvl(就诊次数,0)+1 as 就诊次数 From 保险帐户 where 险类=" & TYPE_临沧奉庆 & " and 病人id=" & lng病人ID
            zlDatabase.OpenRecordset rsTemp, gstrSQL, "获取就诊次数"
            
            '更新保险帐户
            lng就诊次数 = Nvl(rsTemp!就诊次数, 1)
            g病人身份_奉庆.门诊号 = lng病人ID & "_" & lng就诊次数
            
            '更新保险帐户
            gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & TYPE_临沧奉庆 & ",'就诊次数','" & lng就诊次数 & "')"
    ...
    '交易特定输出数据:
    '                  费用总额|统筹支付|账户支付|现金支付|大病垫付|所有17项动态信息|交易流水号
    '交易类型定义如下：
    '   1正常结算 (出院结算)
    '   0住院中途结算
    '   -1反结算
    ...
    'strInput = strInput & IIf(g病人身份_奉庆.下个人帐户, "1", "0")
    If 业务请求_奉庆(奉庆_结算, StrInput, strOutput) = False Then
        Exit Function
    End If
    ...
    Call Get帐户信息(TYPE_临沧奉庆, lng病人ID, Year(datCurr), int住院次数累计, cur帐户增加累计, cur帐户支出累计, cur进入统筹累计, cur统筹报销累计)
                
    gstrSQL = "zl_帐户年度信息_insert(" & lng病人ID & "," & TYPE_临沧奉庆 & "," & Year(datCurr) & "," & _
   ...
   '插入保险结算记录
    ...
    ' 费用总额|统筹支付|账户支付|现金支付|大病垫付|所有17项动态信息|交易流水号
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & TYPE_临沧奉庆 & "," & lng病人ID & "," & Year(datCurr) & "," & _
            cur帐户增加累计 & "," & cur帐户支出累计 & "," & cur进入统筹累计 & "," & cur统筹报销累计 & "," & int住院次数累计 & ",0,0," & IIf(g病人身份_奉庆.下个人帐户, 1, 0) & "," & _
            dbl费用总额 & ",0,0," & _
            cur统筹支付 & "," & cur统筹支付 & ",0,0," & cur个人帐户 & ",'" & _
            str交易流水号 & "',NULL,NULL,'" & g病人身份_奉庆.门诊号 & "|" & str动态信息 & "')"
...
Err反结算:
    ...
    StrInput = StrInput & str操作员
  '  strInput = strInput & IIf(g病人身份_奉庆.下个人帐户, "1", "0")
    Call 业务请求_奉庆(奉庆_结算, StrInput, strOutput)
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
    End If
End Function
Public Function 更新就诊信息_奉庆(ByVal bytType As Byte, strOutPutstring As String, Optional ByVal str入院日期 As String = "", Optional ByVal str出院日期 As String = "", Optional bln反结算 As Boolean = False) As Boolean
        ...
        Select Case bytType
            Case 0 '门诊
                '只更新医疗类别,
                '住院号|更新标志|医疗类别|入院日期|入院疾病名称|出院日期|确诊疾病编码|经办人|帐户余额|住院科室|动态信息
                '返回:未用
                ...
                ElseIf g病人身份_奉庆.病种编码 = "" Or g病人身份_奉庆.病种编码 = "000000" Then
                    '不更新诊断
                    StrInput = StrInput & "1101011101" & "|"
                    StrInput = StrInput & g病人身份_奉庆.医疗类别 & "|"
                    StrInput = StrInput & strDate & "|"
                    StrInput = StrInput & strDate & "|"
                Else
                    ...
                    StrInput = StrInput & g病人身份_奉庆.医疗类别 & "|"
                    StrInput = StrInput & strDate & "|"
                    StrInput = StrInput & g病人身份_奉庆.病种名称 & "|"
                    StrInput = StrInput & strDate & "|"
                    StrInput = StrInput & g病人身份_奉庆.病种编码 & "|"
                End If
                StrInput = StrInput & gstrUserName & "|"
                StrInput = StrInput & g病人身份_奉庆.帐户余额 & "|"
                StrInput = StrInput & g病人身份_奉庆.动态信息
                DebugTool "更新就诊信息:帐户余额:" & g病人身份_奉庆.帐户余额
        ...
        Case 1  '入院登记
                ...
                strDate = Format(zlDatabase.Currentdate, "yyyy-mm-dd")
                StrInput = g病人身份_奉庆.住院号 & "|"
                
                If g病人身份_奉庆.病种编码 = "" Or g病人身份_奉庆.病种编码 = "000000" Then
                    '不更新诊断
                ...
                StrInput = StrInput & gstrUserName & "|"
                StrInput = StrInput & g病人身份_奉庆.帐户余额 & "|"
                StrInput = StrInput & g病人身份_奉庆.住院科室 & "|"
                StrInput = StrInput & g病人身份_奉庆.动态信息
        ...
        If 业务请求_奉庆(奉庆_更新就诊信息, StrInput, strOutPutstring) = False Then Exit Function
        ...
        If Val(strArr(1)) = 1 Then
            ...
            '更新保险帐户中的帐户余额
            If sCard_属性值(3, strTemp) = False Then Exit Function
            '读取个人帐户余额
            '帐户余额
            ...
            gstrSQL = "zl_保险帐户_更新信息(" & g病人身份_奉庆.病人ID & "," & TYPE_临沧奉庆 & ",'帐户余额','''" & g病人身份_奉庆.帐户余额 & "''')"
...
Public Function 门诊结算冲销_奉庆(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    '功能：将门诊收费的明细和结算数据转发送医保前置服务器确认；
    '参数：lng结帐ID     收费记录的结帐ID；，从预交记录中可以检索医保号和密码
    '      cur个人帐户   从个人帐户中支出的金额
    ...
    str医保证号 = g病人身份_奉庆.医保证号
    If 获取参保人员信息_奉庆() = False Then Exit Function
    If str医保证号 <> g病人身份_奉庆.医保证号 Then
        ShowMsgbox "卡插入错误!"
        Exit Function
    End If
    
    If 更新就诊信息_奉庆(0, strOutput, , , True) = False Then
        ShowMsgbox "更新就诊信息失败!"
        Exit Function
    End If
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_临沧奉庆 & " and 记录ID=" & lng结帐ID
    ...
    '   -1反结算
    '   -2IC卡挂失后出院结算，本次结算（只针对住院）将所有费用转为现金支付，待到医保中心报销。
    g病人身份_奉庆.门诊号 = Split(Nvl(rsTemp!备注) & "|", "|")(0)
    StrInput = "-1|"
    StrInput = StrInput & g病人身份_奉庆.门诊号 & "|"
    StrInput = StrInput & g病人身份_奉庆.门诊号 & "|"
    ...
    '帐户年度信息
    Call Get帐户信息(TYPE_临沧奉庆, lng病人ID, Year(curDate), int住院次数累计, dbl帐户增加累计, dbl帐户支出累计, dbl进入统筹累计, dbl统筹报销累计)
            
    gstrSQL = "zl_帐户年度信息_insert(" & lng病人ID & "," & TYPE_临沧奉庆 & "," & Year(curDate) & "," & _
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng冲销ID & "," & TYPE_临沧奉庆 & "," & lng病人ID & "," & Year(curDate) & "," & _
       ...
       str流水号 & "',NULL,0,'" & g病人身份_奉庆.门诊号 & "|" & str动态信息 & "')"
    Call zlDatabase.ExecuteProcedure(gstrSQL, "更新保险结算信息")
    门诊结算冲销_奉庆 = True
    Exit Function
Err冲正:
    Call 业务请求_奉庆(奉庆_冲正交易, str流水号 & "|10|" & gstrUserName, strOutput)
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Function 病人入院登记处理(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    '保存将交易流水号
    gstrSQL = "zl_保险帐户_更新信息(" & lng病人ID & "," & TYPE_临沧奉庆 & ",'顺序号','''" & str交易流水号 & "''')"
    Call zlDatabase.ExecuteProcedure(gstrSQL, "保存交易流水号")
    
    
    If 更新就诊信息_奉庆(1, strOutput, Nvl(rsTemp!入院日期)) = False Then
        GoTo Err冲正:
    End If
...
Err冲正:
        ...
        StrInput = StrInput & gstrUserName
        If 业务请求_奉庆(奉庆_冲正交易, StrInput, strOutput) = False Then
        End If
        Exit Function
ErrHand:
    If ErrCenter = 1 Then
...
Public Function 入院登记_奉庆(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
    ...
    lng就诊次数 = Nvl(rsTemp!就诊次数, 1)
    g病人身份_奉庆.住院号 = lng病人ID & "_" & lng就诊次数
    ...
    '将病人的状态进行修改
    gstrSQL = "zl_保险帐户_入院(" & lng病人ID & "," & TYPE_临沧奉庆 & ")"
...
ErrHand:
    ...
    入院登记_奉庆 = False
End Function
Private Function Get交易代码(ByVal intType As 业务类型_奉庆, Optional bln读名称 As Boolean = False) As String
    Select Case intType
        Case 奉庆_病人登记
    ...
    End Select
End Function
Public Function 业务请求_奉庆(ByVal intType As 业务类型_奉庆, strInputString As String, strOutPutstring As String) As Boolean
    '-----------------------------------------------------------------------------------------------------------
    '--功  能:对所有业务进行业务请求
...
Public Function 入院登记撤销_奉庆(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    StrInput = StrInput & "01" & "|"
    StrInput = StrInput & gstrUserName
    If 业务请求_奉庆(奉庆_冲正交易, StrInput, strOutput) = False Then Exit Function
    
    
    '更新医保帐户
    gstrSQL = "zl_保险帐户_出院(" & lng病人ID & "," & TYPE_临沧奉庆 & ")"
...
Public Function 出院登记_奉庆(lng病人ID As Long, lng主页ID As Long) As Boolean
...
errHandle:
    ...
    出院登记_奉庆 = False
End Function
Public Function 出院登记撤销_奉庆(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    '出院登记撤消
     '改变病人状态
...
Public Function 住院结算_奉庆(lng结帐ID As Long, ByVal lng病人ID As Long, Optional strAdvance As String = "") As Boolean
  '功能：将需要本次结帐的费用明细和结帐数据发送医保前置服务器确认；
    ...
    Dim rsTemp As New ADODB.Recordset
    Dim StrInput As String, strOutput As String, str操作员 As String, str动态信息 As String
    Dim lng主页ID As Long, intMouse As Integer, int住院次数累计 As Integer, i As Integer
    ...
    StrInput = g病人身份_奉庆.住院号 & "|" & IIf(g病人身份_奉庆.下个人帐户, "1", "0")
    If 业务请求_奉庆(奉庆_预结算, StrInput, strOutput) = False Then
        Exit Function
    End If
    strArr = Split(strOutput, "|")
    ...
    If g病人身份_奉庆.中途结算 = True Then
        StrInput = 0 & "|"
    Else
        StrInput = 1 & "|"
    End If
    StrInput = StrInput & g病人身份_奉庆.住院号 & "|"
    ...
    '检查虚拟结算是否一至
    With 虚拟结算数据
        If Round(.费用总额, 2) <> Round(objData.费用总额, 2) Or Round(.统筹支付, 2) <> Round(objData.统筹支付, 2) Or _
            Round(.账户支付, 2) <> Round(objData.账户支付, 2) Or _
            Round(.大病垫付, 2) <> Round(objData.大病垫付, 2) Then
            
            str结算方式 = "个人帐户|" & objData.账户支付
            If objData.统筹支付 <> 0 Then str结算方式 = str结算方式 & "||医保基金|" & objData.统筹支付
            If objData.大病垫付 <> 0 Then str结算方式 = str结算方式 & "||大病垫付|" & objData.大病垫付
   ...
   '插入保险结算记录
    ...
    gstrSQL = "zl_保险结算记录_insert(2," & lng结帐ID & "," & TYPE_临沧奉庆 & "," & lng病人ID & "," & Year(datCurr) & "," & _
            cur帐户增加累计 & "," & cur帐户支出累计 & "," & cur进入统筹累计 & "," & cur统筹报销累计 & "," & int住院次数累计 & ",0,0," & IIf(g病人身份_奉庆.下个人帐户, "1", "0") & "," & _
            g病人身份_奉庆.费用总额 & ",0,0," & _
            objData.统筹支付 & "," & objData.统筹支付 & ",0,0," & objData.账户支付 & ",'" & _
            objData.交易流水号 & "'," & lng主页ID & "," & IIf(g病人身份_奉庆.中途结算, 1, 0) & ",'" & g病人身份_奉庆.住院号 & "'" & IIf(blnOld, "", ",1") & ")"
    ...
    Exit Function
Err冲正:
    Call 业务请求_奉庆(奉庆_冲正交易, objData.交易流水号 & "|10|" & gstrUserName, strOutput)
    Exit Function
ErrHand:
    If ErrCenter() = 1 Then
        Resume
...
Public Function 住院结算冲销_奉庆(lng结帐ID As Long) As Boolean
     '----------------------------------------------------------------
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_临沧奉庆 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "结算冲销")
    If rsTemp.EOF = True Then
        MsgBox "原单据的医保记录不存在，不能作废。", vbInformation, gstrSysName
        Exit Function
    ...
    If 获取参保人员信息_奉庆() = False Then Exit Function
    If str医保证号 <> g病人身份_奉庆.医保证号 Then
        ShowMsgbox "不是该医保病人的医保证"
        Exit Function
    End If
     
    If 更新就诊信息_奉庆(2, strOutput, , , True) = False Then
        ShowMsgbox "更新就诊信息失败!"
        Exit Function
    End If
    
    str流水号 = rsTemp("支付顺序号")
    g病人身份_奉庆.住院号 = Split(Nvl(rsTemp!备注, "|"), "|")(0)
    g病人身份_奉庆.下个人帐户 = IIf(Nvl(rsTemp!实际起付线, 0) = 1, True, False)
    g病人身份_奉庆.中途结算 = Nvl(rsTemp!中途结帐, 0) = 1
    ...
    '帐户年度信息
    Call Get帐户信息(TYPE_临沧奉庆, rsTemp("病人ID"), Year(curDate), int住院次数累计, cur帐户增加累计, cur帐户支出累计, cur进入统筹累计, cur统筹报销累计)
            
    gstrSQL = "zl_帐户年度信息_insert(" & rsTemp("病人ID") & "," & TYPE_临沧奉庆 & "," & Year(curDate) & "," & _
   ...
   '插入保险结算记录
    ...
    '   进入统筹金额_IN(统筹支付),统筹报销金额_IN(统筹支付),大病自付金额_IN(无),超限自付金额_IN(无),个人帐户支付_IN(个人帐户支付),"
    '   支付顺序号_IN(结算时产生流水号),主页ID_IN,中途结帐_IN,备注_IN
    gstrSQL = "zl_保险结算记录_insert(2," & lng冲销ID & "," & TYPE_临沧奉庆 & "," & rsTemp("病人ID") & "," & Year(curDate) & "," & _
        cur帐户增加累计 & "," & cur帐户支出累计 - rsTemp("个人帐户支付") & "," & cur进入统筹累计 & "," & cur统筹报销累计 & "," & int住院次数累计 & ",0,0," & Nvl(rsTemp!实际起付线, 0) & "," & _
        Nvl(rsTemp("发生费用金额"), 0) * -1 & ",0,0," & _
...
Private Function Get流水号(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng收费细目ID As Long, ByVal dbl数量 As Double, ByVal dbl单价 As Double, lng冲销ID As Long) As Variant
    ...
    gstrSQL = " Select id, 摘要 From 病人费用记录" & _
              " Where 收费细目ID=" & lng收费细目ID & " And 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID & _
              " And 记录状态=1 And Nvl(是否上传,0)=1 and A.数次*nvl(A.付数,1)=" & dbl数量 & " and Round(A.实收金额/(A.数次*nvl(A.付数,1)),4))=" & dbl单价 & " And Nvl(实收金额,0)>0 And Rownum<2"
...
Private Function 处方上传(ByVal lng记录性质 As Long, lng记录状态 As Long, ByVal str单据号 As String) As Boolean
   ...
   '读出该张单据的费用明细
    gstrSQL = "Select A.ID,A.NO,A.序号,A.病人ID,A.主页ID,to_char(A.发生时间,'yyyy-mm-dd hh24:mi:ss') as 登记时间,Round(A.实收金额,4) 实收金额 " & _
              "         ,A.收费细目ID,A.数次*nvl(A.付数,1) as 数量,Decode(A.数次*nvl(A.付数,1),0,0,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4)) as 价格 " & _
              "         ,C.项目编码,C.附注,J.类别 as 收费类别,C.是否医保,B.编码,B.名称,A.是否急诊,nvl(A.开单人,A.操作员姓名) as 医生,y.名称 as 开单科室,A.操作员姓名,B.计算单位,E.规格,G.名称 剂型,M.医保号,M.就诊次数 " & _
              "  From 病人费用记录 A,收费类别 J,收费细目 B,保险帐户 M,(Select  项目编码, 附注,是否医保,收费细目ID From 保险支付项目 where 险类=" & TYPE_临沧奉庆 & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G,部门表 Y " & _
              "  where a.病人id=M.病人id  and M.险类=" & TYPE_临沧奉庆 & " and A.NO='" & str单据号 & "' and A.记录性质=" & lng记录性质 & " and A.记录状态=1 And Nvl(A.是否上传,0)=0 " & _
              "        and A.收费类别=J.编码(+)  and A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=" & TYPE_临沧奉庆 & _
              "        and A.收费细目ID=B.ID and a.开单部门id=y.id(+) and A.收费细目ID=C.收费细目ID(+) " & _
    ...
    With rs明细
        ...
        Do While Not .EOF
            '单价检查
            g病人身份_奉庆.医保证号 = Nvl(!医保号)
            If 检查中心维护单价(Nvl(!收费细目ID, 0), Nvl(!价格, 0)) = False Then Exit Function
            
'            If Val(!数量) < 0 Or Val(!价格) < 0 Then
'                '任意取一笔正常记录的流水号，做为冲销流水号
'                str冲销明细流水号 = Get流水号(!病人ID, !主页ID, !收费细目ID, Val(!数量), Val(!价格), lng冲销ID)
'                If Trim(str冲销明细流水号) = "" Or lng冲销ID = 0 Then
'                    MsgBox "没有找到可以冲销的记录！[" & !编码 & "]" & !名称, vbInformation, gstrSysName
'                    Exit Function
'                End If
'            Else
'                str冲销明细流水号 = ""
'            End If
    ...
    With rs明细
        Do Until .EOF
            gstrSQL = "Select * from 医保收费目录 where 类别=" & Val(Nvl(!附注, 0)) & " and 编码='" & Nvl(!项目编码) & "'"
            Call OpenRecordset(rsTemp, "处方上传")
'            If rsTemp.EOF Then
...
'                strArr摘要 = Get流水号(!病人ID, !主页ID, !收费细目ID, Val(!数量), Val(!价格), lng冲销ID)
'                '需冲正相应的流水号数据
'                '交易特定输入数据：  被冲正交易流水号|被冲正交易类型代码|操作员代码
'                '交易特定输出数据:   未用
'                strInput = strArr摘要(0) & "|"
...
'
'                '标明该记录已经被冲销
'                gstrSQL = "ZL_病人费用记录_更新医保(" & lng冲销ID & ",NULL,NULL,NULL,NULL,1,'" & strArr摘要(0) & "|" & strArr摘要(1) & "|" & strArr摘要(2) & "|" & strArr摘要(3) & "|" & strArr摘要(4) & "|" & strArr摘要(5) & "|" & "1" & "')"
'                zlDatabase.ExecuteProcedure gstrSQL, "打上上传标志"
'            Else
                '上传明细记录
                '确定审批编号
                If Nvl(!是否医保, 1) = 0 Then
                  '个人编号|项目编号|处方日期
                    ...
                    If Val(Nvl(!附注)) = 1 Then
                        '吕说:药品只能传医院编码,而诊疗只能等只能传医保编码
                        StrInput = StrInput & Nvl(!编码, "9999999999") & "|"
                    Else
                        StrInput = StrInput & Nvl(!项目编码, "9999999999") & "|"
                    End If
                ...
                lng病人ID = Nvl(!病人ID, 0)
                g病人身份_奉庆.住院号 = lng病人ID & "_" & Nvl(!就诊次数, 0)
                '上传明细
                '交易特定输入数据：住院(门诊)号|处方号|处方内序号|审批编号|医院内码|医保编码|项目名称|
                '                  费用等级|费用类别|单价|数量|金额|单位|规格|剂型|开方日期|开方科室|开方医生|录入标志
                
                '交易特定输出数据:   该处方对应项目的实际单价|实际等级|交易流水号。
                
                str处方号 = rs明细!NO & "_" & lng记录性质
                StrInput = Nvl(!病人ID) & "_" & Nvl(!就诊次数, 0) & "|"
                StrInput = StrInput & str处方号 & "|"
                StrInput = StrInput & Nvl(!序号) & "|"
                StrInput = StrInput & str审批编号 & "|"
                StrInput = StrInput & Nvl(!编码) & "|"
                StrInput = StrInput & IIf(bln单方, "9999999999", Nvl(!项目编码, "9999999999")) & "|"
                StrInput = StrInput & Nvl(!名称) & "|"
                If rsTemp.EOF Or bln单方 Then
                    StrInput = StrInput & "3" & "|"
                    StrInput = StrInput & Split(Get费用类别(Nvl(!收费类别)), "-")(0) & "|"
                Else
                ...
                StrInput = StrInput & Nvl(rs明细!开单科室) & "|"
                StrInput = StrInput & Nvl(rs明细!医生) & "|"
                ...
                gstrSQL = "ZL_病人费用记录_更新医保(" & Nvl(!ID, 0) & ",NULL,NULL,NULL,NULL,1,'" & strArr(3) & "|" & str审批编号 & "|" & g病人身份_奉庆.住院号 & "|" & str处方号 & "|" & strArr(1) & "|" & strArr(2) & "')"
                zlDatabase.ExecuteProcedure gstrSQL, "打上上传标志"
'           End If
            .MoveNext
        Loop
...
End Function
Public Function 处方登记_奉庆(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    '-----------------------------------------------------------------------------------------------------------
    '--功  能:上传处理明细数据
    '--入参数:
    '--出参数:
...
Private Function 处方作废(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    ...
    gstrSQL = " Select 摘要,A.ID,a.收费细目id,A.序号,A.数次*nvl(A.付数,1) as 数量,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4) as 单价 " & _
              ...
              " where a.病人id=b.病人id and A.NO='" & str单据号 & "' and A.记录性质=" & lng记录性质 & " and A.记录状态=3 and   Nvl(附加标志,0)<>9  order by A.病人id"
    ...
    gstrSQL = " Select * " & _
              " From 病人费用记录 A,保险帐户 b" & _
              " where a.病人id=b.病人id and A.NO='" & str单据号 & "' and A.记录性质=" & lng记录性质 & " and A.记录状态=2 and    Nvl(附加标志,0)<>9 AND nvl(a.是否上传,0)=0 "
    Call OpenRecordset(rs明细, "处方明细上传")
    ...
    With rs明细
        ...
        Do While Not .EOF
            ...
            If lng病人ID <> Nvl(!病人ID, 0) Then
                ...
                '"交易流水号|审批编号|住院(门诊)号|处方号(门诊:门诊号；住院:单据号+记录性质)|实际交易单价|实际等级"
                strArr = Split(Nvl(rs原明细!摘要) & "|||||", "|")
                g病人身份_奉庆.住院号 = strArr(2)
                str处方号 = strArr(3)
                StrInput = g病人身份_奉庆.住院号 & "|"
                StrInput = StrInput & str处方号
                If 业务请求_奉庆(奉庆_处方退方, StrInput, strOutput) = False Then Exit Function
                lng病人ID = Nvl(!病人ID, 0)
            End If
            .MoveNext
        Loop
...
ErrHand:
   ...
   End If
End Function
Private Function Read模拟数据(ByVal int业务类型 As 业务类型_奉庆, ByVal strInputString As String, ByRef strOutPutstring As String)
    '----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Private Function Get病人信息(ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = "select a.*,b.姓名,b.性别, b.年龄, b.出生日期, b.身份证号,b.工作单位 " & _
             ...
             " WHERE a.病人id=" & lng病人ID & " AND a.病人id=b.病人id and a.险类=" & TYPE_临沧奉庆
...
Private Sub OpenRecordset_奉庆(rsTemp As ADODB.Recordset, ByVal strCaption As String, Optional strSQL As String = "", Optional cnOracle As ADODB.Connection)
    '功能：打开记录集
    ...
    If cnOracle Is Nothing Then
        rsTemp.Open IIf(strSQL = "", gstrSQL, strSQL), gcnOracle_奉庆, adOpenStatic, adLockReadOnly
    Else
...
Public Function 住院虚拟结算_奉庆(rsExse As Recordset, ByVal lng病人ID As Long, Optional bln结帐处 As Boolean = True) As String
    ...
    gstrSQL = "" & _
        "   Select 入院日期,出院日期 " & _
        "   From 病案主页 where 病人ID=" & g结算数据.病人ID & " and 主页ID=" & lng主页ID
    ...
    Do While Not rsExse.EOF
        g病人身份_奉庆.费用总额 = g病人身份_奉庆.费用总额 + rsExse("金额")
        rsExse.MoveNext
    Loop
    g病人身份_奉庆.费用总额 = Round(g病人身份_奉庆.费用总额, 2)
    ...
    g病人身份_奉庆.确诊诊断编码 = Nvl(rsTemp!确诊编码)
    
    If 更新就诊信息_奉庆(2, strOutput, "", str出院日期) = False Then Exit Function
    ...
    With 虚拟结算数据
        ...
        End If
        If .费用总额 <> g病人身份_奉庆.费用总额 Then
            ShowMsgbox "中心费用结算总额(" & .费用总额 & " ) 不等于医院实际发生费用总额(" & g病人身份_奉庆.费用总额 & ")"
            Exit Function
        End If
    End With
    
    住院虚拟结算_奉庆 = str结算方式
    g病人身份_奉庆.病人ID = lng病人ID   '表示该病人已经进行了虚拟结算
    Exit Function
ErrHand:
    If ErrCenter = 1 Then
        Resume
...
Private Function Get费用类别(ByVal str收费类别 As String) As String
    ...
    If zlCommFun.ActualLen(str收费类别) = 1 Then
            ...
            End If
    End If
    gstrSQL = "Select * From 保险参数 where 参数名='" & str收费 & "' and 险类=" & TYPE_临沧奉庆
...
Private Function 补传住院明细记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select A.ID,A.NO,A.记录性质,J.类别 as 收费类别,A.记录状态,A.序号,A.病人ID,A.主页ID,A.发生时间 as 登记时间,Round(A.实收金额,4) 实收金额" & _
              "         ,A.收费细目ID,A.数次*nvl(A.付数,1) as 数量,Decode(A.数次*nvl(A.付数,1),0,0,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4)) as 价格 " & _
              "         ,C.项目编码,C.附注,C.是否医保,B.编码,B.名称,A.是否急诊,nvl(A.开单人,A.操作员姓名) as 医生,y.名称 as 开单科室,A.操作员姓名,B.计算单位,E.规格,G.名称 剂型 " & _
              "  From 病人费用记录 A,收费类别 J,收费细目 B, (Select 项目编码, 附注,是否医保,收费细目ID From 保险支付项目 where 险类=" & TYPE_临沧奉庆 & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G ,部门表 Y" & _
              "  where A.病人ID=" & lng病人ID & " and A.主页ID=" & lng主页ID & " and A.记帐费用=1 and nvl(A.实收金额,0)<>0 and nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0 " & _
              "         and a.开单部门id=y.id(+) and A.收费类别=J.编码(+) and A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=" & TYPE_临沧奉庆 & _
...
'----200410刘兴宏加入
Public Function 医保设置_奉庆() As Boolean
    医保设置_奉庆 = frmSet奉庆.参数设置
    
End Function
'
'Public Function 下载服务项目目录_奉庆(ByVal bytType As Byte, ByVal objProgss As Object) As Boolean
'    '--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
'    '功能:下载服务项目目录
...
'    If 业务请求_奉庆(奉庆_收费目录下载预处理, strInput, strOutput) = False Then Exit Function
'    strArr = Split(strOutput, "|")
...
'   For i = 1 To lngCount
'        '正试下载
'        If 业务请求_奉庆(奉庆_收费目录下载处理, strInput, strOutput) = False Then Exit Function
'        strArr = Split(strOutput, "|")
'        '更新收费目录
'
...
'        End Select
'        gcnOracle_奉庆.Execute strSql, , adCmdStoredProc
'        If Not objProgss Is Nothing Then
'            objProgss.Value = i
'        Else
...
Public Function 下载服务项目目录_奉庆(ByVal bytType As Byte, ByVal objProgss As Object) As Boolean
    ...
    OpenRecordset_奉庆 rsData, "获取" & strTitle & "数据", , gcnOracle_中心
    ...
    With rsData
        Do While Not .EOF
            ...
            strSQL = "ZL_医保收费目录_UPDATE("
            '    类别_IN IN 医保收费目录.类别%TYPE,
            strSQL = strSQL & bytType & ","
            Select Case bytType
            Case 1
                    '    编码_IN IN 医保收费目录.编码%TYPE,
                    '    AKA060  VARCHAR2(60)    N   药品编码    省目录编号
                    
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA060)) & "',"    'AKA060  VARCHAR2(20)                   药品编码
                    '    名称_IN IN 医保收费目录.名称%TYPE,
                    '    AKA061  VARCHAR2(100)   Y   中文名称    通用名
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA061)) & "',"   'AKA061 VARCHAR2(50)  Y                中文名称
                    '    AKA062  VARCHAR2(50)    Y   英文名称
                    '    英文名称_IN IN 医保收费目录.英文名称%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA062)) & "',"  'AKA062 VARCHAR2(50)  Y                英文名称
                    '    AKA063  VARCHAR2(3) Y   收费类别    11西药费 12成药 13草药
                    '    收费类别_IN IN 医保收费目录.收费类别%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA063)) & "'," 'AKA063 VARCHAR2(3)   Y                收费类别
                    '    AKA065  VARCHAR2(3) Y   收费项目等级    1甲类 2乙类 3自费
                    '    收费等级_IN IN 医保收费目录.收费等级%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA065)) & "'," 'AKA065 VARCHAR2(3)   Y                收费项目等级
                    '    AKA066  VARCHAR2(30)    Y   助记码
                    '    助记码_IN IN 医保收费目录.助记码%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA066)) & "',"  'AKA066 VARCHAR2(14)  Y                助记码
                    '    AKA067  VARCHAR2(20)    Y   单位
                    '    单位_IN IN 医保收费目录.单位%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA067)) & "',"  'AKA067 VARCHAR2(20)  Y                单位
                    '    AKA068  NUMBER(8,2) Y   标准价格
                    '    标准价格_IN IN 医保收费目录.标准价格%TYPE,
                strSQL = strSQL & "" & Val(Nvl(!AKA068)) & ","  'AKA068 NUMBER(8,2)   Y                标准价格
                    '    AKA070  VARCHAR2(50)    Y   剂型
                    '    剂型_IN IN 医保收费目录.剂型%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA070)) & "',"   'AKA070 VARCHAR2(50)  Y                剂型
                    '    AKA074  VARCHAR2(50)    Y   规格
                    '    规格_IN IN 医保收费目录.规格%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA074)) & "',"    'AKA074 VARCHAR2(50)  Y                规格
                    '    AAE013  VARCHAR2(100)   Y   备注
                    '    备注_IN IN 医保收费目录.备注%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AAE013)) & "',"      'AAE013 VARCHAR2(100) Y                备注
                    '    AAE035  DATE    Y   变更日期
                ...
                strSQL = strSQL & "'" & strProcess(Nvl(!AAA104, "1")) & "',"    'AAA104 VARCHAR2(3)   Y                代码可维护标志
                    '    支付标准_IN IN 医保收费目录.支付标准%TYPE,
                strSQL = strSQL & "NULL,"  'AAA104 VARCHAR2(3)   Y                支付标准
                    '    药品种类_IN IN 医保收费目录.药品种类%TYPE,
                    '    AKA305  VARCHAR2(3) Y   药品种类
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA305)) & "',"      'AKA305 VARCHAR2(3)   Y                药品种类
                    '    AKA064  VARCHAR2(3) Y   处方药标志
                    '    处方药标志_IN IN 医保收费目录.处方药标志%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA064)) & "',"      'AKA064 VARCHAR2(3)   Y                处方药标志
                    '    AKA069  NUMBER(5,4) Y   自付比例
                    '    自付比例_IN IN 医保收费目录.自付比例%TYPE,
                strSQL = strSQL & "" & Val(Nvl(!AKA069)) & ","       'AKA069 NUMBER(5,4)   Y                自付比例
                    
                    '    AKA071  NUMBER(5,2) Y   每次用量
                    '    每次用量_IN IN 医保收费目录.每次用量%TYPE,
                strSQL = strSQL & "" & Val(Nvl(!AKA071)) & ","      'AKA071 NUMBER(5,2)   Y                每次用量
                    '    AKA072  VARCHAR2(20)    Y   使用频次
                    '    使用频次_IN IN 医保收费目录.使用频次%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA072)) & "',"      'AKA072 VARCHAR2(20)  Y                使用频次
                    '    AKA073  VARCHAR2(50)    Y   用法
                    '    用法_IN IN 医保收费目录.用法%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA073)) & "',"      'AKA073 VARCHAR2(50)  Y                用法
                    '    AKA030  VARCHAR2(3) Y   当前使用标志
                    '    当前使用标志_IN IN 医保收费目录.当前使用标志%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA030)) & "',"      'AKA030 VARCHAR2(3)   Y                当前使用标志
                    '    社保经办机构_IN IN 医保收费目录.社保经办机构%TYPE,
                strSQL = strSQL & "NULL,"      'AAB034 VARCHAR2(14)  Y                社保经办机构
                ...
                '    AKA075  VARCHAR2(40)    Y   通用名编码
                    '    通用名编码_IN   医保收费目录.通用名编码%type:=NULL,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA075)) & "',"
                '    AKA076  VARCHAR2(100)   Y   商品名
                    '    商品名_IN   医保收费目录.商品名%type:=NULL,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA076)) & "',"
                '    AKA078  VARCHAR2(20)    Y   小单位
                '    小单位_IN   医保收费目录.小单位%type:=NULL,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA078)) & "',"
                '    AKA079  NUMBER(8,2) Y   最小单位零售价
                    '    最小单位售价_IN 医保收费目录.最小单位售价%type:=NULL,
                strSQL = strSQL & "" & Val(Nvl(!AKA079)) & ","
                '    AKA031  VARCHAR2(100)   Y   厂家
                    '    厂家_IN     医保收费目录.厂家%type:=NULL,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA031)) & "',"
                '    AKA032  VARCHAR2(1) Y   新旧目录标志    0原目录、1省目录
                '    新旧目录标志_IN 医保收费目录.新旧目录标志%type:=NULL,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA032)) & "',"
                
                '    AKA033  VARCHAR2(3) Y   特殊用药标志    0为正常用药；1为民族用药
                '    特殊用药标志_IN 医保收费目录.特殊用药标志%type:=NULL,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA033)) & "')"
                '    财务分类_IN 医保收费目录.财务分类%type:=NULL,
                '    二类最高限价_IN 医保收费目录.二类最高限价%type:=NULL,
                '    三类最高限价_IN 医保收费目录.三类最高限价%type:=NULL,
                '    标准单位_IN 医保收费目录.标准单位%type:=NULL,
                '    特检标志_IN 医保收费目录.特检标志%type:=NULL,
                '    新旧标志_IN 医保收费目录.新旧标志%type:=NULL
            Case 2
            
                '    AKA090  VARCHAR2(20)        项目编码    加长
                '编码_IN IN 医保收费目录.编码%TYPE,
                strSQL = strSQL & "'" & strProcess(Nvl(!AKA090)) & "',"
                '    AKA091  VARCHAR2(200)   Y   项目名称    加长
                '名称_IN IN 医保收费目录.名称%TYPE,
                strSQL = strSQL & "'" & Replace(Nvl(!AKA091), "'", "‘") & "',"
                '    英文名称_IN IN 医保收费目录.英文名称%TYPE,
                strSQL = strSQL & "'" & "" & "',"
                '    AKA063  VARCHAR2(3) Y   收费类别    医保费用类别（chargetype_info有对应值）
                '    收费类别_IN IN 医保收费目录.收费类别%TYPE,
                ...
                '    AKA068  NUMBER(8,2) Y   一类最高限价    一类医院最高限价（三级医院）
                '    标准价格_IN IN 医保收费目录.标准价格%TYPE,
                ...
                '    规格_IN IN 医保收费目录.规格%TYPE,
                strSQL = strSQL & "NUll,"
                '    AAE013  VARCHAR2(1000)  Y   备注
                '    备注_IN IN 医保收费目录.备注%TYPE,
                strSQL = strSQL & "'" & Nvl(!AAE013) & "',"
                '    AAE035  DATE    Y   变更日期
                '    变更时间_IN IN 医保收费目录.变更时间%TYPE,
                strSQL = strSQL & "to_date('" & Format(!AAE035, "yyyy-mm-dd HH:MM:SS") & "','yyyy-mm-dd hh24:mi:ss'),"
                'NULL和1是医院维护，0 是中心维护吗?,将NULL这种情况转换成1
                '    维护标志_IN IN 医保收费目录.维护标志%TYPE,
                ...
                '    处方药标志_IN IN 医保收费目录.处方药标志%TYPE,
                strSQL = strSQL & "NUll,"
                '    AKA069  NUMBER(5,4) Y   普通人员自付比例(1-支付比例1)
                '    自付比例_IN IN 医保收费目录.自付比例%TYPE,
                ...
                '    AKA030  VARCHAR2(1) Y   停用标志    1在用,2停用
                '    当前使用标志_IN IN 医保收费目录.当前使用标志%TYPE,
                ...
                '    ZKA001  NUMBER(5,4) Y   医疗照顾人员自付比例(1-支付比例2)
                '   住院自费比例_IN IN 医保收费目录.住院自费比例%TYPE,
                ...
                '    特殊用药标志_IN 医保收费目录.特殊用药标志%type:=NULL,
                strSQL = strSQL & "NULL,"
                '    AKA064  VARCHAR2(4) Y   财务分类    卫生部定义的财务分类
                '    财务分类_IN 医保收费目录.财务分类%type:=NULL,
                strSQL = strSQL & "'" & Nvl(!AKA064) & "',"
                '    AKA070  NUMBER(8,2) Y   二类最高限价    二类医院最高限价（二级医院）
                '    二类最高限价_IN 医保收费目录.二类最高限价%type:=NULL,
                strSQL = strSQL & "" & Val(Nvl(!AKA070)) & ","
                '    AKA071  NUMBER(8,2) Y   三类最高限价    三类医院最高限价（一级医院）
                '    三类最高限价_IN 医保收费目录.三类最高限价%type:=NULL,
                strSQL = strSQL & "" & Val(Nvl(!AKA071)) & ","
                '    AKA072  VARCHAR2(200)   Y   标准单位
                '    标准单位_IN 医保收费目录.标准单位%type:=NULL,
                strSQL = strSQL & "'" & Nvl(!AKA072) & "',"
                '    AKA031  VARCHAR2(1) Y   特检标志    0普通,1特殊
                '    特检标志_IN 医保收费目录.特检标志%type:=NULL,
                strSQL = strSQL & "'" & Nvl(!AKA031) & "',"
                '    AKA032  VARCHAR2(1) Y   新旧标志    0旧,1新
                '    新旧标志_IN 医保收费目录.新旧标志%type:=NULL
            ...
            Case 3
                ...
                '    AKA030  VARCHAR2(3) Y   当前使用标志    1在用,2停用
                '当前使用标志_IN IN 医保收费目录.当前使用标志%TYPE,
                strSQL = strSQL & "'" & Nvl(!AKA030) & "',"
                '社保经办机构_IN IN 医保收费目录.社保经办机构%TYPE,
                strSQL = strSQL & "NULL,"
            ...
            Case 88
            '   变更表：medicine_info_modified
            '   增加了"药品变更信息表"，此表为中心维护的新药品目录变更查询表。
            ...
            '    AKA032  VARCHAR2(1) Y   新旧目录标志    0原目录、1省目录
            '    AKA033  VARCHAR2(3) Y   特殊用药标志    0为正常用药；1为民族用药
            '    AKA030  VARCHAR2(3) Y   当前使用标志    0在用、1停用
            '    FILENAME    VARCHAR2(100)       更新文件名称
            '    CHANGETYPE  VARCHAR2(3)     变更类型
            '    SIGN    VARCHAR2(3)     新旧标志
            '    AAE036  DATE        经办日期
...
Public Function GetItemInfo_奉庆(ByVal lngPatiID As Long, ByVal lngItemID As Long, Optional ByVal str摘要 As String, Optional intType As Integer = 0) As String
    '-----------------------------------------------------------------------------------------------------------
    '--功  能:获取大连病人的相关提示信息
    '--入参数:
    '--出参数:
    ...
    gstrSQL = "Select a.项目编码,a.附注,b.编码 from 保险支付项目 a,收费细目 b where a.收费细目id=b.id and  nvl(a.是否医保,0)=0 and 险类=" & TYPE_临沧奉庆 & " and a.收费细目iD=" & lngItemID
    OpenRecordset rsTemp, "获取是否特殊治疗，特殊检查和贵重药品!", gstrSQL
    
    If rsTemp.EOF Then
        Exit Function
    End If
    
    str项目编码 = Nvl(rsTemp!项目编码, "9999999999")
    str医院内码 = Nvl(rsTemp!编码, "9999999999")
    bln药品 = Val(Nvl(rsTemp!附注)) = 1
     
    gstrSQL = "Select 医保号 From 保险帐户 where 病人id=" & lngPatiID
    OpenRecordset rsTemp, "获取医保证号", gstrSQL
    If rsTemp.EOF Then
   ...
   If bln药品 Then
    ...
    End If
End Function
Public Function 获取参保人员信息_奉庆() As Boolean
    '获取参保人员信息
    Dim StrInput As String
    Dim strOutput As String
    Dim strArr
    ...
    '＊从卡上获取医保证号
    If 解析卡_奉庆() = False Then Exit Function
    
    '＊根据医保证号获取参保信息
    StrInput = g病人身份_奉庆.医保证号
    If 业务请求_奉庆(奉庆_获得参保人员信息, StrInput, strOutput) = False Then
