...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select nvl(帐户余额,0) as 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_徐州市
    Call OpenRecordset(rsTemp, gstrSysName)
...
'        MsgBox "没有需要上传的收费记录", vbExclamation, gstrSysName
    ...
    Else
        gstrSQL = "Select nvl(退休证号,0) as 顺序号 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_徐州市
    End If
    Call OpenRecordset(rsTemp, gstrSysName)
...
'            gstrSQL = "Select * From 收费细目 Where ID=" & rs明细!收费细目ID
            gstrSQL = "select a.费用类型,A.名称,C.项目编码 as 编码,A.计算单位,B.产地,decode(B.药品来源,'国产',1,'合资',2,'进口',3,null) 产地特征,B.规格" & _
                  " from 收费细目 A,药品目录 B,保险支付项目 C where A.id = C.收费细目id and A.id=B.药品id(+) and A.id =" & rs明细!收费细目ID & " And C.险类=" & TYPE_徐州市
            Call OpenRecordset(rsTemp, gstrSysName)
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_徐州市 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "Select * from 保险帐户 where 病人ID=" & lng病人ID & " And 险类=" & TYPE_徐州市
    Call OpenRecordset(rsTemp, gstrSysName)
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_徐州市 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select nvl(顺序号,0) as 顺序号,帐户余额 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_徐州市
    Call OpenRecordset(rsTemp, gstrSysName)
    str就诊编号 = rsTemp!顺序号
    cur余额 = rsTemp!帐户余额
...
'功能：通过医保中心号码和医保号求出病人ID
    ...
    On Error GoTo errHandle
    gstrSQL = "select 病人ID from 保险帐户 where 险类 = '" & TYPE_徐州市 & _
            "' and 医保号 = '" & str医保号 & "'"
    Call OpenRecordset(rsTmp, gstrSysName)
