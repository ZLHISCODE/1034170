...
Private Sub cmd取数_Click()
    ...
    Call OpenRecordset(rsTemp, "普通门诊")
    ...
    gstrSQL = "SELECT  " & _
             ...
             " And A.并发症=" & int保险类别 & " And A.险类=" & TYPE_贵阳市 & _
             " AND A.结算时间 BETWEEN TO_DATE('" & str开始日期 & "','YYYY-MM-DD HH24:MI:SS') " & _
             " AND TO_DATE('" & str结束日期 & "','YYYY-MM-DD HH24:MI:SS')"
    Call OpenRecordset(rsTemp, "特殊门诊")
    ...
    gstrSQL = "SELECT  " & _
             ...
             " AND B.清算方式=1 And A.并发症=" & int保险类别 & " And A.险类=" & TYPE_贵阳市 & _
             " AND A.结算时间 BETWEEN TO_DATE('" & str开始日期 & "','YYYY-MM-DD HH24:MI:SS') " & _
             " AND TO_DATE('" & str结束日期 & "','YYYY-MM-DD HH24:MI:SS')"
    Call OpenRecordset(rsTemp, "控制线住院")
    ...
    gstrSQL = "SELECT  " & _
             ...
             " FROM 保险结算记录 A,ZLGYYB.结算附加信息 B,病人预交记录 C " & _
             " WHERE A.记录ID=B.结帐ID AND A.记录ID=C.结帐ID AND A.性质=2 " & _
             " AND B.清算方式=2 And A.并发症=" & int保险类别 & " And A.险类=" & TYPE_贵阳市 & _
             " AND A.结算时间 BETWEEN TO_DATE('" & str开始日期 & "','YYYY-MM-DD HH24:MI:SS') " & _
             " AND TO_DATE('" & str结束日期 & "','YYYY-MM-DD HH24:MI:SS')"
    Call OpenRecordset(rsTemp, "重症住院")
    ...
    gstrSQL = "SELECT  " & _
             ...
             " FROM 保险结算记录 A,ZLGYYB.结算附加信息 B,病人预交记录 C " & _
             " WHERE A.记录ID=B.结帐ID AND A.记录ID=C.结帐ID AND A.性质=2 " & _
             " AND B.清算方式=4 And A.并发症=" & int保险类别 & " And A.险类=" & TYPE_贵阳市 & _
             " AND A.结算时间 BETWEEN TO_DATE('" & str开始日期 & "','YYYY-MM-DD HH24:MI:SS') " & _
             " AND TO_DATE('" & str结束日期 & "','YYYY-MM-DD HH24:MI:SS')"
    Call OpenRecordset(rsTemp, "日包干住院")
    ...
    gstrSQL = " SELECT DISTINCT" & _
             ...
             "  AND A.结算时间 BETWEEN TO_DATE('" & str开始日期 & "','YYYY-MM-DD HH24:MI:SS') " & _
             "  AND TO_DATE('" & str结束日期 & "','YYYY-MM-DD HH24:MI:SS')"
    Call OpenRecordset(rsTemp, "日包干住院天数")
    With rsTemp
        Do While Not .EOF
            If !入院期号 <> str期号 Then
                '当期以前入的院
...
Private Sub cmd申报_Click()
    ...
    Call InsertChild(mdomInput.documentElement, "INSURETYPE", cbo保险类别.ListIndex + 1)
    ...
    Call InsertChild(mdomInput.documentElement, "ZYPSNSA", Val(txt就诊人次(控制线住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYACCTA", Val(txt个人帐户(控制线住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND1A", Val(txt统筹基金(控制线住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND2A", Val(txt大额统筹(控制线住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND3A", Val(txt医疗补助(控制线住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYPSNSB", Val(txt就诊人次(重症住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYACCTB", Val(txt个人帐户(重症住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND1B", Val(txt统筹基金(重症住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND2B", Val(txt大额统筹(重症住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND3B", Val(txt医疗补助(重症住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYPSNSC", Val(txt就诊人次(日包干住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYDAYSC", Val(txt住院天数(日包干住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYACCTC", Val(txt个人帐户(日包干住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND3C", Val(txt医疗补助(日包干住院).Text))
    Call InsertChild(mdomInput.documentElement, "ZYPSNSD", Val(txt就诊人次(包干结算).Text))
    Call InsertChild(mdomInput.documentElement, "ZYACCTD", Val(txt个人帐户(包干结算).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND1D", Val(txt统筹基金(包干结算).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND2D", Val(txt大额统筹(包干结算).Text))
    Call InsertChild(mdomInput.documentElement, "ZYFUND3D", Val(txt医疗补助(包干结算).Text))
    '调用接口
    ...
    gstrSQL = "ZL_清算单_INSERT(" & mlngID & ",0,'" & Me.cbo期号.Text & "'," & Me.cbo保险类别.ListIndex + 1 & "," & _
...
Private Sub Form_Load()
    ...
    gstrSQL = "SELECT  " & _
             ...
             " WHERE A.ID=B.清算单ID AND A.ID=" & mlngID
    Call OpenRecordset(rsData, "读取申报单数据")
