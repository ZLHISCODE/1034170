...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    '取保险参数“离休就诊录入诊断”
    gstrSQL = "Select Nvl(参数值,0) AS 参数值 From 保险参数 Where 险类=" & TYPE_重庆市 & " And 参数名='离休就诊录入诊断'"
    Call OpenRecordset(rsTemp, "取保险参数“离休就诊录入诊断”")
    ...
    If lngTemp <> 0 Then
        ...
        lng病人ID = lngTemp
        '提取上次就诊的业务类型
        gstrSQL = "Select 业务类型,帐户余额,退休证号,病种名称,并发症 From 保险帐户 Where 险类=" & TYPE_重庆市 & " ANd 病人ID=" & lng病人ID
        Call OpenRecordset(rsTemp, "提取业务类型")
        int业务类型 = rsTemp!业务类型
        ...
        gstrSQL = " Select A.姓名,A.性别,A.年龄,A.身份证号,A.出生日期,B.人员身份 AS 人员类别,A.工作单位 AS 单位名称,B.业务类型,C.HIS流水号 AS 顺序号 " & _
                  " From 病人信息 A,保险帐户 B,就诊登记记录 C" & _
                  " Where C.主页ID=0 And C.记录ID Is Not NULL And C.病人ID=A.病人ID And A.病人ID=B.病人ID And B.险类=C.险类" & _
                  " And C.险类=" & TYPE_重庆市 & " And C.就诊时间=to_Date('" & str上次就诊时间 & "','yyyy-MM-dd hh24:mi:ss') And C.病人ID=" & lng病人ID
        Call OpenRecordset(rsTemp, "取病人信息")
...
'        Else
            ...
            If int类别 = 13 Or int类别 = 15 Or int类别 = 14 Or (Mid(str医保号, 1, 1) = "2" And bln离休就诊录入诊断) Then
                If int类别 = 13 Then
                    '获得审批信息
                    StrInput = "07|" & str医保号
                    If HandleBusiness(StrInput, arrOutput) = False Then Exit Function
...
'        Call zlDatabase.ExecuteProcedure(gstrSQL, "更新病种")
    ...
    gstrSQL = "zl_就诊登记记录_UPDATE(" & _
        ...
        "'" & str疾病编码 & "-" & str疾病名称 & "','" & str并发症 & "','" & str封锁信息 & "','" & str门诊号 & "','" & str门诊号 & "',NULL)"
...
'返回: 返回个人帐户余额的金额
    ...
    If mstr医保号 = "" Or strSelfNo <> mstr医保号 Then
        gstrSQL = "Select 帐户余额 From 保险帐户 where 险类=" & TYPE_重庆市 & " and 中心=0 and 医保号='" & strSelfNo & "'"
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    Dim dbl医保基金_CUR As Double, dbl个人帐户_CUR As Double, dbl公务员补助_CUR As Double, dbl大额统筹_CUR As Double, dbl公务员返还_CUR As Double, dbl医院收支_CUR As Double
    Dim dbl医保基金 As Double, dbl个人帐户 As Double, dbl公务员补助 As Double, dbl大额统筹 As Double, dbl公务员返还 As Double, dbl医院收支 As Double
    ...
    Do Until rs明细.EOF
        gstrSQL = "select A.名称,A.编码,A.类别,A.规格,A.计算单位,B.项目编码,B.附注,A.计算单位,E.规格,G.名称 剂型 " & _
                  ...
                  "where A.ID=" & rs明细("收费细目ID") & " and A.ID=B.收费细目ID and B.险类=" & TYPE_重庆市 & _
        ...
        If Val(Nvl(rs明细("实收金额"), 0)) <> 0 Then
            ...
            If HandleBusiness(StrInput, arrOutput) = False Then Exit Function
            Call AddMessage(strMessage, arrOutput, ToVarchar(rsTemp("名称"), 50), Format(rs明细!实收金额 / Round(rs明细!数量, 2), "0.0000"))
        End If
        rs明细.MoveNext
    Loop
...
'    cur发生费用 = Val(Format(cur发生费用, "#0.00;-#0.00;0;"))
    ...
    gstrSQL = "Select 退休证号 As 病种编码,病种名称,并发症,封锁信息" & _
        " From 保险帐户 " & _
        " Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_重庆市
    ...
    If mbln结算 = False Then
        ...
        If Not mbln多单据收费 Then
            '取保险参数“离休就诊录入诊断”
            gstrSQL = "Select Nvl(参数值,0) AS 参数值 From 保险参数 Where 险类=" & TYPE_重庆市 & " And 参数名='个人帐户'"
            Call OpenRecordset(rsTemp, "取保险参数“离休就诊录入诊断”")
            If rsTemp.RecordCount <> 0 Then
        ...
        If Not mbln多单据收费 And bln门诊 Then
            ...
            If pre_Balance.cur个人帐户 <> cur个人帐户 Then
                blnRevise = True
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & TYPE_重庆市 & "," & lng病人ID & "," & _
        ...
        cur统筹支付 & "," & cur统筹支付 & ",0," & g结算数据.超限自付金额 & "," & cur个人帐户 & ",'" & mstr结算流水号 & "',NULL,NULL,'" & str备注 & "'" & _
...
'    Loop
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_重庆市 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "重庆医保")
    If rsTemp.EOF = True Then
...
'返回：交易成功返回true；否则，返回false
    ...
    '调用入院接口
    StrInput = "02|" & GetIdentify(lng病人ID, lng主页ID, True) & "|" & IIf(rsTemp("入院方式") = "转入", "22", "21") & "|" & str医保号 & "|" & _
...
'返回：交易成功返回true；否则，返回false
    ...
    If bln零费用出院 = True Then
        ...
        gstrSQL = "Select 顺序号 from 保险帐户 where 病人ID=" & lng病人ID & " and 险类=" & TYPE_重庆市
        Call OpenRecordset(rsTemp, "病人出院")
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    '1.3 读出本次住院上次中结时间，如果为空，上次中结结束时间为入院日期
     gstrSQL = " Select Max(发生时间) AS 结束日期 From 病人费用记录" & _
               " Where 结帐Id=(" & _
               "    Select Max(ID) As 结帐ID" & _
               "    From 病人结帐记录" & _
               "    Where 病人ID=" & g结算数据.病人ID & " And 记录状态 = 1 " & _
               "    And 收费时间>to_date('" & str入院日期 & " 00:00:00','yyyy-MM-dd hh24:mi:ss'))" & _
               "    And Nvl(附加标志,0)<>9"
    Call OpenRecordset(rsTemp, "读出本次住院上次中结时间")
    If rsTemp.RecordCount = 0 Then
        str上次结算日期 = str入院日期
    Else
    ...
    If g结算数据.中途结帐 = 0 Or bln费用查询 Then
        ...
        gstrSQL = "Select A.ID,A.NO,A.记录性质,A.记录状态,A.序号,A.病人ID,A.主页ID,A.发生时间 as 登记时间,Round(A.实收金额,4) 实收金额" & _
                  ...
                  "  where A.病人ID=" & lng病人ID & " and A.主页ID=" & g结算数据.主页ID & " and A.记帐费用=1 and Nvl(A.实收金额,0)<>0 and nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0 " & _
                  "        and A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=" & TYPE_重庆市 & _
...
'        Loop
    ...
    str总金额医保 = Format(pre_Balance.cur统筹支付 + pre_Balance.cur个人帐户 + pre_Balance.cur公务员补助 + pre_Balance.cur大额统筹 + pre_Balance.cur医院收支 + Val(arrOutput(4)), "#####0.00")
...
'      3)由于结帐之后，可能使用结帐作废交易，这时需要结帐时执行结算交易的交易号，因此我们需要同时结帐交易号。(由于门诊收费作废时，已经不再和医保有关系，所以不需要保存结帐的交易号)
    ...
    Dim cur大额统筹 As Double, cur公务员补助 As Double, cur公务员返还 As Double, cur医院收支 As Double
    ...
    If Trim(str病种编码) <> "" Then
        ...
        Call OpenRecordset(rsTemp, "读取病种分类，供判断是否是单病种", gstrSQL, mcnYB)
...
'                    lng结算标志 = 10 '中途结算
    ...
    If Not (cur个人帐户 = pre_Balance.cur个人帐户 And cur统筹支付 = pre_Balance.cur统筹支付 And _
        ...
        cur公务员返还 = pre_Balance.cur公务员返还 And cur医院收支 = pre_Balance.cur医院收支) Then
...
Public Function 住院结算冲销_重庆(lng结帐ID As Long) As Boolean
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_重庆市 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "结算冲销")
    If rsTemp.EOF = True Then
        MsgBox "原单据的医保记录不存在，不能作废。", vbInformation, gstrSysName
        Exit Function
...
'返回:
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.主页ID,A.发生时间 as 登记时间,Round(A.实收金额,4) 实收金额 " & _
              ...
              "         ,C.项目编码,B.编码,B.名称,A.是否急诊,nvl(A.开单人,A.操作员姓名) as 医生,A.操作员姓名,B.计算单位,E.规格,G.名称 剂型 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
              "  where A.NO='" & str单据号 & "' and A.记录性质=" & int性质 & " and A.记录状态=1 And Nvl(A.是否上传,0)=0 And Nvl(A.实收金额,0)<>0 " & _
              "        and A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=" & TYPE_重庆市 & IIf(lng病人ID = 0, "", " and A.病人ID=" & lng病人ID) & _
              "        and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类=D.险类 " & _
              "        AND B.ID=E.药品ID(+) AND E.药名ID=F.药名ID(+) AND F.剂型=G.编码(+) " & _
...
'返回:
    ...
    gstrSQL = "Select nvl(count(A.ID),0) as 总数,nvl(sum(A.是否上传),0) 上传数 " & _
              ...
              "  where A.NO='" & str单据号 & "' and A.记录性质=" & int性质 & " and A.记录状态<>2 And Nvl(A.记录状态,0)<>0 and nvl(A.实收金额,0)<>0  " & _
              "        and A.病人ID=B.病人ID and A.主页ID=B.主页ID And B.险类=" & TYPE_重庆市 & " and A.收费细目ID=C.收费细目ID and B.险类=C.险类"
    ...
    gstrSQL = " Select A.ID,A.收费细目ID,A.NO,A.记录性质,A.记录状态,A.序号,A.病人ID,A.主页ID,A.发生时间 as 登记时间,Round(A.实收金额,4) 实收金额" & _
              ...
              "         ,C.项目编码,B.编码,B.名称,A.是否急诊,nvl(A.开单人,A.操作员姓名) as 医生,A.操作员姓名,B.计算单位,E.规格,G.名称 剂型 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
              "  where A.NO='" & str单据号 & "' and A.记录性质=" & int性质 & " and A.记录状态=2 and nvl(A.是否上传,0)=0 And Nvl(A.实收金额,0)<>0" & _
              "        and A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=" & TYPE_重庆市 & _
              "        and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类=D.险类 " & _
              "        AND B.ID=E.药品ID(+) AND E.药名ID=F.药名ID(+) AND F.剂型=G.编码(+) " & _
              "  Order by A.发生时间,A.记录性质,Decode(A.记录状态,2,2,1)"
...
Private Function UploadNegative(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lngID As Long, ByVal lng收费细目ID As Long) As String
    ...
    gstrSQL = "Select A.ID,A.NO,A.记录性质,A.记录状态,A.病人ID,A.主页ID,A.发生时间 as 登记时间,Round(A.实收金额,4) 实收金额 " & _
              ...
              "         ,C.项目编码,B.编码,B.名称,A.是否急诊,nvl(A.开单人,A.操作员姓名) as 医生,A.操作员姓名,B.计算单位,E.规格,G.名称 剂型 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
              "  where A.病人ID=D.病人ID and A.主页ID=D.主页ID And D.险类=" & TYPE_重庆市 & _
              "        And A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类=D.险类 And Nvl(A.是否上传,0)=0" & _
              "        AND B.ID=E.药品ID(+) AND E.药名ID=F.药名ID(+) AND F.剂型=G.编码(+) And A.ID=" & lngID
    Call OpenRecordset(rsSource, "读取本次待冲销总数")
    ...
    '读取本记录已冲销的数量
    gstrSQL = " Select SUM(数量) AS 已冲销数量 From 负数冲销明细 Where 费用ID=" & lngID
    Call OpenRecordset(rsTemp, "读取本记录已冲销的数量")
    ...
    Call OpenRecordset(rsFilter, "读取存在剩余数量可冲销的原始记录")
...
Private Function TrackRecordInsure(ByVal lng费用ID As Long, ByVal lng收费细目ID As Long) As Boolean
    ...
    gstrSQL = "Select A.类别,B.项目编码 " & _
        ...
        " Where A.ID=B.收费细目ID And B.险类=" & TYPE_重庆市 & " And A.ID=" & lng收费细目ID
...
Private Function GetRegisted(ByVal str医保号 As String, ByRef str就诊时间 As String) As Long
    ...
    gstrSQL = " Select A.病人ID,A.就诊时间" & _
              ...
              " And B.险类=" & TYPE_重庆市 & " And B.医保号='" & str医保号 & "'" & _
              " And A.就诊时间 Between to_date('" & strStart & "','yyyy-MM-dd hh24:mi:ss')" & _
              " And to_date('" & strEnd & "','yyyy-MM-dd hh24:mi:ss')"
    Call OpenRecordset(rsTemp, "判断该病人当天是否就诊过")
    If rsTemp.RecordCount = 0 Then Exit Function
