...
Public Function MakeTransNO() As String
    Randomize
    MakeTransNO = Format(Date, "yymmdd") & Format(Time, "hhmmss") & Format(999999 * Rnd + 1, "0#####")
End Function

Public Function Get医保号(lng病人ID As Long) As String
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select * From 保险帐户 Where 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "获取医保号")
    If rsTemp.EOF Then
        Get医保号 = ""
    Else
...
Public Function openConn徐州() As Boolean
    ...
    If gcn徐州.State <> adStateOpen Then
        gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & TYPE_徐州
...
'        gcn徐州.ConnectionString = "Provider=SQLOLEDB.1;Initial Catalog=" & strDatabase & ";Password=" & strPass & ";Persist Security Info=True;User ID=" & strUser & ";Data Source=" & strServer
...
Public Function 医保初始化_徐州() As Boolean
    ...
    gstrSQL = "Select * From 保险类别 Where 序号=" & TYPE_徐州
...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_徐州
...
'字段:病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    While Not rs明细.EOF
        gstrSQL = "Select A.类别,B.项目编码,B.项目名称 From 收费细目 A,保险支付项目 B Where A.ID=B.收费细目ID " & _
            "And A.ID=" & rs明细!收费细目ID & " And B.是否医保=1 And B.险类=" & TYPE_徐州
        Call OpenRecordset(rsTemp, gstrSysName)
...
Public Function 门诊结算_徐州(lng结帐ID As Long, cur个帐支付 As Currency, str医保号 As String, cur全自付 As Currency, cur先自付 As Currency, cur医保基金 As Currency) As Boolean
    ...
    gstrSQL = "Select * From 保险帐户 Where 医保号='" & str医保号 & "' And 险类=" & TYPE_徐州
    Call OpenRecordset(rsTemp, "取医保信息")
    ...
    gstrSQL = "Select * From 病人信息 Where 病人ID=" & lng病人ID
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "Select * From 保险帐户 Where 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取帐户信息")
    ...
    '取原单据交易数据
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_徐州 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'    If rsTemp.EOF Then
'        WriteInfo "取原医保记录失败"
'        MsgBox "前置机数据库中未找到原有交易记录，不能作废", vbInformation, gstrSysName
'        Exit Function
'    End If
...
'返回:交易成功返回true；否则，返回false
    ...
    gstrSQL = "Select * From 保险帐户 Where 病人ID = " & lng病人ID & " And 险类=" & TYPE_徐州
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
...
Public Function 医保出院(lng病人ID As Long, lng主页ID As Long, ByRef str请求 As String) As Boolean
    ...
    gstrSQL = "Select * From 病人费用记录 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID & " And " & "门诊标志=2"
    Call OpenRecordset(rsTemp, gstrSysName)
...
Public Function 记帐传输_徐州(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人ID As Long = 0) As Boolean
    ...
    If str单据号 <> "" Then
        gstrSQL = " Select A.* From 病人费用记录 A,保险帐户 B" & _
                  ...
                  " And A.病人ID=B.病人ID And B.险类=" & TYPE_徐州 & _
...
'    gcnOracle.Execute "Delete From 县医保明细 Where 病人id=" & lng病人id & " And 主页ID=" & lng主页ID
    ...
    While Not rs明细.EOF
        ...
        gstrSQL = "Select * From 保险支付项目 Where 收费细目ID=" & rs明细!收费细目ID & " And 险类=" & _
            TYPE_徐州 & " And 是否医保=1"
...
'                str项目编码 = "$$" & rsTemp!项目编码
        ...
        gstrSQL = "Select * From 医保病人状态 Where to_char(日期,'yyyy-mm-dd')='" & Format(rs明细!发生时间, "yyyy-mm-dd") & "' And 病人ID=" & lng病人ID
        Call OpenRecordset(rsTemp, "取病人医保状态")
        If rsTemp.EOF Then
            str医保状态 = "0"
        Else
...
Public Function 住院虚拟结算_徐州(rs明细 As Recordset, lng病人ID As Long, str医保号 As String) As String
    ...
    gstrSQL = "Select NO From 病人费用记录 Where 门诊标志=2 And 病人id=" & lng病人ID & " And 主页ID=" & lng主页ID & " Group By NO"
    Call OpenRecordset(rsTemp, gstrSysName)
    
    mcur全自理 = 0: mcur部分自理 = 0: mcur总额 = 0
    
    While Not rsTemp.EOF
        gstrSQL = "Select A.类别,B.实收金额,A.ID,B.NO From 收费细目 A,病人费用记录 B Where A.ID=B.收费细目ID And " & _
            "B.门诊标志=2 And B.NO='" & rsTemp!NO & "' And B.病人ID=" & lng病人ID & " And B.主页ID=" & lng主页ID & " And nvl(实收金额,0)<>0"
        Call OpenRecordset(rs费用明细, gstrSysName)
        ...
        While Not rs费用明细.EOF
            gstrSQL = "Select * From 保险支付项目 Where 收费细目ID=" & rs费用明细!ID
            Call OpenRecordset(rs徐州, gstrSysName)
            If rs徐州.EOF Then
...
Public Function 住院结算_徐州(lng结帐ID As Long, lng病人ID As Long) As Boolean
    ...
    '改写IC卡状态
    If WriteCard(0) = False Then
        MsgBox "写卡状态时失败，但病人出院的操作不受影响", vbInformation, "写卡"
    End If
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_徐州 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'        MsgBox "住院结算冲销前请先撤消病人出院", vbInformation, gstrSysName
'        Exit Function
'    End If
