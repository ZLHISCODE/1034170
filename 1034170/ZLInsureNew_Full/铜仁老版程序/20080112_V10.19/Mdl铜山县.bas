...
''
        '取医院编码
        gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_铜山县
        ...
        Else
            ...
            Call OpenRecordset(rsTemp, "铜山县医保", gstrSQL, gcn铜山县)
            
            If rsTemp.EOF Then
                gstrSQL = "Select * from czry Where P_GLY=1"
                Call OpenRecordset(rsCsh, "铜山县医保", gstrSQL, gcn铜山县)
...
'    End If
    '>>End 结算前要求刷卡
    gstrSQL = "Select 就诊类别 From 保险帐户 Where 险类=" & intInsure & " ANd 医保号='" & g常用信息_铜山.个人编号 & "'"
    Call OpenRecordset(rsMzxnjs, "取就诊类别")
    str就诊类型 = rsMzxnjs!就诊类别
    
    rsHis.Filter = "实收金额 <> 0"
    Set rs明细 = rsHis
    
    gstrSQL = "Select * from 部门表 where ID=" & rs明细!开单部门ID
    Call OpenRecordset(rsMzxnjs, "部门表")
    str科室编码 = rsMzxnjs!编码
    gstrSQL = "Select * from 人员表 where 姓名='" & rs明细!开单人 & "'"
    Call OpenRecordset(rsMzxnjs, "人员表")
    str医生号 = rsMzxnjs!编号
    ...
    'If lng结帐ID = 0 Then
        ...
        Do Until rs明细.EOF
            ...
            Call OpenRecordset(rsMzxnjs, "取收费类别")
            ...
            Call OpenRecordset(rsMzxnjs, "ypzlk", , gcn铜山县)
...
'TODO:门诊结算
    ...
    Call OpenRecordset(rsMzjs, "门诊结算明细", gstrSQL)
...
'TODO:门诊冲销
    ...
    gstrSQL = "Select * from 保险帐户 where 病人ID=" & lng病人ID & " And 险类=" & TYPE_铜山县
    Call OpenRecordset(rsMzJsCx, "保险帐户")
    If rsMzJsCx.EOF Then
        MsgBox "不是铜山县医保参保人员,不能冲销!", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    Call OpenRecordset(rsMzJsCx, "保险结算记录")
    lng冲销ID = rsMzJsCx!结帐ID
    
    gstrSQL = "Select * From 保险结算记录 Where 性质=1 And 记录ID=" & lng结帐ID & " and 险类=" & TYPE_铜山县
    Call OpenRecordset(rsMzJsCx, "取结算流水号")
    If rsMzJsCx.RecordCount = 0 Then
        MsgBox "没有找到原始结算记录，无法进行门诊结算冲销！"
        Exit Function
    End If
...
'***************************************************************************************************
    ...
    gstrSQL = "Select * from 保险帐户 where 病人ID=" & lng病人ID & " and 险类=" & TYPE_铜山县 & _
              " and nvl(备注,'0')<>'0'"
                  
    Call OpenRecordset(rsRydj, "保险帐户")
    ...
    gstrSQL = "Select A.*,B.编码 as 病区 from 病案主页 A ,部门表 B" & _
               " where A.病人ID=" & lng病人ID & _
               " And A.主页ID=" & lng主页ID & _
               " And A.险类=" & TYPE_铜山县 & _
               " And A.入院科室ID=B.ID"
               
    Call OpenRecordset(rsRydj, "病案主页")
    If rsRydj.EOF Then
...
'TODO:入院登记撤销
    ...
    '结过帐的不允许撤销
    gstrSQL = "Select sum(nvl(结帐金额,0)) as 结帐金额 from 病人费用记录 where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsRydjCx, "保险帐户")
    ...
    gstrSQL = "Select * from 保险帐户 where 险类=" & intInsure & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsRydjCx, "保险帐户")
    str流水号 = rsRydjCx!顺序号
    ...
    If lngReturn = 0 Then
        '改变所有已上传记录为未上传
        gstrSQL = "Update 病人费用记录 Set 是否上传=0 where nvl(是否上传,0)=1 and 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
...
'****************************************
    ...
    Call OpenRecordset(rsCydj, "病案主页")
    If rsCydj.EOF Then
    ...
    gstrSQL = "Select * from 部门表 where ID=" & str入院科室
    Call OpenRecordset(rsCydj, "入院科室")
    If rsCydj.EOF Then
   ...
   gstrSQL = "Select * from 人员表 A,人员性质说明 B where A.ID=B.人员ID and B.人员性质='医生' and  A.姓名='" & str住院医生 & "'"
    Call OpenRecordset(rsCydj, "医生")
    If rsCydj.EOF Then
    ...
    gstrSQL = "Select * from 人员表 A,人员性质说明 B where A.ID=B.人员ID and B.人员性质='医生' and  A.姓名='" & str门诊医生 & "'"
    Call OpenRecordset(rsCydj, "医生")
    If rsCydj.EOF Then
    ...
    If str出院特征 <> 0 Then
        ...
        Call OpenRecordset(rsCydj, "医院列表", , gcn铜山县)
        strTmp = ""
        strTmp1 = ""
        Do Until rsCydj.EOF
    ...
    Call OpenRecordset(rsCydj, "保险帐户")
    If rsCydj.EOF Then
    ...
    Call OpenRecordset(rsCydj, "病种ID")
    If rsCydj.EOF Then
            '强制选择病种
...
'******************************************
    ...
    gstrSQL = "Select Nvl(帐户余额,0) AS 个人帐户 From 保险帐户 " & _
              " Where 病人ID=" & lng病人ID & " and 险类=" & TYPE_铜山县
              
    Call OpenRecordset(rsTemp, "提取病人ID")
...
'*********************************
    ...
    Call OpenRecordset(rsZyjs, "保险帐户")
    ...
    Call OpenRecordset(rsZyjs, "ICD10", , gcn铜山县)
    If rsZyjs.RecordCount > 0 Then
        str病种编码 = rsZyjs!病种编码
    End If
    
    gstrSQL = "Select * from 病人信息 where 病人ID=" & lng病人ID & " And 险类=" & intInsure
    Call OpenRecordset(rsZyjs, "病案主页")
    If Nvl(rsZyjs!出院时间, 0) = 0 Then
        MsgBox "不支持中途结算，请先办理出院后再进行结算！", vbInformation, gstrSysName
        Exit Function
    End If
    str出院日期 = Format(Nvl(rsZyjs!出院时间, Now()), "yyyyMMdd")
    lng主页ID = Nvl(rsZyjs!住院次数, 0)
    
    gstrSQL = "Select *  From 诊断情况 Where 病人ID=" & lng病人ID & _
              " And 主页ID=" & lng主页ID & " And 诊断类型=3 And 诊断次序=1"
              
    Call OpenRecordset(rsZyjs, "出院情况")
    If rsZyjs.EOF Then
        str出院性质 = 2
    Else
    ...
    '>>Beging 欠款重结
    If lng欠款重结ID > 0 Then
        gstrSQL = "Select * from 保险结算记录 where 记录ID=" & lng欠款重结ID
        Call OpenRecordset(rsZyjs, "保险结算记录")
        If rsZyjs.EOF Then
            MsgBox "未找到原始结帐记录，不能执行后续操作！", vbInformation, gstrSysName
            Exit Function
        End If
        str流水号 = rsZyjs!支付顺序号
        '>>Beging 调QKCJS交易
...
'*********************************
    ...
    gstrSQL = "Select distinct 记录性质,记录状态,NO From 病人费用记录 A,保险帐户 B,病人信息 C " & _
              ...
              " AND A.实收金额 IS NOT NULL And B.病人ID=" & lng病人ID & " And B.险类=" & intInsure
     Call WriteBusinessLOG("取病人费用记录", gstrSQL, "")
     
     Call OpenRecordset(rsZyxnjs, "取病人费用记录")
...
'         End If
    ...
    '>>Beging 欠款重结
    gstrSQL = "Select * from 保险帐户 where 险类=" & intInsure & " And 病人ID=" & lng病人ID & " And nvl(欠款重结ID,0)>0"
    Call OpenRecordset(rsZyxnjs, "保险帐户")
    ...
    gstrSQL = "Select * from icd10 where ID=" & str病种编码
    Call OpenRecordset(rsZyxnjs, "ICD10", , gcn铜山县)
    ...
    gstrSQL = "Select * from 病人信息 where 病人ID=" & lng病人ID & " And 险类=" & intInsure
    Call OpenRecordset(rsZyxnjs, "病案主页")
    ...
    Call OpenRecordset(rsZyxnjs, "出院情况")
    If rsZyxnjs.EOF Then
        str出院性质 = 2
    Else
    ...
    If lngReturn <> -1 Then
        '>Beging 检查总金额是否相等
        gstrSQL = "Select sum(nvl(实收金额,0))-sum(nvl(结帐金额,0)) as 未结费用 From 病人费用记录 Where nvl(记录状态,0)<>0 and 记帐费用=1 And 病人ID= " & lng病人ID
        Call OpenRecordset(rsZyxnjs, "未结费用", gstrSQL)
...
'*********************************
    ...
    gstrSQL = "Select * From 结算方式 Where 名称 In (" & _
                    ...
                    "where 结帐ID=" & lng结帐ID & ") And 性质>=3 And  性质<=4"
    Call OpenRecordset(rsZyjsCx, "结算方式")
    If Not rsZyjsCx.EOF Then
        'MsgBox "本医保不支持冲销操作!" & vbCrLf & "如要冲销已结算单据，请到医保中心办理！", vbInformation, gstrSysName
        bln结算冲销 = True
    End If
    
    gstrSQL = "Select * From 保险结算记录 Where 性质=2 And 记录ID=" & lng结帐ID & " and 险类=" & intInsure
    Call OpenRecordset(rsZyjsCx, "取结算流水号")
    If rsZyjsCx.EOF Then
        MsgBox "没有找到原始结算记录，无法进行住院结算冲销！"
        Exit Function
    End If
    ...
    Else
        ...
        Call OpenRecordset(rsZyjsCx, gstrSysName)
        lng冲销ID = rsZyjsCx("ID")
        
        gstrSQL = "Select * from 保险帐户 where 病人ID=" & lng病人ID & " And 险类=" & TYPE_铜山县
        Call OpenRecordset(rsZyjsCx, "保险帐户")
        ...
        If lngReturn <> -1 Then
            gstrSQL = "Select * From 保险结算记录 Where 性质=2 And 记录ID=" & lng结帐ID & " and 险类=" & TYPE_铜山县
            Call OpenRecordset(rsZyjsCx, "保险结算记录")
...
'    '或者
    ...
    gstrSQL = "Select distinct  decode(nvl(A.医嘱序号,-99),-99,to_char(A.发生时间,'YYYY-MM-DD'),to_char(A.登记时间,'YYYY-MM-DD')) as 费用日期,A.病人ID,A.主页ID from 病人费用记录 A,保险帐户 B " & _
            "where A.病人ID=B.病人ID And A.记录性质=" & int性质 & _
            "  And nvl(A.附加标志,0)<>9 And Nvl(a.实收金额, 0)<>0 and  A.记录状态=" & int状态 & " And A.NO='" & str单据号 & "' " & _
            " And B.险类=" & intInsure
    If lng病人ID <> 0 Then
        gstrSQL = gstrSQL & " And A.病人ID=" & lng病人ID
    End If
    Call WriteBusinessLOG("取要上传单据(处方上传)", gstrSQL, "")
    Call OpenRecordset(rsCfsc, "取病人ID")
    ...
    Do Until rsCfsc.EOF
        ...
        gstrSQL = "Select * from 保险帐户 where 险类=" & intInsure & " And 病人ID=" & lngPatiID
        Call OpenRecordset(rsMzxnjs, "保险帐户")
        ...
        Do Until rs记帐明细.EOF
            ...
            gstrSQL = "select A.*,B.药品类型 from 收费细目 A,药品信息 B,药品目录 C " & _
                      " where A.id=C.药品ID(+) and C.药名ID=B.药名ID(+) And A.id=" & rs记帐明细!收费细目ID
            Call OpenRecordset(rsMzxnjs, "取项目类别")
            Select Case rsMzxnjs!类别
                Case "5", "6", "7"
                    str药品 = "0"
                Case Else
                    str药品 = "1"
...
'            Call OpenRecordset(rs记帐明细, "记帐明细", gstrSQL)
                ...
                gstrSQL = "Select B.编码 from 病案主页 A,部门表 B where A.病人ID=" & lngPatiID & " And A.主页ID=" & lngPageID & " And A.入院病区ID=B.ID"
                Call OpenRecordset(rsMzxnjs, "zlypk")
                If rsMzxnjs.EOF = False Then
                    str部门 = rsMzxnjs!编码
                ...
                Call OpenRecordset(rsMzxnjs, "zlypk", , gcn铜山县)
                If rsMzxnjs.EOF = False Then
                    lngReturn = tsx_setstringparam(P_ZBM, lngCount, rsMzxnjs!自编码) 'C20 自编码  费用明细自编码
                    Call WriteBusinessLOG("tsx_setstringparam", "P_ZBM" & ", " & lngCount & "," & rsMzxnjs!自编码, lngReturn)
                    If lngReturn = -1 Then
        ...
        '3 调用接口
            ...
            '无反回值
                ...
                Else
                    ...
                    Call OpenRecordset(rsMzxnjs, "已上传明细的ID")
                    Do Until rsMzxnjs.EOF
                        ...
                        '>>>Beging 取xybmx里的状态
                        gstrSQL = "Select * from 保险帐户 where 险类=" & TYPE_铜山县 & " And 病人ID=" & lngPatiID
                        Call WriteBusinessLOG("取xybmx里的状态", gstrSQL, "")
                        Call OpenRecordset(rsXybmx, "保险帐户")
                        str个人编号 = rsXybmx!医保号
                        ...
                        gstrSQL = "Select b.名称, b.计算单位 As 单位, Nvl(a.实收金额, 0) As 实收金额," & _
                                  ...
                                  " From 病人费用记录 A,收费细目 B,(Select * From 保险支付项目 where 险类=" & TYPE_铜山县 & ") C " & _
                                  " Where  Nvl(a.实收金额, 0) <>0 and A.收费细目ID=B.ID And A.收费细目ID=C.收费细目ID(+) and A.ID=" & rsMzxnjs!ID
                        Call WriteBusinessLOG("查费用明细准备写xybmx", gstrSQL, "")
                        Call OpenRecordset(rs费用记录, "费用记录")
                        
                        If rs费用记录.EOF = False Then
                            gstrSQL = "Select * from XYBMX Where 记录ID=" & rsMzxnjs!ID
                            Call WriteBusinessLOG("查xybmx", gstrSQL, "")
                            Call OpenRecordset(rsXybmx, "费用记录", , gcn铜山县)
...
'TODO:医保项目信息
    ...
    Call OpenRecordset(rsYbxm, "ypzlk", , gcn铜山县)
