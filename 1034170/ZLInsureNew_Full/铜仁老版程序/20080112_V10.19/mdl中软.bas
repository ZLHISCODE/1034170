        ...
        With gIC中软
            ...
            If bytType = 1 Then
                ...
                gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
                        " From 保险病种 A where 1=2 And A.险类=" & TYPE_自贡市
                Call OpenRecordset(rsSelected, "获取已选择的病种")
...
VirusSelect:
            ...
            '得到中心序号
            gstrSQL = "select 序号 from 保险中心目录 where 险类=" & TYPE_自贡市 & " and 编码='" & .CenterCode & "'"
            ...
            '得到原住院病种
            If bytType <> 1 Then
                gstrSQL = "Select Nvl(病种ID,0) 病种ID From 保险帐户 Where 险类=" & TYPE_自贡市 & " And 医保号='" & TrimStr(.MediAccountNo) & "'"
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    If bln离休 = False Then
        If ReadICCard(ic门诊读卡) <> 0 Then
            MsgBox "收费时读卡失败。", vbInformation, gstrSysName
            Exit Function
        End If
    ...
    If bln离休 = False Then
        ...
        If lngReturn Then
            MsgBox "费用写入卡失败。" & 错误信息_中软(lngReturn), vbInformation, gstrSysName
            Exit Function
        End If
    End If
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select 病人ID,发生费用金额,全自付金额,首先自付金额,进入统筹金额,年度  From 保险结算记录 Where 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "中软医保")
    ...
    If bln离休 = False Then
        If ReadICCard(ic门诊读卡) <> 0 Then
            MsgBox "退费时读卡失败。", vbInformation, gstrSysName
            Exit Function
        End If
        If ic门诊读卡.InpatientFlag = "1" Then
            MsgBox "该病人仍然在院，不能继续。", vbInformation, gstrSysName
    ...
    gstrSQL = "select B.编码,B.序号 " & _
            ...
            " where A.病人ID=" & lng病人ID & " and A.险类=" & TYPE_自贡市 & _
    ...
    If lng原医保年 < g结算数据.年度 Then
       MsgBox "不能冲销非本医保年度的门诊收费记录。", vbInformation, gstrSysName
       Exit Function
    End If
    dat当前日期 = zlDatabase.Currentdate
    ...
    If bln离休 = False Then
        ...
        lngReturn = ChargeWrite(ic门诊写卡)
        If lngReturn Then
            MsgBox "退费时写入卡失败。" & 错误信息_中软(lngReturn), vbInformation, gstrSysName
            Exit Function
        End If
    End If
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    With g结算数据
        ...
        '避免在出院结帐后再次进行结帐
        gstrSQL = "SELECT 病人ID FROM 保险结算记录 WHERE 中途结帐=0 AND 病人ID=" & .病人ID & " AND 主页ID=" & .主页ID & " AND 险类=" & TYPE_自贡市
        Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    gstrSQL = "select A.病种ID,B.编码,B.序号 " & _
            " from 保险帐户 A,保险中心目录 B " & _
            " where A.病人ID=" & g结算数据.病人ID & " and A.险类=" & TYPE_自贡市 & _
            "  and A.险类=B.险类 and A.中心=B.序号"
    Call OpenRecordset(rsTemp, "虚拟结算")
    If rsTemp.EOF = True Then
        MsgBox "请系统管理员完成医保中心的设置。", vbInformation, gstrSysName
...
'    Call OpenRecordset(rs特准项目大类, "获取该病人所有病种的批准项目大类")
    ...
    gstrSQL = "select max(年度) as 年度 from 保险结算记录 where 病人id=" & g结算数据.病人ID & " and 主页id=" & g结算数据.主页ID
    Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    gstrSQL = "select nvl(sum(A.起付线),0) as 起付线,nvl(sum(A.进入统筹金额),0) as 进入统筹金额 " & _
              ...
              " And A.险类 = " & TYPE_自贡市 & " And A.记录ID = B.ID "
    Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    With g结算数据
        gstrSQL = "select A.中心,A.人员身份,A.在职,A.年龄段," & _
                  ...
                  "     and B.年度(+)=" & .年度 & " and A.病人ID=" & .病人ID & " and A.险类=" & TYPE_自贡市
        Call OpenRecordset(rsTemp, "虚拟结算")
...
'            Call zlDatabase.ExecuteProcedure(gstrSQL, "更新医保大类ID")
    ...
    '2.2、计算进入统筹金额
    gstrSQL = "select ID,编码,算法,统筹比额,特准定额,特准天数,是否医保 FROM 保险支付大类  where 险类=" & TYPE_自贡市
    Call OpenRecordset(rs大类, "中软医保")
...
'返回：交易成功返回true；否则，返回false
    ...
    If bln离休 = False Then
        If ReadICCard(ic住院读卡) <> 0 Then
            MsgBox "结算时读卡失败。", vbInformation, gstrSysName
            Exit Function
        End If
...
'----------------------------------------------------------------
    ...
    '只允许对中途结帐进行作废
    gstrSQL = "Select * " & _
              "  From 保险结算记录 Where 性质=2 and 中途结帐=1 and 记录ID='" & lng结帐ID & "'"
    Call OpenRecordset(rsTemp, "中软医保")
    
    If rsTemp.RecordCount = 0 Then
        MsgBox "该病人的医保结算不是中途结帐，不能作废。", vbInformation, gstrSysName
        Exit Function
    End If
    lng原医保年 = rsTemp("年度")
    '只能冲销本医保年度的医保结算记录
    If lng原医保年 < g结算数据.年度 Then
       MsgBox "不能冲销非本医保年度的医保结算记录。", vbInformation, gstrSysName
       Exit Function
    End If
    ...
    Call OpenRecordset(rs结算计算, "中软医保")
...
'功能：判断该病人的卡是否插入正确
    ...
    gstrSQL = "select A.卡号,A.医保号,B.编码 from 保险帐户 A,保险中心目录 B " & _
              " where A.险类=" & TYPE_自贡市 & " and A.病人ID=" & lng病人ID & " and a.险类=B.险类 and A.中心=B.序号"
    Call OpenRecordset(rsTemp, "中软医保")
...
Private Function 检查医保服务器_中软() As Boolean
    ...
    '读出连接医保服务器的配置
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_自贡市
    Call OpenRecordset(rsTemp, "中软医保")
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select 在职,医保号 from 保险帐户 where 险类=" & TYPE_自贡市 & " and 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "中软医保")
