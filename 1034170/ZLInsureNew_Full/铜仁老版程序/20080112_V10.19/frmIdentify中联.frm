...
'返回：
    ...
    cmb中心.Clear
    gstrSQL = "select A.具有中心,B.序号,B.编码,B.名称 from 保险类别 A,保险中心目录 B where A.序号=" & mint险类 & " and A.序号=b.险类"
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmb中心_Click()
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & mint险类 & " and 中心=" & cmb中心.ItemData(cmb中心.ListIndex)
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & mint险类 & " and 中心=" & lng中心 & " and 医保号='" & Trim(txtEdit(Text医保号).Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
