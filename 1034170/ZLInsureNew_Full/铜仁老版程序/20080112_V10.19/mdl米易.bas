...
Public Function 医保初始化_米易() As Boolean
    ...
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & type_米易
    ...
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & type_米易
...
Public Function 身份标识_米易(Optional bytType As Byte, Optional lng病人ID As Long) As String
    ...
    '强制把报销比例等数据填入
    gstrSQL = "Select 病人ID From 保险帐户 Where 险类=" & type_米易 & " And 医保号='" & gComInfo_米易.个人编号 & "'"
...
Public Function 门诊虚拟结算_米易(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    With rs明细
        ...
        Do While Not .EOF
            ...
            gstrSQL = " Select 项目编码 From 保险支付项目" & _
                      " Where 险类=" & type_米易 & " And 收费细目ID=" & !收费细目ID
    ...
    gstrSQL = "Select count(*) Records From 保险结算记录 Where 性质=1 And 险类=" & type_米易 & _
...
Public Function 门诊结算_米易(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    gstrSQL = " Select A.病人ID,Nvl(B.类别,0) 特病种 " & _
              ...
              " Where A.险类=" & type_米易 & " And A.医保号='" & strSelfNo & "' And A.病种ID=B.ID(+)"
    ...
    gstrSQL = "Select Rownum 标识号,A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,A.登记时间,A.开单人 as 医生," & _
            ...
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+) And A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类=" & type_米易 & _
            " Order by A.ID"
...
Private Sub 费用分隔(ByVal lng病人ID As Long, lng主页ID As Long)
    ...
    gstrSQL = "SELECT SUM(发生费用金额) 发生费用金额,SUM(进入统筹金额) 进入统筹金额,SUM(统筹报销金额) 统筹报销金额, " & _
             ...
             "      ) A,保险结算记录 B " & _
             " WHERE A.病人ID=B.病人ID AND B.记录ID=A.结帐ID AND B.险类=" & type_米易 & " AND B.性质=2 " & _
             " GROUP BY A.病人ID"
...
Public Function 获取病人相关信息(ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = " Select 卡号,密码,医保号 个人编号,顺序号 就诊编号,Nvl(病种ID,0) 病种ID From 保险帐户" & _
            " Where 险类=" & type_米易 & " And 病人ID=" & lng病人ID
...
Public Sub 获取病种编码(ByVal lng病种ID As Long)
    ...
    gstrSQL = "Select nvl(类别,0) 类别 From 保险病种 Where 险类=" & type_米易 & " And ID=" & lng病种ID
...
Public Sub 补充上传门诊明细()
    ...
    gstrSQL = "Select A.ID,A.病人ID,A.NO,A.序号,A.记录性质,A.记录状态,A.登记时间,A.开单人 as 医生," & _
            ...
            " From (Select * From 病人费用记录 Where 记录性质=1 And Nvl(是否上传,0)=0 And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E,保险结算记录 F,保险帐户 G,(Select * From 保险病种 Where 险类=" & type_米易 & ") H " & _
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+) And A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类=" & type_米易 & _
