...
'功能：测试是否可以连接到前置服务器上
    ...
    '首先读出参数，打开连接
    gstrSQL = "Select 参数名,参数值 From 保险参数 Where 险类=" & TYPE_华东
...
Public Function 个人余额_华东(lng病人ID As Long) As Currency
    ...
    gstrSQL = "Select * From 保险帐户 Where 病人id=" & lng病人ID & " And 险类=" & TYPE_华东
    Call OpenRecordset(rsTemp, gstrSysName)
...
'    str结算方式  "报销方式;金额;是否允许修改|...."
    ...
    lng病人ID = rs明细!病人ID
    gstrSQL = "Select 卡号 From 保险帐户 Where 病人id=" & lng病人ID & " And 险类=" & TYPE_华东
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
    ...
    '判断是否有医保编码未对应
    Do Until rs明细.EOF
        gstrSQL = "select A.项目编码,B.名称 from (select * from 保险支付项目 where 险类=" & TYPE_华东 & ") A, 收费细目 B where A.收费细目id(+)=B.id and B.id = " & rs明细!收费细目ID
        Call OpenRecordset(rsTemp, gstrSysName)
    ...
    rs明细.MoveFirst
    While Not rs明细.EOF
        gstrSQL = "Select A.项目编码,B.ID,B.名称,B.规格,B.计算单位 From 保险支付项目 A,收费细目 B Where B.ID=A.收费细目ID And A.收费细目id=" & rs明细!收费细目ID & " And A.险类=" & TYPE_华东
...
Public Function 门诊结算冲销_华东(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_华东 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'    str结算方式  "报销方式;金额;是否允许修改|...."
    ...
    datCurr = zlDatabase.Currentdate
    gstrSQL = "Select A.卡号,B.住院号 From 保险帐户 A,病人信息 B Where A.病人ID=B.病人ID And A.病人id=" & lng病人ID & " And A.险类=" & TYPE_华东
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
        MsgBox "没有找到病人信息或医保选择错误", vbInformation, gstrSysName
        Exit Function
...
Public Function 住院结算冲销_华东(lng结帐ID As Long) As Boolean
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_华东 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF = True Then
        MsgBox "原单据的医保记录不存在，不能作废。", vbInformation, gstrSysName
        住院结算冲销_华东 = False
...
Public Function 记帐传输_华东(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人ID As Long = 0) As Boolean
    ...
    '先读取保险参数中关于床位费的设置
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & TYPE_华东 & " And 参数名='床位费限额'"
    Call OpenRecordset(rsTemp, "取床位费限额")
    If rsTemp.RecordCount <> 0 Then dbl限额 = Nvl(rsTemp!参数值, 0)
    If dbl限额 <> 0 Then
        '取床位费自费码
        gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & TYPE_华东 & " And 参数名='床位费自费码'"
        Call OpenRecordset(rsTemp, "取床位费自费码")
    ...
    Else
        gstrSQL = "Select * From 病人费用记录 Where Nvl(实收金额,0)<>0 And 记录状态<>0 And nvl(附加标志,0)<>9 and 病人id=" & lng病人ID & " And 主页id=" & lng主页ID & " And NVl(是否上传,0)=0 order by 发生时间,记录性质,NO,序号"
    End If
    ...
    datCurr = zlDatabase.Currentdate
    gstrSQL = "Select A.卡号,A.退休证号,B.住院号 From 保险帐户 A,病人信息 B Where A.病人ID=B.病人ID And A.病人id=" & lng病人ID & " And A.险类=" & TYPE_华东
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
        MsgBox "没有找到病人信息或医保选择错误", vbInformation, gstrSysName
        Exit Function
