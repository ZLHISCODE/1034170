...
Begin VB.Form frmSet奉庆 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmSet奉庆.frx":0000
      ...
      TabCaption(1)   =   "收费类别(&T)"
      TabPicture(1)   =   "frmSet奉庆.frx":001C
...
Private Function LoadCbo() As Boolean
    ...
    If gcnOracle_奉庆 Is Nothing Then Exit Function
...
End Function
 Private Function iniData() As Boolean
    ...
    Call initGrid
    strSQL = "" & _
        "   Select A.类别,b.参数值 From 收费类别 a,(Select * From 保险参数 where 险类=" & TYPE_临沧奉庆 & ") b " & _
        "   Where A.类别=b.参数名(+) " & _
        "   order by A.编码 "
...
Private Sub cmdCardPara_Click()
    If sCard_SetupCardOption_奉庆 = False Then Exit Sub
End Sub
...
Private Sub Form_Activate()
    ...
    gstrSQL = "Select * From 保险参数 where 险类=" & TYPE_临沧奉庆
...
Private Function SaveData() As Boolean
    ...
    '删除已经数据
    gstrSQL = "zl_保险参数_Delete(" & TYPE_临沧奉庆 & ",null)"
    ...
    With mshBill
        For lngRow = 1 To .Rows - 1
            If .TextMatrix(lngRow, mColHead.收费类别) <> "" Then
                '新增参数数据
                gstrSQL = "zl_保险参数_Insert(" & TYPE_临沧奉庆 & ",null,'" & .TextMatrix(lngRow, mColHead.收费类别) & "' ,'" & .TextMatrix(lngRow, mColHead.保费项目) & "'," & lngRow + 2 & ")"
                Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
            End If
        Next
    End With
    ...
    '新增参数数据
    gstrSQL = "zl_保险参数_Insert(" & TYPE_临沧奉庆 & ",null,'医保用户名','" & txtEdit(text医保用户).Text & "',1)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_临沧奉庆 & ",null,'医保用户密码','" & txtEdit(Text医保密码).Tag & "',2)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_临沧奉庆 & ",null,'医保服务器','" & txtEdit(Text医保服务器).Text & "',3)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    
    
    gstrSQL = "zl_保险参数_Insert(" & TYPE_临沧奉庆 & ",null,'中心用户名','" & txtEdit(text中心用户).Text & "',4)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_临沧奉庆 & ",null,'中心用户密码','" & txtEdit(Text中心密码).Text & "',5)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_临沧奉庆 & ",null,'中心服务器','" & txtEdit(Text中心服务器).Text & "',6)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
