...
Private Const clng退出 As Long = 9999
...
Private Function 铜山县医保工具包(ByVal lng功能号 As Long) As Boolean
    ...
    Case c铜山县_清除上传标志
        ...
        gstrSQL = "Select a.病人id||'_'||a.主页id as ID,a.病人id, a.主页id, b.住院号, b.住院次数, b.姓名, b.性别, b.年龄, b.身份证号, a.入院日期" & vbNewLine & _
                ...
                "Where a.病人id = b.病人id And a.主页id = Nvl(b.住院次数, 0) And a.出院日期 Is Null And A.险类=" & TYPE_铜山县
...
Private Function 昭通_下载药品执行库() As Boolean
    ...
    For lngLoop = 1 To frmConn昭通.mlngRows + 1
        If frmConn昭通.Query(lngLoop - 1, 1, "正在更新数据(" & lngLoop & "/" & frmConn昭通.mlngRows + 1 & ")......") = False Then gcnOracle.RollbackTrans: Exit Function
...
Private Function 昭通_下载标准药品目录() As Boolean
    ...
    If frmConn昭通.strReturnInfo = "" Then
        ...
        End If
    Else
        str更新状态 = Split(frmConn昭通.strReturnInfo, vbTab)(2)
    End If
    ...
    DoEvents
    For lngLoop = 1 To frmConn昭通.mlngRows + 1
        If frmConn昭通.Query(lngLoop - 1, 1, "正在更新数据(" & lngLoop & "/" & frmConn昭通.mlngRows + 1 & ")......") = False Then
            gcn昭通.RollbackTrans
            Exit Function
        End If
...
Private Function 昭通_下载材料项目() As Boolean
    ...
    If frmConn昭通.strReturnInfo = "" Then
        ...
        End If
    Else
        str更新状态 = Split(frmConn昭通.strReturnInfo, vbTab)(1)
    End If
...
'    strData = Split(frmConn昭通.strReturnInfo, Chr(10))
    ...
    DoEvents
    For lngLoop = 1 To frmConn昭通.mlngRows + 1
        If frmConn昭通.Query(lngLoop - 1, 1, "正在更新数据(" & lngLoop & "/" & frmConn昭通.mlngRows + 1 & ")......") = False Then Exit Function
        strTemp = frmConn昭通.strReturnInfo
