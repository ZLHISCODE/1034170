...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = _
        ...
        " From 病人费用记录 a,部门表 c, " & _
        "       (   Select * From 保险支付项目 Where 险类=" & TYPE_开县 & ") D" & _
        " Where  nvl(a.实收金额,0)<>0 and a.收费细目ID=d.收费细目id(+) and  a.执行部门ID=c.id(+) and Nvl(a.附加标志,0)<>9 And a.结帐ID=" & lng结帐ID & _
        " order by a.ID"
    Call OpenRecordset(rsList, "开县医保")
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    gstrSQL = "" & _
        ...
        " from 病人费用记录 A,部门表 B,收费细目 d ," & _
        "       (   Select * From 保险支付项目 Where 险类=" & TYPE_开县 & ") C" & _
        " where nvl(a.实收金额,0)<>0 and  A.开单部门ID=B.ID(+) And A.收费细目ID=C.收费细目ID(+) and a.收费细目id=d.id " & _
        "       And A.病人ID=" & rsExse!病人ID & " And A.记帐费用=1 And Nvl(A.是否上传,0)=0 And A.记录状态<>0" & _
        " Order by A.记录性质,A.记录状态,A.NO,A.ID"
        
    Call OpenRecordset(rsExpen, "开县医保")
...
'调用模块：1121-门诊收费
    ...
    gstrSQL = "" & _
        ...
        "   From 病人费用记录 A,部门表 B,(Select * From 保险支付项目 Where 险类=" & TYPE_开县 & ") C,收费细目 D " & _
    ...
    Call OpenRecordset(rsExse, "开县医保")
