...
'    Dim InvokeServer As String
    ...
    gstrSQL = " Select 退休证号||'||||' AS 退休证号 From 保险帐户 Where 险类=" & TYPE_南京市 & " And 病人ID=" & rs明细!病人ID
    Call OpenRecordset(rsTemp, "提取当前病人子医保相关信息")
...
Public Function 门诊结算_南京市(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    '提取当前病人子医保相关信息(保险序号|优惠类别|医保号|余额|停用)
    gstrSQL = " Select 退休证号||'||||' AS 退休证号 From 保险帐户 Where 险类=" & TYPE_南京市 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "提取当前病人子医保相关信息")
    intSubInsure = Val(Split(rsTemp!退休证号, "|")(0))
    strYHLB = Split(rsTemp!退休证号, "|")(1)
    strSubInsureNO = Split(rsTemp!退休证号, "|")(2)
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & TYPE_南京市 & "," & mstrPatID & "," & Year(zlDatabase.Currentdate) & ",null,null,null,null,null,null,null,null," & _
              ...
              "0," & mFeeBalance.个人帐户支付 & ",'" & mFeeBalance.单据号 & IIf(str出参 = "", "", "|" & Split(str出参, "|")(0) & "|" & intSubInsure) & "',null,null,'" & gPatInfo_南京市.病种编码 & "|" & gPatInfo_南京市.病种名称 & "|" & mFeeBalance.医保卡号 & "')"
...
'    使用人_IN IN 票据使用明细.使用人%TYPE
    If gblnBill Then
        ...
        If strRecord <> "" Then
            ...
            MsgBox "本次医保使用票据号：" & strRecord, vbInformation, gstrSysName
        End If
    End If
    门诊结算_南京市 = True
    Exit Function
...
'    Call writeTxtFile(strFile, "")
    ...
    Do Until rsExse.EOF
        ...
        gstrSQL = "select Rownum 序号,decode(A.类别,'5',0,'6',0,'7',0,1) 标志,A.名称,A.编码,C.项目编码,A.计算单位,B.产地,decode(B.药品来源,'国产',1,'合资',2,'进口',3,null) 产地特征,B.规格" & _
                  " from 收费细目 A,药品目录 B,保险支付项目 C where A.id = C.收费细目id and A.id=B.药品id(+) and A.id =" & rsExse!收费细目ID & " And C.险类=" & TYPE_南京市
...
'                .单价 = Format(rsTemp!现价, "#0.0000;-#0.0000;0;")
            ...
            gstrSQL = "select b.电子邮件 as 医生编码,a.开单人 as 医生 from 病人费用记录 a,人员表 b where a.NO='" & rsExse!NO & "' and a.序号=" & rsExse!序号 & _
                    " and mod(a.记录性质,10)=" & rsExse!记录性质 & " and a.记录状态=" & rsExse!记录状态 & "And a.开单人 = b.姓名"
            Call OpenRecordset(rsTemp, "医生编码")
            If rsTemp.RecordCount <> 0 Then
...
'    使用人_IN IN 票据使用明细.使用人%TYPE
    If gblnBill Then
        ...
        gcnNJSYB.Execute gstrSQL, , adCmdStoredProc
        MsgBox "本次医保使用票据号：" & strNO, vbInformation, gstrSysName
    End If
