...
'                    收费细目ID,险类,大类ID,项目编码,项目名称,附注
                    ...
                    '保存到中间库中
                        gstrSQL = "Select * from ypzlk where 收费细目ID=" & .RowData(lngRow)
                        Call OpenRecordset(rsDzk, "对照库")
                        If rsDzk.EOF Then
                ...
                Else
                    ...
                    Select Case mint险类
                        Case TYPE_四川自贡
                            ...
                            Call OpenRecordset(rsTemp, "医保中心明细", gstrSQL, mcnYB)
                        Case TYPE_昭通
...
'陈东 2005-3-31
                        ...
                        Case TYPE_山西
                            ...
                            Call OpenRecordset(rsTemp, "医院等级")
                            If rsTemp.EOF Then
                                str医院等级 = ""
                            Else
                        ...
                        Case TYPE_浙江
                            ...
                            End If
                            If Left(tvwMain_S.SelectedItem.Key, 1) = "D" Or Left(tvwMain_S.SelectedItem.Key, 1) = "E" Or Left(tvwMain_S.SelectedItem.Key, 1) = "F" Then
                                gstrSQL = "Select AKA060 As 医保编码, AKA061 As 项目名称, trim(to_char(nvl(AKA069,0),'99990.00')) As 自付比例, AKA066 As 拼音码, AKA070 As 剂型, Decode(AKA065,'1','甲类药','2','乙类药','丙类药') As 类别 From KA02 " & _
                                    "Where AKA060 Like " & strTemp & " Or AKA061 Like " & strTemp & " Or AKA066 Like " & strTemp
                            Else
                                gstrSQL = "Select AKA090 As 医保编码, AKA091 As 项目名称, trim(to_char(nvl(AKA069,0),'99990.00')) As 自付比例, AKA066 As 拼音码, Decode(AKA065,'1','甲类','2','乙类','丙类') As 类别 From KA03 " & _
                    ...
                    If blnReturn = False Then
                        ...
                        If rsTemp.RecordCount > 0 Then
                            ...
                            Exit Sub
                        Else
                            If Not (mint险类 = TYPE_四川自贡 Or mint险类 = TYPE_沈阳市 Or mint险类 = TYPE_北京 Or mint险类 = TYPE_泸州市 Or mint险类 = TYPE_铜仁 Or mint险类 = TYPE_毕节 Or mint险类 = TYPE_黔南 Or mint险类 = TYPE_宁海 Or mint险类 = TYPE_渝北农医 Or mint险类 = TYPE_慈溪农医 Or mint险类 = TYPE_重庆市) Then
                                .Text = strText
...
'功能：根据当前行的保险项目编码，得到其它信息
    ...
    With mshSum_S
        ...
        ElseIf mint险类 = TYPE_浙江 Then
            If Left(tvwMain_S.SelectedItem.Key, 1) = "D" Or Left(tvwMain_S.SelectedItem.Key, 1) = "E" Or Left(tvwMain_S.SelectedItem.Key, 1) = "F" Then
                gstrSQL = "Select AKA060 As 医保编码, AKA061 As 项目名称, trim(to_char(nvl(AKA069,0),'99990.00')) As 自付比例, AKA066 As 拼音码, Decode(AKA065,'1','甲类药','2','乙类药','丙类药') As 类别 From KA02 Where AKA060='" & .TextMatrix(.Row, COL医保编码) & "'"
            Else
                gstrSQL = "Select AKA090 As 医保编码, AKA091 As 项目名称, trim(to_char(nvl(AKA069,0),'99990.00')) As 自付比例, AKA066 As 拼音码, Decode(AKA065,'1','甲类','2','乙类','丙类') As 类别 From KA03 Where AKA090='" & .TextMatrix(.Row, COL医保编码) & "'"
            End If
...
Private Sub mshSum_S_MouseUp(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    If mshSum_S.MouseRow = 0 Then
        ...
        With rsTemp.Fields
            ...
            .Append "名称", adVarChar, 80, adFldIsNullable
...
Private Sub Fill大类()
    ...
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & cmb险类.ItemData(cmb险类.ListIndex)
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub SetEditVisible()
    ...
    gstrSQL = "Select count(*) Records from 医保对照类别 Where 险类=" & cmb险类.ItemData(cmb险类.ListIndex) & " And 编码<>0 "
    Call OpenRecordset(rsTemp, "提取医保对照类别")
...
Private Sub GetCodeLength()
    ...
    Call OpenRecordset(rsTemp, "取项目编码")
...
Private Sub ReadAdvance(ByVal lng收费细目ID As Long)
    ...
    Call OpenRecordset(rsTemp, "提取已完成的对照信息")
    With rsTemp
...
Public Function ISHIS10() As Boolean
    Dim rsTemp As New ADODB.Recordset
    On Error GoTo errHand
    
    'HIS9与HIS10的系统不可能混装，因此只要取出任一条记录，也就是任一个系统的版本号大于9，则可认为是HIS10的系统
    gstrSQL = "Select 版本号 From zlSystems Where rownum<2"
    Call OpenRecordset(rsTemp, "判断HIS版本")
    If rsTemp!版本号 Like "10*" Then
        ISHIS10 = True
    End If
