    ...
    If bytType = 0 Then
       ...
       ' Call 调用接口_准备_慈溪农医(gstrFunc慈溪农医_AllowQuery, StrInput)
        ...
        gstrSQL = " Select B.名称 AS 挂号科室,执行人 AS 医生 " & _
                  ...
                  " Where A.记录性质=4 And 记录状态=1 And 病人ID=" & lng病人ID & _
                  " And 登记时间 Between to_date('" & Format(zlDatabase.Currentdate, "yyyy-MM-dd") & " 00:00:00','yyyy-MM-dd hh24:mi:ss')" & _
      ...
      '  End If
        ...
        '取疾病编码
        gstrSQL = "Select 编码 From 疾病编码目录 Where ID=(Select nvl(病种ID,0) From 保险帐户 Where 险类=" & TYPE_慈溪农医 & " And 病人ID=" & lng病人ID & ")"
        Call OpenRecordset(rsTemp, "取疾病编码")
...
'返回：初始化成功，返回true；否则，返回false
    ...
    If mblnInit = False Then
        '取医院编码
        gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_慈溪农医
        Call OpenRecordset(rsTemp, "读取医院编码")
        gComInfo_慈溪农医.医院编码 = Nvl(rsTemp!医院编码)
        
        '取保险参数
        gstrSQL = "Select 参数名,参数值 From 保险参数 Where 险类=" & TYPE_慈溪农医
        Call OpenRecordset(rsTemp, "读取保险参数")
        ...
        Do While Not .EOF
            ...
            lngrowcount = lngrowcount + 1
             gstrSQL = " Select A.类别 AS 收费类别,A.名称,A.规格,A.计算单位 AS 单位,B.项目编码 From 收费细目 A,保险支付项目 B" & _
                      " Where A.ID=B.收费细目ID(+) And B.险类(+)=" & TYPE_慈溪农医 & " And A.ID=" & !收费细目ID
            Call OpenRecordset(rsItem, "提取项目信息")
...
'    Cfdkbje numeric(10,2)   超封顶有效金额(用于报销大病)
     If gComInfo_慈溪农医.结算类型 <> "事后补报" Then
        ...
        If Format(gComInfo_慈溪农医.总费用_中心, "#0.00") <> Format(gComInfo_慈溪农医.总费用, "#0.00") Then
            MsgBox "医院的总费用与医保中心的总费用不一致！" & vbCrLf & _
...
'   Tbsbje  numeric(10,2)   特病实报金额
    ...
    Call OpenRecordset(rsTemp, "取该病人的ID")
    lng病人ID = rsTemp!病人ID
     If gComInfo_慈溪农医.结算类型 <> "事后补报" Then
    ...
    Else
       ...
       dbl现金 = 0
       str结算单号 = "事后补报"
        gstrSQL = "insert into 病人费用记录_事后补报慈系 Select * From 病人费用记录 Where 结帐ID=" & lng结帐ID
...
Public Function 门诊结算冲销_慈溪农医(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    '取卡证号码
    gstrSQL = "Select 卡号 From 保险帐户 Where 险类=" & TYPE_慈溪农医 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "读卡证号码")
    ...
    If rsTemp.RecordCount = 0 Then
        MsgBox "没有找到原始结算记录，无法进行门诊结算冲销！", vbInformation, gstrSysName
        Exit Function
    End If
...
'    Czy     Varchar(10) 操作员姓名
   If str结算单号 <> "事后补报" Then
    ...
    Else '事后补报的如果要退费要先判断是否已经补报，如果已经补报则需要先在补报处先退费，然后在这里退费
       gstrSQL = "Select * From 病人费用记录_事后补报慈系 Where 是否上传='1' and 结帐ID=" & lng结帐ID
       Call OpenRecordset(rsTemp1, "取事后补报记录的状态")
...
Public Function 入院登记_慈溪农医(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
    ...
    '取病人的医疗卡号
    gstrSQL = "Select 卡号,Nvl(病种ID,0) 病种ID,Nvl(业务类型,'21') AS 结算类型 From 保险帐户 Where 险类=" & TYPE_慈溪农医 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取病人的医疗卡号")
    strCardNO = rsTemp!卡号
...
Public Function 个人余额_慈溪农医(strSelfNo As String) As Currency
    ...
    gstrSQL = "Select Nvl(帐户余额,0) AS 个人帐户 From 保险帐户 Where 医保号='" & strSelfNo & "'"
...
Public Function 处方上传_慈溪农医(ByVal int性质 As Integer, ByVal int状态 As Integer, ByVal strNO As String) As Boolean
    ...
    gstrSQL = " Select A.ID,A.记录性质,A.记录状态,A.NO,A.序号,A.收费类别,A.病人ID,A.主页ID,A.收费细目ID,A.登记时间,A.实收金额," & _
              ...
              " Order by A.病人ID"
    Call OpenRecordset(rsDetail, "提取本次待上传的处方明细")
...
Public Function 住院虚拟结算_慈溪农医(rsExse As Recordset, ByVal lng病人ID As Long) As String
    ...
    '取结算类型
    gstrSQL = "Select 卡号,业务类型,Nvl(退休证号,0) 承担比例 From 保险帐户 Where 险类=" & TYPE_慈溪农医 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsItem, "取结算类型")
    ...
    Call OpenRecordset(rsItem, "取主页ID")
    lng主页ID = rsItem!主页ID
    strKey = GetKey(lng病人ID, lng主页ID)
    
    '取出院日期
    gstrSQL = "Select 出院日期 From 病案主页 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsItem, "取出院日期")
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.记录性质,A.记录状态,A.序号,A.收费细目ID,C.项目编码 AS 医保项目编码,B.编码,B.名称,A.实收金额 AS 金额" & _
              ...
              "  where A.病人ID=" & lng病人ID & " and A.主页ID=" & lng主页ID & " and A.记帐费用=1 And A.操作员姓名 is not null AND Nvl(A.实收金额,0)<>0 " & _
              "        And Nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0 and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & TYPE_慈溪农医 & _
 ...
 '提取本次费用明细1
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.记录性质,A.记录状态,A.序号,A.收费细目ID,C.项目编码 AS 医保项目编码,B.编码,B.名称,A.实收金额 AS 金额" & _
              ...
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C " & _
              "  where A.病人ID=" & lng病人ID & " and A.主页ID=" & lng主页ID & " and A.记帐费用=1 And A.操作员姓名 is not null AND Nvl(A.实收金额,0)<>0 " & _
              "        And Nvl(A.记录状态,0)<>0 and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & TYPE_慈溪农医 & _
              "  Order by A.病人ID,A.发生时间"
    Call OpenRecordset(rs明细1, "提取本次费用明细")
...
Public Function 住院结算_慈溪农医(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    If Not 医保病人已经出院(lng病人ID) Then
        MsgBox "必须先出院，才能进行结算！", vbInformation, gstrSysName
        Exit Function
    End If
...
Public Function 住院结算冲销_慈溪农医(lng结帐ID As Long) As Boolean
    ...
    '取结算流水号
    gstrSQL = "Select * From 保险结算记录 Where 性质=2 And 记录ID=" & lng结帐ID
    Call OpenRecordset(rsBalance, "取结算流水号")
    ...
    '取结算类型
    gstrSQL = "Select 卡号 From 保险帐户 Where 险类=" & TYPE_慈溪农医 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取结算类型")
    strCardNO = rsTemp!卡号
...
'    Czy Varchar(10) 操作员姓名
    ...
    gstrSQL = "Select * From 结算附加信息 Where 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "提取结算附加信息")
    If rsTemp.RecordCount <> 0 Then
...
Private Function IsYBPatient(ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = " Select 1 From 病案主页 Where 险类=" & TYPE_慈溪农医 & " And (病人ID,主页ID) IN " & _
              "     (Select 病人ID,住院次数 From 病人信息 Where 病人ID=" & lng病人ID & ")"
    Call OpenRecordset(rsTemp, "判断指定病人本次是否以医保身份就诊")
...
Private Sub UploadDetail(ByVal int记录性质 As Integer, ByVal int记录状态 As Integer, ByVal strNO As String, ByVal lng病人ID As Long)
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = " Select NO,序号,记录性质,记录状态 From 病人费用记录" & _
              " Where 记录性质=" & int记录性质 & " And 记录状态=" & int记录状态 & " And NO='" & strNO & "' And 病人ID=" & lng病人ID & " And Nvl(是否上传,0)=0"
    Call OpenRecordset(rsTemp, "提取该病人的处方明细")
...
Public Function 更新在院信息_慈溪农医(ByVal lng病人ID As Long, ByVal lng主页ID As Long, Optional ByVal bln出院 As Boolean = False) As Boolean
    ...
    strKey = GetKey(lng病人ID, lng主页ID)
    '取病人的医疗卡号
    gstrSQL = "Select 卡号,Nvl(病种ID,0) 病种ID From 保险帐户 Where 险类=" & TYPE_慈溪农医 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取病人的医疗卡号")
    strCardNO = rsTemp!卡号
    lngDisease = rsTemp!病种ID
    ...
    gstrSQL = " Select A.入院日期,B.名称 科室,A.住院医师 医生 From 病案主页 A,部门表 B " & _
              " Where A.病人ID=" & lng病人ID & " And A.主页ID=" & lng主页ID & " And A." & IIf(bln出院 = False, "入院科室ID", "出院科室ID") & "=B.ID"
    Call OpenRecordset(rsTemp, "取科室与医生")
    strInHospitalDate = Format(rsTemp!入院日期, mstrDateFormat)
    strRegisterDoctor = Nvl(rsTemp!医生)
    strRegisterOffice = Nvl(rsTemp!科室)
...
Public Function get小儿用药编码(ByVal lng病人ID As Long, ByVal lng收费细目ID As Long, ByVal str编码 As String) As String
    ...
    '判断本次入院是否是小儿用药的方式入院
    gstrSQL = "Select Nvl(小儿用药,0) AS 小儿用药 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_慈溪农医
    Call OpenRecordset(rsTemp, "判断本次入院是否是小儿用药的方式入院")
    If rsTemp.RecordCount = 0 Then Exit Function
    If rsTemp!小儿用药 = 0 Then Exit Function
    
    '提取小儿用药编码并返回
    gstrSQL = "Select 附注 From 保险支付项目 Where 险类=" & TYPE_慈溪农医 & " ANd 收费细目ID=" & lng收费细目ID
    Call OpenRecordset(rsTemp, "提取小儿用药编码")
    If rsTemp.RecordCount = 0 Then Exit Function
