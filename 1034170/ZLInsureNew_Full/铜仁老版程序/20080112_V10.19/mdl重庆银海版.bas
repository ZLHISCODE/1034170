        ...
        For intDO = 1 To 2
            ...
            Do While Not .EOF
                ...
                If bln上传 Then
                    ...
                    gstrSQL = "Select 类别,编码,名称 From 收费细目 Where ID=" & !收费细目ID
                    ...
                    If str退单处方流水号 <> str处方流水号 Then
                        gstrSQL = "Select to_char(开方日期,'yyyyMMdd hh24:mi:ss') 开方日期 From " & mstrOwner & ".中间库_处方明细 Where 处方流水号='" & str退单处方流水号 & "'"
...
Private Function MakeFile_RecipeCalculated(ByVal str流水号 As String, Optional ByVal str处方流水号 As String) As Boolean
    ...
    gstrSQL = " Select A.病人ID,A.住院次数 AS 主页ID From 病人信息 A,保险帐户 B" & _
              " Where A.病人ID=B.病人ID ANd B.流水号='" & str流水号 & "'"
    Call OpenRecordset(rsTemp, "取病人ID、主页ID")
    lng病人ID = rsTemp!病人ID
    lng主页ID = rsTemp!主页ID
                        ...
                        With rsRecipe
                            ...
                            Do While Not .EOF
                                If Nvl(!是否上传, 0) = 0 Then
                                    ...
                                    Call OpenRecordset(rsTemp, "判断", gstrSQL, gcn重庆银海版)
...
Public Function 住院结算_重庆银海版(lng结帐ID As Long, ByVal lng病人ID As Long, Optional ByRef strAdvance As String) As Boolean
    ...
    Call OpenRecordset(rsTemp, "如果存在需要审批的项目却未审批，不允许结算", gstrSQL, gcn重庆银海版)
...
'功能：获得保险参数
    ...
    gstrSQL = "select A.参数名,A.参数值 from 保险参数 A " & _
              " where A.参数名='" & str参数名 & "' and A.险类=" & TYPE_重庆银海版 & " and A.中心 is null "
