...
'返回：成功返回True
    ...
    mint险类 = int险类
    strSQL = "Select * from 医保支付项目 Where 险类=" & mint险类 & " And 中心=" & mint中心 & " and 项目编码='" & strCode & "'"
    Call OpenRecordset(rsTmp, "查询支付大类", strSQL)
    If rsTmp.EOF Then
        ...
        gstrSQL = "select A.ID,E.项目编码 as 编码,F.名称 as 类别," & _
                  ...
                  "where A.ID=B.收费细目ID and B.收入项目ID=D.ID And " & _
                  "nvl(B.终止日期,to_date('3000-01-01','YYYY-MM-DD'))=to_date('3000-01-01','YYYY-MM-DD') and " & _
                  "A.编码='" & strCode & "' and A.ID=E.收费细目ID And E.险类=F.险类 And E.大类ID=F.ID and F.险类=" & mint险类 & " And E.中心=" & mint中心
    End If
    Call OpenRecordset(mrsTMP, "保险项目选择")
...
Private Sub cmd查询_Click()
   ...
   Dim int大类id As Integer
    ...
    gstrSQL = "select nvl(ID,0) as ID from 保险支付大类 where 险类=" & mint险类 & " And 名称='" & cmd类别.Text & "'"
    Call OpenRecordset(rsTmp, "查询支付大类", gstrSQL)
    int大类id = rsTmp!ID
...
Private Sub txt编号_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = vbKeyReturn Then
        ...
        strSQL = "Select * from 医保支付项目 Where 险类=" & mint险类 & " And 中心=" & mint中心 & " and 项目编码='" & txt编号.Text & "'"
        Call OpenRecordset(rsTmp, "查询支付大类", gstrSQL)
        If rsTmp.EOF Then
            ...
            gstrSQL = "select A.ID,E.项目编码 as 编码,F.名称 as 类别," & _
                      ...
                      "where A.ID=B.收费细目ID and B.收入项目ID=D.ID And " & _
                      "nvl(B.终止日期,to_date('3000-01-01','YYYY-MM-DD'))=to_date('3000-01-01','YYYY-MM-DD') and " & _
                      "A.编码='" & txt编号.Text & "' and A.ID=E.收费细目ID And E.险类=F.险类 And E.大类ID=F.ID and F.险类=" & mint险类 & " And E.中心=" & mint中心
        End If
        Call OpenRecordset(mrsTMP, "保险项目选择")
