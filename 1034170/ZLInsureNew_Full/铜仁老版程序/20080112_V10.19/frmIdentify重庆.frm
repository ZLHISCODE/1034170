...
Private Sub cmdOK_Click()
    ...
    If mint类别 <> 21 Then
        Call SaveSetting("ZLSOFT", "公共模块\" & App.ProductName & "\" & Me.Name, "上次医保号", Me.txtEdit(0).Text & "|" & Format(zldatabase.Currentdate, "yyyyMMdd"))
    End If
...
Private Sub Form_Load()
    ...
    int参数 = 0
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & TYPE_重庆市 & " And 参数名='保存医保号'"
    Call OpenRecordset(rsTemp, "判断是否保存上次医保号")
    ...
    If strData <> "" And int参数 = 1 Then
        If InStr(1, strData, "|") <> 0 Then
            ...
            If arrData(1) = Format(zldatabase.Currentdate, "yyyyMMdd") Then Me.txtEdit(0).Text = arrData(0)
        End If
    End If
End Sub
...
Private Sub txtEdit_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
    ...
    If Index = 0 Then
        ...
        lng病人ID = GetRegisted(txtEdit(0).Text)
        If lng病人ID = 0 Then Exit Sub
        
        '重新恢复上次的业务类型
        gstrSQL = "Select 业务类型 From 保险帐户 Where 险类=" & TYPE_重庆市 & " ANd 病人ID=" & lng病人ID
...
Private Function GetRegisted(ByVal str医保号 As String) As Long
    ...
    gstrSQL = " Select A.病人ID From 病人费用记录 A,保险结算记录 B " & _
              ...
              " And A.病人ID+0 =(Select 病人ID From 保险帐户 Where 险类=" & TYPE_重庆市 & " ANd 医保号='" & str医保号 & "')"
