...
Private Sub cmdTest_Click()
    ...
    If OpenSQLServer(mcnTest, txtEdit(Text医保服务器).Text, txtEdit(text医保用户).Text, txtEdit(Text医保密码).Tag, txtEdit(Text实例名).Text) = False Then
        Exit Sub
    End If
...
Private Function SaveData() As Boolean
    ...
    '删除已经数据
    gstrSQL = "zl_保险参数_Delete(" & TYPE_渝北农医 & ",null)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    
    '新增参数数据
    gstrSQL = "zl_保险参数_Insert(" & TYPE_渝北农医 & ",null,'医保用户名','" & txtEdit(text医保用户).Text & "',1)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_渝北农医 & ",null,'医保用户密码','" & txtEdit(Text医保密码).Tag & "',2)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_渝北农医 & ",null,'医保服务器','" & txtEdit(Text医保服务器).Text & "',3)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    gstrSQL = "zl_保险参数_Insert(" & TYPE_渝北农医 & ",null,'医保实例名','" & txtEdit(Text实例名).Text & "',4)"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
...
'功能：设置与东大阿尔派的医保接口
    ...
    Do Until rsTemp.EOF
        ...
        Select Case rsTemp("参数名")
            ...
            Case "医保实例名"
                txtEdit(Text实例名).Text = str参数值
        End Select
        rsTemp.MoveNext
    Loop
    
    mblnChange = False
    frmSet渝北农医.Show vbModal, frm医保类别
