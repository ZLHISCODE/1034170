...
Private Function 获取个人帐户支付() As Double
    ...
    gstrSQL = "Select * From 病人预交记录 where 结帐ID=" & g病人身份_神木大兴.结帐ID & " and  结算方式='个人帐户'"
...
Public Function 门诊结算冲销_神木大兴(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    gstrSQL = "Select * From 病人费用记录 where  结帐ID =" & lng结帐ID & " And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0 and nvl(实收金额,0)<>0"
...
Public Function 处方登记_神木大兴(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
   ...
   gstrSQL = "" & _
              "  Select A.*,M.医保号" & _
              "  From 病人费用记录 A,病案主页 C,保险帐户 M" & _
              "  where a.NO='" & str单据号 & "' and A.记录性质=" & lng记录性质 & " and A.记录状态 = " & lng记录状态 & " " & _
              "        and A.病人ID=C.病人ID and nvl(a.实收金额,0)<>0 and A.主页ID=C.主页ID  And a.病人id=M.病人id and M.险类=" & TYPE_陕西大兴 & " and  C.险类=" & TYPE_陕西大兴 & _
...
Public Function 补传住院明细记录_神木大兴(ByVal lng病人ID As Long, ByVal lng主页ID As Long, Optional bln忽略 As Boolean = False) As Boolean
   ...
   gstrSQL = "" & _
              "  Select A.*,M.医保号" & _
              "  From 病人费用记录 A,病案主页 C,保险帐户 M" & _
              "  where Nvl(A.是否上传,0)=0 And Nvl(附加标志,0)<>9  and a.病人id=" & lng病人ID & " and A.主页id= " & lng主页ID & " " & _
              "        and  A.病人ID=C.病人ID and nvl(a.实收金额,0)<>0 and A.主页ID=C.主页ID  And a.病人id=M.病人id and M.险类=" & TYPE_陕西大兴 & " and  C.险类=" & TYPE_陕西大兴 & _
              "  Order by A.病人ID,A.NO,A.发生时间"
...
Public Function 住院虚拟结算_神木大兴(rsExse As Recordset, ByVal lng病人ID As Long, Optional bln结帐处 As Boolean = True) As String
    ...
    End If
    'intMouse = Screen.MousePointer
    gstrSQL = "Select a.*,b.* From 保险帐户 a,病人信息 b where a.病人id=b.病人id and a.病人id=" & lng病人ID
...
Public Function 住院结算_神木大兴(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
    ...
    If dbl费用总额 <> Round(g结算数据.费用总额, 2) Then
        If dbl费用总额 - Round(g结算数据.费用总额, 2) <= 0.1 Then
            ...
            End If
        Else
            ShowMsgbox "虚拟结算数据的费用总额(" & Format(g结算数据.费用总额, "####0.00;-###0.00;0;0") & ")" & vbCrLf & "与本次结算的费用总额(" & Format(dbl费用总额, "####0.00;-###0.00;0;0") & ")不等，请检查处方是否正确!"
            Exit Function
        End If
    End If
    g病人身份_神木大兴.结帐ID = lng结帐ID
...
Public Function Open中间库_神木大兴() As Boolean
    ...
    '重新建立到医保服务器的公共连接
    '中间库连接
    gstrSQL = "select 参数名,参数值 from 保险参数 where  险类=" & TYPE_陕西大兴
    Call OpenRecordset(rsTemp, "神木大兴核工业医保")
