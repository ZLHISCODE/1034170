...
Public Function ShowMeFromOut(ByRef frmParent As Object, ByVal lng结帐ID As Long) As Boolean
    ...
    rsTmp.Filter = 0
    strSQL = "Select 结算方式,金额 From 医保核对表 Where 结帐id =[1] "
    Set rsTmp = OpenSQLRecord(strSQL, "保险结算管理", lng结帐ID)
    mstr保险结算 = ""   '结算方式|结算金额||
    For i = 1 To rsTmp.RecordCount
...
Public Function ShowME(ByRef frmParent As Object, ByVal lng结帐ID As Long, ByVal cur实收金额 As Currency, _
        ...
        ByVal str缺省结算方式 As String, ByVal str缺省金额位数 As String, ByVal byt缺省分币方式 As Byte) As Boolean
...
Private Sub Form_Load()
    ...
    strSQL = "Select 应用场合,结算方式 From 结算方式应用 Where 应用场合='收费'"
...
Private Sub ShowMoney(Optional ByVal blnAutoSet As Boolean)
    ...
    If blnAutoSet Then      '根据差额自动补平并计算
        For i = 1 To mshBalance.Rows - 1
            If mshBalance.RowData(i) <> PayType.医保个人帐户 And mshBalance.RowData(i) <> PayType.医保其它结算 Then
                curPay = curPay + Val(mshBalance.TextMatrix(i, 1))
            End If
        Next
        curBalance = mcur实收金额 - (curPay + mcurMediCare + mcur冲预交额)
        If mintDefault > 0 Then
            '剩余部份尝试设置到缺省结算方式上
            If mshBalance.RowData(mintDefault) = PayType.现金 And mblnCent Then   '现金时要进行分币处理
                mshBalance.TextMatrix(mintDefault, 1) = Format(CentMoney(Val(mshBalance.TextMatrix(mintDefault, 1)) + curBalance), "0.00")
            Else
                mshBalance.TextMatrix(mintDefault, 1) = Format(Val(mshBalance.TextMatrix(mintDefault, 1)) + curBalance, "0.00")
            End If
            If Val(mshBalance.TextMatrix(mintDefault, 1)) = 0 Then mshBalance.TextMatrix(mintDefault, 1) = ""
        End If
        bln存在补款 = mintDefault = 0
    Else
        bln存在补款 = True
    End If
    ...
    mcur收费误差 = Format(curPay + mcurMediCare + mcur冲预交额 - mcur实收金额, mstrDec)
    txtPay.ToolTipText = "正式结算后,误差金额:" & mcur收费误差
    
    If bln存在补款 Then
       txtMargin.Text = Format(mcur实收金额 - (curPay + mcurMediCare + mcur冲预交额), "0.00")
    Else
        txtMargin.Text = "0.00"
    End If
