...
'功能：判断IC卡是否合法
    ...
    If mbln判断在院 = True Then
        gstrSQL = "Select Nvl(当前状态,0) AS 状态 From 保险帐户 Where 险类=" & TYPE_四川自贡 & " And 医保号='" & Me.txt医保编号 & "'"
        Call OpenRecordset(rsTemp, "判断当前病人是否在院")
...
Private Sub cmd确认_Click()
    ...
    '取该病人的当前状态
    gstrSQL = "Select Nvl(当前状态,0) AS 当前状态 From 保险帐户 Where 医保号='" & Trim(txt医保编号.Text) & "' And 险类=" & TYPE_四川眉山
    Call OpenRecordset(rsTemp, "取该病人的当前状态")
    int当前状态 = 0
    If rsTemp.RecordCount <> 0 Then
        int当前状态 = rsTemp!当前状态
    End If
    ...
    '得到原住院病种
    If mbytType <> 1 Then
        gstrSQL = "Select Nvl(病种ID,0) 病种ID From 保险帐户 Where 险类=" & TYPE_四川自贡 & " And 医保号='" & TrimStr(txt医保编号.Text) & "'"
...
'    End If
    ...
    '提取医保中心的名称与序号
    gstrSQL = "Select 序号,名称 From 保险中心目录 Where 编码='" & arrOutput(cint中心代码) & "' And 险类=" & TYPE_四川自贡
    Call OpenRecordset(rsTemp, "提取医保中心的名称与序号")
    If rsTemp.RecordCount = 0 Then
