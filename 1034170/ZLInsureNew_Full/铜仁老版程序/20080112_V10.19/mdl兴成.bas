...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select nvl(帐户余额,0) as 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_兴成核工业
    ...
    With rs明细
        ...
        Do While Not .EOF
            gstrSQL = "" & _
                ...
                "   where a.收费细目id=b.ID and  a.险类=" & TYPE_兴成核工业 & _
                "           and a.收费细目id=" & Nvl(!收费细目ID, 0)
...
'            End If
            ...
            If rsTemp.EOF Then
                ...
                "   From 收费细目 B  " & _
                "   where  B.id=" & Nvl(!收费细目ID, 0)
                Call OpenRecordset(rsTemp, "确定非医保项目名称")
                StrInput = StrInput & "|0"
...
Private Function InsertIntoSQLServer_门诊(ByVal str确认串 As String) As Boolean
    ...
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select b.名称 as 开单部门,开单人 from 病人费用记录 a,部门表 b where a.开单部门id=b.id(+) and 结帐id=" & g病人身份_兴成.结帐ID & " and rownum=1"
...
Private Function 获取个人帐户支付() As Double
    ...
    gstrSQL = "Select * From 病人预交记录 where 结帐ID=" & g病人身份_兴成.结帐ID & " and  结算方式='个人帐户'"
...
Public Function 门诊结算冲销_兴成(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    Call OpenRecordset(rs原明细, "获取冲销记录")
    ...
    With rs明细
        Do While Not .EOF
            ...
            If rs原明细.EOF Then
                ShowMsgbox "没有找到原始记录!" & "no='" & Nvl(!NO) & "' and 记录性质=" & Nvl(!记录性质, 0) & " and 序号='" & Nvl(!序号, 0) & "' and 执行状态=" & Nvl(!执行状态, 0)
                gcnOracle_兴成.RollbackTrans
                gcnSQLSEVER_兴成.RollbackTrans
                Exit Function
            End If
...
Public Function 住院结算_兴成(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
  '功能：将需要本次结帐的费用明细和结帐数据发送医保前置服务器确认；
   ...
   '更新结算标志
    gstrSQL = "Select ID,结帐金额 as 实收金额 From 病人费用记录 where 结帐ID=" & lng结帐ID
    ...
    If dbl费用总额 <> g结算数据.交易费用总额 Then
        ShowMsgbox "虚拟结算数据的费用总额与本次结算的费用总额不等，请检查处方是否正确!"
        Exit Function
    End If
...
Public Function 住院结算冲销_兴成(lng结帐ID As Long) As Boolean
    ...
    '----------------------------------------------------------------
    ShowMsgbox "本医保不支持住院结算冲销,具体请咨询接口商!"
    住院结算冲销_兴成 = False
    Exit Function
ErrHand:
    If ErrCenter() = 1 Then
...
Public Function 处方登记_兴成(ByVal lng记录性质 As Long, ByVal lng记录状态 As Long, ByVal str单据号 As String) As Boolean
  ...
  gstrSQL = "" & _
              ...
              "  From 病人费用记录 A,收费细目 B,病案主页 C" & _
              "  where A.NO='" & str单据号 & "' and A.记录性质=" & lng记录性质 & " and A.记录状态 = " & lng记录状态 & " " & _
              "        and A.收费细目ID=B.ID and A.病人ID=C.病人ID  and A.主页ID=C.主页ID And C.险类=" & TYPE_兴成核工业 & _
    ...
    With rs明细
        ...
        Do While Not .EOF
            gstrSQL = "Select * From 保险支付项目 where 险类=" & TYPE_兴成核工业 & " and 收费细目id=" & Nvl(!收费细目ID, 0)
            Call OpenRecordset(rsTemp, "确定医保支付项目")
            If rsTemp.EOF Then
...
Public Function Open中间库_兴成() As Boolean
    ...
    '重新建立到医保服务器的公共连接
    '中间库连接
    gstrSQL = "select 参数名,参数值 from 保险参数 where  险类=" & TYPE_兴成核工业
    Call OpenRecordset(rsTemp, "兴成核工业医保")
