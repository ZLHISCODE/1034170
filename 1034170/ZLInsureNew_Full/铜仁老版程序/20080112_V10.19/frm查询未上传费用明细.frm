...
Private Sub Form_Load()
    ...
    gstrSQL = "Select DECODE(A.记录性质,3,'自动记帐','记帐') AS 单据,DECODE(A.记录状态,2,'冲销','正常') AS 类型,A.No,A.序号,E.名称," & _
             " trim(to_char(Nvl(A.数次,0)*Nvl(A.付数,1),'99999990.00')) As 数量,trim(to_char(A.标准单价,'99999990.00')) AS 标准单价," & _
             " trim(to_char(A.实收金额,'99999990.00')) AS 实收金额,F.项目编码 AS 医保编码" & _
             " From 病人费用记录 A,病人信息 B,病案主页 C,保险帐户 D,收费细目 E,保险支付项目 F " & _
             " Where A.病人ID=B.病人ID And B.病人ID=C.病人ID And A.主页ID=C.主页ID And A.病人ID=D.病人ID And D.险类=" & mintInsure & _
             " And Nvl(记录状态,0)<>0 And Nvl(附加标志,0)<>9 And Nvl(实收金额,0)<>0 And Nvl(A.是否上传,0)=0 " & _
             " And (Nvl(A.门诊标志,0)<>1 And Nvl(A.门诊标志,0)<>4)" & _
             " And A.收费细目ID=E.Id And E.ID=F.收费细目ID(+) And F.险类(+)=" & mintInsure & _
             " And A.病人ID=" & mlng病人ID & " And A.主页ID=" & mlng主页ID & _
             " Order By A.登记时间,No,序号"
    Call OpenRecordset(rsTemp, "提取指定病人指定住院的所有未上传明细")
