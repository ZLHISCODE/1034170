...
'功能：通过医保中心号码和医保号求出病人ID
    ...
    gstrSQL = "select 病人ID from 保险帐户 where 险类 = '" & TYPE_浙江 & "' and 医保号 = '" & str医保号 & "'"
...
Private Function Get卡号(lng病人ID As Long, Optional flag As Boolean = False) As String
    Dim rsTemp As New ADODB.Recordset
    On Error GoTo errHandle
    gstrSQL = "Select * From 保险帐户 Where 险类=" & TYPE_浙江 & " And 病人id=" & lng病人ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'功能：测试是否可以连接到前置服务器上
    ...
    '如果连接已经打开，那就不用再测试
    If gcn浙江.State = adStateOpen Then
        gstrSQL = "Select * From 保险中心目录 Where 险类=" & TYPE_浙江
        Call OpenRecordset(rsTemp, "医保初始化")
        gstr医保机构编码 = rsTemp!编码
        gstrSQL = "Select * From 保险类别 Where 序号=" & TYPE_浙江
        Call OpenRecordset(rsTemp, "医保初始化")
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    WriteInfo "准备冲销"
    '取原单据交易流水号
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_浙江 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'    str结算方式  "报销方式;金额;是否允许修改|...."
    ...
    Do Until rs明细.EOF
        ...
        If bln门诊 = True Then
            ...
            gstrSQL = "Select 名称 as 科室 From 部门表 Where Id In (Select 部门ID From 部门人员 Where 缺省=1 And 人员ID In (Select A.Id From 人员表 A,人员性质说明 B Where A.ID=B.人员ID And B.人员性质='医生' and 姓名='" & rs明细!开单人 & "'))"
            Call OpenRecordset(rsTemp, "门诊传明细取科室")
        ...
        Select Case rsTemp!类别
            ...
            Case Else
                str收费类别 = "91"
        End Select
        gstrSQL = "Select * From 保险支付项目 Where 险类=" & TYPE_浙江 & " And 是否医保=1 And 收费细目ID=" & rs明细!收费细目ID
'        gstrSQL = "Select A.*,B.名称 As 大类名称,B.编码 As 大类编码 from 保险支付项目 A,保险支付大类 B Where A.险类=B.险类 And " & _
        ...
        If rsTemp.EOF Then
            ...
            If InStr("11 12", str收费类别) > 0 Then
                gstrSQL = "Select * From 药品目录 Where 药品ID=" & rs明细!收费细目ID
                Call OpenRecordset(rs药品, "取药品信息")
                If rs药品.EOF Then
                    str药品等级 = "1"
        ...
        Else
            ...
            Else
                ...
                If Nvl(rsTemp!大类id, "") <> "" Then
                    gstrSQL = "Select * From 保险支付大类 Where ID=" & rsTemp!大类id & " And 险类=" & TYPE_浙江
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    WriteInfo "准备冲销"
    '取原单据交易流水号
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_浙江 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
