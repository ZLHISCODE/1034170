...
Public Function 个人余额_大连(ByVal lng病人ID As Long, ByVal intInsure As Integer) As Currency
    ...
    '读卡失败则退出
    gstrSQL = "Select Nvl(帐户余额,0) 帐户余额,退休证号 From 保险帐户 Where 险类=" & intInsure
    ...
    With rs明细
        ...
        Do While Not .EOF
            ...
            gstrSQL = " Select 项目编码,项目名称 From 保险支付项目" & _
                      " Where 险类=" & intInsure & " And 收费细目ID=" & !收费细目ID
...
Public Function 入院登记_大连(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String, ByVal intInsure As Integer) As Boolean
    ...
    gstrSQL = "Select 病区ID,床号,房间号 From 床位状况记录 D where 病区ID=" & Nvl(rsTemp!当前病区ID, 0) & " And 床号=" & Nvl(rsTemp!当前床号, 0)
...
Private Function Get获取历次病人结帐信息(ByVal lng病人ID As Long, lng主页ID As Long, ByVal intInsure As Integer) As Boolean
    ...
    gstrSQL = "" & _
            ...
            "   Where A.险类=" & intInsure & " And A.病人ID=" & lng病人ID & _
...
'            "where mod(记录性质,10)=2 and 病人id=" & lng病人ID & " and 结帐id=" & lng结帐ID
    gstrSQL = "select Nvl(主页id,0) as 主页id from 病人费用记录 " & _
            "where rownum=1 and 病人id=" & lng病人ID & " and 结帐id=" & lng结帐ID
...
Public Sub 更新病种_大连(ByVal lngPatiID As Long, ByVal lngPageID As Long, intInsure As Integer)
    ...
    gstrSQL = "select 描述信息 from 诊断情况 where 诊断类型=3 and 诊断次序=1 and 病人ID=" _
            & lngPatiID & " and 主页ID=" & lngPageID - 1
    OpenRecordset rsTemp, "提取上次出院诊断", gstrSQL, gcnOracle
...
Public Function IsParaBig(intInsure) As Boolean
    ...
    gstrSQL = "Select 参数值 From 保险参数 where 参数名='门诊大病使用住院比例' and 险类=" & intInsure
    OpenRecordset rsTemp, "提取参数", gstrSQL, gcnOracle
...
Public Function IsParaQ(intInsure) As Boolean
    ...
    End If
    On Error GoTo ErrName
    gstrSQL = "Select 参数值 From 保险参数 where 参数名='门诊企离使用住院比例' and 险类=" & intInsure
    OpenRecordset rsTemp, "提取参数", gstrSQL, gcnOracle
