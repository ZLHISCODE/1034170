...
On Error GoTo ErrHand
    ...
    If SetZYRegister(医保号, Nvl(rsTmp!住院医师, rsTmp!门诊医师), rsTmp!入院科室, CStr(Format(rsTmp!入院日期, "YYYY-MM-DD")), CStr(rsTmp!病人ID) & "_" & CStr(rsTmp!主页ID), R住院结算号, S就诊类型, S来院状态, S医生代码, S入院诊断, S入院类型) <> 1 Then
        MsgBox "错误信息" & GetMyLastError() & "补充登记失败", vbInformation, "合医返回信息"
        入院登记_铜梁合医 = False
        Exit Function
    Else
...
On Error GoTo ErrHand
    ...
    Else
        ...
        gstrSQL = "select * from 病人费用记录 where 病人id=" & lng病人ID & " and 主页id=" & lng主页ID
...
On Error GoTo errHandle
    ...
    If ZYcheckout(s住院结算单号, s合医编码, s出院日期, s病种代码, s手术名称, UserInfo.姓名, R发生总费用, R药品总费用, R检查治疗总费用 _
        ...
        MsgBox "错误信息：" & GetMyLastError() & vbLf & "         预结失败，不能结算", vbInformation, "合医返回信息"
        住院结算_铜梁合医 = False
...
On Error GoTo ErrHand
    ...
    If StrickthebalanceZYFYBX(rs暂用!支付顺序号) <> 1 Then
        ...
        住院结算冲销_铜梁合医 = False
    Else
        gstrSQL = "Select ID From 病人费用记录 Where 记录性质=2 and 结帐id In (Select b.结帐id From 病人预交记录 a /*销帐记录*/,病人预交记录 b/*原结帐记录*/ Where a.结帐id=" & lng结帐ID & " And a.病人id=b.病人id And a.主页id=b.主页id And b.记录性质=2)"
...
On Error GoTo ErrHand
    If int状态 = 1 Then
        ...
        Conn上传.ConnectionString = gcnOracle.ConnectionString
        Conn上传.Open
        '记帐表的情况先取病人id然后循环上传
        gstrSQL = "select distinct 病人id from 病人费用记录 where 记录性质=2 and  no='" & str单据号 & "'"
...
Conn冲销.ConnectionString = gcnOracle.ConnectionString
Conn冲销.Open
    ...
    If int状态 = 2 Then
        ...
        '查出要冲销的合医流水号
           gstrSQL = "Select a.Id," & _
                             ...
                             "b.保险编码," & _
                             "b.摘要 As 冲销流水号" & _
                    " From 病人费用记录 a/*新记录*/, 病人费用记录 b/*原记录*/" & _
                    " Where b.No = '" & str单据号 & "'" & _
                            " And b.记录性质 = " & int性质 & _
                            " And b.记录状态 = 3" & _
                            " And b.是否上传 = 1" & _
