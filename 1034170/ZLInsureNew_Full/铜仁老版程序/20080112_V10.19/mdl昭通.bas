...
Public Function 医保初始化_昭通(Optional ByVal bln操作员检查 As Boolean = True) As Boolean
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & TYPE_昭通
    ...
    gstrSQL = "Select * From 保险类别 Where 序号=" & TYPE_昭通
    Call OpenRecordset(rsTemp, "医保初始化")
...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_昭通
...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    While Not rs明细.EOF
        ...
        gstrSQL = "Select * From 保险支付项目 Where 险类=103 And 收费细目ID=" & rs明细!收费细目ID
        Call OpenRecordset(rsTemp, "取保险支付项目")
        If rsTemp.EOF Then             '如果没有对码则不允许使用
            gstrSQL = "Select * From 收费细目 Where ID=" & rs明细!收费细目ID
            Call OpenRecordset(rsTemp, "取收费细目")
            MsgBox "项目[" & rsTemp!名称 & "]没有对应的医保项目,请先进行对码", vbInformation, "门诊结算"
        ...
        ElseIf Nvl(rsTemp!附注) <> "已审批" And InStr(" 5 6 7 ", " " & strTemp & " ") > 0 Then
            ...
            Call OpenRecordset(rsCheck, "如果是省目录的甲、乙类项目，不需要检查审批标志", gstrSQL, gcn昭通)
            If rsCheck.RecordCount <> 0 Then
                If rsCheck!lb = 15 Then
                    gstrSQL = "Select * From 收费细目 Where ID=" & rs明细!收费细目ID
                    Call OpenRecordset(rsTemp, "取收费细目")
                    MsgBox "项目[" & rsTemp!名称 & "]" & strTemp & "，不能使用", vbInformation, "门诊结算"
...
'            End If
    ...
    Call OpenRecordset(rsTemp, "取医保病人信息")
...
errHandle:
    ...
    If blnBalance Then
        MsgBox "请通过医保工具将当前这笔门诊收费单据冲销，以下信息请记录：" & vbCrLf & _
            "就诊编号：" & str就诊编号 & "；个人帐户：" & cur个人帐户, vbInformation, gstrSysName
    End If
End Function
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "Select 病人ID,结帐金额,操作员编号 From 病人费用记录 Where nvl(附加标志,0)<>9 and 结帐ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
        MsgBox "找不到单据的明细记录,不能进行冲销", vbInformation, "门诊冲销"
        Exit Function
    End If
    lng病人ID = rsTemp!病人ID
    If rsTemp!操作员编号 <> str操作员 Then
        MsgBox "医保规定必须由执行本单据门诊结算的操作员进行冲销", vbInformation, "门诊冲销"
        Exit Function
    End If
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_昭通 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'    Call OpenRecordset(rsTemp, gstrSysName)
    ...
    If frmConn昭通.Query(0, 1) = False Then Exit Function
    str住院序号 = frmConn昭通.strReturnInfo
...
'返回：交易成功返回true；否则，返回false
    ...
    Call OpenRecordset(rsTemp, "只要存在病人费用记录则不允许办理撤销入院,只能出院登记")
...
'    str结算方式  "报销方式;金额;是否允许修改|...."
    ...
    gstrSQL = "Select nvl(sum(金额),0) From 病人预交记录 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'如果住院费用明细传递失败，就直接结束函数，返回函数失败
    ...
    Call OpenRecordset(rsTemp, gstrSysName)
    lng主页ID = rsTemp(0)
    gstrSQL = "Select A.出院病床,B.名称 From 病案主页 A,部门表 B Where A.出院科室ID=B.ID And 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsTemp, gstrSysName)
    ...
    gstrSQL = "Select * From 病人预交记录 Where 结帐id=" & lng结帐ID & " And 结算方式='个人帐户'"
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
        cur个帐支付 = 0
    Else
    ...
    If cur个帐支付 <> 0 Then
        strPara = str住院号 & vbTab & rsTemp!卡号 & vbTab & IIf(Nvl(rsTemp!密码, "") = "", " ", rsTemp!密码) & vbTab & cur个帐支付
    Else
...
'            If Not frmConn昭通.Execute("I320", 0, str住院序号, "正在进行医保交易,请稍候......") Then Exit Function
            ...
            If strPara <> "" Then   '如果有参数表示该病人有数据，调用接口上传
                ...
                If frmConn昭通.Query(0, 1) = False Then Exit Function
                str批号 = Left(frmConn昭通.strReturnInfo, InStr(frmConn昭通.strReturnInfo, vbTab) - 1)
                strRetu = Split(Mid(frmConn昭通.strReturnInfo, InStr(frmConn昭通.strReturnInfo, vbTab) + 1), ";")
            
                For lngLoop = 1 To frmConn昭通.mlngRows + 1
                    If frmConn昭通.Query(lngLoop - 1, 1, "正在读取明细上传返回的结果集：" & lngLoop & "/" & (frmConn昭通.mlngRows + 1)) = False Then Exit Function
                    If Split(frmConn昭通.strReturnInfo, ",")(2) = "0" Then
                        gstrSQL = "zl_病人记帐记录_上传 ('" & strID(lngLoop - 1) & "')"
                        Call zlDatabase.ExecuteProcedure(gstrSQL, gstrSysName)
                    Else
                        gstrSQL = "Select B.名称 From 病人费用记录 A,收费细目 B Where A.收费细目ID=B.ID And A.ID=" & strID(lngLoop - 1)
                        Call OpenRecordset(rsTemp, gstrSysName)
                        If rsTemp.RecordCount <> 0 Then
                            strMessage = strMessage & "项目[" & rsTemp(0) & "]上传失败，返回信息：" & Split(frmConn昭通.strReturnInfo, ",")(3) & Chr(13) & Chr(10)
                        Else
        ...
        gstrSQL = "Select * From 保险支付项目 Where 收费细目ID=" & rs明细!收费细目ID & " And 险类=" & TYPE_昭通
        Call OpenRecordset(rsTemp, gstrSysName)
        If rsTemp.EOF Then
            gstrSQL = "Select 编码||名称 From 收费细目 Where ID=" & rs明细!收费细目ID
            Call OpenRecordset(rsTemp, gstrSysName)
...
Public Function 出院登记_昭通(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) Then
        '不存在未结费用,再检查是否结算过
        gstrSQL = "SELECT 1 FROM 病人费用记录 WHERE 结帐ID IS NOT NULL AND  病人ID=" & lng病人ID & " AND 主页ID=" & lng主页ID & " AND ROWNUM<2"
        Call OpenRecordset(rsTemp, "是否结算过")
        If rsTemp.RecordCount = 0 Then
...
Public Function GetItemInfo_昭通(ByVal intType As Integer, ByVal lng病人ID As Long, ByVal lng细目ID As Long, _
    ...
    gstrSQL = "Select 类别 From 收费细目 Where ID=" & lng细目ID
    Call OpenRecordset(rsTemp, "判断是否是中草药")
    If rsTemp.RecordCount = 0 Then Exit Function
    If InStr(1, "5,6", rsTemp!类别) <> 0 Then
        gstrSQL = "Select 项目编码,附注 From 保险支付项目 Where 收费细目ID=" & lng细目ID & " And 险类=" & TYPE_昭通
        Call OpenRecordset(rsTemp, "判断是否对码")
        If rsTemp.RecordCount = 0 Then Exit Function
...
Public Function CheckInsureItem_昭通(ByVal lng收费细目ID As Long) As Boolean
    ...
    gstrSQL = "Select 项目编码,附注 From 保险支付项目 Where 险类=" & TYPE_昭通 & " And 收费细目ID=" & lng收费细目ID
    Call OpenRecordset(rsTemp, "检查项目是否通过中心的审批")
    If rsTemp.RecordCount <> 0 Then
        str附注 = Nvl(rsTemp!附注)
        str项目编码 = rsTemp!项目编码
...
Public Sub SaveOutExse(ByVal str流水号 As String)
    ...
    For lngLoop = 1 To frmConn昭通.mlngRows + 1
        If frmConn昭通.Query(lngLoop - 1, 1, "正在更新数据(" & lngLoop & "/" & (frmConn昭通.mlngRows + 1) & ")......") = False Then Exit Sub
...
Private Function Get自费药品(ByVal strCode As String) As Boolean
    ...
    Call OpenRecordset(rsCheck, "检查药品类型", gstrSQL, gcn昭通)
