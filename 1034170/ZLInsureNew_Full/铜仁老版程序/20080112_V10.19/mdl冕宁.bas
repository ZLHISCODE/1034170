...
'功能说明:本医保不允许冲销
    ...
    门诊结算冲销_冕宁 = False
    MsgBox "冕宁医保规定:不允许冲销已结算单据!", vbInformation, gstrSysName
End Function
...
'Beging 取医保号
lng病人ID = rs明细记录!病人ID
mstrCdSql = "Select * from 保险帐户 where 病人ID=" & lng病人ID & " And 险类=" & TYPE_冕宁
Call OpenRecordset(mrsCdTmp, "病人信息", mstrCdSql)
...
Do Until rs明细记录.EOF
    mstrCdSql = "select * from 保险支付项目 where 险类=" & TYPE_冕宁 & " and 收费细目ID=" & rs明细记录!收费细目ID
    Call OpenRecordset(mrsCdTmp, "医保明细", mstrCdSql)
    If mrsCdTmp.EOF Then
        mstrCdSql = "Select * from 收费项目目录 where ID=" & rs明细记录!收费细目ID
        Call OpenRecordset(mrsCdTmp, "收费项目目录", mstrCdSql)
...
mstrCdSql = "Select ID,NO,序号,记录性质,登记时间 as 结算时间,病人ID,收费类别,收据费目,计算单位,开单人, " & _
            ...
            "from 病人费用记录 " & _
            "where 结帐ID=" & lng结帐ID
Call OpenRecordset(rscd明细记录, "费用记录", mstrCdSql)
...
mstrCdSql = "Select * from 保险帐户 where 险类=" & TYPE_冕宁 & " And 病人ID=" & lng病人ID
Call OpenRecordset(mrsCdTmp, "保险帐户", mstrCdSql)
sbh = mrsCdTmp.Fields!医保号
rylx = mrsCdTmp.Fields!就诊类别
net = mrsCdTmp.Fields!密码
Zhzf = cur个帐支付

mstrCdSql = "Select sum(nvl(冲预交,0)) as 金额 from 病人预交记录 where 结算方式='现金' And 结帐ID=" & lng结帐ID
Call OpenRecordset(mrsCdTmp, "保险帐户", mstrCdSql)
xjzf = Nvl(mrsCdTmp!金额, 0)

mstrCdSql = "Select sum(nvl(冲预交,0)) as 金额 from 病人预交记录 where 结算方式='统筹基金' And 结帐ID=" & lng结帐ID
Call OpenRecordset(mrsCdTmp, "保险帐户", mstrCdSql)
...
Select Case tmp
    ...
    Case 2
        ...
        Exit Function
    Case 3
        MsgBox "网络或刷卡错误", vbInformation, gstrSysName
        Exit Function
    Case Else
        MsgBox "错误", vbInformation, gstrSysName
        Exit Function
...
'    ryappend          : 医保入院登记交易
    ...
    mstrCdSql = "Select * from 保险帐户 where 险类=" & TYPE_冕宁 & " And 病人ID=" & lngPatiID
    Call OpenRecordset(mrsCdTmp, "保险帐户", mstrCdSql)
    sbh = mrsCdTmp!医保号
    yyzyh = lngPatiID & "_" & lngPageID
    net = mrsCdTmp.Fields!密码
    
    mstrCdSql = "Select * from 病案主页 where 险类=" & TYPE_冕宁 & " And 病人ID=" & lngPatiID & " And 主页ID=" & lngPageID
    Call OpenRecordset(mrsCdTmp, "病案主页", mstrCdSql)
    ...
    izhyj = 0
    zt = 99
    mstrCdSql = "select 诊断类型,描述信息 From 诊断情况 Where 病人ID=" & lngPatiID & " And 主页ID=" & lngPageID & " And 诊断类型>=1 and 诊断类型<=2 order by 诊断类型 desc"
    Call OpenRecordset(mrsCdTmp, "入院诊断", mstrCdSql)
...
mstrCdSql = "select A.入院日期,A.出院日期,B.住院号,D.名称 as 住院科室,A.入院病床,A.住院医师,C.卡号," & _
          ...
          "A.入院科室ID = D.ID And A.主页ID = " & lngPageID & " And A.病人ID = " & lngPatiID & _
          " and C.险类=" & TYPE_冕宁

Call OpenRecordset(rsCd, "保险病人", mstrCdSql)
If rsCd.EOF Then
    出院登记撤销_冕宁 = False
...
'    cwappend: 床位登记
    ...
    gstrSQL = "Select B.名称||' '||A.床号||'床' as 床位号,to_char(开始时间,'YYYY-MM-DD') as 开始日期" & _
            ...
            "where A.科室ID=B.ID And A.病人ID=" & lngPatiID & " And A.主页ID=" & lngPageID & _
            " And A.终止时间 is null And A.床号 is not null"
    Call OpenRecordset(mrsCdTmp, "变动记录", gstrSQL)
...
' (无)
    ...
    gstrSQL = "Select Nvl(帐户余额,0) AS 个人帐户 From 保险帐户 " & _
              " Where 医保号='" & str医保号 & "' and 险类=" & TYPE_冕宁
...
'功能:冕宁医保不支持结算冲销
    ...
    MsgBox "冕宁医保不支持结算冲销!", vbInformation, gstrSysName
    住院结算冲销_冕宁 = False
End Function
...
'其他参数由住院初始化返回
    '提取结帐单号
    gstrSQL = "Select NO,病人ID From 病人结帐记录 Where ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "提取结帐单号")
    strNO = "2" & rsTemp!NO
    lng病人ID = rsTemp!病人ID
    ...
    '2005 0520 添加出院诊断
    gstrSQL = "select * From 诊断情况 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID & " And 诊断类型=3"
    Call OpenRecordset(rsTemp, "诊断情况", gstrSQL)
    ...
    If m_cyzd = "无" Then
        gstrSQL = "select * From 诊断情况 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID & " And 诊断类型=1"
        Call OpenRecordset(rsTemp, "诊断情况", gstrSQL)
        If Not rsTemp.EOF Then
            m_cyzd = Trim(Nvl(rsTemp!描述信息, "无"))
        Else
...
'    jzdel            : 记帐费用删除
    ...
    If int状态 = 1 Then
    gstrSQL = "Select distinct  A.病人ID from 病人费用记录 A,保险帐户 B " & _
            "where A.病人ID=B.病人ID And A.记录性质=" & int性质 & _
            " And A.记录状态=" & int状态 & " And A.NO='" & str单据号 & "' " & _
            " And B.险类=" & TYPE_冕宁 & " And A.实收金额<0"
        Call OpenRecordset(rsCd, "是否有负数记录")
    ...
    Do Until rsCd.EOF
        ...
        'If int状态 = 1 Then
            gstrSQL = "Select A.*,D.项目编码,nvl(A.付数,1)*nvl(A.数次,0) as 数量,A.实收金额 as 金额," & _
                              ...
                              " from 病人费用记录 A,保险帐户 B,部门表 C,保险支付项目 D " & _
                              " where A.NO='" & str单据号 & _
                                    "'And A.记录性质=" & int性质 & _
                                    " And A.记录状态=" & int状态 & _
...
'    cycsh          :出院初始化
    ...
    gstrSQL = "select * from 病人信息 where 病人ID=" & lng病人ID & " And 险类=" & TYPE_冕宁
    Call OpenRecordset(mrsCdTmp, "病人信息", gstrSQL)
    If mrsCdTmp.EOF Then
    ...
    '>beging 补传未上传记录
       ...
       gstrSQL = "Select distinct 记录性质,记录状态,NO From 病人费用记录 A,保险帐户 B,病人信息 C " & _
                 ...
                 " AND A.实收金额 IS NOT NULL And B.病人ID=" & lng病人ID & " And B.险类=" & TYPE_冕宁
        Call OpenRecordset(mrsCdTmp, "病人费用记录", gstrSQL)
        Do Until mrsCdTmp.EOF
    ...
    If tmp = 0 Then
        ...
        Call OpenRecordset(mrsCdTmp, "未结费用", gstrSQL)
...
str_SQL = "Select * from 人员表 where id=" & UserInfo.ID
Call OpenRecordset(rsRy, "人员表", str_SQL)
...
'    inkb    :添加科别
    ...
    mstrCdSql = "Select * from 人员表 where 姓名='" & str姓名 & "'"
    Call OpenRecordset(rsTemp, "人员表", mstrCdSql)
    lng人员ID = rsTemp!ID
    str医师编码 = rsTemp!编号
    str医师姓名 = rsTemp!姓名
    ...
    If Nvl(rsTemp!个人简介, "9") <> "1" Then
        ...
        mstrCdSql = "select * from 部门表 where ID in (select 部门ID from 部门人员 where 缺省=1 and 人员ID=" & lng人员ID & ")"
        Call OpenRecordset(rsTemp, "部门表", mstrCdSql)
...
'    inkb    :添加科别
    ...
    mstrCdSql = "select * from 部门表 where ID =" & lng科室ID
    Call OpenRecordset(rsTemp, "部门表", mstrCdSql)
