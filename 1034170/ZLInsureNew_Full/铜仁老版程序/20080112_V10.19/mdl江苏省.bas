...
'            gstrSQL = "Select * From 收费细目 Where ID=" & rs明细!收费细目ID
            gstrSQL = "select a.费用类型,A.名称,C.项目编码 as 编码,A.计算单位,B.产地,decode(B.药品来源,'国产',1,'合资',2,'进口',3,null) 产地特征,B.规格" & _
                  " from 收费细目 A,药品目录 B,保险支付项目 C where A.id = C.收费细目id and A.id=B.药品id(+) and A.id =" & rs明细!收费细目ID & " And C.险类=" & TYPE_江苏
            Call OpenRecordset(rsTemp, gstrSysName)
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_江苏 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'返回：交易成功返回true；否则，返回false
    ...
    '获取住院医师代码
    gstrSQL = "Select * from 人员表 Where 姓名='" & rsTemp!门诊医师 & "'"
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_江苏 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
reTrans:
 ...
 '以下增加出院函数调用功能2007-09-28陈云
    ...
    Else
        gstrSQL = "select max(主页id) 主页id from 病案主页 where 病人id=" & lng病人ID & " And 险类=" & TYPE_江苏
        Call OpenRecordset(rsTemp, "提取主页id")
        lng主页ID = rsTemp!主页ID
...
Public Function 记帐传输_江苏(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人ID As Long = 0) As Boolean
    ...
    If str消息 = "" Then
        gstrSQL = " Select A.* From 病人费用记录 A,保险帐户 B" & _
                  ...
                  " and A.病人ID=B.病人ID And B.险类=" & TYPE_江苏 & _
                  " order by A.NO,A.序号"
    Else
        gstrSQL = " Select A.* From 病人费用记录 A,保险帐户 B" & _
