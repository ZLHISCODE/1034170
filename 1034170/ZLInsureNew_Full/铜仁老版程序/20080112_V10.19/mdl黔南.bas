        ...
        Do Until rs明细.EOF
            ...
            If Val(Nvl(rs明细("实收金额"), 0)) <> 0 Then
                ...
                Else
                    ...
                    '吕说:中草药的需传1
                    If InStr(1, "7中草药", Nvl(!收费类别)) And Nvl(rsTemp!项目编码, "9999999999") = "9999999999" Then
                        StrInput = StrInput & "1" & "|"
                    Else
...
Private Function 检查中心维护单价(ByVal lng病人ID As Long, ByVal lng细目ID As Long, ByVal dbl单价 As Double) As Boolean
    ...
    gstrSQL = "" & _
        "   Select * From 医保收费目录 " & _
        "   Where (类别,编码) in (select 附注,项目编码 From 保险支付项目 where 险类=" & TYPE_黔南 & " and 收费细目id=" & lng细目ID & " )"
...
Public Function 门诊结算_黔南(lng结帐ID As Long, cur个人帐户 As Currency, str医保号 As String, cur全自付 As Currency) As Boolean
    ...
    gstrSQL = "Select a.*,a.*,a.付数*a.数次 as 数量,a.实收金额/(nvl(a.付数,1)*nvl(a.数次,1)) as 单价 From 病人费用记录 a Where nvl(a.实收金额,0)<>0 and  结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0"
...
Public Function 门诊结算冲销_黔南(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    gstrSQL = "select 备注 from 保险结算记录 where 性质=1 and 险类=" & TYPE_黔南 & " and 记录ID=" & lng结帐ID
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_黔南 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "获取原来的结算记录")
...
Public Function 住院结算_黔南(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
  '功能：将需要本次结帐的费用明细和结帐数据发送医保前置服务器确认；
    ...
    If 医保病人已经出院(lng病人ID) = False Then
        MsgBox "医保病人必须先出院后才能办理结算!", vbInformation, gstrSysName
        Exit Function
    End If
...
Public Function 住院结算冲销_黔南(lng结帐ID As Long) As Boolean
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_黔南 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "结算冲销")
    If rsTemp.EOF = True Then
        MsgBox "原单据的医保记录不存在，不能作废。", vbInformation, gstrSysName
        Exit Function
...
Private Function 处方上传(ByVal lng记录性质 As Long, lng记录状态 As Long, ByVal str单据号 As String) As Boolean
   ...
   '读出该张单据的费用明细
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.主页ID,to_char(A.发生时间,'yyyy-mm-dd hh24:mi:ss') as 登记时间,Round(A.实收金额,4) 实收金额 " & _
              ...
              "  From 病人费用记录 A,收费类别 J,收费细目 B,保险帐户 M,(Select * From 保险支付项目 where 险类=" & TYPE_黔南 & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
              "  where a.病人id=M.病人id  and M.险类=" & TYPE_黔南 & " and A.NO='" & str单据号 & "' and A.记录性质=" & lng记录性质 & " and A.记录状态=1 And Nvl(A.是否上传,0)=0 " & _
    ...
    With rs明细
        Do Until .EOF
            gstrSQL = "Select * from 医保收费目录 where 类别=" & Val(Nvl(!附注)) & " and 编码='" & Nvl(!项目编码) & "'"
...
'            End If
                ...
                Else
                    '刘兴宏:2007/09/28加入,自费部分又是中药的,对应为甲类
                    '吕说:中草药的需传1
                    If InStr(1, "7中草药", Nvl(!收费类别)) And Nvl(!项目编码, "9999999999") = "9999999999" Then
                        StrInput = StrInput & "1" & "|"
                    Else
...
Private Function 补传住院明细记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    gstrSQL = "Select A.ID,A.NO,A.记录性质,J.类别 as 收费类别,A.记录状态,A.序号,A.病人ID,A.主页ID,to_char(A.发生时间,'yyyy-mm-dd hh24:mi:ss')  as 登记时间,Round(A.实收金额,4) 实收金额" & _
              ...
              "  From 病人费用记录 A,收费类别 J,收费细目 B, (Select * From 保险支付项目 where 险类=" & TYPE_黔南 & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
