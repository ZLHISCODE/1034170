...
Begin VB.Form frm保险项目查找广元旺苍 
   ...
   End
End
Attribute VB_Name = "frm保险项目查找广元旺苍"
...
Private Sub Form_Load()
    Dim nod As Node
    '取出当前类别
    Set nod = frm保险项目_广元旺苍.tvwMain_S.SelectedItem
    With frm保险项目_广元旺苍.cmb险类
...
Private Sub cmdFind_Click()
    ...
    If txtEdit(3).Text <> "" Then
        str医保支付项目 = "(select 收费细目ID,险类 from 医保支付项目 where upper(项目编码) like '" & IIf(optMatch(1).Value = True, "%", "") & UCase(txtEdit(3).Text) & "%')"
    Else
        str医保支付项目 = "医保支付项目"
    End If
...
Private Sub LocateItem()
    ...
    With frm保险项目_广元旺苍.tvwMain_S
        lngID = mrsFind("ID")
        If mrsFind("类别") = "5" Or mrsFind("类别") = "6" Or mrsFind("类别") = "7" Then
            '这条细目是药品,其定位要复杂一些
            gstrSQL = "select B.材质分类,B.用途分类ID from 药品目录 A,药品信息 B " & _
                      " Where A.药名ID = B.药名ID And A.药品ID =" & lngID
    ...
    End With
    frm保险项目_广元旺苍.FillSum
        
    lngID = mrsFind("ID")
    With frm保险项目_广元旺苍.mshSum_S
