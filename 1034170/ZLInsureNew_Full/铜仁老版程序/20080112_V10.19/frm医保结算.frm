        ...
        Case 1 '-收费
            ...
            Case TYPE_重庆市
                gstrSQL = _
                    ...
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.进入统筹金额),'999990.00')) as 进入统筹,Decode(C.超限自付金额,13,'特殊门诊',14,'急诊抢救','普通门诊') 门诊类别,C.备注 病种" & _
            ...
            Case TYPE_陕西大兴
                ...
                gstrSQL = _
                    ...
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.个人帐户支付),'999990.00')) as 个人帐户," & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.进入统筹金额),'99999990.00')) as 统筹支付," & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.大病自付金额),'99999990.00')) as 公务员床补," & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.起付线),'999990.00')) as 押金总额, " & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.封顶线),'999990.00')) as 公费床位费, " & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.实际起付线),'999990.00')) as 自费床位费, " & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.全自付金额),'999990.00')) as 公费调温费, " & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.超限自付金额),'999990.00')) as 自费调温费, " & _
                    "   Ltrim(To_Char(decode(A.记录状态,2,-1,1)*Sum(C.首先自付金额),'999990.00')) as 应补现金额 " & _
                    " From 病人费用记录 A,部门表 B,保险结算记录 C,保险帐户 D,保险中心目录 E,保险人群 F" & _
                    " Where A.记录性质 = 1 And A.操作员姓名 IS NOT NULL AND A.开单部门ID = B.ID(+) And A.登记时间>=" & strBegin & " and A.登记时间<=" & strEnd & _
                    "       and A.序号=1 and A.结帐ID=C.记录ID and C.性质=1 and C.险类=" & mint险类 & _
                    "       and A.病人ID=D.病人ID and D.险类=" & mint险类 & IIf(mstrCardCond = "", "", " And D.医保号='" & mstrCardCond & "'") & " And D.险类=E.险类 and D.中心=E.序号 and D.险类=F.险类 and D.在职=F.序号 " & _
