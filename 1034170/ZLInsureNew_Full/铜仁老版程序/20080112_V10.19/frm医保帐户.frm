...
#Const gverControl = 0  ' 0-不支持动态医保(9.19以前),1-支持动态医保无附加参数(9.22以前) , _
...
Private Sub mnuEditDisease_Click()
    ...
    With msh帐户_S
        ...
        ElseIf mint险类 = TYPE_泸州市 Then
            ...
            '提取病案主页
            gstrSQL = "Select Nvl(是否上传,0) AS 是否上传 From 病案主页 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
            Call OpenRecordset(rsTemp, "提取病案主页")
            int上传 = rsTemp!是否上传
            
            '提取病人选择的病种
            gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
                    " From 保险病种 A,zlyb.病种信息 B where A.险类=" & mint险类 & " And B.病人ID=" & lng病人ID & " And B.主页ID=" & lng主页ID & " ANd B.状态=" & int上传 & " And A.ID=B.病种ID And A.险类=B.险类"
            Call OpenRecordset(rsSelected, "提取上次已选择的病种")
                ...
                If frm多病种选择.ShowSelect(rs病种, "ID", "医保病种选择", "请选择医保病种：", rsSelected, False) = True Then
                    ...
                    With rs病种
                        ...
                        If int上传 = 1 Then
                            ...
                            Call OpenRecordset(rsTemp, "判断是否存在出院结算记录")
...
Private Sub mnuEditOut_Click()
    ...
    Case TYPE_昆明市, TYPE_云南省
        ...
        If Not 出院登记_云南(lng病人ID, lng主页ID, rsTemp!顺序号, mint险类) Then Exit Sub
        MsgBox "该医保病人成功补办出院手续！", vbInformation, gstrSysName
    Case TYPE_沈阳市
...
Private Sub mnuEditRollIncome_Click()
    ...
    With msh帐户_S
        ...
        '曾明春屏蔽
'        If mint险类 = TYPE_成都德阳 Or mint险类 = TYPE_广元旺苍 Or mint险类 = TYPE_南充阆中 Or mint险类 = TYPE_重庆壁山 Then
'        Else
'            If IsNull(rsTemp("顺序号")) = True Then
'                MsgBox "病人 " & msh帐户_S.TextMatrix(.Row, col姓名) & " 登记信息不完整，可能己做了其它登记。", vbInformation, gstrSysName
'                Exit Sub
...
Private Sub SetMenuState()
    ...
    mnuEditRollIncome.Enabled = (mint险类 = TYPE_重庆市 Or mint险类 = TYPE_重庆银海版 _
                             ...
                             Or mint险类 = type_铜梁合医 Or mint险类 = TYPE_铜仁)
    'Modified by 朱玉宝 20031218 地区：福州
