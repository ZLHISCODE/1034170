...
'        需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select A.NO,A.登记时间,A.开单人 as 医生," & _
            ...
            " Where A.收费细目ID=B.ID And A.开单部门ID=C.ID And A.收费细目ID=D.收费细目ID And D.险类=" & TYPE_成都市 & _
            " Order by A.ID"
    ...
    '求个人帐户支付金额
    gstrSQL = "Select 冲预交 From 病人预交记录 Where 结算方式='个人帐户' And 记录性质 Not In (11,1) And 结帐ID=" & lng结帐ID
...
'返回：交易成功返回true；否则，返回false
    ...
    With rsTmp
        ...
        .Open gstrSQL, gcnSybase, adOpenKeyset
        If Trim(.Fields(0).Value) <> "0000" Then
            MsgBox "交易""z010""出现错误""" & !code & """:" & vbCrLf & String(2, "　") & GetErrInfo(!code, TYPE_成都市) & String(2, vbTab), vbInformation, gstrSysName
...
'返回：交易成功返回true；否则，返回false
    ...
    gstrSQL = "Select A.入院日期,A.入院病床,B.名称,D.住院号,SysDate as 经办时间,C.卡号,C.医保号,C.密码 " & _
            ...
            " And A.入院科室ID=B.ID And A.病人ID=C.病人ID And C.险类=" & TYPE_成都市
    Call OpenRecordset(rsTmp, "成都医保")
...
'        (由于门诊收费作废时，已经不再和医保有关系，所以不需要保存结帐的交易号)
    ...
    strSQL = "select Nvl(Sum(实收金额),0) as 未上传金额 from 病人费用记录 " & _
    ...
    If rsTmp!未上传金额 <> 0 Then
       MsgBox "病人还存在未上传费用，请重新预结算后再结帐。", vbInformation, gstrSysName
       住院结算_成都 = False
       Exit Function
    End If
    ...
    If rsTmp.EOF Then
        ...
        Exit Function
    ElseIf Trim(rsTmp!code) <> "0000" Then
        MsgBox "交易""z008""出现错误""" & rsTmp!code & """:" & vbCrLf & String(2, "　") & GetErrInfo(rsTmp!code, TYPE_成都市) & String(2, vbTab) & vbCrLf & _
               "请在处理后重新提取病人信息。", vbInformation, gstrSysName
        Exit Function
    End If
...
'----------------------------------------------------------------
    ...
    If rsTmp.EOF Then
        ...
        Exit Function
    ElseIf Trim(rsTmp!code) <> "0000" Then
        MsgBox "交易""z013""出现错误""" & rsTmp!code & """:" & vbCrLf & String(2, "　") & GetErrInfo(rsTmp!code, TYPE_成都市) & String(2, vbTab), vbInformation, gstrSysName
        Exit Function
    End If
