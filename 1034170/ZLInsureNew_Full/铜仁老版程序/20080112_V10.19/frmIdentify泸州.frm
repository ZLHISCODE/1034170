...
Begin VB.Form frmIdentify泸州 
   ...
   Begin VB.Image Image1 
      Height          =   480
      Left            =   135
      Picture         =   "frmIdentify泸州.frx":030A
      Top             =   405
      Width           =   480
   End
...
Private Sub cmdOK_Click()
    If Trim(txt病种.Text) <> "" And Val(txt病种.Tag) = 0 Then
        Call txt病种_KeyPress(vbKeyReturn)
    End If
    ...
    If IsValid = False Then
        ...
        Exit Sub
    End If
    mlng病种ID = IIf(txt病种.Tag = "", 0, txt病种.Tag)
...
'功能：修改用户IC卡密码
    ...
    If lngReturn <> 0 Then
        '读取失败
        MsgBox 错误信息_泸州(lngReturn), vbInformation, gstrSysName
        Exit Function
    End If
    mIC泸州 = ic
...
'功能：判断IC卡是否合法
    ...
    If TruncZero(mIC泸州.Password) <> "9999" Then
        If mbln远程验证 = False Then
            If TruncZero(mIC泸州.Password) <> txtPwd.Text Then
    ...
    If txt新密码(0).Visible = False Then
        If mint场合 = 1 And txt病种.Tag = "" Then
            ...
            If txt病种.Visible And txt病种.Enabled Then txt病种.SetFocus
            Exit Function
        End If
'        If mint场合 = 0 And txt病种.Tag <> "" Then
'            '检查是否支付该报销
'            gstrSQL = "SELECT A.类别 FROM 保险病种 A " & _
'                      "  WHERE A.险类=80 AND A.编码='" & mstr病种编码 & "' and A.类别>'0'"
...
'        End If
        ...
        gstrSQL = "SELECT B.有效期,B.是否可用,A.运行模式,A.开展慢病报销,A.开展大病报销 " & _
                   ...
                   " WHERE A.险类=" & TYPE_泸州市 & " AND A.编码='" & mIC泸州.CenterCode & "' AND A.主机编码=B.编码 AND A.险类=B.险类 "
        If rsTemp.State = adStateOpen Then rsTemp.Close
        rsTemp.Open gstrSQL, gcn泸州, adOpenStatic, adLockReadOnly
...
'        End If
        ...
        If mIC泸州.InPerAcc - mIC泸州.OutPerAcc < 0 Then
...
Private Sub cmd病种_Click()
    ...
    gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
            " From 保险病种 A where A.险类=" & TYPE_泸州市 & IIf(mstr病种ID = "", " And 1=2", " And A.ID In (" & Replace(mstr病种ID, "|", ",") & ")")
    Call OpenRecordset(rsSelected, "提取上次已选择的病种")
    
    '门诊选择慢特病
    gstrSQL = " Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特种病','普通病') as 类别 " & _
            " From 保险病种 A where A.险类=" & TYPE_泸州市 & " And A.类别 IN (" & IIf(mint场合 = 0, "1,2)", "0)")
    ...
    If rs病种.RecordCount > 0 Then
        If frm多病种选择.ShowSelect(rs病种, "ID", "医保病种选择", "请选择医保病种：", rsSelected, False) = True Then
            ...
            With rs病种
                ...
                End If
                Do While Not .EOF
                    mstr病种ID = mstr病种ID & "|" & rs病种!ID
                    str病种 = str病种 & "," & rs病种!名称
                    .MoveNext
                Loop
                If mstr病种ID <> "" Then
                    mstr病种ID = Mid(mstr病种ID, 2)
                    str病种 = Mid(str病种, 2)
                End If
            End With
...
Public Function GetPatient(ByVal int场合 As Integer, ByVal bln修改密码 As Boolean, lng病种ID As Long, str病种ID As String) As Boolean
    ...
    mblnOK = False
    timRead.Enabled = True
    txtPwd.MaxLength = Len(mIC泸州.Password)
...
Private Function ReadIC泸州(Optional ByVal blnMessage As Boolean = False) As Boolean
'功能：读IC卡上的信息
    Dim lngReturn As Long
    ...
    If chk离休.Value = 0 Then
        ...
        Else
            ...
            If blnUpload = False Then
                If frmSock沪州.CommIC(mstr远程地址, True, IIf(mint场合 = 1, 1, 0), txtPwd.Text & "|" & txt新密码(1).Text) = False Then
                    Exit Function
                End If
                blnUpload = True
                mIC泸州 = gIC泸州Temp
    ...
    Else
        ...
        If Get离休病人_泸州(Trim(txtPwd.Text), mIC泸州, False) = False Then
    ...
    If lngReturn = 0 Then
        '读取成功
        lbl卡号.Caption = "卡号：" & TruncZero(mIC泸州.Cardno)
        lbl姓名.Caption = "姓名：" & TruncZero(mIC泸州.Name)
...
Private Sub txt病种_KeyPress(KeyAscii As Integer)
    ...
    gstrSQL = "Select A.ID,A.编码,A.名称,A.简码,decode(A.类别,1,'慢性病',2,'特殊病','普通病') 类别 " & _
             "   FROM 保险病种 A WHERE A.险类=" & TYPE_泸州市 & " And A.类别 IN (" & IIf(mint场合 = 0, "1,2)", "0)") & " And (" & _
    ...
    If rsTemp.RecordCount > 0 Then
        ...
        If rsTemp.RecordCount > 1 Then
            ...
            blnReturn = frmListSel.ShowSelect(TYPE_泸州市, rsTemp, "ID", "医保病种选择", "请选择特定的医保病种：")
        Else
            blnReturn = True
        End If
    End If
