...
'返回：初始化成功，返回true；否则，返回false
    ...
    '将以前的参数取出来显示在界面中
    gstrSQL = "Select 参数名,Nvl(参数值,0) Value From 保险参数 Where 险类= " & type_成都郊县
...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    gstrSQL = "select nvl(当前状态,0) as 当前状态,顺序号 from 保险帐户 where 医保号='" & str医保号 & "' and 险类=" & type_成都郊县
    Call OpenRecordset(rsTemp, "成都郊县医保")
    ...
    gstrSQL = "Select * From 保险帐户 Where 医保号='" & str医保号 & "' And 险类=" & type_成都郊县
...
'返回: 返回个人帐户余额的金额
    ...
    Else
        ...
        gstrSQL = "Select 帐户余额 From 保险帐户 where 险类=" & type_成都郊县 & " and 中心=0 and 医保号='" & strSelfNo & "'"
        Call OpenRecordset(rsTemp, "成都郊县医保")
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    gstrSQL = "select 医保号,顺序号 as 就诊序号,退休证号 as 中心编号,人员身份 as 人员类别  " & _
              "from 保险帐户 where 病人ID=" & lng病人ID & " and 险类=" & type_成都郊县
    ...
    Do Until rs明细.EOF
        ...
        gstrSQL = "select A.名称,A.编码,A.类别,A.规格,A.计算单位,B.项目编码,B.附注,C.类别 as 大类 " & _
                 ...
                 " where A.类别=C.编码 and  A.ID=" & rs明细("收费细目ID") & " and A.ID=B.收费细目ID and B.险类=" & type_成都郊县
        Call OpenRecordset(rsTemp, "门诊预算")
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = "Select * From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Nvl(记录状态,0)<>0"
...
'    End If
    ...
    '获得医保号
    gstrSQL = "select 医保号,卡号,顺序号 as 就诊序号,退休证号 as 中心编号 from 保险帐户 where 险类=" & type_成都郊县 & " and 病人ID=" & lng病人ID
...
'参数：lng病人ID=是否只上传单据中指定病人的费用
    ...
    Call OpenRecordset(rs收费类别, "成都郊县医保")
    ...
    gstrSQL = _
        ...
        " And D.险类=" & type_成都郊县 & " And E.险类=" & type_成都郊县 & " And F.险类=" & type_成都郊县 & _
    ...
    Do Until rsBill.EOF
        ...
        If lng当前病人 <> rsBill("病人ID") Then
            ...
            gstrSQL = "Select 医保号,顺序号 as 就诊序号,退休证号 as 中心编号,人员身份 as 人员类别  " & _
                      ...
                      "  where B.病人ID=" & lng当前病人 & " and B.主页ID=" & rsBill("主页ID") & " and A.病人ID=B.病人ID and A.险类=" & type_成都郊县 & " and A.年度=to_char(B.入院日期,'yyyy')" & _
...
'    End If
    ...
    gstrSQL = "Select 医保号,顺序号 as 就诊序号,退休证号 as 中心编号,人员身份 as 人员类别  " & _
              ...
              "  From 帐户年度信息 A,病案主页 B,保险帐户 C " & _
              "  where B.病人ID=" & lng病人ID & " and B.主页ID=" & g结算数据.主页ID & " and A.病人ID=B.病人ID and A.险类=" & type_成都郊县 & " and A.年度=to_char(B.入院日期,'yyyy')" & _
              "     and C.病人ID=A.病人ID and C.险类=A.险类"
...
'      3)由于结帐之后，可能使用结帐作废交易，这时需要结帐时执行结算交易的交易号，因此我们需要同时结帐交易号。(由于门诊收费作废时，已经不再和医保有关系，所以不需要保存结帐的交易号)
    ...
    If mint适用地区_成都郊县 = 适用地区.温江区 Then
       ...
       If IsNull(str发票号) Then
          MsgBox "社保局报销要求必须传输发票号，请重新结算并输入发票号！！", vbInformation, gstrSysName
          住院结算_成都郊县 = False
          Exit Function
       End If
    End If
...
'功能：得到医院的医保编码
    ...
    On Error GoTo errHandle
    '获取医院编码
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & type_成都郊县
    Call OpenRecordset(rsTmp, "成都郊县医保")
...
Public Sub 核对帐户支付_成都效县(ByVal lng病人ID As Long)
    ...
    gstrSQL = "Select Sum(A.冲预交) 帐户支付,Count(*) 记录数  " & _
             ...
             "      (Select 病人ID,结帐ID  " & _
             "      From 病人费用记录 " & _
             "      Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID & ") B " & _
...
Public Sub 核对费用结算_成都效县(ByVal lng病人ID As Long)
    ...
    gstrSQL = "SELECT SUM(发生费用金额) 发生费用,SUM(进入统筹金额) 进入统筹,SUM(统筹报销金额) 统筹报销, " & _
             ...
             "      ) A,保险结算记录 B " & _
             " WHERE A.病人ID=B.病人ID AND B.记录ID=A.结帐ID AND B.险类=" & type_成都郊县 & " AND B.性质=2 " & _
             " GROUP BY A.病人ID"
    Call OpenRecordset(rsTemp, "取帐户支付额及记录数")
    int记录数_Client = Nvl(rsTemp!记录数, 0)
