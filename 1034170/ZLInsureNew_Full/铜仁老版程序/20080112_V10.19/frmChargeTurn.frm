...
'返回:转入数据的登记时间
    ...
    On Error GoTo errH
    strSQL = "Select Max(发生时间) 发生时间 From 病人费用记录" & vbNewLine & _
            "Where 记录性质 = 2 And 记录状态 In(1,3) And 病人id = " & lngPatient & " And 主页id Is Null And 标识号 Is Null"

    'His9低版本公共部件不支持绑定变量方式
    'Set rsTmp = zldatabase.OpenSQLRecord(strSQL, "检查是否存在已转单据", lngPatient)
    Call zlDatabase.OpenRecordset(rsTmp, strSQL, "检查是否存在已转单据")
...
'功能:读取并显示病人指定天数内的门诊费用单据
    ...
    strSQL = "Select '√' as 选择,'可转入' as 类别,Decode(B.险类,Null,'','√') as 医保, A.NO As 单据号, A.实际票号 As 票据号, A.开单人," & vbNewLine & _
            ...
            "From 病人费用记录 A,保险结算记录 B" & vbNewLine & _
            "Where A.记录性质 = 1 And A.记录状态 = 1 And A.病人id+0 = " & lngPatient & " And A.发生时间 Between " & strBegin & " And " & strEnd & " And A.结帐ID = B.记录ID(+) And B.性质(+) = 1" & vbNewLine & _
            ...
            "From 病人费用记录 A,保险结算记录 B" & vbNewLine & _
            "Where A.记录性质 = 1 And A.记录状态 = 3 And A.病人id+0 = " & lngPatient & " And A.发生时间 Between " & strBegin & " And " & strEnd & " And A.结帐ID = B.记录ID(+) And B.性质(+) = 1" & vbNewLine & _
            "Group By A.NO, A.实际票号, A.开单人, A.发生时间, A.结帐ID, B.险类" & vbNewLine & _
    ...
    Call zlDatabase.OpenRecordset(rsList, strSQL, "读取单据")
...
'功能:获取一张单据中的医保结算方式串
    ...
    strSQL = "Select A.结算方式 From 病人预交记录 A, 结算方式 B" & vbNewLine & _
            "Where A.结算方式 = B.名称 And B.性质 In (3, 4) And A.NO = '" & strNO & "'"
    'Set rsTmp = zldatabase.OpenSQLRecord(strSQL, Me.Caption, strNO)
    Call zlDatabase.OpenRecordset(rsTmp, strSQL, "读取结算方式")
...
Private Sub ShowDetail(ByVal strNO As String)
    ...
    strSQL = "Select C.名称 As 类别, Nvl(E.名称, B.名称) As 名称, B.规格, A.计算单位 As 单位, Avg(Nvl(A.付数, 1) * A.数次) As 数量," & vbNewLine & _
            "       LTrim(To_Char(Sum(A.标准单价), '999990.00000')) As 单价, LTrim(To_Char(Sum(A.应收金额), '99999" & gstrDec & "')) As 应收金额," & vbNewLine & _
            "       LTrim(To_Char(Sum(A.实收金额), '99999" & gstrDec & "')) As 实收金额, D.名称 As 执行科室" & vbNewLine & _
            "From 病人费用记录 A, 收费项目目录 B, 收费项目类别 C, 部门表 D, 收费项目别名 E" & vbNewLine & _
