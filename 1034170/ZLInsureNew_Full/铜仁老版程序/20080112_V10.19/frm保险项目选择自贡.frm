   ...
   Begin MSComctlLib.ImageList img16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm保险项目选择自贡.frx":0E42
            Key             =   "Detail"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frm保险项目选择自贡.frx":1C94
            Key             =   "Class"
         EndProperty
      EndProperty
   End
...
'返回：成功返回True
    ...
    '取医院编码
    gstrSQL = "Select 医院编码 From 保险类别 Where 序号=" & TYPE_四川自贡
    Call OpenRecordset(rsTemp, "取医院编码")
    mstr医院编码 = Nvl(rsTemp!医院编码)
    If mstr医院编码 = "" Then
        MsgBox "请先设置医院编码！", vbInformation, gstrSysName
        Unload Me
        Exit Function
    End If
    ...
    '显示药品类别
    gstrSQL = "Select lpad(MK_ID,2,'0') AS 编码,MK_NAME AS 名称 From MediKind " & _
             " Order by MK_ID"
    ...
    If rsTemp.EOF = True Then
        MsgBox "医保前置服务器中没有医保大类数据，无法选择。", vbInformation, gstrSysName
        Unload Me
        Exit Function
    End If
    ...
    Do Until rsTemp.EOF
        Set nod = tvwClass.Nodes.Add(, , "K" & rsTemp("编码"), rsTemp("名称"), "Class", "Class")
        nod.Sorted = True
        rsTemp.MoveNext
    Loop
    ...
    frm保险项目选择自贡.Show vbModal, frm保险项目
    '返回值
    If mblnOK = True Then
        strCODE = mstrCode
        strName = mstrName
...
'功能：显示当前类别下的医保明细
    ...
    Dim strCODE As String, blnColSet As Boolean
    Dim lngCol  As Long
    ...
    '如果大类的级别高于医院的级别，则为非医保；否则以项目明细中的是否医保为准
    gstrSQL = "Select SerialNO 医保编码,ItemName 项目名称,SPELLNO 简码,UNIT 单位,A.MEDIKINDCODE 大类编码,DECODE(SIGN(B.SELFMEDIKINDCODE-C.HOSLEVELCODE),1,0,ISMEDICARE) 是否医保, " & _
             "        HIGHLIMIT 最高限价,SELFPAYRATIO 自付比例,CONNOTATION 包含,EXCLUSION 除外,EXPLAINS 说明 " & _
             " From ITEMCENTER A,MEDIKIND B,HOSINFO C" & _
             " Where lpad(B.MK_ID,2,'0')=A.MEDIKINDCODE AND A.MEDIKINDCODE='" & strCODE & "' And C.HOSCODE='" & mstr医院编码 & "'"
