...
Private Sub Form_Load()
    On Error GoTo ErrHand
    '提取HIS项目的编码与名称
    gstrSQL = "Select '['||编码||']'||名称 AS 项目信息 From 收费细目 Where ID=" & mlng收费细目ID
    Call OpenRecordset(mrsTemp, "提取HIS项目的编码与名称")
    Me.txt项目信息.Text = mrsTemp!项目信息
    
    '提取医保对照类别
    gstrSQL = "Select 编码,名称 From 医保对照类别 Where 险类=" & mint险类 & " And Nvl(编码,0)<>0 Order by 编码"
    Call OpenRecordset(mrsTemp, "提取医保对照类别")
    With mrsTemp
        Me.cbo类别.Clear
        Do While Not .EOF
            cbo类别.AddItem !名称
...
Private Sub RefreshData()
    ...
    gstrSQL = "Select A.类别 AS 类别编码,B.名称 AS 类别名称,A.收费细目ID,A.项目编码,C.名称 AS 项目名称,A.说明 " & _
        ...
        " Where A.险类=B.险类 And A.险类=" & mint险类 & " And A.收费细目ID=" & mlng收费细目ID & _
        " And C.险类=A.险类 And C.编码=A.项目编码 And A.类别=B.编码 And B.编码<>0" & _
        " Order by A.类别,A.项目编码"
    Call OpenRecordset(mrsTemp, "提取已完成的对照信息")
    With mrsTemp
...
Private Sub txt医保项目信息_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    gstrSQL = "Select 编码,名称,简码,附注 From 保险项目 " & _
        " Where 险类=" & mint险类 & _
        " And (编码 Like '" & strInput & "%' Or 名称 Like '" & strInput & "%' Or Upper(简码) Like '" & strInput & "%')"
