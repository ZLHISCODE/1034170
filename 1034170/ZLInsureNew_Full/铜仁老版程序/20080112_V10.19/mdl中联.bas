...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    If intInsure = TYPE_自贡市 Then
        ...
        gstrSQL = "select B.参数值 " & _
                ...
                " where A.病人ID=" & g结算数据.病人ID & " and A.险类=" & intInsure & _
    ...
    gstrSQL = "select nvl(sum(A.起付线),0) as 起付线,nvl(sum(A.进入统筹金额),0) as 进入统筹金额 " & _
              ...
              " And A.险类 = " & intInsure & " And A.记录ID = B.ID "
    Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    With g结算数据
        gstrSQL = "select A.中心,A.人员身份,A.在职,A.年龄段," & _
                  ...
                  "     and B.年度(+)=" & .年度 & " and A.病人ID=" & .病人ID & " and A.险类=" & intInsure
        Call OpenRecordset(rsTemp, "虚拟结算")
        ...
        gstrSQL = "select 年龄段,nvl(全额统筹,0) as 全额统筹 ,nvl(无起付线,0) as 无起付线 ,nvl(无封顶线,0) as 无封顶线 " & _
                " from 保险年龄段" & _
                " where 险类=" & intInsure & " and nvl(中心,0)=" & lng中心 & _
    ...
    With g结算数据
        ...
        gstrSQL = "select max(decode(A.性质,'A',A.金额,0)) as 封项线 ,max(decode(A.性质,'1',A.金额,0)) as 起付线 " & _
                  ...
                  "  from 保险支付限额 A " & _
                  "  where A.险类=" & intInsure & " and A.中心=" & lng中心 & " and A.年度=" & .年度
        Call OpenRecordset(rsTemp, "虚拟结算")
    ...
    gstrSQL = "select B.档次,B.下限,B.上限,A.比例 " & _
              ...
              "  Where A.险类 =" & intInsure & " And A.中心 =" & lng中心 & " And A.年度 =" & g结算数据.年度 & " And A.在职 =" & lng在职 & " And A.年龄段 =" & lng年龄段 & _
...
Public Function 住院结算冲销_中联(lng结帐ID As Long, ByVal intInsure As Integer) As Boolean
    ...
    gstrSQL = "select B.住院次数累计,B.帐户增加累计,B.帐户支出累计,B.进入统筹累计,B.统筹报销累计 " & _
              " from 保险帐户 A,帐户年度信息 B " & _
              " where A.病人ID=B.病人ID(+) and A.险类=B.险类(+) and B.年度(+)=" & Year(zlDatabase.Currentdate) & " and A.病人ID=" & rsTemp("病人ID") & " and A.险类=" & intInsure
...
Public Function 获取费用档次额_中联(ByVal lng大类ID As Long, ByVal dbl金额 As Double) As Double
    ...
    gstrSQL = "" & _
        ...
        "              decode(sign(" & dbl金额 & "-nvl(下限,99999999999999.99)),1, " & _
