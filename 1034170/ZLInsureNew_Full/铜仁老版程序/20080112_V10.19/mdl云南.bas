...
Private Declare Sub yh_feebalance_yns Lib "Hisint" Alias "int_feebalance" _
...
Private Declare Sub yh_feebalance_kms Lib "Hisint" Alias "int_feebalance" _
    (ByVal Serial_No As String, ByVal Arranger As String, ByVal Section_Name As String, ByVal SickSortCode As String, ByVal Transaction_No As String, _
    Selfpay As Double, Hookpay As Double, Tcpay As Double, Tcselfpay As Double, Basepay As Double, _
    outpay As Double, Preqpay As Double, Preqselfpay As Double, ActualselfPay As Double, SubsidyPay As Double, _
    OfficialPay As Double, ByVal ryzt As String, ByVal initinstitution As String, tsggzfbf As Double, _
...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    Dim str就诊诊断 As String, str事务控制号 As String, str公务员 As String
    ...
    '---------特殊门诊使用--------
    Dim str诊断编码 As String, str诊断名称 As String
    '-----------------------------
    ...
    If bytType = 0 Then
        ...
        If lng病种ID = 0 Then
            Call yh_outpatientidentify(str卡类型, strArranger, gstr医院编码, strArranger, strSection, strPatiNo, _
                ...
                mstr医保号, str姓名, str性别, str出生日期, str身份证号, str初始化机构, mcur帐户余额, str公务员, mstrErr)
        Else
            Call yh_cardinfo(str卡类型, mcur帐户余额, str卡号, str姓名, str性别, str身份证号, lng年龄, mstrErr)
        End If
    Else
        Call yh_cardinfo(str卡类型, mcur帐户余额, str卡号, str姓名, str性别, str身份证号, lng年龄, mstrErr)
    End If
    ...
    If bytType = 0 Then
        ...
        gstrSQL = "Select Count(病人ID) Records From 保险帐户 Where nvl(当前状态,0)=1 And 医保号='" & mstr医保号 & "' And 险类=" & intInsure
    ...
    End If
    If Not (bytType = 0 And lng病种ID = 0) Then
        '只有普通门诊需要顺序号
        mstr顺序号 = ""
    End If
    ...
    gstrSQL = "update 保险帐户 set 卡号='" & str卡号 & "' where 病人ID in(select 病人ID from 病人信息 where 姓名='" & str姓名 & " ' and 身份证号='" & str身份证号 & "') "
    Call OpenRecordset(rsTemp, "更新卡号")
    gstrSQL = "commit"
    ...
    If bytType = 0 And lng病种ID > 0 Then
        ...
        Call yh_admit(str卡类型, LeftDB(UserInfo.姓名, 8), gstr医院编码, LeftDB(UserInfo.姓名, 8), "门诊", _
            ...
            Format(dat当前, "yyyy-MM-dd"), str诊断名称, str事务号, mstr顺序号, str卡号, _
            mstr医保号, str姓名, str性别, str出生日期, str身份证号, str初始化机构, str单位编码, str单位名称, mstrErr)
...
Public Function 个人余额_云南(strSelfNo As String, ByVal bytPlace As Byte, ByVal intInsure As Integer) As Currency
'功能: 提取参保病人个人帐户余额
'参数: strSelfNO-病人个人编号
...
'返回: 返回个人帐户余额的金额
    ...
    Else
        '读IC卡上的余额
        Call Get卡余额(strSelfNo, 个人余额_云南, intInsure)
    End If
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    Do Until rs明细.EOF
        ...
        If rsTemp.EOF = True Then
            ...
            Exit Function
        End If
        str大类 = Get大类编码(Nvl(rsTemp!项目编码), intInsure)
        If str病种类别 = "1" Then
    ...
    cur余额 = 个人余额_云南(mstr医保号, 10, intInsure)
    If cur特殊人员统筹 > 0 Then
        cur病人自费 = cur特殊人员自付
    Else
...
Public Function 门诊结算_云南(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String, ByVal intInsure As Integer) As Boolean
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
   ...
   gstrSQL = " Select Nvl(病种ID,0) AS 病种ID From 保险帐户 " & _
              " Where 险类=" & intInsure & " And 病人ID=" & _
              "     (Select 病人ID From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Rownum<2)"
    Call OpenRecordset(rsTemp, "判断本次是否为门诊特殊病就诊")
    bln单病种 = (rsTemp!病种ID <> 0)
    ...
    gstrSQL = "Select A.ID,A.病人ID,A.NO,A.登记时间,A.开单人 as 医生," & _
            ...
            "   C.名称 as 科室名称" & _
            " From (Select * From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0) A,收费细目 B,部门表 C,保险支付项目 D " & _
    ...
    If lng病种ID = 0 Then
        ...
        Do Until rs明细.EOF
            ...
            str大类 = Get大类编码(Nvl(rs明细!收费项目), intInsure)
            
            Call DebugTool("明细上传")
            mstrErr = Space(4)
            Call yh_feedetailtrans(mstr顺序号, rs明细("ID"), str大类, rs明细("收费项目"), LeftDB(rs明细("项目名称"), 24), _
                rs明细("数量"), rs明细("实际价格"), str产地, str规格, " ", str医生, str科室, mstr明细事务号, str医生, str发生时间, _
                cur自付比例, cur自付金额, cur报销金额, mstrErr)
...
'        Loop
    ...
    If intInsure = gint昆明市 And bln单病种 Then
        ...
        '检查，目前只允许选择血透单病种1301，没有则为空串，有则只能有1301
        strSicks = Trim(strSicks)
        If InStr(1, "$" & strSicks & "$", "$1301$") = 0 Then
            strSicks = ""
            strSickSel = ""
        Else
            strSicks = "1301"
             If strSicks <> "" Then strSickSel = "1301"
        End If
'        '由于门诊只有一种特殊病，因此不必选择
'        '弹出病种供操作员选择
'        strSickSel = frm昆明市特殊病种选择.ShowSelect(strSicks)
    ...
    mstrErr = Space(4)
    If intInsure = gint昆明市 Then
        Call yh_feebalance_kms(mstr顺序号, str医生, str科室, strSickSel, str结算事务号, _
    ...
    Else
        Call yh_feebalance_yns(mstr顺序号, str医生, str科室, strSickSel, str结算事务号, _
            cur全自付, cur挂钩自付, cur统筹支付, cur统筹自付, cur基数自付, cur超限自付, cur大病统筹, _
            cur大病自付, cur特殊人员自付, cur特殊人员统筹, cur公务员统筹, str人员状态, str初始化机构, cur特殊挂钩支付部分, _
            strFeeBalanceType, dblXTCS, mstrErr)
    End If
    ...
    gstrSQL = "zl_保险结算记录_insert(1," & lng结帐ID & "," & intInsure & "," & lng病人ID & "," & _
        ...
        ",NULL,NULL,'" & strSicks & "|" & strSickSel & "|" & strFeeBalanceType & "|" & dblXTCS & "|" & mstrAverageFeeType & "|" & mstrTsyybz & "')"
    Call zlDatabase.ExecuteProcedure(gstrSQL, "云南医保")
    '------------------------------------------------------------------------------------------------------------------------------------------------------------
    '五、特殊门诊要进行出院信息修改
    If lng病种ID <> 0 Then
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "Select 病人ID,结帐金额  From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Nvl(附加标志,0)<>9 And Nvl(记录状态,0)<>0"
    Call OpenRecordset(rsTemp, "云南医保")
...
'返回：交易成功返回true；否则，返回false
    ...
    '0092-特殊群体门诊和0094-住院都必须设置成普通病
    mstrErr = Space(4)
    If str疾病编码 = "0093" Then    '只有市医保才支持年终结转住院，张建云要求病种编码传为零
        Call yh_kndadmit(LeftDB(UserInfo.姓名, 8), str医保号, gstr医院编码, LeftDB(UserInfo.姓名, 8), LeftDB(rsTemp("入院科室"), 8), _
            LeftDB(lng病人ID, 12), LeftDB(rsTemp("住院号"), 12), IIf(rsTemp("病种类别") <> "0", "1", "0"), "0", _
            Format(rsTemp!入院日期, "yyyy-MM-dd"), LeftDB(获取入出院诊断(lng病人ID, lng主页ID, True, False), 50), str事务号, mstr顺序号, str卡号, _
            str姓名, str性别, str出生日期, str初始化机构, str单位编码, str单位名称, mstrErr)
    Else
        Call yh_admit(str卡类型, LeftDB(UserInfo.姓名, 8), gstr医院编码, LeftDB(UserInfo.姓名, 8), LeftDB(rsTemp("入院科室"), 8), _
            ...
            Format(rsTemp!入院日期, "yyyy-MM-dd"), LeftDB(获取入出院诊断(lng病人ID, lng主页ID, True, False), 50), str事务号, mstr顺序号, str卡号, _
            str医保号, str姓名, str性别, str出生日期, str身份证号, str初始化机构, str单位编码, str单位名称, mstrErr)
    End If
    ...
    '强行把保帐户中的卡号改为最新读出的卡号
    gstrSQL = "update 保险帐户 set 卡号='" & str卡号 & "' where 病人ID=" & lng病人ID & " and 险类=" & intInsure & ""
    Call OpenRecordset(rsTemp, "更改保险帐户中的卡号为最新")
    gstrSQL = "commit"
    Call OpenRecordset(rsTemp, "提交")
...
Public Function 出院登记_云南(lng病人ID As Long, lng主页ID As Long, str顺序号 As String, ByVal intInsure As Integer, Optional ByVal bln结帐出院 As Boolean = False) As Boolean
'功能：将出院信息发送医保前置服务器确认；
'参数：lng病人ID-病人ID；lng主页ID-主页ID
'返回：交易成功返回true；否则，返回false
            '取入院登记验证所返回的顺序号
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) Then
        ...
        mstrErr = Space(4)
        If intInsure = gint昆明市 Then                                                              '有多个单病种时只能选一个传给银海进行结算，所以改为传操作员选择的strSickSel
            Call yh_feebalance_kms(mstr顺序号, LeftDB(UserInfo.姓名, 8), LeftDB(UserInfo.部门, 24), strSickSel, str事务号, cur全自付, _
        ...
        Else
            Call yh_feebalance_yns(mstr顺序号, LeftDB(UserInfo.姓名, 8), LeftDB(UserInfo.部门, 24), "", str事务号, cur全自付, _
                cur挂钩自付, cur统筹支付, cur统筹自付, cur基数自付, cur超限自付, _
                cur大病统筹, cur大病自付, cur特殊人员自付, cur特殊人员统筹, cur公务员统筹, _
                str人员状态, str初始化机构, cur特殊挂钩支付部分, strFeeBalanceType, dblXTCS, mstrErr)
        End If
        ...
        If rsTemp.RecordCount <> 0 Then
            gstrSQL = "zl_保险结算记录_insert(2," & rsTemp!记录ID & "," & rsTemp!险类 & "," & rsTemp!病人ID & "," & _
                ...
                rsTemp!个人帐户支付 & ",'" & rsTemp!支付顺序号 & "'," & rsTemp!主页ID & ",NULL,'" & str备注 & "')"
            Call zlDatabase.ExecuteProcedure(gstrSQL, "云南医保")
        End If
        
        blnTrans = True
        '更新出院诊断（仅市医保支持年终结转，传4）
        mstrErr = Space(4)
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    Dim str发生时间 As String, str本次入院时间 As String, str大类 As String
    Dim rsTemp As New ADODB.Recordset
    ...
    str本次入院时间 = Format(rsTemp!入院时间, "yyyy-MM-dd")
    ...
    Do Until rsExse.EOF
        ...
        str大类 = Get大类编码(Nvl(rsExse!医保项目编码), intInsure)
        mstrErr = Space(4)
        ...
        '如果登记时间小于本次住院时间则不上传
        If Format(rsExse("发生时间"), "yyyy-MM-dd") >= str本次入院时间 Then
            Call DebugTool("上传明细串：" & mstr顺序号 & "," & str数据批号 & "," & str大类 & "," & rsExse("医保项目编码") & "," & _
                rsExse("收费名称") & "," & dbl数量 & "," & dbl金额 & "," & str产地 & "," & str规格 & "," & "" & "," & str医生 & "," & str科室 & "," & str事务号 & "," & str医生 & "," & str发生时间 & "," & _
    ...
    cur个人帐户 = 个人余额_云南(mstr医保号, 40, intInsure)
    If cur特殊人员统筹 > 0 Then
        cur自付总额 = cur特殊人员自付
    Else
...
'      3)由于结帐之后，可能使用结帐作废交易，这时需要结帐时执行结算交易的交易号，因此我们需要同时结帐交易号。(由于门诊收费作废时，已经不再和医保有关系，所以不需要保存结帐的交易号)
    ...
    Dim str卡号 As String, str姓名 As String, str性别 As String, str身份证号 As String, lng年龄 As Double
    ...
    If CDbl(cur个人帐户) <> 0 Then
        ...
        If mstrErr <> "0000" Then
            MsgBox GetErrInfo(mstrErr, intInsure) & "（如果下卡失败,请补缴现金￥" & Format(cur个人帐户, "#####0.00") & "）", vbInformation, gstrSysName
            cur个人帐户 = 0
        End If
    End If
    ...
    If Not 存在未结费用(lng病人ID, lng主页ID) And 医保病人已经出院(lng病人ID) Then
        ...
        If rsTemp.EOF Then
            MsgBox "没有此病人或此病人不是医保病人，无法办理出院手续！（请在医保帐户中办理补充出院手续）", vbExclamation, gstrSysName
            Exit Function
        End If
        If IsNull(rsTemp!顺序号) Then
            MsgBox "未发现该病人的住院交易顺序号,无法办理出院手续！（请在医保帐户中办理补充出院手续）", vbInformation, gstrSysName
            Exit Function
        End If
...
Public Function 住院结算冲销_云南(lng结帐ID As Long, ByVal intInsure As Integer) As Boolean
    ...
    mstrErr = Space(4)
    Call yh_recedefeebalance(str顺序号, "0", lng结帐ID, String(18, "1"), mstrErr) '目前都是用预结算在处理
       mstrErr = Trim(mstrErr)
    If mstrErr <> "0000" Then
        MsgBox GetErrInfo(mstrErr, intInsure), vbInformation, gstrSysName
        Exit Function
    ...
    If CDbl(cur个人帐户) <> 0 Then
        ...
        Call yh_recedefeebalance(str顺序号, "4", lng结帐ID, String(18, "1"), mstrErr) '目前都是用预结算在处理
           mstrErr = Trim(mstrErr)
        If mstrErr <> "0000" Then
            MsgBox GetErrInfo(mstrErr, intInsure) & "（回退个人帐户失败,请退还医保病人现金：￥" & Format(cur个人帐户, "#####0.00") & "现金）", vbInformation, gstrSysName
            cur个人帐户 = 0
        End If
    End If
...
'功能：撤消急诊登记
    ...
    mstrErr = Space(4)
    Call yh_feebalance_yns(mstr顺序号, str医生, str科室, strSickSel, str事务号, _
            cur全自付, cur挂钩自付, cur统筹支付, cur统筹自付, cur基数自付, cur超限自付, cur大病统筹, _
            cur大病自付, cur特殊人员自付, cur特殊人员统筹, cur公务员统筹, str人员状态, str初始化机构, cur特殊挂钩支付部分, _
            strFeeBalanceType, dblXTCS, mstrErr)
       mstrErr = Trim(mstrErr)
...
Public Function Get大类编码(ByVal str项目编码 As String, ByVal intInsure As Integer) As String
    ...
    '获取某个医保项目的大类编码
    gstrSQL = "Select 大类编码 From 保险项目 Where 险类=" & intInsure & " And 编码='" & str项目编码 & "'"
