...
Private Sub cmd确定_Click()
  ...
  ' txt疾病信息.Tag = 124
    ...
    If mbytType <> 2 Then
        '检查病人状态
        gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & TYPE_慈溪农医 & " and 医保号='" & txt医疗证号 & "'"
...
Private Sub Form_Load()
    ...
    '取IC端口号
    gstrSQL = "Select 参数值 From 保险参数 Where 险类=" & TYPE_慈溪农医 & " ANd 参数名='IC端口号'"
    Call OpenRecordset(rsTemp, "取IC端口号")
    If rsTemp.RecordCount = 0 Then
        IntPort = 1
    Else
