...
'返回：
    ...
    cmb中心.Clear
    gstrSQL = "select A.具有中心,B.序号,B.编码,B.名称 from 保险类别 A,保险中心目录 B where A.序号=" & TYPE_四川眉山 & " and A.序号=b.险类"
    Call OpenRecordset(rsTemp, Me.Caption)
    ...
    '1-在职;2-退休;3-离休
    gstrSQL = "Select 序号,名称 From 保险人群 Where 险类=" & TYPE_四川眉山
    Call OpenRecordset(rsTemp, Me.Caption)
    Cbo当前状态.Clear
...
Private Sub cmb中心_Click()
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & TYPE_四川眉山 & " and 中心=" & cmb中心.ItemData(cmb中心.ListIndex)
    Call OpenRecordset(rsTemp, Me.Caption)
...
Private Sub cmdOK_Click()
    ...
    '检查病人状态
    gstrSQL = "select nvl(当前状态,0) as 状态,灰度级,备注 from 保险帐户 where 险类=" & TYPE_四川眉山 & " and 中心=" & lng中心 & " and 医保号='" & Trim(txt卡号.Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
    ...
    gstrSQL = " Select Nvl(进入统筹累计,0) 统筹累计 From 帐户年度信息 " & _
              ...
              "     (Select 病人ID From 保险帐户 where 险类=" & TYPE_四川眉山 & " and 中心=" & lng中心 & " and 医保号='" & Trim(txt卡号.Text) & "')"
...
'从已经存在的记录中读出帐户信息
    ...
    If Not rs帐户 Is Nothing Then
        ...
        If mbytType = 2 Or mbytType = 4 Then
            gstrSQL = "Select 密码 From 保险帐户 Where 险类=" & TYPE_四川眉山 & " And 医保号='" & txt卡号 & "'"
            Call OpenRecordset(rsTemp, Me.Caption)
...
'功能：检查数据的正确性
    ...
    '校验密码正确性
    gstrSQL = "Select 密码 From 保险帐户 Where 险类=" & TYPE_四川眉山 & " And 医保号='" & Trim(txt卡号.Text) & "'"
    Call OpenRecordset(rsTemp, Me.Caption)
    If Not rsTemp.EOF Then
...
'返回：病人ID
    ...
    If Len(Trim(strInfo)) <> 0 Then
        ...
        If lng病人ID > 0 Then
            '该病人已经存在
            gstrSQL = "Select nvl(病人ID,0) 病人ID from 保险帐户 where 医保号='" & CStr(array信息(1)) & "' and 险类=" & TYPE_四川眉山
