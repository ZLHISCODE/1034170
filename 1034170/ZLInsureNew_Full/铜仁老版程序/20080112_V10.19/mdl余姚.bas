...
Public Function makeICInfo(lng病人ID As Long) As String
    ...
    gstrSQL = "Select A.卡号,B.姓名,B.性别,A.单位编码 From 保险帐户 A,病人信息 B Where A.病人ID=" & lng病人ID & _
        " And A.险类=" & TYPE_余姚 & " And A.病人ID=B.病人ID"
...
'    Call OpenRecordset(rsTemp, gstrSysName)
    ...
    While Not rs费用明细.EOF
        ...
        gstrSQL = "select * from 药品目录 where 药品ID=" & rs费用明细!收费细目ID
        Call OpenRecordset(rsTemp, gstrSysName)
        ...
        '从保险支付项目中查找是否有该医保项目
        gstrSQL = "Select 项目编码,项目名称 From 保险支付项目 Where 是否医保=1 And 险类=" & TYPE_余姚 & " And 收费细目ID=" & rs费用明细!收费细目ID
        Call OpenRecordset(rsTemp, gstrSysName)
...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    On Error GoTo errHandle
    datCurr = zlDatabase.Currentdate
    gstrSQL = "Select * From 病人费用记录 Where 门诊标志=1 and 结帐ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
    lng病人ID = rsTemp!病人ID
    While Not rsTemp.EOF
        ...
        ''''cd 2005 0301
        '卫材在药品中，但类型为诊疗，所以不能直接按rstemp!类别来判断
        gstrSQL = "select * from 药品目录 where 药品ID=" & rsTemp!收费细目ID
        Call OpenRecordset(rs记录, gstrSysName)
        ...
        '从保险支付项目中查找是否有该医保项目
        gstrSQL = "Select 项目编码,项目名称 From 保险支付项目 Where 是否医保=1 And 险类=" & TYPE_余姚 & " And 收费细目ID=" & rsTemp!收费细目ID
        Call OpenRecordset(rs记录, gstrSysName)
    ...
    gstrSQL = "select 编码,名称 from 部门表 where ID=(Select 开单部门ID From 病人费用记录 Where 门诊标志=1 and 序号=1 and 结帐ID=" & lng结帐ID & ")"
    Call OpenRecordset(rsTemp, gstrSysName)
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    '取原单据交易流水号
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_余姚 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
Public Function 记帐传输_余姚(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人ID As Long = 0) As Boolean
    ...
    While Not rs明细.EOF
        ...
        '^^^^^^^^^^^^^^^
        '从保险支付项目中查找是否有该医保项目
        gstrSQL = "Select 项目编码,项目名称 From 保险支付项目 Where 是否医保=1 And 险类=" & TYPE_余姚 & " And 收费细目ID=" & rs明细!收费细目ID
        Call OpenRecordset(rsTemp, gstrSysName)
        If rsTemp.EOF Then      '没有项目处理
            If str项目类型 = "药品" Then    '类型为药品时，医保类型为“丙类”
                str医保类型 = "丙类": str自付比例 = "1"
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_余姚 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
