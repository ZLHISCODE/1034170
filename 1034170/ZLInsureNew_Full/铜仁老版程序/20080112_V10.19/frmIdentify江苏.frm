...
Private Sub cmdComp_Click()
    ...
    ElseIf mbytType = 0 Then                    '无刷卡器读卡时可以这样操作，如果改用读卡方式就需要进行修改
        gstrSQL = "Select * From 保险帐户 where 险类=" & TYPE_江苏 & " And 医保号='" & strVoucherID & "'"
...
'        psOthers1       :pChar;     13     // O其它(保留)[C200]
    ...
    If mbytType = 3 Or mbytType = 0 Then
        ...
        '取病人ID
        gstrSQL = "select 病人ID from 保险帐户 where 险类=" & TYPE_江苏 & " and 卡号='" & Substr((strArrInfo(0)), 1, 11) & "'"
        Call OpenRecordset(rsTemp, "取病人id")
        If rsTemp.RecordCount <> 0 Then
            lng病人ID = rsTemp!病人ID
            gstrSQL = " Select MAX(结帐ID) AS 结帐ID From 病人费用记录" & _
                      " Where 记录性质=1 and 收费类别 in('5','6','7') And 记录状态=1 And 病人ID=" & lng病人ID & _
            ...
            Call OpenRecordset(rsTemp, "取有效期内最后一次有效的挂号数据")
