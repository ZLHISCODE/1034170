            ...
            If Nvl(!是否医保, 0) <> 0 And Nvl(!实收金额, 0) <> 0 Then
               ...
               '         "Where B.ID=" & !收费细目ID & " And A.收费细目ID=B.ID And A.险类=" & TYPE_宁海
                ...
                gstrSQL = "Select C.名称 as 大类,B.名称,B.规格,A.项目编码 AS 医保编码 " & _
                        ...
                        "Where B.ID=" & !收费细目ID & " And A.收费细目ID=B.ID  And A.大类ID=C.ID And A.险类=" & TYPE_宁海
...
Public Function 门诊结算_宁海(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    '取余额
    gstrSQL = "Select 往年帐户余额,本年帐户余额 From 保险帐户 Where 险类=" & TYPE_宁海 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取余额")
...
Public Function 住院虚拟结算_宁海(rsExse As Recordset, ByVal lng病人ID As Long) As String
    ...
    '先提取该病人的住院流水号
    gstrSQL = "Select A.业务类型,A.医保号,A.顺序号,A.IC,A.病种ID,B.住院次数 主页ID From 保险帐户 A,病人信息 B Where A.病人ID=B.病人ID And A.病人ID=" & lng病人ID
    ...
    gstrSQL = "Select D.名称 as 大类,A.ID,A.NO,A.病人ID,A.收费类别,A.记录性质,A.记录状态,A.序号,A.收费细目ID,C.项目编码 AS 医保项目编码,B.编码,B.名称,A.实收金额 AS 金额" & _
              "         ,A.数次*nvl(A.付数,1) as 数量,Decode(A.数次*nvl(A.付数,1),0,0,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4)) as 价格,A.开单人 AS 医生,A.登记时间 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C,保险支付大类 D " & _
              "  where A.病人ID=" & lng病人ID & " and A.主页ID=" & lng主页ID & " and A.记帐费用=1 And A.操作员姓名 is not null AND Nvl(A.实收金额,0)<>0 " & _
              "        and nvl(A.是否上传,0)=0 And C.大类ID=D.ID And Nvl(A.记录状态,0)<>0 and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & TYPE_宁海 & _
              "  Order by A.病人ID,A.发生时间"
...
Public Function IS无卡病人(ByVal lng病人ID As Long) As String
    ...
    gstrSQL = "Select Nvl(灰度级,0) AS 卡标志,医保号 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_宁海
    Call OpenRecordset(rsTemp, "是否无卡病人")
