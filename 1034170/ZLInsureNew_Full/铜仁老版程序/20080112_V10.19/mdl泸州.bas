...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    If frmIdentify泸州.GetPatient(bytType, False, lng病种ID, str病种ID) = True Then
        ...
        With gIC泸州
            ...
            '得到中心序号
            gstrSQL = "select 序号 from 保险中心目录 where 险类=" & TYPE_泸州市 & " and 编码='" & .CenterCode & "'"
            ...
            '得到原住院病种
            If bytType <> 1 Then
                gstrSQL = "Select Nvl(病种ID,0) 病种ID From 保险帐户 Where 险类=" & TYPE_泸州市 & " And 医保号='" & TrimStr(.MediAccountNo) & "'"
...
'      2)理论上，由于我们保证了个人帐户结算金额不大于个人帐户余额，因此交易必然成功。但从安全角度考虑；当辅助结算交易失败时，需要使用费用删除交易处理；如果辅助结算交易成功，但费用分割结果与我们处理结果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    '取中心序号
    gstrSQL = "Select 序号 From 保险中心目录 Where 编码='" & ic门诊.CenterCode & "'"
    Call OpenRecordset(rsTemp, "取中心序号")
    m泸州.中心序号 = rsTemp!序号
...
'注意：1)该函数主要使用模拟结算交易，查询结果返回获取基金报销额；
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.记录性质,A.记录状态,A.序号,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              ...
              "  And A.结帐ID Is NULL And nvl(A.是否上传,0)=0 And Nvl(A.记录状态,0)<>0 and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & TYPE_泸州市 & _
    ...
    gstrSQL = "select A.病种ID,A.医保年,B.编码,B.序号 " & _
            ...
            " where A.病人ID=" & m泸州.病人ID & " and A.险类=" & TYPE_泸州市 & _
...
'----------------------------------------------------------------
    ...
    '取中心序号
    gstrSQL = "Select 序号 From 保险中心目录 Where 编码='" & ic住院.CenterCode & "'"
    Call OpenRecordset(rsTemp, "取中心序号")
    m泸州.中心序号 = rsTemp!序号
...
'返回:
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.收费类别,A.记录状态,A.收费细目ID,C.项目编码,B.编码,B.名称,A.实收金额 " & _
              "         ,A.数次*nvl(A.付数,1) as 数量,Decode(A.数次*nvl(A.付数,1),0,0,Round(A.实收金额/(A.数次*nvl(A.付数,1)),4)) as 单价 " & _
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C,病案主页 D,病人信息 E " & _
              "  where A.NO='" & str单据号 & "' and A.记录性质=" & int性质 & " and A.记录状态=1 And Nvl(A.是否上传,0)=0 " & _
              "        and A.收费细目ID=B.ID and A.收费细目ID=C.收费细目ID and C.险类= " & TYPE_泸州市 & _
              "        and A.病人ID=D.病人ID And A.病人ID=E.病人ID And D.主页ID=E.住院次数 and D.险类=" & TYPE_泸州市 & _
              "  Order by A.病人ID,A.发生时间"
...
'功能：判断该病人的卡是否插入正确
    ...
    gstrSQL = "select A.卡号,A.医保号,B.编码 from 保险帐户 A,保险中心目录 B " & _
              " where A.险类=" & TYPE_泸州市 & " and A.病人ID=" & lng病人ID & " and a.险类=B.险类 and A.中心=B.序号"
    Call OpenRecordset(rsTemp, "泸州医保")
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select A.在职 from 保险帐户 A where A.险类=" & TYPE_泸州市 & " and A.病人ID=" & 病人ID
    Call OpenRecordset(rsTemp, "泸州医保")
...
'参数：返回病人的医保号
    ...
    gstrSQL = "select A.医保号,A.密码,B.身份证号 from 保险帐户 A,病人信息 B where A.险类=" & _
        TYPE_泸州市 & " and A.病人ID=" & 病人ID & " And A.病人ID=B.病人ID"
    Call OpenRecordset(rsTemp, "泸州医保")
            ...
            Else
                ...
                If bln门诊 = False Then
                    If rs费用明细!记录状态 = 2 Then
                        ...
                        rsGet.Open "Select 保险大类ID,保险项目否,保险编码,统筹金额,nvl(限价,0) AS 限价 From 病人费用记录 " & _
                            " Where (NO,记录性质,记录状态,序号) IN " & _
                            "     (Select NO,记录性质,3,序号 From 病人费用记录 Where ID=" & rs费用明细!ID & ") And (Nvl(是否上传,0)=1 or nvl(结帐id,0)>0)", gcnOracle
...
'                        End If
    ...
    With m泸州
        gstrSQL = "select A.人员身份,A.在职,A.年龄段," & _
                  ...
                  "     and B.年度(+)=" & .年度 & " and A.病人ID=" & .病人ID & " and A.险类=" & TYPE_泸州市
        Call OpenRecordset(rsTemp, "虚拟结算")
...
Public Function 身份标识_泸州2(ByVal strCard As String, ByVal strPass As String, Optional lng病人ID As Long) As String
    ...
    '读取医保病人的ID
    gstrSQL = "Select 病人ID From 保险帐户 Where 险类=" & TYPE_泸州市 & " And 卡号='" & mIC泸州.Cardno & "'"
