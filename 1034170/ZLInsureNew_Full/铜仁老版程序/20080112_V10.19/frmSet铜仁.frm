...
Private Sub cmdTest_Click()
    If OraDataOpen(gcn铜仁, TxtEdit(Text医保服务器).Text, TxtEdit(text医保用户).Text, TxtEdit(Text医保密码).Tag) = False Then
        Exit Sub
    End If
...
Private Function SaveData() As Boolean
    ...
    If gcn铜仁.State = adStateClosed Then
        If OraDataOpen(gcn铜仁, TxtEdit(Text医保服务器).Text, TxtEdit(text医保用户).Text, TxtEdit(Text医保密码).Tag, False) = False Then
...
Private Sub txtEdit_Change(Index As Integer)
    ...
    If Index = Text医保服务器 Or Index = Text医保密码 Or Index = text医保用户 Then
        ...
        If gcn铜仁.State = adStateOpen Then gcn铜仁.Close
    End If
End Sub
...
'功能：设置与东大阿尔派的医保接口
    ...
    gstrSQL = "select 参数名,参数值 from 保险参数 " & _
              " where 险类=" & mlng险类 & " and (中心 is null or 中心=" & lng中心 & ")"
