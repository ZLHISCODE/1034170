...
Public Function 医保初始化_广元() As Boolean
    ...
    gstrSQL = "Select 参数名,参数值 From 保险参数 Where 险类=" & TYPE_广元
...
CheckCard:
    ...
    If bln处方上传 Then
        ...
        gstrSQL = "Select Nvl(参数值,1) AS 参数值 From 保险参数 Where 险类=" & TYPE_广元 & " And 参数名='实时上传'"
        Call OpenRecordset(rsTemp, "是否实时上传")
...
'    gstr医保机构编码 = rsTemp!中心
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_广元 & " and 记录ID=" & lng结帐ID: lngErr = 4
    Call OpenRecordset(rsTemp, gstrSysName)
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_广元 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
CheckCard:
    ...
    Call OpenRecordset(rsTemp, "取病人出院日期")
...
Public Function 记帐传输_广元(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人ID As Long = 0) As Boolean
    ...
    If str单据号 <> "" Then
        gstrSQL = " Select A.* From 病人费用记录 A,保险帐户 B" & _
                  ...
                  " and A.病人ID=B.病人ID And B.险类=" & TYPE_广元 & _
                  " order by A.主页ID,A.序号"
    Else
        '提取该病人本次住院的主页ID
        gstrSQL = "Select 住院次数 From 病人信息 Where 病人ID=" & lng病人ID
        Call OpenRecordset(rsTemp, "提取该病人本次住院的主页ID")
