...
Private Sub Form_Load()
    ...
    gstrSQL = "select 序号,编码,名称 from 保险中心目录 where 序号<>0 and 险类=" & mint险类 & " order by 序号"
...
Private Sub mnuCX_Click()
    ...
    gstrSQL = "select A.ID,E.项目编码 as 编码,F.名称 as 类别," & _
              ...
              "A.ID=E.收费细目ID And E.险类=" & mint险类 & " And E.中心=" & mint中心 & _
    ...
    Do While Not rsTmp.EOF
       If rsTmp!启用标志 = 0 And intID <> rsTmp!ID Then
            ...
            gstrSQL = "select nvl(ID,0) as ID from 保险支付大类 where 险类=" & mint险类 & " And 名称='" & strTmpArr(2) & "'"
            Call OpenRecordset(rsTmp1, "查询支付大类", gstrSQL)
            int大类id = rsTmp1!ID
...
Private Sub mnuSB_Click()
    ...
    gstrSQL = "select A.ID,A.编码,decode(A.类别,'J','服务','1','服务','5','药品','6','药品','7','药品','诊疗') as 类别," & _
              ...
              "A.ID=E.收费细目ID(+) And E.险类(+)=" & mint险类 & " And E.中心(+)=" & mint中心
    Call OpenRecordset(rsTmp, "保险项目选择")
    ...
    Do While Not rsTmp.EOF
       If IsNull(rsTmp!项目编码) And intID <> rsTmp!ID Then
            ...
            gstrSQL = "select nvl(ID,0) as ID from 保险支付大类 where 险类=" & mint险类 & " And 名称='" & rsTmp!类别 & "'"
            Call OpenRecordset(rsTmp1, "查询支付大类", gstrSQL)
            int大类id = rsTmp1!ID
                    ...
                    Else
                        ...
                        If mint险类 = TYPE_成都德阳 Then
                            gstrSQL = "Select 编码  医保编码,名称,简码,附注 " & _
                                         "   FROM 医保收费项目_德阳 WHERE 险类=" & mint险类 & " and 中心=" & mint中心 & " and (编码 like '" & strText & "%' or Upper(名称) like '" & str前 & UCase(strText) & "%' Or Upper(简码) like '" & str前 & UCase(strText) & "%')"
                        Else
                            gstrSQL = "Select 编码  医保编码,名称,简码,附注 " & _
                                         "   FROM 医保收费项目 WHERE 险类=" & mint险类 & " and 中心=" & mint中心 & " and (编码 like '" & strText & "%' or Upper(名称) like '" & str前 & UCase(strText) & "%' Or Upper(简码) like '" & str前 & UCase(strText) & "%')"
                        End If
                        Call OpenRecordset(rsTemp, Me.Caption)
...
'功能：根据当前行的保险项目编码，得到其它信息
    ...
    With mshSum_S
        If mint险类 = TYPE_成都德阳 Then
            gstrSQL = "select 名称,大类编码,附注 from 医保收费项目_德阳 where 编码='" & .TextMatrix(.Row, COL医保编码) & _
                      "' and 险类=" & mint险类 & " and 中心=" & cmb险类.ItemData(cmb险类.ListIndex)
        Else
            gstrSQL = "select 名称,大类编码,附注 from 医保收费项目 where 编码='" & .TextMatrix(.Row, COL医保编码) & _
                      "' and 险类=" & mint险类 & " and 中心=" & cmb险类.ItemData(cmb险类.ListIndex)
        End If
        Call OpenRecordset(rsTemp, Me.Caption)
...
'功能:装入各种统计数据
    ...
    gstrSQL = "select A.ID,A.编码,A.名称,A.规格,A.剂型,A.产地,A.单位,A.是否变价,D.价格,'' as 改变方式" & _
               ...
               " Where A.ID=B.收费细目ID(+) and B.大类ID=c.id(+)   and b.险类(+)=" & mint险类 & " and  B.中心(+)= " & mint中心 & _
               "       and A.ID=D.收费细目ID(+)  "
...
Public Sub ShowForm(frmParent As Form, ByVal int险类 As Integer)
    ...
    gstrSQL = "select 序号,名称 from 保险中心目录 where 序号<>0 and 险类=" & mint险类 & " order by 序号"
