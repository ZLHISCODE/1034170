...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_重庆壁山 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "壁山医保")
...
'lng病人ID  默认为0，表示传输整张单据，否则为单据中指定病人的。（主要是因为医嘱在保存记帐单时，是分病人在提交数据而不是一起提交）
    ...
    Else
        gstrSQL = "select A.ID,A.发生时间,A.序号,A.NO,A.收费类别,A.开单人,A.登记时间," & _
                ...
                " and A.病人ID=E.病人ID And E.险类=" & TYPE_重庆壁山 & " And D.险类 = " & TYPE_重庆壁山 & _
                " and A.病人ID = " & lng病人ID
    End If
    Call OpenRecordset(rsTmp, "壁山医保")
    ...
    Do While Not rs费用明细.EOF
        ...
        If IIf(IsNull(rs费用明细("是否上传")), "0", rs费用明细("是否上传")) = "0" Then
            gstrSQL = "select A.ID,A.发生时间,A.序号,A.收费类别,A.NO,A.开单人,A.登记时间," & _
                    ...
                    "A.收费细目ID = D.收费细目ID and D.险类 = " & TYPE_重庆壁山
            Call OpenRecordset(rsTmp, "壁山医保")
            If Not rsTmp.BOF Then
                '判断该笔费用是否已经上传
                blnInsert = True
...
'将病人的费用进行结算，由于壁山医保不需要进行出院登记，因此不进行出院登记
    ...
    Call OpenRecordset(rsTmp, "获取本次个人帐户支付额")
    cur个人帐户支付 = 0
...
Public Function 住院结算冲销_壁山(lng结帐ID As Long) As Boolean
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_重庆壁山 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "结算冲销")
    If rsTemp.EOF = True Then
        MsgBox "原单据的医保记录不存在，不能作废。", vbInformation, gstrSysName
        Exit Function
...
Private Function IS出院(ByVal lng病人ID As Long) As Boolean
    ...
    Call OpenRecordset(rsTemp, "判断病人是否已经出院")
