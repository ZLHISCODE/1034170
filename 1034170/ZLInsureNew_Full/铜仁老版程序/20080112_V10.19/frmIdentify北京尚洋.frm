...
Private Sub txtNo_KeyPress(KeyAscii As Integer)
    ...
    If txtNO <> "" Then
        ...
        If mbytType = 1 Then
            Set rsTemp = gcn尚洋.Execute("Select * From SICK_VISIT_INFO Where PERSONAL_NUMBER='" & txtNO.Text & "' And HOSPITAL_NUMBER='" & gstr医院编码 & "'")
        Else
            Set rsTemp = gcn尚洋.Execute("Select * From MED_RECEIPT_RECORD_MASTER Where PERSONAL_NUMBER='" & txtNO.Text & "' And HOSPITAL_NUMBER='" & gstr医院编码 & "'")
        End If
        If rsTemp.EOF Then
            Text1.Enabled = True
            Combo1.Enabled = True
        ...
        Else
            ...
            Text1.Text = rsTemp!Name                                    '姓名
            Text1.Tag = rsTemp!UNIT_NUMBER                              '单位编码
            Combo1.ListIndex = IIf(Trim(Nvl(rsTemp!Sex, "0")) = "1", 0, 1)
            If mbytType = 1 Then
                Call DebugTool("出生日期:" & rsTemp!BIRTHMONTH)
                txtNO.Tag = Mid(rsTemp!BIRTHMONTH, 1, 4) & "-" & Mid(rsTemp!BIRTHMONTH, 5, 2) & "-01"     '出生日期
            Else
                txtNO.Tag = Format(rsTemp!BIRTH_DATE, "yyyy-MM-dd")         '出生日期
            End If
            Combo1.Tag = rsTemp!PERSONAL_TYPE                           '人员类别
            cmdOK.SetFocus
        End If
    End If
End Sub
