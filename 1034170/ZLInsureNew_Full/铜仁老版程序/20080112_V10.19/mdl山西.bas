...
Public Function 身份标识_山西(Optional bytType As Byte, Optional lng病人ID As Long) As String
    ...
    If strIdeReturn = "-1" Then
        身份标识_山西 = ""
    Else
        mstrSQL = "Select * from 保险帐户 where 病人ID=" & lng病人ID & " and 险类=" & TYPE_山西
        Call OpenRecordset(mrsTMP, "病人信息", mstrSQL)
        ...
        mstrSQL = "select * from 保险病种 where ID=(" & _
                        "Select 病种ID from 保险帐户 where 病人ID=" & lng病人ID & _
                                                     " and 险类=" & TYPE_山西 & ") " & _
                                           " and 险类=" & TYPE_山西
        Call OpenRecordset(mrsTMP, "病种信息", mstrSQL)
...
Public Function 个人余额_山西(strSelfNo As String) As Currency
    ...
    gstrSQL = "Select Nvl(帐户余额,0) AS 个人帐户 From 保险帐户 " & _
              " Where 医保号='" & strSelfNo & "' and 险类=" & TYPE_山西
...
mstrSQL = "Select * from 保险帐户 where 病人ID=" & rs明细记录!病人ID & " and 险类=" & TYPE_山西
Call OpenRecordset(mrsTMP, "病人信息", mstrSQL)
...
Do Until rs明细记录.EOF
    '未对码的情况
    mstrSQL = "select * from 保险支付项目 where 险类=" & TYPE_山西 & " and 收费细目ID=" & rs明细记录!收费细目ID
    Call OpenRecordset(mrsTMP, "医保明细", mstrSQL)
    If mrsTMP.EOF Then
       ' Call 回退_山西
        mstrSQL = "Select * from 收费项目目录 where ID=" & rs明细记录!收费细目ID
        Call OpenRecordset(rsTmpcd, "收费项目目录", mstrSQL)
    ...
    Call OpenRecordset(mrsTMP, "医保明细", mstrSQL, gcnSxDr)
    If mrsTMP.EOF Then
...
Call OpenRecordset(rs结算明细, "门诊结算明细", mstrSQL)
lng病人ID = rs结算明细!病人ID
...
If 门诊虚拟结算_山西(rs结算明细, str预算信息, 9) Then
    ...
    If mlngReturn = -1 Then
        Call 回退_山西
        门诊结算_山西 = False
        MsgBox Trim(mstrOutput), vbInformation, gstrSysName
    Else
...
Public Function 门诊结算冲销_山西(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    strNO = rsTemp!NO
    '取结算流水号
    gstrSQL = "Select * From 保险结算记录 Where 性质=1 And 记录ID=" & lng结帐ID & " and 险类=" & TYPE_山西
    ...
    If str门诊号 = "" Or str医疗类别 = "" Then
        MsgBox "到原始结算记录交易号不全，无法进行门诊结算冲销！", vbInformation, gstrSysName
        门诊结算冲销_山西 = False
        Exit Function
    End If
...
mstrSQL = "select * from 保险帐户 where 卡号='" & g个人基本信息.卡号05 & "' and 险类=" & TYPE_山西
Call OpenRecordset(mrsTMP, "保险帐户", mstrSQL)
...
mstrSQL = "select * from 保险病种 where ID=" & lng病种ID & " and 险类=" & TYPE_山西
Call OpenRecordset(mrsTMP, "保险病种", mstrSQL)
...
Call OpenRecordset(mrsTMP, "保险病人", mstrSQL)
...
'>beging 提取病种
mstrSQL = "select * from 保险病种 where ID=(" & _
                "Select 病种ID from 保险帐户 where 病人ID=" & lngPatiID & _
                                             " and 险类=" & TYPE_山西 & ") " & _
                                   " and 险类=" & TYPE_山西
Call OpenRecordset(mrsTMP, "病种信息", mstrSQL)
...
Public Function 撤销入院登记_山西(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
    ...
    Call OpenRecordset(rsTemp, "发生费用不允许撤销入院")
    If rsTemp.RecordCount <> 0 Then
        MsgBox "该病人已发生费用，不允许撤销入院登记！", vbInformation, gstrSysName
        Exit Function
    End If
    ...
    '取保险病人相关信息
    gstrSQL = "Select * From 保险帐户 Where 险类=" & TYPE_山西 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取保险病人相关信息")
    str卡号 = rsTemp!卡号
    str密码 = Nvl(rsTemp!密码)
...
'    入院登记前必须首先进行真实读卡，然后调用待遇次查检测函数，
    ...
    If Not 存在未结费用(lngPatiID, lngPageID) Then
        '是否已结帐，已结帐病人不允许撤销入院
        bln结帐 = False
        gstrSQL = "Select 1 From 病人费用记录 Where 病人ID=" & lngPatiID & " And 主页ID=" & lngPageID & " And Nvl(结帐ID,0)<>0 and Rownum<2"
    ...
    mstrSQL = "select * from 保险病种 where ID=(" & _
                    "Select 出院病种ID from 保险帐户 where 病人ID=" & lngPatiID & _
                                                 " and 险类=" & TYPE_山西 & ") " & _
                                       " and 险类=" & TYPE_山西
    Call OpenRecordset(mrsTMP, "病种信息", mstrSQL)
    If mrsTMP.EOF Then
...
Public Function 住院虚拟结算_山西(rsExse As Recordset, ByVal lng病人ID As Long) As String
    ...
    '读取病人IC卡密码
    gstrSQL = "Select 医保号,密码,卡号,就诊类别 From 保险帐户 Where 险类=" & TYPE_山西 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "读取病人IC卡密码")
    ...
    gstrSQL = "  Select A.ID,A.NO,A.病人ID,A.收费类别,A.记录性质,A.记录状态,A.序号,A.收费细目ID,C.项目编码 AS 医保项目编码,B.编码,B.名称,A.实收金额 AS 金额" & _
              ...
              "  From 病人费用记录 A,收费细目 B,保险支付项目 C,部门表 E" & _
    ...
    Call OpenRecordset(rsDetail, "提取本次费用明细")
    With rsDetail
        Do While Not .EOF
            '入参说明
            ...
            gstrSQL = " Select A.类别,A.ID AS 收费细目ID,A.编码 As 医院编码,A.名称 AS 医院名称,B.项目编码 As 医保编码,B.项目名称 AS 医保名称,B.附注" & _
                    " From 收费细目 A,(Select * From 保险支付项目 Where 险类=" & TYPE_山西 & ") B" & _
                    " Where A.ID=" & !收费细目ID & " And A.ID=B.收费细目ID(+) "
            Call OpenRecordset(rsTemp, "提取收费项目信息")
            ...
            End If
            Call OpenRecordset(rsTemp, "提取中心项目比例、类别", gstrSQL, gcnSxDr)
            If rsTemp.RecordCount = 0 Then
...
Public Function 住院结算_山西(ByVal lng结帐ID As Long) As Boolean
    ...
    '读取病人IC卡密码
    gstrSQL = "Select 医保号,密码,就诊类别 From 保险帐户 Where 险类=" & TYPE_山西 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "读取病人IC卡密码")
    strPin = Nvl(rsTemp!密码)
    str就诊类别 = Nvl(rsTemp!就诊类别, "11")
    str医保号 = rsTemp!医保号
...
mstrSQL = "select A.入院日期,A.出院日期,B.住院号,D.名称 as 住院科室,A.入院病床,A.住院医师,C.卡号," & _
          "C.密码,D.编码 As 科室编码,C.就诊类别 from 病案主页 A,病人信息 B,保险帐户 C,部门表 D " & _
          "Where A.病人ID = B.病人ID And A.病人ID = C.病人ID And " & _
          "A.入院科室ID = D.ID And A.主页ID = " & lng主页ID & " And A.病人ID = " & lng病人ID & _
          " and C.险类=" & TYPE_山西
...
mstrSQL = "select * from 保险病种 where ID=(" & _
                "Select 出院病种ID from 保险帐户 where 病人ID=" & lng病人ID & _
                                             " and 险类=" & TYPE_山西 & ") " & _
                                   " and 险类=" & TYPE_山西
Call OpenRecordset(mrsTMP, "病种信息", mstrSQL)
If mrsTMP.EOF Then
...
Public Function 记帐上传_山西(int性质 As Integer, int状态 As Integer, str单据号 As String) As Boolean
    ...
    Do Until rsCd.EOF
        ...
        Call OpenRecordset(rsTemp, "读取卡号", gstrSQL)
...
Public Function 住院结算冲销_山西(lng结帐ID As Long) As Boolean
    ...
    If str住院号 = "" Or str医疗类别 = "" Then
        MsgBox "原始结算记录交易号不全，无法进行门诊结算冲销！", vbInformation, gstrSysName
        住院结算冲销_山西 = False
        Exit Function
    End If
