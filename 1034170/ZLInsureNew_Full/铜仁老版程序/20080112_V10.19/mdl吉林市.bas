...
'        果不一致，需要执行恢复结算交易和费用删除交易。这样才能保证数据的完全统一。
    ...
    gstrSQL = " " & _
        ...
        "  From (Select * From 病人费用记录 Where nvl(实收金额,0)<>0 and  记录状态<>0 and 结帐ID=" & lng结帐ID & " and  Nvl(附加标志,0)<>9 ) A,收费细目 B,部门表 C,保险支付项目 D,部门表 E,  " & _
        "       (Select distinct Q.药品id,Q.标识码,T.名称 From 药品目录 Q,药品信息 R,药品剂型 T  Where  Q.药名id=R.药名id and R.剂型=T.编码 ) J, " & _
        "       人员表 H,保险帐户 L" & _
        "  Where A.收费细目ID=B.ID And A.开单部门ID=C.ID(+)  and A.病人id=L.病人id  and L.险类=" & TYPE_吉林 & " and a.收费细目id=J.药品id(+) " & _
        "        and A.执行部门ID=E.ID(+) And A.收费细目ID=D.收费细目ID And D.险类= " & TYPE_吉林 & " and a.开单人=H.姓名(+) " & _
        "  Order by A.ID"
    ...
    gstrSQL = "Select * From 保险病种 Where ID=" & Nvl(rs明细!病种ID, 0) & " And 险类=" & TYPE_吉林
...
'      cur个人帐户   从个人帐户中支出的金额
   ...
   '插入保险结算记录
    ...
    gstrSQL = "Select * From 保险结算记录 Where 记录ID=" & lng结帐ID & " And 险类=" & TYPE_吉林
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
...
Public Function 入院登记_吉林(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
    ...
    gstrSQL = "Select * From 床位状况记录 D where 病区ID=" & Nvl(rsTemp!当前病区ID, 0) & " And 床号='" & Nvl(rsTemp!当前床号) & "'"
...
Go病种:
    ...
    gstrSQL = "" & _
        ...
        "       (Select 病人id,主页id,max(DECODE(a.诊断次序,1,b.编码,'')) AS 出院编码 From 诊断情况 A ,疾病编码目录 B Where a.疾病ID = b.ID And a.诊断类型 = 3 and a.主页id=" & lng主页ID & " and a.病人id=" & lng病人ID & " Group by 病人id,主页id)   G" & _
...
'    gstrSQL = "Select * From 保险病种 where id=" & Nvl(rs明细!病种ID, 0)
    ...
    Do While Not rs明细.EOF
        ...
        End Select
        If strOutput = "" Then
            ShowMsgbox "在获取药品等信息时出现返回值为空,请与医保提供商联系!" & vbCrLf & " 输入参数为:" & StrInput
