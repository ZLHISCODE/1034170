...
Public Function 门诊虚拟结算_成都内江(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    With rs明细
        Do While Not .EOF
            ...
            Else
                MsgBox "存在重复的收费细目，请合并后再进行预结算！", vbInformation, gstrSysName
                Exit Function
            End If
            .MoveNext
        Loop
...
Private Function 获取个人帐户支付() As Double
    ...
    gstrSQL = "Select * From 病人预交记录 where 结帐ID=" & g结算数据.结帐ID & " and  结算方式='个人帐户'"
        ...
        Do While Not .EOF
            ...
            If Val(Nvl(rs明细("实收金额"), 0)) <> 0 Then
                ...
                gstrSQL = "select A.名称,A.编码,A.类别,A.规格,A.计算单位,B.项目编码,B.附注,B.是否医保,A.计算单位,E.规格,G.名称 剂型,B.大类编码 " & _
                          ...
                          "             where a.险类=b.险类 and a.项目编码=b.编码 and A.收费细目ID =" & rs明细!收费细目ID & " and a.险类=" & TYPE_成都内江 & ") B,药品目录 E ,药品信息 F,药品剂型 G " & _
...
Public Function 住院结算_成都内江(lng结帐ID As Long, ByVal lng病人ID As Long) As Boolean
  '功能：将需要本次结帐的费用明细和结帐数据发送医保前置服务器确认；
    ...
    If g病人身份_成都内江.lng病人ID <> lng病人ID Then
        ...
        Exit Function
    End If
    gstrSQL = "Select Sum(nvl(结帐金额,0)) as 总额 from 病人费用记录 where 结帐id=" & lng结帐ID
    ...
    If g病人身份_成都内江.费用总额 + g病人身份_成都内江.生育总费用 <> Nvl(rsTemp!总额, 0) Then
        ...
        ShowMsgbox "虚结算费用总额不等于本次结算总额:" & vbCrLf & "虚结算总额:" & Format(g病人身份_成都内江.费用总额 + g病人身份_成都内江.生育总费用, "#####0.00;-####0.00;0;0") & vbCrLf & "当前结算总额为:" & Format(Nvl(rsTemp!总额, 0), "#####0.00;-####0.00;0;0")
        Exit Function
    End If
...
Private Function 处方上传(ByVal lng记录性质 As Long, lng记录状态 As Long, ByVal str单据号 As String) As Boolean
    ...
    gstrSQL = "Select A.ID,A.NO,A.病人ID,A.主页ID,to_char(A.发生时间,'yyyymmdd') as 发生时间,to_char(A.发生时间,'yyyy-mm-dd hh24:mi:ss') as 登记时间,Round(A.实收金额,4) 实收金额 " & _
              ...
              "  From 病人费用记录 A,部门表 Z,收费类别 J,收费细目 B,保险帐户 M,(Select O.*,Z.大类编码 From 保险支付项目 O,保险项目 Z where O.险类=Z.险类 and O.项目编码=Z.编码 and O.险类=" & TYPE_成都内江 & ") C,病案主页 D,药品目录 E ,药品信息 F,药品剂型 G " & _
                ...
                Else
                    ...
                    If Nvl(rsTemp!附加诊断) = "" Then
                        ...
                        If Substr(str保险编码, 1, 1) = "M" Then
                            gstrSQL = "Select 保险编码 from 病人费用记录 Where 记录状态=1 and 摘要<>'不上传'" & _
...
Public Function 住院虚拟结算_成都内江(rsExse As Recordset, ByVal lng病人ID As Long, Optional bln结帐处 As Boolean = True) As String
        ...
        Do While Not .EOF
            ...
            If Nvl(rsExse!保险大类id, 0) = 0 Then
                gstrSQL = "Select Nvl(大类编码,0) As 大类编码 From 保险项目 Where 险类=" & TYPE_成都内江 & _
                          " And 编码=(Select nvl(保险编码,'0') From 病人费用记录 Where 收费细目ID=" & rsExse!收费细目ID & _
    ...
    gstrSQL = "Select A.ID From 病人费用记录 a,药品收发记录 B where A.no=b.No and B.单据 in (9,10) and a.id=b.费用ID and a.病人id=" & lng病人ID & " and 主页id=" & lng主页ID & " and b.扣率 like '_3%' and rownum<=2"
...
Public Function GetItemInsure_成都内江(lng病人ID As Long, lng收费细目ID As Long, bln门诊 As Boolean) As String
    ...
    gstrSQL = "Select B.类别,A.编码,A.名称,B.说明 From 保险项目 A,医保对照明细 B" & _
        " Where B.险类=" & TYPE_成都内江 & " And A.险类=B.险类 And A.编码=B.项目编码 And B.收费细目ID=" & lng收费细目ID
...
Public Function 并发症选择_成都内江(lng病人ID As Long, lng主页ID As Long)
    ...
    gstrSQL = "Select * from 病案主页 Where 出院日期 is null and 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsBfz, "是否在院")
    If rsBfz.RecordCount > 0 Then
        gstrSQL = "Select * from 保险帐户 Where 病人ID=" & lng病人ID
        Call OpenRecordset(rsBfz, "取统筹地区编码")
    Else
