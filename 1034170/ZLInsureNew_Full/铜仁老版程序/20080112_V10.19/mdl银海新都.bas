...
'    End If
    ...
    gstrSQL = "Select 参数名,参数值 From 保险参数 Where 险类=" & TYPE_新都
...
'      3)识别正确，而个人信息缺少某项，必须以空格填充；
    ...
    gstrSQL = "select nvl(当前状态,0) as 当前状态,顺序号 from 保险帐户 where 卡号='" & str卡号 & "' and 险类=" & TYPE_新都
...
'    End If
    ...
    gstrSQL = "Select * From 保险帐户 Where 医保号='" & str医保号 & "' And 险类=" & TYPE_新都
...
'返回: 返回个人帐户余额的金额
    ...
    Else
        ...
        gstrSQL = "Select 帐户余额 From 保险帐户 where 险类=" & TYPE_新都 & " and 中心=0 and 医保号='" & strSelfNo & "'"
        Call OpenRecordset(rsTemp, gstrSysName)
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    gstrSQL = "select 医保号,顺序号 as 就诊序号,退休证号 as 中心编号,人员身份 as 人员类别  " & _
              "from 保险帐户 where 病人ID=" & lng病人ID & " and 险类=" & TYPE_新都
    ...
    Do Until rs明细.EOF
        ...
        gstrSQL = "select A.名称,A.编码,A.类别,A.规格,A.计算单位,B.项目编码,B.附注,C.类别 as 大类 " & _
                 ...
                 " where A.类别=C.编码 and  A.ID=" & rs明细("收费细目ID") & " and A.ID=B.收费细目ID and B.险类=" & TYPE_新都
        Call OpenRecordset(rsTemp, "门诊预算")
...
'参数：lng病人ID=是否只上传单据中指定病人的费用
    ...
    gstrSQL = _
        ...
        " And D.险类=" & TYPE_新都 & " And E.险类=" & TYPE_新都 & " And F.险类=" & TYPE_新都 & _
    ...
    Do Until rsBill.EOF
        ...
        If lng当前病人 <> rsBill("病人ID") Then
            ...
            gstrSQL = "Select 医保号,顺序号 as 就诊序号,退休证号 as 中心编号,人员身份 as 人员类别  " & _
                      ...
                      "  where B.病人ID=" & lng当前病人 & " and B.主页ID=" & rsBill("主页ID") & " and A.病人ID=B.病人ID and A.险类=" & TYPE_新都 & " and A.年度=to_char(B.入院日期,'yyyy')" & _
...
'    End If
    ...
    gstrSQL = "Select 医保号,顺序号 as 就诊序号,退休证号 as 中心编号,人员身份 as 人员类别  " & _
              ...
              "       ,NVL(A.基本统筹限额,0) as 住院统筹限额,NVL(A.大额统筹限额,0) as 实足年龄,NVL(A.大额统筹累计,0) as 统筹报销比例" & _
              "  From 帐户年度信息 A,病案主页 B,保险帐户 C " & _
              "  where B.病人ID=" & lng病人ID & " and B.主页ID=" & g结算数据.主页ID & " and A.病人ID=B.病人ID and A.险类=" & TYPE_新都 & " and A.年度=to_char(B.入院日期,'yyyy')" & _
              "     and C.病人ID=A.病人ID and C.险类=A.险类"
