...
Private Sub cmd统计_Click()
    ...
    gstrSQL = "" & _
             ...
             "        trim(to_char(A.费用总额,'9999990.00')) AS 费用总额,trim(to_char(A.药品费,'9999990.00')) AS 药品费,trim(to_char(A.非目录内药品费,'9999990.00')) AS 非目录内药品费, " & _
    ...
    gstrSQL = gstrSQL & "Sum(DECODE(A.收费类别,'5',Nvl(A.实收金额,0),'6',Nvl(A.实收金额,0),'7',Nvl(A.实收金额,0),0)) As 药品费, " & _
             ...
             " Where A.病人ID =B.病人ID And B.病人ID=C.病人ID And A.主页ID=C.主页ID And A.病人ID=D.病人ID And D.险类=" & mintInsure
    Call OpenRecordset(rsTemp, "统计病人结算数据")
