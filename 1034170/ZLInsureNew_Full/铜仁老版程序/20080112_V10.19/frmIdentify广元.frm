...
Begin VB.Form frmIdentify广元 
   ...
   End
End
Attribute VB_Name = "frmIdentify广元"
...
Public mstrPatient As String, mstrOther As String, mstr就诊编号 As String, mstr就诊次数 As String
...
Private Sub cmdChangPass_Click()
    initType
    mblnReturn = gy_changePassword(gstr医保机构编码, gstr医院编码, gstrOutPara)
    TrimType
...
Private Sub cmdOK_Click()
    ...
    Const gstr广元市医保 As String = "430700"

    '检查中心代码与卡前6位是否相符，不符则不允许办理医保登记
    If gstr广元市医保 <> Mid(txtInfo(2).Text, 1, 6) Then
        MsgBox "选择医保中心错误，请重新选择！（本次选择的是广元市医保）", vbInformation, gstrSysName
        Exit Sub
    End If
    ...
    With gstrOutPara
        ...
        mstrPatient = mstrPatient & txtInfo(7).Text & ";"                   '7 单位名称
...
'    mblnReturn = gy_dall(gstr医保机构编码, gstr医院编码, "2003121000031", gstrOutPara)
        mblnReturn = gy_reg(gstr医保机构编码, gstr医院编码, 0, UserInfo.姓名, Format(zlDatabase.Currentdate, "yyyy-MM-dd"), "0", gstrOutPara)
        TrimType
        If mblnReturn = False Then
...
Private Sub cmdRead_Click()
    ...
    initType
    mblnReturn = gy_readicxx(gstr医保机构编码, gstr医院编码, "0", gstrOutPara)
    If mblnReturn = False Then
        MsgBox gstrOutPara.errtext, vbInformation, Me.Caption
        Exit Sub
    End If
    ...
    With gstrOutPara
        ...
        txtInfo(7).Text = .out8
        txtInfo(8).Text = .out9
        txtInfo(9).Text = IIf(.out10 = "01", "在职", IIf(.out10 = "02", "退休", "下岗"))
