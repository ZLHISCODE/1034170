...
Private Function 检查医保服务器_自贡() As Boolean
    ...
    '读出连接医保服务器的配置
    gstrSQL = "select 参数名,参数值 from 保险参数 where 参数名 like '医保%' and 险类=" & TYPE_四川自贡
    Call OpenRecordset(rsTemp, "中软医保")
...
Public Function 门诊虚拟结算_自贡(rs明细 As ADODB.Recordset, str结算方式 As String) As Boolean
    ...
    '提取开单部门名称
    gstrSQL = "Select 名称 From 部门表 Where ID=" & rs明细!开单部门ID
    Call OpenRecordset(rsTemp, "提取开单部门名称")
    ...
    With rs明细
        ...
        Do While Not .EOF
            ...
            gstrSQL = " Select A.项目编码,B.名称 From 保险支付项目 A,收费细目 B" & _
                      " Where A.收费细目ID=B.ID And A.险类=" & TYPE_四川自贡 & " And 收费细目ID=" & !收费细目ID
            Call OpenRecordset(rsTemp, "提取项目的医保编码")
...
Public Function 门诊结算冲销_自贡(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long) As Boolean
    ...
    Call OpenRecordset(rsTemp, "自贡医保")
    lng冲销ID = rsTemp("结帐ID")
    
    '取出上次结算的收据号
    gstrSQL = "Select 支付顺序号 From 保险结算记录 Where 险类=" & TYPE_四川自贡 & " And 性质=1 And 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "取上次结算的收据号")
    If rsTemp.RecordCount = 0 Then
        MsgBox "没有找到原始的收费记录，无法完成门诊冲销！", vbInformation, gstrSysName
        Exit Function
    End If
    strBalanceNO = Nvl(rsTemp!支付顺序号)
    If strBalanceNO = "" Then
        MsgBox "无效的门诊收据号，无法完成门诊冲销！", vbInformation, gstrSysName
        Exit Function
    End If
...
Public Function 个人余额_自贡(ByVal lng病人ID As Long) As Currency
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select Nvl(帐户余额,0) AS 余额 From 保险帐户 Where 险类=" & TYPE_四川自贡 & " And 病人ID=" & lng病人ID
...
Public Function 入院登记_自贡(lng病人ID As Long, lng主页ID As Long) As Boolean
    ...
    Call OpenRecordset(rsSelected, "获取已选择的病种", gstrSQL, gcn自贡)
    gstrSQL = " Select A.SickSerialNo AS ID,A.SickNum AS 编码,A.SickName AS 名称,A.SickSpell AS 简码 " & _
            " From SickDefine A Where 1=1"
    Set rs病种 = New ADODB.Recordset
    Call OpenRecordset(rs病种, "身份验证", gstrSQL, gcn自贡)
...
VirusSelect:
    ...
    Call OpenRecordset(rsTemp, "提取病人的入院科室、医生与入院日期")
    str科室 = Nvl(rsTemp!科室)
    str医生 = Nvl(rsTemp!医生)
...
Public Function 处方上传_自贡(ByVal str单据号 As String, ByVal int性质 As Integer, ByVal int状态 As Integer, Optional ByVal lng病人ID As Long = 0, Optional ByVal bln结算 As Boolean = False) As Boolean
    ...
    If Not bln结算 Then
        '检查是否存在未对码的项目开处方
        gstrSQL = " Select 1 " & _
                  " From 病人费用记录 A,(Select * From 保险支付项目 Where 险类=" & TYPE_四川自贡 & ") B,保险帐户 C,病案主页 D,病人信息 E,收费细目 F,部门表 G" & _
                  ...
                  " And C.险类=" & TYPE_四川自贡 & " And Nvl(A.是否上传,0)=0" & _
                  " And B.项目编码 Is NULL And Rownum<2"
        Call OpenRecordset(rsCheck, "检查是否存在未对码的项目开处方")
    ...
    gstrSQL = " Select A.NO,A.记录性质,A.记录状态,A.病人ID,A.主页ID,SUM(A.实收金额) AS 金额,G.名称 AS 开单科室,A.开单人" & _
              " From 病人费用记录 A,保险帐户 C,病案主页 D,病人信息 E,部门表 G" & _
              " Where A.NO='" & str单据号 & "' And A.记录性质=" & int性质 & " And A.记录状态=" & int状态 & _
              IIf(lng病人ID = 0, "", " And A.病人ID=" & lng病人ID) & _
              " And E.病人ID=D.病人ID And E.住院次数=D.主页ID And A.病人ID=E.病人ID And A.开单部门ID=G.ID(+)" & _
              " And C.病人ID=A.病人ID And C.险类=" & TYPE_四川自贡 & " And Nvl(A.是否上传,0)=0" & _
              " Group by A.NO,A.记录性质,A.记录状态,A.病人ID,A.主页ID,G.名称,A.开单人"
    Call OpenRecordset(rsHead, "提取本次需要上传的处方主表")
    ...
    With rsHead
        Do While Not .EOF
            ...
            gstrSQL = " Select A.NO,A.记录性质,A.记录状态,A.序号,A.收费细目ID,B.项目编码,A.付数*A.数次 AS 数量,A.实收金额,F.名称 AS 项目名称,F.计算单位," & _
                      ...
                      " From 病人费用记录 A,保险支付项目 B,保险帐户 C,病案主页 D,病人信息 E,收费细目 F,部门表 G" & _
                      " Where A.NO='" & !NO & "' And A.记录性质=" & !记录性质 & " And A.记录状态=" & !记录状态 & " And A.病人ID=" & !病人ID & _
                      " And E.病人ID=D.病人ID And E.住院次数=D.主页ID And A.病人ID=E.病人ID And A.开单部门ID=G.ID(+)" & _
                      " And C.病人ID=A.病人ID And A.收费细目ID=B.收费细目ID And A.收费细目ID=F.ID" & _
                      " And C.险类=B.险类 And C.险类=" & TYPE_四川自贡 & " And Nvl(A.是否上传,0)=0 And Nvl(A.实收金额,0)<>0" & _
                      " Order by A.NO,A.记录性质,A.记录状态,A.序号"
            Call OpenRecordset(rsDetail, "提取本次需要上传的处方明细")
...
Public Function 住院虚拟结算_自贡(ByVal rsExse As ADODB.Recordset, ByVal lng病人ID As Long) As String
    ...
    '处方明细上传(仅提取出还未上传的处方)
    gstrSQL = " Select NO,记录性质,记录状态,count(*) Records From 病人费用记录 A,病人信息 B " & _
              " Where A.病人ID=" & lng病人ID & " And A.病人ID=B.病人ID And A.主页ID=B.住院次数 " & _
              " And Nvl(记录状态,0)<>0 And Nvl(实收金额,0)<>0 And Nvl(是否上传,0)=0" & _
              " Having Count(*)>0" & _
              " Group by NO,记录性质,记录状态"
    Call OpenRecordset(rsUpload, "处方明细上传")
    With rsUpload
        Do While Not .EOF
    ...
    gstrSQL = " Select Nvl(A.金额,0) AS 费用总额 From 病人未结费用 A,病人信息 B" & _
              " Where A.病人ID =" & lng病人ID & " And A.病人ID=B.病人ID And A.主页ID=B.住院次数"
    Call OpenRecordset(rsTemp, "取本次费用总额")
    If rsTemp.RecordCount <> 0 Then
        cur总费用 = rsTemp!费用总额
    Else
        cur总费用 = 0
    ...
    gstrSQL = "Select A.病人ID,A.主页ID,出院日期,出院方式 From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.主页ID=B.住院次数 And A.病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取出院日期")
...
'    gcn自贡.Execute gstrSQL
    ...
    intRecur = 0
    gstrSQL = "Select 人员身份 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_四川自贡
    Call OpenRecordset(rsTemp, "检查是否伤残军人")
...
'    Dim str结算单号 As String
    ...
    On Error GoTo ErrHand
    '如果病人已出院，则是出院结算；否则为中途结算(结算类型。0结算，1中结)
    gstrSQL = "Select A.病人ID,A.主页ID,出院日期 From 病案主页 A,病人信息 B Where A.病人ID=B.病人ID And A.主页ID=B.住院次数 And A.病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取出院日期")
    lng主页ID = rsTemp!主页ID
    str住院号 = Get住院号(rsTemp!病人ID)
...
Public Function 住院结算冲销_自贡(ByVal lng结帐ID As Long) As Boolean
    ...
    '获取结算单号，结算类型
    gstrSQL = "Select 病人ID,主页ID,支付顺序号,中途结帐 From 保险结算记录 Where 性质=2 And 险类=" & TYPE_四川自贡 & " And 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, "取结算单号")
    If rsTemp.RecordCount = 0 Then
        MsgBox "没有找到原始的收费记录，无法完成住院结算冲销！", vbInformation, gstrSysName
        Exit Function
    End If
    lng主页ID = rsTemp!主页ID
    ...
    gstrSQL = "Select NO from 病人结帐记录 Where ID=" & lng冲销ID
    Call OpenRecordset(rsTemp, "取结算单号")
...
Private Sub InsertDisease(ByVal strTable As String, ByVal strKey As String, ByVal strDisease As String)
    ...
    Call OpenRecordset(rsTemp, "打开病种记录集", gstrSQL, gcn自贡)
...
Private Function Get序列(ByVal lng病人ID As Long) As Integer
    Dim rsTemp As New ADODB.Recordset
    '仅供住院使用，用于产生序列
    gstrSQL = "Select Nvl(退休证号,0) AS 序列 From 保险帐户 Where 险类=" & TYPE_四川自贡 & " And 病人ID=" & lng病人ID
    Call OpenRecordset(rsTemp, "取序列")
    Get序列 = rsTemp!序列 + 1
...
Private Function Get住院号(ByVal lng病人ID As Long) As String
    Dim rsTemp As New ADODB.Recordset
    '提取病人的住院登记流水号
    gstrSQL = "Select 顺序号 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_四川自贡
    Call OpenRecordset(rsTemp, "取病人的住院流水号")
...
Private Function GetPass(ByVal lng病人ID As Long) As String
    Dim rsTemp As New ADODB.Recordset
    '提取病人的密码（不允许空密码）
    gstrSQL = "Select 密码 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_四川自贡
    Call OpenRecordset(rsTemp, "取密码")
