...
'    gstr医保机构编码 = rsTemp!中心
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=1 and 险类=" & TYPE_涪陵 & " and 记录ID=" & lng结帐ID: lngErr = 4
    Call OpenRecordset(rsTemp, gstrSysName)
...
'      cur个人帐户   从个人帐户中支出的金额
    ...
    gstrSQL = "select * from 保险结算记录 where 性质=2 and 险类=" & TYPE_涪陵 & " and 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
'             16补充医疗记入累计,17门诊统筹记入累计,18未报销费用,19医保支付,20个人现金支付,21个人帐户余额
   ...
   If gstrOutPara.out24 <> "" Then
            ...
            cur公务员补助 = CCur(gstrOutPara.out24)
...
CheckCard:
    ...
    gstrSQL = "Select Nvl(帐户余额,0) AS 帐户余额,nvl(顺序号,0) as 顺序号,中心 From 保险帐户 Where 病人ID=" & lng病人ID & " And 险类=" & TYPE_涪陵
    Call OpenRecordset(rsTemp, gstrSysName)
    str就诊编号 = rsTemp!顺序号
    cur帐户余额 = rsTemp!帐户余额
'    gstr医保机构编码 = rsTemp!中心
...
Public Function 记帐传输_涪陵(ByVal str单据号 As String, ByVal int性质 As Integer, str消息 As String, Optional ByVal lng病人ID As Long = 0) As Boolean
    ...
    Else
        ...
        gstrSQL = "Select * From 病人费用记录 Where 记录状态<>0 And Nvl(是否上传,0)=0 And Nvl(实收金额,0)<>0 And nvl(附加标志,0)<>9 and 病人id=" & lng病人ID & " And 主页id=" & lng主页ID & " order by 主页ID,序号"
        Call OpenRecordset(rsTemp, gstrSQL)
        记帐传输_涪陵 = 费用明细传递_涪陵(0, rsTemp)
    End If
End Function
