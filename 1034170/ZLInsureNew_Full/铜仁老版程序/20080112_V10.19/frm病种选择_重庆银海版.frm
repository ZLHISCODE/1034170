...
Private Sub Form_Load()
    ...
    '如果病种编码不为空,提取该病种相关信息
    If mstr病种编码 <> "" Then
        gstrSQL = "Select 类别,ID From 保险病种 Where 险类=" & TYPE_重庆银海版 & " And 编码='" & mstr病种编码 & "'"
        Call OpenRecordset(rsTemp, "取病种信息,定位到历史病种选择")
...
Private Sub tvw_s_NodeClick(ByVal Node As MSComctlLib.Node)
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select ID,编码,名称,简码 From 保险病种 Where 险类=" & TYPE_重庆银海版 & " And 类别='" & Mid(tvw_s.SelectedItem.Key, 2) & "'"
    Call OpenRecordset(rsTemp, "取该大类下所有病种")
