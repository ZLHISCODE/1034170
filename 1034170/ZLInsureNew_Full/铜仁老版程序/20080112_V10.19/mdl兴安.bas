...
Public Function 医保初始化_兴安() As Boolean
    ...
    zlDatabase.OpenRecordset rsTemp, gstrSQL, "保险参数"
...
Private Function Get明细记录(ByRef lng结帐ID As Long, Optional strNO As String, Optional lng记录性质 As Long, Optional lng记录状态 As Long) As String
    ...
    If lng结帐ID = 0 Then
            strSQL = " " & _
                ...
                "  From (Select * From 病人费用记录 Where  nvl(实收金额,0)<>0  and 记录状态<>0 and NO='" & strNO & "' and 记录性质=" & lng记录性质 & " and 记录状态=" & lng记录状态 & " and  Nvl(附加标志,0)<>9 ) A,部门表 C," & _
                "       保险帐户 L,收费细目 J " & _
...
Public Function 门诊结算_兴安(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String) As Boolean
    ...
    If Val(g病人身份_兴安.虚拟结算(3)) <> Val(strArr(2)) Then
        ShowMsgbox "注意:" & vbCrLf & "   虚拟结算与正式结算的数据不等!" & vbCrLf & "  虚拟结算为:" & Val(g病人身份_兴安.虚拟结算(3)) & "   正式结算为:" & Val(strArr(2))
    End If
...
Private Function Get冲销ID(ByVal lng结帐ID As Long, Optional bln门诊 As Boolean = True) As Long
    ...
    Else
        gstrSQL = "select distinct A.ID 结帐ID from 病人结帐记录 A,病人结帐记录 B where A.NO=B.NO and  A.记录状态=2 and B.ID=" & lng结帐ID
    End If
...
Public Function 个人余额_兴安(ByVal lng病人ID As Long) As Currency
    ...
    gstrSQL = "Select Nvl(帐户余额,0) 帐户余额 From 保险帐户 Where 险类=" & TYPE_兴安
...
Public Function 住院虚拟结算_兴安(rsExse As Recordset, ByVal lng病人ID As Long, Optional bln结帐处 As Boolean = True) As String
    ...
    Else
        gstrSQL = "Select ID From 病人费用记录 where nvl(是否上传,0)=0 and  (记录状态=3 or 记录状态=1) and nvl(实收金额,0)<>0 and rownum<=1 and 病人id=" & lng病人ID & " and 主页id =" & lng主页ID
        ...
        Do While Not .EOF
            ...
            If Nvl(!是否上传, 0) <> 1 And Nvl(!实际金额, 0) <> 0 Then
                    ...
                    If Nvl(!记录状态, 0) <> 1 Then
                        ...
                        '需确定原始记录中的项目序号
                        DebugTool " 处方明细:冲销单据"
                        gstrSQL = "Select 摘要 From 病人费用记录 where (mod(记录状态,3)=0 or 记录状态=1) and NO='" & Nvl(!NO) & "' and 记录性质=" & Nvl(!记录性质, 0) & " and 序号=" & Nvl(!序号)
...
Private Function 补传住院明细记录(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As Boolean
   ...
   If 业务请求_兴安(住院事务开始, InitInfor_兴安.医院编码, "") = False Then Exit Function
    ...
    gstrSQL = " " & _
         ...
         "      A.收费类别,A.摘要,A.操作员姓名 as 经办人," & _
         "      L.险类,L.中心,L.卡号,L.医保号,L.人员身份,L.顺序号,L.病种ID,L.就诊时间,J.编码 ,J.名称 as 商品名,J.规格" & _
         "  From (Select * From 病人费用记录 Where (记录状态=3 or 记录状态=1) and nvl(实收金额,0)<>0 and 病人id=" & lng病人ID & " and 主页id=" & lng主页ID & " and nvl(是否上传,0)=0 and  Nvl(附加标志,0)<>9 ) A,部门表 C," & _
         "       保险帐户 L,收费细目 J " & _
         "  Where A.开单部门id=C.id(+) and  A.病人id=L.病人id and a.收费细目id=J.id and L.险类=" & TYPE_兴安 & _
         "   Order by NO,记录性质,记录状态,序号"
         
    Call OpenRecordset(rs明细, "虚拟结算", gstrSQL)
    ...
    gstrSQL = " " & _
         ...
         "      A.收费类别,A.摘要,A.操作员姓名 as 经办人," & _
         "      L.险类,L.中心,L.卡号,L.医保号,L.人员身份,L.顺序号,L.病种ID,L.就诊时间,J.编码 ,J.名称 as 商品名,J.规格" & _
         "  From (Select * From 病人费用记录 Where 记录状态=2 and nvl(实收金额,0)<>0 and 病人id=" & lng病人ID & " and 主页id=" & lng主页ID & " and nvl(是否上传,0)=0 and  Nvl(附加标志,0)<>9 ) A,部门表 C," & _
         "       保险帐户 L,收费细目 J " & _
         "  Where A.开单部门id=C.id(+) and  A.病人id=L.病人id and a.收费细目id=J.id and L.险类=" & TYPE_兴安 & _
         "   Order by NO,记录性质,记录状态,序号"
