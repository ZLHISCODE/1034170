...
Private Sub txt姓名_KeyPress(KeyAscii As Integer)
    ...
    If txt姓名.Tag = "." Then Exit Sub
        ...
        End If
        '门诊记帐时必须要挂号建档
        strSQL = "Select A.病人id,A.姓名,A.标识号,A.性别,A.年龄 From 病人费用记录 A Where A.NO='" & strSQL & "' And A.记录性质=4 And A.记录状态=1 "
...
'            Else
            ...
            gstrSQL = "select A.医保号,B.姓名,B.性别,B.年龄,c.ID,C.编码,c.名称" & _
                ...
                " Where A.医保号='" & a & "' AND A.险类=65 and A.病人ID=B.病人ID and a.病种id=c.id(+)"
            Set rsTemp1 = OpenSQLRecord(gstrSQL, "提取病人信息", CStr(txt姓名.Text), CInt(mintInsure))
