...
'        End If
        ...
        strSQL = "Select A.*,B.医保号 From 病人信息 A," & _
            " (Select * From 保险帐户" & _
            " Where 险类=" & intInsure & _
            " And 医保号='" & CStr(array信息(1)) & "'" & _
            " And Nvl(中心,0)=" & lng中心 & ") B" & _
...
'    rsTemp.Open IIf(strSql = "", gstrSQL, strSql), gcnOracle, adOpenStatic, adLockReadOnly
    ...
    Set rsTemp = gcnConnect.Execute(IIf(strSQL = "", gstrSQL, strSQL))
    Call SQLTest
End Sub
...
'调用方式为：Set rsPati=OpenSQLRecord(strSQL, Me.Caption, CDate(Format(rsMove!转出日期,"yyyy-MM-dd")),dtp时间.Value, lng病人ID, "张%", "男", 20, 21)
    ...
    For i = 0 To UBound(arrPar)
        ...
        Case "Variant()" '数组
            ...
            Case "String" '字符
                intMax = zlCommFun.ActualLen(varValue(lngLeft))
                If intMax = 0 Or intMax < 10 Then intMax = 10
...
'    gstrSQL = "Select Count(*) as 次数 From 病案主页 Where Nvl(出院日期,Sysdate)=To_Date(To_Char(Sysdate,'YYYY')||'-01-01','YYYY-MM-DD') And 病人ID=" & lng病人id
    gstrSQL = "Select Count(*) as 次数 From 病案主页" & _
        " Where Nvl(主页ID,0)<>0 And Nvl(出院日期,Sysdate)=To_Date(To_Char(Sysdate,'YYYY')||'-01-01','YYYY-MM-DD') And 病人ID=" & lng病人ID
...
'200308z012:新增几个返回参数
    ...
    '帐户年度信息
    gstrSQL = "Select * From 帐户年度信息 Where 病人ID=" & lng病人ID & " And 险类=" & intInsure & " And 年度=" & str年度
...
'功能：检查特殊病人标志,成都蒲江地区使用
    ...
    gstrSQL = "Select to_char(入院日期,'YYYY') as 入院年度 From 病案主页 Where 病人ID=" & lng病人ID & " And 主页ID=" & lng主页ID
    Call OpenRecordset(rsTemp, "医保接口")
    str入院年度 = rsTemp("入院年度")
...
'字段：病人ID,收费细目ID,数量,单价,实收金额,统筹金额,保险支付大类ID,是否医保
    ...
    gstrSQL = "select A.收费细目ID from 保险特准项目 A,保险帐户 B " & _
            "where A.病种ID=B.病种ID and B.病人ID=" & lng病人ID & " and 险类=" & intInsure
    ...
    '2.2、计算进入统筹金额
    gstrSQL = "select ID,算法,统筹比额,特准定额,特准天数,是否医保 FROM 保险支付大类  where 险类=" & intInsure
...
'功能：判断是否全额统筹病人(注意：传的病人ID可能非医保病人的)
    ...
    Else
        gstrSQL = _
            ...
            " And A.病人ID=" & 病人ID & " And A.险类=" & intInsure
...
Public Function 医保病人已经出院(ByVal lng病人ID As Long) As Boolean
    ...
    gstrSQL = " Select DECODE(出院日期,NULL,0,1) AS 出院状态 From 病案主页 " & _
              " Where (病人ID,主页ID) IN (Select 病人ID,住院次数 From 病人信息 Where 病人ID=" & lng病人ID & ")"
...
'    strSql = "Select * From 病人信息 A,病案主页 B Where A.病人ID=B.病人ID(+) And A.病人ID=" & lngID & " Order by 主页ID"
    ...
    strSQL = _
        ...
        " Where A.病人ID=B.病人ID(+) And A.病人ID=" & lngID & _
...
Private Function MergePatient(ByVal lngOld As Long, ByVal lngInsure As Long) As Long
    ...
    '合并后应只剩一个病人
    strSQL = "Select 病人ID From 病人信息 Where 病人ID IN(" & lngOld & "," & lngInsure & ")"
    rsTmp.CursorLocation = adUseClient
    rsTmp.Open strSQL, gcnOracle, adOpenKeyset
...
Public Function IsApartComponents(ByVal intInsure As Integer) As Boolean
    ...
    If rsInsure.State = 0 Then
        gstrSQL = "Select 序号,医保部件 From 保险类别 "
        Call OpenRecordset(rsInsure, "提取医保部件名称")
    End If
...
Public Function CreateObject_Insure(ByVal intInsure As Integer, ByRef intOrder As Integer) As Boolean
...
Public Function ChooseInsure_Base(ByVal intInsure As Integer) As Integer
    ...
    Call OpenRecordset(rsTemp, "检查是否存在独立部件")
...
Public Sub 多单据收费_退费(ByVal lng结帐ID As Long)
    ...
    Call OpenRecordset(rsTemp, "判断是否为多单据收费")
    If InStr(1, rsTemp!备注, "多单据收费") = 0 Then Exit Sub        '不是多单据收费直接退出
    '提取本次结帐的相关信息
    gstrSQL = " Select NO,病人ID,登记时间 From 病人费用记录 Where 结帐ID=" & lng结帐ID & " And Rownum<2"
    Call OpenRecordset(rsTemp, "提取单据号与病人ID")
    lng病人ID = rsTemp!病人ID
    strNO = rsTemp!NO
    str登记时间 = Format(rsTemp!登记时间, "yyyy-MM-dd HH:mm:ss")
    '根据票据打印内容判断是否多单据收费
    gstrSQL = " Select NO From 票据打印内容 Where ID=(Select ID From 票据打印内容 Where 数据性质=1 And NO='" & strNO & "')"
    Call OpenRecordset(rsTemp, "根据票据打印内容判断是否多单据收费")
    If rsTemp.RecordCount > 1 Then Exit Sub                       '多条记录说明未勾选系统参数，HIS认为是多单据收费
    '提取登记时间，病人ID相同的单据清单，提示操作员
    gstrSQL = " Select Distinct NO From 病人费用记录 " & _
              " Where 记录性质=1 And 记录状态=1 And 病人ID=" & lng病人ID & _
              " And 登记时间=to_date('" & str登记时间 & "','yyyy-MM-dd hh24:mi:ss')"
    Call OpenRecordset(rsTemp, "提取登记时间，病人ID相同的单据清单")
    With rsTemp
        Do While Not .EOF
...
Public Function GetPara(ByVal varPara As Variant, Optional ByVal lngSys As Long, Optional ByVal lngModul As Long, _
    ...
    If IsToolInPara Then
        GetPara = CallByName(zlDatabase, "GetPara", VbMethod, varPara, lngSys, lngModul, blnPrivate, strDefault, blnNotCache)
    Else
        If TypeName(varPara) = "String" Then
            gstrSQL = "Select 参数值 From 系统参数表 where 参数名=[1]"
            Set rsTemp = OpenSQLRecord(gstrSQL, "获取系统参数", CStr(varPara))
        Else
            gstrSQL = "Select 参数值 From 系统参数表 where 参数号=[1]"
            Set rsTemp = OpenSQLRecord(gstrSQL, "获取系统参数", Val(varPara))
        End If
...
Public Function IsToolInPara() As Boolean
    ...
    '取系统版本号
    gstrSQL = "Select 版本号 From zlSystems Where  编号 =" & glngSys
    Call zlDatabase.OpenRecordset(rsTemp, gstrSQL, "取系统版本号")
...
Public Function IsZLHIS10() As Boolean
    ...
    Call OpenRecordset(rsTemp, "取系统版本号")
