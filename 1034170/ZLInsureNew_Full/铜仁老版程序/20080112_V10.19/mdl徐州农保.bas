...
Public Function openConn徐州农保() As Boolean
    ...
    If gcn徐州农保.State <> adStateOpen Then
        gstrSQL = "select 参数名,参数值 from 保险参数 where 险类=" & TYPE_徐州农保
...
'        gcn徐州农保.ConnectionString = "Provider=MSDASQL.1;Password=" & strPass & ";Persist Security Info=True;User ID=" & strUser & ";Data Source=" & strServer
...
Public Function 医保初始化_徐州农保() As Boolean
    ...
    gstrSQL = "Select * From 保险类别 Where 序号=" & TYPE_徐州农保
...
'返回: 返回个人帐户余额
    ...
    gstrSQL = "select 帐户余额 from 保险帐户 where 病人ID='" & lng病人ID & "' and 险类=" & TYPE_徐州农保
...
Public Function 入院登记_徐州农保(lng病人ID As Long, lng主页ID As Long, ByRef str医保号 As String) As Boolean
     '将病人的状态进行修改
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select * From 保险帐户 Where 病人id=" & lng病人ID & " And 险类=" & TYPE_徐州农保
    Call OpenRecordset(rsTemp, gstrSysName)
    If rsTemp.EOF Then
...
Public Function 住院虚拟结算_徐州农保(rs明细 As ADODB.Recordset, lng病人ID As Long) As String
    ...
    gstrSQL = "Select * From 病人费用记录 Where 门诊标志=2 And 记录状态<>0 And Nvl(是否上传,0)=0 And nvl(附加标志,0)<>9 and nvl(实收金额,0)<>0 and 病人id=" & lng病人ID & " And 主页id=" & lng主页ID & " order by 主页ID,序号"
    Call OpenRecordset(rsTemp, gstrSysName)
    
    gcn徐州农保.BeginTrans
    
    While Not rsTemp.EOF
        gstrSQL = "Select nvl(附注,0) As 附注 From 保险支付项目 Where 险类=" & TYPE_徐州农保 & " And 收费细目ID=" & rsTemp!收费细目ID
        Call OpenRecordset(rs项目, gstrSysName)
        If rs项目.EOF Then          '注意询问项目不是医保项目时如何处理
            gstrSQL = "Select * From 收费细目 Where ID=" & rsTemp!收费细目ID
            Call OpenRecordset(rs项目, gstrSysName)
...
Public Function 住院结算冲销_徐州农保(lng结帐ID As Long) As Boolean
    ...
    datCurr = zlDatabase.Currentdate
    gstrSQL = "Select 备注,病人ID From 保险结算记录 Where 性质=2 And 记录ID=" & lng结帐ID
    Call OpenRecordset(rsTemp, gstrSysName)
...
Public Function GetItemInfo_徐州(ByVal lngPatiID As Long, ByVal lngItemID As Long, Optional ByVal str摘要 As String, Optional intType As Integer = 0, Optional ByVal blnMsg As Boolean = False) As String
    ...
    '获取当前病人的险类
    gstrSQL = "Select * From 病人信息 Where 病人ID=" & lngPatiID
    Call OpenRecordset(rsTemp, "获取病人信息")
    int险类 = Nvl(rsTemp!险类, 0)
    If int险类 = 0 Then Exit Function
    
    WriteInfo "开始取项目信息(" & int险类 & ")"
    
    gstrSQL = "Select * From 保险支付项目 Where 险类=" & int险类 & " And 是否医保=1 And 项目名称 Is Not Null And 收费细目ID=" & lngItemID
    Call OpenRecordset(rsTemp, "获取保险项目信息")
    If rsTemp.EOF Then
...
Public Function 门诊虚拟结算_徐州农保(rs明细 As ADODB.Recordset, str结算方式 As String, Optional ByRef strAdvance As String = "") As Boolean
    ...
    With rs明细
        ...
        Do While Not .EOF
            gstrSQL = "Select nvl(附注,0) As 附注 From 保险支付项目 Where 险类=" & TYPE_徐州农保 & " And 收费细目ID=" & !收费细目ID
            Call OpenRecordset(rsTemp, "提取医保项目ID")
            If rsTemp.RecordCount = 0 Then         '注意询问项目不是医保项目时如何处理
                gstrSQL = "Select * From 收费细目 Where ID=" & !收费细目ID
                Call OpenRecordset(rsTemp, "提取HIS项目编码与名称")
...
Public Function 门诊结算_徐州农保(lng结帐ID As Long, cur个人帐户 As Currency, strSelfNo As String, Optional ByRef strAdvance As String = "") As Boolean
    ...
    Call OpenRecordset(rs明细, "提取病人ID")
    lng病人ID = rs明细!病人ID
...
'    End If
    ...
    With rs明细
        ...
        Do While Not .EOF
            gstrSQL = "Select 附注 From 保险支付项目 Where 险类=" & TYPE_徐州农保 & " And 收费细目ID=" & !收费细目ID
            Call OpenRecordset(rsTemp, "取医保项目序号")
...
Public Function 门诊结算冲销_徐州农保(lng结帐ID As Long, cur个人帐户 As Currency, lng病人ID As Long, Optional ByRef strAdvance As String = "") As Boolean
    ...
    '提取所有费用明细,供产生明细表用
    gstrSQL = "Select * From 病人费用记录 Where 结帐ID=" & lng冲销ID
    Call OpenRecordset(rs明细, "提取病人ID")
'    str发票号 = Nvl(rs明细!实际票号)
