...
Begin VB.Form frmIdentify黔南 
   ...
   Begin VB.Image img 
      Height          =   480
      Left            =   60
      Picture         =   "frmIdentify黔南.frx":0000
      Top             =   105
      Width           =   480
   End
   Begin VB.Label lbl 
   ...
   End
End
Attribute VB_Name = "frmIdentify黔南"
...
Private Function IsValid() As Boolean
    ...
    If 参保资格审查_黔南 = False Then
        Exit Function
    End If
 
    '住院状态判断 1 在住院  2 正常（非住院）  0 初始化值（同2）  4 转院
    If 获取住院状态_黔南(lng状态) = False Then
        Exit Function
    End If
    ...
    If mbytType <> 2 Then
        ...
        Else
            '检查病人状态
            gstrSQL = "select nvl(当前状态,0) as 状态 from 保险帐户 where 险类=" & TYPE_黔南 & " and 医保号='" & g病人身份_黔南.医保证号 & "'"
...
Private Sub cmd确定_Click()
    ...
    g病人身份_黔南.医疗类别 = Split(cbo类别.Text, "-")(0)
    If cbo审批类别.ListIndex >= 0 Then
        g病人身份_黔南.审批类别 = Split(cbo审批类别.Text, "-")(0)
    Else
        g病人身份_黔南.审批类别 = ""
    End If
    lng疾病ID = Val(txt病种.Tag)
    ...
    If lng疾病ID <> 0 And txt病种.Text <> "" Then
        g病人身份_黔南.病种编码 = Mid(txt病种.Text, 2, InStr(1, txt病种.Text, "]") - 2)
        g病人身份_黔南.病种名称 = Mid(txt病种.Text, InStr(1, txt病种.Text, "]") + 1)
    Else
        g病人身份_黔南.病种编码 = "000000"
    End If
    g病人身份_黔南.下个人帐户 = IIf(chk个人帐户.Value = 1, True, False)
    g病人身份_黔南.病种ID = lng疾病ID
    
    g病人身份_黔南.医疗类别 = Mid(cbo类别.Text, 1, InStr(1, cbo类别.Text, "-") - 1)
    int当前状态 = 0
    '确定病人是否慢病
    If (mbytType = 0 Or mbytType = 3) And g病人身份_黔南.医疗类别 = "13" Then
        If 获得通过的审批信息() = False Then Exit Sub
    End If
    
    If mbytType = 4 Then
        '需确定当前状态,因为当前状态是不能改变的
        gstrSQL = "Select * from 保险帐户 where 险类=" & TYPE_黔南 & " and  医保号='" & g病人身份_黔南.医保证号 & "'"
    ...
    mlng病人ID = BuildPatiInfo(0, strIdentify & strAddition, mlng病人ID, TYPE_黔南)
    ...
    If mbytType = 0 Or mbytType = 3 Then
        ...
        g病人身份_黔南.门诊号 = mlng病人ID & "_" & lng就诊次数
        '更新保险帐户
        gstrSQL = "zl_保险帐户_更新信息(" & mlng病人ID & "," & TYPE_黔南 & ",'就诊次数','" & lng就诊次数 & "')"
        ...
        '保存将交易流水号
        gstrSQL = "zl_保险帐户_更新信息(" & mlng病人ID & "," & TYPE_黔南 & ",'顺序号','''" & str交易流水号 & "''')"
        Call zlDatabase.ExecuteProcedure(gstrSQL, "保存交易流水号")
        If 更新就诊信息_黔南(0, strOutput) = False Then Exit Sub
    Else
...
'        If 业务请求_黔南(获得通过的审批信息, strInput, strOutput) = False Then Exit Function
...
Private Sub LoadCtrlData()
    ...
    gstrSQL = "Select 病种ID,密码 from 保险帐户 where 医保号='" & g病人身份_黔南.医保证号 & "' and 险类=" & TYPE_黔南
    zlDatabase.OpenRecordset rsTemp, gstrSQL, "获取相关病种"
    If rsTemp.EOF Then Exit Sub
    g病人身份_黔南.医疗类别 = Nvl(rsTemp!密码)
    ...
    For i = 0 To cbo类别.ListCount - 1
        If InStr(1, cbo类别.List(i), g病人身份_黔南.医疗类别 & "-") <> 0 Then
            cbo类别.ListIndex = i
            Exit For
        End If
    Next
