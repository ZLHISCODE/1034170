...
Public Function Save病情TO保险帐户() As Boolean
    ...
    If Val(txt病种(0).Tag) <> 0 Then
        gstrSQL = "select 编码,名称 From 医保病种目录 where id=" & Val(txt病种(0).Tag)
    ...
    If Val(txt病种(1).Tag) <> 0 Then
        gstrSQL = "select 编码,名称 From 医保病种目录 where id=" & Val(txt病种(1).Tag)
        OpenRecordset rsTemp, Me.Caption
    ...
    If Val(txt病种(2).Tag) <> 0 Then
        gstrSQL = "select 编码,名称 From 医保病种目录 where id=" & Val(txt病种(2).Tag)
        OpenRecordset rsTemp, Me.Caption
...
Public Function Load历史看病信息() As Boolean
    ...
    gstrSQL = "" & _
        ...
        "   From 保险帐户 a" & _
        "   where a.险类=" & TYPE_重庆渝北 & " and a.医保号='" & g病人身份_重庆渝北.个人编号 & "'"
    
    OpenRecordset rsTemp, "获取医保其他信息"
    ...
    If Val(txt病种(0).Tag) <> 0 Then
        ...
        gstrSQL = "select 编码,名称 From 医保病种目录 where id=" & lng病种ID
        OpenRecordset rsTemp, Me.Caption
