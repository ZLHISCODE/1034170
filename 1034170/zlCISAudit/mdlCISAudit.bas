...
Public Function ReturnParaData(ByVal lngSys As Long, ByVal str参数号IN As String, Optional lngModule As Long = 0, Optional ByVal strSplit As String = "") As ADODB.Recordset
    '-------------------------------------------------------------------------------------------
    '功能:获取指定的参数值,返回一个记录集
    '参数:lngSys-系统
    '     str参数号IN-参数号In,以逗号分离
    '     lngModule-模块号
    '     strSplit-在参数值的前面和后面加入特定附号
    '返回:参数记录集
    '编制:刘兴宏
    '日期:2007/12/17
    '-------------------------------------------------------------------------------------------
    ...
    strSQL = "" & _
        "   Select  /*+ Rule*/ 参数号," & _
            IIf(strSplit = "", "", "'" & strSplit & "'||") & "参数值" & IIf(strSplit = "", "", "||'" & strSplit & "'") & " as 参数值,参数名 ,参数说明 " & _
        "   From zlParameters A,Table(Cast(f_Num2List([2]) As zlTools.t_NumList)) B" & _
        "   where A.参数号 = B.Column_Value and a.系统=[1] and nvl(A.私有,0)=0  and nvl(a.模块,0)=[3] " & _
        "   order by 参数号"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "获取参数值", lngSys, str参数号IN, lngModule)
