...
Begin VB.Form frm病案接收参数 
   ...
   Begin VB.CheckBox chk打印 
      ...
      Width           =   3495
   End
   Begin VB.CheckBox chk显示 
      Caption         =   "不显示历史已编目未登记病案数据(&B)"
      Height          =   300
      Left            =   480
      TabIndex        =   4
      Top             =   1200
...
Private Function SaveSet() As Boolean
    ...
    If mlngModule = 201 Then
        Call zlDatabase.SetPara("不显示历史已编目未登记", IIf(chk显示.Value = 1, "1", "0"), glngSys, mlngModule)
    Else
...
Public Sub 参数设置(ByVal frmMain As Form, ByVal lngModule As Long, ByVal strPrivs As String)
    ...
    If mlngModule = 201 Then
        chk显示.Value = IIf(Val(zlDatabase.GetPara("不显示历史已编目未登记", glngSys, mlngModule, , Array(chk显示), mblnHavePriv)) = 1, 1, 0)
        Chk打印.Value = False
    Else
        chk显示.Value = False
