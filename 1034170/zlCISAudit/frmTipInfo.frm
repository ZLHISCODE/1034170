...
Public Sub ShowTipInfo(ByVal lngHwnd As Long, ByVal strInfo As String, Optional blnMultiRow As Boolean, Optional X As Long, Optional Y As Long)
'功能：显示或者隐藏提示
...
Private Sub timTip_Timer()
    ...
    If lngHwnd <> mlngHwnd Then
        Call HideMe
    ElseIf Abs(mlngX - vPos.X) > 5 Or Abs(mlngY - vPos.Y) > 5 Then
        Call HideMe
    Else
