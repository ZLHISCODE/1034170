...
Begin VB.Form frmChildQuestionEdit 
   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.Frame fra 
         ...
         Begin VB.CommandButton cmd 
            ...
            Index           =   0
            Left            =   3465
            Picture         =   "frmChildQuestionEdit.frx":0040
            Style           =   1  'Graphical
            TabIndex        =   5
            TabStop         =   0   'False
            Top             =   465
   ...
   Begin MSComctlLib.ImageList imgColor24 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmChildQuestionEdit.frx":6892
            Key             =   ""
         EndProperty
      EndProperty
   End
   Begin MSComctlLib.ImageList img41 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         NumListImages   =   1
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmChildQuestionEdit.frx":6F8C
            Key             =   ""
         EndProperty
      EndProperty
   End
...
'        cbo(2).Locked = True
    ...
    Case "显示文件"
        ...
        With cbo(1)
            ...
            If mstr文件id <> "" Then
                ...
                Case "护理记录"
                    Set rs = gclsPackage.GetEPRFileStruct(mstr文件id)
                Case "医嘱报告"
                    Set rs = gclsPackage.GetEPRFile(mstr文件id, mlng医嘱id)
                Case Else
                    If IsNumeric(mstr文件id) Then
                        Set rs = gclsPackage.GetEPRFile(mstr文件id)
                    Else
                        If Not gobjEmr Is Nothing Then
                            Set rs = gclsPackage.GetEMRFile(mstr文件id)
                        End If
                    End If
                End Select
    ...
    Case "读取数据"
        ...
        Set rs = gclsPackage.GetQuestion(mrsCondition, "", 1, Val(varParam(0)))
        If rs.BOF = False Then
...
On Error GoTo errH
    ...
    End If
    If strFileID = "0" Then
        gstrSQL = "Select /*+ rule */ '' as 图标,A.ID, A.分类ID,A.编码,A.适用对象 as 适用编码 ,A.适用环节,A.审查依据,A.名称, A.简码, A.分值,A.分制,b.名称 as 分类,decode(a.适用对象,1,'住院医嘱',2,'住院病历',3,'护理病历',4,'护理记录',5,'首页记录',6,'医嘱报告',7,'疾病证明',8,'知情文件','未定义') as 适用对象,a.说明,a.审查依据,适用对象 as 适用编码,文件ID,适用环节 From 病案审查目录  A ,病案审查分类 B,病案审查方案 C where  A.分类ID =B.id And B.方案ID =C.ID And C.启用时间 is Not Null And A.适用对象 = " & CStr(intObject)
    Else
        gstrSQL = "Select /*+ rule */ '' as 图标,A.ID, A.分类ID,A.编码 , A.适用对象 as 适用编码 ,A.适用环节,A.审查依据,A.名称,A.文件ID, A.简码, A.说明,A.分制,A.分值,b.名称 as 分类 From 病案审查目录 A ,病案审查分类 B,病案审查方案 C  where A.分类ID =B.id And B.方案ID =C.ID And C.启用时间 is Not Null And A.适用对象 = " & CStr(intObject) & " And (A.文件ID is null or instr(','|| A.文件ID || ',' , ','|| '" & strFileID & "' || ',')>0 )"
    End If
    If shortName <> "" Then
    ...
    If rsData.RecordCount > 0 Then
        ...
        If rsData.RecordCount = 1 Then
            '只有一条匹配直接填写
            rsData.MoveFirst
            If zlCheck.Connection_ChkRsState(rsData) Then Exit Sub
            txt(0).Text = zlCommFun.NVL(rsData!名称)
            cmd(0).Tag = zlCommFun.NVL(rsData!ID, 0)
            txt(0).Tag = ""
            usrSaveItem.反馈意见 = txt(0).Text
            
            If zlCommFun.NVL(rsData!分制, 0) = 0 Then
                '扣分制
                cbo(3).ListIndex = 0
                txt(4).Text = IIf(zlCommFun.NVL(rsData!分制) = "", 0, zlCommFun.NVL(rsData!分值, 0))
                mlng分值 = IIf(zlCommFun.NVL(rsData!分值) = "", 0, zlCommFun.NVL(rsData!分值, 0))
            Else
                '否决制
                cbo(3).ListIndex = 1
                txt(4).Text = 0
        ...
        Call frmAuditItemShow.zlInitData(rsData, Val(intObject), strFileID)
        Load frmAuditItemShow
        frmAuditItemShow.Show vbModal, Me
        If frmAuditItemShow.mlngOk Then
            '    Set rsData = zlDatabase.ShowSelect(Me, gstrSQL, 0)
            If zlCheck.Connection_ChkRsState(rsData) Then Exit Sub
            txt(0).Text = frmAuditItemShow.mstr名称
            cmd(0).Tag = frmAuditItemShow.mstrID
            txt(0).Tag = ""
            usrSaveItem.反馈意见 = txt(0).Text
            
            If zlCommFun.NVL(frmAuditItemShow.mstr分制, 0) = 0 Then
                '扣分制
                cbo(3).ListIndex = 0
                txt(4).Text = IIf(frmAuditItemShow.mstr分值 = "", 0, frmAuditItemShow.mstr分值)
                mlng分值 = IIf(frmAuditItemShow.mstr分值 = "", 0, frmAuditItemShow.mstr分值)
            Else
                '否决制
                cbo(3).ListIndex = 1
                txt(4).Text = 0
