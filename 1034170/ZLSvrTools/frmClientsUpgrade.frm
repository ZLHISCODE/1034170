...
Private mcllAllConn         As New Collection '所有的客户端信息，WinSock用来连接处理
Private marrCurConn         As Variant      '当前连接信息
...
Private Sub SetCtlEnabled()
    ...
    blnNoClients = True
    With vsClients
        bln应用 = (.Col = .ColIndex("服务器")) Or (.Col = .ColIndex("预升时点")) Or (.Col = .ColIndex("预升完成") Or (.Col = .ColIndex("部件检查")))
        For i = 1 To .Rows - 1
...
Private Sub LoadClientsInfor(Optional blnRefresh As Boolean)
    ...
    strSQL = "Select a.工作站, a.Ip, a.部门, zlSpellCode(a.部门) As 部门简码, a.用途, a.Cpu, a.内存, a.硬盘, a.操作系统, a.升级服务器, a.Ftp服务器," & vbNewLine & _
            "       To_Char(a.预升时点, 'hh24:mi') 预升时点, a.升级标志, a.收集标志," & vbNewLine & _
            "       Decode(a.升级情况, 0, '未升级', 1, '完成', 2, '失败', 3, '正在升级', Decode(Nvl(a.升级标志, 0), 0, Null, '未升级')) 升级情况," & vbNewLine & _
            "       Decode(a.预升完成, 0, '未升级', 1, '完成', 2, '失败', 3, '正在升级', Decode(Nvl(a.升级标志, 0), 0, Null, '未升级')) 预升情况," & vbNewLine & _
            "       Decode(a.收集状态, 0, '未检查', 1, '环境正常', 2, '环境异常', 3, '正在检查', Decode(Nvl(a.收集标志, 0), 0, Null, '未检查')) 检查情况," & vbNewLine & _
            "       a.升级情况 As 升级状态, a.预升完成, a.收集状态, a.说明, a.禁止使用, a.连接数, a.管理员用户, a.管理员密码, Decode(c.Terminal, Null, 0, 1) As 状态, d.参数值 端口" & vbNewLine & _
            "From zlClients A, (Select Distinct Terminal From GV$session) C," & vbNewLine & _
            "     (Select 机器名, Nvl(b.参数值, 0) 参数值" & vbNewLine & _
            "       From zlParameters A, zlUserParas B" & vbNewLine & _
            "       Where a.参数名 = '允许远程控制' And Nvl(a.模块, 0) = 0 And Nvl(a.系统, 0) = 0 And a.Id = b.参数id) D" & vbNewLine & _
            "Where Upper(a.工作站) = Upper(c.Terminal(+)) And a.工作站 = d.机器名(+)" & vbNewLine & _
            "Order By Ip"
...
'            strKey = IIf(Val(strKey) = 0, "", strKey)
            ...
            If mrsFilePreUpgrade.EOF = False Then
                .TextMatrix(i, .ColIndex("预升时点")) = Nvl(mrsClients!预升时点)
            Else
            ...
            .TextMatrix(i, .ColIndex("检查结果")) = Nvl(mrsClients!检查情况)
            .TextMatrix(i, .ColIndex("管理员")) = Nvl(mrsClients!管理员用户)
            .TextMatrix(i, .ColIndex("密码")) = Decipher(Nvl(mrsClients!管理员密码))
            .TextMatrix(i, .ColIndex("状态")) = Nvl(mrsClients!状态)
            .TextMatrix(i, .ColIndex("端口")) = Nvl(mrsClients!端口)
            ...
            If Val(Nvl(mrsClients!收集标志)) = 1 Then
                .Cell(flexcpChecked, i, .ColIndex("部件检查")) = flexChecked
            Else
                .Cell(flexcpChecked, i, .ColIndex("部件检查")) = flexUnchecked
            End If
            i = i + 1
            mrsClients.MoveNext
        Loop
...
Private Function SaveData() As Boolean
    ...
    Dim int预升完成 As Integer, str部件检查 As String
    Dim strIp As String
    ...
    timerConnect.Enabled = False '先暂停
    With vsClients
        For i = 1 To .Rows - 1
            If Trim(.TextMatrix(i, .ColIndex("工作站"))) <> "" Then
                ...
                Else
                    ...
                    End If
                End If
                str部件检查 = IIf(.Cell(flexcpChecked, i, .ColIndex("部件检查")) = flexChecked, 1, "NULL")
                
                If .TextMatrix(i, .ColIndex("状态")) = "1" Then
                    ...
                    End If
                    strJobs = IIf(int升级 = 0, "", 1) & "," & IIf(str部件检查 = "1", str部件检查, "")
                ...
                End If
                If mintUpType = 0 Then
                    strSQL = "Zl_Zlclients_Control(2,Null,'" & strIp & "'," & int升级 & "," & str服务器号 & "," & str预升时点 & "," & int预升完成 & ",NULL," & str部件检查 & ")"
                Else
                    strSQL = "Zl_Zlclients_Control(2,Null,'" & strIp & "'," & int升级 & ",Null," & str预升时点 & "," & int预升完成 & "," & str服务器号 & "," & str部件检查 & ")"
                End If
                Call ExecuteProcedure(strSQL, Me.Caption)
...
Private Sub timerConnect_Timer()
    ...
    If mlngConnTimes >= M_EXPIRED_TIMES Then
        ...
        mlngConnTimes = 1
    End If
    If mlngConnTimes = 1 And winSock.State = sckClosed And mcllAllConn.Count <> 0 Then '开始发送消息
...
Private Sub vsClients_BeforeEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    Select Case Col
    Case vsClients.ColIndex("服务器"), vsClients.ColIndex("升级"), vsClients.ColIndex("预升时点"), vsClients.ColIndex("预升完成"), vsClients.ColIndex("部件检查")
        '只有服务器列和升级列才能更改
    Case Else
        '其他列不能更改
        Cancel = True
...
Private Sub vsClients_DblClick()
    ...
    If vsClients.Row > 0 Then
        ...
        intType = -1
        If vsClients.Col = vsClients.ColIndex("部件检查") And vsClients.TextMatrix(vsClients.Row, vsClients.ColIndex("部件检查")) = "需升级" Then
            intType = 1
        ElseIf vsClients.TextMatrix(vsClients.Row, vsClients.ColIndex("升级情况")) <> "未升级" And vsClients.Col = vsClients.ColIndex("升级情况") Then
            intType = 0
        ElseIf vsClients.TextMatrix(vsClients.Row, vsClients.ColIndex("升级情况")) <> "未升级" Then
            intType = 0
        ElseIf vsClients.TextMatrix(vsClients.Row, vsClients.ColIndex("部件检查")) = "需升级" Then
            intType = 1
        End If
        If intType <> -1 Then
...
Private Sub winSock_DataArrival(ByVal bytesTotal As Long)
    ...
    If strData = "MESSAGE:" & marrCurConn(2) & ",STATE:1" Then
        mlngConnTimes = M_EXPIRED_TIMES '标志已经处理
    End If
End Sub
