...
Public Function GetSequence(ByVal str所有者 As String, cnOwner As ADODB.Connection, Optional blnCurrUser As Boolean, Optional strTables As String) As ADODB.Recordset
    ...
    If strTables <> "" Then
        strTemp = " And s.Table_Name In(" & strTables & ") "
    End If
    ...
    If blnCurrUser Then
        strSQL = "Select User, Decode(s.Sequence_Name, '病人费用记录_ID', '病人费用记录', s.Table_Name) as Table_Name, c.Column_Name, s.Sequence_Name" & vbNewLine & _
                ...
                "Where c.Table_Name = s.Table_Name And c.Column_Name = s.Column_Name" & strTemp & vbNewLine & _
    ...
    Else
        ...
        strSQL = "Select c.Owner, Decode(s.Sequence_Name, '病人费用记录_ID', '病人费用记录', s.Table_Name) as Table_Name, c.Column_Name, s.Sequence_Name" & vbNewLine & _
                ...
                "Where c.Table_Name = s.Table_Name And c.Column_Name = s.Column_Name And c.Owner = s.Sequence_Owner" & strTemp & vbNewLine & _
...
Public Sub AdjustNameSequece(ByVal strTable As String, cnOwner As ADODB.Connection, Optional ByVal strColumn As String = "ID")
'功能：整理序列的当前号码
...
'      strColumn 表中对应的序列字段名,一般为ID,可以为指定的其他字段
    ...
    If Not rsVal.EOF Then
        If Not IsNull(rsVal!MAXID) Then
            If IsType(rsVal.Fields("MAXID").type, adNumeric) Then
                dblTableID = CDbl(rsVal!MAXID)
            ElseIf IsNumeric(rsVal!MAXID & "") Then
                dblTableID = CDbl(rsVal!MAXID)
            Else
                Exit Sub
            End If
...
Public Function CheckSpaceIsUse(ByVal strType As String, ByVal strName As String, ByVal strOwner As String, Optional lngSysNum As Long) As Boolean
...
'      strOwner         以区别其它用户的所有者名
    ...
    On Error Resume Next
    If strType = "表空间" Then
        gstrSQL = "Select Distinct 系统 From Zltables Where 表空间 = '" & strName & "' And 系统 <> '" & lngSysNum & "'"
        rsTemp.Open gstrSQL, gcnOracle, , adLockReadOnly
        ...
        '可能没有zltables表
        Else
            '标准版和lis都有ZLMSGDATA表空间
            If strName = "ZLMSGDATA" And (lngSysNum = 100 Or lngSysNum = 2500) Then
                gstrSQL = "select count(*) from Zlsystems where 编号 in(100,2500)"
                rsTemp.Open gstrSQL, gcnOracle, , adLockReadOnly
...
'功能: 将超过指定长度字符串,转换成数组
    ...
    ReDim arrStr(0)
    Do While Len(strTxt) > intLength And Len(strDeli) = 1 And InStr(1, strTxt, strDeli) > 0 '仅支持单字符分隔符
        arrStr(UBound(arrStr)) = Left(strTxt, InStrRev(strTxt, strDeli, intLength) - 1)
        strTxt = Mid(strTxt, Len(arrStr(UBound(arrStr))) + Len(strDeli) + 1)
        If strTxt <> "" Then ReDim Preserve arrStr(UBound(arrStr) + 1)
    Loop
    If strTxt <> "" Then arrStr(UBound(arrStr)) = strTxt
...
Public Sub UnInstall(ByVal frmMain As Form, ByVal blnShare As Boolean, ByRef stbThis As StatusBar, ByVal strUser As String, ByVal strSpace As String, _
                     ByVal lngSame As Long, Optional ByVal lngSys As Long)
    '功能:管理工具创建失败、系统安装失败和拆卸系统时清理已安装的数据
    '参数:blnShare:True-共享系统,False-非共享系统;strUser-用户名;strSpace-表空间;
    '     lngSys-系统编号
    Dim strSQL As String
    Dim i As Long
    Dim strTemp As String
    ...
    Dim rsTemp As New ADODB.Recordset
    Dim blnHaveZLTalbe  As Boolean
    
    strUser = Trim(UCase(strUser))
    strSpace = UCase(strSpace)
    If blnShare = False Then
        stbThis.Panels(2).Text = "正在删除已创建的用户..."
        On Error Resume Next
        gcnOracle.Execute "Drop User " & strUser & " Cascade"
        If InStr(err.Description, "ORA-01918:") > 0 Then
            'ORA-01918: 用户 'ZLTOOLS' 不存在
            err.Clear
        End If
    End If
    On Error GoTo errH
    If lngSame = 0 Then
        stbThis.Panels(2).Text = "正在删除已创建的表空间和相关约束..."
        If blnShare Then
            strSQL = "Select b.Table_Name, b.Constraint_Name, b.Constraint_Type" & vbNewLine & _
                    "From (Select 表名 From Zltables Where 系统 = [2]) a," & vbNewLine & _
                    "     (Select Constraint_Name, Table_Name, Constraint_Type, Delete_Rule From Dba_Constraints Where Owner = [1]) b" & vbNewLine & _
                    "Where a.表名 = b.Table_Name And b.Constraint_Type <> 'C'"
            If rsTemp.State = adStateOpen Then rsTemp.Close
            Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, frmMain.Caption, strUser, lngSys)
            '先删除外键
            With rsTemp
                .Filter = "Constraint_Type='R'"
                Do While Not .EOF
                    strSQL = "Alter Table " & !Table_Name & " Drop Constraint " & !Constraint_Name
                    gcnOracle.Execute strSQL
                    .MoveNext
                Loop
                .Filter = "Constraint_Type<>'R'"
                Do While Not .EOF
                    strSQL = "Alter Table " & !Table_Name & " Drop Constraint " & !Constraint_Name & " Cascade Drop Index"
                    gcnOracle.Execute strSQL
                    .MoveNext
                Loop
            End With
        End If
        varTemp = Split(strSpace, ",")
        For i = 0 To UBound(varTemp)
'            If CheckSpaceIsUse("表空间", varTemp(i), strUser, lngSys) = False Then
'                If UCase(varTemp(i)) = "ZLTOOLSTMP" Then 'ORA-03217: 变更 TEMPORARY TABLESPACE 无效的选项
'                    On Error Resume Next
'                End If
'                gcnOracle.Execute "Alter Tablespace " & varTemp(i) & " Offline"
'                If UCase(varTemp(i)) = "ZLTOOLSTMP" Then 'ORA-03217: 变更 TEMPORARY TABLESPACE 无效的选项
'                    If err.Number <> 0 Then err.Clear
'                    On Error GoTo errH
'                End If
'                gcnOracle.Execute "Drop Tablespace " & varTemp(i) & " Including Contents And Datafiles Cascade Constraints"
'            End If
            strSpa = IIf(strSpa = "", "'" & varTemp(i) & "'", strSpa & "," & "'" & varTemp(i) & "'")
        Next
    End If
    '管理工具只删除相应表，应用系统删除zlMandata.sql中的数据
    stbThis.Panels(2).Text = "正在删除已安装数据…"
    If strUser <> "ZLTOOLS" Then
        gcnOracle.BeginTrans: BeginTrans = True
        strTemp = "Zlbigtables,zlTables"
        varTemp = Split(strTemp, ",")
        For i = 0 To UBound(varTemp)
            strSQL = "Delete From " & varTemp(i) & " Where 系统=" & lngSys
            gcnOracle.Execute strSQL
        Next
        strSQL = "Delete From zlSystems Where 编号=" & lngSys + lngSame
        gcnOracle.Execute strSQL
        gcnOracle.CommitTrans: BeginTrans = False
        Call frmHistorySpaceSet.ShowInstall(frmMain, gcnOracle, gstrUserName, gstrPassword, lngSys, 1, 0)
    End If
    '删除表、索引
    If strSpa <> "" Then
        strSpa = " And a.Tablespace_Name Not In(" & strSpa & ")"
    End If
    strSQL = "Select Count(1) 计数 From All_Tables A Where a.Owner = 'ZLTOOLS' And a.Table_Name = 'ZLTABLES'"
    Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, frmMain.Caption, lngSys, strUser)
    If rsTemp!计数 = 0 Then
        blnHaveZLTalbe = False
    Else
        blnHaveZLTalbe = True
        strSQL = "Select 'Drop Table ' || b.表名 as SQL" & vbNewLine & _
            "From Dba_Tables a, Zltables b" & vbNewLine & _
            "Where a.Owner = [2] And a.Table_Name = b.表名 And b.系统 = [1] And Not Exists" & vbNewLine & _
            " (Select 1 From Zltables Where 系统 <> [1] And 表名 = b.表名)" & strSpa & vbNewLine & _
            "Union All" & vbNewLine & _
            "Select 'Drop Index ' || a.Index_Name as SQL" & vbNewLine & _
            "From Dba_Indexes a, Zltables b" & vbNewLine & _
            "Where a.Owner = [2] And a.Table_Name = b.表名 And b.系统 = [1] And Not Exists" & vbNewLine & _
            " (Select 1 From Zltables Where 系统 <> [1] And 表名 = b.表名)" & strSpa
         Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, frmMain.Caption, lngSys, strUser)
    End If
    If blnHaveZLTalbe Then
        With rsTemp
            Do While Not .EOF
                DoEvents
                gcnOracle.Execute !SQL
                .MoveNext
            Loop
        End With
    End If
    ...
    strTemp = ""
    If blnHaveZLTalbe Then
        strSQL = "Select 表名 From Zltables a Where 系统 = [1] And Not Exists (Select 1 From Zltables Where 系统 <> [1] And 表名 = a.表名)"
        If rsTemp.State = adStateOpen Then rsTemp.Close
        Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, frmMain.Caption, lngSys)
        With rsTemp
            Do While Not .EOF
                strTemp = IIf(strTemp = "", "'" & !表名 & "'", strTemp & ",'" & !表名 & "'")
                .MoveNext
            Loop
        End With
    End If
    Set rsTemp = GetSequence(strUser, gcnOracle, gblnDBA, strTemp)
    With rsTemp
        Do While Not .EOF
            DoEvents
            strSQL = "Drop Sequence " & rsTemp!sequence_name
            gcnOracle.Execute strSQL
            .MoveNext
        Loop
    End With
...
Public Function CheckSegmentDefer()
    Dim strSQL As String, rsTmp As New ADODB.Recordset
    
    On Error GoTo errH
    
    strSQL = "Select Value From v$parameter Where Name = 'deferred_segment_creation'"
    Set rsTmp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, "检查表段延时创建")
    
    If rsTmp.RecordCount = 0 Then Exit Function
    CheckSegmentDefer = UCase(rsTmp!value) = "TRUE"
    
    Exit Function
errH:
    MsgBox "检查表段延时创建发生错误" & vbNewLine & err.Description, vbCritical, "错误"
End Function
