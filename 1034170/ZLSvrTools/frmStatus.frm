...
Private mfrmChild As frmHistSql
...
Private Sub LoadSession()
    ...
    With rsTemp
        ...
        Do While Not .EOF
            If strSID <> .Fields("AUDSID").value Then
                ...
                Case "INACTIVE"
                    objItem.SubItems(9) = "正常"
                Case Else
                    objItem.SubItems(9) = "其他"
                End Select
                If gblnRac Then
    ...
    With lvw上机用户
        .ColumnHeaders(11).Width = IIf(gblnRac, 615, 0)
...
Private Sub Form_Resize()
    ...
    With lvw上机用户
        ...
        If gblnDBA Then
            ...
            pctHorLine.Top = .Top + .Height
            pctHorLine.Width = .Width
            pctHorLine.Left = .Left
        Else
...
Private Sub pctHorLine_MouseMove(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    '防止拖动过度，导致界面异常
    If pctHorLine.Top + y < 2000 Then Exit Sub
    If pctHorLine.Top + y > Me.ScaleHeight - 1000 Then Exit Sub
    
    intOldHeight = lvw上机用户.Height
    pctHorLine.Top = pctHorLine.Top + y
    lvw上机用户.Height = Abs(pctHorLine.Top - lvw上机用户.Top)
    With lvw上机用户
        lblCon.Left = .Left
        lblCon.Top = .Top + .Height + 100
        lblConCount.Left = lblCon.Left + 700
    ...
    tabMore.Top = pctHorLine.Top + 400
    tabMore.Height = Me.ScaleHeight - tabMore.Top - 200
    Me.Refresh
