...
Private Function CheckGrant() As String
'功能:检查当前用户是否有特定系统表、视图的转授权限
'返回:缺失转授权限的对象名称
    Dim strSql As String, rsTmp As New ADODB.Recordset
    Dim strObj As String, blnQuery As Boolean
    Dim arrObj() As String, strResult As String, i As Integer
    
    On Error GoTo errH
    
    If gblnDBA Then
        'O7参数为True,DBA用户拥有系统视图的转授权限
        strSql = "Select 1 From v$parameter Where Name='O7_DICTIONARY_ACCESSIBILITY' And Value ='TRUE'"
        Set rsTmp = gclsBase.OpenSQLRecord(gcnOracle, strSql, "检查数据库O7参数")
   ...
   Else
    ...
    If Not blnQuery Then Exit Function
    
    strObj = "GV_$SESSION,V_$SESSION,V_$PARAMETER,V_$PROCESS"
    arrObj = Split(strObj, ",")
    strSql = "Select Table_Name, Grantable" & vbNewLine & _
                    "From User_Tab_Privs" & vbNewLine & _
                    "Where Table_Name In" & vbNewLine & _
                    "      (Select * From Table(f_Str2list('" & strObj & "', ','))) And" & vbNewLine & _
                    "      Grantable = 'YES'"
    Set rsTmp = gclsBase.OpenSQLRecord(gcnOracle, strSql, "查询权限")
    
    If rsTmp.RecordCount <> 0 Then
        For i = 0 To UBound(arrObj)
            rsTmp.Filter = "Table_Name = '" & arrObj(i) & "'"
            
            If rsTmp.RecordCount = 0 Then
                strResult = strResult & IIf(strResult = "", "", "、") & arrObj(i)
            End If
        Next
    End If
    
    If strResult <> "" Then
        strResult = "当前用户缺失以下对象的转授权限：" & strResult
    End If
    CheckGrant = strResult
    Exit Function
errH:
    CheckGrant = "获取用户系统权限出现错误。"
End Function
