...
Public Function CopyFileToTargetServer(ByVal strSourcePath As String, ByVal strTargetPath As String, _
    ...
    With FileList
        ...
        For i = 0 To .ListCount - 1
            ...
            Else
                If objFile.FileExists(strTarGetFile) Then
                    If objFile.GetFile(strSourceFile).Size <> objFile.GetFile(strTarGetFile).Size Then
                        If intCount Mod 3 = 0 And intCount <> 0 Then
                            strErrFile = strErrFile & "," & vbNewLine & .List(i)
                        Else
                            strErrFile = strErrFile & "," & .List(i)
                        End If
                        intCount = intCount + 1
                    End If
                End If
        ...
        Next
        If strErrFile <> "" Then
            MsgBox "如下文件本地和上传到服务器上的文件存在大小差异，" & Mid(strErrFile, 2) & ",请仔细检查！", vbInformation, gstrSysName
        End If
