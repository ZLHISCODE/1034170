...
Begin VB.Form frmScriptEdit 
   ...
   Begin VB.CommandButton cmd 
      ...
      Index           =   0
      Left            =   7425
      Picture         =   "frmScriptEdit.frx":3AAE2
      Style           =   1  'Graphical
      TabIndex        =   2
      TabStop         =   0   'False
      ToolTipText     =   "选择文件"
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfCom 
      ...
      FormatString    =   $"frmScriptEdit.frx":41334
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
'    Set rs = gmobjCommon.zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
     Call OpenRecordset(rs, strSQL, Me.Caption)
...
'            Set rsMaxID = gmobjCommon.zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
...
'            gcnOracle.BeginTrans
            ...
            ",修改日期=" & IIf(IsDate(CStr(m_strEditDate)), "to_date('" & CStr(dateEdit) & "','yyyy-mm-dd hh24:mi:ss')", "''") & ",文件说明='" & str文件说明 & "',强制覆盖=" & byt强制覆盖 & ",自动注册=" & byt自动注册 & " where upper(文件名)='" & strName & "'"
...
'        gcnOracle.BeginTrans
        strSQL = "insert into zlFilesUpgrade (序号,文件类型,文件名,版本号,修改日期,业务部件,所属系统,安装路径,文件说明,强制覆盖,自动注册) values (" & strMax序号 + 1 & "," & lngTypeNum & "," & _
        "'" & strName & "','" & strVision & "',to_date('" & CStr(dateEdit) & "','yyyy-mm-dd hh24:mi:ss'),'" & str引用文件 & "','" & str所属系统 & "','" & strPath & "','" & str文件说明 & "'," & byt强制覆盖 & " ," & byt自动注册 & ")"
...
Private Function CheckFile(ByVal strFile As String) As Boolean
    ...
    strSQL = "select 所属系统,业务部件,安装路径,文件说明,1 from zlFilesUpgrade where upper(文件名) = upper('" & strFile & "')"
'    Set rs = gmobjCommon.zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strFile)
    Call OpenRecordset(rs, strSQL, Me.Caption)
