...
Public grsTableInfor As ADODB.Recordset
'变量定义
...
'初始化本地需检查路径的记录集
    ...
    With IniFilePathRecordset
        ...
        .Fields.Append "SystemName", adVarChar, 50, adFldIsNullable
...
'功能：初始化解析本地SQL文件数据保存的记录集
    ...
    With InitProDataRecordset
        ...
        .Fields.Append "约束字段", adVarChar, 100, adFldIsNullable
        .CursorLocation = adUseClient
        .LockType = adLockOptimistic
        .CursorType = adOpenStatic
        .Open
    ...
    While Not mrsFildFromFile.EOF
        ...
        If mrsFildFromDB.RecordCount = 0 Then
            ...
            While strTableName = mrsFildFromFile!表名
                ...
                strSQL = IIf(strSQL = "", "", strSQL & "," & vbNewLine) & strFild
                DoEvents
                mrsFildFromFile.MoveNext
                lngProgress = lngProgress + 1
            Wend
            grsTableInfor.Filter = "系统编号=" & mlngNum & " And 表名='" & strTableName & "'"
            If grsTableInfor.RecordCount > 0 Then
                strSQL = "CREATE" & IIf(grsTableInfor!临时表 = "是", " GLOBAL TEMPORARY", "") & " TABLE " & strTableName & "(" & vbNewLine & strSQL & ")" & vbNewLine & grsTableInfor!表信息
                mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名"), Array(mstrSysName, strSQL, "表", "严重", _
...
'检查约束
    ...
    For i = 1 To mrsConstraintFromFile.RecordCount
        ...
        If mrsConstraintFromDB.RecordCount = 0 Then
            mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名", "约束字段"), Array(mstrSysName, mrsConstraintFromFile!SQL, "约束", "较重", _
                "数据库中该约束不存在，可能影响产品相关功能的正常使用", "添加该约束", strName, mrsConstraintFromFile!字段)
        Else
            ...
            If mrsConstraintFromDB!字段 <> mrsConstraintFromFile!字段 Then
                ...
                Else
                    ...
                    mrsProData.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名", "约束字段"), Array(mstrSysName, strSQL, "约束", "较重", _
                        "数据库中该约束列(" & mrsConstraintFromDB!字段 & ")与标准产品(" & mrsConstraintFromFile!字段 & ")不一致，可能影响产品相关查询性能", "删除该约束后再重建约束", strName, mrsConstraintFromFile!字段)
                End If
            End If
        End If
        DoEvents
    ...
    For i = 1 To mrsIndexFromFile.RecordCount
        ...
        Else
            If mrsIndexFromDB!Status <> "VALID" Then
                strSQL = "Alter Index " & strName & " rebuild nologging"
                            ...
                            Else
                                If mblnIndex Then
                                    ...
                                    If blnSpace = False And IsNull(mrsIndexFromDB!表空间) = False And strSpace <> "" And mrsIndexFromDB!表名 <> "输血性质" Then
                                        strSQL = "Alter Index " & strName & " rebuild tablespace " & strSpace & " nologging"
