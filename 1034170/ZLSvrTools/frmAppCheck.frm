...
Private Sub cmdFunction_Click(Index As Integer)
    ...
    Select Case Index
        Case 0
            ...
            lblStatus.Caption = "正在检查ZTOOLS之外用户中的ZLTOOLS对象..."
            Call CheckOwnerObject(rsProData)
            lblStatus.Caption = "正在检查全局临时表..."
            Call CheckTemporaryTable(rsProData)
            picStatus.Visible = False
            rsProData.Filter = ""
            frmMDIMain.Enabled = True
            Me.Enabled = True
            If rsProData.RecordCount > 0 Then
                Call frmAppChkRpt.ShowMe(1, rsProData, lblMainPath.Tag, mrsDataFromFile)
            Else
                MsgBox "未检查出需修复的对象！"
            End If
            Call Release
        ...
        Case 3
            ...
            Set rsTemp = GetSequence("", gcnOracle)
            With rsTemp
                Do Until .EOF
                    lblStatus.Caption = "正在修正序列：" & !sequence_name
                    pgbState.value = .AbsolutePosition / .RecordCount * 100: DoEvents
                    Call AdjustNameSequece(!Owner & "." & !Table_Name, gcnOracle, !Column_Name)
                    .MoveNext
                Loop
            ...
            MsgBox "序列修正完毕！", vbInformation, gstrSysName
        Case 4
            Call ShowFlash("正在收集和检查数据库过程,请稍候！")
            Set rsTemp = Nothing
            Set rsTemp = CheckProcedure
            Call ShowFlash
            If Not rsTemp Is Nothing Then
                Call frmAppChkRpt.ShowMe(0, rsTemp)
            End If
    End Select
End Sub

Private Sub CheckTemporaryTable(ByRef rsResult As ADODB.Recordset)
'检查脚本中的全局临时表在数据库中类型是否一致
'说明：和管理工具是否勾选有关
    Dim rsTemp As ADODB.Recordset
    Dim strSQL As String

    grsTableInfor.Filter = "临时表='是'"
    Do While Not grsTableInfor.EOF
        gstrSQL = "Select a.Temporary From User_Tables a Where a.Table_Name =[1]"
        Set rsTemp = gclsBase.OpenSQLRecord(IIf(grsTableInfor!系统名称 = "服务器管理工具", gcnTools, gcnOracle), gstrSQL, "获取表信息", grsTableInfor!表名)
        Do While Not rsTemp.EOF
            If rsTemp!Temporary = "N" Then
                strSQL = GetComTable(grsTableInfor!表名, grsTableInfor!系统编号, grsTableInfor!表信息, True)
                strSQL = "Drop Table " & grsTableInfor!表名 & "{JM|SQL分隔符}" & vbNewLine & strSQL
                rsResult.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名"), Array(grsTableInfor!系统名称, strSQL, "表", "较重", _
                    "脚本中为全局临时表，而数据库为常规表", "删除表再重新创建", grsTableInfor!表名)
            End If
            rsTemp.MoveNext
        Loop
        grsTableInfor.MoveNext
    Loop
End Sub

Private Function GetComTable(ByVal strTable As String, ByVal strSys As String, ByVal strTableInfo As String, Optional blnTempTable As Boolean) As String
'根据字段获取创建表的完整SQL
    Dim strTemp As String, strFild As String

    mrsFildFromFile.Filter = "系统编号=" & strSys & " And 表名='" & strTable & "'"
    Do While Not mrsFildFromFile.EOF
        strFild = mrsFildFromFile!字段 & " " & mrsFildFromFile!字段类型 & IIf("" & mrsFildFromFile!字段长度 = "", "", "(" & mrsFildFromFile!字段长度 & ")") & _
            IIf("" & mrsFildFromFile!字段信息 = "", "", " " & mrsFildFromFile!字段信息)
        strTemp = IIf(strTemp = "", "", strTemp & "," & vbNewLine) & strFild
        mrsFildFromFile.MoveNext
    Loop
    strTemp = "CREATE" & IIf(blnTempTable = True, " GLOBAL TEMPORARY", "") & " TABLE " & grsTableInfor!表名 & "(" & vbNewLine & strTemp & ")" & vbNewLine & strTableInfo
    GetComTable = strTemp
End Function

Private Sub CheckOwnerObject(ByRef rsResult As ADODB.Recordset)
'检查系统所有者的对象在ZLTOOLS中是否存在,包括：同名的表、公共同义词、索引、约束、序列、存储过程与函数、包、视图、触发器等对象
'说明：和勾选系统无关
    Dim rsTemp As ADODB.Recordset
    
    gstrSQL = "Select a.Owner, a.Object_Name, a.Object_Type, 'Drop ' || a.Object_Type || ' ' || a.Owner || '.' || a.Object_Name 修正sql" & vbNewLine & _
            "From All_Objects a" & vbNewLine & _
            "Where a.Owner Not In ('ZLTOOLS', 'PUBLIC') And" & vbNewLine & _
            "      a.Object_Type In ('TABLE', 'SEQUENCE', 'PROCEDURE', 'FUNCTION', 'VIEW', 'TYPE', 'PACKAGE', 'TRIGGER') And" & vbNewLine & _
            "      (a.Object_Name, a.Object_Type) In" & vbNewLine & _
            "      (Select b.Object_Name, b.Object_Type" & vbNewLine & _
            "       From All_Objects b" & vbNewLine & _
            "       Where b.Owner = 'ZLTOOLS' And b.Object_Name <> 'ZL_ERRORCENTER')" & vbNewLine & _
            "Order By Owner, Object_Type, Object_Name"

    Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, gstrSQL, "对象获取")
    Do While Not rsTemp.EOF
        rsResult.AddNew Array("系统名称", "修正SQL", "类别", "严重程度", "问题描述", "修正说明", "对象名"), _
            Array(rsTemp!Owner, rsTemp!修正SQL, "ZLTOOL对象", "轻微", "该ZLTOOLS对象在其他用户中存在", "删除ZLTOOLS以外用户下的该对象", rsTemp!Object_Name)
        rsTemp.MoveNext
    Loop
End Sub

Private Function CheckProcedure() As ADODB.Recordset
'检查数据库过程规范性
    Dim strSQL As String
    Dim rsTemp As ADODB.Recordset
    ...
    strPro = ",ZL1_AUTOCPTONE,ZL_病人费用汇总_INSERT,ZL_药品收发汇总_INSERT,"  '需排除的固定的过程
    If CheckUser = False Then Exit Function

    Set rsProcedure = GetProcedure

    Call setControlVisible(1)
    If Not rsProcedure Is Nothing Then
        If rsProcedure.RecordCount > 0 Then
            rsProcedure.MoveFirst
            Set rsTemp = CopyNewRec(Nothing, True, , Array("过程名称", adVarChar, 50, Empty, "问题描述", adVarChar, 100, Empty, "原始SQL", adVarChar, 20000, Empty))
        End If
        DoEvents
        Do While Not rsProcedure.EOF
            lblProgress.Caption = "正在检查数据库过程：" & rsProcedure!过程名称
            lblProgress.Refresh
            strSQL = UCase(rsProcedure!原始SQL)
            If InStr(strPro, "," & rsProcedure!过程名称 & ",") = 0 And Not rsProcedure!过程名称 Like "ZL*_DATAMOVE*" Then
                If InStr(strSQL, "PRAGMA AUTONOMOUS_TRANSACTION") = 0 And InStr(strSQL, " COMMIT;") > 0 Then
                    rsTemp.AddNew Array("过程名称", "问题描述", "原始SQL"), Array(rsProcedure!过程名称, "非自治事务的过程或函数中使用了Commit,请检查是否属于因批量数据处理而需要进行分批提交的情况", rsProcedure!原始SQL)
                End If
            End If
            If InStr(strSQL, "ZL_ERRORCENTER") = 0 And InStr(strSQL, rsProcedure!过程名称 & " WRAPPED") = 0 Then
                rsTemp.AddNew Array("过程名称", "问题描述", "原始SQL"), Array(rsProcedure!过程名称, "过程中没有调用错误处理中心 (zl_ErrorCenter)", rsProcedure!原始SQL)
            End If
            '此过程故意不用的，特意排除
            If Not rsProcedure!过程名称 Like "ZL*_DATAMOVEOUT1" Then
                varTemp = Split(strSQL, "EXECUTE IMMEDIATE")
                If UBound(varTemp) > 1 Then
                    For i = 1 To UBound(varTemp)
                        ...
                        Else
                            If InStr(strTemp, " ") > 0 Then
                                strVsql = Mid(strTemp, 1, InStr(strTemp, " ") - 1)
                            Else
                                strVsql = strTemp
                            End If
                            strExeSQl = Mid(varTemp(i - 1), 1, InStrRev(varTemp(i - 1), ";") - 1)
                            If InStr(strExeSQl, ";") > 0 Then strExeSQl = Mid(strExeSQl, InStrRev(strExeSQl, ";") + 1)
                            strExeSQl = Replace(strExeSQl, " ", "")
                            If InStr(strExeSQl, strVsql & ":=") > 0 Then
                                strExeSQl = Mid(strExeSQl, InStr(strExeSQl, strVsql & ":="))
                                strVarName = strTemp
                            End If
                        End If
                        If strVarName <> "" Then
                            If InStr(strExeSQl, "WHERE") > 0 Then
                                strExeSQl = Mid(strExeSQl, InStr(strExeSQl, "WHERE"))
                                If InStr(strExeSQl, "||") > 0 And InStr(strVarName, "USING") = 0 Then
                                    rsTemp.Filter = "过程名称='" & rsProcedure!过程名称 & "'"
                                    If rsTemp.RecordCount = 0 Then
                                        rsTemp.AddNew Array("过程名称", "问题描述", "原始SQL"), Array(rsProcedure!过程名称, "动态执行的SQL，没有使用绑定变量", rsProcedure!原始SQL)
                                    End If
                                End If
                            End If
                        End If
    ...
    Call setControlVisible(0)
    Set CheckProcedure = rsTemp
    Call ShowFlash
End Function
...
'检查当前登录用户是否是系统所有者
    ...
    With vsfSelSys
        For i = 1 To .Rows - 1
            If InStr("," & strTemp, "," & .TextMatrix(i, Col_所有者) & ",") = 0 Then
                strTemp = strTemp & .TextMatrix(i, Col_所有者) & ","
            End If
        Next
    End With
    If strTemp <> "" Then strTemp = "," & strTemp
    If InStr(strTemp, "," & gstrUserName & ",") = 0 Then
        MsgBox "当前用户不是系统所有者，无法进行检查！"
        Exit Function
    End If
    CheckUser = True
End Function

Private Sub setControlVisible(ByVal bytType As Byte)
'过程检查状态栏显示;bytType:其它-对应控件显示，0-不显示

    lblProgress.Visible = True
    lblStatus.Visible = IIf(bytType = 0, True, False)
    Linepgb.Visible = IIf(bytType = 0, True, False)
    pgbState.Visible = IIf(bytType = 0, True, False)
    pgbProgress.Visible = IIf(bytType = 0, True, False)
    picStatus.Visible = IIf(bytType = 0, False, True)
    lblProgress.Caption = ""
    lblProgress.Refresh
End Sub

Private Function GetProcedure() As ADODB.Recordset
'获取数据库当前用户下的过程
    Dim rsTemp As ADODB.Recordset
    Dim rsProcedure As ADODB.Recordset
    Dim strSQL As String
    Dim strName As String
    Dim strOraProcedure As String
    
    On Error GoTo errH
    Set rsProcedure = CopyNewRec(Nothing, True, , Array("过程名称", adVarChar, 100, Empty, "原始SQL", adVarChar, 20000, Empty))
    strSQL = "Select a.Name, a.Text" & vbNewLine & _
            "From User_Source a" & vbNewLine & _
            "Where (a.Type = 'PROCEDURE' Or a.Type = 'FUNCTION')" & vbNewLine & _
            "Order By a.Name, a.Line"
    Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, "过程/函数")
    Do While Not rsTemp.EOF
        If strName <> rsTemp!Name Then
            If strName <> "" Then
                rsProcedure.AddNew Array("过程名称", "原始SQL"), Array(strName, strOraProcedure)
                strOraProcedure = ""
            End If
            strName = rsTemp!Name
        End If
        strOraProcedure = strOraProcedure & rsTemp!Text
        rsTemp.MoveNext
    Loop
    Set GetProcedure = rsProcedure
    Exit Function
errH:
...
'功能：初始化脚本数据记录集
    ...
    Set mrsFildFromFile = CopyNewRec(Nothing, True, , Array("系统编号", adDouble, 10, 0, "SQL", adVarChar, 2000, Empty, "表名", adVarChar, 100, Empty, _
                        "字段", adVarChar, 200, Empty, "字段类型", adVarChar, 20, Empty, "字段长度", adVarChar, 10, Empty, "字段信息", adVarChar, 100, Empty))
    ...
    Set grsTableInfor = CopyNewRec(Nothing, True, , Array("系统编号", adDouble, 10, 0, "系统名称", adVarChar, 100, Empty, "表名", adVarChar, 100, Empty, "表信息", adVarChar, 1000, Empty, "临时表", adVarChar, 10, Empty))
End Sub
...
'参数：rsFileInfor：保存所选系统的列表内容
    Dim i As Long
    Dim strFile As String, strSysNum As String
        ...
        For i = .FixedRows To .Rows - .FixedRows
            If .Cell(flexcpChecked, i, Col_选择) = flexChecked Then
                ...
                Else
                    ...
                    End If
                End If
                strSysNum = IIf(strSysNum = "", "", strSysNum & ",") & IIf(.TextMatrix(i, Col_系统编号) = "", 0, .TextMatrix(i, Col_系统编号))
            End If
...
'                        MsgBox gstrUserName & "不是DBA用户，也不是ZLTOOLS用户，需连接管理工具用户才能对管理工具进行检查！"
    ...
    Loop
    If Not CheckSpecialSpScript(lblMainPath.Tag, strSysNum, IIf(InStr("," & strSysNum & ",", ",0,"), True, False)) Then Exit Function
    CheckChoose = True
End Function
...
'参数：rsChoose：所勾选的系统信息
    ...
    With rsChoose
        .MoveFirst
        Do While Not .EOF
            If CheckSetFile(!配置文件, !系统编号, !系统名称) Then
                varTemp = Split(!当前版本, ".")
                If varTemp(2) <> 0 Then
                    ...
                    Call AddUpFile(rsTemp, !当前版本, !共享号, !系统名称)
                End If
            Else
                picStatus.Visible = False
                Enabled = True
    ...
    With mrsLocalFile
        ...
        Do While Not .EOF
            ...
            Call CollectObjFromFile(!systemname)
            Call DealSpeObj
            pgbState.value = .AbsolutePosition / .RecordCount * 100
            DoEvents
            .MoveNext
        Loop
        '可选脚本中涉及到删除表和表字段的语句，需放在最后处理一次
        .Filter = "FileName like '*OPTIONAL*'"
        Do While Not .EOF
            lblStatus.Caption = "正在收集脚本对象信息：" & !FilePath
            mlngSysNum = !SystemNum
            mlngShare = IIf(IsNull(!共享号) = True, 0, !共享号)
...
Private Sub AddUpFile(ByVal rsTemp As ADODB.Recordset, ByVal strCurrver As String, ByVal lngShare As Long, ByVal strSysName As String)
...
'参数：rsTemp：根据公共函数获取的升级脚本;strCurrver：当前当前版本
    ...
    '10.34.50_OPTIONAL这个文件有行数有很多，功能不需要，为了不影响效率，故排除掉
    strFilter = "FullSPVer>" & strVer & " or FileName like '*HISTORY*' or FileName like '*_10.34.50_OPTIONAL*'"
    Call RecDelete(rsTemp, strFilter)
    
    rsTemp.Filter = "": rsTemp.Sort = "FullSPVer Asc"
    rsTemp.MoveFirst
    For i = 1 To rsTemp.RecordCount
        mrsLocalFile.AddNew Array("FilePath", "SystemNum", "SystemName", "FileName", "FileType", "FullVer", "共享号"), _
            Array(rsTemp!FilePath, rsTemp!系统编号, strSysName, UCase(rsTemp!Filename), "升级脚本", rsTemp!fullspver, lngShare)
        rsTemp.MoveNext
    Next
End Sub
...
Private Sub CollectObjFromFile(Optional ByVal strSysName As String)
    '存储本地脚本的数据对象
    Dim i As Long
    Dim lngSys As Long
    Dim strSQL As String
    ...
    Dim strTableSpace As String, strTableInfor As String
    Dim strFildLength As String
    Dim rsTemp As ADODB.Recordset
    Dim blnTempTable As Boolean
    ...
    ElseIf mrsLocalFile!Filename = "ZLTABLE.SQL" Then
        If mclsRunScript.OpenFile(mrsLocalFile!FilePath) Then
            Do While Not mclsRunScript.EOF
                ...
                If strSQL Like "CREATE TABLE*" Or strSQL Like "CREATE GLOBAL TEMPORARY TABLE*" Then
                    strSQL = Replace(strSQL, "NUMERIC", "NUMBER")
                    If strSQL Like "CREATE TABLE*" Then
                        strTemp = Trim(Replace(strSQL, "CREATE TABLE", ""))
                        blnTempTable = False
                    Else
                        strTemp = Trim(Replace(strSQL, "CREATE GLOBAL TEMPORARY TABLE", ""))
                        blnTempTable = True
                    End If
                    ...
                    ElseIf InStr(strTemp, "||)") > 0 Then
                        strFild = Mid(strTemp, InStr(strTemp, "(") + 1, InStr(strTemp, "||)") - InStr(strTemp, "(") - 1)
                    ElseIf InStr(strTemp, ")||") > 0 Then
                        strFild = Mid(strTemp, InStr(strTemp, "(") + 1, InStr(strTemp, ")||") - InStr(strTemp, "(") - 1)
                    Else
                        strFild = Mid(strTemp, InStr(strTemp, "(") + 1)
                    End If
                    strTemp = Replace(strFild, "||", vbCrLf)
                    strTableInfor = Trim(Mid(strSQL, InStr(strSQL, strTemp) + Len(strTemp)))
                    If InStr(strTableInfor, ")") < 5 Then strTableInfor = Mid(strTableInfor, InStr(strTableInfor, ")") + 1)
                    ...
                    For i = LBound(varTemp) To UBound(varTemp)
                        ...
                        If varTemp(i) <> "" And varTemp(i) <> ")" And InStr(varTemp(i), "TABLESPACE") = 0 Then
                            ...
                            Else
                                ...
                                '字段的名称和字段列赋的相同的值
                            End If
                            '记录字段后面的信息，如 default 0 not Null
                            strTemp = Mid(varTemp(i), InStr(varTemp(i), strFildType) + Len(strFildType))
                            If InStr(strTemp, "(") > 0 Then strTemp = Mid(strTemp, InStr(strTemp, ")") + 1)
                            If InStr(strTemp, ",") > 0 Then
                            ...
                            End If
                            If strFild <> "" Then
                                mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                                    Array(mlngSysNum, strSQL, strTableName, strFild, strFildType, strFildLength, strTemp)
                            End If
                        End If
                    Next
                    grsTableInfor.AddNew Array("系统编号", "系统名称", "表名", "表信息", "临时表"), Array(mlngSysNum, strSysName, strTableName, strTableInfor, IIf(blnTempTable = True, "是", "否"))
                End If
                DoEvents
    ...
    ElseIf mrsLocalFile!Filename = "ZLREPORT.SQL" Then
        If mclsRunScript.OpenFile(mrsLocalFile!FilePath) Then
            Do While Not mclsRunScript.EOF
                strSQL = UCase(mclsRunScript.SQLInfo.PartSQL)
        ...
        End If
    Else
        Call GetAnyObject(strSysName)
    End If
End Sub

Private Sub GetOptionalFile(ByVal strSQL As String)
'获取可选脚本中的内容
    Dim varTemp As Variant, varFild As Variant
    Dim strName As String, strTableName As String, strTemp As String
    Dim strFilter As String, strFild As String
    Dim i As Long
    Dim rsTemp As ADODB.Recordset
    
    varTemp = Split(strSQL, "EXECUTE IMMEDIATE")
    For i = 1 To UBound(varTemp)
        If InStr(varTemp(i), "DROP") > 0 Or InStr(varTemp(i), "RENAME COLUMN") > 0 Then
            If InStr(varTemp(i), "'") > 0 Then
                strTemp = Mid(varTemp(i), InStr(varTemp(i), "'") + 1, InStr(varTemp(i), ";") - InStr(varTemp(i), "'") - 2)
                If strTemp Like "ALTER TABLE*DROP COLUMN*" Then
                    varFild = Split(strTemp, "DROP COLUMN")
                    strTableName = Trim(Replace(varFild(0), "ALTER TABLE", ""))
                    strTemp = Trim(varFild(1))
                    strFilter = "表名='" & strTableName & "' and 字段='" & strTemp & "' and 系统编号=" & mlngSysNum
                    Call RecDelete(mrsFildFromFile, strFilter)
                ElseIf strTemp Like "DROP TABLE*" Then
                    strName = Trim(Replace(strTemp, "DROP TABLE", ""))
                    ...
                    Call RecDelete(mrsDataFromFile, strFilter)
                    strFilter = "表名 = '" & strName & "' and 系统编号=" & mlngSysNum
                    Call RecDelete(grsTableInfor, strFilter)
                ElseIf strTemp Like "ALTER TABLE*RENAME COLUMN*" Then
                    varFild = Split(strTemp, "RENAME COLUMN")
                    strTableName = Trim(Replace(varFild(0), "ALTER TABLE", ""))
                    varFild = Split(varFild(1), "TO")
                    strFild = Trim(varFild(0))
                    varFild(1) = Trim(varFild(1))
                    If varFild(1) Like "*BAK" Then
                    ...
                    Else
                        ...
                        If mrsFildFromFile.RecordCount > 0 Then
                            mrsFildFromFile!字段 = varFild(1)
                            mrsFildFromFile.Update
                        End If
                    '处理约束
                    End If
                    mrsConstraintFromFile.Filter = ""
                    Set rsTemp = CopyNewRec(mrsConstraintFromFile)
                    rsTemp.Filter = "表名='" & strTableName & "' And 系统编号=" & mlngSysNum
                    Do While Not rsTemp.EOF
                        If InStr("," & rsTemp!字段 & ",", "," & strFild & ",") > 0 Then
                            If varFild(1) Like "*BAK" Then
                                If InStr(varFild(1), ",") = 0 Then
                                    strFilter = "表名='" & strTableName & "' And 名称='" & rsTemp!名称 & "' And 系统编号=" & mlngSysNum
                                    Call RecDelete(mrsConstraintFromFile, strFilter)
                                Else
                                    strTemp = Replace("," & rsTemp!字段 & ",", "," & strFild & ",", ",")
                                    strTemp = Mid(strTemp, 2, Len(strTemp) - 2)
                                    mrsConstraintFromFile.Filter = "表名='" & strTableName & "' And 名称='" & rsTemp!名称 & "' And 系统编号=" & mlngSysNum
                                    If mrsConstraintFromFile.RecordCount > 0 Then
                                        mrsConstraintFromFile!字段 = strTemp
                                        mrsConstraintFromFile.Update
                                    End If
                                End If
                            Else
                                strTemp = Replace("," & rsTemp!字段 & ",", "," & strFild & ",", "," & varFild(1) & ",")
                                strTemp = Mid(strTemp, 2, Len(strTemp) - 2)
                                mrsConstraintFromFile.Filter = "表名='" & strTableName & "' And 名称='" & rsTemp!名称 & "' And 系统编号=" & mlngSysNum
                                If mrsConstraintFromFile.RecordCount > 0 Then
                                    mrsConstraintFromFile!字段 = strTemp
                    ...
                    Loop
                    '处理索引
                    mrsIndexFromFile.Filter = ""
                    Set rsTemp = CopyNewRec(mrsIndexFromFile)
                    rsTemp.Filter = "表名='" & strTableName & "' And 系统编号=" & mlngSysNum
                    Do While Not rsTemp.EOF
                        If InStr("," & rsTemp!字段 & ",", "," & strFild & ",") > 0 Then
                            If varFild(1) Like "*BAK" Then
                                ...
                                Else
                                    strTemp = Replace("," & rsTemp!字段 & ",", "," & strFild & ",", ",")
                                    strTemp = Mid(strTemp, 2, Len(strTemp) - 2)
                                    mrsIndexFromFile.Filter = "表名='" & strTableName & "' And 名称='" & rsTemp!名称 & "' And 系统编号=" & mlngSysNum
                                    If mrsIndexFromFile.RecordCount > 0 Then
                                        mrsIndexFromFile!字段 = strTemp
                                        mrsIndexFromFile.Update
                                    End If
                                End If
                            Else
...
Private Sub GetAnyObject(Optional ByVal strSysName As String)
'保存升级脚本中所有可能的SQL对象
    Dim strSQL As String
    ...
    If mclsRunScript.OpenFile(mrsLocalFile!FilePath) Then
        Do While Not mclsRunScript.EOF
            ...
            If InStr(strSQL, "INSERT INTO ZLREPORTS") > 0 Then strSQL = mclsRunScript.SQLInfo.PartSQL
            strIniSQL = iniSQL(strSQL)
            If strIniSQL <> "" Then
                If mrsLocalFile!Filename Like "*OPTIONAL*" Then
                    Call GetOptionalFile(strIniSQL)
                Else
                    If strIniSQL Like "CREATE*" Then
                        ...
                        ElseIf strIniSQL Like "CREATE SEQUENCE*" Then
                            ...
                            Call RecDelete(mrsSequenceFromFile, strFilter)
                            mrsSequenceFromFile.AddNew Array("系统编号", "SQL", "名称"), Array(mlngSysNum, strSQL, strName)
                        ElseIf strIniSQL Like "CREATE TABLE*" Or strIniSQL Like "CREATE GLOBAL TEMPORARY TABLE*" Then
                            If strIniSQL Like "CREATE TABLE*" Then
                                strTemp = Trim(Replace(strIniSQL, "CREATE TABLE", ""))
                                blnTempTable = False
                            Else
                                strTemp = Trim(Replace(strIniSQL, "CREATE GLOBAL TEMPORARY TABLE", ""))
                                blnTempTable = True
                            End If
                            strTableName = Trim(Replace(Mid(strTemp, 1, InStr(strTemp, "(") - 1), vbCrLf, ""))
                            ...
                            '获取创建表的后面表空间等内容
                            strTemp = Replace(strFild, "||", vbCrLf)
                            strTableInfor = Trim(Mid(strIniSQL, InStr(strIniSQL, strTemp) + Len(strTemp)))
                            If InStr(strTableInfor, ")") < 5 Then strTableInfor = Mid(strTableInfor, InStr(strTableInfor, ")") + 1)
                            varTemp = Split(strFild, "||")
                            For i = LBound(varTemp) To UBound(varTemp)
                                varTemp(i) = Trim(varTemp(i))
                                ...
                                If varTemp(i) <> "" And varTemp(i) <> ")" And InStr(varTemp(i), "TABLESPACE") = 0 Then
                                    strFildType = ""
                                    strFildLength = ""
                                    strFild = TrimEx(Mid(varTemp(i), 1, InStr(varTemp(i), " ")), True)
                                    strTemp = Trim(Mid(varTemp(i), InStr(varTemp(i), " ") + 1))
                                    If InStr(strTemp, "DATE") > 0 Then
                                        strFildType = "DATE"
                                    ElseIf InStr(strTemp, "LONG RAW") > 0 Then
                            ...
                            If InStr(strIniSQL, "CONSTRAINT") > 0 Then
                                If InStr(strIniSQL, "_CK_") = 0 Then
                                    If strIniSQL Like "ALTER TABLE * ADD CONSTRAINT *" Then
                                        ...
                                        '获取约束名称
                                        strName = TrimEx(Mid(strName, 1, InStr(strName, " ") - 1), True)
                                        '获取表名
                                    ...
                                    ElseIf strIniSQL Like "ALTER TABLE*DROP CONSTRAINT*" Then
                                        ...
                                        strName = Trim(Replace(varTemp(1), "CONSTRAINT", ""))
                                        If InStr(strName, " ") > 0 Then strName = TrimEx(Mid(strName, 1, InStr(strName, " ") - 1), True)
                                        strTableName = Trim(Replace(varTemp(0), "ALTER TABLE", ""))
                                        strFilter = "表名='" & strTableName & "' and 名称='" & strName & "' and 系统编号=" & mlngSysNum
                                        Call RecDelete(mrsConstraintFromFile, strFilter)
                                        Call RecDelete(mrsIndexFromFile, strFilter)
                                    ...
                                    ElseIf strIniSQL Like "ALTER TABLE*RENAME CONSTRAINT*" Then
                                        ...
                                        If varTemp(1) Like "*BAK*" Then
                                            strFilter = "表名='" & strTableName & "' and 名称='" & varTemp(0) & "' and 系统编号=" & mlngSysNum
                                            Call RecDelete(mrsConstraintFromFile, strFilter)
                                            strFilter = "名称='" & varTemp(0) & "' and 系统编号=" & mlngSysNum
                                            Call RecDelete(mrsIndexFromFile, strFilter)
                                        Else
                            ...
                            Else
                                If strIniSQL Like "ALTER TABLE*ADD*" Then
                                    If strIniSQL = "ALTER TABLE 时间段 ADD (" & vbNewLine & _
                                        ...
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "时间段", "站点", "VARCHAR2", 1, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "时间段", "号类", "VARCHAR2", 10, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "时间段", "出诊预留时间", "NUMBER", 18, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "时间段", "休息时段", "VARCHAR2", 200, "")
                                    ElseIf strIniSQL = "ALTER TABLE 人员收缴记录 ADD(" & vbNewLine & _
                                                        ...
                                                        " 是否结帐 NUMBER(1))" Then
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "人员收缴记录", "是否挂号", "NUMBER", 1, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "人员收缴记录", "是否就诊卡", "NUMBER", 1, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "人员收缴记录", "是否消费卡", "NUMBER", 1, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "人员收缴记录", "是否收费", "NUMBER", 1, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "人员收缴记录", "预交类别", "NUMBER", 2, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, "人员收缴记录", "是否结帐", "NUMBER", 1, "")
                                    ElseIf strIniSQL = "ALTER TABLE ZLREPORTS ADD (执行人员 VARCHAR2(20), 最后执行时间 DATE)" Then
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                        Array(mlngSysNum, strIniSQL, "ZLREPORTS", "执行人员", "VARCHAR2", 20, "")
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                        Array(mlngSysNum, strIniSQL, "ZLREPORTS", "最后执行时间", "DATE", "", "")
    ...
    '                                    strIniSQL = "ALTER TABLE 血液输血常规 ADD 是否婴儿 NUMBER (1))"
                                    ...
                                    Else
                                        ...
                                        Call RecDelete(mrsFildFromFile, strFilter)
                                        '记录字段后面的信息，如 default 0 not Null
                                        strTemp = Mid(strIniSQL, InStr(strIniSQL, strFildType) + Len(strFildType))
                                        If InStr(strTemp, "(") > 0 Then strTemp = Mid(strTemp, InStr(strTemp, ")") + 1)
                                        If InStr(strTemp, ",") > 0 Then
                                            strTemp = Mid(strTemp, 1, InStr(strTemp, ",") - 1)
                                        ElseIf InStr(strTemp, ")") > 0 Then
                                        ...
                                        End If
                                        mrsFildFromFile.AddNew Array("系统编号", "SQL", "表名", "字段", "字段类型", "字段长度", "字段信息"), _
                                            Array(mlngSysNum, strIniSQL, strTableName, strFild, strFildType, strFildLength, strTemp)
                                    End If
                                ElseIf strIniSQL Like "ALTER TABLE*MODIFY*" Then
                                    ...
                                    If InStr(varTemp(1), "NULL") = 0 And InStr(varTemp(1), "DEFAULT") = 0 Then
                                        ...
                                        Else
                                            ...
                                            strFildLength = ""
                                        End If
                                        mrsFildFromFile.Filter = "系统编号=" & mlngSysNum & " And 表名='" & strTableName & "' And 字段='" & strFild & "'"
                                        If mrsFildFromFile.RecordCount > 0 Then
                                ...
                                ElseIf strIniSQL Like "ALTER TABLE*DROP COLUMN*" Then
                                    ...
                                    strFilter = "表名='" & strTableName & "' and 字段='" & strFild & "' and 系统编号=" & mlngSysNum
                                    Call RecDelete(mrsFildFromFile, strFilter)
                                    '列改名，数据库中对应的索引或约束中的字段会自动改名，需同步处理
                                    '处理约束
                                    mrsConstraintFromFile.Filter = ""
                                    Set rsTemp = CopyNewRec(mrsConstraintFromFile)
                                    rsTemp.Filter = "表名='" & strTableName & "' And 系统编号=" & mlngSysNum
                                    Do While Not rsTemp.EOF
                                        strTemp = rsTemp!字段
                                        If InStr("," & strTemp & ",", "," & strFild & ",") > 0 Then
                                            If InStr(strTemp, ",") = 0 Then
                                                strFilter = "表名='" & strTableName & "' and 名称='" & rsTemp!名称 & "' and 系统编号=" & mlngSysNum
                                                Call RecDelete(mrsConstraintFromFile, strFilter)
                                            Else
                                                strTemp = Replace("," & strTemp & ",", "," & strFild & ",", ",")
                                                strTemp = Mid(strTemp, 2, Len(strTemp) - 2)
                                                mrsConstraintFromFile.Filter = "表名='" & strTableName & "' And 名称='" & rsTemp!名称 & "' And 系统编号=" & mlngSysNum
                                                If mrsConstraintFromFile.RecordCount > 0 Then
                                                    mrsConstraintFromFile!字段 = strTemp
                                    ...
                                    Do While Not rsTemp.EOF
                                        If InStr("," & rsTemp!字段 & ",", "," & strFild & ",") > 0 Then
                                            If varTemp(1) Like "*BAK" Then
                                                ...
                                                End If
                                            Else
                                                strTemp = Replace("," & rsTemp!字段 & ",", "," & strFild & ",", "," & varTemp(1) & ",")
                                                strTemp = Mid(strTemp, 2, Len(strTemp) - 2)
                                                mrsConstraintFromFile.Filter = "表名='" & strTableName & "' And 名称='" & rsTemp!名称 & "' And 系统编号=" & mlngSysNum
                                                If mrsConstraintFromFile.RecordCount > 0 Then
                                                    mrsConstraintFromFile!字段 = strTemp
                                ...
                                ElseIf strIniSQL Like "ALTER TABLE*RENAME TO*" Then
                                    ...
                                    If strTemp Like "*BAK" Then
                                        ...
                                        Call RecDelete(mrsDataFromFile, strFilter)
                                        strFilter = "系统编号=" & mlngSysNum & " And 表名='" & strTableName & "'"
                                        Call RecDelete(grsTableInfor, strFilter)
                                    Else
                                        grsTableInfor.Filter = "系统编号=" & mlngSysNum & " And 表名='" & strTableName & "'"
                                        If grsTableInfor.RecordCount = 1 Then
                                            grsTableInfor!表名 = strTemp
                                            grsTableInfor.Update
                                        End If
                    ...
                    ElseIf strIniSQL Like "UPDATE ZLTABLES*" Then
                        ...
                        If InStr(strTemp, "表名") > 0 Then
                            ...
                            End If
                        End If
                    ElseIf strIniSQL Like "DCLARE*" Or strIniSQL Like "BEGIN*" Then
                        Call GetOptionalFile(strIniSQL)
                    End If
                End If
            End If
...
Private Function iniSQL(ByVal strSQL As String) As String
    ...
    Else
        If strSQL Like "CREATE TABLE*" Or strSQL Like "CREATE GLOBAL TEMPORARY TABLE*" Or strSQL Like "CREATE OR REPLACE PROCEDURE*" Or strSQL Like "CREATE OR REPLACE FUNCTION*" Then
...
'功能：去掉写在单行strSQL语句后面的"--"或者"/"注释和每行的前后空格(只针对本模块，主要是用于去掉表或者过程/函数字段或参数后面的注释)
...
Private Function CheckSetFile(ByVal strPath As String, ByVal strSysNum As Long, ByVal strSysName As String) As Boolean
'功能：检查本地安装脚本
'参数：strPath-主路径，strSysNum-系统编号
    ...
    If strSysNum = 0 Then
        Call AddFilePath(UCase(strPath), "ZLSERVER.SQL", "管理工具文件", strSysNum, strProblem, strSysName)
        CheckSetFile = True
        Exit Function
    End If
    ...
    '检查安装脚本是否存在
    Call AddFilePath(strMainPath & "ZLSEQUENCE.SQL", "ZLSEQUENCE.SQL", "序列文件", strSysNum, strProblem, strSysName)
    Call AddFilePath(strMainPath & "ZLTABLE.SQL", "ZLTABLE.SQL", "数据表文件", strSysNum, strProblem, strSysName)
    Call AddFilePath(strMainPath & "ZLCONSTRAINT.SQL", "ZLCONSTRAINT.SQL", "约束文件", strSysNum, strProblem, strSysName)
    Call AddFilePath(strMainPath & "ZLINDEX.SQL", "ZLINDEX.SQL", "索引文件", strSysNum, strProblem, strSysName)
    Call AddFilePath(strMainPath & "ZLVIEW.SQL", "ZLVIEW.SQL", "视图文件", strSysNum, strProblem, strSysName)
    Call AddFilePath(strMainPath & "ZLPROGRAM.SQL", "ZLPROGRAM.SQL", "函数过程文件", strSysNum, strProblem, strSysName)
    Call AddFilePath(strMainPath & "ZLMANDATA.SQL", "ZLMANDATA.SQL", "管理数据文件", strSysNum, strProblem, strSysName)
    Call AddFilePath(strMainPath & "ZLREPORT.SQL", "ZLREPORT.SQL", "报表数据文件", strSysNum, strProblem, strSysName)
    ...
    If Dir(strMainPath & "ZLPACKAGE.SQL") <> "" Then
        mrsLocalFile.AddNew Array("FilePath", "SystemNum", "SystemName", "FileName", "FileType"), _
                            Array(strMainPath & "ZLPACKAGE.SQL", strSysNum, strSysName, "ZLPACKAGE.SQL", "安装脚本")
    End If
    CheckSetFile = True
End Function

Private Sub AddFilePath(ByVal strPath As String, ByVal strFileName As String, ByVal strFileType As String, ByVal strSysNum As Long, ByRef strProblem As String, ByVal strSysName As String)

    If Dir(strPath) = "" Then
        strProblem = strProblem & vbCr & strFileType & strPath
    Else
        mrsLocalFile.AddNew Array("FilePath", "SystemNum", "SystemName", "FileName", "FileType"), Array(strPath, strSysNum, strSysName, strFileName, "安装脚本")
    End If
End Sub
...
Private Sub cmdFunction_MouseMove(Index As Integer, Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    For i = 0 To cmdFunction.UBound
        If i = Index Then
            ...
            Case 3
                lblNote.Caption = "修正每个序列的当前值，保证序列与实际应用相匹配；" & vbNewLine & _
                            vbCrLf & "当系统提示“…(ID)出现重复！”类似的错误时，一般可解决。"
            Case 4
                lblNote.Caption = "存储过程和函数的规范性检查，包括：" & vbNewLine & _
                            vbCrLf & "1.缺少错误处理中心的调用(Zl_Errorcenter)" & vbNewLine & _
                                    "错误处理中心可以将一些常见的错误信息转换成友好的提示信息反馈给用户。" & vbNewLine & _
                            vbCrLf & "2.动态执行的SQL(Execute Immediate)，如果不使用绑定变量，每次执行SQL进行硬解析，将会耗用大量cpu资源， 以及在数据库的共享池内存区域产生大量无法共享的SQL文本及执行计划，并且可能将其他共享SQL挤出内存。" & vbNewLine & _
                            vbCrLf & "3.非自治事务的过程或函数中使用Commit，可能造成部分数据提交，不完整的数据可能引发各种不可预知的问题。"
            End Select
        End If
    Next
End Sub
...
'获取本地文件的系统版本号
    ...
    With vsfSelSys
        ...
        For i = .FixedRows To .Rows - .FixedRows
            ...
            If blnExist Then
                strMaxVer = ""
                If InStr(.TextMatrix(i, Col_配置文件), "TOOLS") = 0 Then
                    '有的系统升级时可能出现没有脚本文件的情况，为了避免检查不到，故脚本版本取zlSetup.ini中的版本号
                    Set rsTemp = ReadINIToRec(.TextMatrix(i, Col_配置文件))
                    rsTemp.Filter = "项目='版本号'"
                    If rsTemp.RecordCount = 1 Then
                        .Cell(flexcpText, i, Col_脚本版本) = rsTemp!内容
                    Else
...
Private Sub Form_Resize()
    ...
    cmdFunction(4).Top = lblNote.Top + lblNote.Height - cmdFunction(4).Height
    cmdFunction(4).Left = cmdFunction(0).Left
    
    cmdFunction(3).Top = cmdFunction(4).Top - cmdFunction(3).Height - 50
    cmdFunction(3).Left = cmdFunction(0).Left
    
    cmdFunction(2).Top = cmdFunction(3).Top - cmdFunction(2).Height - 50
    cmdFunction(2).Left = cmdFunction(0).Left
    
    cmdFunction(1).Top = cmdFunction(2).Top - cmdFunction(1).Height - 50
    cmdFunction(1).Left = cmdFunction(0).Left
