...
Private Function ExecuteCommand(ByVal strCommand As String, ParamArray varParam() As Variant) As Boolean
    ...
    Case "初始控件"
        ...
        With mclsVsf
            ...
            Call .AppendColumn("升级脚本", 2800, flexAlignLeftCenter, flexDTString, , "", True)
    ...
    If rsData.BOF = False Then
        ...
        For lngLoop = 0 To rsData.RecordCount - 1
            ...
            If gobjFile.FileExists(str最新标准过程路径 & "\" & strProcName & ".sql") Then
                strSQL = "Select A.ID,A.名称,B.内容 From zlProcedure A,zlProcedureText B Where A.ID = B.过程ID And B.性质 = 4 And A.ID=[1] Order By B.序号"
...
'                    rs.MoveFirst
    ...
    strCommand = GetWinSystemPath & "\wincmp3.exe " & str上次标准过程路径 & "\ " & str最新标准过程路径 & "\ /G:HE " & str对比报告路径
    err.Clear
    DoEvents
...
'                    Call gclsBase.SQLRecordAdd(rsSQL, strSQL)
                    strSQL = "Zl_Zlproceduretext_Update(" & Nvl(rsData("ID").value) & ",4," & (lngLoop + 1) & ",'" & strArr(lngLoop) & "')"
                    Call gclsBase.SQLRecordAdd(rsSQL, strSQL)
                Next
            Else
                '标准过程在升级前后无变化
                strProcName = Nvl(rsData("名称").value)
                strSQL = "Select A.ID,A.类型,A.名称,B.内容 From zlProcedure A,zlProcedureText B Where A.ID = B.过程ID And B.性质 = 3 And A.名称=[1]"
