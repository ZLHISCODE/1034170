...
Public Function GetFTPFileSize(ByVal strFileName As String) As Double
'功能：获取FTP上文件的大小，由于FTPAPI使用高32和低32位表示文件大小，因此，返回值使用Double
    Dim udtFindData     As WIN32_FIND_DATA, lnghFile    As Long
    Dim strFindFile     As String

    lnghFile = FtpFindFirstFile(lngINetConn, strFileName, udtFindData, INTERNET_FLAG_RELOAD Or INTERNET_FLAG_NO_CACHE_WRITE, 0)
    strFindFile = Left(udtFindData.cFileName, InStr(udtFindData.cFileName, vbNullChar) - 1)
    If strFindFile <> "" Then
        GetFTPFileSize = CDbl(udtFindData.nFileSizeHigh) * (2 ^ 32) + udtFindData.nFileSizeLow
    End If
    If lnghFile <> 0 Then
