...
--oracle 8.17上必须使用显示游标才能用调用者权限访问All_Objects视图
  ...
  Cursor c_Module Is
    Select Object_Name 对象, Owner 所有者
    From All_Objects A
    Where Owner = v_所有者 And Object_Type In ('TABLE', 'VIEW', 'SEQUENCE', 'PROCEDURE', 'FUNCTION') And Exists
     (Select 1 From zlProgPrivs B Where B.系统 = 系统_In And B.序号 = 序号_In) And Not Exists
     (Select 1 From User_Synonyms C Where C.Table_Owner = A.Owner And C.Synonym_Name = A.Object_Name);
