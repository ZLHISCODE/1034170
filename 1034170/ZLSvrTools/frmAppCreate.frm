...
Private Sub cmdNext_Click()
    ...
    ElseIf fraSetup(4).Visible Then
        ...
        Else
            ...
            DoEvents
            For i = 0 To tbsSpace.Tabs.Count - 1
                strTemp = IIf(strTemp = "", txtSpaceName(i).Caption, strTemp & "," & txtSpaceName(i).Caption)
            Next
            Call UnInstall(Me, IIf(chkEnjoy.value = 0, False, True), stbThis, txtOwnerUsr.Text, strTemp, mlng帐套, intDefSysCode * 100)
        End If
        cmdNext.Enabled = True
        Unload Me
    End If
            ...
            Else
                ...
                For intItems = 0 To UBound(aryVal)
                    If Dir(mstrIniPath & "zlSelData" & intCount & intItems & ".sql") <> "" Then
                        ...
                        If Len(lstData(intOptions)) > lngWidthMax Then
                            lngWidthMax = Len(lstData(intOptions)) + 2
                        End If
                    End If
                Next
            End If
            If lngWidthMax > lstData(intOptions).Width Then
                SendMessage lstData(intOptions).hwnd, LB_SETHORIZONTALEXTENT, lngWidthMax, ByVal 0
            End If
...
Private Function SysInstall() As Boolean
    ...
    Dim arrObj() As String, strObj As String, i As Integer
    ...
    If chkEnjoy.value = 0 Then
        ...
        gcnOracle.Execute gstrSQL
        gstrSQL = "Grant Select on sys.dba_role_privs to " & strUserName
        gcnOracle.Execute gstrSQL
        
        strObj = "gv_$session,v_$session,v_$parameter,v_$process"  '需要授予系统视图查询和转授权限
        arrObj = Split(strObj, ",")
        For i = 0 To UBound(arrObj)
            gstrSQL = "Grant Select on sys." & arrObj(i) & " to " & strUserName & " With Grant Option"
            gcnOracle.Execute gstrSQL
        Next
