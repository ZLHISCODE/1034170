...
Option Explicit
Dim mintSystem As Integer    '系统编号
Dim mstrOwner  As String     '系统所有者
Dim mrsObject As New ADODB.Recordset

Dim msngStartX As Single     '移动前鼠标的位置
Dim mblnItem As Boolean
Dim mstr序号 As String        '上一个tvwMain.SelectedItem的Key属性
Dim mstr功能 As String    '上一个Tab的Caption属性
...
Public Function ProgPriv(ByVal intSystem As Integer) As Boolean
    '初始化表格
    With bill
        .Cols = 7
        .TextMatrix(0, 0) = "数据对象"
    ...
    End With
    '初始化数据
    mintSystem = intSystem

    mstrOwner = GetOwnerName(Val(intSystem), gcnOracle)
