...
'功能：检查系统调度（统计信息收集相关）
'******************************************************************************************************************
Private Function CheckScheduler() As Boolean
    Dim strSQL          As String
    Dim rsTmp           As ADODB.Recordset
    ...
    '系统调度处理，区分10G与11G
    If mbln10G Then
        strSQL = "Select a.Owner,a.Job_Name" & vbNewLine & _
            "From Dba_Scheduler_Jobs A" & vbNewLine & _
            "Where a.Job_Name In ('GATHER_STATS_JOB', 'AUTO_SPACE_ADVISOR_JOB') And a.Enabled = 'TRUE'"
    Else
        strSQL = "Select 'SYS' Owner,a.Client_Name Job_Name" & vbNewLine & _
                "From Dba_Autotask_Client A" & vbNewLine & _
                "Where a.Client_Name In ('auto optimizer stats collection', 'auto space advisor') And a.Status = 'ENABLED'"
    End If
    Set rsTmp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, SQL_CAPTION)
    Do While Not rsTmp.EOF
        Call AddCheckItem(rsTmp!Owner & "", rsTmp!Job_Name & "", rsTmp!Job_Name & "", IIf(mbln10G, "TRUE", "ENABLED"), CC_Scheduler, "系统调度", IIf(mbln10G, "TRUE", "DISABLED"), IIf(mbln10G, "Call dbms_scheduler.disable('" & rsTmp!Owner & "." & rsTmp!Job_Name & "')", "Call DBMS_AUTO_TASK_ADMIN.DISABLE(client_name => '" & rsTmp!Job_Name & "',operation => NULL,window_name => NULL)"), "自动统计信息收集影响系统升级", "升级期间禁用", True, True)
        rsTmp.MoveNext
    Loop
    CheckScheduler = True
    Exit Function
ErrH:
    Call ShowFlash("")
    MsgBox "检查系统调度出现错误：" & err.Description, vbInformation, gstrSysName
    err.Clear
End Function
'******************************************************************************************************************
'功能：检查特殊用户的对象权限
