...
Private Sub winSock_DataArrival(ByVal bytesTotal As Long)
    ...
    Case "SYS-READED"               '工作站发来的回置消息已读标志
        ...
        If UCase(varData(5)) = UCase(mstrLocalIP) Then
            strSQL = "UPDATE zlNoticeRec SET 已读标志=1 WHERE 提醒序号=" & Val(varData(0)) & " AND 用户名='" & varData(1) & "'"
...
Private Function CheckNoticeAll() As Boolean
    ...
    strSQL = "SELECT DISTINCT USERNAME,MACHINE FROM V$Session WHERE USERNAME IS NOT NULL"
    Set rs = zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
    If rs.BOF = False Then
        Do While Not rs.EOF
            strMachine = Trim(NVL(rs("MACHINE").Value))
...
Private Function CheckNoticeOne(ByVal Item As MSComctlLib.ListItem, Optional ByVal StartUp As Boolean = False) As Boolean
    ...
    '2.找出用户所属的系统及部门名称
    strSQL = "Select S.* From zlSystems S"
    Set rs = zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
    ...
    '3.找出本用户的提醒消息
    strSQL = "SELECT A.*,B.程序ID AS 模块,B.名称 AS 报表名称,B.系统 As 报表系统 " & _
                "FROM zlNotices A," & _
                    "(SELECT * FROM zlReports WHERE 发布时间 IS NOT NULL) B " & _
                "WHERE (A.序号 IN (" & _
    ...
    If rs.BOF = False Then
        ...
        Do While Not rs.EOF
            ...
            If CheckNotice(Item.SubItems(2), rs("序号").Value, str提醒内容) Then
                ...
                '--检查用户是否已经提醒-----------------------------------------------------------------------------------------------------------
                strSQL = "SELECT * FROM zlNoticeRec WHERE 提醒序号=[1] AND 用户名=[2] AND 已读标志<>1"
...
Private Function CheckNotice(ByVal strUser As String, ByVal lngNo As Long, ByRef str提醒内容 As String) As Boolean
    ...
    strSQL = "SELECT A.*,B.检查时间,B.提醒时间," & _
                    ...
                    "FROM zlNotices A,(SELECT * FROM zlNoticeRec WHERE 用户名=[1])B " & _
    ...
    If blnCheck Then
        ...
        If IsNull(rs("提醒条件").Value) = False Then
            ...
            If Err = 0 Then
                ...
                Else
                    '提醒条件不成立,删除记录
                    strSQL = "DELETE FROM zlNoticeRec WHERE 提醒序号=" & lngNo & " AND 用户名='" & strUser & "'"
                    gcnOracle.Execute strSQL
...
'            End If
        ...
        strSQL = "INSERT INTO zlNoticeRec(提醒序号,用户名,检查时间,检查结果,提醒内容,提醒标志,已读标志) VALUES (" & lngNo & ",'" & UCase(strUser) & "',SYSDATE," & IIf(blnHaveResult, 1, 0) & ",'" & str提醒内容 & "',0,0)"
        gcnOracle.Execute strSQL
    End If
    ...
    If blnNotice Then
        gcnOracle.Execute "UPDATE zlNoticeRec SET 提醒时间=SYSDATE,提醒标志=1 WHERE 提醒序号=" & lngNo & " AND 用户名='" & strUser & "'"
    End If
...
Private Function GetClientIP(ByVal strWorkStation As String) As String
    ...
    strSQL = "Select IP From zlClients Where Upper(工作站)=[1] And IP Is Not Null"
