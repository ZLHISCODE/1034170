...
Public Sub ShowMe(ByVal strSqlId As String, intType As Integer)
'执行计划来源:  1-v$视图 2-历史数据
...
Private Sub LoadSqlText(ByVal strSqlId As String)
    ...
    If mintType = 1 Then
        strSQL = "Select sql_fulltext sql_text from v$Sql where Sql_id = [1]"
    Else
...
Private Sub LoadSqlPlan(ByVal strSqlId As String)
    ...
    If rsTmp.RecordCount = 0 Then
        ...
        With vsfPlan(1)
            .Rows = 2
            .Cell(flexcpText, .Rows - 1, 0, .Rows - 1, .Cols - 1) = "执行计划已从缓存移除或未收集至历史库中。"
...
Private Sub GetSqlPlanByChild(vsfPlan As VSFlexGrid, strSqlId As String, intChild As Integer)
    ...
    If mintType = 1 Then
        strSQL = "Select * From (" & vbNewLine & _
                                                ...
                                                "  From v$sql_Plan" & vbNewLine & _
                                                "  Where Sql_Id = [1] And Child_Number = [2])" & vbNewLine & _
                                                "Select *" & vbNewLine & _
