...
Private Sub cmdExec_Click()
    ...
    If VilidateUpgrade Then
        ...
        If optUpMode(1).value Then
            ...
            End If
        End If
        If Not frmAppUpgradePreproccess.ShowMe(optUpMode(1).value) Then
            Call LoadData(0)
            Exit Sub
        End If
...
Private Sub EnableTriggerAndAutoJobs()
    If Val(cmdFunc(FCT_触发器).Tag) = 0 Or Val(cmdFunc(FCT_后台作业).Tag) = 0 Then
        If MsgBox("是否启用禁用的触发器和后台自动作业？", vbInformation + vbYesNo, gstrSysName) = vbYes Then
            If Val(cmdFunc(FCT_触发器).Tag) = 0 Then
                Call cmdFunc_Click(FCT_触发器)
            End If
            If Val(cmdFunc(FCT_后台作业).Tag) = 0 Then
                Call cmdFunc_Click(FCT_后台作业)
            End If
        End If
    End If
End Sub
...
Private Sub cmdFunc_Click(Index As Integer)
    If cmdFunc(Index).ToolTipText <> "" Then
        ...
        Exit Sub
    Else
        If MsgBox("你确认要""" & cmdFunc(Index).Caption & """?", vbInformation + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
            Exit Sub
        End If
    End If
'    WriteTraceLog
...
Private Sub picMain_Resize(Index As Integer)
    ...
    Else
        ...
        lblExplain.Refresh
        '系统控制标签的位置设置
        Call SetCtrlPosOnLine(True, -1, lblExplain, 60, lblManage, 60, cmdFunc(FCT_客户端), 90, lblUpgrade, 60, lblMainPath, 90, lblUpMode, 90, lblConfigureFile, 30, vsSysSel)
...
'参数：fctTmp=功能类型
    ...
    Dim i       As Long, lngTotal   As Long, sgnPer As Single
    '获取操作日志对象
    ...
    rsSQL.Filter = ""
    rsSQL.Sort = "序号"
    lngTotal = rsSQL.RecordCount
    On Error Resume Next
    Do While Not rsSQL.EOF
        i = i + 1
        Call ShowFlash(rsSQL!步骤 & "", (i * 100 / lngTotal), frmMDIMain, True)
        mobjOprateLog.WriteLine "执行SQL:" & rsSQL!内容
        If rsSQL!DBA = 1 Then
            If rsSQL!CmdType = 0 Then
                cnDBA.Execute rsSQL!内容
            Else
                cnDBA.Execute rsSQL!内容, , adCmdStoredProc
            End If
        Else
            If rsSQL!CmdType = 0 Then
                gcnOracle.Execute rsSQL!内容
            Else
                gcnOracle.Execute rsSQL!内容, , adCmdStoredProc
            End If
        End If
        If err.Number <> 0 Then
...
'         blnLock=是否禁用对象
    ...
    Dim rsDBAJobs   As ADODB.Recordset
    ...
    '序号=SQL执行顺序，以及步骤顺序
    '类型=0：SQL,1:步骤以及进度
    'CmdType-DBMS_JOB包必须指定adCmdStoredProc,其余的DBMS包可以使用adCmdText，或者不传，常规SQL可以使用adCmdText，或者不传
    Set rsSQLStep = CopyNewRec(Nothing, True, , Array("序号", adInteger, 5, 0, "步骤", adVarChar, 100, Empty, "进度", adDouble, 5, 0, "内容", adVarChar, 2000, Empty, "DBA", adInteger, Empty, Empty, "CmdType", adInteger, Empty, Empty))
    Select Case fctTmp
        Case FCT_客户端
            '加载SQL以及进度
            lngAll = lngAll + 2
            ...
            Else
                ...
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "禁用客户端", "启用客户端"), (lngSort * 100 / lngAll), strAdjustSQL, 0, 0)
            End If
        Case FCT_用户账户
            If blnLock Then
                '标记锁定的用户
            ...
            '标记锁定的用户
            If blnLock Then
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, "标记锁定的用户", (lngSort * 100 / lngAll), strAdjustSQL, 0, 0)
            End If
            '锁定或解锁用户
            Do While Not rsObjectSQL.EOF
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "锁定用户账户", "解锁用户账户"), (lngSort * 100 / lngAll), rsObjectSQL!SQL, 0, 0)
                rsObjectSQL.MoveNext
            Loop
            '取消用户锁定标记
            If Not blnLock Then
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, "取消用户锁定标记", (lngSort * 100 / lngAll), strAdjustSQL, 0, 0)
            End If
        Case FCT_后台作业
            If blnLock Then
                '标记停用的后台作业
                ...
                Else
                    strSQL = "Select a.Client_Name Job_Name," & vbNewLine & _
                            "       'Call DBMS_AUTO_TASK_ADMIN.DISABLE(client_name => ' || Chr(39) || a.Client_Name || Chr(39) ||" & vbNewLine & _
                            "        ',operation => NULL,window_name => NULL)' Sql" & vbNewLine & _
                            "From Dba_Autotask_Client A where a.CLIENT_NAME in('auto optimizer stats collection','auto space advisor') and a.STATUS='ENABLED'"
                End If
            ...
            Else
                ...
                strSQL = "Select 'Dbms_Job.Broken(' || b.Job || ', False)' Sql" & vbNewLine & _
                        "From Dba_Jobs B, (Select ',' || Max(内容) || ',' 内容 From Zlupgradeconfig A Where a.项目 = '禁用的自动作业') C" & vbNewLine & _
                        "Where Instr(c.内容, ',' || b.Job || ',') > 0"
                Set rsDBAJobs = gclsBase.OpenSQLRecord(gcnOracle, strSQL, gstrSysName)
                lngAll = lngAll + rsObjectSQL.RecordCount + 1
                '取消停用标记
                ...
                If strTmp <> "" Then
                    ...
                    Else
                        strSQL = "Select a.Client_Name Job_Name," & vbNewLine & _
                                "       'Call DBMS_AUTO_TASK_ADMIN.enable(client_name => ' || Chr(39) || a.Client_Name || Chr(39) ||" & vbNewLine & _
                                "        ',operation => NULL,window_name => NULL)' Sql" & vbNewLine & _
                                "From Dba_Autotask_Client A where a.CLIENT_NAME in(" & strTmp & ")"
                    End If
                    Set rsSysScheduler = gclsBase.OpenSQLRecord(gcnOracle, strSQL, gstrSysName)
            ...
            '标记停用的后台作业
            If blnLock Then
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, "标记停用的后台作业", (lngSort * 100 / lngAll), strAdjustSQL, 0, 0)
            End If
            '停用或启用后台作业
            Do While Not rsObjectSQL.EOF
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "停用后台作业", "启用后台作业"), (lngSort * 100 / lngAll), rsObjectSQL!SQL, 0, 1)
                rsObjectSQL.MoveNext
            Loop
            
            '启用DBA后台作业
            If Not rsDBAJobs Is Nothing Then
                Do While Not rsDBAJobs.EOF
                    lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "停用后台作业", "启用后台作业"), (lngSort * 100 / lngAll), rsDBAJobs!SQL, 1, 1)
                    rsDBAJobs.MoveNext
                Loop
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "停用后台作业", "启用后台作业"), (lngSort * 100 / lngAll), "Update Zlupgradeconfig Set 内容 = Null Where 项目 = '禁用的自动作业度'", 0, 0)
            End If
            '取消停用标记
            If Not blnLock Then
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, "取消停用标记", (lngSort * 100 / lngAll), strAdjustSQL, 0, 0)
            End If
            '停用或启用系统调度
            If Not rsSysScheduler Is Nothing Then
                Do While Not rsSysScheduler.EOF
                    lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "停用系统调度", "启用系统调度"), (lngSort * 100 / lngAll), rsSysScheduler!SQL, 1, 0)
                    If blnLock Then
            ...
            '标记停用系统调度或取消系统调度停用标记
            If strTmp <> "" Then
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "标记停用系统调度", "取消系统调度停用标记"), (lngSort * 100 / lngAll), "Update Zlupgradeconfig Set 内容 = " & IIf(blnLock, "'" & strTmp & "'", "Null") & " Where 项目 = '禁用的系统调度'", 0, 0)
            End If
        Case FCT_触发器
            If blnLock Then
                '标记禁用的触发器
                strAdjustSQL = "Insert Into Zltriggers" & vbNewLine & _
                                        ...
                                        "  Where A.Table_Owner In (Select Upper(所有者)" & vbNewLine & _
            ...
            '标记禁用的触发器
            If blnLock Then
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, "标记禁用的触发器", (lngSort * 100 / lngAll), strAdjustSQL, 0, 0)
            End If
            '启用或禁用触发器
            Do While Not rsObjectSQL.EOF
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, IIf(blnLock, "停用后台作业", "启用后台作业"), (lngSort * 100 / lngAll), rsObjectSQL!SQL, 1, 0)
                rsObjectSQL.MoveNext
            Loop
            '删除触发器禁用标志
            If Not blnLock Then
                lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, "删除触发器禁用标志", (lngSort * 100 / lngAll), strAdjustSQL, 0, 0)
            End If
    End Select
    '记录执行情况
    lngSort = lngSort + 1: rsSQLStep.AddNew Array("序号", "步骤", "进度", "内容", "DBA", "CmdType"), Array(lngSort, "执行标记", (lngSort * 100 / lngAll), "Update Zlupgradeconfig Set 内容 = " & IIf(blnLock, 0, 1) & " Where 项目 = '" & strInfoName & "'", 0, 0)
