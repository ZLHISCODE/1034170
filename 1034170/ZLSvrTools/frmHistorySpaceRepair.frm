...
Public Function ShowRepair(ByVal frmMain As Form, ByVal lng系统 As Long, ByVal blnUpdate As Boolean, Optional ByVal strBakUser As String, Optional ByVal strBakDB As String, Optional ByVal blnCurDB As Boolean = True, Optional ByRef rsRepairSQL As ADODB.Recordset, Optional cnDBBAK As ADODB.Connection, Optional strDbLink As String) As Boolean
    ...
    If Not blnUpdate Then
        ...
        Else
            ...
            mstrBakDB = Nvl(rsTemp!名称) '历史库名称并不等于表空间,暂时这样处理
        End If
        'DBLINK服务器获取
        If mstrDBLink <> "" Then
        ...
        If mstrDBLink <> "" Then
            '历史表空间所有者验证,DBLINK必须先历史库验证，不然存在表空间查询没有权限问题
            If Not frmUserCheckLogin.ShowLogin(UCT_CurZLBAK, mcnBakDB, mstrBakOwnerName, mstrServer, mlngSys) Then Exit Function
        End If
    Else
        mstrBakOwnerName = strBakUser
        mblnCurDB = blnCurDB
        mstrDBLink = strDbLink
        mstrBakDB = strBakDB '历史库名称并不等于表空间,暂时这样处理
    ...
    End If
    '历史库表空间获取
    If mcnBakDB Is Nothing Or mcnBakDB = "" Then
        strSQL = "Select a.Tablespace_Name From All_Tables" & mstrDBLink & " A Where a.Table_Name = 'ZLBAKINFO' And a.Owner = [1]"
        Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, "获取历史库表空间", UCase(mstrBakOwnerName))
    Else
        strSQL = "Select a.Tablespace_Name From User_Tables A Where a.Table_Name = 'ZLBAKINFO'"
        Set rsTemp = gclsBase.OpenSQLRecord(mcnBakDB, strSQL, "获取历史库表空间")
    End If
    If Not rsTemp.EOF Then
        mstrBakDB = rsTemp!Tablespace_Name
    Else
    ...
    '获取索引表空间与LOB表空间
    If mcnBakDB Is Nothing Or mcnBakDB = "" Then
        strSQL = "Select a.Name Tablespace_Name" & vbNewLine & _
            "From V$TABLESPACE A" & vbNewLine & _
            "Where a.Name In ('" & mstrBakDB & "', '" & mstrBakDB & "_IDX', '" & mstrBakDB & "_LOB')"
        Set rsTemp = gclsBase.OpenSQLRecord(gcnOracle, strSQL, "获取历史库表空间", mstrBakDB)
    Else
        strSQL = "Select a.Tablespace_Name" & vbNewLine & _
                "From User_Tablespaces A" & vbNewLine & _
                "Where a.Tablespace_Name In ('" & mstrBakDB & "', '" & mstrBakDB & "_IDX', '" & mstrBakDB & "_LOB')"
        Set rsTemp = gclsBase.OpenSQLRecord(mcnBakDB, strSQL, "获取历史库表空间", mstrBakDB)
    End If
    rsTemp.Filter = "Tablespace_Name='" & mstrBakDB & "_IDX'"
    If Not rsTemp.EOF Then
        mstrBakIndexDB = rsTemp!Tablespace_Name
    Else
        mstrBakIndexDB = mstrBakDB
    End If
    rsTemp.Filter = "Tablespace_Name='" & mstrBakDB & "_LOB'"
    If Not rsTemp.EOF Then
        mstrBakLobDB = rsTemp!Tablespace_Name
    Else
