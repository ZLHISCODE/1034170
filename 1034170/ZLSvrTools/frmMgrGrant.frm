...
'功能：填充功能
    ...
    End If
    '显示该用户具有的角色
    gstrSQL = "Select /*+Rule */ 编号,标题 From zlSvrTools Where 编号 In (Select Column_Value From Table(Cast(f_Str2list('" & strProg & "') As Zltools.t_Strlist)))"
    rsTemp.Open gstrSQL, gcnOracle, adOpenStatic, adLockReadOnly
    Do Until rsTemp.EOF
        Set objItem = lvwProg.ListItems.Add(, , rsTemp("标题"))
        On Error Resume Next
        objItem.Icon = ImgBig.ListImages.Item(rsTemp!标题 & "").Index
