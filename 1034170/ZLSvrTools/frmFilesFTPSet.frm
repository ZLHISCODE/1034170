...
Private mblnSamleFTP As Boolean
...
Private Function CopyFileToServer(ByVal strFileServer As String, ByVal strSourcePath As String, ByVal strSharePath As String, _
    ...
    Dim strErrFile  As String, intCount As Integer
    ...
    If rsTmp.EOF Then
        mblnSamleFTP = False
    Else
        mblnSamleFTP = Val(rsTmp!使用简易ftp工具 & "") = 1
    End If
'   3.检查是否源文件与文件服务器是否一至
...
'        DoEvents
    ...
    With FileList
        ...
        For lngFile = 0 To .ListCount - 1
            ...
            If FtpupFile(strFile, strRemoteFile) Then
                If GetFTPFileSize(.List(lngFile)) <> CDbl(objFile.GetFile(strFile).Size) Then
                    If intCount Mod 3 = 0 And intCount <> 0 Then
                        strErrFile = strErrFile & "," & vbNewLine & .List(lngFile)
                    Else
                        strErrFile = strErrFile & "," & .List(lngFile)
                    End If
                    intCount = intCount + 1
                End If
            Else
    ...
    End With
    If strErrFile <> "" Then
        MsgBox "如下文件本地和上传到服务器上的文件存在大小差异，" & Mid(strErrFile, 2) & ",可能是因为简易FTP软件不支持查询命令，请仔细检查！", vbInformation, gstrSysName
    End If
...
'        Dim RetVal      As Long  '返回值
        ...
        For lngRow = 1 To .Rows - 1
                ...
                Else
                    '7z进行压缩
                    '4个文件不需要压缩 特殊处理
                    If InStr(G_NOT_ZIP_FILES, ";" & UCase(strFilename) & ";") > 0 Then
