...
Private Sub cmdOK_Click()
    ...
    Call zlDatabase.SetPara("排队分组类型", IIf(optGroupType(0).value, 0, 1), glngSys, glngModul)
...
Private Sub ReadLocalPara()
   ...
   '读取叫号方式
    ...
    If zlDatabase.GetPara("排队分组类型", glngSys, glngModul, "0") = 0 Then
        optGroupType(0).value = True
    Else
        optGroupType(1).value = True
    End If
