...
'        Next
    ElseIf Nvl(rsData("排队状态")) = 8 Then
        rriItem(mCol.排队状态).value = "已接诊"
    Else
...
'            Call comMenu_刷新
    ...
    If Control.Id = conMenu_Queue_CallThis Or Control.Id = conMenu_Queue_CallNext Then
        For i = 0 To rptCallList.Rows.Count - 1
            If rptCallList.Rows(i).GroupRow <> True Then
                If rptCallList.Rows(i).Record(mCol.业务ID).value = mstrCurrentWorkID And rptCallList.Rows(i).Record(mCol.业务类型).value = mlngCurrentWorkType Then
                    rptCallList.Rows(i).Selected = True
...
Private Sub InitQueueList()
   ...
   '初始化排队队列显示字段
    ...
    With Me.rptCallList.Columns
        ...
        Set Column = .Add(mCol.病人ID, "ID", Val(Split(str呼叫列宽, ",")(2)), False)
        Column.Visible = False
        ...
        Set Column = .Add(mCol.诊室, "诊室", Val(Split(str呼叫列宽, ",")(11)), True)
...
'        rptQueueList.SetImageList Imglist
        ...
        Set Column = .Add(mCol.病人ID, "ID", Val(Split(str排队列宽, ",")(2)), False)
        Column.Visible = False
        ...
        Set Column = .Add(mCol.诊室, "诊室", Val(Split(str排队列宽, ",")(11)), True)
...
Private Sub comMenu_查找()
    ...
    strCurKindInfo = GetIDKindInfo("就诊卡")
    If Trim(strCurKindInfo) = "" Then strCurKindInfo = GetIDKindInfo("就诊卡号")
    ...
    Call frmFind.ShowFind(mcnOracle, lngKindLen, Me)
...
Private Sub rptCallList_SelectionChanged()
    ...
    If rptCallList.SelectedRows(0).GroupRow = True Then
        If rptCallList.SelectedRows(0).Childs.Count > 0 Then
            ...
            mstrCurrentWorkID = rptCallList.SelectedRows(0).Childs(0).Record(mCol.业务ID).value
    ...
    mstrCurrentWorkID = rptCallList.SelectedRows(0).Record(mCol.业务ID).value
...
Private Sub rptQueueList_SelectionChanged()
    ...
    If rptQueueList.SelectedRows(0).GroupRow = True Then
        If rptQueueList.SelectedRows(0).Childs.Count > 0 Then
            ...
            mstrCurrentWorkID = rptQueueList.SelectedRows(0).Childs(0).Record(mCol.业务ID).value
    ...
    mstrCurrentWorkID = rptQueueList.SelectedRows(0).Record(mCol.业务ID).value
