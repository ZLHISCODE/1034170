...
'Public Sub OpenRecordset(rsTemp As ADODB.Recordset, ByVal strCaption As String)
''功能：打开记录集
...
'    strSQL = "Select Sum(发票金额) 发票金额 " & _
'             "From (Select Sum(Decode(记录性质, -1, Nvl(计划金额, 0), Nvl(发票金额, 0))) As 发票金额 " & _
'             "      From 应付记录 " & _
'             "      Where 付款序号 = [1] And (记录性质 <> 2 or 记录性质 = 2 and 计划金额 <> 发票金额) " & _
'             "      Union all " & _
'             "      Select Sum(b.计划金额) * -1 " & _
'             "      From 应付记录 A, 应付记录 B " & _
'             "      Where a.Id = b.Id And a.付款序号 = [1] And b.记录性质 = 2) "
'    strSQL = "Select Sum(Case When 记录性质 = 2 Or Not 记录性质 In (-1, 2) And Nvl(计划金额, 0) <> 发票金额 Then " & _
'             "             计划金额 " & _
'             "           Else " & _
'             "             发票金额 " & _
'             "           End) 发票金额 " & _
...
Public Sub zlinitSystemPara()
    ...
    With gSystemPara
        ...
        .Para_输入方式 = zldatabase.GetPara(44, 0, 0):        .Para_输入方式 = IIf(.Para_输入方式 = "", "11", .Para_输入方式)
     End With
