...
Private Const conMenu_FileRetry As Long = 32771 '记录单重算数据行
...
'2.其他命令根据主窗体业务的不同，可能不同
    ...
    With objMenu.CommandBar.Controls
        ...
        Set objControl = .Add(xtpControlButton, conMenu_FileRetry, "重算数据行(&T)"): objControl.IconId = 9024: objControl.BeginGroup = True
    End With
    ...
    With objTool.Controls
        ...
        Set objControl = .Add(xtpControlButton, conMenu_FileRetry, "重算"): objControl.IconId = 9024: objControl.Style = xtpButtonIconAndCaption: objControl.ToolTipText = "重算此病人的记录单文件": objControl.BeginGroup = True
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.ID
        ...
        Case conMenu_Delete
            ...
            If Val(rptList.FocusedRow.Record.Item(c_保留).Value) = -1 Then
                gstrSQL = "SELECT A.ID,B.开始时间" & _
                    ...
                    " WHERE B.ID=[1] And A.文件ID=B.ID And C.记录ID = a.id And  RowNum<2"
            Else
                gstrSQL = "SELECT A.ID,B.开始时间" & _
                    " FROM 病人护理数据 A,病人护理打印 C,病人护理文件 B,病人护理明细 D" & _
                    " WHERE B.ID=[1] And A.文件ID=B.ID and A.文件ID=C.文件ID And A.ID=C.记录ID And A.id=D.记录id And RowNum<2"
            End If
            Call SQLDIY(gstrSQL)
        ...
        Case conMenu_RetryPage '记录单页码重整
            ...
            End If
        Case conMenu_FileRetry
            If frmTendFileRetry.ShowMe(mlng病人ID, mlng主页ID, mint婴儿, mstrPrivs) Then
                mblnSaved = True
            End If
