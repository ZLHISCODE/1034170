...
Private Sub Form_Load()
    ...
    'arrFormat(纸张|纸向|高|宽|上边距|下边距|左边距|右边距|行高|固定行数|标题栏字体名|标题栏字体大小|标题文本|表上项字体名|表上项字体大小|表上项文本)
...
Public Function AnaliseData(ByVal objParent As Object, ByVal lngFileID As Long, ByVal lngPatiID As Long, ByVal lngPageId As Long) As Boolean
    Dim arrStyle() As String
    Dim rsTemp As New ADODB.Recordset
    'arrFormat(纸张|纸向|高|宽|上边距|下边距|左边距|右边距|行高|固定行数|标题栏字体名|标题栏字体大小|标题文本|表上项字体名|表上项字体大小|表上项文本)
    gstrSQL = "Select A.id 格式id, b.格式, d.Id as 文件id, d.内容文本, d.要素名称 " & vbNewLine & _
              "  From 病历文件列表 A, 病历页面格式 B, 病历文件结构 C, 病历文件结构 D, 病人护理文件 E " & vbNewLine & _
              "  Where a.种类 = 3  and e.Id = [1] And a.编号 = b.编号 And a.种类 = b.种类 And a.Id = c.文件id And c.内容文本 = '表格样式' And c.Id = d.父id And" & vbNewLine & _
              "  a.Id = e.格式id " & vbNewLine & _
              "  order by d.id "
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取该病人使用的记录单文件", lngFileID)
    With rsTemp
        If .RecordCount > 0 Then
            .Filter = ""
            mlngFormat = !格式ID
            arrStyle = Split(!格式, ";")
            arrFormat(0) = arrStyle(0)
            arrFormat(1) = arrStyle(1)
            arrFormat(2) = arrStyle(2)
            arrFormat(3) = arrStyle(3)
            arrFormat(4) = arrStyle(4)
            arrFormat(5) = arrStyle(5)
            arrFormat(6) = arrStyle(6)
            arrFormat(7) = arrStyle(7)
            Do While Not .EOF
                Select Case !要素名称
                    Case "最小行高"
                        arrFormat(8) = !内容文本
                    Case "表头层数"
                        arrFormat(9) = !内容文本
                    Case "标题字体"
                        arrFormat(10) = Split(!内容文本, ",")(0)
                        arrFormat(11) = Split(!内容文本, ",")(1)
                    Case "标题文本"
                        arrFormat(12) = !内容文本
                    Case "文本字体"
                        arrFormat(13) = Split(!内容文本, ",")(0)
                        arrFormat(14) = Split(!内容文本, ",")(1)
                End Select
                .MoveNext
            Loop
        Else
...
'    arrFormat = Split(strTitle, "|")
    ...
    If Not ReadStruDef Then
        ...
        Exit Function
    End If
    If Not ReadData(lngFileID) Then Exit Function
    Unload frmTendFilePreview
    AnaliseData = True
End Function

Private Function ReadData(ByVal lng文件ID As Long) As Boolean
    ...
    gstrSQL = _
        " Select 病人ID,id,主页ID,婴儿,格式id, ID, 科室id" & vbNewLine & _
        " From 病人护理文件 A" & vbNewLine & _
        " Where id=[1] "
    
    Set rsPati = zlDatabase.OpenSQLRecord(gstrSQL, "提取该病人使用的记录单文件", lng文件ID)
...
'        mobjParent.Caption = strCaption & Space(2) & "一共有" & rsPati.RecordCount & "份护理文件，正在处理：" & rsPati.AbsolutePosition
        mlngFormat = rsPati!格式ID
        Call ReadStruDef
        '活动项目处理(数据列放在最后)
        Call PreActiveCOL(rsPati!ID)
        '装入数据
        mlngFile = Val(NVL(rsPati!ID))
        Call SQLCombination
        gstrSQL = mstrSQL
'        If mlngFile = 86 Then Stop
        lng病人ID = Val(rsPati!病人ID)
        lng主页ID = Val(rsPati!主页ID)
        intBaby = Val(rsPati!婴儿)
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取护理数据", CLng(rsPati!ID), lng病人ID, lng主页ID, intBaby)
    ...
    For lngRow = 1 To lngCount
        ...
        If lngRecord <> 0 Then
            ...
            If blnSave = True Then
                ...
                gstrSQL = "ZL_病人护理打印_UPDATE(" & mlngFile & ",to_date('" & strTime & "','yyyy-MM-dd hh24:mi:ss')" & "," & lngMutilRow & ",null,1)"
...
Private Sub SQLCombination()
    mstrSQL = "Select 备用,发生时间," & Mid(mstrSQL列, 12) & mstrSQLActive列 & vbCrLf & _
                ...
                "               Where l.Id = c.记录id And l.文件ID+0=f.ID+0 " & _
