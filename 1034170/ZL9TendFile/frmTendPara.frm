...
Public Function ShowPara(ByVal frmMain As Object, ByVal strPrivs As String) As Boolean
    ...
    chk(5).Enabled = chk(0).Value = 1
    chk(5).Value = Val(zlDatabase.GetPara("允许数据同步", glngSys, 1255, "0", Array(chk(5)), InStr(mstrPrivs, "护理选项设置") > 0))
...
Private Sub chk_Click(Index As Integer)
    ...
    If Index = 0 Then chk(5).Enabled = chk(0).Value = 1
...
Private Sub cmdOK_Click()
    ...
    Call zlDatabase.SetPara("允许数据同步", chk(5).Value, glngSys, 1255, InStr(mstrPrivs, "护理选项设置") > 0)
