...
Begin VB.Form frmEditAssistant 
   ...
   Begin VB.PictureBox picDef 
      ...
      Begin VB.CommandButton cmdAdd 
         Height          =   270
         Left            =   5280
         Picture         =   "frmEditAssistant.frx":4E3FE
         Style           =   1  'Graphical
         TabIndex        =   20
      ...
      Begin VB.Label lblPrompt 
         BackStyle       =   0  'Transparent
         Caption         =   $"frmEditAssistant.frx":4E4C8
         Height          =   645
         Left            =   450
         TabIndex        =   27
         Top             =   495
      ...
      Begin VB.Image imgClose 
         Height          =   285
         Left            =   5445
         Picture         =   "frmEditAssistant.frx":4E573
         Stretch         =   -1  'True
         Top             =   45
         Width           =   270
      End
            ...
            Do While Not rsLisItem.EOF
                ...
                Do While Not rsItem.EOF
                    Select Case rsItem!node_name & ""
                        Case "检验报告id"
                            L_Item.检验报告id = Val(rsItem!node_value & "")
                        Case "申请id"
                            L_Item.申请id = Val(rsItem!node_value & "")
                        Case "紧急标志"
                            L_Item.紧急标志 = Val(rsItem!node_value & "")
                        Case "检验项目"
                            L_Item.检验项目 = rsItem!node_value & ""
                        Case "标本序号"
                            L_Item.标本序号 = Val(rsItem!node_value & "")
                        Case "是否微生物"
                    ...
                    rsItem.MoveNext
                Loop
                Set objNode = tvw_s.Nodes.Add("％", tvwChild, "％" & L_Item.检验报告id & "_" & L_Item.申请id, L_Item.检验项目 & "[" & L_Item.审核时间 & "]", "Close")
...
Private Function ShowList(Optional ByVal lng分类id As Long) As Boolean
    ...
    ElseIf Mid(tvw_s.SelectedItem.Key, 1, 1) = "=" Then
        ...
        strSQL = "" & _
            ...
            "      c.执行性质 In (1, 2, 3, 4) And Nvl(d.执行分类, 0) = [4] And d.Id = c.诊疗项目id And a.相关id = c.Id And c.上次执行时间 Is Not Null" & vbNewLine & _
...
'      strPreview=预览医嘱内容效果
    ...
    For i = 1 To Len(strText)
        ...
        End If
    Next
    If intLeft = 0 And intRight = 0 Then GoTo GOCHECK
    If intLeft <> intRight Then
        strMsg = """[""与""]""括号不配对。"
        GoTo EndLine
    End If
