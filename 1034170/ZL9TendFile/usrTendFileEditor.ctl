...
Private Sub DataMap_Init(ByVal rsSource As ADODB.Recordset)
    ...
    Call Record_Init(mrsCellMap, "ID," & adLongVarChar & ",50|页号," & adDouble & ",18|行号," & adDouble & ",18|" & _
            ...
            "标记," & adLongVarChar & ",100|汇总," & adDouble & ",1|记录组号," & adDouble & ",1|删除," & adDouble & ",1|分组汇总," & adDouble & ",1")
...
Private Sub InitEnv()
    ...
    '打开现存在的所有护理记录项目
    gstrSQL = " Select   项目序号,upper(项目名称) AS 项目名称,分组名,项目类型,项目性质,项目长度,项目小数,项目表示,项目单位,项目值域,缺省值,护理等级,应用方式,说明,分组汇总" & _
              " From 护理记录项目 B" & _
              " Order by 项目序号"
...
Private Function SignName(ByVal strStart As String, ByVal strSignTime As String, ByVal bln审签 As Boolean, _
    ...
    '43588:刘鹏飞,2012-09-13,添加记录单审签模式
    Set oSign = frmTendFileSign.ShowMe(Me, mstrPrivs, mlng文件ID, mlng病人ID, mlng主页ID, mlng病区ID, mintVerify_Last, strSource, bln审签, str状态, str错误, mintSignMode, blnExchange)
    On Error GoTo ErrHand
    ...
    '                    strSQLTime(ReDimArray(strSQLTime)) = gstrSQL
        ...
        '                    他人记录_IN IN NUMBER := 1,
                            ...
                            If strRelationNO = "" Then
                                gstrSQL = gstrSQL & ",NULL,'" & NVL(!标记) & IIF(Val(NVL(!分组汇总)) = 1, "',1", "'") & ")"
                            Else
                                gstrSQL = gstrSQL & "," & Val(strRelationNO) & ",'" & NVL(!标记) & IIF(Val(NVL(!分组汇总)) = 1, "',1", "'") & ")"
                            End If
                            strSQL(ReDimArray(strSQL)) = gstrSQL
                            blnSaved = True
                        Next
                    ...
                    ElseIf !列号 = 0 Then
                        ...
                        For intPos = 0 To intMax
                            ...
                            End If
                            strPart = NVL(!部位)
                            gstrSQL = "Zl_护理二次汇总_Update(" & mlng文件ID & ",to_date('" & IIF(strPart <> "", strPart, strDatetime) & "','yyyy-MM-dd hh24:mi:ss'),to_date('" & strDatetime & "','yyyy-MM-dd hh24:mi:ss'),1," & _
                                    arrOrder(intPos) & ",'" & arrValue(intPos) & "',null" & _
                                    ",NULL," & IIF(IsNull(mrsCellMap!记录组号), IIF(NVL(!汇总, 0) = 1, 0, "NULL"), Val(NVL(mrsCellMap!记录组号))) & ",1," & IIF(arrValue(intPos) = "", 1, 0) & ")"
                            strSQL(ReDimArray(strSQL)) = gstrSQL
                            blnSaved = True
                        Next
                        mrsItems.Filter = 0
...
Private Sub cmdOK_Click()
    ...
    Dim lng分组汇总 As Long, strFields As String, strValues As String
        ...
        Do While Not .EOF
            If NVL(mrsItems!分组汇总) <> "" Then
                arrItem = Split(mstrColCorrelative, "|")
                    ...
                    If InStr(1, "," & Join(arrColCollect, ",") & ",", "," & Split(arrCorrelative(1), ",")(0) & ",") > 0 Then
                        arrCorrelative = Split(arrItem(lngCount), ";")
                        Set rsCategorical = CaclGroup(Split(arrCorrelative(0), ",")(1) & ":" & Split(arrCorrelative(1), ",")(1), strStartDate, strEndDate)
                        
                        
                        Do While Not rsCategorical.EOF
                            If InStr(1, "|" & NVL(mrsItems!分组汇总) & "|", "|" & NVL(rsCategorical!项目名称) & "|") > 0 Then
'                                mrsCellMap.Filter = "部位 = '" & rsCategorical!发生时间 & "' and 标记='" & Val(NVL(mrsItems!项目序号)) & "'"
                                lngRow = lngStart + Val(VsfData.TextMatrix(lngStart, mlngRowCurrent))
                                strFields = "ID|页号|行号|列号|起始行号|记录ID|数据|汇总|记录组号|删除|标记|分组汇总|部位"
                                strKey = mint页码 & "," & lngRow & "," & 0
                                strValue = strKey & "|" & mint页码 & "|" & lngRow & "|" & 0 & "|" & lngStart & "|" & _
                                Val(rsCategorical!明细ID) & "|" & Val(NVL(rsCategorical!项目内容)) & "|0|0|0|" & Val(!项目序号) & "|1|" & CStr(rsCategorical!发生时间)
'                                If Not mrsCellMap.RecordCount > 0 Then
                                    Call Record_Add(mrsCellMap, strFields, strValue)
'                                End If
                            End If
                            rsCategorical.MoveNext
                        Loop
                    End If
                Next
            End If
...
Private Function CheckValid(strReturn As String, ByVal strOrders As String, strInfo As String) As Boolean
            ...
            If strText <> "" Then
                ...
                If blnCheck Then
                    If mrsItems!项目类型 = 0 And InStr(1, "0,4", mrsItems!项目表示) <> 0 Then
                        blnCanves = IsCanves(Val(strOrders))
                        blnNumber = True
                        If NVL(mrsItems!项目小数, 0) <> 0 Then   '等于零是通过控件的MaxLength来控制的,存在违规录入小数点
                            ...
                            If InStr(1, strText, ".") <> 0 Then
                                ...
                                End If
                            End If
                            strText = IIF(IsNumeric(arrData(i)) Or blnCanves, arrData(i), Val(arrData(i)))
    ...
    End If
    strQHZ = SubstrFormat(strFormat1, arrOrder)
    If InStr(strFormat, strQHZ) = Len(strFormat) And InStr(1, strFormat1, "心率") > 0 And InStr(1, strFormat1, "脉搏") > 0 Then strFormat = Replace(strFormat, strQHZ, "")
...
Private Function CaclGroup(ByVal strItem As String, ByVal strStart As String, ByVal strEnd As String) As ADODB.Recordset
'--------------------------------------------------------
'功能:获取汇总项目分组汇总信息
'参数:
'    strItem:分类项目和汇总项目的项目序号,格式:6:7
...
'--------------------------------------------------------
    ...
    gstrSQL = "Select 发生时间, 项目名称, 项目内容,明细ID" & vbNewLine & _
        "       From (Select 发生时间, Max(项目名称) 项目名称, Nvl(Max(项目内容), 0) 项目内容, Max(明细ID) 明细ID" & vbNewLine & _
        "              From (Select a.Id, B.id as 明细ID,a.发生时间, Decode(b.项目序号, d.C1, b.记录内容, Null) 项目名称, Decode(b.项目序号, d.C2, b.记录内容, Null) 项目内容" & vbNewLine & _
        "                     From 病人护理数据 a, 病人护理明细 b, 病人护理打印 c," & vbNewLine & _
        "                          (Select C1, C2 From Table(Cast(f_Num2list2([2]) As Zltools.t_Numlist2))) d" & vbNewLine & _
        "                     Where a.Id = b.记录id And a.Id = c.记录id And Nvl(a.汇总类别, 0) = 0 And a.文件id = [1] And" & vbNewLine & _
        "                           (b.项目序号 = d.C1 Or b.项目序号 = d.C2) And b.终止版本 Is NULL And b.记录类型=1 And " & vbNewLine & _
