...
Private Sub cmdOK_Click()
    ...
    If mbln执行报到时结算 Then
        If Not mobjSquareCard Is Nothing Then
            If mobjSquareCard.zlSquareAffirm(Me, p医技工作站, mstrPrivs, mlng病人ID, mlng卡类别ID, False, , , mlng医嘱ID & "") = False Then
                Exit Sub
            End If
        Else
    ...
    StrSQL = "ZL_病人医嘱执行_Plan(" & mlng医嘱ID & "," & mlng发送号 & ",1," & _
        ...
        "'" & txtItem(3).Text & "','" & NeedName(cboSex.Text) & "','" & txtItem(4).Text & "'," & _
...
Private Sub Form_Load()
    ...
    Else
        ...
        If Not IsNull(rsTmp!性别) Then
            Call GetCboIndex(cboSex, rsTmp!性别, True)
        End If
    ...
    If Not IsNull(rsTmp!执行间) Then
        Call GetCboIndex(cboRoom, rsTmp!执行间, True)
    End If
