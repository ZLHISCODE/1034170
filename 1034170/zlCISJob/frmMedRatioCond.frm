...
Private Sub Form_Load()
    ...
    If IsNumeric(strDiffDate) Then
        ...
        End Select
    Else
        Call zlControl.CboSetIndex(cboTim.hwnd, 4)
...
'设置时间范围
    ...
    If intDateCount = -1 Then
        ...
        If Not frmSelectTime.ShowMe(Me, mdatOutBegin, mdatOutEnd, cboOutTime) Then
            '取消时恢复原来的选择
            Call zlControl.CboSetIndex(cboOutTime.hwnd, mintOutPreTime)
            Exit Sub
        End If
    Else
        mdatOutEnd = mdatCurr
        ...
        Set objListItem = lvwDept.ListItems.Add(, "_" & rsTmp!ID, "" & rsTmp!名称)
            ...
            If bytWay <> 0 Then
                With cboDept
                    ...
                    If InStr("," & strDepIDs & ",", "," & rsTmp!ID & ",") <> 0 Then Call zlControl.CboSetIndex(cboDept.hwnd, .NewIndex)
                End With
            End If
        rsTmp.MoveNext
    Loop
    If cboDept.ListCount > 0 Then
        If cboDept.ListIndex = -1 Then Call zlControl.CboSetIndex(cboDept.hwnd, 0)
        If bytWay = 1 Then Call zlControl.CboLocate(cboDept, mstrPreDocDepID, True)
        If bytWay = 2 Then Call zlControl.CboLocate(cboDept, mstrPrePatDepID, True)
        Call cboDept_Click
    End If
    Screen.MousePointer = 0
    Exit Sub
    ...
    If optWay(2).Value Then
        ...
        Do While Not rsTmp.EOF
            With cboDoc
                ...
                .ItemData(.NewIndex) = rsTmp!ID
                If UserInfo.ID = rsTmp!ID Then Call zlControl.CboSetIndex(cboDoc.hwnd, .NewIndex)
            End With
            rsTmp.MoveNext
        Loop
        ...
        If cboDoc.ListCount > 0 Then
            If Not zlControl.CboLocate(cboDoc, mstrPrePatDocID, True) Then cboDoc.ListIndex = 0
        End If
    End If
