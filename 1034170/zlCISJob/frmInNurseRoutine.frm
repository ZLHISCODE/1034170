...
'    If rsThis Is Nothing Then
        ...
        '通过在主界面直接查找的出院病人和转科病人可能不在范围内，此处需要重新加载
 ...
 '功能：刷新子窗体数据及状态
    ...
    Else
        With mPatiInfo
            ...
            Case "护理病历"
                Call mclsTendEPRs.zlRefresh(mlng病人ID, .主页ID, mlng病区ID, True, False, .数据转出)
            Case "监护"
...
'功能：选择某次住院记录时，读取相关的病人信息
	...
	lbl床号.Caption = "床:" & Nvl(!出院病床)
        ...
        Else
			...
			Call CheckDeath(mlng病人ID, mlng主页ID)
        End If
...
Private Sub AddPages()
    ...
    End If
    If bln留观 = True Then
        Call zlControl.CboSetWidth(cboPages.hwnd, 2000)
    End If
    Call cboPages_Click
