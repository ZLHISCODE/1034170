...
'共公接口
    ...
    Call frmOPSExamine.Show(0, frmParent)
...
'加载权限比操作员低的医生
    ...
    If tbcSub.Selected.Tag = "待审核" Then
        ...
        strTmp = ",(Select Distinct 开嘱医生 From 病人医嘱记录 F Where  f.审核状态 = 1 and f.医嘱状态=1 And F.开嘱时间 Between [3] And [4] And f.诊疗类别 ='F') F"
    Else
        ...
        strTmp = ",(Select Distinct f.开嘱医生 From 病人医嘱记录 F,病人医嘱状态 G Where f.id=g.医嘱id and G.操作类型 in (11,12)" & _
            " And G.操作时间 Between [3] And [4]  And f.诊疗类别 ='F') F"
    End If
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(StrSQL, Me.Caption, mlngLevel, Val(cboDept.ItemData(cboDept.ListIndex)), datBegint, datEnd)
    ...
    With vsAudit
        For i = lngBegin To lngEnd
            ...
            If Val(.Cell(flexcpData, i, col_选择) & "") <> "0" Then
                If Val(.Cell(flexcpData, i, col_选择) & "") = 1 Then lngMsgRow = i
        ...
        With vsAudit
            If Val(.TextMatrix(lngMsgRow, col_病人来源)) = 2 Then
                ...
                Call ZLHIS_CIS_001(mclsMipModule, Val(.TextMatrix(lngMsgRow, col_病人Id)), .TextMatrix(lngMsgRow, col_病人姓名), .TextMatrix(lngMsgRow, col_住院号), "", 2, Val(.TextMatrix(lngMsgRow, col_主页ID)), _
                    Val(.TextMatrix(lngMsgRow, COL_当前病区ID)), "", Val(.TextMatrix(lngMsgRow, COL_出院科室ID)), "", "", .TextMatrix(lngMsgRow, COL_当前床号), Val(.TextMatrix(lngMsgRow, col_医嘱ID)), 0, 1, "F", rsTmp!操作类型 & "", _
...
Private Sub Form_Load()
    ...
    strHead = strHead & ";门诊号;住院号;当前床号;开嘱医生;开嘱时间;开嘱科室id;出院科室id;当前病区id;主项目ID"
...
'参数：是否加载已审核医嘱,为空为加载待审核医嘱
    ...
    StrSQL = "Select a.Id, a.相关id, Nvl(a.相关id, a.Id) As 组id, a.诊疗类别, Null As 选择, Null As 输入, A.姓名, p.当前床号 As 床号," & vbNewLine & _
            ...
            ",p.门诊号,p.住院号,p.当前床号,a.开嘱医生,To_Char(a.开嘱时间,'YYYY-MM-DD HH24:MI') As 开嘱时间,a.开嘱科室id,e.出院科室id,e.当前病区id,a.诊疗项目id" & _
            ...
            "p.门诊号,p.住院号,p.当前床号,a.开嘱医生, a.开嘱时间 ,a.开嘱科室id,e.出院科室id,e.当前病区id,a.诊疗项目id" & _
