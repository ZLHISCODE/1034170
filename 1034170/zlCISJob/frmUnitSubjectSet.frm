...
Begin VB.Form frmUnitSubjectSet 
   ...
   Begin XtremeCommandBars.ImageManager ImgMain 
      ...
      _ExtentY        =   635
      _StockProps     =   0
      Icons           =   "frmUnitSubjectSet.frx":185E
   End
...
'功能显示现有图片信息
    ...
    '获取计算picback的位置的宽度
    vRect = GetControlRect(imaCustom.hwnd)
    vRect1 = GetControlRect(fraInfo.hwnd)
...
'------------------------------------------------------------------------------------------
    ...
    Set Item = Record.AddItem("")
    If lngImgIndex > mlngImgIndex And lngImgIndex <= zlCommFun.GetPaitSignImageList(0).ListImages.Count - 1 Then
        Item.Icon = zlCommFun.GetPaitSignImageList(0).ListImages(lngImgIndex).Index
    End If
    If blnParent Then Item.BackColor = RGB(255, 255, 255)
        ...
        With UnitReportControl.FocusedRow
            If Not UnitReportControl.FocusedRow Is Nothing Then
                ...
                Else
                    ...
                    chkSpecial.Enabled = fraUnit.Tag <> ""
                    If m病区ID = 0 Then
...
'----------------------------------------------------------
    ...
    If lngTracerID <> 0 Then
        strSQL = "select 1 From 病区标记记录" & _
            "   where  " & IIf(lngUnitID = 0, " 病区Id IS NULL ", " 病区Id=[1] ") & " and 主题序号=[2] and 标记序号=[3] and rownum<2"
    ...
    Else
        strSQL = _
            ...
            " WHERE  " & IIf(lngUnitID = 0, " B.病区ID IS NULL ", " A.病区ID=B.病区ID ") & " And A.主题序号=B.主题序号 And " & IIf(lngUnitID = 0, " A.病区ID IS NULL ", " A.病区ID=[1] ") & " And A.主题序号=[2]  " & vbNewLine & _
            " and rownum<2"
...
'--------------------------------------------------------------------
    ...
    strSQL = _
        ...
        " From 病区标记内容" & _
        " Where " & IIf(lng病区ID = 0, " 病区Id IS NULL ", " 病区Id=[1] ") & " and 主题序号=[2]"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng病区ID, lngPreVId)
    If rsTemp.EOF Then
        lngPreID = 1
    Else
        lngPreID = Val(zlCommFun.Nvl(rsTemp!PreID, 0)) + 1
    End If
...
'------------------------------------------------------------------------
    ...
    If lng主题序号 = 0 Then
        strSQL = _
            ...
            " where " & IIf(lng病区ID = 0, " 病区Id IS NULL ", " 病区Id=[1] ") & " And 标记序号=0"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "病区标记内容", lng病区ID)
    Else
        strSQL = _
               " select 主题序号,说明 from 病区标记内容" & _
               " where " & IIf(lng病区ID = 0, " 病区Id IS NULL ", " 病区Id=[1] ") & " and 主题序号<>[2] and 标记序号=0"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "病区标记内容", lng病区ID, lng主题序号)
    End If
