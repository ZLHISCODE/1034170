...
Private Function IsValid() As Boolean
    ...
    '部门性质同时设置药库药房/卫材库发料部门时，检查是否有药品/卫材设置了药库分批药房不分批时
    For i = 1 To lvw性质.ListItems.Count
        If lvw性质.ListItems(i).Checked = True Then
            str部门性质 = IIF(str部门性质 = "", "", str部门性质 & ";") & lvw性质.ListItems(i)
        End If
    Next
    
    If InStr(str部门性质, "药库") > 0 And (InStr(str部门性质, "药房") > 0 Or InStr(str部门性质, "制剂室") > 0) Then
        gstrSQL = "Select Distinct b.编码, b.名称, b.规格" & vbNewLine & _
                        "From 药品规格 A, 收费项目目录 B" & vbNewLine & _
                        "Where a.药品id = b.Id And a.药库分批 = 1 And a.药房分批 = 0" & vbNewLine & _
        ...
        Do While Not rsTemp.EOF
            i = i + 1
            If i > 3 Then Exit Do
            strMsg = IIF(strMsg = "", "", strMsg & vbCrLf) & "[" & rsTemp!编码 & "]" & rsTemp!名称 & "(" & rsTemp!规格 & ")" & "；"
            rsTemp.MoveNext
        Loop
        
        If rsTemp.RecordCount > 0 Then
            If InStr(mstr部门性质, "药库") > 0 And (InStr(mstr部门性质, "药房") > 0 Or InStr(mstr部门性质, "制剂室") > 0) Then
                MsgBox "当前部门工作性质同时设置了药库和药房，" & vbCrLf & strMsg & vbCrLf & IIF(i > 3, "等以上超过3种", "以上" & i & "种") & "药品设置了药库分批药房不分批，请注意查看！", vbExclamation, gstrSysName
            Else
                MsgBox strMsg & vbCrLf & IIF(i > 3, "等以上超过3种", "以上" & i & "种") & "药品设置了药库分批药房不分批，在设置工作性质时不能同时设置药库和药房！", vbExclamation, gstrSysName
                Exit Function
            End If
        End If
    End If

    If (InStr(str部门性质, "卫材库") > 0 Or InStr(str部门性质, "虚拟库房") > 0) And (InStr(str部门性质, "发料部门") > 0 Or InStr(str部门性质, "制剂室") > 0) Then
        gstrSQL = "Select Distinct b.编码, b.名称, b.规格" & vbNewLine & _
                        "From 材料特性 A, 收费项目目录 B" & vbNewLine & _
                        "Where a.材料id = b.Id And a.库房分批 = 1 And a.在用分批 = 0" & vbNewLine & _
                        "      And (b.撤档时间 Is Null Or b.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD'))" & vbNewLine & _
                        "      And Rownum < 5"
        ...
        If rsTemp.RecordCount > 0 Then
            If (InStr(mstr部门性质, "卫材库") > 0 Or InStr(mstr部门性质, "虚拟库房") > 0) And (InStr(mstr部门性质, "发料部门") > 0 Or InStr(mstr部门性质, "制剂室") > 0) Then
                MsgBox "当前部门工作性质同时设置了卫材库和发料部门，" & vbCrLf & strMsg & vbCrLf & IIF(i > 3, "等以上超过3种", "以上" & i & "种") & "卫材设置了卫材库房分批发料部门不分批，请注意查看！", vbExclamation, gstrSysName
            Else
                MsgBox strMsg & vbCrLf & IIF(i > 3, "等以上超过3种", "以上" & i & "种") & "卫材设置了卫材库房分批发料部门不分批，在设置工作性质时不能同时设置卫材库和发料部门！", vbExclamation, gstrSysName
                Exit Function
            End If
        End If
    End If
...
'        lvw性质.Top = cmb诊疗科目编码.Top
    ...
    For i = 1 To lvw性质.ListItems.Count
        If lvw性质.ListItems(i).Checked = True Then
            mstr部门性质 = IIF(mstr部门性质 = "", "", mstr部门性质 & ";") & lvw性质.ListItems(i)
        End If
    Next
