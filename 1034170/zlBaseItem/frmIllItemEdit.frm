...
Private Sub Init手术操作类型(Optional ByVal str手术操作类型 As String)
'功能:初始化手术操作类型数据
'参数:str手术操作类型-指向指定的手术操作类型
    Dim rsTemp As New ADODB.Recordset
    
    gstrSQL = "Select 名称, 编码 From 手术操作类型 Order By 编码"
    
    Err = 0: On Error GoTo ErrHand:
    Set rsTemp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption)
    
    cbo手术操作类型.AddItem ""
    Call zlControl.CboAddData(cbo手术操作类型, rsTemp, False)
    Call zlControl.CboSetText(cbo手术操作类型, str手术操作类型)
...
Private Sub Init手术类型(Optional ByVal str手术类型 As String)
    ...
    '----------------------------------------------------------------------------
    Dim rsTemp As New ADODB.Recordset
    gstrSQL = "Select 名称, 编码 From 手术类型 Order By 编码"
    ...
    cbo手术类型.AddItem ""
    Call zlControl.CboAddData(cbo手术类型, rsTemp, False)
    Call zlControl.CboSetText(cbo手术类型, str手术类型)
...
'返回值:成功返回True,否则为False
    ...
    If mstrID = "" Then       '新增一条记录
        ...
        gstrSQL = gstrSQL & "" & cbo手术操作类型.ItemData(cbo手术操作类型.ListIndex) & ")"
    Else    '修改
        ' Zl_疾病编码目录_Update
...
'重新显示某一行,用于刷新
    ...
    gstrSQL = "" & _
    "   Select A.ID,A.编码,附码,A.名称,A.简码 As 拼音码,A.五笔码,A.说明,A.手术类型,B.名称 手术操作类型,A.统计码,A.性别限制," & _
    ...
    "   From 疾病编码目录 A,手术操作类型 B  Where A.手术操作类型 = B.编码(+) And A.ID=[1]"
...
'        Height = Height - 450
    ...
    If strID <> "" Then
        ...
        gstrSQL = "select A.ID,A.编码,A.序号,A.附码,A.名称,A.简码 As 拼音码,A.五笔码,A.性别限制,A.疗效限制,A.分娩,A.手术类型,C.名称 手术操作类型,A.统计码,A.说明 " & _
                ",B.序号 as 统计序号,B.ID as 分类ID,B.名称 as 分类名称,B.编码范围,Nvl(A.适用范围,0) As 适用范围 " & _
                " from 疾病编码目录 A,疾病编码分类 B, 手术操作类型 C " & _
                " where B.ID(+)=A.分类ID and a.手术操作类型 = c.编码(+) And A.ID=[1] "
        ...
        Call Init手术操作类型(Nvl(rsTemp!手术操作类型))
