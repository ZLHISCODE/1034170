...
Private Const mstr疾病 As String = "编码,1200,0,1;附码,1200,0,0;名称,2440,0,2;拼音码,1000,0,0;五笔码,1000,0,0;性别限制,900,0,0;手术类型,900,0,0;手术操作类型,1100,0,0;统计码,800,0,0;提醒疗效,900,0,0;分娩信息,800,0,0;适用范围,1000,0,0;说明,3000,0,0;建档时间,1400,0,0;撤档时间,1400,0,0"
...
'功能:更新ListView中的数据
    ...
    Else
        ...
        End If
    End If
    If gstrSQL <> "" Then gstrSQL = " And " & gstrSQL
    gstrSQL = "" & _
    "   Select A.ID,A.编码,附码,A.名称,A.简码 as 拼音码,A.五笔码,A.说明,A.性别限制,A.疗效限制 as 提醒疗效,A.手术类型,B.名称 手术操作类型," & _
    ...
    "   From 疾病编码目录 A,手术操作类型 B  " & _
    "   Where A.手术操作类型 = B.编码(+) " & gstrSQL & IIF(mnuViewStop.Checked, "", " and (a.撤档时间 is null or a.撤档时间>=to_date('3000-01-01','yyyy-mm-dd'))")
...
'参数:strFilePath-保存文件的位置
    ...
    gstrSQL = "Select a.Id, NULL AS ID_TEMP, a.编码, a.序号, a.附码, a.统计码, a.名称, a.简码, a.五笔码, a.说明, a.性别限制, a.疗效限制, a.手术类型,a.手术操作类型, a.分娩, a.分类id, NULL As 分类ID_TEMP, a.适用范围, a.类别" & vbNewLine & _
    ...
    With rsContent
        ...
        strTitle = "Insert Into 疾病编码目录 (ID, 分类id, 类别, 编码, 序号, 附码, 名称, 简码, 五笔码, 说明, 性别限制, 疗效限制, 手术类型, 手术操作类型, 分娩, 适用范围)" & vbCrLf
        For i = 1 To .RecordCount
            strName = FuncGetStr(!名称)
            strTemp = "Select " & !ID_TEMP & "," & !分类ID_TEMP & ",'" & !类别 & "','" & !编码 & "'," & !序号 & ",'" & !附码 & "','" & strName & "','" & !简码 & "','" & !五笔码 & "','" & !说明 & "','" & !性别限制 & "','" & _
                    !疗效限制 & "','" & !手术类型 & "'," & !手术操作类型 & ",'" & !分娩 & "','" & !适用范围 & "' From Dual UNION ALL" & vbCrLf
