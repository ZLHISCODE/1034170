...
Private Sub GetItemList(ByVal strInput As String, ByVal strItemType As String, Optional ByVal lngItemID As Long = 0)
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, strInput & "%", "%" & UCase(strInput) & "%", strInput, strItemType, lngItemID)
...
Private Function IsValidate() As Boolean
    ...
    For mintStage = 1 To Me.UdStage.Value - 1
        ...
        If curStart < Val(Me.txtMoney(mintStage - 1).Text) Then
            MsgBox "第" & mintStage + 1 & "段错误，应收段值必须由小到大。", vbExclamation, gstrSysName
            txtMoney(mintStage).SetFocus
            Exit Function
        End If
        If dblTax = Val(Me.txtTax(mintStage - 1).Text) Then
            MsgBox "第" & mintStage + 1 & "段错误，相邻段实收比率相同，无意义。", vbExclamation, gstrSysName
