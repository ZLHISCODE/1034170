...
'加载当前登录用户具有管理权限的角色和系统
    ...
    On Error GoTo errH
    strSQL = "Select Substr(a.角色, 4) 角色 From zlUserRoles A Where a.用户 = User And a.管理 = 1 Order By a.角色"
    ...
    strSQL = "Select Distinct m.编号, m.名称, m.共享号, m.所有者, m.安装日期, m.正常安装, m.版本号" & vbNewLine & _
            ...
            "     (Select a.角色 From zlUserRoles A Where a.用户 = User And a.管理 = 1) N" & vbNewLine & _
            "Where r.系统 = m.编号 And r.角色 = n.角色" & vbNewLine & _
            "Order By m.编号"
...
'加载人员
    ...
    On Error GoTo errH
    strSQL = "Select a.用户名, d.姓名, Decode(e.角色, Null, 0, 1) 权限" & vbNewLine & _
            "From 上机人员表 A, 部门人员 B, 部门表 C, 人员表 D, (Select a.用户, a.角色 From zlUserRoles A Where a.角色 = [2]) E" & vbNewLine & _
            "Where a.人员id = b.人员id And b.部门id = c.Id And d.Id = a.人员id And a.用户名 = e.用户(+) And b.部门id = [1] And a.用户名 <> [3]"
