...
Sub SaveDate()
    ...
    With Me.mshGroupItem
        For i = 1 To .Rows - 1
            If Len(Trim(.TextMatrix(i, 0))) > 0 Then
                lngId = zldatabase.GetNextId("收费项目组成")
        ...
        With Me.mshGroupItem
            If .Col = 0 Then
                ...
                gstrSQL = _
                    "SELECT A.编码,A.名称,A.规格,A.计算单位," & _
                    " ltrim(rtrim(to_char(Sum(nvl(D.现价,0)),'9999999990.00'))) 价格,A.ID" & _
                    " FROM 收费项目目录 A,收费价目 D, 收费项目别名 B " & _
                    " WHERE A.ID=D.收费细目ID(+) And A.ID = B.收费细目id And a.ID>0 And A.类别 Not In ('5', '6', '7') " & _
                    " And (A.撤档时间=to_date('3000-01-01','yyyy-mm-dd') or A.撤档时间 is null)" & _
                    " And D.执行日期 <= SYSDATE AND (D.终止日期 > SYSDATE OR D.终止日期 IS NULL)" & _
                    " And (A.编码 Like [1] Or A.名称 Like [2] Or B.简码 Like [2]) " & _
                    " Group By A.编码,A.名称,A.规格,A.计算单位,A.ID"
                    
                Set rsTmp = zldatabase.OpenSQLRecord(gstrSQL, Me.Caption, strKey & "%", "%" & strKey & "%")
