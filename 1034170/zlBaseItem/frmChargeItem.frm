...
Dim mcol价目 As New Collection  '保存的是收费价目的ID，以收入项目的ID作Key。免得同一收入项目失去原有价目ID
...
Public Function IsValid收入项目和从属关系() As Boolean
    ...
    If medit方式 = EditNew Or medit方式 = EditCopy Or medit方式 = EditRaise Then
        If Me.msh价目.Rows > 2 Then
            If Me.msh价目.TextMatrix(2, mcstCol原价) <> "" Then
                blnIs存在多个收入项目 = True
            Else
                blnIs存在多个收入项目 = False
            End If
...
Public Function 编辑项目(ByVal str分类ID As String, Optional strID As String = "", _
    ...
    '参数:str分类ID   收费项目的分类ID   '为数字表示ID，否则为类别名
    '     strID           本收费项目的的ID
    '     bln末级项目     本收费项目是否末级
...
'    End If
    ...
    If edit方式 <> EditNew Then
        ...
        If rsTmp.RecordCount > 0 Then
            mstr分类ID = zlCommFun.Nvl(rsTmp!分类ID)
        Else
...
Private Sub Save基本()
    ...
    If medit方式 <> EditModify Then
        ...
        For i = 0 To 6
            ...
            End If
        Next
        mstrID = zlDatabase.GetNextId("收费项目目录")
        gstrSQL = "zl_收费细目_insert(" & int特殊项目 & "," & mstrID & ",'" & mstr类别 & "','" & UCase(txtEdit(text编码).Text) & "','" & txtEdit(text标识主码).Text & "','" & txtEdit(Text标识子码).Text & "','" & txtEdit(Text备选码).Text & "','" & txtEdit(Text名称).Text & _
    ...
    Else
        '修改
        gstrSQL = "zl_收费细目_update(" & int特殊项目 & "," & mstrID & ",'" & mstr类别 & "','" & UCase(txtEdit(text编码).Text) & "','" & txtEdit(text标识主码).Text & "','" & txtEdit(Text标识子码).Text & "','" & txtEdit(Text备选码).Text & "','" & txtEdit(Text名称).Text & _
...
Private Sub Save价目()
    ...
    str开始时间 = Format(IIF(Me.ChkNow.Value = 1, strTemp, dtpBegin.Value), mstrCurrentDateFormat)
    ...
    '2、有权限则审核，没有权限则不审核
    With msh价目
        lng调价ID = zlDatabase.GetNextId("收费价目")
            ...
            If mblnVerifyFlow = True Then
                For i = 1 To .Rows - 1
                    If .RowData(i) > 0 Then
                        ...
                        End If
                        gstrSQL = "Zl_收费调价记录_Insert(" & _
                            lng价目ID & "," & IIF(mcol价目("C" & .RowData(i)) = 0, "null", mcol价目("C" & .RowData(i))) & "," & _
                            mstrID & "," & _
                            .RowData(i) & "," & .TextMatrix(i, mcstCol原价) & "," & .TextMatrix(i, mcstCol现价) & "," & _
                            IIF(Val(.TextMatrix(1, mcstCol缺省价格)) = 0, "Null", Val(.TextMatrix(1, mcstCol缺省价格))) & "," & _
                            IIF(.TextMatrix(i, mcstCol附加手术收费率) = "", 0, .TextMatrix(i, mcstCol附加手术收费率)) & "," & _
                            IIF(.TextMatrix(i, mcstCol加班加价率) = "", 0, .TextMatrix(i, mcstCol加班加价率)) & ",'" & _
                            txtEdit(text调价说明).Text & "'," & _
                            lng调价ID & ",'" & _
                            gstrUserName & "'," & _
                            "to_date('" & str填制日期 & "','YYYY-MM-DD HH24:MI:SS')," & _
                            "to_date('" & str开始时间 & "','YYYY-MM-DD HH24:MI:SS'),1,'" & _
                            strNo & "'," & _
                            intRow + 1 & ")"
                        Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
                        ...
                        If mblnVerifyPris = True Then
                            ...
                            lng价目ID & "," & 1 & ",'" & gstrUserName & "', to_date('" & str填制日期 & "','YYYY-MM-DD HH24:MI:SS'))"
                            Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
                        End If
                        intRow = intRow + 1
                    End If
            ...
            Else
                ...
                If mblnVerifyPris = True Then
                    ...
                    If (medit方式 = EditRaise Or (medit方式 = EditModify And mblnShow收费价目)) _
                        ...
                        '填写以前价目的终止日期
                        gstrSQL = "zl_收费价目_stop(" & mstrID & ",to_date('" & str终止时间 & "','YYYY-MM-DD HH24:MI:SS'))"
                        Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
                    End If
                    
                    If chk变价.Value = 0 Or medit方式 <> EditRaise Or mblnNew Then
                        If chk变价.Value = 1 And medit方式 = EditModify And mblnShow收费价目 Then
                            gstrSQL = "zl_收费价目_update(" & mstrID & "," & .RowData(1) & " ," & .TextMatrix(1, mcstCol原价) & "," & .TextMatrix(1, mcstCol现价) & _
                                "," & IIF(.TextMatrix(1, mcstCol附加手术收费率) = "", 0, .TextMatrix(1, mcstCol附加手术收费率)) & "," & IIF(.TextMatrix(1, mcstCol加班加价率) = "", 0, .TextMatrix(1, mcstCol加班加价率)) & _
                                ",'" & txtEdit(text调价说明).Text & "'," & lng调价ID & ",'" & gstrUserName & "'," & IIF(Val(.TextMatrix(1, mcstCol缺省价格)) = 0, "Null", Val(.TextMatrix(1, mcstCol缺省价格))) & ")"
                            Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
                            Exit Sub
                        End If
                        ...
                        For i = 1 To .Rows - 1
                            If .RowData(i) > 0 Then
                                ...
                                gstrSQL = "zl_收费价目_insert(" & _
                                    lng价目ID & "," & IIF(mcol价目("C" & .RowData(i)) = 0, "null", mcol价目("C" & .RowData(i))) & "," & mstrID & "," & _
                                    .RowData(i) & "," & .TextMatrix(i, mcstCol原价) & "," & .TextMatrix(i, mcstCol现价) & "," & IIF(.TextMatrix(i, mcstCol附加手术收费率) = "", 0, .TextMatrix(i, mcstCol附加手术收费率)) & "," & IIF(.TextMatrix(i, mcstCol加班加价率) = "", 0, .TextMatrix(i, mcstCol加班加价率)) & _
                                    ",'" & txtEdit(text调价说明).Text & "'," & lng调价ID & ",'" & gstrUserName & "',to_date('" & str开始时间 & "','YYYY-MM-DD HH24:MI:SS'),1,'" & strNo & "'," & intRow + 1 & "," & IIF(Val(.TextMatrix(1, mcstCol缺省价格)) = 0, "Null", Val(.TextMatrix(1, mcstCol缺省价格))) & ")"
                                Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
                                intRow = intRow + 1
                            End If
                        Next
...
Private Sub Save执行()
    ...
    If medit方式 <> EditDept And opt科室(4).Value = False And Not (opt科室(0).Value And mstrServerObj <> "1") Then Exit Sub
...
'                    End If
    ...
    gstrSQL = "zl_收费细目_dept(" & mstrID & "," & lng科室 & "," & lng应用 & "," & _
        IIF(mstr分类ID = "", "Null", mstr分类ID) & ",'" & mstr类别 & "','" & str科室 & "','" & strOut & "','" & strIn & "')"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then Resume
...
Private Sub cmdFind_Click()
    ...
    For i = mlngFind To Lvw科室.ListItems.Count
        If zlCommFun.SpellCode(Mid(Lvw科室.ListItems(i).Text, InStr(Lvw科室.ListItems(i).Text, "-") + 1)) Like UCase(IIF(gstrLike <> "", "*", "") & strFind & "*") Or _
...
Private Sub cmb费用类型_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyReturn Then
        zlCommFun.PressKey vbKeyTab
    Else
        lngIdx = MatchIndex(cmb费用类型.hwnd, KeyAscii)
...
Public Function IsRaiseByDate(ByVal strID As String) As Boolean
    ...
    strSql = "Select Id" & _
            ...
            " And 执行日期<>trunc(执行日期,'dd') And 收费细目id=[1] "
...
Private Sub Lvw科室_KeyPress(KeyAscii As Integer)
    ...
    For i = 1 To Lvw科室.ListItems.Count
        If zlCommFun.SpellCode(Mid(Lvw科室.ListItems(i).Text, InStr(Lvw科室.ListItems(i).Text, "-") + 1)) Like UCase(Chr(KeyAscii)) & "*" Then
    ...
    If KeyCode = vbKeyReturn Then
        ...
        If mshAlias.Col = 0 Then
            If zlCommFun.StrIsValid(strTemp, mlng别名长度) = False Then
                ...
                End If
            Else
                mshAlias.TextMatrix(mshAlias.Row, 1) = zlGetSymbol(strTemp, 0, mint简码长度)
                mshAlias.TextMatrix(mshAlias.Row, 2) = zlGetSymbol(strTemp, 1, mint简码长度)
            ...
            End If
        Else
            Cancel = Not zlCommFun.StrIsValid(strTemp, mint简码长度)
            If Cancel = True Then
                If mshAlias.Active And mshAlias.Visible Then
                    mshAlias.TxtSetFocus
                End If
...
Private Sub msh价目_AfterAddRow(Row As Long)
    If chk变价.Value = 1 Then
        zlCommFun.PressKey vbKeyTab
    End If
End Sub

Private Sub msh价目_BeforeDeleteRow(Row As Long, Cancel As Boolean)
    If msh价目.RowData(Row) <> 0 Then
        mcol价目.Remove "C" & msh价目.RowData(Row)
    End If
End Sub

Private Sub msh价目_LostFocus()
    If chk变价.Value = 1 Then
        msh价目.Rows = 2
    End If
    msh价目.CmdVisible = False
End Sub
...
Private Sub msh价目_CommandClick()
    On Error GoTo ErrHandle
    Dim strSql As String
    Dim blnRe As Boolean
    Dim strTemp As String
    ...
    With msh价目
        ...
        If blnRe Then
            On Error Resume Next
            mcol价目.Add 0, "C" & strID
            If Err <> 0 Then
                MsgBox "该收入项目已设置了价目。", vbExclamation, gstrSysName
                Exit Sub
            End If
            If .RowData(lngRow) > 0 Then mcol价目.Remove "C" & .RowData(lngRow)
...
Private Sub msh价目_KeyDown(KeyCode As Integer, Shift As Integer, Cancel As Boolean)
    ...
    With msh价目
        ...
        Select Case .Col
        Case mcstCol收费项目
            If IsRecord("收入项目", .Text) = False Then
                Cancel = True
                .TxtSetFocus
                Exit Sub
            End If
...
Private Sub SetCodeNO()
    ...
    If medit方式 = EditNew Or medit方式 = EditCopy Then
        ...
        If mstr分类编码 = "" Then
            ...
            strTmp = zlDatabase.GetMax("收费项目目录", "编码", lngMaxLen, " where 分类id=" & mstr分类ID)
            ...
            If zlCommFun.Nvl(rsTmp!项目数, 0) > 0 Then
        ...
        Else
            ...
            If Nvl(rsTmp!最大编码, "") = "" Then
                strTmp = mstr分类编码 & "01"
            Else
                strTmp = zlCommFun.IncStr(rsTmp!最大编码)
            End If
...
Private Sub cmbClass_Click()
    ...
    If InStr(1, gstrPrivs, ";医保类型;") = 0 Then
    ...
    If TabExist("收费价目") = True Then
        If mstr类别 = "F" Then
            msh价目.TextMatrix(0, mcstCol附加手术收费率) = "附加手术收费率"
            If msh价目.ColWidth(mcstCol附加手术收费率) = 0 Then
               msh价目.ColWidth(mcstCol附加手术收费率) = 1500
            End If
        Else
            msh价目.ColWidth(mcstCol附加手术收费率) = 0
        End If
    End If
    ...
    '显示当前应用哪个类别
    Me.optApply(3).Caption = "应用于 " & strClass & " 类别下所有项目(&U)"
    '产地
    If strClass = "材料" Then
...
On Error GoTo ErrHandle
    ...
    Case Text名称
        ...
        If txtEdit(Text名称).Text <> strTmp Then
            ...
            Me.txtEdit(text简码).Text = zlGetSymbol(strTmp, 0, mint简码长度)
            Me.txtEdit(text五笔).Text = zlGetSymbol(strTmp, 1, mint简码长度)
        End If
        txtEdit(text简码).Text = zlGetSymbol(txtEdit(Text名称).Text, 0, mint简码长度)
        txtEdit(text五笔).Text = zlGetSymbol(txtEdit(Text名称).Text, 1, mint简码长度)
    ...
    Case text最高限价, text最低限价
        If IsNumeric(txtEdit(Index).Text) Then
            If text最高限价 = Index Then
                ...
                If chk变价.Value = 1 Then
                    msh价目.TextMatrix(1, mcstCol现价) = Format(mdbl最高限价, "0.000")
                End If
            Else
                mdbl最低限价 = Val(txtEdit(Index).Text)
                If chk变价.Value = 1 Then
                    msh价目.TextMatrix(1, mcstCol原价) = Format(mdbl最低限价, "0.000")
                End If
            End If
        End If
    End Select
        ...
        Select Case True
            ...
            Case mstrSel = 2
                If Not lvwSel.SelectedItem Is Nothing Then
                    ...
                    If rsTmp.RecordCount > 0 Then
                        ...
                        For i = 1 To rsTmp.RecordCount
                            If i = rsTmp.RecordCount Then
                                strSql = strSql & "[" & zlCommFun.Nvl(rsTmp!编码) & "]" & zlCommFun.Nvl(rsTmp!名称)
                            Else
                                strSql = strSql & "[" & zlCommFun.Nvl(rsTmp!编码) & "]" & zlCommFun.Nvl(rsTmp!名称) & vbCrLf
                            End If
                            rsTmp.MoveNext
                        Next
                    ...
                    If rsTmp.RecordCount = 1 Then
                        txtEdit(text标识主码).Text = zlCommFun.Nvl(rsTmp!项目编码)
                        If medit方式 = EditNew Then
                            '名称
                            txtEdit(Text名称).Text = zlCommFun.Nvl(rsTmp!项目名称)
                            txtEdit(text简码).Text = zlGetSymbol(txtEdit(Text名称).Text, 0, mint简码长度)
                            txtEdit(text五笔).Text = zlGetSymbol(txtEdit(Text名称).Text, 1, mint简码长度)
                            '单位
                            If zlCommFun.Nvl(rsTmp!计价单位) <> "" Then
                                cmb计算单位.Text = zlCommFun.Nvl(rsTmp!计价单位)
                            End If
                            '别名
                            If mshAlias.Rows > 2 And Trim(mshAlias.TextMatrix(mshAlias.Rows - 1, 0)) <> "" Then
                                mshAlias.Rows = mshAlias.Rows + 1
                            End If
                            mshAlias.TextMatrix(mshAlias.Rows - 1, 0) = zlCommFun.Nvl(rsTmp!项目别名)
                            mshAlias.TextMatrix(mshAlias.Rows - 1, 1) = zlGetSymbol(zlCommFun.Nvl(rsTmp!项目别名), 0, mint简码长度)
                            mshAlias.TextMatrix(mshAlias.Rows - 1, 2) = zlGetSymbol(zlCommFun.Nvl(rsTmp!项目别名), 1, mint简码长度)
                            '最高与最低限价
                            txtEdit(text最高限价).Text = Format(zlCommFun.Nvl(rsTmp!最高限价, 0), "0.00")
                            txtEdit(text最低限价).Text = Format(zlCommFun.Nvl(rsTmp!最低限价, 0), "0.00")
                            If chk变价.Value = 1 Then
                                msh价目.Rows = 2
                                msh价目.TextMatrix(1, mcstCol现价) = txtEdit(text最高限价).Text
                                msh价目.TextMatrix(1, mcstCol原价) = txtEdit(text最低限价).Text
                            End If
                            mdbl最高限价 = Format(zlCommFun.Nvl(rsTmp!最高限价, 0), "0.00")
                            mdbl最低限价 = Format(zlCommFun.Nvl(rsTmp!最低限价, 0), "0.00")
                            '项目价格
                            mdbl医价价格 = zlCommFun.Nvl(rsTmp!项目价格, 0)
...
Private Sub txtEdit_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
    ...
    Case Text分类   '分类
        If KeyCode = 13 Then
            ...
            If rsTmp.RecordCount = 1 Then
                txtEdit(Text分类).Text = zlCommFun.Nvl(rsTmp!名称)
                txtEdit(text编码).Text = zlCommFun.Nvl(rsTmp!编码)
                mstr分类编码 = zlCommFun.Nvl(rsTmp!编码)
                mstr分类ID = rsTmp!ID
                Call SetCodeNO
                txtEdit(text编码).MaxLength = mlng编码长度
                zlCommFun.PressKey vbKeyTab
            ElseIf rsTmp.RecordCount > 0 Then
                ...
                Call InitLvwSel
                For i = 1 To rsTmp.RecordCount
                    Set ObjItem = lvwSel.ListItems.Add(, , zlCommFun.Nvl(rsTmp!编码))
    ...
    Case text标识主码    '标识主码
        ...
        If KeyCode = 13 And gstr医价接口编号 <> "" And gbln允许医价收费项目 = True Then
            ...
            End If
            '取那个价格项目
            strSql = "select 项目编码, 项目名称, 拼音码, 项目别名, 计价单位, 项目内涵, 除外内容, 项目说明, 项目价格, 重复标志, 医院等级, 注销标志, 财务编码, 最高限价, 最低限价, 调价日期 from 标准医价规范 where nvl(注销标志,0) =0 and  upper(项目编码) like [1] or upper(项目名称) LIKE [2] or upper(拼音码) LIKE [2] "
            ...
            If rsTmp.RecordCount = 1 Then
                txtEdit(Index).Text = zlCommFun.Nvl(rsTmp!项目编码)
                If medit方式 = EditNew Then
                    '名称
                    txtEdit(Text名称).Text = zlCommFun.Nvl(rsTmp!项目名称)
                    txtEdit(text简码).Text = zlGetSymbol(txtEdit(Text名称).Text, 0, mint简码长度)
            ...
            ElseIf rsTmp.RecordCount > 1 Then
                ...
                Call InitLvwSel
                For i = 1 To rsTmp.RecordCount
                    Set ObjItem = lvwSel.ListItems.Add(, , zlCommFun.Nvl(rsTmp!项目编码))
                    ObjItem.SubItems(1) = zlCommFun.Nvl(rsTmp!项目名称)
                    ObjItem.Tag = zlCommFun.Nvl(rsTmp!项目编码)
                    rsTmp.MoveNext
                Next
                lvwSel.ListItems(1).Selected = True
                lvwSel.SelectedItem.EnsureVisible
...
'        ShowTab "基本信息"
        If Index = text最高限价 Or Index = text最低限价 Then
            ...
            If Len(Trim(mstrID)) > 0 And (Val(txtEdit(text最高限价).Text) <> 0 Or Val(txtEdit(text最低限价).Text) <> 0) Then
                strSql = "Select 现价 From 收费价目 Where" & _
                    " Decode(终止日期,to_date('3000-01-01','YYYY-MM-DD'),Null,终止日期) is Null And 收费细目ID =" & mstrID
                Call zlDatabase.OpenRecordset(rsTmp, strSql, Me.Caption)
                If Not rsTmp.EOF Then
                    If Val(txtEdit(text最高限价).Text) <> 0 And rsTmp(0) > Val(txtEdit(text最高限价).Text) Then Cancel = True
                    If Val(txtEdit(text最低限价).Text) <> 0 And rsTmp(0) < Val(txtEdit(text最低限价).Text) Then Cancel = True
                    If Cancel Then
                        MsgBox "现行价格（" & Format(rsTmp(0), "#0.000") & "）不在当前设置的限价内，请调价或重新设置限价！", vbInformation, gstrSysName
                        mblnEditCancel = False
                        Exit Sub
                    End If
                End If
            ElseIf Val(txtEdit(text最高限价).Text) <> 0 Or Val(txtEdit(text最低限价).Text) <> 0 Then
                If Len(Trim(Me.msh价目.TextMatrix(1, mcstCol现价))) > 0 Then
                    If Val(txtEdit(text最高限价).Text) <> 0 And Val(Me.msh价目.TextMatrix(1, mcstCol现价)) > Val(txtEdit(text最高限价).Text) Then Cancel = True
                    If Val(txtEdit(text最低限价).Text) <> 0 And Val(Me.msh价目.TextMatrix(1, mcstCol现价)) < Val(txtEdit(text最低限价).Text) Then Cancel = True
                    If Cancel Then
                        MsgBox "现行价格（" & Format(Me.msh价目.TextMatrix(1, mcstCol现价), "#0.000") & "）不在当前设置的限价内，请调价或重新设置限价！", vbInformation, gstrSysName
                        mblnEditCancel = False
                        Exit Sub
                    End If
                End If
            End If
        End If
    Case Index = text调价说明 Or Index = Text说明
'        ShowTab "收费价目"
    Case Index = text标识主码
        ShowTab "基本信息"
...
'        index <> Text规格 And index <> Text说明 And index <> Text产地 Then KeyAscii = 0: Exit Sub
    ...
    ElseIf KeyAscii = vbKeyReturn Or KeyAscii = vbKeyTab Then
        KeyAscii = 0
        If (mstr类别 = "M" And Index = text产地) Or (mstr类别 <> "M" And Index = Text说明) Or (Index = text调价说明) Then
            If TabMain.Tabs.Count > 1 Then
                If Index <> text调价说明 Then
                    ShowTab "收费价目"
                Else
                    ShowTab "执行科室"
                End If
...
Private Sub Form_Activate()
    ...
    Case "基本信息"
        ...
        End If
    Case "收费价目"
        If msh价目.Visible And msh价目.Active Then
            msh价目.SetFocus
        End If
    Case "从属项目"
...
Private Sub tabMain_Click()
    ...
    Case "收费价目"
        ...
        fra(2).Visible = True
        fra(2).ZOrder
        If msh价目.Active And msh价目.Visible Then
            msh价目.SetFocus
        End If
...
Private Sub ClearContext(Optional ByVal bln完全 As Boolean = True)
    ...
    For lngCol = 1 To mcol价目.Count
        mcol价目.Remove 1
    Next
    For lngCol = 1 To msh价目.Rows - 1
        If msh价目.RowData(lngCol) > 0 Then
            mcol价目.Add 0, "C" & msh价目.RowData(lngCol)
            msh价目.TextMatrix(lngCol, 1) = "0.000"
        End If
    Next
    ...
    msh价目.Rows = 2
    msh价目.RowData(1) = 0
    For lngCol = 0 To msh价目.Cols - 1
        msh价目.TextMatrix(1, lngCol) = ""
    Next
    txtEdit(text调价说明).Text = ""
...
Private Function init基本() As Boolean
    ...
    If rsTmp.RecordCount > 0 Then
        rsTmp.MoveFirst
        For i = 1 To rsTmp.RecordCount
            cmbClass.AddItem zlCommFun.Nvl(rsTmp!编码) & "-" & zlCommFun.Nvl(rsTmp!名称)
            If i = 1 Then
                cmbClass.ListIndex = 0
            ElseIf zlCommFun.Nvl(rsTmp!编码) = "E" Then
    ...
    If medit方式 <> EditNew And IsNumeric(mstrID) Then
        ...
        If Me.cmbClass.ListCount > 0 Then
            For i = 0 To Me.cmbClass.ListCount
                If Me.cmbClass.List(i) = zlCommFun.Nvl(rsTmp!类别) & "-" & zlCommFun.Nvl(rsTmp!类别名称) Then
        ...
        '求出包括子节点在内的最长编码
        txtEdit(text标识主码).Text = zlCommFun.Nvl(rsTmp("标识主码"))
        txtEdit(text标识主码).Tag = zlCommFun.Nvl(rsTmp("标识主码"))
        txtEdit(Text标识子码).Text = zlCommFun.Nvl(rsTmp("标识子码"))
        txtEdit(Text备选码).Text = zlCommFun.Nvl(rsTmp("备选码"))
        
        txtEdit(Text名称).Text = rsTmp("名称")
        txtEdit(Text规格).Text = zlCommFun.Nvl(rsTmp("规格"))
        txtEdit(Text说明).Text = zlCommFun.Nvl(rsTmp("说明"))
        txtEdit(text产地).Text = zlCommFun.Nvl(rsTmp("产地"))
        ...
        SetStationNo IIF(IsNull(rsTmp("站点")), "", rsTmp("站点"))
    ...
    ElseIf Trim(mstrID) = "" Then    '新增
        ...
        If rsTmp.RecordCount > 0 Then
            If Me.cmbClass.ListCount > 0 Then
                For i = 0 To Me.cmbClass.ListCount
                    If Me.cmbClass.List(i) = zlCommFun.Nvl(rsTmp!编码) & "-" & zlCommFun.Nvl(rsTmp!名称) Then
                        Me.cmbClass.ListIndex = i
                        Exit For
                    End If
                Next
        ...
        Call SetCodeNO
        mstr编码 = txtEdit(text编码).Text
        txtEdit(Text建档时间).Text = Format(zlDatabase.Currentdate, "yyyy-MM-dd")
        txtEdit(text简码).Text = ""
        txtEdit(text五笔).Text = ""
...
End Sub
Private Function init价目() As Boolean
    On Error GoTo ErrHandle
    '功能:初始化收费价目表和从属项目表
    Dim rsTmp As New ADODB.Recordset
    Dim str名称 As String
    ...
    End If
    '装入数据
    gstrSQL = "select 名称,是否变价,加班加价,最高限价,最低限价  from 收费项目目录 where ID=[1] "
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(mstrID))
        
    If Not mblnShow收费价目 Then
        chk变价.Tag = IIF(rsTmp("是否变价") = 1, 1, 0)
        chk变价.Value = IIF(rsTmp("是否变价") = 1, 1, 0)
        msh价目.ColWidth(mcstCol缺省价格) = IIF(rsTmp("是否变价") = 1, 1000, 0)
    End If
    chk加班加价.Value = IIF(rsTmp("加班加价") = 1, 1, 0)
    If Not mblnShow收费价目 Then
        mdbl最高限价 = zlCommFun.Nvl(rsTmp!最高限价, 0)
        mdbl最低限价 = zlCommFun.Nvl(rsTmp!最低限价, 0)
    Else
    ...
    Call chk加班加价_Click
    '显示收费价目
    gstrSQL = "select a.ID,a.原价ID,a.收费细目ID,Nvl(a.原价,0) As 原价,Nvl(a.现价,0) As 现价,Nvl(a.缺省价格,0) As 缺省价格,a.收入项目ID,b.名称,a.加班加价率,a.附术收费率,a.变动原因,a.调价说明,a.执行日期,a.终止日期 " & _
        " from 收费价目 a,收入项目 b Where a.收入项目ID=b.ID and decode(a.终止日期,to_date('3000-01-01','YYYY-MM-DD'),null,a.终止日期) is null And a.收费细目ID = [1] "
    Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(mstrID))
        
    msh价目.Rows = IIF(rsTmp.RecordCount = 0, 2, rsTmp.RecordCount + 1)
    msh价目.Tag = msh价目.Rows
    
    mblnNew = rsTmp.RecordCount = 0 '新价格
    If rsTmp.RecordCount = 0 Then
        ...
        Else
            ...
            If chk变价.Value = 0 Then '1 不是变价项目
                If mblnIsSpecialItem Then       '1.1 是特殊项目
                    If DateDiff("s", rsTmp("执行日期"), zlDatabase.Currentdate) > 0 Then        '1.1.1 上次开始时间小于当前时间
                        If DateDiff("s", rsTmp("执行日期"), Format(zlDatabase.Currentdate, "yyyy-mm-dd 00:00:00")) > 0 Then     '1.1.1.1 上次开始时间大于当天零点时间
                            ChkNow.Visible = True
                            dtpBegin.MinDate = zlDatabase.Currentdate
                    ...
                    Else        '1.1.2 上次开始时间大于当前时间
                        dtpBegin.Value = DateAdd("d", 1, Format(rsTmp("执行日期"), "yyyy-mm-dd h:m:s"))
                        dtpBegin.MinDate = DateAdd("d", 1, Format(rsTmp("执行日期"), "yyyy-mm-dd h:m:s"))
                        ChkNow.Visible = False
                    End If
                Else        '1.2 不是特殊项目
                    If DateDiff("s", rsTmp("执行日期"), zlDatabase.Currentdate) > 0 Then        '1.2.1 上次开始时间小于当前时间
                    ...
                    Else    '1.2.2 上次开始时间大于当前时间
                        dtpBegin.Value = Format(DateAdd("d", 1, Format(rsTmp("执行日期"), "yyyy-mm-dd h:m:s")), "yyyy-mm-dd 00:00:00")
                        dtpBegin.MinDate = DateAdd("s", 1, Format(rsTmp("执行日期"), "yyyy-mm-dd h:m:s"))
                    End If
                    ChkNow.Visible = True
                End If
                txtEdit(text调价说明).Text = ""
            Else    '2 是变价项目
                dtpBegin.Value = Format(rsTmp("执行日期"), "yyyy-mm-dd h:m:s")
                dtpBegin.Enabled = False
            End If
        End If
        Do Until rsTmp.EOF
            msh价目.TextMatrix(lngCol, mcstCol收费项目) = rsTmp("名称")
            If chk变价.Value = 1 Then '变价
                msh价目.TextMatrix(lngCol, mcstCol原价) = Format(rsTmp("原价"), "###########0.000;-##########0.000;0.000;0.000")
                msh价目.TextMatrix(lngCol, mcstCol现价) = Format(rsTmp("现价"), "###########0.000;-##########0.000;0.000;0.000")
                msh价目.TextMatrix(lngCol, mcstCol缺省价格) = Format(rsTmp("缺省价格"), "###########0.000;-##########0.000;0.000;0.000")
            Else
                msh价目.TextMatrix(lngCol, mcstCol原价) = Format(rsTmp("现价"), "###########0.000;-##########0.000;0.000;0.000")
                If medit方式 = EditCopy Then msh价目.TextMatrix(lngCol, mcstCol现价) = Format(rsTmp("现价"), "###########0.000;-##########0.000;0.000;0.000")
            End If
            msh价目.TextMatrix(lngCol, mcstCol附加手术收费率) = IIF(IsNull(rsTmp("附术收费率")), 0, rsTmp("附术收费率"))
            msh价目.TextMatrix(lngCol, mcstCol加班加价率) = IIF(IsNull(rsTmp("加班加价率")), 0, rsTmp("加班加价率"))
            msh价目.RowData(lngCol) = rsTmp("收入项目ID")
            lng原有ID = rsTmp("ID")
            mcol价目.Add lng原有ID, "C" & rsTmp("收入项目ID")
            lngCol = lngCol + 1
            rsTmp.MoveNext
        Loop
    End If
...
Private Sub init执行()
    ...
    '显示当前应用哪个类别
    Me.optApply(1).Caption = "应用于" & zlCommFun.Nvl(rsTmp("名称")) & "分类同级的所有项目(&G)"
    Me.optApply(2).Caption = "应用于" & zlCommFun.Nvl(rsTmp("名称")) & "分类下所有项目(&L)"
    Me.optApply(3).Caption = "应用于" & zlCommFun.Nvl(rsTmp("类别")) & "类别下所有项目(&U)"
...
Private Function IsValid价目() As Boolean
    ...
    With msh价目
        ...
        End If
    End With
    If zlCommFun.StrIsValid(txtEdit(text调价说明).Text, txtEdit(text调价说明).MaxLength) = False Then
        ShowTab "收费价目"
        If txtEdit(text调价说明).Enabled And txtEdit(text调价说明).Visible Then
            txtEdit(text调价说明).SetFocus
            zlControl.TxtSelAll txtEdit(text调价说明)
        End If
        Exit Function
    End If
    IsValid价目 = True
...
Private Function IsRecord(ByVal strTable As String, ByVal strWhere As String) As Boolean
    ...
    Else
        If strTable = "收入项目" Then
            strReturn = zlCommFun.Nvl(rsTmp!编码) & "," & zlCommFun.Nvl(rsTmp!名称) & "," & zlCommFun.Nvl(rsTmp!收据费目) & "," & zlCommFun.Nvl(rsTmp!病案费目) & "," & zlCommFun.Nvl(rsTmp!ID, 0)
        Else
            strReturn = zlCommFun.Nvl(rsTmp!编码) & "," & zlCommFun.Nvl(rsTmp!名称) & "," & zlCommFun.Nvl(rsTmp!规格) & "," & zlCommFun.Nvl(rsTmp!计算单位) & "," & zlCommFun.Nvl(rsTmp!价格) & "," & zlCommFun.Nvl(rsTmp!ID, 0)
        End If
    End If
    If strTable = "收入项目" Then
        On Error Resume Next
        strTemp = "C" & Split(strReturn, ",")(UBound(Split(strReturn, ",")))
        mcol价目.Add 0, strTemp
        If Err <> 0 Then
            MsgBox "收入项目“" & Split(strReturn, ",")(1) & "”已设置了价目。", vbExclamation, gstrSysName
            Exit Function
        End If
        If msh价目.RowData(msh价目.Row) > 0 Then mcol价目.Remove "C" & msh价目.RowData(msh价目.Row)
        msh价目.RowData(msh价目.Row) = CLng(Split(strReturn, ",")(UBound(Split(strReturn, ","))))
        msh价目.TextMatrix(msh价目.Row, mcstCol收费项目) = Split(strReturn, ",")(1)
        If msh价目.TextMatrix(msh价目.Row, mcstCol原价) = "" Then msh价目.TextMatrix(msh价目.Row, mcstCol原价) = "0.000"
    Else
        For i = 0 To msh从属.Rows - 1
        ...
        gstrSQL = "SELECT a.id,a.是否变价,sum(b.原价) 原价,sum(b.现价) 现价," & vbCrLf & _
                ...
                " And b.执行日期 <= SYSDATE AND (b.终止日期 > SYSDATE OR b.终止日期 IS NULL)" & _
                "GROUP BY a.id,a.是否变价"
        Set rsTmp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(msh从属.RowData(msh从属.Row)))
...
Private Sub GetDefineSize()
    ...
    txtEdit(text简码).MaxLength = mint简码长度
    txtEdit(text五笔).MaxLength = mint简码长度
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then
        Resume
