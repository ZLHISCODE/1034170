...
Begin VB.Form frmChargeManage 
   ...
   Begin VB.PictureBox picPage 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsWholeSet 
         ...
         FormatString    =   $"frmChargeManage.frx":0442
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin MSComctlLib.ImageList iltdept 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmChargeManage.frx":0670
            Key             =   "Dept"
         EndProperty
      EndProperty
   End
   ...
   Begin VB.PictureBox picPage 
      ...
      Begin VB.CheckBox chk价格 
         ...
         Width           =   1425
      End
      Begin MSHierarchicalFlexGridLib.MSHFlexGrid msh价目 
         Height          =   1830
         Left            =   45
         TabIndex        =   30
         Top             =   570
      ...
      Begin VB.Image img价目 
         ...
         Height          =   540
         Left            =   0
         Picture         =   "frmChargeManage.frx":098C
         Top             =   0
         Width           =   540
      End
...
Private Const mstrLvw As String = "名称,1500,0,1;编码,1000,0,2;标识主码,1400,0,0;标识子码,900,0,0;备选码,1400,0,0;规格,550,0,0;计算单位,900,0,0;所属类别,1000,0,2;费用类型,900,0,0;服务对象,900,0,0;说明,1440,0,0;屏蔽费别,900,0,0;是否变价,900,0,0;加班加价,900,0,0;补充摘要,900,0,0;项目特性,1100,0,2;最高限价,1000,1,0;最低限价,1000,1,0;建档时间,1100,0,0;撤档时间,1100,0,0;所属分类,1300,0,2;病案费目,1300,0,2"
...
On Error GoTo ErrHandle
    ...
    msh价目.ColWidth(0) = 1200
    msh价目.ColWidth(1) = 2200
    msh价目.ColWidth(2) = 2200
    msh价目.ColWidth(3) = 1000
    msh价目.ColWidth(4) = 1000
    msh价目.ColWidth(5) = 1000
    msh价目.ColWidth(6) = 1000
    msh价目.ColWidth(7) = 1000
    msh价目.ColWidth(8) = 3000
    msh价目.ColWidth(9) = 1000
    msh价目.ColWidth(10) = 800
    ...
    msh价目.TextMatrix(0, 0) = "单据号"
    msh价目.TextMatrix(0, 1) = "执行日期"
    msh价目.TextMatrix(0, 2) = "终止日期"
    msh价目.TextMatrix(0, 3) = "收入项目"
    msh价目.TextMatrix(0, 4) = "原价"
    msh价目.TextMatrix(0, 5) = "现价"
    msh价目.TextMatrix(0, 6) = "附加手术收费率"
    msh价目.TextMatrix(0, 7) = "加班加价率"
    msh价目.TextMatrix(0, 8) = "调价说明"
    msh价目.TextMatrix(0, 9) = "缺省价格"
    msh价目.TextMatrix(0, 10) = "调价人"
    ...
    msh价目.Row = 1
    msh价目.MergeCol(0) = True
    msh价目.MergeCol(1) = False
    mshAlias.Col = 0
    mshAlias.Row = 1
    mshAlias.ColSel = 1
    mshAlias.RowSel = 0
...
Private Sub mnuPriceRaiseVerify_Click()
    frmChargePriceVerify.ShowMe Me
End Sub
...
On Error GoTo ErrHandle
    ...
    If InStr(Item.Icon, "No") > 0 Then
        mnuEditStart.Enabled = True
    Else
        mnuEditStart.Enabled = False
    End If
    
    mnuEditStop.Enabled = Not mnuEditStart.Enabled
    Toolbar1.Buttons("Start").Enabled = mnuEditStart.Enabled
    Toolbar1.Buttons("Stop").Enabled = mnuEditStop.Enabled
    
    mnuEditCopy.Enabled = mnuEditStop.Enabled
    mnuEditModify.Enabled = mnuEditStop.Enabled
    mnuEditDept.Enabled = mnuEditStop.Enabled
    mnuEditDelete.Enabled = mnuEditStop.Enabled
    mnuEditSlave.Enabled = mnuEditStop.Enabled
    mnuEditItemGroup.Enabled = mnuEditStop.Enabled
    mnuPriceChargeSet.Enabled = mnuEditStop.Enabled
    mnuPriceRaise.Enabled = mnuEditStop.Enabled
    If gstr医价接口编号 <> "" And gbln允许医价收费项目 Then
        mnuPriceRaiseMass.Enabled = False
    Else
        mnuPriceRaiseMass.Enabled = mnuEditStop.Enabled
    End If
    mnuPriceHistory.Enabled = mnuEditStop.Enabled
    
    Toolbar1.Buttons("Child").Enabled = mnuEditChild.Enabled
    Toolbar1.Buttons("Raise").Enabled = mnuEditStop.Enabled
    Toolbar1.Buttons("Modify").Enabled = mnuEditStop.Enabled
    Toolbar1.Buttons("Delete").Enabled = mnuEditStop.Enabled
    '当类别不为挂号、护理、床位的
    Dim i As Long
...
Private Sub mnuPirveHistory_Click()
On Error GoTo ErrHandle
    '因为加班加价率有时有，有时无，故每一行都加上。
    Dim strSql As String
    If Mid(lvwMain_S.SelectedItem.Key, 2, 1) = "F" Then
        strSql = "select a.ID,a.执行日期,a.终止日期,a.调价说明,b.名称 as 收入项目 ,a.原价 ,a.现价,a.加班加价率,a.附术收费率 " & _
        " from 收费价目 a,收入项目 b where a.收入项目ID=b.id and a.收费细目ID=" & Mid(lvwMain_S.SelectedItem.Key, 3) & " order by a.执行日期 desc "
    Else
        strSql = "select a.ID,a.执行日期,a.终止日期,a.调价说明,b.名称 as 收入项目 ,a.原价 ,a.现价,a.加班加价率" & _
        " from 收费价目 a,收入项目 b where a.收入项目ID=b.id and a.收费细目ID=" & Mid(lvwMain_S.SelectedItem.Key, 3) & " order by a.执行日期 desc "
    End If
    Exit Sub
ErrHandle:
...
'删除未执行价格
    ...
    On Error GoTo ErrHandle
    MousePointer = 11
    gstrSQL = "zl_收费价目_Delete(" & lvwMain_S.SelectedItem.Tag & ")"
    Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
...
On Error GoTo ErrHandle
    ...
    With frmChargeBatchPrice
        ...
        '求最大执行日期
        gstrSQL = "select max(B.执行日期) as 最大日期 from " & _
        ...
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, .lbl类别.Tag, .lbl分类.Tag)
        ...
        '求最小金额（但不包含变价项目）
        gstrSQL = "select min(B.现价) as 最小金额 from " & _
                "(select id from 收费项目目录 where (撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or 撤档时间 Is Null) And 是否变价=0 and " & IIF(.lbl分类.Tag = "", "类别=[1] and 分类ID is null", "分类ID=[2] ") & ") A" & _
...
On Error GoTo ErrHandle
    ...
    ReportOpen gcnOracle, glngSys, "ZL1_INSIDE_1009", Me, strCaption, gstrUserName, "分类=" & lng分类id
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub msh费别_DblClick()
    ...
    If mnuPriceChargeSet.Enabled = True Then
        If lvwMain_S.SelectedItem Is Nothing Then Exit Sub
        frmChargeSortItemEdit.ShowMe Me, 2, strCharge, Val(Me.lvwMain_S.SelectedItem.Tag), Me.lvwMain_S.SelectedItem.Text
        Call frmChargeManage.FillTree
    End If
    Exit Sub
ErrHandle:
...
On Error GoTo ErrHandle
    ...
    End If
    msh价目.Rows = 2
    msh价目.TextMatrix(1, 0) = ""
    msh价目.TextMatrix(1, 1) = ""
    msh价目.TextMatrix(1, 2) = ""
    msh价目.TextMatrix(1, 3) = ""
    msh价目.TextMatrix(1, 4) = ""
    msh价目.TextMatrix(1, 5) = ""
    msh价目.TextMatrix(1, 6) = ""
    msh价目.TextMatrix(1, 7) = ""
    msh价目.TextMatrix(1, 8) = ""
    msh价目.TextMatrix(1, 10) = ""
    chk价格.Value = 0
    opt科室(0).Value = 1
    opt科室(1).Value = 0
    opt科室(2).Value = 0
...
'新增项目
    ...
    If lvwMain_S.ListItems.Count > 0 And Not lvwMain_S.SelectedItem Is Nothing Then
        If IsNumeric(lvwMain_S.SelectedItem.Tag) Then
            If CLng(lvwMain_S.SelectedItem.Tag) > 0 Then
                Call frmChargeItem.编辑项目("C" & IIF(mstrClass = "", " ", mstrClass) & tvwMainItem.SelectedItem.Tag, , , , mbln启动医价系统)
                Exit Sub
            End If
        End If
    ...
    Else
        '此时的类别为空
        Call frmChargeItem.编辑项目("C " & tvwMainItem.SelectedItem.Tag, , , , mbln启动医价系统)
    End If
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then
...
On Error GoTo ErrHandle
    If lvwMain_S.SelectedItem Is Nothing Then Exit Sub
    Call frmChargeItem.编辑项目("C" & IIF(mstrClass = "", " ", mstrClass) & IIF(tvwMainItem.SelectedItem.Key = "Root", "", tvwMainItem.SelectedItem.Tag), lvwMain_S.SelectedItem.Tag, , 5, mbln启动医价系统)  'editCopy
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then
        Resume
...
On Error GoTo ErrHandle
    ...
    With frmChargeSort
        ...
        If rsTmp.RecordCount > 0 Then
            .txtSymbol = zlCommFun.Nvl(rsTmp!简码)
        Else
            .txtSymbol = ""
        End If
...
On Error GoTo ErrHandle
    If mnuEdit.Visible = False Then Exit Sub
    If lvwMain_S.SelectedItem Is Nothing Then Exit Sub
    Call frmChargeItem.编辑项目("C" & IIF(mstrClass = "", " ", mstrClass) & tvwMainItem.SelectedItem.Tag, lvwMain_S.SelectedItem.Tag, , 1, mbln启动医价系统)  'EditMode.editModify
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then
        Resume
...
On Error GoTo ErrHandle
    If ActiveControl Is tvwMainItem And edit方式 < 2 Then  'editRaise
        With tvwMainItem.SelectedItem
            Call frmChargeItem.编辑项目("C" & IIF(mstrClass = "", " ", mstrClass) & .Tag, .Tag, 0, edit方式, mbln启动医价系统)
        End With
    Else
        If lvwMain_S.SelectedItem Is Nothing Then Exit Sub
        If checkNotPrice(lvwMain_S.SelectedItem.Tag) = False Then
            ...
            Exit Sub
        End If
        Call frmChargeItem.编辑项目("C" & IIF(mstrClass = "", " ", mstrClass) & IIF(tvwMainItem.SelectedItem.Key = "Root", "", tvwMainItem.SelectedItem.Tag), lvwMain_S.SelectedItem.Tag, , edit方式, mbln启动医价系统)
    End If
    Exit Sub
ErrHandle:
    If ErrCenter() = 1 Then
...
Private Function checkNotPrice(ByVal lng收费细目id As Long) As Boolean
    ...
    On Error GoTo ErrHandle
    If lng收费细目id <> 0 Then
        gstrSQL = "Select 1 From 收费调价记录 Where 审核标志 = 0 And 收费细目id=[1] and Rownum <= 1"
    Else
        gstrSQL = "Select 1 From 收费调价记录 Where 审核标志 = 0 and rownum<=1"
    End If
    Set rsData = zlDatabase.OpenSQLRecord(gstrSQL, "未生效单据查询", lng收费细目id)
...
Private Function FillWholeItem(ByVal lng分类id As Long) As Boolean
    ...
    LockWindowUpdate lvwWholeSetItem_S.hwnd
    mblnNotClick = True
    With lvwWholeSetItem_S
        .ListItems.Clear
        Do While Not rsTemp.EOF
...
Private Function FillWholeSetItemChildData(ByVal lng成套ID As Long) As Boolean
    ...
    "             部门表 E,人员表 G," & _
    "             (Select 收费细目id, Sum(现价) as 现价  From 收费价目 J Where Sysdate Between J.执行日期 And Nvl(J.终止日期, To_Date('3000-1-1', 'YYYY-MM-DD')) Group By 收费细目id ) J1 " & _
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, lng成套ID)
    With vsWholeSet
        .redraw = flexRDNone
        .Clear 1
...
'功能:装入收费类别和收费细目的所有分类到tvwMainItem
    ...
    Me.stbThis.Refresh
    On Error GoTo ErrHandle
    LockWindowUpdate tvwMainItem.hwnd
...
'参数:str分类 分类的标识
    ...
    If mnuViewShowAll.Checked = True Then
            gstrSQL = _
                ...
                "        Nvl(B.名称,'') As 所属分类,a.病案费目" & vbCrLf & _
                " From " & vbCrLf & _
                "   (Select Id,名称 From 收费分类目录" & vbCrLf & _
                "   Start With 上级id  = " & str分类 & vbCrLf & _
                "    Connect By Prior id=上级id) B," & vbCrLf & _
                "    收费项目目录 A,收费项目类别 C " & vbCrLf & _
                "Where  A.类别=C.编码 and A.分类id=B.Id And A.类别<>'5'  And  A.类别<>'6'  And  A.类别<>'7'" & strTemp & vbCrLf & _
                "Union" & vbCrLf & _
                "Select A.ID,A.分类ID,A.类别,C.名称 所属类别,C.固定 类别固定,A.编码,A.标识主码,A.标识子码,A.最高限价,A.最低限价,A.备选码,A.名称,A.规格,A.计算单位,A.费用类型, " & vbCrLf & _
                "       Decode(A.服务对象,1,'门诊',2,'住院',3,'门诊与住院','无') as 服务对象," & vbCrLf & _
                "       decode(A.补充摘要,1,'√','') as 补充摘要, A.说明,decode(A.屏蔽费别,1,'√','') as 屏蔽费别," & vbCrLf & _
                ...
                "       decode(A.类别,'1',decode(A.项目特性,1,'急诊项目','挂号项目'),'H',decode(A.项目特性,1,'护理等级',2,'基本护理','')) As 项目特性," & vbCrLf & _
                "        Nvl(B.名称,'') As 所属分类,a.病案费目" & vbCrLf & _
                " From 收费项目目录 A,收费分类目录 B,收费项目类别 C " & vbCrLf & _
                "Where A.类别=C.编码 and A.分类id  = [1] And A.分类id=B.ID And A.类别<>'5'  And  A.类别<>'6'  And  A.类别<>'7'" & strTemp
    Else
            gstrSQL = _
                "Select A.ID,A.分类ID,A.类别,C.名称 所属类别,C.固定 类别固定,A.编码,A.标识主码,A.标识子码,A.最高限价,A.最低限价,A.备选码,A.名称,A.规格,A.计算单位,A.费用类型, " & vbCrLf & _
                "       decode(A.服务对象,1,'门诊',2,'住院',3,'门诊与住院','无') as 服务对象," & vbCrLf & _
                ...
                "        Nvl(B.名称,'') As 所属分类,a.病案费目" & vbCrLf & _
                "From 收费项目目录 A  ,收费分类目录 B,收费项目类别 C " & vbCrLf & _
                "Where A.类别=C.编码 and A.分类id=B.Id And  A.类别<>'5'  And A.类别<>'6' And A.类别<>'7'" & strTemp & " And  A.分类id  = [1] "
    End If
    
    Set rs项目 = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(str分类))
    'Call zlInitLvwHeadCol(1 )
    LockWindowUpdate lvwMain_S.hwnd
    With lvwMain_S.ListItems
        ...
        If rs项目.RecordCount > 0 Then
            ...
            For i = 0 To rs项目.RecordCount - 1
                ...
                Next
                '记录该项目的类别
                lst.ListSubItems(2).Tag = zlCommFun.Nvl(rs项目("类别"))
                rs项目.MoveNext
            Next
        End If
    End With
...
'参数:str项目 项目的标识
    ...
    If str项目 = "" Then
        ...
        msh价目.Rows = 2
        For i = 0 To msh价目.Cols - 1
            msh价目.TextMatrix(1, i) = ""
        Next
        msh从属.Rows = 2
    ...
    If rsTemp.RecordCount > 0 Then
        ...
        End If
        mstrClass = zlCommFun.Nvl(rsTemp!编码)
        mstrClassName = zlCommFun.Nvl(rsTemp!类别)
    Else
        mstrClass = ""
        mstrClassName = ""
        mstr上级Key = ""
    ...
    If rsTemp("是否变价") = 1 Then
        msh价目.Rows = 2
        msh价目.TextMatrix(0, 4) = "最低限价"
        msh价目.TextMatrix(0, 5) = "最高限价"
        msh价目.ColWidth(9) = 1000
    Else
        msh价目.TextMatrix(0, 4) = "原价"
        msh价目.TextMatrix(0, 5) = "现价"
    ...
    gstrSQL = "select a.No,a.ID,a.原价ID,a.收入项目ID,a.原价,a.现价,Nvl(a.缺省价格,0) As 缺省价格,a.收费细目ID" & _
        ",b.名称,a.加班加价率,a.附术收费率,a.变动原因,a.调价说明,a.执行日期,a.终止日期,a.调价人 " & _
        " from 收费价目 a,收入项目 b Where a.收入项目ID=b.ID  And a.收费细目ID = [1] " & _
        IIF(chk价格.Value = 1, "", " and nvl(A.终止日期,to_date('3000','yyyy'))>= sysdate ") & " order by a.执行日期 desc "
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(strID))
        
    msh价目.Rows = IIF(rsTemp.RecordCount = 0, 2, rsTemp.RecordCount + 1)
    msh价目.redraw = False
    msh价目.FillStyle = flexFillRepeat
    msh价目.AllowBigSelection = True
    msh价目.Col = 0
    msh价目.Row = 1
    msh价目.ColSel = 10
    msh价目.RowSel = msh价目.Rows - 1
    msh价目.CellBackColor = &H80000005
    If rsTemp.RecordCount = 0 Then
        For i = 0 To 8
            msh价目.TextMatrix(1, i) = ""
        Next
    Else
        i = 1
        Do Until rsTemp.EOF
            msh价目.TextMatrix(i, 0) = IIF(IsNull(rsTemp("No")), "", rsTemp("No"))
            msh价目.TextMatrix(i, 1) = Format(rsTemp("执行日期"), "yyyy年MM月dd日 hh:mm:ss")
            msh价目.TextMatrix(i, 2) = IIF(IsNull(rsTemp("终止日期")), " ", Format(rsTemp("终止日期"), "yyyy年MM月dd日 hh:mm:ss"))
            msh价目.TextMatrix(i, 3) = rsTemp("名称")
            msh价目.TextMatrix(i, 4) = Format(rsTemp("原价"), "###########0.000;-##########0.000; ; ")
            msh价目.TextMatrix(i, 5) = Format(rsTemp("现价"), "###########0.000;-##########0.000; ; ")
            msh价目.TextMatrix(i, 6) = IIF(IsNull(rsTemp("附术收费率")), 0, rsTemp("附术收费率"))
            msh价目.TextMatrix(i, 7) = IIF(IsNull(rsTemp("加班加价率")), 0, rsTemp("加班加价率"))
            msh价目.TextMatrix(i, 8) = IIF(IsNull(rsTemp("调价说明")), "", rsTemp("调价说明"))
            msh价目.TextMatrix(i, 9) = Format(rsTemp("缺省价格"), "###########0.000;-##########0.000; ; ")
            msh价目.TextMatrix(i, 10) = IIF(IsNull(rsTemp("调价人")), "", rsTemp("调价人"))
            msh价目.RowData(i) = rsTemp("ID")
            '看它是不是现行价格
            If rsTemp("执行日期") <= datCurr Then
                If IIF(IsNull(rsTemp("终止日期")), CDate("3000-01-01"), rsTemp("终止日期")) >= datCurr Then
                    msh价目.Col = 0
                    msh价目.Row = i
                    msh价目.ColSel = 10
                    msh价目.RowSel = i
                    msh价目.CellBackColor = &HDFFFFF
                End If
            End If
            i = i + 1
            rsTemp.MoveNext
    ...
    End If
    '显示别名
    gstrSQL = "select decode( 性质,1,'正名',2,'英文名',3,'拉丁名',4,'化学名',5,'商品名',9,'其他别名','') 名称种类,名称,decode(码类,1,'拼音码',2,'五笔码',3,'数字码','')  码类,nvl(简码,'') 简码 from 收费项目别名 where 收费细目ID=[1] order by 名称种类 ,名称 "
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(strID))
    ...
    Else
        ...
        Do Until rsTemp.EOF
            ...
            mshAlias.TextMatrix(i, 3) = zlCommFun.Nvl(rsTemp("简码"))
            i = i + 1
            rsTemp.MoveNext
        Loop
    ...
    With msh费别
        ...
        Else
            ...
            Do Until rsTemp.EOF
                ...
                .TextMatrix(i, 2) = Format(rsTemp.Fields("实收比率").Value, "###0.00;-##0.00;0.00;0.00")
...
Private Sub chk价格_Click()
    ...
    gstrSQL = "select a.NO,a.ID,a.原价ID,a.收入项目ID,a.原价,a.现价,a.收费细目ID,b.名称,a.加班加价率,a.附术收费率,a.变动原因,a.调价说明,a.执行日期,a.终止日期 " & _
        " from 收费价目 a,收入项目 b Where a.收入项目ID=b.ID  And a.收费细目ID = [1] " & _
        IIF(chk价格.Value = 1, "", " and nvl(A.终止日期,to_date('3000','yyyy'))>= sysdate ") & " order by a.执行日期 desc "
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(strID))
...
On Error GoTo ErrHandle
    ...
    Dim blnNew As Boolean     '代表各种新增
    Dim blnModify As Boolean  '代表各种修改、删除
    Dim blnPrice As Boolean   '代表各种调价、执行科室、从属项目的设置
    Dim blnStart As Boolean   '启用的状态
    Dim blnStop As Boolean    '停用的状态
    Dim blnPrint As Boolean   '打印
    Dim blnExpand As Boolean  '加长编码
    Dim strTemp As String
    ...
    If ActiveControl Is tvwMainItem Then
        ...
        With tvwMainItem
            ...
            blnModify = True
            blnNew = True
            blnExpand = .SelectedItem.Children > 0 Or lvwMain_S.ListItems.Count > 0
        End With
    Else
        With lvwMain_S
            ...
            If .ListItems.Count = 0 Or .SelectedItem Is Nothing Then
                ...
                blnStop = False
                blnModify = False
            ElseIf InStr(.SelectedItem.Icon, "No") Then
                '停用
                blnStart = (.SelectedItem.Icon = "ItemNo")
                blnStop = False
                blnModify = False
            Else
                blnStart = False
                blnStop = (.SelectedItem.Icon = "Item")
                blnModify = (.SelectedItem.Icon = "Item")
            End If
        End With
    End If
    With lvwMain_S
    ...
    If InStr(mstrPrivs, "类别管理") > 0 Then
        ...
        If ActiveControl Is tvwMainItem Then
            '增加
            mnuEditParent.Enabled = blnNew '增加分类
            mnuEditChild.Enabled = blnNew  '增加项目
            mnuEditCopy.Enabled = blnNew    '复制拷贝
            Toolbar1.Buttons("Parent").Enabled = blnNew
            Toolbar1.Buttons("Child").Enabled = blnNew
        End If
    End If
    '编辑
    If lvwMain_S.SelectedItem Is Nothing Then mnuEditCopy.Enabled = False
    
    mnuEditModify.Enabled = blnModify '修改
    mnuEditDelete.Enabled = blnModify '删除
    mnuEditDept.Enabled = blnPrice   '执行科室
    mnuEditSlave.Enabled = blnPrice '从属项目
    mnuEditItemGroup.Enabled = (InStr(mstrPrivs, "价目管理") > 0)
    mnuPriceChargeSet.Enabled = (InStr(mstrPrivs, "费别设置") > 0) And Not lvwMain_S.SelectedItem Is Nothing
    mnuEditStart.Enabled = blnStart  '启用
    mnuEditStop.Enabled = blnStop   '停用
    mnuEditItemGroup.Enabled = blnModify
    ...
    mnuPriceHistory.Enabled = blnPrice
...
Private Sub 权限控制()
    ...
    Else
        ...
        If rsTmp.RecordCount > 0 Then
            gstr医价接口编号 = zlCommFun.Nvl(rsTmp!编号)
            gbln允许医价收费项目 = CStr(zlCommFun.Nvl(rsTmp!医疗)) = "1"
...
On Error GoTo ErrHandle
    ...
    Call OpenRecordset(rsTemp, "收费项目目录")
...
Private Sub picPage_Resize(Index As Integer)
        ...
        With picPage(Index)
                ...
                Case 1
                    msh价目.Left = 50
                    msh价目.Height = .ScaleHeight - 200 - msh价目.Top
                ...
                Case 2
                    ...
                    lvwOutIn.Height = Frame1.Height - lvwOutIn.Top - 100
                Case 3
                    msh从属.Top = 800
