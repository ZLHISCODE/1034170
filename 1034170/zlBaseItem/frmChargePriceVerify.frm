...
Begin VB.Form frmChargePriceVerify 
   ...
   Begin TabDlg.SSTab ssTdetails 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfList 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   -1  'True
         FormatString    =   $"frmChargePriceVerify.frx":6A5D
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
...
Private Sub GetNotVerifyData()
    ...
    vsfNotList.Rows = 1
    gstrSQL = "Select a.Id, a.调价id, a.审核标志, c.名称 As 收费细目, b.名称 As 收入项目, a.原价, a.现价, a.缺省价格, a.填制人, a.填制日期, a.执行日期, a.序号, a.说明" & vbNewLine & _
            "From 收费调价记录 A, 收入项目 B, 收费项目目录 C" & vbNewLine & _
            "Where a.收入项目id = b.Id And a.收费细目id = c.Id And 审核标志 = 0" & vbNewLine & _
            "Order By a.Id, a.调价id, a.序号"
    
    Set rsData = zldatabase.OpenSQLRecord(gstrSQL, "未审核单据查询")
    With vsfNotList
...
Private Sub GetVerifyData()
    ...
    If cobDateArea.Text <> "自定义" Then
        ...
        Case "一个月内"
            ...
            datEndDate = zldatabase.Currentdate
        Case "三个月内"
    ...
    End If
    gstrSQL = "Select a.Id, a.调价id, a.审核标志, c.名称 As 收费细目, b.名称 As 收入项目, a.原价, a.现价, a.缺省价格, a.填制人, a.填制日期, a.执行日期, a.序号, 审核人, 审核日期," & vbNewLine & _
                "       a.说明" & vbNewLine & _
                "From 收费调价记录 A, 收入项目 B, 收费项目目录 C" & vbNewLine & _
                "Where a.收入项目id = b.Id And a.收费细目id = c.Id And (审核标志 = 1 Or 审核标志 = 2)  And a.审核日期 Between [1] And [2]" & vbNewLine & _
                "Order By a.Id, a.调价id, a.序号"
    
    Set rsData = zldatabase.OpenSQLRecord(gstrSQL, "已审核单据查询", datBeginDate, datEndDate)
    With vsfList
        .Rows = 1
        Do While Not rsData.EOF
            .Rows = .Rows + 1
...
Private Sub Form_Load()
    ...
    dtpDateEnd = zldatabase.Currentdate
