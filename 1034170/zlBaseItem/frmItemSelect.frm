...
Public Function ShowSelect(frmParent As Object, _
    ...
    Optional ByVal bln包含核算材料 As Boolean = False) As Long
    ...
    mbln包含核算材料 = bln包含核算材料: mint病人来源 = int病人来源: mbln药房单位 = bln药房单位
    mstr类别 = str类别: mstr排除类别 = str排除类别: mstr输入 = str输入: mlngHwnd = lngHwnd
...
'说明：应放在排序处理之前
    ...
    With vsItem
        ...
        If strPos <> "" Then
            ...
            rsPos.Open
'            Set rsPos.ActionConnection = Nothing
...
'      strIDs=读取的项目ID集,用于判断输入时是否别名不同的同一个收费项目
    ...
    strSQLItem = " And (A.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or A.撤档时间 IS NULL) "
    ...
    If blnLoad Then
        ...
        strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
            ...
            " A.编码,A.名称,NULL as 商品名," & IIF(mstr输入 <> "", "A.简码,", "") & _
            " A.计算单位 as 单位,A.规格,A.产地,A.费用类型,A.医保大类,A.说明," & _
            " Decode(A.是否变价,1,'变价',LTrim(To_Char(Sum(C.现价),'9999990.00000'))) as 单价" & _
