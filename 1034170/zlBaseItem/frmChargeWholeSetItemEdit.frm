...
'        "   Order by 级数ID,编码"
    ...
    vRect = zlGetControlRect(cbo人员.hwnd)
    ...
    End If
    If bytStyle = 0 Then
        intIdx = SeekCboIndex(cbo人员, Val(Nvl(rsTemp!ID)))
        If intIdx <> -1 Then
    ...
    For Each objCtl In Me.Controls
        ...
        Case UCase("TextBox")
            ...
            If Not objCtl Is txt科室 Then
                If zlCommFun.ActualLen(Trim(objCtl.Text)) > objCtl.MaxLength Then
...
Private Sub txtName_Change()
    mblnChange = True
    txtSymbol.Text = zlGetSymbol(txtName, 0, 20)
    txtWB.Text = zlGetSymbol(txtName, 1, 20)
End Sub
...
Private Sub vsWholeSet_KeyPressEdit(ByVal Row As Long, ByVal Col As Long, KeyAscii As Integer)
    With vsWholeSet
        Select Case .Col
            Case .ColIndex("收费项目")
                VsFlxGridCheckKeyPress vsWholeSet, Row, Col, KeyAscii, m文本式
            Case .ColIndex("缺省数量"), .ColIndex("缺省价格"), .ColIndex("缺省付数")
                VsFlxGridCheckKeyPress vsWholeSet, Row, Col, KeyAscii, m金额式
        End Select
    End With
End Sub
...
Private Function Select收费项目(Optional strSearch As String) As Boolean
    ...
    With vsWholeSet
        ...
        int病人来源 = 2
        If strSearch = "" Then
            lng项目id = frmItemSelect.ShowSelect(Me, int病人来源, True, str类别, , , "", zl获取中药形态(.Row), str排除类别)
        Else
            'Call CalcPosition(sngLeft, sngTop, vsWholeSet)
            lng项目id = frmItemSelect.ShowSelect(Me, int病人来源, True, str类别, strSearch, .EditWindow, "", zl获取中药形态(.Row), str排除类别)
        End If
...
Private Function SearchPreLevel(ByVal strInput As String) As Boolean
    ...
    vRect = zlGetControlRect(txtParent.hwnd)
...
Private Function SearchUseDept(ByVal strInput As String) As Boolean
    ...
    vRect = zlGetControlRect(txt科室.hwnd)
...
Private Function SaveData() As Boolean
    ...
    If mEditType = EdI_增加 Then
        lngId = zlDatabase.GetNextId("成套收费项目")
        'Zl_成套收费项目_Insert
