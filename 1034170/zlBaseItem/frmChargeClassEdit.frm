...
Dim blnOk As Boolean
    ...
    lvwMain_ItemClick objList
    For i = 1 To lvwMain.ListItems.Count
        If UCase(zldatabase.GetMax("收费项目类别", "编码", 1)) = UCase(lvwMain.ListItems(i).Text) Then
            blnOk = True
        End If
    Next
    If blnOk = True Then
        txtEdit(1).Text = ""
    Else
        txtEdit(1).Text = zldatabase.GetMax("收费项目类别", "编码", 1)
    End If
...
If blnItemClick = True Then Exit Sub
    ...
    ElseIf Index = 2 Then
        txtEdit(3).Text = zlCommFun.SpellCode(Trim(txtEdit(2).Text))
