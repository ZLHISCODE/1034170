...
Private Sub RaisEffect(picBox As PictureBox, Optional IntStyle As Integer, Optional strTittle As String = "")
    '-----------------------------------------------------------------------------------
    ...
    With picBox
        ...
        .ScaleMode = lngTmp
        If strTittle <> "" Then
            .CurrentX = (.ScaleWidth - .TextWidth(strTittle)) / 2
            .CurrentY = (.ScaleHeight - .TextHeight(strTittle)) / 2
            picBox.Print strTittle
        End If
    End With
End Sub
...
Private Function Runing() As Boolean
    On Error Resume Next
    Err = 0
    Debug.Print 1 / 0
    Runing = (Err = 0)
