...
Dim mintColumn(1 To 2) As Integer
    ...
    If Index = 2 Then
        With lvw_S(2)
            ...
            Else
                ...
                If (Val(Split(lvw_S(1).SelectedItem.Tag & ",", ",")(0)) <> 1 And Val(Split(lvw_S(1).SelectedItem.Tag & ",", ",")(0)) <> 2 _
                    And Val(Split(lvw_S(1).SelectedItem.Tag & ",", ",")(0)) <> 7 And Val(Split(lvw_S(1).SelectedItem.Tag & ",", ",")(0)) <> 8) And tabMain.SelectedItem.Caption = "补结算" Then
                    Source.DragIcon = ils32.ListImages("No").Picture
                    Exit Sub
                End If
...
Private Sub lvw_S_ItemClick(Index As Integer, ByVal Item As MSComctlLib.ListItem)
    ...
    If Index = 2 Then
        ...
        If Not Item Is Nothing Then
            If Val(varData(0)) > 2 Then
                Me.mnuEditDefault.Enabled = False
            Else
                Me.mnuEditDefault.Enabled = True
            End If
            If Val(varData(1)) = 1 Then
                mnuEditDefault.Enabled = False
            End If
        End If
     '李南春：当结算性质为9时，不允许修改和删除
    ElseIf Index = 1 Then
        varData = Split(Item.Tag & ",", ",")
        If Not Item Is Nothing Then
            mnuEditDefault.Enabled = False
...
Private Sub mnuEditModify_Click()
    With lvw_S(mintIndex).SelectedItem
        If mintIndex = 1 Then
            If frmBalanceEdit.编辑结算方式(Mid(.Key, 2)) = True Then
                Fill结算方式
                Fill结算应用
            End If
        Else
...
Private Sub Toolbar1_ButtonClick(ByVal Button As MSComctlLib.Button)
    Select Case Button.Key
        ...
        Case "Help"
            mnuhelptopic_Click
        Case "View"
            mnuViewIcon(lvw_S(mintIndex).View).Checked = False
            If lvw_S(mintIndex).View = 3 Then
                mnuViewIcon(0).Checked = True
                lvw_S(mintIndex).View = 0
            Else
                mnuViewIcon(lvw_S(mintIndex).View + 1).Checked = True
                lvw_S(mintIndex).View = lvw_S(mintIndex).View + 1
            End If
    End Select
End Sub
...
Private Sub Toolbar1_ButtonMenuClick(ByVal ButtonMenu As MSComctlLib.ButtonMenu)
    ...
    lvw_S(mintIndex).View = ButtonMenu.Index - 1
End Sub
...
'功能:把结算方式装入到lvw_S(1)中
    ...
    Call OpenRecordset(rs结算方式, Me.Caption)
...
'功能:把结算方式装入到tabMain中
    ...
    gstrSQL = "select 名称,编码,简码 from 结算场合"
    Call OpenRecordset(rs结算场合, Me.Caption)
...
'功能:把结算方式装入到lvw_S(2)中
    ...
    gstrSQL = _
        ...
        " Where A.结算方式=B.名称(+) And A.应用场合=[1] "
