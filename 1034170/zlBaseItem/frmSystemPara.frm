...
'            chk(chk_启用血库管理系统).Visible = Not rsTmp.EOF
        ...
        Case 267
            chk(chk_门诊处方自动发送).Value = Val(zlCommFun.Nvl(rsTemp("参数值"), 0))
        Case 247
        ...
        Case 301
            chk(chk_血液接收后才允许执行登记).Value = IIF(Val(zlCommFun.Nvl(rsTemp("参数值"), 0)) = 0, 0, 1)
        Case 302
            chk(chk_会诊科室下达医嘱由会诊申请科室处理).Value = IIF(Val(zlCommFun.Nvl(rsTemp("参数值"), 0)) = 0, 0, 1)
        Case 306 '已失效药品禁止入库
            chk(chk_已失效药品禁止入库).Value = IIF(Val(zlCommFun.Nvl(rsTemp("参数值"), 0)) = 0, 0, 1)
        End Select
        rsTemp.MoveNext
    Loop
...
'    strTemp = strTemp & "183," & chk(chk_时价药品取上次售价).Value & ","
    ...
    strTemp = strTemp & "267," & chk(chk_门诊处方自动发送).Value & ","
    ...
    strTemp = strTemp & "301," & chk(chk_血液接收后才允许执行登记).Value & ","
    strTemp = strTemp & "302," & chk(chk_会诊科室下达医嘱由会诊申请科室处理).Value & ","
    strTemp = strTemp & "306," & chk(chk_已失效药品禁止入库).Value & ","
...
Private Sub Save记帐报警线()
    ...
    If mstrDel适用病人 <> "" Then
        ...
        For i = 0 To UBound(strArr) - 1
            If strArr(i) <> "" Then
                str适用病人 = strArr(i) & "|"
                gstrSQL = "zl_记帐报警线_Modify('" & str适用病人 & "',NULL,1)"
                Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
            End If
        Next
    End If
    ...
    For i = 1 To tab报警.Tabs.Count
        ...
        Do While Not mrsWarn.EOF
            strTemp = Nvl(mrsWarn!病区ID) & "," & mrsWarn!报警方法 & "," & _
                mrsWarn!报警值 & "," & Nvl(mrsWarn!报警标志1) & "," & Nvl(mrsWarn!报警标志2) & "," & Nvl(mrsWarn!报警标志3) & "," & Nvl(mrsWarn!催款下限) & "," & Nvl(mrsWarn!催款标准)
                
             If zlCommFun.ActualLen(strValues & "," & strTemp) > 4000 Then
                strValues = str适用病人 & "|" & Mid(strValues, 2)
                gstrSQL = "zl_记帐报警线_Modify(" & "'" & strValues & "',NULL," & IIF(intCount = 0, 1, 0) & ")"
                Call zlDatabase.ExecuteProcedure(gstrSQL, Me.Caption)
                intCount = intCount + 1
                strValues = ""
            End If
       ...
       Case Index = chk_项目执行前必须收费或审核
        ...
        Case Index = chk_启用血库管理系统
            If chk(Index).Value = 1 Then
                ...
                chk(chk_血液接收后才允许执行登记).Enabled = True
                lbl(61).Enabled = True
            ...
            Else
                ...
                chk(chk_血液接收后才允许执行登记).Value = 0
                chk(chk_血液接收后才允许执行登记).Enabled = False
