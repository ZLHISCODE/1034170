    ...
    If Not mobjESign Is Nothing Then
        If mobjESign.RegisterCertificate(arrData, Val(Mid(lvwMain.SelectedItem.Key, 2))) Then
            ...
            If CStr(arrData(7)) <> "" Then
                If Not Savelob(glngSys, 14, Val(Mid(lvwMain.SelectedItem.Key, 2)) & "," & Trim(arrData(2)), CStr(arrData(7)), 1) Then
                    GoTo errH
                End If
            End If
...
'参数:
    ...
    Call OpenRecordset(rs部门, Me.Caption)
...
'参数:str部门ID 部门的标识
    ...
    Call zlCommFun.ShowFlash("正在获取人员信息数据,请稍候 ...", Me)
...
Private Function SaveSignPIC(ByVal lng人员id As Long, ByVal strFileName As String) As Boolean
    ...
    rsTemp.Open gstrSQL, gcnOracle, adOpenStatic, adLockOptimistic
'    Set rsTemp.ActiveConnection = Nothing
    blnOk = zlDatabase.SavePicture(strFileName, rsTemp, "签名图片")
    SaveSignPIC = blnOk
    Exit Function
ErrHandle:
    If ErrCenter = 1 Then Resume
...
'1-体质类型调养;2-体检体辨结论(该图片只有读取，没有保存);3-体检申报记录;4-体检任务人员
...
'返回：成功返回True，失败返回False
    ...
    If GetlobSql(lngSys, Action, KeyWord, strFile, arrSql, bytFunc) Then
        Call ExecuteProcedureBeach(arrSql, "zllobSave", False, False)
    Else
        Savelob = False
    End If
    Savelob = True
...
'1-体质类型调养;2-体检体辨结论;3-体检申报记录;4-体检任务人员
