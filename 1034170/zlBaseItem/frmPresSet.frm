...
Private Sub ClearContext()
    ...
    txtEdit(Text编号).Text = zlDatabase.GetMax("人员表", "编号", 6)
    mblnChange = False
End Sub
...
Private Function Save人员() As Boolean
    ...
    If mstrID = "" Then       '新增一条记录
        lng人员id = zlDatabase.GetNextId("人员表")
...
Public Function 编辑人员(Optional strID As String = "", Optional ByVal str部门ID As String = "") As Boolean
    ...
    If strID <> "" Then
        ...
        gstrSQL = "Select  a.ID, a.编号,b.编码 as 执业类别编码, a.姓名, a.简码, a.身份证号, a.出生日期, a.性别, a.民族, a.工作日期, a.办公室电话,a.移动电话, a.电子邮件,b.名称 as 执业类别, a.执业范围, a.管理职务, a.专业技术职务,a.聘任技术职务, a.学历, a.所学专业, a.留学时间, a.留学渠道," & _
                          " a.接受培训 , a.科研课题, a.个人简介, a.别名, a.站点, a.签名, a.签名图片, a.执业证号, a.资格证书号, a.执业开始日期, a.处方权标志,a.手术等级,a.顺序 " & _
                   " From 人员表 a,执业类别 b" & _
        ...
        If Len(rsTemp.Fields("签名图片").Value) > 0 Then
            strTempFile = zlDatabase.ReadPicture(rsTemp, "签名图片")
            If strTempFile <> "" Then
...
'        If rsTemp.State = adStateOpen Then rsTemp.Close
        ...
        '处理图片
        gstrSQL = "select 照片 from 人员照片 where 人员ID=[1]"
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, Val(strID))
        ...
        Call zlControl.CboSetIndex(cmbKss(0).hwnd, Val(rsTemp!级别 & ""))
        '门诊抗菌药物权限
        strSQL = "Select Max(级别) 级别 from 人员抗菌药物权限 where 人员ID=[1] And 记录状态=1 and 场合 = 2 "
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, Val(strID))
        Call zlControl.CboSetIndex(cmbKss(1).hwnd, Val(rsTemp!级别 & ""))
...
Private Sub txtEdit_Change(Index As Integer)
    ...
    If Index = Text姓名 Then
        txtEdit(text简码).Text = zlCommFun.SpellCode(txtEdit(Text姓名).Text)
...
'    If SendMessage(cmb编码(Index).hwnd, CB_GETDROPPEDSTATE, 0, 0) = 0 And KeyAscii <> 27 And KeyAscii <> 13 Then zlCommFun.PressKey vbKeyF4
    ...
    If Index = code民族 Then
        ...
        End If
    Else
        lngIdx = MatchIndex(cmb编码(Index).hwnd, KeyAscii)
...
Dim rsTemp As New ADODB.Recordset, blnOk As Boolean
    ...
    On Error GoTo errHandle
    gstrSQL = "select ID,签名图片 from 人员表 where ID=" & lng人员id
    rsTemp.Open gstrSQL, gcnOracle, adOpenStatic, adLockOptimistic '注意打开方式
    If blnClear Then
        rsTemp.Fields("签名图片").Value = Null
        rsTemp.Update
        blnOk = True
    Else
        blnOk = zlDatabase.SavePicture(pic签名图片.Tag, rsTemp, "签名图片")
        Kill pic签名图片.Tag
    End If
    Save签名图片 = blnOk
    Exit Function
errHandle:
    ...
    Call SaveErrLog
End Function
Private Function Save照片(ByVal lng人员id As Long) As Boolean
    '-------------------------------------------------------------
    ...
    FileNum = FreeFile
    Open img照片.Tag For Binary Access Read As FileNum
    ...
    gstrSQL = "select 人员ID,照片 from 人员照片 where 人员ID=" & lng人员id
    Call SQLTest(App.ProductName, Me.Caption, gstrSQL)
    rsTable.Open gstrSQL, gcnOracle, adOpenStatic, adLockOptimistic '注意打开方式
'    Set rsTable.ActiveConnection = Nothing
    Call SQLTest
    
    rsTable.AddNew
    rsTable("人员ID") = lng人员id
    ...
    For j = 0 To intBolcks
        ...
        rsTable("照片").AppendChunk aryChunk()
    Next j
    rsTable.Update
    Close FileNum
    Save照片 = True
