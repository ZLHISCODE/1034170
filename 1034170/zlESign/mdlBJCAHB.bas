...
Public Function BJCAHB_CheckCert(ByVal strCurrCertSn As String, Optional ByRef strSigCert As String, Optional ByRef CertID As String) As Boolean
    '功能：读取USB进行设备初始化并登录
     ...
     If Not BJCAHB_InitObj Then
        MsgBox "检查部件是否初始化！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
     End If
      '获取证书
...
Public Function BJCAHB_Sign(ByVal strCurrCertSn As String, ByVal strSource As String, ByRef strSignData As String, _
    ...
    If strErr <> "" Then
        MsgBox strErr, vbOKOnly + vbInformation, gstrSysName
        BJCAHB_Sign = False
        Exit Function
    End If
    BJCAHB_Sign = True
    Exit Function
errH:
     MsgBox "签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
'''验证签名函数
...
'验证签名
    ...
    If strStampCode <> "" Then
        ...
        If lngRuslt <> 0 Then
            MsgBox "验证时间戳失败！" & GetReturnInfo(lngRuslt), vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
    End If
...
Private Sub ValidateCertView(retValidateCert)
    Select Case retValidateCert
        ...
        Case -5
            MsgBox "证书未生效！"
        Case Else
            MsgBox "验证服务器证书失败！验证返回值:" & retValidateCert
    End Select
End Sub
