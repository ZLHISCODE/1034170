...
106     Set mobjSoapClient = CreateObject("MSSOAP.SoapClient30")  'SOAP连接对象，
        ...
        If Err.Number <> 0 Then
            MsgBox "系统初始化失败！" & vbCrLf & vbCrLf & "客户端未安装SOAP！" & vbCrLf & vbCrLf & "错误信息如下：" & vbCrLf & vbCrLf & Err.Description, vbCritical, "电子签名部件"
            Exit Function
        End If
...
'        strURL = "http://202.102.85.153:8080/HealthWebService.asmx?WSDL"
        ...
        If Err.Number <> 0 Then
            MsgBox "系统初始化失败！" & vbCrLf & vbCrLf & "服务器地址出错！" & vbCrLf & vbCrLf & "错误信息如下：" & vbCrLf & vbCrLf & Err.Description, vbCritical, "电子签名部件"
            Exit Function
        End If
        Err.Clear: On Error GoTo 0
...
116     mblnInit = JSCA_InitObj
        Exit Function
errH:
118     MsgBox "创建江苏CA接口部件失败！" & vbNewLine & Err.Description, vbQuestion, "电子签名部件"
...
errH:
    MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, "电子签名部件"
End Function
...
Public Function JSCA_GetCertList(ByRef strName As String, Optional ByRef strUniqueID As String, Optional ByRef strCert As String, Optional ByVal strUserUnigueID As String) As Boolean
    ...
    If Not JSCA_Client Is Nothing Then
        ...
        If strUniqueID = "" Then
            MsgBox "请您插入Key！", vbInformation + vbOKOnly, "电子签名部件"
            Exit Function
        End If
    ...
    Else
        MsgBox "江苏CA部件初始化失败。", vbInformation, "电子签名部件"
        Exit Function
    End If
    JSCA_GetCertList = True
    Exit Function
errH:
    MsgBox "读取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, "电子签名部件"
...
'                    JSCA_Sign = True
'                Else
'                    MsgBox "时间戳请求失败！", vbExclamation, "电子签名部件"
'                End If
            Else
                MsgBox "签名失败！", vbExclamation, "电子签名部件"
            End If
        Else
            MsgBox "签名失败！", vbExclamation, "电子签名部件"
        End If
        Exit Function
errH:
114     MsgBox "签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, "电子签名部件"
End Function
...
108            MsgBox "该证书未注册在您的名下，不能使用！", vbInformation + vbOKOnly, "电子签名部件"
               Exit Function
           End If
110
...
122     End If
        ...
        Exit Function
errH:
124     MsgBox "检查USBKEY失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, "电子签名部件"
End Function
...
'        If lngTimes < -1 Then
'            MsgBox "密码验证失败！请重新注册CA部件【CACltCore.dll】。", vbInformation + vbOKOnly, "电子签名部件"
'            Exit Function
'        End If
    ...
    If (intDay <= 30 And intDay > 0 And Not gblnShow) Then
        MsgBox "您的证书还有" & intDay & "天过期。", vbInformation + vbOKOnly, "电子签名部件"
        gblnShow = True
        GetCertLogin = True
    ElseIf (intDay <= 0) Then
        MsgBox "您的证书已过期 " & Abs(intDay) & " 天。", vbInformation + vbOKOnly, "电子签名部件"
        GetCertLogin = False
    End If
    GetCertLogin = True
End Function
...
'        If strTmp <> "0" Then
'            MsgBox "时间戳验证失败！", vbExclamation, "电子签名部件"
'            Exit Function
'        End If
        strTmp = mobjSoapClient.verifySignedData(strSigCert, strSource, strSignData)
        If strTmp = "0" Then
            MsgBox "验证成功，该电子签名数据有效!", vbInformation, "电子签名部件"
        Else
            MsgBox "验证签名失败！", vbExclamation, "电子签名部件"
            Exit Function
        End If
        ...
        Exit Function
errH:
104     MsgBox "验证签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, "电子签名部件"
End Function
