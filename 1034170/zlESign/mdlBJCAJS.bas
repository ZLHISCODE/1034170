...
106     Set BJCAJS_svs = CreateObject("BJCA_SVS_ClientCOM.BJCASVSEngine.1")   '创建证书验证控件
        If Not BJCAJS_GetPara Then Exit Function
...
106     If BJCAJS_TS Is Nothing And gudtPara.blnISTS Then Set BJCAJS_TS = CreateObject("BJCA_TS_ClientCom.BJCATSEngine")     '创建时间戳控件
...
102 If BJCAJS_TS Is Nothing And gudtPara.blnISTS Then Set BJCAJS_TS = CreateObject("BJCA_TS_ClientCom.BJCATSEngine")     '创建时间戳控件
    '验证时间戳
    If gudtPara.blnISTS And strTimeStampCode <> "" Then
104     strTmp = ""
...
2       If BJCAJS_Client Is Nothing Then Set BJCAJS_Client = CreateObject("XTXAppCOM.XTXApp.1")
3       If BJCAJS_Pic Is Nothing Then Set BJCAJS_Pic = CreateObject("GetKeyPic.GetPic")
        '获取证书
4       strUsbkeyList = BJCAJS_Client.SOF_GetUserList()
5       If (strUsbkeyList = "") Then
6           strName = ""
7           MsgBox "请插入证书Key！", vbOKOnly + vbInformation, gstrSysName
8           BJCAJS_GetCertList = False
9           Exit Function
10      Else
11          arrUserList = Split(strUsbkeyList, "&&&") '宿迁人民四(测试)||999000100089956/6001201312021788&&&宿迁人民二(测试)||999000100089948/6002201309019595&&&
12          If UBound(arrUserList) > 1 Then  '多个KEY
13              For i = LBound(arrUserList) To UBound(arrUserList) - 1
14                  strUser = strUser & "&&&" & Split(arrUserList(i), "||")(0)
15              Next
16              If strUser <> "" Then strUser = Mid(strUser, 4)
17              strName = frmSelectUser.ShowMe(strUser)
            
18              For i = LBound(arrUserList) To UBound(arrUserList) - 1
19                 If strName = Split(arrUserList(i), "||")(0) Then
20                      strCertID = Split(arrUserList(i), "||")(1)
21                      Exit For
22                 End If
23              Next
24          Else
25              arrUserList = Split(arrUserList(0), "||")
26              strName = arrUserList(0)      '证书CN通用名
27              strCertID = arrUserList(1)    '证书ID
28          End If
        
29          strCert = BJCAJS_Client.SOF_ExportUserCert(strCertID) '3.导出签名证书。
        
30          If strCertSn <> "0" Then strCertSn = BJCAJS_Client.SOF_GetCertInfo(strCert, 2) '证书序列号 签名时要用
31          If strCertDN <> "0" Then strCertDN = BJCAJS_Client.SOF_GetCertInfo(strCert, 33) '证书DN
32          If strCertUserID <> "0" Then
33              strCertUserID = BJCAJS_Client.SOF_GetCertInfoByOid(strCert, "1.2.156.112562.2.1.1.1") '2.获取证书唯一标识（一般为身份证号）SF+身份证号
34              If strCertUserID = "" Then strCertUserID = BJCAJS_Client.SOF_GetCertInfoByOid(strCert, "2.16.840.1.113732.2") '新邵县人民医院获取证书唯一标识
35              strCertUserID = Right(strCertUserID, 18)
36          End If
37      End If
38      BJCAJS_GetCertList = True
39      Exit Function
errH:
40        MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
Public Function BJCAJS_GetPara() As Boolean
'设置湖北CA服务器地址
    Dim arrList As Variant
    On Error GoTo errH
    gstrPara = gobjComLib.zlDatabase.GetPara(90000, glngSys)
    gudtPara.blnISTS = Val(gstrPara) = 1
    BJCAJS_GetPara = True
    Exit Function
errH:
...
Public Function BJCAJS_SetParaStr() As String
    BJCAJS_SetParaStr = IIf(gudtPara.blnISTS, "1", "0")
End Function
