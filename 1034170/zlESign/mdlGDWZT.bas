...
126 WZT_InitObj = True
    ...
    Exit Function
errMsg:
128 MsgBox "创建网证通对象失败！请确认【网证通安全客户端】已安装且该部件【NetcaPkiCom.dll】已注册成功。" & vbCrLf & "错误行:" & Erl, vbExclamation, gstrSysName
    Exit Function
errH:
130  MsgBox "创建接口部件失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbExclamation, gstrSysName
End Function
...
'                "", "GDCA信任号TrustID")
        ...
        If strPic = "" Then
            ...
            varRet = mobjPDFSign.SelectCert("", 1)
            varRet = mobjPDFUtilTool.GetImageFromDevicByCert(mobjPDFSign.SignCertBase64Encode)
            strBase64 = Base64Encode(varRet)
            If strBase64 <> "" Then
                strPic = SaveBase64ToFile("gif", strUniqueID, strBase64)
            End If
        End If
