...
110     mblnInit = GDCA_initObj
        Exit Function
errH:
112     MsgBox "调用初始化接口失败！" & vbNewLine & Err.Description, vbQuestion, gstrSysName
End Function
...
116             GDCA_RegCert = True
        ...
        Exit Function
errH:
118     MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
104         If strTmp = strEndData Then GDCA_VerifySign = True
        ...
        Exit Function
errH:
106     MsgBox "验证签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
106             If strCurrCertSn = strKeyId Then GDCA_CheckCert = True
        ...
        Exit Function
errH:
108     MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
132     ReadUSBKeyLogin = True
        ...
        Exit Function
errH:
134     MsgBox "初始化KEY失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
128         VerifyCert = True
        Else
130         MsgBox "证书验证失败！证书效期" & strCertTime, vbQuestion, gstrSysName
132         VerifyCert = False
        End If
        Exit Function
errH:
134     MsgBox "验证证书失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
