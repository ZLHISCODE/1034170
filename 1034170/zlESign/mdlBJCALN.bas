...
116     mblnInit = BJCALN_InitObj
        ...
        Exit Function
errH:
118     MsgBox CStr(Erl()) & "行，创建接口部件失败！" & vbNewLine & Err.Description, vbQuestion, gstrSysName
End Function
...
'    Exit Function
'errH:
'    MsgBox "证书显示失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
'End Function
...
Private Function GetCertLogin(ByVal strUniqueID As String, ByVal strPassword As String, ByVal strCert As String, ByRef dDate As Integer, ByRef strWebserviceUrl As String) As Boolean
    ...
    Else
        ...
        'result:非0:不成功
        If mintLogin >= 8 Then
            MsgBox "已经输入了" & mintLogin & "次错误密码，超过了最大输入次数！"
            Exit Function
        End If
...
'            strExtLib = mobjBJCALN.GetUserInfo(strUniqueID, 15) '后面没有用
        ...
        Else
            ...
            MsgBox "证书密码可能不正确，您已经输入了" & mintLogin & "次密码，还可以输入" & 8 - mintLogin & "次!"
            GetCertLogin = False
