...
4000    Set HUBEI_PIC = CreateObject("HBCA_SOFSeal.Seal")
        ...
        If UBound(arrList) <> 3 Then
            MsgBox "签名服务器地址配置格式有误,请到基础参数进行重新设置。", vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
        lngRet = -1
        strSIGNIP = arrList(0)
        ...
        If lngRet <> 0 Then
            MsgBox "签名服务器初始化失败！", vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
        lngRet = -1
        strTSIP = arrList(2)
        ...
        If lngRet <> 0 Then
            MsgBox "时间戳服务器初始化失败！", vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
    ...
    Exit Function
errH:
     MsgBox "创建接口部件失败！" & vbNewLine & Err.Description, vbQuestion, gstrSysName
End Function
...
'strPicPath     缺省不读取
    ...
    If (strUsbkeyList = "") Then
        strName = ""
        MsgBox "请插入证书Key！", vbOKOnly + vbInformation, gstrSysName
    ...
    Exit Function
errH:
     MsgBox "读取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbQuestion, gstrSysName
End Function
...
'            Exit Function
'        End If
'        lngRet = HUBEI_Client.SOF_Login(strCertID, strPIN)
'        If lngRet <> 0 Then
...
'   strPID --用户身份标识（一般为身份证号）
    ...
    blnCheck = HUBEI_CheckCert(strCurrCertSn, strCertID)
    ...
    If blnCheck Then                '验证当前USB是否是签名用户的，并获取签名证书
        ...
        strSignData = HUBEI_Client.SOF_SignDataByP7(strCertID, strSource)
        If strSignData <> "" Then
            lngRet = -1
            'detach
            ...
            If lngRet = 0 Then
                strTimeStampCodeID = HUBEI_TS.SOF_CreateTimeStampResponse(strSignData)  '时间戳ID保存数据库
                If strTimeStampCodeID = "" Then
                    MsgBox "生成时间戳ID失败！" & HUBEI_TS.SOF_GetErrorMsg(), vbOKOnly + vbInformation, gstrSysName
                    Exit Function
                End If
                strTimeStamp = Format(gobjComLib.zlDatabase.Currentdate, "yyyy-MM-dd HH:mm:ss")
            Else
                MsgBox "验证签名失败！" & HUBEI_SVS.SOF_GetErrorMsg(), vbOKOnly + vbInformation, gstrSysName
                Exit Function
            End If
        Else
            MsgBox "签名失败！" & HUBEI_Client.SOF_GetErrorMsg(), vbOKOnly + vbInformation, gstrSysName
            Exit Function
        End If
    Else
        Exit Function
    ...
    Exit Function
errH:
     MsgBox "签名失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
Private Function GetCertLogin(ByVal strCertID As String) As Boolean
    ...
    If lngRet <> 0 Then
        MsgBox "登陆失败！" & HUBEI_Client.SOF_GetErrorMsg(), vbOKOnly + vbInformation, gstrSysName
        Exit Function
    End If
