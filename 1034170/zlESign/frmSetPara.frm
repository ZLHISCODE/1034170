...
Private Sub cmdPara_Click(Index As Integer)
    ...
    If Index = CMD_OK Then
        Select Case gintCA
        Case ca_湖北
            gstrPara = HUBEI_SetParaStr
        Case CA_河北CA邯郸
            gstrPara = HBCA_SetParaStr
        Case CA_辽宁三院
            gstrPara = LNCA_SetParaStr
        Case CA_江苏CA
            gstrPara = JSCA_SetParaStr
        Case CA_北京
            gstrPara = BJCA_SetParaStr
        Case CA_新疆CA
            gstrPara = XJCA_SetParaStr
        Case CA_河南CA商丘
            gstrPara = objCA.HNCA_SetParaStr
        Case ca_深圳
            gstrPara = SZCA_SetParaStr
        Case CA_北京CA广西
            gstrPara = BJCAGX_SetParaStr
        Case CA_吉林安信
            gstrPara = ANXIN_SetParaStr
        Case CA_内蒙古
            gstrPara = NMG_SetParaStr
        Case CA_网证通
            gstrPara = WZT_SetParaStr
        Case CA_上海CA
            gstrPara = SHCA_SetParaStr
        End Select
        On Error GoTo errH
...
Private Sub Form_Load()
    ...
    Case CA_辽宁三院
        ...
        chkTS(4).Visible = True
        Call LNCA_GetPara
        lblNote.Caption = "参数格式:http://218.25.86.214:2010/ssoworker"
        txtPara(6).Text = gudtPara.strSignURL
        chkTS(4).Value = gudtPara.bytSignVersion
    Case CA_江苏CA
        Me.Width = 7080
        Me.Height = 2970
        picPara(2).Visible = True
    ...
    Exit Sub
errH:
    MsgBox "参数设置加载失败！" & vbNewLine & Err.Description, vbQuestion, gstrSysName
End Sub
...
Private Sub Form_Resize()
    ...
    Case CA_辽宁三院, CA_江苏CA, CA_新疆CA, CA_河南CA商丘
