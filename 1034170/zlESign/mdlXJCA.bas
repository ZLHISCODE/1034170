...
108         Set mobjXJCA_Client = CreateObject("xjcaTechATL.xjcaTechATLLib.1")
            ...
            If mobjXJCA_Client Is Nothing Or mobjGseal Is Nothing Then
                MsgBox "CA对象创建失败！", vbOKOnly, gstrSysName
                Exit Function
            End If
        Else
            mbytType = 2
...
116     mblnInit = XJCA_InitObj
        Exit Function
errH:
118     MsgBox "创建新疆CA接口部件失败！" & vbNewLine & Err.Description, vbQuestion, gstrSysName
...
'  strSigCert -签名证书内容
        ...
        If Not XJCA_InitObj() Then
             MsgBox "部件未初始化！", vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
...
'   mstrPara =http://124.117.245.71:48080/webServices/ssoService|4028f6d24a2d7182014a2d83333e001a|华大   华大KEY
    ...
    If strTmp <> "" Then
        ...
        If strTmp = "FALSE" Then
            MsgBox "登录认证失败!", vbInformation + vbOKOnly, gstrSysName
...
'
        ...
        Else
            MsgBox "验证签名失败！", vbExclamation, gstrSysName
            Exit Function
        End If
...
'功能：检查CA环境
    ...
    If lngRet <> 10000 Then
        MsgBox "证书驱动未安装！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    '2-判断证书是否就绪
    lngRet = mobjXJCA_Client.XJCA_KeyInsert(strTmp)
    If lngRet <> 10000 Then
        MsgBox "证书KEY未插入！", vbInformation + vbOKOnly, gstrSysName
        Exit Function
    End If
    CheckIsXJCA = True
End Function
