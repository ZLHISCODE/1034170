...
Public Function ZGRYY_initObj() As Boolean
    ...
    If Err.Number <> 0 Then
        MsgBox "安信签名控件没有安装！", vbExclamation, gstrSysName
        Exit Function
    End If
    
    Set mobjAXSVR = CreateObject("AnXEgovCom.AnXEgovSOF")
    If Err.Number <> 0 Then
        MsgBox "安信服务端组件没有安装！", vbExclamation, gstrSysName
        Exit Function
    End If
    ...
    If Err.Number <> 0 Then
        MsgBox "时间戳控件没有安装！", vbExclamation, gstrSysName
        Exit Function
    End If
    ZGRYY_initObj = True
End Function
...
104         MsgBox "电子签名控件未正确安装。", vbExclamation, gstrSysName
            Exit Function
        End If
...
128             strImage = ""
            End If
        Else
130         MsgBox mobjAXCA.GetLastError
            Exit Function
        End If
...
140     ZGRYY_RegCert = True
    Exit Function
errH:
142 MsgBox "注册证书-第" & CStr(Erl()) & "行," & Err.Description, vbQuestion, "电子签名"
End Function
...
110         mLastPWD = ""
112         MsgBox "当前数字证书验证失败。" & mobjAXCA.GetLastError, vbExclamation, gstrSysName
            Exit Function
        End If
...
114     ZGRYY_CheckCert = True
        Exit Function
hErr:
116     MsgBox "检查证书-第" & CStr(Erl()) & "行," & Err.Description, vbExclamation, gstrSysName
End Function
...
108 strClientSignData = mobjAXCA.SignString(strSource, True)
    ' MsgBox strTimeStamp '不对，签名时没取到时间，验证时才取的时间戳。
...
120     MsgBox mobjAXCA.GetLastError, vbExclamation, gstrSysName
        Exit Function
    End If
...
126     MsgBox "安信服务端签名验证失败，错误码" & intSvrVerfy
        Exit Function
    End If
...
132 ZGRYY_Sign = True
    Exit Function
errH:
134 MsgBox "签名-第" & CStr(Erl()) & "行," & Err.Description, vbExclamation, gstrSysName
End Function
...
Public Function ZGRYY_VerifySign(ByVal strCurrCertSn As String, ByVal strSignData As String, ByVal strSource As String, ByVal strTime As String) As Boolean
    ...
    If verify_Timestamp(strSource) = False Then
        MsgBox "时间戳验证失败！", vbExclamation, gstrSysName
        Exit Function '时间戳验证
    End If
    ...
    If Not ZGRYY_VerifySign Then
        MsgBox "签名验证失败！" & mobjAXCA.GetLastError, vbExclamation, gstrSysName
        Exit Function
    Else
        MsgBox "签名验证成功！"
    End If
...
106         MsgBox "申请时间戳失败！" & strSign, vbExclamation, gstrSysName
            Times_Tamp = False
            Exit Function
        Else
108         intCount = 0
...
120                     strTimeStamp = Format(CDate(strTimeStamp), "yyyy-MM-dd HH:mm:ss")
                        ...
                        Exit Function
                    Else
122                     MsgBox "获取的时间戳不是一个日期！" & strTimeStamp, vbExclamation, gstrSysName
                    End If
124             ElseIf strSign <> "1003" Then
126                 strSign = GetReturnInfo(strSign)
128                 MsgBox "获取时间戳失败！" & strSign, vbExclamation, gstrSysName
                    Exit Function
                End If
...
132     Times_Tamp = True
        Exit Function
hErr:
134     MsgBox "取时间戳-第" & CStr(Erl()) & "行," & Err.Description, vbExclamation, gstrSysName
End Function
...
Private Function verify_Timestamp(ByVal strSource As String) As Boolean
    ...
    If strData <> "2010" Then
        MsgBox "验证时间戳失败！" & GetReturnInfo(strData), vbExclamation, gstrSysName
        Exit Function
    End If
...
Private Function verify_getTimestamp(ByVal strSource As String) As String
    ...
    If strData = "2001" Then
        MsgBox "获取验证时间戳失败！" & GetReturnInfo(strData), vbExclamation, gstrSysName
...
Private Function AxServer_VerifyCert() As Boolean
    ...
    If intCheck <> 0 Then
        mLastPWD = ""
        MsgBox "当前数字证书服务端验证失败！错误码为" & intCheck
        Exit Function
    End If
