...
errH:
118     MsgBox "初始化电子签名部件失败！" & vbNewLine & "第" & CStr(Erl()) & "行：" & Err.Description, vbInformation, gstrSysName
...
errH:
    MsgBox "证书注册失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
1052                strTSCert = FuncEncodeBase64Byte(arrCertData, lngCertLen)
                Else
                    MsgBox "时间戳证书获取失败！", vbOKOnly + vbInformation, gstrSysName
                    Exit Function
                End If
            End If
        Else
...
1090
    ...
    Exit Function
errH:
    MsgBox "获取证书信息失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
1050
    ...
    Exit Function
errH:
    MsgBox "证书登录失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
1008        MsgBox "该证书未注册在您的名下，不能使用！", vbInformation + vbOKOnly, gstrSysName
            Exit Function
        End If
...
1016
        ...
        Exit Function
errH:
124     MsgBox "检查USBKEY失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
...
1090
    Exit Function
errH:
    MsgBox "验签失败！" & vbNewLine & "第" & CStr(Erl()) & "行 " & Err.Description, vbInformation, gstrSysName
End Function
