...
Private WithEvents mfrmQueueQuick As frmQueueQuick     '快捷叫号窗口
Attribute mfrmQueueQuick.VB_VarHelpID = -1
Private mblnResult As Boolean           '快捷窗口操作结果
...
Private Sub mfrmQueueQuick_DoExecute(ByVal lngType As Long, ByVal lngID As Long, ByVal strValue As String, ByRef blnResult As Boolean)
    mblnResult = blnResult
    Select Case lngType
        Case 1  '顺呼
            mblnIsSelectedCallingList = False
            Call comMenu_顺呼
        Case 2  '重呼
            Call SetPatientFocus(qftCalledQueue, lngID, strValue)
            Call comMenu_广播
        Case 3  '接诊
            Call SetPatientFocus(qftCalledQueue, lngID, strValue)
            Call comMenu_接诊
    ...
    blnResult = mblnResult
End Sub

Private Sub SetPatientFocus(ByVal lngQueueFromType As TQueueFromType, lngID As Long, strValue As String)
    ...
    '以下焦点转移操作增加标题栏颜色的处理。
    If lngQueueFromType = qftWaitQueue Then
    ...
    lngIdIndex = GetColIndex("ID", objList)
    lngCurPatient = GetRowIndex(lngQueueFromType, "队列名称", strValue)
    
    '通过ID在呼叫列表中选中当前呼叫的病人
    If lngCurPatient >= 0 Then
        With objList
            For i = lngCurPatient To .Rows.Count - 1
                If .Rows(i).GroupRow Then Exit For
                If Val(.Rows(i).Record(lngIdIndex).value) = lngID Then
...
Private Sub mfrmQueueQuick_IsQueueing(ByVal lngID As Long, blnResult As Boolean)
    blnResult = CheckIsQueueing(lngID)
End Sub

Private Sub mfrmQueueQuick_SetPatientFocus(ByVal blnQueueList As Boolean, ByVal lngID As Long, ByVal strValue As String)
    If blnQueueList Then
        Call SetPatientFocus(qftWaitQueue, lngID, strValue)
    Else
        Call SetPatientFocus(qftCalledQueue, lngID, strValue)
    End If
End Sub

Private Sub mfrmQueueQuick_DoUnload()
    Set mfrmQueueQuick = Nothing
End Sub
...
'检查权限是否存在
    ...
    If App.LogMode = 0 Then CheckPopedom = True
...
Private Sub InitCommandBars()
    ...
    With cbrToolBar1.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Queue_Quick, "快捷叫号"): cbrControl.IconId = 7052: cbrControl.ToolTipText = "弹出快捷叫号窗口"
...
'获取排队队列的数据显示状态，排队，暂停，弃呼，完成
    ...
    Dim objReportControl As ReportControl
    
     '得到当前选中的排队状态值
    lngState = -1
    
    If mblnIsSelectedCallingList Then
        Set objReportControl = rptCallList
    Else
        Set objReportControl = rptQueueList
    End If
    ...
    If rptQueueList.SelectedRows.Count > 0 Then
        If rptQueueList.SelectedRows(0).GroupRow = False Then
            ...
            ElseIf strState = "待预呼" Then
                lngState = TQueueSelState.qss待预呼
            ElseIf strState = "已预呼" Then
                lngState = TQueueSelState.qss已预呼
            Else
...
'                            TQueueState.qsComplete)
        ...
        '说明：状态5：待预呼   状态6 已预呼
        If strFilter = "" Then
            strFilter = "排队状态=0 or 排队状态=2 or 排队状态=3 or 排队状态=4 or 排队状态=5 or 排队状态=6 "
        Else
            strFilter = strFilter & " or 排队状态=5 or 排队状态=6 "
        End If
    Else
        ...
        Else
            ...
            ElseIf rptControl.Columns(i).Caption = "排队状态" Then
                ...
                Select Case Val(rsData!排队状态)
                    ...
                    Case TQueueState.qsBeforeWaitCall
                        rptItems(lngColIndex).value = "待预呼"
                    Case TQueueState.qsAfterWaitCall
                        rptItems(lngColIndex).value = "已预呼"
                End Select
            ...
            If lngColIndex = lngFirstCol Then
                ...
                ElseIf lngQueueState = TQueueState.qsCalled Or lngQueueState = TQueueState.qsAfterWaitCall Then
...
On Error GoTo errHandle
    ...
    If Not mfrmQueueQuick Is Nothing Then
        mfrmQueueQuick.RefreshQueueQuick rptQueueList, rptCallList, mstrQueryQueueNames
    End If
...
On Error GoTo errHandle
    ...
    '对实际有刷新数据而没有触发队列数据刷新的操作，需要在这里刷新快捷界面
    Select Case Control.Id
        '插队、修改、顺呼、直呼、弃号、暂停、恢复、重排
        Case conMenu_Queue_CallThis, conMenu_Queue_CallNext, conMenu_Queue_Restore, conMenu_Queue_Abandon, conMenu_Queue_Pause, conMenu_Queue_InsertQueue, conMenu_Queue_Update, conMenu_Queue_RestartQueue
            If Not mfrmQueueQuick Is Nothing Then
                mfrmQueueQuick.RefreshQueueQuick rptQueueList, rptCallList, mstrQueryQueueNames
            End If
    End Select
...
Public Sub zlUpdateCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    Dim intQueueState As Integer
    
    '获取排队队列的显示状态
    intQueueState = GetWaitQueueSelState()
    ...
    Select Case Control.Id
        ...
        Case conMenu_Queue_CallNext     '顺呼           '只有处于排队中的数据，才能进行顺乎操作
          ...
          Control.Enabled = Not mblnIsSelectedCallingList And (intQueueState = TQueueSelState.qss排队中 Or intQueueState = TQueueSelState.qss待预呼 Or intQueueState = TQueueSelState.qss已预呼) And Not mblnIsFindQueue
        ...
        Case conMenu_Queue_Broadcast    '广播           '只有呼叫后的数据才能进行广播
          ...
          If mblnIsSelectedCallingList Then
                Control.Caption = "重呼"
                Control.ToolTipText = "对呼叫列表的患者重新呼叫"
                Control.Enabled = CheckPopedom("广播") And rptCallList.SelectedRows.Count > 0 And Not mblnIsFindQueue
            Else
                Control.Caption = "预呼"
                Control.ToolTipText = "对排队列表患者进行呼叫让患者提前准备"
                Control.Enabled = CheckPopedom("预呼") And rptQueueList.SelectedRows.Count > 0 And _
                Not mblnIsFindQueue And intQueueState <> TQueueSelState.qss已暂停 And intQueueState <> TQueueSelState.qss已弃号
            End If
        ...
        Case conMenu_Queue_RestartQueue '重排
          ...
          Control.Enabled = intQueueState <> -1
          
        Case conMenu_Queue_RecDiagnose  '接诊
          Control.Visible = CheckPopedom("接诊")
          Control.Enabled = mblnIsSelectedCallingList Or mblnIsFindQueue Or _
            (Not mblnIsSelectedCallingList And intQueueState <> TQueueSelState.qss已完成 And intQueueState <> -1 And intQueueState <> TQueueSelState.qss排队中 And intQueueState <> TQueueSelState.qss待预呼 And intQueueState <> TQueueSelState.qss已预呼)
          
        Case conMenu_Queue_Pause        '暂停
          Control.Visible = CheckPopedom("暂停")
          Control.Enabled = (Not mblnIsSelectedCallingList And intQueueState <> TQueueSelState.qss已暂停 And intQueueState <> -1) Or mblnIsFindQueue Or mblnIsSelectedCallingList
          
        Case conMenu_Queue_Abandon      '弃号           '已呼叫数据可以进行弃号
          Control.Visible = CheckPopedom("弃号")
          Control.Enabled = (Not mblnIsSelectedCallingList And intQueueState <> TQueueSelState.qss已弃号 And intQueueState <> -1) Or mblnIsFindQueue Or mblnIsSelectedCallingList
          
        Case conMenu_Queue_Restore      '恢复
          Control.Visible = CheckPopedom("恢复")
          Control.Enabled = mblnIsSelectedCallingList Or mblnIsFindQueue Or _
            (intQueueState <> TQueueSelState.qss排队中 And intQueueState <> -1 And Not mblnIsSelectedCallingList And intQueueState <> TQueueSelState.qss待预呼 And intQueueState <> TQueueSelState.qss已预呼)
...
'判断数据是否处于排队中
    ...
    CheckIsQueueing = IIf(lngCurQueueState = qss排队中 Or lngCurQueueState = qss待预呼 Or lngCurQueueState = qss已预呼, True, False)
End Function
...
'获取对应值所在的行
    ...
    If lngQueueFromType = qftFindQueue Or lngQueueFromType = qftWaitQueue Then
        GetRowIndex = GetWillRowIndex(strColName, strValue, rptQueueList)

    Else
        GetRowIndex = GetWillRowIndex(strColName, strValue, rptCallList)
...
'    End If
    ...
    '执行呼叫处理
    If mobjQueueManage.SpecifiedCall(lngQueueId, strCallContext, cwOrder) <= 0 Then Exit Sub
...
'    End If
    ...
    '执行呼叫处理
    If mobjQueueManage.SpecifiedCall(lngQueueId, strCallContext, cwSpecify) <= 0 Then Exit Sub
...
'    If DoCallPreBefore(lngQueueId, TCallWay.cwBroadcast, strCallContext) = False Then Exit Sub
    ...
    If Not mobjQueueManage.WaitRoomCall(lngQueueId, strCallContext) Then Exit Sub
    
    Call DoCallPreAfter(lngQueueId, TCallWay.cwWaitRoom)
...
Public Sub comMenu_快捷()
On Error GoTo errHandle
    If mfrmQueueQuick Is Nothing Then
        Set mfrmQueueQuick = New frmQueueQuick
        mfrmQueueQuick.ShowQueueQuick Me, rptQueueList, rptCallList, mstrQueryQueueNames
    End If
    Exit Sub
errHandle:
    If ErrCenter = 1 Then Resume
...
'    mdtLastVoiceDate = Timer
    ...
    Else
        ...
        If lngRowIndex >= 0 Then
            If GetListValue(qftCalledQueue, lngRowIndex, "排队状态") <> "接诊中" Then
...
Public Sub CloseQueueQuick()
    If Not mfrmQueueQuick Is Nothing Then
        mfrmQueueQuick.UnloadMe
    End If
End Sub

Public Sub OpenQueueQuick(objOwer As Object)
    If mfrmQueueQuick Is Nothing Then
        Set mfrmQueueQuick = New frmQueueQuick
        mfrmQueueQuick.ShowQueueQuick objOwer, rptQueueList, rptCallList, mstrQueryQueueNames
    End If
End Sub
