...
Public Function GetWillRowIndex(ByVal strColName As String, ByVal strValue As String, objList As ReportControl) As Long
'获取对应值所在的行
    ...
    For i = 0 To objList.Rows.Count - 1
        If objList.Rows(i).GroupRow = False Then
            If objList.Rows(i).Record(lngColIndex).value = strValue Then
                GetWillRowIndex = objList.Rows(i).Index
                Exit Function
            End If
        End If
    Next i
