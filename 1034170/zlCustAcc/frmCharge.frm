...
Public Sub MainProc()
    ...
    Else '新增
        ...
        If mstrInNO <> "" Then '修改单据  如果是在后备表中，不会执行到这里，在前面已退出
            Call ImportBill(mstrInNO, mlngRows)
            If mobjBill.NO = "" Then
            ...
            Else
                ...
                mblnDo = False
                    cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, lngPre)
                    ...
                    Call FillDoctor(i)
                    cbo开单人.ListIndex = GetCboIndex(cbo开单人, strPre)
...
'功能：显示病人信息
    With mobjBill
        txtPatient.Text = .姓名
        cbo性别.ListIndex = GetCboIndex(cbo性别, .性别)
        txt年龄.Text = .年龄
        cbo费别.ListIndex = GetCboIndex(cbo费别, .费别)
...
'注意：如果开单科室的Tag属性有设置的话，还要进行相应的处理
    If cbo开单科室.Tag <> "" Then
        Select Case cbo开单科室.Tag
            Case "C1" '病人所有科室
                cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, IIf(mobjBill.科室ID = 0, lngID, mobjBill.科室ID))
            Case "C2" '操作员所在科室
                cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, IIf(mlngDeptID = 0, UserInfo.部门ID, mlngDeptID))
            Case Else '指定科室
                cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, Val(cbo开单科室.Tag))
    ...
    Else
        cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, lngID)
    End If
...
Private Sub cbo收费类别_KeyPress(Index As Integer, KeyAscii As Integer)
    ...
    lngIdx = MatchIndex(cbo收费类别(Index).hwnd, KeyAscii)
    ...
    ElseIf mcolDetails.Count = 1 Then
        ...
        If mrsInfo.State = 1 And mbytUseType <> Use门诊 Then
            If Not IsNull(mrsInfo!险类) And MCPAR.实时监控 Then
                If gclsInsure.CheckItem(mrsInfo!险类, 1, 0, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), Index)) = False Then
                    ClearDetail Index '删除刚刚想要加入的费用行
                    txt收费项目(Index).Text = ""
                    Exit Function
                End If
...
Private Sub cmd细目选择_Click(Index As Integer)
    ...
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, str类别, , , str特准项目)
    Me.Refresh
...
'      4.如果变价细目已经计算出收入项目(按第2步),并手动更改(也可能未改)了单价,则按该单价计算。
    ...
    strSQL = _
        ...
        "   And A.ID=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mobjBill.Details("R" & lngRow).收费细目ID)

    If Not rsTmp.EOF Then
        If mobjBill.Details("R" & lngRow).Detail.变价 Then
            If mobjBill.Details("R" & lngRow).InComes.Count = 0 Then
                '第一次计算金额取缺省值
                curMoney = IIf(IsNull(rsTmp!缺省价格), 0, rsTmp!缺省价格)
            Else '获取操作员以前输入的变价金额
                curMoney = mobjBill.Details("R" & lngRow).InComes(1).标准单价
                '如果用户输入的变价不满足变价范围，则取缺省值
                If Abs(curMoney) > Abs(IIf(IsNull(rsTmp!现价), 0, rsTmp!现价)) Then
                    curMoney = IIf(IsNull(rsTmp!缺省价格), 0, rsTmp!缺省价格)
                End If
            End If
        End If
    ...
    With objBillDetail
        ...
        If mbytInState = sta执行 Then
            ...
            If mrsUnit.RecordCount <> 0 Then
                'byZT200302
                cbo执行科室(lngRow).ListIndex = GetCboIndex(cbo执行科室(lngRow), mrsUnit!名称)
            ...
            Else
                ...
                If strTmp <> "" Then
                    cbo执行科室(lngRow).ListIndex = GetCboIndex(cbo执行科室(lngRow), strTmp)
                    If cbo执行科室(lngRow).ListIndex = -1 Then
...
Private Function GetDetails(ByVal lngHwnd As Long, ByVal str输入 As String, Optional ByVal str类别 As String)
    ...
    If str类别 <> "" Then str类别 = "'" & str类别 & "'"
    lng项目id = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, str类别, str输入, lngHwnd, str特准项目)
    If lng项目id <> 0 Then
        Call GetInputDetail(lng项目id)
...
Private Sub ImportBill(strNo As String, ByVal Rows As Integer)
...
'说明：因为可能现时项目价格信息已作调整,所以费用相关内容重新计算
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strNo, mlng记帐ID)
...
'参数：strFullNo=单据号
    ...
    mblnDo = False
        ...
        '科室确定医生
        cbo开单科室.ListIndex = FindCboIndex(cbo开单科室, Nvl(rsTmp!开单部门ID, 0))
        ...
        End If
        cbo开单人.ListIndex = GetCboIndex(cbo开单人, Nvl(rsTmp!开单人))
    ...
    For i = 0 To mlngRows - 1
        ...
        If Not rsTmp.EOF Then
            If rsTmp("序号") = i + 1 Then
                ...
                strTmp = rsTmp("执行部门")
                If strTmp <> "" Then
                    cbo执行科室(i).ListIndex = GetCboIndex(cbo执行科室(i), strTmp)
        ...
        If blnDeal = False And Val(txt收费项目(i).Tag) <= 0 Then
            cbo收费类别(i).ListIndex = FindCboIndex(cbo收费类别(i), Val(cbo收费类别(i).Tag))
            txt收费项目(i).Text = ""
            txt计算单位(i).Text = ""
            txt数次(i).Text = ""
            txt标准单价(i).Text = ""
