...
Public Function PrintOrPreviewBodyStateNew(objOut As Object, _
    ...
    Dim lngCur60Y As Long '脉搏60的Y坐标
...
ErrNext:
        ...
        If mbln呼吸曲线 = False Then
            rsDrawItems.Filter = "项目序号=" & gint脉搏 & " Or 项目序号=" & gint心率
            rsDrawItems.Sort = "项目序号 DESC"
            If rsDrawItems.RecordCount > 0 Then
                lngCur60Y = GetYCoordinate(objDraw, rsDrawItems, Val(rsDrawItems!项目序号), 60)
                If Not (lngCur60Y >= T_DrawClient.体温区域.Top And lngCur60Y <= T_DrawClient.体温区域.Bottom) Then
                    lngCur60Y = -999
                End If
            End If
        End If
        ...
        lngFont = CreateFontIndirect(T_Font)
        lngOldFont = SelectObject(lngDC, lngFont)
        Call DrawBodyRecordItemNew(lngDC, objDraw, ArrNewString, rsItems, lngCurX, T_DrawClient.体温区域.Bottom, T_DrawClient.体温区域.Right, intRepairRows, lngCurY, lngCur60Y, sngScale)
        Call SelectObject(lngDC, lngOldFont)
        Call DeleteObject(lngFont)
        Call ReleaseFontIndirect(objDraw)
...
'-------------------------------------------------------------------------------------------------------
    ...
    For lngRow = 0 To T_DrawClient.总列数 - 1
        ...
        If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
            Call DrawLine(lngDC, lngCurX, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0, PS_SOLID, intLineMode), IIf(lngRow Mod 5 = 0 And sinRowStep >= 4 And bln粗线, intBold, 1), RGB_BLACK)
        End If
    Next
...
'                    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Format(sin刻度, "#0"), zlCommFun.Nvl(!记录色, RGB_BLACK))
                ...
                If Val(Format(sin刻度, "#0.00")) <= Val(Format(zlCommFun.Nvl(!最小值), "#0.00")) Or Format(lngCurY, "#0") > T_DrawClient.刻度区域.Bottom Then
                    ...
                    If blnDoubleRow = False And (sinAlertness < Val(Nvl(!最大值)) And sinAlertness > Val(Nvl(!最小值))) Then
                        ...
                        Call DrawLine(lngDC, T_DrawClient.体温区域.Left, lngCurAlerY, lngMaxX, lngCurAlerY, PS_SOLID, intBold, RGB_RED)
                    End If
        ...
        Do While Not .EOF
            ...
            If lngCurveRows > 0 Then
                ...
                For lngRow = 1 To lngCurveRows
                    ...
                    End If
                    If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
                        Call DrawLine(lngDC, lngCurX + 1, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0, PS_SOLID, intLineMode), IIf(lngRow Mod 5 = 0 And sinRowStep >= 4 And bln粗线, intBold, 1), RGB_BLACK)
                    End If
                Next
                '画底线
...
Public Sub DrawBodyRecordItemNew(ByVal lngDC As Long, ByVal objDraw As Object, strValue() As String, ByVal rsItems As ADODB.Recordset, ByVal lngX As Long, ByVal lngY As Long, _
    ByVal lngLeft As Long, ByVal intRepairRows As Integer, lngOutY As Long, Optional ByVal lngCur60Y As Long = -999, Optional sngScale As Single = 1)
'-----------------------------------------------------------------------------------------------------------------------
'输出病人基本信息
...
'    lngCur60Y 四川地区要求呼吸机输出在脉搏60刻度下，此变量为脉搏60度的Y坐标(lngCur60Y<>-999说明呼吸机显示在脉搏60刻度下)
'出参:lngOutY 返回绘图后的上边距
'-----------------------------------------------------------------------------------------------------------------------
                    ...
                    For intCOl = 0 To UBound(arrTmpString1)
                        ...
                        Else
                            ...
                            If IsNumeric(strTmp) Then
                                ...
                                Else
                                    strPic = "BREATH"
                                    If Not lngCur60Y = -999 Then Call SetTextColor(lngDC, lngColor)
                                End If
                                    ...
                                    If intCOl Mod 2 = intValue Then
                                        ...
                                        Else
                                            ...
                                            Else
                                                ...
                                                Call DrawPicture(objDraw, strPic, objDraw.ScaleX(lngX1 + ((T_DrawClient.列单位 - mintBmpW * IIf(blnPrinter = True, msngTwips, 1)) / 2), vbPixels, vbTwips), _
                                                    objDraw.ScaleY(lngCur60Y + 1, vbPixels, vbTwips), _
                                                    objDraw.ScaleX(lngX1 + ((T_DrawClient.列单位 - mintBmpW * IIf(blnPrinter = True, msngTwips, 1)) / 2) + mintBmpW * IIf(blnPrinter = True, msngTwips, 1), vbPixels, vbTwips), _
                                                    objDraw.ScaleY(lngCur60Y + 1 + mintBmpH * IIf(blnPrinter = True, msngTwips, 1), vbPixels, vbTwips), True)
                                            End If
                                        End If
                                    Else
                                        If strPic = "" Then
