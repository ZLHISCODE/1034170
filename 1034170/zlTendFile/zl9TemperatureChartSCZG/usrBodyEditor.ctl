   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Begin VSFlex8Ctl.VSFlexGrid mshDownTab 
            ...
            AccessibleRole  =   24
         End
         Begin zl9TemperatureChartSCZG.VsfGrid vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   28
            Top             =   3480
...
Private Function InitBody(ByVal lng文件ID As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal int婴儿 As Integer) As Boolean
    ...
    '读取此病人的体温单总页数
    '------------------------------------------------------------------------------------------------------------------
    strSQL = " SELECT /*+ RULE */ 入院时间,实际入院时间,出院时间,1 + TRUNC((TO_DATE(TO_CHAR(出院时间,'yyyy-MM-dd'),'yyyy-MM-dd') -TO_DATE(TO_CHAR(入院时间,'yyyy-MM-dd'),'yyyy-MM-dd')) / " & T_BodyStyle.lng天数 & ") AS 页数,发生时间,记录 " & _
             "  From (" & _
    ...
    With mshUpTab
        ...
        Do While Not rsTmp.EOF
            ...
            If Not rsTmp.EOF Then
                If bln术后显示 And DateDiff("d", CDate(Format(str时间, "YYYY-MM-DD")), str结束时间) < 14 Then
                    str结束时间 = Format(DateAdd("D", lng天数 - 1, CDate(mstr开始时间)), "YYYY-MM-DD") & " 23:59:59"
                End If
            End If
...
Private Function ShowDowntab() As Boolean
    ...
    Dim blnRespiratorShow As Boolean '呼吸机图形是否输出在脉搏60刻度下
    Dim lngCurX As Long, lngCurY As Long '呼吸机图形输出在脉搏60刻度下的坐标
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "提取体温表格数据", T_Patient.lng文件ID, T_Patient.lng病人ID, T_Patient.lng主页ID, _
                    CDate(dtBegin), CDate(dtEnd), strItems, T_Patient.lng婴儿, T_Patient.lng护理等级, IIf(T_Patient.lng婴儿 = 0, 1, 2), T_Patient.lng科室ID, T_BodyItem.str表格内容)
...
ErrNext:
    ...
    int呼吸位置 = 0
    blnValue = False
    blnRespiratorShow = False
    '四川自贡地区要求,呼吸机的次数显示在表格，符号显示在对应兰脉搏60刻度下
    '满足条件：呼吸为表格、体温单存在脉搏和呼吸，并且脉搏60刻度的坐标在曲线区域内
    If mbln呼吸曲线 = False Then
        mrsDrawItems.Filter = "项目序号=" & gint脉搏 & " Or 项目序号=" & gint心率
        mrsDrawItems.Sort = "项目序号 DESC"
        If mrsDrawItems.RecordCount > 0 Then
            lngItemCode = Val(mrsDrawItems!项目序号)
            lngCurY = GetYCoordinate(mobjDraw, mrsDrawItems, lngItemCode, 60)
            If lngCurY >= T_DrawClient.刻度区域.Top And lngCurY <= T_DrawClient.刻度区域.Bottom Then
                blnRespiratorShow = True
            End If
            'Y坐标+1，避免图形与脉搏60刻度曲线重叠
            lngCurY = lngCurY + 1
        End If
    End If
    
    If blnRespiratorShow = True Then
        ...
        If IsNumeric(str结果) Then
            ...
            If int呼吸表格输出格式 = 0 Then '顺序上下显示
                If intCOl Mod 2 = intValue Then
                    ...
                    Else
                        If vsf.Body.OwnerDraw <> flexODNone Then
                            vsf.ColData(intCOl) = 1
                        Else
                            vsf.ColData(intCOl) = 0
                            lngCurX = ((intCOl - vsf.FixedCols) * T_DrawClient.列单位) + T_DrawClient.体温区域.Left
                            If T_DrawClient.列单位 - gintBmpW > 0 Then
                                lngCurX = lngCurX + ((T_DrawClient.列单位 - gintBmpW) / 2)
                            End If
                            Call DrawMarker(False, gint呼吸, "呼吸机", lngCurX, lngCurY)
                        End If
                    End If
                Else
        ...
        If mbln显示皮试 = True And mintRepairRows > Val(mshDownTab.Tag) And mintRepairRows > 0 Then
            strSQL = _
               "SELECT 时间,F_LIST2STR(CAST(COLLECT(药物名) AS T_STRLIST)) 药物名 FROM (" & vbNewLine & _
                ...
                "   AND A.开始执行时间  BETWEEN [4] AND [5] AND A.诊疗项目id=b.id(+) ORDER BY A.开始执行时间,A.皮试结果)" & vbNewLine & _
                " GROUP BY 时间"
...
Public Sub Paint_Assistant()
    ...
    Call OutPutTextNew(mobjDraw, mrsDrawItems, mlngMemDC, mrsNote, mstr开始时间)
    Exit Sub
Errhand:
    If ErrCenter = 1 Then
        Resume
...
Public Function PrintState(ByVal intPrintRange As Integer, ByVal blnPrint As Boolean, Optional lngBeginY As Long, _
    ...
    '读取此病人的体温单总页数
    '------------------------------------------------------------------------------------------------------------------
    strSQL = " SELECT /*+ RULE */ 入院时间,出院时间,1 + TRUNC((TO_DATE(TO_CHAR(出院时间,'yyyy-MM-dd'),'yyyy-MM-dd') -TO_DATE(TO_CHAR(入院时间,'yyyy-MM-dd'),'yyyy-MM-dd')) / " & T_BodyStyle.lng天数 & ") AS 页数,发生时间 " & _
            "  From (" & _
                " SELECT DECODE(D.开始时间,NULL,DECODE(C.出生时间,NULL," & IIf(strMarkDate = "", "B.入院时间", strMarkDate) & ",C.出生时间)," & vbNewLine & _
                "               DECODE(SIGN(D.开始时间 - DECODE(C.出生时间,NULL," & IIf(strMarkDate = "", "B.入院时间", strMarkDate) & ",C.出生时间))," & vbNewLine & _
                "                      1," & vbNewLine & _
