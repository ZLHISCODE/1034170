...
Private mblnReadOnly As Boolean                         '文件是否已经提交到病案室审查
...
Public Event EprReadOnly(ByVal blnReadOnly As Boolean)
...
Public Sub zlExecuteCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_FileMan
        If mblnReadOnly = True Then
            MsgBox "该病人病案已提交审查，不能修改文件，请取消审查后再试！", vbInformation, gstrSysName
            Exit Sub
        End If
...
'        End With
    ...
    Case conMenu_Edit_UnArchive
        If mblnReadOnly = True Then
            MsgBox "该病人病案已提交审查，不能取消归档，请取消审查后再试！", vbInformation, gstrSysName
            Exit Sub
        End If
    ...
    Case conMenu_File_PrintDayDetail, conMenu_Edit_Curve, conMenu_Edit_CurveTable, conMenu_Edit_Curve_Show '批量录入,设置记录,显示
...
Public Sub zlUpdateCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_Curve, conMenu_Edit_CurveTable, conMenu_Edit_Curve_Show  '设置记录和显示
    ...
    Case conMenu_Tool_Option '护理选项
        ...
        End If
        Control.Enabled = (mlngPatiID > 0) And (Val(vfgFile.TextMatrix(vfgFile.ROW, mCol.fID)) > 0)
    End Select
...
'        Call FormSetCaption(mclsDockAduits.zlGetFormTendBody, False, False)
    ...
    Case "刷新数据"
        ...
        If mlngPatiID <> 0 Then
            ...
            mblnReadOnly = (Split(EprIsCommit, "|")(0) = 0)
        End If
        
        RaiseEvent EprReadOnly(mblnReadOnly)
    ...
    Case "显示文件内容"
        ...
        If Not mblnDoctorStation And mblnEdit = True Then
            If Val(vfgFile.TextMatrix(vfgFile.ROW, mCol.fID)) = 0 And InStr(1, mstrPrivs, "护理文件管理") > 0 And strPatient <> mlngPatiID & "|" & mlngPageId & "|" & mintBaby Then
                '病人病案已提交审查,不能添加文件
                If mblnReadOnly = True Then Exit Function
        ...
        Case conMenu_Delete
            ...
            If Val(vfgFile.TextMatrix(Me.vfgFile.ROW, mCol.f保留)) = -1 Then
                gstrSQL = "SELECT A.ID,B.开始时间" & _
                    " FROM 病人护理数据 A,病人护理文件 B" & _
                    " WHERE B.ID=[1] And A.文件ID=B.ID And RowNum<2"
            Else
                gstrSQL = "SELECT A.ID,B.开始时间" & _
                    " FROM 病人护理数据 A,病人护理打印 C,病人护理文件 B" & _
                    " WHERE B.ID=[1] And A.文件ID=B.ID and A.文件ID=C.文件ID And A.ID=C.记录ID And RowNum<2"
            End If
            Call SQLDIY(gstrSQL)
...
Private Sub cbsMain_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    Select Case Control.ID
        Case conMenu_Add, conMenu_Modify, conMenu_Delete
            Control.Visible = (InStr(1, mstrPrivs, "护理文件管理") > 0 And mblnDoctorStation = False And Not gblnMoved)
            Control.Enabled = (mlngPatiID > 0) And Not mblnArchive And Control.Visible And mblnEdit And Not mblnReadOnly
