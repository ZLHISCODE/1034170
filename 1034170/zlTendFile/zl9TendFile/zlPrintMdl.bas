...
'    On Error GoTo errHand
    ...
    If bytCollType = 2 Or bytCollType = 4 Then
        If InStr(1, "|" & frmTendFileReader.GetCollectCols & ";", "|" & COL - (glngHideCols + objGrid.FixedCols - 1) & ";") = 0 Then
            bytCollType = 0
        End If
    End If
    
    If bytCollType = 1 Then
        lngRowCount = Val(objGrid.TextMatrix(ROW, frmTendFileReader.GetFixedCol("行数")))
        ...
        If lngRowCount > 1 Then
            If lngRowCount = lngRowCurrent Then '不跨页的情况
                If FormatValue(objGrid.TextMatrix(ROW, frmTendFileReader.GetFixedCol("行数"))) = lngRowCount & "|1" Then
                    bytCollType = 3
                ElseIf FormatValue(objGrid.TextMatrix(ROW, frmTendFileReader.GetFixedCol("行数"))) = lngRowCount & "|" & lngRowCount Then
                    bytCollType = 4
                Else
                    bytCollType = 0
                End If
            Else '跨页的情况
                If lngRowCurrent > 1 Then
                    ...
                    ElseIf FormatValue(objGrid.TextMatrix(ROW, frmTendFileReader.GetFixedCol("行数"))) = lngRowCount & "|" & lngRowCount Then
                        bytCollType = 4
                    ElseIf FormatValue(objGrid.TextMatrix(ROW, frmTendFileReader.GetFixedCol("行数"))) = lngRowCount & "|" & lngRowCurrent Then
                        bytCollType = 4
                    ElseIf FormatValue(objGrid.TextMatrix(ROW, frmTendFileReader.GetFixedCol("行数"))) = lngRowCount & "|" & (lngRowCount - lngRowCurrent) + 1 Then
                        bytCollType = 3
                    Else
                        bytCollType = 0
                    End If
