...
Private mlngStartSpread As Long             '判断数据是否在打印起始页首行跨页：1-是，其它-否
...
Private Function ReadStruDef() As Boolean
    ...
    gstrSQL = "Select   d.对象序号, d.对象属性, d.内容行次, d.内容文本, upper(d.要素名称) AS 要素名称, d.要素单位,d.要素表示 " & _
        " From 病历文件结构 d, 病历文件结构 p" & _
...
''                    End If
        ...
        mstrSQL中 = UCase(mstrSQL中 & ",MAX(签名级别) AS 签名级别,MAX(签名信息) AS 签名信息,MAX(交班签名人) AS 交班签名人,MAX(文件ID) AS 文件ID,MAX(记录ID) AS 记录ID,MAX(行数) AS 行数,MAX(实际行数) AS 实际行数,MAX(开始行号) AS 开始行号,MAX(打印结束页号) as 打印结束页号,MAX(打印标识) AS 打印标识,MAX(汇总类别) AS 汇总类别,MAX(汇总文本) AS 汇总文本,MAX(汇总标记) AS 汇总标记,MAX(汇总日期) AS 汇总日期,MAX(打印页号) AS 打印页号,MAX(打印行号) AS 打印行号")
        mstrSQL内 = UCase(mstrSQL内 & ",l.签名级别,l.签名人 AS 签名信息,l.交班签名人,l.文件ID,C.记录ID,P.行数||'' AS 行数,DECODE(SIGN(P.结束页号-P.开始页号),1,DECODE(SIGN([5]-P.开始页号),1, P.结束行号,P.行数-P.结束行号 ),P.行数) AS 实际行数,DECODE(SIGN(P.结束页号-P.开始页号),1,DECODE(SIGN([5]-P.开始页号),1,P.开始行号+P.行数-P.结束行号,P.开始行号),P.开始行号) 开始行号,P.打印结束页号,P.打印标识,NVL(L.汇总类别,0) AS 汇总类别,L.汇总文本,L.汇总标记,to_char(L.发生时间,'yyyy-MM-dd hh24:mi:ss')||'' AS 汇总日期,p.打印页号,p.打印行号")
        mstrSQL列 = UCase(mstrSQL列 & ",签名级别,签名信息,交班签名人,文件ID,记录ID,行数,实际行数,开始行号,打印结束页号,打印标识,汇总类别,汇总文本,汇总标记,汇总日期,打印页号,打印行号")
...
Private Sub InitRecords()
    ...
    If Not mblnInit Then
        ...
        mlngPrintedTag = mlngPrintedEndPage + 1 '56134:刘鹏飞,2012-12-19
...
Public Function ShowMe(ByVal frmParent As Form, ByVal lngFileID As Long, ByVal lngPatiID As Long, ByVal lngPageId As Long, ByVal intBaby As Integer, Optional ByVal strPages As String = "") As Boolean
    ...
    gstrSQL = "Select A.id" & vbNewLine & _
        ...
        " Where  A.病人id = [1] And A.主页id = [2] And Nvl(A.婴儿, 0) = [3]" & vbNewLine & _
        " Start With A.续打id = [4]" & vbNewLine & _
        " Connect By Prior A.Id = A.续打id" & vbNewLine & _
        " Order By Level Desc"
