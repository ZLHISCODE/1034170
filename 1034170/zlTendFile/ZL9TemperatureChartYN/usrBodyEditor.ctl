   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Begin VSFlex8Ctl.VSFlexGrid mshDownTab 
            ...
            AccessibleRole  =   24
         End
         Begin zl9TemperatureChartYN.VsfGrid vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   28
            Top             =   3480
...
Private Sub RaiseShowTipInfo(ByVal vfgObj As Object, ByVal intType As Byte, ByVal X As Single, ByVal Y As Single)
    ...
    For i = 0 To vfgObj.Rows - 1
        If mshDownTab.RowHidden(i) = False Then
    ...
    With rsTemp
        Do While Not .EOF
            ...
            If lngCurveRows > 0 Then
                ...
                '完成刻度区域的绘制
                Call DrawLine(mlngMemDC, lngCurX, lngCurY, lngCurX, lngMaxY, PS_SOLID, 1, RGB_BLACK)
                Call DrawLine(mlngMemDC, lngCurX + Fix(lng刻度宽度 / 2), lngCurY, lngCurX + Fix(lng刻度宽度 / 2), lngMaxY, PS_SOLID, 1, RGB_BLACK)
                ...
                For lngRow = 1 To lngCurveRows
                    ...
                    If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
                        If blnAche Then
                            Call DrawLine(mlngMemDC, lngCurX - lng刻度宽度 + Fix(lng刻度宽度 / 2) + 1, lngCurY, lngCurX, lngCurY, PS_DOT, 1, RGB_FleetGRAY)
                        End If
                        Call DrawLine(mlngMemDC, lngCurX + 1, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0 And blnAche = False, PS_SOLID, intLineMode), IIf(lngRow Mod 5 = 0 And sigRowStepNew >= 4 And bln粗线 And blnAche = False, 2, 1), RGB_FleetGRAY)
                    End If
                Next
                lngCurY = lngY
...
'                        lngCurX = lngCurX - (mobjDraw.ScaleY(mobjDraw.TextHeight("1"), vbTwips, vbPixels) \ 3)
                        lngCurX = T_DrawClient.体温区域.Left - Fix(lng刻度宽度 / 4) - mobjDraw.ScaleX(mobjDraw.TextWidth(Val(Format(sin刻度, "#0.0"))) / 2, vbTwips, vbPixels)
