   ...
   Begin VB.PictureBox picTab 
      ...
      Begin VB.Frame FraTable 
         ...
         Begin VB.PictureBox picColor 
            ...
            Visible         =   0   'False
            Width           =   1575
            Begin zl9TemperatureChart.ColorPicker usrColor 
               Height          =   2190
               Left            =   120
               TabIndex        =   31
               Top             =   -450
   ...
   Begin VB.PictureBox picCurve 
      ...
      Begin VB.Frame FraData 
         ...
         Begin VB.PictureBox picValue 
            ...
            Visible         =   0   'False
            Width           =   1575
            Begin zl9TemperatureChart.ColorPicker usrValue 
               Height          =   2190
               Left            =   120
               TabIndex        =   43
               Top             =   -360
         ...
         Begin VB.PictureBox pic未记 
            ...
            End
         End
         Begin zl9TemperatureChart.VsfGrid vsfCurve 
            Height          =   1485
            Left            =   120
            TabIndex        =   10
            Top             =   225
        ...
        Case conMenu_Edit_Append * 10, conMenu_Edit_Append * 10 + 1, conMenu_Edit_Append * 10 + 2, conMenu_Edit_Append * 10 + 3, conMenu_Edit_Append * 10 + 4, conMenu_Edit_Append * 10 + 5, conMenu_Edit_Append * 10 + 6
            ...
            Else '不存在记录就新增数据
                If Trim(strValue) <> "" Then
                    ...
                    gstrValues = GetMaxID(mrsCurve) & "|2)体温表格项目|" & strValue & "|" & strPart & "|" & _
        ...
        Do While Not .EOF
            For intRow = vsfCurve.FixedRows To vsfCurve.Rows - 1
                ...
                If !分组名 = vsfCurve.TextMatrix(intRow, COL_分组名) And !项目序号 = lngNO Then
                    ...
                    End If
                    If lngNO = gint体温 And (IsNumeric(zlCommFun.Nvl(!数值)) Or zlCommFun.Nvl(!数值) <> "不升") Then
                        vsfCurve.TextMatrix(intRow, COL_复查) = IIf(Val(zlCommFun.Nvl(!复查)) = 1, "√", "")
                    End If
                    lngColor = 255
                    If Val(zlCommFun.Nvl(!数据来源)) <> 0 Then
                        If zlCommFun.Nvl(!数值) = "不升" And lngNO = gint体温 Then
                            lngColor = 255
                        ElseIf lngNO = gint体温 Or lngNO = gint疼痛强度 Or (lngNO = 2 And mint心率应用 = 2 And mblnEdit心率 = True) Then
...
Private Function GetTableRowName() As String
    ...
    If mblnTemType = False Then
        mstrSQL = " Select A.排列序号,A.记录名 项目名,A.项目序号 as 项目号,A.记录法,A.入院首测," & _
                ...
                " Where c.项目ID=B.ID(+) And A.项目序号=C.项目序号 And (A.记录法<>2 OR (A.记录法=2 And A.项目序号=3)) And Nvl(C.应用方式,0)=1 AND C.护理等级>=[1] And Nvl(C.适用病人,0) In (0,[3]) " & _
    ...
    Else '专科体温单项目提取
        ...
        mstrSQL = " Select /*+ RULE */ A.排列序号,A.记录名 项目名,A.项目序号 as 项目号,A.记录法,A.入院首测," & _
                " C.项目值域,C.项目类型,C.项目长度,C.项目小数,nvl(A.记录频次,2) 记录频次,C.分组名,C.项目表示,C.项目单位 " & _
    ...
    strEndTime = DateAdd("d", T_BodyStyle.lng监测次数, CDate(Format(Format(mstrDate, "YYYY-MM-DD") & " 23:59:59", "YYYY-MM-DD HH:mm:ss")))
    If strEndTime > mstrETime Then strEndTime = mstrETime
    '提取固定表格项目和有数值的活动项目信息
    If Not mblnTemType Then
    ...
    With rsTemp
        Do While Not .EOF
           str值域 = Replace(zlCommFun.Nvl(!项目值域), ":", "")
            ...
            Titem.入院首测 = Val(zlCommFun.Nvl(!入院首测, 0))
            Titem.记录名 = IIf(Titem.项目号 = 4, "血压", zlCommFun.Nvl(!记录名))
...
'-----------------------------------------------------------------------------------------------------------------
    ...
    If blnCurve = True Then
        ...
        Else '专科体温单
            ...
            End If
            Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, Me.Caption, mT_Patient.lng文件ID, mT_Patient.lng病人ID, mT_Patient.lng主页ID, mT_Patient.lng婴儿, _
                 CDate(Format(mstrBegin, "YYYY-MM-DD HH:mm:ss")), CDate(Format(mstrEnd, "YYYY-MM-DD HH:mm:ss")), mstrCurveItem)
        End If
        With rsTmp
            Do While Not .EOF
                '添加记录集
                ...
                If (lng项目序号 = gint体温 Or lng项目序号 = gint疼痛强度) And int标记 = 1 Then
                    mrsCurve.Filter = "项目序号=" & lng项目序号 & " and 时间='" & Format(zlCommFun.Nvl(!时间), "YYYY-MM-DD HH:mm:ss") & "' and 标记<>1"
                    If mrsCurve.RecordCount > 0 Then
                        If InStr(1, ",0,9,", "," & Val(zlCommFun.Nvl(mrsCurve!数据来源)) & ",") = 0 And InStr(1, ",0,9,", "," & Val(zlCommFun.Nvl(!数据来源)) & ",") = 0 Then
                            intModify = 1
                        Else
                    ...
                    For intRow = vsfCurve.FixedRows To vsfCurve.Rows - 1
                        ...
                        If !分组名 = vsfCurve.TextMatrix(intRow, COL_分组名) And !项目序号 = lng项目序号 Then
                            ...
                            Else
                                ...
                                vsfCurve.TextMatrix(intRow, Col_未记说明) = ""
                            End If
                            If lng项目序号 = gint体温 And (IsNumeric(zlCommFun.Nvl(!数值)) Or zlCommFun.Nvl(!数值) <> "不升") Then
                                vsfCurve.TextMatrix(intRow, COL_复查) = IIf(Val(zlCommFun.Nvl(!复查)) = 1, "√", "")
                            End If
                            lngColor = 255
                            If InStr(1, ",0,9,", Val(zlCommFun.Nvl(!数据来源))) = 0 Then
                                If zlCommFun.Nvl(!数值) = "不升" And lng项目序号 = gint体温 Then
                                    lngColor = 255
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, "读取上下标等信息", mT_Patient.lng文件ID, mT_Patient.lng病人ID, mT_Patient.lng主页ID, _
            mT_Patient.lng婴儿, CDate(mstrBegin), CDate(mstrEnd))
        With rsTmp
            Do While Not .EOF
    ...
    If blnTab = True Then
        ...
        If vsfTab.Tag <> "NO" Then
            ...
            Set rsTmp = zlDatabase.OpenSQLRecord(mstrSQL, Me.Caption, _
                                                ...
                                                CDate(dtBegin), _
                                                CDate(dtEnd), _
                                                strItems, mT_Patient.lng婴儿, mT_Patient.lng护理等级, IIf(mT_Patient.lng婴儿 = 0, 1, 2), mT_Patient.lng科室ID, T_BodyItem.str表格内容)
        ...
        If NewCol = COL_数据 Or NewCol = Col_未记说明 Then
            ...
            If InStr(1, ",0,9,", "," & Val(marrDate(NewRow)) & ",") = 0 Then
                If NewCol = COL_数据 Then
                    If lng项目序号 = gint体温 And strDate = "不升" Then GoTo NotEdit
...
'选择未记说明
    ...
    Select Case Col
        Case Col_未记说明
            ...
            mrsCurInfo.Filter = "": mrsCurInfo.Sort = "编码"
            lst未记.Clear
            If mrsCurInfo.RecordCount > 0 Then
                i = 0
                With mrsCurInfo
                    Do While Not .EOF
...
Private Sub vsfCurve_KeyPressEdit(ByVal Row As Long, ByVal Col As Long, KeyAscii As Integer)
    ...
    strDate = vsfCurve.TextMatrix(Row, COL_项目名)
...
CheckPoint:
    If Trim(vsfCurve.EditText) <> "" And str值域 <> "" Then
        ...
        If lngNO <> 1 And lngNO <> gint疼痛强度 And Not (lngNO = 2 And mint心率应用 = 2 And mblnEdit心率 = True) Then
        ...
        For i = 0 To lngCount
            ...
            If i = 0 Then
                ...
                If InStr(1, IIf(lngNO = gint体温, ",不升,", ""), "," & strText & ",") = 0 Then
                    blnOk = False
                Else
                    blnOk = True
                End If
        ...
        Next i
    End If
    If strInfo <> vsfCurve.EditText Then vsfCurve.EditText = strInfo
    strDate = vsfCurve.EditText
    '对于数据来源<>0,9的 体温,脉搏数据 进行编辑(无物理降温和脉搏短轴可以录入物理降温,脉搏短轴)
    If InStr(1, ",0,9,", "," & Val(marrDate(Row)) & ",") = 0 Then
        If Col = COL_数据 Then
            If lngNO = gint体温 Or lngNO = gint疼痛强度 Or (lngNO = 2 And mint心率应用 = 2 And mblnEdit心率 = True) Then
...
'------------------------------------------------------------------------
    ...
    If Not blnOper Then
        lngNO = Val(vsfCurve.TextMatrix(intRow, COL_项目序号))
        strName = vsfCurve.TextMatrix(intRow, COL_项目名)
       ...
       If lngNO = 2 And mint心率应用 = 2 And mblnEdit心率 = True Then
        ...
        Else '新增数据
            If strValue <> "" Or str未记 <> "" Then
                gstrFields = "序号|分组名|数值|部位|标记|时间|项目序号|项目名称|复查|未记说明|数据来源|修改|显示|来源ID|共用|状态|列号|记录类型"
                gstrValues = GetMaxID(mrsCurve) & "|1)体温曲线项目|" & strValue & "|" & str部位 & "|" & _
    ...
    ElseIf intType = 2 And intCOl = COL_数据 And picValue.Visible = True And picValue.Tag <> vsfCurve.Cell(flexcpBackColor, intRow, COL_颜色) Then
        mblnChage = True
        mblnCurveChange = True
    ElseIf lngNO = gint体温 And intCOl = COL_复查 Then
        mblnChage = True
        mblnCurveChange = True
    ...
    With vsfTab
        ...
        If NewRow >= .FixedRows And NewCol >= .FixedRows Then
            If NewCol < .FixedCols + (Split(.TextMatrix(NewRow, COL_tab字符串), ",")(3)) Then
                mrsCurve.Filter = "项目序号=" & Val(.TextMatrix(NewRow, COL_tab项目序号)) & " and 项目名称='" & .TextMatrix(NewRow, COL_tab项目名) & "'" & _
...
'--------------------------------------------------------
    ...
    With mrsCurve
        Do While Not .EOF
            ...
            If lngItemCode = 4 And zlCommFun.Nvl(!项目名称) = "血压" And Not mrsCurInfo.EOF Then
...
'---------------------------------------------------------------
    ...
    For intRow = vsfTab.FixedRows To vsfTab.Rows - 1
        If Val(vsfTab.TextMatrix(intRow, COL_tab项目序号)) = lngItemNO And vsfTab.TextMatrix(intRow, COL_tab项目名) = strName Then
            Exit For
        End If
    Next intRow
...
ErrHour:
            ...
            Else '不存在记录就新增数据
                If Trim(strText) <> "" Then
                    ...
                    gstrFields = "序号|分组名|数值|部位|标记|时间|项目序号|项目名称|复查|未记说明|数据来源|修改|显示|来源ID|共用|状态|列号|记录类型"
                    gstrValues = GetMaxID(mrsCurve) & "|2)体温表格项目|" & IIf(blnTrue = True, strHour, strText) & "|" & strPart & "|" & _
...
'----------------------------------------------------------------------------------------------------------------
    ...
    Dim lngRow As Long
    Dim arrValue() As String
    Dim lngCount As Long, i As Integer, blnOk As Boolean, strText As String, strTmp As String
    Dim blnAllow As Boolean
    
    strName = vsfTab.TextMatrix(intRow, COL_tab项目名)
    lngRow = intRow - vsfTab.FixedRows + 1
...
Private Sub picToolBarReSize()
    ...
    picToolBar.Left = FraTime.Width - picToolBar.Width - 50
    If picToolBar.Left < picPre.Left + picPre.Width + 50 Then
        picToolBar.Left = picPre.Left + picPre.Width + 50
    End If
