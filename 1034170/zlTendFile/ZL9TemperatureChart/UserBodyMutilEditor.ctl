...
Private mrsTemperatureInfo As New ADODB.Recordset
...
Private mblnSpecialWave As Boolean  '是否是专科体温单
...
Private Sub ReSetFontSize()
    ...
    '移动控件位置
    lblFormat.Left = 60
    cboFormat.Left = lblFormat.Left + lblFormat.Width + TextWidth("刘") / 2
    lblDate.Left = cboFormat.Left + cboFormat.Width + TextWidth("刘")
    ...
    lblEntry.Left = cmdAddUser.Left + cmdAddUser.Width + TextWidth("刘") + 75
    lblEntry.Top = lblPati.Top
    lblEntry.Height = cmdAddUser.Height
    optLevel(0).Left = lblEntry.Left + lblEntry.Width + TextWidth("刘")
    optLevel(0).Top = cmdAddUser.Top + 10
    optLevel(0).Height = cmdAddUser.Height
    optLevel(1).Left = optLevel(0).Left + optLevel(0).Width + TextWidth("刘") + 75
    optLevel(1).Height = optLevel(0).Height
    optLevel(1).Top = optLevel(0).Top
    pic过滤条件.Width = optLevel(1).Left + optLevel(1).Width + TextWidth("刘") / 2
    ...
    If lstFilter.Height < 240 Then lstFilter.Height = 240
...
'提取病人历史数据信息
    ...
    mstrSQL = "SELECT B.ID,B.发生时间,C.项目序号,C.记录内容,C.数据来源,C.体温部位,C.未记说明,C.来源ID,C.共用,C.显示,DECODE(C.项目序号,-1,1,C.记录标记) 记录标记" & vbNewLine & _
        ...
        " AND Mod(C.记录类型,5)<>5 And C.终止版本 IS NULL  AND B.发生时间 Between [2] And [3] And C.项目序号=D.项目序号 And D.项目序号=E.项目序号 " & _
        IIf(mblnSpecialWave = True, "", " AND nvl(D.护理等级,3) >=[4] And Nvl(D.适用病人,0) In (0,[5])") & vbNewLine & _
            ...
            With rsTemp
                Do While Not .EOF
                    If lngItemNO <> 4 Then
                        ...
                        If InStr(1, "," & gint体温 & "," & gint脉搏 & "," & gint心率 & "," & gint疼痛强度 & ",", "," & Val(Nvl(!项目序号)) & ",") > 0 Then
                            Select Case Val(Nvl(!项目序号))
                                Case gint体温
                                    If gint体温 = lngItemNO Then blnAllow = True
                                Case gint疼痛强度
                                    If gint疼痛强度 = lngItemNO Then blnAllow = True
                                Case gint脉搏
                ...
                If lngItemNO = 4 Then
                    If InStr(1, strContent, "/") <> 0 Then
                        ...
                        mrsTemperatureInfo.Filter = "名称='" & Split(strContent, "/")(0) & "'"
                        If Not mrsTemperatureInfo.EOF Then
...
Public Sub RefreshPatiList(ByVal lngFormatID As Long)
    '刷新病人清单
    Call LoadPatient(lngFormatID)
...
'功能：初始化体温单批量录入过滤条件
    ...
    Dim strFilterContent As String
    Dim arrFilterContent() As String
    Dim strContentID  As String
    
    strSel = zlDatabase.GetPara("体温单过滤条件", glngSys, 1255)
    
    mstrSQL = "SELECT Zl_Getpatienttype(" & mlng科室ID & ") as 内容 FROM dual"
    Set rsContent = zlDatabase.OpenSQLRecord(mstrSQL, "提取过滤条件")
    strFilterContent = Trim(Nvl(rsContent!内容))
    If strFilterContent <> "" Then
        arrFilterContent = Split(strFilterContent, ";")
        
        For i = 0 To UBound(arrFilterContent)
            If i = UBound(arrFilterContent) Then
                strContentID = i + 1
                strFilterID = strFilterID & strContentID
            Else
                strContentID = i + 1
                strFilterID = strFilterID & strContentID & ";"
            End If
        ...
        Else
            arrSel = Split(strSel, ";")
            strSel = strSel & String(IIf(UBound(arrFilterContent) - UBound(arrSel) < 0, 0, UBound(arrFilterContent) - UBound(arrSel)), ";")
        End If
        
        arrSel = Split(strSel, ";")
        txtFilter.Tag = ""
        txtFilter.Text = ""
        strFilter = strFilterContent '"全部;入院三天内的病人;手术三天内的病人;三天内体温存在超过37.5度的病人;危/重病人;转入三天内的病人;一级及以上护理等级的病人" & IIf(mstr科室性质 = "产科" Or mstr科室性质 = "所有", ";分娩后三天内的病人", "")
    '    strFilterID = "0;1;2;3;4;5;6" & IIf(mstr科室性质 = "产科" Or mstr科室性质 = "所有", ";7", "")
        ...
        blnSelAll = True
        lstFilter.Clear
        lstFilter.AddItem "全部": lstFilter.ItemData(lstFilter.NewIndex) = 0
...
Private Function InitEnv() As Boolean
    Dim curDate As Date
    ...
    blnType = Val(GetSetting("ZLSOFT", "私有模块\UserBodyMutilEditor\" & gstrUserName, "Value")) = 0
    If blnType Then
    ...
    '打开现存在的所有护理记录项目
    mstrSQL = " Select DECODE(A.记录法,2,DECODE(A.项目序号,3,'1)体温曲线项目','2)体温表格项目'),'1)体温曲线项目') 分组名," & _
              "     B.项目序号,B.项目名称,B.项目类型,B.项目性质,B.项目长度,B.项目小数,B.项目表示,B.项目单位,B.项目值域,B.护理等级,B.适用病人,B.应用方式,nvl(A.记录法,2) 记录法" & _
              " From 护理记录项目 B,体温记录项目 A" & _
              " Where B.项目序号=A.项目序号(+) " & _
              " Order by B.项目序号"
    Set mrsItems = zlDatabase.OpenSQLRecord(mstrSQL, "打开现存在的所有护理记录项目")
    
    '提取未记说明信息
    mstrNote = ""
    mstrSQL = "Select 编码,名称 From 常用体温说明 Order by 编码"
    Call zlDatabase.OpenRecordset(mrsTemperatureInfo, mstrSQL, "未记说明信息")
    lstNote.Clear
    With mrsTemperatureInfo
        Do While Not .EOF
    ...
    cbo体温标识.Clear
    cbo体温标识.AddItem "1次/日"
    cbo体温标识.AddItem "2次/日"
    cbo体温标识.AddItem "3次/日"
    cbo体温标识.AddItem "4次/日"
    cbo体温标识.AddItem "5次/日"
...
Private Sub LoadPatient(ByVal lngFormatID As Long)
    ...
    If chkScope(在院).Value = 1 Then
        strSQL = strSQL & IIf(strSQL <> "", " Union All ", "") & _
            ...
            " Where B.病人类型=Z.名称(+) And A.病人ID=B.病人ID And A.主页ID=B.主页ID And Nvl(B.主页ID,0)<>0 And Nvl(B.状态,0)<>1" & _
    ...
    End If
    '再次过滤出有体温单文件的病人
    strTime = Format(Format(dtpDate.Value, "YYYY-MM-DD") & " 23:59:59", "YYYY-MM-DD HH:mm:ss")
    ...
    strSQL = "SELECT A.排序,A.排序2,A.类型,A.病人ID,A.主页ID,A.门诊号,A.住院号,A.姓名,A.性别,A.年龄,A.科室,A.科室ID,A.住院医师,A.责任护士,A.病案状态," & _
            ...
            " Where A.病人ID=B.病人ID and A.主页ID=B.主页ID And nvl(B.婴儿,0)=0 And B.归档人 is null and B.结束时间 is null and B.格式ID=[5] And B.开始时间<=[6]"
    ...
    Set mrsPati = zlDatabase.OpenSQLRecord(strSQL, "提取病人列表", mlng病区ID, _
        ...
        mintChange, lngFormatID, CDate(strTime))
...
Private Sub cboFormat_KeyPress(KeyAscii As Integer)
    Call zlControl.CboMatchIndex(cboFormat.hWnd, KeyAscii)
End Sub
...
'        End If
        If mblnChage = True Then
            If MsgBox("数据已经发生了变化但未保存,此操作会导致录入的数据丢失,请问是否继续?" & vbCrLf & "操作：日期发生变化", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                CheckEditData = True
                Exit Function
            Else
                mblnChage = False
...
Private Sub cmdAddUser_Click()
    ...
    If mrsPati.State = 0 Or mlng格式ID <> Val(cboFormat.ItemData(cboFormat.ListIndex)) Then
        Call RefreshPatiList(Val(cboFormat.ItemData(cboFormat.ListIndex)))   '刷新病人列表信息
    End If
...
'根据用户设置的过滤条件过滤病人信息
    If mblnChage = True Then
        If MsgBox("数据已经发生了变化但未保存,此操作会导致录入的数据丢失,请问是否继续?" & vbCrLf & "操作：重新过滤数据", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
    End If
    mblnInit = False
    mlng科室ID = Val(cboUnit.ItemData(cboUnit.ListIndex))
...
'-----------------------------------------------------
    ...
    If mlng格式ID <> Val(cboFormat.ItemData(cboFormat.ListIndex)) Then
        Set mrsPati = New ADODB.Recordset
        mlng格式ID = Val(cboFormat.ItemData(cboFormat.ListIndex))
    End If
    cboFormat.Tag = mlng格式ID
...
'----------------------------------------
    ...
    mstrItemNo = ""
    mblnSpecialWave = False
    mstrSQL = "Select 子类 From 病历文件列表 Where ID=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(mstrSQL, "体温单批量录入", mlng格式ID)
    If rsTemp.RecordCount > 0 Then
        mblnSpecialWave = (Nvl(rsTemp!子类) = "1")
    End If
    If Not mblnSpecialWave Then
        Select Case mintPatiNo
            Case 1
        ...
        '提取要录入的表格数据信息
        mstrSQL = _
                " SELECT /*+ RULE */ A.项目序号,DECODE(A.项目序号,4,'血压',A.记录名) || DECODE(nvl(A.单位,''),'','', '(' || A.单位 || ')') 项目名称,A.排列序号," & _
                "   DECODE(A.记录法,2,DECODE(A.项目序号,3,'1)体温曲线项目','2)体温表格项目'),'1)体温曲线项目') 分组名" & _
                " FROM 体温记录项目 A,诊治所见项目 C, 护理记录项目 B" & vbNewLine & _
    ...
    Else
        strFind = ""
        mstrSQL = "Select d.对象序号, d.内容文本, d.要素名称, d.要素表示 " & _
            " From 病历文件结构 D, 病历文件结构 P " & _
            " Where p.Id = d.父id And p.文件id = [1] And p.对象类型 = 1 And p.内容文本 = '曲线项目定义'" & _
            " Order By d.对象序号"
        Set rsTemp = zlDatabase.OpenSQLRecord(mstrSQL, "体温项目定义", mlng格式ID)
        Do While Not rsTemp.EOF
            If InStr(1, "," & strFind & ",", "," & Val(Nvl(rsTemp!内容文本)) & ",") = 0 Then
                strFind = IIf(strFind = "", "", strFind & ",") & Nvl(rsTemp!内容文本)
            End If
        rsTemp.MoveNext
        Loop
        mstrSQL = "Select d.对象序号, d.内容文本,d.要素名称,d.要素表示 " & _
        ...
        rsTemp.MoveNext
        Loop
        If Left(strFind, 1) = "," Then strFind = Mid(strFind, 2)
        '提取项目信息
        mstrSQL = _
            " SELECT /*+ RULE */ A.项目序号,DECODE(A.项目序号,4,'血压',A.记录名) || DECODE(nvl(A.单位,''),'','', '(' || A.单位 || ')') 项目名称,A.排列序号," & _
            "   DECODE(A.记录法,2,DECODE(A.项目序号,3,'1)体温曲线项目','2)体温表格项目'),'1)体温曲线项目') 分组名" & _
            " FROM 体温记录项目 A,诊治所见项目 C, 护理记录项目 B,Table(Cast(f_num2list([1]) As zlTools.t_Numlist)) D" & vbNewLine & _
            " WHERE  B.项目ID=C.ID(+) AND A.项目序号=B.项目序号  And A.项目序号<>5 And B.项目性质=1 " & vbNewLine & _
            " And A.项目序号=D.Column_Value And NOT (B.应用方式=2 And B.项目序号=-1)" & vbNewLine & _
            " ORDER BY Decode(A.项目序号,1,0,1),A.排列序号"
        Set rsTemp = zlDatabase.OpenSQLRecord(mstrSQL, "提取体温单项目", strFind)
    End If
    '提取体温曲线项目
    ...
    With VsfData
        ...
        For lngCol = 0 To .Cols - 1
            ...
            Else
                ...
                .TextMatrix(2, lngCol) = IIf(mblnSpecialWave = True, 0, Val(Nvl(mrsItems!适用病人, 0)))
            End If
        Next lngCol
...
'-----------------------------------------------------------------------------------------------------------------
    ...
    Dim strFind As String, strTime As String
    Dim lngTypeAll As Long
    ...
    '70637:刘鹏飞,2014-3-3,修改始终过滤的是所有病人问题。
    ...
    '没有勾选的条件为空，勾选的条件传序号
    '如：入院3天的病人;危重病人,序号为1;2。只勾选了危重，条件传入的格式为:";2"
    arrFilter = Split(strFilter, ";")
    For i = 0 To UBound(arrFilter)
        If Val(arrFilter(i)) = 1 Then
            strType = strType & i & ";"
        Else
            strType = strType & ";"
        End If
    ...
    '58890:刘鹏飞,2013-02-26,在院病人读取性能优化(关联在院病人表进行查询)
    '此处对于出院病人不进行提取
'    If Val(txtFilter.Tag) = 0 Then '提取对应病区科室所有在院病人
'        strPatient = "" & _
'                " SELECT 1 AS 性质,B.病人ID, B.主页ID, NVL(b.姓名,a.姓名) 姓名, NVL(b.性别,a.性别) 性别, NVL(b.年龄,a.年龄) 年龄, B.住院号, lpad(nvl(B.出院病床,' '),10,' ') AS 床号,0 AS 婴儿" & _
...
'                " And A.病人ID=C.病人ID And C.病区ID=[3]" & _
...
'                    " SELECT 1 AS 性质,B.病人ID,B.主页ID, NVL(b.姓名,a.姓名) 姓名, NVL(b.性别,a.性别) 性别, NVL(b.年龄,a.年龄) 年龄,B.住院号, lpad(nvl(B.出院病床,' '),10,' ') AS 床号,0 AS 婴儿" & vbNewLine & _
'                    " FROM 病人信息 A,病案主页 B,在院病人 F, 病人护理文件 C ,病人护理数据 D,病人护理明细 E" & vbNewLine & _
'                    " WHERE A.病人ID = B.病人ID And A.主页ID=B.主页ID And A.病人ID=F.病人ID AND NVL(B.主页ID, 0) <> 0 AND F.病区ID = [3]" & vbNewLine & _
...
'                   " Where A.病人ID = b.病人ID And A.主页ID=B.主页ID And A.病人ID=F.病人ID And NVL(b.主页ID, 0) <> 0 And F.病区ID = [3]" & _
'                   " AND Nvl(B.病案状态,0)<>5 AND B.封存时间 is NULL" & _
...
'                " Where A.病人ID = b.病人ID And A.主页ID=B.主页ID And NVL(b.主页ID, 0) <> 0 And b.病人id = c.病人id And b.主页id = c.主页id " & vbNewLine & _
...
'    End If
    ...
    strPatient = "Select 1 AS 性质,B.病人ID,B.主页ID, NVL(b.姓名,a.姓名) 姓名, NVL(b.性别,a.性别) 性别, NVL(b.年龄,a.年龄) 年龄,B.住院号, lpad(nvl(B.出院病床,' '),10,' ') AS 床号,0 AS 婴儿 " & vbNewLine & _
                "From 病人信息 A ,病案主页 B,在院病人 C," & vbNewLine & _
                "(Select distinct  C1 病人ID,C2 主页ID from table(Zl_Getpatient(to_date('" & strBegin & "','yyyy-mm-dd hh24:mi:ss'),to_date('" & strEnd & "','yyyy-mm-dd hh24:mi:ss')," & mlng病区ID & "," & mlng格式ID & ",'" & strType & "'," & lngTypeAll & "))) D" & vbNewLine & _
                "Where A.病人ID = b.病人ID And A.主页ID=B.主页ID And A.病人ID=C.病人ID And NVL(b.主页ID, 0) <> 0 and b.病人ID=D.病人ID and b.主页ID=D.主页ID " & IIf(mlng科室ID = -1, "", " And C.科室ID=[4]")
    ...
    mstrSQL = " SELECT  A.性质,A.病人ID,A.主页ID,A.婴儿,A.姓名,A.年龄,lpad(A.床号,10,' ') as 床号,nvl(zl_PatitTendGrade(A.病人ID,A.主页ID),3) 护理等级,C.信息值 AS 体温标识, MAX(B.ID) AS 文件ID,B.开始时间" & _
              ...
              " And B.归档人 IS NULL And B.结束时间 IS NULL And B.格式ID=[5] And B.开始时间<=[6]" & _
              " GROUP BY A.性质,A.病人ID,A.主页ID,A.婴儿,C.信息值,A.姓名 ,A.年龄,A.床号,B.开始时间" & _
              " Order by A.性质,A.床号,A.婴儿"
    Set rsTemp = zlDatabase.OpenSQLRecord(mstrSQL, "提取病人清单", CDate(strBegin), CDate(strEnd), mlng病区ID, mlng科室ID, mlng格式ID, CDate(strTime))
    ...
    With rsTemp
        Do While Not .EOF
            ...
            VsfData.TextMatrix(.AbsolutePosition + VsfData.FixedRows - 1, c护理等级) = IIf(mblnSpecialWave = True, 0, Val(!护理等级))
...
Private Sub cmdFilterOK_Click()
    ...
    txtFilter.SetFocus
    picFilter.Visible = False
    If lstFilter.ListCount <= 1 Then Exit Sub
    '保存过滤条件信息
    If Val(txtFilter.Tag) = 0 Then
        strValue = ""
        For i = 1 To lstFilter.ListCount - 1
...
Private Sub cmdFilterUserOk_Click()
    ...
    If Trim(strPatient) = "" Then Call InitCons: Exit Sub
    On Error GoTo Errhand:
    
    If mlng格式ID <> Val(cboFormat.ItemData(cboFormat.ListIndex)) And mlng格式ID <> 0 And mblnChage = True Then
        If MsgBox("数据已经发生了变化但未保存,此操作会导致录入的数据丢失,请问是否继续?" & vbCrLf & "操作：文件格式变化", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
            '隐藏所有PIC
            Call InitCons
            Exit Sub
        End If
    ...
    Call InitCons
    '如果还未添加列此处需要添加列信息
    If Not mblnNullRow Or mlng格式ID <> Val(cboFormat.ItemData(cboFormat.ListIndex)) Then
        mlng格式ID = Val(cboFormat.ItemData(cboFormat.ListIndex))
        mstrDate = Format(dtpDate.Value, "YYYY-MM-DD")
        Call InitVariable
    ...
    With rsTemp
        Do While Not .EOF
            ...
            VsfData.TextMatrix(lngTempRow, c护理等级) = IIf(mblnSpecialWave = True, 0, Val(!护理等级))
...
Private Sub optLevel_MouseUp(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    If optLevel(0).Value Then
         SaveSetting "ZLSOFT", "私有模块\UserBodyMutilEditor\" & gstrUserName, "Value", 0
    Else
        SaveSetting "ZLSOFT", "私有模块\UserBodyMutilEditor\" & gstrUserName, "Value", 1
    End If
End Sub
...
Private Sub UserControl_Terminate()
    ...
    If Not (mrsTemperatureInfo Is Nothing) Then Set mrsTemperatureInfo = Nothing
End Sub
...
Private Sub VsfData_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    Dim lngItemNO As Long
    Dim strText As String, strPart As String, strKey As String, strTmp As String
    Dim lngCol As Long
    Dim cbrControl As CommandBarControl
    ...
    If mrsItems.RecordCount <> 0 Then
        ...
        ElseIf lngItemNO = gint疼痛强度 Then
            strInfo = strInfo & Space(4) & "疼痛减痛:6/2"
        ElseIf lngItemNO = gint脉搏 And mint心率应用 = 2 Then
            strInfo = strInfo & Space(4) & "脉搏短轴:100/120"
        ElseIf lngItemNO = 4 Then
            strTmp = ""
            mrsTemperatureInfo.Filter = "": mrsTemperatureInfo.Sort = "编码"
            Do While Not mrsTemperatureInfo.EOF
                strTmp = strTmp & "、" & Nvl(mrsTemperatureInfo!名称)
                mrsTemperatureInfo.MoveNext
            Loop
            strTmp = Mid(strTmp, 2)
                    
            strInfo = strInfo & Space(4) & "收缩压/舒张压:110/90" & IIf(strTmp = "", "", "或(" & strTmp & ")")
        End If
...
Private Sub VsfData_EnterCell()
    ...
    If rsObj.RecordCount > 0 Then
        ...
        If InStr(1, ",0,9,", "," & Val(rsObj!数据来源) & ",") = 0 Then
            If Not (lngItemNO = gint体温 Or lngItemNO = gint疼痛强度 Or (lngItemNO = gint脉搏 And mint心率应用 = 2)) Then
            ...
            Else
                If mintModify = 1 Then
                    ...
                    ElseIf lngItemNO = gint疼痛强度 Then
                        strInfo = "同步过来的[" & strName & "]数据如果包含疼痛减痛不能进行修改."
                    Else
...
'----------------------------------
    ...
    If intType = 0 Then
        If lngItemNO <> 4 Then
            ...
            If InStr(1, ",2,3,5,", "," & Val(Nvl(mrsItems!项目表示)) & ",") = 0 Then
                ...
                If lngItemNO = gint体温 Or lngItemNO = gint疼痛强度 Or (lngItemNO = gint脉搏 And mint心率应用 = 2) Then
    ...
    Select Case mintType
        ...
        Case 4 '血压
            ...
            If InStr(1, strText, "/") = 0 Then strText = strText & "/"
...
'                End If
    ...
    mintType = 0
    Dim intMoveCol As Integer
    '开始移动行或列
    With VsfData
        If KeyCode = vbKeyReturn Then
            If blnType = False Then
NextRow3: '问题号：56592,李涛，纵向跳转方式。
                If .Row < .Rows - 1 Then
                  .Row = .Row + 1
                   If .RowHidden(.Row) = True Then GoTo NextRow3
                Else
                    intMoveCol = .Col
NextCol3:
                    If intMoveCol < .Cols - 1 Then
                      intMoveCol = intMoveCol + 1
                      If .ColHidden(intMoveCol) = True Then GoTo NextCol3
                      .Select .FixedRows, intMoveCol
                    Else
                       intMoveCol = .Col
                      .Select .FixedRows, intMoveCol
                    End If
...
ErrRow:
                ...
                If lngItemCode = gint脉搏 And mint心率应用 = 2 Then
                    If mbln脉搏共用显示 And InStr(1, strValue, "/") > 0 Then
                        strValue = Split(strValue, "/")(1) & "/" & Split(strValue, "/")(0)
                    End If
                End If
                
                mrsTemperatureInfo.Filter = "名称='" & Nvl(!数据) & "'"
                If lngItemCode = 4 And (Not mrsTemperatureInfo.EOF) Then
...
'    Next i
    ...
    mstrSQL = "SELECT C.项目序号,C.记录内容,C.数据来源,C.体温部位,C.未记说明,C.来源ID,C.共用,C.显示,DECODE(C.项目序号,-1,1,C.记录标记) 记录标记" & vbNewLine & _
        ...
        " AND nvl(C.来源ID,0)>0 And Mod(C.记录类型,5)<>5 AND C.终止版本 IS NULL  AND B.发生时间=[4] And C.项目序号=D.项目序号 " & _
        IIf(mblnSpecialWave = True, "", " AND NVL(D.护理等级,3) >=[5] And Nvl(D.适用病人,0) In (0,[6])") & vbNewLine & _
...
'-------------------------------------------------------------
    ...
    If mlngBaby <> 0 And mbln出院 = True Then
        strBabyOutTime = GetAdviceOutTime(mlng病人ID, mlng主页ID, mlngBaby)
        If strBabyOutTime <> "" Then
            '补录小时检查
            If Format(DateAdd("H", mlngHours, strBabyOutTime), "yyyy-MM-dd HH:mm") < Format(strCurrDate, "yyyy-MM-dd HH:mm") Then
    ...
    If strText <> "" Then
        If mrsItems!项目类型 = 0 And InStr(1, "0,4", Nvl(mrsItems!项目表示, 0)) <> 0 Then
            If lngItemNO = 4 Then
                ...
                If InStr(1, strText, "/") = 0 Then
                    strInfo = "[血压]数据格式错误：收缩压/舒张压" & IIf(strTmp <> "", "或(" & strTmp & ")", "") & "！"
                    Exit Function
                End If
                
                '--问题号53505,修改人：李涛，血压显示文字
                mrsTemperatureInfo.Filter = "名称='" & Trim(Split(strText, "/")(0)) & "'"
                If Not mrsTemperatureInfo.EOF Then
                ...
                Else
                    If Trim(Split(strText, "/")(0)) = "" Or Trim(Split(strText, "/")(1)) = "" Then
                        strInfo = "[血压]数据格式录入错误：收缩压/舒张压" & IIf(strTmp <> "", "或(" & strTmp & ")", "") & "！"
                        Exit Function
                    End If
                End If
            ElseIf lngItemNO = gint脉搏 And mint心率应用 <> 2 Then
                If InStr(1, strText, "/") <> 0 Then
                    ...
                    Exit Function
                End If
            ElseIf lngItemNO <> gint体温 And lngItemNO <> gint疼痛强度 And Not (lngItemNO = gint脉搏 And mint心率应用 = 2) And blnAllow = True Then
                If InStr(1, strText, "/") <> 0 Then
        ...
        If lngItemNO = gint体温 Or lngItemNO = gint疼痛强度 Or (lngItemNO = gint脉搏 And mint心率应用 = 2) Then
            ...
            If InStr(1, strValue, "/") = 0 Then
                If Trim(strFormat1) <> Trim(strValue) Then
                    ...
                    ElseIf lngItemNO = gint疼痛强度 Then
                        strInfo = "同步过来的[" & strName & "]数据只允许修改疼痛减痛部分."
                    Else
            ...
            Else
                If mintModify = 1 Then
                    If strFormat <> strValue Then
                        ...
                        ElseIf lngItemNO = gint疼痛强度 Then
                            strInfo = "同步过来的[" & strName & "]数据如果包括疼痛减痛,不允许修改."
                        Else
