...
Begin VB.Form frmCaseTendBody 
   ...
   Begin VB.PictureBox picPati 
      ...
      End
   End
   Begin zl9TemperatureChart.usrBodyEditor BodyEdit 
      Height          =   5625
      Left            =   975
      TabIndex        =   20
      Top             =   825
...
Private t_OldInfo As type_Patient '记录原始病人信息(因为原始大小可以切换病人)
...
Private mblnFileChange As Boolean
Private mlngFileID As Long
Private mlngBaby As Long

Public Property Get FileChange() As Boolean
   FileChange = mblnFileChange
End Property
...
Public Property Get BabyNo() As Long
   BabyNo = mlngBaby
End Property
...
Public Function ShowEdit(ByVal frmMain As Object, strParam As String, Optional ByVal bytMode As Byte = 1, Optional strPrivs As String) As Boolean
    ...
    gstrSQL = "Select A.格式ID From 病人护理文件 A, 病历文件列表 B Where a.格式id = b.Id And b.种类 = 3 And b.保留 = -1 And A.Id = [1]"
    If mblnMove = True Then gstrSQL = Replace(gstrSQL, "病人护理文件", "H病人护理文件")
    Set RS = zlDatabase.OpenSQLRecord(gstrSQL, "病人护理文件", T_Info.lng文件ID)
    T_Info.lng格式ID = RS!格式ID
    
    t_OldInfo.lng病人ID = T_Info.lng病人ID
    t_OldInfo.lng主页ID = T_Info.lng主页ID
    t_OldInfo.lng文件ID = T_Info.lng文件ID
    ...
    If blnShowing = False Then
        ...
        If mblnChanged = True And mlngFileID <> T_Info.lng文件ID Then
            glngCurPage = T_Info.lngPage
        End If
    End If
...
Private Function OpenPatientMap() As Boolean
    ...
    '提取病人体温单文件列表
    gstrSQL = "Select A.ID 文件Id,B.ID 格式Id,A.开始时间,B.编号 From 病人护理文件 A,病历文件列表 B" & _
        "   Where A.格式ID=B.ID And B.种类=3 And B.保留=-1 And A.病人ID=[1] And A.主页ID=[2] And NVl(A.婴儿,0)=[3]" & _
        "   Order by A.开始时间,B.编号"
    If mblnMove = True Then gstrSQL = Replace(gstrSQL, "病人护理文件", "H病人护理文件")
    Set RS = zlDatabase.OpenSQLRecord(gstrSQL, "提取病人文件信息", T_Info.lng病人ID, T_Info.lng主页ID, T_Info.lng婴儿)
    Do While Not RS.EOF
        If t_OldInfo.lng病人ID = T_Info.lng病人ID And t_OldInfo.lng主页ID = T_Info.lng主页ID Then
            If RS!格式ID = T_Info.lng格式ID And RS!文件ID = t_OldInfo.lng文件ID Then
                lngFileID = RS!文件ID
                Exit Do
            End If
        Else
            If RS!格式ID = T_Info.lng格式ID Then
                lngFileID = RS!文件ID
                Exit Do
            End If
        End If
    ...
    If RS.RecordCount > 0 Then
        RS.MoveFirst
        If t_OldInfo.lng病人ID = T_Info.lng病人ID And t_OldInfo.lng主页ID = T_Info.lng主页ID And lngFileID = 0 Then
            RS.Filter = "格式ID=" & T_Info.lng格式ID
            RS.Sort = "开始时间,编号"
            If RS.RecordCount > 0 Then lngFileID = RS!文件ID
        End If
        If lngFileID = 0 Then
            RS.Filter = ""
            RS.Sort = "开始时间,编号"
...
Private Sub BodyEdit_ShowTipInfo(ByVal vsfObj As Object, ByVal strInfo As String, ByVal blnMultiRow As Boolean)
    Call zlCommFun.ShowTipInfo(vsfObj.hWnd, Trim(strInfo), blnMultiRow)
End Sub

Private Sub BodyEdit_zlDataChange(ByVal blnChange As Boolean)
    mblnChanged = blnChange
End Sub

Private Sub BodyEdit_zlFileChange(ByVal blnRefresh As Boolean, ByVal lngFileID As Long, ByVal lngBaby As Long)
    mblnFileChange = blnRefresh: mlngFileID = lngFileID: mlngBaby = lngBaby
End Sub
...
Private Sub AddPages()
    ...
    strSQL = " Select DISTINCT A.主页ID,A.病人性质 From 病案主页 A,病人护理文件 B,病历文件列表 C" & _
...
Private Sub GetPatiInfo()
    ...
    strSQL = "Select NVL(B.姓名,A.姓名) 姓名,B.出院病床 床号, C.颜色,B.当前病区ID,B.病案状态,B.出院科室id" & vbNewLine & _
...
ErrGO:
        ...
        strSQL = "SELECT A.排序,A.排序2,A.类型,A.病人ID,A.主页ID,A.门诊号,A.住院号,A.姓名,A.性别,A.年龄,A.科室,A.科室ID,A.住院医师,A.责任护士,A.病案状态," & _
                ...
                " From (" & strSQL & ") A Where exists " & _
                "   (SELECT B.ID From 病人护理文件 B,病历文件列表 C" & _
                "   Where B.病人ID=A.病人ID And B.主页ID=A.主页ID And NVL(B.婴儿,0)=0 And B.格式ID=C.ID And C.种类=3 And C.保留=-1 And ROWNUM<2)"
