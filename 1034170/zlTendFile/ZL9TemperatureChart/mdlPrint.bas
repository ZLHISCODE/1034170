...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    For lngPage = 1 To lngCountPage
        ...
        If bln打印医院名称 = True Then
            ...
            Call DeleteObject(lngFont)
            Call ReleaseFontIndirect(objDraw)
            Y = lngCurY + T_Size.H + 12 * msngTwips
        Else
        ...
        If bln体温单显示诊断 Then
            ...
            Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "最后诊断", "最后诊断", lng病人ID, lng主页ID, CDate(strTmp))
            If rsTmp.BOF = False Then
                If intBaby = 0 Then
        ...
        If bln显示皮试 = True Then
            strSQL = _
               "SELECT 时间,F_LIST2STR(CAST(COLLECT(药物名) AS T_STRLIST)) 药物名 FROM (" & vbNewLine & _
                "   SELECT TO_CHAR(开始执行时间,'YYYY-MM-DD') 时间,DECODE(皮试结果,'(+)',255," & lngColor & ") || '-#' || REPLACE(REPLACE(医嘱内容,',',''),'-#','') || DECODE(皮试结果,'阳性','阴性')  药物名" & vbNewLine & _
...
ErrClare:
    ...
    T_DrawClient.独立曲线总行数 = M_DrawClient.独立曲线总行数
...
'            If RS.BOF = False Then lng诊疗项目id = zlCommFun.Nvl(RS("ID").Value)
            ...
            strSQL = _
               ...
               "    From 病人变动记录 A,部门表 b" & vbNewLine & _
               "    Where b.id(+)=a.病区ID and a.开始原因 = 15 And A.病人id = [1] And A.主页id = [2]  And [3]=0 And A.开始时间 Between [4] And [5] " & vbNewLine & _
               "    Union " & vbNewLine & _
            ...
            End If
            strSQL = strSQL & vbNewLine & _
               "    Select '' As 科室, Nvl(b.开始执行时间, a.出院日期) As 时间," & strWay & "8 As 行号" & vbNewLine & _
...
'               "    Where A.病人id = [1] And A.主页id = [2] And A.出院日期 Between [4] And [5] And a.病人id=b.病人id(+) And a.主页id=b.主页id(+) "
            ...
            If lng婴儿 <> 0 Then
                '提取婴儿医嘱记录(转科,出院,死亡)
                If blnOutWay = True Then
                    strWay = "Decode(操作类型,3,'转出',5,'出院','死亡') 内容,"
                Else
                    strWay = "Decode(操作类型,3,'转出',5,Decode(医嘱内容,NULL,'出院',医嘱内容),Decode(医嘱内容,NULL,'死亡',医嘱内容)) 内容,"
                End If
                strNewSql = _
                    "   Select /*+ RULE */ 科室,开始执行时间," & strWay & "Decode(操作类型,'3',3,8) 行号 From (" & vbNewLine & _
...
Private Function GetDataSQL(ByVal strName As String) As String
    ...
    Select Case strName
        Case "提取文件时间范围"
            ...
            '------------------------------------------------------------------------------------------------------------------
            strSQL = _
               "Select /*+ RULE */ Decode(b.出生时间,Null,a.开始,b.出生时间) As 开始,decode(E.记录,0,Decode(Sign(NVL(E.婴儿时间,a.终止) - d.发生时间), 1,NVL(E.婴儿时间,a.终止) ,d.发生时间),NVL(E.婴儿时间,a.终止)) 终止,E.记录" & vbNewLine & _
                "       From" & vbNewLine & _
