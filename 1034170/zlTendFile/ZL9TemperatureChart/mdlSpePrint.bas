...
Public Function PrintOrPreviewBodyStateNew(objOut As Object, _
    ...
    Dim blnCurBeginTop As Boolean '参数:体温标志输出位置
    '其他绘图变量
    ...
    Dim str心率符号  As String, lng心率颜色 As Long
    ...
    Else
        intOpFormat = 0
    End If
    '70452:刘鹏飞,2014-08-02,体温标志输出位置:曲线顶格开始、体温42开始
    ...
    str心率符号 = "○"
    lng心率颜色 = RGB_RED
    strSQL = "Select a.应用方式,b.记录符,b.记录色 From 护理记录项目 a,体温记录项目 b Where a.项目序号=-1 And a.项目序号=b.项目序号"
    ...
    If rsTemp.BOF = False Then
        ...
        'lng心率颜色 = Val(Nvl(rsTemp!记录色, RGB_RED))
    Else
        int心率应用 = 0
    End If
    ...
    If int心率应用 = 2 And int脉搏 <> -1 Then
        ReDim Preserve strEditors(UBound(strEditors) + 1)
        strTmp = "-1||心率||" & Split(strEditors(int脉搏), "||")(2) & "||" & Split(strEditors(int脉搏), "||")(3) & "||" & str心率符号 & "||" & lng心率颜色 & "||" & _
    ...
    For lngPage = 1 To lngCountPage
        ...
        lngCurY = Y
        '读取病人科室、床号等信息
        VarPatiInfo(1) = GetElementValue("年龄", lng病人ID, lng主页ID, intBaby, strTmpDay)
        VarPatiInfo(3) = ""
        VarPatiInfo(4) = ""
        strTmp = "": strTime = ""
        ...
        Do While Not rsTmp.EOF
            ...
            If Not rsTmp.EOF Then
                If bln术后显示 And DateDiff("d", CDate(Format(strTime, "YYYY-MM-DD")), str结束时间) < 14 Then
                    str结束时间 = Format(DateAdd("D", T_BodyStyle.lng天数 - 1, CDate(strTmpDay)), "YYYY-MM-DD") & " 23:59:59"
                End If
            End If
            ...
            Do While Not .EOF
                ...
                If (Not IsNull(!未记说明)) And zlCommFun.Nvl(!数值) <> "不升" Then
                    ...
                    gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & !项目序号 & "|99|" & _
                        !未记说明 & "|" & lngNoRecordColor & "|" & SinX & "|0|0|0|0|" & zlCommFun.Nvl(!显示)
        ...
        For j = 0 To 1
            ...
            arrTmpValue = Array()
            rsPoints.Filter = "项目序号=" & lng项目序号 & " And 标记=0"
            With rsPoints
                Do While Not .EOF
                    ReDim Preserve arrTmpValue(UBound(arrTmpValue) + 1)
            ...
            rsPoints.Filter = "项目序号=" & lng项目序号
            If rsPoints.RecordCount > 0 Then rsPoints.MoveFirst
            For i = 0 To UBound(arrTmpValue)
                rsPoints.Filter = "项目序号=" & lng项目序号 & " and 标记=1 and X坐标=" & Val(Split(CStr(arrTmpValue(i)), ";")(3)) & " And 时间='" & Format(Split(CStr(arrTmpValue(i)), ";")(5), "yyyy-MM-dd HH:mm:ss") & "'"
                If rsPoints.RecordCount <> 0 Then
            ...
            arrTmpValue = Array()
            rsPoints.Filter = "项目序号=" & lng项目序号 & " and 标记=1"
            With rsPoints
                Do While Not .EOF
                    ReDim Preserve arrTmpValue(UBound(arrTmpValue) + 1)
                    arrTmpValue(UBound(arrTmpValue)) = !序号 & ";" & !项目序号 & ";" & !数值 & ";" & !X坐标 & ";" & !Y坐标 & ";" & Format(!时间, "yyyy-MM-dd HH:mm:ss")
            ...
            If rsPoints.RecordCount > 0 Then rsPoints.MoveFirst
            For i = 0 To UBound(arrTmpValue)
                rsPoints.Filter = "项目序号=" & lng项目序号 & " and 标记=0 and X坐标=" & Val(Split(CStr(arrTmpValue(i)), ";")(3)) & " And 时间='" & Format(Split(CStr(arrTmpValue(i)), ";")(5), "yyyy-MM-dd HH:mm:ss") & "'"
                If rsPoints.RecordCount = 0 Then
                    rsPoints.Filter = "项目序号=" & lng项目序号 & " and 标记=1 and X坐标=" & Val(Split(CStr(arrTmpValue(i)), ";")(3)) & " And 时间='" & Format(Split(CStr(arrTmpValue(i)), ";")(5), "yyyy-MM-dd HH:mm:ss") & "'"
                    rsPoints.Delete
                End If
        ...
        With rsPoints
            Do While Not .EOF
                If Not IsNull(!序号) Then
                    If Not ((Val(!项目序号) = gint体温 Or Val(!项目序号) = gint疼痛强度) And Val(!标记) = 1) Then
                        If lng项目序号 <> 0 Then
                    ...
                    If zlCommFun.Nvl(!类型) = 99 Then
                        If byt未记显示位置 = 1 Then '显示在体温的下面
                            ...
                            For i = 1 To Len(strTime)
                                If sinY < T_DrawClient.体温区域.Bottom Then
                                    ...
                                    If T_DrawClient.体温区域.Bottom - sinY >= T_Size.H - (1 * msngTwips) Then
...
ErrNext:
        ...
        lngFont = CreateFontIndirect(T_Font)
        lngOldFont = SelectObject(lngDC, lngFont)
        Call DrawBodyRecordItemNew(lngDC, objDraw, ArrNewString, rsItems, lngCurX, T_DrawClient.曲线总区域.Bottom, T_DrawClient.体温区域.Right, intRepairRows, lngCurY, sngScale)
        Call SelectObject(lngDC, lngOldFont)
        Call DeleteObject(lngFont)
        Call ReleaseFontIndirect(objDraw)
...
'-------------------------------------------------------------------------------------------------------
    ...
    lngMaxX = lng刻度宽度 + (T_BodyStyle.lng天数 * T_BodyStyle.lng监测次数 * lngColStep) + T_DrawClient.偏移量X    '刻度+7*宽度+偏移量X
    ...
    T_DrawClient.曲线总区域.Bottom = lngMaxY
        ...
        Do While Not .EOF
            ...
            If lngCurveRows > 0 Then
                ...
                For lngRow = 1 To lngCurveRows
                    ...
                    End If
                    If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
                        Call DrawLine(lngDC, lngCurX + 1, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0, PS_SOLID, intLineMode), IIf(lngRow Mod 5 = 0 And sinRowStep >= 4 And bln粗线, intBold, intFine), RGB_BLACK)
                    End If
                Next
                '画底线
...
'                        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), Val(Format(sin刻度, "#0.0")), Nvl(!记录色, RGB_BLACK))
        ...
        .MoveNext
        Loop
       T_DrawClient.曲线总区域.Bottom = 2 * mintNullRow * lngInitRowStep + (T_DrawClient.总列数 + T_DrawClient.独立曲线总行数) * sinRowStep + T_DrawClient.偏移量Y
    End With
...
'-------------------------------------------------------------------------------------
    ...
    Dim strWaveReview As String '参数:体温复试合格的符号
    On Error GoTo Errhand
    ...
    With rsPoint
        Do While Not .EOF
            For i = 0 To UBound(strEditors)
                ...
                End If
            Next i
            If Not ((zlCommFun.Nvl(!项目序号) = gint体温 Or zlCommFun.Nvl(!项目序号) = gint疼痛强度) And Val(zlCommFun.Nvl(!标记)) = 1) Then
                ...
                If !项目序号 = gint体温 Then
                    If zlCommFun.Nvl(!复查) = 1 Then '复试合格
                        ...
                        Call GetTextRect(objDraw, !X坐标, !Y坐标 - T_DrawClient.行单位, strWaveReview, T_DrawClient.列单位, True, , sngScale)
            ...
            If zlCommFun.Nvl(!重叠) = 0 And zlCommFun.Nvl(!重叠项目) = "空" Then '未重叠的项目
                ...
                Select Case zlCommFun.Nvl(!项目序号)
                    ...
                    Case gint呼吸
                        ...
                        End If
                    Case Else
                        If Val(Nvl(!项目序号)) = gint疼痛强度 And zlCommFun.Nvl(!标记) = 1 Then '疼痛减痛符号
                            lngRGB = RGB_RED
                            strChar = "○"
                        Else
                            strChar = strTmp
                ...
                If arrTmpString0(0) = "3" Then '呼吸项目
                    ...
                    For intCOl = 0 To UBound(arrTmpString1)
                        ...
                        Else
                            ...
                            If strPart <> "自主呼吸" And IsNumeric(strTmp) Then
                            ...
                            If IsNumeric(strTmp) Then
                                If strPart = "自主呼吸" Or blnBreatheShowType = True Then
                                    Call SetTextColor(lngDC, lngColor)
                    ...
                    If blnBreatheShowType = True Then
                        ...
                        For i = 0 To UBound(arrBreathe)
                            ...
                            strPart = "呼吸机"
                            Call SetTextColor(lngDC, lngColor)
                            T_Size.H = objDraw.TextHeight("呼") / T_TwipsPerPixel.Y
                            T_Size.W = objDraw.TextWidth("呼") / T_TwipsPerPixel.X
                            '由于GetTextRect函数默认给X+1所以此处-1
                            If intBegin = intEnd Then
                                If T_DrawClient.列单位 >= T_Size.W + (6 * msngTwips) Then
                                    lngBreatheX = T_DrawClient.体温区域.Left + (intBegin - 1) * T_DrawClient.列单位 + ((T_DrawClient.列单位 - T_Size.W - (6 * msngTwips)) \ 2) - 1
                                Else
                                    lngBreatheX = T_DrawClient.体温区域.Left + (intBegin - 1) * T_DrawClient.列单位 - ((T_Size.W + (6 * msngTwips) - T_DrawClient.列单位)) - 1
                                End If
                            Else
                                If T_DrawClient.列单位 >= T_Size.W + (3 * msngTwips) Then
                                    lngBreatheX = T_DrawClient.体温区域.Left + (intBegin - 1) * T_DrawClient.列单位 + ((T_DrawClient.列单位 - T_Size.W - (3 * msngTwips)) \ 2) - 1
                                Else
                                    lngBreatheX = T_DrawClient.体温区域.Left + (intBegin - 1) * T_DrawClient.列单位 - ((T_Size.W + (3 * msngTwips) - T_DrawClient.列单位)) - 1
                                End If
                            End If
                            ...
                            Else
                                ...
                                lngBreatheX = lngBreatheX + T_Size.W + (3 * msngTwips)
                                Call DrawLine(lngDC, lngBreatheX, lngBottomY - lngBreatheY - (T_Size.H \ 2), lngBreatheX, lngBottomY - (T_Size.H \ 2), PS_SOLID, intFine, lngColor, True)
                                lngBreatheX = T_DrawClient.体温区域.Left + (intEnd - 1) * T_DrawClient.列单位 + T_DrawClient.列单位 \ 2
                                Call DrawLine(lngDC, lngBreatheX, lngBottomY - (T_Size.H \ 2), lngBreatheX, lngBottomY - lngBreatheY - (T_Size.H \ 2), PS_SOLID, intFine, lngColor, True)
                            End If
                        Next i
                    End If
...
Public Sub OutPutTextNew(ByVal objDraw As Object, ByVal rsDrawItems As ADODB.Recordset, ByVal lngDC As Long, ByVal rsNote As ADODB.Recordset, _
    ...
    'blnBeginTop 决定体温自动标志是从曲线顶格输出还是42度开始输出
    ...
    Else
        dblCurveHeight = Format(GetYCoordinate(objDraw, rsDrawItems, gint体温, 40) - T_DrawClient.体温区域.Top, "#0.00")
    End If
    ...
    With rsNote
        ...
        Else
            lngY = T_DrawClient.体温区域.Top
        End If
        
        '调整入出转 手术，分娩到达最大X坐标有多列式的Y坐标
        If bln事件显示规则 = False Then
...
Public Sub GetPainDegreeNO()
    Dim rsTemp As New ADODB.Recordset
'功能:获取疼痛强度的项目序号
    '74547:刘鹏飞,2014-07-07,疼痛强度也可以录入减痛，例如体温的物理降温
    '由于疼痛强度不是固定项目，目前只能根据项目名称区分
    gint疼痛强度 = -999
    On Error GoTo Errhand
    gstrSQL = "Select 项目序号 from 体温记录项目 Where 记录名=[1]"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "体温记录项目", "疼痛强度")
    If rsTemp.RecordCount > 0 Then
        gint疼痛强度 = Val(rsTemp!项目序号)
    End If
    Exit Sub
Errhand:
    If ErrCenter = 1 Then
...
Public Function GetAdviceOutTime(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng婴儿 As Long) As String
'功能:获取病人或婴儿的医嘱出院时间
    Dim rsTemp As New ADODB.Recordset
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "获取病人医嘱出院时间", lng病人ID, lng主页ID, lng婴儿, strTmp)
...
Public Function GetElementValue(ByVal strElementName As String, ByVal lng病人ID As Long, ByVal lng主页ID As Long, Optional ByVal lng婴儿 As Long = 0, Optional ByVal strDate As String = "") As String
'功能:获取对应要素信息
    Dim strSQL As String
    Dim rsTmp As New ADODB.Recordset
    Dim strValue As String
    ...
    If IsDate(strDate) Then
        strSQL = "Select Zl_Replace_Element_Value([1],[2],[3],2,NULL,[4],[5]) As 信息值 From Dual"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "Zl_Replace_Element_Value", strElementName, lng病人ID, lng主页ID, lng婴儿, CDate(strDate))
    Else
        strSQL = "Select Zl_Replace_Element_Value([1],[2],[3],2,NULL,[4]) As 信息值 From Dual"
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "Zl_Replace_Element_Value", strElementName, lng病人ID, lng主页ID, lng婴儿)
    End If
    If rsTmp.BOF = False Then
        strValue = Nvl(rsTmp!信息值)
    Else
        strValue = ""
    End If
