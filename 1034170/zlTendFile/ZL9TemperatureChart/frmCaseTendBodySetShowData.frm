...
'---------------------------------------------------------------
    ...
    With rsTmp
        Do While Not .EOF
            ...
            '处理物理降温
            If (lng项目序号 = gint体温 Or lng项目序号 = gint疼痛强度) And zlCommFun.Nvl(!记录标记) = 1 Then
                mrsPoint.Filter = "项目序号=" & lng项目序号 & " and 时间='" & Format(zlCommFun.Nvl(!时间), "YYYY-MM-DD HH:mm:ss") & "' and 标记<>1"
                If mrsPoint.RecordCount > 0 Then
                    strParam = "序号|" & mrsPoint("序号")
                    strFidlds = "数值|ID"
                    gstrValues = Split(mrsPoint("数值"), "/")(0) & "/" & zlCommFun.Nvl(!数值) & "|" & Val(zlCommFun.Nvl(!序号))
        ...
        Do While Not .EOF
            If Val(!状态) = 2 Then
                ...
                Else
                    ...
                    If InStr(1, "," & gint疼痛强度 & "," & gint体温 & "," & gint脉搏 & ",", "," & lngItemCode & ",") <> 0 Then
                        lngID = Val(!Id)
...
Private Sub vfgThis_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    With vfgShow
        If NewRow >= vfgThis.FixedRows And NewCol >= vfgThis.FixedCols Then
            mintColSel = intCOl
            If Val(vfgThis.TextMatrix(intRow, 0)) = gint体温 Then  '体温项目
                .Rows = 4
                .TextMatrix(0, 0) = "时点"
                .TextMatrix(1, 0) = "数值"
                .TextMatrix(2, 0) = "复查"
            ...
            Else
             ...
             Do While Not mrsPoint.EOF
                .ColWidth(-1) = 1200 + 1200 * mintBigSize / 3
                 ...
                 If Val(vfgThis.TextMatrix(intRow, 0)) = gint体温 Then
