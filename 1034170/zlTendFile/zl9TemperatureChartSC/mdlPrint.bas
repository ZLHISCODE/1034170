        ...
        For i = 1 To 7
            If DateDiff("d", Int(CDate(strTmpDay)), Int(CDate(strEndDay))) + 1 >= i Then
                ...
                If Trim(strOpdays(i)) <> "" And lngValue + UBound(arrOperDay) < 12 Then
                    ...
                    '修改说明：四川地区要求手术当天不显示次数标记内容(I,II,III...)
                    If blnStopFlag Then '手术标注后天数在次手术时停止前一次标注
'                        If strTmp = "Ⅰ" Then
'                            strOpValue(i) = 0
'                        Else
...
'-------------------------------------------------------------------------------------------------------
    ...
    For lngRow = 0 To T_DrawClient.总列数
        ...
        End If
        '画体温单的所有行(四川地区要求刻度之间的连线为虚线)
        If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
            Call DrawLine(lngDC, lngCurX, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0, PS_SOLID, IIf(blnPrinter, PS_DASH, PS_DOT)), IIf(lngRow Mod 10 = 0 And sinRowStep >= 4 And bln粗线, intBold, 1), RGB_BLACK)
        End If
    Next
...
'            If RS.BOF = False Then lng诊疗项目id = zlCommFun.Nvl(RS("ID").Value)
            ...
            strSQL = _
               ...
               "    Select 科室,时间,内容,行号 From (Select * From (Select  b.名称 As 科室,A.终止时间 As 时间, '入科' As 内容,9 As 行号 " & vbNewLine & _
               "    From 病人变动记录 A,部门表 B" & vbNewLine & _
