...
Begin VB.UserControl UserMutilEditor 
   ...
   Begin VB.PictureBox picFilter 
      ...
      Begin VB.CommandButton cmdFilterCancel 
         Height          =   315
         Left            =   1530
         Picture         =   "UserMutilEditor.ctx":0000
         Style           =   1  'Graphical
         TabIndex        =   46
         ToolTipText     =   "取消"
         Top             =   1320
      ...
      Begin VB.CommandButton cmdFilterOK 
         Height          =   315
         Left            =   990
         Picture         =   "UserMutilEditor.ctx":058A
         Style           =   1  'Graphical
         TabIndex        =   45
         ToolTipText     =   "确认"
         Top             =   1320
   ...
   Begin VB.PictureBox picPati 
      ...
      Begin VB.CommandButton cmdFilterUserOk 
         Height          =   315
         Left            =   3990
         Picture         =   "UserMutilEditor.ctx":0B14
         Style           =   1  'Graphical
         TabIndex        =   38
         ToolTipText     =   "确认"
         Top             =   6435
      ...
      Begin VB.CommandButton cmdFilterUserCancle 
         Height          =   315
         Left            =   4530
         Picture         =   "UserMutilEditor.ctx":109E
         Style           =   1  'Graphical
         TabIndex        =   37
         ToolTipText     =   "取消"
         Top             =   6435
   ...
   Begin MSComctlLib.ImageList imgRPT 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "UserMutilEditor.ctx":1628
            Key             =   "woman"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "UserMutilEditor.ctx":7E8A
            Key             =   "man"
         EndProperty
      EndProperty
   End
   Begin MSComctlLib.ImageList ils16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         NumListImages   =   1
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "UserMutilEditor.ctx":E6EC
            Key             =   ""
         EndProperty
      EndProperty
   End
   Begin VB.PictureBox pic过滤条件 
      ...
      Begin VB.CommandButton cmdSift 
         ...
         Height          =   260
         Left            =   6560
         Picture         =   "UserMutilEditor.ctx":EA86
         Style           =   1  'Graphical
         TabIndex        =   8
         TabStop         =   0   'False
         ToolTipText     =   "选择项目(F4)"
   ...
   Begin VB.PictureBox picMain 
      ...
      Begin VB.ListBox lstSelect 
         ...
         Height          =   930
         Index           =   0
         ItemData        =   "UserMutilEditor.ctx":EB7C
         Left            =   1080
         List            =   "UserMutilEditor.ctx":EB89
         TabIndex        =   19
         Top             =   3120
         Visible         =   0   'False
         Width           =   1215
      ...
      Begin VB.ListBox lstSelect 
         ...
         Height          =   1080
         Index           =   1
         ItemData        =   "UserMutilEditor.ctx":EBA2
         Left            =   2280
         List            =   "UserMutilEditor.ctx":EBAF
         Style           =   1  'Checkbox
         TabIndex        =   20
         Top             =   3120
         Visible         =   0   'False
      ...
      Begin VB.ListBox lstNote 
         ...
         EndProperty
         Height          =   930
         ItemData        =   "UserMutilEditor.ctx":EBC8
         Left            =   120
         List            =   "UserMutilEditor.ctx":EBD2
         TabIndex        =   14
         Top             =   3120
         Visible         =   0   'False
         Width           =   975
      ...
      Begin VSFlex8Ctl.VSFlexGrid VsfData 
         ...
         FormatString    =   $"UserMutilEditor.ctx":EBE2
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.PictureBox picHistory 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfHistory 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"UserMutilEditor.ctx":EC44
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
...
'提取病人历史数据信息
    ...
    '时间范围
    If Val(txt显示天数.Text) <= 0 Then txt显示天数.Text = 1
    ...
    If strTime <> "" Then
        ...
        If Val(txt显示天数.Text) < 0 Then txt显示天数.Text = 0
    End If
    If CDate(strEnd) < CDate(strStart) Then strEnd = strStart
            ...
            With rsTemp
                ...
                If mrsItems.RecordCount > 0 Then
                    If Nvl(mrsItems!记录法, 2) = 1 Or lngItemNO = gint呼吸 Then
                        ...
                        Else
                            If strContent <> "" Then
                                 '四川地区要求，脉搏短绌的录入方式为：心率/脉搏
                                If lngItemNO = gint脉搏 And InStr(1, strContent, "/") > 0 Then
                                    strContent = Split(strContent, "/")(1) & "/" & Split(strContent, "/")(0)
                                End If
                            End If
                        End If
                    Else
...
'功能：初始化体温单批量录入过滤条件
    ...
    Else
        arrSel = Split(strSel, ";")
        strSel = strSel & String(IIf(4 - UBound(arrSel) < 0, 0, 4 - UBound(arrSel)), ";")
    End If
    arrSel = Split(strSel, ";")
    txtFilter.Tag = ""
    txtFilter.Text = ""
    strFilter = "全部;入院三天内的病人;手术三天内的病人;三天内体温存在超过37.5度的病人;危/重病人;转入三天内的病人"
    strFilterID = "0;1;2;3;4;5"
...
'-----------------------------------------------------------------------------------------------------------------
    ...
    If Val(arrFilter(4)) = 1 Then '转入三天内的病人
        ...
        strPatient = strPatient & _
            " SELECT 1 AS 性质,B.病人ID, B.主页ID, A.姓名, A.性别,B.年龄, B.住院号, lpad(nvl(B.出院病床,' '),10,' ') AS 床号,0 AS 婴儿" & vbNewLine & _
            " FROM 病人信息 A,病案主页 B,病人变动记录 C,在院病人 R" & vbNewLine & _
            ...
            " AND C.开始时间 BETWEEN [1] AND [2] And A.病人ID=R.病人ID And R.病区ID=[3]" & vbNewLine & _
             IIf(mlng科室id = -1, "", " And R.科室ID=[4]")
    End If
...
Private Sub dtpDate_Validate(Cancel As Boolean)
    ...
    End If
    mstrDate = Format(dtpDate.Value, "YYYY-MM-DD")
    If lngRow = VsfData.Row And lngRow >= VsfData.FixedRows And lngRow < VsfData.Rows Then
        Call RefreshHistoryData(lngRow)
    End If
End Sub
...
Private Sub VsfData_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    If mrsItems.RecordCount <> 0 Then
        ...
        ElseIf lngItemNO = gint脉搏 And mint心率应用 = 2 Then
            strInfo = strInfo & Space(4) & "脉搏短绌格式【心率/脉搏】如:120/100"
        ...
        If lngItemNO <> 0 Then
            ...
            Else
                ...
                If mrsCell.RecordCount > 0 Then
                    strPart = Trim(Nvl(mrsCell!部位))
                Else
                    If InStr(1, strText, ":") <> 0 And InStr(1, "," & gint体温 & "," & gint呼吸 & "," & gint脉搏 & ",", "," & lngItemNO & ",") > 0 Then
...
Private Sub VsfData_EnterCell()
    ...
    '如果病人选择的日期不在病人入院和出院日期之间不能编辑
    strTime = VsfData.TextMatrix(VsfData.Row, c日期)
    If strTime <> "" Then
        If InStr(1, strTime, ";") <> 0 Then strBegin = Format(Split(strTime, ";")(0), "YYYY-MM-DD")
        If InStr(1, strTime, ";") <> 0 Then strEnd = Format(Split(strTime, ";")(1), "YYYY-MM-DD")
        If Val(VsfData.TextMatrix(VsfData.Row, c出院)) = 0 Then
            strEnd = DateAdd("d", mintPreDays, CDate(strEnd))
        End If
        If Not (CDate(Format(dtpDate.Value, "YYYY-MM-DD")) >= CDate(strBegin) And CDate(Format(dtpDate.Value, "YYYY-MM-DD")) <= CDate(strEnd)) Then
            strInfo = "当前数据录入的日期[" & Format(dtpDate.Value, "YYYY-MM-DD") & "],不在病人入院时间[" & strBegin & "] 到 " & _
                IIf(Val(VsfData.TextMatrix(VsfData.Row, c出院)) = 0, "超期录入时间", "出院时间") & " [" & strEnd & "]有效范围内."
        ...
        If InStr(1, ",0,9,", "," & Val(rsObj!数据来源) & ",") = 0 Then
            ...
            Else
                If mintModify = 1 Then
                    ...
                    Else
                        strInfo = "同步过来的[" & strName & "]数据如果包含脉搏短绌不能进行修改."
                    End If
                    RaiseEvent AfterRowColChange(strInfo, True)
                    Exit Sub
                End If
...
ErrRow:
                ...
                If lngItemCode = gint脉搏 And mint心率应用 = 2 And InStr(1, strValue, "/") > 0 Then
...
Private Function CheckValid(strReturn As String, ByVal lngItemNO As Long, strInfo As String) As Boolean
    ...
    '对于数据来源<>0,9的 体温,脉搏数据 进行编辑(无物理降温和脉搏短绌可以录入物理降温,脉搏短绌)
    If InStr(1, ",0,9,", "," & mint数据来源 & ",") = 0 Then
        If lngItemNO = gint体温 Or (lngItemNO = gint脉搏 And mint心率应用 = 2) Then
            strValue = picInput.Tag
            If InStr(1, strFormat1, "/") <> 0 Then
                strFormat1 = Split(strFormat1, "/")(IIf(lngItemNO = 1, 0, 1))
            End If
            If InStr(1, strValue, "/") = 0 Then
                If Trim(strFormat1) <> Trim(strValue) Then
                    ...
                    Else
                        strInfo = "同步过来的[" & strName & "]数据只允许修改脉搏短绌部分,格式【心率/脉搏】."
                    End If
            ...
            Else
                If mintModify = 1 Then
                    If strFormat <> strValue Then
                        ...
                        Else
                            strInfo = "同步过来的[" & strName & "]数据如果包括脉搏短绌,不允许修改."
                        End If
                        txtInput.Text = strValue
                        Exit Function
                    End If
                Else
                    If strFormat1 <> Split(strValue, "/")(IIf(lngItemNO = 1, 0, 1)) Then
                        If lngItemNO = 1 Then
                            strInfo = "同步过来的[" & strName & "]数据只允许修改物理降温部分."
                        Else
                            strInfo = "同步过来的[" & strName & "]数据只允许修改脉搏短绌部分,格式【心率/脉搏】."
...
Private Sub vsfHistory_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If InStr(1, "," & gint体温 & "," & gint呼吸 & "," & gint脉搏 & ",", "," & lngItemNO & ",") > 0 Then
        If InStr(1, strText, ":") > 0 Then
            strPart = Trim(Split(strText, ":")(0))
            strText = Trim(Split(strText, ":")(1))
        End If
        If InStr(1, strOldValue, ":") > 0 Then strOldValue = Trim(Split(strOldValue, ":")(1))
    End If
            ...
            Else
                ...
                Else
                    If InStr(1, strOldValue, "/") <> 0 Then
                        strInfo = Split(strOldValue, "/")(IIf(lngItemNO = gint体温, 0, 1))
                    Else
                        strInfo = strOldValue
                    End If
                    ...
                    If strInfo <> "" Then
                        ...
                        Else
                            ...
                            Else
                                strText = Split(strText, "/")(0) & "/" & strInfo
                            End If
                        End If
                    End If
                End If
    ...
    mblnClearRow = False
    For intCOl = c时间 To VsfData.Cols - 1
