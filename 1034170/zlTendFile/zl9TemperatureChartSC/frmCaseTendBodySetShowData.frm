...
Private Function ChekPatientOut(ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal lng婴儿ID As Long) As Boolean
    ...
    strSQL = _
            "   SELECT 病人ID,主页ID,婴儿时间,DECODE(nvl(婴儿,0),0, DECODE(NVL(出院日期,''),'',0,1), DECODE(NVL(婴儿时间,''),'',0,1))记录" & vbNewLine & _
...
'---------------------------------------------------------------
    ...
    '四川地区要求，脉搏短绌的录入方式为：心率/脉搏(此处统一处理)
    mrsPoint.Filter = ""
    mrsPoint.Filter = "项目序号=" & gint脉搏
    Do While Not mrsPoint.EOF
        strTime = Nvl(mrsPoint!数值)
        If InStr(1, strTime, "/") > 0 Then
            mrsPoint!数值 = Split(strTime, "/")(1) & "/" & Split(strTime, "/")(0)
            mrsPoint.Update
        End If
    mrsPoint.MoveNext
    Loop
