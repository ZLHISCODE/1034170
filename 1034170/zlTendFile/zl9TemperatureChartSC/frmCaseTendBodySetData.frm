...
Begin VB.Form frmCaseTendBodySetData 
   ...
   Begin VB.Frame fraOper 
      ...
      Top             =   4005
      Width           =   5415
      Begin zl9TemperatureChartSC.VsfGrid vsfOper 
         Height          =   570
         Left            =   165
         TabIndex        =   46
         Top             =   285
   ...
   Begin VB.PictureBox picTab 
      ...
      Begin VB.Frame FraTable 
         ...
         Begin VB.PictureBox picColor 
            ...
            Visible         =   0   'False
            Width           =   1575
            Begin zl9TemperatureChartSC.ColorPicker usrColor 
               Height          =   2190
               Left            =   120
               TabIndex        =   29
               Top             =   -450
   ...
   Begin VB.PictureBox picCurve 
      ...
      Begin VB.Frame FraData 
         ...
         Begin VB.PictureBox picValue 
            ...
            Visible         =   0   'False
            Width           =   1575
            Begin zl9TemperatureChartSC.ColorPicker usrValue 
               Height          =   2190
               Left            =   120
               TabIndex        =   45
               Top             =   -360
         ...
         Begin VB.PictureBox pic未记 
            ...
            End
         End
         Begin zl9TemperatureChartSC.VsfGrid vsfCurve 
            Height          =   1215
            Left            =   120
            TabIndex        =   10
            Top             =   240
            ...
            Else '不存在记录就新增数据
                If Trim(strValue) <> "" Then
                    ...
                    gstrValues = GetMaxID & "|2)体温表格项目|" & strValue & "|" & strPart & "|" & _
                        0 & "|" & strTime & "|" & lngItemNO & "|" & strName & "|0||0|0|0|0|0|1|" & lngCol - vsfTab.FixedCols + 1
...
'-----------------------------------------------------------------------------------------------------------------
    ...
    If blnCurve = True Then
            ...
            Do While Not .EOF
                ...
                If mint心率应用 = 2 And lng项目序号 = -1 Then
                    ...
                    If mrsCurve.RecordCount > 0 Then
                        ...
                        '脉搏短绌时心率未未记说明只显示脉搏，脉搏为未记说明时就显示未记说明
        ...
        '四川地区要求，脉搏短绌的录入方式为：心率/脉搏(此处同一处理)
        mrsCurve.Filter = ""
        mrsCurve.Filter = "项目序号=" & gint脉搏
        Do While Not mrsCurve.EOF
            strTmp = Nvl(mrsCurve!数值)
            If InStr(1, strTmp, "/") > 0 Then
                mrsCurve!数值 = Split(strTmp, "/")(1) & "/" & Split(strTmp, "/")(0)
                mrsCurve.Update
            End If
        mrsCurve.MoveNext
        Loop
...
GoNext:
    ...
    If Trim(vsfCurve.TextMatrix(NewRow, COL_分组名)) = "1)体温曲线项目" Then
        Select Case lng项目序号
            ...
            Case 2
                If mint心率应用 = 2 And mblnEdit心率 Then strTmp = strTmp & Space(4) & "脉搏短拙录入格式【心率/脉搏】如：130/100"
        End Select
...
NotEdit:
            ...
            ElseIf lng项目序号 = 2 And mint心率应用 = 2 And mblnEdit心率 = True Then
                lblStb.Caption = "同步过来的[" & strName & "]数据只允许修改脉搏短绌部分,格式【心率/脉搏】."
            Else
        ...
        If lngNO = 2 And mint心率应用 = 2 And mblnEdit心率 = True Then
            If InStr(1, strInfo, "/") > 0 Then
                If Split(Trim(strInfo), "/")(1) = "" Or Split(Trim(strInfo), "/")(0) = "" Then
                    strMsg = strName & "数据录入错误" & Space(4) & "脉搏短绌:心率/脉搏"
                    GoTo ErrInfo
                Else
    ...
    If InStr(1, ",0,9,", "," & Val(marrDate(Row)) & ",") = 0 Then
        If Col = col_数据 Then
            If lngNO = 1 Or (lngNO = 2 And mint心率应用 = 2 And mblnEdit心率 = True) Then
                ...
                strText = strDate
                If InStr(1, strDate, "/") <> 0 Then
                    strDate = Split(strDate, "/")(IIf(lngNO = 1, 0, 1))
                End If
                If InStr(1, mArrValue(Row), "/") = 0 Then
                    If strDate <> mArrValue(Row) Then
                        ...
                        Else
                            strMsg = "同步过来的[" & strName & "]数据只允许修改脉搏短绌部分,格式【心率/脉搏】."
                        End If
                ...
                Else
                    If mArrModfy(Row) <> 0 Then
                        If strText <> mArrValue(Row) Then
                            If lngNO = 1 Then
                                strMsg = "同步过来的[" & strName & "]数据如果包括物理降温,不允许修改."
                            Else
                                strMsg = "同步过来的[" & strName & "]数据如果包括脉搏短绌,不允许修改."
                            End If
                        ...
                        End If
                    Else
                        If strDate <> Split(mArrValue(Row), "/")(IIf(lngNO = 1, 0, 1)) Then
                            If lngNO = 1 Then
                                strMsg = "同步过来的[" & strName & "]数据只允许修改物理降温部分."
                            Else
                                strMsg = "同步过来的[" & strName & "]数据只允许修改脉搏短绌部分,格式【心率/脉搏】.."
...
'--------------------------------------------------------
    ...
    '四川地区要求，脉搏短绌的录入方式为：心率/脉搏,保留HIS原有数据保存格式：脉搏/心率
    mrsCurve.Filter = ""
    mrsCurve.Filter = "项目序号=" & gint脉搏
    Do While Not mrsCurve.EOF
        strTmp = Nvl(mrsCurve!数值)
...
'----------------------------------------------------------------------------------------------------------------
    ...
    If str值域 = "" Then
        If int表示 <> 4 And blnAllow = True Then  '大便不进行检查
