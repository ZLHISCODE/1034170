...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    For lngPage = 1 To lngCountPage
        ...
        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), strTmp, 0)
...
'            Call DrawText(lngDC, strTmp, -1, T_LableRect, DT_CENTER)
        ...
        gstrFields = "时间," & adLongVarChar & ",20|项目序号," & adDouble & ",18|类型," & adDouble & ",2|" & _
            ...
            "禁用," & adInteger & ",1|显示," & adDouble & ",1|字体显示," & adDouble & ",1"
        ...
        With rsTmp
            Do While Not .EOF
                ...
                If (Not IsNull(!未记说明)) And zlCommFun.Nvl(!数值) <> "不升" Then
                    ...
                    gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用|显示|字体显示"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
                    gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & !项目序号 & "|99|" & _
                        !未记说明 & "|" & RGB_BLUE & "|" & SinX & "|0|0|0|0|" & zlCommFun.Nvl(!显示) & "|0"
...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
                ...
                If bytShow > 0 Then
                    ...
                    If rsNotes.BOF Then
                        gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & lng行号 & "|" & !记录类型 & "|" & _
                            str内容 & "|" & lngColor & "|" & SinX & "|0|0|0|0|0|" & IIf(!记录类型 = 4, "1", "0")
                        Call Record_Add(rsNotes, gstrFields, gstrValues)
                    Else
                    ...
                    If bytShow > 0 Then
                        ...
                        If rsNotes.BOF Then
                            gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & lng行号 & "|3|" & _
                                str内容 & "|" & RGB_RED & "|" & SinX & "|0|0|0|0|0|1"
                            Call Record_Add(rsNotes, gstrFields, gstrValues)
                        Else
                            rsNotes!时间 = Format(!时间, "yyyy-MM-dd HH:mm:ss")
                            rsNotes!内容 = str内容
                        ...
                        If bytShow > 0 Then
                            ...
                            If rsNotes.BOF Then
                                gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & lng行号 & "|13|" & _
                                    str内容 & "|" & RGB_RED & "|" & SinX & "|0|0|0|0|0|0"
                                Call Record_Add(rsNotes, gstrFields, gstrValues)
                            Else
                                rsNotes!时间 = Format(!时间, "yyyy-MM-dd HH:mm:ss")
                                rsNotes!内容 = str内容
        ...
        If Left(strTmpString0, 1) = ";" Then
            gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用|显示|字体显示"
...
Private Sub DrawBodyPageFooter(ByVal lngDC As Long, objDraw As Object, X As Long, Y As Long, ByVal LeftX As Long, ByVal intPageNo As Integer, _
    ...
    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), strNOPage, RGB_BLACK)
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), "日     期", RGB_BLACK)
'    Call DrawText(lngDC, "日     期", -1, T_LableRect, DT_CENTER)
    ...
    For i = 0 To UBound(arrTmpTime)
        ...
        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), CStr(arrTmpTime(i)), RGB_BLACK)
'        Call DrawText(lngDC, CStr(arrTmpTime(i)), -1, T_LableRect, DT_CENTER)
    ...
    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), strDay, RGB_BLACK)
'    Call DrawText(lngDC, strDay, -1, T_LableRect, DT_CENTER)
    ...
    For i = 0 To UBound(arrTmpDay)
        ...
        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), CStr(arrTmpDay(i)), RGB_BLUE)
'        Call DrawText(lngDC, CStr(arrTmpDay(i)), -1, T_LableRect, DT_CENTER)
    ...
    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), "手术后天数", RGB_BLACK)
'    Call DrawText(lngDC, "手术后天数", -1, T_LableRect, DT_CENTER)
    ...
    For i = 0 To UBound(arrOptDay)
        ...
        Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), CStr(arrOptDay(i)), lngColor)
'        Call DrawText(lngDC, CStr(arrOptDay(i)), -1, T_LableRect, DT_CENTER)
    ...
    Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), "时      间", RGB_BLACK)
'    Call DrawText(lngDC, "时      间", -1, T_LableRect, DT_CENTER)
    ...
    For i = 0 To 6
        ...
        For j = 0 To 5
            ...
            Call DrawTextPrint(objDraw, objDraw.ScaleX(T_LableRect.Left, vbPixels, vbTwips), objDraw.ScaleY(T_LableRect.Top, vbPixels, vbTwips), strTmp, lngColor)
'            Call DrawText(lngDC, strTmp, -1, T_LableRect, DT_CENTER)
        Next j
    Next i
...
Public Sub DrawRotateText(ByVal objDraw As Object, ByVal lngDC As Long, ByVal X As Single, _
    ...
    Else
        If strText = "—" Then
            X1 = X + objDraw.TextWidth(strText) / T_TwipsPerPixel.Y / 2 + T_DrawClient.列单位 / 2 + msngTwips '+ (T_DrawClient.列单位 - objDraw.TextWidth(strText) / T_TwipsPerPixel.X) / 2
        Else
           X1 = X + objDraw.TextHeight(strText) / T_TwipsPerPixel.Y + (T_DrawClient.列单位 - objDraw.TextHeight(strText) / T_TwipsPerPixel.X) / 2 + msngTwips
        End If
        X = X1
...
'        Call GetTextRect(objDraw, X, Y, strText, T_DrawClient.列单位, False, , sglScale)
        ...
        Call TextOut(lngDC, X, Y, strText, LenB(StrConv(strText, vbFromUnicode)))
...
Public Sub OutPutText(ByVal objDraw As Object, ByVal rsDrawItems As ADODB.Recordset, ByVal lngDC As Long, ByVal rsNote As ADODB.Recordset, ByVal mstrBeginDate As String, Optional ByVal sngScale As Single = 1)
    ...
    If Not TypeName(objDraw) = "Printer" Then
        msngTwips = 1
    End If
    ...
    With rsNote
        ...
        Dim k As Integer
        Dim N As Integer
        Dim bln标记显示 As Boolean
        Dim bln相同坐标 As Boolean
        Dim X坐标 As Long
        Dim lngOldX As Long, lngOutX As Long
        j = 1
        .Filter = "字体显示=1"
        ...
        N = 0
        ReDim arr标记(.RecordCount - 1) As String
        Do While Not .EOF
            If Not (!类型 = 2 Or !类型 = 99) Then
               arr标记(N) = !项目序号 & "|" & !X坐标
               N = N + 1
        ...
        Do While Not .EOF
            ...
            If strComment <> "" Then
                X = Val(IIf(Trim(!打印X坐标) <> "", !打印X坐标, !X坐标))
                 '问题号:53367,修改人：李涛，问题描述：体温单上的标志说明，卫生部要求必须显示在42-40度之间，但是现在的体温单标志显示不能满足规范
                
                bln相同坐标 = Get相同坐标(arr标记, !X坐标, !项目序号)
                If (Len(strComment) >= 11 And !字体显示 = 1) Or (bln标记显示 And !字体显示 = 1 And bln相同坐标 And k <= 1) Then
                    gstdSet.Name = "宋体"
                    gstdSet.Size = GetFontHeights(objDraw, strComment, sngScale)
                    Call SetFontIndirect(gstdSet, lngDC, objDraw)
                    lngFont = CreateFontIndirect(T_Font)
                    lngOldFont = SelectObject(lngDC, lngFont)
                Else
                ...
                T_Size.W = objDraw.TextWidth("我") / T_TwipsPerPixel.X
                    
                '入院，入科显示在同一列
                If bln标记显示 And !字体显示 = 1 And bln相同坐标 Then
                    If lngOldX <> !X坐标 Then
                        lngOldX = !X坐标
                        k = 0
                        j = 1
                        lngOutX = X - T_DrawClient.列单位 / 2
                    End If
                    If T_Size.W < T_DrawClient.列单位 Then
                        lngOutX = lngOutX - (T_DrawClient.列单位 - T_Size.W) / 2
                    End If
                    If k <= 1 Then
                        If j = 1 Then
                            X = lngOutX
...
Public Function GetYCoordinate(ByVal objDraw As Object, ByVal rsDrawItems As ADODB.Recordset, ByVal int项目序号 As Integer, ByVal strInput As String, Optional ByVal bln坐标 As Boolean = True, Optional lngDC As Long = 0, Optional ByVal blnOutput As Boolean = False) As String
    ...
    If bln坐标 Then
        ...
        sinScale = Format((rsDrawItems!最大值 - Val(strInput)) / rsDrawItems!单位值 * Val(Split(rsDrawItems!单位刻度, ",")(0)), "#0.00")
        GetYCoordinate = Format(sinCurY + sinScale, "#0.00")
    ...
    Else
        ...
        '(最大值-单位刻度*单位值)得到实际数据
        sinScale = Format((sinCurY - Split(rsDrawItems!最大值坐标, ",")(1)) / Val(Split(rsDrawItems!单位刻度, ",")(0)), "#0.00")
        GetYCoordinate = Format(rsDrawItems!最大值 - sinScale * rsDrawItems!单位值, "#0.00")
    End If
...
Private Function Get相同坐标(arrString() As String, ByVal X坐标 As Long, ByVal lng项目序号 As Long) As Boolean
    Dim i As Integer

    
    For i = 0 To UBound(arrString)
        If X坐标 = Split(arrString(i), "|")(1) And lng项目序号 <> Split(arrString(i), "|")(0) Then
            Get相同坐标 = True
            Exit For
        End If
    Next
...
'根据区域大小缩放字体大小
Private Function GetFontHeights(ByVal objDraw As Object, ByVal strTmp As String, Optional sngScale As Single = 1) As Long
    Dim FontHeigth As Long
    Dim strText As String
    Dim lngRectHeight As Long
    Dim intSize As Integer
    Dim i As Integer
    Dim sngD As Single
    Dim blnPrinter As Boolean
    ...
    For i = 1 To Len(strTmp)
        strText = Mid(strTmp, i, 1)
        FontHeigth = FontHeigth + IIf(Asc(strText) < 0, objDraw.TextHeight(strText), objDraw.TextHeight(strText) / 2)
    Next
    lngRectHeight = T_DrawClient.行单位 * (42 - 40) / 0.1 * sngScale
    
ErrGoTo:
    If FontHeigth / T_TwipsPerPixel.Y > lngRectHeight + T_DrawClient.行单位 / 2 Then
        sngD = Round((FontHeigth / T_TwipsPerPixel.Y - lngRectHeight) / lngRectHeight, 4)
        If sngD > 0 Then
            intSize = Format(Round((1 - sngD), 2) * intSize - 0.5, "#0")
            If intSize < 6 Then intSize = 6
            intSize = intSize * sngScale
            ...
            For i = 1 To Len(strTmp)
                ...
                FontHeigth = FontHeigth + IIf(Asc(strText) < 0, objDraw.TextHeight(strText), objDraw.TextHeight(strText) / 2)
            Next
            If (FontHeigth / T_TwipsPerPixel.Y) > lngRectHeight + T_DrawClient.行单位 / 2 And intSize > 6 Then GoTo ErrGoTo
        End If
    Else
        intSize = 9
    End If
    
    If blnPrinter = False And intSize = 6 Then intSize = 7
    
    GetFontHeights = intSize
    Exit Function
Errhand:
    If ErrCenter() = 1 Then
        Resume
...
Public Sub ReleaseFontIndirect(objDraw As Object)
    objDraw.Font.Name = "宋体"
    objDraw.Font.Size = 9
    objDraw.Font.Bold = False
