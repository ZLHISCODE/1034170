...
'-------------------------------------------------------------------------------------------------------
    ...
    With rsTemp
        Do While Not .EOF
            ...
            lngRowCount = lng最高行 + (Val(zlCommFun.Nvl(!最大值, 0)) - 36) / 0.1
        .MoveNext
        Loop
    End With
    ...
    For lngRow = 0 To T_DrawClient.总列数
        ...
        If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
            Call DrawLine(lngDC, lngCurX, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0, PS_SOLID, intLineMode), IIf(IIf(lngRow - lngRowCount > 0, (lngRow - lngRowCount) Mod 5, lngRow Mod 10) = 0 And sinRowStep >= 4 And bln粗线, intBold, intFine), RGB_BLACK)
        End If
    Next
...
'---------------------------------------------------------------------------------------
    ...
    With rsPiont
        Do While Not .EOF
            If lngX = Val(!X坐标) And lngY = Val(!Y坐标) Then
                strFields = "重叠标识|重叠数目|项目序号"
                rsConVerPoint.Filter = "重叠标识='" & lngX & "," & lngY & "'"
                ...
                Else
                    ...
                    Call Record_Update(rsConVerPoint, strFields, strValues, "重叠标识|" & lngX & "," & lngY)
                End If
...
Public Function GetXCoordinate(ByVal strInput As String, ByVal strBeginDate As String, Optional ByVal bln坐标 As Boolean = True) As String
    ...
    If bln坐标 Then
        ...
        SinX = T_DrawClient.体温区域.Left + (T_DrawClient.列单位 * (intDay * 6 + intTime))
        GetXCoordinate = SinX
    Else
        '计算得到相差多少个刻度
        SinX = Val(strInput)
...
Public Function GetYCoordinate(ByVal objDraw As Object, ByVal rsDrawItems As ADODB.Recordset, ByVal int项目序号 As Integer, ByVal strInput As String, Optional ByVal bln坐标 As Boolean = True, Optional lngDC As Long = 0, Optional ByVal blnOutput As Boolean = False) As String
    ...
    Dim lngCurX As Long, sinCurY As Single, lngScale As Long
    ...
    If bln坐标 Then
        ...
        lngScale = (rsDrawItems!最大值 - Val(strInput)) / rsDrawItems!单位值 * Val(Split(rsDrawItems!单位刻度, ",")(0))
        GetYCoordinate = sinCurY + lngScale
    ...
    Else
        ...
        '(最大值-单位刻度*单位值)得到实际数据
        lngScale = (sinCurY - Split(rsDrawItems!最大值坐标, ",")(1)) / Val(Split(rsDrawItems!单位刻度, ",")(0))
        GetYCoordinate = rsDrawItems!最大值 - lngScale * rsDrawItems!单位值
    End If
