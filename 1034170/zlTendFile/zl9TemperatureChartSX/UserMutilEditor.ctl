...
'功能：初始化体温单批量录入过滤条件
    ...
    Else
        arrSel = Split(strSel, ";")
        strSel = strSel & String(IIf(5 - UBound(arrSel) < 0, 0, 5 - UBound(arrSel)), ";")
    End If
    arrSel = Split(strSel, ";")
    txtFilter.Tag = ""
    txtFilter.Text = ""
    strFilter = "全部;入院三天内的病人;手术三天内的病人;三天内体温存在超过37.5度的病人;危/重病人;转入三天内的病人;一级及以上护理等级的病人"
    strFilterID = "0;1;2;3;4;5;6"
    ...
    If picFilter.Width < 2145 Then picFilter.Width = 2145
    lstFilter.Height = lstFilter.ListCount * IIf(mintBigSize = 0, 210, 270) + 30
    picFilter.Height = lstFilter.Height + cmdFilterOK.Height + 120
...
'-----------------------------------------------------------------------------------------------------------------
    ...
    If Val(arrFilter(5)) = 1 Then
        ...
        strPatient = strPatient & _
               ...
               " Where A.病人ID = b.病人ID And A.主页ID=B.主页ID And NVL(b.主页ID, 0) <> 0 " & _
               " AND Nvl(B.病案状态,0)<>5 AND B.封存时间 is NULL" & _
               " AND zl_PatitTendGrade(B.病人ID,B.主页ID)<=1 And A.病人ID=R.病人ID And R.病区ID=[3]" & _
               IIf(mlng科室id = -1, "", " And R.科室ID=[4]")
    End If
