...
Private Type Type_In
    ...
    lng护理等级 As Long
    blnMoved As Boolean
    bln婴儿显示出院 As Boolean
    bln入科显示入院 As Boolean
End Type
Private mIn As Type_In
...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    Dim strTmp As String, strTime As String, strTmp1 As String, strDepart As String
    ...
    Dim lngColor As Long, lngOtherColor As Long
    ...
    bln体温不升显示方式 = (Val(zlDatabase.GetPara("体温不升显示方式", glngSys, 1255, "0")) = 1)
    ...
    If rsTemp.BOF = False Then
        ...
        VarPatiInfo(7) = zlCommFun.Nvl(rsTemp("住院号").Value)
        VarPatiInfo(6) = Format(zlCommFun.Nvl(rsTemp("入院时间").Value), "yyyy.MM.dd")
        VarPatiInfo(2) = zlCommFun.Nvl(rsTemp("年龄").Value)
        VarPatiInfo(1) = zlCommFun.Nvl(rsTemp("性别").Value)
    End If
    ...
    If intBaby <> 0 Then
        ...
        If rsTemp.BOF = False Then
            ...
            VarPatiInfo(1) = zlCommFun.Nvl(rsTemp("婴儿性别").Value)
            VarPatiInfo(2) = "新生儿"
    ...
    For lngPage = 1 To lngCountPage
        ...
        If bln打印医院名称 = True Then
            ...
            lngFont = CreateFontIndirect(T_Font)
            lngOldFont = SelectObject(lngDC, lngFont)
            strTmp = IIf(GetUnitName = "-", "", GetUnitName) & IIf(intBaby <> 0, "婴儿", "")
            Call GetTextExtentPoint32(lngDC, strTmp, Len(strTmp), T_Size)
        ...
        '判断当前时间是否小于结束时间
        Dim strDateTime As String, strCurDate As String
        strCurDate = Format(zlDatabase.Currentdate, "YYYY-MM-DD HH:mm:ss")
        If CDate(Format(strEndDay, "YYYY-MM-DD HH:mm:ss")) > CDate(strCurDate) Then
            strDateTime = Format(strEndDay, "YYYY-MM-DD HH:mm:ss")
        Else
            strDateTime = strCurDate
        End If
        ...
        VarPatiInfo(5) = ""
        strTmp = "": strTime = ""
        strSQL = " Select  c.名称 As 科室,b.名称 As 病区, b.编码 As 病区编码,a.床号,a.开始原因 " & _
                    " From 病人变动记录 a,部门表 b,部门表 c " & _
                    " Where a.病人id=[1] And a.主页id=[2] And a.科室id Is Not Null And a.病区id=b.id and a.科室id=c.id " & _
                    " And nvl(a.附加床位,0)=0 and a.开始时间-4/24<=[3] And Nvl(a.终止时间," & strDateTime & ")>=[4] Order By a.开始时间"
        ...
        '获取当前页的房间号
        VarPatiInfo(4) = GetPatiRoom(lng病人ID, lng主页ID, intBaby, CDate(Format(strTmpDay, "YYYY-MM-DD HH:mm:ss")))
...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
        ...
        With mIn
            ...
            .lng护理等级 = lng护理等级
            .bln入科显示入院 = bln入科显示入院
            .bln婴儿显示出院 = bln婴儿体温单显示出院
        End With
        Call GetInOutInfo(rsNotes)
        ...
        If Left(strTmpString0, 1) = ";" Then
            ...
            For i = 0 To UBound(Split(strTmpString0, ";"))
                ...
                If rsNotes.RecordCount > 0 Then
                    rsNotes!内容 = IIf(bln体温不升显示方式, "↓ ", "不升") & ";" & zlCommFun.Nvl(rsNotes!内容)
                    rsNotes.Update
                Else
        ...
        '8.心率脉搏短轴连线
        rsPoints.Filter = ""
        If strTmp <> "" And bln打印脉搏短绌 = True Then
            Call CreatePoly(rsPoints, objDraw, lngDC, strTmpDay, strTmp)
        End If
        '9输出说明信息
        '  先处理未及说明和下标说明
        ...
        '61924:刘鹏飞,2013-07-08,要求固定项目也显示为蓝色(同呼吸的颜色)
        rsItems.Filter = ""
        rsItems.Filter = "项目序号=" & gint呼吸
        If rsItems.RecordCount > 0 Then
            lngOtherColor = Val(Nvl(rsItems!记录色, RGB_BLUE))
        Else
            lngOtherColor = RGB_BLUE
        End If
        rsItems.Filter = ""
...
ErrNext:
            ...
            Else
                ...
                For j = 0 To 6
                    ...
                    With rsDownTab
                        Do While Not .EOF
                            lngColor = lngOtherColor
                            str结果 = zlCommFun.Nvl(!记录内容)
                            intCOl = Val(!序号)
                            intCOl = intCOl + j * int频次
                            If blnColor Then lngColor = Val(zlCommFun.Nvl(!未记说明, lngOtherColor))
        ...
        If bln显示皮试 = True Then
            ...
            '61924:刘鹏飞,2013-07-08,山西省体温单要求将皮试结果显示为药物过敏
            ArrNewString(UBound(ArrNewString)) = "-999;药物过敏" & ";" & strNewTmpString
        End If
...
'-------------------------------------------------------------------------------------
    ...
    Dim blnPrinter As Boolean
    Dim intBold As Integer, intFine As Integer
    Dim bln符号 As Boolean, lngWith As Long
    On Error GoTo ErrHand
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    VarPatiInfo = Split(strPatiInfo, "'")
    bln是否输出诊断 = (UBound(VarPatiInfo) > 7)
    'strPatiName = "姓名:'性别:'年龄:'入院日期:'住院号:'科室:'床号:" & IIf(bln是否输出诊断 = True, "'诊断:", "")
    strPatiName = "姓名:'性别:'年龄:'科别:'病室;'床号:'入院日期:'住院病历号:" & IIf(bln是否输出诊断 = True, "'诊断:", "")
    ...
    For i = 0 To UBound(VarPatiInfo)
        ...
        If lngCurX > lngLeft And i <> 0 Then
            If Not (UBound(VarPatiInfo) = 8 And (Pname_SIZE(i).cx - lngX) < lngWidth / 2) Then
...
OutPutPatiInfo:
    ...
    For i = 0 To UBound(VarPatiInfo)
        ...
        '诊断内容如果过多在下一行显示剩余部分
        If UBound(VarPatiInfo) = 8 And i = UBound(VarPatiInfo) Then
            strText1 = ""
...
Public Sub DrawRotateText(ByVal objDraw As Object, ByVal lngDC As Long, ByVal X As Single, _
    ...
    ElseIf strText <> "—" And Asc(strText) > 0 Then
        '正常输出数字
        Call GetTextRect(objDraw, X, Y, strText, T_DrawClient.列单位, False, , sglScale)
        Call DrawText(lngDC, strText, -1, T_LableRect, DT_CENTER)
         '反转90度输出字体
        ...
        Do While Not .EOF
            If Trim(zlCommFun.Nvl(!内容)) <> "" Then
                If Not (!类型 = 2 Or !类型 = 99) Then
                    ...
                    !高度 = GetNumberHeiht(lngDC, zlCommFun.Nvl(!内容))
                    .Update
        ...
        Do While Not .EOF
            ...
            If strComment <> "" Then
                ...
                For i = 1 To Len(strComment)
                    ...
                    If blnNum = True And IsNumeric(strText) Then
                    
                    Else
                        ReDim Preserve ArrString(UBound(ArrString) + 1)
                    End If
                    If blnNum = True And IsNumeric(strText) Then
                        ArrString(UBound(ArrString)) = ArrString(UBound(ArrString)) & strText
                        blnNum = False
                    Else
                        If IsNumeric(strText) Then
                            ArrString(UBound(ArrString)) = strText
                            blnNum = True
                        Else
                            ArrString(UBound(ArrString)) = strText
                            blnNum = False
                ...
                For i = 0 To UBound(ArrString)
                    If Y < T_DrawClient.刻度区域.Bottom Then
                        ...
                        Call GetTextExtentPoint32(lngDC, ArrString(i), Len(ArrString(i)), T_Size)
                        
                        If Asc(ArrString(i)) < 0 Or IsNumeric(ArrString(i)) Then
                        ...
                        '输出字体信息
                        Call DrawRotateText(objDraw, lngDC, X, Y, ArrString(i), !颜色, sngScale)

                        If Asc(ArrString(i)) < 0 Then
                            Y = Y + T_Size.H
                            intAscCharNum = 0
                        ElseIf Asc(ArrString(i)) > 0 Then
                            Y = Y + T_Size.H
                            intAscCharNum = 0
                        Else
                            Y = Y + T_Size.H / 2
...
Public Function GetNumberHeiht(ByVal lngDC As Long, ByVal strComment As String) As Double
    '------------------------------------------------------------------------------------
    '获取数字时间字符串高度
    '------------------------------------------------------------------------------------
    Dim intAscCharNum As Integer
    ...
    For i = 1 To Len(strComment)
        ...
        If Asc(strText) < 0 Then
            Y = Y + T_Size.H
            intAscCharNum = 0
        ElseIf Asc(strText) > 0 And IsNumeric(strText) Then
            Y = Y + T_Size.H
            intAscCharNum = 0
        Else
            Y = Y + T_Size.H / 2
...
'根据参数获取体温时间颜色
'---------------------------------------------
    '61924:刘鹏飞,2013-07-08,山西省体温单不分白班夜班,时间统一显示为黑色
...
Public Function GetInOutInfo(ByRef rsNotes As ADODB.Recordset)
        Dim bytShow As Byte
        Dim str内容 As String
        ...
        If mIn.blnMoved Then
            strSQL = Replace(strSQL, "病人护理文件", "H病人护理文件")
            strSQL = Replace(strSQL, "病人护理数据", "H病人护理数据")
            strSQL = Replace(strSQL, "病人护理明细", "H病人护理明细")
        End If

        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "读取手术、上下标等信息", mIn.lng文件ID, mIn.lng病人ID, mIn.lng主页ID, Int(CDate(mIn.strTmpDay)), CDate(mIn.strEndDay), mIn.intBaby, mIn.lng护理等级)
        With rsTmp
            Do While Not .EOF
                bytShow = 1
                ...
                If bytShow > 0 Then
                    SinX = Val(GetXCoordinate(Format(!时间, "YYYY-MM-DD HH:mm:ss"), mIn.strTmpDay))
        ...
        '1-入院；2-入科；3-转科；4-换床
        gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
        Set rsTmp = GetDataFromHis(mIn.lng病人ID, mIn.lng主页ID, mIn.intBaby, CDate(mIn.strTmpDay), CDate(mIn.strEndDay), 2)
        With rsTmp
            Do While Not .EOF
                If Trim(zlCommFun.Nvl(!内容)) <> "" Then
                    bytShow = 0
                    ...
                    Case 8
                        ...
                        If mIn.intBaby > 0 Then
                            bytShow = IIf(mIn.bln婴儿显示出院, bytShow, 0)
                        End If
                    Case 9
                    ...
                    If bytShow > 0 Then
                        If lng行号 = 9 And mIn.bln入科显示入院 = True Then str内容 = "入院"
        ...
        If mIn.intBaby > 0 Then
            gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用"  '入出转缺省是红色,上下标及未记说明缺省是蓝色
            Set rsTmp = GetDataFromHis(mIn.lng病人ID, mIn.lng主页ID, mIn.intBaby, CDate(mIn.strTmpDay), CDate(mIn.strEndDay), 3)
            With rsTmp
                Do While Not .EOF
                    bytShow = 0
                    If Trim(zlCommFun.Nvl(!内容)) <> "" Then
...
Public Function GetPatiRoom(ByVal lngPatID As Long, ByVal lngPageId As Long, ByVal lngBaby As Long, ByVal dtBegin As Date) As String
'功能:提取病人房间号,体温单每页显示自之前的最后一次房间号
    Dim RS As New ADODB.Recordset
    Dim strPeriod As String
    On Error GoTo ErrHand
    
    '59887:刘鹏飞,2013-07-08,提取房间号
    If lngBaby = 0 Then
    ...
    End If
    Set RS = zlDatabase.OpenSQLRecord(gstrSQL, "取入院日期或出生日期", lngPatID, lngPageId, lngBaby)

    If Format(dtBegin, "yyyy-MM-dd HH:mm:ss") < Format(RS!开始时间, "yyyy-MM-dd HH:mm:ss") Then
        strPeriod = Format(RS!开始时间, "yyyy-MM-dd HH:mm:ss")
    Else
        strPeriod = Format(dtBegin, "yyyy-MM-dd HH:mm:ss")
    End If
       
    '提取病人房间号
    gstrSQL = "Select  A.房间号" & vbNewLine & _
            " From (Select B.房间号, A.开始时间, Nvl(A.终止时间, To_Date('3000-01-01', 'yyyy-mm-dd')) As 终止时间" & vbNewLine & _
            "            From 病人变动记录 A,床位状况记录 B" & vbNewLine & _
            "            Where A.病人id = [1] And A.主页id = [2] And A.病区ID=B.病区ID And A.床号=B.床号 And NVl(A.附加床位,0)=0) A" & vbNewLine & _
            " Where [3] >= A.开始时间" & vbNewLine & _
            " Order By A.开始时间 DESC"
    Set RS = zlDatabase.OpenSQLRecord(gstrSQL, "提取病人房间号", lngPatID, lngPageId, CDate(strPeriod))
    If RS.RecordCount > 0 Then
        strPeriod = Nvl(RS!房间号)
    Else
        strPeriod = ""
    End If
    GetPatiRoom = strPeriod
