...
'
    ...
    For intItem = 1 To lstColumnUsed.ListItems.Count
        ...
        For intRow = mobjVsf.FixedRows To mobjVsf.Rows - 1
            If Val(Split(mobjVsf.TextMatrix(intRow, COL_tab字符串), ",")(5)) = 2 Then
                If lngItemCode = Val(mobjVsf.TextMatrix(intRow, COL_tab项目序号)) And strItemName = Split(mobjVsf.TextMatrix(intRow, COL_tab项目名称), "(")(0) Then
                    blnAdd = False
                    Exit For
                End If
            End If
        ...
        If blnAdd = True Then
            ...
            For i = LBound(arrTmp) To UBound(arrTmp)
                ...
                If Val(varCode(2)) = lngItemCode Then
                    ...
                    mobjVsf.TextMatrix(intRow, COL_tab项目名称) = strItemName
                    '定位到新添加的行
    ...
    For intRow = mobjVsf.FixedRows To mobjVsf.Rows - 1
        ...
        If Val(Split(mobjVsf.TextMatrix(intRow, COL_tab字符串), ",")(5)) = 2 Then
            ...
            strItemName = Split(mobjVsf.TextMatrix(intRow, COL_tab项目名称), "(")(0)
            blnDelete = True
            For intItem = 1 To lstColumnUsed.ListItems.Count
...
'---------------------------------------------------------------------
    ...
    For intRow = mobjVsf.FixedRows To mobjVsf.Rows - 1
        If Val(Split(mobjVsf.TextMatrix(intRow, COL_tab字符串), ",")(5)) = 2 Then
            strText = ""
            strText = "" & mobjVsf.TextMatrix(intRow, COL_tab项目序号) & " 项目序号,'" & Split(mobjVsf.TextMatrix(intRow, COL_tab项目名称), "(")(0) & "' 项目名称,1 标识"
    ...
    mstrSQL = "" & _
            ...
            "                           Nvl(a.记录频次, 2) 记录频次,A.入院首测, b.分组名, b.项目表示,b.项目性质,a.单位 项目单位" & vbNewLine & _
        ...
        Do While Not .EOF
            ...
            If strItemCode = "" Then
                ...
                mstrActiveItem = zlCommFun.Nvl(!分组名, "2)体温表格项目") & "'" & str值域 & "," & zlCommFun.Nvl(!项目类型) & "," & _
                    ...
                    zlCommFun.Nvl(!项目序号) & "'" & Replace(zlCommFun.Nvl(!项目名称) & IIf(zlCommFun.Nvl(!项目单位, "") = "", "", "(" & !项目单位 & ")"), ";", ":")

            Else
                If InStr(1, "," & strItemCode & ",", "," & !项目序号 & ",") = 0 Then
                    strItemCode = strItemCode & "," & !项目序号
            ...
            If !标识 = 1 Then
                ...
                lstItem.SubItems(1) = Replace(zlCommFun.Nvl(!项目名称), ";", ":")
