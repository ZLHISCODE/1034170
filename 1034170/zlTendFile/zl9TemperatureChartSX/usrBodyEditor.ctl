   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Begin VB.PictureBox picDraw 
            ...
            Width           =   7335
         End
         Begin zl9TemperatureChartSX.VsfGrid vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   26
            Top             =   3480
...
'功能:获取病人信息
    Dim rsTmp As New ADODB.Recordset
    Dim strTmp As String, strTime As String, strStart As String, strTo As String, strDepart As String
    Dim intCOl As Integer
    Dim bln入科显示入院 As Boolean
    Dim strDateTime As String, strCurData As String
    On Error GoTo hErr
    ...
    '判断当前时间是否小于结束时间
    strCurData = Format(zlDatabase.Currentdate, "YYYY-MM-DD HH:mm:ss")
    If CDate(Format(mstr结束时间, "YYYY-MM-DD HH:mm:ss")) > CDate(strCurData) Then
        strDateTime = Format(mstr结束时间, "YYYY-MM-DD HH:mm:ss")
    Else
        strDateTime = strCurData
    End If
    ...
    mstrSQL = _
            " Select  c.名称 As 科室,b.名称 As 病区,b.编码 As 病区编码,a.床号,a.开始原因 " & _
            " From 病人变动记录 a,部门表 b,部门表 c " & _
            " Where a.病人id=[1] And a.主页id=[2] And a.科室id Is Not Null And a.病区id=b.id " & _
            "   And a.科室id=c.id And nvl(a.附加床位,0)=0 and a.开始时间-4/24<=[3] And Nvl(a.终止时间," & strDateTime & ")>=[4] Order By a.开始时间"
    ...
    '提取房间号
    txtCard(8).Text = GetPatiRoom(Val(T_Patient.lng病人ID), Val(T_Patient.lng主页ID), Val(T_Patient.lng婴儿), CDate(mstr开始时间))
...
'---------------------------------------------------------------------
    ...
    If mbln显示皮试 And mintRepairRows > Val(mshDownTab.Tag) And mintRepairRows > 0 Then
        ...
        mshDownTab.TextMatrix(intRow, 0) = strPace & "药物过敏" & strPace
    End If
...
ErrNext:
    ...
    '61924:刘鹏飞,2013-07-08,要求固定项目也显示为蓝色(同呼吸相同)
    'Debug.Print "数据开始---" & Now
    '提取表格项目数据信息
    With mshDownTab
        .Cell(flexcpForeColor, .FixedRows, 4, Val(.Tag) - 1, .Cols - 1) = Val(vsf.Tag)
        lngItemCode = 0
        str项目名称 = ""
            ...
            If str项目名称1 & ";" & .RowData(intRow) <> str项目名称 & ";" & lngItemCode Then
                ...
                For intDay = 0 To 6
                    ...
                    Do While Not rsDownTab.EOF
                        ...
                        lngColor = Val(vsf.Tag)
                        If blnColor Then lngColor = Val(zlCommFun.Nvl(rsDownTab!未记说明, lngColor))
...
Private Sub picCard_Resize(Index As Integer)
    On Error Resume Next
    txtCard(8).Move txtCard(8).Left, txtCard(8).Top, picCard(Index).Width - txtCard(8).Left - 45
...
Private Sub SaveMemory(ByVal blnAutoAdjust As Boolean)
    ...
    Dim bln婴儿体温单显示出院 As Boolean, bln入科显示入院 As Boolean, bln体温不升显示方式 As Boolean
    ...
    If Left(strTmpString0, 1) = ";" Then
        ...
        For i = 0 To UBound(Split(strTmpString0, ";"))
            ...
            mrsNote.Sort = "项目序号"
            If mrsNote.RecordCount > 0 Then
                mrsNote!内容 = IIf(bln体温不升显示方式 = True, "↓ ", "不升") & IIf(bytTag = 1, ";", " ") & zlCommFun.Nvl(mrsNote!内容)
                mrsNote.Update
            Else
