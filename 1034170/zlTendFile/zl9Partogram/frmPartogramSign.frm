...
Private Function Validation() As Boolean
    ...
    If chkEsign.Value = vbChecked Then
        ...
        End If
        lngCertID = 0
        strSign = objESign.signature(mstrSource, UCase(gcnOracle.Properties(23)), lngCertID, str时间戳, , str时间戳信息) '返回签名信息,lngCertID返回签名使用的证书记录ID
        If strSign = "" Then
...
Private Sub RefControls()
    ...
    Else
        ...
        Else
            mlngPassType = 0
        End If
        If mlngPassType = 1 And CertificateStoped(gstrUserName) = True Then mlngPassType = 0
    End If
...
'功能：检查签名人的证书是否停用，停用的话将不使用电子签名
    ...
    Err.Clear
    Set objSign = CreateObject("zl9ESign.clsESign")
    If Err <> 0 Then Err = 0
    
    If objSign Is Nothing Then Exit Function
    Call objSign.Initialize(gcnOracle, glngSys)
    CertificateStoped = objSign.CertificateStoped(strName)
