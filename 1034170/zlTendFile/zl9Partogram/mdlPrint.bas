...
'---------------------------------------------------------------------------------------------------
    ...
    For i = lngFileIndex To lngFileCount
        ...
        For j = lngPageIndex To mintPageCount
            ...
            '页脚图形输出
            If blnPrint = True Then
                Call frmPartogramRead.PrintRTBData(mobjDraw, False, lngBottom, "第 " & j & " 页" & IIf(lngFileIndexCount > 1, "(" & i & ")", ""))
            End If
...
Private Function ReadStruDef() As Boolean
    ...
    With rsTemp
        ...
        '程序内部控制增加固定列
        mstrSQL中 = UCase(mstrSQL中)
        mstrSQL内 = UCase(mstrSQL内)
        mstrSQL列 = UCase(mstrSQL列)
