...
Begin VB.Form frmPartogramRead 
   ...
   EndProperty
   Icon            =   "frmPartogramRead.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   3750
   ...
   Begin VB.PictureBox picBack 
      ...
      Begin RichTextLib.RichTextBox rtbHead 
         ...
         OLEDragMode     =   0
         OLEDropMode     =   0
         TextRTF         =   $"frmPartogramRead.frx":000C
      ...
      Begin RichTextLib.RichTextBox rtbFoot 
         ...
         OLEDragMode     =   0
         OLEDropMode     =   0
         TextRTF         =   $"frmPartogramRead.frx":00A9
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "宋体"
            Size            =   10.5
            Charset         =   134
   ...
   End
End
Attribute VB_Name = "frmPartogramRead"
...
Public Function InitRechBox(ByVal lngFileReadID As Long) As Boolean
    ...
    strSQL = "Select  /*+ RULE */ 格式, 页脚, 种类||'-'||编号 AS KEY From 病历页面格式" & _
        "   Where 种类 = 3 And 编号 = (select 页面 from 病历文件列表 where ID=[1])"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "读取病历页面格式", lngFileReadID)
...
Public Function PrintRTBData(ByVal objOutTo As Object, ByVal blnHead As Boolean, ByVal lngCurY As Long, Optional ByVal strPage As String = "") As Boolean
    ...
    objOutTo.CurrentY = objOutTo.Height - objOutTo.ScaleX(Val(lngCurY * sinTwipsPerPixelY / 56.7), vbMillimeters, vbTwips)
    
    If blnHead Then
        objOutTo.Print ""
    Else
        lngFoot = 180
        objOutTo.CurrentX = (objOutTo.Width - 90 * LenB(StrConv(strPage, vbFromUnicode))) / 2
        objOutTo.Print strPage
    End If
    ...
    With rcDrawTo
        If blnHead Then
            ...
            .Bottom = objOutTo.ScaleX(Val(lngCurY * sinTwipsPerPixelY), vbMillimeters, vbTwips) - 30
        Else
            .Left = lngOffsetLeft
            .Top = objOutTo.Height - objOutTo.ScaleX(lngCurY * sinTwipsPerPixelY / conRatemmToTwip, vbMillimeters, vbTwips)
            .Right = objOutTo.Width - lngOffsetLeft
            .Bottom = objOutTo.Height
        End If
    End With
