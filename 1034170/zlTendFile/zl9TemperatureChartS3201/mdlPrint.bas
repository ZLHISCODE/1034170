...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    '求得首列宽，求左边标尺总共有多少行
    '求得体温表项目的总行数
    strSQL = "Select Max((nvl(A.最大值,0)-nvl(A.最小值,0))/Decode(A.单位值,0,1,A.单位值)+nvl(A.最高行,0)) as 总高度 " & _
    ...
    For lngPage = 1 To lngCountPage
        ...
        '3201医院要求 blnStopFlag=false 如果发生3次手术 格式为:1/8/14,只显示1/8,不显示第一次手术信息
        For i = 1 To 7
            j = 1
            Do While True
                If InStr(j, strOpValue(i), "/") <> 0 Then
                    If j = 1 Then
                        j = InStr(j, strOpValue(i), "/") + 1
                    Else
                        j = InStr(j, strOpValue(i), "/")
                        Exit Do
                    End If
                Else
                    j = Len(strOpValue(i)) + 1
                    Exit Do
                End If
            Loop
            strOpValue(i) = Mid(strOpValue(i), 1, j - 1)
        Next i
        ...
        For i = 1 To 7
            If DateDiff("d", Int(CDate(strTmpDay)), Int(CDate(strEndDay))) + 1 >= i Then
                If Trim(strOpdays(i)) <> "" And lng次数 < 12 Then
                    ...
                    Else
                        If Trim(strOpValue(i)) <> "" Then
                            strOpValue(i) = strOpValue(i) ' strOpValue(i) & "/" & strTmp
                        Else
                            strOpValue(i) = "" ' strTmp
                        End If
                    End If
            ...
            Do While Not .EOF
                ...
                If (Not IsNull(!未记说明)) And zlCommFun.Nvl(!数值) <> "不升" Then
                    ...
                    gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & !项目序号 & "|99|" & _
                        !未记说明 & "|" & RGB_RED & "|" & SinX & "|0|0|0|0|" & zlCommFun.Nvl(!显示)
        ...
        With rsTmp
            Do While Not .EOF
                ...
                If !记录类型 = 4 Then
                    ...
                    lngColor = RGB_RED
                Else
                    lngColor = IIf(Not IsNumeric(Nvl(!未记说明)), RGB_RED, Val(Nvl(!未记说明)))
                End If
