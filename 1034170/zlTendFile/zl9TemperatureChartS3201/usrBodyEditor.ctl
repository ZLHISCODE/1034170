...
Begin VB.UserControl usrBodyEditor 
   ...
   Begin VB.PictureBox picBack 
      ...
      Begin VB.PictureBox picMain 
         ...
         Top             =   120
         Width           =   9375
         Begin zl9TemperatureChartS3201.VsfGrid Vsf 
            Height          =   255
            Left            =   120
            TabIndex        =   26
            Top             =   3480
...
Public Function zlMenuClick(ByVal strMenuItem As String, Optional ByVal strParam As String) As Boolean
    ...
    Select Case strMenuItem
        ...
        Case "装载数据"
            ...
            If strStartDate & ";" & strEndDate <> picMain.Tag Then mblnRedraw = True
        ...
        Case "体温数据显示设置"
            ...
            If mstr开始时间 <> "" Then
                ...
                If frmCaseTendBodySetShowData.ShowEdit(UserControl.Extender.ParentForm, strInput, CDate(strTime), CDate(Format(mstrEndDate, "YYYY-MM-DD HH:mm:ss")), mint心率应用, mblnMoved) = True Then
                    '保存成功后刷新体温单显示
                    strParam = mstrParam2
...
'               intCOl = (lblCur.Left - mshUpTab.ColWidth(0) - mshUpTab.Left - ((mshUpTab.ColWidth(1) - lblCur.Width) / 2)) / mshUpTab.ColWidth(1) + 1
                ...
                strInput = T_Patient.lng病人ID & ";" & T_Patient.lng主页ID & ";" & T_Patient.lng文件ID & ";" & T_Patient.lng婴儿 & ";" & T_Patient.lng科室ID & ";" & T_Patient.lng护理等级
                If frmCaseTendBodySetData.ShowEditor(UserControl.Extender.ParentForm, strInput, strTime, mstr开始时间, mint心率应用, mblnMoved) = True Then
                    '保存成功后刷新体温单显示
...
'---------------------------------------------------------------------
    ...
    '加载固定表格项目
    Set rsTemp = GetGridItem(T_Patient.lng护理等级, IIf(T_Patient.lng婴儿 = 0, 1, 2), T_Patient.lng科室ID)
    With rsTemp
        If rsTemp.RecordCount > 0 Then
            rsTemp.MoveFirst
            mshDownTab.Rows = 0
    ...
    '加载活动有数据的表格项目
    If mstr开始时间 <> "" And mstr结束时间 <> "" Then
        Set rsTemp = GetAppendGridItem(T_Patient.lng文件ID, T_Patient.lng病人ID, T_Patient.lng主页ID, T_Patient.lng护理等级, T_Patient.lng婴儿, Int(CDate(mstr开始时间)), CDate(mstr结束时间), IIf(T_Patient.lng婴儿 = 0, 1, 2), T_Patient.lng科室ID, 2, mblnMoved)
        Call AppenGridItem(rsTemp)
    End If
...
'----------------------------------------------------------------
    ...
    With mshUpTab
        ...
        If picMain.Tag <> "" Then
            ...
            With picDisplay
                .Left = ((((intMinCol - 1) \ 6) + 1) * 6 - 1) * mshUpTab.ColWidth(intMinCol) + mshUpTab.ColWidth(0)
...
'        Next i
        ...
        '3201医院要求 mblnStopFlag=false 如果发生3次手术 格式为:1/8/14,只显示1/8,不显示第一次手术信息
        For i = 1 To 7
            j = 1
            Do While True
                If InStr(j, mstrOpValue(i), "/") <> 0 Then
                    If j = 1 Then
                        j = InStr(j, mstrOpValue(i), "/") + 1
                    Else
                        j = InStr(j, mstrOpValue(i), "/")
                        Exit Do
                    End If
                Else
                    j = Len(mstrOpValue(i)) + 1
                    Exit Do
                End If
            Loop
            mstrOpValue(i) = Mid(mstrOpValue(i), 1, j - 1)
        Next i
        ...
        For i = 1 To 7
            If DateDiff("d", mstr开始时间, mstr结束时间) + 1 >= i Then
                If Trim(mstrOpdays(i)) <> "" And lng次数 < 12 Then
                    lng次数 = lng次数 + 1
                    strTmp = Switch(lng次数 = 1, "Ⅰ", lng次数 = 2, "Ⅱ", lng次数 = 3, "Ⅲ", lng次数 = 4, "Ⅳ", lng次数 = 5, "Ⅴ", lng次数 = 6, "Ⅵ", lng次数 = 7, "Ⅶ", lng次数 = 8, "Ⅷ", lng次数 = 9, "Ⅸ", lng次数 = 10, "Ⅹ", lng次数 = 11, "Ⅺ", lng次数 = 12, "Ⅻ")
        ...
        '设定手术 分娩文本颜色
        mshUpTab.Cell(flexcpForeColor, 2, mshUpTab.FixedCols, 2, mshUpTab.Cols - 1) = 16711680 ' 255
...
Private Function ShowDowntab() As Boolean
    ...
    '提取病人体温表格记录
    gstrSQL = " SELECT C.ID,a.发生时间 As 时间,C.显示,C.记录内容 As 结果,C.体温部位,C.未记说明,nvl(C.数据来源,0) 数据来源," & _
        ...
        "   AND D.项目序号=C.项目序号  AND c.记录类型=1  AND E.项目序号=D.项目序号 " & _
        "   AND nvl(E.护理等级,0)>=[8]  AND A.发生时间 BETWEEN [4] And [5] And C.终止版本 Is Null AND D.记录法=2 "
    
    '提取非体温表格的汇总项目
    strSQL = "  SELECT C.ID,a.发生时间 As 时间,C.显示,C.记录内容 As 结果,C.体温部位,C.未记说明,nvl(C.数据来源,0) 数据来源," & _
    ...
    gstrSQL = "Select ID,时间,显示,结果,体温部位,未记说明,数据来源,项目名称,项目序号,来源ID,共用,项目性质 From (" & _
...
ErrNext:
    ...
    For i = 1 To 42
        ...
        '3201医院规则如下：要求呼吸机R要顶格书写，不分上下。呼吸机R后的第1次自主呼吸记录也要居上表示。翻页后第一次呼吸也居上。
        If Val(str结果) > 0 Then
            ...
            If int呼吸表格输出格式 = 0 Then '顺序上下显示
                ...
                Else
                    If strPart <> "呼吸机" Then
                        Vsf.ColData(intCOl) = 0
                        intAlign = flexAlignCenterBottom
                    Else
                        intAlign = flexAlignCenterTop
                        Vsf.ColData(intCOl) = 1
                        blnValue = False
            ...
            Vsf.Cell(flexcpAlignment, 1, intCOl, 1, intCOl) = intAlign
        End If
    Next i
            ...
            If str项目名称1 & ";" & .RowData(intRow) <> str项目名称 & ";" & lngItemCode Then
                ...
                For intDay = 0 To 6
                    ...
                    Set rsDownTab = ReturnItemRecord(rsTemp, Int(CDate(strBegin)), CDate(mstrEnterDate), lngItemCode & ";" & str项目名称 & ";" & _
                                int记录频次 & ";" & int项目表示 & ";" & int项目性质 & ";" & int入院首测, bln汇总当天)
...
Private Sub cboBaby_Click()
    ...
    mstrSQL = "select A.ID from 病人护理文件 A,病历文件列表 B" & _
       "    where A.病人ID=[1] and A.主页Id=[2] and nvl(A.婴儿,0)=[3] and A.科室ID=[4] and A.格式ID=B.ID and B.种类=3 and B.保留=-1"
    If mblnMoved = True Then
        mstrSQL = Replace(mstrSQL, "病人护理文件", "H病人护理文件")
    End If
    Set RS = zlDatabase.OpenSQLRecord(mstrSQL, "提取文件ID", T_Patient.lng病人ID, T_Patient.lng主页ID, T_Patient.lng婴儿, T_Patient.lng科室ID)
...
Public Sub Paint_Reset()
    ...
    gstrFields = "项目序号," & adDouble & ",18|最大值," & adDouble & ",18|最小值," & adDouble & ",18|" & "单位值," & adDouble & ",18|最大值坐标," & adLongVarChar & ",20|最小值坐标," & adLongVarChar & ",20|" & "单位刻度," & adLongVarChar & ",20|显示模式," & adDouble & ",5|颜色," & adDouble & ",18"
...
Public Sub Paint_Canvas(Optional ByVal blnAdjust As Boolean = False)
    ...
    With rsTemp
        Do While Not .EOF
            ...
            lngMaxRows = lng最高行 + (Val(zlCommFun.Nvl(!最大值, 0)) - Val(zlCommFun.Nvl(!最小值, 0))) / 0.1 + 10
            If Val(zlCommFun.Nvl(!最小值, 0)) >= 35 Then
                lngMaxRows = lngMaxRows + 10
            End If
    ...
    lngMaxY = 4 * lngInitRowStep + T_DrawClient.总列数 * sigRowStepNew + T_DrawClient.偏移量Y '（为表格大小，还需加上起始Y坐标,上面固定留上6行为输出时间信息）

    If blnAdjust = False And Not mblnRedraw Then mblnRefresh = False: Exit Sub
    ...
    If blnAdjust Then
        ...
        If lngMaxY > mlng高度 Then
            lngMaxY = mlng高度 - 4 * lngInitRowStep
        ...
        '计算刻度的最大坐标
        lngMaxY = T_DrawClient.总列数 * IIf(blnDoubleRow, sinRowStep, sigRowStepNew) + T_DrawClient.偏移量Y + lngInitRowStep * 4
    ...
    'If Not mblnRedraw Then mblnRefresh = False: Exit Sub
...
'    Next
    ...
    For lngRow = 0 To T_DrawClient.总列数
        ...
        If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
            Call DrawLine(mlngMemDC, lngCurX + 1, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0, PS_SOLID, intLineMode), IIf(lngRow Mod 10 = 0 And sigRowStepNew >= 4 And bln粗线, 2, 1), RGB_FleetGRAY)
        End If
    Next
    ...
    With rsTemp
        Do While Not .EOF
            ...
            '如果是最小模式,从第30行开始输出标识
            'If blnDoubleRow Then lngCurY = lngCurY + lngInitRowStep * 4
            ...
            Do While True
                ...
                If Format(sin刻度, "#0.00") = Format(intAlertness, "#0.00") And Not blnDoubleRow Then
                    ...
                    Call DrawLine(mlngMemDC, T_DrawClient.体温区域.Left, lngCurY, lngMaxX, lngCurY, intLineMode, 1, RGB_RED)
                End If
                ...
                '如果不在有效范围内，或者超出画布则退出
                If Val(Format(sin刻度, "#0.00")) = Val(Format(!最小值, "#0.00")) Or lngCurY > T_DrawClient.刻度区域.Bottom Then
                    ...
                    gstrValues = zlCommFun.Nvl(!项目序号) & "|" & zlCommFun.Nvl(!最大值) & "|" & zlCommFun.Nvl(!最小值) & "|" & dbl单位值 & "|" & str最大值坐标 & "|" & str最小值坐标 & "|" & T_DrawClient.行单位 & "," & T_DrawClient.列单位 & "|" & intTens_digit & "|" & !记录色
            ...
            If Val(zlCommFun.Nvl(!状态)) <> 3 Then
                ...
                If !重叠 = 0 Then
                    If dbl数值 <= dblMinValue Then
                        Call DrawLine(mlngMemDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标 + T_DrawClient.列单位, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标, PS_SOLID, 1, lngRGB, True)
                    End If
            ...
            If Val(zlCommFun.Nvl(!状态)) <> 3 Then
                If !项目序号 = gint体温 And !标记 = 1 Then
                    ...
                    If Val(!数值) > Val(str原值) Then
                        ...
                        '现在失败也为虚线(医院要求)
                        Call DrawLine(mlngMemDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标, sinX坐标原 + T_DrawClient.列单位 / 2, sinY坐标原, PS_DOT, 1, RGB_RED, True)
...
Private Sub SaveMemory(ByVal blnAutoAdjust As Boolean)
            ...
            If mrsItems.RecordCount > 0 Then
                ...
                If (Not IsNull(!未记说明)) And zlCommFun.Nvl(!数值) <> "不升" Then
                    ...
                    gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & !项目序号 & "|99|" & _
                        !未记说明 & "|" & RGB_RED & "|" & SinX & "|0|0|0|0|" & Val(zlCommFun.Nvl(!显示))
    ...
    gstrSQL = "" & _
             ...
             " AND C.记录类型 <> 1  AND A.发生时间 BETWEEN [4]  And [5]"
    ...
    Dim bytTag As Byte
    bytTag = Val(zlDatabase.GetPara("未记说明显示位置", glngSys, 1255, "0"))
    str符号 = ""
    '处理体温不升 体温不升始终显示在 35 度下面
    If Left(strTmpString0, 1) = ";" Then
        ...
        For i = 0 To UBound(Split(strTmpString0, ";"))
            ...
            mrsNote.Sort = "项目序号"
            If mrsNote.RecordCount > 0 Then
                mrsNote!内容 = "不升" & ";" & zlCommFun.Nvl(mrsNote!内容)
                mrsNote.Update
            Else
