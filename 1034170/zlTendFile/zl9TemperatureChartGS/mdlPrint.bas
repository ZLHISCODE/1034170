...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    If bln体温单显示诊断 Then
        ReDim Preserve VarPatiInfo(UBound(VarPatiInfo) + 1)
        '提取病人诊断信息
        strSQL = "Select Zl_Replace_Element_Value([1],[2],[3],2,NULL,0,[4]) As 最后诊断 From Dual"
        Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "最后诊断", "最后诊断", lng病人ID, lng主页ID, CDate(strBeginDate))
        If rsTemp.BOF = False Then
            If intBaby = 0 Then
                VarPatiInfo(UBound(VarPatiInfo)) = zlCommFun.Nvl(rsTemp("最后诊断").Value)
            Else
    ...
    strSQL = "Select Rownum-1 序号 ,项目序号,项目名称,记录色,项目单位,项目值域, 部位,记录频次,入院首测,项目性质,项目表示,项目类型 From (" & _
            ...
            "   where A.项目序号=B.项目序号 and A.项目序号<>5  order by 项目性质,Decode(A.项目序号,3 ,0,1 ),A.排列序号)"
    ...
    '求得首列宽，求左边标尺总共有多少行
    '求得体温表项目的总行数
    strSQL = "Select Max((nvl(A.最大值,0)-nvl(A.最小值,0))/Decode(A.单位值,0,1,A.单位值)+nvl(A.最高行,0)) as 总高度 From 体温记录项目 A Where A.记录法=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlPrint", 1)
    If rsTmp.RecordCount > 0 Then
        rsTmp.MoveFirst
        intDrawLineRows = zlCommFun.Nvl(rsTmp!总高度, 0)
    Else
        CloseRs rsTmp
        GoTo ErrPrint
    End If
    ...
    strSQL = "Select nvl(A.最大值,0) 最大值,nvl(A.最小值,0) 最小值 ,nvl(A.单位值,0.1) ,nvl(A.最高行,0)-2  最高行 From 体温记录项目 A Where A.项目序号=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlPrint", gint体温)
    If rsTmp.RecordCount > 0 Then
        intDrawLineRows = (Val(rsTmp!最大值) - Val(rsTmp!最小值)) / 0.1 + IIf(Val(rsTmp!最高行) < 0, 0, Val(rsTmp!最高行)) + 10
        If Val(rsTmp!最小值) >= 35 Then
            intDrawLineRows = intDrawLineRows + 10
        End If
    End If
    ...
    For lngPage = 1 To lngCountPage
        ...
        strTime = ""
        '显示但前段的手术标记
        strSQL = "Select B.发生时间 时间,C.项目名称" & vbNewLine & _
            " From 病人护理文件 A,病人护理数据 B,病人护理明细 C" & vbNewLine & _
        ...
        For i = 1 To 7
            If DateDiff("d", Int(CDate(strTmpDay)), Int(CDate(strEndDay))) + 1 >= i Then
                If Trim(strOpdays(i)) <> "" And lng次数 < 12 Then
                    lng次数 = lng次数 + 1
                    strTmp = Switch(lng次数 = 1, "Ⅰ", lng次数 = 2, "Ⅱ", lng次数 = 3, "Ⅲ", lng次数 = 4, "Ⅳ", lng次数 = 5, "Ⅴ", lng次数 = 6, _
...
'                        End If
'                    End If
                '中医院要求手术当天显示手术类型名称 如：手术 第一次为手术 第二次为手术II
                strOpValue(i) = strOpdays(i) & IIf(strTmp = "Ⅰ", "", strTmp)
                End If
            End If
        Next i
        ...
        If T_DrawClient.行单位 > 7 * msngTwips Then T_DrawClient.行单位 = 7 * msngTwips
        If T_DrawClient.行单位 < 5 * msngTwips Then T_DrawClient.行单位 = 5 * msngTwips
        ...
        str体温说明 = DrawCanvas(lngDC, objDraw, rsTemp, rsDrawItems, sngScale)
        Call SelectObject(lngDC, lngOldFont)
        Call DeleteObject(lngFont)
...
'        End With
        ...
        strSQL = "" & _
                 ...
                 " AND c.记录类型 <> 1  AND B.发生时间 BETWEEN [4]  And [5]"
        If blnMoved Then
            strSQL = Replace(strSQL, "病人护理文件", "H病人护理文件")
            strSQL = Replace(strSQL, "病人护理数据", "H病人护理数据")
            strSQL = Replace(strSQL, "病人护理明细", "H病人护理明细")
...
'                    rsNotes!内容 = "不升" & ";" & zlCommFun.Nvl(rsNotes!内容)
'                    rsNotes.Update
'                Else
...
'        End If
        ...
        '8.心率脉搏短轴连线
        rsPoints.Filter = ""
        If strTmp <> "" Then Call CreatePoly(rsPoints, objDraw, lngDC, strTmpDay, strTmp)
        '9输出说明信息
        '  先处理未及说明和下标说明
        ...
        '提取所有表格项目数据信息
        strSQL = "SELECT C.Id,a.发生时间 As 时间,C.显示,C.记录内容 As 结果,C.体温部位,C.未记说明,nvl(C.数据来源,0) 数据来源," & vbNewLine & _
        ...
        strSQL = _
            "   Select ID,时间,显示,结果,体温部位,未记说明,数据来源,项目名称,项目序号,来源ID,共用,项目性质 From (" & strSQL & ")" & _
            ...
            Else
                ...
                For j = 0 To 6
                    ...
                    Set rsDownTab = ReturnItemRecord(rsTmp, Int(CDate(strBegin)), CDate(strBeginDate), lng项目序号 & ";" & str项目名称 & ";" & _
                                    int频次 & ";" & Val(strTmp) & ";" & int项目性质 & ";" & int入院首测, bln汇总当天)
                    ...
                    If blnDataTrue = True Then
                        ...
                        For intCOl = 0 To UBound(arrTmpString1)
                            ...
                            Else  '开始进行画表格线
                                ...
                                If blnOutText = True Then Call DrawTabText(lngDC, objDraw, strTmp, -1, T_LableRect, DT_CENTER, LPoint, sngScale)
    ...
    With rsPoint
        Do While Not .EOF
            ...
            If Not (zlCommFun.Nvl(!项目序号) = gint体温 And Val(zlCommFun.Nvl(!标记)) = 1) Then
                ...
                If Val(zlCommFun.Nvl(!数值)) >= dblMaxValue Then
                ...
                ElseIf Val(zlCommFun.Nvl(!数值)) <= dblMinValue Then
                    Call DrawLine(lngDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标 + T_DrawClient.行单位 * 2, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标, PS_SOLID, intFine, lngRGB, True)
                End If
            Else
                '体温的物理降温
                ...
                If Val(!数值) > Val(dblvalue) Then
                    ...
                    '现在失败也为虚线(医院要求)
                    Call DrawLine(lngDC, !X坐标 + T_DrawClient.列单位 / 2, !Y坐标, SinX + T_DrawClient.列单位 / 2, sinY, IIf(blnPrinter, PS_DASH, PS_DOT), 1, RGB_RED, True)
...
Public Function DrawCanvas(ByVal lngDC As Long, ByVal objDraw As Object, ByVal rsTemp As ADODB.Recordset, rsDrawItems As ADODB.Recordset, Optional sngScale As Single = 1) As String
'------------------------------------------------------------------------------------------------------
...
'-------------------------------------------------------------------------------------------------------
    ...
    Dim intAlertness  As Integer             '警戒线,起辅助作用
    ...
    lngMaxY = 4 * lngInitRowStep + T_DrawClient.总列数 * sinRowStep + T_DrawClient.偏移量Y '（为表格大小，还需加上起始Y坐标）
    ...
    For lngRow = 0 To T_DrawClient.总列数
        ...
        If ((blnDoubleRow Or bln双行) And lngRow Mod 2 = 0) Or (Not blnDoubleRow And Not bln双行) Then
            Call DrawLine(lngDC, lngCurX, lngCurY, lngMaxX, lngCurY, IIf(lngRow Mod 10 = 0, PS_SOLID, intLineMode), IIf(lngRow Mod 10 = 0 And sinRowStep >= 4 And bln粗线, intBold, intFine), RGB_BLACK)
        End If
    Next
        ...
        Do While Not .EOF
            ...
            Select Case !项目序号
                ...
                Case gint体温  '体温整数时输出刻度
                    ...
                    dbl单位值 = 0.1
                    intAlertness = zlCommFun.Nvl(!警示线, 37)
                ...
                Case gint脉搏, gint心率  '脉搏/心跳按10的倍数输出刻度
                    ...
                    dbl单位值 = 2
                    intAlertness = zlCommFun.Nvl(!警示线, 52)
                ...
                Case gint呼吸  '呼吸按5的倍数输出刻度
                    ...
                    intAlertness = zlCommFun.Nvl(!警示线, 0)
            ...
            '赋初值
            lngCurY = lngCurY + (lngInitRowStep * 4)   '固定前4行的高度不输出刻度
            ...
            Do While True
                ...
                If bln显示刻度 = True Then sinBegin刻度 = sinBegin刻度 - IIf(T_DrawClient.双倍, sin刻度间隔 * 2, sin刻度间隔)
                If sinBegin刻度 < 0 Then sinBegin刻度 = 0
                
                '辅助线或警示线必须输出
                If Format(sin刻度, "#0.00") = Format(intAlertness, "#0.00") And Not blnDoubleRow Then
                    '非最小显示模式的情况下才输出警戒辅助线（如果当前行能整除10，说明输出的是粗线）
                    Call DrawLine(lngDC, T_DrawClient.体温区域.Left, lngCurY, lngMaxX, lngCurY, intLineMode, intBold, RGB_RED)
                End If
                If bln显示刻度 Then
                ...
                End If
                '如果不在有效范围内，或者超出画布则退出
                If Val(Format(sin刻度, "#0.00")) = Val(Format(zlCommFun.Nvl(!最小值), "#0.00")) Or lngCurY > T_DrawClient.刻度区域.Bottom Then
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    For i = 0 To UBound(arrOptDay)
        lngCurX = T_DrawClient.刻度区域.Right + i * 6 * T_DrawClient.列单位
        Call SetTextColor(lngDC, RGB_RED)
...
Public Function GetCurveDate(ByVal intCOl As Integer, _
    ...
    strBegin = strBegin & " " & Split(varTime(lng列号), ",")(0)
    strEnd = strEnd & " " & Split(varTime(lng列号), ",")(1)
...
Public Function GetGridItem(ByVal int护理等级 As Integer, ByVal byt适用病人 As Byte, ByVal lng科室ID As Long, Optional int项目性质 As Integer = 1) As ADODB.Recordset
    ...
    '提取表格项目
    gstrSQL = "Select A.排列序号,A.项目序号,A.记录名,A.记录法,A.记录符,A.记录色,A.最大值,A.最小值,A.单位值,nvl(A.记录频次,2) 记录频次,A.入院首测,B.项目性质," & _
        "   B.项目类型,B.项目长度,B.项目表示,B.项目小数,B.项目单位 单位" & _
        "   From 体温记录项目 A,护理记录项目 B,诊治所见项目 C" & _
        "   Where A.项目序号=B.项目序号 And B.项目ID=C.Id(+) And A.记录法=2 And B.项目性质=[4]" & _
...
Public Function GetAppendGridItem(ByVal lng文件ID As Long, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal int护理等级 As Integer, ByVal int婴儿 As Long, dt开始时间 As Date, dt结束时间 As Date, ByVal byt适用病人 As Byte, ByVal lng科室ID As Long, Optional int项目性质 As Integer = 2, Optional blnMove As Boolean = False) As ADODB.Recordset
    '**************************************************************************
    '功能:提取活动有数据的体温表格项目
    '**************************************************************************
    Dim rsTemp As New ADODB.Recordset
    ...
    Set rsTemp = GetGridItem(int护理等级, byt适用病人, lng科室ID, int项目性质)
    ...
    gstrSQL = "Select distinct D.排列序号,D.项目序号,C.体温部位,C.体温部位 || D.记录名  记录名,D.记录法,D.记录符,D.记录色,D.最大值,D.最小值,D.单位值,nvl(D.记录频次,2) 记录频次,D.入院首测," & _
        ...
        "   AND d.记录法=2 and D.项目序号=F.项目序号  order by D.排列序号"
    ...
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSQL, "", lng文件ID, lng病人ID, lng主页ID, int护理等级, int婴儿, dt开始时间, dt结束时间)
...
'            If RS.BOF = False Then lng诊疗项目id = zlCommFun.Nvl(RS("ID").Value)
            ...
            If lng婴儿 <> 0 Then
                '提取婴儿医嘱记录(转科,出院,死亡)
                strNewSql = _
                    "   select 科室,开始执行时间,decode(操作类型,3,'转出',5,'出院','死亡') 内容,Decode(操作类型,'3',3,8) 行号 From (" & vbNewLine & _
...
Public Function ReturnItemRecord(ByVal rsCollect As ADODB.Recordset, ByVal dtDate As Date, ByVal dtBegin As Date, _
    ByVal strEditor As String, ByVal bln汇总当天 As Boolean, Optional ByVal blnEdit As Boolean = False) As ADODB.Recordset
'---------------------------------------------------------------------------------------------------------------------------------------------
...
'      bln汇总昨天 参数：汇总、波动项目显示(True)当天数据,(false)昨天数据  blnEdit 是否是编辑状态（备用参数）
'---------------------------------------------------------------------------------------------------------------------------------------------
    Dim rsDayData As New ADODB.Recordset
    Dim rsTemp As New ADODB.Recordset
    Dim DtDay As Date
    Dim intType As Integer, intHour As Integer, int类别 As Integer, int序号 As Integer
    ...
    Dim lngNO As Long
    Dim i As Integer, intCount As Integer, intColFirst As Integer
    Dim bln波动 As Boolean
    ...
    strFileds = "ID," & adDouble & ",18|时间," & adLongVarChar & ",20|项目序号," & adDouble & ",18|项目名称," & adLongVarChar & ",20|记录内容," & adLongVarChar & ",100|" & _
        "体温部位," & adLongVarChar & ",20|未记说明," & adLongVarChar & ",100|数据来源," & adDouble & ",1|显示," & adDouble & ",1|来源ID," & adDouble & ",18|共用," & adDouble & ",1|序号," & adDouble & ",1"
    Call Record_Init(rsDayData, strFileds)
    strFileds = "ID|时间|项目序号|项目名称|记录内容|体温部位|未记说明|数据来源|显示|来源ID|共用|序号"
    ...
    With mrsTabTime
        ...
        Do While Not .EOF
            ...
            If int项目表示 = 4 Then '汇总项目
                ...
                If rsCollect.RecordCount > 0 Then
                    rsCollect.MoveFirst
                    If lngID = 0 Then lngID = Val(Nvl(rsCollect!Id))
                End If
                
                If int项目性质 = 2 Then '活动项目按部位统计自身
                    If lngID <> 0 Then
                        strValues = lngID & "|" & CDate(strCenter) & "|" & lngItemNo & "|" & strName & "|" & _
                                IIf(dblData = 0, "", dblData) & "|" & strName & "|" & int未记说明 & "|" & _
                                int数据来源 & "|" & 1 & "|" & lng来源ID & "|" & int共用 & "|" & int序号
                ...
                Do While Not rsTemp.EOF
                    ...
                    If Val(Nvl(rsTemp!序号, 0)) <> lngItemNo Then
                        rsCollect.Filter = 0
                        rsCollect.Filter = "项目序号=" & Val(Nvl(rsTemp!序号, 0)) & " And 数据来源<>9 " & " And " & strFind
                        Do While Not rsCollect.EOF
                            dblData = dblData + Val(Nvl(rsCollect!结果))
                            If lng来源ID = 0 Then
                                If InStr(1, ",0,9,", "," & Val(Nvl(rsCollect!数据来源)) & ",") = 0 Then
                                ...
                                Case 1 '第一条
                                    If rsCollect.RecordCount > 0 Then rsCollect.MoveFirst
                                        strValues = Val(Nvl(rsCollect!Id)) & "|" & CDate(rsCollect!时间) & "|" & Val(Nvl(rsCollect!项目序号)) & "|" & Nvl(rsCollect!项目名称) & "|" & _
                                            ...
                                            Val(Nvl(rsCollect!显示)) & "|" & Val(Nvl(rsCollect!来源ID)) & "|" & Val(Nvl(rsCollect!共用)) & "|" & int序号
                                    Exit Do
                                Case 2 '中间一条
                                    strValues = Val(Nvl(rsCollect!Id)) & "|" & CDate(rsCollect!时间) & "|" & Val(Nvl(rsCollect!项目序号)) & "|" & Nvl(rsCollect!项目名称) & "|" & _
                                            Nvl(rsCollect!结果) & "|" & Nvl(rsCollect!体温部位) & "|" & Nvl(rsCollect!未记说明) & "|" & Val(Nvl(rsCollect!数据来源)) & "|" & _
