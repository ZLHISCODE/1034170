...
Public Function PrintOrPreviewBodyState(objOut As Object, _
    ...
    For lngPage = 1 To lngCountPage
        ...
        '62576:刘鹏飞,2013-06-08,修改体温单第一页无法显示门诊诊断
        If bln体温单显示诊断 Then
            '提取诊断的最小时间
...
'        '处理体温不升 体温为不升需要在35度下纵向输出体温不升二字
        ...
        gstrFields = "时间," & adLongVarChar & ",20|项目序号," & adDouble & ",18|类型," & adDouble & ",2|" & _
            ...
            "Y坐标," & adDouble & ",20|高度," & adDouble & ",20|打印X坐标," & adDouble & ",20|" & _
            "禁用," & adInteger & ",1|显示," & adDouble & ",1"
        Call Record_Init(rsPoint, gstrFields)
        rsNotes.Filter = "类型=99 and 内容='△'"
        rsNotes.Sort = "项目序号"
        gstrFields = "时间|项目序号|类型|内容|颜色|X坐标|Y坐标|高度|打印X坐标|禁用"
        With rsNotes
            Do While Not .EOF
                gstrValues = Format(!时间, "yyyy-MM-dd HH:mm:ss") & "|" & !项目序号 & "|99|" & _
                            !内容 & "|" & RGB_BLUE & "|" & !X坐标 & "|0|0|0|0|" & Val(zlCommFun.Nvl(!显示))
                Call Record_Add(rsPoint, gstrFields, gstrValues)
                rsNotes.Delete
                rsNotes.MoveNext
            Loop
        End With
        ...
        '输出内容
        Call OutText(rsPoint, rsDrawItems, lngDC, objDraw)
...
Public Sub SQLDIY(strSQL As String, ByVal blnMoved As Boolean)
    If blnMoved Then
        strSQL = Replace(strSQL, "病人护理文件", "H病人护理文件")
        strSQL = Replace(strSQL, "病人护理数据", "H病人护理数据")
        strSQL = Replace(strSQL, "病人护理明细", "H病人护理明细")
...
Private Sub OutText(ByVal rsPoint As ADODB.Recordset, ByVal rsDrawItems As ADODB.Recordset, ByVal lngDC As Long, ByVal objDraw As Object)
'-----------------------------------------------------
'未记说明的外出和拒测以"△"显示在37刻度线上。
'-----------------------------------------------------
    Dim SinY37 As Single
    Dim Y As Long
    Dim X As Long
    Dim lngHeight As Long
    Dim strText As String
    SinY37 = GetYCoordinate(objDraw, rsDrawItems, gint体温, 37, lngDC)
    With rsPoint
        If .RecordCount > 0 Then .MoveFirst
        Do While Not .EOF
            X = Val(!X坐标)
            ...
            Y = SinY37
            '输出字体信息
            If T_DrawClient.行单位 < 3 Then
                lngHeight = 5
            Else
                lngHeight = T_DrawClient.行单位
            End If
            Call DrawRotateText(objDraw, lngDC, X, Y - lngHeight, strText, !颜色)
            .MoveNext
        Loop
    End With
End Sub
