...
Begin VB.Form frmDiagItem 
   ...
   Begin TabDlg.SSTab SSTab 
      ...
      TabCaption(2)   =   "适用范围(&2)"
      TabPicture(2)   =   "frmDiagItem.frx":0A30
...
'        Me.Height = 7740
    ...
    '名称等填写
    gstrSql = "select ID,编码,名称,说明,Nvl(适用范围,0) As 适用范围 " & _
            " From 疾病诊断目录" & _
            " Where ID=[1] "
...
Private Sub txtItem_KeyUp(Index As Integer, KeyCode As Integer, Shift As Integer)
    Select Case Index
    Case con诊断名称
        Me.txtItem(con名称拼音码).Text = zlGetSymbol(Trim(Me.txtItem(Index).Text), 0)
        Me.txtItem(con名称五笔码).Text = zlGetSymbol(Trim(Me.txtItem(Index).Text), 1)
    Case con其他别名
        Me.txtItem(con别名拼音码).Text = zlGetSymbol(Trim(Me.txtItem(Index).Text), 0)
        Me.txtItem(con别名五笔码).Text = zlGetSymbol(Trim(Me.txtItem(Index).Text), 1)
    End Select
End Sub
