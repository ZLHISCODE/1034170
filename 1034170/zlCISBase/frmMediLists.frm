...
Private mintNodeIndex As Integer    '用来记录被点击的分类
...
Private Sub mnuClassStar_Click()
    ...
    Call zlRefClasses
    Me.tvwClass.Nodes(mintNodeIndex).Selected = True
    mstrKey = ""
    Call tvwClass_NodeClick(tvwClass.Nodes(mintNodeIndex))
...
Private Sub mnuEditStart_Click()
    Dim rsTemp As ADODB.Recordset
    Dim rs是否有库存 As ADODB.Recordset
...
'            End If
    ...
    Else
        With Me.lvwSpecs
            ...
            gstrSql = "Select 1 From 药品规格 Where 药库分批 = 1 And 药房分批 = 0 and 药品id=[1]"
            Set rsTemp = zldatabase.OpenSQLRecord(gstrSql, "分批属性", Mid(.SelectedItem.Key, 2))
            
            gstrSql = "Select 1 From 药品库存 Where  药品id=[1] and 性质=1"
            Set rs是否有库存 = zldatabase.OpenSQLRecord(gstrSql, "是否有库存", Mid(.SelectedItem.Key, 2))
            
            If rsTemp.RecordCount > 0 And rs是否有库存.RecordCount = 0 Then
                ...
                Set rsTemp = zldatabase.OpenSQLRecord(gstrSql, "是否有部门性质同时设置了药库药房")
                If rsTemp.RecordCount > 0 Then
                    MsgBox "当前有部门的工作性质同时设置了药库药房，启用该规格后请同时设置药库药房分批或不分批！", vbInformation, gstrSysName
                    mbln启用 = True
                    Call mnuEditSpecMod_Click
                    mbln启用 = False
                End If
            End If
        End With
...
Private Sub tvwClass_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    mintNodeIndex = tvwClass.SelectedItem.Index
...
'        Me.tlbThis.Buttons("Spec").Visible = True
    ...
    If Me.tvwClass.Tag >= 3 Then
       ...
       If Val(Me.tvwClass.Tag) = 4 Then
            ...
            arrDrugID = GetArrayByStr(mstrDrugId, 4000, ",")
            
            For i = 0 To UBound(arrDrugID)
                str药名ID = arrDrugID(i)
                Set rsTemp = zldatabase.OpenSQLRecord(gstrSql, Me.Caption, mstrType, str药名ID)
                
                With rsTemp
                    Do While Not .EOF
                        For n = 1 To tvwClass.Nodes.Count
                            If tvwClass.Nodes.Item(n).Key = "_" & !ID Then
                                Exit For
                            Else
                                If n = tvwClass.Nodes.Count Then
    ...
    gstrSql = "select ID,上级ID,编码,名称,简码,Nvl(To_Char(撤档时间, 'YYYY-MM-DD'), '3000-01-01') 撤档时间 " & _
            ...
            " start with 上级ID is null" & IIf(mnuViewList.Checked = False, " And Nvl(To_Char(撤档时间, 'YYYY-MM-DD'), '3000-01-01') = '3000-01-01' ", "") & _
            " connect by prior ID=上级ID " & IIf(mnuViewList.Checked = False, " And Nvl(To_Char(撤档时间, 'YYYY-MM-DD'), '3000-01-01') = '3000-01-01' ", "") & _
            " Order By Level, 编码 "
...
Public Sub zlRefRecords(Optional lngItem As Long)
    ...
    If Val(Me.tvwClass.Tag) <= 2 Then
        ...
        Else
            gstrSql = gstrSql & " I.分类ID IN " & _
                ...
                "  Start With ID=[1] " & IIf(mnuViewList.Checked = False, " And Nvl(To_Char(撤档时间, 'YYYY-MM-DD'), '3000-01-01') = '3000-01-01' ", "") & _
                " Connect By Prior ID=上级ID " & IIf(mnuViewList.Checked = False, " And Nvl(To_Char(撤档时间, 'YYYY-MM-DD'), '3000-01-01') = '3000-01-01' ", "") & ")"
        End If
        If Me.mnuViewStoped.Checked = False Then
            gstrSql = gstrSql & " and (I.撤档时间 is null or I.撤档时间=to_date('3000-01-01','YYYY-MM-DD'))"
        End If
    ...
    Else        '单独处理过滤结果
        ...
        gstrSql = "select I.ID,I.编码,I.名称,I.计算单位,T.药品剂型," & _
                ...
                "        decode(T.是否辅助用药,1,'是',' ') as 是否辅助用药," & _
                "        decode(nvl(t.抗生素,0),0,'',1,'非限制使用',2,'限制使用','特殊使用') as 抗菌药物 " & _
                " from 诊疗项目目录 I,药品特性 T" & IIf(mnuViewList.Checked = False, ",诊疗分类目录 Z", "") & _
                " where I.ID=T.药名ID " & IIf(Val(Me.tvwClass.Tag) = 4, IIf(mstrDrugId <> "", "And I.ID IN (Select Column_Value From Table(f_Num2list([1]))) ", ""), "") & strType
        ...
        If mnuViewList.Checked = False Then
            gstrSql = gstrSql & " and Nvl(To_Char(z.撤档时间, 'YYYY-MM-DD'), '3000-01-01') = '3000-01-01' And i.分类id = z.Id "
        End If
        ...
        For i = 0 To UBound(arrDrugID)
            ...
            Set rsTemp = zldatabase.OpenSQLRecord(gstrSql, "zlRefRecords", str药名ID)
...
Private Sub StartStop(ByVal int分类品种规格 As Integer)
    '停用启用功能
    'int分类品种规格：0-分类，1-品种，2-规格
    Dim rsStartStop As New ADODB.Recordset
    Dim i As Integer
    Dim lng分类颜色 As Long
    
    Select Case int分类品种规格
        Case 0 '分类
            If Not (Me.tvwClass.SelectedItem.Parent Is Nothing) Then    '选择子节点
                If tvwClass.SelectedItem.Parent.ForeColor = mconColor_Stop Then '父节点停用后子节点都不能启用停用
                    mnuClassStar.Enabled = False
                    tlbThis.Buttons("Start").Enabled = False
                    mnuClassStop.Enabled = False
                    tlbThis.Buttons("Stop").Enabled = False
                    mnuClassAdd.Enabled = False
                    mnuClassMod.Enabled = False
                    mnuEditItemAdd.Enabled = False
                Else
                    If tvwClass.SelectedItem.ForeColor = mconColor_Stop Then
                        mnuClassStar.Enabled = True
                        tlbThis.Buttons("Start").Enabled = True
                        mnuClassStop.Enabled = False
                        tlbThis.Buttons("Stop").Enabled = False
                        mnuClassAdd.Enabled = False
                        mnuClassMod.Enabled = False
                    ...
                    Else
                        mnuClassStar.Enabled = False
                        tlbThis.Buttons("Start").Enabled = False
                        mnuClassStop.Enabled = True
                        tlbThis.Buttons("Stop").Enabled = True
                        mnuClassAdd.Enabled = True
                        mnuClassMod.Enabled = True
                    End If
                End If
        ...
        Case 1 '品种
            If lvwItems.SelectedItem Is Nothing Then
                ...
                mnuEditStart.Enabled = False
            Else
                gstrSql = "Select id,名称 From 诊疗分类目录 Where ID In (Select 分类id From 诊疗项目目录 A Where a.Id = [1])"
                Set rsStartStop = zldatabase.OpenSQLRecord(gstrSql, Me.Caption, Val(Mid(lvwItems.SelectedItem.Key, 2)))
                
                For i = 1 To tvwClass.Nodes.Count
                    If tvwClass.Nodes.Item(i).Key = "_" & rsStartStop!ID Then
                        Exit For
                    End If
                Next
                If tvwClass.SelectedItem.ForeColor = mconColor_Stop Then
                ...
                If i > tvwClass.Nodes.Count Then
                    lng分类颜色 = tvwClass.SelectedItem.ForeColor
                Else
                    lng分类颜色 = tvwClass.Nodes.Item(i).ForeColor
                End If
                
                If Val(Me.tvwClass.Tag) > 2 Then
                    gstrSql = "Select 1 " & vbNewLine & _
                                    "From 诊疗分类目录" & vbNewLine & _
                                    "Where ID In (Select 分类id From 诊疗项目目录 A Where a.Id =[1]) And" & vbNewLine & _
                                    "      Nvl(To_Char(撤档时间, 'YYYY-MM-DD'), '3000-01-01') = '3000-01-01'"

                    Set rsStartStop = zldatabase.OpenSQLRecord(gstrSql, Me.Caption, Val(Mid(lvwItems.SelectedItem.Key, 2)))
                    If rsStartStop.RecordCount = 0 Then lng分类颜色 = mconColor_Stop
                End If
                
                If lng分类颜色 = mconColor_Stop Then
                    mnuEditStart.Enabled = False
                    tlbThis.Buttons("Start").Enabled = False
                    mnuEditStop.Enabled = False
                    tlbThis.Buttons("Stop").Enabled = False
                Else
                    If lvwItems.SelectedItem.ForeColor = mconColor_Stop Then
        ...
        Case 2 '规格
            ...
            Else
                If lvwSpecs.SelectedItem Is Nothing Then
                    ...
                    tlbThis.Buttons("Stop").Enabled = False
                Else
                    If lvwSpecs.SelectedItem.ForeColor = mconColor_Stop Then
                        mnuEditStart.Enabled = True
                        tlbThis.Buttons("Start").Enabled = True
                        mnuEditStop.Enabled = False
                        tlbThis.Buttons("Stop").Enabled = False
