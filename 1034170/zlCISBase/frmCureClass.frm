...
Begin VB.Form frmCureClass 
   ...
   Begin VB.Label lblNote 
      Caption         =   "可根据临床与各科诊疗应用操作的特点进行应用参考的统一分类设置。"
      Height          =   345
      Left            =   990
      TabIndex        =   14
      Top             =   210
...
Private Sub cmdOK_Click()
    ...
    If Me.Tag = "增加" Then
        lngItemID = zlDatabase.GetNextId("诊疗参考分类")
        gstrSql = "zl_诊疗参考分类_Insert(" & _
            lngItemID & "," & _
            Me.txtParent.Tag & "," & _
...
Private Sub Form_Activate()
    With Me.lblKind
        ...
        Case 1
            .Caption = "西药"
            Me.Caption = "西药参考分类编辑"
            Me.lblNote.Caption = "西药参考通常可按照药品药理作用和化学作用，结合临床各科用药区别等进行分类设置。"
        Case 2
            .Caption = "中成药"
            Me.Caption = "中成药参考分类编辑"
            Me.lblNote.Caption = "中成药参考通常可以根据其组成的性味与功用等进行分类，也可选择其他分类方法进行分类。"
        Case 3
            .Caption = "中草药"
            Me.Caption = "中草药参考分类编辑"
            Me.lblNote.Caption = "中草药参考通常可以根据其性味归经功用等进行分类，也可选择如自然系统属性的分类方法。"
        Case 4
            .Caption = "中药配方"
            Me.Caption = "中药配方参考分类编辑"
            Me.lblNote.Caption = "配方分类参照中成药的分类方法进行，根据配方多少可以更细致或粗略地进行。"
        Case 5
            .Caption = "诊疗操作"
            Me.Caption = "诊疗操作参考分类编辑"
            Me.lblNote.Caption = "诊疗操作参考可根据临床与医技各科诊疗应用操作的特点进行统一分类设置。"
        End Select
    End With
    ...
    Else
        gstrSql = "select ID,上级ID,编码,名称,简码" & _
                " From 诊疗参考分类" & _
                " Where 类型 = [1] " & _
                " and  id not in (select id from 诊疗参考分类 start with ID = [2] connect by prior id=上级id ) " & _
                " start with 上级ID is null" & _
                " connect by prior ID=上级ID"
    End If
...
Private Sub zlDefaultCode()
    ...
    If Me.txtParent.Tag = 0 Then
        Me.txtParent.Text = "(无)"
        Me.txtUpCode.Text = ""
        gstrSql = "select max(编码) as 编码 From 诊疗参考分类 Where 类型=[1] and 上级ID is null "
    ...
    Else
        With Me.tvwClass
            ...
            If .SelectedItem.Children = 0 Then
                ...
                Me.chkCodeLen.Enabled = False
            Else
                gstrSql = "select nvl(max(编码),'') as 编码  From 诊疗参考分类 Where 类型=[1] and 上级ID=[2] "
