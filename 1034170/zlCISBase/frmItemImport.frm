...
Private Sub SaveType()
    ...
    With mobjWSType.UsedRange
        For lngRow = 3 To .Rows.Count
            If mintType = 1 Then
                lngItemId = zlDatabase.GetNextId("收费分类目录")
            ...
            Next
            '简码
            strSql = strSql & "'" & zlCommFun.SpellCode(.cells(lngRow, lngCol - 1)) & "',"
...
'            mobjWS.cells(lngRow, lngCol) = "1111"
            ...
            strID = zlDatabase.GetNextId("收费项目目录")
            ...
            '别名
            lngCol = mCollItemCols.Item("名称")
            If zlGetSymbol(.cells(lngRow, lngCol)) <> "" Then
                str别名 = "1''" & .cells(lngRow, lngCol) & "''1''" & zlGetSymbol(.cells(lngRow, lngCol)) & "''"
            End If

            If zlGetSymbol(.cells(lngRow, lngCol), 1) <> "" Then
                str别名 = str别名 & "1''" & .cells(lngRow, lngCol) & "''2''" & zlGetSymbol(.cells(lngRow, lngCol), 1) & "''"
            End If
    ...
    With mobjWS.UsedRange
        For lngRow = 3 To .Rows.Count
            ...
            If InStr(1, str编码, "[" & .cells(lngRow, lngCol) & "]") <= 0 Then
                ...
                str编码 = str编码 & strTemp & "," & lng药名id & "[" & .cells(lngRow, lngCol) & "]"
                ...
                '拼音简码
                strSql = strSql & "'" & zlGetSymbol(.cells(lngRow, lngCol)) & "',"
    
                '五笔简码
                strSql = strSql & "'" & zlGetSymbol(.cells(lngRow, lngCol), 1) & "',"
            ...
            Else
                '编码格式:类别,ID[编码]...
                '返回编码对应的类别和ID:类别,ID
                strTemp = Mid(Mid(str编码, 1, InStr(1, str编码, "[" & .cells(lngRow, lngCol) & "]") - 1), InStrRev(Mid(str编码, 1, InStr(1, str编码, "[" & .cells(lngRow, lngCol) & "]") - 1), "]") + 1)
