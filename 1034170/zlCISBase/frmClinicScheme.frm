...
Public Function ShowMe(ByVal frmParent As Object, ByVal strPrivs As String, ByVal byt状态 As Byte, _
    ...
    Call OpenRecord(rsTemp, gstrSql, Me.Caption)
    With rsTemp
        If .BOF Or .EOF Then
...
Private Sub cmdOK_Click()
    ...
    If txt名称(0).Text <> strTmp Then
        txt名称(0).Text = strTmp
        Me.txt拼音(0).Text = zlGetSymbol(Me.txt名称(0).Text, 0)
        Me.txt五笔(0).Text = zlGetSymbol(Me.txt名称(0).Text, 1)
    End If
    strTmp = MoveSpecialChar(txt名称(1).Text)
    If txt名称(1).Text <> strTmp Then
        txt名称(1).Text = strTmp
        Me.txt拼音(1).Text = zlGetSymbol(Me.txt名称(1).Text, 0)
        Me.txt五笔(1).Text = zlGetSymbol(Me.txt名称(1).Text, 1)
    End If
...
'        If zlClinicCodeRepeat(str编码) = True Then Exit Sub
    Else
...
Private Sub txt名称_KeyUp(Index As Integer, KeyCode As Integer, Shift As Integer)
    Me.txt拼音(Index).Text = zlGetSymbol(Me.txt名称(Index).Text, 0, txt拼音(Index).MaxLength)
    Me.txt五笔(Index).Text = zlGetSymbol(Me.txt名称(Index).Text, 1, txt五笔(Index).MaxLength)
End Sub
