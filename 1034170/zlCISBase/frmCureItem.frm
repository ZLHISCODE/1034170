...
Private Sub msf别名_KeyDown(KeyCode As Integer, Shift As Integer, Cancel As Boolean)
    ...
    With Me.msf别名
        If .Col = 1 Then
            ...
            End If
            If strTemp <> "" Then
                .TextMatrix(.Row, 1) = strTemp: .TextMatrix(.Row, 2) = zlGetSymbol(strTemp): .TextMatrix(.Row, 3) = zlGetSymbol(strTemp, 1)
            Else
...
Private Sub txtItem_KeyUp(Index As Integer, KeyCode As Integer, Shift As Integer)
    Select Case Index
    Case con目录名称
        Me.txtItem(con名称拼音).Text = zlGetSymbol(Trim(Me.txtItem(Index).Text), 0)
        Me.txtItem(con名称五笔).Text = zlGetSymbol(Trim(Me.txtItem(Index).Text), 1)
    End Select
End Sub
