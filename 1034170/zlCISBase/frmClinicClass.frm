...
Private Sub cmdSelect_Click()
    ...
    mstrID = txtParent.Tag
    If Me.Tag = "增加" Then
        gstrSql = "select ID,上级ID,编码,名称,简码" & _
                " From 诊疗分类目录" & _
                " Where 类型 = " & Val(Me.lblKind.Tag) & _
                " start with 上级ID is null And (撤档时间 Is Null Or 撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD')) " & _
                " connect by prior ID=上级ID And (撤档时间 Is Null Or 撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD'))"
