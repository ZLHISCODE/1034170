...
Public Sub ShowMe(ByVal frmParent As Object, ByVal blnEdit As Boolean, Optional ByVal lng项目id As Long)
    ...
    With rsTemp
        ...
        Else
            ...
            Me.msfLabs.Tag = zlCommFun.Nvl(!标本部位)
...
'        Call SQLTest
    With rsTemp
        ...
        Do While Not .EOF
            ...
            objItem.SubItems(Me.lvwItems.ColumnHeaders("计算单位").Index - 1) = zlCommFun.Nvl(!计算单位)
            objItem.Tag = zlCommFun.Nvl(!标本部位)
            .MoveNext
        Loop
...
'            Call SQLTest
        With rsTemp
            ...
            Do While Not .EOF
                ...
                objItem.Icon = "ItemUse": objItem.SmallIcon = "ItemUse"
                objItem.SubItems(Me.lvwItems.ColumnHeaders("编码").Index - 1) = !编码
                objItem.SubItems(Me.lvwItems.ColumnHeaders("简码").Index - 1) = zlCommFun.Nvl(!简码)
                .MoveNext
            Loop
            Me.lvwItems.ListItems(1).Selected = True
        End With
...
'            Call SQLTest
        With rsTemp
            ...
            Do While Not .EOF
                ...
                objItem.Icon = "ItemUse": objItem.SmallIcon = "ItemUse"
                objItem.SubItems(Me.lvwItems.ColumnHeaders("编码").Index - 1) = !编码
                objItem.SubItems(Me.lvwItems.ColumnHeaders("英文名").Index - 1) = zlCommFun.Nvl(!英文名)
                Select Case zlCommFun.Nvl(!类型, 0)
                Case 0
                ...
                End Select
                objItem.SubItems(Me.lvwItems.ColumnHeaders("单位").Index - 1) = zlCommFun.Nvl(!单位)
                .MoveNext
            Loop
            Me.lvwItems.ListItems(1).Selected = True
        End With
    ...
    Else
        ...
        With rsTemp
            ...
            If .RecordCount = 1 Then
                ...
                End Select
                Me.msfLabs.TextMatrix(Me.msfLabs.Row, 5) = zlCommFun.Nvl(!单位)
                Exit Sub
            End If
            Me.lvwItems.ListItems.Clear
            Do While Not .EOF
...
Private Sub zlLabsRef(lngItemId As Long)
    ...
    With rsTemp
        ...
        Do While Not .EOF
            ...
            Me.msfLabs.TextMatrix(.AbsolutePosition, 2) = zlCommFun.Nvl(!报告项目ID, 0)
            If Me.msfLabs.TextMatrix(.AbsolutePosition, 2) = "0" Then
                Me.msfLabs.TextMatrix(.AbsolutePosition, 3) = ""
            Else
                Me.msfLabs.TextMatrix(.AbsolutePosition, 3) = zlCommFun.Nvl(!报告项目)
            End If
            Select Case IIf(IsNull(!类型), 0, !类型)
            Case 0
            ...
            End If
            Me.msfLabs.TextMatrix(.AbsolutePosition, 5) = zlCommFun.Nvl(!单位)
            .MoveNext
        Loop
    End With
    Exit Sub
