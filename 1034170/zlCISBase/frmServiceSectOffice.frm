...
Private Sub msfServiceSectOffice_CommandClick()
    ...
    Call frmServiceSelect.ShowMe(frmServiceSectOffice, mintRow, mstr服务对象)
End Sub
...
Private Function Check服务科室() As Boolean
    ...
    gstrSql = gstrSql & " And Instr([1], ',' || b.服务对象 || ',') > 0"
    ...
    If mrs科室.RecordCount = 0 Then
        MsgBox "当前库房不是药房，或该库房不服务于病人" & vbCrLf & "或该库房未设置临床(ICU、检查、检验、手术、治疗、护理)部门性质！[部门管理]", vbInformation, gstrSysName
        ...
        With msfServiceSectOffice
            If .Col = 3 Then
                ...
                If bln科室 = True Then
                    ...
                    .TextMatrix(.Row, 4) = ""
                Else
                    gstrSql = " Select A.ID,A.编码,A.名称,A.简码 From 部门表 A,部门性质说明 B,部门性质分类 C " & _
                    ...
                    If rsrecord.RecordCount > 1 Then
                        mintRow = .Row
                        Call frmServiceSelect.ShowMe(frmServiceSectOffice, mintRow, mstr服务对象, strKey)
...
Private Sub ShowData()
    ...
    If mblnFirst Then
        ...
        If mlng药品ID <> 0 Then
            ...
            If mint材质分类 <> "7" Then
                Me.lblSpec.Caption = "规格：" & IIf(IsNull(rsTemp!规格), "", rsTemp!规格) & _
                    "   厂牌：" & IIf(IsNull(rsTemp!产地), "", rsTemp!产地) & _
                    "   单位：" & IIf(IsNull(rsTemp!单位), "", rsTemp!单位)
            Else
                Me.lblSpec.Caption = "产地：" & IIf(IsNull(rsTemp!产地), "", rsTemp!产地) & "   单位：" & IIf(IsNull(rsTemp!单位), "", rsTemp!单位)
            End If
        End If
        ...
        Call OpenRecordset(rsTemp, "根据药品的用途分类提取所允许存储的库房(其他库房)")
        mstr全部库房ID = ""
        Do While Not rsTemp.EOF
            ...
            If mlng药品ID <> Mid(.SelectedItem.Key, 2) Then
                ...
                If mint材质分类 <> "7" Then
                    Me.lblSpec.Caption = "规格：" & .SelectedItem.SubItems(2) & _
                        "   厂牌：" & .SelectedItem.SubItems(3) & _
                        "   单位：" & .SelectedItem.SubItems(4)
                Else
                    Me.lblSpec.Caption = "产地：" & .SelectedItem.SubItems(3) & "   单位：" & .SelectedItem.SubItems(4)
                End If
                Call ShowData
            End If
            Me.txtMedi.SetFocus
