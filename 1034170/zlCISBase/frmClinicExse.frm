...
Private Function SaveData() As Boolean
    ...
    If mGen0(UBound(mGen0)) <> "" Then
        ...
        If blnErr = False Then Exit Function
    End If
    ...
    If mGen3(UBound(mGen3)) <> "" Then
        ...
        If blnErr = False Then Exit Function
    End If
    If SaveArryData(0, 0, mGen0, mPlace0, mAppend0) = False Then
        tabDept.Tabs(1).Selected = True
        Exit Function
    End If
    
    For i = LBound(mDept1) To UBound(mDept1)
        If mDept1(i) <> "" Then
            If SaveArryData(1, Val(Split(mDept1(i), "|")(0)), mGen1, mPlace1, mAppend1) = False Then
                tabDept.Tabs(2).Selected = True
                Exit Function
            End If
        End If
    Next
    ...
    For i = LBound(mDept2) To UBound(mDept2)
        If mDept2(i) <> "" Then
            If SaveArryData(2, Val(Split(mDept2(i), "|")(0)), mGen2, mPlace2, mAppend2) Then
                tabDept.Tabs(3).Selected = True
                Exit Function
            End If
        End If
    Next
    ...
    For i = LBound(mDept3) To UBound(mDept3)
        If mDept3(i) <> "" Then
            If SaveArryData(3, Val(Split(mDept3(i), "|")(0)), mGen3, mPlace3, mAppend3) Then
                tabDept.Tabs(4).Selected = True
                Exit Function
            End If
        End If
    Next
...
Private Sub msfExse_CommandClick()
    ...
    Set rsTemp = Nothing
    frmClinicExseSelect.ShowMe Me, str服务对象, rsTemp
...
Private Sub msfExse_KeyDown(KeyCode As Integer, Shift As Integer, Cancel As Boolean)
    ...
    Do While Not rsTemp.EOF
        '使用编码的原因：一个收费项目可设置多个收入项目，此时售价应相加；一个收费项目可能有多个不同的规格，此时售价不应该相加
        '编码是可以区分这两种情况的字段
        Set objItem = lvwItems.FindItem(rsTemp!编码, lvwSubItem)
        ...
        Else
            objItem.SubItems(Me.lvwItems.ColumnHeaders("售价").Index - 1) = Format(Val(objItem.SubItems(Me.lvwItems.ColumnHeaders("售价").Index - 1)) + Val(IIf(IsNull(rsTemp!售价), 0, rsTemp!售价)), "0.0000")
        End If
        rsTemp.MoveNext
    Loop
