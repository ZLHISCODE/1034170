...
Begin VB.Form frmMediLimit 
   ...
   Begin VB.ComboBox cboDrugUnit 
      Height          =   300
      ItemData        =   "frmMediLimit.frx":058A
      Left            =   1800
      List            =   "frmMediLimit.frx":059A
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsfLimit 
      ...
      FormatString    =   $"frmMediLimit.frx":1236
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
Dim mrsNormal As New ADODB.Recordset
Dim mintCount As Integer
Dim mlng库房ID As Long
Dim mlngFind As Long
Dim mlngFindFirst As Long
Dim mrsFindName As ADODB.Recordset
Dim mblnChanged As Boolean
Dim mstr分类 As String
Dim mstr分类ID  As String
Dim mstr剂型 As String
Dim mblnActive As Boolean
Dim mint药品名称显示 As Integer         '0-显示通用名，1-显示商品名，2-同时显示通用名和商品名
...
Private Sub IniGrid()
    With vsfLimit
        ...
        .TextMatrix(0, .ColIndex("产地")) = IIf(Me.Tag <> "7", "厂牌", "产地")
        .ColWidth(.ColIndex("规格")) = 1500 'IIf(Me.Tag <> "7", 1500, 0)
        .ColWidth(.ColIndex("产地")) = IIf(Me.Tag <> "7", 1200, 900)
...
Private Sub zlLimitRef()
    ...
    gstrSql = "Select " & strRule & " I.ID,I.编码,I.名称, i.商品名,I.规格,I.产地," & _
         Switch(Me.cboRoom.Tag = "售价单位", "I.计算单位 as 单位,1 as 包装,", _
            ...
            " From (Select  I.ID,I.编码,I.名称, b.名称 As 商品名,I.规格,I.产地,I.计算单位,S.门诊单位,S.门诊包装, " & _
    ...
    gstrSql = gstrSql & " Where I.ID = P.收费细目id And I.Id=L.药品id(+) And I.ID = K.药品id(+) And (p.终止日期 Is Null Or Sysdate Between p.执行日期 And Nvl(p.终止日期, To_Date('3000-01-01', 'yyyy-MM-dd'))) "
...
Private Function Select货位(ByVal strKey As String) As Boolean
    ...
    Call CalcPosition(sngX, sngY, Me.vsfLimit)
    sngY = sngY - vsfLimit.CellHeight
    sngH = vsfLimit.CellHeight
    
    If chk货位.Value = 0 Then
        If strKey <> "" Then
            gstrSql = " Select Rownum As ID,编码,名称,简码 From 药品库房货位 " & _
                    " Where 末级 = 1 And (编码 Like [2] Or 名称 Like [3] Or 简码 Like [3]) Order By 编码"
        Else
            gstrSql = "Select 编码 As ID, 上级 As 上级id, 名称, 简码, 末级" & vbNewLine & _
                    "From 药品库房货位" & vbNewLine & _
                    "Start With 上级 Is Null" & vbNewLine & _
                    "Connect By Prior 编码 = 上级"
            blnCancel = frmTreeLeafSel.ShowTree(gstrSql, strID, str名称, "库房货位")
            
            If blnCancel = True Then
                With Me.vsfLimit
                    .Redraw = flexRDNone
                    .TextMatrix(.Row, .ColIndex("货位")) = Nvl(str名称)
    ...
    Else
        If strKey <> "" Then
            gstrSql = "Select Rownum As ID, 编码, 名称 " & _
                " From (Select Distinct B.编码, B.名称" & _
                " From 药品储备限额 A, 药品库房货位 B " & _
                " Where A.库房货位 = B.名称 And B.末级 = 1 And A.库房id = [1] And " & _
                " (B.编码 Like [2] Or B.名称 Like [3] Or B.简码 Like [3])) " & _
                " Order By 编码 "
        Else
            gstrSql = "Select Rownum As ID, 编码, 名称 " & _
                " From (Select Distinct B.编码, B.名称" & _
                " From 药品储备限额 A, 药品库房货位 B " & _
                " Where A.库房货位 = B.名称 And B.末级 = 1 And A.库房id = [1]) " & _
                " Order By 编码 "
        End If
    End If
   
    Set rsTemp = zlDatabase.ShowSQLSelect(Me, gstrSql, 0, "库房货位选择器", False, "", "选择库房货位", False, False, True, sngX, sngY, sngH, blnCancel, False, False, mlng库房ID, strKey & "%", gstrMatch & strKey & "%")
    
    If blnCancel = True Then Exit Function
    
    If rsTemp Is Nothing Then
        If chk货位.Value = 1 Then
            If IsHaveStock(strKey) = False Then
                vsfLimit.Cell(flexcpForeColor, vsfLimit.Row, vsfLimit.ColIndex("货位"), vsfLimit.Row, vsfLimit.ColIndex("货位")) = vbRed
            Else
                vsfLimit.Cell(flexcpForeColor, vsfLimit.Row, vsfLimit.ColIndex("货位"), vsfLimit.Row, vsfLimit.ColIndex("货位")) = vbBlack
            End If
        Else
            vsfLimit.Cell(flexcpForeColor, vsfLimit.Row, vsfLimit.ColIndex("货位"), vsfLimit.Row, vsfLimit.ColIndex("货位")) = vbRed
        End If
    ...
    With Me.vsfLimit
        .Redraw = flexRDNone
        .TextMatrix(.Row, .ColIndex("货位")) = Nvl(rsTemp!名称)
        vsfLimit.Cell(flexcpForeColor, .Row, .ColIndex("货位"), .Row, .ColIndex("货位")) = vbBlack
        .Redraw = flexRDBuffered
    End With
    ...
    Exit Function
ErrHand:
    vsfLimit.Redraw = flexRDBuffered
    If ErrCenter = 1 Then
        Resume
    End If
End Function
