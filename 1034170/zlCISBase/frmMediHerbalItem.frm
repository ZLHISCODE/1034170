...
End Sub
Private Sub chk辅助用药_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyReturn Then Call zlCommFun.PressKey(vbKeyTab)
End Sub
Private Sub cmdCancel_Click()
    ...
    strTemp = txt分类.Text & "|" & txt编码.Text & "|" & txt名称.Text & "|" & txt拼音.Text & "|" & txt五笔.Text & "|" & txt英文.Text & "|" & cbo单位.Text & "|" & _
                ...
                cbo处方职务.Text & "|" & cbo医保职务.Text & "|" & txt处方限量.Text & "|" & cbo适用性别.Text & "|" & chk辅助用药.Value & "|" & chk单独应用.Value & "|" & chk原料药.Value & "|" & _
                str别名
...
Private Sub cmdOK_Click()
    ...
    If Me.cmdCancel.Tag = "增加" Then
        gstrSql = "zl_草药品种_INSERT('7'," & gstrSql & ",'" & strTemp & "'," & IIf(mbln自管药 = True, "1)", "Null") & "," & Me.chk辅助用药.Value & ")"
    Else
        gstrSql = "zl_草药品种_UPDATE(" & gstrSql & ",'" & strTemp & "'," & IIf(mbln自管药 = True, "1)", "Null") & "," & Me.chk辅助用药.Value & ")"
    End If
    ...
    If mblnLoad = False Then
        ...
        Else
            ...
            gstrSql = "select I.分类ID,I.编码,I.名称,I.计算单位,I.单独应用,T.是否原料,T.药品剂型," & _
                    ...
                    "        I.建档时间,nvl(I.撤档时间,to_date('3000-01-01','YYYY-MM-DD')) as 撤档时间,B.名称 as 参考名称,I.参考目录id,nvl(T.品种医嘱,0) as 品种医嘱,Nvl(I.适用性别,0) AS 适用性别,Nvl(t.是否辅助用药, 0) As 是否辅助用药" & _
