...
Begin VB.Form frmDiagClass 
   ...
   Begin VB.Label lblNote 
      Caption         =   "通常建议按照疾病诊断学科分类的原则，进行疾病诊断的分类设置。"
      Height          =   345
      Left            =   990
      TabIndex        =   14
      Top             =   210
...
Private Sub cmdOK_Click()
    ...
    If Me.chkCodeLen.Value = 1 Then
        If Me.txtParent.Tag = 0 Then
            gstrSql = "select nvl(max(length(编码)),0) LEN from 疾病诊断分类 start with 上级id is null connect by prior id=上级id"
        Else
            gstrSql = "select nvl(max(length(编码)),0) LEN from 疾病诊断分类 start with 上级id=[1] connect by prior id=上级id"
        End If
        ...
        If Len(Trim(Me.txtCode.Text)) - Int(Me.txtCode.Tag) + intTmp > 6 Then
            MsgBox "编码的子级会超6位字符长！", vbExclamation, gstrSysName
            Me.txtCode.SetFocus
            Exit Sub
        End If
    End If
    ...
    If Me.Tag = "增加" Then
        lngItemID = zlDatabase.GetNextId("疾病诊断分类")
        gstrSql = "zl_疾病诊断分类_Insert(" & _
            lngItemID & "," & _
            Me.txtParent.Tag & "," & _
            "'" & Me.txtUpCode.Text & Trim(Me.txtCode.Text) & "'," & _
            "'" & Trim(Me.txtName.Text) & "'," & _
            "'" & Trim(Me.txtSymbol.Text) & "'," & _
            IIf(Me.lblKind.Caption = "西医", 1, 2) & "," & _
            Me.chkCodeLen.Value & ")"
    Else
        lngItemID = Me.Tag
        gstrSql = "zl_疾病诊断分类_Update(" & _
            lngItemID & "," & _
            Me.txtParent.Tag & "," & _
            "'" & Me.txtUpCode.Text & Trim(Me.txtCode.Text) & "'," & _
            "'" & Trim(Me.txtName.Text) & "'," & _
...
Private Sub Form_Activate()
    ...
    If Me.Tag <> "增加" Then
        gstrSql = gstrSql & " and id not in (select id from 疾病诊断分类 start with 上级id = [2] " & _
            "                connect by prior id = 上级id )" & _
            " and id <> [2] "
    End If
    
    gstrSql = gstrSql & " start with 上级ID is null" & _
                        " connect by prior ID=上级ID"
    Set rsTemp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, IIf(Me.lblKind.Caption = "西医", 1, 2), Val(Me.Tag))
...
Private Sub zlDefaultCode()
    ...
    If Me.txtParent.Tag = 0 Then
        Me.txtParent.Text = "(无)"
        Me.txtUpCode.Text = ""
        gstrSql = "select nvl(max(编码),'') as 编码 From 疾病诊断分类 Where 类别=[1] and 上级ID is null "
        Set rsTemp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, IIf(Me.lblKind.Caption = "西医", 1, 2))
    ...
    Else
        With Me.tvwClass
            ...
            If .SelectedItem.Children = 0 Then
                ...
                Me.chkCodeLen.Enabled = False
            Else
                gstrSql = "select nvl(max(编码),'') as 编码  From 疾病诊断分类 Where 类别=[1] and 上级ID=[2]"
                Set rsTemp = zlDatabase.OpenSQLRecord(gstrSql, Me.Caption, IIf(Me.lblKind.Caption = "西医", 1, 2), Val(Mid(.SelectedItem.Key, 2)))
