...
Begin VB.Form frmAntiList 
   Caption         =   "抗生素管理"
   ClientHeight    =   6435
   ClientLeft      =   165
   ClientTop       =   555
   ClientWidth     =   8970
   Icon            =   "frmAntiList.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   6435
   ScaleWidth      =   8970
      ...
      Begin MSComctlLib.ImageList imgList 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmAntiList.frx":058A
               Key             =   ""
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmAntiList.frx":0B24
               Key             =   ""
            EndProperty
         EndProperty
      End
...
Private mfrmEdit As frmAntiEdit

Private mintEditState As Integer    '当前编辑状态：0-非编辑状态,1-抗生素编辑,2-药敏组编辑
Private mlngItemID As Long, mintGroup As Integer
...
'以下为内部公共程序
'-----------------------------------------------------
Public Function zlRefList(Optional lngItemID As Long, Optional intGroup As Integer) As Long
    '功能：刷新装入指定种类的病历文件清单，并定位到指定的文件上
    Dim rsTemp As New ADODB.Recordset
    gstrSql = "Select 0 As 性质, ID, 编码, 中文名, 英文名, 简码, 说明" & vbNewLine & _
            "From 检验用抗生素" & vbNewLine & _
            "Union" & vbNewLine & _
            "Select 1 As 性质, I.ID, I.编码, I.名称, I.英文, I.简码, '包含' || Count(G.抗生素id) || '种抗生素' As 说明" & vbNewLine & _
            "From 检验抗生素组 I, 检验抗生素用药 G" & vbNewLine & _
            "Where I.ID = G.抗生素分组id(+)" & vbNewLine & _
            "Group By I.ID, I.编码, I.名称, I.英文, I.简码"
    ...
    With rsTemp
        Do While Not .EOF
            ...
            Set rptItem = rptRcd.AddItem(Val(!性质)): rptItem.Icon = rptItem.Value
            If !性质 = 0 Then
                rptRcd.AddItem CStr("抗生素目录")
            Else
                rptRcd.AddItem CStr("药敏组目录")
            End If
    ...
    If lngItemID <> 0 Then
        For Each rptRow In Me.rptList.Rows
            If rptRow.GroupRow = False Then
                If Val(rptRow.Record(mCol.ID).Value) = lngItemID And Val(rptRow.Record(mCol.图标).Value) = intGroup Then
        ...
        mintGroup = Val(Me.rptList.FocusedRow.Record(mCol.图标).Value)
    ElseIf Me.rptList.Rows.Count > 0 Then
...
Private Sub cbsThis_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_Save:
        ...
        If lngRetuId <> 0 Then
            ...
            If mintEditState = 1 Then
                Call zlRefList(mlngItemID, 0)
            Else
                Call zlRefList(mlngItemID, 1)
            End If
            mintEditState = 0: Me.picList.Enabled = True
        End If
    Case conMenu_Edit_Untread:
        Call mfrmEdit.zlEditCancel
        Call zlRefList(mlngItemID, mintGroup)
        mintEditState = 0: Me.picList.Enabled = True
    Case conMenu_Edit_NewParent
        If mfrmEdit.zlEditStart(True, mlngItemID, 1) Then mintEditState = 2: Me.picList.Enabled = False
        Me.dkpMan.FindPane(conPane_Edit).Select
    Case conMenu_Edit_NewItem
        If mfrmEdit.zlEditStart(True, mlngItemID, 0) Then mintEditState = 1: Me.picList.Enabled = False
        Me.dkpMan.FindPane(conPane_Edit).Select

    Case conMenu_Edit_Modify
        If mlngItemID = 0 Then Exit Sub
        If mfrmEdit.zlEditStart(False, mlngItemID, mintGroup) Then mintEditState = IIf(mintGroup = 0, 1, 2)
    ...
    Case conMenu_Edit_Delete
        ...
        With Me.rptList
            ...
            If mintGroup = 0 Then
                strMsg = "真的删除该抗生素吗？" & vbCrLf & "——" & .FocusedRow.Record(mCol.中文名).Value
                If MsgBox(strMsg, vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
                gstrSql = "Zl_检验用抗生素_Edit(3," & mlngItemID & ")"
            Else
                strMsg = "真的删除该药敏组合吗？" & vbCrLf & "——" & .FocusedRow.Record(mCol.中文名).Value
                If MsgBox(strMsg, vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
                gstrSql = "Zl_检验抗生素组_Edit(3," & mlngItemID & ")"
            End If
            ...
            ElseIf lngRetuId > 0 Then
                If .Rows(lngRetuId - 1).GroupRow = False Then mlngItemID = .Rows(lngRetuId - 1).Record(mCol.ID).Value
            End If
            Call Me.zlRefList(mlngItemID, mintGroup)
        End With
...
Private Sub cbsThis_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_NewParent
        Control.Enabled = (InStr(1, mstrPrivs, "抗生素组增删改") > 0 And mintEditState = 0)
    Case conMenu_Edit_NewItem
        Control.Enabled = (InStr(1, mstrPrivs, "抗生素增删改") > 0 And mintEditState = 0)
    Case conMenu_Edit_Modify, conMenu_Edit_Delete
        Control.Enabled = ((InStr(1, mstrPrivs, "抗生素增删改") > 0 Or InStr(1, mstrPrivs, "抗生素组增删改") > 0) _
                    And mintEditState = 0 And mlngItemID <> 0)
...
Private Sub dkpMan_AttachPane(ByVal Item As XtremeDockingPane.IPane)
    ...
    Case conPane_Edit
        If mfrmEdit Is Nothing Then Set mfrmEdit = New frmAntiEdit
...
Private Sub Form_Load()
    ...
    mLngEditWidth = frmAntiEdit.picSingle.ScaleWidth
    ...
    With cbrMenuBar.CommandBar.Controls
        Set cbrControl = .Add(xtpControlButton, conMenu_Edit_NewParent, "新药敏组(&N)")
        Set cbrControl = .Add(xtpControlButton, conMenu_Edit_NewItem, "新抗生素(&A)")
    ...
    With cbrToolBar.Controls
        ...
        Set cbrControl = .Add(xtpControlButton, conMenu_Edit_NewParent, "新药敏组"): cbrControl.BeginGroup = True
        Set cbrControl = .Add(xtpControlButton, conMenu_Edit_NewItem, "新抗生素")
    ...
    With Me.rptList
        ...
        Set rptCol = .Columns.Add(mCol.性质, "性质", 70, False): rptCol.Editable = False: rptCol.Groupable = True: rptCol.Visible = False
        ...
        Set rptCol = .Columns.Add(mCol.说明, "说明", 120, True): rptCol.Editable = False: rptCol.Groupable = False
...
Private Sub rptList_SelectionChanged()
    With Me.rptList
        ...
        ElseIf .FocusedRow.GroupRow = True Then
            mlngItemID = 0
            mintGroup = .FocusedRow.Childs(0).Record.Item(mCol.图标).Value
        Else
            mlngItemID = Val(.FocusedRow.Record.Item(mCol.ID).Value)
            mintGroup = .FocusedRow.Record.Item(mCol.图标).Value
        End If
        Call mfrmEdit.zlRefresh(mlngItemID, mintGroup)
    End With
End Sub
