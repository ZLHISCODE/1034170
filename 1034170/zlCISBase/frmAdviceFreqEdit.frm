...
Private Sub Form_Load()
    ...
    If mstrCode <> "" Then
        ...
        cbo间隔单位.Text = zlcommfun.Nvl(rsTmp!间隔单位)
        txtEdit(0).Text = zlcommfun.Nvl(rsTmp!编码)
        txtEdit(1).Text = zlcommfun.Nvl(rsTmp!名称)
        txtEdit(2).Text = zlcommfun.Nvl(rsTmp!简码)
        txtEdit(3).Text = zlcommfun.Nvl(rsTmp!英文名称)
        txtEdit(4).Text = zlcommfun.Nvl(rsTmp!频率次数)
        txtEdit(5).Text = zlcommfun.Nvl(rsTmp!频率间隔)
        
        mstr间隔单位 = zlcommfun.Nvl(rsTmp!间隔单位)
        mint频率次数 = zlcommfun.Nvl(rsTmp!频率次数)
        mint频率间隔 = zlcommfun.Nvl(rsTmp!频率间隔)
    Else
        '新增
        txtEdit(0).Text = ""
        txtEdit(1).Text = ""
        ...
        Set rsTmp = OpenRecord(rsTmp, strSQL, Me.Caption)
...
Private Sub txtEdit_Validate(Index As Integer, Cancel As Boolean)
    If Not StrIsValid(txtEdit(Index).Text, txtEdit(Index).MaxLength) Then
        Cancel = True
    ElseIf Index = 4 Or Index = 5 Then
