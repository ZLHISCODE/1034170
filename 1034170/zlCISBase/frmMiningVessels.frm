...
Private Sub cbsMain_Update(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case mconMenu_Manage_Cancle '取消
        Control.Enabled = vfgList.Editable = flexEDKbdMouse
    Case mconMenu_Manage_ModifyNo, mconMenu_Manage_Delete
...
Private Sub LoadData()
    ...
    With vfgList
        ...
        Do Until rsTmp.EOF
            ...
            .TextMatrix(.Rows - 1, .ColIndex("编码")) = zlCommFun.Nvl(rsTmp.Fields("编码"))
            .TextMatrix(.Rows - 1, .ColIndex("名称")) = zlCommFun.Nvl(rsTmp.Fields("名称"))
            .TextMatrix(.Rows - 1, .ColIndex("规格")) = zlCommFun.Nvl(rsTmp.Fields("规格"))
            .TextMatrix(.Rows - 1, .ColIndex("添加剂")) = zlCommFun.Nvl(rsTmp.Fields("添加剂"))
            .TextMatrix(.Rows - 1, .ColIndex("采血量")) = zlCommFun.Nvl(rsTmp.Fields("采血量"))
            .TextMatrix(.Rows - 1, .ColIndex("Modify")) = zlCommFun.Nvl(rsTmp.Fields("编码"))
            
            .Cell(flexcpFloodPercent, .Rows - 1, .ColIndex("颜色")) = 100
            lngColor = Val(zlCommFun.Nvl(rsTmp.Fields("颜色")))
            If lngColor = 0 Then lngColor = -214748363
            .Cell(flexcpFloodColor, .Rows - 1, .ColIndex("颜色")) = lngColor
            
            .TextMatrix(.Rows - 1, .ColIndex("材料ID")) = zlCommFun.Nvl(rsTmp.Fields("材料ID"))
            .TextMatrix(.Rows - 1, .ColIndex("对应材料")) = Trim(zlCommFun.Nvl(rsTmp.Fields("对应材料")))
            .Rows = .Rows + 1
            rsTmp.MoveNext
        Loop
        ...
        '--------------------------------------------------------------------------------------
            ...
            If strInput = "" Then
                strSQL = "Select A.ID, A.编码, A.名称, A.规格, A.计算单位, B.现价, A.费用类型," & vbNewLine & _
                        ...
                        "From (Select 现价, 收费细目id From 收费价目 Where 终止日期 Is Null Or 终止日期 = To_Date('3000-01-01', 'YYYY-MM-DD')) B," & vbNewLine & _
            ...
            Else
                strSQL = "Select /*+ rule */" & vbNewLine & _
                    ...
                    "     (Select 现价, 收费细目id From 收费价目 Where 终止日期 Is Null Or 终止日期 = To_Date('3000-01-01', 'YYYY-MM-DD')) B," & vbNewLine & _
                    "     收费项目目录 A" & vbNewLine & _
            ...
            If Not blnCanel And rsTmp.State <> 0 Then
                If Not rsTmp.EOF Then
                    With vfgList
                        .EditText = Trim(zlCommFun.Nvl(rsTmp.Fields("编码")) & " " & zlCommFun.Nvl(rsTmp.Fields("名称")))
                        .TextMatrix(.Row, .ColIndex("对应材料")) = Trim(zlCommFun.Nvl(rsTmp.Fields("编码")) & " " & zlCommFun.Nvl(rsTmp.Fields("名称")))
                        .TextMatrix(.Row, .ColIndex("材料ID")) = zlCommFun.Nvl(rsTmp.Fields("ID"), "")
                    End With
                End If
                Set rsTmp = Nothing
            End If
