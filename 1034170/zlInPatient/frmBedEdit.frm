...
'        If lngIdx <> -2 Then cbo等级.ListIndex = lngIdx
    Else
        ...
        If cbo等级.ListIndex <> -1 Then
            '弹出列表时,又在文本框输入了内容
            If strText <> cbo等级.List(cbo等级.ListIndex) Then Call zlControl.CboSetIndex(cbo等级.hWnd, -1)
        End If
        If strText = "" Then
            cbo等级.ListIndex = -1
    ...
    Else
        ...
        Else
            ...
            With mrptRecord
                ...
                cbo科室.ListIndex = FindCboIndex(cbo科室, Val(.Item(mCol.科室ID).Value))
                ...
                cbo性别.ListIndex = GetCboIndex(cbo性别, .Item(mCol.性别分类).Value)
                cbo等级.ListIndex = GetCboIndex(cbo等级, .Item(mCol.等级).Value)
                If cbo等级.ListIndex = -1 Then isCheckBedLevelExists .Item(mCol.等级).Value, True
                cbo编制.ListIndex = GetCboIndex(cbo编制, .Item(mCol.床位编制).Value)
                txt床号.MaxLength = 10
...
Public Function zlEditSave() As String
    ...
    If mblnAdd Then
        ...
        gstrSQL = "zl_床位状况记录_INSERT('" & str床号 & "'," & mlngUnit & "," & _
