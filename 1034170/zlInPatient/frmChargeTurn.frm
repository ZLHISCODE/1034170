...
Private Sub cmdSave_Click()
    ...
    mstrNOS = ""
    If mlngPatient = 0 Then
        MsgBox "注意:" & vbCrLf & "    你选择的病人错误,请检查!", vbInformation, gstrSysName
        Exit Sub
    End If
            ...
            If .TextMatrix(i, .ColIndex("类别")) = "可转入" And .TextMatrix(i, .ColIndex("选择")) = "√" Then
                ...
                If .TextMatrix(i, .ColIndex("单据")) = "收费单" Then
                    If CheckBillExistReplenishData(1, , .TextMatrix(i, .ColIndex("单据号"))) Then
                        MsgBox "注意:" & vbCrLf & "    单据号为" & .TextMatrix(i, .ColIndex("单据号")) & "的记录存在补充结算,不能继续!", vbInformation, gstrSysName
                        mstrNOS = "": Exit Sub
                    End If
                End If
                If Val(.TextMatrix(i, .ColIndex("险类"))) <> 0 Then
                    ...
                    If IsYBSingle(.TextMatrix(i, .ColIndex("单据号")), Val(.TextMatrix(i, .ColIndex("险类")))) = False Then
                        If CheckInsureAll(.TextMatrix(i, .ColIndex("结帐ID"))) = False Then
                            MsgBox "注意:" & vbCrLf & "    医保单据号为" & .TextMatrix(i, .ColIndex("单据号")) & "的记录本次未转出全部相关结算单据,不能继续!", vbInformation, gstrSysName
                            mstrNOS = "": Exit Sub
                        End If
                    End If
                Else
                    If CheckAllTurn(.TextMatrix(i, .ColIndex("单据号"))) = True Then
                        If CheckInsureAll(.TextMatrix(i, .ColIndex("结帐ID"))) = False Then
                            MsgBox "注意:" & vbCrLf & "    单据号为" & .TextMatrix(i, .ColIndex("单据号")) & "的记录本次未转出全部相关结算单据,不能继续!", vbInformation, gstrSysName
                            mstrNOS = "": Exit Sub
                        End If
                    End If
                End If
    ...
    End With
    If mstrNOS = "" Then
        MsgBox "注意:" & vbCrLf & "    你还未选择要转成住院的单据,不能续继!", vbInformation, gstrSysName
        Exit Sub
    End If
