...
Private Sub cmdOK_Click()
    ...
    If CDate(txtDate.Text) > curDate Then
        If CDate(txtDate.Text) - curDate > 30 Then
            MsgBox "入病区时间比当前时间大得过多,请检查！", vbInformation, gstrSysName
            txtDate.SetFocus: Exit Sub
        End If
        If MsgBox("入病区间大于了当前系统时间,要继续吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
            txtDate.SetFocus: Exit Sub
        End If
    End If
    ...
    strSQL = "zl_病人变动记录_InUnit(" & mlng病人ID & "," & mlng主页ID & ",'" & strBed & "'," & _
            mlngUnit & "," & lngNurseGrade & ",'" & NeedName(cbo病况.Text) & "'," & chk陪伴.Value & ",'" & NeedName(cbo责任护士.Text) & "'," & _
