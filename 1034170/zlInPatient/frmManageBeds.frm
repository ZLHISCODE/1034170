...
Public Enum mCol
    图标 = 0: 床号: 科室: 房间号: 状态: 性别分类: 等级: 床位编制: 单价: 姓名: 性别: 年龄: 住院状态: 病区id: 科室ID: 病人id: 等级ID: 共用
End Enum
...
'功能：初始化病人列表表格
    ...
    With rptList
        ...
        Set objCol = .Columns.Add(mCol.床号, "床号", 50, True):  objCol.Groupable = False: .SortOrder.Add objCol: objCol.AutoSize = False
...
Private Sub zlRptPrint(ByVal bytMode As Byte)
    ...
    Set objAppRow = New zlTabAppRow
            ...
            Call objAppRow.Add("病区:" & NeedName(rptUnit.FocusedRow.Record(2).Value))  'cboUnit.Text
...
Private Function ReadBeds(lngUnitID As Long) As Boolean
    ...
    gstrSQL = _
        "Select LPad(a.床号, [1], ' ') 床号, a.病区id, a.房间号, a.性别分类, a.床位编制, a.等级id, a.状态, a.病人id, a.共用, e.现价 As 价格," & vbNewLine & _
        "       Nvl(b.名称, Decode(a.共用, 1, '<共用病床>', Null)) As 科室, a.科室id, c.名称 As 等级, d.姓名, d.性别, d.年龄, Decode(d.状态,0,'正常住院',2,'准备转出',3,'准备出院'||'('||To_Char(f.开始时间,'YYYY-MM-DD HH24:MI:SS')||')' )  AS 住院状态" & vbNewLine & _
        "From 床位状况记录 A, 部门表 B, 收费项目目录 C, (Select M.病人ID,NVL(n.姓名,m.姓名) 姓名,NVL(n.性别,m.性别) 性别,NVL(n.年龄,m.年龄) 年龄, n.状态 From 病人信息 M,病案主页 N Where M.病人ID=n.病人id And m.主页Id=n.主页id and n.当前病区ID=[2]) D, 收费价目 E, " & vbNewLine & _
        "        (Select q.病人ID ,开始时间 From 病案主页 P,病人变动记录 Q Where P.病人ID=Q.病人ID And P.主页ID=Q.主页ID And 开始原因=10 And P.当前病区ID=[2] And P.状态=3) F " & vbNewLine & _
        "Where a.科室id = b.Id(+) And a.等级id = c.Id(+) And a.病人id = d.病人id(+) And e.收费细目id = c.id And a.病人id = f.病人ID(+) And a.病区id = [2]" & vbNewLine & _
        "    And Sysdate Between e.执行日期 And Nvl(e.终止日期,To_Date('3000-1-1', 'YYYY-MM-DD'))" & vbNewLine & _
