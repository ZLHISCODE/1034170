...
'                    " Order By A.简码"
    ...
    '光标定位
    cbo医疗小组.ListIndex = GetCboIndex(cbo医疗小组, IIf(IsNull(mrsPatiInfo!医疗小组), "", mrsPatiInfo!医疗小组))
    cbo住院医师.ListIndex = GetCboIndex(cbo住院医师, IIf(IsNull(mrsPatiInfo!住院医师), "", mrsPatiInfo!住院医师))
    cbo主治医师.ListIndex = GetCboIndex(cbo主治医师, IIf(IsNull(mrsPatiInfo!主治医师), "", mrsPatiInfo!主治医师))
...
Private Sub cmdOK_Click()
    ...
    strSQL = "zl_病人变动记录_ChangeGroup(" & mlng病人ID & "," & mlng主页ID & "," & _
            cbo医疗小组.ItemData(cbo医疗小组.ListIndex) & ",'" & NeedName(cbo住院医师.Text) & "','" & NeedName(cbo主治医师.Text) & "',To_Date('" & txtDate.Text & "','YYYY-MM-DD HH24:MI:SS')," & _
    ...
    On Error Resume Next
     '住院医师变动后触发消息
    If NeedName(cbo住院医师.Text) <> Nvl(mrsPatiInfo!住院医师) Or NeedName(cbo主治医师.Text) <> Nvl(mrsPatiInfo!主治医师) Then
