...
'    End If
    ...
    If txtPatient.Text = "" And Not txtPatient.Locked And Me.ActiveControl Is txtPatient Then
        ...
        Else
            '刷身份证时校验姓名
            If strName <> Nvl(mrsInfo!姓名) And strName <> "" Then
                If CreatePublicPatient() Then
                    If gobjPublicPatient.CheckPatiIn(mrsInfo!病人id) Then
                        '正在就诊
                        MsgBox "病人原来的姓名【" & mrsInfo!姓名 & "】与身份证上面的姓名【" & strName & "】不一致，请先修改成正确的姓名后再进行登记。", vbInformation, gstrSysName
                        Call ClearCard:  Exit Sub
                    Else
                        MsgBox "病人原来的姓名【" & mrsInfo!姓名 & "】与身份证上面的姓名【" & strName & "】不一致，系统将更新为身份证上面的姓名。", vbInformation, gstrSysName
                        If gobjPublicPatient.SavePatiBaseInfo(mrsInfo!病人id, 0, strName, strSex, Nvl(mrsInfo!年龄), Format(datBirthDay, "yyyy-MM-dd"), Me.Caption, 1, "", True, mblnEMPI) Then
        ...
        If lngTmp > 0 Then
            If CreatePublicPatient() Then
                If gobjPublicPatient.CheckPatiIdcard(Trim(txt身份证号.Text), str出生日期, strAge, str性别, strErrInfo, CDate(txt入院时间.Text)) Then
                    ...
                    If gblnPatiByID And Trim(txt身份证号.Text) <> txt身份证号.Tag And Not (mrsInfo Is Nothing And mlng病人ID = 0) Then
                        If gobjPublicPatient.CheckPatiExistByID(Trim(txt身份证号.Text), CLng(txtPatient.Tag)) Then
    ...
    If mbytInState = 0 Then
        ...
        If mrsInfo Is Nothing Then
            ...
            If Not (mbytMode = EMode.E接收预约 Or mbytMode = EMode.E正常登记 And mlng病人ID <> 0) Then
                If Not CheckPatiIsExist() Then Exit Sub
                '病人ID检查:自动替换新的
        ...
        If txt住院号.Visible And (mbytKind = 0 Or mbytMode = 2) Then
            ...
            blnTrue = False
            If mbytMode = EMode.E接收预约 Then blnTrue = True
...
Private Function zlInterfacePrayMoney(ByRef cllPro As Collection, ByRef cllThreeSwap As Collection) As Boolean
    ...
    End If
    '问题：126188，2018-06-19,入院登记失败
    If lng卡类别ID = 0 Then zlInterfacePrayMoney = True: Exit Function
...
Private Function LoadPati(ByVal strPatiXML As String) As Boolean
    ...
    If byt信息更新模式 = 0 Or lng病人ID = 0 Or (lng病人ID <> 0 And byt信息更新模式 = 2 And txt身份证号.Text = "") Then
        If strValue <> "" Then txt身份证号.Text = strValue: txt身份证号.Tag = strValue
    End If
    '    其他证件    Varchar2    20
    ...
    If Not rsSimilar Is Nothing Then
        If gblnPatiByID And Trim(txt身份证号.Text) <> "" Then
            ...
            If rsSimilar.RecordCount > 0 Then
                ...
                If gobjPublicPatient.ShowSelect(rsSimilar, "ID", "病人选择", strSimilar, , , "0|1000|1000|500|800|1000|1800|1000|1000|1500|1200", True) Then
        ...
        If rsSimilar.RecordCount > 1 Then
            ...
            If gobjPublicPatient.ShowSelect(rsSimilar, "ID", "病人选择", strSimilar, , , "0|1000|1000|500|800|1000|1800|1000|1000|1500|1200") Then
                lngPatiID = Val(rsSimilar!病人id & "")
            Else
                lngPatiID = -1
            End If
        ElseIf rsSimilar.RecordCount = 1 Then
            strSimilar = "ID:" & rsSimilar!病人id & ",门诊号:" & Nvl(rsSimilar!门诊号, "无") & ",住院号:" & Nvl(rsSimilar!住院号, "无") & ",身份证号:" & rsSimilar!身份证号 & ",地址:" & rsSimilar!地址 & ",登记日期:" & rsSimilar!登记日期
