...
Private Sub cboSeekKey_KeyPress(KeyAscii As Integer)
    zlcontrol.CboAppendText cboSeekKey, KeyAscii
End Sub
...
Private Sub cmdOK_Click()
    If Trim(txtSeekValue.Text) = "" Then
        MsgBox "请输入要查找的" & NeedName(cboSeekKey.Text) & "！", vbInformation, gstrSysName
        txtSeekValue.SetFocus
        Exit Sub
    End If
    
    mstrSeekKey = NeedName(cboSeekKey.Text)
...
Private Sub Form_Activate()
    mblnOK = False
    If mstrSeekKey <> "" Then
        zlcontrol.CboSetText cboSeekKey, mstrSeekKey
    Else
...
Private Sub txtSeekValue_KeyPress(KeyAscii As Integer)
    If NeedName(cboSeekKey.Text) = "住院号" Then
        If InStr("0123456789" & Chr(8), Chr(KeyAscii)) = 0 Then KeyAscii = 0
    '问题30342 by lesfeng 2010-06-01
    ElseIf NeedName(cboSeekKey.Text) = "就诊卡号" Then
