...
'        Set mrsPati = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lngDeptID)
    ...
    If mrsPati.EOF Then
        ...
        Call SetMenu(False)
    Else
        mshPati.Rows = mrsPati.RecordCount + 1
        
        '获取列和列位置
        Set colIx = New Collection
        For i = 0 To mshPati.Cols - 1
            colIx.Add i, mshPati.TextMatrix(0, i)
        Next
        
        '加载到表格中
        For i = 1 To mrsPati.RecordCount
            For j = 0 To mrsPati.Fields.Count - 1
                lngCol = colIx(mrsPati.Fields(j).Name)
                mshPati.TextMatrix(i, lngCol) = NVL(mrsPati.Fields(j).Value)
            Next
            mrsPati.MoveNext
        Next
