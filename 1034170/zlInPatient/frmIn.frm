...
Private Sub cbo床号_Click()
    zlControl.CboSetWidth cbo床号.hWnd, cbo床号.width * 1.8
End Sub
...
Private Sub cbo护理等级_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii <> 13 Then
        ...
        lngIdx = MatchIndex(cbo护理等级.hWnd, KeyAscii)
...
Private Sub cbo医疗小组_Click()
    ...
    If cbo医疗小组.ListIndex <> -1 And cbo医疗小组.ListIndex <> cbo医疗小组.ListCount - 1 Then
        If Val(cbo医疗小组.ItemData(cbo医疗小组.ListIndex)) > 0 Then
            ...
            If lng医师 <> 0 Then Call zlControl.CboSetIndex(cbo住院医师.hWnd, cbo.FindIndex(cbo住院医师, lng医师))
            ...
            If lng医师 <> 0 Then Call zlControl.CboSetIndex(cbo主治医师.hWnd, cbo.FindIndex(cbo主治医师, lng医师))
        End If
    Else
        Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mstrIDs & "," & mlngUnit, "主任医师,副主任医师,主治医师,医师,医士", gstrNodeNo)
        cbo住院医师.Clear
...
Private Sub cbo医疗小组_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii <> 13 Then
        ...
        lngIdx = MatchIndex(cbo医疗小组.hWnd, KeyAscii)
...
Private Sub cbo主任医师_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii <> 13 Then
        ...
        lngIdx = MatchIndex(cbo主任医师.hWnd, KeyAscii)
    ...
    Else
        ...
        If cbo住院医师.ListIndex <> -1 Then
            ...
            If rsTmp.RecordCount > 0 Then
                rsTmp.MoveFirst
                Call zlControl.CboSetIndex(cbo医疗小组.hWnd, GetCboIndex(cbo医疗小组, Nvl(rsTmp!名称)))
                Exit Sub
            End If
        End If
        If cbo主治医师.ListIndex <> -1 Then
            ...
            If rsTmp.RecordCount > 0 Then
                ...
                Call zlControl.CboSetIndex(cbo医疗小组.hWnd, GetCboIndex(cbo医疗小组, Nvl(rsTmp!名称)))
            Else
                Call zlControl.CboSetIndex(cbo医疗小组.hWnd, cbo医疗小组.ListCount - 1)
            End If
        Else
            Call zlControl.CboSetIndex(cbo医疗小组.hWnd, cbo医疗小组.ListCount - 1)
        End If
...
Private Sub Form_Load()
    ...
    With mrsPatiInfo
        ...
        '转科入住
        If mbyt入住方式 = 1 Then
            If Not zlControl.CboLocate(cbo住院医师, "" & !住院医师) Then
            ...
            If Not rsTmp.EOF Then
                If Not zlControl.CboLocate(cbo主治医师, "" & rsTmp!信息值) Then
            ...
            If Not rsTmp.EOF Then
                If Not zlControl.CboLocate(cbo主任医师, "" & rsTmp!信息值) Then
                    Call GetPersonnelIDCode("" & rsTmp!信息值, strID, strCode)
...
Private Sub SeekDoctor(cbo As ComboBox, Optional strPre As String)
    ...
    If strPre <> "" Then
        For i = 0 To cbo.ListCount - 1
            If NeedName(cbo.List(i)) = strPre Then cbo.ListIndex = i: Exit Sub
        Next
    End If
...
Private Sub cmdOK_Click()
    ...
    If CDate(txtDate.Text) > curDate Then
        If CDate(txtDate.Text) - curDate > 30 Then
            MsgBox "入住时间比当前时间大得过多,请检查！", vbInformation, gstrSysName
            txtDate.SetFocus: Exit Sub
        End If
        If MsgBox("入住时间大于了当前系统时间,要继续吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
            txtDate.SetFocus: Exit Sub
        End If
    End If
    ...
    If cbo科室.ItemData(cbo科室.ListIndex) <> mrsPatiInfo!入住科室id Then
        ...
        If mint调整科室 = 0 And mbyt入住方式 = 0 Then
            MsgBox "当前选择的科室【" & NeedName(cbo科室.Text) & "】不是病人原先登记的科室【" & mrsPatiInfo!当前科室 & "】，不能操作！", vbInformation, gstrSysName
            If cbo科室.Enabled Then cbo科室.SetFocus
            Exit Sub
        Else
            If MsgBox("当前选择的科室【" & NeedName(cbo科室.Text) & "】不是病人原先登记的科室【" & mrsPatiInfo!当前科室 & "】,要继续吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then Exit Sub
        End If
    End If
    ...
    strSQL = "zl_病人变动记录_InDept(" & mlng病人ID & "," & mlng主页ID & ",'" & strBed & "'," & _
            ...
            lngNurseGrade & ",'" & NeedName(cbo病况.Text) & "'," & _
            "'" & NeedName(cbo责任护士.Text) & "','" & NeedName(cbo门诊医师.Text) & "'," & _
            "'" & NeedName(cbo住院医师.Text) & "'," & chk陪伴.Value & "," & _
            ...
            NeedName(cbo主治医师.Text) & "','" & NeedName(cbo主任医师.Text) & "','" & strMainBed & "','" & int母婴转科标志 & "')"
    ...
    If mclsMipModule.IsConnect = True Then
        ...
        If mbyt入住方式 = 0 Then '正常入科
            ...
            mclsXML.appendData "in_dept_title", NeedName(cbo科室.Text), xsString  '入院科室
            ...
            If cbo护理等级.ListIndex <> -1 Then
                mclsXML.appendData "in_tendgrade", NeedName(cbo护理等级.Text), xsString
            Else
            ...
            mclsXML.appendData "in_doctor", NeedName(cbo住院医师.Text), xsString
            'duty_nurse      责任护士    0..1    S
            mclsXML.appendData "duty_nurse", NeedName(cbo责任护士.Text), xsString
        ...
        Else '转科入科
            ...
            mclsXML.appendData "in_dept_title", NeedName(cbo科室.Text), xsString
            mclsXML.AppendNode "in_hospital", True
            '转科入科
