...
'参数：bytstyle=0-大图标,1-小图标,2-列表,3-详细资料
    ...
    '大图标床位和姓名换行处理
    If mobjLVW Is lvwBeds_s Then
        Call LoadList(True, False, False, False)
    End If
End Sub
        ...
        If Not .EOF Then
            ...
            For i = 1 To .RecordCount
                If Not (!状态 = "占用" And IsNull(!病人ID)) Then
                    ...
                    End If
                    '以床位为单位显示,以床号为关键字 大图标姓名换行处理
                    Set objItem = lvwBeds_s.ListItems.Add(, "_" & !床号, Space(bytLen - Len(!床号)) & !床号 & IIf(IsNull(!姓名), "", ":" & IIf(lvwBeds_s.View = lvwIcon, vbCrLf, "") & !姓名), strIcon, strIcon)
