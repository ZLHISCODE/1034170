...
Private Function GetOutState(ByVal lng病人ID As Long, ByVal lng主页ID As Long) As String
    ...
    If rsTmp.RecordCount > 0 Then GetOutState = rsTmp!出院方式
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
'返回：相似记录的病人ID串,如"234,235,236"
    ...
    On Error GoTo errH
    strSQL = "Select Rownum + 1 AS ID, 病人id, 姓名, 性别, 年龄, 出生日期, 身份证号, 门诊号, 住院号, 地址, To_Char(登记时间, 'YYYY-MM-DD') As 登记日期" & vbNewLine & _
            "From (Select 病人id, 姓名, 性别, 年龄, To_Char(出生日期, 'YYYY-MM-DD') As 出生日期, Nvl(身份证号, '未登记') 身份证号, 门诊号, 住院号, Nvl(家庭地址, '未登记') 地址," & vbNewLine & _
            "              就诊时间, 入院时间, 登记时间" & vbNewLine & _
            "       From 病人信息" & vbNewLine & _
            "       Where (国籍=[1] And 民族=[2] And 性别=[3] And 姓名=[4] " & _
            "           And 出生日期=TO_DATE([5],'YYYY-MM-DD')) Or 身份证号=[6] " & vbNewLine & _
            "       Order By Nvl(Nvl(就诊时间, 入院时间), 登记时间) Desc)"
...
Public Function CheckPhoneNumber(str手机号 As String, Optional ByVal blnShowMsg As Boolean = True, Optional ByRef strErrMsg As String) As Boolean
    ...
    On Error GoTo errH
    '检查是否满足手机号格式
    If str手机号 = "" Then CheckPhoneNumber = True: Exit Function
    If Not IsNumeric(str手机号) Or Len(str手机号) <> 11 Then
        If blnShowMsg Then
            MsgBox "输入的手机号格式不正确，请重新录入！", vbInformation, gstrSysName
        Else
            strErrMsg = "输入的手机号格式不正确，请重新录入！"
        End If
        Exit Function
    End If
    '中国移动
    ...
    '中国电信
    strMobileRange = strMobileRange & ",133,153,189,180,181,177,173,199"
    '虚拟运营商
    strMobileRange = strMobileRange & ",170,"
    If InStr(strMobileRange, "," & Mid(str手机号, 1, 3) & ",") = 0 Then
        If blnShowMsg Then
            MsgBox "输入的手机号格式不正确，请重新录入！", vbInformation, gstrSysName
        Else
            strErrMsg = "输入的手机号格式不正确，请重新录入！"
