...
Private mblnSavelog As Boolean
...
'    Loop
    ...
    If mblnSavelog Then
        TraceFileWrite strLog, LTT_FullDate, 1
    End If
...
Public Sub GetMidTabSize(ByRef lngMinSize As Long, ByRef lngMaxSize As Long)
    ...
    lngMinSize = 3000: lngMaxSize = 1000000
     If Not gblnIsZlhis Then Exit Sub
...
'lngMinSize,lngMaxSize - 判定中型表的区间,不传则默认为3000-1000000
    ...
    If lngMinSize = 0 Then lngMinSize = 3000
    ...
    Select Case intMod
        Case 1
            strSQL = "Select distinct  Table_Name 表名" & vbNewLine & _
                            ...
                            IIf(gblnIsZlhis, _
                                "Where Num_Rows Between " & lngMinSize & " And " & lngMaxSize & vbNewLine & strSQL, _
                                "Where Num_Rows >1000000")   'ZLHIS环境取中型表/zltables中的表,非ZLHIS环境区百万行以上的表
        ...
        Case 2
            strSQL = "Select distinct Index_Name 索引名" & vbNewLine & _
                            ...
                            IIf(gblnIsZlhis, _
                            " ( Select Table_Name 表名 From Dba_Tab_Statistics Where Num_Rows Between " & lngMinSize & " And " & lngMaxSize & vbNewLine & strSQL, _
                            " ( Select Table_Name from Dba_Tab_Statistics  where Num_Rows >1000000") & ")"

        Case 3
        
            If gblnIsZlhis Then Exit Function
...
'日志方法
Public Sub TraceFileWrite(Optional ByVal strText As String, Optional ByVal lttAddTime As LogTimeType = LTT_None, Optional ByVal lngReturnLines As Long)
...
'     lngReturnLines=在日志文本之后写多少行换行符,0-不写换行符
    Dim strTracePath As String
    
    '判断文件\目录是否存在
    If mobjTrace Is Nothing Then
        strTracePath = App.Path & IIf(Right(App.Path, 1) = "\", "", "\") & "Log"
        gstrTraceFile = strTracePath & "\" & App.EXEName & CStr(Format(Now, "YYYY-MM-DD")) & ".log"
        
        If Not gobjFSO.FolderExists(strTracePath) Then
            gobjFSO.CreateFolder strTracePath
        End If
        If Not gobjFSO.FileExists(gstrTraceFile) Then
            gobjFSO.CreateTextFile gstrTraceFile
        End If
        
        Set mobjTrace = gobjFSO.OpenTextFile(gstrTraceFile, ForAppending)
    End If

    '添加日志时间
    If lttAddTime <> LTT_None Then strText = LogTime(lttAddTime = LTT_OnlyTime) & vbNewLine & strText

    '写日志文本
    If strText = "" Then
         mobjTrace.WriteBlankLines 1
    Else
        mobjTrace.Write strText
        If lngReturnLines > 0 Then mobjTrace.WriteBlankLines lngReturnLines
    End If

End Sub

Public Sub TraceFileClose()
    If Not mobjTrace Is Nothing Then
        mobjTrace.Close
        Set mobjTrace = Nothing
    End If
End Sub
