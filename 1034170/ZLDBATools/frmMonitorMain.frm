   ...
   Begin VB.PictureBox pctReport 
      ...
      Begin VB.PictureBox pctAWRSQL 
         ...
         Begin VB.Label lblSQLAWR 
            ...
            Height          =   180
            Left            =   2280
            MouseIcon       =   "frmMonitorMain.frx":009D
            TabIndex        =   29
            Top             =   120
            Width           =   1260
         End
         Begin VB.Label lblSqlID 
            BackStyle       =   0  'Transparent
            Caption         =   "SQLID"
            Height          =   180
            Left            =   120
...
Private mstrAWRSelected As String  '记录表格的选中行，用于获取AWR报告,记录形式如： _1,_2
Private mstrActiveFile(3) As String '查看的文件
Private mdblDBID  As Double
Private mstrADDMSelected As String

'记录当前AWR报告的 Inst_ID\Bid\Eid
Private mintInst As Integer
Private mlngBid As Long
Private mlngEid As Long
    ...
    Select Case cmdGetReport.Caption
        Case "查看AWR报告"
            ...
            With vsfReport
                ...
                mdblDBID = .TextMatrix(intRow1, .ColIndex("Dbid"))
                mintInst = cboInst.ItemData(cboInst.ListIndex)
                mlngBid = .TextMatrix(IIf(intRow1 > intRow2, intRow1, intRow2), .ColIndex("Snap_Id"))
                mlngEid = .TextMatrix(IIf(intRow1 < intRow2, intRow1, intRow2), .ColIndex("Snap_Id"))
            ...
            Set rsTmp = OpenSQLRecord(strSQL, "GetAWR", mdblDBID, mintInst, mlngBid, mlngEid)
...
'功能：根据传入的Tab号加载表格数据
    ...
    Exit Sub
errH:
    If InStr(1, Err.Description, "ORA-01843", vbTextCompare) > 0 Then
        MsgBox "本地Oracle客户端版本过低，部分功能无法使用。" & vbNewLine & _
                    "为确保工具正常使用，请安装10g以上的32位客户端。"
    Else
        MsgBox Err.Description
    End If
...
Private Sub lblSQLAWR_Click()
    Dim strSQL  As String, rsTmp As New ADODB.Recordset
    Dim strFile As String, strHtml As String, strSqlID As String
    
    On Error GoTo errH
    strSqlID = Trim(txtSQLID.Text)
    
    If Len(strSqlID) <> 13 Or InStr(1, "'", strSqlID) > 0 Then
        MsgBox "请输入合法的SQLID", , "提示"
        Exit Sub
    End If
    
    strSQL = "Select Output From Table(Dbms_Workload_Repository.Awr_Sql_Report_Html([1], [2], [3],[4],[5])) A "
    Set rsTmp = OpenSQLRecord(strSQL, "SQLAWR", mdblDBID, mintInst, mlngBid, mlngEid, txtSQLID.Text)
   
     If rsTmp.RecordCount = 0 Then
        MsgBox "语句" & txtSQLID.Text & "在当前快照期间没有生成AWR报告", , "提示"
        Exit Sub
     End If
    ...
    strFile = gstrFilePath & "SqlAwrTmp.html"
    gobjFile.CreateTextFile(strFile).Write strHtml
    
    frmAwrSql.ShowMe txtSQLID.Text, strFile
    Exit Sub
errH:
   MsgBox "获取语句" & txtSQLID.Text & "的AWR报告发生错误" & vbNewLine & "错误原因: " & vbNewLine & Err.Description, , "错误"
End Sub
...
Private Sub pctLine_MouseMove(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    Select Case mintActiveTab
        Case tabAWR
            ...
            pctAWRSQL.Top = webReport.Top + 120
        Case tabAddm
...
Private Sub pctReport_Resize()
    ...
    pctAWRSQL.Left = webReport.Width + webReport.Left - pctAWRSQL.Width - 300
    pctAWRSQL.Top = webReport.Top + 120
End Sub
...
Private Sub txtSQLID_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        lblSQLAWR_Click
    End If
End Sub
