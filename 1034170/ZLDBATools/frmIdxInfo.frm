...
'功能： 初始化表格，加载表格数据。
    ...
    If gblnIsZlhis Then
        ...
        If gblnHasZltables Then
            strTblRange = " And C.Owner In (Select 所有者 From zlsystems Union Select 'ZLTOOLS' From dual) " & vbNewLine & _
                                    IIf(chkZlhis.Value = 1, "And c.Table_Name Not  In (Select 表名 From Zltables Where 分类 In ('A1', 'A2', 'A3'))", "") & vbNewLine
        Else
            strTblRange = "And C.Owner In (Select  所有者 From zlsystems Union Select 'ZLTOOLS' From dual) " & vbNewLine & _
                                    IIf(chkZlhis.Value = 1, "And  c.Table_Name Not In (Select 表名 From zlBaseCode) ", "") & vbNewLine
        End If
    Else
        strTblRange = "And C.Table_Name in (Select Table_Name From Dba_Tables Where Num_Rows > " & Val(txtRange.Text) & ")" & vbNewLine
    End If
    ...
    gstrSQL = "Select Main_Table, Child_Table, Foreign_Key, Columns, Delete_Rule, Owner" & vbNewLine & _
                        ...
                        "c.Table_Name <> b.Table_Name" & vbNewLine & _
                        strTblRange & _
