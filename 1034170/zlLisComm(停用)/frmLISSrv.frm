...
Private Sub DevComm_DataReceived(Index As Integer)
    If Index = 0 Then Exit Sub
    If blnDataReceived Then Exit Sub
    
    blnDataReceived = True
    With Me.WinsockS
        .SendData .LocalIP & ";" & strSend_Refresh
    End With
    
'    ListSeq strWhere
'    Call SendMessage(ParentWnd.hwnd, WM_PAINT, 0, 0)
...
Private Sub DevComm_DevOnComm(Index As Integer, ByVal comPort As String, ByVal lngEvent As Long, ByVal strR As String)
    Dim strCOM As String
    If frmView Is Nothing Then Exit Sub
    
    If InStr(comPort, ".") <= 0 Then strCOM = "COM" & Val(comPort)
    
    If InStr(cboDev.List(cboDev.ListIndex), strCOM) > 0 Then
        If strR <> "" Then
         '  显示收到的信息
            Call frmView.ShowInOut(0, strR, lngEvent)
        End If
    End If
End Sub

Private Sub DevComm_DevSenComm(Index As Integer, ByVal comPort As String, ByVal strR As String, ByVal intErr As Integer)
    Dim strCOM As String
    If frmView Is Nothing Then Exit Sub
    ...
    If InStr(cboDev.List(cboDev.ListIndex), strCOM) > 0 Then
        If strR <> "" Then
         '  显示发送的信息
            Call frmView.ShowInOut(IIf(intErr = 0, 1, 2), strR)
        End If
    End If
End Sub
...
Private Sub mnuFileSetup_Click()
    ...
    If frmParaSet.ShowMe(Me) Then
        If Not ReadPara Then Unload Me
    End If
End Sub
...
Private Function ReadPara() As Boolean
    ...
    If gblnFromDB Then
        ...
        '从数据库读参数
        '设置格式:  仪器id,类型,COM口,波特率,数据位,校验位,停止位,握手,TCPIP端口,IP地址,字符模式,另存为的仪器ID,主机,自动应答
            ...
            For i = LBound(varSet) To UBound(varSet)
                ...
                If varSet(i) <> "" Then
                    ...
                    If lngID > 0 Then
                        ...
                        Set rsTmp = gobjDatabase.OpenSQLRecord("Select 编码,名称 From 检验仪器 where ID=[1]", "取另存检验仪器名", lngID)
                        Do Until rsTmp.EOF
                        ...
                        If g仪器(i).编码名称 <> "" Then
                            ...
                            Loop
                             ...
                             Me.DevComm(Me.DevComm.Count - 1).InitContrl i
...
Private Function CheckFiled(ByVal strTable As String, ByVal strFileds As String) As Boolean
    ...
    strSQL = "Select Data_Type As 类型, Data_Precision As 整数, Data_Scale As 小数, Data_Length As 长度" & vbNewLine & _
            ...
            "Where upper(Table_Name) = [1] And upper(Column_Name) = [2]"
