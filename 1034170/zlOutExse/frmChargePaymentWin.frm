...
Private Sub LoadData()
 ...
 gstrSQL = gstrSQL & " Union ALL " & _
    "   Select   A.ID,Mod(A.记录性质,10) as 记录性质,A.结算方式,nvl(b.结算金额,0) as 冲预交,A.摘要,A.卡类别ID,A.结算卡序号, " & _
    ...
    "              卡消费接口目录 M" & IIf(mbln异常作废, ",Table( f_Num2list( [3])) Q ", "") & _
...
Private Function DelInsureSingle(ByVal blnExistThreeBalance As Boolean, ByRef strSaveCussNo As String) As Boolean
    ...
    For i = UBound(varData) To 0 Step -1
        ...
        If dbl误差金额 <> 0 Then
            strSQL = "zl_门诊收费误差_Insert('" & varData(i) & "'," & dbl误差金额 & ",1)"
            zlAddArray cllPro, strSQL
        End If
        '先产生票据，医保接口才能取到
...
Private Function DelSwapThree(ByVal str冲销IDs As String, ByVal lng退费结算序号 As String, blnCommit As Boolean) As Boolean
    ...
    "             Sum(A.冲预交) as 冲预交" & _
    "   From 病人预交记录 A ,医疗卡类别 C,卡消费接口目录 M" & _
...
Private Function DelInsureMulitCallOneInterfrace(ByVal varNos As Variant, ByRef blnCommit As Boolean) As Boolean
    ...
    MsgBox "应退金额" & vbCrLf & NeedName(cbo支付方式.Text) & "：" & Format(dbl退款合计, "0.00") & "元", vbInformation + vbOKOnly, gstrSysName
...
Private Function BrushcardStrikePrepay() As Boolean
    ...
    If zlDatabase.PatiIdentify(Me, glngSys, mlng病人ID, Val(txt冲预交), mlngModule, 1, mlngBrushCardTypeID) Then
        lbl冲预交.Tag = "1"
       ' txt冲预交.ForeColor = d
       txt冲预交.BackColor = Me.BackColor
...
Private Function SaveCharge(Optional bln预交 As Boolean) As Boolean
    ...
    Dim dblMoney As Double, strErrMsg As String, dbl退支付额 As Double
    ...
    ElseIf mCurCardPay.bln支票 Then
        ...
        Else
            ...
            If dbl剩余金额 < 0 Then
                ...
                End If
                dbl退支付额 = -1 * Val(txt找补.Text)
                mstrBalances = mstrBalances & "|" & mstr退支票 & ":" & -1 * dbl退支付额 & ":2"
            End If
        End If
    Else
        '问题:58344
...
GoOver:
    ...
    If Not blnHaveMoney Then
        If mlng结算ID = 0 Then
            ...
            blnSaveBilling = True
        End If
        If ChargeOver(blnNotCommit, dbl退支付额) = False Then
