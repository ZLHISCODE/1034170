...
'缓存开单人，当权限或模块变化则重新读取
Private mstrDoctorFilter As String
...
Public Sub GetDoctor(lng科室ID As Long, ByRef rsTmp As ADODB.Recordset, _
    ...
    On Error GoTo errH
    strDoctorFilter = IIf(bln仅操作员部门, "1", "0") & ";" & str部门性质
    ...
    strSQL = _
        "Select Distinct a.Id, b.部门id, a.编号, a.姓名, Upper(a.简码) As 简码, c.人员性质," & vbNewLine & _
        "       Nvl(a.聘任技术职务, 0) As 职务, a.专业技术职务, b.缺省, d.服务对象" & vbNewLine & _
        "From 人员表 A, 部门人员 B, 人员性质说明 C, 部门性质说明 D" & vbNewLine & _
        "Where a.Id = b.人员id And a.Id = c.人员id And b.部门id = d.部门id" & vbNewLine & _
        "      And c.人员性质 In ('医生', '护士') And d.服务对象 In (1, 2, 3)" & vbNewLine & _
        "      And  (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null)" & vbNewLine & _
        "      And (a.站点 = '" & gstrNodeNo & "' Or a.站点 Is Null)" & strWhere
    If mrsDoctor Is Nothing Then
        Set mrsDoctor = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", UserInfo.ID, str部门性质)
    ElseIf mrsDoctor.State <> adStateOpen Then
        Set mrsDoctor = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", UserInfo.ID, str部门性质)
    ElseIf mstrDoctorFilter <> strDoctorFilter Then
        '权限或模块变化了也要重新读数据
        Set mrsDoctor = zlDatabase.OpenSQLRecord(strSQL, "mdlOutExse", UserInfo.ID, str部门性质)
    End If
    mstrDoctorFilter = strDoctorFilter
   
    '开单人允许有护士，并且收费项目允许治疗及材料时才读取护士
    bln护士 = gbln护士 And (gstr收费类别 = "" Or gstr收费类别 Like "*'E'*" Or gstr收费类别 Like "*'M'*" Or gstr收费类别 Like "*'4'*")
    mrsDoctor.Filter = _
        "(服务对象=" & gint病人来源 & _
            IIf(lng科室ID = 0, "", " And 部门ID=" & lng科室ID) & _
            IIf(bln护士, "", " And 人员性质='医生'") & _
        ") Or " & _
        "(服务对象=3" & _
            IIf(lng科室ID = 0, "", " And 部门ID=" & lng科室ID) & _
            IIf(bln护士, "", " And 人员性质='医生'") & _
    ...
    Set rsTmp = zlDatabase.CopyNewRec(mrsDoctor, , "ID,部门ID,编号,姓名,简码,人员性质,职务,专业技术职务,缺省")
    
    If rsTmp.RecordCount > 0 Then
        rsTmp.Sort = IIf(gbyt开单人显示 = 1, "简码", "编号") & ",缺省 Desc"
    End If
...
Public Sub SetBalanceVal(colBalance As Collection, ByVal intPage As Integer, ByVal strBalance As String)
...
Public Function CheckInsureBalanceValid(ByRef rs结算方式 As ADODB.Recordset, _
    ...
    Dim p As Integer, i As Integer, strNone As String
    Dim arrBalance As Variant
    
    On Error GoTo ErrHandler
    If colBalance Is Nothing Then Exit Function
    
    For p = 1 To colBalance.Count
        For i = 0 To UBound(colBalance(p))
            '结算方式;原始(最大)金额;可否修改;改后金额
            arrBalance = Split(colBalance(p)(i), ";")
            
            If rs结算方式 Is Nothing Then
                If InStr("," & strNone & ",", "," & arrBalance(0) & ",") = 0 Then
                    strNone = strNone & "," & arrBalance(0)
                End If
            Else
                rs结算方式.Filter = "(名称='" & arrBalance(0) & "' And 性质=3) Or (名称='" & arrBalance(0) & "' And 性质=4)"
                If rs结算方式.EOF Then
                    If InStr("," & strNone & ",", "," & arrBalance(0) & ",") = 0 Then
                        strNone = strNone & "," & arrBalance(0)
                    End If
