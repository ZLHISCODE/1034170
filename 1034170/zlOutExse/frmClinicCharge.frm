...
Private Sub cmdRegist_Click()
    ...
    Dim blnBillPrintTerminated As Boolean
    ...
    If blnBillPrintTerminated Then GoTo ReCreateBillPrint:
...
ErrHandler:
    ...
    If blnBillPrintTerminated = False Then Exit Sub
ReCreateBillPrint:
    On Error Resume Next
...
Private Function InsurePreSwapAll(ByVal strDate As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:门诊预结算(多单据一次结算)
    '编制:刘兴洪
    ...
    If strBalance <> "" Then
        ...
        Call SetBalanceVal(mcolBalance, p, strBalance)
    End If
...
Private Function 门诊预结算(ByRef strNone As String) As Boolean
    ...
    If MCPAR.多单据分单据结算 Then
        If InsurePreSwapNo(strDate) = False Then Exit Function
    ElseIf MCPAR.一次结算分单据退费 Then
        If InsurePreSwapDelNo(strDate) = False Then Exit Function
    Else
        If InsurePreSwapAll(strDate) = False Then Exit Function
    End If
    strNone = CheckInsureBalanceValid(mrs结算方式, mcolBalance)
...
Private Function InsurePreSwapNo(ByVal strDate As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:门诊预结区分单据
    '返回:成功,返回true,否则返回false
    '编制:刘兴洪
...
Private Function InsurePreSwapDelNo(ByVal strDate As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:门诊预结，一次结算分单据退费
    '返回:成功,返回true,否则返回false
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    For i = 0 To UBound(varAdvance)
        ...
        '报销方式;金额;是否允许修改|....
        SetBalanceVal mcolBalance, p, str结算方式
    Next
...
ReDO:
    ...
    On Error GoTo errH
    If strSQL <> "" Then
        Set mrsInfo = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, Val(Mid(strInput, 2)), strInput, strTemp)
