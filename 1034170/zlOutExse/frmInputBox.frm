...
Public Function InputBox(frmParent As Object, ByVal 标题 As String, ByVal 提示 As String, _
    ...
    ByVal 允许回车 As Boolean, ByRef 内容 As String, _
    Optional ByRef blnNotCancelCmd As Boolean, Optional sngLeft As Single = 0, Optional sngTop As Single = 0) As Boolean
    '------------------------------------------------------------------------------------------------------------------------
    '功能：程序入口:进入输入框
    '入参：frmParent-主窗体
    '          标题-窗体标题
    '          提示-输入框的提示
    '           blnNotCancelCmd:true-取消取消按钮(仅只有确定按钮),false -两种按钮都有
    '出参：
    '返回：点击确定时,返回true,否则返回False
    '编制：刘兴洪
    '日期：2010-05-27 17:01:46
    '说明：
    '------------------------------------------------------------------------------------------------------------------------
    ...
    End If
    txt.Visible = True:  cmdCancel.Visible = Not blnNotCancelCmd
    Caption = 标题
    lblInfo.Caption = 提示
    txt.MaxLength = 长度
    ...
    If sngLeft <> 0 Then Me.Left = sngLeft
    If sngTop <> 0 Then Me.Top = sngTop
    If blnNotCancelCmd Then
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    If mblnOK = False And cmdCancel.Visible = False Then
        Cancel = 1: Exit Sub
    End If
End Sub
