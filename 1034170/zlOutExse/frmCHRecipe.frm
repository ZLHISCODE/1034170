...
Option Explicit
'入口参数：
Private mbytInFun As Byte '0-收费,1-划价,2-门诊记帐
...
Private mcur非中药金额 As Currency
Private mbln允许不设置医保项目 As Boolean
Private mcur病人余额 As Currency
...
Public Function ShowMe(frmParent As Object, ByVal strPrivs As String, ByVal lngModul As Long, ByVal bytFun As Byte, ByVal bytBilling As Byte, ByVal curModiMoney As Currency, _
    ByVal lng病人ID As Long, ByVal lng病人科室ID As Long, ByVal lng开单科室ID As Long, ByVal lng中药房 As Long, _
    ByVal objDetails As BillDetails, ByVal str费别 As String, ByVal str动态费别 As String, _
    ByVal int险类 As Integer, ByVal bln加班 As Boolean, ByVal str煎法 As String, rsWarn As ADODB.Recordset, ByVal colStock As Collection, _
    ...
    mint险类 = int险类
    If mint险类 <> 0 Then mbln允许不设置医保项目 = gclsInsure.GetCapability(support允许不设置医保项目, mlng病人ID, int险类)
    ...
    For i = 1 To objDetails.Count
        With objDetails(i)
            If .收费类别 = "7" Then
                Call mobjDetails.Add(.费别, .Detail, .收费细目ID, .序号, .从属父号, _
                    .收费类别, .计算单位, .发药窗口, .付数, .数次, .附加标志, .执行部门ID, _
                    .InComes, "", .保险项目否, .保险大类ID, .保险编码, .摘要)
             Else
...
Private Sub cbo药房_Click()
    ...
    Dim strSQL As String, i As Integer, strIDs As String
        ...
        With mobjDetails(i)
            If .付数 * .数次 > .Detail.库存 Then
                If InStr(1, mstrPrivs, "显示库存") > 0 Then
                    strStock = FormatEx(.Detail.库存, 5) & IIf(gbln药房单位, .Detail.药房单位, .计算单位)
                End If
                ...
                If Not gbln分离发药 Then
                    If .Detail.分批 Or .Detail.变价 Then
                        ...
                        Exit For
                    ElseIf mcolStock("_" & .执行部门ID) <> 0 And InStr(mstrPrivs, "不检查库存") = 0 Then
                        ...
                        End If
                    End If
                ElseIf gblnStock And InStr(mstrPrivs, "不检查库存") = 0 And gstr中药房 <> "" Then
                    MsgBox "[" & .Detail.名称 & "]的当前库存" & strStock & " 不足输入数量。", vbInformation, gstrSysName
                    Exit For
                End If
            End If
            ...
            For j = 0 To .COLS - 1 Step MCOLS
                ...
                For q = 1 To mobjDetails.Count
                    If lng药名ID = mobjDetails(q).Detail.药名ID Then
                        '重新赋值
                        With mobjDetails(q)
                            ObjBillDetails.Add .费别, .Detail, .收费细目ID, intRow, .从属父号, .收费类别, .计算单位, .发药窗口, .付数, .数次, .附加标志, .执行部门ID, .InComes, _
                                 .Key, .保险项目否, .保险大类ID, .保险编码, .摘要, .原始数量, .原始执行部门ID
                        End With
                        intRow = intRow + 1
                    End If
                Next
...
Private Sub cmd形态_Click()
    ...
    With vsBill
        ...
        If zlGetDetail(lng药品ID, dbl数量, objDetail) = False Then
            Exit Sub
        End If
        dbl数量 = RoundEx(dbl数量 / IIf(objDetail.剂量系数 = 0, 1, objDetail.剂量系数), 5)
...
'功能：初始化相应的数据
    ...
    '读取病人信息
    If mlng病人ID <> 0 Then
        strSQL = "Select A.姓名,A.病人ID,Nvl(A.当前病区ID,0) as 当前病区ID,A.担保额, Nvl(B.预交余额,0) as 预交余额,Nvl(B.费用余额,0) as 费用余额" & _
            IIf(mbytInFun = 2, ",zl_PatiDayCharge(A.病人ID) as 当日额, Zl_Patiwarnscheme(A.病人id) As 适用病人", "") & _
            " From 病人信息 A,病人余额 B" & _
            " Where A.病人ID=B.病人ID(+) And B.性质(+)=1" & _
            " And A.病人ID=[1]"
        Set mrsPati = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病人ID)
        
        If mbytInFun = 2 And Not mrsWarn Is Nothing Then
            mcur病人余额 = mrsPati!预交余额 - mrsPati!费用余额
            If gbln报警包含划价费用 Then mcur病人余额 = mcur病人余额 - GetPriceMoneyTotal(0, mlng病人ID) + IIf(mbytBilling = 1, mcurModiMoney, 0)   '修改划价单且报警要算划价单时,加当前单据金额
        End If
    End If
    ...
    If gbln分离发药 Then
        ...
        cbo药房.Visible = False
    Else
        Set rsTmp = GetDepartments("'中药房'", gint病人来源 & ",3")
...
'功能：全部刷新显示当前配方内容
        ...
        For i = 1 To UBound(varData)
            ...
            For j = 1 To mobjDetails.Count
                If Val(varData(i)) = mobjDetails(j).Detail.药名ID Then
                    ...
                    If gbln药房单位 Then    '52722
                        dblTemp = dblTemp * mobjDetails(j).Detail.药房包装
                    End If
                    dbl数量 = dbl数量 + dblTemp
    ...
    If mobjDetails.Count > 0 Then
        If Not gbln分离发药 Then
            cbo药房.ListIndex = FindCboIndex(cbo药房, mobjDetails(1).执行部门ID)
        End If
         ...
         If str规格数量 <> "" Then
                ...
                If rsTemp.RecordCount > 0 Then
                    ...
                    If lng药品ID <> Val(rsTemp!药品ID) Then
                        If mlng病人ID <> 0 Then
                            lngTemp = Get收费执行科室ID("7", Val(Nvl(rsTemp!药品ID)), Nvl(rsTemp!执行科室, 0), mlng病人科室ID, mlng开单科室ID, gint病人来源, , , mlng中药房, , mrsPati!当前病区ID)
                        Else
                            lngTemp = Get收费执行科室ID("7", Val(Nvl(rsTemp!药品ID)), Nvl(rsTemp!执行科室, 0), mlng病人科室ID, mlng开单科室ID, gint病人来源, , , mlng中药房)
                        End If
                       '设置中药房
                        If Not gbln分离发药 Then
                            If lngTemp <> lng药房ID And lngTemp <> 0 Then
                                cbo药房.ListIndex = FindCboIndex(cbo药房, lngTemp)
                            End If
                        End If
                    End If
                End If
...
Private Sub txt付数_Validate(Cancel As Boolean)
    ...
    If mobjDetails.Count > 0 Then
        If Val(txt付数.Text) = mobjDetails(1).付数 Then Exit Sub
    End If
     If Get中药形态 = 0 Then
        '散装形态的,先要检查库存
        For i = 1 To mobjDetails.Count
    ...
    If KeyAscii = 13 Then
        ...
        ElseIf Col Mod MCOLS = col剂量 Then
            ...
            If int中药形态 = 0 Then
                ...
                If CheckStock(lng药品ID, RoundEx(Val(strTmp) / dbl系数, 5)) = False Then
                     vsBill.TextMatrix(Row, Col) = CStr(vsBill.Cell(flexcpData, Row, Col)): Exit Sub
                End If
            Else
            ...
            Call GetBillTotal(cur应收, cur实收, blnOverFlow)
            If blnOverFlow Then
                '溢出,恢复输入
            ...
            If mbytInFun = 2 Then
                ...
                If Not mrsPati Is Nothing And Not mrsWarn Is Nothing Then
                    Call GetBillTotal(, cur实收)
                    If cur实收 > 0 Then
                        gbytWarn = BillingWarn(mstrPrivs, mrsPati!姓名, mrsPati!适用病人, mrsWarn, _
                            mcur病人余额, Val("" & mrsPati!当日额) - mcurModiMoney, cur实收 + mcur非中药金额, Val("" & mrsPati!担保额), "7", "中草药", mstrWarn)
                            
                        If gbytWarn = 2 Or gbytWarn = 3 Then
                            Call 分解中药规格(lng药名ID, Val(vsBill.TextMatrix(Row, Col)))
                            Call Show中药规格(lng药名ID, Val(vsBill.TextMatrix(Row, Col)))
                            vsBill.TextMatrix(Row, Col) = CStr(vsBill.Cell(flexcpData, Row, Col))
                            sta.Panels(2).Text = "预交:" & Format(mrsPati!预交余额, "0.00") & "/费用:" & Format(mrsPati!预交余额 - mcur病人余额, "0.00") & "/剩余:" & Format(mcur病人余额, "0.00")
                            Exit Sub
                        End If
                    End If
                End If
...
'功能：重新计算指定药品行的价格和金额
    Dim rsTmp As ADODB.Recordset
    Dim objInCome As New BillInCome
    
    Dim dblAllTime As Double
    Dim dblMoney As Double, dblPrice As Double, dbl加班加价率 As Double
    Dim str费别 As String, cur金额 As Currency
    Dim strInfo As String, strSQL As String, i As Long, strAdvance As String, dblPriceSingle As Double
    ...
    If Not rsTmp.EOF Then
        ...
        If objBillDetail.Detail.变价 Then
            If InStr(",5,6,7,", objBillDetail.收费类别) > 0 Then
                ...
                If gbln药房单位 Then
                    '库存时价按售价数量进行计算
                    dblAllTime = dblAllTime * objBillDetail.Detail.药房包装
                End If
                If dblAllTime <> 0 Then
                    dblPrice = Get时价药品应收金额(objBillDetail.执行部门ID, objBillDetail.收费细目ID, objBillDetail.收费类别 = "4", dblAllTime, gstrDec, dblPriceSingle)
                    If dblAllTime <> 0 Then
                        '数量未分解完毕
            ...
            With objInCome
                ...
                If objBillDetail.Detail.变价 Then
                    If InStr(",5,6,7,", objBillDetail.收费类别) > 0 And gbln药房单位 Then
                        .标准单价 = Format(dblMoney * objBillDetail.Detail.药房包装, gstrFeePrecisionFmt)
                    Else
                        .标准单价 = Format(dblMoney, gstrFeePrecisionFmt)
                    End If
                Else
                    If InStr(",5,6,7,", objBillDetail.收费类别) > 0 And gbln药房单位 Then
                        .标准单价 = Format(Nvl(rsTmp!现价, 0) * objBillDetail.Detail.药房包装, gstrFeePrecisionFmt)
                    Else
                ...
                dblAllTime = objBillDetail.付数 * objBillDetail.数次
                If gbln药房单位 Then dblAllTime = dblAllTime * objBillDetail.Detail.药房包装

                If objBillDetail.Detail.屏蔽费别 Then
                    .实收金额 = .应收金额
                    objBillDetail.费别 = mstr费别
                Else
                    str费别 = IIf(glngSys Like "8??" Or mbytInFun = 2, mstr费别, mstr动态费别)
                    
                    .实收金额 = CCur(Format(ActualMoney(str费别, .收入项目ID, .应收金额, _
                        objBillDetail.收费细目ID, objBillDetail.执行部门ID, dblAllTime, dbl加班加价率), gstrDec))
                    objBillDetail.费别 = str费别
                End If

                '获取项目保险信息,门诊只有医保病人才算
                If mint险类 <> 0 And mbytInFun = 0 Then
...
Private Sub GetBillTotal(Optional cur应收 As Currency, Optional cur实收 As Currency, Optional blnOvweFlow As Boolean)
...
Public Function zl获取中药形态(Optional lngRow As Long = -1) As Integer
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取单据是否录入了中草药的
    '入参:intPage-当前第几页
    '出参:
    ...
    zl获取中药形态 = -1
    '如果未指定页,则用当前页
    If mobjDetails Is Nothing Then
        zl获取中药形态 = mint中药形态
        Exit Function
    End If
    If mint中药形态 <> -1 Then
        zl获取中药形态 = mint中药形态: Exit Function
    End If
        
    With mobjDetails
        For i = 1 To .Count
            If InStr(1, ",6,7,", "," & .Item(i).收费类别 & ",") > 0 And .Item(i).收费细目ID <> 0 And lngRow <> i Then
...
Private Function CheckStock(ByVal lng药品ID As Long, dbl数量 As Double, Optional objDetail As Detail) As Boolean
    ...
    With objDetail
        If InStr(1, mstrPrivs, "显示库存") > 0 Then
            strStock = FormatEx(.库存, 5) & IIf(gbln药房单位, .药房单位, .计算单位)
        End If
                
        If Not gbln分离发药 Then
            If .分批 Or .变价 Then
                If Val(txt付数.Text) * dbl数量 > .库存 Then
                    MsgBox """" & .名称 & """为分批或时价药品，当前可用库存" & strStock & " 不足输入数量。", vbInformation, gstrSysName
                    Exit Function
                End If
            ElseIf mcolStock("_" & lng药房ID) <> 0 And InStr(mstrPrivs, "不检查库存") = 0 Then
                If Val(txt付数.Text) * dbl数量 > .库存 Then
                    If mcolStock("_" & lng药房ID) = 1 Then
                        If MsgBox("""" & .名称 & """的当前可用库存" & strStock & " 不足输入数量，要继续吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
                             Exit Function
                        End If
                    ElseIf mcolStock("_" & lng药房ID) = 2 Then
                        MsgBox """" & .名称 & """的当前可用库存" & strStock & " 不足输入数量。", vbInformation, gstrSysName
                         Exit Function
                    End If
                End If
            End If
...
Private Function SelectChineDrug(ByVal strInput As String) As Boolean
    ...
    Dim blnYb As Boolean, strSubTable As String
    
    int中药形态 = Get中药形态
    '门诊费用多这个条件mbytFun = 0 And,排开了划价

    If cbo药房.ListIndex < 0 Then cbo药房.ListIndex = FindCboIndex(cbo药房, lng药房ID)
    If cbo药房.ListIndex < 0 And cbo药房.ListCount > 0 Then cbo药房.ListIndex = 0
    If cbo药房.ListIndex < 0 Then
    ...
    If strInput <> "" Then
            ...
            If int中药形态 = 0 Then
                ...
                "       Decode(D.是否变价,1,'时价',LTrim(To_Char(Sum(F.现价)" & _
                        ...
                        IIf(InStr(1, mstrPrivs, "显示库存") > 0, " LTrim(To_Char(X.库存" & IIf(gbln药房单位, "/Nvl(C." & gstr药房包装 & " ,1)", "") & ",'9999990.00000'))", "Decode(Sign(X.库存),1,'有','无')") & " as 库存" & _
                " From 药品特性 E,药品规格 C,收费项目目录 D,收费价目 F, " & vbNewLine & _
                            IIf(blnYb, "保险支付项目 M,保险支付大类 N,", "") & vbNewLine & _
                ...
                "        And exists(Select 1 From 收费执行科室 A1 Where A1.收费细目ID=C.药品ID And A1.执行科室ID=[4]   And (A1.病人来源 is NULL Or A1.病人来源=[7]) and (A1.开单科室ID is null or A1.开单科室ID=[8])  ) " & vbNewLine & _
                         IIf(blnYb, " And C.药品ID=M.收费细目ID(+) And M.险类(+)=[5] And M.大类ID=N.ID(+)" & vbNewLine, "") & _
                "        And Sysdate Between F.执行日期 and Nvl(F.终止日期,TO_DATE('3000-01-01','YYYY-MM-DD'))" & _
                         str规格 & str特性 & str特准项目 & _
                " Group by A.ID,A.计算单位,C.药品ID,D.编码,A.名称,D.规格,D.产地,D.费用类型,d.执行科室,D.是否变价," & IIf(blnYb, "N.名称,", "") & "X.库存," & _
                        IIf(gbln药房单位, "C." & gstr药房单位 & ",C." & gstr药房包装, "D.计算单位") & _
                " Order by D.编码"
            Else
                 '非散装时按品种显示，且不显示库存
                strSQL = strSQL & _
            ...
            vPoint = GetCoordPos(vsBill.hWnd, vsBill.CellLeft, vsBill.CellTop)
            Set rsTemp = zlDatabase.ShowSQLSelect(Me, strSQL, 0, "中草药", False, "", "", False, False, True, vPoint.X, vPoint.Y, _
                        vsBill.CellHeight, blnCancel, True, True, strInput & "%", gstrLike & strInput & "%", gbytCode + 1, lng药房ID, mint险类, int中药形态, gint病人来源, mlng开单科室ID)
    Else
            If int中药形态 = 0 Then
                '散装才显示到规格级,保持原来不变
                ...
                "       NULL as 产地,NULL as 费用类型 , NULL as 执行科室_ID" & IIf(blnYb = False, "", ",Null as 医保大类") & ",NULL as 单价,NULL as 库存,NULL as 药品ID" & _
                " From 诊疗分类目录 " & _
                ...
                "  From ( " & _
                            " Select 1 as 末级,A.ID As ID,A.ID as 药名ID,A.分类ID as 上级ID,D.编码,D.名称,D.规格,A.计算单位 as 剂量单位," & _
                                        IIf(gbln药房单位, " C." & gstr药房单位, "D.计算单位") & " as 单位,D.产地,D.费用类型,d.执行科室 as 执行科室_ID" & IIf(blnYb = False, "", ",N.名称 医保大类") & "," & _
                            "           Decode(D.是否变价,1,'时价',LTrim(To_Char(Sum(F.现价)" & _
                                        IIf(gbln药房单位, "*Nvl(C." & gstr药房包装 & ",1)", "") & ",'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                                        IIf(InStr(1, mstrPrivs, "显示库存") > 0, " LTrim(To_Char(X.库存" & IIf(gbln药房单位, "/Nvl(C." & gstr药房包装 & ",1)", "") & ",'9999990.00000'))", "Decode(Sign(X.库存),1,'有','无')") & " as 库存,C.药品ID" & _
                            " From  诊疗项目目录 A,药品特性 E,药品规格 C,收费项目目录 D,收费价目 F," & _
                                        IIf(blnYb = False, "", "           保险支付项目 M,保险支付大类 N,") & _
                            "           (" & strStock & ") X" & _
                            " Where A.ID=E.药名ID And A.ID=C.药名ID And C.药品ID=D.ID And C.药品ID =F.收费细目ID And A.类别='7'  " & _
                                    IIf(blnYb = False, "", "       And C.药品ID=M.收费细目ID(+) And   M.险类(+)=" & mint险类 & " And M.大类ID=N.ID(+)") & _
                            "       And C.药品ID=X.药品ID(+) " & _
                            "        And exists(Select 1 From 收费执行科室 A1 Where A1.收费细目ID=C.药品ID And A1.执行科室ID=[4]   And (A1.病人来源 is NULL Or A1.病人来源=[7]) and (A1.开单科室ID is null or A1.开单科室ID=[8])  ) " & vbNewLine & _
                            "       And Sysdate Between F.执行日期 and Nvl(F.终止日期,TO_DATE('3000-01-01','YYYY-MM-DD'))" & _
                            "       And D.服务对象 IN(" & gint病人来源 & ",3)" & str特准项目 & str规格 & str撤档时间 & _
                            " Group by A.ID,A.计算单位,A.分类ID,D.编码,D.名称,D.规格,D.产地,D.费用类型,d.执行科室" & IIf(blnYb = False, "", ",N.名称") & ",D.是否变价,X.库存,C.药品ID," & _
                                 IIf(gbln药房单位, "C." & gstr药房单位 & ",C." & gstr药房包装, "D.计算单位") & _
                " )"
            Else
                strSQL = "" & _
                ...
                "           And exists(Select 1 From 诊疗执行科室 A1 Where A1.诊疗项目ID=A.ID And A1.执行科室ID=[4]   And (A1.病人来源 is NULL Or A1.病人来源=[7]) and (A1.开单科室ID is null or A1.开单科室ID=[8])  ) " & vbNewLine
            End If
            Set rsTemp = zlDatabase.ShowSQLSelect(Me, strSQL, 2, "中草药", False, "", "", False, True, False, 0, 0, 0, blnCancel, False, True, "", "", "", lng药房ID, "", int中药形态, gint病人来源, mlng开单科室ID)
    End If
    With vsBill
...
Public Function CheckDrugDataValied(ByVal lng药品ID As Long, Optional strName As String = "") As Boolean
    ...
    If Not rsTemp Is Nothing Then
        ...
        ElseIf (IIf(IsNull(rsTemp!价值分类), "", rsTemp!价值分类) = "贵重" _
            ...
            And InStr(mstrPrivs, "贵重药品记帐") = 0 Then
...
Private Function zlGetDetail(ByVal lng药品ID As Long, Optional dbl数量 As Double, Optional ByRef objDetail As Detail) As Boolean
    ...
    If mint险类 > 0 Then
        ...
        "       C." & gstr药房单位 & " as 药房单位,C." & gstr药房包装 & " as 药房包装,J1.名称 as 诊疗名称,J1.计算单位 as 剂量单位,C.剂量系数" & _
    ...
    If mint险类 <> 0 And Not mbln允许不设置医保项目 Then
    ...
    objDetail.药房包装 = Nvl(rsTmp!药房包装, 1)
    objDetail.药房单位 = Nvl(rsTmp!药房单位)
    ...
    If Not gbln分离发药 Then
        ...
        If gbln药房单位 Then
            objDetail.库存 = objDetail.库存 / objDetail.药房包装
        End If
    ElseIf gstr中药房 <> "" Then
...
'        If mint险类 <> 0 Then '90304
            str摘要 = gclsInsure.GetItemInfo(mint险类, mlng病人ID, lng药品ID, str摘要, 1)       '35307
'        End If
    End If
    Call Set摘要(lng药品ID, str摘要)
    Dim dblTemp As Double
    
    dblTemp = RoundEx(dbl数量 / objDetail.剂量系数, 5)
    If gbln药房单位 Then    '52722
        dblTemp = dblTemp / IIf(objDetail.药房包装 = 0, 1, objDetail.药房包装)
        Set objBillDetail = mobjDetails.Add(mstr费别, objDetail, lng药品ID, lngRow, 0, objDetail.类别, objDetail.药房单位, "", Val(txt付数.Text), dblTemp, 0, lng药房ID, objInComes, lngRow & "_" & lng药品ID, , , , str摘要)
    Else
        Set objBillDetail = mobjDetails.Add(mstr费别, objDetail, lng药品ID, lngRow, 0, objDetail.类别, objDetail.计算单位, "", Val(txt付数.Text), dblTemp, 0, lng药房ID, objInComes, lngRow & "_" & lng药品ID, , , , str摘要)
    End If
    mblnChange = True
    SetBillDetail = True
    Exit Function
...
'功能：获取当前药品的所有可用的散装规格
    ...
    If gblnStock Then
        ...
        End If
    End If
    strSQL = "Select a.药名id, a.药品id, d.规格, d.产地, a.剂量系数,A." & gstr药房单位 & " as 药房单位 ,A." & gstr药房包装 & "  as 药房包装,D.计算单位," & _
    ...
    Set Get散装规格 = zlDatabase.OpenSQLRecord(strSQL, "规格列表", lng药名ID, lng药房ID, gint病人来源, lng药房ID)
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
        ...
        Else
            ...
            Else
                strSQL = "" & _
                    ...
                    "           A." & gstr药房单位 & " as 药房单位 ,A." & gstr药房包装 & "  as 药房包装,D.计算单位 From 药品规格 A,收费项目目录 D,Table(f_Num2List([1])) B" & vbNewLine & _
            ...
            For i = .FixedRows To .Rows - 1
                ...
                Else
                    ...
                    Else
                        ...
                        .Cell(flexcpData, i, .ColIndex("剂量系数")) = RoundEx(Val(.Cell(flexcpData, i, .ColIndex("数量"))) / IIf(Val(Nvl(rsTmp!剂量系数)) = 0, 1, Val(Nvl(rsTmp!剂量系数))), 5) '售价单位数量
                        .TextMatrix(i, .ColIndex("数量")) = RoundEx(Val(.Cell(flexcpData, i, .ColIndex("数量"))) / IIf(Val(Nvl(rsTmp!剂量系数)) = 0, 1, Val(Nvl(rsTmp!剂量系数))), 5)
                        .Cell(flexcpData, i, .ColIndex("数量")) = Val(.Cell(flexcpData, i, .ColIndex("数量"))) & ":" & IIf(Val(Nvl(rsTmp!剂量系数)) = 0, 1, Val(Nvl(rsTmp!剂量系数))) & ":" & IIf(Val(Nvl(rsTmp!药房包装)) = 0, 1, Val(Nvl(rsTmp!药房包装)))
                        '用包装单位显示
                        If gbln药房单位 Then
                             .TextMatrix(i, .ColIndex("数量")) = RoundEx(Val(.TextMatrix(i, .ColIndex("数量"))) / IIf(Val(Nvl(rsTmp!药房包装)) = 0, 1, Val(Nvl(rsTmp!药房包装))), 5) & Nvl(rsTmp!药房单位)
                        Else
    ...
    With vs中药规格
        ...
        Case .ColIndex("规格")
            ...
            Else
                ...
                If 分解中药规格(Val(Nvl(rsTmp!药名ID)), dbl数量, objDetail, , 0) = False Then
                        mcll规格.Remove "_" & rsTmp!药名ID
                        mcll规格.Add str规格数量, "_" & rsTmp!药名ID
                        .TextMatrix(Row, Col) = CStr(.Cell(flexcpData, Row, Col)): Exit Sub
                End If
                If IsCheckStockEnough(lng药品ID, RoundEx(dbl数量 / IIf(Val(Nvl(rsTmp!剂量系数)) = 0, 1, Val(Nvl(rsTmp!剂量系数))), 5), objDetail) = False Then
                ...
                .Cell(flexcpData, Row, .ColIndex("剂量系数")) = RoundEx(dbl数量 / IIf(Val(Nvl(rsTmp!剂量系数)) = 0, 1, Val(Nvl(rsTmp!剂量系数))), 5)
...
Private Function Get中药规格(ByVal lng药名ID As Long, Optional ByVal lng形态 As Long = -1) As ADODB.Recordset
    ...
    Else
        ...
        Set Get中药规格 = zlDatabase.OpenSQLRecord(strSQL, "读取中药规格", lng药名ID, gint病人来源, lng形态, lng药房ID)
    End If
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
...
Private Sub ShowSpecs(ByVal lng药名ID As Long)
    ...
    "       A." & gstr药房单位 & " as 药房单位 ,A." & gstr药房包装 & "  as 药房包装,D.计算单位," & _
    "      Sum(nvl(M.可用数量,0))/nvl(A." & gstr药房包装 & ",1)  as 可用数量" & _
    ...
    "            And A.药名ID = [1]   " & IIf(lng药房ID = 0, "", " And M.库房ID=[3] ") & _
     "           And (D.撤档时间=To_Date('3000-01-01','YYYY-MM-DD') Or D.撤档时间 IS NULL) And D.服务对象 IN([2],3)" & _
     "           And (D.站点='" & gstrNodeNo & "' Or D.站点 is Null) " & _
     "  Group by E.名称,D.编码,D.规格,D.名称 ,D.计算单位,A." & gstr药房单位 & ",A." & gstr药房包装 & _
     "  Having Sum(nvl(M.可用数量,0))>0 " & _
     "  Order by D.编码"
     
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, "读取中药规格", lng药名ID, gint病人来源, lng药房ID)
    intCols = 3
    With vsSpecShow
        .Clear
        .Rows = 0: .COLS = intCols
        ...
        Do While Not rsTemp.EOF
            If intCol >= intCols Then
                ...
                intCol = 0
            End If
           .TextMatrix(lngRow, intCol) = IIf(lng药房ID = 0, "(" & Nvl(rsTemp!药房) & ")", "") & Nvl(rsTemp!规格, "无规格") & ":" & IIf(InStr(1, mstrPrivs, ";显示库存;") = 0, "有库存", Val(Nvl(rsTemp!可用数量)) & Nvl(rsTemp!药房单位))
           intCol = intCol + 1
            rsTemp.MoveNext
        Loop
        If rsTemp.RecordCount = 0 Then
