...
Begin VB.Form frmMakeupPrintBill 
   ...
   Begin VB.PictureBox PicDetail 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsDetail 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmMakeupPrintBill.frx":03F1
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.PictureBox picList 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsList 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmMakeupPrintBill.frx":0407
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   Begin VB.PictureBox picCon 
      ...
      Top             =   135
      Width           =   14475
      Begin zlIDKind.PatiIdentify PISelect 
         Height          =   255
         Left            =   60
         TabIndex        =   29
         Top             =   255
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         IDKindStr       =   "病人|病人|0|0|0|0|0|0;险类|险类|0|0|0|0|0|0"
      ...
      Begin zlIDKind.IDKindNew IDKind 
         ...
         _ExtentY        =   661
         Appearance      =   2
         IDKindStr       =   $"frmMakeupPrintBill.frx":041D
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "宋体"
            Size            =   12
            Charset         =   134
   ...
   Begin MSComctlLib.StatusBar stbThis 
      ...
      BeginProperty Panels {8E3867A5-8586-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty Panel1 {8E3867AB-8586-11D1-B16A-00C0F0283628} 
            ...
            Object.Width           =   2355
            MinWidth        =   882
            Picture         =   "frmMakeupPrintBill.frx":04B3
            Text            =   "中联软件"
            TextSave        =   "中联软件"
            Key             =   "ZLFLAG"
   ...
   Begin MSComctlLib.ImageList imlPaneIcons 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmMakeupPrintBill.frx":0D47
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmMakeupPrintBill.frx":109B
            Key             =   ""
         EndProperty
      EndProperty
   End
...
Private mintPrintMode As Integer '按1-病人/2-险类打印
Private mintPatiRow As Integer   '按险类打印时,选中的病人列表行
...
Private Function InitPanel()
    ...
    With dkpMan
        ...
        Set objTemp = .CreatePane(mPanel.Pane_Pati, 100, 50, DockBottomOf, objPane)
        objTemp.Tag = mPanel.Pane_Pati
        objTemp.Title = "病人信息": objTemp.Options = PaneNoCloseable Or PaneNoHideable
        objTemp.Handle = picPati.hWnd

        Set objPane = .CreatePane(mPanel.Pane_List, 300, 100, DockBottomOf, objTemp)
        objPane.Tag = mPanel.Pane_List
        objPane.Title = "单据列表": objPane.Options = PaneNoCloseable Or PaneNoHideable
        objPane.Handle = picList.hWnd

        Set objTemp = .CreatePane(mPanel.Pane_Balance, 100, 100, DockRightOf, objPane)
        objTemp.Tag = mPanel.Pane_Balance
        objTemp.Title = "结算信息": objTemp.Options = PaneNoCloseable Or PaneNoHideable
        objTemp.Handle = picBalance.hWnd
       '
        Set objTemp = .CreatePane(mPanel.Pane_Detail, 300, 100, DockBottomOf, objPane)
        objTemp.Tag = mPanel.Pane_Detail
        objTemp.Title = "单据明细列表": objTemp.Options = PaneNoCloseable Or PaneNoHideable
        objTemp.Handle = picDetail.hWnd
...
Private Sub cbo险类_Click()
    ...
    With cbo险类
        If mintPrintMode = 1 Then Exit Sub
        If Val(.Tag) = .ListIndex Then Exit Sub
        mintPatiRow = 0
        LoadPatiFromInsure (.ItemData(.ListIndex))
        .Tag = .ListIndex
    End With
    Exit Sub
ErrHandler:
...
Private Sub cmdBrush_Click()
    On Error GoTo ErrHandler
    If mintPrintMode = 1 Then '按病人打印
        Call ReadListData
    Else                      '按险类打印
        If cbo险类.ListIndex <> -1 Then LoadPatiFromInsure (cbo险类.ItemData(cbo险类.ListIndex))
    End If
    Exit Sub
ErrHandler:
    If ErrCenter() = 1 Then
...
Private Function CheckFP(ByVal strNos As String, ByVal strUserType As String, _
    ByVal strBillNameDemo As String, ByRef strFactNO As String, ByRef lng领用ID As Long, _
    ByRef lngShareID As Long) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:检查发票是否正确
    ...
    With vsList
        For lngRow = 1 To .Rows - 1
            If .IsSubtotal(lngRow) Then
                '分组行不处理
            ElseIf GetVsGridBoolColVal(vsList, lngRow, .ColIndex("选择")) Or mintPrintMode = 2 Then
...
Private Sub cmdOK_Click()
    Dim str病人IDs As String, i  As Integer
    Dim strPreUseType As String
    ...
    If CheckPati(str病人IDs) = False Then cmdOK.Enabled = True: Exit Sub
    For i = 0 To UBound(Split(str病人IDs, ","))
        mrsInfo.Filter = "病人ID=" & Split(str病人IDs, ",")(i)
        Call ReadListData(Split(str病人IDs, ",")(i))
        Call RePrintBill(strPreUseType)
        zlcontrol.StaShowPercent (i + 1) / (UBound(Split(str病人IDs, ",")) + 1), stbThis.Panels(2), Me
    Next
    stbThis.Panels(2) = ""
    '刷新数据
    If cbo险类.ListIndex <> -1 Then LoadPatiFromInsure (cbo险类.ItemData(cbo险类.ListIndex))
...
Private Sub RePrintBill(Optional ByRef strPreUseType As String)
    ...
    If cllPrint.Count = 0 And cllRegistNos.Count = 0 And mintPrintMode = 1 Then
    ...
    If cllPrint.Count > 0 Then
        For j = 1 To cllUseType.Count
            ...
            If InputFactNo(strUseType, strBillNameDemo, lng领用ID, lngShareUseID, strFactNO, strPreUseType) = False Then GoTo PrintEnd:
            strPreUseType = strUseType
            ...
            For i = 1 To cllPrint.Count
                ...
                If arrPrint(4) = strUseType Then
                    ...
                    If Not zlMakeupPrint(lng病人ID, arrPrint(3), strUseType, strBillNameDemo, Val(arrPrint(5)), _
                        ...
                        strFactNO, str结帐IDs) Then GoTo PrintEnd:
                    blnPrintSccess = True
...
Private Function InputFactNo(ByVal strUseType As String, ByVal strBillNameDemo As String, ByRef lng领用ID As Long, _
                             ByRef lngShareUseID As Long, ByRef strFactNO As String, _
                             Optional ByVal strPreUseType As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:输入有效的发票号
    '入参:strUseType-使用类别
...
ReInput:
    Do
        ...
        If strPreUseType <> strUseType Then
...
Private Sub Form_Load()
    ...
    Call InitPanel
    PISelect.NotAutoAppendKind = True
    PISelect.IDKindStr = "病人|病人|0|0|0|0|0|0;险类|险类|0|0|0|0|0|0"
    mintPrintMode = PISelect.IDKindIDX
    Call Init险类
    Call ResizeForm(mintPrintMode)
...
Private Sub lvwPati_ItemCheck(ByVal Item As MSComctlLib.ListItem)
...
Private Sub lvwPati_ItemClick(ByVal Item As MSComctlLib.ListItem)
    ...
    If Val(lvwPati.Tag) = Item.Index Then Exit Sub
    lvwPati.Tag = Item.Index
    mintPatiRow = Item.Index
    Call ReadListData(Mid(Item.Key, 2))
    Exit Sub
errH:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub zlClearPatiInfor()
    ...
    If lbl总计.Visible Then lbl总计.Caption = "补打总计:" & Format(0, "0.00")
...
Private Sub picPati_Resize()
    Err = 0: On Error Resume Next
    With picPati
        lvwPati.Left = .ScaleLeft
        lvwPati.Width = .ScaleWidth
        lvwPati.Height = .ScaleHeight - lbl总计.Height
        lvwPati.Top = .ScaleTop
        lbl总计.Left = .ScaleLeft
        lbl总计.Top = lvwPati.Top + lvwPati.Height
    End With
    Call picBalance_Resize
End Sub

Private Sub PISelect_ItemClick(Index As Integer, objCard As zlIDKind.Card)
    On Error GoTo ErrHandler
    Call ResizeForm(Index)
    mintPrintMode = Index
    Set mrsInfo = New ADODB.Recordset
    If mintPrintMode = 2 Then
        txtPatient.Text = ""
        If cbo险类.ListIndex = -1 Then
            MsgBox "没有发现有效险类,请先设置险类！", vbInformation, gstrSysName
            Exit Sub
        End If
        Call LoadPatiFromInsure(cbo险类.ItemData(cbo险类.ListIndex))
    Else
        Call ReadListData: Call ShowDetail
    End If
    Exit Sub
ErrHandler:
    If ErrCenter() = 1 Then
...
Private Sub ShowDetail(Optional ByVal byt记录性质 As Byte = 1, Optional ByVal strNO As String)
    ...
    On Error GoTo ErrHandler
    strSQL = _
    " Select C.名称 as 类别,Nvl(E.名称, B.名称) As 名称," & _
            IIf(gTy_System_Para.byt药品名称显示 = 2, "E1.名称 As 商品名,", "") & "B.规格," & _
...
Private Function InitBlanceData(ByVal str费用结算 As String, ByVal str挂号结算 As String) As Boolean
    ...
    strSQL = _
        ...
        "       Decode(Mod(s.记录性质, 10), 1, '冲预交', s.结算方式) As 结算方式, Sum(s.冲预交) As 金额, t.结算序号,s.病人id" & vbNewLine & _
        " From 病人预交记录 S, (" & strSQL & ") T" & vbNewLine & _
        " Where s.结算序号 = t.结算序号 Or s.结帐id = t.结算序号" & vbNewLine & _
        " Group By t.单据, t.结算序号, Decode(Mod(s.记录性质, 10), 1, '冲预交', s.结算方式),s.病人id"
    ...
    With vsList
        ...
        For lngRow = 1 To .Rows - 1
            If .IsSubtotal(lngRow) = False Then
                ...
                If str单据类型 = "挂号单" Then
                    lng结算序号 = Val(.TextMatrix(lngRow, .ColIndex("结帐ID")))
                Else
                    lng结算序号 = Val(.TextMatrix(lngRow, .ColIndex("结算序号ID")))
                End If
    ...
    With vsBalance
        ...
        For i = 1 To .Rows - 1
            ...
            Else
                ...
                .TextMatrix(i, .ColIndex("结算金额")) = Format(Val(.TextMatrix(i, .ColIndex("结算金额"))), "0.00")
            End If
        Next
        '首列合并
...
Private Function ReadListData(Optional ByVal lng病人ID As Long = 0) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:读取需要明细数据
    '参数:lng病人ID-按险类打印时传入,按病人打印时不传
    ...
    If mintPrintMode = 1 Then
        ...
        Else
            lng病人ID = Val(Nvl(mrsInfo!病人ID))
        End If
        Call LoadAllList(lng病人ID)
    Else
        If mrsList Is Nothing Then
            If cbo险类.ListIndex = -1 Then Exit Function
            lng险类 = cbo险类.ItemData(cbo险类.ListIndex)
            Call LoadAllList(lng病人ID, lng险类)
        Else
            mrsList.Filter = "病人ID=" & lng病人ID
        End If
    End If
     ...
     '   zl_vsGrid_Para_Restore mlngModule, vsList, Me.Caption, "单据列表", False
        ...
        If chkRegistFee.Value = vbChecked Then
            ...
            For i = 1 To .Rows - 1
                .MergeRow(i) = False
                If .IsSubtotal(i) Then
                    If mintPrintMode = 1 Then .TextMatrix(i, .ColIndex("选择")) = "-1"
                    For j = 0 To .COLS - 1
                        If j > IIf(mintPrintMode = 1, .ColIndex("选择"), .ColIndex("单据号") - 1) Then
        ...
        .ColHidden(.ColIndex("门诊标志")) = True
        
        .Editable = flexEDKbdMouse
        .Redraw = flexRDBuffered
        vsList_AfterRowColChange 0, 0, .Row, .Col
        .ColHidden(.ColIndex("选择")) = mintPrintMode = 2 '按险类打印单据时,隐藏“选择”列
        .ColHidden(.ColIndex("医保")) = mintPrintMode = 2 '按险类打印单据时,隐藏“医保”列
    End With
...
Private Sub InitPatiInsure()
    ...
    If mintPrintMode = 1 Then
        ...
        End With
    Else
        mintInsure = cbo险类.ItemData(cbo险类.ListIndex)
    End If
    Exit Sub
ErrHandler:
    If ErrCenter() = 1 Then
...
Private Sub Init险类()
    ...
    strSQL = "Select a.序号, a.名称, a.说明, Nvl(a.外挂, 0) As 外挂 From 保险类别 A Where Nvl(是否禁止, 0) = 0 Order By a.序号"
    Set rsTemp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption)
    If Not rsTemp.EOF Then
        For i = 1 To rsTemp.RecordCount
            cbo险类.AddItem rsTemp!名称
            cbo险类.ItemData(cbo险类.NewIndex) = rsTemp!序号
            rsTemp.MoveNext
        Next
        cbo险类.ListIndex = 0
    Else
        MsgBox "没有发现有效险类,请设置险类！", vbInformation, gstrSysName
        Exit Sub
    End If
...
Private Sub ResizeForm(ByVal intType As Integer)
    '功能:根据病人和险类重新加载窗体
    '参数:intType-1:病人;2:险类
    On Error GoTo ErrHandler
    dtpBegin.Enabled = intType = 2: dtpEnd.Enabled = intType = 2
    chkDate.Visible = intType = 1: lbl发生时间.Visible = intType = 2
    IDKind.Visible = intType = 1: txtPatient.Visible = intType = 1
    cbo险类.Visible = intType = 2
    dkpMan.Panes(Pane_Pati).Closed = intType = 1
    lbl总计.Visible = intType = 2
    Exit Sub
ErrHandler:
    If ErrCenter() = 1 Then
        Resume
...
Private Sub LoadPatiFromInsure(ByVal lng险类 As Long)
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:根据险类加载病人信息
    '参数:str险类-保险类别.序号
    '编制:焦博
    '日期:2018-08-10 14:18:06
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim i As Long, j As Long, strSQL As String
    Dim dtStartDate As Date, dtEndDate As Date
    Dim objItem As ListItem, strWhere As String
    Dim str病人IDs As String
    
    On Error GoTo ErrHandler
    If Val(lng险类) = 0 Then Exit Sub
    ...
    "       From (Select Mod(a.记录性质, 10) As 记录性质,a.No,Max(a.实际票号) As 实际票号,Sum(Nvl(a.付数, 1) * a.数次) As 剩余数量,a.病人id," & vbNewLine & _
    "                    Max(Decode(Decode(a.记录性质, 4, 1, a.记录性质), 1, Decode(a.记录状态, 1, a.结帐id, 3, a.结帐id, 0), 0)) As 结帐id,max(开单部门ID) as 开单部门ID" & vbNewLine & _
    "             From 门诊费用记录 A" & vbNewLine & _
    "             Where a.记录状态 In (1, 2, 3) And Nvl(a.附加标志, 0) <> 9 And Nvl(a.费用状态, 0) <> 1 And" & vbNewLine & _
    "                   a.发生时间 Between [2] And [3]" & strWhere & vbNewLine & _
    "             Group By Mod(a.记录性质, 10), a.No, a.病人id) A, 保险结算记录 B,部门表 C" & vbNewLine & _
    "       Where a.结帐id = b.记录id And b.性质 = 1 And a.实际票号 Is Null And a.病人id = b.病人id And b.险类 =[1] And" & vbNewLine & _
    "             a.开单部门id = c.Id And(c.站点 = '" & gstrNodeNo & "' Or c.站点 Is Null) And " & vbNewLine & _
    "             (a.记录性质 = 1 And a.剩余数量 <> 0 Or a.记录性质 = 4 And a.剩余数量 <> 0)" & vbNewLine & _
    "       Group By a.记录性质, a.No, a.结帐id, a.病人id"


    strSQL = "Select a.病人id, b.姓名, b.性别, b.年龄, b.门诊号 From (" & strSQL & ") A, 病人信息 B Where a.病人id = b.病人id Order By 姓名"

    Set mrsInfo = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, lng险类, dtStartDate, dtEndDate)
    ...
    Else
        str病人IDs = ""
        Do While mrsInfo.EOF = False
            str病人IDs = str病人IDs & "," & mrsInfo!病人ID
            With lvwPati
                Set objItem = .ListItems.Add(, "_" & mrsInfo!病人ID, Nvl(mrsInfo!姓名))
                objItem.SubItems(1) = Nvl(mrsInfo!性别)
                objItem.SubItems(2) = Nvl(mrsInfo!年龄)
                objItem.SubItems(3) = Nvl(mrsInfo!门诊号)
            End With
            mrsInfo.MoveNext
        Loop
        str病人IDs = Mid(str病人IDs, 2)
        Call LoadAllList(str病人IDs, lng险类)
        Call cmdSelAll_Click
        If mintPatiRow <> 0 And mintPatiRow <= lvwPati.ListItems.Count Then lvwPati.ListItems(mintPatiRow).Selected = True
...
Private Function CheckPati(ByRef str病人IDs As String) As Boolean
    '功能:按险类打印时,检查是否选择了病人
    Dim i As Long

    On Error GoTo ErrHandler
    If mintPrintMode = 1 Then CheckPati = True: Exit Function
    With lvwPati
        If .ListItems.Count = 0 Then
            MsgBox "当前险类在【" & Format(dtpBegin.Value, "yyyy-mm-dd") & " 00:00:00" & "】～【" & Format(dtpEnd.Value, "yyyy-mm-dd") & " 23:59:59" & "】时间范围之内未找到病人！", vbInformation, gstrSysName
            Exit Function
        End If
        For i = 1 To .ListItems.Count
            If .ListItems(i).Checked = True Then
                str病人IDs = str病人IDs & "," & Mid(.ListItems(i).Key, 2)
            End If
        Next
    End With
    str病人IDs = Mid(str病人IDs, 2)
    If str病人IDs = "" Then
        MsgBox "当前未选择任何病人！", vbInformation, gstrSysName
        Exit Function
    End If
    CheckPati = True
    Exit Function
...
Private Sub LoadAllList(ByVal str病人IDs As String, Optional ByVal lng险类 As Long)
    '功能:读取单据信息
    Dim strTable As String
    Dim strSQL As String, strSubSql As String
    ...
    If chkDate.Value = 0 And chkDate.Visible Or mintPrintMode = 2 Then
    ...
    If zlStr.ActualLen(str病人IDs) <= 4000 Then
        strSubSql = "Select Column_Value As 病人ID From Table(f_Str2list([1]))"
    Else
        strSubSql = FromStrListBulidSQL(str病人IDs, "病人ID")
    End If
    ...
    strTable = "" & _
            ...
            "        Sum(Nvl(a.付数, 1) * a.数次) As 剩余数量,Max(a.门诊标志) As 门诊标志, a.病人id" & vbNewLine & _
            " From 门诊费用记录 A,(" & strSubSql & ") B" & vbNewLine & _
            " Where a.记录状态 In (1, 2, 3) And a.病人ID =b.病人ID " & strWhere & vbNewLine & _
            "       And Nvl(a.附加标志, 0) <> 9 And Nvl(a.费用状态, 0) <> 1" & vbNewLine & _
            " Group By Mod(a.记录性质,10),a.No,a.病人ID"
    ...
    strSQL = "Select /*+ RULE */" & _
            ...
            "  Nvl(Max(t.险类), 0) As 险类id, Max(a.剩余数量) As 剩余数量,Max(a.门诊标志) As 门诊标志, a.病人id" & _
            ...
            "            Or a.记录性质=4 And a.剩余数量<>0)" & _
                    IIf(lng险类 = 0, "", " And t.险类=[4]") & _
            " Group By a.记录性质,a.No, a.结帐id, a.病人id" & _
            " Order By 单据,结帐id Desc,单据号"
            
    Set mrsList = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, str病人IDs, dtStartDate, dtEndDate, lng险类)
...
Private Sub ReadPatiTotal()
    '功能:读取所有病人补打票据的金额合计
    Dim dblMoney As Double, i As Integer
    
    On Error GoTo ErrHandler
    If Not mrsList Is Nothing Then
        If mrsList.RecordCount > 0 Then mrsList.MoveFirst
        For i = 1 To lvwPati.ListItems.Count
            If lvwPati.ListItems(i).Checked = True Then
                mrsList.Filter = "病人ID=" & Mid(lvwPati.ListItems(i).Key, 2)
                Do While Not mrsList.EOF
                    If RoundEx(mrsList!剩余数量, 6) <> 0 Then dblMoney = dblMoney + Val(mrsList!实收金额)
                    mrsList.MoveNext
                Loop
            End If
        Next
        mrsList.Filter = 0
    End If
    lbl总计.Caption = "补打总计:" & Format(dblMoney, "0.00")
    Exit Sub
ErrHandler:
    If ErrCenter() = 1 Then
        Resume
