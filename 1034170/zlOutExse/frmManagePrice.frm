...
Private Sub mnuViewFilter_Click()
    ...
    If gblnOK Then
        ...
        With frmPriceFilter
            ...
            SQLCondition.ChargeKind = NeedName(.cbo费别.Text)
    ...
    For i = IIf(blnHead, 1, mlngGo) To mshList.Rows - 1
        ...
        With frmPriceGo
            ...
            If .cbo操作员.ListIndex > 0 Then
                blnFill = blnFill And mshList.TextMatrix(i, GetColNum("划价人")) = NeedName(.cbo操作员.Text)
            End If
            If .txt姓名.Text <> "" Then
...
'     blnSort=不重新读取数据,仅重新显示已排序的内容
    ...
    If Not blnSort Then
        ...
        strIF = " Where 记录性质=1 And " & IIf(mbln收费, " 记录状态 IN(1,3)", "记录状态=0") & _
                " And 门诊标志 " & IIf(gint病人来源 = 1, " IN(1,4)", "= 2") & " And 划价人 is Not NULL And 操作员姓名 IS " & IIf(mbln收费, "NOT", "") & " NULL " & strIF
