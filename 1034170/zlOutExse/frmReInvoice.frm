...
Public Function ShowMe(frmParent As Object, ByVal strNO As String, _
    ...
    Optional bln补结算 As Boolean, Optional ByVal int票种 As Integer = 1) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:进入退费发票号选择
    '入参:frmParent-调用的父窗口
    '     strNO-退费的单据号
    ...
    With mshInvoice
        ...
        For i = 1 To rsTmp.RecordCount
            .TextMatrix(i, 0) = IIf(bln补结算, "√", "")  '√:35217
            .TextMatrix(i, 1) = rsTmp!号码
            rsTmp.MoveNext
        Next
        If bln补结算 Then .ColWidth(0) = 0
    End With
    ...
    If bln补结算 Then
        ...
        txtThis.Visible = False
        Me.Caption = "票据收回确认"
        lblPrompt.Caption = vbCrLf & "注意:在当前操作下,请注意收回以下票据。"
        cmdOK.Top = lblTotal.Top + lblTotal.Height
        cmd取消.Top = cmdOK.Top + cmd取消.Height + 100
    End If
...
Private Function GetInvoice(ByVal strNos As String, ByVal int票种 As Integer) As ADODB.Recordset
    ...
    On Error GoTo errH
    '80704,冉俊明,2014-12-12,挂号使用收费票据时回收不了原始票据(票种为1[收费]),其实无需判断票种
    '退费发票(红票),原因为6的不回收
    If mbln补结算 Then
        strSQL = _
        ...
        "           And A.打印id in  (Select ID From 票据打印内容 Where 数据性质 = [2] And NO In (Select Column_Value From Table(f_str2list([1]))))" & vbNewLine & _
        "Minus" & vbNewLine & _
        "Select A.号码" & vbNewLine & _
        "From 票据使用明细 A" & vbNewLine & _
    ...
    Else
        ...
        "   From 票据使用明细 A" & vbNewLine & _
        "   Where A.性质 = 1 And a.原因 <> 6 " & vbNewLine & _
        "           And A.打印id = (Select Max(ID) From 票据打印内容 Where 数据性质 = [2] And NO In (Select Column_Value From Table(f_str2list([1]))))" & vbNewLine & _
        "Minus" & vbNewLine & _
        "Select A.号码" & vbNewLine & _
        "From 票据使用明细 A" & vbNewLine & _
        "Where A.性质 = 2 And a.原因 <> 6 " & vbNewLine & _
