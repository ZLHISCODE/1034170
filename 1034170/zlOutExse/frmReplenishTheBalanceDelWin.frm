...
Private Sub InitFace()
    ...
    If mblnUnLoad = False Then
        ...
        Set mrsOldBalance = zlFromIDGetChargeBalance(2, mobjDelBalance.CurDelNos, , , , IIf(mblnRegister, 4, 1))
...
Private Sub SetCtrlVisible()
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    cmdOK.Visible = True
    cmdExit.Visible = mbytFunc = EM_BalanceReDel
End Sub
...
'    If mdblDelMoney = 0 Then Exit Sub
    zl9LedVoice.Speak "#21 " & Format(mdblDelMoney, "0.00")
    mbln已报价 = True
End Sub
...
'    If mdblDelMoney = 0 Then Exit Sub
    ...
    If objCard.结算性质 = 1 Then
        zl9LedVoice.DispCharge mdblDelMoney, 0, 0
    Else '部分支付现金时的处理
