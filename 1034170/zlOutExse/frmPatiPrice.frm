...
Private Sub cbo调整费别_Click()
    Dim strSQL As String
     ...
     If txt费别.Text = cbo调整费别.Text Then Exit Sub
     If InStr(1, mstrPrivs, ";调整病人费别;") = 0 Then Exit Sub
     
    If MsgBox("注意:" & vbCrLf & "你是否要将费别『" & Trim(txt费别.Text) & "』调整为『" & cbo调整费别.Text & "』吗?" & vbCrLf & "  调整费别将直接影响相关的收费划价单!", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbNo Then
    ...
    With mshList
        For i = 1 To .Rows - 1
            If .TextMatrix(i, .ColIndex("单据号")) <> "" Then
                If blnCls Then
                    If gTy_Module_Para.bln禁止取消挂号划价单 _
                        And Val(.TextMatrix(i, .ColIndex("挂号划价单"))) = 1 Then
                        '禁止取消挂号划价单
                    Else
...
Private Sub mshList_KeyPress(KeyAscii As Integer)
    ...
    With mshList
        If .TextMatrix(.Row, .ColIndex("单据号")) <> "" Then
            ...
            Else
                If gTy_Module_Para.bln禁止取消挂号划价单 _
                    And Val(.TextMatrix(.Row, .ColIndex("挂号划价单"))) = 1 Then
                    '禁止取消挂号划价单
                    MsgBox "单据『" & .TextMatrix(.Row, .ColIndex("单据号")) & _
                        "』为挂号产生的划价单据，要求必须进行收费，不允许取消选择！", vbInformation, gstrSysName
                    Exit Sub
                Else
                    .TextMatrix(.Row, .ColIndex("选择")) = ""
                End If
...
Private Sub SetHeader()
    ...
    Dim i As Integer
    strHead = "诊断,1,3500|选择,4,500|单据号,4,850|开单科室,1,1200|医生,1,800|姓名,1,800|性别,4,500|年龄,4,500|" & _
        "应收金额,7,850|实收金额,7,850|划价人,1,800|划价时间,4,1850|皮试,4,500|挂号划价单,4,0"
    With mshList
        .Redraw = flexRDNone
        ...
        For i = 0 To UBound(Split(strHead, "|"))
            ...
            If Split(Split(strHead, "|")(i), ",")(0) = "挂号划价单" Then .ColHidden(i) = True
        Next
        .RowHeight(0) = 320
