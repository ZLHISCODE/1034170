...
Begin VB.Form frmDoubleBalanceRefund 
   BorderStyle     =   0  'None
   Caption         =   "frmDoubleBalanceRefund"
   ClientHeight    =   6960
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   10200
   ...
   Begin VB.PictureBox picMain 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfMain 
         ...
         FormatString    =   $"frmDoubleBalanceRefund.frx":0000
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "frmDoubleBalanceRefund"
    ...
    If intType = 0 Then
        Select Case frmReplenishTheBalanceManage.cboDate.ListIndex
            ...
            Case 2 '前二天至今日
                ...
                dtEndDate = CDate(Format(frmReplenishTheBalanceManage.dtpStartDate.MaxDate, "yyyy-mm-dd") & " 23:59:59")
            Case 3 '前三天至今日
                dtStartDate = CDate(Format(DateAdd("d", -3, frmReplenishTheBalanceManage.dtpStartDate.MaxDate), "yyyy-mm-dd") & " 00:00:00")
                dtEndDate = CDate(Format(frmReplenishTheBalanceManage.dtpStartDate.MaxDate, "yyyy-mm-dd") & " 23:59:59")
            Case 4  '前一周至今日
                dtStartDate = CDate(Format(DateAdd("d", -7, frmReplenishTheBalanceManage.dtpStartDate.MaxDate), "yyyy-mm-dd") & " 00:00:00")
                dtEndDate = CDate(Format(frmReplenishTheBalanceManage.dtpStartDate.MaxDate, "yyyy-mm-dd") & " 23:59:59")
            Case 5  '
                dtStartDate = CDate(Format(DateAdd("m", -1, frmReplenishTheBalanceManage.dtpStartDate.MaxDate), "yyyy-mm-dd") & " 00:00:00")
                dtEndDate = CDate(Format(frmReplenishTheBalanceManage.dtpStartDate.MaxDate, "yyyy-mm-dd") & " 23:59:59")
            Case Else
        ...
        End Select
        strSQL = "Select NO, 附加标志, 姓名, 性别, 年龄, -1 * Sum(冲预交) As 冲预交, 操作员姓名, 登记时间, 结算序号" & _
            "   From (Select Distinct a.No, Decode(Nvl(a.附加标志, 0), 1, '挂号', '收费') As 附加标志, b.姓名, b.性别," & _
            "                b.年龄, c.冲预交, 结帐id, a.操作员姓名, a.登记时间, a.结算序号" & _
            "          From 费用补充记录 A, 病人信息 B, 病人预交记录 C" & _
            "          Where a.登记时间 Between [1] And [2] And A.操作员姓名 = [3] " & _
            "                And Nvl(a.费用状态, 0) = 1 And a.病人id = b.病人id And a.记录状态 = 2" & _
            "                And c.结帐id In (Select Distinct 结帐id From 病人预交记录 Where 结算序号 = a.结算序号))" & _
            "   Group By NO, 附加标志, 姓名, 性别, 年龄, 操作员姓名, 登记时间, 结算序号"
        ...
        If rsMain.RecordCount <> 0 Then
            frmReplenishTheBalanceManage.tabMain.Item(2).Caption = "异常退费记录(" & rsMain.RecordCount & ")"
        Else
            frmReplenishTheBalanceManage.tabMain.Item(2).Caption = "异常退费记录"
        End If
        Call SetMain
    End If
    If intType = 1 Then
        '使用IDKIND条件读取
        strSQL = " Select A.No, Decode(Nvl(A.附加标志,0),1,'挂号','收费'), B.姓名, B.性别, B.年龄, -1*Sum(C.冲预交), A.操作员姓名, A.登记时间, A.结算序号" & _
                 " From 费用补充记录 A, 病人信息 B, 病人预交记录 C  " & _
                 " Where B.病人ID= [1] And Nvl(A.费用状态,0)=1 And A.病人ID=B.病人ID And A.记录状态 = 2 And A.操作员姓名 = [2] " & _
                 "      And C.结帐ID In (Select Distinct 结帐ID From 病人预交记录 Where 结算序号=A.结算序号) " & _
...
Private Sub vsfMain_DblClick()
    Call frmReplenishTheBalanceManage.ViewBalance(2)
End Sub
