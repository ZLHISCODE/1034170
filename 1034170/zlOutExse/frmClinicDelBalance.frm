...
Private Sub cbo支付方式_Click()
    ...
    '105432
    If mlngPre支付方式 > 0 And mCurCarge.dbl当前未退 < 0 Then
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
    Case vbKeyF4
        If gTy_Module_Para.bln使用加减切换 = False Then Exit Sub
        If Me.ActiveControl Is txt缴款 Then SwitchPaymentWay cbo支付方式, True
     Case vbKeyF12
...
Private Sub Form_KeyPress(KeyAscii As Integer)
    ...
    Case "+", "-"
        If gTy_Module_Para.bln使用加减切换 = False Then Exit Sub
        If Me.ActiveControl Is txt缴款 Then SwitchPaymentWay cbo支付方式, (Chr(KeyAscii) = "+")
    End Select
End Sub

Private Sub SwitchPaymentWay(cboPaymentWay As ComboBox, ByVal blnNext As Boolean)
    '切换退款方式
    Dim i As Integer
    
    If cboPaymentWay.Enabled = False Then Exit Sub
    
    i = cboPaymentWay.ListIndex
    If blnNext Then
        i = IIf(i >= cboPaymentWay.ListCount - 1, 0, i + 1)
    Else
        i = IIf(i <= 0, cboPaymentWay.ListCount - 1, i - 1)
    End If
    cboPaymentWay.ListIndex = i
End Sub
