...
Private mstr类别 As String, mstr排除类别 As String
...
Public Function ShowSelect(frmParent As Object, ByVal strPrivs As String, _
    ...
    Optional ByVal lngHwnd As Long, Optional ByVal str特准项目 As String, _
    Optional ByVal int中药形态 As Integer = -1, Optional str排除类别 As String = "", _
    Optional ByVal bln包含核算材料 As Boolean = False, _
    Optional ByVal bln条码扫描 As Boolean, _
    Optional ByRef lng批次 As Long) As Long
    ...
    '     str排除类别-不能选择的类别'7','6'等
    '     bln包含核算材料-是否包含卫材料中的核算材料:36112
    '出参:
    ...
    mbln包含核算材料 = bln包含核算材料
    mstrPrivs = strPrivs: mlngHwnd = lngHwnd
    mint病人来源 = int病人来源: mint险类 = int险类
    mbln药房单位 = bln药房单位: mstr输入 = str输入
    mstr类别 = str类别: mstr排除类别 = str排除类别
    ...
    If gblnFeeKindCode And mstr输入 <> "" Then
...
Private Sub cmdOK_Click()
    ...
    mlng批次 = Val(vsItem.TextMatrix(vsItem.Row, vsItem.ColIndex("批次")))
...
Private Sub Form_Load()
    ...
    Else
        ...
        Else
            '无数据,提示,非取消退出
            If mbln条码扫描 Then
                MsgBox "与输入条码[" & UCase(mstr输入) & "]相符的卫生材料项目库存不足。", vbInformation, gstrSysName
            Else
...
Private Function FillTree() As Boolean
    ...
    '药品和卫材料按诊疗分类
   If (mstr类别 = "" Or InStr(mstr类别, "'5'") > 0) And InStr(mstr排除类别, "'5'") = 0 Then strTmp = strTmp & ",1"
    If (mstr类别 = "" Or InStr(mstr类别, "'6'") > 0) And InStr(mstr排除类别, "'6'") = 0 Then strTmp = strTmp & ",2"
    If (mstr类别 = "" Or InStr(mstr类别, "'7'") > 0) And InStr(mstr排除类别, "'7'") = 0 Then strTmp = strTmp & ",3"
    If (mstr类别 = "" Or InStr(mstr类别, "'4'") > 0) And InStr(mstr排除类别, "'4") = 0 Then strTmp = strTmp & ",7"
    str类型 = Mid(strTmp, 2)
    If str类型 <> "" Then
        strSQL = _
            " Select 0 as 级,类型,To_Number('99999999'||类型) as ID,-NULL as 上级ID," & _
            " CHR(13)||Decode(类型,1,'西成药',2,'中成药',3,'中草药',7,'卫生材料') as 名称" & _
            " From 诊疗分类目录 Where Instr([1],','||类型||',')>0 And (撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or 撤档时间 Is Null)" & _
            " Group by 类型"
    ...
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Name, "," & str类型 & ",", mstr排除类别)
...
'      strIDs=读取的项目ID集,用于判断输入时是否别名不同的同一个收费项目
    ...
    Dim i As Long, j As Long, bln批次 As Boolean
    Dim strAntibiotic As String '抗生素条件
    ...
    If blnLoad Then
        ...
        If Not gbln分离发药 Then
            ...
            If lng药房ID <> 0 Then
                strStock = _
                    " Select A.药品ID, Max(decode(instr([19],','||A.库房ID||','),0,1,0)) as 非所属部门,Sum(Nvl(A.可用数量,0)) as 库存" & _
                    " From 药品库存 A" & _
            ...
            ElseIf blnStock And Not (glng西药房 = 0 And glng成药房 = 0 And glng中药房 = 0) Then
                '明确类别下未指定库房,不处理库存
                '不明确类别时都未指定库房,不处理库存
                strStock = _
                    " Select A.药品ID,0 as 非所属部门,Sum(Nvl(A.可用数量,0)) as 库存" & _
        ...
        If mstr输入 <> "" Then
            ...
            If mint险类 = 0 Then
                strMain = _
                    " Select Distinct A.ID,A.类别,A.编码," & IIf(gTy_System_Para.byt输入药品显示 = 1, "C2.名称 ,C1.名称 as 商品名,'' as  简码,", "B.名称,Null as 商品名,B.简码,") & " " & _
                    "       A.计算单位,A.规格,A.产地,A.费用类型,Null 医保大类,A.说明,A.是否变价" & _
                    " From 收费项目别名 B,收费项目目录 A" & IIf(gTy_System_Para.byt输入药品显示 = 1, ",收费项目别名 C2,收费项目别名 C1", "") & _
                    " Where A.ID=B.收费细目ID And A.服务对象 IN([9],3)" & strSQLItem & _
                            IIf(gTy_System_Para.byt输入药品显示 = 1, " And A.ID=C1.收费细目ID(+) And C1.码类(+)=1 And C1.性质(+)=3", "") & _
                            IIf(gTy_System_Para.byt输入药品显示 = 1, " And A.ID=C2.收费细目ID(+) And C2.码类(+)=1 And C2.性质(+)=1", "") & _
                            IIf(str类别 <> "", " And A.类别=[4]", "") & IIf(mstr类别 <> "", " And Instr([5],A.类别)>0", "") & _
                            IIf(mstr排除类别 <> "", " And Instr([18],A.类别)=0", "") & mstr特准项目 & strInput
            Else
                strMain = _
                    " Select Distinct A.ID,A.类别,A.编码," & IIf(gTy_System_Para.byt输入药品显示 = 1, "C2.名称 ,C1.名称 as 商品名,'' as  简码,", "B.名称,Null as 商品名,B.简码,") & " " & _
                    "       A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价" & _
                    " From 收费项目别名 B,收费项目目录 A,保险支付项目 C,保险支付大类 D" & IIf(gTy_System_Para.byt输入药品显示 = 1, ",收费项目别名 C2,收费项目别名 C1", "") & _
                    " Where A.ID=B.收费细目ID And A.服务对象 IN([9],3)" & strSQLItem & _
                            IIf(gTy_System_Para.byt输入药品显示 = 1, " And A.ID=C1.收费细目ID(+) And C1.码类(+)=1 And C1.性质(+)=3", "") & _
                            IIf(gTy_System_Para.byt输入药品显示 = 1, " And A.ID=C2.收费细目ID(+) And C2.码类(+)=1 And C2.性质(+)=1", "") & _
        ...
        Else
            If mint险类 = 0 Then
                strMain = _
                    " Select A.ID,A.类别,A.编码,Nvl(B.名称,A.名称) as 名称," & IIf(gTy_System_Para.byt药品名称显示 = 2, "C1.名称 as 商品名,", "Null as 商品名,") & "" & _
                    "       A.计算单位,A.规格,A.产地,A.费用类型,Null 医保大类,A.说明,A.是否变价" & _
                    " From 收费项目别名 B,收费项目目录 A" & IIf(gTy_System_Para.byt药品名称显示 = 2, ",收费项目别名 C1", "") & _
                    " Where A.ID=B.收费细目ID(+) And B.码类(+)=1 And B.性质(+)=[10]" & _
                    "       And A.服务对象 IN([9],3)" & strSQLItem & _
                            IIf(gTy_System_Para.byt药品名称显示 = 2, " And A.ID=C1.收费细目ID(+) And C1.码类(+)=1 And C1.性质(+)=3", "") & _
                            IIf(str类别 <> "", " And A.类别=[4]", "") & IIf(mstr类别 <> "", " And Instr([5],A.类别)>0", "") & _
                            IIf(mstr排除类别 <> "", " And Instr([18],A.类别)=0", "") & mstr特准项目
            Else
                strMain = _
                    " Select" & _
                    " A.ID,A.类别,A.编码,Nvl(B.名称,A.名称) as 名称," & IIf(gTy_System_Para.byt药品名称显示 = 2, "C1.名称 as 商品名,", "Null as 商品名,") & "" & _
                    " A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价" & _
                    " From 收费项目别名 B,收费项目目录 A,保险支付项目 C,保险支付大类 D" & IIf(gTy_System_Para.byt药品名称显示 = 2, ",收费项目别名 C1", "") & _
                    " Where A.ID=B.收费细目ID(+) And B.码类(+)=1 And B.性质(+)=[10]" & _
                    "       And A.服务对象 IN([9],3)" & strSQLItem & _
                    "       And A.ID=C.收费细目ID(+) And C.险类(+)=[16] And C.大类ID=D.ID(+)" & vbNewLine & _
                            IIf(gTy_System_Para.byt药品名称显示 = 2, " And A.ID=C1.收费细目ID(+) And C1.码类(+)=1 And C1.性质(+)=3", "") & _
        ...
        '60623:李南春,2015/08/24,门诊收费处不能选择特殊使用级抗菌药物进行收费
        '92727,通过参数控制是否允许录入特殊使用的抗生素
        '药品特性.抗生素：0-不是抗生素，1-非限制使用的抗生素，2-限制使用的抗生素，3-特殊使用的抗生素
        If gbln允许录入特殊使用的抗生素 = False Then
            strAntibiotic = " And F.抗生素 <> 3"
        End If
        If strStock = "" Then
            strSQL = _
                " Select A.ID,A.类别 as 类别ID,B.序号 as 顺序ID,B.名称 as 类别," & _
                    ...
                    " Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价)" & _
                        IIf(mbln药房单位, "*Nvl(D." & str药房包装 & ",1)", "") & ",'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                    " NULL as 库存,-1 as 批次" & _
                " From (" & strMain & ") A,收费项目类别 B,收费价目 C,药品规格 D,诊疗项目目录 E,药品特性 F" & _
                " Where A.类别=B.编码 And A.ID=C.收费细目ID" & _
                "       And A.ID=D.药品ID And D.药名ID=E.ID And D.药名ID=F.药名ID " & strAntibiotic & str分类ID & _
        ...
        Else
            ...
            strSQL = _
                ...
                "       Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价)" & _
                        IIf(mbln药房单位, "*Nvl(D." & str药房包装 & ",1)", "") & ",'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                        IIf(InStr(1, mstrPrivs, "显示库存") > 0, _
                              IIf(gbyt库存显示方式 <= 0, " LTrim(To_Char(X.库存" & IIf(mbln药房单位, "/Nvl(D." & str药房包装 & ",1)", "") & ",'9999990.00000'))", _
                                                                      " Decode(max(nvl(X.非所属部门,0)),1,Decode(Sign(X.库存),1,'有','无'), LTrim(To_Char(X.库存" & IIf(mbln药房单位, "/Nvl(D." & str药房包装 & ",1)", "") & ",'9999990.00000')))"), _
                            "Decode(Sign(X.库存),1,'有','无')") & " as 库存,-1 as 批次" & _
                " From (" & strMain & ") A,收费项目类别 B,收费价目 C," & _
                "       药品规格 D,诊疗项目目录 E,药品特性 F,(" & strStock & ") X" & _
                " Where A.类别=B.编码 And A.ID=C.收费细目ID" & _
                "       And A.ID=D.药品ID And D.药名ID=E.ID And D.药名ID=F.药名ID " & strAntibiotic & strTmp & str分类ID & _
                        IIf(mint中药形态 = -1, "", " And ( (instr( ';6;7;',';'||A.类别||';',1)>0 and nvl(D.中药形态,0)=[17]) or A.类别='5')  ") & _
                "       And Sysdate Between C.执行日期 and Nvl(C.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
                " Group by A.ID,A.类别,B.序号,B.名称,A.编码,A.名称,A.商品名," & IIf(mstr输入 <> "", "A.简码,", "") & _
    ...
    If blnLoad Then
        ...
        If mstr输入 <> "" Then
            ...
            If mbln条码扫描 And Len(mstr输入) > 10 Then
                strInput = " And (A.编码 Like [6] Or M.商品条码=[20] Or M.内部条码=[20] Or B.简码 Like [7]) And B.码类=[8]"
                bln批次 = True
            ElseIf IsNumeric(mstr输入) Then                          '10,11.输入全是数字时只匹配编码
            ...
            If bln批次 Then
                If mint险类 = 0 Then
                    strMain = _
                        ...
                        " A.计算单位,A.规格,A.产地,A.费用类型,Null 医保大类,A.说明,A.是否变价,M.批次 " & _
                        " From 收费项目别名 B,收费项目目录 A,药品库存 M" & _
                        " Where A.服务对象 IN([9],3) And A.ID=B.收费细目ID And A.类别='4'" & mstr特准项目 & strInput & strSQLItem & _
                        "           And A.ID=M.药品ID And (M.效期 Is Null Or M.效期 > Trunc(Sysdate)) " & _
                        "           And Nvl(M.可用数量,0) >0 "
                Else
                    strMain = _
                        " Select Distinct A.ID,A.类别,A.编码,B.名称,B.简码," & _
                        " A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价,M.批次 " & _
                        " From 收费项目别名 B,收费项目目录 A,药品库存 M,保险支付项目 C,保险支付大类 D" & _
                        " Where A.服务对象 IN([9],3) And A.ID=B.收费细目ID And A.类别='4'" & mstr特准项目 & strInput & strSQLItem & _
                        " And A.ID=C.收费细目ID(+) And C.险类(+)=[16] And C.大类ID=D.ID(+)" & _
                        "           And A.ID=M.药品ID And (M.效期 Is Null Or M.效期 > Trunc(Sysdate)) " & _
                        "           And Nvl(M.可用数量,0) >0 "
                End If
            Else
        ...
        Else
            If mint险类 = 0 Then
                strMain = _
                    ...
                    " From 收费项目目录 A" & _
                    " Where A.服务对象 IN([9],3) And A.类别='4'" & mstr特准项目 & strSQLItem
            Else
                strMain = _
        ...
        If bln批次 Then
            If glng发料部门 = 0 Then
                ...
                "           Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价),'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                "           NULL as 库存,A.批次 " & _
                " From (" & strMain & ") A,收费项目类别 B,收费价目 C,材料特性 D,诊疗项目目录 E" & _
                " Where A.类别=B.编码 And A.ID=C.收费细目ID" & _
                "           And A.ID=D.材料ID And D.诊疗ID=E.ID" & IIf(mbln包含核算材料, "", " And nvl(D.核算材料 ,0)=0 ") & str分类ID & _
                "           And Sysdate Between C.执行日期 and Nvl(C.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
                " Group by A.ID,A.类别,B.序号,B.名称,A.编码,A.名称," & IIf(mstr输入 <> "", "A.简码,", "") & _
                "           A.规格,A.产地,A.费用类型,A.医保大类,A.说明,A.是否变价,A.计算单位,A.批次 "
            Else
                '卫材不受此系统参数和权限影响:分离下药时指定药房时限定库存,权限:不检查库存
                    ...
                    " Where A.类别=B.编码 And A.ID=C.收费细目ID And A.ID=X.药品ID(+)" & _
                    "       And A.ID=D.材料ID And D.诊疗ID=E.ID" & IIf(mbln包含核算材料, "", " And nvl(D.核算材料 ,0)=0 ") & str分类ID & _
                    "       And Sysdate Between C.执行日期 and Nvl(C.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
                    " Group by A.ID,A.类别,B.序号,B.名称,A.编码,A.名称," & IIf(mstr输入 <> "", "A.简码,", "") & _
                    "           A.规格,A.产地,A.费用类型,A.医保大类,A.说明,A.是否变价,A.计算单位, X.库存,D.跟踪在用,A.批次 " '问题:47541
            End If
        Else
            If glng发料部门 = 0 Then
                strSQL = strSQL & IIf(strSQL = "", "", " Union ALL ") & _
                    ...
                    " Where A.类别=B.编码 And A.ID=C.收费细目ID" & IIf(mbln包含核算材料, "", " And nvl(D.核算材料 ,0)=0 ") & _
                        ...
                        " And Sysdate Between C.执行日期 and Nvl(C.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
                    " Group by A.ID,A.类别,B.序号,B.名称,A.编码,A.名称," & IIf(mstr输入 <> "", "A.简码,", "") & _
                        "A.规格,A.产地,A.费用类型,A.医保大类,A.说明,A.是否变价,A.计算单位"
            Else
                '卫材不受此系统参数和权限影响:分离下药时指定药房时限定库存,权限:不检查库存
                ...
                strSQL = strSQL & IIf(strSQL = "", "", " Union ALL ") & _
                        " Select A.ID,A.类别 as 类别ID,B.序号 as 顺序ID,B.名称 as 类别," & _
                            ...
                            " A.计算单位 as 单位,A.规格,A.产地,A.费用类型,A.医保大类,A.说明," & _
                            " Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价),'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                            IIf(InStr(1, mstrPrivs, "显示库存") > 0, " Decode(D.跟踪在用,1,To_Char(X.库存,'9999990.00000'),NULL)", "Decode(Sign(X.库存),1,'有','无')") & " as 库存,-1 as 批次" & _
                        " From (" & strMain & ") A,收费项目类别 B,收费价目 C,材料特性 D,诊疗项目目录 E,(" & strStock & ") X" & _
                        " Where A.类别=B.编码 And A.ID=C.收费细目ID And A.ID=X.药品ID(+)" & IIf(mbln包含核算材料, "", " And nvl(D.核算材料 ,0)=0 ") & _
                            " And A.ID=D.材料ID And D.诊疗ID=E.ID" & str分类ID & _
                            " And Sysdate Between C.执行日期 and Nvl(C.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
                        " Group by A.ID,A.类别,B.序号,B.名称,A.编码,A.名称," & IIf(mstr输入 <> "", "A.简码,", "") & _
                            "A.规格,A.产地,A.费用类型,A.医保大类,A.说明,A.是否变价,A.计算单位, X.库存,D.跟踪在用 " '问题:47541
            End If
        End If
    End If
    ...
    If blnLoad Then
            ...
            If mint险类 = 0 Then
                strMain = _
                    ...
                    " Where A.ID=B.收费细目ID And A.服务对象 IN([9],3) And A.类别 Not IN('4','5','6','7','1')" & strSQLItem & _
                     IIf(str类别 <> "", " And A.类别=[4]", "") & IIf(mstr类别 <> "", " And Instr([5],A.类别)>0", "") & _
                      IIf(mstr排除类别 <> "", " And Instr([18],A.类别)=0", "") & str分类ID & mstr特准项目 & strInput
            Else
                strMain = _
                    " Select Distinct A.ID,A.类别,A.编码,B.名称,B.简码," & _
                    " A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价" & _
            ...
            If mint险类 = 0 Then
                strMain = _
                    ...
                    " Where A.服务对象 IN([9],3) And A.类别 Not IN('4','5','6','7','1')" & strSQLItem & _
                     IIf(str类别 <> "", " And A.类别=[4]", "") & IIf(mstr类别 <> "", " And Instr([5],A.类别)>0", "") & _
                      IIf(mstr排除类别 <> "", " And Instr([18],A.类别)=0", "") & str分类ID & mstr特准项目
            Else
                strMain = _
                    " Select A.ID,A.类别,A.编码,A.名称," & _
                    " A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价" & _
        ...
        strSQL = strSQL & IIf(strSQL = "", "", " Union ALL ") & _
            ...
            " A.编码,A.名称,NULL as 商品名," & IIf(mstr输入 <> "", "A.简码,", "") & _
            " A.计算单位 as 单位,A.规格,A.产地,A.费用类型,A.医保大类,A.说明," & _
            " Decode(A.是否变价,1,'变价',LTrim(To_Char(Sum(C.现价),'999999" & gstrFeePrecisionFmt & "'))) as 单价,NULL as 库存,-1 as 批次" & _
    ...
    Set mrsItem = zlDatabase.OpenSQLRecord(strSQL, Me.Name, int类型, lng诊疗分类ID, lng收费分类ID, str类别, mstr类别, _
        UCase(mstr输入) & "%", mstrLike & UCase(mstr输入) & "%", gbytCode + 1, mint病人来源, IIf(gTy_System_Para.byt药品名称显示 = 1, 3, 1), _
        lng药房ID, glng西药房, glng成药房, glng中药房, glng发料部门, mint险类, mint中药形态, mstr排除类别, gstr所属部门ID, UCase(mstr输入))
    'byt药品名称显示：0-显示通用名，1-显示商品名，2-同时显示通用名和商品名
...
'    .byt药品名称显示 = Val(zlDatabase.GetPara("药品名称显示", glngSys, 0, "0")) '：0-显示通用名，1-显示商品名，2-同时显示通用名和商品名
    ...
    For i = 1 To vsItem.COLS - 1
        If vsItem.TextMatrix(0, i) = "商品名" Then
            If (gTy_System_Para.byt输入药品显示 = 0 And mstr输入 <> "") Or (mstr输入 = "" And gTy_System_Para.byt药品名称显示 <> 2) Then
        ...
        ElseIf vsItem.TextMatrix(0, i) = "简码" Then
            If gTy_System_Para.byt输入药品显示 = 1 And mstr输入 <> "" Then
                vsItem.ColHidden(i) = True
            End If
        End If
