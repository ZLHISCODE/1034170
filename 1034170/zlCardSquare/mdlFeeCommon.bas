...
Public Function ExistShareBill(lngID As Long, bytKind As Byte) As Boolean
'功能：判断是否存在指定的共用票据领用
    Dim rsTmp As ADODB.Recordset
    Dim strSql As String
        
    On Error GoTo errH
    If bytKind = 6 Then '消费卡
        strSql = "Select ID From 消费卡领用记录 Where ID=[1] And 使用方式=2"
    Else
        strSql = "Select ID From 票据领用记录 Where ID=[1] And 票种=[2] And 使用方式=2"
    End If
    Set rsTmp = zlDatabase.OpenSQLRecord(strSql, "检查领用ID", lngID, bytKind)
    ExistShareBill = Not rsTmp.EOF
    Exit Function
errH:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
