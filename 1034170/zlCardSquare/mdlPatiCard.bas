...
Public Function zlGet医疗卡类别(Optional cnOracle As ADODB.Connection) As ADODB.Recordset
    ...
    '77872,李南春,2014/10/28:是否支持转帐及代扣
    gstrSQL = "" & _
    "   Select A.Id, A.编码, A.名称, A.短名, A.前缀文本, A.卡号长度, A.缺省标志, A.是否固定, A.是否严格控制,nvl( A.是否刷卡,0) as 是否刷卡, " & _
    ...
    "           B.性质  as 结算性质, nvl(A.是否启用,0) as 是否启用," & _
    "           nvl(A.是否转帐及代扣,0) as 是否转帐及代扣," & _
    "           nvl(A.是否持卡消费,0) as 是否持卡消费,nvl(A.发送调用接口,0) as 发送调用接口," & _
    "           A.设备是否启用回车, nvl(A.发卡控制,0) as 发卡控制, " & _
    "           nvl(A.是否退款验卡,0) as 是否退款验卡,Nvl(A.是否缺省退现,0) as 是否缺省退现" & _
...
Public Function zlCreatePatiCardObject(ByVal objCard As clsCard, ByRef objCardObject As Object, Optional blnAdviceSend As Boolean = False) As Boolean
    ...
    If objCard.接口程序名 = "" Then
        ...
        If objCard.消费卡 Then
            MsgBox objCard.名称 & "未设置接口部件，请与系统管理员联系!"
        Else
            MsgBox objCard.名称 & "未设置接口部件，请在【医疗卡类别管理】中设置部件名!"
        End If
        Exit Function
    End If
    strCommpentName = GetCardComponentsStr(objCard.接口程序名, "zl9Card_")
...
Public Function zlItemsMoney(ByVal strIDs As String) As ADODB.Recordset
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:获取指定项目的数据
    '入参:传入ID集，用逗号分离
    '返回:相关价格集
   ...
   Set zlItemsMoney = OpenSQLRecordNew(strSql, "获取挂号价格", varData)
End Function
...
Public Function zlGetSubTable(ByVal BytType As Byte, ByVal strValues_IN As String, _
    ...
    zlGetSubTable = True
End Function
Public Function OpenSQLRecordNew(ByVal strSql As String, ByVal strTitle As String, ParamArray arrInput() As Variant) As ADODB.Recordset
...
'               如果为一个参数,定义成Vardata() as Variant变量,则按该参数个数匹对.
...
'调用方式为：Set rsPati=OpenSQLRecord(strSQL, Me.Caption, CDate(Format(rsMove!转出日期,"yyyy-MM-dd")),dtp时间.Value, lng病人ID, "张%", "男", 20, 21)
    ...
    If UBound(arrInput) = 0 Then
       If TypeName(arrInput(0)) = "Variant()" Then
           varPara = arrInput(0)
       End If
    End If
...
'    Loop
    ...
    Set OpenSQLRecordNew = cmdData.Execute
    Call SQLTest
End Function
...
Public Function GetIDKindStr(Optional strIDKindStr As String = "姓|姓名|0;医|医保号|0;身|身份证号|0;IC|IC卡号|1;门|门诊号|0;住|住院号|0;就|就诊卡|0", Optional blnOnlyAccouct As Boolean = False, Optional objSquare As clsCardSquare) As String
    ...
    '出参:
    '返回: 短名|全名|读卡标志|卡类别ID|卡号长度|缺省标志(1-当前缺省;0-非缺省)|是否存在帐户(1-存在帐户;0-不存在帐户)|卡号密文(第几位至第几位加密,空为不加密);…
        ...
        If Trim(varData(i)) <> "" Then
            ...
            If blnFind Then
                strNewIdKindStr = strNewIdKindStr & ";" & objCard.短名 & "|" & objCard.名称 & "|" & IIf(objCard.是否刷卡, 0, 1) & _
                                ...
                                "|" & IIf(objCard.是否存在帐户, 1, 0) & "|" & objCard.卡号密文规则
                strTemp = strTemp & "," & objCard.接口序号
            Else
                '短名|全名|读卡标志|卡类别ID(-1代表模糊查找)|卡号长度|缺省标志(1-当前缺省;0-非缺省)|是否存在帐户(1-存在帐户;0-不存在帐户)|卡号密文(第几位至第几位加密,空为不加密);…
...
Public Function GetAvailabilityCardType() As String
    ...
    '返回:格式:短|全名|读卡标志|卡类别ID(消费卡序号)|长度|是否消费卡|结算方式|是否密文|是否自制卡|是否退现|是否全退;…
    '编制:刘兴洪
    ...
    '是否有一卡通消费操作的虚拟模块
    strPrivs = ";" & GetPrivFunc(glngSys, 1151) & ";"
        ...
        If objYlCardObjs(i).CardPreporty.启用 And objYlCardObjs(i).CardPreporty.是否存在帐户 Then
            If Not objYlCardObjs(i).CardObject Is Nothing Then
                ...
                If blnAdd Then
                    ...
                    strCardStr = strCardStr & "|" & IIf(objCard.是否退现, 1, 0) & "|" & IIf(objCard.是否退现, 1, 0)
                    strCardStr = strCardStr & "|" & IIf(objCard.是否全退, 1, 0) & "|" & IIf(objCard.是否全退, 1, 0)
                    strCardStr = strCardStr & "|" & IIf(objCard.是否退款验卡, 1, 0)
                    '问题:50120
                End If
            End If
        End If
...
Public Function GetPatiID(ByVal strCardType As String, _
    ...
    Optional lngShowCardNoTypeID As Long = 0) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If Not frmSel.ShowSelect(frmMain, cnOracle, glngSys, glngModul, objCtl, strSql, "病人选择", "当前搜索出多个病人信息,请选择指定的病人", True, blnShowMergePati, _
                             IIf(objCtl Is Nothing, False, True), "", "密码ID,ID", rsTemp, Val(strCardType), strCardNo, Val(str标识号), lng病人ID, lngShowCardNoTypeID) Then
