...
End Enum
 ...
 Private Sub SetCtrlEnable()
    ...
    chk缺省退现.Enabled = chkEdit(mchkIdx.idx_允许退现).Enabled
    '96938:李南春,2016/5/30,医疗卡是否部分退款
    ...
    cbo结算方式.Enabled = blnEdit And Not bln院内卡
...
errHandle:
 ...
 Private Function CheckDepent() As Boolean
    ...
    strSQL = "Select 名称 From 结算方式 Where 性质 =7 and nvl(应付款,0)=0 "
...
Private Function LoadCardData() As Boolean
    ...
    "            nvl(A.是否刷卡,0)  as   是否刷卡,  nvl(A.是否自制,0)  as    是否自制," & _
    ...
    "           nvl(A.是否持卡消费,0) as 是否持卡消费, " & _
    "           nvl(A.发送调用接口,0) as 发送调用接口, " & _
    ...
    chkOpenEnter.Enabled = chkEdit(mchkIdx.idx_刷卡方式).value = 1
...
Private Function Select险类() As Boolean
    ...
    vRect = GetControlRect(txtEdit(mtxtIdx.idx_险类).hWnd)
...
Private Function Select卡费(ByVal strInput As String) As Boolean
    ...
    vRect = GetControlRect(txtEdit(mtxtIdx.idx_医疗卡费).hWnd)
...
Private Sub txtEdit_Change(Index As Integer)
    ...
    '问题号:54259
    If Index = mtxtIdx.idx_卡号长度 Then
       If Val(Trim(txtEdit(mtxtIdx.idx_卡号长度).Text)) > 25 Then txtEdit(mtxtIdx.idx_卡号长度).Text = 25
    End If
End Sub
Private Function SaveData() As Boolean
    ...
    strSQL = strSQL & "" & chkEdit(mchkIdx.idx_刷卡方式).value & ","
