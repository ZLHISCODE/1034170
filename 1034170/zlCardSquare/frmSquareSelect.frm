...
Private Sub Form_Load()
    ...
    With lvw所有
        For i = 1 To objYlCardObjs.Count
            '问题:48005
            If Not (objYlCardObjs(i).消费卡 And objYlCardObjs(i).CardPreporty.自制卡) Or (objYlCardObjs(i).CardPreporty.自制卡 And objYlCardObjs(i).CardPreporty.接口程序名 <> "") Then
                If Not (objYlCardObjs(i).消费卡 = False And (objYlCardObjs(i).自制卡 Or Not objYlCardObjs(i).CardPreporty.是否存在帐户)) Or (objYlCardObjs(i).CardPreporty.自制卡 And objYlCardObjs(i).CardPreporty.接口程序名 <> "") Then
                    Set lvwItem = .ListItems.Add(, IIf(objYlCardObjs(i).消费卡, "X", "K") & objYlCardObjs(i).接口序号, objYlCardObjs(i).CardPreporty.接口编码, , 1)
                    lvwItem.SubItems(1) = objYlCardObjs(i).CardPreporty.名称
                End If
            End If
        Next
...
Public Function zlShowSelect(ByVal frmMain As Object, _
    ByRef lng卡类别ID As Long, _
    Optional ByRef bln消费卡 As Boolean) As Boolean
    '------------------------------------------------------------------------------------------------------------------------
    '功能：选择结算卡接口
    '入参：frmMain-调入的主窗口
