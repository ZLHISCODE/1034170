...
'                If objYlCardObjs(i).接口序号 = lngCardTypeID _
'                    And objYlCardObjs(i).消费卡 = bln消费卡 Then
'                    objYlCardObjs(i).CardPreporty.启用 = IIf(.TextMatrix(.Row, .ColIndex("启用")) = "×", False, True)
'                End If
'            Next
            Set gObjYLCardObjs = Nothing: Set gObjYLCards = Nothing '88185,将这两个全局变量置为nothing，以便其它地方更新医疗卡缓存数据
        End If
...
End Sub
 Private Sub vfgList_AfterRowColChange(ByVal OldRow As Long, ByVal OldCol As Long, ByVal NewRow As Long, ByVal NewCol As Long)
    ...
    With vfgList
        ...
        '问题48005
        If InStr(.RowData(NewRow), "X") <> 1 Then
