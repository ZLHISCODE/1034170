...
Private Function LoadPatiInfor() As Boolean
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Set mrsInfor = objDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病人ID)
    If mrsInfor.RecordCount = 0 Then
        MsgBox "当前病人信息不存在，请检查"
        Exit Function
    End If
    
    '读取其他证件
    strSQL = "Select a.信息名 as 证件类型, a.信息值 as 证件号码 From 病人信息从表 A, 证件类型 B Where a.病人id = [1] And a.信息名 = b.名称  Order by 信息名"
    Set mrsOtherCertificate = objDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病人ID)
    
    '读取过敏药物
    strSQL = "  Select 过敏药物,过敏反应 from 病人过敏药物 where 病人ID=[1] order by 过敏药物"
    Set mrsDrug = objDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病人ID)
    ...
    "   Where 病人id = [1] " & _
    "   Order By 接种时间"
    Set mrsBacterin = objDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病人ID)
    LoadPatiInfor = True
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
...
Private Sub LoadPhoto()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    '说明:
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim strSQL As String, strTempFile As String
    ...
    End If
    '显示照片
    strSQL = "select 照片 from 病人照片 where 病人ID=[1]"
    Set rsTemp = objDatabase.OpenSQLRecord(strSQL, Me.Caption, mlng病人ID)
    picPhoto.Cls
