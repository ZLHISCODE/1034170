...
Private Const mstrLvw As String = "名称,2174.74,0,1;编码,799.9371,0,2;短名,629.8583,2,0;前缀文本,929.7639,2,0;" & _
    "卡号长度,989.8583,0,1;缺省,630,2,1;固定项,760,2,0;严格控制,1000,2,0;磁卡,800,0,0;卡类别,800,2,0;存在帐户,1000,2,0;" & _
    "全退,1400,2,0;部件,1500,0,0;医疗卡费,1500,0,0;结算方式,1620,0,0;卡号密文,1000,0,0;启用,600,2,0;备注,2000,0,0;" & _
    "模糊查找,1000,2,0;是否制卡,1000,2,0;是否发卡,1000,2,0;是否写卡,1000,2,0;转帐及代扣,1200,2,0;必须持卡消费,1300,2,0;" & _
    "发送调用接口,1300,2,0;启用回车,1000,2,0;是否退款验卡,1300,2,0;是否缺省退现,1300,2,0" '问题号:56508
...
Private Sub LoadData()
    ...
    '103310,李南春,2016/12/6:卡号后增加回车符位
    strSQL = "" & _
    ...
    "           decode(nvl(A.是否刷卡,0),1,'√','') as   磁卡, decode(nvl(A.是否自制,0),1,'院内卡','院外卡') as    卡类别," & _
    ...
    "           decode(nvl(A.是否转帐及代扣,0),1,'√','') as   转帐及代扣," & _
    ...
    "           Decode(Nvl(a.是否缺省退现,0),1,'√','') As 是否缺省退现" & _
    ...
    Do While Not rsTemp.EOF
        ...
        objItem.Tag = IIf(Nvl(rsTemp!启用) = "√", 0, 1) & "-" & IIf(Nvl(rsTemp!固定项) = "√", 1, 0)
        '根据ListView的列名从数据库取数
         ...
         If MsgBox("你确认要" & IIf(blnStop, "停用", "启用") & "医疗卡""" & lvwMain.SelectedItem.Text & """吗？", vbQuestion + vbYesNo + vbDefaultButton2, gstrSysName) = vbYes Then
            ...
            Else
                ...
                For i = 1 To .ColumnHeaders.count
                    ...
                    End If
                Next
                .SelectedItem.Tag = IIf(blnStop, 1, 0) & "-" & varTemp(1)
            End If
        End If
    End With
...
Private Sub ModifyData()
    ...
    varTemp = Split(lvwMain.SelectedItem.Tag & "-", "-")
    If Val(varTemp(0)) = 1 Then Exit Sub
...
Private Function IsModifyOrDelete() As Boolean
    ...
    varTemp = Split(lvwMain.SelectedItem.Tag & "-", "-")
    IsModifyOrDelete = Val(varTemp(0)) = 0 And Val(varTemp(1)) = 0
End Function
...
Public Sub zlUpdateCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_Edit_NewItem
        Control.Visible = zlCheckPrivs(mstrPrivs, "增加")
        Control.Enabled = Control.Visible
    Case conMenu_Edit_Modify
        Control.Visible = zlCheckPrivs(mstrPrivs, "修改")
        Control.Enabled = Control.Visible And IsModify
    Case conMenu_Edit_Delete
        Control.Visible = zlCheckPrivs(mstrPrivs, "删除")
        Control.Enabled = Control.Visible And IsModifyOrDelete
    Case conMenu_Edit_Reuse
        Control.Visible = zlCheckPrivs(mstrPrivs, "启用")
        Control.Enabled = Control.Visible And IsStart
    Case conMenu_Edit_Stop
        Control.Visible = zlCheckPrivs(mstrPrivs, "停用")
