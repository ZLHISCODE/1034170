...
Private Function GetFilter() As Boolean
    ...
    Else
        cllFilter.Add Split(cbo类型.Text, "-")(1), "卡类型"
    End If
    cllFilter.Add Array(Trim(txtEdit(mtxtIdx.idx_开单卡号).Text), Trim(txtEdit(mtxtIdx.idx_结束卡号).Text)), "卡号范围"
...
Private Sub InitData()
    ...
    gstrSQL = "Select 编码, 名称, 缺省面额, 缺省折扣, 缺省标志 From 消费卡类型"
