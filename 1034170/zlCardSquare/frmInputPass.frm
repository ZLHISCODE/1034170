...
Private mblnShowPatiInfor As Boolean
...
Public Function zlBrushPay(frmMain As Object, _
    ...
    Optional ByVal bln转预交 As Boolean = False) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:刷卡处理
    '入参:frmMain-调用的主窗体
    '        lngCardTypeID-卡类别ID(0-表示只刷一卡通)
    '        rsClassMoney-收费类别,实收金额
    '        blnShowPatiInfor-是否显示病人信息
    '       dbl金额-传入需要扣款的金额
    ...
    mblnShowPatiInfor = blnShowPatiInfor: mbln退费 = bln退费
...
Private Sub InitTaskPancel()
    ...
    If mblnShowPatiInfor Then
        ...
        Call Item.SetMargins(0, -19, 0, IIf(mblnShowPatiInfor, -10, -4))
    End If
...
Private Sub SetWindowHeight()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    If tkpGroup.Expanded Then
        sngHeight = sngHeight + picBlance.Height + IIf(mblnShowPatiInfor = False, 200, 0)
    End If
...
Private Sub InitFace()
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    picPati.Visible = mblnShowPatiInfor
...
Private Function SetBrushObject() As Boolean
    ...
    End With
    '支持刷卡或读卡
    lbl卡号.BorderStyle = IIf(Not mcurCardObject.CardPreporty.是否刷卡, 1, 0)
   ...
   ' If CheckBrush消费卡(Trim(txt卡号.Tag)) = False Then Exit Function
    ...
    If Not mbln退费 Then
        ...
        If Round(mdbl帐户余额, 6) < Round(mdbl本次扣款额, 6) Then
            MsgBox "当前支付金额(" & Format(mdbl本次消费总额, "0.00") & ")大于了帐户余额(" & Format(mdbl本次消费总额, "0.00") & "),不能继续!", vbInformation, gstrSysName
            If txtPass.Enabled And txtPass.Visible Then txtPass.SetFocus
            Exit Function
        End If
    End If
...
Private Function ReadCardNo() As Boolean
    ...
    'strOutPatiInforXml  XML Out 见strOutPatiInforXml参数说明
    '参数: blnOlnyCardNO=true时,返回空
    If mcurCardObject.CardObject.zlReadCard(Me, mlngModule, True, "", strCardNo, strOutPatiXML) = True Then
         txt卡号.Text = strCardNo ' GetCardNODencode(strCardNo, mlngCardTypeID, mcurCardObject.CardPreporty.卡号密文规则, mbln消费卡)
         txt卡号.Tag = strCardNo
    End If
    ...
    'txt卡号.PasswordChar = ""
    If mbln消费卡 Then
        If CheckBrush消费卡(strCardNo) Then Exit Function
        ReadCardNo = True: Exit Function
    End If
...
Public Function CheckBrushSquareCard(ByVal lngCardTypeID As Long, _
   ...
   '---------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Dim lng卡类别ID As Long, dbl余额 As Double, str名称 As String
    ...
    If Nvl(rsTemp!有效期, "3000-01-01 00:00:00") < Format(zlDatabase.Currentdate, "yyyy-mm-dd HH:MM:SS") Then
       ...
       If Val(Nvl(rsTemp!可否充值)) = 1 Then
          ...
          dbl失效面额 = zlGet失效面额(Val(Nvl(rsTemp!id)), lng卡类别ID)
          dbl余额 = dbl余额 - dbl失效面额
    ...
    If mbln退费 Then
        If Not mVarData Is Nothing Then
            ...
            For i = 1 To mVarData.count
                ...
                varData = mVarData(i)
                If Val(varData(0)) = lngCardTypeID And strCardNo = varData(3) Then
                    blnFind = True
