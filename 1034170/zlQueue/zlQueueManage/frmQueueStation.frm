...
Private mlngCurrentWorkID As Long           '当前选中的业务ID
Private mlngCurrentWorkType As Long         '当前业务类型
Private mlngCurrentQueueId As Long          '当前队列ID
...
Private Sub SetReportRecordItem(rriItem As ReportRecord, rsData As ADODB.Recordset)
    ...
    rriItem(mCol.排队序号).value = Nvl(rsData("排队序号"))
...
Public Function zlRefresh(str队列名称() As String, ByVal strCur队列名称 As String, ByVal strCur业务ID As String, _
    ...
    mstr队列名称 = str队列名称
    'mstrCurrent队列名称 = strCur队列名称 '刷新时刷新所有的队列数据，不需要覆盖当前队列名称
    'mlngCurrentWorkID = lngCur业务ID
...
'    Call DebugMsg("ZLQueueInterface", "ZLRefresh", "str诊室=" & str诊室 & ", str医生=" & str医生 & ", strExcludeData=" & strExcludeData & ", intViewDataType=" & intViewDataType)
    ...
    Else
       ...
       '如果没有传递队列名称，则直接退出
'       MsgBox "没有可显示的叫号队列信息，请检查相关排队科室设置。", vbInformation, "排队叫号系统"
...
'    Call DebugMsg("ZLQueueInterface", "ZLRefresh", "str执行状态=" & str执行状态)
    ...
    "           " & strFilter & _
    "   Order by 优先 Desc, 排队状态 desc, 排队序号, 回诊排序号, 排队时间, 排队号码 "
    ...
    rsLocal.Sort = "队列名称, 排队状态 desc,优先 Desc, 排队序号, 回诊排序号, 排队时间, 排队号码"
...
'*************************************************************************************
    ...
    For i = 0 To rptQueueList.Rows.Count - 1
        If rptQueueList.Rows(i).GroupRow <> True Then
            If rptQueueList.Rows(i).Record(mCol.业务ID).value = str业务ID _
                And rptQueueList.Rows(i).Record(mCol.业务类型).value = lng业务类型 _
                And rptQueueList.Rows(i).Record(mCol.队列名称).value = mstrCurrent队列名称 Then
    ...
    If Not blnFind Then
        ...
        For i = 0 To rptCallList.Rows.Count - 1
            If rptCallList.Rows(i).GroupRow <> True Then
                If rptCallList.Rows(i).Record(mCol.业务ID).value = str业务ID _
                    And rptCallList.Rows(i).Record(mCol.业务类型).value = lng业务类型 _
                    And rptCallList.Rows(i).Record(mCol.队列名称).value = mstrCurrent队列名称 Then
...
'            Call comMenu_刷新
    ...
    Call zlRefresh(mstr队列名称, mstrCurrent队列名称, mlngCurrentWorkID, mstr诊室条件, mstr医生条件, mstrExcludeData, mintViewDataType)
    ...
    If Control.Id = conMenu_Queue_CallThis Or Control.Id = conMenu_Queue_CallNext Then
        For i = 0 To rptCallList.Rows.Count - 1
            If rptCallList.Rows(i).GroupRow <> True Then
                If rptCallList.Rows(i).Record(mCol.业务ID).value = mlngCurrentWorkID And rptCallList.Rows(i).Record(mCol.业务类型).value = mlngCurrentWorkType Then
                    rptCallList.Rows(i).Selected = True
...
Private Sub InitLocalParas(blnIsLISForm As Boolean)
    ...
    mstrLocateType = GetSetting("ZLSOFT", strReg, "定位方式", "就诊卡号")
...
Private Sub InitQueueList()
   ...
   '初始化排队队列显示字段
    ...
    With Me.rptCallList.Columns
        ...
        Set Column = .Add(mCol.队列名称, IIf(mlngQueueGroupType = 0, "", "队列"), 0, False)
        If mlngQueueGroupType = 0 Then
        ...
        Set Column = .Add(mCol.病人ID, "ID", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.排队标记, "标记", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.排队号码, "号码", 50, True)
        Column.Visible = True
        
        Set Column = .Add(mCol.排队序号, "排队序号", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.患者姓名, "患者姓名", 90, True)
        Column.Visible = True
        
        Set Column = .Add(mCol.优先, "优先", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.回诊序号, "回诊序号", 60, True)
        Column.Visible = IIf(InStr(1, mstrShowCalledColumnInf, ",回诊序号,") > 0, True, False)
        
        Set Column = .Add(mCol.回诊排序号, "回诊排序号", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.科室ID, "科室ID", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.诊室, "诊室", 60, True)
        Column.Visible = IIf(InStr(1, mstrShowCalledColumnInf, ",诊室,") > 0, True, False)
        
        Set Column = .Add(mCol.医生姓名, IIf(mlngQueueGroupType = 1, "", "医生姓名"), 60, True)
        ...
        Set Column = .Add(mCol.排队状态, "排队状态", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.排队时间, "排队时间", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.呼叫医生, "呼叫人", 60, True)
        Column.Visible = IIf(InStr(1, mstrShowCalledColumnInf, ",呼叫医生,") > 0, True, False)
        
        Set Column = .Add(mCol.业务类型, "业务类型", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.业务ID, "业务ID", 0, False)
        Column.Visible = False
                
        Set Column = .Add(mCol.呼叫时间, "呼叫时间", 125, True)
    ...
    With Me.rptCallList
        ...
        If mlngQueueGroupType = 0 Then
            .GroupsOrder.Add .Columns(mCol.队列名称)
        Else
...
'        rptQueueList.SetImageList Imglist
        ...
        Set Column = .Add(mCol.排队标记, "标记", 30, False)
        Column.Visible = False
        ...
        Set Column = .Add(mCol.优先, "优先", 40, True)
        ...
        Set Column = .Add(mCol.回诊排序号, "回诊排序号", 60, True)
        Column.Visible = False
        ...
        Set Column = .Add(mCol.排队状态, "排队状态", 50, True)
        Column.Visible = IIf(InStr(1, mstrShowColumnInf, ",排队状态,") > 0, True, False)
        
        Set Column = .Add(mCol.排队时间, "排队时间", 125, True)
        Column.Visible = IIf(InStr(1, mstrShowColumnInf, ",排队时间,") > 0, True, False)
        
        Set Column = .Add(mCol.呼叫医生, "呼叫人", 0, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.业务类型, "业务类型", 120, False)
        Column.Visible = False
        
        Set Column = .Add(mCol.业务ID, "业务ID", 60, False)
        Column.Visible = False
                
        Set Column = .Add(mCol.呼叫时间, "呼叫时间", 0, False)
        Column.Visible = False
    End With
...
Private Sub comMenu_直呼()
    ...
    If mstrCurrent队列名称 <> "" Then
        ...
        If Not blnCancel Then
            strSql = "ZL_排队叫号队列_呼叫('" & mstrCurrent队列名称 & "',1,'" & mstrLoginUserName & "','" & mstr呼叫站点名称 & "'," & mlngCurrentWorkType & ",'" & mlngCurrentWorkID & "')"
...
Private Sub comMenu_顺呼()
    ...
    Else
        '获取队列名称
        If rptQueueList.SelectedRows(0).GroupRow <> True Then
            strTempQueueName = rptQueueList.SelectedRows(0).Record(mCol.队列名称).value
        Else
            strTempQueueName = rptQueueList.SelectedRows(0).Childs(0).Record(mCol.队列名称).value
        End If
    End If
    ...
    If mstrCurrent队列名称 <> "" Then
        ...
        For i = 0 To rptQueueList.Rows.Count - 1
            If rptQueueList.Rows(i).GroupRow <> True Then
                If rptQueueList.Rows(i).Record(mCol.队列名称).value = strTempQueueName Then
                    strSql = "select ID,业务类型,业务ID from 排队叫号队列 where 队列名称=[1] and 业务ID=[2] and 业务类型=[3] and 排队状态=0"
                    Set rsTemp = zlDatabase.OpenSQLRecord(strSql, "顺呼", strTempQueueName, CStr(Nvl(rptQueueList.Rows(i).Record(mCol.业务ID).value)), Val(Nvl(rptQueueList.Rows(i).Record(mCol.业务类型).value)))
                ...
                End If
            Else
                If rptQueueList.Rows(i).Childs(0).Record(mCol.队列名称).value = strTempQueueName Then
...
Private Sub comMenu_暂停()
    ...
    On Error GoTo errHandle
        ...
        If mstrCurrent队列名称 <> "" Then
            ...
            If Not blnCancel Then
                strSql = "ZL_排队叫号队列_呼叫('" & mstrCurrent队列名称 & "',3,'" & mstrLoginUserName & "','" & mstr呼叫站点名称 & "'," & mlngCurrentWorkType & ",'" & mlngCurrentWorkID & "')"
...
Private Sub comMenu_完成()
    ...
    On Error GoTo errHandle
        ...
        If mstrCurrent队列名称 <> "" Then
            ...
            If Not blnCancel Then
                strSql = "ZL_排队叫号队列_呼叫('" & mstrCurrent队列名称 & "',4,'" & mstrLoginUserName & "','" & mstr呼叫站点名称 & "'," & mlngCurrentWorkType & ",'" & mlngCurrentWorkID & "')"
...
Private Sub comMenu_优先()
    ...
    If mstrCurrent队列名称 <> "" Then
        With rptQueueList
            ...
            If .Rows.Count > 0 Then
                ...
                If .SelectedRows(0).GroupRow <> True Then
                    strTempQueueName = .SelectedRows(0).Record(mCol.队列名称).value
                    strSelectedName = .SelectedRows(0).Record(mCol.排队号码).value & .SelectedRows(0).Record(mCol.患者姓名).value & "," & .SelectedRows(0).Record(mCol.Id).value
                Else
                    strTempQueueName = .SelectedRows(0).Childs(0).Record(mCol.队列名称).value
                    strSelectedName = .SelectedRows(0).Childs(0).Record(mCol.排队号码).value & .SelectedRows(0).Childs(0).Record(mCol.患者姓名).value & "," & .SelectedRows(0).Record(mCol.Id).value
                End If
        ...
        '调用优先原因窗体
        Call frmPriorityCause.ShowPriorityCause(Me, mstrCurrent队列名称, mlngCurrentWorkID, strTempQueueName, strSelectedName)
    End If
...
Private Sub comMenu_恢复()
    ...
    On Error GoTo errHandle
        ...
        If mstrCurrent队列名称 <> "" Then
            ...
            If Not blnCancel Then
                strSql = "ZL_排队叫号队列_呼叫('" & mstrCurrent队列名称 & "',0,'" & mstrLoginUserName & "','" & mstr呼叫站点名称 & "'," & mlngCurrentWorkType & ",'" & mlngCurrentWorkID & "')"
...
Private Sub comMenu_弃号()
    ...
    On Error GoTo errHandle
        ...
        If mstrCurrent队列名称 <> "" Then
            ...
            If Not blnCancel Then
                strSql = "ZL_排队叫号队列_呼叫('" & mstrCurrent队列名称 & "',2,'" & mstrLoginUserName & "','" & mstr呼叫站点名称 & "'," & mlngCurrentWorkType & ",'" & mlngCurrentWorkID & "')"
...
Private Sub comMenu_刷新()
    'Call zlRefresh(mstr队列名称, mstrCurrent队列名称, mlngCurrentWorkID)
    Call zlRefresh(mstr队列名称, mstrCurrent队列名称, mlngCurrentWorkID, mstr诊室条件, mstr医生条件, mstrExcludeData, mintViewDataType)
End Sub
...
Private Sub comMenu_修改()
    ...
    '已经呼叫的数据不能进行修改
    '记录当前的队列名称和工作ID
    lng病人id = Val(Nvl(rptQueueList.SelectedRows(0).Record(mCol.病人ID).value))
    str队列名称 = Nvl(rptQueueList.SelectedRows(0).Record(mCol.队列名称).value)
    str患者姓名 = Nvl(rptQueueList.SelectedRows(0).Record(mCol.患者姓名).value)
    str诊室 = Nvl(rptQueueList.SelectedRows(0).Record(mCol.诊室).value)
    str医生姓名 = Nvl(rptQueueList.SelectedRows(0).Record(mCol.医生姓名).value)
    lng业务类型 = Val(Nvl(rptQueueList.SelectedRows(0).Record(mCol.业务类型).value))
    
    str业务ID = Nvl(rptQueueList.SelectedRows(0).Record(mCol.业务ID).value)
    lng科室ID = Val(Nvl(rptQueueList.SelectedRows(0).Record(mCol.科室ID).value))
    ...
    If frmUpdateInfo.zlShowMe(Me, mstr队列名称, str队列名称, str患者姓名, str诊室, str医生姓名) = True Then
        ...
        If str队列名称 <> rptQueueList.SelectedRows(0).Record(mCol.队列名称).value Then
            
            On Error GoTo DBError
            Call mcnOracle.BeginTrans
            
            strSql = "ZL_排队叫号队列_DELETE('" & rptQueueList.SelectedRows(0).Record(mCol.队列名称).value & "','" & str业务ID & "')"
...
DBError:
        ...
        Else    '没有修改队列名称，则直接修改信息即可
            strSql = "ZL_排队叫号队列_UPDATE('" & str队列名称 & "'," & lng业务类型 & ",'" & rptQueueList.SelectedRows(0).Record(mCol.业务ID).value _
                    & "'," & rptQueueList.SelectedRows(0).Record(mCol.科室ID).value & ",'" & str患者姓名 & "','" _
                    & str诊室 & "','" & str医生姓名 & "')"
...
Private Sub rptCallList_SelectionChanged()
    ...
    If rptCallList.SelectedRows(0).GroupRow = True Then
        If rptCallList.SelectedRows(0).Childs.Count > 0 Then
            mstrCurrent队列名称 = rptCallList.SelectedRows(0).Childs(0).Record(mCol.队列名称).value
            
            mlngCurrentWorkID = Val(rptCallList.SelectedRows(0).Childs(0).Record(mCol.业务ID).value)
            mlngCurrentWorkType = StrNvl(rptCallList.SelectedRows(0).Childs(0).Record(mCol.业务类型).value, 0)
    ...
    '记录当前的队列名称和工作ID
    mstrCurrent队列名称 = rptCallList.SelectedRows(0).Record(mCol.队列名称).value
    mlngCurrentWorkID = Val(rptCallList.SelectedRows(0).Record(mCol.业务ID).value)
    mlngCurrentWorkType = StrNvl(rptCallList.SelectedRows(0).Record(mCol.业务类型).value, 0)
...
Private Sub rptQueueList_SelectionChanged()
    ...
    If rptQueueList.SelectedRows(0).GroupRow = True Then
        If rptQueueList.SelectedRows(0).Childs.Count > 0 Then
            mstrCurrent队列名称 = rptQueueList.SelectedRows(0).Childs(0).Record(mCol.队列名称).value
            
            mlngCurrentWorkID = Val(rptQueueList.SelectedRows(0).Childs(0).Record(mCol.业务ID).value)
            mlngCurrentWorkType = StrNvl(rptQueueList.SelectedRows(0).Childs(0).Record(mCol.业务类型).value, 0)
    ...
    '记录当前的队列名称和工作ID
    mstrCurrent队列名称 = rptQueueList.SelectedRows(0).Record(mCol.队列名称).value
    mlngCurrentWorkID = Val(rptQueueList.SelectedRows(0).Record(mCol.业务ID).value)
    mlngCurrentWorkType = StrNvl(rptQueueList.SelectedRows(0).Record(mCol.业务类型).value, 0)
...
Private Function LocateQueueData(ByVal findType As String, ByVal findData As String) As Boolean
    ...
    If findType = "挂号单" Then Exit Function   '不根据挂号单定位
    
    If findType <> "排队号" Then
        If findType = "姓名" Then
'            If Not mobjSquareCard.zlGetPatiID("upper(" & findType & ")", UCase(findData), , lngPatientId) Then Exit Sub
            ...
            For i = 0 To rptQueueList.Rows.Count - 1
                If rptQueueList.Rows(i).GroupRow = True Then
                    For j = 0 To rptQueueList.Rows(i).Childs.Count - 1
                        ...
                        If findType = "姓名" Then
                            blnFind = IIf(rptQueueList.Rows(i).Childs(j).Record(mCol.患者姓名).value Like findData & "*", True, False)
                        Else
                            blnFind = IIf(rptQueueList.Rows(i).Childs(j).Record(mCol.病人ID).value = lngPatientId, True, False)
                        End If
        ...
        Else
            For i = 0 To rptQueueList.Rows.Count - 1
                If UCase(rptQueueList.Rows(i).Record.Item(mCol.排队号码).value) = UCase(findData) Then
            ...
            For i = 0 To rptCallList.Rows.Count - 1
                If rptCallList.Rows(i).GroupRow = True Then
                    For j = 0 To rptCallList.Rows(i).Childs.Count - 1
                        ...
                        If findType = "姓名" Then
                            blnFind = IIf(rptCallList.Rows(i).Childs(j).Record(mCol.患者姓名).value Like findData & "*", True, False)
                        Else
                            blnFind = IIf(rptCallList.Rows(i).Childs(j).Record(mCol.病人ID).value = lngPatientId, True, False)
                        End If
        ...
        Else
            For i = 0 To rptCallList.Rows.Count - 1
                If UCase(rptCallList.Rows(i).Record.Item(mCol.排队号码).value) = UCase(findData) Then
    ...
    Else
        If mblnIsGroup Then
            For i = 0 To rptQueueList.Rows.Count - 1
                If rptQueueList.Rows(i).GroupRow = True Then
                    For j = 0 To rptQueueList.Rows(i).Childs.Count - 1
                        If UCase(rptQueueList.Rows(i).Childs(j).Record(mCol.排队号码).value) = UCase(findData) Then
        ...
        If mblnIsGroup Then
            For i = 0 To rptCallList.Rows.Count - 1
                If rptCallList.Rows(i).GroupRow = True Then
                    For j = 0 To rptCallList.Rows(i).Childs.Count - 1
                        If UCase(rptCallList.Rows(i).Childs(j).Record(mCol.排队号码).value) = UCase(findData) Then
...
On Error Resume Next
    ...
    If Trim(strCurKindInfo) <> "" Then
        ...
        lngKindId = Val(aryKindInfo(3))     '卡类别ID
        lngCardType = Val(aryKindInfo(2))   '刷卡类型
        blnIsPwdInput = Val(aryKindInfo(7)) '是否密文显示
    End If
    
    If lngCardType = 1 Then '处理读卡
        If lngKindId <> 0 Then
...
Private Sub txtLocateValue_KeyPress(KeyAscii As Integer)
    ...
    If Trim(strCurKindInfo) <> "" Then
        aryKindInfo = Split(strCurKindInfo, "|")
        lngKindId = Val(aryKindInfo(3))     '卡类别ID
        lngKindLen = Val(aryKindInfo(4))    '卡号长度
        blnIsPwdInput = Val(aryKindInfo(7)) '是否密文显示
        lngCardType = Val(aryKindInfo(2))   '刷卡类型
    End If
