...
Private mlngCruuentWorkID As Long      '业务ID
...
Public Sub ShowPriorityCause(frmParent As Form, ByVal strCurrentCaption As String, ByVal lngCurrentWorkID As Long, _
                            ByVal strTempQueueName As String, ByVal strSelectedName As String)
    
    '保存传入参数
    mstrQueueName = strCurrentCaption
    mlngCruuentWorkID = lngCurrentWorkID
...
On Error GoTo errHandle
    ...
    '执行插队 队列修改以及原因写入
    strSql = "ZL_排队叫号队列_优先('" & mstrQueueName & "'," & mlngCruuentWorkID & ",'" & Trim(cboPriorityCause.Text) & "'," & intNeedJQueueID & "," & intJQueueID & ")"
