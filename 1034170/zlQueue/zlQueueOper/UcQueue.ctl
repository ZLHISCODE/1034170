...
'获取排队队列的数据显示状态，排队，暂停，弃呼，完成
    ...
    If lngState = -1 Then lngState = TQueueSelState.qss排队中      '当未选择任何状态时，则只加载排队中的数据
...
'查询队列数据到数据集
    ...
    If Not blnUseCustom Then
        ...
        If lngTimePoint <= 4 Then
            strStartTime = To_Date(Format(strNow - 1, "yy-mm-dd 20:00:00"))
            strEndTime = To_Date(Format(strNow, "yy-mm-dd 08:00:00"))
        Else
            strStartTime = To_Date(Format(strNow, "yy-mm-dd 00:00:00"))
            strEndTime = To_Date(Format(strNow, "yy-mm-dd 23:59:59"))
        End If
...
Private Sub LoadDataToList(objCurQueueList As ReportControl, rsData As ADODB.Recordset)
'载入队列数据
...
'加载数据到列表
    ...
    If blnIsWaitQueue Then
        ...
        strFilter = "排队状态=" & lngQueryQueueState
...
'初始化排队列表
    ...
    With objCurQueueList
        ...
        If Trim(strGroupCol) <> "" Then
            ...
            .GroupsOrder(0).SortAscending = False ' True
        End If
    ...
    For i = 0 To rptControl.Columns.Count - 1
        ...
        Else
            ...
            ElseIf rptControl.Columns(i).Caption = "队列名称" Then
                If Nvl(rsData!队列名称) = mstrLastFixedQueue Then
...
Public Sub zlUpdateCommandBars(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Select Case Control.Id
        ...
        Case conMenu_Queue_Broadcast    '广播           '只有呼叫后的数据才能进行广播
          ...
          Control.Enabled = mblnIsSelectedCallingList
        ...
        Case conMenu_Queue_RecDiagnose  '接诊
          ...
          Control.Enabled = mblnIsSelectedCallingList Or mblnIsFindQueue Or (Not mblnIsSelectedCallingList And mintQueueState <> TQueueSelState.qss已完成 And mintQueueState <> TQueueSelState.qss排队中)
          
        Case conMenu_Queue_Pause        '暂停
          Control.Visible = CheckPopedom("暂停")
          Control.Enabled = (Not mblnIsSelectedCallingList And mintQueueState <> TQueueSelState.qss已暂停) Or mblnIsFindQueue Or mblnIsSelectedCallingList
          
        Case conMenu_Queue_Abandon      '弃号           '已呼叫数据可以进行弃号
          Control.Visible = CheckPopedom("弃号")
          Control.Enabled = (Not mblnIsSelectedCallingList And mintQueueState <> TQueueSelState.qss已弃号) Or mblnIsFindQueue Or mblnIsSelectedCallingList
          
        Case conMenu_Queue_Restore      '恢复
          Control.Visible = CheckPopedom("恢复")
          Control.Enabled = mblnIsSelectedCallingList Or (mintQueueState <> TQueueSelState.qss排队中 And Not mblnIsSelectedCallingList) Or mblnIsFindQueue
