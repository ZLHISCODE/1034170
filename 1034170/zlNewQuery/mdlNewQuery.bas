...
Public Function ReadPicByField(objField As Field) As StdPicture
    ...
    Set ReadPicByField = VB.LoadPicture(strFile)
    Kill strFile
    Exit Function
errH:
    Close intFile
...
Public Function CreateFileByField(objField As Field, strFile As String) As Boolean
...
Public Function ReadFlashByField(objField As Field) As String
    Dim lngFileSize As Long, arrBin() As Byte
    Dim strFile As String, intFile As Integer
...
Public Function SaveFlash(ByVal strFile As String, ByVal byt性质 As Byte, ByVal byt类型 As Byte, Optional ByVal vKey As Long = 0, Optional ByVal vOldName As String = "") As Long
    ...
    If vKey = 0 Then
        ...
        Call zlDatabase.OpenRecordset(gRs, "select 序号,性质,名称,类型,图形,宽度,高度,固定,修改日期 From 咨询图片元素 where 序号=0", "mdlNewQuery", adOpenStatic, adLockOptimistic)
        On Error GoTo errHand
    ...
    Else
        lngKey = vKey
        Call zlDatabase.OpenRecordset(gRs, "select 序号,性质,名称,类型,图形,宽度,高度,固定,修改日期 from 咨询图片元素 where 序号=" & vKey, "mdlNewQuery", adOpenStatic, adLockOptimistic)
        On Error GoTo errHand
        gcnOracle.BeginTrans
    End If
    ...
    If vOldName = "" Then gRs!名称 = StrReverse(Mid(StrReverse(strFile), 5, InStr(StrReverse(strFile), "\") - 5))
    gRs!性质 = byt性质
    
    gRs!类型 = byt类型
    
    gRs!宽度 = vFlashHeader.lMWidth
    gRs!高度 = vFlashHeader.lMHeight
    
    gRs!修改日期 = zlDatabase.Currentdate
    
    If Not WriteBinByFile(strFile, gRs.Fields("图形")) Then
        gRs.CancelUpdate
...
Public Function SavePicture(ByVal strFile As String, imgTmp As Object, ByVal byt性质 As Byte, ByVal byt类型 As Byte, Optional ByVal vKey As Long = 0, Optional ByVal vOldName As String = "") As Long
    ...
    If vKey = 0 Then
        lngKey = NextValue("咨询图片元素", "序号")
        strSQL = "select " & zlGetFeeFields("咨询图片元素") & " from 咨询图片元素 where 序号=0"
        Call zlDatabase.OpenRecordset(gRs, strSQL, "mdlNewQuery", adOpenStatic, adLockOptimistic)
        On Error GoTo errHand
        gcnOracle.BeginTrans
    
        gRs.AddNew
    Else
        lngKey = vKey: strSQL = "select  " & zlGetFeeFields("咨询图片元素") & "  from 咨询图片元素 where 序号=" & lngKey
         Call zlDatabase.OpenRecordset(gRs, strSQL, "mdlNewQuery", adOpenStatic, adLockOptimistic)
        On Error GoTo errHand
        gcnOracle.BeginTrans
    End If
    ...
    gRs!宽度 = imgTmp.ImageWidth
    gRs!高度 = imgTmp.ImageHeight
...
Public Function SaveMidea(ByVal strFile As String, ByVal byt性质 As Byte, ByVal byt类型 As Byte, Optional ByVal vKey As Long = 0, Optional ByVal vOldName As String = "") As Long
    ...
    If vKey = 0 Then
        lngKey = NextValue("咨询图片元素", "序号")
        gstrSQL = "select " & zlGetFeeFields("咨询图片元素") & " from 咨询图片元素 where 序号=0"
        Call zlDatabase.OpenRecordset(gRs, gstrSQL, "mdlNewQuery", adOpenStatic, adLockOptimistic)
        On Error GoTo errHand
        gcnOracle.BeginTrans
    ...
    Else
        lngKey = vKey
        gstrSQL = "select " & zlGetFeeFields("咨询图片元素") & " from 咨询图片元素 where 序号=[1]"
        Set gRs = zlDatabase.OpenSQLRecord(gstrSQL, "mdlNewQuery", vKey)
        On Error GoTo errHand
        gcnOracle.BeginTrans
...
'功能:检查本地图片,并更新本地图片
    ...
    On Error GoTo errHand
    '2.检查本系统中可能使用到的图片
    gstrSQL = "select 序号,性质,名称,类型,图形,宽度,高度,固定,修改日期 from 咨询图片元素"
    Set gRs = zlDatabase.OpenSQLRecord(gstrSQL, "mdlNewQuery")
    If gRs.BOF = False Then
        While Not gRs.EOF
            strFileName = IIf(IsNull(gRs!名称), "", gRs!名称)
            If strFileName <> "" Then
                Call CheckFile(strFileName, IIf(IsNull(gRs!类型), 0, gRs!类型), gRs!图形, gRs!修改日期, vFileData)
            End If
            gRs.MoveNext
        Wend
    End If
    ...
    '3.检查医生的照片
    gstrSQL = "select B.姓名,C.照片 from 咨询专家清单 A,人员表 B,人员照片 C where A.人员id=B.id and C.人员id=A.人员id And (b.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or b.撤档时间 Is Null) "
    Set gRs = zlDatabase.OpenSQLRecord(gstrSQL, "mdlNewQuery")
    If gRs.BOF = False Then
        While Not gRs.EOF
            ...
            If strFileName <> "" Then
                ...
                Call CreateFile(strFileName, gRs!照片, Format(zlDatabase.Currentdate, "YYYY-MM-DD HH:MM:SS"))
            End If
            gRs.MoveNext
        Wend
    End If
...
Public Sub CheckFile(ByRef strName As String, ByVal byt类型 As Byte, objFieldMedia As Field, objFieldDate As Field, flObj As FileSystemObject)
...
Public Function zlGetFeeFields(Optional strTableName As String = "门诊费用记录", Optional blnReadDatabase As Boolean = False) As String
    ...
    Case "人员缴款记录"
        ...
        Exit Function
    Case "咨询图片元素"
        zlGetFeeFields = "序号,性质,名称,类型,图形,宽度,高度,固定,修改日期"
        Exit Function
    End Select
ReadDataBaseFields:
    Err = 0: On Error GoTo errHand:
