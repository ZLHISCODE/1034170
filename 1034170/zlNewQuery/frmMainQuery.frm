...
'说明:这是查询内容显示的主体部份,显示查询内容
    ...
    '开始生成自定义查询页面
    gstrSQL = "select 页面序号,段落序号,标题文本,标题图标,标题隐藏,标题位置,标题字体,返回页首,段落文本,段落字体,插表序号,插表位置,插图序号,插图位置 from 咨询段落目录 where 页面序号=[1] order by 段落序号"
    Set gRs = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption, PageNo)
    If gRs.BOF = False Then
        While Not gRs.EOF
            ...
            '2.加载纯文本内容
            If IsNull(gRs!段落文本) = False And IsNull(gRs!插表序号) And IsNull(gRs!插图序号) Then
                strTmp = IIf(IsNull(gRs!段落字体), "宋体;12;0;0;0", gRs!段落字体)
                j = objDraw.NextTxtIndex
                
                vWidth = FrameDefault.ClientWidth - 330
                
                Call objDraw.AddPageItemTxt(j, vNextX, vNextY, gRs!段落文本 & Chr(13) & Chr(10), strTmp, vWidth, vHeight)
                vNextY = vNextY + vHeight + 150
            End If
            
            '3.加载纯图片内容
            If IsNull(gRs!段落文本) And IsNull(gRs!插图序号) = False Then
            ...
            '4.加载纯表格内容
            If IsNull(gRs!段落文本) And IsNull(gRs!插表序号) = False Then
                vHeight = 0
            ...
            '6.图片和文字混合内容
            If IsNull(gRs!段落文本) = False And IsNull(gRs!插表序号) And IsNull(gRs!插图序号) = False Then
                ...
                Case 0
                    ...
                    Call objDraw.AddPageItemTxt(j, vNextX + vTmp1 + 60, vNextY, gRs!段落文本 & Chr(13) & Chr(10), strTmp, vWidth, vHeight)
                Case 1
                    ...
                    Call objDraw.AddPageItemTxt(j, vNextX, vNextY, gRs!段落文本 & Chr(13) & Chr(10), strTmp, vTmp1, vHeight)
                End Select
            ...
            '7.表格和文字混合内容
            If IsNull(gRs!段落文本) = False And IsNull(gRs!插表序号) = False And IsNull(gRs!插图序号) Then
...
'说明:这是固定内容部份，是由ZLHIS9中提取的人员部份信息
    ...
    If gRs.BOF = False Then
        While Not gRs.EOF
            ...
            '2.照片和文字混合内容
            gstrSQL = "select A.姓名,B.照片,A.个人简介 from 人员表 A,人员照片 B where A.ID=B.人员id(+) And (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null) and A.ID=[1]"
...
Private Sub tmrCheckConnect_Timer()
    ...
    If mvarCheckConnectCounter >= mvarCheckConnectInternal Then
        ...
        '重新连接
        If gstrConnect <> "" Then
            Call OraDataOpen(Split(gstrConnect, ";")(0), Split(gstrConnect, ";")(1), Split(gstrConnect, ";")(2))
        End If
