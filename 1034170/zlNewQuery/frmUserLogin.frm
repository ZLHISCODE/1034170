    ...
    With rsOraHome
        ...
        Do While Not .EOF
            ...
            If strPath = "" And !Name & "" = "" Then
                strPath = GetKeyValue(HKEY_LOCAL_MACHINE, "SOFTWARE\Oracle" & !Name, "ORA_CRS_HOME")
            End If
            If strPath <> "" Then
                strFile = strPath & "\network\ADMIN\tnsnames.ora" 'Oracle 8i以上
                If mobjFSO.FileExists(strFile) Then Exit Do
...
'功能:通过OracleHome键获取Oracle信息
    Dim arrTmp As Variant
    arrTmp = Split(strOraHome, "_") 'Key_Ora*版本Home_32Bit 或Key_Ora*版本_Home*
    If UBound(arrTmp) = 2 Or UBound(arrTmp) = 1 Then
        If UBound(arrTmp) = 2 Then
            If UCase(arrTmp(2)) Like "HOME*" Then intTimes = ValEx(arrTmp(2))
        End If
        If UCase(arrTmp(1)) Like "*HOME*" Then
            intTimes = Val(Mid(arrTmp(1), InStr(UCase(arrTmp(1)), "HOME") + 4))
        End If
        intVer = ValEx(arrTmp(1))
        If UCase(arrTmp(1)) Like "*CLIENT*" Then
            intServer = 2
        End If
        GetOraInfoByRegKey = True
    Else
        GetOraInfoByRegKey = False
    End If
End Function
