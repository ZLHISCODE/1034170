...
Dim StrTime(6) As String, i As Integer
        ...
        On Error GoTo ErrHandle
        gstrSQL = "select A.姓名,A.性别,trunc(A.出生日期) as 生日,A.专业技术职务 as 专业,A.聘任技术职务 as 聘任,A.个人简介 as 简介,B.照片" & _
        " from 人员表 A,人员照片 B" & _
        " where A.ID = B.人员ID(+) And (a.撤档时间 = To_Date('3000-01-01', 'YYYY-MM-DD') Or a.撤档时间 Is Null) and  (A.ID ='" + MSHHead.TextMatrix(MSHHead.Row, 7) + "' or A.姓名 = '" + MSHHead.TextMatrix(MSHHead.Row, 2) + "')"
        If rsTmp.State = adStateOpen Then rsTmp.Close
...
Private Sub Load就诊卡()
    ...
    strSQL = "Select Id,名称 From 医疗卡类别 Where 是否启用 = 1 And 是否刷卡 = 1 And 是否固定 =1 Order By 缺省标志 Desc"
