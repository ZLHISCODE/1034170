...
Private Sub Form_Activate()
    ...
    If mOrder > 0 Then
        gstrSQL = "select 段落文本,段落字体,标题文本,标题位置,标题字体,标题隐藏,返回页首,标题图标,插表序号,插图序号,插表位置,插图位置 from 咨询段落目录 A where A.页面序号=[1] and A.段落序号=[2]"
...
Private Function SaveData() As Boolean
    ...
    If cmdOK.Tag <> "" Then
        ...
        If mOrder = 0 Then
'            If CheckItemLimit(mKey) = False Then
'                MsgBox "每一页面下最多只能设置16个查询项！", vbOKOnly + vbInformation, gstrSysName
'                Exit Function
'            End If
            lng段号 = CalcOrder(mKey)
            strSQL(1) = "zl_咨询段落目录_insert(" & mKey & "," & lng段号 & ",'" & txt(0).Text & "'," & IIf(chk(2).Value = 1, IIf(Val(UsrPicture(1).Tag) = 0, "NULL", Val(UsrPicture(1).Tag)), 0) & "," & chk(0).Value & "," & cbo(0).ListIndex & ",'" & IIf(txt(1).Text = "", "宋体;12;0;0;0", txt(1).Text) & "'," & chk(1).Value & "," & Val(Split(strTable, ";")(0)) & "," & Val(Split(strTable, ";")(1)) & "," & Val(Split(strPic, ";")(0)) & "," & Val(Split(strPic, ";")(1)) & ")"
        Else
            lng段号 = mOrder
            strSQL(1) = "zl_咨询段落目录_update(" & mKey & "," & lng段号 & ",'" & txt(0).Text & "'," & IIf(chk(2).Value = 1, IIf(Val(UsrPicture(1).Tag) = 0, "NULL", Val(UsrPicture(1).Tag)), 0) & "," & chk(0).Value & "," & cbo(0).ListIndex & ",'" & txt(1).Text & "'," & chk(1).Value & "," & Val(Split(strTable, ";")(0)) & "," & Val(Split(strTable, ";")(1)) & "," & Val(Split(strPic, ";")(0)) & "," & Val(Split(strPic, ";")(1)) & ")"
        ...
        If tbs.TabEnabled(1) = True Then
            ...
            gstrSQL = "select 段落序号,页面序号,段落字体,段落文本 from 咨询段落目录 where 页面序号=" & mKey & " and 段落序号=" & lng段号
            Call zlDatabase.OpenRecordset(rs, gstrSQL, Me.Caption, 3, 3)

            If rs.BOF = False Then
                rs!段落字体 = VisualTxt.FontName & ";" & VisualTxt.FontSize & ";" & VisualTxt.FontBold & ";" & VisualTxt.FontItalic & ";" & VisualTxt.ForeColor
                rs!段落文本 = VisualTxt.Text
...
Private Sub ShowPicture(ByVal PicNo As Long, ByVal Index As Long)
    ...
    If rs.BOF = False Then
        Call UsrPicture(Index).ShowPictureByField(rs!图形, rs!宽度 * Screen.TwipsPerPixelX, rs!高度 * Screen.TwipsPerPixelY, IIf(IsNull(rs!类型), 0, rs!类型))
