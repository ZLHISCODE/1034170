...
Private Function InitFreeRegist() As Boolean
   ...
   End If
    ...
    "      Where a.号码 = [1] And a.科室ID=E.Id And a.项目id = d.主项id And d.从项id = b.ID And b.ID = c.收费细目id And Nvl(a.停用日期, Sysdate + 1) > Sysdate"
   On Error GoTo hErr
...
Private Sub InitPicture()
    ...
    gstrSQL = "select 名称,图形,宽度,高度,类型 from 咨询图片元素 where 性质=7 order by 修改日期 desc,序号 desc "
    Set rsPic = zlDatabase.OpenSQLRecord(gstrSQL, Me.Caption)
    If rsPic.BOF = False Then
      On Error Resume Next
       Me.imgBackgroundImg.Picture = ReadPicByField(rsPic!图形) 'LoadPicture('zlDatabase.ReadPicture(rsPic, "图形"))
    End If
