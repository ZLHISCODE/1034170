...
Public glngkey_preWinProc As Long '用来保存窗口信息
Public glngkey_idHotKey As Long
Public gblnHotKeyPress As Boolean '是否按了热键
'管道获取CMD输出
...
Public Function keyWndproc(ByVal hwnd As Long, ByVal Msg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
      ...
      End If
'       将消息传送给指定的窗口
      keyWndproc = CallWindowProc(glngkey_preWinProc, hwnd, Msg, wParam, lParam)
End Function
