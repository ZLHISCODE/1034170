...
'返回：真-处理成功
    ...
    'HIS系统参数
    '---------------------------------------------------------
    strSQL = "Select 参数号,参数名,Nvl(参数值,缺省值) as 参数值 from Zlparameters Where 系统 = [1] And Nvl(私有, 0) = 0 And 模块 Is Null"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "mdlCISJob", glngSys)
    
    '就诊卡号密文显示
    rsTmp.Filter = "参数号=12"
    If Not rsTmp.EOF Then gblnCardHide = zlCommFun.NVL(rsTmp!参数值, 0) <> 0
    
'    '就诊卡号码的长度
'    gbytCardLen = 7
'    rsTmp.Filter = "参数号=20"
'    If Not rsTmp.EOF Then
'        gbytCardLen = CByte(Split(zlCommFun.NVL(rsTmp!参数值, "7|7|7|7|7"), "|")(4))
'    End If
    
    '挂号有效天数
    rsTmp.Filter = "参数号=21"
    If Not rsTmp.EOF Then gint挂号天数 = zlCommFun.NVL(rsTmp!参数值, 0)
