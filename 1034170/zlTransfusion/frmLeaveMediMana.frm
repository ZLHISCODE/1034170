...
'                    "From 药品信息 D,药品规格 C,(Select 现价, 收费细目id From 收费价目 Where 终止日期 Is Null Or 终止日期 = To_Date('3000-01-01', 'YYYY-MM-DD')) B," & vbNewLine & _
...
'                strSQL = "Select A.ID, A.编码, A.名称,A.规格 , A.计算单位, B.现价, A.费用类型, Decode(A.服务对象, 1, '门诊', '门诊和住院') As 服务对象," & vbNewLine & _
'                    "       A.执行科室,C.剂量系数,C.门诊单位,C.门诊包装,C.容量,D.剂量单位" & vbNewLine & _
'                    "From 收费项目别名 E,药品信息 D,药品规格 C,(Select 现价, 收费细目id From 收费价目 Where 终止日期 Is Null Or 终止日期 = To_Date('3000-01-01', 'YYYY-MM-DD')) B," & vbNewLine & _
'                    "     收费项目目录 A" & vbNewLine & _
...
'                        "            nvl(e.门诊单位,'') as 门诊单位 , nvl(e.门诊包装,0) 门诊包装, nvl(e.容量,0) as 容量, h.现价, (Nvl(i.发送数次, 0) / Nvl(e.剂量系数, 0)) As 可存数量, c.收费细目id," & vbNewLine & _
...
'                        "           a.病人id = " & pMediMaster.病人ID & " And a.执行科室id = " & pMediMaster.科室ID & " And F.入出系数(+)=1 And a.诊疗类别 = 'E' And a.挂号单 = '" & pMediMaster.挂号单 & "'" & vbNewLine & _
...
'                        "            nvl(e.门诊单位,'') as 门诊单位, nvl(e.门诊包装,0) as 门诊包装, nvl(e.容量,0) as 容量, h.现价, (Nvl(i.发送数次, 0) / Nvl(e.剂量系数, 0)) As 可存数量, c.收费细目id," & vbNewLine & _
'                        "            (Nvl(f.数量, 0) * Nvl(f.入出系数, 0)) As 已存数量, d.计算单位" & vbNewLine & _
'                        "From (Select 现价, 收费细目id From 收费价目 Where 终止日期 Is Null Or 终止日期 = To_Date('3000-01-01', 'YYYY-MM-DD')) h," & vbNewLine & _
'                        "        药品信息 g, 暂存药品记录 f, 药品规格 e, 收费项目目录 d, 病人医嘱记录 c, 诊疗项目目录 b, 病人医嘱发送 i," & vbNewLine & _
'                        "        病人医嘱记录 a" & vbNewLine & _
