...
Private Sub chkPrint_Click(Index As Integer)
    If Index = 1 Then
        '输液单
        If mintLabelState = 1 Then
            chkLabel.Value = chkPrint(Index).Value
        Else
            chkLabel.Value = 0
        End If
        '输液单未勾选，不允许勾选输液瓶签
        chkLabel.Enabled = chkPrint(Index).Value = 1
    End If
End Sub
    ...
    If Not mGps输液 Is Nothing Then
        If mGps输液.选择组数 > 0 Then
            ...
            Else
                mGps输液.p提醒 = -1
            End If
            mGps输液.SelectGroupThingNew mlng部门ID, chkPrint(1).Value = 1, 1, Me, chkView.Value = 1, chkLabel.Value = 1
...
hExit:
    ...
    Exit Sub
hErr:
    SaveLog "单击刷新，第" & CStr(Erl()) & "行，" & Err.Description
End Sub
...
hErr:
    mblnReadCard = False
    SaveLog "单据输入框，第" & CStr(Erl()) & "行，" & Err.Description
End Sub
...
Private Sub SaveLog(ByVal strInput As String)
    ...
    strFileName = App.Path & "\log" & Format(Date, "yyyyMMdd") & ".txt"
