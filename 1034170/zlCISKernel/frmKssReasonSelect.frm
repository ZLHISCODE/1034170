...
Private Sub Form_Load()
    ...
    End If
    On Error GoTo errH
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mstrFind, "%" & UCase(mstrFind) & "%", UserInfo.姓名)
...
Private Sub vsgMain_ValidateEdit(ByVal Row As Long, ByVal Col As Long, Cancel As Boolean)
    ...
    If Row = vsgMain.Rows - 1 And Col = col名称 Then
        ...
        If mintType = 1 Or mintType = 3 Then
            ...
            If mintType = 3 Then strSQL = strSQL & " And 性质=1 And 人员=[2]"
            On Error GoTo errH
            Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, vsgMain.EditText, UserInfo.姓名)
            '如果已经有了，提示用户是否继续。
