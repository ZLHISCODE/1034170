...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Case conMenu_FontSet_FontSize_S '小字体
        If mbytSize <> 0 Then
            mbytSize = 0
            Call zlControl.VSFSetFontSize(vsColumn, IIF(mbytSize = 0, 9, 12))
            Call zlControl.VSFSetFontSize(vsQuery, IIF(mbytSize = 0, 9, 12))
        End If
...
Private Function LoadQueryData() As Boolean
    ...
    If InStr(",0,1,4,", tbcQuery.Selected.Index) > 0 Then
        ...
        strSQLSend = "Select B.状态,C.NO,D.序号,C.库房ID,C.对方部门ID,D.姓名,D.标识号 as 住院号,D.床号," & _
            ...
            " C.单量,C.频次,C.用法,A.发送时间 as 时间,A.发送人 as 人员,C.付数" & _
        ...
        strSQLSend = "Select 状态,NO,序号,库房ID,对方部门ID,姓名,住院号,床号,药品ID,单价," & _
            " Sum(数量) as 数量,Sum(金额) as 金额,期效,单量,频次,用法,时间,人员,付数" & _
            " From (" & strSQLSend & ")" & _
            " Group by 状态,NO,序号,库房ID,对方部门ID,姓名,住院号,床号,药品ID,单价,期效,单量,频次,用法,时间,人员,付数"
    End If
    ...
    If InStr(",2,3,4,", tbcQuery.Selected.Index) > 0 Then '退药部份
        ...
        Else
            strCondDel = strCondDel & " And A.申请时间 Between To_Date('2007-01-01','YYYY-MM-DD') And Sysdate"
        End If
        ...
        strSQLDel = "Select Distinct -1 as 状态,D.NO,B.序号,D.库房ID,D.对方部门ID,B.姓名,B.标识号 as 住院号,B.床号," & _
            ...
            " D.单量,D.频次,D.用法,A.申请时间 as 时间,A.申请人 as 人员,D.付数" & _
    ...
    If tbcQuery.Selected.Index = 0 Or tbcQuery.Selected.Index = 2 Then '发药明细、退药明细
        ...
        strSQL = _
            " Select /*+ Rule*/ A.状态,A.NO,A.序号,I.名称 as 药房,H.名称 as 开嘱科室,A.姓名,A.住院号,A.床号," & _
            ...
            " From (" & strSQL & ") A,药品规格 E,收费项目目录 F,诊疗项目目录 G,部门表 H,部门表 I,收费项目别名 X" & _
            " Where A.药品ID=E.药品ID And A.药品ID=F.ID And E.药名ID=G.ID" & _
            " And A.对方部门ID=H.ID And A.库房ID=I.ID" & _
...
'参数：bytStyle=1-打印,2-预览,3-输出到Excel
    ...
    objRow.Add "病区：" & Get部门名称(mvQuery.病区ID) & "/药房：" & Get部门名称(mvQuery.药房ID)
