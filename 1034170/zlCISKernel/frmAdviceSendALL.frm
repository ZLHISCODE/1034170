...
'功能：初始化清单格式
    ...
    strHead = ",300,4;科室,850,1;姓名,750,1;住院号,750,1;床号,500,4;费别,750,1;婴儿,550,1;期效,550,1;医嘱内容,2000,1;规格,2000,1;总量,600,7;单位,450,1;" & _
        ...
        "紧急标志;医嘱状态;执行频率;新开操作时间;计算方式;执行安排;病人性质;开嘱时间;开始时间;执行分类;会诊医嘱ID;用药理由"
...
'       bln会诊 出参数，是否要读取会诊医嘱IDs
    ...
    If blnDo Then
        If gbln会诊科室下达医嘱处理 Then
            strTmp = " And (A.开嘱科室ID In (Select /*+cardinality(E,10)*/ e.Column_Value From Table(f_Num2list([4])) E) and nvl(a.会诊医嘱id,0)=0 or instr(','||[12]||',',','||nvl(a.会诊医嘱id,0)||',')>0)"
            bln会诊 = True
        Else
...
'功能：按病人读取医嘱发送清单
    ...
    If bln药品 Then
        ...
        strSQL药品 = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
            ...
            " A.可否分零,A.计价特性,A.执行性质,A.执行标记," & str药房置换 & " as 执行科室ID,Nvl(F.名称,Decode(Nvl(A.执行性质,0),5,'-')) as 执行科室,A.摘要,A.医嘱状态,A.医嘱期效,A.首次用量,E.计算方式,e.执行安排,a.用药理由,a.会诊医嘱ID" & _
    ...
    If bln其他 Then
        ...
        strSQL其他 = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
            ...
            " A.标本部位,A.检查方法,A.执行标记,A.计价特性,E.操作类型,E.执行分类,E.试管编码,E.执行频率," & IIF(mblnAutoVerify, "s.操作时间 as 新开操作时间,", "") & _
        ...
        If bln药品 Then
            ...
            If lngModle = 0 Then
                If gstr输液配置中心 <> "" Then
                    If bln可接收病区 Then
                        If mbln接收自备药 Then
                            str输液药品排除 = "and not (instr([11],a.医嘱期效)>0 and a.诊疗类别 in ('5','6')" & IIF(mstr配液给药途径 = "", "", " and  instr(','||[10]|| ',',','||x.诊疗项目id||',')>0") & vbNewLine & _
                                "and Exists (select 1 from 诊疗项目目录 y where y.id=x.诊疗项目id and y.操作类型='2' and y.执行分类=1)" & vbNewLine & _
                                "and exists (select 1 from 病人医嘱记录 z where z.相关id=a.相关id and  instr(','||[9]||',0,',','||nvl(z.执行科室id,0)||',')>0)" & vbNewLine & _
                                "or" & vbNewLine & _
                                " instr([11],a.医嘱期效)>0 and a.诊疗类别='E' and e.操作类型='2' and e.执行分类=1" & IIF(mstr配液给药途径 = "", "", " and  instr(','||[10]|| ',',','||a.诊疗项目id||',')>0") & vbNewLine & _
                                "and exists (select 1 from 病人医嘱记录 z where z.相关id=a.id and instr(','||[9]||',0,',','||nvl(z.执行科室id,0)||',')>0)" & vbNewLine & _
                                ")"
                        Else
                            str输液药品排除 = "and not (instr([11],a.医嘱期效)>0 and a.诊疗类别 in ('5','6')" & IIF(mstr配液给药途径 = "", "", " and  instr(','||[10]|| ',',','||x.诊疗项目id||',')>0") & vbNewLine & _
                                "and Exists (select 1 from 诊疗项目目录 y where y.id=x.诊疗项目id and y.操作类型='2' and y.执行分类=1)" & vbNewLine & _
                                "and exists (select 1 from 病人医嘱记录 z where z.相关id=a.相关id and  instr(','||[9]||',0,',','||nvl(z.执行科室id,0)||',')>0)" & vbNewLine & _
                                "and not exists (select 1 from 病人医嘱记录 z where z.id=a.相关id and  nvl(z.执行性质,0)=5 and nvl(z.执行标记,0)=0)" & vbNewLine & _
                                "and not exists (select 1 from 病人医嘱记录 z where z.相关id=a.相关id and  nvl(z.执行性质,0)=5 and nvl(z.执行标记,0) in (0,2))" & vbNewLine & _
                                "or" & vbNewLine & _
                                "instr([11],a.医嘱期效)>0 and a.诊疗类别='E' and e.操作类型='2' and e.执行分类=1" & IIF(mstr配液给药途径 = "", "", " and  instr(','||[10]|| ',',','||a.诊疗项目id||',')>0") & vbNewLine & _
                                "and not (nvl(a.执行性质,0)=5 and nvl(a.执行标记,0)=0)" & vbNewLine & _
                                "and exists (select 1 from 病人医嘱记录 z where z.相关id=a.id and instr(','||[9]||',0,',','||nvl(z.执行科室id,0)||',')>0)" & vbNewLine & _
                                "and not exists (select 1 from 病人医嘱记录 z where z.相关id=a.id and  nvl(z.执行性质,0)=5 and nvl(z.执行标记,0) in (0,2))" & vbNewLine & _
                                ")"
                        End If
            ...
            Set rsSend药品 = zlDatabase.OpenSQLRecord(strSQL药品, Me.Caption, datEnd, Val(arrPati(k)), lng药房ID, strDepts, mlng病区ID, Format(dkpExecTime.value, "YYYY-MM-DD HH:mm"), Format(dkpExecTime.value, "HH:mm"), _
                "," & strNoneIDs & ",", gstr输液配置中心, mstr配液给药途径, IIF(mint输液配置期效 = -1, "01", mint输液配置期效), str会诊医嘱IDs)
        End If
        If bln其他 Then
...
'rsSQL!类型=1-校对(如果不需要先校对),2-医嘱计价,3-住院记帐,4-执行科室替换，5-医嘱发送，6-自动发料,7-输液配药
    ...
    With vsAdvice
        ...
        strTmp = ""
        blnExists = False
        strMinDate = "3000-01-01 00:00": strMaxDate = "1888-01-01 00:00"
        ...
        For i = .FixedRows To .Rows - 1
            If .Cell(flexcpData, i, COL_选择) = 0 And Not .Cell(flexcpPicture, i, COL_选择) Is Nothing Then
                ...
                If .TextMatrix(i, COL_末次时间) > strMaxDate Then
                    strMaxDate = .TextMatrix(i, COL_末次时间)
                End If
                            ...
                            If Not mrsPrice.EOF Then
                                ...
                                For j = 1 To mrsPrice.RecordCount
                                    ...
                                    If Nvl(mrsPrice!收费细目ID, 0) <> 0 And Nvl(mrsPrice!数量, 0) <> 0 Then '对照数量为0的自动过滤掉
                                        If InStr("," & strCuvetteNumber & ",", "," & Nvl(mrsPrice!收费细目ID, 0) & ",") = 0 Then
                                            strCuvetteNumber = strCuvetteNumber & "," & Nvl(mrsPrice!收费细目ID, 0)
                                            blnBool = True
                                        End If
                                    End If
                                    If blnBool Then
                    ...
                    If blnSend Then
                        ...
                        If strSQL <> "" Then
                            ...
                            If Not rsPrice.EOF Then
                                int计费状态 = 1 '已计费
                                Set rsClone = zlDatabase.CopyNewRec(rsPrice)
                            End If
...
MoneyItemBegin:
                                ...
                                If rsPrice!费用性质 & "_" & rsPrice!ID <> str收费项目 Then
                                    If Not AdviceMoneyMake(lng病人ID, lng主页ID, rsMoneyNow, rsMoneyDay, _
                                        ...
                                        .TextMatrix(i, COL_频率), Val(.TextMatrix(i, COL_单量)), .Cell(flexcpData, i, COL_医嘱期效), rsPrice!费用性质, .TextMatrix(i, COL_诊疗类别), strCuvetteNumber, strMinDate & ";" & strMaxDate) Then
                                        '跳过当前收费项目(多个收入项目)
