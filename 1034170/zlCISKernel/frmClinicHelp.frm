...
Private mblnShowInTaskBar As Boolean
Private mstrItems As String
Private mblnMult As Boolean
Private mrsMult As ADODB.Recordset

Public Sub ShowMe(ByVal bytModal As Byte, ByVal frmParent As Object, Optional ByVal lngItemID As Long, Optional ByVal blnShowInTaskBar As Boolean, Optional ByVal strItems As String)
    '---------------------------------------------
    '功能：根据上级程序要求，以模态或非模态显示诊疗参考
    '入参：frmParent-父窗体；
    '      blnModal-是否模态显示（通常和上级窗体一致）；
    '      lngItemId-要显示的诊疗项目ID，不为0时，缺省不显示目录区；
    '      blnShowInTaskBar 设置窗体是否在任务条上显示
    '      strItems 传入多个诊疗项目ID拼串，逗号分割
    '               当传入多个诊疗项目ID拼串时，左侧以列表方式展示，并缺省当为lngItemId对应的参考项
    '---------------------------------------------
    mlngItemID = lngItemID
    mblnShowInTaskBar = blnShowInTaskBar
    mstrItems = strItems
...
Private Sub Form_Load()
    ...
    If mstrItems <> "" Then
        If InStr(mstrItems, ",") = 0 Then
            mlngItemID = Val(mstrItems)
            mstrItems = ""
        End If
    End If
    
    If mstrItems <> "" Then
        mlngItemID = GetList(mstrItems, mblnMult) '该方法会对 mrsMult mblnMult 赋值
    End If
    ...
    Else
        ...
        err = 0: On Error GoTo Errhand
        Set rsTemp = zlDatabase.OpenSQLRecord(mstrSQL, Me.Caption, mlngItemID)
        mlngItemID = rsTemp.Fields(0).value
    ...
    If mblnMult Then
        ...
        With mrsMult
            ...
            Loop
        End With
        If Not blnSel Then Me.lvwFind.ListItems(1).Selected = True
        Me.lvwFind.SelectedItem.EnsureVisible
        Me.cmdShow(0).Enabled = True
        Call cmdShow_Click(0)
        Me.stbCatalog.Tab = 1
    End If
    ...
    Call zlcommfun.SetWindowsInTaskBar(Me.hwnd, mblnShowInTaskBar)
    Exit Sub
Errhand:
    If ErrCenter() = 1 Then Resume
    Call SaveErrLog
...
Private Function GetList(ByVal strIDs As String, ByRef blnMult As Boolean) As Long
'功能：外部调用时一次传入多个诊疗项目ID时根据传入的判断是否要显示多条
'参数：strIDs 诊疗项目ID拼串逗号分割，暂时不考虑id串超过四千的情况
'      blnMult 出参，表示当前的数据是否有多行
'说明：如果只查询到一条则按以前显示一个的方式，如果有多条参考则显示左侧的列表中
    ...
    If Len(strAll) > 4000 Then
        ...
        Do While True
            If Len(strAll) < 4000 Then
                p = Len(strAll) + 1
            Else
                p = InStrRev(Mid(strAll, 1, 4000), ",")
            End If
            strThis = Mid(strAll, 1, p - 1)
            
            If n > 10 Then
                strTab = strTab & vbNewLine & " Union All " & Replace(strTemp, "[1]", "'" & strThis & "'")
            Else
                varPar(n) = strThis
                strTab = IIF(strTab = "", "", strTab & vbNewLine & " Union All ") & Replace(strTemp, "[1]", "[" & (n + 1) & "]")
            End If
            
            n = n + 1
            strAll = Mid(strAll, p + 1)
            If strAll = "" Then Exit Do
        Loop
        strTmp = " and a.ID In (" & strTab & ")"
        
        strSQL = "Select Distinct i.分类id, i.Id, i.编码, i.名称,a.id as 诊疗项目id" & vbNewLine & _
            " From 诊疗参考目录 I, 诊疗参考别名 N, 诊疗项目目录 A" & vbNewLine & _
            " Where i.Id = n.参考目录id And i.Id = a.参考目录id " & strTmp & _
            " order by i.编码"
        Set mrsMult = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, varPar(0), varPar(1), varPar(2), varPar(3), varPar(4), varPar(5), varPar(6), varPar(7), varPar(8), varPar(9), varPar(10))
        
    Else
        strSQL = "Select Distinct i.分类id, i.Id, i.编码, i.名称,a.id as 诊疗项目id" & vbNewLine & _
            " From 诊疗参考目录 I, 诊疗参考别名 N, 诊疗项目目录 A" & vbNewLine & _
            " Where i.Id = n.参考目录id And i.Id = a.参考目录id " & _
            " And a.Id IN (Select /*+cardinality(x,10)*/ x.Column_Value From Table(Cast(f_Num2list([1]) As zlTools.t_Numlist)) X)" & _
            " order by i.编码"
        Set mrsMult = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, strAll)
    End If
        
    If mrsMult.RecordCount = 1 Then
        GetList = Val(mrsMult!ID & "")
    ElseIf mrsMult.RecordCount > 1 Then
        blnMult = True
    End If
