...
'功能：读取指定范围内的病人列表
    ...
    End If
    With vsPati
        Set rsTmp = GetPatiRsByUnit(lngUnitID, mlng病人ID, False, False, True, True, lng医嘱处理范围)
        vsPati.Rows = rsTmp.RecordCount + 1
        blnIsWowen = DeptIsWoman(0, Get科室IDs(lngUnitID))
        For i = 1 To rsTmp.RecordCount
            If Not (blnIsWowen And chkOneByOne.value = 1 And rsTmp!婴儿病区ID <> "" And (rsTmp!婴儿病区ID & "") = mlng病区ID And rsTmp!婴儿姓名 & "" = "") And Not (mblnOnePati And rsTmp!病人ID <> mlng病人ID) Then
...
Private Sub chkOneByOne_Click()
    If (optBaby(2).value Or optBaby(0).value) And fraBaby.Visible = True Then
        Call cboUnit_Click
    End If
End Sub
...
'参数：LoadWaitPrint-true过滤待打印的，否则显示全部
    ...
    If blnIsLoadWart Then
        ...
        End If
        strSql = "Select distinct a.病人id, a.主页id," & IIF(optBaby(1).value = False And fraBaby.Visible And chkOneByOne.value = 1, "NVL(A.婴儿,0)", "0") & " as 婴儿" & vbNewLine & _
...
'      bytType=0-打印所有医嘱，1-只打印停止到上次打印时间之前的医嘱,2-续打单个病人的执行单
    ...
    Dim str期效 As String, str病人 As String, blnParSet As Boolean
    ...
    If str病人IDs = "" Then
        ...
        vsPati.SetFocus: Exit Sub
    End If
    blnParSet = InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0
    '逐个打印婴儿医嘱时不保存参数
    If bytType = 0 Then
        If UBound(Split(str病人IDs, ",")) = 0 And Val(str病人IDs) = mlng病人ID Then
            Call zlDatabase.SetPara("发送病人", "", glngSys, p住院医嘱发送, blnParSet)
        Else
            Call zlDatabase.SetPara("发送病人", cboUnit.ItemData(cboUnit.ListIndex) & ":" & str病人IDs, glngSys, p住院医嘱发送, blnParSet)
        End If
        Call zlDatabase.SetPara("执行单打印报表", strReports, glngSys, p住院医嘱发送, blnParSet)
    End If
    '保存设置
    curDate = zlDatabase.Currentdate
    If bytType = 0 Then
        Call zlDatabase.SetPara("常用报表期效", chk期效(0).value & chk期效(1).value, glngSys, p住院医嘱发送, blnParSet)
        Call zlDatabase.SetPara("常用报表开始时点", Format(dtpBegin.value, "HH:mm:ss"), glngSys, p住院医嘱发送, blnParSet)
        Call zlDatabase.SetPara("常用报表开始间隔", Int(CDate(Format(dtpBegin.value, "yyyy-MM-dd")) - CDate(Format(curDate, "yyyy-MM-dd"))), glngSys, p住院医嘱发送, blnParSet)
        Call zlDatabase.SetPara("常用报表结束时点", Format(dtpEnd.value, "HH:mm:ss"), glngSys, p住院医嘱发送, blnParSet)
        Call zlDatabase.SetPara("常用报表结束间隔", Int(CDate(Format(dtpEnd.value, "yyyy-MM-dd")) - CDate(Format(curDate, "yyyy-MM-dd"))), glngSys, p住院医嘱发送, blnParSet)
    End If
    If mblnOnePati = False Then Call zlDatabase.SetPara("常用报表逐个打印", chkOneByOne.value, glngSys, p住院医嘱发送, blnParSet)
    '只显示待打印的病人
    ...
    If chkOneByOne.value = 0 Then
        ...
        If strReports = "" Or bytMode = 1 Then ' 只预览当前行
            Call mobjReport.ReportOpen(gcnOracle, glngSys, Mid(lvwReport.SelectedItem.Key, 2), Me, _
        ...
        For z = 1 To IIF(strReports = "" Or bytMode = 1 Or bytType = 2, 1, lvwReport.ListItems.Count)
            If lvwReport.ListItems(z).Checked Or (strReports = "" Or bytMode = 1 Or bytType = 2) Then
                For i = 0 To UBound(arrPati)
                    ...
                    If (optBaby(2).value Or optBaby(0).value) And chkOneByOne.value = 1 And j <> -1 Then
                        '婴儿列表
...
Private Sub Form_Load()
    Dim curDate As Date
    Dim strTmp As String, lngTmp As Long, blnParSet As Boolean
    ...
    '缺省医嘱期效
    strTmp = zlDatabase.GetPara("常用报表期效", glngSys, p住院医嘱发送, "11", Array(chk期效(0), chk期效(1)), blnParSet)
    chk期效(0).value = Val(Left(strTmp, 1))
    chk期效(1).value = Val(Right(strTmp, 1))

    
    strTmp = zlDatabase.GetPara("常用报表开始时点", glngSys, p住院医嘱发送, "00:00:00", Array(dtpBegin), blnParSet)
    lngTmp = Val(zlDatabase.GetPara("常用报表开始间隔", glngSys, p住院医嘱发送, "0", Array(dtpBegin), blnParSet))
    dtpBegin.value = Format(curDate + lngTmp, "yyyy-MM-dd " & strTmp)
    dtpViewBegin.value = dtpBegin.value
    
    strTmp = zlDatabase.GetPara("常用报表结束时点", glngSys, p住院医嘱发送, "23:59:59", Array(dtpEnd), blnParSet)
    lngTmp = Val(zlDatabase.GetPara("常用报表结束间隔", glngSys, p住院医嘱发送, "0", Array(dtpEnd), blnParSet))
    dtpEnd.value = Format(curDate + lngTmp, "yyyy-MM-dd " & strTmp)
    dtpViewEnd.value = dtpEnd.value
    
    If mblnOnePati = False Then chkOneByOne.value = Val(zlDatabase.GetPara("常用报表逐个打印", glngSys, p住院医嘱发送, "0", Array(chkOneByOne), blnParSet))
    mlng医嘱处理范围 = Val(zlDatabase.GetPara("医嘱处理范围", glngSys, p住院医嘱发送, "0"))
    If mblnOnePati Then
        cboUnit.Enabled = False
        chkOneByOne.value = 1
        chkOneByOne.Visible = False
    ...
    If Val(zlDatabase.GetPara("显示已打印信息", glngSys, p住院医嘱发送, "0", , blnParSet)) = 0 Then
        Call cmdView_Click
    End If
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    mlng病人ID = 0
    mlng病区ID = 0
    Call zlDatabase.SetPara("显示已打印信息", IIF(PicView.Visible, 1, 0), glngSys, p住院医嘱发送, InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0)
        ...
        For i = 0 To UBound(varPar)
            If varPar(i) <> "" Then
                ...
                arrSQL(UBound(arrSQL)) = "Zl_医嘱执行单_打印('" & varPar(i) & "'," & lngReportID & "," & _
                    ...
                    "To_Date('" & Format(IIF(mintType = 1, dtpViewEnd.value, dtpEnd.value), "yyyy-MM-dd HH:mm:ss") & "','YYYY-MM-DD HH24:MI:SS')," & IIF(chkOneByOne.value = 1, mlngLastRow, 0) & ")"
            End If
        Next
