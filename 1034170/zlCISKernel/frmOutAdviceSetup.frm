...
Private Sub cmdOK_Click()
    ...
    '诊间支付
    Call zlDatabase.SetPara("门诊医嘱发送后启用诊间支付", chkSendPay.value, glngSys, p门诊医嘱下达, blnSetup)
    Call zlDatabase.SetPara("输血申请注意事项", cmdBloodTip.Tag, glngSys, p门诊医嘱下达, blnSetup)
    Call zlDatabase.SetPara("检验医嘱单独产生单据", chk检验单据.value, glngSys, p门诊医嘱下达, blnSetup)
    gblnOK = True
    Unload Me
End Sub
...
Private Sub Form_Load()
    ...
    chk检验单据.value = Val(zlDatabase.GetPara("检验医嘱单独产生单据", glngSys, p门诊医嘱下达, , Array(chk检验单据), blnSetup))
    
    '指引单打印
    optPoint(Val(zlDatabase.GetPara("指引单打印方式", glngSys, p门诊医嘱下达, , Array(optPoint(0), optPoint(1), optPoint(2)), blnSetup))).value = True
    
    '诊间支付
    chkSendPay.value = Val(zlDatabase.GetPara("门诊医嘱发送后启用诊间支付", glngSys, p门诊医嘱下达, , Array(chkSendPay), blnSetup))
    '诊间支付才需要设置发药窗口
...
Private Sub optSendNO_Click(Index As Integer)
    ...
    chk检验单据.Enabled = optSendNO(1).value
End Sub
