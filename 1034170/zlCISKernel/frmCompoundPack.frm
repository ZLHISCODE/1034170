...
Public Sub ShowMe(ByVal intType As Integer, ByRef frmParent As Object, ByVal lng病区ID As Long, ByVal lng病人ID As Long, Optional ByVal lng医护科室ID As Long, _
    ...
    Me.lblInfo(0).Caption = "当前病区：" & Get部门名称(IIF(mlng婴儿病区ID <> 0 And mlng婴儿病区ID = mlng医护科室ID, lng婴儿病区ID, lng病区ID))
...
'参数：blnIsUnpack=true加载已打包医嘱,false为加载待打包医嘱
    ...
    If strIDs <> "" Then
        strSql = "Select 配药ID,操作类型,操作人员,操作时间 from 输液配药状态 Where 配药ID in(select Column_Value From Table(Cast(f_num2list([1]) As ZLTOOLS.t_numlist)))"
