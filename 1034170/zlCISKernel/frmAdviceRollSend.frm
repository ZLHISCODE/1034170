...
Private Sub chkOnlyPati_Click()
    
    Call zldatabase.SetPara("只显示当前病区的医嘱", chkOnlyPati.value & "", glngSys, p住院医嘱发送, InStr(GetInsidePrivs(p住院医嘱发送), ";医嘱选项设置;") > 0)
    '单病人模式
    If mblnOnePati Then
...
Private Sub Form_Load()
    ...
    chkOnlyPati.value = Val(zldatabase.GetPara("只显示当前病区的医嘱", glngSys, p住院医嘱发送, "0", Array(chkOnlyPati), InStr(GetInsidePrivs(p住院医嘱发送), "医嘱选项设置") > 0))
...
'功能：初始化清单格式
    ...
    strHead = ",300,4;科室,850,1;姓名,750,1;住院号,750,1;床号,500,4;婴儿,550,1;" & _
        ...
        "次数;计算量;单量;剂量系数;住院包装;可否分零;执行性质;上次;操作类型;执行分类"
...
'参数：str病人IDs=包含病人ID的字符串
    ...
    For k = 0 To UBound(Split(str病人IDs, ","))
        strSql = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
            ...
            " A.可否分零,Decode(Instr(',5,6,',A.诊疗类别),0,NULL,G.名称) as 给药途径,A.首次用量,e.操作类型,e.执行分类" & _
            ...
            IIF(chkOnlyPati.value = 1, " And instr(',' || [3] || ',',',' || Decode(NVL(A.婴儿,0),0,a.病人科室ID,NVL(b.婴儿科室ID,a.病人科室ID)) || ',')>0 ", "") & _
                ...
                For i = 1 To rsAdvice.RecordCount
                    ...
                    ElseIf InStr(",5,6,", rsAdvice!诊疗类别) > 0 Then
                        ...
                        Else
                            ...
                            lng药品ID = GetLastSendMediCineID(Val(rsAdvice!ID), CDate(rsAdvice!上次执行时间))
                        End If
        ...
        For i = .FixedRows To .Rows - 1
            If Val(.TextMatrix(i, COL_选择)) <> 0 And Val(.TextMatrix(i, COL_执行性质)) <> 0 Then '排开叮嘱
                If mbln超期负数 Then
                    ...
                    If CheckAllPrice(Val(.TextMatrix(i, COL_ID)), dbl总量) Then
                        strNO = "调整划价单"
                    Else
...
'功能：医嘱中是否存在已经被配液的
    ...
    strSql = "Select 1 from 病人医嘱记录 A,输液配药记录 B Where a.Id=b.医嘱id And b.操作状态 In (4,5,6,7,8) And b.执行时间>a.执行终止时间" & _
...
Private Function CheckAllPrice(ByVal lng医嘱ID As Long, ByVal dbl收回总量 As Double) As Boolean
'功能：检查收回次数的医嘱对应费用是否全是未审核的划价单，以便确定直接修改划价单，无需取新的单据号
    ...
    strSql = "Select Sum(a.发送数次) 发送数次" & vbNewLine & _
            ...
            " (Select 1 From 住院费用记录 B Where a.医嘱id = b.医嘱序号 And a.No = b.No And b.记录性质 = 2 And 记录状态 <> 0)"
    On Error GoTo errH
