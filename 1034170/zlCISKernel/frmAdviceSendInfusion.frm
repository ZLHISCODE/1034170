...
'功能：初始化清单格式
    ...
    strHead = ",300,4;科室,850,1;姓名,750,1;住院号,750,1;床号,500,4;费别,750,1;" & _
        ...
        "药品ID;剂量系数;住院包装;住院单位;可否分零;药房分批;是否变价;库存;次数;分解时间;操作类型;试管编码;标本部位;检查方法;跟踪在用;紧急标志;医嘱状态;执行频率;新开操作时间;开始时间;会诊医嘱ID"
...
'       bln会诊 出参数，是否要读取会诊医嘱IDs
    ...
    If blnDo Then
        If gbln会诊科室下达医嘱处理 Then
            strTmp = " And (A.开嘱科室ID In (Select /*+cardinality(E,10)*/ e.Column_Value From Table(f_Num2list([4])) E) and nvl(a.会诊医嘱id,0)=0 or instr(','||[7]||',',','||nvl(a.会诊医嘱id,0)||',')>0)"
            bln会诊 = True
        Else
...
'功能：按病人读取医嘱发送清单
    ...
    strSQL药品 = "Select A.ID,A.相关ID,Nvl(A.相关ID,A.ID) as 组ID,Nvl(X.序号,A.序号) as 组号," & _
             ...
             " A.可否分零,A.计价特性,A.执行性质,A.执行标记," & str药房置换 & " as 执行科室ID,Nvl(F.名称,Decode(Nvl(A.执行性质,0),5,'-')) as 执行科室,A.摘要,A.医嘱状态,A.医嘱期效,A.首次用量,g.执行标记 as 给药执行标记,A.会诊医嘱ID" & _
    ...
    For k = 0 To UBound(arrPati)
        ...
        Set rsSend药品 = zlDatabase.OpenSQLRecord(strSQL药品, Me.Caption, datEnd, Val(arrPati(k)), gstr输液配置中心, strDepts, mlng病区ID, "," & strNoneIDs & ",", str会诊医嘱IDs)
...
'rsSQL!类型=1-校对(如果不需要先校对),2-医嘱计价,3-住院记帐,4-执行科室替换，5-医嘱发送，6-自动发料,7-输液配药
    ...
    Dim strHaveSub As String, strNoneSub As String, strCuvetteNumber As String
                            ...
                            If Not mrsPrice.EOF Then
                                ...
                                For j = 1 To mrsPrice.RecordCount
                                    ...
                                    If Nvl(mrsPrice!收费细目ID, 0) <> 0 And Nvl(mrsPrice!数量, 0) <> 0 Then '对照数量为0的自动过滤掉
                                        If InStr("," & strCuvetteNumber & ",", "," & Nvl(mrsPrice!收费细目ID, 0) & ",") = 0 Then '重复的收费细目ID过滤掉
...
MoneyItemBegin:
                                ...
                                If rsPrice!费用性质 & "_" & rsPrice!ID <> str收费项目 Then
                                    If Not AdviceMoneyMake(lng病人ID, lng主页ID, rsMoneyNow, rsMoneyDay, _
                                        ...
                                        Val(.TextMatrix(i, COL_ID)), lng发送号, Val(rsPrice!数量 & ""), rsExec, , , , , , .TextMatrix(i, COL_诊疗类别), , strMinDate & ";" & strMaxDate) Then
                                        '跳过当前收费项目(多个收入项目)
