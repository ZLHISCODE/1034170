...
'功能：根据输入内容或按*（或按选择按钮）弹出并返回选择的记录集
    ...
    'mint场合 3-成套方案(路径项目设置)调用无法确定开单科室ID
    If lng形态 = 0 Then
        str存储库房 = " And Exists(select 1 from 收费执行科室 f Where f.收费细目id=d.id " & IIF(mint场合 = 3, "", "And (f.开单科室id Is Null Or f.开单科室id=[8])") & " And f.执行科室id=" & lng药房ID & ")"
    Else
        str存储库房 = " And Exists(select 1 from 诊疗执行科室 f Where f.诊疗项目id=a.id " & IIF(mint场合 = 3, "", "And (f.开单科室id Is Null Or f.开单科室id=[8])") & " And f.执行科室id=" & lng药房ID & ")"
    End If
    ...
    If mint场合 <> 3 Then
        ...
        If InStr(strTsPrivs, "下达精神药嘱") = 0 Then
            str特性 = str特性 & " And E.毒理分类 Not IN('精神I类','精神II类')"
        End If
