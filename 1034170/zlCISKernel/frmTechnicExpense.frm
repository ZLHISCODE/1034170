...
Private Function CheckValid() As Boolean
    ...
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, Me.Caption, mstrOriginalNO, mlng发送号)
...
Private Sub Find开单科室(ByVal lng科室id As Long)
    ...
    '没找到,自动增加
    strSQL = "Select ID,编码,名称,简码 From 部门表 Where ID=[1]"
    Set rsTmp = zlDatabase.OpenSQLRecord(strSQL, "开单科室", lng科室id)
    If rsTmp.RecordCount = 0 Then Exit Sub
    If Not rsTmp.EOF Then
        cbo开单科室.RemoveItem cbo开单科室.ListCount - 1
        cbo开单科室.AddItem IIF(zlIsShowDeptCode, rsTmp!编码 & "-", "") & rsTmp!名称
