...
Public Function AdviceMoneyMake(ByVal lng病人ID As Long, ByVal lng主页ID As Long, rsMoneyNow As Recordset, rsMoneyDay As ADODB.Recordset, _
    ...
    Optional ByVal int费用性质 As Integer, Optional ByVal str诊疗类别 As String, Optional ByVal str样本条码 As String, Optional ByVal strTimRange As String) As Boolean
'功能：判断指定的医嘱费用是否应该产生
...
'      lng当前医嘱ID=当前行医嘱id
'      str样本条码=检验医嘱传入样本条码
'      strTimRange 用于查询已发关的医嘱中收费情况的时间范围，格式：2019-04-29 10:00;2019-04-30 10:00
'以下是计量计时医嘱的数量组织规则
...
'      rsExec=医嘱执行计价的内容
    ...
    Dim str最小日期 As String, strMaxDate As String
            ...
            For i = 1 To rsDays.RecordCount
                If i = 1 Then
                    If rsMoneyDay Is Nothing Then
                        str最小日期 = Split(strTimRange, ";")(0)
                        strMaxDate = Split(strTimRange, ";")(1)
                        If str最小日期 = "" Then
                            str最小日期 = Format(Split(str分解时间, ";")(0), "yyyy-MM-dd")
                        End If
                        If strMaxDate = "1888-01-01 00:00" or strMaxDate = "" Then
                            strMaxDate = "3000-01-01"
                        End If
                        
                        Call GetPatiDayMoneyDetail(rsMoneyDay, lng病人ID, lng主页ID, byt来源, CDate(str最小日期), strMaxDate)
                    End If
                End If
...
Private Function GetPatiDayMoneyDetail(rsMoneyDay As ADODB.Recordset, ByVal lng病人ID As Long, ByVal lng主页ID As Long, ByVal byt来源 As Byte, ByVal dat收费时间 As Date, ByVal strMaxDate As String) As Boolean
'功能：获取指定病人某天(dat收费时间)之后医嘱产生的费用项目明细
'参数：lng主页ID=住院病人才使用
'      byt来源:1-门诊(含住院临嘱发送到门诊)，2-住院
'      dat收费时间 需要判断的最早的时间
'      strMaxDate 最大日期,格式：2014-11-01
        ...
        If Val(rsTmp!医嘱期效 & "") = 1 Then
            If Val(rsTmp!频率间隔 & "") = 1 And rsTmp!间隔单位 & "" = "天" Or rsTmp!间隔单位 & "" = "小时" Or rsTmp!间隔单位 & "" = "分钟" Then
                For j = 1 To rsTmp!天数
                    ...
                    End If
                    If strDay > strMaxDate Then Exit For
                Next
            Else
                '对于临嘱这里可能会漏算
                strDay = Format(rsTmp!首次时间, "yyyy-MM-dd")
...
'返回：str分类=当前天数最后一个路径项目所属的分类
    ...
    strSQL = "Select a.路径记录id, a.当前阶段id, a.当前天数, a.开始日期, b.日期, b.分类" & vbNewLine & _
            "From (Select a.Id As 路径记录id, a.当前阶段id, a.当前天数, Max(b.Id) 执行id,Min(b.日期) As 开始日期" & vbNewLine & _
            "       From 病人临床路径 A, 病人路径执行 B" & vbNewLine & _
