...
Begin VB.Form frmBloodApplyNew 
   ...
   Begin VB.PictureBox picLisInfo 
      Appearance      =   0  'Flat
      BackColor       =   &H80000005&
      BorderStyle     =   0  'None
      ForeColor       =   &H80000008&
   ...
   Begin VB.CommandButton cmd常用嘱托 
      Height          =   300
      Left            =   5355
      Picture         =   "frmBloodApplyNew.frx":D0A4
      Style           =   1  'Graphical
      TabIndex        =   95
      TabStop         =   0   'False
   ...
   Begin VB.CommandButton cmdDate 
      ...
      Index           =   1
      Left            =   10215
      Picture         =   "frmBloodApplyNew.frx":D62E
      Style           =   1  'Graphical
      TabIndex        =   81
      TabStop         =   0   'False
      ToolTipText     =   "编辑(F4)"
   ...
   Begin VB.CommandButton cmdDate 
      ...
      Index           =   0
      Left            =   4200
      Picture         =   "frmBloodApplyNew.frx":D724
      Style           =   1  'Graphical
      TabIndex        =   80
      TabStop         =   0   'False
      ToolTipText     =   "编辑(F4)"
   ...
   Begin VB.PictureBox picGet 
      ...
      Begin VB.CommandButton cmdGet 
         ...
         Index           =   1
         Left            =   2970
         Picture         =   "frmBloodApplyNew.frx":D81A
         Style           =   1  'Graphical
         TabIndex        =   72
         TabStop         =   0   'False
         ToolTipText     =   "编辑(F4)"
   ...
   Begin VB.PictureBox picInfo 
      ...
      Begin VB.ComboBox cboInfo 
         ...
         Height          =   360
         Index           =   1
         ItemData        =   "frmBloodApplyNew.frx":D910
         Left            =   -25
         List            =   "frmBloodApplyNew.frx":D912
         Style           =   2  'Dropdown List
         TabIndex        =   27
         Top             =   -30
         Width           =   1335
   ...
   Begin VSFlex8Ctl.VSFlexGrid vsLIS 
      ...
      FormatString    =   $"frmBloodApplyNew.frx":D914
      ScrollTrack     =   -1  'True
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   2
   ...
   Begin MSComctlLib.ImageList img16 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":DB67
            Key             =   "c0"
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":E101
            Key             =   "c1"
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":E69B
            Key             =   "o0"
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "frmBloodApplyNew.frx":EC35
            Key             =   "o1"
         EndProperty
      EndProperty
   End
   Begin VB.PictureBox picPreBlood 
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfBlood 
         ...
         FormatString    =   $"frmBloodApplyNew.frx":F1CF
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
      ...
      Begin VSFlex8Ctl.VSFlexGrid vsfList 
         ...
         ColWidthMax     =   0
         ExtendLastCol   =   0   'False
         FormatString    =   $"frmBloodApplyNew.frx":F344
         ScrollTrack     =   -1  'True
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
...
'功能：检查数据正确性
    ...
    If mintType <> 3 Then
        ...
        '检查所有LIS结果是否都已做
        If imgLisInfo.Tag <> "" And picLisInfo.Visible = True Then
            If MsgBox(imgLisInfo.Tag & vbCrLf & "请问您是否要继续？", vbQuestion + vbYesNo + vbDefaultButton2, Me.Caption) = vbNo Then
                Exit Function
            End If
        End If
        If mint调用场合 = 0 Then
...
'功能：保存数据
    ...
    Else
        ...
        arrSQL(UBound(arrSQL)) = "ZL_病人医嘱记录_Insert(" & lng医嘱ID & ",NULL," & lng医嘱序号 & "," & int病人来源 & "," & mlng病人ID & "," & strTmp主页ID & "," & mbytBaby & ",1,1,'K'," & mlng输血项目ID & _
                                 ...
                                 ",NULL," & mlng病人科室id & "," & mlng开单科室ID & ",'" & UserInfo.姓名 & "'," & _
                                 "to_date('" & Format(IIF(curDate > CDate(txtInfo(txt申请日期).Text), txtInfo(txt申请日期).Text, curDate), "yyyy-MM-dd HH:mm:ss") & "','yyyy-MM-dd HH24:MI:SS')," & _
                                 strPar & "," & IIF(int检查方法 = 0, "NULL", "'" & int检查方法 & "'") & ",0,NULL," & IIF(mstr摘要输血 = "", "null", "'" & mstr摘要输血 & "'") & ",'" & UserInfo.姓名 & "',Null,Null,'" & cboInfo(cbo输血目的).Text & "'," & str审核状态 & "," & lng申请序号 & ")"
...
Private Sub cbsMain_Execute(ByVal Control As XtremeCommandBars.ICommandBarControl)
    ...
    Dim lngRow As Long, strItemIDs As String
    Select Case Control.ID
        ...
        Case conMenu_Tool_Reference
            ...
            With vsfBlood
                For lngRow = .FixedRows To .Rows - 1
                    If Val(.Cell(flexcpData, lngRow, COL_P_选择)) = 1 Then
                        strItemIDs = IIF(strItemIDs = "", "", strItemIDs & ",") & Val(.TextMatrix(lngRow, COL_P_ID))
                    End If
                Next
            End With
            If strItemIDs = "" Then
                MsgBox "您还没有选择血液信息，请选择后在操作！", vbInformation, gstrSysName
                Exit Sub
            End If
            '获取选择的血液信息
            Call frmClinicHelp.ShowMe(1, Me, Val(strItemIDs), False, IIF(InStr(1, strItemIDs, ",") > 0, strItemIDs, ""))
    End Select
End Sub
...
Private Sub InitCommandBar()
    ...
    With objBar.Controls
        ...
        Set objControl = .Add(xtpControlButton, conMenu_Tool_Reference, " 诊疗参考(&R)")
    ...
    With cbsMain.KeyBindings
        ...
        .Add FALT, vbKeyR, conMenu_Tool_Reference
    End With
...
Private Sub CheckListResult(ByVal str诊疗编码 As String, ByVal str诊疗名称 As String, ByVal strListResult As String)
    '功能：电子病历四级评审要求，检验对应的检验项目及指标是否出了结果
    '
    'strLisResult结果格式：
    ...
    Dim arrCode, blnFind As Boolean '存放做了的诊疗项目编码
    Dim rsLis As New ADODB.Recordset
    Dim strMsg As String, strChildMsg As String
    
    On Error GoTo ErrHand
    With rsLis.Fields
        .Append "诊疗编码", adVarChar, 50
        .Append "指标代码", adVarChar, 100
        .Append "指标名称", adVarChar, 100
    End With
    rsLis.CursorLocation = adUseClient
    rsLis.LockType = adLockOptimistic
    rsLis.CursorType = adOpenStatic
    rsLis.Open
    
    arrCode = Array()
    
    strTmp = strListResult
    strTmp = Replace(strTmp, "<split1>", "")
    strTmp = Replace(strTmp, "<split2>", "")
    strTmp = Replace(strTmp, "<split3>", "")
    strTmp = Trim(strTmp)
    If strTmp <> "" Then
        arrTmp1 = Split(strListResult, "<split3>")
        For i = 0 To UBound(arrTmp1)
            If Replace(Replace(CStr(arrTmp1(i)), "<split1>", ""), "<split2>", "") <> "" Then 'strResult会存在<split1><split1><split3>这种
                ...
                If InStr(1, "," & str诊疗编码 & ",", "," & CStr(arrTmp2(1)) & ",") <> 0 Then
                    blnFind = True
                    If UBound(arrCode) >= 0 Then
                        If InStr(1, "," & Join(arrCode, ",") & ",", "," & CStr(arrTmp2(1)) & ",") = 0 Then
                            blnFind = False
                        End If
                    Else
                        blnFind = False
                    ...
                    If blnFind = False Then
                        ...
                        arrCode(UBound(arrCode)) = CStr(arrTmp2(1))
                    End If
                End If
                If arrTmp2(8) = "" Then '只检查没有结果的指标
                    rsLis.AddNew
                    rsLis!诊疗编码 = CStr(arrTmp2(1))
                    rsLis!指标代码 = CStr(arrTmp2(4))
                    rsLis!指标名称 = CStr(arrTmp2(5))
                    rsLis.Update
                End If
            End If
        Next
    ...
    '进行未检查项目和未出结果指标的检查
    strMsg = ""
    arrTmp1 = Split(str诊疗编码, ",")
    arrTmp2 = Split(str诊疗名称, "'")
    For i = 0 To UBound(arrTmp1)
        blnFind = False
        For j = 0 To UBound(arrCode)
            If CStr(arrTmp1(i)) = CStr(arrCode(j)) Then
                blnFind = True
                Exit For
            End If
        Next j
        If blnFind = True Then
            '找到了再去找是否存在未做的指标
            strChildMsg = ""
            rsLis.Filter = "诊疗编码='" & CStr(arrTmp1(i)) & "'"
            Do While Not rsLis.EOF
                strChildMsg = strChildMsg & IIF(strChildMsg = "", "", "、") & "[" & rsLis!指标代码 & "-" & rsLis!指标名称 & "]"
                rsLis.MoveNext
            Loop
            If strChildMsg <> "" Then
                strMsg = strMsg & IIF(strMsg = "", "", vbCrLf) & "检验项目【" & arrTmp2(i) & "】中存在以下指标还未出结果{" & strChildMsg & "}"
            End If
        Else
            '没有找到说明这个检验项目都没有做
            strMsg = strMsg & IIF(strMsg = "", "", vbCrLf) & "检验项目【" & arrTmp2(i) & "】还未做"
        End If
    Next i
    If strMsg <> "" Then
        strMsg = "申请的血液项目还存在未做或未出结果的检查，详细信息信息：" & vbCrLf & strMsg
    End If
    imgLisInfo.Tag = strMsg
    picLisInfo.Visible = strMsg <> ""
    Exit Sub
ErrHand:
    If ErrCenter = 1 Then
        Resume
...
'功能：初始化输血项目对应的检验项目指标表格（备血申请可能有所个输血项目）
    ...
    Dim str检验编码 As String, str检验名称 As String
    ...
    picLisInfo.Visible = False: imgLisInfo.Tag = ""
    '启用了血库时，用血申请不需要检查结果
    ...
    If UBound(arrBloodID) > 0 Then
        strSQL = "Select /*+ CARDINALITY(C 10) */ A.检验项目ID,B.编码,B.名称 from 输血检验对照 A,诊疗项目目录 B,Table(f_Num2list([1])) C " & _
    ...
    Else
        strSQL = "Select A.检验项目ID,B.编码,B.名称 from 输血检验对照 A,诊疗项目目录 B Where A.检验项目ID=B.ID And A.项目ID=[1] Order by B.编码"
    ...
    Do While Not rsLis.EOF
        ...
        End If
        If InStr(1, "'" & str检验名称 & "'", "'" & rsLis!名称 & "'") = 0 Then
            str检验名称 = str检验名称 & "'" & rsLis!名称
        End If
        rsLis.MoveNext
    Loop
    str检验编码 = Mid(str检验编码, 2)
    str检验名称 = Mid(str检验名称, 2)
    ...
    With vsLIS
        ...
        End If
         ...
         '检查是有要的项目
        Call CheckListResult(str检验编码, str检验名称, strResult)
...
Private Sub imgLisInfo_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    zlCommFun.ShowTipInfo picLisInfo.hWnd, imgLisInfo.Tag, True
End Sub
