...
'设置窗口透明度
Private Sub SetFormToAlpha(ByVal lngAlpha As Long)
'该方法只适用于窗口对象
    Dim tmpLog As Long
...
Private Function HasProperty(obj As Object, ByVal strProName As String) As Boolean
'判断组件是否具备指定的属性
On Error GoTo errHandle
    CallByName obj, strProName, VbGet
...
On Error GoTo errHandle
    ...
    For Each objControl In UserControl.ContainedControls
        If HasProperty(objControl, "visible") Then
