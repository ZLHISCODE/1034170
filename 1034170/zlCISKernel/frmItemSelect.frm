...
Private Sub Form_Load()
    ...
    mlng发料部门ID = Val(zlDatabase.GetPara(IIF(mint病人来源 = 2, "住院", "门诊") & "缺省发料部门", glngSys, p医嘱附费管理))
    If mstr输入 = "" Then
        '读取类别失败,已提示,非取消退出
...
'      strIDs=读取的项目ID集,用于判断输入时是否别名不同的同一个收费项目
    ...
    If blnLoad Then
        ...
        If mstr输入 <> "" Then
            ...
            If mint险类 = 0 Then
                strMain = _
                    " Select Distinct A.ID,A.类别,A.编码," & IIF(gbyt输入药品显示 = 1, "C2.名称 ,C1.名称 as 商品名,'' as  简码,", "B.名称,Null as 商品名,B.简码,") & " " & _
                    ...
                    " Where A.ID=B.收费细目ID And A.服务对象 IN([9],3)" & strSQLItem & _
                            IIF(gbyt输入药品显示 = 1, " And A.ID=C1.收费细目ID(+) And C1.码类(+)=1 And C1.性质(+)=3", "") & _
                            IIF(gbyt输入药品显示 = 1, " And A.ID=C2.收费细目ID(+) And C2.码类(+)=1 And C2.性质(+)=1", "") & _
                        IIF(str类别 <> "", " And A.类别=[4]", "") & IIF(mstr类别 <> "", " And Instr([5],A.类别)>0", "") & mstr特准项目 & strInput
            Else
                strMain = _
                    " Select Distinct A.ID,A.类别,A.编码," & IIF(gbyt输入药品显示 = 1, "C2.名称 ,C1.名称 as 商品名,'' as  简码,", "B.名称,Null as 商品名,B.简码,") & " " & _
                    " A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价" & _
                    " From 收费项目别名 B,收费项目目录 A,保险支付项目 C,保险支付大类 D" & IIF(gbyt输入药品显示 = 1, ",收费项目别名 C2,收费项目别名 C1", "") & _
                    " Where A.ID=B.收费细目ID And A.服务对象 IN([9],3)" & strSQLItem & _
        ...
        Else
            If mint险类 = 0 Then
                strMain = _
                    " Select" & _
                    " A.ID,A.类别,A.编码,Nvl(B.名称,A.名称) as 名称," & IIF(gbyt药品名称显示 = 2, "C1.名称 as 商品名,", "Null as 商品名,") & "" & _
                    " A.计算单位,A.规格,A.产地,A.费用类型,Null 医保大类,A.说明,A.是否变价" & _
                    " From 收费项目别名 B,收费项目目录 A" & IIF(gbyt药品名称显示 = 2, ",收费项目别名 C1", "") & _
                    " Where A.ID=B.收费细目ID(+) And B.码类(+)=1 And B.性质(+)=[10]" & strSQLItem & _
                            IIF(gbyt药品名称显示 = 2, " And A.ID=C1.收费细目ID(+) And C1.码类(+)=1 And C1.性质(+)=3", "") & _
            ...
            Else
                strMain = _
                    ...
                    " A.ID,A.类别,A.编码,Nvl(B.名称,A.名称) as 名称," & IIF(gbyt药品名称显示 = 2, "C1.名称 as 商品名,", "Null as 商品名,") & "" & _
                    " A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价" & _
                    " From 收费项目别名 B,收费项目目录 A,保险支付项目 C,保险支付大类 D" & IIF(gbyt药品名称显示 = 2, ",收费项目别名 C1", "") & _
                    " Where A.ID=B.收费细目ID(+) And B.码类(+)=1 And B.性质(+)=[10]" & strSQLItem & _
                    " And A.ID=C.收费细目ID(+) And C.险类(+)=[16] And C.大类ID=D.ID(+)" & vbNewLine & _
                            IIF(gbyt药品名称显示 = 2, " And A.ID=C1.收费细目ID(+) And C1.码类(+)=1 And C1.性质(+)=3", "") & _
                            IIF(str类别 <> "", " And A.类别=[4]", "") & IIF(mstr类别 <> "", " And Instr([5],A.类别)>0", "") & mstr特准项目
        ...
        Else
            ...
            strSQL = _
                " Select A.ID,A.类别 as 类别ID,B.序号 as 顺序ID,B.名称 as 类别," & _
                    ...
                    " Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价)" & _
                        IIF(mbln药房单位, "*Nvl(D." & str药房包装 & ",1)", "") & ",'9999990.00000'))) as 单价," & _
                    IIF(InStr(1, mstrPrivs, "显示库存") > 0, "LTrim(To_Char(X.库存" & IIF(mbln药房单位, "/Nvl(D." & str药房包装 & ",1)", "") & ",'9999990.00000'))", "Decode(Sign(X.库存),1,'有','无')") & " as 库存" & _
    ...
    If blnLoad Then
        ...
        Else
                strSQL = strSQL & IIF(strSQL = "", "", " Union ALL ") & _
                 ...
                 "           A.计算单位 as 单位,A.规格,A.产地,A.费用类型,A.医保大类,A.说明," & _
                 "       Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价),'9999990.00000'))) as 单价," & _
                IIF(InStr(1, mstrPrivs, ";显示库存;") > 0, " Decode(D.跟踪在用,1,To_Char(X.库存,'9999990.00000'),NULL)", "Decode(Sign(X.库存),1,'有','无')") & " as 库存" & _
    ...
    Set mrsItem = zlDatabase.OpenSQLRecord(strSQL, Me.Name, int类型, lng诊疗分类ID, lng收费分类ID, str类别, mstr类别, _
        ...
        lng药房ID, mlng西药房, mlng成药房, mlng中药房, 0, mint险类, mint中药形态, mlng发料部门ID)
