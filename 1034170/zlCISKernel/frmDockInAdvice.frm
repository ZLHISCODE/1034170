...
Private Enum COL医嘱清单
    ...
    COL_签名否 = COL_ID + 41
    COL_文件ID = COL_ID + 42
    COL_报告项 = COL_ID + 43 '0-无报告，1-有报告并按编辑格式打印，2-有报告并按报表格式打印。
    COL_报告ID = COL_ID + 44
    COL_查阅状态 = COL_ID + 45
    COL_收费细目ID = COL_ID + 46
    COL_单量单位 = COL_ID + 47
    COL_开嘱科室ID = COL_ID + 48
    COL_审核状态 = COL_ID + 49
    COL_申请序号 = COL_ID + 50
    COL_审核标记 = COL_ID + 51
    COL_基本药物 = COL_ID + 52
    COL_高危药品 = COL_ID + 53
    COL_标本部位 = COL_ID + 54   'PASS  药品名称
    COL_检查报告ID = COL_ID + 55
    COL_用药目的 = COL_ID + 56
    COL_RIS预约ID = COL_ID + 57
    COL_处方审查状态 = COL_ID + 58
    COL_处方审查结果 = COL_ID + 59
    COL_RIS报告ID = COL_ID + 60
    COL_LIS报告ID = COL_ID + 61
    COL_检查方法 = COL_ID + 62 '区分是备血医嘱还是用血医嘱
    COL_RIS预约状态 = COL_ID + 63
    COL_危急值ID = COL_ID + 64 '医嘱关和危急值关联
End Enum
...
Private Sub DefCommandsInDoctor(ByVal cbsMain As Object)
    ...
    With objMenu.CommandBar.Controls
        ...
        End If
        If mint场合 = 0 And InStr(GetInsidePrivs(p住院医嘱下达), "病人用药清单") > 1 Then
            Set objControl = .Add(xtpControlButton, conMenu_Edit_DrugList, "病人用药清单"): objControl.BeginGroup = True
...
Public Sub zlExecuteCommandBars(ByVal Control As CommandBarControl)
    ...
    Case conMenu_Manage_ReportLisView  '检验报告浏览
        ...
        End If
    Case conMenu_Edit_DrugList '病人用药清单
        If mlng病人ID <> 0 And mlng主页ID <> 0 Then Call frmDrugList.ShowMe(Me, True, mlng病人ID, mlng主页ID)
...
'  3.无数据的情况
    ...
    Case conMenu_Edit_MediAudit '药嘱审查(中药不显示)
        ...
        End If
    Case conMenu_Edit_DrugList '病人用药清单
        Control.Enabled = mlng病人ID <> 0 And mlng主页ID <> 0
    End Select
...
'功能：初始化医嘱清单格式
    ...
    strHead = "ID;相关ID;序号;婴儿ID;医嘱状态;诊疗类别;操作类型;毒理分类;标志;" & _
              ...
              "停嘱时间,1530,1;停嘱护士,850,1;确认停嘱时间,1530,1;打印时间,1530,1;" & _
...
'参数：blnRefreshNotify刷新医嘱提醒(F5手动刷新,新开医嘱，停止医嘱，作废医嘱时)
    ...
    "Select /*+ RULE */ A.ID,A.相关ID,A.序号,Nvl(A.婴儿,0) as 婴儿ID,A.医嘱状态," & _
             ...
             " A.审核标记,D.基本药物,d.高危药品,A.标本部位 , Max(y.检查报告id) || '' As 检查报告id,a.用药目的,f.预约ID as RIS预约ID,J.状态 as 处方审查状态,J.审查结果 as 处方审查结果,max(y.RISID) as RIS报告ID,max(y.报告ID) as LIS报告ID,Max(a.检查方法) as 检查方法,f.是否调整 as RIS预约状态,max(h.危急值id) as 危急值ID, (Select To_Char(Max(p.打印时间), 'YYYY-MM-DD HH24:MI') From 病人医嘱打印 P Where a.Id = p.医嘱id) As 打印时间"
    strSQL = strSQL & _
    ...
    If mdat重整 <> CDate("1900-01-01") Then
        ...
        Else
            ...
            " Select * From (" & strSQL & " And Nvl(A.重整标志,1)=0 And A.医嘱状态 Not IN(1,2) " & strGroupBy & "  Order by 婴儿ID,序号)" & _
                     ...
                     " -Null as 申请序号,-Null as 审核标记 ,Null as 基本药物,-Null as 高危药品,-Null as 标本部位,-NULL AS 检查报告id,-Null as 用药目的,-null as RIS预约ID,-null as 处方审查状态,-null as 处方审查结果,-null as RIS报告ID,-null as LIS报告ID,-null as 检查方法,-null as RIS预约状态,-null as 危急值ID,-null as 打印时间 From Dual" & _
                     " Union ALL" & _
...
'功能：医嘱表格中加数据
    ...
    With vsAdvice
        ...
        For i = 1 To rsData.RecordCount
            ...
            .TextMatrix(i, COL_打印时间) = Nvl(rsData!打印时间)
