...
Public Sub ShowMe(frmParent As Object, ByVal int场合 As Integer, ByVal int范围 As Integer, ByVal lng病区ID As Long, lng科室id As Long, Optional ByVal blnBySave As Boolean)
...
'功能：保存面板显示与否
    '都以医嘱下达模块为准
    Call zlDatabase.SetPara("显示快捷输入面板", IIF(mblnShow, 1, 0), glngSys, IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达), InStr(GetInsidePrivs(IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达)), ";医嘱选项设置;") > 0)
End Sub
...
Private Sub Form_Load()
    Dim strSql As String
    Dim objControl As CommandBarControl
    Dim intType As Integer, blnSetup As Boolean
    ...
    blnSetup = InStr(GetInsidePrivs(IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达)), "医嘱选项设置") = 0
    
    '0-个人，1-科室，2-全院
    intType = Val(zlDatabase.GetPara("快捷面板成套范围", glngSys, IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达), , _
            Array(fraScope, optScope(0), optScope(1), optScope(2)), blnSetup))
    ...
    If (intType = 3 Or intType = 15) And blnSetup Then
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    '都以医嘱下达模块为准
    blnSetup = InStr(GetInsidePrivs(IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达)), ";医嘱选项设置;") > 0
    Call zlDatabase.SetPara("快捷输入面板类型", mintType - 1, glngSys, IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达), blnSetup)
    ...
    If mfrmParent.WindowState = 0 Then
        ...
        If mintType = 8 Then
            '都以医嘱下达模块为准
            Call zlDatabase.SetPara("快捷面板成套范围", Val("" & tbcScheme.Tag), glngSys, IIF(mint范围 = 1, p门诊医嘱下达, p住院医嘱下达), blnSetup)
        End If
    End If
