...
Private Sub cmdStart_Click()
    ...
    Dim i As Long, lngNum As Long, strSysNum As String
    ...
    With vsfModule
        ...
        For i = 1 To .Rows - 1
             If .Cell(flexcpChecked, i, 0) = flexChecked Then
                lngNum = lngNum + 1
                strSysNum = strSysNum & IIf(strSysNum = "", "", ",") & .TextMatrix(i, .ColIndex("编号"))
            End If
        Next
        ...
        If Not CheckSpecialSpScript(lblTargetPath.Caption, strSysNum, False) Then Exit Sub
...
Private Function GetChangedProc(ByRef rsProcs As ADODB.Recordset, ByRef strErr As String) As Boolean
    '遍历传入的过程集合,判断是否是变动过程
    '有错误返回False
    Dim strProc As String, strProcNc As String
    ...
    With rsProcs
        .Filter = 0
        Do While Not .EOF
            strProc = LoadBaseProcs(!P_Name, strProcNc)
            
            If ConvertStr(strProc) = "" Then
                .Delete adAffectCurrent
            ElseIf ConvertStr(!P_Define) = "" Then
                .Delete adAffectCurrent
            ElseIf IsProcCollected(!P_Name, !P_Ver) Then '已经收集的过程不需要收集
                .Delete adAffectCurrent
            ElseIf ConvertStr(strProcNc) = ConvertStr(!P_DefineNC) Or ConvertStr(!P_Define) = ConvertStr(strProc) Then
                If IsZlProcExist(!P_Name) Then   '说明数据库中保存了低版本的过程
                    DeleteProcByName !P_Name
                End If
                mrsSysProc.Filter = "P_Name = '" & !P_Name & "'"
                If mrsSysProc.RecordCount = 0 Then mrsSysProc.AddNew
                mrsSysProc.Update Array("P_Name", "P_Ver"), Array(!P_Name, !P_Ver)
                .Delete adAffectCurrent
            Else    '检测到与当前过程不同,需要判断是否属于其他系统低版本
                mrsSysProc.Filter = "P_Name = '" & !P_Name & "'"
                If mrsSysProc.RecordCount > 0 Then
                    If GetFileVer(!P_Ver) <= GetFileVer(mrsSysProc!P_Ver) Then
                        .Delete adAffectCurrent
                    End If
                End If
            End If
