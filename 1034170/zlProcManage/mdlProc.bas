...
Public Function LoadBaseProcs(ByVal strProcName As String, Optional ByRef strProcNc As String) As String
    ...
    Set rsSource = OpenSQLRecord(strSQL, "获取数据库过程源码", strProcName)
...
Public Function GetProcIdByName(ByVal strName As String, Optional ByVal intProp As Integer, Optional ByVal intStat As Integer) As Long
    ...
    Set rsTmp = OpenSQLRecord(strSQL, "获取ID", strName, intProp, intStat)
...
Public Function GetPorcTxtByName(ByVal strName As String, ByVal intType As Integer) As String
    ...
    Set rsTmp = OpenSQLRecord(strSQL, "获取过程文本", strName, intType)
...
Public Function CheckProcManage() As Boolean
    ...
    Set rsTmp = OpenSQLRecord(strSQL, "检查变动过程模块")
    ...
    Set rsTmp = OpenSQLRecord(strSQL, "检查变动过程结构")
...
Public Sub GetZlProcs()
    ...
    Set mrsProcs = OpenSQLRecord(strSQL, "获取系统全部过程")
    Exit Sub
errH:
...
Public Function CheckSpecialSpScript(ByVal strInitPath As String, ByVal strSystem As String, ByVal blnSvrTools As Boolean, Optional ByVal btnShowFrm As Boolean = True) As Boolean
    ...
    Dim strSQL As String, strTools As String, rsTmp As New ADODB.Recordset
    Dim strPath As String, blnHas As Boolean
    Dim strTip As String, lngTmp As Long
    ...
    Set rsTmp = OpenSQLRecord(strSQL, "获取特殊Sp执行信息", strSystem)
    If rsTmp.RecordCount = 0 Then
    ...
    Do While Not rsTmp.EOF
        blnHas = False
        If rsTmp!系统 <> 101 Then    '业务系统
            strPath = strInitPath & "\" & Decode(rsTmp!系统 \ 100, 1, "ZLHIS10", 3, "ZLMEDREC10", 4, "ZLMATERIAL10", _
                                                        6, "ZLDEVICE10", 21, "ZLPEIS10", 22, "ZLBLOOD10", _
                                                        23, "ZLINFECT10", 24, "ZLOPER10", _
                                                        25, "ZLLIS10", 26, "ZLPSS10", 27, "ZLHEC10") & "\升级脚本\"
        ...
        If gobjFile.FolderExists(strPath) Then
            ...
            If lngTmp <> rsTmp!系统 Then
                ...
                If lngTmp = 101 Then
                    strFile = strPath & "\ZLUpgrade" & rsTmp!大版本号 & "." & Mid(rsTmp!当前版本, InStrRev(rsTmp!当前版本, ".") + 1) & ".SQL"     '管理工具脚本 :形如ZLUpgrade10.35.70.sql
                Else
                    strFile = strPath & "\ZL" & lngTmp \ 100 & "_" & rsTmp!大版本号 & "." & Mid(rsTmp!当前版本, InStrRev(rsTmp!当前版本, ".") + 1) & ".SQL"  '业务系统脚本: zl系统号\100_版本.SQL
                End If
            ...
            If Not IsNull(rsTmp!结果版本) Then
                If lngTmp = 101 Then
                    strFile = strPath & "\ZLUpgrade" & rsTmp!结果版本 & ".SQL"     '管理工具脚本 :形如ZLUpgrade10.35.70.sql
                Else
                    strFile = strPath & "\ZL" & rsTmp!系统 \ 100 & "_" & rsTmp!结果版本 & ".SQL"  '业务系统脚本: zl系统号\100_版本.SQL
                End If
            ...
            End If
        Else
            strTip = strTip & IIf(strTip = "", "", vbNewLine) & "所选目录下缺失" & rsTmp!名称 & rsTmp!大版本号 & ".0的全部升级脚本"
        End If
