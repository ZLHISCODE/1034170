...
Private mrsSysProc As New ADODB.Recordset
...
Private Sub Form_Load()
    ...
    With mrsSysProc
        ...
        .Fields.Append "P_Ver", adVarChar, 32  '过程版本
...
Private Sub LoadFiles(ByVal strSystem As String, Optional ByVal strCurInitPath As String)
    ...
    With vsfFiles
        ...
        For i = 0 To UBound(arrSys)
            ...
            rsTmpTarget.Filter = 0: rsTmpCur.Filter = 0
...
Private Sub GetChangedProc(ByVal lngRow As Long)
    ...
    With vsfFiles
        ...
        For i = lngRow To .Rows - 1
            ...
            '遍历表格,根据表格中的文件获取过程
            If i <= .Rows - 1 Then
                lngSysNum = .TextMatrix(i, .ColIndex("系统编号"))
                strSysOwner = GetOwnerName(.TextMatrix(i, .ColIndex("系统编号")), gcnOracle)
                
                If .TextMatrix(i, .ColIndex("脚本类型")) = "当前" Then
                    '获取同一系统下的当前版本脚本
                    GetProceduresByFile .TextMatrix(i, .ColIndex("文件")), rsCurrent, .TextMatrix(i, .ColIndex("版本")), lngSysNum, .TextMatrix(i, .ColIndex("系统名称")), strSysOwner
                Else
                    '升级脚本
            ...
            If blnCheck Or i = .Rows - 1 Then
                ...
                If rsCurrent.RecordCount > 0 Then rsCurrent.MoveFirst
                If rsUpdate.RecordCount > 0 Then rsUpdate.MoveFirst
                ...
                Do While Not rsCurrent.EOF
                    ...
                    '从数据库取出过程和记录集中的做对比
                    strProcTxt = LoadBaseProcs(rsCurrent!P_Name, strProcNc)
                    If ConvertStr(strProcNc) = "" Or ConvertStr(rsCurrent!P_DefineNC) = "" Then
                        rsCurrent.Delete adAffectCurrent
                    ElseIf IsProcCollected(rsCurrent!P_Name, rsCurrent!P_Ver) Then '已经收集的过程不需要收集
                        rsCurrent.Delete adAffectCurrent
                    ElseIf ConvertStr(rsCurrent!P_DefineNC) = ConvertStr(strProcNc) Or ConvertStr(rsCurrent!P_Define) = ConvertStr(strProcTxt) Then '相同就删除
                        If IsZlProcExist(rsCurrent!P_Name) Then   '说明数据库中保存了低版本的过程
                            DeleteProcByName rsCurrent!P_Name
                        End If
                        mrsSysProc.Filter = "P_Name = '" & rsCurrent!P_Name & "'"
                        If mrsSysProc.RecordCount = 0 Then mrsSysProc.AddNew
                        mrsSysProc.Update Array("P_Name", "P_Ver"), Array(rsCurrent!P_Name, rsCurrent!P_Ver)
                        rsCurrent.Delete adAffectCurrent
                    Else
                        mrsSysProc.Filter = "P_Name = '" & rsCurrent!P_Name & "'"
                        If mrsSysProc.RecordCount > 0 Then
                            If GetFileVer(rsCurrent!P_Ver) <= GetFileVer(mrsSysProc!P_Ver) Then
                                rsCurrent.Delete adAffectCurrent
                            End If
                        End If
