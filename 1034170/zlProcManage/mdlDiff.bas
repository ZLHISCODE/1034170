...
Public Sub CompareIt(ByRef strTxt1 As String, ByRef strTxt2 As String)
    ...
    If UBound(Split(strPrefix, vbNewLine)) = UBound(arrTxt1) And UBound(arrTxt1) = UBound(arrTxt2) Then
        Exit Sub
    End If
    '获取尾部的相同内容
    ...
    If ConvertStr(strTxt1) = "" Or ConvertStr(strTxt2) = "" Then
        If ConvertStr(strTxt1) = "" And ConvertStr(strTxt2) = "" Then
            If UBound(arrTxt1) > UBound(arrTxt2) Then
                For i = 0 To UBound(arrTxt1)
                    strTxt2 = IIf(i = 0, " ", strTxt2 & vbNewLine)
                Next
            Else
                For i = 0 To UBound(arrTxt2)
                    strTxt1 = IIf(i = 0, " ", strTxt1 & vbNewLine)
                Next
            End If
        ElseIf ConvertStr(strTxt1) = "" Then
            For i = 0 To UBound(arrTxt2)
                strTxt1 = IIf(i = 0, " ", strTxt1 & vbNewLine)
            Next
        ElseIf ConvertStr(strTxt2) = "" Then
            For i = 0 To UBound(arrTxt1)
                strTxt2 = IIf(i = 0, " ", strTxt2 & vbNewLine)
            Next
        End If
...
Private Function GetCommonSuffix(ByVal arrTxt1 As Variant, ByVal arrTxt2 As Variant, Optional ByRef lngIdx As Long) As String
    ...
    For i = 0 To lngMin
        If ConvertStr(arrTxt1(UBound(arrTxt1) - i)) <> ConvertStr(arrTxt2(UBound(arrTxt2) - i)) Then
            Exit For
        Else
            If i = 0 Then
...
Private Sub GetBeforeAndAfter(arrTxt As Variant, arrMid As Variant, ByRef strBefore As String, ByRef strAfter As String)
    ...
    For i = 0 To UBound(arrTxt)
        If ConvertStr(arrTxt(i)) = ConvertStr(arrMid(0)) Then
            For j = 1 To UBound(arrMid)
                If ConvertStr(arrTxt(i + j)) = ConvertStr(arrMid(j)) Then
                    blnTmp = True
                Else
                    blnTmp = False
                    Exit For
...
Public Sub MergeDiffInto2SynEdit(ByVal strTxt1 As String, ByVal strTxt2 As String, txt1 As SyntaxEdit, txt2 As SyntaxEdit, colDiff As Collection)
    ...
    For i = 0 To UBound(arrTxt1)
        ...
        If ConvertStr(arrTxt1(i)) <> ConvertStr(arrTxt2(i)) Then
            ...
            If ConvertStr(arrTxt1(i)) = "" Then '新增
                txt2.SetRowColor i + 1, 绿色
                colDiff.Add arrTxt2(i), "_" & i + 1
            ElseIf ConvertStr(arrTxt2(i)) = "" Then
    ...
    For i = 0 To UBound(arrTxt1)
        With txtEdit
            ...
            If ConvertStr(arrTxt1(i)) <> ConvertStr(arrTxt2(i)) Then
                ...
                If ConvertStr(arrTxt1(i)) = "" Then '新增文本
                    colDiff.Add "空空", "_" & (i + 1)
                    If blnDisplay Then
                        .SetRowColor i + 1, 绿色
                    End If
                    .RowText(i + 1) = arrTxt2(i)
                ElseIf ConvertStr(arrTxt2(i)) = "" Then '删除本文
                    If blnDisplay Then
