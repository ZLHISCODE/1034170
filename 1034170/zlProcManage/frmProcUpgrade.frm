...
Private Sub cmdCheck_Click()
    Dim strSys As String, strSysNum As String, i As Integer
        ...
        For i = 1 To .Rows - 1
            If .Cell(flexcpChecked, i, 0) = flexChecked Then
                ...
                If strSys = "" Then
                    ...
                    strSysNum = .TextMatrix(i, .ColIndex("编号"))
                Else
                    strSys = strSys & ";" & .TextMatrix(i, .ColIndex("编号")) & "," & .TextMatrix(i, .ColIndex("系统名称")) & "," & _
                                                    .TextMatrix(i, .ColIndex("当前版本号")) & "," & .TextMatrix(i, .ColIndex("目标版本号")) & "," & strInitFile
                    strSysNum = strSysNum & "," & .TextMatrix(i, .ColIndex("编号"))
                End If
                intNum = intNum + 1
            End If
        Next
...
Private Sub cmdList_Click()
    ...
    vsfAlter.SaveGrid App.Path & "\升级变动过程清单.xls", flexFileExcel, True
    MsgBox "保存成功，已经保存至" & VB.App.Path & "\升级变动过程清单.xls"

    Exit Sub
errH:
    MsgBox "变动过程清单导出失败." & vbNewLine & Err.Description
End Sub
...
Private Sub pctBottom_Resize()
    ...
    cmdList.Left = cmdExport.Left - cmdList.Width - 40
    cmdManual.Left = cmdList.Left - cmdManual.Width - 40
End Sub
    ...
    With vsfModule
        For i = 1 To .Rows - 1
            ...
            If gobjFile.FileExists(strInitFile) Then
                If GetUpgradeFiles(Nothing, Val(.TextMatrix(i, .ColIndex("编号"))), .TextMatrix(i, .ColIndex("当前版本号")), strInitFile, , , , strTarget, , True, True) Is Nothing Then
...
Private Sub vsfSp_LostFocus()
    If vsfSp.Visible Then
        lblSp_Click (1)
    End If
End Sub
