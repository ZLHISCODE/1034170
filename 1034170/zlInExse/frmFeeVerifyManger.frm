        ...
        For lngRow = 1 To .Rows - 1
            If Val(.TextMatrix(lngRow, .ColIndex("ID"))) <> 0 Then
                ...
                ElseIf Val(.Cell(flexcpData, lngRow, .ColIndex("审核"))) = 0 And blnSel Then
                    If .TextMatrix(lngRow, .ColIndex("单据")) = "收费单" Then
                        If CheckBillExistReplenishData(1, , .TextMatrix(lngRow, .ColIndex("单据号"))) Then
                            ShowMsgbox "单据号为" & .TextMatrix(lngRow, .ColIndex("单据号")) & "的记录存在补充结算，不能转为住院费用！"
                            Exit Function
                        End If
                    End If
