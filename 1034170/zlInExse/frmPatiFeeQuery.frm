...
Object = "{AF9744ED-CAFC-4877-8437-2C20C14CEA4E}#6.1#0"; "ZLIDKIND.OCX"
...
Private Sub ExecBalance()
    ...
    frmBalance.mlngPatientID = mlng病人ID
    frmBalance.mstr主页Id = mlng主页ID '38537
...
Private Sub Form_Load()
    ...
    Dim objPan As Pane, objCondition As Pane
            ...
            Else
                With rptPati.SelectedRows(0)
                    ...
                    Call mclsFeeQuery.zlRefresh(mlng病人ID, mlng主页ID, Val(.Record(GetRptRsColumn("住院号")).Value), Val(.Record(GetRptRsColumn("当前病区ID")).Value), _
                        Val(.Record(GetRptRsColumn("险类")).Value), blnDateMoved, Trim(.Record(GetRptRsColumn("出院时间")).Value) <> "", Trim(.Record(GetRptRsColumn("结清")).Value) <> "")
                End With
            End If
        Case "医嘱与费用"
                If mlng病人ID <> 0 Then
    ...
    If chk预交款.Value = 1 Then
        ...
        If cboState.Text = "在院病人" Or cboState.Text = "出院病人" Then
            strSql = "Select A.病人ID, B.主页ID, A.登记时间, B.状态, B.病人性质, B.数据转出, B.出院科室id As 当前科室id, A.就诊卡号, B.险类, E.密码, B.当前病区ID, Nvl(b.姓名, a.姓名) As 姓名, B.住院号, B.出院病床 As 床号," & vbNewLine & _
                ...
                "      F.医保号 = E.医保号(+) And F.险类 = E.险类(+) And F.中心 = E.中心(+) And B.当前病区ID = H.ID" & vbNewLine & _
...
Private Sub ExecFindPati(ByVal objCard As Card, Optional blnNext As Boolean, Optional blnCard As Boolean = True)
    ...
    If Not IsNumeric(strValue) And strKind = "住院号" Then
        ...
        Call txtFind.SetFocus: Call SelAll(txtFind)
        Exit Sub
    End If
...
End Sub
 Private Function ExecPrePayMoney() As Boolean
    ...
    '---------------------------------------------------------------------------------------------------------------------------------------------
    Dim dbl缴款额 As Double, rsTemp As ADODB.Recordset
    On Error GoTo errHandle
    If mobjPatient Is Nothing Then
        ...
        End If
    End If
    gstrSQL = "Select Zl1_Getdef_Prepaymoney([1],[2]) as 缴款额 from dual"
    ...
    '    '---------------------------------------------------------------------------------------------------------------------------------------------
    ExecPrePayMoney = mobjPatient.PlusDeposit(glngSys, gcnOracle, Me, gstrDBUser, 1, mlng病人ID, mlng主页ID, dbl缴款额)
    Exit Function
errHandle:
    If ErrCenter() = 1 Then
        Resume
