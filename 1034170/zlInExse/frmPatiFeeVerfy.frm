...
Private Sub imgColList_Click(Index As Integer)
    ...
    vRect = GetControlRect(picImgList(Index).hWnd)
    lngLeft = vRect.Left
...
Private Sub LoadFeeList(ByVal lng医嘱ID As Long, Optional lng相关ID As Long)
    ...
    On Error GoTo errHandle
    strSql = "" & _
    " Select b.No as 单据号, b.序号, b.收费细目id,B.记录状态, q.名称 As 收费名称, q.规格, b.数量, Decode(r.住院单位, Null, q.计算单位, r.住院单位) As 单位, " & _
    ...
    "               Avg(Nvl(a.付数, 1) * Nvl(a.数次, 1)) As 数量, Avg(a.标准单价) As 单价, Sum(a.应收金额) As 应收金额, Sum(a.实收金额) As 实收金额 " & _
    "           From 住院费用记录 A,病人医嘱记录 B" & _
    "           Where A.医嘱序号=B.ID And  A.病人id= [1] And a.主页id = [2] and (B.ID in ([3],[4]) or nvl(B.相关ID,-2) in ([3],[4] )) " & _
    "           Group By a.No, Nvl(a.价格父号, a.序号), a.收费细目id, a.执行部门id, a.记录状态, a.执行状态, a.登记时间) B, 部门表 J, 收费项目目录 Q, 药品规格 R " & _
