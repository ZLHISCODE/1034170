...
Public Function ShowSelect(frmParent As Object, ByVal strPrivs As String, _
    ...
    Optional ByVal str排除类别 As String = "", _
    ...
    mint病人来源 = int病人来源: mint险类 = int险类: mbln药房单位 = bln药房单位
    mstr类别 = str类别: mstr排除类别 = str排除类别: mstr输入 = str输入
    mlngHwnd = lngHwnd: mstr特准项目 = str特准项目: mint中药形态 = int中药形态
    mbln条码扫描 = bln条码扫描
    mlng批次 = lng批次
...
Private Sub cmdOK_Click()
    ...
    mlng批次 = Val(vsItem.TextMatrix(vsItem.Row, vsItem.Cols - 1))
...
'      strIDs=读取的项目ID集,用于判断输入时是否别名不同的同一个收费项目
        ...
        Else
            ...
            strSql = _
                " Select A.ID,A.类别 as 类别ID,B.序号 as 顺序ID,B.名称 as 类别," & _
                    ...
                    " Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价)" & _
                        IIf(mbln药房单位, "*Nvl(D." & str药房包装 & ",1)", "") & ",'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                    IIf(InStr(1, mstrPrivsOpt, ";显示库存;") > 0, " LTrim(To_Char(X.库存" & IIf(mbln药房单位, "/Nvl(D." & str药房包装 & ",1)", "") & ",'9999990.00000'))", "Decode(Sign(X.库存),1,'有','无')") & " as 库存,-1 as 批次" & _
    ...
    If blnLoad Then
        ...
        If mstr输入 <> "" Then
            ...
            ElseIf IsNumeric(mstr输入) Then                     '10,11.输入全是数字时只匹配编码,内部码,商品码
                If Mid(gstrMatchMode, 1, 1) = "1" Then strInput = " And (A.编码 Like [6] Or M.商品条码=[19] Or M.内部条码=[19]) And B.码类=[8]"
            ElseIf zlCommFun.IsCharAlpha(mstr输入) Then         '01,11.输入全是字母时只匹配简码
                If Mid(gstrMatchMode, 2, 1) = "1" Then strInput = " And B.简码 Like [7] And B.码类=[8]"
            ElseIf zlCommFun.IsCharChinese(mstr输入) Then
                strInput = " And B.名称 Like [7] And B.码类=[8]"
            ...
            Else
                If mint险类 = 0 Then
                    strMain = _
                        " Select Distinct A.ID,A.类别,A.编码,B.名称,B.简码," & _
                        " A.计算单位,A.规格,A.产地,A.费用类型,Null 医保大类,A.说明,A.是否变价,-1 As 批次 " & _
                        " From 收费项目别名 B,收费项目目录 A,药品库存 M" & _
                        " Where A.服务对象 IN([9],3) And A.ID=B.收费细目ID And A.类别='4'" & mstr特准项目 & strInput & strSQLItem & _
                        "           And A.ID=M.药品ID(+)"
                Else
                    strMain = _
                        " Select Distinct A.ID,A.类别,A.编码,B.名称,B.简码," & _
                        " A.计算单位,A.规格,A.产地,A.费用类型,D.名称 医保大类,A.说明,A.是否变价,-1 As 批次 " & _
                        " From 收费项目别名 B,收费项目目录 A,药品库存 M,保险支付项目 C,保险支付大类 D" & _
                        " Where A.服务对象 IN([9],3) And A.ID=B.收费细目ID And A.类别='4'" & mstr特准项目 & strInput & strSQLItem & _
                        " And A.ID=C.收费细目ID(+) And C.险类(+)=[16] And C.大类ID=D.ID(+)" & _
                        "           And A.ID=M.药品ID(+)"
        ...
        If bln批次 Then
            ...
            Else
                '卫材不受此系统参数和权限影响:分离下药时指定药房时限定库存,权限:不检查库存
                    ...
                    "           A.计算单位 as 单位,A.规格,A.产地,A.费用类型,A.医保大类,A.说明," & _
                    "           Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价),'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                                IIf(InStr(1, mstrPrivsOpt, ";显示库存;") > 0, "Decode(D.跟踪在用,1,To_Char(X.库存,'9999990.00000'),NULL)", "Decode(Sign(X.库存),1,'有','无')") & " as 库存,A.批次 " & _
        ...
        Else
            ...
            Else
                '卫材不受此系统参数和权限影响:分离下药时指定药房时限定库存,权限:不检查库存
                    ...
                    "           A.计算单位 as 单位,A.规格,A.产地,A.费用类型,A.医保大类,A.说明," & _
                    "           Decode(A.是否变价,1,'时价',LTrim(To_Char(Sum(C.现价),'999999" & gstrFeePrecisionFmt & "'))) as 单价," & _
                                IIf(InStr(1, mstrPrivsOpt, ";显示库存;") > 0, "Decode(D.跟踪在用,1,To_Char(X.库存,'9999990.00000'),NULL)", "Decode(Sign(X.库存),1,'有','无')") & " as 库存,-1 As 批次" & _
                    " From (" & strMain & ") A,收费项目类别 B,收费价目 C,材料特性 D,诊疗项目目录 E,(" & strStock & ") X" & _
                    " Where A.类别=B.编码 And A.ID=C.收费细目ID And A.ID=X.药品ID(+)" & _
                    "       And A.ID=D.材料ID And D.诊疗ID=E.ID" & IIf(mbln包含核算材料, "", " And nvl(D.核算材料 ,0)=0 ") & str分类ID & _
                    "       And Sysdate Between C.执行日期 and Nvl(C.终止日期,To_Date('3000-01-01','YYYY-MM-DD'))" & _
    ...
    Set mrsItem = zlDatabase.OpenSQLRecord(strSql, Me.Name, int类型, lng诊疗分类ID, lng收费分类ID, str类别, mstr类别, _
        ...
        lng药房ID, glng西药房, glng成药房, glng中药房, glng发料部门, mint险类, mint中药形态, mstr排除类别, UCase(mstr输入))
