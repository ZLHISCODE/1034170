...
Private Sub Bill_CommandClick()
    ...
    lng项目ID = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, gbln住院单位, str类别, , , str特准项目, zl获取中药形态(lng病人ID, Bill.Row))
    If lng项目ID <> 0 Then
        Bill.Text = lng项目ID
        mblnSelect = True
            ...
            Case "病人" '读取病人信息
                If Bill.Text <> "" Then
                    ...
                    Else
                        ...
                        gbytWarn = BillingWarn(mstrPrivsOpt, Trim(Nvl(mrsInfo!姓名)), Val("" & mrsInfo!病区ID), GetPatiWarnRange(Val(Nvl(mrsInfo!病人ID)), IIf(IsNull(mrsInfo!主页ID), 0, mrsInfo!主页ID)), _
                        ...
                        If mobjBill.Details.Count >= Bill.Row Then
                            ...
                            With mobjBill.Details(Bill.Row)
                                ...
                                .费别 = NeedName(Bill.TextMatrix(Bill.Row, BillCol.费别))
            ...
            Case "项目"
                ...
                If Bill.Text <> "" Then
                    ...
                    Else
                        ...
                        lng项目ID = frmItemSelect.ShowSelect(Me, mstrPrivs, int病人来源, int险类, gbln住院单位, str类别, Bill.Text, Bill.TxtHwnd, str特准项目, zl获取中药形态(lngCur病人ID, Bill.Row))
                        If lng项目ID <> 0 Then
                   ...
                   '保险项目对应检查
                    If int险类 > 0 Then
                        If Not CheckMediCareItem(mobjDetail.ID, int险类, mobjDetail.名称, mobjDetail.变价 = False) Then
                            Bill.Text = "": Bill.TxtSetFocus: Cancel = True: Exit Sub
                        End If
                    End If
                    ...
                    If mrsWarn.RecordCount > 0 And mobjBill.Details.Count = Bill.Row Then
                        ...
                        If curTotal > 0 Then
                            ...
                            gbytWarn = BillingWarn(mstrPrivsOpt, mobjBill.Details(Bill.Row).姓名, lng病人病区ID, GetPatiWarnRange(mobjBill.Details(Bill.Row).病人ID, mobjBill.Details(Bill.Row).主页ID), _
                    ...
                    If int险类 <> 0 And mobjBill.Details(Bill.Row).数次 <> 0 Then
                        If gclsInsure.GetCapability(support实时监控, mobjBill.Details(Bill.Row).病人ID, int险类) Then
                            If gclsInsure.CheckItem(int险类, 1, 0, MakeDetailRecord(mobjBill, NeedName(cbo开单人.Text), NeedName(cbo开单科室.Text), Bill.Row)) = False Then
                    ...
                    With mobjBill.Details(Bill.Row)
                        ...
                        If mrsWarn.RecordCount > 0 Then
                            ...
                            If curTotal > 0 Then
                                ...
                                gbytWarn = BillingWarn(mstrPrivsOpt, .姓名, .病区ID, GetPatiWarnRange(.病人ID, .主页ID), mrsWarn, _
                    ...
                    If mobjBill.Details(Bill.Row).Detail.变价 And mobjBill.Details(Bill.Row).InComes.Count > 0 Then
                        ...
                        If mrsWarn.RecordCount > 0 Then
                            ...
                            If curTotal > 0 Then
                                ...
                                If gbln报警包含划价费用 Then cur余额 = cur余额 - GetPriceMoneyTotal(1, mobjBill.Details(Bill.Row).病人ID) + IIf(gbytBilling = 1, mcurModiMoney, 0)
                                gbytWarn = BillingWarn(mstrPrivsOpt, mobjBill.Details(Bill.Row).姓名, mobjBill.Details(Bill.Row).病区ID, GetPatiWarnRange(mobjBill.Details(Bill.Row).病人ID, mobjBill.Details(Bill.Row).主页ID), mrsWarn, _
...
With mobjBill.Details(lngMainRow)
    ...
    For i = 1 To mcolDetails.Count
        ...
        If Val(mobjBill.Details(lngMainRow).发药窗口) > 0 Then
            If Not CheckMediCareItem(mcolDetails(i).ID, Val(mobjBill.Details(lngMainRow).发药窗口), mcolDetails(i).名称, mcolDetails(i).变价 = False) Then
                Exit Sub
            End If
        End If
...
Private Sub cbo开单科室_Click()
    ...
    If Not gblnFromDr Then '当不是由开单人定科室时
        If mobjBill.Details.Count > 0 And cbo开单科室.ListIndex <> mlngPreUnit And Visible And Not txtIn.Enabled Then
            MsgBox "提醒：你已经更改开单科室为""" & NeedName(cbo开单科室.Text) & """,注意检查单据中的病人是否属于该科室！", vbInformation, gstrSysName
        End If
    End If
...
Private Sub cbo开单人_Click()
    ...
    If gblnFromDr Then
        If cbo开单人.ListIndex <> -1 Then
            ...
            If lng开单科室ID > 0 Then Call zlControl.CboSetIndex(cbo开单科室.hWnd, FindCboIndex(cbo开单科室, lng开单科室ID))
        ...
        For i = 1 To mobjBill.Details.Count
            ...
            If lng病人ID <> mobjBill.Details(i).病人ID And lng险类 > 0 Then
                ...
                If gclsInsure.GetCapability(support实时监控, lng病人ID, lng险类) Then
                    If gclsInsure.CheckItem(lng险类, 1, 2, MakeDetailRecord(mobjBill, mobjBill.开单人, str开单科室)) = False Then
                       Exit Sub
                    End If
                End If
            End If
        ...
        If mrsWarn.RecordCount > 0 Then
            ...
            For i = 1 To mobjBill.Details.Count
                ...
                If mobjBill.Details(i).病人ID <> arrPati(0) Then
                    ...
                    arrPati(1) = CStr(mobjBill.Details(i).姓名) '姓名
...
Private Sub Form_Load()
    ...
    Else '新增
        ...
        If mstrInNO <> "" Then '修改单据
            Set mobjBill = ImportBill(mstrInNO, True, Me, True, gbln住院单位, True)
            If mobjBill.NO = "" Then
...
Private Sub txtIn_KeyPress(KeyAscii As Integer)
    ...
    Else
        ...
        Set tmpBill = ImportBill(txtIn.Text, True, Me, False, gbln住院单位, False, mlngUnitID)
        If tmpBill.NO = "" Then
            MsgBox "读取单据失败。", vbExclamation, gstrSysName
            SelAll txtIn: txtIn.SetFocus
        Else
            '单据修改及显示
    ...
    If Not mrsInfo.EOF Then
        ...
        If cbo开单科室.ListIndex <> -1 Then
            If mrsInfo!科室ID <> cbo开单科室.ItemData(cbo开单科室.ListIndex) And mrsInfo!病区ID <> cbo开单科室.ItemData(cbo开单科室.ListIndex) Then
                MsgBox "提醒：该住院病人不属于""" & NeedName(cbo开单科室.Text) & """！", vbInformation, gstrSysName
            End If
        End If
                ...
                If InStr(",5,6,7,", .收费类别) > 0 Or (.收费类别 = "4" And .Detail.跟踪在用) Then
                    ...
                    If dblAllTime <> 0 Then
                        ...
                        If dblAllTime <> 0 Then
                            '数量未分解完毕
                            If InStr(",5,6,7,", .收费类别) > 0 Then
                                MsgBox "时价药品""" & .Detail.名称 & """库存不足,无法计算价格！", vbInformation, gstrSysName
                            Else
                                MsgBox "时价卫生材料""" & .Detail.名称 & """库存不足,无法计算价格！", vbInformation, gstrSysName
                            End If
                            dblMoney = 0
                        Else
        ...
        If mobjBillDetail.数次 <> 0 Then
            ...
            For Each mobjBillIncome In mobjBillDetail.InComes
                ...
                With mobjBillDetail
                    ...
                    '收集卫料发料部门,以便自动发料
                    If gbytBilling = 0 And gbln自动发料 Then
...
Public Sub SetStatuPatiInfor(ByVal str姓名 As String, ByVal dbl预交 As Double, dblFee As Double, dbl剩余 As Double, Optional dbl应收 As Double = 0)
    ...
    strTemp = str姓名 & "预交:" & Format(Val(cmdOK.Tag), "0.00")
