...
Private Function InitBlanceData(ByVal strBalance As String, ByVal strOldBalance As String) As Boolean
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:初始化结算数据
    '入参:strBalance-指定的结算序号,以逗号分离:'0001,0002
    '     strOldBalance-指定的结帐ID
    '出参:
    '返回:
    '编制:刘尔旋
    '日期:2014-06-19
    ...
    If mintType = 2 Then
        ...
        Exit Function
    End If
    If strBalance = "" And strOldBalance = "" Then InitBlanceData = True: Exit Function
    ...
    If strOldBalance <> "" Then
        ...
        "       Where a.结帐id In (Select /*+ rule */" & vbNewLine & _
        "                        Distinct d.结帐id" & vbNewLine & _
        "                        From 门诊费用记录 C, 门诊费用记录 D, Table(f_Str2list([3])) E" & vbNewLine & _
        "                        Where c.结帐id = e.Column_Value And c.No = d.No And Mod(d.记录性质, 10) = 1) And a.记录性质 In (1, 11, 3) And a.病人id=[2] And" & vbNewLine & _
        "             Nvl(a.冲预交, 0) <> 0" & vbNewLine & _
        "" & vbNewLine & _
        "       Group By Decode(a.记录性质, 3, a.结算方式, Null)) A, 结算方式 B" & vbNewLine & _
        "Where a.结算方式 = b.名称(+)"
    ...
    Set mrsBalance = zlDatabase.OpenSQLRecord(strSql, Me.Caption, Replace(strBalance, "'", ""), mlng病人ID, Replace(strOldBalance, "'", ""))
...
Private Sub SetBlanceShow()
    ...
    Dim strSelNos As String, strNo As String, strSql As String, strOldNOs As String
    Dim strOldBalance As String
    If mintType = 2 Then Exit Sub
    With vsFee
        bln全选 = True: bln未选 = True
        For lngRow = 1 To .Rows - 1
            strBalance = Trim(.TextMatrix(lngRow, .ColIndex("结算序号")))
            strOldBalance = Trim(.TextMatrix(lngRow, .ColIndex("结帐ID")))
            ...
            If .TextMatrix(lngRow, .ColIndex("选择")) = "√" And Val(strBalance) > 0 Then
                If InStr(1, strOldNOs & ",", "," & strOldBalance & ",") = 0 Then
                    strOldNOs = strOldNOs & "," & strOldBalance
                    bln未选 = False
                End If
            End If
            
            If InStr(1, strSelNos & ",", "," & strBalance & ",") = 0 Or InStr(1, strOldNOs & ",", "," & strOldBalance & ",") = 0 Then bln全选 = False
        Next
    End With
    If strSelNos <> "" Then strSelNos = Mid(strSelNos, 2)
    If strOldNOs <> "" Then strOldNOs = Mid(strOldNOs, 2)
    bln退款 = False
    ...
    If Not mrsBalance Is Nothing Then
        ...
        If SetPicBack(strSelNos) = True Then
            txtSum.Text = InitPatialBalance(strSelNos, strOldNOs)
        Else
            Call InitBlanceData(strSelNos, strOldNOs)
        End If
        ...
        If Not mrsBalance.EOF Then
            ...
            Next
        End If
        If strSelNos = "" And strOldNOs = "" Then
...
Private Function InitPatialBalance(ByVal strBalance As String, ByVal strOldBalance As String) As String
    '---------------------------------------------------------------------------------------------------------------------------------------------
    '功能:初始化部分退费的结算数据
    '入参:strBalance-指定的结算序号,以逗号分离:'0001,0002
    '出参:
    ...
    If mintType = 2 Then
        ...
        Exit Function
    End If
    If strBalance = "" And strOldBalance = "" Then InitPatialBalance = 0: Exit Function
    
    Call InitBlanceData(strBalance, strOldBalance)
...
'        "Where a.结算方式 = b.名称 And B.性质 = 9"
    ...
    If strOldBalance <> "" Then
        ...
        "              Distinct d.结帐id" & vbNewLine & _
        "              From 门诊费用记录 C, 门诊费用记录 D," & vbNewLine & _
        "                    Table(f_Str2list([3])) E" & vbNewLine & _
        "              Where c.结帐id = e.Column_Value And c.No = d.No And Mod(d.记录性质, 10) = 1 And Not Exists" & vbNewLine & _
        "               (Select 1" & vbNewLine & _
        "                     From 门诊费用记录" & vbNewLine & _
        "                     Where 结帐id In (Select Max(结帐id)" & vbNewLine & _
        "                                    From 门诊费用记录" & vbNewLine & _
        ...
        "                                                 Where k.结帐id In (Select Column_Value From Table(f_Str2list([3]))))) And Mod(记录性质, 10) = 1) And Mod(记录性质, 10) = 1 And" & vbNewLine & _
        "                           记录状态 = 2)) K" & vbNewLine & _
        "       Where a.结帐id = k.结帐id And a.记录性质 In (1, 11) And a.病人id = [2] And Nvl(a.冲预交, 0) <> 0" & vbNewLine & _
        "       Group By Decode(a.记录性质, 3, a.结算方式, Null)) A, 结算方式 B" & vbNewLine & _
        "Where a.结算方式 = b.名称(+) "
        ...
        "                        Distinct d.结帐id" & vbNewLine & _
        "                        From 门诊费用记录 C, 门诊费用记录 D, Table(f_Str2list([3])) E" & vbNewLine & _
        "                        Where c.结帐id = e.Column_Value And c.No = d.No And Mod(d.记录性质, 10) = 1) K" & _
        "       Where a.结帐id=K.结帐id  And a.记录性质 = 3 And a.病人id=[2] And Nvl(a.冲预交, 0) <> 0" & vbNewLine & _
        "       Group By Decode(a.记录性质, 3, a.结算方式, Null)) A, 结算方式 B" & vbNewLine & _
        "Where a.结算方式 = b.名称 And B.性质 = 8"
        '全退记录(不能退现的三方账户)
...
'-----------------------------------------------------------------------------------------------------------------------
    ...
    If intType = 1 Then
        ...
        If Val(strBalance) = 0 Then Exit Sub
        lngBalance = Val(vsFee.TextMatrix(NewRow, vsFee.ColIndex("结帐ID")))
        If Val(strBalance) < 0 Then
            strSql = "" & _
        ...
        Else
            strSql = "" & _
                ...
                "                           Where Mod(c.记录性质, 10) = 1 And c.记录状态 <> 0 And c.结帐id = [2])) B" & vbNewLine & _
                "       Where a.记录性质 = 3 And a.结帐id = b.结帐id)" & vbNewLine & _
                "Group By 结算方式" & vbNewLine & _
                "Union" & vbNewLine & _
                "Select 结算方式, Sum(冲预交) As 结算金额" & vbNewLine & _
        ...
        Set rsTmp = zlDatabase.OpenSQLRecord(strSql, Me.Caption, Val(strBalance), lngBalance)
        With vsBalanceStyle
...
Public Function ReadListData(ByVal strFindNo As String, ByVal strFindFpNo As String, _
    ...
    With vsFee
        ...
        For lngRow = 1 To .Rows - 1
            ...
            End If
            If Val(strNo) > 0 Then
                strOldNOs = strOldNOs & "," & Trim(.TextMatrix(lngRow, .ColIndex("结帐ID")))
            End If
        Next
        .Editable = flexEDNone
    End With
    ...
    '加载结算方式
    Call CheckInsure
    Call InitBlanceData(strALLNOs, strOldNOs)
    Call SetBlanceShow
    Call CalcSUMMony
...
Public Function SaveData() As Boolean
    ...
    Else
        For i = 1 To cllDelNO.Count
            If ExecuteDelBill(strDelDate, IIf(cllDelNO(i)(1) <> "", cllDelNO(i)(1), cllDelNO(i)(0)), Val(cllDelNO(i)(2)), Val(cllDelNO(i)(3)), cllDelNO(i)(4), cllDelNO(i)(5)) = False Then
                    Exit Function
            End If
...
Private Function ExecuteThreeSwap(lngBalance As Long, lng冲销ID As Long, Optional ByRef str交易流水号 As String, Optional ByRef str交易说明 As String) As Boolean
    ...
    Else
        strSql = _
            ...
            "      (Select Distinct k.结帐id" & vbNewLine & _
            "        From 病人预交记录 I, 门诊费用记录 J, 门诊费用记录 K" & vbNewLine & _
            "        Where i.结帐ID = [1] And i.记录性质 = 3 And i.结帐id = j.结帐id And k.No = j.No And Mod(k.记录性质, 10) = 1) C, 医疗卡类别 D" & vbNewLine & _
            " Where a.结算方式 = b.名称 And b.性质 = 7 And a.结帐id = c.结帐id And a.卡类别id = d.id And d.是否退现 = 0 And a.校对标志 <> 1 " & vbNewLine & _
            " Group By a.卡类别id, a.卡号" & vbNewLine & _
            " Having Sum(a.冲预交) <> 0"
    End If
