...
'功能：初始化相应的数据
    ...
    If mlng病人ID <> 0 Then
        ...
        End If
        '76451,冉俊明,2014-8-19
        strSql = "Select A.姓名,A.病人ID,A.主页ID 主页Id,Nvl(A.当前病区ID,0) as 病区ID,Zl_Patiwarnscheme(A.病人id, A.主页ID) As 适用病人," & _
   ...
   ' If InStr(mstrPrivsOpt, "精神药品记帐") = 0 Then str特性 = str特性 & " And E.价值分类 Not IN('精神I类','精神II类')"
    ...
    With vsBill
            ...
            If lng药品ID <> Val(Nvl(rsTemp!药品ID)) Then
                ...
                If Not gbln分离发药 Then
                    ...
                    If lngTmp <> lng药房ID And lng药房ID <> 0 Then
                        cbo药房.ListIndex = FindCboIndex(cbo药房, lng药房ID)
                        '改变了库房,需要重新刷新规格
                         Call 重新刷新所有中药规格
                    End If
                End If
        ...
        Else
            ...
            If cbo药房.ListIndex < 0 Then
                    If mlng中药房 <> 0 Then
                        cbo药房.ListIndex = FindCboIndex(cbo药房, mlng中药房)
                    Else
    ...
    If KeyAscii = 13 Then
        ...
        ElseIf Col Mod MCOLS = col剂量 Then
            ...
            If mrsWarn.RecordCount > 0 And Not mrsPati Is Nothing Then
                ...
                If cur实收 > 0 Then
                    gbytWarn = BillingWarn(mstrPrivsOpt, mrsPati!姓名, Val("" & mrsPati!病区ID), mrsPati!适用病人, mrsWarn, mrsPati!病人余额, _
            ...
            If InStr(",5,6,7,", objBillDetail.收费类别) > 0 Then
                ...
                If dblAllTime <> 0 Then
                    ...
                    If dblAllTime <> 0 Then
                        ...
                        dblMoney = 0
                    Else
                        '注意：货币型最多只能保留4位小数,且按formatex处理,所以需要手工舍入;而用其它型在计算精度上又有问题
                        dblAllTime = objBillDetail.付数 * objBillDetail.数次
                        If gbln住院单位 Then
                            '按售价数量计算实价
                            dblAllTime = dblAllTime * objBillDetail.Detail.住院包装
