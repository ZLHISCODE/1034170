   ...
   Begin VB.PictureBox picPane 
      ...
      Begin VB.PictureBox pic 
         ...
         Begin VB.PictureBox picLine 
            ...
            Width           =   15
         End
         Begin zl9BodyEditor.VsfGrid vsf 
            Height          =   270
            Left            =   450
            TabIndex        =   5
            Top             =   4440
...
Public Sub ReSetFontSize()
    ...
    Dim objToolBar页面 As CommandBarControl
    ...
    opt(0).FontSize = bytFontSize
    opt(1).FontSize = bytFontSize
    cboBaby.FontSize = bytFontSize
    picTmp.FontSize = bytFontSize
    picTmp.Height = cboBaby.Height + 60
    opt(0).Width = picTmp.TextWidth(opt(0).Caption & "刘") + 30
    opt(1).Width = picTmp.TextWidth(opt(1).Caption & "刘") + 30
    opt(1).Left = opt(0).Left + opt(0).Width + picTmp.TextWidth("刘鹏")
    cboBaby.Left = opt(1).Left + opt(1).Width + 30
    cboBaby.Top = (picTmp.Height - cboBaby.Height) \ 2
    opt(0).Top = cboBaby.Top + (cboBaby.Height - opt(0).Height) \ 2
    opt(1).Top = opt(0).Top
    picTmp.Width = cboBaby.Left + cboBaby.Width + 30
    picTmp.Visible = True
    
    '重新加载工具栏(因为控件放大后，picTmp会覆盖调后面的页面信息，此处需要重新处理)
    If mcbrToolBar Is Nothing Then Exit Sub
    Set objToolBar = cbsMain.Add("婴儿", xtpBarTop)
    objToolBar.ShowTextBelowIcons = False
    objToolBar.EnableDocking xtpFlagHideWrap

    Set objCustom = objToolBar.Controls.Add(xtpControlCustom, conMenu_View_Option, "")
    objCustom.Handle = picTmp.hWnd
    picTmp.Visible = mblnBabys
    If Not mcbrToolBar页面 Is Nothing Then
        Set objToolBar页面 = objToolBar.Controls.Add(xtpControlPopup, conMenu_Edit_NewItem, "页面"): objToolBar页面.BeginGroup = True
        objToolBar页面.IconId = conMenu_Edit_Modify
        objToolBar页面.Style = xtpButtonIconAndCaption
        For Each objCtrl In mcbrToolBar页面.CommandBar.Controls
            Set cbrItem = objToolBar页面.CommandBar.Controls.Add(xtpControlButton, objCtrl.Id, objCtrl.Caption, -1, False)
            cbrItem.Parameter = objCtrl.Parameter
            If Val(Split(cbrItem.Parameter, ";")(4)) = Page Then objToolBar页面.Caption = cbrItem.Caption
        Next
        mcbrToolBar页面.Delete
    End If
    For Each objCtrl In mcbrToolBar.Controls
        objCtrl.Delete
    Next
    
    If Not mcbrToolBar Is Nothing Then mcbrToolBar.Delete
    If Not objToolBar Is Nothing Then Set mcbrToolBar = objToolBar
    If Not objToolBar页面 Is Nothing Then Set mcbrToolBar页面 = objToolBar页面
...
Private Function BlowUp(ByRef dblChange As Double) As Double
    '放大：字体，单元格宽度
    BlowUp = dblChange
    If FontSize <> 1 Then Exit Function
...
Public Function zlMenuClick(ByVal strMenuItem As String, Optional ByVal strParam As String) As Boolean
    ...
    Case "初始数据"
        ...
        gstrSQL = "Select a.序号,Decode(a.婴儿姓名,Null,NVL(C.姓名,B.姓名) ||'之子'||Trim(To_Char(a.序号,'9')),a.婴儿姓名) As 婴儿姓名" & _
            ...
            " Where B.病人ID=C.病人ID And C.病人ID=A.病人ID And C.主页ID=A.主页ID And C.病人id=[1] And C.主页id=[2] Order By a.序号"
...
'            Call UserControl_Resize
    ...
    Case "添加项目"
        ...
        If rsData.BOF = False Then
            If ShowTxtSelDialog(mfrmParent, Nothing, "名称,1500,0,1;单位,900,0,0;最小值,900,0,0;最大值,900,0,0", mfrmParent.Name & "\护理项目选择", "请从下面选择一个护理项目。", rsData, rsTmp, BlowUp(6000), BlowUp(3000), , , 2, False, IIf(FontSize = 0, 9, IIf(FontSize = 1, 12, FontSize))) Then
...
'            End If
    ...
    Case "显示病人姓名"
        ...
        Case Else
            ...
            gstrSQL = "Select Decode(a.婴儿姓名,Null,NVL(C.姓名,B.姓名) ||'之子'||Trim(To_Char(a.序号,'9')),a.婴儿姓名) As 婴儿姓名,a.婴儿性别,a.出生时间 " & _
                " From 病人信息 B,病案主页 C,病人新生儿记录 A " & _
...
Private Function CheckTimeRange(ByVal intCOl As Integer, Optional ByVal blnCheckInput As Boolean = False) As Boolean
    ...
    If strTo > strCurrTIme Then strTo = strCurrTIme
    intCount = 0
...
Private Function ReadPatiInfo() As Boolean
    ...
    '填写病人姓名、住院号
    gstrSQL = "Select NVL(B.姓名,A.姓名) 姓名,B.住院号 From 病人信息 A,病案主页 B Where A.病人ID=B.病人ID And B.病人id=[1] And B.主页ID=[2]"
...
'    Erase mvarDataType
    ...
    mshScale.Cell(flexcpAlignment, 1, 0, mshScale.Rows - 1, mshScale.FixedCols - 1) = 4
            ...
            If mshUpTab.ColData(intCOl) = 1 Or mshUpTab.ColData(intCOl) = 2 And lng次数 < 12 Then
                ...
                Else
                    ...
                    If mshUpTab.TextMatrix(2, intCOl) <> "" Then
                        mshUpTab.TextMatrix(2, intCOl) = strTmp & "/" & mshUpTab.TextMatrix(2, intCOl)
                    Else
...
'    End With
    ...
    With mshDownTab
        ...
        For intRow = 1 To .Rows - 1
            For intCOl = .FixedCols To .Cols - 1
                .TextMatrix(intRow, intCOl) = ""
                                .TextMatrix(intRow, 1) = String(13, ";") '赋初值,避免切换期间后数据不变,导致数据更新错误,比如第一页的第二天录入了大便次数,第二页的第二天也在大便次数填写数据后会错误的更新到第一页的大便次数中
            Next
  ...
  '读取数据
    ...
    If strItemName <> "" Then
        ...
        With rsTmp
            vsf.Cell(flexcpText, 0, 2, 0, vsf.Cols - 1) = ""
                ...
                For i = 0 To rsTmp.RecordCount - 1
                    ...
                    Case mItemNo.呼吸
                        ...
                        If intCOl < vsf.Cols Then
                            ...
                            vsf.TextMatrix(0, intCOl) = Format(!时间, "YYYY-MM-DD HH:mm:ss")
                        End If
                    ...
                    Case Else
                        ...
                        intCOl = Int((!时间 - Int(dt开始时间)) * 24) \ 12 + mshDownTab.FixedCols
                        strTime = Format(Int(dt开始时间) + (intCOl - mshDownTab.FixedCols) \ 2, "YYYY-MM-DD")
                        If Not (!项目序号 = mItemNo.血压 And Format(!时间, "yyyy-MM-dd") = Format(txtCard(3).Text, "yyyy-MM-dd")) Then
...
Private Function DrawScale() As Boolean
    ...
    If picScale.Tag <> "" Then
        Call CalcMinMaxCol(picScale.Tag, intMinCol, intMaxCol)
        lblCur.Left = intMinCol * HOUR_STEP_Twips + Int((HOUR_STEP_Twips - picScale.TextWidth(lblCur.Caption)) / 2)
    End If
...
'                End If
                ...
                If intMode = OperateType.新增操作 Or intMode = OperateType.修改操作 Or intMode = OperateType.删除操作 Then
                    ...
                    mstrSQL = mstrSQL & lngItemCode & ","
                    mstrSQL = mstrSQL & "1,"
                    mstrSQL = mstrSQL & "Null,Null,1,0,0,0," & strMakeTime
                    mstrSQL = mstrSQL & ")"
                    strSQL(ReDimArray(strSQL)) = mstrSQL
                    ...
                    Else
                        For i = 0 To UBound(aryData)
                            ...
                            mstrSQL = mstrSQL & IIf((lngItemCode = mItemNo.体温 Or lngItemCode = mItemNo.呼吸 Or lngItemCode = mItemNo.脉搏) And strValues = "", "'" & Split(.TextMatrix(GraphDataRow.部位标志, intCOl), ";")(intCount + 1) & "'", "''") & ",1,0,"
    ...
    With vsf
        For intCOl = 2 To .Cols - 1
            ...
            If Not IsDate(.TextMatrix(0, intCOl)) Then
                ...
                End If
            Else
                strTime = Format(.TextMatrix(0, intCOl), "YYYY-MM-DD HH:mm:ss")
                strEnd = strTime
            End If
        ...
        If .Tag = "1" And .RowData(1) > 0 Then
            For intCOl = .FixedCols To .Cols - 1 Step 2
                ...
                For intCount = 0 To .Rows - 2
                    '求出名称
                    strItem = .TextMatrix(intCount + 1, 1)
                    arrItem = Split(mshDownTab.TextMatrix(intCount + 1, 1) & String(13 - UBound(Split(mshDownTab.TextMatrix(intCount + 1, 1), ";")), ";"), ";")
                    '操作数据
                    ...
                    For intLoop = intCOl To intCOl + (int频次 - 1)
                       ...
                       If Not IsDate(arrItem(intLoop - .FixedCols)) Then
                            If int频次 = 2 Then
                            ...
                            End If
                        Else
                            strTime = Format(arrItem(intLoop - .FixedCols), "YYYY-MM-DD HH:mm:ss")
                            strEnd = strTime
                        End If
...
'
'                        End If
                        If (intMode = 2 Or intMode = 3 Or intMode = 4) Then
                            
                            If Val(.RowData(intCount + 1)) = mItemNo.血压 Then
                                If mvarStrValue = "" Then mvarStrValue = "/"
                                mstrSQL = "ZL_电子护理记录_UPDATE("
                                mstrSQL = mstrSQL & Val(mrsParam("病人id")) & ","
                                mstrSQL = mstrSQL & Val(mrsParam("主页id")) & ","
                                mstrSQL = mstrSQL & Val(mrsParam("婴儿")) & ","
                                ...
                                mstrSQL = mstrSQL & Val(.RowData(intCount + 1)) & ","
                                mstrSQL = mstrSQL & "0,"
                                mstrSQL = mstrSQL & "'" & Split(mvarStrValue, "/")(0) & "',NULL,1,0"
                                mstrSQL = mstrSQL & ")"
                                ...
                                mstrSQL = mstrSQL & mItemNo.舒张压 & ","
                                mstrSQL = mstrSQL & "0,"
                                mstrSQL = mstrSQL & "'" & Split(mvarStrValue, "/")(1) & "',NULL,1,0"
                                mstrSQL = mstrSQL & ")"
                            ...
                            Else
                                ...
                                mstrSQL = mstrSQL & "0,"
                                mstrSQL = mstrSQL & "'" & mvarStrValue & "'"
                                mstrSQL = mstrSQL & ",Null,1,0,0," & IIf(IsNumeric(mvarStrValue), 0, 1) & ")"
...
Private Sub picScale_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    intLeft = Int((HOUR_STEP_Twips - picScale.TextWidth(lblCur.Caption)) / 2)
    '获得最小最大时间范围
    Call CalcMinMaxCol(picScale.Tag, intMinCol, intMaxCol)
    ...
    Case 39     '右移动
        If lblCur.Left + HOUR_STEP_Twips <= (intMaxCol * HOUR_STEP_Twips) + intLeft Then lblCur.Left = lblCur.Left + HOUR_STEP_Twips
    Case 13     'Enter输入数据
...
'    If X > (Int((CDate(aryValue(1)) - Int(CDate(aryValue(0)))) * 24) \ 4 + 1) * HOUR_STEP_Twips - 15 Then Exit Sub
    intCOl = Int(X / HOUR_STEP_Twips)
    If intCOl < intMinCol Then Exit Sub
    If intCOl > intMaxCol Then Exit Sub
    lblCur.Left = intCOl * HOUR_STEP_Twips + Int((HOUR_STEP_Twips - picScale.TextWidth(lblCur.Caption)) / 2)
    Exit Sub
ErrHead:
    If ErrCenter() = 1 Then
        Resume
