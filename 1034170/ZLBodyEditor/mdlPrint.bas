...
Public Function DrawPatiInfo(objDraw As Object, X As Long, Y As Long, strInfo As String, ByVal lngPaperWidth As Long) As Boolean
    ...
    Call DrawText(objDraw, X + objDraw.TextWidth("姓名:"), Y, Split(strInfo, "'")(0), 0)
    X = X + objDraw.TextWidth("姓名:" & Split(strInfo, "'")(0)) + W_9pt / 3
    
    Call DrawText(objDraw, X, Y, "性别:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth("性别:"), Y, Split(strInfo, "'")(5), 0)
    X = X + objDraw.TextWidth("性别:" & Split(strInfo, "'")(5)) + W_9pt / 3
    
    Call DrawText(objDraw, X, Y, "年龄:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth("年龄:"), Y, Split(strInfo, "'")(6), 0)
    X = X + objDraw.TextWidth("年龄:" & Split(strInfo, "'")(6)) + W_9pt / 3
    
    Call DrawText(objDraw, X, Y, "入院日期:", 0)
    Call DrawText(objDraw, X + objDraw.TextWidth("入院日期:"), Y, Split(strInfo, "'")(3), 0)
    ...
    If X + objDraw.TextWidth("住院号:" & Split(strInfo, "'")(1) + "科室:" & Split(strInfo, "'")(2) + "床号:" & Split(strInfo, "'")(4) + strTmp) + W_9pt / 3 > lngPaperWidth Then
        ...
        Call DrawText(objDraw, X + objDraw.TextWidth("住院号:"), Y, Split(strInfo, "'")(1), 0)
        ...
        Call DrawText(objDraw, X + objDraw.TextWidth("科室:"), Y, Split(strInfo, "'")(2), 0)
        X = X + objDraw.TextWidth("科室:" & Split(strInfo, "'")(2)) + W_9pt / 3

        Call DrawText(objDraw, X, Y, "床号:", 0)
        Call DrawText(objDraw, X + objDraw.TextWidth("床号:"), Y, Split(strInfo, "'")(4), 0)
    ...
    Else
        ...
        If strTmp <> "" Then
            ...
            Call DrawText(objDraw, X, Y, "诊断:", 0)
            Call DrawText(objDraw, X + objDraw.TextWidth("诊断:"), Y, Split(strInfo, "'")(7), 0)
        End If
                ...
                If Split(strValue(intRow), ";")(0) = "呼吸" Then
                    ...
                    For intCOl = 0 To 42
                        If (intCOl) Mod 6 = 0 Then
                            Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCOl, Y + intFactRow * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * intCOl, Y + intFactRow * (H_9pt + H_9pt \ 2) + 2 * (H_9pt + H_9pt \ 2), IIf(intCOl = 0 Or intCOl = 42, 0, 255), , 2)
                        Else
                        ...
                        For intCOl = 0 To 14
                            ...
                            Else
                                ...
                                If intCOl Mod 2 = 1 Then
                                    ...
                                    If (Split(strValue(intRow), ";")(intCOl + 2) <> "" And Split(strValue(intRow), ";")(intCOl + 3) <> "") Or Val(Split(strValue(intRow), ";")(1)) = 2 Then
                                        ...
                                        If (intCOl + 1) Mod 2 = 0 Then
                                            Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), IIf(intCOl = 1, 0, 255), , 2)
                                        End If
                                    ...
                                    Else
                                        ...
                                        Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 6 * Fix(intCOl / 2), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), IIf(intCOl = 1, 0, 255), , 2)
                                ...
                                Else
                                    ...
                                    If (intCOl + 1) Mod 2 = 0 Then
                                        Call DrawLine(objDraw, lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2), lngTmpX + colFirstWidth + HOUR_STEP_Twips * 3 * (intCOl - 1), Y + (intFactRow + intAdd) * (H_9pt + H_9pt \ 2) + (H_9pt + H_9pt \ 2), IIf(intCOl = 0, 0, 255), , 2)
                                    End If
                                End If
                            End If
...
Private Sub DrawBodyPageFooter(objDraw As Object, X As Long, Y As Long, ByVal intPageNo As Integer, ByVal intBeginPage As Integer)
    ...
    If blnOper = True Then
        ...
        lngX = X + (6 * W_9pt + HOUR_STEP_Twips * 6 * 7) - objDraw.TextWidth(strNOPage)
        
        Call DrawCell(objDraw, strNOPage, lngX - W_9pt * 2, Y, W_9pt * 9, H_9pt + H_9pt \ 2, , , , , , objDraw.Font, "0000", 0, 0)
    End If
End Sub
...
Private Function DrawBodyInfo(objDraw As Object, X As Long, Y As Long, ByVal colFirstWidth As Long, _
    ...
    For intDay = 0 To 6
        ...
        Call DrawLine(objDraw, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y + H_9pt + H_9pt \ 2, IIf(intDay = 0, 0, 255), , 2)
        ...
        Call DrawLine(objDraw, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y + H_9pt * 1 + H_9pt \ 2, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y + H_9pt * 1 + H_9pt \ 2 + H_9pt + H_9pt \ 2, IIf(intDay = 0, 0, 255), , 2)
        ...
        End If
        Call DrawLine(objDraw, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y + (H_9pt + H_9pt \ 2) * 2, lngLeft + (HOUR_STEP_Twips * 6) * intDay, Y + (H_9pt + H_9pt \ 2) * 2 + H_9pt + H_9pt \ 2, IIf(intDay = 0, 0, 255), , 2)
...
'                objDraw.FontSize = 9
        ...
        For j = lngHourBegin To 12 - (4 - lngHourBegin) Step 4
            intCount = intCount + 1
            If intCount = 2 Or intCount = 3 Then
                lngColor = RGB(255, 0, 0)
            Else
                lngColor = 0
            End If
...
'                objDraw.FontSize = 9
    ...
    For i = 1 To 7
        ...
        Call DrawLine(objDraw, X, Y, X, Y + (H_9pt * 2 + H_9pt \ 2), IIf(i = 7, 0, 255), 1, 2)
    Next
...
'        If (intRow + 1) Mod 5 = 0 Then
        If intRow Mod 5 = 0 Then
            If (intRow - 1) = 24 Then
                Call DrawLine(objDraw, lngTmpX + 10, Y0 + ROWHEIGHT * 5, lngTmpX + HOUR_STEP_Twips * 6 * 7, Y0 + ROWHEIGHT * 5, RGB(255, 0, 0), , 2)
            Else
    ...
    For intCOl = 0 To 6
        ...
        Call DrawLine(objDraw, X0 + HOUR_STEP_Twips * 6, lngTmpY, X0 + HOUR_STEP_Twips * 6, Y, IIf(intCOl = 6, COLOR.黑色, COLOR.红色), 1, 2)
    Next
...
'            objOut.SetPages
    ...
    strSQL = " Select  NVL(A.姓名,B.姓名) 姓名,A.住院号,A.入院日期 入院时间,NVL(A.性别,b.性别) 性别,NVL(A.年龄,b.年龄) 年龄" & _
...
Public Function ShowTxtSelDialog(ByVal frmParent As Object, _
                                    ...
                                    Optional ByVal bytSize As Byte = 9) As Boolean
    '******************************************************************************************************************
    '功能:打开树型+列表结构
    '返回:出错返回2;成功返回1;取消返回0
    '50807,刘鹏飞,2012-08-07,字体大小
    '******************************************************************************************************************
    ...
    If frmSelectDialog.ShowSelect(frmParent, WinStyle, rsData, strLvw, strDescrible, lngX, lngY, lngCX, lngCY, lngObjHeight, strInitKey, strSavePath, , False, blnMuliSel, , blnSort, bytSize) Then
        ...
        Do While Not .EOF
            If Trim(NVL(!结果)) <> "" Then
                ...
                For intCount = 1 To Len(!结果)
                    ...
                    End If
                    Call DrawRotateText(objDraw, sinX1 - objDraw.TextWidth(strChar) / 2, sinY1 + 15, strChar, IIf(!类型 = 2, -2147483635, 255))
                    If Asc(strChar) < 0 Then
                        intCharNumber = 0
