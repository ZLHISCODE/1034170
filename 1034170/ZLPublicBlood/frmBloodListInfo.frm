...
Public Function zlRefresh(ByVal lng医嘱ID As Long, Optional ByVal lngFontSize As Long = 9, Optional ByVal blnMoved As Boolean = False) As Boolean
    ...
    If blnShowBlood = True Then
        ...
        strSql = _
            ...
            "       (Select Max(f.配血结论) From 诊疗项目目录 g, 血液配血方法 f Where f.配血方法id = g.Id(+) And f.收发id = a.Id) 配血结论,a.发血人,A.发血日期,a.取血人" & vbNewLine & _
...
'返回：是否需要读取和显示血液列表
    ...
    If int执行标记 = -1 Then '读取标记未用的原因
        ...
        End If
        Set rsTmp = gobjDatabase.OpenSQLRecord(strSql, Me.Caption, mlng医嘱ID, 20)
...
Private Sub Form_Resize()
    ...
    With rtfOther
        ...
        .Left = 0
        .Width = Me.Width
        .Height = IIf(vsList.Tag = "可见", Me.Height - vsList.Height - fraExecUD.Height, Me.Height)
    End With
    With fraExecUD
        .Left = 0
        .Top = rtfOther.Top + rtfOther.Height
        .Visible = (vsList.Tag = "可见")
    End With
    With vsList
        .Left = 0
