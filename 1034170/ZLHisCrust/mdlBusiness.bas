    ...
    If Not rsTmp.EOF Then
        ...
        If IsFileUpgade(strFile, rsTmp!版本号 & "", rsTmp!修改日期 & "", rsTmp!MD5 & "") Then
            If gclsConnect.IsServerFileExists(rsTmp!标准文件名) Then
                ...
                Else
                    If SetupOneFile("ZLHISCRUSTCOM.DLL", gstrTempPath & "\ZLHISCRUSTCOM.DLL", strFile, True, strErr) Then
                        strMsg = ""
                        gobjTrace.WriteInfo "UpgradeBase", "下载安装", strFile
                        If Not gclsRegCom.RegCom(strFile, strMsg, RFT_NormalReg) Then
                            gobjTrace.WriteInfo "UpgradeBase", "ZLHISCRUSTCOM.DLL注册失败", strMsg
                            Call RecordErrMsg(MT_InitEnv, "ZLHISCRUSTCOM.DLL注册失败", strMsg)
                        Else
                            gobjTrace.WriteInfo "UpgradeBase", "ZLHISCRUSTCOM.DLL注册成功", ""
                        End If
                    Else
                        strMsg = "安装ZLHISCRUSTCOM.DLL失败，" & strErr
                    End If
                End If
            Else
...
'功能：启动ZLRunas
    ...
    If strPwd <> "" And strUser <> "" Then
        ...
        '重新启动升级外壳
        If objShell.Run(strPath & " " & strCommandPara & " -ex """ & App.Path & "\ZLHISCRUST.EXE"" -lwp", strReturn, , 30000) Then
